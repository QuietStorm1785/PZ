...
function ISVehicleBloodUI:close()
	if self.vehicle then
		for _,_id in ipairs({"Front", "Rear", "Left", "Right"}) do
			local args = { vehicle = self.vehicle:getId(), id = _id, intensity = self.sliderBlood[_id].currentValue / 100 }
...
function ISVehicleBloodUI:new(playerObj)
	local width = 300
	local height = 270
...
function debugVehicleBloodUI(playerObj, vehicle)
	ui = ui or ISVehicleBloodUI:new(playerObj)
	ui:setVisible(true)
	ui:addToUIManager()
