...
function ISVehicleMechanics:doMenuTooltip(part, option, lua, name)
	...
	if VehicleUtils.RequiredKeyNotFound(part, self.chr) then
		tooltip.description = tooltip.description .. " " .. ISVehicleMechanics.bhs .. " " .. getText("Tooltip_vehicle_keyRequired") .. " <LINE>";
	end
	...
	if keyvalues.requireUninstalled and (vehicle:getPartById(keyvalues.requireUninstalled) and vehicle:getPartById(keyvalues.requireUninstalled):getInventoryItem()) then
		tooltip.description = tooltip.description .. " " .. ISVehicleMechanics.bhs .. " " .. getText("Tooltip_vehicle_requireUnistalled", getText("IGUI_VehiclePart" .. keyvalues.requireUninstalled)) .. " <LINE>";
	end
	...
	if perks and perks ~= "" then
		for _,perk in ipairs(perks:split(";")) do
			...
			tooltip.description = tooltip.description .. " " .. ISVehicleMechanics.ghs .. " " .. getText("Tooltip_vehicle_recommendedSkill", getText("IGUI_perks_" .. name), self.chr:getPerkLevel(Perks.FromString(name)) .. "/" .. level) .. " <LINE> <LINE>";
		end
	end
	...
	if success < 100 and failure > 0 then
		...
		getCore():getGoodHighlitedColor():interp(getCore():getGoodHighlitedColor(), failure/100, failCol);
...
function ISVehicleMechanics:doDrawItem(y, item, alt)
	...
	if item.item.cat then
		...
		if not item.item.part:getInventoryItem() and item.item.part:getTable("install") then
			self:drawText(item.item.name, 20, y, getCore():getBadHighlitedColor():getR(), getCore():getBadHighlitedColor():getG(), getCore():getBadHighlitedColor():getB(), 1, UIFont.Small);
		else
			...
			self:drawText(" (" .. condition .. "%)", getTextManager():MeasureStringX(UIFont.Small, item.item.name) + getTextManager():MeasureStringX(UIFont.Small, charge) + 22, y, condRGB.r, condRGB.g, condRGB.b, self.parent.partRGB.a, UIFont.Small)
		end
	end
...
function ISVehicleMechanics:renderCarOverlayTooltip(partProps, part, carType)
	...
	if self:isMouseOverPart(self:getMouseX(), self:getMouseY(), part) then
		...
		if (not inventoryItem and part:getTable("install")) then
			self.tooltip.description = ISVehicleMechanics.bhs .. " " .. getText("IGUI_Missing");
		else
...
function ISVehicleMechanics:renderPartDetail(part)
	...
	elseif part:getItemType() and part:getTable("install") then
		self:drawText(getText("IGUI_Item") .. ": " .. getText("IGUI_Missing"), x, y, getCore():getBadHighlitedColor():getR(), getCore():getBadHighlitedColor():getG(), getCore():getBadHighlitedColor():getB(), 1);
		y = y + lineHgt;
	end
	...
	if part:getItemType() and part:getInventoryItem() then
		if part:getId() == "GasTank" and round(part:getContainerContentAmount(), 3) <= 0.1 then
			...
			self:drawText(capacity, x, y, getCore():getBadHighlitedColor():getR(), getCore():getBadHighlitedColor():getG(), getCore():getBadHighlitedColor():getB(), 1);
			y = y + lineHgt;
		elseif part:getContainerContentType() then
			...
			if round(part:getContainerContentAmount(), 2) < 5 then
				self:drawText(label, x, y, getCore():getBadHighlitedColor():getR(), getCore():getBadHighlitedColor():getG(), getCore():getBadHighlitedColor():getB(), 1);
			else
				self:drawText(label, x, y, getCore():getGoodHighlitedColor():getR(), getCore():getGoodHighlitedColor():getG(), getCore():getGoodHighlitedColor():getB(), 1);
			end
			y = y + lineHgt;
