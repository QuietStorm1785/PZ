...
function ISVehicleHSV:createChildren()
	...
	self.nextSkinButton = ISButton:new(button5:getRight() + 10, button:getBottom() + 10, 130, 16, "Next skin", self, self.onButtonNextSkin)
...
function ISVehicleHSV:onButtonNextSkin()
	if not self.vehicle then return end
...
function ISVehicleHSV:new(playerObj)
	local width = 300
	local height = 270
...
function debugVehicleColor(playerObj, vehicle)
	ui = ui or ISVehicleHSV:new(playerObj)
	ui:setVisible(true)
	ui:addToUIManager()
