...
ISBlacksmithMenu.doBuildMenu = function(player, context, worldobjects, test)
    ...
    if not disableFurnaceAnvil then
        ...
        if playerInv:getItemCount("Base.Stone") < 1 then
            toolTip.description = toolTip.description .. " <LINE> " .. ISBlacksmithMenu.bhs .. getItemNameFromFullType("Base.Stone") .. " " .. playerInv:getItemCount("Base.Stone") .. "/50" ;
            furnaceOption.notAvailable = true;
        else
            toolTip.description = toolTip.description .. " <LINE> " .. ISBlacksmithMenu.ghs .. getItemNameFromFullType("Base.Stone") .. " " .. playerInv:getItemCount("Base.Stone") .. "/50" ;
        end
        ...
        if canBeCrafted then
            ...
            if not ingots then
                toolTip.description = toolTip.description .. " <LINE> " .. ISBlacksmithMenu.bhs .. getItemNameFromFullType("Base.IronIngot") .. " " .. (metalAmount/100) .. " / " .. (ISBlacksmithMenu.metalForAnvil/100);
                anvilOption.notAvailable = true;
            else
                toolTip.description = toolTip.description .. " <LINE> " .. ISBlacksmithMenu.ghs .. getItemNameFromFullType("Base.IronIngot") .. " " .. (metalAmount/100) .. " / " .. (ISBlacksmithMenu.metalForAnvil/100);
            end
        else
            anvilOption.notAvailable = true;
        end
        ...
        if not playerInv:contains("MetalDrum") then
            toolTip.description = toolTip.description .. " <LINE> " .. ISBlacksmithMenu.bhs .. getItemNameFromFullType("Base.MetalDrum") .. " 0/1" ;
            drumOption.notAvailable = true;
        else
            toolTip.description = toolTip.description .. " <LINE> " .. ISBlacksmithMenu.ghs .. getItemNameFromFullType("Base.MetalDrum") .. " 1/1" ;
        end
...
--        ISBlacksmithMenu.checkMetalWeldingFurnitures = function(metalPipes, smallMetalSheet, metalSheet, hinge, scrapMetal, torchUse, skill, player, toolTip)
        ...
        if playerObj:isRecipeKnown("Make Metal Containers") or ISBuildMenu.cheat then
            ...
            local shelvesOption = subMenuContainer:addOption(getText("ContextMenu_MetalShelves"), worldobjects, ISBlacksmithMenu.onMetalShelves, player,"2");
            local toolTip = ISBlacksmithMenu.addToolTip(shelvesOption, getText("ContextMenu_MetalShelves"), "furniture_shelving_01_28");
            local canDo, toolTip = ISBlacksmithMenu.checkMetalWeldingFurnitures(2,1,0,0,1,2,2,playerObj,toolTip);
            if not canDo then shelvesOption.notAvailable = true; end

            local crateOption = subMenuContainer:addOption(getText("ContextMenu_MetalCrate"), worldobjects, ISBlacksmithMenu.onMetalCrate, player,"2");
            local toolTip = ISBlacksmithMenu.addToolTip(crateOption, getText("ContextMenu_MetalCrate"), "constructedobjects_01_44");
            local canDo, toolTip = ISBlacksmithMenu.checkMetalWeldingFurnitures(2,2,2,0,1,2,4,playerObj,toolTip);
            if not canDo then crateOption.notAvailable = true; end

            local metalCounterOption = subMenuContainer:addOption(getText("ContextMenu_MetalCounter"), worldobjects, ISBlacksmithMenu.onMetalCounter, player,"2");
            local toolTip = ISBlacksmithMenu.addToolTip(metalCounterOption, getText("ContextMenu_MetalCounter"), "fixtures_counters_01_35");
            local canDo, toolTip = ISBlacksmithMenu.checkMetalWeldingFurnitures(2,4,0,2,0,2,5,playerObj,toolTip);
            if not canDo then metalCounterOption.notAvailable = true; end

            local metalCounterCornerOption = subMenuContainer:addOption(getText("ContextMenu_MetalCounterCorner"), worldobjects, ISBlacksmithMenu.onMetalCounterCorner, player,"2");
            ...
            local bigLockerOption = subMenuContainer:addOption(getText("ContextMenu_BigLocker"), worldobjects, ISBlacksmithMenu.onBigLocker, player,"3");
            local toolTip = ISBlacksmithMenu.addToolTip(bigLockerOption, getText("ContextMenu_BigLocker"), "furniture_storage_02_12");
            local canDo, toolTip = ISBlacksmithMenu.checkMetalWeldingFurnitures(8,4,0,2,0,3,9,playerObj,toolTip);
    ...
    --    end
        ...
        if playerObj:isRecipeKnown("Make Metal Walls") or ISBuildMenu.cheat then
            keepMenu = true;
            local frame = subMenuWall:addOption(getText("ContextMenu_MetalWallFrame"), worldobjects, ISBlacksmithMenu.onMetalWallFrame, player,"2");
            local toolTip = ISBlacksmithMenu.addToolTip(frame, getText("ContextMenu_MetalWallFrame"), "constructedobjects_01_68");
            local canDo, toolTip = ISBlacksmithMenu.checkMetalWeldingFurnitures(0,0,0,0,0,2,3,playerObj,toolTip, 3);
        ...
        if playerObj:isRecipeKnown("Make Metal Fences") or ISBuildMenu.cheat then
            ...
            local fenceOption = subMenuWall:addOption(getText("ContextMenu_MetalFence"), worldobjects, ISBlacksmithMenu.onMetalFence, player,"1",fenceSprite);
            local toolTip = ISBlacksmithMenu.addToolTip(fenceOption, getText("ContextMenu_MetalFence"), fenceSprite.sprite);
            local canDo, toolTip = ISBlacksmithMenu.checkMetalWeldingFurnitures(1,2,0,0,3,1,3,playerObj,toolTip);
            if not canDo then fenceOption.notAvailable = true; end

            local fencePoleOption = subMenuWall:addOption(getText("ContextMenu_MetalPoleFence"), worldobjects, ISBlacksmithMenu.onMetalPoleFence, player,"1");
            local toolTip = ISBlacksmithMenu.addToolTip(fencePoleOption, getText("ContextMenu_MetalPoleFence"), "constructedobjects_01_62");
            local canDo, toolTip = ISBlacksmithMenu.checkMetalWeldingFurnitures(3,0,0,0,0,1,3,playerObj,toolTip);
            if not canDo then fencePoleOption.notAvailable = true; end

            local wiredfenceOption = subMenuWall:addOption(getText("ContextMenu_WiredFence"), worldobjects, ISBlacksmithMenu.onWiredFence, player,"1");
            local toolTip = ISBlacksmithMenu.addToolTip(wiredfenceOption, getText("ContextMenu_WiredFence"), "fencing_01_25");
            local canDo, toolTip = ISBlacksmithMenu.checkMetalWeldingFurnitures(2,0,0,0,1,1,4,playerObj,toolTip,0,1);
--            local canDo2, toolTip = ISBlacksmithMenu.checkWire(1,playerObj,toolTip);
            ...
            local bigWiredFenceOption = subMenuWall:addOption(getText("ContextMenu_BigWiredFence"), worldobjects, ISBlacksmithMenu.onBigWiredFence, player,"1");
            local toolTip = ISBlacksmithMenu.addToolTip(bigWiredFenceOption, getText("ContextMenu_BigWiredFence"), "fencing_01_57");
            local canDo, toolTip = ISBlacksmithMenu.checkMetalWeldingFurnitures(3,0,0,0,4,1,5,playerObj,toolTip,0,3);
--            local canDo2, toolTip = ISBlacksmithMenu.checkWire(3,playerObj,toolTip);
            ...
            local bigFenceOption = subMenuWall:addOption(getText("ContextMenu_BigMetalFence"), worldobjects, ISBlacksmithMenu.onBigMetalFence, player,"2");
            local toolTip = ISBlacksmithMenu.addToolTip(bigFenceOption, getText("ContextMenu_BigMetalFence"), "constructedobjects_01_78");
            local canDo, toolTip = ISBlacksmithMenu.checkMetalWeldingFurnitures(5,0,0,0,2,2,7,playerObj,toolTip);
            ...
            local fenceGateOption = subMenuDoor:addOption(getText("ContextMenu_MetalFenceGate"), worldobjects, ISBlacksmithMenu.onFenceGate, player,"1");
            local toolTip = ISBlacksmithMenu.addToolTip(fenceGateOption, getText("ContextMenu_MetalFenceGate"), "fixtures_doors_fences_01_28");
            local canDo, toolTip = ISBlacksmithMenu.checkMetalWeldingFurnitures(3,0,0,2,2,1,4,playerObj,toolTip);
            if not canDo then fenceGateOption.notAvailable = true; end
            
            local bigFenceGateOption = subMenuDoor:addOption(getText("ContextMenu_BigMetalFenceGate"), worldobjects, ISBlacksmithMenu.onBigMetalFenceGate, player,"2");
            ...
            local canDo, toolTip = ISBlacksmithMenu.checkMetalWeldingFurnitures(5,0,0,2,4,2,7,playerObj,toolTip);
            if not canDo then bigFenceGateOption.notAvailable = true; end
    
            local doubleDoorOption = subMenuDoor:addOption(getText("ContextMenu_BigMetalDoubleDoor"), worldobjects, ISBlacksmithMenu.onDoublePoleDoor, player,"2");
            ...
            local canDo, toolTip = ISBlacksmithMenu.checkMetalWeldingFurnitures(10,0,0,2,4,2,8,playerObj,toolTip);
            if not canDo then doubleDoorOption.notAvailable = true; end
    
            local doubleDoorOption = subMenuDoor:addOption(getText("ContextMenu_Double_Metal_Door"), worldobjects, ISBlacksmithMenu.onDoubleMetalDoor, player,"2");
            ...
            local canDo, toolTip = ISBlacksmithMenu.checkMetalWeldingFurnitures(8,0,0,2,2,2,7,playerObj,toolTip,0,4);
--            local canDo2, toolTip = ISBlacksmithMenu.checkWire(4,playerObj,toolTip);
    ...
    --        end
        ...
        if playerObj:isRecipeKnown("Make Metal Roof") or ISBuildMenu.cheat then
            keepMenu = true;
            local metalFloorOption = subMenuMW:addOption(getText("ContextMenu_MetalRoof"), worldobjects, ISBlacksmithMenu.onMetalFloor, player,"1");
...
ISBlacksmithMenu.checkWire = function(wireUses, player, toolTip)
    ...
    if totalUse > wireUses then
        toolTip.description = toolTip.description .. " <LINE> " .. ISBlacksmithMenu.ghs .. getItemNameFromFullType("Base.Wire") .. " " .. totalUse .. "/" .. wireUses .. " " .. getText("ContextMenu_Uses");
    else
        toolTip.description = toolTip.description .. " <LINE> " .. ISBlacksmithMenu.bhs .. getItemNameFromFullType("Base.Wire") .. " " .. totalUse .. "/" .. wireUses .. " " .. getText("ContextMenu_Uses");
        canDo = false;
    end
...
ISBlacksmithMenu.checkMetalWeldingFurnitures = function(metalPipes, smallMetalSheet, metalSheet, hinge, scrapMetal, torchUse, skill, player, toolTip, metalBar, wire)
    ...
    if blowTorch then
        if torchUse > 0 then
            if blowTorchUseLeft < torchUse then
                toolTip.description = toolTip.description .. " <LINE> " .. ISBlacksmithMenu.bhs .. getItemNameFromFullType("Base.BlowTorch") .. " " .. getText("ContextMenu_Uses") .. " " .. blowTorchUseLeft .. "/" .. torchUse;
                isOk = false;
            else
                toolTip.description = toolTip.description .. " <LINE> " .. ISBlacksmithMenu.ghs .. getItemNameFromFullType("Base.BlowTorch") .. " " .. getText("ContextMenu_Uses") .. " " .. blowTorchUseLeft .. "/" .. torchUse;
            end
        end
    else
        toolTip.description = toolTip.description .. " <LINE> " .. ISBlacksmithMenu.bhs .. getItemNameFromFullType("Base.BlowTorch") .. " 0" .. "/" .. torchUse;
        isOk = false;
    end
    ...
    if rodUse > 0 then
        if weldingRods < rodUse then
            toolTip.description = toolTip.description .. " <LINE> " .. ISBlacksmithMenu.bhs .. getItemNameFromFullType("Base.WeldingRods") .. " " .. getText("ContextMenu_Uses") .. " " .. weldingRods .. "/" .. rodUse;
            isOk = false;
        else
            toolTip.description = toolTip.description .. " <LINE> " .. ISBlacksmithMenu.ghs .. getItemNameFromFullType("Base.WeldingRods") .. " " .. getText("ContextMenu_Uses") .. " " .. weldingRods .. "/" .. rodUse;
        end
    end
    ...
    if not inv:containsEvalRecurse(predicateWeldingMask) then
        local weldingMask = 0
        toolTip.description = toolTip.description .. " <LINE> " .. ISBlacksmithMenu.bhs .. getItemNameFromFullType("Base.WeldingMask") .. " " .. weldingMask .. "/1" ;
        isOk = false;
    else
        local weldingMask = 1
        toolTip.description = toolTip.description .. " <LINE> " .. ISBlacksmithMenu.ghs .. getItemNameFromFullType("Base.WeldingMask") .. " " .. weldingMask .. "/1" ;
    end
    if metalBar and metalBar > 0 then
        local count = ISBlacksmithMenu.getMaterialCount(player, "MetalBar")
        if count < metalBar then
            toolTip.description = toolTip.description .. " <LINE> " .. ISBlacksmithMenu.bhs .. getItemNameFromFullType("Base.MetalBar") .. " " .. count .. "/" .. metalBar;
            isOk = false;
        else
            toolTip.description = toolTip.description .. " <LINE> " .. ISBlacksmithMenu.ghs .. getItemNameFromFullType("Base.MetalBar") .. " " .. count .. "/" .. metalBar ;
        end
    end
    if metalPipes > 0 then
        local count = ISBlacksmithMenu.getMaterialCount(player, "MetalPipe")
        if count < metalPipes then
            toolTip.description = toolTip.description .. " <LINE> " .. ISBlacksmithMenu.bhs .. getItemNameFromFullType("Base.MetalPipe") .. " " .. count .. "/" .. metalPipes;
            isOk = false;
        else
            toolTip.description = toolTip.description .. " <LINE> " .. ISBlacksmithMenu.ghs .. getItemNameFromFullType("Base.MetalPipe") .. " " .. count .. "/" .. metalPipes ;
        end
    end
    if smallMetalSheet > 0 then
        ...
        if count < smallMetalSheet then
            toolTip.description = toolTip.description .. " <LINE> " .. ISBlacksmithMenu.bhs .. getItemNameFromFullType("Base.SmallSheetMetal") .. " " .. count .. "/" .. smallMetalSheet;
            isOk = false;
        else
            toolTip.description = toolTip.description .. " <LINE> " .. ISBlacksmithMenu.ghs .. getItemNameFromFullType("Base.SmallSheetMetal") .. " " .. count .. "/" .. smallMetalSheet ;
        end
    end
    if metalSheet > 0 then
        local count = ISBlacksmithMenu.getMaterialCount(player, "SheetMetal")
        if count < metalSheet then
            toolTip.description = toolTip.description .. " <LINE> " .. ISBlacksmithMenu.bhs .. getItemNameFromFullType("Base.SheetMetal") .. " " .. count .. "/" .. metalSheet;
            isOk = false;
        else
            toolTip.description = toolTip.description .. " <LINE> " .. ISBlacksmithMenu.ghs .. getItemNameFromFullType("Base.SheetMetal") .. " " .. count .. "/" .. metalSheet ;
        end
    end
    if hinge > 0 then
        local count = ISBlacksmithMenu.getMaterialCount(player, "Hinge")
        if count < hinge then
            toolTip.description = toolTip.description .. " <LINE> " .. ISBlacksmithMenu.bhs .. getItemNameFromFullType("Base.Hinge") .. " " .. count .. "/" .. hinge;
            isOk = false;
        else
            toolTip.description = toolTip.description .. " <LINE> " .. ISBlacksmithMenu.ghs .. getItemNameFromFullType("Base.Hinge") .. " " .. count .. "/" .. hinge ;
        end
    end
    if scrapMetal > 0 then
        local count = ISBlacksmithMenu.getMaterialCount(player, "ScrapMetal")
        if count < scrapMetal then
            toolTip.description = toolTip.description .. " <LINE> " .. ISBlacksmithMenu.bhs .. getItemNameFromFullType("Base.ScrapMetal") .. " " .. count .. "/" .. scrapMetal;
            isOk = false;
        else
            toolTip.description = toolTip.description .. " <LINE> " .. ISBlacksmithMenu.ghs .. getItemNameFromFullType("Base.ScrapMetal") .. " " .. count .. "/" .. scrapMetal ;
        end
    end
    if wire ~= nil and wire > 0 then
        local count = ISBlacksmithMenu.getMaterialUses(player, "Wire");
        if count < wire then
            toolTip.description = toolTip.description .. " <LINE> " .. ISBlacksmithMenu.bhs .. getItemNameFromFullType("Base.Wire") .. " " .. getText("ContextMenu_Uses") .. " " .. count .. "/" .. wire;
            isOk = false;
        else
            toolTip.description = toolTip.description .. " <LINE> " .. ISBlacksmithMenu.ghs .. getItemNameFromFullType("Base.Wire") .. " " .. getText("ContextMenu_Uses") .. " " .. count .. "/" .. wire;
        end
    end
    toolTip.description = toolTip.description .. " <LINE> ";
    if skill > 0 then
        if player:getPerkLevel(Perks.MetalWelding) < skill then
            toolTip.description = toolTip.description .. " <LINE> " .. ISBlacksmithMenu.bhs .. getText("IGUI_perks_MetalWelding") .. " " .. player:getPerkLevel(Perks.MetalWelding) .. "/" .. skill;
            isOk = false;
        else
            toolTip.description = toolTip.description .. " <LINE> " .. ISBlacksmithMenu.ghs .. getText("IGUI_perks_MetalWelding") .. " " .. player:getPerkLevel(Perks.MetalWelding) .. "/" .. skill ;
        end
    end
    if ISBuildMenu.cheat then
        return true, toolTip;
...
--    fence.firstArg = tonumber(torchUse)
    ...
    fence.player = player
    fence.corner = "constructedobjects_01_51";
