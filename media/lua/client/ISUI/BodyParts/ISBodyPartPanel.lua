...
function ISBodyPartPanel:setSelectLinesAnchor( _anchorX, _anchorY )
    self.selectlines.anchorX = PZMath.clampFloat(_anchorX, 0, self:getWidth());
    self.selectlines.anchorY = PZMath.clampFloat(_anchorY, 0, self:getHeight());
end
...
function ISBodyPartPanel:setValue( _bodyPartType, _value, _force )
    local target, bp;
    local val = PZMath.clampFloat(_value, self.minValue, self.maxValue);
    for i=1, #self.bps do
        bp = self.bps[i];
        if bp.bodyPartType == _bodyPartType then
...
function ISBodyPartPanel:setColorForValue( _val, _color )
    local val = PZMath.clampFloat(_val, self.minValue, self.maxValue);
    local colS = self.colorScheme[1].color;
    local colE = colS;
    local s, e = 0, 1;
    for j=1,#self.colorScheme do
