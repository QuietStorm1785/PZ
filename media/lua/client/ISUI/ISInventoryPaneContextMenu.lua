...
function ISInventoryPaneContextMenu.doLiteratureMenu(context, items, player)
	...
	for i,k in ipairs(actualItems) do
		if getSpecificPlayer(player):getTraits():isIlliterate() then   
			...
			nope.notAvailable = true -- THIS IS HOW YOU MAKE CONTEXT MENU OPTIONS RED! VERY IMPORTANT!
			local tooltip = ISInventoryPaneContextMenu.addToolTip();
			tooltip.description = getText("ContextMenu_Illiterate");
			nope.toolTip = tooltip;	
			return	
		elseif k:getLvlSkillTrained() ~= -1 and SkillBook[k:getSkillTrained()].perk and	k:getLvlSkillTrained() > getSpecificPlayer(player):getPerkLevel(SkillBook[k:getSkillTrained()].perk) + 1 then   
			local nope = context:addOption(getText("ContextMenu_Read"));	
			nope.notAvailable = true -- THIS IS HOW YOU MAKE CONTEXT MENU OPTIONS RED! VERY IMPORTANT!
			local tooltip = ISInventoryPaneContextMenu.addToolTip();
...
ISInventoryPaneContextMenu.checkConsolidate = function(drainable, player, context, previousPourInto)
    ...
    if #consolidateList > 0 then
        ...
        if #consolidateList > 1 then
            subMenuConsolidate:addOption(getText("ContextMenu_MergeAll"), drainable, ISInventoryPaneContextMenu.onConsolidateAll, consolidateList, player)
        end
...
ISInventoryPaneContextMenu.onConsolidate = function(drainable, intoItem, player)
    ISTimedActionQueue.add(ISConsolidateDrainable:new(player, drainable, intoItem, 90, nil));
end
...
ISInventoryPaneContextMenu.onConsolidateAll = function(drainable, consolidateList, player)
	if drainable:getUsedDelta() < 1 then
		...
		ISTimedActionQueue.add(ISConsolidateDrainable:new(player, drainable, intoItem, 90, consolidateList))
	else
...
ISInventoryPaneContextMenu.doDrinkForThirstMenu = function(context, playerObj, waterContainer)
    ...
    if waterContainer:isTaintedWater() and getSandboxOptions():getOptionByName("EnableTaintedWaterText"):getValue() then
        tooltip.description = tooltip.description .. " <BR> <RGB:1,0.5,0.5> " .. getText("Tooltip_item_TaintedWater")
    end
    option.toolTip = tooltip
--[[
    ...
    if button.internal == "OK" then
        ...
        if button.parent.entry:getText() and button.parent.entry:getText() ~= "" then
			if length <= MAXIMUM_RENAME_LENGTH then 
				...
				player:Say(getText("IGUI_PlayerText_ItemNameTooLong"));
			end
        end
    end
    if JoypadState.players[playerNum+1] then
...
--	end
	...
	if item:getRequireInHandOrInventory() then
        ...
        for j=1, items:size() do
            if typesTable[items:get(j-1):getFullType()] then
...
ISInventoryPaneContextMenu.doMakeUpMenu = function(context, makeup, playerObj)
    ...
    if playerObj:getInventory():contains("Mirror") or (playerObj:getVehicle() ~= null) or playerObj:getInventory():contains("Base.MakeupFoundation") or makeup:getFullType() == "Base.MakeupFoundation" then
        mirror = true;
    end
