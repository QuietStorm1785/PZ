...
function ISContextMenu:onMouseDownOutside(x, y)
	if self.player == 0 then
		...
		if JoypadState.players[self.player+1] and self.parent == nil then
			setJoypadFocus(self.player, self.origin)
		end
	end
end
...
function ISContextMenu:render()
	...
	self:drawRect(0, dy - self:getYScroll(), iconSize + iconShiftX*2, self.height - dy * 2, 0.25, 0.1, 0.1, 0.1);
...
function ISContextMenu:insertOptionBefore(nextOptionName, name, target, onSelect, param1, param2, param3, param4, param5, param6, param7, param8, param9, param10)
	...
	if index == 1 then
		return self:addOptionOnTop(name, target, onSelect, param1, param2, param3, param4, param5, param6, param7, param8, param9, param10)
	end

	for i = #self.options, index, -1 do
		self.options[i+1] = self.options[i]
	end
...
function ISContextMenu:removeOptionByName(optName)
	...
	for i, v in ipairs(self.options) do
		index = i
		if v.name == optName then
			table.insert(self.optionPool, v)
			isFound = true
			break
		end
	end
	if not isFound then return end
	for i = index+1, #self.options do
		self.options[i-1] = self.options[i]
		self.options[i-1].id = i-1
	end
