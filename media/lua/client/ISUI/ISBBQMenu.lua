...
function ISBBQMenu.OnFillWorldObjectContextMenu(player, context, worldobjects, test)
	...
	if (#tinderList > 0) and (startFire or lighter or matches) and not bbq:isLit() then
		lightFromLiterature = bbq
	end
	if (startFire or lighter or matches) and petrol and not bbq:isLit() and bbq:hasFuel() then
		lightFromPetrol = bbq
	end
	...
	if lightFromPetrol or lightFromLiterature or lightFromKindle then
		...
		if lightFromPetrol then
			if startFire then
				subMenuLight:addOption(petrol:getName()..' + '..startFire:getName(), worldobjects, ISBBQMenu.onLightFromPetrol, player, startFire, petrol, lightFromPetrol)
			end
			if lighter and lighter ~= startFire then
			...
			if matches and matches ~= startFire then
		...
		for i,v in pairs(tinderList) do
			...
			if startFire then
				local subOption = subMenuLight:addActionsOption(label..' + '..startFire:getName(), ISBBQMenu.onLightFromLiterature, v:getFullType(), startFire, lightFromLiterature)
				local tooltip = ISWorldObjectContextMenu.addToolTip()
				tooltip.description = getText("IGUI_BBQ_FuelAmount", ISCampingMenu.timeString(ISCampingMenu.getFuelDurationForItem(v)))
				subOption.toolTip = tooltip
			end
