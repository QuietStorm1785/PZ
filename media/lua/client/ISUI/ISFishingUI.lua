...
--    if not self.selectedRod then return; end
    ...
    local indexToSelect = -1;
    for _,v in ipairs(sorted) do
        self.lures:addOption(v.item:getDisplayName() .. " (" .. v.item:getContainer():getNumberOfItem(v.item:getFullType()) .. ")", v.item, v.item:getTexture());
...
function ISFishingUI:render()
    ...
    getCore():getBadHighlitedColor():interp(getCore():getGoodHighlitedColor(), self.zoneProgress/100, color);
    local zoneProgressBar = {r=color:getR(), g=color:getG(), b=color:getB(), a=1}
    ...
    if currentAction and (currentAction.Type == "ISFishingAction") and currentAction.action then
        local fishingProgressBar = {r=getCore():getGoodHighlitedColor():getR(), g=getCore():getGoodHighlitedColor():getG(), b=getCore():getGoodHighlitedColor():getB(), a=1}
        self:drawProgressBar(10, self.barY, self.width - 20, self.barHgt, currentAction.action:getJobDelta(), fishingProgressBar)
