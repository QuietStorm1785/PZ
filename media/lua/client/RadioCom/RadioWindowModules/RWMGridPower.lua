...
function RWMGridPower:render()
    ...
    if self.deviceData then
        ...
        if self.deviceData:canBePoweredHere() then
            ...
            self:drawText(getText("IGUI_RadioRequiresPowerNearby"), x, y, getCore():getBadHighlitedColor():getR(),getCore():getBadHighlitedColor():getG(),getCore():getBadHighlitedColor():getB(),1, UIFont.Small);
        end
    end
end
