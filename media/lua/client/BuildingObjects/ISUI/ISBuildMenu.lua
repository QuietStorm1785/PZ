                ...
                if not ISBuildMenu.cheat then
                    for x=0,stage:getItemsToKeep():size()-1 do
                        ...
                        if item then
                            ...
                            if hasItem then
								...
								tooltip.description = tooltip.description .. ISBuildMenu.bhs .. item:getName() .. " <LINE> ";
                                notAvailable = true;
                            end
                        end
                    end
                        ...
                        if item then
                            if instanceof(item, "DrainableComboItem") then
                                ...
                                if useLeft >= tonumber(v) then
									tooltip.description = tooltip.description .. ISBuildMenu.ghs .. item:getName() .. " " .. useLeft .. "/" .. v .. " <LINE> ";
                                else
									tooltip.description = tooltip.description .. ISBuildMenu.bhs .. item:getName() .. " " .. useLeft .. "/" .. v .. " <LINE> ";
                                    notAvailable = true;
                                end
                            else
                                ...
                                if nbOfItem >= tonumber(v) then
									tooltip.description = tooltip.description .. ISBuildMenu.ghs .. item:getName() .. " " .. nbOfItem .. "/" .. v .. " <LINE> ";
                                else
									tooltip.description = tooltip.description .. ISBuildMenu.bhs .. item:getName() .. " " .. nbOfItem .. "/" .. v .. " <LINE> ";
                                    notAvailable = true;
                                end
                            end
                        end
                    ...
                    for x,v in pairs(perks) do
                        ...
                        if playerObj:getPerkLevel(x) >= tonumber(v) then
							tooltip.description = tooltip.description .. ISBuildMenu.ghs .. getText("IGUI_perks_" .. perk:getType():toString()) .. " " .. playerObj:getPerkLevel(x) .. "/" ..  v .. " <LINE>";
                        else
							tooltip.description = tooltip.description .. ISBuildMenu.bhs .. getText("IGUI_perks_" .. perk:getType():toString()) .. " " .. playerObj:getPerkLevel(x) .. "/" ..  v .. " <LINE>";
                            notAvailable = true;
                        end
                    end
                    ...
                    if knownRecipe then
                        ...
                        if playerObj:isRecipeKnown(stage:getKnownRecipe()) then
							tooltip.description = tooltip.description .. ISBuildMenu.ghs .. getText("Tooltip_vehicle_requireRecipe", getRecipeDisplayName(knownRecipe)) .. " <LINE>"
                        else
							tooltip.description = tooltip.description .. ISBuildMenu.bhs .. getText("Tooltip_vehicle_requireRecipe", getRecipeDisplayName(knownRecipe)) .. " <LINE>"
                            notAvailable = true
                        end
                    end
	...
	if not ISBuildMenu.cheat then
		...
		for x=0,stage:getItemsToKeep():size()-1 do
			...
			if item then
				...
				if hasItem then
					tooltipTxt = tooltipTxt .. ISBuildMenu.ghs .. item:getName() .. " <LINE> ";
				else
					tooltipTxt = tooltipTxt .. ISBuildMenu.bhs .. item:getName() .. " <LINE> ";
					available = false;
				end
			end
		end
		...
		for x,v in pairs(items) do
			...
			if item then
				if instanceof(item, "DrainableComboItem") then
					...
					if useLeft >= tonumber(v) then
						tooltipTxt = tooltipTxt .. ISBuildMenu.ghs .. item:getName() .. " " .. useLeft .. "/" .. v .. " <LINE> ";
					else
						tooltipTxt = tooltipTxt .. ISBuildMenu.bhs .. item:getName() .. " " .. useLeft .. "/" .. v .. " <LINE> ";
						available = false;
					end
				else
					local nbOfItem = playerInv:getCountTypeEvalRecurse(x, buildUtil.predicateMaterial);
					...
					if nbOfItem >= tonumber(v) then
						tooltipTxt = tooltipTxt .. ISBuildMenu.ghs .. item:getName() .. " " .. nbOfItem .. "/" .. v .. " <LINE> ";
					else
						tooltipTxt = tooltipTxt .. ISBuildMenu.bhs .. item:getName() .. " " .. nbOfItem .. "/" .. v .. " <LINE> ";
						available = false;
					end
				end
			end
		...
		for x,v in pairs(perks) do
			local perk = PerkFactory.getPerk(x);
			if playerObj:getPerkLevel(x) >= tonumber(v) then
				tooltipTxt = tooltipTxt .. ISBuildMenu.ghs .. getText("IGUI_perks_" .. perk:getType():toString()) .. " " .. playerObj:getPerkLevel(x) .. "/" ..  v .. " <LINE>";
			else
				tooltipTxt = tooltipTxt .. ISBuildMenu.bhs .. getText("IGUI_perks_" .. perk:getType():toString()) .. " " .. playerObj:getPerkLevel(x) .. "/" ..  v .. " <LINE>";
				available = false;
			end
		end
		local knownRecipe = stage:getKnownRecipe()
		if knownRecipe then
			tooltipTxt = tooltipTxt .. " <LINE> "
			if playerObj:isRecipeKnown(stage:getKnownRecipe()) then
				tooltipTxt = tooltipTxt .. ISBuildMenu.ghs .. getText("Tooltip_vehicle_requireRecipe", getRecipeDisplayName(knownRecipe)) .. " <LINE>"
			else
				tooltipTxt = tooltipTxt .. ISBuildMenu.bhs .. getText("Tooltip_vehicle_requireRecipe", getRecipeDisplayName(knownRecipe)) .. " <LINE>"
				available = false;
			end
		end
	end
...
ISBuildMenu.buildMiscMenu = function(subMenu, option, player)
	...
	if stones < 6 then
		toolTip.description = toolTip.description .. ISBuildMenu.bhs .. getItemNameFromFullType("Base.Stone") .. " " .. stones .. "/6 ";
		if not ISBuildMenu.cheat then
			stonePileOption.onSelect = nil;
			stonePileOption.notAvailable = true;
		end
	else
		toolTip.description = toolTip.description .. ISBuildMenu.ghs .. getItemNameFromFullType("Base.Stone") .. " " .. stones .. "/6 ";
	end
	...
	if ropes == 0 then
		toolTip.description = toolTip.description .. ISBuildMenu.bhs .. getItemNameFromFullType("Base.SheetRope") .. " " .. ropes .. "/1 ";
		if not ISBuildMenu.cheat then
			woodenPicketOption.onSelect = nil;
			woodenPicketOption.notAvailable = true;
		end
	else
		toolTip.description = toolTip.description .. ISBuildMenu.ghs .. getItemNameFromFullType("Base.SheetRope") .. " " .. ropes .. "/1 ";
	end
...
--	fence.crossSpeed = 0.3;
	...
	fence.craftingBank = "BuildFenceSandbagFoley";
...
--    fence.crossSpeed = 0.3;
	...
	fence.craftingBank = "BuildFenceGravelbagFoley";
...
ISBuildMenu.buildLightMenu = function(subMenu, player)
    ...
    if not playerInv:containsTypeRecurse("Torch") and not ISBuildMenu.materialOnGround["Torch"] then
		toolTip.description = toolTip.description .. ISBuildMenu.bhs .. getItemNameFromFullType("Base.Torch") .. " " .. torch .. "/1 ";
        if not ISBuildMenu.cheat then
            lampOption.onSelect = nil;
            lampOption.notAvailable = true;
        end
    else
		toolTip.description = toolTip.description .. ISBuildMenu.ghs .. getItemNameFromFullType("Base.Torch") .. " " .. torch .. "/1 ";
    end
    ...
    if not playerInv:containsTypeRecurse("Rope") and not ISBuildMenu.materialOnGround["Rope"] then
		toolTip.description = toolTip.description .. " <LINE> " .. ISBuildMenu.bhs .. getItemNameFromFullType("Base.Rope") .. " " .. rope .. "/1 ";
        if not ISBuildMenu.cheat then
            lampOption.onSelect = nil;
            lampOption.notAvailable = true;
        end
    else
		toolTip.description = toolTip.description .. " <LINE> " .. ISBuildMenu.ghs .. getItemNameFromFullType("Base.Rope") .. " " .. rope .. "/1 ";
    end
...
--	ISBuildMenu.requireHammer(wallOption)
    ...
    if numLog < 4 then
		tooltip.description = tooltip.description .. ISBuildMenu.bhs .. getItemNameFromFullType("Base.Log") .. " " .. numLog .. "/4 <LINE> ";
        if not ISBuildMenu.cheat then
            logOption.onSelect = nil;
            logOption.notAvailable = true;
        end
    else
		tooltip.description = tooltip.description .. ISBuildMenu.ghs .. getItemNameFromFullType("Base.Log") .. " " .. numLog .. "/4 <LINE> ";
    end
    ...
    if numRippedSheets >= 4 then
		tooltip.description = tooltip.description .. ISBuildMenu.ghs .. getItemNameFromFullType("Base.RippedSheets") .. " " .. numRippedSheets .. "/4 ";
    elseif numTwine >= 4 then
		tooltip.description = tooltip.description .. ISBuildMenu.ghs .. getItemNameFromFullType("Base.Twine") .. " " .. numTwine .. "/4 ";
    elseif numRope >= 2 then
		tooltip.description = tooltip.description .. ISBuildMenu.ghs .. getItemNameFromFullType("Base.Rope") .. " " .. numRope .. "/2 ";
    else
		tooltip.description = tooltip.description .. ISBuildMenu.bhs .. getItemNameFromFullType("Base.RippedSheets") .. " " .. numRippedSheets .. "/4 <LINE> " .. getText("ContextMenu_or") .. " " .. getItemNameFromFullType("Base.Twine") .. " " .. numTwine .. "/4 <LINE> " .. getText("ContextMenu_or") .. " " .. getItemNameFromFullType("Base.Rope") .. " " .. numRope .. "/2 ";
        if not ISBuildMenu.cheat then
            logOption.onSelect = nil;
            logOption.notAvailable = true;
        end
...
ISBuildMenu.canBuild = function(plankNb, nailsNb, hingeNb, doorknobNb, baredWireNb, carpentrySkill, option, player)
	...
	if ISBuildMenu.planks < plankNb then
		tooltip.description = tooltip.description .. ISBuildMenu.bhs .. getItemNameFromFullType("Base.Plank") .. " " .. ISBuildMenu.planks .. "/" .. plankNb .. " <LINE>";
		result = false;
	elseif plankNb > 0 then
		tooltip.description = tooltip.description .. ISBuildMenu.ghs .. getItemNameFromFullType("Base.Plank") .. " " .. ISBuildMenu.planks .. "/" .. plankNb .. " <LINE>";
	end
	if ISBuildMenu.nails < nailsNb then
		tooltip.description = tooltip.description .. ISBuildMenu.bhs .. getItemNameFromFullType("Base.Nails") .. " " .. ISBuildMenu.nails .. "/" .. nailsNb .. " <LINE>";
		result = false;
	elseif nailsNb > 0 then
		tooltip.description = tooltip.description .. ISBuildMenu.ghs .. getItemNameFromFullType("Base.Nails") .. " " .. ISBuildMenu.nails .. "/" .. nailsNb .. " <LINE>";
	end
	if ISBuildMenu.doorknob < doorknobNb then
		tooltip.description = tooltip.description .. ISBuildMenu.bhs .. getItemNameFromFullType("Base.Doorknob") .. " " .. ISBuildMenu.doorknob .. "/" .. doorknobNb .. " <LINE>";
		result = false;
	elseif doorknobNb > 0 then
		tooltip.description = tooltip.description .. ISBuildMenu.ghs .. getItemNameFromFullType("Base.Doorknob") .. " " .. ISBuildMenu.doorknob .. "/" .. doorknobNb .. " <LINE>";
	end
	if ISBuildMenu.hinge < hingeNb then
		tooltip.description = tooltip.description .. ISBuildMenu.bhs .. getItemNameFromFullType("Base.Hinge") .. " " .. ISBuildMenu.hinge .. "/" .. hingeNb .. " <LINE>";
		result = false;
	elseif hingeNb > 0 then
		tooltip.description = tooltip.description .. ISBuildMenu.ghs .. getItemNameFromFullType("Base.Hinge") .. " " .. ISBuildMenu.hinge .. "/" .. hingeNb .. " <LINE>";
	end
	if getSpecificPlayer(player):getPerkLevel(Perks.Woodwork) < carpentrySkill then
		tooltip.description = tooltip.description .. ISBuildMenu.bhs .. getText("IGUI_perks_Carpentry") .. " " .. getSpecificPlayer(player):getPerkLevel(Perks.Woodwork) .. "/" .. carpentrySkill .. " <LINE>";
		result = false;
	elseif carpentrySkill > 0 then
		tooltip.description = tooltip.description .. ISBuildMenu.ghs .. getText("IGUI_perks_Carpentry") .. " " .. getSpecificPlayer(player):getPerkLevel(Perks.Woodwork) .. "/" .. carpentrySkill .. " <LINE>";
	end
	if ISBuildMenu.cheat then
		return tooltip;
	end
