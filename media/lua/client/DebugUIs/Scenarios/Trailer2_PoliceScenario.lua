...
debugScenarios.Trailer2_PoliceScenario = {
	name = "LIFE 2 - POLICE STATION",
--	forceLaunch = true, -- use this to force the launch of THIS scenario right after main menu was loaded, save more clicks! Don't do multiple scenarii with this options
	startLoc = {x=11903, y=6950, z=0 }, -- WP police station
	setSandbox = function()
	...
	onStart = function()
		...
		local bDef = chr:getCurrentSquare():getBuilding():getDef();
		addZombiesInBuilding(bDef, 3, "Police", bDef:getRoom("meeting"), nil);
		addZombiesInBuilding(bDef, 1, "OfficeWorker", bDef:getRoom("meeting"), 0);
		addZombiesInBuilding(bDef, 1, "OfficeWorkerSkirt", bDef:getRoom("meeting"), 100);
	
		addZombiesInOutfit(11901, 6945, 0, 1, "Police", nil);
		addZombiesInOutfit(11901, 6951, 0, 1, "Police", nil);
