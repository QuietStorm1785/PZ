...
function ServerWorkshopItemScreen:prerender()
	ISPanelJoypad.prerender(self)

	if self.joyfocus and self.joypadIndexY > 0 then
		self.ISButtonA = nil
		self.ISButtonB = nil
		self.installButton:clearJoypadButton()
		self.disconnectButton:clearJoypadButton()
	elseif self.joyfocus and self.joypadIndexY == 0 then
		self:setISButtonForA(self.installButton)
		self:setISButtonForB(self.disconnectButton)
	end
end
...
function ServerWorkshopItemScreen:onGainJoypadFocus(joypadData)
	ISPanelJoypad.onGainJoypadFocus(self, joypadData)
end

function ServerWorkshopItemScreen:onJoypadDirUp(joypadData)
end

function ServerWorkshopItemScreen:onJoypadDirDown(joypadData)
end
