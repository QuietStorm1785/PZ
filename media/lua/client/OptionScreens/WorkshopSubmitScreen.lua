...
function Page1:aboutToShow(joypadData)
	self.listbox:clear()
...
function Page1:onButtonNext()
	...
	if err then
		...
		self.parent.page2:aboutToShow(self.joyfocus)
...
function Page1:onJoypadDirUp(joypadData)
	self.listbox:onJoypadDirUp(joypadData)
end

function Page1:onJoypadDirDown(joypadData)
...
function Page2:create()
	...
	self.tags = TagsList:new(padX, label6:getBottom() + 2, entryWid * 0.75, itemHgt * 4)
...
--]]
	...
	self:insertNewLineOfButtons(self.editDescription)
	self:insertNewLineOfButtons(self.tags)
	self:insertNewLineOfButtons(self.visibility)
end

function Page2:aboutToShow(joypadData)
	if joypadData then
...
function Page2:updateWhenVisible()
	...
	if self.joyfocus and self.joypadIndexY > 0 then
		...
		self.overlayButton:clearJoypadButton()
...
function Page2:onButtonNext()
	...
	if self.item:getID() then
		...
		self.parent.page3:aboutToShow(self.joyfocus)
...
function Page2:onJoypadDown(button, joypadData)
...
function Page2:onJoypadDirUp(joypadData)
...
function Page2:onJoypadDirDown(joypadData)
	if self.joypadIndexY == 0 then
		return
	end
...
function Page3:aboutToShow(joypadData)
	if joypadData then
		self:clearJoypadFocus(joypadData)
		self.joypadIndexY = 1
	end
...
function Page3:onButtonEnterID()
	...
	self.parent.page4:aboutToShow(self.joyfocus)
...
function Page3:onJoypadDown(button, joypadData)
	if button == Joypad.BButton then
		if self.joypadIndexY > 0 and not self:isFocusOnControl() then
			self:clearJoypadFocus(joypadData)
			self.joypadIndexY = 0
...
function Page3:onJoypadDirDown(joypadData)
	if self.joypadIndexY == 0 then
		return
	end
	if self.joypadIndexY == #self.joypadButtonsY then
...
function Page4:aboutToShow(joypadData)
	if joypadData then
		self:clearJoypadFocus(joypadData)
		self.joypadIndexY = 1
	end
...
function Page4:onJoypadDown(button, joypadData)
	if button == Joypad.BButton then
		if self.joypadIndexY > 0 and not self:isFocusOnControl() then
			self:clearJoypadFocus(joypadData)
			self.joypadIndexY = 0
...
function Page4:onJoypadDirUp(joypadData)
	if self.joypadIndexY == 0 then
		self.joypadIndexY = #self.joypadButtonsY
		self:restoreJoypadFocus(joypadData)
		return
...
function Page4:onJoypadDirDown(joypadData)
	if self.joypadIndexY == 0 then
		return
	end
	if self.joypadIndexY == #self.joypadButtonsY then
...
function Page5:onButtonBack()
	self:setVisible(false)
	if self.prevPage.aboutToShow then
		self.prevPage:aboutToShow(self.joyfocus)
	end
...
function Page5:onJoypadDown(button, joypadData)
	if button == Joypad.BButton then
		if self.joypadIndexY > 0 and not self:isFocusOnControl() then
			self:clearJoypadFocus(joypadData)
			self.joypadIndexY = 0
...
function Page5:onJoypadDirUp(joypadData)
	if self.joypadIndexY == 0 then
		self.joypadIndexY = #self.joypadButtonsY
		self:restoreJoypadFocus(joypadData)
		return
...
function Page5:onJoypadDirDown(joypadData)
	if self.joypadIndexY == 0 then
		return
	end
	if self.joypadIndexY == #self.joypadButtonsY then
...
function Page6:onJoypadDown(button, joypadData)
	if button == Joypad.BButton then
		if self.joypadIndexY > 0 and not self:isFocusOnControl() then
			self:clearJoypadFocus(joypadData)
			self.joypadIndexY = 0
...
function Page6:onJoypadDirUp(joypadData)
	if self.joypadIndexY == 0 then
		self.joypadIndexY = 1
		self:restoreJoypadFocus(joypadData)
		return
...
function Page6:onJoypadDirDown(joypadData)
	if self.joypadIndexY == 0 then
		return
	end
	if true then
...
function Page8:onGainJoypadFocus(joypadData)
	ISPanelJoypad.onGainJoypadFocus(self, joypadData)
	self:setISButtonForB(self.backButton)
end
...
function Page9:aboutToShow(joypadData)
...
function Page9:onButtonViewSubscriptions()
	self:setVisible(false)
	self.parent.page10:aboutToShow(self.joyfocus)
	self.parent.page10:setVisible(true, self.joyfocus)
end

function Page9:onButtonCreateAndUpdate()
	self:setVisible(false)
	self.parent.page1:aboutToShow(self.joyfocus)
	self.parent.page1:setVisible(true, self.joyfocus)
end
...
function Page9:onGainJoypadFocus(joypadData)
	ISPanelJoypad.onGainJoypadFocus(self, joypadData)
	self:setISButtonForB(self.backButton)
	self:restoreJoypadFocus()
end
...
function Page10:aboutToShow(joypadData)
end
...
function TagsList:doDrawItem(y, item, alt)
	...
	if self.enabled and item.index == self.selected and self.joyfocus then
		self:drawRect(0, y, self.width, item.height, 0.2, 1, 1, 1, 1)
	elseif self.enabled and item.index == self.mouseoverselected then
...
function TagsList:setJoypadFocused(focused, joypadData)
	if focused then
...
function TagsList:onJoypadDown(button, joypadData)
...
function TagsList:onJoypadDownInParent(button, joypadData)
	if button == Joypad.AButton then
...
function WorkshopSubmitScreen:fillList()
	...
	self.page9:aboutToShow(JoypadState.getMainMenuJoypad())
end
...
function WorkshopSubmitScreen:onGainJoypadFocus(joypadData)
	ISPanelJoypad.onGainJoypadFocus(self, joypadData)
	joypadData.focus = self.page9
	updateJoypadFocus(joypadData)
end
