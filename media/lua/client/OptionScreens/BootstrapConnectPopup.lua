...
function BootstrapConnectPopup:connect(host, port, serverPassword)
	...
	if getSteamModeActive() then
		if isValidSteamID(host) then
			...
			ConnectToServer.instance:connectCoop(MainScreen.instance.bottomPanel, host)
			return
		end
		...
		ServerConnectPopup.instance:setVisible(true, joypadData)
	end
end
...
function BootstrapConnectPopup:onGainJoypadFocus(joypadData)
