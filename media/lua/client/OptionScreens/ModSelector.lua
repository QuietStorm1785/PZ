...
function ModListBox:doDrawItem(y, item, alt)
	...
	local isJoypadSelected = self.joyfocus and (self.selected == item.index)
	...
	if self.joyfocus and modInfo:isAvailable() and (self.selected == item.index) then
...
function ModInfoPanel:onGainJoypadFocus(joypadData)
...
function ModSelector:onDblClickMap(item)
	if not self.modorderui or not self.modorderui:isVisible() then
		...
		if not joypadData or wasMouseActiveMoreRecentlyThanJoypad() then
