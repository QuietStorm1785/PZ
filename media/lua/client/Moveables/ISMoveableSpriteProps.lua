...
function ISMoveableSpriteProps:placeMoveableInternal( _square, _item, _spriteName )
    ...
    elseif _spriteName == "appliances_misc_01_0" then
...
function ISMoveableSpriteProps:addScrapItemToList( _list, _item, _max, _chance, _skillMod, _doSizeMod )
    ...
    if _item and _max and _chance then
		...
		if _item == "Base.Nails" and self.object:isFloor() and amount > 1 then
			amount = 1
		end
        if amount > 0 then
            for i = 1, amount do
            ...
            if tools then
                ...
                for _,v in ipairs(tools) do
                    if v == "Base.BlowTorch" then
                        local item = inventory:getFirstTypeEvalRecurse(v, function(it) return it:getUsedDelta() >= 0.1 end)
                        if item then
                            return item
                        end
                    else
