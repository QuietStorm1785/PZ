...
ISHealthPanel.onCheatOtherPlayer = function(bodyPart, action, player, otherPlayer)
    local args = { bodyPartIndex = BodyPartType.ToIndex(bodyPart:getType()), action = action, id = player:getOnlineID() }
    sendClientCommand(otherPlayer, "player", "onHealthCheat", args)
end
...
function ISHealthPanel:update()
    ...
    if self.otherPlayer then
		...
		if (math.abs(self.otherPlayer:getX() - self.character:getX()) > 2 or
            math.abs(self.otherPlayer:getY() - self.character:getY()) > 2) and not ISHealthPanel.cheat then
...
function ISHealthBodyPartListBox:doDrawItem(y, item, alt)
    ...
    if item.itemindex == self.selected then
        self:drawRect(0, y, self:getWidth(), item.height, 0.2, 1.0, 1.0, 1.0);
