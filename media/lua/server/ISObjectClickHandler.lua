...
ISObjectClickHandler.doRClick = function (object, x, y)
    ...
    local hoppable = IsoObjectPicker.Instance:PickHoppable(x, y)
    if hoppable ~= nil then
        table.insert(objects, hoppable)
    end
...
ISObjectClickHandler.onObjectLeftMouseButtonDown = function(object, x, y)
    ...
    local buildingObj = getCell():getDrag(playerNum)
    if buildingObj and buildingObj.onObjectLeftMouseButtonDown and buildingObj.onObjectLeftMouseButtonDown(object, x, y) then
        return
    end
