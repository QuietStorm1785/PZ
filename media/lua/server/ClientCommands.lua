...
Commands.clothingWasher.toggle = function(player, args)
	...
	if not object then return end
	object:setActivated(not object:isActivated())
	object:sendObjectChange("washer.state")
end

-- -- -- -- --

local function findComboWasherDryer(x, y, z)
	local gs = getCell():getGridSquare(x, y, z)
	if not gs then return nil end
	for i=1,gs:getObjects():size() do
		local obj = gs:getObjects():get(i-1)
		if obj and instanceof(obj, 'IsoCombinationWasherDryer') then
			return obj
		end
	end
	noise('no clothing washer at '..x..','..y..','..z)
...
Commands.comboWasherDryer = {}

Commands.comboWasherDryer.toggle = function(player, args)
	local object = findComboWasherDryer(args.x, args.y, args.z)
	if not object then return end
	object:setActivated(not object:isActivated())
	object:sendObjectChange(object:isModeWasher() and "washer.state" or "dryer.state")
end

Commands.comboWasherDryer.setMode = function(player, args)
	local object = findComboWasherDryer(args.x, args.y, args.z)
	if not object then return end
	if args.mode == "washer" then
		object:setModeWasher()
	else
		object:setModeDryer()
	end
	object:sendObjectChange("mode")
end
...
Commands.player.onHealthCheat = function(player, args)
	local otherPlayer = getPlayerByOnlineID(args.id)
	if otherPlayer and player:isAccessLevel("admin") then
