...
function Vehicles.Create.Radio_HAM(vehicle, part)
	local deviceData = part:createSignalDevice()
	local invItem = VehicleUtils.createPartInventoryItem_HAMRadio(part)
	if deviceData and invItem then -- safety in case mods interfere with this function
		deviceData:setDeviceName(invItem:getDeviceData():getDeviceName())
		deviceData:setIsTwoWay(invItem:getDeviceData():getIsTwoWay())
		deviceData:setTransmitRange(invItem:getDeviceData():getTransmitRange())
...
function Vehicles.InstallComplete.Radio (vehicle, part) -- ,player)
	...
	-- Disabled for future work
	-- if media and deviceData then
		-- deviceData:addMediaItem(media)
	-- end
	vehicle:transmitPartItem(part);
end

	-- Disabled for future work
function Vehicles.UninstallComplete.Radio (vehicle, part, item) -- ,player)
	-- local media
	-- if part and part:getDeviceData()  then
		-- media = part:getDeviceData():removeMediaItem(player:getInventory())
	-- end
	-- if media and item:getDeviceData() then
		-- item:getDeviceData():addMediaItem(media)
	-- end
end
...
function Vehicles.Update.Radio(vehicle, part, elapsedMinutes)
	...
	-- kludge so pre-existing radios are updated with new radio funtionality
	if part and part:getInventoryItem() and deviceData then 
		local invItem = VehicleUtils.createPartInventoryItem(part)
		if invItem then 
			deviceData:setDeviceName(invItem:getDeviceData():getDeviceName())
			deviceData:setIsTwoWay(invItem:getDeviceData():getIsTwoWay())
...
function VehicleUtils.createPartInventoryItem_Radio(part)
	if not part:getItemType() or part:getItemType():isEmpty() then return nil end
	local item;
	if not part:getInventoryItem() then
		local v = part:getVehicle();
...
function VehicleUtils.createPartInventoryItem_HAMRadio(part)
	if not part:getItemType() or part:getItemType():isEmpty() then return nil end
	local item;
	if not part:getInventoryItem() then
		local v = part:getVehicle();
