...
#include <cstdint>
#include "gnu/trove/decorator/TIntSetDecorator/1.h"
...
class TIntSetDecorator : public AbstractSet {
public:
    ...
    int i = that.size();
            ...
            while (i-- > 0) {
    void* val = it.next();
               if (!(dynamic_cast<int*>(val) != nullptr)) {
    return false;
               }
    ...
    bool remove(void* value) {
      return dynamic_cast<int*>(value) != nullptr && this._set.remove((int)value);
   }
   ...
   public Iterator<int> iterator() {
    ...
    bool contains(void* o) {
      return !(dynamic_cast<int*>(o) != nullptr) ? false : this._set.contains((int)o);
   }
