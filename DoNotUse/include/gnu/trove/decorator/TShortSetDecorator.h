...
#include <cstdint>
#include "gnu/trove/decorator/TShortSetDecorator/1.h"
...
class TShortSetDecorator : public AbstractSet {
public:
    ...
    int i = that.size();
            ...
            while (i-- > 0) {
    void* val = it.next();
               if (!(dynamic_cast<int16_t*>(val) != nullptr)) {
    return false;
               }
    ...
    bool remove(void* value) {
      return dynamic_cast<int16_t*>(value) != nullptr && this._set.remove((int16_t)value);
   }

   public Iterator<int16_t> iterator() {
    ...
    bool contains(void* o) {
      return !(dynamic_cast<int16_t*>(o) != nullptr) ? false : this._set.contains((int16_t)o);
   }
