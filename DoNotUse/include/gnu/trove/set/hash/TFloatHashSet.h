...
#include "gnu/trove/set/hash/TFloatHashSet/TFloatHashIterator.h"
#include <algorithm>
#include <fstream>
#include <iostream>
...
class TFloatHashSet : public TFloatHash {
public:
    ...
    public TFloatHashSet(Collection<? extends) {
    ...
    public TFloatHashSet(TFloatCollection collection) {
      this(Math.max(collection.size(), 10));
      if (dynamic_cast<TFloatHashSet*>(collection) != nullptr) {
    ...
    TFloatIterator iterator() {
      return std::make_shared<TFloatHashIterator>(this, this);
   }
