...
#include "gnu/trove/impl/hash/TByteFloatHash.h"
#include "gnu/trove/iterator/TByteFloatIterator.h"
#include "gnu/trove/map/TByteFloatMap.h"
#include "gnu/trove/map/hash/TByteFloatHashMap/1.h"
#include "gnu/trove/map/hash/TByteFloatHashMap/TByteFloatHashIterator.h"
#include "gnu/trove/map/hash/TByteFloatHashMap/TKeyView.h"
#include "gnu/trove/map/hash/TByteFloatHashMap/TValueView.h"
...
class TByteFloatHashMap : public TByteFloatHash {
public:
   ...
   protected transient float[] _values;
    ...
    public TByteFloatHashMap(int initialCapacity, float loadFactor, uint8_t noEntryKey, float noEntryValue) {
    ...
    public TByteFloatHashMap(TByteFloatMap map) {
      super(map.size());
      if (dynamic_cast<TByteFloatHashMap*>(map) != nullptr) {
    ...
    float doPut(uint8_t key, float value, int index) {
    ...
    void putAll(Map<? extends, ? extends) {
      ...
      for (Entry<? : public uint8_t, ? : public float> entry : map.entrySet()) {
   ...
   public float[] values(float[] array) {
    ...
    TByteFloatIterator iterator() {
      return std::make_shared<TByteFloatHashIterator>(this, this);
   }
    ...
    bool equals(void* other) {
      if (!(dynamic_cast<TByteFloatMap*>(other) != nullptr)) {
    return false;
      } else {
