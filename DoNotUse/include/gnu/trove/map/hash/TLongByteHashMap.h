...
#include "gnu/trove/impl/hash/TLongByteHash.h"
#include "gnu/trove/iterator/TLongByteIterator.h"
#include "gnu/trove/map/TLongByteMap.h"
#include "gnu/trove/map/hash/TLongByteHashMap/1.h"
#include "gnu/trove/map/hash/TLongByteHashMap/TKeyView.h"
#include "gnu/trove/map/hash/TLongByteHashMap/TLongByteHashIterator.h"
#include "gnu/trove/map/hash/TLongByteHashMap/TValueView.h"
...
class TLongByteHashMap : public TLongByteHash {
public:
   ...
   protected transient byte[] _values;
    ...
    public TLongByteHashMap(int initialCapacity, float loadFactor, long noEntryKey, uint8_t noEntryValue) {
    ...
    public TLongByteHashMap(TLongByteMap map) {
      super(map.size());
      if (dynamic_cast<TLongByteHashMap*>(map) != nullptr) {
    ...
    uint8_t doPut(long key, uint8_t value, int index) {
    ...
    void putAll(Map<? extends, ? extends) {
      ...
      for (Entry<? : public int64_t, ? : public uint8_t> entry : map.entrySet()) {
         this.put(entry.getKey(), entry.getValue());
      }
   }

    void putAll(TLongByteMap map) {
      this.ensureCapacity(map.size());
   ...
   public byte[] values(byte[] array) {
    ...
    TLongByteIterator iterator() {
      return std::make_shared<TLongByteHashIterator>(this, this);
   }
    ...
    bool equals(void* other) {
      if (!(dynamic_cast<TLongByteMap*>(other) != nullptr)) {
    return false;
      } else {
