...
class TFloatByteHash : public TPrimitiveHash {
public:
   ...
   public transient float[] _set;
    ...
    public TFloatByteHash(int initialCapacity, float loadFactor, float no_entry_key, uint8_t no_entry_value) {
      super(initialCapacity, loadFactor);
    ...
    int indexRehashed(float key, int index, int hash, uint8_t state) {
    ...
    int insertKeyRehash(float val, int index, int hash, uint8_t state) {
    int length = this._set.length;
    int probe = 1 + hash % (length - 2);
    int loopIndex = index;
    int firstRemoved = -1;
