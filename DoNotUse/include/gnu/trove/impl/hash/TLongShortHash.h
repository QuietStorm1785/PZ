...
class TLongShortHash : public TPrimitiveHash {
public:
   ...
   public transient long[] _set;
    ...
    public TLongShortHash(int initialCapacity, float loadFactor, long no_entry_key, short no_entry_value) {
      super(initialCapacity, loadFactor);
    ...
    int indexRehashed(long key, int index, int hash, uint8_t state) {
    ...
    int insertKeyRehash(long val, int index, int hash, uint8_t state) {
    int length = this._set.length;
    int probe = 1 + hash % (length - 2);
    int loopIndex = index;
    int firstRemoved = -1;
