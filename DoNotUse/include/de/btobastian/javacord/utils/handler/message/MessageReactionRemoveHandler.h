...
#include "de/btobastian/javacord/utils/handler/message/MessageReactionRemoveHandler/1.h"
...
class MessageReactionRemoveHandler : public PacketHandler {
public:
    static const Logger logger = LoggerUtil.getLogger(MessageReactionRemoveHandler.class);
    ...
    CustomEmoji customEmoji = message.getChannelReceiver().getServer().getCustomEmojiById(emojiId);
            ...
            reaction = ((ImplMessage)message).removeCustomEmojiReactionToCache(customEmoji, this.api.getYourself().getId() == userId));
         } else {
            reaction = ((ImplMessage)message).removeUnicodeReactionToCache(emoji.getString("name"), this.api.getYourself().getId() == userId));
         }
