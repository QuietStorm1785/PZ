#pragma once
#include <string>
#include <vector>
#include <memory>
#include <unordered_map>
#include <unordered_set>
#include <cstdint>
#include "zombie/core/SpriteRenderer.h"
#include "zombie/core/textures/Texture.h"

namespace zombie {
namespace ui {


class VehicleGauge : public UIElement {
public:
    int needleX;
    int needleY;
    float minAngle;
    float maxAngle;
    float value;
    Texture texture;
    int needleWidth = 45;

    public VehicleGauge(Texture var1, int var2, int var3, float var4, float var5) {
      this.texture = var1;
      this.needleX = var2;
      this.needleY = var3;
      this.minAngle = var4;
      this.maxAngle = var5;
      this.width = var1.getWidth();
      this.height = var1.getHeight();
   }

    void setNeedleWidth(int var1) {
      this.needleWidth = var1;
   }

    void render() {
      if (this.isVisible()) {
         super.render();
         this.DrawTexture(this.texture, 0.0, 0.0, 1.0);
         double var1 = this.minAngle < this.maxAngle
            ? Math.toRadians(this.minAngle + (this.maxAngle - this.minAngle) * this.value)
            : Math.toRadians(this.maxAngle + (this.maxAngle - this.minAngle) * (1.0F - this.value));
    double var3 = this.needleX;
    double var5 = this.needleY;
    double var7 = this.needleX + this.needleWidth * Math.cos(var1);
    double var9 = Math.ceil(this.needleY + this.needleWidth * Math.sin(var1));
    int var11 = this.getAbsoluteX().intValue();
    int var12 = this.getAbsoluteY().intValue();
         SpriteRenderer.instance.renderline(nullptr, var11 + (int)var3, var12 + (int)var5, var11 + (int)var7, var12 + (int)var9, 1.0F, 0.0F, 0.0F, 1.0F);
      }
   }

    void setValue(float var1) {
      this.value = Math.min(var1, 1.0F);
   }

    void setTexture(Texture var1) {
      this.texture = var1;
   }
}
} // namespace ui
} // namespace zombie
