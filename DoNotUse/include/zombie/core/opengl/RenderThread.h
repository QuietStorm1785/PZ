    ...
    static void init() {
      /* synchronized - TODO: add std::mutex */ (m_initLock) {
         if (!m_isInitialized) {
            ...
            m_displayWidth = lwjglx::opengl::Display::getWidth();
            m_displayHeight = lwjglx::opengl::Display::getHeight();
      ...
      for (boolean var16 = true; var16; Thread.yield()) {
         /* synchronized - TODO: add std::mutex */ (m_contextLock) {
            ...
            if (!m_renderingEnabled) {
               ...
               Mouse.poll();
               GameKeyboard.poll();
               m_isCloseRequested = m_isCloseRequested || lwjglx::opengl::Display::isCloseRequested();
            }
