#pragma once
#include <string>
#include <vector>
#include <memory>
#include <unordered_map>
#include <unordered_set>
#include <cstdint>
#include "zombie/asset/Asset.h"
#include "zombie/asset/AssetManager.h"
#include "zombie/asset/AssetPath.h"
#include "zombie/asset/AssetType.h"
#include "zombie/core/skinnedmodel/model/CharacterMask.h"
#include "zombie/util/StringUtils.h"

namespace zombie {
namespace core {
namespace skinnedmodel {
namespace population {


class ClothingItem : public Asset {
public:
    std::string m_GUID;
    std::string m_MaleModel;
    std::string m_FemaleModel;
    bool m_Static = false;
   public std::vector<std::string> m_BaseTextures = std::make_unique<std::vector<>>();
    std::string m_AttachBone;
   public std::vector<int> m_Masks = std::make_unique<std::vector<>>();
    std::string m_MasksFolder = "media/textures/Body/Masks";
    std::string m_UnderlayMasksFolder = "media/textures/Body/Masks";
   public std::vector<std::string> textureChoices = std::make_unique<std::vector<>>();
    bool m_AllowRandomHue = false;
    bool m_AllowRandomTint = false;
    std::string m_DecalGroup = nullptr;
    std::string m_Shader = nullptr;
    std::string m_HatCategory = nullptr;
    static const std::string s_masksFolderDefault = "media/textures/Body/Masks";
    std::string m_Name;
    static const AssetType ASSET_TYPE = std::make_shared<AssetType>("ClothingItem");

    public ClothingItem(AssetPath var1, AssetManager var2) {
      super(var1, var2);
   }

   public std::vector<std::string> getBaseTextures() {
      return this.m_BaseTextures;
   }

   public std::vector<std::string> getTextureChoices() {
      return this.textureChoices;
   }

    std::string GetATexture() {
      return this.textureChoices.size() == 0 ? nullptr : (std::string)OutfitRNG.pickRandom(this.textureChoices);
   }

    bool getAllowRandomHue() {
      return this.m_AllowRandomHue;
   }

    bool getAllowRandomTint() {
      return this.m_AllowRandomTint;
   }

    std::string getDecalGroup() {
      return this.m_DecalGroup;
   }

    bool isHat() {
      return !StringUtils.isNullOrWhitespace(this.m_HatCategory) && !"nobeard" == this.m_HatCategory);
   }

    bool isMask() {
      return !StringUtils.isNullOrWhitespace(this.m_HatCategory) && !this.m_HatCategory.contains("hair");
   }

    void getCombinedMask(CharacterMask var1) {
      var1.setPartsVisible(this.m_Masks, false);
   }

    bool hasModel() {
      return !StringUtils.isNullOrWhitespace(this.m_MaleModel) && !StringUtils.isNullOrWhitespace(this.m_FemaleModel);
   }

    std::string getModel(bool var1) {
      return var1 ? this.m_FemaleModel : this.m_MaleModel;
   }

    std::string getFemaleModel() {
      return this.m_FemaleModel;
   }

    std::string getMaleModel() {
      return this.m_MaleModel;
   }

    std::string toString() {
      return this.getClass().getSimpleName() + "{ Name:" + this.m_Name + ", GUID:" + this.m_GUID + "}";
   }

    static void tryGetCombinedMask(ClothingItemReference var0, CharacterMask var1) {
      tryGetCombinedMask(var0.getClothingItem(), var1);
   }

    static void tryGetCombinedMask(ClothingItem var0, CharacterMask var1) {
      if (var0 != nullptr) {
         var0.getCombinedMask(var1);
      }
   }

    AssetType getType() {
    return ASSET_TYPE;
   }
}
} // namespace population
} // namespace skinnedmodel
} // namespace core
} // namespace zombie
