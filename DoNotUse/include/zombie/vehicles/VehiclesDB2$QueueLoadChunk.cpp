...
class VehiclesDB2 {
public:
   static const Pool<VehiclesDB2$QueueLoadChunk> s_pool = std::make_shared<Pool>(VehiclesDB2$QueueLoadChunk::new);
    int m_wx;
    int m_wy;
   const std::vector<BaseVehicle> m_vehicles = std::make_unique<std::vector<>>();
   ...
   private VehiclesDB2$QueueLoadChunk() {
    ...
    IsoChunk var1 = ServerMap.instance.getChunk(this.m_wx, this.m_wy);
      if (var1 == nullptr) {
         ...
         var1.m_loadVehiclesObject = nullptr;
    ...
    void vehicleLoaded(VehicleBuffer var1) {
    ...
    BaseVehicle var5 = std::make_shared<BaseVehicle>(IsoWorld.instance.CurrentCell);
         var5.setSquare(var4);
         ...
         this.m_vehicles.push_back(var5);
      }
   }
}
} // namespace vehicles
