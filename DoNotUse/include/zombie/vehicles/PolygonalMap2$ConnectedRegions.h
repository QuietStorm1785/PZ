...
public:
    PolygonalMap2 map;
   std::unordered_set<Chunk> doneChunks = std::make_unique<std::unordered_set<>>();
    int minX;
    int minY;
    int maxX;
    int maxY;
   ...
   private PolygonalMap2$ConnectedRegions() {
      this.visited = std::make_shared<BooleanGrid>(this.WIDTH, this.WIDTH);
   }

    void findAdjacentChunks(int var1, int var2) {
      this.doneChunks.clear();
      this.minX = this.minY = int.MAX_VALUE;
      this.maxX = this.maxY = int.MIN_VALUE;
    Chunk var3 = this.map.getChunkFromSquarePos(var1, var2);
      this.findAdjacentChunks(var3);
   }

    void findAdjacentChunks(Chunk var1) {
      if (var1 != nullptr && !this.doneChunks.contains(var1)) {
         ...
         this.doneChunks.push_back(var1);
    ...
    int var4 = this.MINX + (var3 & 65535);
    int var5 = this.MINY + (var3 >> 16) & 65535;

            while (this.shouldVisit(var4, var5, var4, var5 - 1)) {
               var5--;
            }
    ...
    return false;
      } else if (var4 < this.MINY + this.WIDTH && var4 >= this.MINY) {
    ...
    Square var6 = PolygonalMap2.instance.getSquare(var3, var4, 0);
            return var5 != nullptr && var6 != nullptr ? !this.isBlocked(var5, var6, false) : false;
         }
      } else {
    return false;
      }
    ...
    bool isBlocked(Square var1, Square var2, bool var3) {
    ...
    bool var8 = var6 && var1.has(4) && (var1.x != var2.x || var3 || !var1.has(16384));
    bool var9 = var4 && var1.has(2) && (var1.y != var2.y || var3 || !var1.has(8192));
    bool var10 = var7 && var2.has(4) && (var1.x != var2.x || var3 || !var2.has(16384));
    bool var11 = var5 && var2.has(2) && (var1.y != var2.y || var3 || !var2.has(8192));
            if (!var8 && !var9 && !var10 && !var11) {
    bool var12 = var2.x != var1.x && var2.y != var1.y;
               if (var12) {
    ...
    Square var14 = PolygonalMap2.instance.getSquare(var2.x, var1.y, var1.z);
                  if (!$assertionsDisabled && (var13 == var1 || var13 == var2)) {
                     throw std::make_unique<AssertionError>();
                  }

                  if (!$assertionsDisabled && (var14 == var1 || var14 == var2)) {
                     throw std::make_unique<AssertionError>();
                  }

                  if (var2.x == var1.x + 1 && var2.y == var1.y + 1 && var13 != nullptr && var14 != nullptr && var13.has(4096) && var14.has(2048)) {
    return true;
                  }

                  if (var2.x == var1.x - 1 && var2.y == var1.y - 1 && var13 != nullptr && var14 != nullptr && var13.has(2048) && var14.has(4096)) {
    return true;
                  }

                  if (var13 != nullptr && this.isBlocked(var1, var13, true)) {
    return true;
                  }

                  if (var14 != nullptr && this.isBlocked(var1, var14, true)) {
    return true;
                  }

                  if (var13 != nullptr && this.isBlocked(var2, var13, true)) {
    return true;
                  }

                  if (var14 != nullptr && this.isBlocked(var2, var14, true)) {
    return true;
                  }
               }
