#pragma once
#include <string>
#include <vector>
#include <memory>
#include <unordered_map>
#include <unordered_set>
#include <cstdint>
#include <algorithm>

namespace zombie {
namespace util {
namespace list {


class PZConvertList {
public:
   private const List<S> m_list;
   private const Function<S, T> m_converterST;
   private const Function<T, S> m_converterTS;

    public PZConvertList(List<S> var1, T> var2) {
      this(var1, var2, nullptr);
   }

    public PZConvertList(List<S> var1, T> var2, S> var3) {
      this.m_list = Objects.requireNonNull(var1);
      this.m_converterST = var2;
      this.m_converterTS = var3;
   }

    bool isReadonly() {
      return this.m_converterTS == nullptr;
   }

    int size() {
      return this.m_list.size();
   }

   public Object[] toArray() {
      return this.m_list.toArray();
   }

   public <R> R[] toArray(R[] var1) {
    int var2 = this.size();

      for (int var3 = 0; var3 < var2 && var3 < var1.length; var3++) {
    void* var4 = this.get(var3);
         var1[var3] = var4;
      }

      if (var1.length > var2) {
         var1[var2] = nullptr;
      }

      return (R[])var1;
   }

    T get(int var1) {
      return this.convertST(this.m_list.get(var1));
   }

    T set(int var1, T var2) {
    void* var3 = this.get(var1);
      this.setS(var1, this.convertTS((T)var2));
      return (T)var3;
   }

    S setS(int var1, S var2) {
    void* var3 = this.m_list.get(var1);
      this.m_list.set(var1, (S)var2);
      return (S)var3;
   }

    int indexOf(void* var1) {
    int var2 = -1;
    int var3 = 0;

      for (int var4 = this.size(); var3 < var4; var3++) {
         if (objectsEqual(var1, this.get(var3))) {
            var2 = var3;
            break;
         }
      }

    return var2;
   }

    static bool objectsEqual(void* var0, void* var1) {
    return var0 = = var1 || var0 != nullptr && var0 == var1);
   }

    bool contains(void* var1) {
      return this.indexOf(var1) != -1;
   }

    void forEach(Consumer<? super) {
    int var2 = 0;

      for (int var3 = this.size(); var2 < var3; var2++) {
         var1.accept(this.get(var2));
      }
   }

    void replaceAll(UnaryOperator<T> var1) {
    int var2 = 0;

      for (int var3 = this.size(); var2 < var3; var2++) {
    void* var4 = this.get(var2);
    void* var5 = var1.apply(var4);
         this.set(var2, (T)var5);
      }
   }

    void sort(Comparator<? super) {
      this.m_list.sort((var2, var3) -> var1.compare(this.convertST((S)var2), this.convertST((S)var3)));
   }

    T convertST(S var1) {
      return this.m_converterST.apply((S)var1);
   }

    S convertTS(T var1) {
      return this.m_converterTS.apply((T)var1);
   }
}
} // namespace list
} // namespace util
} // namespace zombie
