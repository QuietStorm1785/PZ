...
public:
    long m_startTime;
    const Row m_rootRow = std::make_shared<Row>();
   private const std::unordered_map<std::string, int> m_keyValueTable = std::make_unique<std::unordered_map<>>();
    PrintStream m_outSegment = nullptr;
    long m_firstFrameNo = -1L;
   private const List<std::string> m_segmentFilePaths = std::make_unique<std::vector<>>();
    int m_numFramesPerFile = 60;
    int m_currentSegmentFrameCount = 0;

    public GameProfileRecording(const std::string& var1) {
      super(var1, "_times");
    ...
    std::string var2 = std::string.format("%s%s_%04d", this.m_fileKey, this.m_valuesFileNameSuffix, this.m_segmentFilePaths.size());
    ...
    void writeDataRow(std::stringstream var1, Row var2) {
    int var3 = 0;
    ...
    void writeFrameTimeRow(std::stringstream var1, Row var2, int var3) {
      appendCell(var1, var2.StartTime / 100L);
      appendCell(var1, var2.EndTime / 100L);
      appendCell(var1, var3);
   }

    void writeSpan(std::stringstream var1, Row var2, Span var3) {
    ...
    std::stringstream var1 = new std::stringstream();
      ...
      for (Entry var3 : this.m_keyValueTable.entrySet()) {
         ...
         var1.append(",");
         var1.append((std::string)var3.getKey());
