#pragma once
#include <string>
#include <vector>
#include <memory>
#include <unordered_map>
#include <unordered_set>
#include <cstdint>
#include "zombie/scripting/ScriptParser/Block.h"
#include "zombie/scripting/ScriptParser/Value.h"
#include "zombie/util/StringUtils.h"

namespace zombie {
namespace sandbox {


class CustomEnumSandboxOption : public CustomSandboxOption {
public:
    const int numValues;
    const int defaultValue;
    std::string m_valueTranslation;

   CustomEnumSandboxOption(std::string var1, int var2, int var3) {
      super(var1);
      this.numValues = var2;
      this.defaultValue = var3;
   }

    static CustomEnumSandboxOption parse(Block var0) {
    int var1 = getValueInt(var0, "numValues", -1);
    int var2 = getValueInt(var0, "default", -1);
      if (var1 > 0 && var2 > 0) {
    CustomEnumSandboxOption var3 = std::make_shared<CustomEnumSandboxOption>(var0.id, var1, var2);
         if (!var3.parseCommon(var0)) {
    return nullptr;
         } else {
    Value var4 = var0.getValue("valueTranslation");
            if (var4 != nullptr) {
               var3.m_valueTranslation = StringUtils.discardNullOrWhitespace(var4.getValue().trim());
            }

    return var3;
         }
      } else {
    return nullptr;
      }
   }
}
} // namespace sandbox
} // namespace zombie
