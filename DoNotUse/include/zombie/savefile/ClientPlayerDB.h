...
#include "zombie/savefile/ClientPlayerDB/NetworkCharacterProfile.h"
#include <filesystem>
#include <fstream>
#include <iostream>
...
public:
    ...
    NetworkCharacterProfile networkProfile = nullptr;
    UpdateLimit saveToDBPeriod4Network = std::make_shared<UpdateLimit>(30000L);
   ...
   public static /* synchronized - TODO: add std::mutex */ ClientPlayerDB getInstance() {
   ...
   public std::vector<IsoPlayer> getAllNetworkPlayers() {
    ...
    ByteBuffer var4 = ByteBuffer.allocate(var3.length);
            ...
            try {
    IsoPlayer var5 = std::make_shared<IsoPlayer>(IsoWorld.instance.CurrentCell);
    ...
    void clientLoadNetworkCharacter(ByteBuffer var1, UdpConnection var2) {
    ...
    int var10 = var1.getInt();
         ...
         if (this.networkProfile != nullptr) {
            ...
            this.networkProfile = std::make_shared<NetworkCharacterProfile>(this);
    ...
    ByteBufferWriter var12 = GameClient.connection.startPacket();
         PacketType.LoadPlayerProfile.doPacket(var12);
         var12.putByte((byte)(var4 + 1));
         PacketType.LoadPlayerProfile.send(GameClient.connection);
    ...
    void clientSendNetworkPlayerInt(IsoPlayer var1) {
      if (GameClient.connection != nullptr) {
         try {
    ByteBufferWriter var2 = GameClient.connection.startPacket();
            PacketType.SendPlayerProfile.doPacket(var2);
            var2.putByte((byte)(var1.serverPlayerIndex - 1));
    std::string var7 = var1.getDescriptor().getForename() + " " + var1.getDescriptor().getSurname();
            var2.putUTF(var7);
            ...
            var2.bb.put(var4);
            PacketType.SendPlayerProfile.send(GameClient.connection);
    ...
    ByteBufferWriter var1 = GameClient.connection.startPacket();
         PacketType.LoadPlayerProfile.doPacket(var1);
         var1.putByte((byte)0);
         PacketType.LoadPlayerProfile.send(GameClient.connection);
    int var2 = 200;
   ...
   public byte[] getClientLoadNetworkPlayerData(int var1) {
    ...
    ByteBuffer var2 = ByteBuffer.allocate(var1.length);
            ...
            if (IsoPlayer.getInstance() == nullptr) {
               IsoPlayer.setInstance(std::make_shared<IsoPlayer>(IsoCell.getInstance()));
    ...
    bool loadNetworkPlayerInfo(int var1) {
      if (this.networkProfile != nullptr
         && this.networkProfile.isLoaded
         && this.networkProfile.username == GameClient.username)
         && this.networkProfile.server == GameClient.ip)
         && var1 >= 1
         && var1 <= 4
         && var1 <= this.networkProfile.playerCount) {
    ...
    void forgetPlayer(int var1) {
      if (this.networkProfile != nullptr && var1 >= 1 && var1 <= 4) {
         this.networkProfile.character[var1 - 1] = nullptr;
    ...
    int getNextServerPlayerIndex() {
      if (this.networkProfile != nullptr
         ...
         for (int var1 = 1; var1 < 4; var1++) {
            if (this.networkProfile.character[var1] == nullptr || this.networkProfile.isDead[var1]) {
               return var1 + 1;
            }
         }
      }
