...
public:
   static const std::vector<WorldMapFeature> s_tempFeatures = std::make_unique<std::vector<>>();
    const WorldMap m_worldMap = std::make_shared<WorldMap>();
    const WorldMapStyle m_style = std::make_shared<WorldMapStyle>();
    const WorldMapRenderer m_renderer = std::make_shared<WorldMapRenderer>();
    const WorldMapMarkers m_markers = std::make_shared<WorldMapMarkers>();
    WorldMapSymbols m_symbols = nullptr;
    const RGBAf m_color = std::make_shared<RGBAf>().init(0.85882354F, 0.84313726F, 0.7529412F, 1.0F);
    const UIWorldMapV1 m_APIv1 = std::make_shared<UIWorldMapV1>(this);
    bool m_dataWasReady = false;
   private const std::vector<BuildingDef> m_buildingsWithoutFeatures = std::make_unique<std::vector<>>();
    ...
    RoomRect var9 = (RoomRect)var7.get(var8);
            ...
            if (!var4) {
               this.m_buildingsWithoutFeatures.push_back(var3);
            }
         }
      }
   }
