...
public:
    Type m_type;
   public const std::vector<WorldMapPoints> m_points = std::make_unique<std::vector<>>();
    int m_minX;
    int m_minY;
    int m_maxX;
    int m_maxY;
   public float[] m_triangles = nullptr;
   public std::vector<TrianglesPerZoom> m_trianglesPerZoom = nullptr;
    int m_vboIndex1 = -1;
    int m_vboIndex2 = -1;
    int m_vboIndex3 = -1;
    int m_vboIndex4 = -1;
    static Clipper s_clipper = nullptr;
    static ByteBuffer s_vertices = nullptr;
    ...
    bool containsPoint(float var1, float var2) {
      return this.m_type == Type.Polygon && !this.m_points.empty() ? this.isPointInPolygon_WindingNumber(var1, var2, 0) != PolygonHit.Outside : false;
   }
    ...
    TrianglesPerZoom var37 = std::make_shared<TrianglesPerZoom>();
                  ...
                  if (this.m_trianglesPerZoom == nullptr) {
                     this.m_trianglesPerZoom = std::make_unique<std::vector<>>();
                  }

                  this.m_trianglesPerZoom.push_back(var37);
               }
            }
         }
      }
