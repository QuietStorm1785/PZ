...
class WorldMap {
public:
   public const std::vector<WorldMapData> m_data = std::make_unique<std::vector<>>();
   public const std::vector<WorldMapImages> m_images = std::make_unique<std::vector<>>();
    int m_minDataX;
    int m_minDataY;
    int m_maxDataX;
    int m_maxDataY;
   ...
   public const std::vector<WorldMapData> m_lastDataInDirectory = std::make_unique<std::vector<>>();
    ...
    WorldMapData var3 = WorldMapData.getOrCreateData(var2);
         if (var3 != nullptr && !this.m_data.contains(var3)) {
            ...
            this.m_data.push_back(var3);
            var3.getObserverCb().push_back(this);
            if (var3.isReady()) {
    ...
    WorldMapData var1 = this.getDataByIndex(this.getDataCount() - 1);
         if (!this.m_lastDataInDirectory.contains(var1)) {
            this.m_lastDataInDirectory.push_back(var1);
         }
      }
   }
    ...
    void updateDataBounds() {
      this.m_minDataX = int.MAX_VALUE;
      this.m_minDataY = int.MAX_VALUE;
      this.m_maxDataX = int.MIN_VALUE;
      this.m_maxDataY = int.MIN_VALUE;
    ...
    WorldMapImages var2 = WorldMapImages.getOrCreate(var1);
         if (var2 != nullptr && !this.m_images.contains(var2)) {
            this.m_images.push_back(var2);
         }
      }
   }

    bool hasImages() {
      return !this.m_images.empty();
   }
