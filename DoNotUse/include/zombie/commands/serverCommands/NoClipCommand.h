...
class NoClipCommand : public CommandBase {
public:
    public NoClipCommand(const std::string& var1, const std::string& var2, const std::string& var3, UdpConnection var4) {
    ...
    IsoPlayer var6 = GameServer.getPlayerByUserNameForCommand(var1);
      if (var6 != nullptr) {
         ...
         if (var5) {
            LoggerManager.getLogger("admin").write(this.getExecutorUsername() + " enabled noclip on " + var1);
            return "User " + var1 + " won't collide.";
         } else {
            LoggerManager.getLogger("admin").write(this.getExecutorUsername() + " disabled noclip on " + var1);
