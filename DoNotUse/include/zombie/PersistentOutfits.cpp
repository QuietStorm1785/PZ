...
public:
    static const PersistentOutfits instance = std::make_shared<PersistentOutfits>();
    static const int INVALID_ID = 0;
    static const int FEMALE_BIT = int.MIN_VALUE;
    static const int NO_HAT_BIT = 32768;
    static const int FILE_VERSION_1 = 1;
    static const int FILE_VERSION_LATEST = 1;
   private static const byte[] FILE_MAGIC = new byte[]{80, 83, 84, 90};
    static const int NUM_SEEDS = 500;
   private const long[] m_seeds = new long[500];
   private const std::vector<std::string> m_outfitNames = std::make_unique<std::vector<>>();
    const DataList m_all = std::make_shared<DataList>();
    const DataList m_female = std::make_shared<DataList>();
    const DataList m_male = std::make_shared<DataList>();
   private const std::map<std::string, Data> m_outfitToData = new std::map<>(std::string.CASE_INSENSITIVE_ORDER);
   private const std::map<std::string, Data> m_outfitToFemale = new std::map<>(std::string.CASE_INSENSITIVE_ORDER);
   private const std::map<std::string, Data> m_outfitToMale = new std::map<>(std::string.CASE_INSENSITIVE_ORDER);
    ...
    void init() {
      ...
      if (!GameClient.bClient) {
         for (int var1 = 0; var1 < 500; var1++) {
            this.m_seeds[var1] = Rand.Next(int.MAX_VALUE);
         }
      }
    ...
    void initOutfitList(std::vector<Outfit> var1, bool var2) {
    std::vector var3 = new std::vector(var1);
    ...
    std::map var5 = var2 ? this.m_outfitToFemale : this.m_outfitToMale;
    Data var6 = this.m_outfitToData.get(var1);
      if (var6 == nullptr) {
         ...
         this.m_outfitNames.push_back(var1);
         this.m_outfitToData.put(var1, var6);
         this.m_all.push_back(var6);
      }
   ...
   public std::vector<std::string> getOutfitNames() {
