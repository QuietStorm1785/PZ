...
public:
   private static const std::vector<ZomboidNetData> MainLoopNetData = std::make_unique<std::vector<>>();
    static const UdpEngine udpEngine = std::make_shared<UdpEngineClient>();
    static const UdpConnection connection = std::make_shared<UdpConnection>(udpEngine);
    ...
    ZomboidNetData var2;
      ...
      /* synchronized - TODO: add std::mutex */ (MainLoopNetData) {
         MainLoopNetData.push_back(var2);
      }
   }
