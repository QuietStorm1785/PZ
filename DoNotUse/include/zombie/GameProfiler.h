...
class GameProfiler {
public:
    static const std::string s_currentSessionUUID = UUID.randomUUID();
   private static const ThreadLocal<GameProfiler> s_instance = ThreadLocal.withInitial(GameProfiler::new);
   private const std::stack<ProfileArea> m_stack = std::make_unique<std::stack<>>();
    const RecordingFrame m_currentFrame = std::make_shared<RecordingFrame>();
    const RecordingFrame m_previousFrame = std::make_shared<RecordingFrame>();
    bool m_isInFrame;
    ...
    std::string var3 = std::string.format("%s_GameProfiler_%s", this.getCurrentSessionUUID(), var2);
      this.m_recorder = std::make_shared<GameProfileRecording>(var3);
   }
   ...
   public <T1> void invokeAndMeasure(std::string var1, T1 var2, ICallback<T1> var3) {
      if (!isRunning()) {
         var3.accept(var2);
      } else {
   ...
   public <T1, T2> void invokeAndMeasure(std::string var1, T1 var2, T2 var3, zombie.util.lambda.Invokers.Params2.ICallback<T1, T2> var4) {
      if (!isRunning()) {
   ...
   public <T1, T2, T3> void invokeAndMeasure(std::string var1, T1 var2, T2 var3, T3 var4, zombie.util.lambda.Invokers.Params3.ICallback<T1, T2, T3> var5) {
      if (!isRunning()) {
    ...
    void end(ProfileArea var1) {
      ...
      if (this.m_stack.peek() != var1) {
         throw RuntimeException("Incorrect exit. ProfileArea " + var1 + " is not at the top of the stack: " + this.m_stack.peek());
      } else {
         this.m_stack.pop();
         if (this.m_stack.empty()) {
    ...
    static void initTriggerWatcher() {
      if (m_gameProfilerRecordingTriggerWatcher == nullptr) {
         /* synchronized - TODO: add std::mutex */ (m_gameProfilerRecordingTriggerLock) {
            if (m_gameProfilerRecordingTriggerWatcher == nullptr) {
               m_gameProfilerRecordingTriggerWatcher = std::make_shared<PredicatedFileWatcher>(
               ...
               DebugFileWatcher.instance.push_back(m_gameProfilerRecordingTriggerWatcher);
            }
         }
      }
   }
