...
public:
    static const MapCollisionData instance = std::make_shared<MapCollisionData>();
   ...
   private const std::stack<PathTask> freePathTasks = std::make_unique<std::stack<>>();
   private const ConcurrentLinkedQueue<PathTask> pathTaskQueue = std::make_unique<ConcurrentLinkedQueue<>>();
   private const ConcurrentLinkedQueue<PathTask> pathResultQueue = std::make_unique<ConcurrentLinkedQueue<>>();
    const Sync sync = std::make_shared<Sync>();
   private const byte[] squares = new byte[100];
   ...
   private static native void n_setGameState(std::string var0, boolean var1);

   private static native void n_setGameState(std::string var0, double var1);

   private static native void n_setGameState(std::string var0, float var1);

   private static native void n_setGameState(std::string var0, int var1);

   private static native void n_setGameState(std::string var0, std::string var1);

   private static native void n_initMetaGrid(int var0, int var1, int var2, int var3);

   private static native void n_initMetaCell(int var0, int var1, std::string var2);
   ...
   private static native void n_initMetaChunk(int var0, int var1, int var2, int var3, int var4);
    ...
    IsoMetaCell var8 = var1.getCellData(var7, var6);
               n_initMetaCell(var7, var6, (std::string)IsoLot.InfoFileNames.get("chunkdata_" + var7 + "_" + var6 + ".bin"));
               if (var8 != nullptr) {
    ...
    void start() {
      if (!this.bClient) {
         if (this.thread == nullptr) {
            this.thread = std::make_shared<MCDThread>(this);
