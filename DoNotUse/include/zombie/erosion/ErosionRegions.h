...
public:
   ...
   public static const std::vector<Region> regions = std::make_unique<std::vector<>>();

    static void addRegion(Region var0) {
      var0.ID = regions.size();
      regions.push_back(var0);
   }
    ...
    std::make_shared<Region>();
      ...
      addRegion(std::make_shared<Region>(1, "blends_street", true, true, false).addCategory(0, std::make_unique<StreetCracks>()));
      addRegion(std::make_shared<Region>(2, nullptr, false, false, true).addCategory(0, std::make_unique<WallVines>()).addCategory(1, std::make_unique<WallCracks>()));
      addRegion(std::make_shared<Region>(3, nullptr, true, true, false).addCategory(0, std::make_unique<Flowerbed>()));
