...
public:
   ...
   private static const std::vector<InventoryItem> existingItems = std::make_unique<std::vector<>>();
   private static const std::vector<InventoryItem> newItems = std::make_unique<std::vector<>>();
    ...
    Zone var8 = var7 == nullptr ? nullptr : var7.getZone();
            if (var8 != nullptr
    ...
    IsoObject var13 = var11[var12];
                        if (!(dynamic_cast<IsoDeadBody*>(var13) != nullptr) && !(dynamic_cast<IsoThumpable*>(var13) != nullptr) && !(dynamic_cast<IsoCompost*>(var13) != nullptr)) {
