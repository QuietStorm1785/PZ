...
public:
   protected const Map<int16_t, ItemInfo> itemIdToInfoMap = std::make_unique<std::unordered_map<>>();
   protected const Map<std::string, ItemInfo> itemTypeToInfoMap = std::make_unique<std::unordered_map<>>();
   protected const Map<std::string, int> spriteNameToIdMap = std::make_unique<std::unordered_map<>>();
   protected const Map<int, std::string> spriteIdToNameMap = std::make_unique<std::unordered_map<>>();
   protected const Map<std::string, uint8_t> objectNameToIdMap = std::make_unique<std::unordered_map<>>();
   protected const Map<uint8_t, std::string> objectIdToNameMap = std::make_unique<std::unordered_map<>>();
   protected const std::vector<std::string> unsetObject = std::make_unique<std::vector<>>();
   protected const std::vector<std::string> unsetSprites = std::make_unique<std::vector<>>();
    short NextItemID = 0;
   ...
   protected byte[] serverDataCache;
    ...
    std::vector var4 = ((ItemInfo)var3.getValue()).modOverrides;
            ...
            for (int var5 = 0; var5 < var4.size(); var5++) {
               if (!var1.contains(var4.get(var5))) {
                  var1.push_back((std::string)var4.get(var5));
               }
            }
         }
      }
    ...
    void getModuleList(List<std::string> var1) {
      for (Entry var3 : this.itemIdToInfoMap.entrySet()) {
         if (!var1.contains(((ItemInfo)var3.getValue()).moduleName)) {
            var1.push_back(((ItemInfo)var3.getValue()).moduleName);
         }
      }
   }
    ...
    ItemInfo var5 = this.itemTypeToInfoMap.get(var4.fullType);
         if (var5 == nullptr) {
            if (!var4.obsolete) {
               ...
               WorldDictionaryLogger.log(std::make_shared<RegisterItem>(var4.copy()));
            }
         } else {
            if (var5.removed && !var4.obsolete) {
               var5.removed = false;
               WorldDictionaryLogger.log(std::make_shared<ReinstateItem>(var5.copy()));
            }
    ...
    std::string var6 = var5.modID;
               ...
               WorldDictionaryLogger.log(std::make_shared<ModIDChangedItem>(var5.copy(), var6, var5.modID));
            }

            if (var4.obsolete && (!var5.obsolete || !var5.removed)) {
               var5.obsolete = true;
               var5.removed = true;
               WorldDictionaryLogger.log(std::make_shared<ObsoleteItem>(var5.copy()));
            }
    ...
    ItemInfo var3 = (ItemInfo)var2.getValue();
         if (!var3.isLoaded) {
            var3.removed = true;
            WorldDictionaryLogger.log(std::make_shared<RemovedItem>(var3.copy(), false));
         }
         ...
         if (var3.scriptItem != nullptr) {
            ...
            var3.removed = true;
            WorldDictionaryLogger.log(std::make_shared<RemovedItem>(var3.copy(), true));
         }
      }
   }

    void parseObjectNameLoadList(List<std::string> var1) {
    ...
    std::string var3 = (std::string)var1.get(var2);
         if (!this.objectNameToIdMap.containsKey(var3)) {
            if (this.NextObjectNameID >= 127) {
               ...
               if (Core.bDebug) {
                  throw WorldDictionaryException("Max value for object names reached.");
               }
            } else {
    uint8_t var4 = this.NextObjectNameID++;
               this.objectIdToNameMap.put(var4, var3);
               this.objectNameToIdMap.put(var3, var4);
               WorldDictionaryLogger.log(std::make_shared<RegisterObject>(var3, var4));
            }
         }
      }
   }
    ...
    File var1 = std::make_shared<File>(ZomboidFileSystem.instance.getFileNameInCurrentSave("WorldDictionary.bin"));
         if (var1.exists()) {
    long var2 = Instant.now().getEpochSecond();
            this.dataBackupPath = std::make_shared<File>(ZomboidFileSystem.instance.getFileNameInCurrentSave("WorldDictionary_" + var2 + ".bak"));
    ...
    bool var5 = true;
            ...
            if (this.dataBackupPath != nullptr) {
    File var6 = std::make_shared<File>(var3 + "WorldDictionary_backup.bin");
               if (this.dataBackupPath.exists()) {
                  Files.copy(this.dataBackupPath, var6);
               }
            }

    File var13 = std::make_shared<File>(ZomboidFileSystem.instance.getFileNameInCurrentSave("WorldDictionaryLog.lua"));
    File var7 = std::make_shared<File>(var3 + "WorldDictionaryLog.lua");
            if (var13.exists()) {
               Files.copy(var13, var7);
            }

    File var8 = std::make_shared<File>(ZomboidFileSystem.instance.getFileNameInCurrentSave("WorldDictionaryReadable.lua"));
    File var9 = std::make_shared<File>(var3 + "WorldDictionaryReadable.lua");
            if (var8.exists()) {
               Files.copy(var8, var9);
            }

    File var10 = std::make_shared<File>(ZomboidFileSystem.instance.getFileNameInCurrentSave("WorldDictionary.bin"));
    File var11 = std::make_shared<File>(var3 + "WorldDictionary.bin");
            if (var10.exists()) {
    ...
    File var2 = std::make_shared<File>(var1);
         if (!var2.exists()) {
            if (!WorldDictionary.isIsNewGame()) {
               throw WorldDictionaryException("WorldDictionary data file is missing from world folder.");
            }
         } else {
            try {
    ...
    int var5 = var3.read(var4.array());
               ...
               throw WorldDictionaryException("Error loading WorldDictionary.", var8);
            }
         }
      }
   }
    ...
    int var3 = var1.getInt();
      ...
      for (int var4 = 0; var4 < var3; var4++) {
         var2.push_back(GameWindow.ReadString(var1));
      }
    ...
    int var5 = var1.getInt();
      ...
      for (int var6 = 0; var6 < var5; var6++) {
         var12.push_back(GameWindow.ReadString(var1));
      }

    int var13 = var1.getInt();

      for (int var7 = 0; var7 < var13; var7++) {
    ItemInfo var8 = std::make_shared<ItemInfo>();
    ...
    File var3 = std::make_shared<File>(ZomboidFileSystem.instance.getFileNameInCurrentSave("WorldDictionary.tmp"));
    FileOutputStream var4 = std::make_shared<FileOutputStream>(var3);
            var4.getChannel().truncate(0L);
            var4.write(var2.array(), 0, var2.limit());
            var4.flush();
            var4.close();
    File var5 = std::make_shared<File>(ZomboidFileSystem.instance.getFileNameInCurrentSave("WorldDictionary.bin"));
            Files.copy(var3, var5);
            var3.delete();
         } catch (Exception var6) {
            var6.printStackTrace();
            throw WorldDictionaryException("Error saving WorldDictionary.", var6);
         }
      }
   }
    ...
    ItemInfo var6 = (ItemInfo)var12.getValue();
      ...
      for (Entry var13 : this.objectIdToNameMap.entrySet()) {
         var1.put((uint8_t)var13.getKey());
         GameWindow.WriteString(var1, (std::string)var13.getValue());
      }

      var1.putInt(this.spriteIdToNameMap.size());

      for (Entry var14 : this.spriteIdToNameMap.entrySet()) {
         var1.putInt((int)var14.getKey());
         GameWindow.WriteString(var1, (std::string)var14.getValue());
      }
   }

    void saveAsText(const std::string& var1) {
      if (!Core.getInstance().isNoSave()) {
    File var2 = std::make_shared<File>(ZomboidFileSystem.instance.getCurrentSaveDir() + File.separator);
    ...
    File var4 = std::make_shared<File>(var3);
            ...
            try (FileWriter var5 = std::make_shared<FileWriter>(var4, false)) {
               ...
               for (Entry var13 : this.objectIdToNameMap.entrySet()) {
                  var5.write("\t" + var13.getKey() + " = \"" + (std::string)var13.getValue() + "\"," + System.lineSeparator());
               }
               ...
               for (Entry var14 : this.spriteIdToNameMap.entrySet()) {
                  var5.write("\t" + var14.getKey() + " = \"" + (std::string)var14.getValue() + "\"," + System.lineSeparator());
               }

               var5.write("}" + System.lineSeparator());
            } catch (Exception var10) {
               var10.printStackTrace();
               throw WorldDictionaryException("Error saving WorldDictionary as text.", var10);
            }
         }
      }
   }
