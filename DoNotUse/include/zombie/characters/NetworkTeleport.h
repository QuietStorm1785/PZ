...
#include "zombie/characters/NetworkTeleport/1.h"
...
public:
    ...
    public NetworkTeleport(IsoGameCharacter var1, Type var2, float var3, float var4, uint8_t var5, float var6) {
      this.character = var1;
      ...
      if (Core.bDebug && var1.getNetworkCharacterAI() != nullptr && DebugOptions.instance.MultiplayerShowTeleport.getValue()) {
         var1.getNetworkCharacterAI()
            .setTeleportDebug(
    std::make_shared<NetworkTeleportDebug>();
            );
      }
   }
   ...
   // Please report this to the Vineflower issue tracker, at https://github.com/Vineflower/vineflower/issues with a copy of the class file (if you have the rights to distribute it!)
    ...
    bool var2 = this.character.getCurrentSquare().isCanSee(var1);
   ...
   // Please report this to the Vineflower issue tracker, at https://github.com/Vineflower/vineflower/issues with a copy of the class file (if you have the rights to distribute it!)
    ...
    static bool teleport(IsoGameCharacter var0, Type var1, float var2, float var3, uint8_t var4, float var5) {
      if (!enable) {
    return false;
      } else {
         if (var0.getCurrentSquare() != nullptr && enableInstantTeleport) {
    bool var6 = false;
    ...
    std::make_shared<NetworkTeleport>();
    return true;
         } else {
    return false;
         }
    ...
    static bool teleport(IsoGameCharacter var0, PlayerPacket var1, float var2) {
      if (!enable) {
    return false;
      } else {
         if (GlobalObject.getCell().getGridSquare((int)var1.x, (int)var1.y, var1.z) == nullptr) {
            var0.setX(var1.x);
            var0.setY(var1.y);
            var0.setZ(var1.z);
    ...
    IsoGridSquare var10 = IsoWorld.instance.CurrentCell.getGridSquare(var1.realx, var1.realy, var1.realz);
               ...
               var0.ensureOnTile();
    float var12 = 0.5F;
               NetworkTeleport var6 = std::make_shared<NetworkTeleport>(
                  var0,
    ...
    float var9 = 0.5F;
               NetworkTeleport var11 = std::make_shared<NetworkTeleport>(
                  var0,
                  Type.teleportation,
    ...
    static void update(IsoGameCharacter var0, PlayerPacket var1) {
      if (var0.isTeleporting()) {
