...
#include "zombie/network/packets/RequestDataPacket/1.h"
#include "zombie/network/packets/RequestDataPacket/RequestID.h"
#include "zombie/network/packets/RequestDataPacket/RequestType.h"
...
public:
    ...
    void setRequest(RequestID var1) {
      this.type = RequestType.Request;
      this.id = var1;
   }

    void setPartData(RequestID var1, ByteBuffer var2) {
    ...
    void setPartDataParameters(int var1, int var2) {
      this.dataSent = var1;
      this.partSize = var2;
   }

    void setACK(RequestID var1) {
      this.type = RequestType.PartDataACK;
      this.id = var1;
   }

    void sendConnectingDetails(UdpConnection var1, LogonResult var2) {
    ...
    void processServer(PacketType var1, UdpConnection var2) {
    ...
    void doSendRequest(UdpConnection var1) {
    ...
    void doProcessRequest(UdpConnection var1) {
    ...
    for (auto& var7 : var2)               if (var7 != nullptr) {
      ...
      if (this.id == RequestID.MetaGrid) {
         try {
            ...
            GameServer.kick(var1, "You have been kicked from this server because map_meta.bin could not be saved.", nullptr);
      ...
      if (this.id == RequestID.MapZone) {
         try {
            ...
            GameServer.kick(var1, "You have been kicked from this server because map_zone.bin could not be saved.", nullptr);
    ...
    void doProcessPart(ByteBuffer var1) {
    ...
    void doProcessData(ByteBuffer var1) {
   ...
   // Please report this to the Vineflower issue tracker, at https://github.com/Vineflower/vineflower/issues with a copy of the class file (if you have the rights to distribute it!)
    void sendNextRequest(RequestID var1) {
    ...
    void saveToFile(ByteBuffer var1, const std::string& var2) {
    File var3 = std::make_shared<File>(ZomboidFileSystem.instance.getFileNameInCurrentSave(var2));

      try (
    FileOutputStream var4 = std::make_shared<FileOutputStream>(var3, false);
    ...
    void receivePlayerZombieDescriptors(ByteBuffer var1) {
    ...
    Descriptor var4 = std::make_shared<Descriptor>();
         var4.load(var1, 195);
    ...
    std::string var1 = "\n\tRequestDataPacket [";
      ...
      if (this.type == RequestType.FullData) {
         ...
         for (int var2 = 0; var2 < Math.min(15, this.buffer.limit()); var2++) {
            var1 = var1 + " 0x" + int.toHexString(this.buffer.get() & 255);
         }
      ...
      if (this.type == RequestType.PartData) {
         ...
         for (int var11 = 0; var11 < Math.min(15, this.buffer.limit() - this.buffer.position()); var11++) {
            var1 = var1 + " " + int.toHexString(this.buffer.get() & 255);
         }
