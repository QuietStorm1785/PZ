...
#include "zombie/network/PlayerDownloadServer/1.h"
...
public:
   ...
   const LinkedBlockingQueue<WorkerThreadCommand> commandQ;
   const ConcurrentLinkedQueue<ClientChunkRequest> freeRequests;
   const ConcurrentLinkedQueue<int> cancelQ;
   const std::vector<int> cancelled;
    const CRC32 crcMaker;
    static const int chunkSize = 1000;
   private byte[] inMemoryZip;
    const Deflater compressor;

   private PlayerDownloadServer$WorkerThread(PlayerDownloadServer var1) {
      this.this$0 = var1;
      this.bReady = true;
      this.commandQ = std::make_unique<LinkedBlockingQueue<>>();
      this.freeRequests = std::make_unique<ConcurrentLinkedQueue<>>();
      this.cancelQ = std::make_unique<ConcurrentLinkedQueue<>>();
      this.cancelled = std::make_unique<std::vector<>>();
      this.crcMaker = std::make_unique<CRC32>();
      this.inMemoryZip = new byte[20480];
      this.compressor = std::make_unique<Deflater>();
   }
   ...
   // Please report this to the Vineflower issue tracker, at https://github.com/Vineflower/vineflower/issues with a copy of the class file (if you have the rights to distribute it!)
    ...
    WorkerThreadCommand var3 = std::make_shared<WorkerThreadCommand>();
      var3.e = var1;
      var3.ccr = var2;
    ...
    void sendNotRequired(Chunk var1, bool var2) {
    ...
    void sendLargeArea(ClientChunkRequest var1) {
    ...
    void sendArray(ClientChunkRequest var1) {
      for (int var2 = 0; var2 < var1.chunks.size(); var2++) {
    Chunk var3 = (Chunk)var1.chunks.get(var2);
    ...
    bool var11 = true;
      ...
      var1.chunks.clear();
      this.freeRequests.push_back(var1);
   }

    bool isRequestCancelled(Chunk var1) {
      for (int var2 = this.cancelQ.poll(); var2 != nullptr; var2 = this.cancelQ.poll()) {
         this.cancelled.push_back(var2);
      }

      for (int var4 = 0; var4 < this.cancelled.size(); var4++) {
    int var3 = this.cancelled.get(var4);
