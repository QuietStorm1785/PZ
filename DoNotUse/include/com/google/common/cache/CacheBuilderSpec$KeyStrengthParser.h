...
public:
   ...
   public CacheBuilderSpec$KeyStrengthParser(Strength strength) {
    ...
    void parse(CacheBuilderSpec spec, const std::string& key, @Nullable std::string) {
      ...
      Preconditions.checkArgument(spec.keyStrength == nullptr, "%s was already set to %s", key, spec.keyStrength);
