...
public:
   ...
   static byte[] convbuffer = new byte[convsize];
    ...
    void* var1 = System.in;
      if (var0.length > 0) {
         try {
            var1 = std::make_shared<FileInputStream>(var0[0]);
         } catch (Exception var28) {
            System.err.println(var28);
         }
      }

    SyncState var2 = std::make_shared<SyncState>();
    StreamState var3 = std::make_shared<StreamState>();
    Page var4 = std::make_shared<Page>();
    Packet var5 = std::make_shared<Packet>();
    Info var6 = std::make_shared<Info>();
    Comment var7 = std::make_shared<Comment>();
    DspState var8 = std::make_shared<DspState>();
    Block var9 = std::make_shared<Block>(var8);
    int var11 = 0;
      var2.init();
    ...
    int var15 = var2.pageout(var4);
         ...
         for (int var16 = 0; var16 < var35.length && var35[var16] != nullptr; var16++) {
            System.err.println(new std::string(var35[var16], 0, var35[var16].length - 1));
         }

         System.err.println("\nBitstream is " + var6.channels + " channel, " + var6.rate + "Hz");
         System.err.println("Encoded by: " + new std::string(var7.vendor, 0, var7.vendor.length - 1) + "\n");
