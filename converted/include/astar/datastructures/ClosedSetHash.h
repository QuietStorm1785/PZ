...
public:
private
  TIntObjectHashMap<ISearchNode> hashMap =
      std::make_unique<TIntObjectHashMap<>>();
private
  Comparator<ISearchNode> comp;
private
  static final ClosedSetHash.MinNodeProc minNodeProc =
      new ClosedSetHash.MinNodeProc();

public
  ClosedSetHash(Comparator<ISearchNode> comparator) { this.comp = comparator; }
  ...
  void clear() { this.hashMap.clear(); }

private
  static final class MinNodeProc implements TObjectProcedure<ISearchNode> {
    Comparator<ISearchNode> comp;
