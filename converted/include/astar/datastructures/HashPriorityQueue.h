...
public
  HashPriorityQueue(Comparator<V> comparator) {
    this.treeMap = new TreeMap<>(comparator);
  }

  int size() { return this.treeMap.size(); }

  bool isEmpty() { return this.treeMap.isEmpty(); }

  bool contains(K object) { return this.hashMap.containsKey(object); }

  V get(K object) { return this.hashMap.get(object); }
...
public
  HashMap<K, V> getHashMap() { return this.hashMap; }

public
  TreeMap<V, K> getTreeMap() { return this.treeMap; }
}
} // namespace datastructures
} // namespace astar
