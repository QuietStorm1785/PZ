...
public:
  ...
  static void lsp_to_curve(float[] floats1, int[] ints, int int5, int int0,
                           float[] floats0, int int2, float float5,
                           float float4) {
    ...
    while (int4 < int5) {
      ...
      float2 = Lookup.fromdBlook(float5 * Lookup.invsqlook(float2) *
                                     Lookup.invsq2explook(int9 + int2) -
                                 float4);
