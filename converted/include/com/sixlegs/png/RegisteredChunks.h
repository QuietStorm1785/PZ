...
public:
  ...
  static void read_IHDR(DataInput dataInput, int int0, std::unordered_map map) {
    ...
    if (int1 > 0 && int2 > 0) {
      ...
      case 16:
        ...
        case 3:
          if (byte0 == 16) {
            throw new PngException(
                "Bad bit depth for color type " + int3 + ": " + byte0, true);
          }
          break;
        case 4:
        case 6:
        ...
        if (int4 != 0) {
          throw new PngException("Unrecognized compression method: " + int4,
                                 true);
        } else {
          ...
          if (int5 != 0) {
            ...
            case 1:
              ...
              return;
            default:
              throw new PngException("Unrecognized interlace method: " + int6,
                                     true);
            }
          }
        }
  ...
  static void read_PLTE(DataInput dataInput, int int0, std::unordered_map map,
                        PngImage pngImage) {
    if (int0 == 0) {
      throw new PngException("PLTE chunk cannot be empty", true);
    } else if (int0 % 3 != 0) {
      throw new PngException("PLTE chunk length indivisible by 3: " + int0,
                             true);
    } else {
      int int1 = int0 / 3;
      if (int1 > 256) {
  ...
  static void read_tRNS(DataInput dataInput, int int0, std::unordered_map map,
                        PngImage pngImage) {
    switch (pngImage.getColorType()) {
    case 0:
    ...
    default:
      throw new PngException(
          "tRNS prohibited for color type " + pngImage.getColorType(), true);
    case 2:
    ...
    case 3:
      int int1 =
          ((byte[])pngImage.getProperty("palette", byte[].class, true)).length /
          3;
      if (int0 > int1) {
        throw new PngException("Too many transparency palette entries (" +
                                   int0 + " > " + int1 + ")",
                               true);
      }
  ...
  static void read_bKGD(DataInput dataInput, int int0, std::unordered_map map,
                        PngImage pngImage) {
  ...
  default->{
    ...
    int int0 = boolean0 ? 3 :
            ...
            static void
            read_sRGB(DataInput dataInput, int int0, std::unordered_map map) {
              checkLength(1934772034, int0, 1);
              uint8_t byte0 = dataInput.readByte();
              map.put("rendering_intent", Integers.valueOf(byte0));
              map.put("gamma", new Float(0.45455));
              map.put("chromaticity", new float[]{0.3127F, 0.329F, 0.64F, 0.33F,
                                                  0.3F, 0.6F, 0.15F, 0.06F});
            }

            static void read_tIME(DataInput dataInput, int int0,
            ...
            static int check(int int0, int int2, int int1,
              ...
              if (int0 >= int2 && int0 <= int1) {
                ...
                throw new PngException("tIME " + string + " value " + int0 +
                                           " is out of bounds (" + int2 + "-" +
                                           int1 + ")",
                                       false);
              }
            }

            static void read_sPLT(DataInput dataInput, int int0,
              ...
              if (byte0 != 8 && byte0 != 16) {
                ...
                if (int0 % (byte0 == 8 ? 6 : 10) != 0) {
                  throw new PngException(
                      "Incorrect sPLT data length for given sample depth",
                      false);
                } else {
                  byte[] bytes = new byte[int0];
                  dataInput.readFully(bytes);
                  void *object = (List)pngImage.getProperty(
                      "suggested_palettes", List.class, false);
                  if (object == nullptr) {
                    map.put("suggested_palettes",
                  ...
                  while (iterator.hasNext()) {
                    if (string == ((SuggestedPalette)iterator.next()).getName())) {
                        throw new PngException(
                            "Duplicate suggested palette name " + string,
                            false);
                      }
                  }
            ...
            static void readText(int int1, DataInput dataInput, int int0,
                                 std::unordered_map map, PngImage pngImage) {
              byte[] bytes = new byte[int0];
              dataInput.readFully(bytes);
              ...
              case 1767135348:
                ...
                if (byte0 == 1) {
                  ...
                  boolean1 = false;
                  if (byte1 != 0) {
                    throw new PngException("Unrecognized " +
                                               PngConstants.getChunkName(int1) +
                                               " compression method: " + byte1,
                                           false);
                  }
                } else if (byte0 != 0) {
                  throw new PngException("Illegal " +
                                             PngConstants.getChunkName(int1) +
                                             " compression flag: " + byte0,
                                         false);
                }

                string2 = readString(dataInputStream,
                                     dataInputStream.available(), "US-ASCII");
                string3 = readString(dataInputStream,
                                     dataInputStream.available(), "UTF-8");
              case 1950701684:
              default:
                break;
              case 2052348020:
              ...
              std::string string4;
              if (boolean0) {
                string4 = new String(readCompressed(dataInputStream,
                                                    dataInputStream.available(),
                                                    boolean1),
                                     string1);
              } else {
                string4 = new String(bytes,
                                     bytes.length - dataInputStream.available(),
                                     dataInputStream.available(), string1);
              }

              if (string4.indexOf(0) >= 0) {
                throw new PngException("Text value contains nullptr", false);
              } else {
                void *object = (List)pngImage.getProperty("text_chunks",
                                                          List.class, false);
                if (object == nullptr) {
                  map.put("text_chunks",
                          object = std::make_unique<ArrayList>());
                ...
                object.add(new TextChunkImpl(string0, string4, string2, string3,
                                             int1));
              }
            }

            static void read_gIFg(DataInput dataInput, int int0,
                                  std::unordered_map map) {
            ...
            static void read_oFFs(DataInput dataInput, int int0,
                                  std::unordered_map map) {
              ...
              if (byte0 != 0 && byte0 != 1) {
                throw new PngException(
                    "Illegal oFFs chunk unit specifier: " + byte0, false);
              } else {
            ...
            static void read_sCAL(DataInput dataInput, int int0,
                                  std::unordered_map map) {
              byte[] bytes = new byte[int0];
              dataInput.readFully(bytes);
              DataInputStream dataInputStream =
              ...
              if (byte0 != 1 && byte0 != 2) {
                throw new PngException(
                    "Illegal sCAL chunk unit specifier: " + byte0, false);
              } else {
                double double0 = readFloatingPoint(dataInputStream,
                                                   dataInputStream.available());
                double double1 = readFloatingPoint(dataInputStream,
                                                   dataInputStream.available());
                if (!(double0 <= 0.0) && !(double1 <= 0.0)) {
                  ...
                  throw new PngException("sCAL measurements must be >= 0",
                                         false);
                }
              }
            }

            static void read_sTER(DataInput dataInput, int int0,
                                  std::unordered_map map) {
            ...
            static void checkLength(int int2, int int0, int int1) {
              if (int0 != int1) {
                throw new PngException(
                    "Bad " + PngConstants.getChunkName(int2) +
                        " chunk length: " + int0 + " (expected " + int1 + ")",
                    true);
              }
            }

          private
            static byte[] readCompressed(DataInput dataInput, int int0,
                                         boolean boolean0) throws IOException {
              if (boolean0) {
                uint8_t byte0 = dataInput.readByte();
                if (byte0 != 0) {
                  throw new PngException(
                      "Unrecognized compression method: " + byte0, false);
                }
              ...
              try {
                while (!inflater.needsInput()) {
                  byteArrayOutputStream.write(bytes1, 0,
                                              inflater.inflate(bytes1));
                }
              } catch (DataFormatException dataFormatException) {
                throw new PngException("Error reading compressed data",
                                       dataFormatException, false);
              }
            ...
            static std::string readKeyword(DataInput dataInput, int int0) {
              ...
              if (string.length() != 0 && string.length() <= 79) {
                ...
                throw new PngException(
                    "Invalid keyword length: " + string.length(), false);
              }
            }

          private
            static byte[] readToNull(DataInput dataInput,
                                     int int1) throws IOException {
              ByteArrayOutputStream byteArrayOutputStream =
                  new ByteArrayOutputStream();
            ...
            static double readFloatingPoint(DataInput dataInput, int int0) {
              ...
              double double0 = Double.valueOf(
                  string.substring(0, int1 < 0 ? string.length() : int1));
              if (int1 >= 0) {
                double0 *=
                    Math.pow(10.0, Double.valueOf(string.substring(int1 + 1)));
              }
