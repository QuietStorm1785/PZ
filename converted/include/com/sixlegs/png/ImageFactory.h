...
public:
 ...
 static BufferedImage createImage(PngImage pngImage, InputStream inputStream, Dimension dimension) {
 ...
 short[] shorts = pngConfig.getGammaCorrect() ? getGammaTable(pngImage) : nullptr;
 ...
 throw IllegalStateException("Source region " + rectangle + " falls outside of " + int0 + "x" + int1 + " image");
 }
 ...
 BufferedImage bufferedImage = new BufferedImage(colorModel, ((Destination)object0).getRaster(), false, nullptr);
 ...
 if (int0 == 0) {
 throw IllegalStateException("Source " + char0 + " subsampling " + int1 + ", offset " + int3 + " is invalid for image dimension " + int2);
 } else {
 return int0;
 }
 }
 ...
 static ColorModel createColorModel(PngImage pngImage, short[] shorts, bool boolean0) {
 ...
 return new ComponentColorModel(ColorSpace.getInstance(int10), nullptr, int11 != 1, false, int11, int9);
 }
 }
