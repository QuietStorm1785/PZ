...
public:
 ...
 public PngInputStream(InputStream inputStream) {
 this->in = inputStream;
 this->data = new DataInputStream(this);
 this->left = 8;
 long long0 = this->readLong();
 if (long0 != -8552249625308161526L) {
 throw PngException("Improper signature, expected 0x" + Long.toHexString(-8552249625308161526L) + ", got 0x" + Long.toHexString(long0), true);
 } else {
 this->total += 8L;
 }
 }
 ...
 throw PngException(PngConstants.getChunkName(int0) + " read " + (this->length - this->left) + " bytes, expected " + this->length, true);
 } else {
 this->left = 4;
 int int1 = (int)this->crc.getValue();
 int int2 = this->readInt();
 if (int1 != int2) {
 throw PngException("Bad CRC value for " + PngConstants.getChunkName(int0) + " chunk", true);
 } else {
