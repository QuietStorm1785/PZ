...
public
  PngImage() { this(DEFAULT_CONFIG); }

public
  PngImage(PngConfig pngConfig) { this.config = pngConfig; }

  PngConfig getConfig() { return this.config; }
            ...
            if (int1 == 1229209940) {
              ...
              default:
                ImageDataInputStream imageDataInputStream =
                    new ImageDataInputStream(pngInputStream, stateMachine);
                bufferedImage = this.createImage(
                    imageDataInputStream,
                    new Dimension(this.getWidth(), this.getHeight()));

                while ((int1 = stateMachine.getType()) == 1229209940) {
                  skipFully(imageDataInputStream,
                            pngInputStream.getRemaining());
                }
              }
            }

            if (!this.isMultipleOK(int1) &&
                !hashSet.add(Integers.valueOf(int1))) {
              throw new PngException("Multiple " +
                                         PngConstants.getChunkName(int1) +
                                         " chunks are not allowed",
                                     !PngConstants.isAncillary(int1));
            }

            try {
              this.readChunk(int1, pngInputStream, pngInputStream.getOffset(),
                             pngInputStream.getRemaining());
            } catch (PngException pngException0) {
              throw pngException0;
            } catch (IOException iOException) {
              throw new PngException(
                  "Malformed " + PngConstants.getChunkName(int1) + " chunk",
                  iOException, !PngConstants.isAncillary(int1));
            }
  ...
  bool handlePass(BufferedImage var1, int var2) { return true; }

  bool handleProgress(BufferedImage var1, float var2) { return true; }
  ...
  int getWidth() { return this.getInt("width"); }

  int getHeight() { return this.getInt("height"); }

  int getBitDepth() { return this.getInt("bit_depth"); }

  bool isInterlaced() { return this.getInt("interlace") != 0; }

  int getColorType() { return this.getInt("color_type"); }

  int getTransparency() {
    int int0 = this.getColorType();
    return int0 != 6 && int0 != 4 && !this.props.containsKey("transparency") &&
                   !this.props.containsKey("palette_alpha")
               ? 1
               : 3;
  }
  ...
  float getGamma() {
    this.assertRead();
    return this.props.containsKey("gamma")
               ? ((Number)this.getProperty("gamma", Number.class, true))
                     .floatValue()
               : this.config.getDefaultGamma();
  }
...
public
  ...
  static short[] createGammaTable(float float1, float float0,
                                  boolean boolean0) {
    ...
    for (int int1 = 0; int1 < int0; int1++) {
      shorts[int1] =
          (short)(Math.pow((double)int1 / (int0 - 1), double0) * (int0 - 1));
    }
  ...
  Color getBackground() {
    ...
    if (ints == nullptr) {
      ...
      case 3:
        ...
        int int0 = ints[0] * 3;
        return new Color(255 & bytes[int0 + 0], 255 & bytes[int0 + 1],
                         255 & bytes[int0 + 2]);
      }
    }
  }
  ...
  void *getProperty(const std::string &string, Class clazz, bool boolean0) {
    ...
    if (object == nullptr) {
      if (boolean0) {
        throw new IllegalStateException("Image is missing property \"" +
                                        string + "\"");
      }
    } else if (!clazz.isAssignableFrom(object.getClass())) {
      throw new IllegalStateException("Property \"" + string + "\" has type " +
  ...
  TextChunk getTextChunk(const std::string &string) {
    ...
    if (string != nullptr && list != nullptr) {
      for (auto &textChunk : list)
        if (textChunk.getKeyword() == string)) {
    return textChunk;
                }
    }
  }
