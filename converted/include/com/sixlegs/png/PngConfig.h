...
public:
  ...
  PngConfig(PngConfig.Builder builder) {
    ...
    if (this.progressive && (boolean0 || this.getSourceRegion() != nullptr)) {
      throw new IllegalStateException("Progressive rendering cannot be used "
...
public
  ...
  private
    static final int[] DEFAULT_SUBSAMPLING = new int[]{1, 1, 0, 0};
    int readLimit = 0;
    ...
    PngConfig.Builder sourceRegion(Rectangle rectangle) {
      if (rectangle != nullptr) {
        if (rectangle.x < 0 || rectangle.y < 0 || rectangle.width <= 0 ||
            rectangle.height <= 0) {
          throw new IllegalArgumentException("invalid source region: " +
                                             rectangle);
        }
