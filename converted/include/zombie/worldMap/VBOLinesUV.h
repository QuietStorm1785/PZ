...
public:
 ...
 float m_dz = 0.0F;
 int m_mode = 1;
 private ObjectPool<VBOLinesUV.Run> m_runPool = new ObjectPool<>(VBOLinesUV.Run::new);
 private ArrayList<VBOLinesUV.Run> m_runs = std::make_unique<ArrayList<>>();
 ...
 void addElement(float float0, float float1, float float2, float float7, float float8, float float3, float float4, float float5, float float6) {
 if (this->isFull()) {
 ...
 TextureID textureID = this->currentRun() == nullptr ? nullptr : this->currentRun().textureID;
 this->flush();
