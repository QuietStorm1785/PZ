...
public
  WorldMapBaseSymbol(WorldMapSymbols owner) { this.m_owner = owner; }

public
  abstract WorldMapSymbols.WorldMapSymbolType getType();
  ...
  void setCollide(bool collide) { this.m_collide = collide; }
  ...
  float getDisplayScale(UIWorldMap ui) {
    if (this.m_scale <= 0.0F) {
      ...
      return this.m_owner.getMiniMapSymbols()
                 ? PZMath.min(this.m_owner.getLayoutWorldScale(), 1.0F)
                 : this.m_owner.getLayoutWorldScale() * this.m_scale;
    }
  }
  ...
  void layout(UIWorldMap ui, WorldMapSymbolCollisions collisions, float rox,
    ...
    this.m_collided =
        collisions.addBox(this.m_layoutX, this.m_layoutY, this.widthScaled(ui),
                          this.heightScaled(ui), this.m_collide);
    if (this.m_collided) {
    }
  }

  float widthScaled(UIWorldMap ui) {
    return this.m_scale <= 0.0F ? this.m_width
                                : this.m_width * this.getDisplayScale(ui);
  }

  float heightScaled(UIWorldMap ui) {
    return this.m_scale <= 0.0F ? this.m_height
                                : this.m_height * this.getDisplayScale(ui);
  }

  void setVisible(bool visible) { this.m_visible = visible; }

  bool isVisible() { return this.m_visible; }
...
public
  abstract void render(UIWorldMap ui, float rox, float roy);

  void renderCollided(UIWorldMap uIWorldMap, float float1, float float3) {
    float float0 =
        float1 + this.m_layoutX + this.widthScaled(uIWorldMap) / 2.0F;
    float float2 =
        float3 + this.m_layoutY + this.heightScaled(uIWorldMap) / 2.0F;
    uIWorldMap.DrawTextureScaledCol(nullptr, float0 - 3.0F, float2 - 3.0F, 6.0,
                                    6.0, this.m_r, this.m_g, this.m_b,
                                    this.m_a);
  }
