...
public:
  ...
  FileSystem m_zipFS;
  final HashMap<String, ImagePyramid.PyramidTexture> m_textures =
  ...
  void setZipFile(const std::string &zipFile) {
    ...
    if (this.m_zipFS != nullptr) {
      try(DirectoryStream directoryStream =
              Files.newDirectoryStream(this.m_zipFS.getPath("/"))) {
...
TextureID getTexture(int x, int y, int z) {
  ...
  if (this.m_textures.containsKey(string)) {
    ...
    try {
      Path path = this.m_zipFS.getPath(String.valueOf(z),
                                       String.format("tile%dx%d.png", x, y));

      try {
        TextureID textureID;
        try(InputStream inputStream = Files.newInputStream(path)) {
          ImageData imageData = new ImageData(inputStream, false);
          ImagePyramid.PyramidTexture pyramidTexture1 =
              this.checkTextureCache(string);
...
void replaceTextureData(ImagePyramid.PyramidTexture pyramidTexture,
                        ImageData imageData) {
  char char0;
  ...
  GL11.glBindTexture(3553, Texture.lastTextureID =
                               pyramidTexture.m_textureID.getID());
  SpriteRenderer.ringBuffer.restoreBoundTextures = true;
  GL11.glTexImage2D(3553, 0, char0, imageData.getWidthHW(),
                    imageData.getHeightHW(), 0, 6408, 5121,
                    imageData.getData().getBuffer());
  imageData.dispose();
}
      ...
      for (int int3 = 0; int3 < int2; int3++) {
        for (int int4 = 0; int4 < int1; int4++) {
          BufferedImage bufferedImage =
              this.getBufferedImage(mipMapLevel, int4, int3, short0);
          this.writeImageToFile(bufferedImage, outputDirectory, int4, int3,
                                int0);
        }
      }
    }
      ...
      for (int int0 = 0; int0 < byte0; int0++) {
        ...
        for (int int3 = 0; int3 < int2; int3++) {
          for (int int4 = 0; int4 < int1; int4++) {
            BufferedImage bufferedImage =
                this.getBufferedImage(mipMapLevel, int4, int3, short0);
            this.writeImageToZip(bufferedImage, zipOutputStream, int4, int3,
                                 int0);
          }
        }
...
BufferedImage getBufferedImage(MipMapLevel mipMapLevel, int int3, int int2,
                               int int0) {
...
void writeImageToZip(BufferedImage bufferedImage,
                     ZipOutputStream zipOutputStream, int int1, int int0,
                     int int2) {
  zipOutputStream.putNextEntry(
      new ZipEntry(String.format("%d/tile%dx%d.png", int2, int1, int0)));
...
ImagePyramid.PyramidTexture checkTextureCache(String string) {
  if (this.m_textures.size() < this.MAX_TEXTURES) {
    ImagePyramid.PyramidTexture pyramidTexture0 =
        new ImagePyramid.PyramidTexture();
    ...
    for (ImagePyramid.PyramidTexture pyramidTexture2 :
         this.m_textures.values()) {
      if (pyramidTexture1 == nullptr ||
          pyramidTexture1.m_requestNumber > pyramidTexture2.m_requestNumber) {
...
void resetRequestNumbers() {
  ...
  arrayList.sort(Comparator.comparingInt(
      pyramidTexturex->pyramidTexturex.m_requestNumber));
...
void destroy() {
  ...
  RenderThread.invokeOnRenderContext(()->{
    for (ImagePyramid.PyramidTexture pyramidTexture :
         this.m_textures.values()) {
...
public
static final class PyramidTexture {
  std::string m_key;
  int m_requestNumber;
