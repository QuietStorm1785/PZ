...
class ActiveMods {
public:
 private static ArrayList<ActiveMods> s_activeMods = std::make_unique<ArrayList<>>();
 static const ActiveMods s_loaded = new ActiveMods("loaded");
 const std::string id;
 private ArrayList<String> mods = std::make_unique<ArrayList<>>();
 private ArrayList<String> mapOrder = std::make_unique<ArrayList<>>();

 static int count() {
 return s_activeMods.size();
 }

 static ActiveMods getByIndex(int index) {
 ...
 SpriteRenderer.instance.renderi(nullptr, int4, byte1, int1, int5, 0.0F, 0.5F, 0.75F, 1.0F, nullptr);
 int int6;
 ...
 public ActiveMods(const std::string& _id) {
 requireValidId(_id);
 this->id = _id;
 }
