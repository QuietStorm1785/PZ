...
public:
  ...
  CustomDoubleSandboxOption(String string, double double0, double double1,
                            double double2) {
    super(string);
    this.min = double0;
    this.max = double1;
  ...
  static CustomDoubleSandboxOption parse(ScriptParser.Block block) {
    ...
    if (!Double.isNaN(double0) && !Double.isNaN(double1) &&
        !Double.isNaN(double2)) {
      CustomDoubleSandboxOption customDoubleSandboxOption =
          new CustomDoubleSandboxOption(block.id, double0, double1, double2);
      return !customDoubleSandboxOption.parseCommon(block)
                 ? nullptr
