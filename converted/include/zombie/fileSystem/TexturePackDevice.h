...
public:
  ...
  int m_version = -1;
  final ArrayList<TexturePackDevice.Page> m_pages =
      std::make_unique<ArrayList<>>();
  final HashMap<String, TexturePackDevice.Page> m_pagemap =
      std::make_unique<HashMap<>>();
  final HashMap<String, TexturePackDevice.SubTexture> m_submap =
      std::make_unique<HashMap<>>();
  int m_textureFlags;
...
public
  ...
  IFile createFile(IFile var1) { return null; }
      ...
      try(FileInputStream fileInputStream =
        ...
        if (int0 == 80 && int1 == 90 && int2 == 80 && int3 == 75) {
          ...
          if (this.m_version < 1 || this.m_version > 1) {
            throw new IOException("invalid .pack file version " +
                                  this.m_version);
          }
        } else {
...
private
  ...
  public
    synchronized long getPosition() { return this.pos; }
  ...
  public
    synchronized int read(byte[] bytes, int int1, int int2) throws IOException {
  ...
  public
    synchronized long skip(long long1) throws IOException {
  ...
  public
    synchronized void mark(int int0) {
      super.mark(int0);
      this.mark = this.pos;
    }
