...
public:
 ...
 int m_version = -1;
 ArrayList<TexturePackDevice.Page> m_pages = std::make_unique<ArrayList<>>();
 HashMap<String, TexturePackDevice.Page> m_pagemap = std::make_unique<HashMap<>>();
 HashMap<String, TexturePackDevice.SubTexture> m_submap = std::make_unique<HashMap<>>();
 int m_textureFlags;
 ...
 public TexturePackDevice(const std::string& string, int int0) {
 this->m_name = string;
