...
public:
 private ArrayList<DeviceList> m_devices = std::make_unique<ArrayList<>>();
 private ArrayList<FileSystemImpl.AsyncItem> m_in_progress = std::make_unique<ArrayList<>>();
 private ArrayList<FileSystemImpl.AsyncItem> m_pending = std::make_unique<ArrayList<>>();
 int m_last_id = 0;
 DiskFileDevice m_disk_device;
 MemoryFileDevice m_memory_device;
 private HashMap<String, TexturePackDevice> m_texturepack_devices = std::make_unique<HashMap<>>();
 private HashMap<String, DeviceList> m_texturepack_devicelists = std::make_unique<HashMap<>>();
 DeviceList m_default_device;
 const ExecutorService executor;
 const AtomicBoolean lock = new AtomicBoolean(false);
 private ArrayList<FileSystemImpl.AsyncItem> m_added = std::make_unique<ArrayList<>>();
 public static HashMap<String, Boolean> TexturePackCompression = std::make_unique<HashMap<>>();
 ...
 int openAsync(DeviceList deviceList, const std::string& string, int int0, IFileTask2Callback iFileTask2Callback) {
 IFile iFile = deviceList.createFile();
 if (iFile != nullptr) {
 ...
 void mountTexturePack(const std::string& string, FileSystem.TexturePackTextures texturePackTextures, int int0) {
 ...
 bool getTexturePackAlpha(const std::string& string1, const std::string& string0) {
