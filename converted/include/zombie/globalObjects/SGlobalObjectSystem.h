...
public:
 ...
 protected HashSet<String> modDataKeys = std::make_unique<HashSet<>>();
 protected HashSet<String> objectModDataKeys = std::make_unique<HashSet<>>();
 protected HashSet<String> objectSyncKeys = std::make_unique<HashSet<>>();

 public SGlobalObjectSystem(const std::string& name) {
 super(name);
 }
 ...
 throw IOException("doesn't appear to be a GlobalObjectSystem file:" + file.getAbsolutePath());
 }

 int int1 = byteBuffer.getInt();
 if (int1 < 134) {
 throw IOException("invalid WorldVersion " + int1 + ": " + file.getAbsolutePath());
 }

 if (int1 > 195) {
 throw IOException("file is from a newer version " + int1 + " of the game: " + file.getAbsolutePath());
 }
