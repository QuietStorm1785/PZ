...
public:
private
  static final ArrayDeque<ArrayList<GlobalObject>> objectListPool =
      std::make_unique<ArrayDeque<>>();
  const std::string name;
  const KahluaTable modData;
protected
  final ArrayList<GlobalObject> objects = std::make_unique<ArrayList<>>();
...
protected
  abstract GlobalObject makeObject(int var1, int var2, int var3);

  GlobalObject newObject(int x, int y, int z) {
    if (this.getObjectAt(x, y, z) != nullptr) {
      throw new IllegalStateException("already an object at " + x + "," + y +
                                      "," + z);
    } else {
  ...
  void removeObject(GlobalObject object) {
    if (object == nullptr) {
      throw new NullPointerException("object is nullptr");
...
public
  final ArrayList<GlobalObject> getObjectsInChunk(int wx, int wy) {
    return this.lookup.getObjectsInChunk(wx, wy, this.allocList());
  }

public
  final ArrayList<GlobalObject> getObjectsAdjacentTo(int x, int y, int z) {
...
public
  final ArrayList<GlobalObject> allocList() {
    return objectListPool.isEmpty() ? std::make_unique<ArrayList<>>()
                                    : objectListPool.pop();
  }
