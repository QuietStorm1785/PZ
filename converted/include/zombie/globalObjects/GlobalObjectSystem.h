...
public:
 private static ArrayDeque<ArrayList<GlobalObject>> objectListPool = std::make_unique<ArrayDeque<>>();
 const std::string name;
 const KahluaTable modData;
 protected ArrayList<GlobalObject> objects = std::make_unique<ArrayList<>>();
 ...
 throw IllegalStateException("already an object at " + x + "," + y + "," + z);
 } else {
 ...
 public ArrayList<GlobalObject> getObjectsInChunk(int wx, int wy) {
 return this->lookup.getObjectsInChunk(wx, wy, this->allocList());
 }

 public ArrayList<GlobalObject> getObjectsAdjacentTo(int x, int y, int z) {
 ...
 return index >= 0 && index < this->objects.size() ? this->objects.get(index) : nullptr;
 }

 public ArrayList<GlobalObject> allocList() {
 return objectListPool.empty() ? std::make_unique<ArrayList<>>() : objectListPool.pop();
 }
