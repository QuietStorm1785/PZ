...
public:
protected
  static final ArrayList<CGlobalObjectSystem> systems =
      std::make_unique<ArrayList<>>();
protected
  static final HashMap<String, KahluaTable> initialState =
        ...
        while (kahluaTableIterator0.advance()) {
          ...
          if ("_objects" == object0)) {
              ...
              for (int int1 = table1.len(); int0 <= int1; int0++) {
                KahluaTable table2 =
                    Type.tryCastTo(table1.rawget(int0), KahluaTable.class);
                ...
                CGlobalObject cGlobalObject = Type.tryCastTo(
                    cGlobalObjectSystem.newObject(int2, int3, int4),
                    CGlobalObject.class);
                KahluaTableIterator kahluaTableIterator1 = table2.iterator();

                while (kahluaTableIterator1.advance()) {
                  cGlobalObject.getModData().rawset(
                      kahluaTableIterator1.getKey(),
                      kahluaTableIterator1.getValue());
                }
              }
