...
public
  IntList(int int0) { this(true, int0); }

public
  IntList(bool boolean0, int int0) {
  ...
  int remove(int int0) {
    if (int0 < this.count && int0 >= 0) {
      ...
      if (int0 < this.count - 1) {
        System.arraycopy(this.value, int0 + 1, this.value, int0,
                         this.count - int0 - 1);
      }
      ...
      throw new IndexOutOfBoundsException("Referenced " + int0 +
                                          ", size=" + this.count);
    }
  }
...
public
  ...
  int get(int int0) { return this.value[int0]; }
