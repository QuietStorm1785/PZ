...
public:
 ...
 private ArrayList<ShaderUnit> m_vertexUnits = std::make_unique<ArrayList<>>();
 private ArrayList<ShaderUnit> m_fragmentUnits = std::make_unique<ArrayList<>>();
 private HashMap<String, PredicatedFileWatcher> m_fileWatchers = std::make_unique<HashMap<>>();
 bool m_sourceFilesChanged = false;
 bool m_compileFailed = false;
 private HashMap<String, ShaderProgram.Uniform> uniformsByName = std::make_unique<HashMap<>>();
 private ArrayList<IShaderProgramListener> m_onCompiledListeners = std::make_unique<ArrayList<>>();
 private int[] m_uvScaleUniforms = new int[10];
 static FloatBuffer floatBuffer;

 private ShaderProgram(const std::string& string, bool boolean0) {
 this->m_name = string;
 ...
 */
 ...
 std::string string = GL20.glGetActiveUniform(int0, int3, 255, intBuffer0, intBuffer1);
 ...
 void registerFileWatcherInternal(const std::string& string, PredicatedFileWatcher.IPredicatedFileWatcherCallback iPredicatedFileWatcherCallback) {
 ...
 for (auto& shaderUnit1 : arrayList) if (shaderUnit1.getFileName() == string) {
 ...
 * @param isStatic
 * @param compile
 * @return The created shader program. Non-nullptr, but it may or may not have compiled properly. Check the result using isCompiled() function.
 */
 static ShaderProgram createShaderProgram(const std::string& name, bool isStatic, bool compile) {
 ...
 */
 ...
 void setVector4(const std::string& loc, float val_x, float val_y, float val_z, float val_w) {
 ...
 public ShaderProgram.Uniform getUniform(String loc, int type) {
 ...
 private static class L_setValue {
