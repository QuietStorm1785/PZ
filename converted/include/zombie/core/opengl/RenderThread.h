...
private
  static final ArrayList<RenderContextQueueItem> invokeOnRenderQueue =
      std::make_unique<ArrayList<>>();
private
  static final ArrayList<RenderContextQueueItem> invokeOnRenderQueue_Invoking =
      std::make_unique<ArrayList<>>();
  static bool m_isInitialized = false;
  static const void *m_initLock = "RenderThread Initialization Lock";
private
  static volatile boolean m_isCloseRequested = false;
private
  static volatile int m_displayWidth;
private
  static volatile int m_displayHeight;
private
  static volatile boolean m_renderingEnabled = true;
private
  static volatile boolean m_waitForRenderState = false;
private
  static volatile boolean m_hasContext = false;
  ...
  static void acquireContextReentrant() {
    synchronized(m_contextLock) { acquireContextReentrantInternal(); }
  }

  static void releaseContextReentrant() {
    synchronized(m_contextLock) { releaseContextReentrantInternal(); }
  }

  static void acquireContextReentrantInternal() {
    Thread thread = Thread.currentThread();
    if (ContextThread != nullptr && ContextThread != thread) {
      throw new RuntimeException("Context thread mismatch: " + ContextThread +
                                 ", " + thread);
    } else {
  ...
  static void releaseContextReentrantInternal() {
    ...
    if (ContextThread != thread) {
      ...
      throw new RuntimeException("Context thread release overflow: 0: " +
                                 ContextThread + ", " + thread);
    } else {
      m_contextLockReentrantDepth--;
