...
private
  final RenderSettings.PlayerRenderSettings[] playerSettings =
      new RenderSettings.PlayerRenderSettings[4];
...
public
  RenderSettings.PlayerRenderSettings getPlayerSettings(int int0) {
    ...
    if (!GameServer.bServer) {
      for (int int0 = 0; int0 < 4; int0++) {
        if (IsoPlayer.players[int0] != nullptr) {
          this.playerSettings[int0].updateRenderSettings(
              int0, IsoPlayer.players[int0]);
        }
      }
    }
  }
  ...
  float getAmbientForPlayer(int int0) {
    RenderSettings.PlayerRenderSettings playerRenderSettings =
        this.getPlayerSettings(int0);
    return playerRenderSettings != nullptr ? playerRenderSettings.getAmbient()
                                           : 0.0F;
  }
  ...
  Color getMaskClearColorForPlayer(int int0) {
    RenderSettings.PlayerRenderSettings playerRenderSettings =
        this.getPlayerSettings(int0);
    return playerRenderSettings != nullptr
               ? playerRenderSettings.getMaskClearColor()
               : this.defaultClear;
  }

public
  static class PlayerRenderSettings {
    ...
    void updateRenderSettings(int int0, IsoPlayer player) {
      ...
      climateManager.getThunderStorm().applyLightningForPlayer(this, int0,
                                                               player);
      ...
      this.applyNightVisionGoggles =
          player != nullptr && player.isWearingNightVisionGoggles();
      this.isExterior =
          player != nullptr &&
          (player.isDead() || player.getCurrentSquare() != nullptr &&
      ...
      if (this.ambient < 0.2F &&
          player.getCharacterTraits().NightVision.isSet()) {
        this.ambient = 0.2F;
      }
      ...
      if (Core.getInstance().RenderShader == nullptr ||
        ...
        this.rmod = GameTime.getInstance().Lerp(1.0F, this.blendInfo.r,
                                                this.blendIntensity);
        this.gmod = GameTime.getInstance().Lerp(1.0F, this.blendInfo.g,
                                                this.blendIntensity);
        this.bmod = GameTime.getInstance().Lerp(1.0F, this.blendInfo.b,
                                                this.blendIntensity);
    ...
    void legacyPostRender(int int0) {
      ...
      if (this.applyNightVisionGoggles) {
        ...
        SpriteRenderer.instance.render(
            RenderSettings.texture, 0.0F, 0.0F,
            Core.getInstance().getOffscreenWidth(int0),
            Core.getInstance().getOffscreenHeight(int0), 0.05F, 0.95F, 0.05F,
            this.goggleMod, nullptr);
        ...
        SpriteRenderer.instance.render(
            RenderSettings.texture, 0.0F, 0.0F,
            Core.getInstance().getOffscreenWidth(int0),
            Core.getInstance().getOffscreenHeight(int0), this.blendInfo.r,
            this.blendInfo.g, this.blendInfo.b, 1.0F, nullptr);
        IndieGL.glBlendFunc(770, 771);
      }
    ...
    Color getBlendColor() { return this.blendColor; }

    float getBlendIntensity() { return this.blendIntensity; }

    float getDesaturation() { return this.desaturation; }

    float getDarkness() { return this.darkness; }

    float getNight() { return this.night; }

    float getViewDistance() { return this.viewDistance; }

    float getAmbient() { return this.ambient; }

    bool isApplyNightVisionGoggles() { return this.applyNightVisionGoggles; }

    float getRmod() { return this.rmod; }

    float getGmod() { return this.gmod; }

    float getBmod() { return this.bmod; }

    bool isExterior() { return this.isExterior; }

    float getFogMod() { return this.fogMod; }
    ...
    Color getMaskClearColor() { return this.maskClearColor; }

    float getSM_Radius() { return this.SM_Radius; }

    float getSM_Alpha() { return this.SM_Alpha; }
  }
}
} // namespace opengl
} // namespace core
