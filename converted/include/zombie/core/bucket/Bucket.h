...
public:
 std::string m_name;
 private HashMap<Path, Texture> m_textures = std::make_unique<HashMap<>>();
 static const FileSystem m_fs = FileSystems.getDefault();
 private PredicatedFileWatcher m_fileWatcher = new PredicatedFileWatcher(string -> this->HasTexture(string), string -> {
