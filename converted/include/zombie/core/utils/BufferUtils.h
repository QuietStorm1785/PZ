...
public:
 ...
 private static ReferenceQueue<Buffer> removeCollected = std::make_unique<ReferenceQueue<>>();
 private static ConcurrentHashMap<BufferUtils.BufferInfo, BufferUtils.BufferInfo> trackedBuffers = std::make_unique<ConcurrentHashMap<>>();
 ...
 static Method loadMethod(const std::string& string1, const std::string& string0) {
 try {
