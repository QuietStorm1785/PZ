...
public:
private
  FibonacciHeap.Entry<T> mMin = nullptr;
  int mSize = 0;
...
public
  FibonacciHeap.Entry<T> enqueue(T object, double double0) {
...
public
  ...
  bool isEmpty() { return this.mMin == nullptr; }

  int size() { return this.mSize; }

public
  static<T> FibonacciHeap<T> merge(FibonacciHeap<T> fibonacciHeap2,
                                   FibonacciHeap<T> fibonacciHeap1) {
  ...
  FibonacciHeap.Entry<T> dequeueMin() {
    if (this.isEmpty()) {
      ...
      if (this.mMin == nullptr) {
        ...
        for (FibonacciHeap.Entry entry2 = this.mMin;
             this.toVisit.isEmpty() || this.toVisit.get(0) != entry2;
             entry2 = entry2.mNext) {
          ...
          while (true) {
            while (entry3.mDegree < this.treeTable.size()) {
              ...
              FibonacciHeap.Entry entry5 =
                  entry4.mPriority < entry3.mPriority ? entry4 : entry3;
              FibonacciHeap.Entry entry6 =
                  entry4.mPriority < entry3.mPriority ? entry3 : entry4;
...
private
  static<T> FibonacciHeap.Entry<T> mergeLists(FibonacciHeap.Entry<T> entry1,
                                              FibonacciHeap.Entry<T> entry0) {
  ...
  void decreaseKeyUnchecked(FibonacciHeap.Entry<T> entry, double double0) {
    ...
    if (entry.mParent != nullptr &&
        entry.mPriority <= entry.mParent.mPriority) {
      this.cutNode(entry);
    }
  ...
  void decreaseKeyUncheckedNode(FibonacciHeap.Entry<IsoGridSquare> entry,
                                double double0) {
    entry.mPriority = double0;
    if (entry.mParent != nullptr &&
        entry.mPriority <= entry.mParent.mPriority) {
...
public
  ...
  private
    FibonacciHeap.Entry<T> mNext;
  private
    FibonacciHeap.Entry<T> mPrev;
  private
    FibonacciHeap.Entry<T> mParent;
  private
    FibonacciHeap.Entry<T> mChild;
    T mElem;
    double mPriority;

    T getValue() { return this.mElem; }

    void setValue(T object) { this.mElem = (T)object; }

    double getPriority() { return this.mPriority; }

  private
    Entry(T object, double double0) {
