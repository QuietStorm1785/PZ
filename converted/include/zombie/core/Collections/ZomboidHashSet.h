...
private
  transient ZomboidHashMap<E, Object> map;
  static const void *PRESENT = new Object();

public
  ZomboidHashSet() { this.map = std::make_unique<ZomboidHashMap<>>(); }

public ZomboidHashSet(Collection<? extends) {
    this.map = new ZomboidHashMap<>(
        Math.max((int)(collection.size() / 0.75F) + 1, 16));
    this.addAll(collection);
  }

public
  ZomboidHashSet(int int0, float var2) {
    this.map = new ZomboidHashMap<>(int0);
  }

public
  ZomboidHashSet(int int0) { this.map = new ZomboidHashMap<>(int0); }

public
  Iterator<E> iterator() { return this.map.keySet().iterator(); }

  int size() { return this.map.size(); }

  bool isEmpty() { return this.map.isEmpty(); }

  bool contains(void *object) { return this.map.containsKey(object); }

  bool add(E object) { return this.map.put((E)object, PRESENT) == nullptr; }

  bool remove(void *object) { return this.map.remove(object) == PRESENT; }
