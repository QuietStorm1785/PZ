...
public:
public
  static ArrayList<Stash> allStashes;
public
  static ArrayList<StashBuilding> possibleStashes;
public
  static ArrayList<StashBuilding> buildingsToDo;
private
  static final ArrayList<String> possibleTrap = std::make_unique<ArrayList<>>();
private
  static ArrayList<String> alreadyReadMap = std::make_unique<ArrayList<>>();
        ...
        if (stash.annotations != nullptr) {
          for (int int0 = 0; int0 < stash.annotations.size(); int0++) {
            ...
            if (stashAnnotation.symbol != nullptr) {
              mapItem.getSymbols().addTexture(
                  stashAnnotation.symbol, stashAnnotation.x, stashAnnotation.y,
                  0.5F, 0.5F, WorldMapBaseSymbol.DEFAULT_SCALE,
                  stashAnnotation.r, stashAnnotation.g, stashAnnotation.b,
                  1.0F);
            } else if (stashAnnotation.text != nullptr) {
              mapItem.getSymbols().addUntranslatedText(
                  stashAnnotation.text, UIFont.Handwritten, stashAnnotation.x,
                  stashAnnotation.y, stashAnnotation.r, stashAnnotation.g,
                  stashAnnotation.b, 1.0F);
            }
          }
        }
  ...
  static void prepareBuildingStash(const std::string &stashName) {
    if (stashName != nullptr) {
      ...
      if (stash != nullptr && !alreadyReadMap.contains(stashName)) {
        ...
        buildingsToDo.add(
            new StashBuilding(stash.name, stash.buildingX, stash.buildingY));
        RoomDef roomDef = IsoWorld.instance.getMetaGrid().getRoomAt(
            stash.buildingX, stash.buildingY, 0);
        if (roomDef != nullptr && roomDef.getBuilding() != nullptr &&
            roomDef.getBuilding().isFullyStreamedIn()) {
  ...
  static bool checkSpecificSpawnProperties(Stash stash, InventoryItem item) {
    return !stash.spawnOnlyOnZed ||
           item.getContainer() != nullptr && item.getContainer().getParent()
        instanceof
        IsoDeadBody
            ? (stash.minDayToSpawn <= -1 ||
               GameTime.instance.getDaysSurvived() >= stash.minDayToSpawn) &&
                  (stash.maxDayToSpawn <= -1 ||
                   GameTime.instance.getDaysSurvived() <= stash.maxDayToSpawn)
            : false;
  }
  ...
  static void removeFromPossibleStash(Stash stash) {
    for (int int0 = 0; int0 < possibleStashes.size(); int0++) {
      ...
      if (stashBuilding.buildingX == stash.buildingX &&
          stashBuilding.buildingY == stash.buildingY) {
  ...
  static void doBuildingStash(BuildingDef def) {
    ...
    for (int int0 = 0; int0 < buildingsToDo.size(); int0++) {
      ...
      if (stashBuilding.buildingX > def.x && stashBuilding.buildingX < def.x2 &&
          stashBuilding.buildingY > def.y && stashBuilding.buildingY < def.y2) {
        if (def.hasBeenVisited) {
          ...
          if (stash != nullptr) {
            ItemPickerJava.ItemPickerRoom itemPickerRoom0 =
                ItemPickerJava.rooms.get(stash.spawnTable);
                ...
                for (int int2 = def.y - 1; int2 < def.y2 + 1; int2++) {
                  for (int int3 = 0; int3 < 8; int3++) {
                    IsoGridSquare square =
                        IsoWorld.instance.CurrentCell.getGridSquare(int1, int2,
                                                                    int3);
                    if (square != nullptr) {
                      for (int int4 = 0; int4 < square.getObjects().size();
                        ...
                        if (object.getContainer() != nullptr &&
                            ...
                            itemPickerRoom0.Containers.containsKey(
                                object.getContainer().getType())) {
                          ItemPickerJava.ItemPickerRoom itemPickerRoom1 =
                              ItemPickerJava.rooms.get(
                                  square.getRoom().getName());
                          bool boolean0 = false;
                          if (itemPickerRoom1 == nullptr ||
                              !itemPickerRoom1.Containers.containsKey(
                                  object.getContainer().getType())) {
                          ...
                          ItemPickerJava.fillContainerType(
                              itemPickerRoom0, object.getContainer(), "",
                              nullptr);
                        ...
                        if (stash.barricades > -1 && barricadeAble != nullptr &&
  ...
  static void doSpecificBuildingProperties(Stash stash,
    ...
    if (stash.containers != nullptr) {
      ...
      for (int int0 = 0; int0 < stash.containers.size(); int0++) {
        ...
        else if (stashContainer.contX > -1 && stashContainer.contY > -1 &&
                 stashContainer.contZ > -1) {
          square0 = IsoWorld.instance.getCell().getGridSquare(
              stashContainer.contX, stashContainer.contY, stashContainer.contZ);
        } else {
        ...
        if (!arrayList0.isEmpty()) {
          RoomDef roomDef1 =
              (RoomDef)arrayList0.get(Rand.Next(0, arrayList0.size()));
          square0 = roomDef1.getFreeSquare();
        }

        if (square0 != nullptr) {
          if (stashContainer.containerItem != nullptr &&
              !stashContainer.containerItem.isEmpty()) {
            ItemPickerJava.ItemPickerRoom itemPickerRoom =
                ItemPickerJava.rooms.get(stash.spawnTable);
            if (itemPickerRoom == nullptr) {
              DebugLog.log("Container distribution " + stash.spawnTable +
                           " not found");
              return;
            }

            InventoryItem item =
                InventoryItemFactory.CreateItem(stashContainer.containerItem);
            if (item == nullptr) {
              DebugLog.General.error("Item " + stashContainer.containerItem +
                                     " Doesn't exist.");
              return;
            }

            ItemPickerJava.ItemPickerContainer itemPickerContainer =
                itemPickerRoom.Containers.get(item.getType());
            ItemPickerJava.rollContainerItem((InventoryContainer)item, nullptr,
                                             itemPickerContainer);
            square0.AddWorldInventoryItem(item, 0.0F, 0.0F, 0.0F);
          } else {
            IsoThumpable thumpable =
                new IsoThumpable(square0.getCell(), square0,
                                 stashContainer.containerSprite, false, null);
            thumpable.setIsThumpable(false);
            thumpable.container = new ItemContainer(
                stashContainer.containerType, square0, thumpable);
            square0.AddSpecialObject(thumpable);
            square0.RecalcAllWithNeighbours(true);
          }
        } else {
          DebugLog.log(
              "No free room was found to spawn special container for stash " +
              stash.name);
        }
      }
    }
    ...
    if (stash.minTrapToSpawn > -1) {
      for (int int2 = stash.minTrapToSpawn; int2 < stash.maxTrapToSpawn;
           int2++) {
        IsoGridSquare square1 = buildingDef.getFreeSquareInRoom();
        if (square1 != nullptr) {
          HandWeapon weapon = (HandWeapon)InventoryItemFactory.CreateItem(
              possibleTrap.get(Rand.Next(0, possibleTrap.size())));
          if (GameServer.bServer) {
            GameServer.AddExplosiveTrap(weapon, square1,
      ...
      for (int int3 = 0; int3 < buildingDef.rooms.size(); int3++) {
        ...
        if (IsoWorld.getZombiesEnabled()) {
          ...
          std::vector arrayList1 =
              VirtualZombieManager.instance.addZombiesToMap(int4, roomDef2,
                                                            false);
  ...
  /**
   * Check if the visited building is in one of our random stash, in that case
  ...
  static void visitedBuilding(BuildingDef def) {
    if (!GameClient.bClient) {
      for (int int0 = 0; int0 < possibleStashes.size(); int0++) {
        StashBuilding stashBuilding = possibleStashes.get(int0);
        if (stashBuilding.buildingX > def.x &&
            stashBuilding.buildingX < def.x2 &&
            stashBuilding.buildingY > def.y &&
            stashBuilding.buildingY < def.y2) {
          possibleStashes.remove(int0);
          int0--;
        }
      }
  ...
  static void load(ByteBuffer input, int WorldVersion) {
    ...
    for (int int1 = 0; int1 < int0; int1++) {
      possibleStashes.add(new StashBuilding(GameWindow.ReadString(input),
    ...
    for (int int3 = 0; int3 < int2; int3++) {
      buildingsToDo.add(new StashBuilding(GameWindow.ReadString(input),
                                          input.getInt(), input.getInt()));
    }
...
public
  static ArrayList<StashBuilding> getPossibleStashes() {
