...
public:
 ...
 public HashMap<Integer, IsoMovingObject> physicsObjectMap = std::make_unique<HashMap<>>();
 bool created = false;
 float offsetX = 0.0F;
 float offsetY = 0.0F;
 long time;
 private ArrayList<BaseVehicle> collideVehicles = new ArrayList<>(4);
 ...
 if (vehicle1 != nullptr && vehicle1.isNetPlayerId(player.getOnlineID()) && this->collideVehicles.contains(vehicle1) {
 ...
 static const PerformanceProfileProbe worldSimulationUpdate = new PerformanceProfileProbe("WorldSimulation.update");
 }
}
} // namespace physics
} // namespace core
