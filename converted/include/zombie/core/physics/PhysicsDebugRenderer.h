...
public:
 private static ObjectPool<PhysicsDebugRenderer> POOL = new ObjectPool<>(PhysicsDebugRenderer::new);
 ...
 void drawSphere(float pX, float pY, float pZ, float radius, float r, float g, float b) {
 }

 void drawTriangle(float aX, float aY, float aZ, float bX, float bY, float bZ, float cX, float cY, float cZ, float r, float g, float b, float alpha) {
 }
