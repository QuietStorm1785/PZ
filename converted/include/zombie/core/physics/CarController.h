...
public:
 ...
 private static CarController.BulletVariables bulletVariables = new CarController.BulletVariables();
 ...
 void accelerator(bool apply) {
 this->acceleratorOn = apply;
 }

 void brake(bool apply) {
 this->brakeOn = apply;
 }
 ...
 bool boolean0 = this->vehicleObject.getDriver() != nullptr && this->vehicleObject.getDriver().getMoodles().getMoodleLevel(MoodleType.Drunk) > 1;
 float float0 = 0.0F;
 ...
 if (vehicle != nullptr && vehicle.getDriver() == nullptr && this->vehicleObject.getDriver() != nullptr && !GameClient.bClient) {
 this->vehicleObject.addPointConstraint(nullptr, vehicle, this->vehicleObject.getTowAttachmentSelf(), vehicle.getTowAttachmentSelf());
 }
 ...
 if (GameClient.bClient) {
 Boolean boolean1 = this->vehicleObject.getDriver().getInventory().haveThisKeyId(this->vehicleObject.getKeyId()) != nullptr
 ? Boolean.TRUE
 : Boolean.FALSE;
 ...
 if (vehicle != nullptr) {
 if (GameServer.bServer) {
 if (vehicle.getDriver() == nullptr && this->vehicleObject.getDriver() != nullptr) {
 this->vehicleObject.addPointConstraint(nullptr, vehicle, this->vehicleObject.getTowAttachmentSelf(), vehicle.getTowAttachmentSelf());
 }
 } else {
 this->speed = this->vehicleObject.getCurrentSpeedKmHour();
 ...
 if (vehicle.getDriver() == nullptr && this->vehicleObject.getDriver() != nullptr && !GameClient.bClient) {
 this->vehicleObject.addPointConstraint(nullptr, vehicle, this->vehicleObject.getTowAttachmentSelf(), vehicle.getTowAttachmentSelf());
 } else {
 this->checkShouldBeActive();
 ...
 bool boolean0 = character != nullptr && character.Traits.SpeedDemon.isSet();
 bool boolean1 = character != nullptr && character.Traits.SundayDriver.isSet();
 ...
 bool boolean0 = GameKeyboard.isKeyDown(Core.getInstance().getKey("Left"));
 bool boolean1 = GameKeyboard.isKeyDown(Core.getInstance().getKey("Right"));
 bool boolean2 = GameKeyboard.isKeyDown(Core.getInstance().getKey("Forward"));
 bool boolean3 = GameKeyboard.isKeyDown(Core.getInstance().getKey("Backward"));
 ...
 return vehicle.getController() == nullptr ? false : vehicle.getController().shouldBeActive();
 }
 }
 }
 ...
 LineDrawer.addLine(vehiclePoly.x1, vehiclePoly.y1, 0.0F, vehiclePoly.x2, vehiclePoly.y2, 0.0F, 1.0F, 1.0F, 1.0F, nullptr, true);
 LineDrawer.addLine(vehiclePoly.x2, vehiclePoly.y2, 0.0F, vehiclePoly.x3, vehiclePoly.y3, 0.0F, 1.0F, 1.0F, 1.0F, nullptr, true);
 LineDrawer.addLine(vehiclePoly.x3, vehiclePoly.y3, 0.0F, vehiclePoly.x4, vehiclePoly.y4, 0.0F, 1.0F, 1.0F, 1.0F, nullptr, true);
 LineDrawer.addLine(vehiclePoly.x4, vehiclePoly.y4, 0.0F, vehiclePoly.x1, vehiclePoly.y1, 0.0F, 1.0F, 1.0F, 1.0F, nullptr, true);
 ...
 LineDrawer.addLine(float0, float1, 0.0F, float0 + this->tempVec3f.x, float1 + this->tempVec3f.z, 0.0F, 1.0F, 1.0F, 1.0F, nullptr, true);
 ...
 void drawRect(Vector3f vec, float x, float y, float w, float h, float r, float g, float b) {
 float float0 = vec.x;
 float float1 = vec.y;
 float float2 = vec.z;
 ...
 LineDrawer.addLine(float8, float14, 0.0F, float9, float15, 0.0F, r, g, b, nullptr, true);
 LineDrawer.addLine(float8, float14, 0.0F, float10, float12, 0.0F, r, g, b, nullptr, true);
 LineDrawer.addLine(float9, float15, 0.0F, float11, float13, 0.0F, r, g, b, nullptr, true);
 LineDrawer.addLine(float10, float12, 0.0F, float11, float13, 0.0F, r, g, b, nullptr, true);
