...
public:
  ...
  SpriteRenderState m_ready = null;
  SpriteRenderState m_rendering = new SpriteRenderState(2);
  SpriteRenderState m_rendered = new SpriteRenderState(1);

  SpriteRenderState getPopulating() { return this.m_populating; }

  /**
   * Returns either the UI state, or populating state. Depends on the value of
   * its stateUI.bActive
   */
  ...
  SpriteRenderState getReady() { return this.m_ready; }

  void setReady(SpriteRenderState ready) { this.m_ready = ready; }

  SpriteRenderState getRendering() { return this.m_rendering; }

  /**
   * Returns either the UI state, or rendering state. Depends on the value of
   * its stateUI.bActive
   */
  ...
  SpriteRenderState getRendered() { return this.m_rendered; }

  void setRendered(SpriteRenderState rendered) { this.m_rendered = rendered; }
