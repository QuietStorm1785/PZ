...
public:
  const int index;
public
  TextureDraw[] sprite = new TextureDraw[2048];
public
  Style[] style = new Style[2048];
  int numSprites;
  TextureFBO fbo;
  bool bRendered;
  bool m_isRendering;
public
  final ArrayList<TextureDraw> postRender = std::make_unique<ArrayList<>>();
...
protected
  GenericSpriteRenderState(int int0) {
    this.index = int0;
  ...
  void onRenderAcquired() { this.m_isRendering = true; }

  bool isRendering() { return this.m_isRendering; }

  void onReady() { this.bRendered = false; }

  bool isReady() { return !this.bRendered; }

  bool isRendered() { return this.bRendered; }
  ...
  void drawSkyBox(Shader shader, int playerIndex, int apiId, int bufferId) {
    ...
    TextureDraw.drawSkyBox(this.sprite[this.numSprites], shader, playerIndex,
                           apiId, bufferId);
    this.style[this.numSprites] = this.defaultStyle;
    this.numSprites++;
  }
  ...
  void drawWater(Shader shader, int playerIndex, int apiId, bool bShore) {
    ...
    TextureDraw.drawWater(this.sprite[this.numSprites], shader, playerIndex,
                          apiId, bShore);
    this.style[this.numSprites] = this.defaultStyle;
    this.numSprites++;
  }
  ...
  void drawPuddles(Shader shader, int playerIndex, int apiId, int z) {
    ...
    TextureDraw.drawPuddles(this.sprite[this.numSprites], shader, playerIndex,
                            apiId, z);
    this.style[this.numSprites] = this.defaultStyle;
    this.numSprites++;
  }
  ...
  void drawParticles(int playerIndex, int var1, int var2) {
    ...
    TextureDraw.drawParticles(this.sprite[this.numSprites], playerIndex, var1,
                              var2);
    this.style[this.numSprites] = this.defaultStyle;
    this.numSprites++;
  }
...
public
  void render(Texture tex, double x1, double y1, double x2, double y2,
    ...
    this.render(tex, x1, y1, x2, y2, x3, y3, x4, y4, r, g, b, a, r, g, b, a, r,
                g, b, a, r, g, b, a, texdModifier);
  }
...
public
  void render(Texture tex, double x1, double y1, double x2, double y2,
    ...
    TextureDraw.Create(this.sprite[this.numSprites], tex, (float)x1, (float)y1,
                       (float)x2, (float)y2, (float)x3, (float)y3, (float)x4,
                       (float)y4, r1, g1, b1, a1, r2, g2, b2, a2, r3, g3, b3,
                       a3, r4, g4, b4, a4, texdModifier);
  ...
  void clearUseVertColorsArray() { this.useVertColorsArray = -1; }
...
public
  void renderdebug(Texture tex, float x1, float y1, float x2, float y2,
    ...
    this.sprite[this.numSprites].reset();
    TextureDraw.Create(this.sprite[this.numSprites], tex, x1, y1, x2, y2, x3,
                       y3, x4, y4, r1, g1, b1, a1, r2, g2, b2, a2, r3, g3, b3,
                       a3, r4, g4, b4, a4, texdModifier);
    this.style[this.numSprites] = this.defaultStyle;
    this.numSprites++;
  }

  void renderline(Texture tex, float x1, float y1, float x2, float y2, float r,
                  float g, float b, float a, int thickness) {
    if (this.numSprites == this.sprite.length) {
      this.CheckSpriteSlots();
    }
    ...
    this.sprite[this.numSprites].reset();
    if (x1 <= x2 && y1 <= y2) {
      TextureDraw.Create(this.sprite[this.numSprites], tex, x1 + thickness,
                         y1 - thickness, x2 + thickness, y2 - thickness,
                         x2 - thickness, y2 + thickness, x1 - thickness,
                         y1 + thickness, r, g, b, a);
    } else if (x1 >= x2 && y1 >= y2) {
      TextureDraw.Create(this.sprite[this.numSprites], tex, x1 + thickness,
                         y1 - thickness, x1 - thickness, y1 + thickness,
                         x2 - thickness, y2 + thickness, x2 + thickness,
                         y2 - thickness, r, g, b, a);
    } else if (x1 >= x2 && y1 <= y2) {
      TextureDraw.Create(this.sprite[this.numSprites], tex, x2 - thickness,
                         y2 - thickness, x1 - thickness, y1 - thickness,
                         x1 + thickness, y1 + thickness, x2 + thickness,
                         y2 + thickness, r, g, b, a);
    } else if (x1 <= x2 && y1 >= y2) {
      TextureDraw.Create(this.sprite[this.numSprites], tex, x1 - thickness,
                         y1 - thickness, x1 + thickness, y1 + thickness,
      ...
      if (a != 0.0F) {
        ...
        if (this.wallShaderTexRender == nullptr) {
          textureDraw = TextureDraw.Create(
              this.sprite[this.numSprites], tex, x, y, float0, y, float0,
              float1, x, float1, int0, int0, int0, int0, texdModifier);
        } else {
          textureDraw = TextureDraw.Create(
              this.sprite[this.numSprites], tex, this.wallShaderTexRender, x, y,
              float0 - x, float1 - y, r, g, b, a, texdModifier);
        }
        ...
        if (this.texture1_cutaway != nullptr) {
          ...
          float float2 =
              this.texture1_cutaway.xEnd - this.texture1_cutaway.xStart;
          float float3 =
              this.texture1_cutaway.yEnd - this.texture1_cutaway.yStart;
          float float4 =
              (float)this.texture1_cutaway_x / this.texture1_cutaway.getWidth();
          float float5 =
              (float)(this.texture1_cutaway_x + this.texture1_cutaway_w) /
              this.texture1_cutaway.getWidth();
          float float6 = (float)this.texture1_cutaway_y /
                         this.texture1_cutaway.getHeight();
          float float7 =
              (float)(this.texture1_cutaway_y + this.texture1_cutaway_h) /
              this.texture1_cutaway.getHeight();
          textureDraw.tex1_u0 = textureDraw.tex1_u3 =
              this.texture1_cutaway.xStart + float4 * float2;
          textureDraw.tex1_v0 = textureDraw.tex1_v1 =
              this.texture1_cutaway.yStart + float6 * float3;
          textureDraw.tex1_u1 = textureDraw.tex1_u2 =
              this.texture1_cutaway.xStart + float5 * float2;
          textureDraw.tex1_v2 = textureDraw.tex1_v3 =
              this.texture1_cutaway.yStart + float7 * float3;
        }
  ...
  void renderRect(int x, int y, int width, int height, float r, float g,
    ...
    if (a != 0.0F) {
      ...
      this.sprite[this.numSprites].reset();
      TextureDraw.Create(this.sprite[this.numSprites], nullptr, x, y, width,
                         height, r, g, b, a, nullptr);
      this.style[this.numSprites] = this.defaultStyle;
      this.numSprites++;
    }
  }
  ...
  void renderPoly(float x1, float y1, float x2, float y2, float x3, float y3,
    ...
    this.sprite[this.numSprites].reset();
    TextureDraw.Create(this.sprite[this.numSprites], nullptr, x1, y1, x2, y2,
                       x3, y3, x4, y4, r, g, b, a);
    this.style[this.numSprites] = this.defaultStyle;
    this.numSprites++;
  }
...
public
  void render(Texture tex, float x, float y, float width, float height, float r,
    ...
    if (a != 0.0F) {
      ...
      this.sprite[this.numSprites].reset();
      TextureDraw.Create(this.sprite[this.numSprites], tex, x, y, width, height,
                         r, g, b, a, u1, v1, u2, v2, u3, v3, u4, v4,
                         texdModifier);
      this.style[this.numSprites] = this.defaultStyle;
      this.numSprites++;
    }
  ...
  void glDoStartFrame(int w, int h, float zoom, int player, bool isTextFrame) {
    ...
    TextureDraw.glDoStartFrame(this.sprite[this.numSprites], w, h, zoom, player,
                               isTextFrame);
    this.style[this.numSprites] = TransparentStyle.instance;
    this.numSprites++;
  }
  ...
  void clearCutawayTexture() { this.texture1_cutaway = nullptr; }
  ...
  void ShaderUpdate1i(int shaderID, int uniform, int uniformValue) {
    ...
    TextureDraw.ShaderUpdate1i(this.sprite[this.numSprites], shaderID, uniform,
                               uniformValue);
    this.style[this.numSprites] = TransparentStyle.instance;
    this.numSprites++;
  }
  ...
  void ShaderUpdate1f(int shaderID, int uniform, float uniformValue) {
    ...
    TextureDraw.ShaderUpdate1f(this.sprite[this.numSprites], shaderID, uniform,
                               uniformValue);
    this.style[this.numSprites] = TransparentStyle.instance;
    this.numSprites++;
  }
  ...
  void ShaderUpdate2f(int shaderID, int uniform, float value1, float value2) {
    ...
    TextureDraw.ShaderUpdate2f(this.sprite[this.numSprites], shaderID, uniform,
                               value1, value2);
    this.style[this.numSprites] = TransparentStyle.instance;
    this.numSprites++;
  }
  ...
  void ShaderUpdate3f(int shaderID, int uniform, float value1, float value2,
    ...
    TextureDraw.ShaderUpdate3f(this.sprite[this.numSprites], shaderID, uniform,
  ...
  void ShaderUpdate4f(int shaderID, int uniform, float value1, float value2,
    ...
    TextureDraw.ShaderUpdate4f(this.sprite[this.numSprites], shaderID, uniform,
  ...
  void StartShader(int iD, int playerIndex) {
    ...
    if (iD != 0 && Shader.ShaderMap.containsKey(iD)) {
      Shader.ShaderMap.get(iD).startMainThread(this.sprite[this.numSprites],
                                               playerIndex);
      std::vector arrayList = this.postRender;
      arrayList.add(this.sprite[this.numSprites]);
    }
