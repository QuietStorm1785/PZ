...
public:
public
  static HashMap<String, Pcx> Cache = std::make_unique<HashMap<>>();
public
  byte[] imageData;
  int imageWidth;
  int imageHeight;
...
public
  ...
  Image getImage() {
    ...
    for (int int2 = 0; int2 < this.imageWidth; int2++) {
      for (int int3 = 0; int3 < this.imageHeight; int3++) {
        ints[int0++] = 0xFF000000 | (this.imageData[int1++] & 255) << 16 |
                       (this.imageData[int1++] & 255) << 8 |
                       this.imageData[int1++] & 255;
      }
    }

    Toolkit toolkit = Toolkit.getDefaultToolkit();
    return toolkit.createImage(new MemoryImageSource(
        this.imageWidth, this.imageHeight, ints, 0, this.imageWidth));
  }
    ...
    try {
      ...
      if (int0 == -1) {
        ...
        if (pcx_t.manufacturer == '\n' && pcx_t.version == 5 &&
            pcx_t.encoding == 1 && pcx_t.bits_per_pixel == '\b' &&
            pcx_t.xmax < 640 && pcx_t.ymax < 480) {
          ...
          if (this.pic != nullptr && this.palette != nullptr) {
            this.imageData =
                new byte[(this.imageWidth + 1) * (this.imageHeight + 1) * 3];

            for (int int8 = 0; int8 < this.imageWidth * this.imageHeight;
                 int8++) {
              this.imageData[int8 * 3] = (byte)this.palette[this.pic[int8] * 3];
              this.imageData[int8 * 3 + 1] =
                  (byte)this.palette[this.pic[int8] * 3 + 1];
              this.imageData[int8 * 3 + 2] =
                  (byte)this.palette[this.pic[int8] * 3 + 2];
            }
  ...
  int loadPCXminusPal(const std::string &string) {
    try {
      if (Cache.containsKey(string)) {
        ...
        this.imageData =
            new byte[(pcx0.imageWidth + 1) * (pcx0.imageHeight + 1) * 3];

        for (int int0 = 0; int0 < pcx0.imageWidth * pcx0.imageHeight; int0++) {
          this.imageData[int0 * 3] = (byte)this.palette[pcx0.pic[int0] * 3];
          this.imageData[int0 * 3 + 1] =
              (byte)this.palette[pcx0.pic[int0] * 3 + 1];
          this.imageData[int0 * 3 + 2] =
              (byte)this.palette[pcx0.pic[int0] * 3 + 2];
        }
        ...
        if (inputStream == nullptr) {
          ...
          if (int1 == -1) {
            ...
            if (pcx_t.manufacturer == '\n' && pcx_t.version == 5 &&
              ...
              if (this.pic != nullptr && this.palette != nullptr) {
                this.imageData = new byte[(this.imageWidth + 1) *
                                          (this.imageHeight + 1) * 3];

                for (int int8 = 0; int8 < this.imageWidth * this.imageHeight;
                     int8++) {
                  this.imageData[int8 * 3] =
                      (byte)this.palette[this.pic[int8] * 3];
                  this.imageData[int8 * 3 + 1] =
                      (byte)this.palette[this.pic[int8] * 3 + 1];
                  this.imageData[int8 * 3 + 2] =
                      (byte)this.palette[this.pic[int8] * 3 + 2];
  ...
  void loadPCXpal(int[] ints) { this.palette = ints; }
