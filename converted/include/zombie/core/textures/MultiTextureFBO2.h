...
public:
 private float[] zoomLevelsDefault = new float[]{2.5F, 2.25F, 2.0F, 1.75F, 1.5F, 1.25F, 1.0F, 0.75F, 0.5F};
 private float[] zoomLevels;
 TextureFBO Current;
 public TextureFBO FBOrendered = nullptr;
 ...
 for (auto& string : strings) if (!string.empty()) {
 try {
 ...
 if (this->bAutoZoom[int0] && IsoCamera.CamCharacter != nullptr && this->bZoomEnabled) {
 ...
 if (IsoPlayer.players[int2] != nullptr || GameServer.bServer && ServerGUI.isCreated()) {
 ((Texture)this->Current.getTexture()).rendershader2(int3, int4, int5, int6, int3, int4, int5, int6, 1.0F, 1.0F, 1.0F, 1.0F);
 } else {
 SpriteRenderer.instance.renderi(nullptr, int3, int4, int5, int6, 0.0F, 0.0F, 0.0F, 1.0F, nullptr);
 }
 }
 ...
 TextureFBO getCurrent(int nPlayer) {
 return this->Current;
 }

 Texture getTexture(int nPlayer) {
