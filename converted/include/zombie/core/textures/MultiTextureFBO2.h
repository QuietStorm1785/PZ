...
public:
private
  final float[] zoomLevelsDefault =
      new float[]{2.5F, 2.25F, 2.0F, 1.75F, 1.5F, 1.25F, 1.0F, 0.75F, 0.5F};
private
  float[] zoomLevels;
  TextureFBO Current;
public
  volatile TextureFBO FBOrendered = nullptr;
public
  final float[] zoom = new float[4];
public
  final float[] targetZoom = new float[4];
public
  final float[] startZoom = new float[4];
  float zoomedInLevel;
  float zoomedOutLevel;
public
  final boolean[] bAutoZoom = new boolean[4];
...
public
  ...
  void setZoomLevelsFromOption(const std::string &levels) {
    ...
    if (levels != nullptr && !levels.isEmpty()) {
