...
public:
  ...
  SmartShader shader;
public
  ArrayList<TextureCombinerShaderParam> shaderParams = nullptr;
public
  static final ObjectPool<TextureCombinerCommand> pool =
      new ObjectPool<>(TextureCombinerCommand::new);

  std::string toString() {
    std::string string = System.lineSeparator();
    return "{" + string + "\tpos: " + this.x + "," + this.y + string +
           "\tsize: " + this.w + "," + this.h + string + "\tmask:" + this.mask +
           string + "\ttex:" + this.tex + string +
           "\tblendSrc:" + this.blendSrc + string +
           "\tblendDest:" + this.blendDest + string +
           "\tblendSrcA:" + this.blendSrcA + string +
           "\tblendDestA:" + this.blendDestA + string +
           "\tshader:" + this.shader + string +
           "\tshaderParams:" + PZArrayUtil.arrayToString(this.shaderParams) +
           string + "}";
  }
  ...
  TextureCombinerCommand initSeparate(Texture texture, int int0, int int1,
                                      int int2, int int3) {
    this.tex = this.requireNonNull(texture);
    this.blendSrc = int0;
    this.blendDest = int1;
  ...
  TextureCombinerCommand init(Texture texture0, SmartShader smartShader,
  ...
  TextureCombinerCommand init(Texture texture, int int0, int int1, int int2,
                              int int3) {
    this.tex = this.requireNonNull(texture);
    this.x = int0;
    this.y = int1;
...
public
  ...
  initSeparate(Texture texture0, SmartShader smartShader,
               ArrayList<TextureCombinerShaderParam> arrayList,
               Texture texture1, int int0, int int1, int int2, int int3) {
    this.tex = this.requireNonNull(texture0);
    this.shader = smartShader;
    this.blendSrc = int0;
    this.blendDest = int1;
...
public
  TextureCombinerCommand init(Texture texture0, SmartShader smartShader,
                              ArrayList<TextureCombinerShaderParam> arrayList,
                              Texture texture1, int int0, int int1) {
    return this.initSeparate(texture0, smartShader, arrayList, texture1, int0,
                             int1, 1, 771);
  }
