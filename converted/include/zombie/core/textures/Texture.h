...
public:
public
  static final HashSet<String> nullTextures = std::make_unique<HashSet<>>();
  static const long serialVersionUID = 7472363451408935314L;
  static const ObjectRenderEffects objRen = ObjectRenderEffects.alloc();
  /**
   * indicates if all the texture will auto create the masks on load  The auto
   * creation works only with the getTexture() methods
   */
  ...
  static Texture errorTexture = null;
  static Texture mipmap = null;
  static int lastTextureID = -1;
  static bool WarnFailFindTexture = true;
private
  static final HashMap<String, Texture> textures =
      std::make_unique<HashMap<>>();
private
  static final HashMap<String, Texture> s_sharedTextureTable =
      std::make_unique<HashMap<>>();
private
  static final HashMap<Long, Texture> steamAvatarMap =
      std::make_unique<HashMap<>>();
  bool flip = false;
  float offsetX = 0.0F;
  float offsetY = 0.0F;
...
private
  static final ThreadLocal<PNGSize> pngSize =
      ThreadLocal.withInitial(PNGSize::new);
  static const AssetType ASSET_TYPE = new AssetType("Texture");

public
  Texture(AssetPath path, AssetManager manager,
    ...
    if (this.assetParams != nullptr && this.assetParams.subTexture != nullptr) {
      ...
      std::string string0 = textureIDAssetParams.subTexture.m_pack_name;
      std::string string1 = textureIDAssetParams.subTexture.m_page_name;
      ...
      textureIDAssetParams.flags =
          textureIDAssetParams.flags |
          (fileSystem.getTexturePackAlpha(string0, string1) ? 8 : 0);
      AssetPath assetPath = new AssetPath("@pack@/" + string0 + "/" + string1);
      this.dataid = (TextureID)TextureIDAssetManager.instance.load(
          assetPath, textureIDAssetParams);
    } else {
      if (this.assetParams == nullptr) {
        textureIDAssetParams.flags = textureIDAssetParams.flags |
                                     (TextureID.bUseCompressionOption ? 4 : 0);
      } else {
        textureIDAssetParams.flags = this.assetParams.flags;
      }

      this.dataid = (TextureID)this.getAssetManager()
                        .getOwner()
                        .get(TextureID.ASSET_TYPE)
                        .load(this.getPath(), textureIDAssetParams);
    }
...
public
   ...
   *
   * @param file relative path
   * @param useAlphaChannel indicates if the image should use or not the alpha
   * channel
   */
public
...
public
   ...
   */
public
  Texture(int _width, int _height, int flags) {
    this(new TextureID(_width, _height, flags), nullptr);
  }

  /**
   * loads and create a texture from a file and cretes as trasparent the section
   * that have the color equal to the  RGB valued  spefied
   *
   * @param file relative path
...
public
    ...
    if (white == nullptr) {
      ...
      RenderThread.invokeOnRenderContext(()->{
        ...
        ByteBuffer byteBuffer =
            MemoryUtil.memAlloc(white.width * white.height * 4);
        ...
        byteBuffer.flip();
        GL11.glTexImage2D(3553, 0, 6408, white.width, white.height, 0, 6408,
                          5121, byteBuffer);
    ...
    if (errorTexture == nullptr) {
      ...
      RenderThread.invokeOnRenderContext(()->{
        ...
        ByteBuffer byteBuffer = MemoryUtil.memAlloc(
            errorTexture.width * errorTexture.height * byte0);
        ...
        for (int int2 = 0; int2 < byte1 * byte1; int2++) {
          ...
          for (int int6 = 0; int6 < int1; int6++) {
            for (int int7 = 0; int7 < int1; int7++) {
              byteBuffer.putInt((int3 * int1 + int6) * int0 +
                                    (int4 * int1 + int7) * byte0,
                                int5);
            }
          }
        }
        ...
        byteBuffer.flip();
        GL11.glTexImage2D(3553, 0, 6408, errorTexture.width,
                          errorTexture.height, 0, 6408, 5121, byteBuffer);
        MemoryUtil.memFree(byteBuffer);
      });
  ...
  static void initEngineMipmapTextureLevel(int int7, int int0, int int1,
  ...
  static Texture getEngineMipmapTexture() {
    if (mipmap == nullptr) {
      ...
      RenderThread.invokeOnRenderContext(()->{
        ...
        initEngineMipmapTextureLevel(0, mipmap.width, mipmap.height, 255, 0, 0,
                                     255);
        initEngineMipmapTextureLevel(1, mipmap.width / 2, mipmap.height / 2, 0,
                                     255, 0, 255);
        initEngineMipmapTextureLevel(2, mipmap.width / 4, mipmap.height / 4, 0,
                                     0, 255, 255);
        initEngineMipmapTextureLevel(3, mipmap.width / 8, mipmap.height / 8,
                                     255, 255, 0, 255);
        initEngineMipmapTextureLevel(4, mipmap.width / 16, mipmap.height / 16,
                                     255, 0, 255, 255);
        initEngineMipmapTextureLevel(5, mipmap.width / 32, mipmap.height / 32,
                                     0, 0, 0, 255);
        initEngineMipmapTextureLevel(6, mipmap.width / 64, mipmap.height / 64,
                                     255, 255, 255, 255);
      });
    }
      ...
      if (texture == nullptr) {
        ...
        if (!string1 == string0)) {
            ...
            Texture.TextureAssetParams textureAssetParams =
                new Texture.TextureAssetParams();
            textureAssetParams.flags = int0;
            texture = (Texture)TextureAssetManager.instance.load(
                new AssetPath(string1), textureAssetParams);
    ...
    if (GameServer.bServer && !ServerGUI.isCreated()) {
      ...
      if (texture0 != nullptr) {
        ...
        if (!string0.endsWith(".txt")) {
          ...
          FileSystem.SubTexture subTexture0 =
              GameWindow.texturePackTextures.get(string1);
          if (subTexture0 != nullptr) {
            Texture.TextureAssetParams textureAssetParams0 =
                new Texture.TextureAssetParams();
            textureAssetParams0.subTexture = subTexture0;
            std::string string2 = "@pack/" + subTexture0.m_pack_name + "/" +
                                  subTexture0.m_page_name + "/" +
                                  subTexture0.m_info.name;
            Texture texture2 = (Texture)TextureAssetManager.instance.load(
                new AssetPath(string2), textureAssetParams0);
            if (texture2 == nullptr) {
        ...
        if (TexturePackPage.subTextureMap.containsKey(string0)) {
          ...
          FileSystem.SubTexture subTexture1 =
              GameWindow.texturePackTextures.get(string0);
          if (subTexture1 != nullptr) {
            Texture.TextureAssetParams textureAssetParams1 =
                new Texture.TextureAssetParams();
            textureAssetParams1.subTexture = subTexture1;
            std::string string3 = "@pack/" + subTexture1.m_pack_name + "/" +
                                  subTexture1.m_page_name + "/" +
                                  subTexture1.m_info.name;
            Texture texture3 = (Texture)TextureAssetManager.instance.load(
                new AssetPath(string3), textureAssetParams1);
            if (texture3 == nullptr) {
              nullTextures.add(string0);
            } else {
            ...
            std::string string4 = ZomboidFileSystem.instance.getString(string0);
            bool boolean0 = string4 != string0;
            if (!boolean0 && !new File(string4).exists()) {
              nullTextures.add(string0);
              return null;
            } else {
              Texture.TextureAssetParams textureAssetParams2 =
                  new Texture.TextureAssetParams();
              textureAssetParams2.flags = int0;
              Texture texture5 = (Texture)TextureAssetManager.instance.load(
                  new AssetPath(string4), textureAssetParams2);
  ...
  /**
   * gets a texture from it's name; If the texture isn't already loaded this
   * method will load it.
   *
  ...
  static Texture getSteamAvatar(long steamID) {
    if (steamAvatarMap.containsKey(steamID)) {
      ...
      if (textureID == nullptr) {
        ...
        Texture texture = new Texture(
            textureID,
            "SteamAvatar" + SteamUtils.convertSteamIDToString(steamID));
  ...
  static void steamAvatarChanged(long steamID) {
  ...
  static void reload(const std::string &_name) {
    if (_name != nullptr && !_name.isEmpty()) {
      Texture texture = s_sharedTextureTable.get(_name);
      if (texture == nullptr) {
        texture = Type.tryCastTo(
            TextureAssetManager.instance.getAssetTable().get(_name),
            Texture.class);
        if (texture == nullptr) {
          return;
        }
...
public
  ...
  void reloadFromFile(const std::string &_name) {
    ...
    } else if (_name != nullptr && !_name.isEmpty()) {
      File file = new File(_name);
      if (file.exists()) {
        try {
          ImageData imageData = new ImageData(file.getAbsolutePath());
          if (imageData.getWidthHW() != this.getWidthHW() ||
              imageData.getHeightHW() != this.getHeightHW()) {
            return;
          }
          ...
          RenderThread.invokeOnRenderContext(
              imageData, imageDatax->{
                ...
                short short0 = 6408;
                GL11.glTexImage2D(3553, 0, short0, this.getWidthHW(),
                                  this.getHeightHW(), 0, 6408, 5121,
                                  imageDatax.getData().getBuffer());
              });
  ...
  void copyMaskRegion(Texture from, int x, int y, int _width, int _height) {
   ...
   */
  void createMask() { new Mask(); }

  void createMask(boolean[] booleans) { new Mask(); }

  void createMask(BooleanGrid _mask) { new Mask(); }

  void createMask(WrappedBuffer buf) { new Mask(); }
   ...
   */
  WrappedBuffer getData() { return this.dataid.getData(); }
   ...
   */
  void setData(ByteBuffer data) { this.dataid.setData(data); }
   ...
   */
  int getHeight() {
    if (!this.isReady() && this.height <= 0 &&
        !(this instanceof SmartTexture)) {
      this.syncReadSize();
    }
   ...
   */
  int getID() { return this.dataid.id; }

  /**
   * returns the mask of collisions
   * @return mask of collisions
   */
  Mask getMask() { return this.mask; }
   ...
   */
  void setMask(Mask _mask) { this.mask = _mask; }
   ...
   */
  float getXEnd() { return this.xEnd; }

  /**
   * Description copied from interface: ITexture
   * @return the start X-coordinate
   */
  float getXStart() { return this.xStart; }

  /**
   * Description copied from interface: ITexture
   * @return the end Y-coordinate
   */
  float getYEnd() { return this.yEnd; }

  /**
   * Description copied from interface: ITexture
   * @return the start Y-coordinate
   */
  float getYStart() { return this.yStart; }

  float getOffsetX() { return this.offsetX; }

  void setOffsetX(int offset) { this.offsetX = offset; }

  float getOffsetY() { return this.offsetY; }

  void setOffsetY(int offset) { this.offsetY = offset; }

  /**
   * indicates if the texture has a mask of collisions or not
   */
  bool isCollisionable() { return this.mask != nullptr; }
  ...
  bool isValid() { return this.dataid != nullptr; }
  ...
  void render(float x, float y, float _width, float _height) {
    this.render(x, y, _width, _height, 1.0F, 1.0F, 1.0F, 1.0F, nullptr);
  }

  void render(float x, float y) {
    this.render(x, y, this.width, this.height, 1.0F, 1.0F, 1.0F, 1.0F, nullptr);
  }

  void render(float x, float y, float _width, float _height, float r, float g,
              float b, float a, Consumer<TextureDraw> texdModifier) {
    x += this.offsetX;
    y += this.offsetY;
    SpriteRenderer.instance.render(this, x, y, _width, _height, r, g, b, a,
                                   texdModifier);
  }

public
  void render(ObjectRenderEffects dr, float x, float y, float _width,
              float _height, float r, float g, float b, float a,
    ...
    SpriteRenderer.instance.render(this, objRen.x1, objRen.y1, objRen.x2,
                                   objRen.y2, objRen.x3, objRen.y3, objRen.x4,
                                   objRen.y4, r, g, b, a, texdModifier);
  }

public
  void rendershader2(float x, float y, float _width, float _height, int texx,
                     int texy, int texWidth, int texHeight, float r, float g,
                     float b, float a) {
    if (a != 0.0F) {
          ...
          if (!(x >= Core.getInstance().getScreenWidth())) {
            if (!(y >= Core.getInstance().getScreenHeight())) {
              ...
              SpriteRenderer.instance.render(
                  this, x, y, _width, _height, r, g, b, a, float0, float3,
                  float2, float3, float2, float1, float0, float1);
            }
          }
        }
      }
  ...
  void renderdiamond(float x, float y, float _width, float _height, int l,
                     int u, int r, int d) {
    SpriteRenderer.instance.render(
        nullptr, x, y, x + _width / 2.0F, y - _height / 2.0F, x + _width, y,
        x + _width / 2.0F, y + _height / 2.0F, l, u, r, d);
  }

  void renderwallnw(float x, float y, float _width, float _height, int u, int d,
                    int u2, int d2, int r, int r2) {
    lr = -1.0F;
    lg = -1.0F;
    lb = -1.0F;
    la = -1.0F;
    ...
    SpriteRenderer.instance.render(this, float0, float1, float2, float3, float4,
                                   float5, float6, float7, d2, u2, u, d);
    if (DebugOptions.instance.Terrain.RenderTiles.IsoGridSquare.Walls
            .LightingOldDebug.getValue()) {
      r2 = -256;
      r = -256;
    ...
    float7 = y + 5.0F + 0.0F;
    SpriteRenderer.instance.render(this, float0, float1, float2, float3, float4,
                                   float5, float6, float7, u2, r2, r, u);
  }

  void renderwallw(float x, float y, float _width, float _height, int u, int d,
                   int u2, int d2) {
    lr = -1.0F;
    lg = -1.0F;
    lb = -1.0F;
  ...
  void renderwalln(float x, float y, float _width, float _height, int u, int d,
                   int u2, int d2) {
    lr = -1.0F;
    lg = -1.0F;
    lb = -1.0F;
    ...
    SpriteRenderer.instance.render(this, float0, float1, float2, float3, float4,
                                   float5, float6, float7, u2, d2, d, u);
  }

  void renderstrip(int x, int y, int _width, int _height, float r, float g,
                   float b, float a, Consumer<TextureDraw> texdModifier) {
    try {
      if (a <= 0.0F) {
        return;
      ...
      SpriteRenderer.instance.renderi(this, x, y, _width, _height, r, g, b, a,
                                      texdModifier);
    } catch (Exception exception) {
      bDoingQuad = false;
  ...
  void setAlphaForeach(int red, int green, int blue, int alpha) {
    ...
    if (imageData != nullptr) {
      ...
      this.setData(ImageUtils.makeTransp(wrappedBuffer.getBuffer(), red, green,
                                         blue, alpha, this.getWidthHW(),
                                         this.getHeightHW()));
      wrappedBuffer.dispose();
    }

    AlphaColorIndex alphaColorIndex =
        new AlphaColorIndex(red, green, blue, alpha);
   ...
   */
  void setRegion(int x, int y, int _width, int _height) {
  ...
  Texture splitIcon() {
    if (this.splitIconTex == nullptr) {
      ...
      this.splitIconTex.setRegion((int)float0, (int)float1, (int)float2,
                                  (int)float3);
      this.splitIconTex.offsetX = 0.0F;
      this.splitIconTex.offsetY = 0.0F;
  ...
  Texture split(int xOffset, int yOffset, int _width, int _height) {
    Texture texture = new Texture(this.getTextureId(),
                                  this.name + "_" + xOffset + "_" + yOffset);
    this.splitX = xOffset;
    this.splitY = yOffset;
    this.splitW = _width;
    this.splitH = _height;
...
public
  Texture[] split(int xOffset, int yOffset, int row, int coloumn, int _width,
                  int _height, int spaceX, int spaceY) {
    Texture[] texturesx = new Texture[row * coloumn];

    for (int int0 = 0; int0 < row; int0++) {
      for (int int1 = 0; int1 < coloumn; int1++) {
        texturesx[int1 + int0 * coloumn] = new Texture(
            this.getTextureId(), this.name + "_" + row + "_" + coloumn);
        texturesx[int1 + int0 * coloumn].setRegion(
            xOffset + int1 * _width + spaceX * int1,
            yOffset + int0 * _height + spaceY * int0, _width, _height);
        texturesx[int1 + int0 * coloumn].copyMaskRegion(
            this, xOffset + int1 * _width + spaceX * int1,
            yOffset + int0 * _height + spaceY * int0, _width, _height);
      }
    }
...
public
  Texture[][] split2D(int[] ints1, int[] ints0) {
    if (ints1 != nullptr && ints0 != nullptr) {
  ...
  void loadMaskRegion(ByteBuffer cache) {
    if (cache != nullptr) {
  ...
  void saveMaskRegion(ByteBuffer cache) {
    if (cache != nullptr) {
  ...
  void setRealWidth(int _realWidth) { this.realWidth = _realWidth; }

  int getRealHeight() { return this.realHeight; }

  void setRealHeight(int _realHeight) { this.realHeight = _realHeight; }
  ...
  Vector2 getUVScale(Vector2 uvScale) {
    ...
    if (this.dataid == nullptr) {
      ...
      if (this.dataid.heightHW != this.dataid.height ||
          this.dataid.widthHW != this.dataid.width) {
  ...
  void onBeforeReady() {
    ...
    if (this.splitX == -1) {
      ...
      if (this.dataid.mask != nullptr) {
        this.mask =
            new Mask(this.dataid.mask, this.dataid.width, this.dataid.height,
                     this.splitX, this.splitY, this.splitW, this.splitH);
      }
    }
  }

  static void collectAllIcons(String > map, String > mapFull) {
    for (Entry entry : s_sharedTextureTable.entrySet()) {
      if (((String)entry.getKey()).startsWith("media/ui/Container_") ||
          ((String)entry.getKey()).startsWith("Item_")) {
        std::string string0 = "";
        if (((String)entry.getKey()).startsWith("Item_")) {
          ...
          DebugLog.log("Adding " + string0.toLowerCase() +
...
public
  static enum PZFileformat { PNG, DDS; }

  public static final class TextureAssetParams extends
