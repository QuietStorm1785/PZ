...
private
  final ArrayList<TextureCombiner.CombinerFBO> fboPool =
  ...
  void combineEnd() { GL13.glActiveTexture(33984); }
  ...
  Texture combine(Texture texture1, Texture texture0) {
    Core.getInstance().DoStartFrameStuff(texture1.width, texture0.width, 1.0F,
                                         0);
...
private
  TextureCombiner.CombinerFBO getFBO(int int2, int int1) {
    for (int int0 = 0; int0 < this.fboPool.size(); int0++) {
      TextureCombiner.CombinerFBO combinerFBO = this.fboPool.get(int0);
      if (combinerFBO.fbo.getWidth() == int2 &&
          combinerFBO.fbo.getHeight() == int1) {
        return combinerFBO;
      }
    }
  ...
  Texture createTexture(int int0, int int1) {
    ...
    if (combinerFBO == nullptr) {
      ...
      texture = combinerFBO.textures.isEmpty() ? new Texture(int0, int1, 16)
                                               : combinerFBO.textures.pop();
      texture.bind();
      GL11.glTexImage2D(3553, 0, 6408, texture.getWidthHW(),
                        texture.getHeightHW(), 0, 6408, 5121,
                        (IntBuffer) nullptr);
  ...
  void releaseTexture(Texture texture) {
    TextureCombiner.CombinerFBO combinerFBO =
        this.getFBO(texture.getWidth(), texture.getHeight());
  ...
  Texture combine(ArrayList<TextureCombinerCommand> arrayList0) {
    ...
    PZGLUtil.checkGLErrorThrow("Core.DoStartFrameStuffFx w:%d, h:%d", int0,
                               int1);

    for (int int2 = 0; int2 < arrayList0.size(); int2++) {
      TextureCombinerCommand textureCombinerCommand =
          (TextureCombinerCommand)arrayList0.get(int2);
      ...
      GL13.glActiveTexture(33984);
      GL11.glEnable(3553);
      Texture texture1 = textureCombinerCommand.tex == null
      ...
      if (textureCombinerCommand.shader != nullptr) {
        if (textureCombinerCommand.shaderParams != nullptr) {
          ...
          for (int int4 = 0; int4 < arrayList1.size(); int4++) {
            TextureCombinerShaderParam textureCombinerShaderParam =
                (TextureCombinerShaderParam)arrayList1.get(int4);
            float float0 = Rand.Next(textureCombinerShaderParam.min,
                                     textureCombinerShaderParam.max);
            textureCombinerCommand.shader.setValue(
                textureCombinerShaderParam.name, float0);
          }
        }
        ...
        if (textureCombinerCommand.mask != nullptr) {
          textureCombinerCommand.shader.setValue(
              "MASK", textureCombinerCommand.mask, 1);
        }
      }

      GL14.glBlendFuncSeparate(
          textureCombinerCommand.blendSrc, textureCombinerCommand.blendDest,
          textureCombinerCommand.blendSrcA, textureCombinerCommand.blendDestA);
      if (textureCombinerCommand.x != -1) {
        ...
        GL13.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
        GL13.glTexCoord2f(0.0F, 1.0F);
        GL13.glVertex2d(textureCombinerCommand.x * float1,
                        textureCombinerCommand.y * float2);
        GL13.glTexCoord2f(0.0F, 0.0F);
        GL13.glVertex2d(textureCombinerCommand.x * float1,
                        (textureCombinerCommand.y + textureCombinerCommand.h) *
                            float2);
        GL13.glTexCoord2f(1.0F, 0.0F);
        GL13.glVertex2d(
            (textureCombinerCommand.x + textureCombinerCommand.w) * float1,
            (textureCombinerCommand.y + textureCombinerCommand.h) * float2);
        GL13.glTexCoord2f(1.0F, 1.0F);
        GL13.glVertex2d((textureCombinerCommand.x + textureCombinerCommand.w) *
                            float1,
                        textureCombinerCommand.y * float2);
        GL13.glEnd();
      } else {
      ...
      PZGLUtil.checkGLErrorThrow("TextureCombinerCommand[%d}: %s", int2,
    ...
    if (arrayList.isEmpty()) {
      ...
      TextureCombinerCommand textureCombinerCommand = findDominantCommand(
          ...
          Comparator.comparingInt(
              textureCombinerCommandx->textureCombinerCommandx.tex.height));
  ...
  static int getResultingWidth(ArrayList<TextureCombinerCommand> arrayList) {
    if (arrayList.isEmpty()) {
      ...
      TextureCombinerCommand textureCombinerCommand = findDominantCommand(
          arrayList,
          Comparator.comparingInt(
              textureCombinerCommandx->textureCombinerCommandx.tex.width));
      if (textureCombinerCommand == nullptr) {
        return 32;
      } else {
        Texture texture = textureCombinerCommand.tex;
  ...
  static TextureCombinerCommand
  findDominantCommand(ArrayList<TextureCombinerCommand> arrayList,
                      Comparator<TextureCombinerCommand> comparator) {
    TextureCombinerCommand textureCombinerCommand0 = null;
    int int0 = arrayList.size();

    for (int int1 = 0; int1 < int0; int1++) {
      TextureCombinerCommand textureCombinerCommand1 =
          (TextureCombinerCommand)arrayList.get(int1);
      if (textureCombinerCommand1.tex != nullptr &&
          (textureCombinerCommand0 == nullptr ||
           comparator.compare(textureCombinerCommand1,
                              textureCombinerCommand0) > 0)) {
...
private
  static final class CombinerFBO {
    TextureFBO fbo;
    final ArrayDeque<Texture> textures = std::make_unique<ArrayDeque<>>();
  }
}
} // namespace textures
} // namespace core
