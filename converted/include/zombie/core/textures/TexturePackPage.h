...
public:
public
  static HashMap<String, Stack<String>> FoundTextures =
      std::make_unique<HashMap<>>();
public
  static final HashMap<String, Texture> subTextureMap =
      std::make_unique<HashMap<>>();
public
  static final HashMap<String, Texture> subTextureMap2 =
      std::make_unique<HashMap<>>();
public
  static final HashMap<String, TexturePackPage> texturePackPageMap =
      std::make_unique<HashMap<>>();
public
  static final HashMap<String, String> TexturePackPageNameMap =
      std::make_unique<HashMap<>>();
public
  final HashMap<String, Texture> subTextures = std::make_unique<HashMap<>>();
  Texture tex = null;
...
public
  static ArrayList<TexturePackPage.SubTextureInfo> TempSubTextureInfo =
...
public
  ...
  static Texture getTexture(const std::string &string) {
    if (string.contains(".png")) {
      ...
      return subTextureMap.containsKey(string) ? subTextureMap.get(string)
                                               : nullptr;
    }
  }
  ...
  void loadFromPackFileDDS(BufferedInputStream bufferedInputStream) {
    ...
    for (int int1 = 0; int1 < int0; int1++) {
      ...
      if (string1.contains("ZombieWalk") && string1.contains("BobZ_")) {
        TempSubTextureInfo.add(new TexturePackPage.SubTextureInfo(
    ...
    if (TempSubTextureInfo.isEmpty()) {
      ...
      Texture texture0 = new Texture(string0, bufferedInputStream, boolean0,
                                     Texture.PZFileformat.DDS);

      for (int int11 = 0; int11 < TempSubTextureInfo.size(); int11++) {
        TexturePackPage.SubTextureInfo subTextureInfo =
            TempSubTextureInfo.get(int11);
        Texture texture1 = texture0.split(subTextureInfo.x, subTextureInfo.y,
                                          subTextureInfo.w, subTextureInfo.h);
        texture1.copyMaskRegion(texture0, subTextureInfo.x, subTextureInfo.y,
                                subTextureInfo.w, subTextureInfo.h);
