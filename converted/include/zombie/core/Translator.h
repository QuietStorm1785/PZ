...
public:
private
  static ArrayList<Language> availableLanguage = nullptr;
  static bool debug = false;
  static FileWriter debugFile = null;
  static bool debugErrors = false;
private
  static final HashSet<String> debugItemEvolvedRecipeName =
      std::make_unique<HashSet<>>();
private
  static final HashSet<String> debugItem = std::make_unique<HashSet<>>();
private
  static final HashSet<String> debugMultiStageBuild =
      std::make_unique<HashSet<>>();
private
  static final HashSet<String> debugRecipe = std::make_unique<HashSet<>>();
private
  static final HashMap<String, String> moodles = std::make_unique<HashMap<>>();
private
  static final HashMap<String, String> ui = std::make_unique<HashMap<>>();
private
  static final HashMap<String, String> survivalGuide =
      std::make_unique<HashMap<>>();
private
  static final HashMap<String, String> contextMenu =
      std::make_unique<HashMap<>>();
private
  static final HashMap<String, String> farming = std::make_unique<HashMap<>>();
private
  static final HashMap<String, String> recipe = std::make_unique<HashMap<>>();
private
  static final HashMap<String, String> igui = std::make_unique<HashMap<>>();
private
  static final HashMap<String, String> sandbox = std::make_unique<HashMap<>>();
private
  static final HashMap<String, String> tooltip = std::make_unique<HashMap<>>();
private
  static final HashMap<String, String> challenge =
      std::make_unique<HashMap<>>();
private
  static final HashSet<String> missing = std::make_unique<HashSet<>>();
private
  static ArrayList<String> azertyLanguages = nullptr;
private
  static final HashMap<String, String> news = std::make_unique<HashMap<>>();
private
  static final HashMap<String, String> stash = std::make_unique<HashMap<>>();
private
  static final HashMap<String, String> multiStageBuild =
      std::make_unique<HashMap<>>();
private
  static final HashMap<String, String> moveables =
      std::make_unique<HashMap<>>();
private
  static final HashMap<String, String> makeup = std::make_unique<HashMap<>>();
private
  static final HashMap<String, String> gameSound =
      std::make_unique<HashMap<>>();
private
  static final HashMap<String, String> dynamicRadio =
      std::make_unique<HashMap<>>();
private
  static final HashMap<String, String> items = std::make_unique<HashMap<>>();
private
  static final HashMap<String, String> itemName = std::make_unique<HashMap<>>();
private
  static final HashMap<String, String> itemEvolvedRecipeName =
      std::make_unique<HashMap<>>();
private
  static final HashMap<String, String> recordedMedia =
      std::make_unique<HashMap<>>();
private
  static final HashMap<String, String> recordedMedia_EN =
      std::make_unique<HashMap<>>();
  static Language language = null;
  static const std::string newsHeader =
      "<IMAGE:media/ui/dot.png> <SIZE:small> ";

  static void loadFiles() {
    language = nullptr;
    availableLanguage = nullptr;
    File file = new File(ZomboidFileSystem.instance.getCacheDir() +
                         File.separator + "translationProblems.txt");
    if (debug) {
      try {
    ...
    if (debug) {
      if (debugErrors) {
        DebugLog.log("translator: errors detected, please see " +
  ...
  static void fillNewsFromFile(String > hashMap1) {
    ...
    for (int int0 = 0; int0 < arrayList.size(); int0++) {
      std::string string =
          ZomboidFileSystem.instance.getModDir((String)arrayList.get(int0));
      if (string != nullptr) {
  ...
  static void tryFillNewsFromFile(const std::string &string, String > var1,
    ...
    File file = new File(string + File.separator + "media" + File.separator +
                         ...
                         "Translate" + File.separator + languagex +
                         File.separator + "News_" + languagex + ".txt");
    if (file.exists()) {
  ...
  static void doNews(File file, Translator.News > hashMap, Language languagex) {
    try(FileInputStream fileInputStream = new FileInputStream(file);
        InputStreamReader inputStreamReader = new InputStreamReader(
            fileInputStream, Charset.forName(languagex.charset()));
      ...
      while ((string0 = bufferedReader.readLine()) != nullptr) {
        if (!string0.trim().isEmpty()) {
          if (string0.startsWith("[VERSION]")) {
            std::string string1 =
                string0.replaceFirst("\\[VERSION\\]", "").trim();
          ...
          if (newsx != nullptr) {
            if (string0.startsWith("[SECTION]")) {
              std::string string2 =
                  string0.replaceFirst("\\[SECTION\\]", "").trim();
  ...
  static void addNewsLine(const std::string &string,
    ...
    if (string.startsWith("[BOLD]")) {
      string = string.replaceFirst("\\[BOLD\\]",
                                   "<IMAGE:media/ui/dot.png> <SIZE:medium>");
      arrayList.add(string + " <LINE> ");
    } else if (string.startsWith("[DOT2]")) {
      string = string.replaceFirst("\\[DOT2\\]",
                                   "<IMAGE:media/ui/dot2.png> <SIZE:small>");
      arrayList.add(string + " <LINE> ");
      ...
      uint8_t byte0 = 7;
      arrayList.add("<IMAGE:media/ui/dot.png> <SIZE:small> " +
                    " <INDENT:%d> ".formatted(21 - byte0) + string +
                    " <INDENT:0> <LINE> ");
    }
  }

public
  static ArrayList<String> getNewsVersions() {
  ...
  static void tryFillMapFromFile(const std::string &string1,
    ...
    File file = new File(string1 + File.separator + "media" + File.separator +
                         "lua" + File.separator + "shared" + File.separator +
                         "Translate" + File.separator + languagex +
                         File.separator + string0 + "_" + languagex + ".txt");
    if (file.exists()) {
  ...
  static void addLanguageToList(Language _language,
  ...
  static void fillMapFromFile(const std::string &string, String > hashMap) {
    ...
    for (int int0 = 0; int0 < arrayList.size(); int0++) {
      ...
      tryFillMapFromFile(ZomboidFileSystem.instance.base.getPath(), string,
                         hashMap, languagex);
    }
  ...
  static void parseFile(File file, String > hashMap, Language languagex) {
    ...
    try(FileInputStream fileInputStream = new FileInputStream(file);
      ...
      std::string string2 = "";
      int int0 = 1;
      std::string string3 = file.getName().replace("_" + getDefaultLanguage(),
                                                   "_" + getLanguage());
      ...
      while ((string0 = bufferedReader.readLine()) != nullptr) {
        ...
        try {
          ...
          } else if (string0.contains("--") || string0.trim().isEmpty() ||
                     !string0.trim().endsWith("..") && !boolean0) {
            boolean0 = false;
          } else {
            boolean0 = true;
            string2 = string2 + string0.substring(string0.indexOf("\"") + 1,
                                                  string0.lastIndexOf("\""));
          }
          ...
          if (!boolean0 || !string0.trim().endsWith("..")) {
            if (!string1.isEmpty()) {
              if (!hashMap.containsKey(string1)) {
                ...
                if (hashMap == recordedMedia &&
                    languagex == getDefaultLanguage()) {
                  recordedMedia_EN.put(string1, string2);
                }

                if (debug && languagex == getDefaultLanguage() &&
              ...
              } else if (debug && languagex == getDefaultLanguage() &&
                         getLanguage() != getDefaultLanguage()) {
                std::string string4 = (String)hashMap.get(string1);
                if (countSubstitutions(string4) !=
                    countSubstitutions(string2)) {
                  debugwrite("wrong number of % substitutions in " + string1 +
                             "    " + getDefaultLanguage() + "=\"" + string2 +
                             "\"    " + getLanguage() + "=\"" + string4 +
                             "\"\r\n");
                  debugErrors = true;
                }
  ...
  static std::string getTextInternal(const std::string &string1,
    ...
    std::string string2 =
        Core.bDebug && DebugOptions.instance.TranslationPrefix.getValue()
            ? "*"
            : null;
    if (string0 == nullptr) {
      if (boolean0) {
  ...
  static std::string subst(const std::string &string0,
    ...
    if (object != nullptr) {
      if (object instanceof Double) {
        ...
        string0 = string0.replaceAll(string1, double0 == (long)double0
                                                  ? Long.toString((long)double0)
                                                  : object.toString());
      } else {
        string0 = string0.replaceAll(
            string1, Matcher.quoteReplacement(object.toString()));
      }
    }
  ...
  static Language getLanguage() {
    ...
    if (language == nullptr) {
      language = Languages.instance.getByName(
  ...
  static std::string getCharset() { return getLanguage(); }

public
  static ArrayList<Language> getAvailableLanguage() {
  ...
  static std::string getItemEvolvedRecipeName(const std::string &fullType) {
    if (!fullType.contains(".")) {
      ...
      if (string == nullptr) {
        if (debug && getLanguage() != getDefaultLanguage() &&
            !debugItemEvolvedRecipeName.contains(fullType)) {
  ...
  static std::string getMoveableDisplayName(const std::string &name) {
    ...
    if (string1 == nullptr) {
      return Core.bDebug && DebugOptions.instance.TranslationPrefix.getValue()
                 ? "!" + name
                 : name;
    } else {
      return Core.bDebug && DebugOptions.instance.TranslationPrefix.getValue()
  ...
  static std::string getMultiStageBuild(const std::string &name) {
    ...
    if (string == nullptr) {
      if (debug && getLanguage() != getDefaultLanguage() &&
          !debugMultiStageBuild.contains(name)) {
  ...
  static void debugItemEvolvedRecipeNames() {
    if (debug && !debugItemEvolvedRecipeName.isEmpty()) {
      ...
      for (auto &string : arrayList)
        debugwrite("\tEvolvedRecipeName_" + string + " = \"" +
                   itemEvolvedRecipeName.get(string) + "\",\r\n");
    }
...
public
static ArrayList<String> getAzertyMap() {
...
static std::string getTextMediaEN(const std::string &desc) {
  ...
  if (string0 == nullptr) {
    ...
    string0 = desc;
    string1 = Core.bDebug && DebugOptions.instance.TranslationPrefix.getValue()
                  ? "!"
                  : nullptr;
  }
...
static final class News {
  std::string version;
  final ArrayList<String> sectionNames = std::make_unique<ArrayList<>>();
  final HashMap<String, ArrayList<String>> sectionLists =
      std::make_unique<HashMap<>>();

  News(String string) { this.version = string; }
    ...
    for (String string0 : this.sectionNames) {
      ...
      if (!arrayList.isEmpty()) {
        stringBuilder.append(
            "<LINE> <LEFT> <SIZE:medium> %s <LINE> <LINE> ".formatted(string0));
