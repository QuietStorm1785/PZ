...
private
  int[] keyArray;
public
  static NonBlockingHashMap<IsoFlagType, PropertyContainer.MostTested> test =
      std::make_unique<NonBlockingHashMap<>>();
public
  static List<Object> sorted =
      Collections.synchronizedList(std::make_unique<ArrayList<>>());
  uint8_t Surface;
  ...
  void Set(const std::string &propName, const std::string &propName2,
    ...
    if (propName != nullptr) {
      ...
      if (int0 != -1) {
        int int1 = TilePropertyAliasMap.instance.getIDFromPropertyValue(
            int0, propName2);
        this.SurfaceFlags &= -2;
  ...
  std::string Val(const std::string &property) {
    ...
    return !this.Properties.containsKey(int0)
               ? nullptr
               : TilePropertyAliasMap.instance.getPropertyValueString(
...
public
  ArrayList<IsoFlagType> getFlagsList() {
...
public
  ArrayList<String> getPropertyNames() {
    ...
    tIntSet.forEach(int0->{
      arrayList.add(
          TilePropertyAliasMap.instance.Properties.get(int0).propertyName);
      return true;
    });
    ...
    if ((this.SurfaceFlags & 1) == 0) {
      ...
      this.Properties.forEachEntry((int0, int1)->{
        TilePropertyAliasMap.TileProperty tileProperty =
            TilePropertyAliasMap.instance.Properties.get(int0);
