...
public:
  ...
  void exec(List<Keyframe> list) {
    AnimationClip animationClip =
        ModelManager.instance.getAnimationClip(this.m_source);

    for (int int0 = 0; int0 < 60; int0++) {
      Keyframe[] keyframes = animationClip.getBoneFramesAt(int0);
      if (keyframes.length != 0) {
        for (int int1 = this.m_sourceFrame1; int1 <= this.m_sourceFrame2;
             int1++) {
          ...
          keyframe1.Time =
              (float)(this.m_frame - 1 + (int1 - this.m_sourceFrame1)) /
              this.m_FPS;
          keyframe1.Position = KeyframeUtil.GetKeyFramePosition(
              keyframes, (float)(int1 - 1) / this.m_sourceFPS,
              animationClip.Duration);
          keyframe1.Rotation = KeyframeUtil.GetKeyFrameRotation(
              keyframes, (float)(int1 - 1) / this.m_sourceFPS,
              animationClip.Duration);
