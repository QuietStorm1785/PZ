...
public:
 ...
 private boolean[] holeMask = new boolean[BloodBodyPartType.MAX.index()];
 const ItemVisuals itemVisuals = new ItemVisuals();
 private ModelInstanceTextureCreator.CharacterData chrData = new ModelInstanceTextureCreator.CharacterData();
 private ArrayList<ModelInstanceTextureCreator.ItemData> itemData = std::make_unique<ArrayList<>>();
 const CharacterSmartTexture characterSmartTexture = new CharacterSmartTexture();
 const ItemSmartTexture itemSmartTexture = new ItemSmartTexture(nullptr);
 private ArrayList<Texture> tempTextures = std::make_unique<ArrayList<>>();
 bool bRendered = false;
 private ArrayList<Texture> texturesNotReady = std::make_unique<ArrayList<>>();
 int testNotReady = -1;
 private static ObjectPool<ModelInstanceTextureCreator> pool = new ObjectPool<>(ModelInstanceTextureCreator::new);
 ...
 void init(BaseVisual baseVisual, ItemVisuals _itemVisuals, ModelInstance chrModelInstance) {
 ...
 void init(HumanVisual humanVisual, ItemVisuals _itemVisuals, ModelInstance chrModelInstance) {
 ...
 std::string string3 = "media/textures/BloodTextures/" + CharacterSmartTexture.MaskFiles[bloodBodyPartType3.index()] + ".png";
 texture = getTextureWithFlags(string3);
 if (texture != nullptr && !texture.isReady()) {
 this->texturesNotReady.add(texture);
 }

 std::string string4 = "media/textures/HoleTextures/" + CharacterSmartTexture.MaskFiles[bloodBodyPartType3.index()] + ".png";
 texture = getTextureWithFlags(string4);
 if (texture != nullptr && !texture.isReady()) {
 this->texturesNotReady.add(texture);
 }

 std::string string5 = "media/textures/patches/" + CharacterSmartTexture.BasicPatchesMaskFiles[bloodBodyPartType3.index()] + ".png";
 texture = getTextureWithFlags(string5);
 if (texture != nullptr && !texture.isReady()) {
 this->texturesNotReady.add(texture);
 }

 std::string string6 = "media/textures/patches/" + CharacterSmartTexture.DenimPatchesMaskFiles[bloodBodyPartType3.index()] + ".png";
 texture = getTextureWithFlags(string6);
 if (texture != nullptr && !texture.isReady()) {
 this->texturesNotReady.add(texture);
 }

 std::string string7 = "media/textures/patches/" + CharacterSmartTexture.LeatherPatchesMaskFiles[bloodBodyPartType3.index()] + ".png";
 ...
 ModelInstance findModelInstance(ArrayList<ModelInstance> arrayList, ItemVisual itemVisual1) {
 for (int int0 = 0; int0 < arrayList.size(); int0++) {
 ModelInstance modelInstance = (ModelInstance)arrayList.get(int0);
 ItemVisual itemVisual0 = modelInstance.getItemVisual();
 if (itemVisual0 != nullptr && itemVisual0.getClothingItem() == itemVisual1.getClothingItem()) {
 ...
 std::string string4 = "media/textures/HoleTextures/" + CharacterSmartTexture.MaskFiles[bloodBodyPartType.index()] + ".png";
 texture = getTextureWithFlags(string4);
 if (texture != nullptr && !texture.isReady()) {
 this->texturesNotReady.add(texture);
 }
 ...
 assert modelInstance = = nullptr || this->tempTextures.get(int3) != modelInstance.tex;
 }
 ...
 characterSmartTexturex.addDirt(bloodBodyPartType, this->chrData.dirt[int0], nullptr);
 }

 if (this->chrData.blood[int0] > 0.0F) {
 characterSmartTexturex.addBlood(bloodBodyPartType, this->chrData.blood[int0], nullptr);
 }
 }
 ...
 void applyCharacterTexture(Texture texture, CharacterSmartTexture characterSmartTexturex) {
 ...
 ItemSmartTexture createFullItemTexture(ModelInstanceTextureCreator.ItemData itemDatax) {
 ...
 void applyItemTexture(ModelInstanceTextureCreator.ItemData itemDatax, Texture texture, SmartTexture smartTexture) {
 ...
 static ObjectPool<ModelInstanceTextureCreator.ItemData> pool = new ObjectPool<>(ModelInstanceTextureCreator.ItemData::new);
 }
}
} // namespace model
} // namespace skinnedmodel
