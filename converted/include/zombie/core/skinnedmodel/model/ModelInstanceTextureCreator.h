...
private
  final boolean[] holeMask = new boolean[BloodBodyPartType.MAX.index()];
  const ItemVisuals itemVisuals = new ItemVisuals();
private
  final ModelInstanceTextureCreator.CharacterData chrData =
      new ModelInstanceTextureCreator.CharacterData();
private
  final ArrayList<ModelInstanceTextureCreator.ItemData> itemData =
      std::make_unique<ArrayList<>>();
  const CharacterSmartTexture characterSmartTexture =
      new CharacterSmartTexture();
  const ItemSmartTexture itemSmartTexture = new ItemSmartTexture(null);
private
  final ArrayList<Texture> tempTextures = std::make_unique<ArrayList<>>();
  bool bRendered = false;
private
  final ArrayList<Texture> texturesNotReady = std::make_unique<ArrayList<>>();
  int testNotReady = -1;
private
  static final ObjectPool<ModelInstanceTextureCreator> pool =
  ...
  void addClothingItem(ModelInstance modelInstance, ItemVisual itemVisual,
    ...
    std::string string0 = modelInstance == null
        ...
        for (int int3 = 0; int3 < this.tempTextures.size(); int3++) {
          for (int int4 = 0; int4 < this.itemData.size(); int4++) {
            ...
            assert modelInstance = =
                null || this.tempTextures.get(int3) != modelInstance.tex;
          }
...
private
  static final class CharacterData {
  ...
  private static final class ItemData {
    ...
    static final ObjectPool<ModelInstanceTextureCreator.ItemData> pool =
