...
public:
private
  static final Pool<ModelSlotDebugRenderData> s_pool =
      new Pool<>(ModelSlotDebugRenderData::new);
  ModelSlotRenderData m_slotData;
  const TFloatArrayList m_boneCoords = new TFloatArrayList();
private
  final ArrayList<Matrix4f> m_boneMatrices = std::make_unique<ArrayList<>>();
  const TFloatArrayList m_squareLights = new TFloatArrayList();
private
  org.joml.Matrix4f m_weaponMatrix;
  float m_weaponLength;

  static ModelSlotDebugRenderData alloc() { return s_pool.alloc(); }
  ...
  ModelSlotDebugRenderData init(ModelSlotRenderData slotData) {
    ...
    for (int int0 = 0; int0 < slotData.modelData.size(); int0++) {
      ModelInstanceRenderData modelInstanceRenderData =
          slotData.modelData.get(int0);
      modelInstanceRenderData.m_debugRenderData =
          ModelInstanceDebugRenderData.alloc().init(slotData,
  ...
  void initBoneAxis() {
    ...
    if (this.m_slotData.animPlayer != nullptr &&
        this.m_slotData.animPlayer.hasSkinningData()) {
      if (DebugOptions.instance.Character.Debug.Render.Bip01.getValue()) {
        this.initBoneAxis("Bip01");
      }

      if (DebugOptions.instance.Character.Debug.Render.PrimaryHandBone
              .getValue()) {
        this.initBoneAxis("Bip01_Prop1");
      }

      if (DebugOptions.instance.Character.Debug.Render.SecondaryHandBone
              .getValue()) {
        this.initBoneAxis("Bip01_Prop2");
      }

      if (DebugOptions.instance.Character.Debug.Render.TranslationData
              .getValue()) {
    ...
    if (DebugOptions.instance.ModelRenderBones.getValue()) {
      ...
      if (this.m_slotData.object instanceof BaseVehicle) {
        for (int int0 = 0; int0 < this.m_slotData.modelData.size(); int0++) {
          ModelInstanceRenderData modelInstanceRenderData =
              this.m_slotData.modelData.get(int0);
          VehicleSubModelInstance vehicleSubModelInstance =
              Type.tryCastTo(modelInstanceRenderData.modelInstance,
  ...
  void initSkeleton(AnimationPlayer animationPlayer) {
    if (animationPlayer != nullptr && animationPlayer.hasSkinningData() &&
        !animationPlayer.isBoneTransformsNeedFirstFrame()) {
      int integer =
          animationPlayer.getSkinningData().BoneIndices.get("Translation_Data");

      for (int int0 = 0; int0 < animationPlayer.modelTransforms.length;
           int0++) {
        if (integer == nullptr || int0 != integer) {
          int int1 =
              animationPlayer.getSkinningData().SkeletonHierarchy.get(int0);
          if (int1 >= 0) {
      ...
      if (this.m_slotData.character != nullptr) {
        if (this.m_slotData.character.getCurrentSquare() != nullptr) {
          ...
          IsoGridSquare.ILighting iLighting =
              this.m_slotData.character.getCurrentSquare().lighting[int0];
  ...
  void initWeaponHitPoint() {
    ...
    if (DebugOptions.instance.ModelRenderWeaponHitPoint.getValue()) {
      if (this.m_slotData.animPlayer != nullptr &&
        ...
        if (this.m_slotData.character != nullptr) {
          int integer =
              this.m_slotData.animPlayer.getSkinningData().BoneIndices.get(
                  "Bip01_Prop1");
          if (integer != nullptr) {
            HandWeapon weapon =
                Type.tryCastTo(this.m_slotData.character.getPrimaryHandItem(),
                               HandWeapon.class);
            if (weapon != nullptr) {
              this.m_weaponLength = weapon.WeaponLength;
              Matrix4f matrix4f =
                  this.m_slotData.animPlayer.modelTransforms[integer];
  ...
  void renderSkeleton() {
    if (!this.m_boneCoords.isEmpty()) {
      ...
      for (byte byte0 = 0; byte0 < this.m_boneCoords.size(); byte0 += 6) {
        Color color =
            Model.debugDrawColours[byte0 % Model.debugDrawColours.length];
  ...
  void renderLights() {
    for (byte byte0 = 0; byte0 < this.m_squareLights.size(); byte0 += 3) {
      ...
      Model.debugDrawLightSource(float0, float1, float2, this.m_slotData.x,
                                 this.m_slotData.y, this.m_slotData.z,
                                 -this.m_slotData.animPlayerAngle);
    }
  }
