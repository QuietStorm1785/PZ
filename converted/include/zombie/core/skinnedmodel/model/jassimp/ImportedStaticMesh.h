...
public:
  VertexBufferObject.VertexArray verticesUnskinned = nullptr;
  int[] elements = nullptr;
  const Vector3f minXYZ = new Vector3f(Float.MAX_VALUE);
  const Vector3f maxXYZ = new Vector3f(-Float.MAX_VALUE);

public
  ImportedStaticMesh(AiMesh aiMesh) { this.processAiScene(aiMesh); }
  ...
  void processAiScene(AiMesh aiMesh) {
    ...
    VertexBufferObject.VertexFormat vertexFormat =
        new VertexBufferObject.VertexFormat(3 + int1);
    ...
    for (int int3 = 0; int3 < int1; int3++) {
      vertexFormat.setElement(
          3 + int3, VertexBufferObject.VertexType.TextureCoordArray, 8);
    }

    vertexFormat.calculate();
    this.verticesUnskinned =
        new VertexBufferObject.VertexArray(vertexFormat, int0);
    ...
    for (int int4 = 0; int4 < int0; int4++) {
      ...
      this.verticesUnskinned.setElement(int4, 0, aiMesh.getPositionX(int4),
                                        aiMesh.getPositionY(int4),
                                        aiMesh.getPositionZ(int4));
      if (aiMesh.hasNormals()) {
        this.verticesUnskinned.setElement(int4, 1, aiMesh.getNormalX(int4),
                                          aiMesh.getNormalY(int4),
                                          aiMesh.getNormalZ(int4));
      } else {
        this.verticesUnskinned.setElement(int4, 1, 0.0F, 1.0F, 0.0F);
      }

      if (aiMesh.hasTangentsAndBitangents()) {
        this.verticesUnskinned.setElement(int4, 2, aiMesh.getTangentX(int4),
                                          aiMesh.getTangentY(int4),
                                          aiMesh.getTangentZ(int4));
      } else {
      ...
      if (int1 > 0) {
        ...
        for (int int6 = 0; int6 < 8; int6++) {
          if (aiMesh.hasTexCoords(int6)) {
            this.verticesUnskinned.setElement(
                int4, 3 + int5, aiMesh.getTexCoordU(int4, int6),
                1.0F - aiMesh.getTexCoordV(int4, int6));
            int5++;
          }
        }
      }
