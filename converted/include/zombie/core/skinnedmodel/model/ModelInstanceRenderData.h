...
public:
 ...
 private static ObjectPool<ModelInstanceRenderData> pool = new ObjectPool<>(ModelInstanceRenderData::new);
 ...
 if (_modelInstance instanceof VehicleModelInstance || vehicleSubModelInstance != nullptr) {
 ...
 if (_modelInstance.model.Mesh != nullptr && _modelInstance.model.Mesh.isReady() && _modelInstance.model.Mesh.m_transform != nullptr) {
 ...
 static Matrix4f makeAttachmentTransform(ModelAttachment attachment, Matrix4f attachmentXfrm) {
 ...
 static void applyBoneTransform(ModelInstance parentInstance, const std::string& boneName, Matrix4f transform) {
 ...
 if (modelInstancex.parent != nullptr && modelInstancex.parent.m_modelScript != nullptr) {
 ...
 if (modelAttachment1.getBone() != nullptr && this->parentBone == animationPlayer.getSkinningBoneIndex(modelAttachment1.getBone(), 0) {
 ...
 if (modelInstanceRenderData.modelInstance.getTextureInitializer() != nullptr) {
