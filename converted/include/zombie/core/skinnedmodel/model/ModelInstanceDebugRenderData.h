...
public:
 private static Pool<ModelInstanceDebugRenderData> s_pool = new Pool<>(ModelInstanceDebugRenderData::new);
 private ArrayList<Matrix4f> m_attachmentMatrices = std::make_unique<ArrayList<>>();

 static ModelInstanceDebugRenderData alloc() {
 return s_pool.alloc();
 }

 ModelInstanceDebugRenderData init(ModelSlotRenderData slotData, ModelInstanceRenderData instData) {
 ...
 void initAttachments(ModelSlotRenderData modelSlotRenderData, ModelInstanceRenderData modelInstanceRenderData) {
 ...
 if (modelInstanceRenderData.model.bStatic || modelAttachment.getBone() == nullptr) {
 matrix4f0.mul(matrix4f2, matrix4f2);
 } else if (modelSlotRenderData.animPlayer != nullptr && modelSlotRenderData.animPlayer.hasSkinningData()) {
