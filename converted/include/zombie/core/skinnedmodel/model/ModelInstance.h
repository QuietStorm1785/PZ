...
public:
 ...
 ModelInstance init(Model _model, IsoGameCharacter _character, AnimationPlayer player) {
 ...
 if (!this->character.amputations.empty() && float0 > 0.0F && this->AnimPlayer != nullptr) {
 ...
 void setItemVisual(ItemVisual _itemVisual) {
 ...
 return modelAttachment = = nullptr ? out.identity() : this->getAttachmentMatrix(modelAttachment, out);
 }
 ...
 void setTextureInitializer(ModelInstanceTextureInitializer textureInitializer) {
 ...
 public static class EffectLight {
 float x;
 float y;
 float z;
 float r;
 ...
 void set(float _x, float _y, float _z, float _r, float _g, float _b, int _radius) {
 this->x = _x;
 this->y = _y;
 this->z = _z;
 this->r = _r;
 ...
 float b;
 int flags;
 public org.lwjgl.util.vector.Vector3f currentColor = new org.lwjgl.util.vector.Vector3f();
 public org.lwjgl.util.vector.Vector3f targetColor = new org.lwjgl.util.vector.Vector3f();
 bool active;
 bool foundThisFrame;
 }
 ...
 private static ObjectPool<ModelInstance.PlayerData> pool = new ObjectPool<>(ModelInstance.PlayerData::new);
 ...
 if (this->frameLights[int4] == nullptr || !this->frameLights[int4].active) {
