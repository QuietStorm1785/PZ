...
public:
 ...
 private HashMap<String, WorldItemAtlas.ItemTexture> itemTextureMap = std::make_unique<HashMap<>>();
 private ArrayList<WorldItemAtlas.Atlas> AtlasList = std::make_unique<ArrayList<>>();
 private WorldItemAtlas.ItemParams itemParams = new WorldItemAtlas.ItemParams();
 private WorldItemAtlas.Checksummer checksummer = new WorldItemAtlas.Checksummer();
 private static Stack<WorldItemAtlas.RenderJob> JobPool = std::make_unique<Stack<>>();
 private ArrayList<WorldItemAtlas.RenderJob> RenderJobs = std::make_unique<ArrayList<>>();
 private ObjectPool<WorldItemAtlas.ItemTextureDrawer> itemTextureDrawerPool = new ObjectPool<>(WorldItemAtlas.ItemTextureDrawer::new);
 private ObjectPool<WorldItemAtlas.WeaponPartParams> weaponPartParamPool = new ObjectPool<>(WorldItemAtlas.WeaponPartParams::new);
 ...
 return this->itemParams.init(item) ? this->getItemTexture(this->itemParams) : nullptr;
 }
 ...
 const int ENTRY_HGT;
 Texture tex;
 public ArrayList<WorldItemAtlas.AtlasEntry> EntryList = std::make_unique<ArrayList<>>();
 bool clear = true;
 ...
 std::string string10 = itemVisual1.getTextureChoice(clothingItem1);
 ...
 this->init(item, model2, nullptr, float3, immutableColor3, float0, false);
 ...
 void init(InventoryItem var1, Model model, ModelScript modelScript, float float0, ImmutableColor immutableColor, float var6, bool boolean0) {
 this->m_model = model;
 ...
 Food food = Type.tryCastTo(item, Food.class);
 return food = = nullptr || this->getFoodState(food) == this->m_foodState;
 } else {
 return false;
 }
 }
 ...
 WorldItemAtlas.ItemParams itemParams = new WorldItemAtlas.ItemParams();
 ...
 private static class ItemTextureDrawer extends TextureDraw.GenericDrawer {
 ...
 if (atlasEntry != nullptr && atlasEntry.ready && atlasEntry.tex.isReady()) {
 ...
 public WorldItemAtlas.ItemParams itemParams = new WorldItemAtlas.ItemParams();
 ...
 return WorldItemAtlas.JobPool.empty() ? new WorldItemAtlas.RenderJob() : WorldItemAtlas.JobPool.pop();
 }
 ...
 return model != nullptr && model.isReady() && model.Mesh != nullptr && model.Mesh.isReady();
 }
 ...
 bool boolean0 = this->renderModel(this->itemParams.m_model, nullptr);
