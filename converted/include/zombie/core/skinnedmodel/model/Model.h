...
public:
  ...
  static const FloatBuffer m_staticReusableFloatBuffer =
      BufferUtils.createFloatBuffer(128);
  static const Matrix4f IDENTITY = new Matrix4f();
public
  static final Color[] debugDrawColours = new Color[] {
    new Color();
    new Color();
    new Color();
    new Color();
...
public
  Model.ModelAssetParams assetParams;
  static Vector3 tempo = std::make_unique<Vector3>();
  static const AssetType ASSET_TYPE = new AssetType("Model");

public
  Model(AssetPath path, AssetManager manager, Model.ModelAssetParams params) {
    super(path, manager);
    ...
    meshAssetParams.animationsMesh = this.assetParams == nullptr
                                         ? nullptr
                                         : this.assetParams.animationsModel;
    this.Mesh = (ModelMesh)MeshAssetManager.instance.load(
        new AssetPath(params.meshName), meshAssetParams);
    if (!StringUtils.isNullOrWhitespace(params.textureName)) {
      if (params.textureName.contains("media/")) {
        this.tex =
            Texture.getSharedTexture(params.textureName, params.textureFlags);
      } else {
        this.tex = Texture.getSharedTexture("media/textures/" +
                                                params.textureName + ".png",
                                            params.textureFlags);
      }
    }
  ...
  static void BoneToWorldCoords(IsoGameCharacter character, int boneIndex,
                                Vector3 vec) {
    AnimationPlayer animationPlayer = character.getAnimationPlayer();
  ...
  static void BoneYDirectionToWorldCoords(IsoGameCharacter character,
                                          int boneIndex, Vector3 vec,
                                          float length) {
    AnimationPlayer animationPlayer = character.getAnimationPlayer();
  ...
  static void BoneToWorldCoords(ModelSlotRenderData slotData, int boneIndex,
                                Vector3 vec) {
  ...
  static void CharacterModelCameraBegin(ModelSlotRenderData slotData) {
    ...
    if (slotData.bInVehicle) {
      ...
      uint8_t byte0 = -1;
      GL11.glTranslatef(slotData.inVehicleX, slotData.inVehicleY,
                        slotData.inVehicleZ * byte0);
      GL11.glScalef(1.5F, 1.5F, 1.5F);
    }
  }

  static void CharacterModelCameraEnd() { ModelCamera.instance.End(); }

  void DrawChar(ModelSlotRenderData slotData,
                ModelInstanceRenderData instData) {
    if (!DebugOptions.instance.Character.Debug.Render.SkipCharacters
             .getValue()) {
      ...
      if (!(slotData.alpha < 0.01F)) {
        if (slotData.animPlayer != nullptr) {
          ...
          if (Core.bDebug &&
            ...
            Shader shader1 = ShaderManager.instance.getOrCreateShader(
                "vehicle_wireframe", this.bStatic);
            if (shader1 != nullptr) {
              shader1.Start();
              if (this.bStatic) {
            ...
            if (!DebugOptions.instance.DebugDraw_SkipDrawNonSkinnedModel
                     .getValue()) {
              GameProfiler.getInstance().invokeAndMeasure(
                  "Mesh.Draw.Call", shader0, this.Mesh,
                  (shader, modelMesh)->modelMesh.Draw(shader));
            }
  ...
  void drawMuzzleFlash(ModelInstanceRenderData modelInstanceRenderData) {
    if (modelInstanceRenderData.m_muzzleFlash) {
      ModelAttachment modelAttachment =
          modelInstanceRenderData.modelInstance.getAttachmentById("muzzle");
      if (modelAttachment != nullptr) {
        ...
        Matrix4f matrix4f1 =
            modelInstanceRenderData.modelInstance.getAttachmentMatrix(
                modelAttachment, matrix4fObjectPool.alloc());
  ...
  void drawVehicleLights(ModelSlotRenderData modelSlotRenderData) {
    ...
    for (int int1 = 0; int1 < 3; int1++) {
      ModelInstance.EffectLight effectLight =
          modelSlotRenderData.effectLights[int1];
  ...
  static void drawBoneMtxInternal(org.lwjgl.util.vector.Matrix4f matrix4f) {
    ...
    drawArrowInternal(float3, float4, float5, float6, float7, float8, float12,
                      float13, float14, float0, float1, float2, 1.0F, 0.0F,
                      0.0F);
    drawArrowInternal(float3, float4, float5, float9, float10, float11, float12,
                      float13, float14, float0, float1, float2, 0.0F, 1.0F,
                      0.0F);
    drawArrowInternal(float3, float4, float5, float12, float13, float14, float6,
                      float7, float8, float0, float1, float2, 0.0F, 0.0F, 1.0F);
  }

private
  static void drawArrowInternal(float float5, float float6, float float7,
                                float float11, float float10, float float8,
                                float float15, float float14, float float12,
                                float float9, float float1, float float13,
    ...
    GL11.glVertex3f(float5 + float11 * float9, float6 + float10 * float9,
                    float7 + float8 * float9);
    GL11.glVertex3f(float5 + float11 * float9, float6 + float10 * float9,
                    float7 + float8 * float9);
    GL11.glVertex3f(float5 + (float11 * float0 + float15 * float13) * float9,
  ...
  void debugDrawLightSource(IsoLightSource ls, float cx, float cy, float cz,
                            float radians) {
    debugDrawLightSource(ls.x, ls.y, ls.z, cx, cy, cz, radians);
  }

  static void debugDrawLightSource(float lx, float ly, float lz, float cx,
                                   float cy, float cz, float radians) {
    ...
    float1 *= 0.67F;
    float float3 =
        (float)(float0 * Math.cos(radians) - float1 * Math.sin(radians));
  ...
  void DrawVehicle(ModelSlotRenderData slotData,
                   ModelInstanceRenderData instData) {
    if (!DebugOptions.instance.ModelRenderSkipVehicles.getValue()) {
      ...
      PZGLUtil.checkGLErrorThrow(
          "Model.drawVehicle Enter inst: %s, instTex: %s, slotData: %s",
          modelInstance, texture, slotData);
      GL11.glEnable(2884);
      GL11.glCullFace(modelInstance.m_modelScript != nullptr &&
                              modelInstance.m_modelScript.invertX
                          ? 1029
                          : 1028);
      GL11.glEnable(2929);
      GL11.glDepthFunc(513);
      ...
      if (Core.bDebug &&
        ...
        GL11.glEnable(2848);
        GL11.glLineWidth(0.75F);
        shader = ShaderManager.instance.getOrCreateShader("vehicle_wireframe",
                                                          this.bStatic);
        if (shader != nullptr) {
          shader.Start();
          if (this.bStatic) {
        ...
        if (shader != nullptr) {
          ...
          if (shader.isVehicleShader()) {
            VehicleModelInstance vehicleModelInstance =
                Type.tryCastTo(modelInstance, VehicleModelInstance.class);
            if (modelInstance instanceof VehicleSubModelInstance) {
              vehicleModelInstance = Type.tryCastTo(modelInstance.parent,
                                                    VehicleModelInstance.class);
            }

            shader.setTexture(vehicleModelInstance.tex, "Texture0", 0);
            GL11.glTexEnvi(8960, 8704, 7681);
            shader.setTexture(vehicleModelInstance.textureRust, "TextureRust",
                              1);
            GL11.glTexEnvi(8960, 8704, 7681);
            shader.setTexture(vehicleModelInstance.textureMask, "TextureMask",
                              2);
            GL11.glTexEnvi(8960, 8704, 7681);
            shader.setTexture(vehicleModelInstance.textureLights,
                              "TextureLights", 3);
            GL11.glTexEnvi(8960, 8704, 7681);
            shader.setTexture(vehicleModelInstance.textureDamage1Overlay,
                              "TextureDamage1Overlay", 4);
            GL11.glTexEnvi(8960, 8704, 7681);
            shader.setTexture(vehicleModelInstance.textureDamage1Shell,
                              "TextureDamage1Shell", 5);
            GL11.glTexEnvi(8960, 8704, 7681);
            shader.setTexture(vehicleModelInstance.textureDamage2Overlay,
                              "TextureDamage2Overlay", 6);
            GL11.glTexEnvi(8960, 8704, 7681);
            shader.setTexture(vehicleModelInstance.textureDamage2Shell,
                              "TextureDamage2Shell", 7);
            GL11.glTexEnvi(8960, 8704, 7681);

            try {
              if (Core.getInstance().getPerfReflectionsOnLoad()) {
                shader.setTexture(
                    (Texture)SkyBox.getInstance().getTextureCurrent(),
                    "TextureReflectionA", 8);
                GL11.glTexEnvi(8960, 8704, 7681);
                GL11.glGetError();
              }
            ...
            try {
              if (Core.getInstance().getPerfReflectionsOnLoad()) {
                shader.setTexture(
                    (Texture)SkyBox.getInstance().getTexturePrev(),
                    "TextureReflectionB", 9);
                GL11.glTexEnvi(8960, 8704, 7681);
                GL11.glGetError();
              }
            ...
            shader.setReflectionParam(SkyBox.getInstance().getTextureShift(),
                                      vehicleModelInstance.refWindows,
                                      vehicleModelInstance.refBody);
            ...
            shader.setTextureLightsEnables1(
                vehicleModelInstance.textureLightsEnables1);
            shader.setTextureLightsEnables2(
                vehicleModelInstance.textureLightsEnables2);
            shader.setTextureDamage1Enables1(
                vehicleModelInstance.textureDamage1Enables1);
            shader.setTextureDamage1Enables2(
                vehicleModelInstance.textureDamage1Enables2);
            shader.setTextureDamage2Enables1(
                vehicleModelInstance.textureDamage2Enables1);
            shader.setTextureDamage2Enables2(
                vehicleModelInstance.textureDamage2Enables2);
            shader.setMatrixBlood1(vehicleModelInstance.matrixBlood1Enables1,
                                   vehicleModelInstance.matrixBlood1Enables2);
            shader.setMatrixBlood2(vehicleModelInstance.matrixBlood2Enables1,
                                   vehicleModelInstance.matrixBlood2Enables2);
            shader.setTextureRustA(vehicleModelInstance.textureRustA);
            shader.setTexturePainColor(vehicleModelInstance.painColor,
                                       slotData.alpha);
            if (this.bStatic) {
              shader.setTransformMatrix(IDENTITY, false);
            } else {
        ...
        if (Core.bDebug && DebugOptions.instance.ModelRenderLights.getValue() &&
            instData == slotData.modelData.get(0)) {
        ...
        ModelCamera.instance.End();
        PZGLUtil.checkGLErrorThrow(
            "Model.drawVehicle Exit inst: %s, instTex: %s, slotData: %s",
            modelInstance, texture, slotData);
      }
    }
  }

  static void debugDrawAxis(float x, float y, float z, float length,
                            float thickness) {
  ...
  void setLights(ModelSlotRenderData modelSlotRenderData, int int1) {
    for (int int0 = 0; int0 < int1; int0++) {
      ...
      this.Effect.setLight(int0, effectLight.x, effectLight.y, effectLight.z,
  ...
  void CreateShader(const std::string &name) {
    if (!ModelManager.NoOpenGL) {
      Lambda.invoke(
          ...
          (model, string)->model.Effect =
              ShaderManager.instance.getOrCreateShader(string, model.bStatic));
    }
  }
...
public
  static final class ModelAssetParams extends AssetManager.AssetParams {
    std::string meshName;
