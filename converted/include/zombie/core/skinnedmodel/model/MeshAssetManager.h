...
public:
 ...
 private HashSet<String> m_watchedFiles = std::make_unique<HashSet<>>();
 const PredicatedFileWatcher m_watcher = new PredicatedFileWatcher(MeshAssetManager::isWatched, MeshAssetManager::watchedFileChanged);
 ...
 ModelMesh modelMesh = (ModelMesh)asset;
 if (!modelMesh.empty() && string1.equalsIgnoreCase(modelMesh.m_fullPath) {
