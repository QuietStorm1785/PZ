...
public:
 ...
 public Shader(const std::string& _name, bool _bStatic) {
 this->name = _name;
 ...
 void startCharacter(ModelSlotRenderData slotData, ModelInstanceRenderData instData) {
 if (this->bStatic) {
 ...
 if (texture1 != nullptr && texture1.getTextureId() != nullptr && texture1.getTextureId().hasMipMaps()) {
 ...
 for (auto& matrix4f : matrix4fs) matrix4f.store(floatBuffer);
 }
 ...
 void setShaderAlpha(float a) {
 ARBShaderObjects.glUniform1fARB(this->Alpha, a);
 }

 void setLight(int index, float x, float y, float z, float r, float g, float b, float rad, float animPlayerAngle, ModelInstance inst) {
 float float0 = 0.0F;
 float float1 = 0.0F;
 float float2 = 0.0F;
