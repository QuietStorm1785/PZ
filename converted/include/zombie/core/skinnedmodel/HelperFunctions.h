...
public:
 ...
 private static Stack<Matrix4f> MatrixStack = std::make_unique<Stack<>>();
 static const AtomicBoolean MatrixLock = new AtomicBoolean(false);
 private static ObjectPool<Vector3f> VectorPool = new ObjectPool<>(Vector3f::new);
 ...
 static Matrix4f CreateFromQuaternion(Quaternion quaternion, Matrix4f matrix4f) {
 ...
 static Matrix4f CreateFromQuaternionPositionScale(Vector3f vector3f0, Quaternion quaternion, Vector3f vector3f1, Matrix4f matrix4f3) {
 Matrix4f matrix4f0 = getMatrix();
 ...
 static void TransformVertices(VertexPositionNormalTangentTextureSkin[] vertexPositionNormalTangentTextureSkins, List<Matrix4f> list) {
 Vector3 vector30 = new Vector3();
 Vector3 vector31 = new Vector3();
 Vector4f vector4f = new Vector4f();

 for (auto& vertexPositionNormalTangentTextureSkin : vertexPositionNormalTangentTextureSkins) vector30.reset();
 vector31.reset();
 ...
 static void ApplyBlendBone(float float0, Matrix4f matrix4f, Vector3 vector30, Vector3 vector32, Vector4f var4, Vector3 vector31, Vector3 vector33) {
 if (float0 > 0.0F) {
 ...
 static void transform(Quaternion quaternion, Vector3f vector3f0, Vector3f vector3f1) {
 ...
 static Vector4f transform(Matrix4f matrix4f, Vector4f vector4f0, Vector4f vector4f1) {
 ...
 static Quaternion ToQuaternion(double double7, double double4, double double1, Quaternion quaternion) {
 ...
 private static class getRotationMatrix {
 static void run() {
