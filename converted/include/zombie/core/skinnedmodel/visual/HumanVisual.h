...
public:
 ...
 int bodyHair = -1;
 private byte[] blood = new byte[BloodBodyPartType.MAX.index()];
 private byte[] dirt = new byte[BloodBodyPartType.MAX.index()];
 private byte[] holes = new byte[BloodBodyPartType.MAX.index()];
 ...
 private static ArrayList<String> itemVisualLocations = std::make_unique<ArrayList<>>();
 ...
 void setSkinColor(ImmutableColor color) {
 ...
 void setBodyHairIndex(int index) {
 this->bodyHair = index;
 }
 ...
 void setSkinTextureIndex(int index) {
 ...
 std::string string = "";
 std::vector arrayList = this->owner.isFemale() ? PopTemplateManager.instance.m_FemaleSkins : PopTemplateManager.instance.m_MaleSkins;
 ...
 void setHairColor(ImmutableColor color) {
 ...
 void setBeardColor(ImmutableColor color) {
 ...
 void setHairModel(const std::string& model) {
 ...
 void setBeardModel(const std::string& model) {
 ...
 this->beardModel = this->owner.isFemale() ? nullptr : _outfit.RandomData.m_beardName;
 this->getSkinTexture();
 }
 }

 void dressInNamedOutfit(const std::string& outfitName, ItemVisuals itemVisuals) {
 itemVisuals.clear();
 ...
 void dressInClothingItem(const std::string& itemGUID, ItemVisuals itemVisuals, bool clearCurrentVisuals) {
 ...
 ItemVisual addBodyVisualFromClothingItemName(const std::string& clothingItemName) {
 ...
 if (this->bodyVisuals.get(int0).getClothingItemName() == clothingItemName) {
 return nullptr;
 }
 }
 ...
 void getItemVisualLocations(ItemVisuals itemVisuals, ArrayList<String> arrayList) {
 arrayList.clear();
 ...
 this->getItemVisualLocations(itemVisuals, itemVisualLocations);
 return this->addClothingItem(itemVisuals, itemVisualLocations, clothingItem.m_Name, nullptr);
 }
 }
 }

 ItemVisual addClothingItem(ItemVisuals itemVisuals, ArrayList<String> arrayList, const std::string& string0, ClothingItemReference clothingItemReference) {
 assert itemVisuals.size() == arrayList.size();

 if (clothingItemReference != nullptr && !clothingItemReference.RandomData.m_Active) {
 return nullptr;
 ...
 void setForceModel(Model model) {
 ...
 static StringBuilder toString(ImmutableColor immutableColor, StringBuilder stringBuilder) {
