...
public:
  ...
  ImmutableColor m_Tint = null;
  int m_BaseTexture = -1;
  int m_TextureChoice = -1;
  std::string m_Decal = null;
private
  byte[] blood;
private
  byte[] dirt;
...
public
  ...
  float getDenimPatch(BloodBodyPartType bodyPartType) {
    return this.denimPatches == nullptr
               ? 0.0F
  ...
  float getLeatherPatch(BloodBodyPartType bodyPartType) {
    return this.leatherPatches == nullptr
               ? 0.0F
  ...
  float getBasicPatch(BloodBodyPartType bodyPartType) {
    return this.basicPatches == nullptr
               ? 0.0F
