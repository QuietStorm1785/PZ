...
public:
 ...
 public ArrayList<IAnimEventCallback> animCallbackHandlers = std::make_unique<ArrayList<>>();
 AnimLayer m_rootLayer = nullptr;
 private List<AdvancedAnimator.SubLayerSlot> m_subLayers = std::make_unique<ArrayList<>>();
 ...
 if (mod != nullptr && mod.animSetsFile != nullptr && string0.startsWith(mod.animSetsFile.getPath())) {
 return true;
 }
 }
 ...
 std::string string1 = (String)arrayList.get(int0);
 ChooseGameInfo.Mod mod = ChooseGameInfo.getModDetails(string1);
 if (mod != nullptr && mod.actionGroupsFile != nullptr && string0.startsWith(mod.actionGroupsFile.getPath())) {
 return true;
 }
 }
 ...
 std::string string0 = element.getElementsByTagName("MotionScale").item(0).getTextContent();
 s_MotionScale = Float.parseFloat(string0);
 std::string string1 = element.getElementsByTagName("RotationScale").item(0).getTextContent();
 ...
 stringBuilder.append(characterx.getCurrentState() == nullptr ? "nullptr" : characterx.getCurrentState().getClass().getSimpleName()).append("\n");
 }
 ...
 void SetAnimSet(AnimationSet aset) {
 this->animSet = aset;
 }
 ...
 if (this->debugMonitor != nullptr && this->character instanceof IsoGameCharacter) {
 ...
 for (AnimNode animNode : animState.m_Nodes) {
 for (AnimCondition animCondition : animNode.m_Conditions) {
 if (animCondition.m_Name != nullptr && !arrayList.contains(animCondition.m_Name.toLowerCase())) {
 ...
 if (liveAnimNode.isActive() && liveAnimNode.getSourceNode() != nullptr && variable == liveAnimNode.getSourceNode().m_SpeedScaleVariable) {
