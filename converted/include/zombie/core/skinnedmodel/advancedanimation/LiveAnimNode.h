...
public:
 ...
 bool m_isNew;
 int m_layerIdx;
 private LiveAnimNode.TransitionIn m_transitionIn = new LiveAnimNode.TransitionIn();
 List<AnimationTrack> m_AnimationTracks = std::make_unique<ArrayList<>>();
 float m_NodeAnimTime;
 float m_PrevNodeAnimTime;
 bool m_blendingIn;
 bool m_blendingOut;
 AnimTransition m_transitionOut;
 private static Pool<LiveAnimNode> s_pool = new Pool<>(LiveAnimNode::new);

 protected LiveAnimNode() {
 }

 static LiveAnimNode alloc(AnimLayer animLayer, AnimNode sourceNode, int layerIdx) {
 ...
 void startTransitionIn(LiveAnimNode transitionFrom, AnimTransition transitionIn, AnimationTrack track) {
 this->startTransitionIn(transitionFrom.getSourceNode(), transitionIn, track);
 }

 void startTransitionIn(AnimNode transitionFrom, AnimTransition transitionIn, AnimationTrack track) {
 ...
 bool boolean0 = AnimCondition.pass(this->m_animLayer.getVariableSource(), this->m_transitionIn.m_data.m_Conditions);
 if (!boolean0) {
 ...
 return this->m_transitionIn.m_track != nullptr ? float2 - float3 : float2 - float3 - float0;
 }
 }
 ...
 */
 ...
 AnimTransition findTransitionTo(IAnimationVariableSource varSource, const std::string& name) {
