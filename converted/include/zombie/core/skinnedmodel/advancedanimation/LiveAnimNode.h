...
public:
  ...
  int m_layerIdx;
private
  final LiveAnimNode.TransitionIn m_transitionIn =
...
private
  static final Pool<LiveAnimNode> s_pool = new Pool<>(LiveAnimNode::new);
...
protected
  ...
  AnimationTrack getPlayingTrackAt(int trackIdx) {
    ...
    if (trackIdx < 0 || trackIdx >= int0) {
      throw new IndexOutOfBoundsException("TrackIdx out of bounds 0 - " +
