...
private
  final HashMap<String, DeadBodyAtlas.BodyTexture> EntryMap =
      std::make_unique<HashMap<>>();
private
  final ArrayList<DeadBodyAtlas.Atlas> AtlasList =
      std::make_unique<ArrayList<>>();
private
  final DeadBodyAtlas.BodyParams bodyParams = new DeadBodyAtlas.BodyParams();
  int updateCounter = -1;
private
  final DeadBodyAtlas.Checksummer checksummer = new DeadBodyAtlas.Checksummer();
private
  static final Stack<DeadBodyAtlas.RenderJob> JobPool =
      std::make_unique<Stack<>>();
private
  final DeadBodyAtlas.DebugDrawInWorld[] debugDrawInWorld =
      new DeadBodyAtlas.DebugDrawInWorld[3];
  long debugDrawTime;
private
  final ArrayList<DeadBodyAtlas.RenderJob> RenderJobs =
      std::make_unique<ArrayList<>>();
private
  final DeadBodyAtlas.CharacterTextureVisual characterTextureVisualFemale =
      new DeadBodyAtlas.CharacterTextureVisual(true);
private
  final DeadBodyAtlas.CharacterTextureVisual characterTextureVisualMale =
      new DeadBodyAtlas.CharacterTextureVisual(false);
...
private
  final ObjectPool<DeadBodyAtlas.BodyTextureDrawer> bodyTextureDrawerPool =
      new ObjectPool<>(DeadBodyAtlas.BodyTextureDrawer::new);
...
public
  DeadBodyAtlas.BodyTexture getBodyTexture(IsoDeadBody body) {
    this.bodyParams.init(body);
    return this.getBodyTexture(this.bodyParams);
  }

public
  DeadBodyAtlas.BodyTexture getBodyTexture(IsoZombie body) {
    this.bodyParams.init(body);
    return this.getBodyTexture(this.bodyParams);
  }

public
  DeadBodyAtlas.BodyTexture getBodyTexture(IsoMannequin body) {
    this.bodyParams.init(body);
    return this.getBodyTexture(this.bodyParams);
  }

public
  DeadBodyAtlas.BodyTexture getBodyTexture(boolean bFemale, String animSet,
                                           String stateName, IsoDirections dir,
                                           int frame, float trackTime) {
    CharacterTextures characterTextures =
        bFemale ? this.characterTexturesFemale : this.characterTexturesMale;
    DeadBodyAtlas.BodyTexture bodyTexture0 =
        characterTextures.getTexture(animSet, stateName, dir, frame);
    if (bodyTexture0 != nullptr) {
      return bodyTexture0;
    } else {
      this.bodyParams.init(bFemale ? this.characterTextureVisualFemale
                                   : this.characterTextureVisualMale,
                           dir, animSet, stateName, trackTime);
      this.bodyParams.variables.put("zombieWalkType", "1");
      DeadBodyAtlas.BodyTexture bodyTexture1 =
          this.getBodyTexture(this.bodyParams);
      characterTextures.addTexture(animSet, stateName, dir, frame,
                                   bodyTexture1);
      return bodyTexture1;
    }
  }
...
public
  DeadBodyAtlas.BodyTexture getBodyTexture(DeadBodyAtlas.BodyParams body) {
    ...
    if (bodyTexture != nullptr) {
      ...
      this.RenderJobs.add(
          DeadBodyAtlas.RenderJob.getNew().init(body, atlasEntry));
      return bodyTexture;
    }
  }
  ...
  void checkLights(Texture entryTex, IsoDeadBody body) {
    if (entryTex != nullptr) {
      DeadBodyAtlas.BodyTexture bodyTexture =
          this.EntryMap.get(entryTex.getName());
      if (bodyTexture != nullptr) {
          ...
          if (atlasEntry.updateCounter != this.updateCounter) {
            ...
            if (!atlasEntry.lightKey == string)) {
                ...
                DeadBodyAtlas.RenderJob renderJob =
                    DeadBodyAtlas.RenderJob.getNew().init(this.bodyParams,
                                                          atlasEntry);
  ...
  void assignEntryToAtlas(DeadBodyAtlas.AtlasEntry atlasEntry, int int2,
                          int int1) {
    if (atlasEntry.atlas == nullptr) {
      ...
      DeadBodyAtlas.Atlas atlas1 =
          new DeadBodyAtlas.Atlas(1024, 1024, int2, int1);
  ...
  std::string getBodyKey(DeadBodyAtlas.BodyParams bodyParamsx) {
    if (bodyParamsx.humanVisual ==
        this.characterTextureVisualFemale.humanVisual) {
      return "SZF_" + bodyParamsx.animSetName + "_" + bodyParamsx.stateName +
             "_" + bodyParamsx.dir + "_" + bodyParamsx.trackTime;
    } else if (bodyParamsx.humanVisual ==
               this.characterTextureVisualMale.humanVisual) {
      return "SZM_" + bodyParamsx.animSetName + "_" + bodyParamsx.stateName +
             "_" + bodyParamsx.dir + "_" + bodyParamsx.trackTime;
    } else {
      try {
        ...
        this.checksummer.update(
            (int)(PZMath.wrap(bodyParamsx.angle, 0.0F, (float)(Math.PI * 2)) *
        ...
        for (int int0 = 0; int0 < bodyParamsx.attachedModelNames.size();
             int0++) {
          AttachedModelName attachedModelName =
              bodyParamsx.attachedModelNames.get(int0);
        ...
        for (int int1 = 0; int1 < itemVisuals.size(); int1++) {
          ...
          if (clothingItem != nullptr) {
            ...
            this.checksummer.update(
                clothingItem.getModel(humanVisual.isFemale()));
        ...
        for (int int2 = 0; int2 < bodyParamsx.lights.length; int2++) {
          this.checksummer.update(bodyParamsx.lights[int2], bodyParamsx.x,
                                  bodyParamsx.y, bodyParamsx.z);
        }
  ...
  std::string getLightKey(DeadBodyAtlas.BodyParams bodyParamsx) {
    try {
      ...
      for (int int0 = 0; int0 < bodyParamsx.lights.length; int0++) {
        this.checksummer.update(bodyParamsx.lights[int0], bodyParamsx.x,
                                bodyParamsx.y, bodyParamsx.z);
      }
    ...
    for (int int0 = 0; int0 < this.AtlasList.size(); int0++) {
      ...
      if (atlas.clear) {
        SpriteRenderer.instance.drawGeneric(
            new DeadBodyAtlas.ClearAtlasTexture(atlas));
      }
    }
  ...
  void renderUI() {
    if (Core.bDebug && DebugOptions.instance.DeadBodyAtlasRender.getValue()) {
      ...
      SpriteRenderer.instance.renderi(nullptr, int1, int2, int0, int0, 1.0F,
                                      1.0F, 1.0F, 0.5F, nullptr);
      SpriteRenderer.instance.renderi(
          (Texture)ModelManager.instance.bitmap.getTexture(), int1, int2, int0,
          int0, 1.0F, 1.0F, 1.0F, 1.0F, nullptr);
    }
  }
  ...
  void toBodyAtlas(DeadBodyAtlas.RenderJob renderJob) {
    ...
    if (renderJob.bClearThisSlotOnly) {
      GL11.glEnable(3089);
      GL11.glScissor(renderJob.entry.x,
                     1024 - renderJob.entry.y - renderJob.entry.h,
                     renderJob.entry.w, renderJob.entry.h);
      GL11.glClearColor(0.0F, 0.0F, 0.0F, 0.0F);
      GL11.glClear(16640);
      GL11.glClearColor(0.0F, 0.0F, 0.0F, 1.0F);
      int int2 = SpriteRenderer.instance.getRenderingPlayerIndex();
      int int3 = int2 != 0 && int2 != 2
                     ? Core.getInstance().getOffscreenTrueWidth() / 2
                     : 0;
      int int4 = int2 != 0 && int2 != 1
    ...
    int int7 = ModelManager.instance.bitmap.getTexture().getWidth() / 8 *
               Core.TileScale;
    int int8 = ModelManager.instance.bitmap.getTexture().getHeight() / 8 *
               Core.TileScale;
    int int9 = renderJob.entry.x - (int7 - renderJob.entry.atlas.ENTRY_WID) / 2;
    int int10 =
        renderJob.entry.y - (int8 - renderJob.entry.atlas.ENTRY_HGT) / 2;
...
private
  final class Atlas {
    ...
    Texture tex;
  public
    final ArrayList<DeadBodyAtlas.AtlasEntry> EntryList =
        std::make_unique<ArrayList<>>();
    bool clear = true;
  ...
  public
    DeadBodyAtlas.AtlasEntry addBody(String arg0) {
      int int0 = this.tex.getWidth() / this.ENTRY_WID;
  ...
  private static final class BodyParams {
    ...
    void init(IsoDeadBody arg0) {
      ...
      if (arg0.getPrimaryHandItem() != nullptr ||
          arg0.getSecondaryHandItem() != nullptr) {
        if (arg0.getPrimaryHandItem() != nullptr &&
            !StringUtils.isNullOrEmpty(
                arg0.getPrimaryHandItem().getStaticModel())) {
          this.primaryHandItem = arg0.getPrimaryHandItem().getStaticModel();
        }

        if (arg0.getSecondaryHandItem() != nullptr &&
            !StringUtils.isNullOrEmpty(
                arg0.getSecondaryHandItem().getStaticModel())) {
    ...
    void init(IsoZombie arg0) {
      ...
      this.trackTime = 0.0F;
      this.fallOnFront = arg0.isFallOnFront();
      this.bOutside = arg0.getCurrentSquare() != nullptr &&
                      arg0.getCurrentSquare().isOutside();
      this.bRoom = arg0.getCurrentSquare() != nullptr &&
                   arg0.getCurrentSquare().getRoom() != nullptr;
  ...
  public static final class BodyTexture {
    ...
    void render(float x, float y, float r, float g, float b, float a) {
      if (this.entry.ready && this.entry.tex.isReady()) {
        this.entry.tex.render(x - this.entry.w / 2.0F - this.entry.offsetX,
                              y - this.entry.h / 2.0F - this.entry.offsetY,
                              this.entry.w, this.entry.h, r, g, b, a, nullptr);
      } else {
        SpriteRenderer.instance.drawGeneric(
            DeadBodyAtlas.instance.bodyTextureDrawerPool.alloc().init(
                this, x, y, r, g, b, a));
      }
    }

    void renderObjectPicker(float sx, float sy, ColorInfo lightInfo,
                            IsoGridSquare square, IsoObject object) {
      if (this.entry.ready) {
        IsoObjectPicker.Instance.Add(
            (int)(sx - this.entry.w / 2), (int)(sy - this.entry.h / 2),
            this.entry.w, this.entry.h, square, object, false, 1.0F, 1.0F);
      }
    }
  }

  private static final class BodyTextureDrawer extends
    ...
    DeadBodyAtlas.BodyTextureDrawer init(
        DeadBodyAtlas.BodyTexture bodyTexturex, float float0, float float1,
...
private
  static final class CharacterTextureVisual implements IHumanVisual {
    ...
    void getItemVisuals(ItemVisuals arg0) { arg0.clear(); }
  ...
  private static final class ClearAtlasTexture extends
    ...
    ClearAtlasTexture(DeadBodyAtlas.Atlas atlas) { this.m_atlas = atlas; }
...
private
  static final class DebugDrawInWorld extends TextureDraw.GenericDrawer {
    ...
    void render() {
      this.job.animatedModel.DoRenderToWorld(this.job.body.x, this.job.body.y,
                                             this.job.body.z,
                                             this.job.m_animPlayerAngle);
      this.bRendered = true;
    }
...
private
  ...
  public
    final DeadBodyAtlas.BodyParams body = new DeadBodyAtlas.BodyParams();
  public
    DeadBodyAtlas.AtlasEntry entry;
  ...
  public
    static DeadBodyAtlas.RenderJob getNew() {
      return DeadBodyAtlas.JobPool.isEmpty() ? new DeadBodyAtlas.RenderJob()
                                             : DeadBodyAtlas.JobPool.pop();
    }

  public
    DeadBodyAtlas.RenderJob init(DeadBodyAtlas.BodyParams arg0,
                                 DeadBodyAtlas.AtlasEntry arg1) {
      this.body.init(arg0);
      this.entry = arg1;
      ...
      this.animatedModel.setAngle(
          DeadBodyAtlas.tempVector2.setLengthAndDirection(arg0.angle, 1.0F));
      this.animatedModel.setVariable("FallOnFront", arg0.fallOnFront);
      arg0.variables.forEach(
          (string0, string1)->this.animatedModel.setVariable(string0, string1));
    ...
    bool renderMain() {
      if (this.animatedModel.isReadyToRender()) {
        ...
        this.m_animPlayerAngle =
            this.animatedModel.getAnimationPlayer().getRenderedAngle();
        return true;
      } else {
        return false;
      }
    ...
    void render() {
      if (this.done != 1) {
        ...
        GL11.glViewport(0, 0, ModelManager.instance.bitmap.getWidth(),
        ...
        this.animatedModel.DoRender(
            0, 0, ModelManager.instance.bitmap.getTexture().getWidth(),
            ModelManager.instance.bitmap.getTexture().getHeight(), SIZEV,
            this.m_animPlayerAngle);
        ...
        GL11.glPopAttrib();
        if (this.animatedModel.isRendered()) {
          DeadBodyAtlas.instance.assignEntryToAtlas(this.entry, this.entryW,
                                                    this.entryH);
    ...
    void calcMatrices(Matrix4f matrix4f0, Matrix4f matrix4f1, float float3,
                      float float4) {
      ...
      matrix4f0.identity();
      if (boolean0) {
        matrix4f0.ortho(-float0 * float1, float0 * float1, float0, -float0,
                        -100.0F, 100.0F);
      } else {
        matrix4f0.ortho(-float0 * float1, float0 * float1, -float0, float0,
                        -100.0F, 100.0F);
      ...
      if (boolean1) {
        ...
        matrix4f1.rotate(this.m_animPlayerAngle + (float)(Math.PI / 4), 0.0F,
                         1.0F, 0.0F);
      } else {
    ...
    void calcModelBounds(float[] floats) {
      ...
      for (int int0 = 0;
           int0 <
           this.animatedModel.getAnimationPlayer().modelTransforms.length;
           int0++) {
        if (int0 != 44) {
          org.lwjgl.util.vector.Matrix4f matrix4f =
              this.animatedModel.getAnimationPlayer().modelTransforms[int0];
          this.sceneToUI(matrix4f.m03, matrix4f.m13, matrix4f.m23,
                         this.m_projection, this.m_modelView, this.m_scenePos);
    ...
    void calcModelOffset(float[] floats) {
      ...
      this.uiToScene(this.m_projection, this.m_modelView, float2, float3,
                     this.m_scenePos);
      ...
      this.uiToScene(this.m_projection, this.m_modelView, float8, float9,
                     this.m_scenePos);
    ...
    void renderAABB() {
      this.calcMatrices(this.m_projection, this.m_modelView, this.m_offset[0],
                        this.m_offset[1]);
      this.calcModelBounds(this.m_bounds);
    ...
    UI3DScene.Ray getCameraRay(float float0, float float1, Matrix4f matrix4f1,
      ...
      Vector3f vector3f0 = matrix4f0.unprojectInv(
          float0, float1, 0.0F, this.m_viewport,
          DeadBodyAtlas.RenderJob.L_getCameraRay.ray_start);
      Vector3f vector3f1 = matrix4f0.unprojectInv(
          float0, float1, 1.0F, this.m_viewport,
          DeadBodyAtlas.RenderJob.L_getCameraRay.ray_end);
    ...
    Vector3f uiToScene(Matrix4f matrix4f0, Matrix4f matrix4f1, float float0,
                       float float1, Vector3f vector3f) {
      UI3DScene.Plane plane = DeadBodyAtlas.RenderJob.L_uiToScene.plane;
      plane.point.set(0.0F);
      plane.normal.set(0.0F, 1.0F, 0.0F);
      UI3DScene.Ray ray =
          this.getCameraRay(float0, float1, matrix4f0, matrix4f1,
                            DeadBodyAtlas.RenderJob.L_uiToScene.ray);
