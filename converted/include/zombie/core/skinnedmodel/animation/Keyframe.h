...
public:
 ...
 static void setIdentity(Vector3f out_pos, Quaternion out_rot, Vector3f out_scale) {
 setIfNotNull(out_pos, 0.0F, 0.0F, 0.0F);
 setIdentityIfNotNull(out_rot);
 setIfNotNull(out_scale, 1.0F, 1.0F, 1.0F);
 }

 static Keyframe lerp(Keyframe a, Keyframe b, float time, Keyframe out_result) {
 ...
 static void setIfNotNull(Vector3f to, Vector3f val, float default_x, float default_y, float default_z) {
 if (to != nullptr) {
 if (val != nullptr) {
 to.set(val);
 } else {
 ...
 static void lerp(Keyframe a, Keyframe b, float time, Vector3f out_pos, Quaternion out_rot, Vector3f out_scale) {
 if (b.Time == a.Time) {
