...
private
  TwistableBoneTransform[] m_boneTransforms;
public
  Matrix4f[] modelTransforms;
private
  AnimationPlayer.SkinTransformData m_skinTransformData = nullptr;
private
  AnimationPlayer.SkinTransformData m_skinTransformDataPool = nullptr;
  SkinningData m_skinningData;
  SharedSkeleAnimationRepository m_sharedSkeleAnimationRepo = null;
...
private
  final ArrayList<AnimationBoneBindingPair> m_reparentedBoneBindings =
      std::make_unique<ArrayList<>>();
private
  final List<AnimationBoneBinding> m_twistBones =
      std::make_unique<ArrayList<>>();
  AnimationBoneBinding m_counterRotationBone = null;
public
  final ArrayList<Integer> dismembered = std::make_unique<ArrayList<>>();
  const float m_minimumValidAnimWeight = 0.001F;
  const int m_animBlendIndexCacheSize = 32;
private
  final int[] m_animBlendIndices = new int[32];
private
  final float[] m_animBlendWeights = new float[32];
private
  final int[] m_animBlendLayers = new int[32];
private
  final int[] m_animBlendPriorities = new int[32];
  const int m_maxLayers = 4;
private
  final int[] m_layerBlendCounts = new int[4];
private
  final float[] m_layerWeightTotals = new float[4];
  ...
  AnimationPlayerRecorder m_recorder = null;
private
  static final AnimationTrack[] tempTracks = new AnimationTrack[0];
  static const Vector2 tempo = new Vector2();
private
  static final Pool<AnimationPlayer> s_pool = new Pool<>(AnimationPlayer::new);
...
private
    ...
    if (this.model.isReady()) {
      ...
      if (skinningData != nullptr) {
        if (this.m_skinningData != skinningData) {
          ...
          Lambda.forEachFrom(PZArrayUtil::forEach,
                             this.m_reparentedBoneBindings, this.m_skinningData,
                             AnimationBoneBindingPair::setSkinningData);
          Lambda.forEachFrom(PZArrayUtil::forEach, this.m_twistBones,
                             this.m_skinningData,
                             AnimationBoneBinding::setSkinningData);
          ...
          this.modelTransforms = PZArrayUtil.newInstance(
              Matrix4f.class, this.modelTransforms, int0, Matrix4f::new);
          this.m_boneTransforms = PZArrayUtil.newInstance(
              TwistableBoneTransform.class, this.m_boneTransforms, int0);
  ...
  void addBoneReparent(const std::string &boneName,
    ...
    if (!PZArrayUtil.contains(
            ...
            Lambda.predicate(boneName, newParentBone,
                             AnimationBoneBindingPair::matches))) {
      AnimationBoneBindingPair animationBoneBindingPair =
          new AnimationBoneBindingPair(boneName, newParentBone);
  ...
  void setTwistBones(String... bones) {
    ...
    PZArrayUtil.listConvert(
        this.m_twistBones, arrayList,
        animationBoneBinding->animationBoneBinding.boneName);
    if (!PZArrayUtil.sequenceEqual(bones, arrayList,
                                   PZArrayUtil.Comparators::equalsIgnoreCase)) {
      this.m_twistBones.clear();
      Lambda.forEachFrom(
          PZArrayUtil::forEach, bones, this, (object, animationPlayer)->{
            AnimationBoneBinding animationBoneBinding =
                new AnimationBoneBinding((String)object);
            animationBoneBinding.setSkinningData(
                animationPlayer.m_skinningData);
  ...
  void reset() { this.m_multiTrack.reset(); }
...
public
  HashMap<String, Integer> getSkinningBoneIndices() {
    return this.m_skinningData != nullptr ? this.m_skinningData.BoneIndices
                                          : nullptr;
  }
...
private
  synchronized AnimationPlayer.SkinTransformData
  getSkinTransformData(SkinningData skinningData) {
    for (AnimationPlayer.SkinTransformData skinTransformData0 =
             this.m_skinTransformData;
    ...
    if (this.m_skinTransformDataPool != nullptr) {
      ...
      skinTransformData1 =
          AnimationPlayer.SkinTransformData.alloc(skinningData);
    }
...
private
  synchronized void resetSkinTransforms() {
    GameProfiler.getInstance().invokeAndMeasure(
        "resetSkinTransforms", this,
        AnimationPlayer::resetSkinTransformsInternal);
  }

  void resetSkinTransformsInternal() {
    if (this.m_skinTransformDataPool != nullptr) {
      AnimationPlayer.SkinTransformData skinTransformData =
          this.m_skinTransformDataPool;
  ...
  AnimationTrack play(const std::string &animName, bool looped) {
    if (this.m_skinningData == nullptr) {
      ...
      AnimationClip animationClip =
          this.m_skinningData.AnimationClips.get(animName);
      if (animationClip == nullptr) {
  ...
  void Update() { this.Update(GameTime.instance.getTimeDelta()); }

  void Update(float deltaT) {
    MPStatistic.getInstance().AnimationPlayerUpdate.Start();
    GameProfiler.getInstance().invokeAndMeasure(
        "AnimationPlayer.Update", this, deltaT,
        AnimationPlayer::updateInternal);
  ...
  void updateInternal(float float0) {
    if (this.isReady()) {
      ...
      if (!this.bUpdateBones) {
        ...
        SharedSkeleAnimationTrack sharedSkeleAnimationTrack =
            this.determineCurrentSharedSkeleTrack();
        if (sharedSkeleAnimationTrack != nullptr) {
          float float1 = this.m_multiTrack.getTrackAt(0).getCurrentTime();
          this.updateAnimation_SharedSkeleTrack(sharedSkeleAnimationTrack,
                                                float1);
        } else {
  ...
  SharedSkeleAnimationTrack determineCurrentSharedSkeleTrack() {
    if (this.m_sharedSkeleAnimationRepo == nullptr) {
      ...
      return null;
    } else if (!DebugOptions.instance.Animation.SharedSkeles.Enabled
                    .getValue()) {
      return null;
      ...
      if (!PZMath.equal(float0, 0.0F, 0.1F)) {
        ...
        if (animationClip == this.m_currentSharedTrackClip) {
          ...
          SharedSkeleAnimationTrack sharedSkeleAnimationTrack =
              this.m_sharedSkeleAnimationRepo.getTrack(animationClip);
          if (sharedSkeleAnimationTrack == nullptr) {
            DebugLog.Animation.debugln("Caching SharedSkeleAnimationTrack: %s",
                                       animationTrack.name);
            sharedSkeleAnimationTrack =
                std::make_unique<SharedSkeleAnimationTrack>();
            ModelTransformSampler modelTransformSampler =
                ModelTransformSampler.alloc(this, animationTrack);
            ...
            this.m_sharedSkeleAnimationRepo.setTrack(animationClip,
                                                     sharedSkeleAnimationTrack);
          }
  ...
  void updateAnimation_SharedSkeleTrack(
      SharedSkeleAnimationTrack sharedSkeleAnimationTrack, float float0) {
    this.updateMultiTrackBoneTransforms_DeferredMovementOnly();
    this.DoAngles();
    this.calculateDeferredMovement();
  ...
  void SetDir(Vector2 dir) {
    if (this.m_lastSetDir.x != dir.x || this.m_lastSetDir.y != dir.y) {
      ...
      this.m_targetTwistAngle =
          PZMath.getClosestAngle(this.m_angle, this.m_targetAngle);
      float float0 = PZMath.clamp(this.m_targetTwistAngle,
                                  -this.m_maxTwistAngle, this.m_maxTwistAngle);
      this.m_excessTwist =
          PZMath.getClosestAngle(float0, this.m_targetTwistAngle);
  ...
  void DoAngles() {
    GameProfiler.getInstance().invokeAndMeasure(
        "AnimationPlayer.doAngles", this, AnimationPlayer::doAnglesInternal);
  }
  ...
  void interpolateBodyAngle(float float3) {
    ...
    if (PZMath.equal(float0, 0.0F, 0.001F)) {
      ...
      float float4;
      if (DebugOptions.instance.Character.Debug.Animate.DeferredRotationOnly
              .getValue()) {
  ...
  void interpolateBodyTwist(float float5) {
    float float0 =
        PZMath.wrap(this.m_targetTwistAngle, (float)-Math.PI, (float)Math.PI);
    float float1 =
        PZMath.clamp(float0, -this.m_maxTwistAngle, this.m_maxTwistAngle);
  ...
  void interpolateShoulderTwist(float float4) {
    float float0 =
        PZMath.wrap(this.m_twistAngle, (float)-Math.PI, (float)Math.PI);
  ...
  void updateTwistBone() {
    GameProfiler.getInstance().invokeAndMeasure(
        "updateTwistBone", this, AnimationPlayer::updateTwistBoneInternal);
  }
  ...
  void updateTwistBoneInternal() {
    if (!this.m_twistBones.isEmpty()) {
      ...
      if (!PZMath.equal(this.m_twistAngle, 0.0F, float0)) {
        if (!DebugOptions.instance.Character.Debug.Animate.NoBoneTwists
                 .getValue()) {
  ...
  void applyTwistBone(SkinningBone skinningBone, float float1) {
    if (skinningBone != nullptr) {
      ...
      Matrix4f matrix4f0 = this.getBoneModelTransform(
          int1, AnimationPlayer.L_applyTwistBone.twistParentBoneTrans);
      Matrix4f matrix4f1 = Matrix4f.invert(
          matrix4f0, AnimationPlayer.L_applyTwistBone.twistParentBoneTransInv);
      if (matrix4f1 != nullptr) {
        Matrix4f matrix4f2 = this.getBoneModelTransform(
            int0, AnimationPlayer.L_applyTwistBone.twistBoneTrans);
        Quaternion quaternion0 =
            AnimationPlayer.L_applyTwistBone.twistBoneTargetRot;
        Matrix4f matrix4f3 = AnimationPlayer.L_applyTwistBone.twistRotDiffTrans;
        matrix4f3.setIdentity();
        AnimationPlayer.L_applyTwistBone.twistRotDiffTransAxis.set(0.0F, 1.0F,
                                                                   0.0F);
        float float0 =
            PZMath.getClosestAngle(this.m_boneTransforms[int0].Twist, float1);
        this.m_boneTransforms[int0].Twist = float1;
        matrix4f3.rotate(
            float0, AnimationPlayer.L_applyTwistBone.twistRotDiffTransAxis);
        Matrix4f matrix4f4 =
            AnimationPlayer.L_applyTwistBone.twistBoneTargetTrans;
        Matrix4f.mul(matrix4f2, matrix4f3, matrix4f4);
        HelperFunctions.getRotation(matrix4f4, quaternion0);
        Quaternion quaternion1 =
            AnimationPlayer.L_applyTwistBone.twistBoneNewRot;
        quaternion1.set(quaternion0);
        Vector3f vector3f0 = HelperFunctions.getPosition(
            matrix4f2, AnimationPlayer.L_applyTwistBone.twistBonePos);
        ...
        HelperFunctions.CreateFromQuaternionPositionScale(
            vector3f0, quaternion1, vector3f1, matrix4f5);
  ...
  void updateMultiTrackBoneTransforms() {
    GameProfiler.getInstance().invokeAndMeasure(
        "updateMultiTrackBoneTransforms", this,
        AnimationPlayer::updateMultiTrackBoneTransformsInternal);
  }
  ...
  void updateMultiTrackBoneTransformsInternal() {
    ...
    if (this.m_totalAnimBlendCount != 0) {
      if (this.isRecording()) {
        this.m_recorder.logAnimWeights(
            this.m_multiTrack.getTracks(), this.m_animBlendIndices,
            this.m_animBlendWeights, this.m_deferredMovement);
      }
  ...
  void updateLayerBlendWeightings() {
    ...
    for (int int1 = 0; int1 < int0; int1++) {
      ...
      if (int2 >= 0 && int2 < 4) {
        if (!(float0 < 0.001F) && (int2 <= 0 || !animationTrack.isFinished())) {
          ...
          if (int4 < 0) {
            DebugLog.General.error(
                "Buffer overflow. Insufficient anim blends in cache. More than "
                "%d animations are being blended at once. Will be truncated to "
                "%d.",
                this.m_animBlendIndices.length, this.m_animBlendIndices.length);
          } else {
        ...
        DebugLog.General.error("Layer index is out of range: %d. Range: 0 - %d",
                               int2, 3);
      }
    }
    ...
    for (int int6 = 0; int6 < this.m_animBlendIndices.length &&
                       this.m_animBlendIndices[int6] >= 0;
         int6++) {
      int int7 = this.m_animBlendLayers[int6];
      this.m_layerWeightTotals[int7] =
          this.m_layerWeightTotals[int7] + this.m_animBlendWeights[int6];
      ...
      if (this.m_boneTransformsNeedFirstFrame) {
        ...
        if (float1 < 1.0F) {
          for (int int10 = 0; int10 < this.m_totalAnimBlendCount; int10++) {
            ...
            if (float1 > 0.0F) {
              this.m_animBlendWeights[int10] =
                  this.m_animBlendWeights[int10] / float1;
            } else {
  ...
  void calculateDeferredMovement() {
    GameProfiler.getInstance().invokeAndMeasure(
        "calculateDeferredMovement", this,
        AnimationPlayer::calculateDeferredMovementInternal);
  }
  ...
  void calculateDeferredMovementInternal() {
    ...
    for (int int0 = this.m_totalAnimBlendCount - 1;
         int0 >= 0 && !(float0 <= 0.001F); int0--) {
      ...
      if (!animationTrack.isFinished()) {
        ...
        if (!(float1 <= 0.001F)) {
          ...
          if (!(float2 <= 0.001F)) {
            ...
            Vector2.addScaled(this.m_deferredMovement,
                              animationTrack.getDeferredMovementDiff(tempo),
                              float3, this.m_deferredMovement);
            if (animationTrack.getUseDeferredRotation()) {
              this.m_deferredAngleDelta =
                  this.m_deferredAngleDelta +
                  animationTrack.getDeferredRotationDiff() * float3;
  ...
  void applyBoneReParenting() {
    GameProfiler.getInstance().invokeAndMeasure(
        "applyBoneReParenting", this,
        AnimationPlayer::applyBoneReParentingInternal);
  }
  ...
  void applyBoneReParentingInternal() {
    ...
    for (int int1 = this.m_reparentedBoneBindings.size(); int0 < int1; int0++) {
      AnimationBoneBindingPair animationBoneBindingPair =
          this.m_reparentedBoneBindings.get(int0);
      if (!animationBoneBindingPair.isValid()) {
        DebugLog.Animation.warn("Animation binding pair is not valid: %s",
                                animationBoneBindingPair);
      } else {
        this.updateBoneAnimationTransform(
            animationBoneBindingPair.getBoneIdxA(), animationBoneBindingPair);
      }
    }
  }

  void updateBoneAnimationTransform(
      int int0, AnimationBoneBindingPair animationBoneBindingPair) {
    this.updateBoneAnimationTransform_Internal(int0, animationBoneBindingPair);
  }

  void updateBoneAnimationTransform_Internal(
      int int1, AnimationBoneBindingPair animationBoneBindingPair) {
    std::vector list = this.m_multiTrack.getTracks();
    ...
    bool boolean0 = animationBoneBinding != null &&
                    animationBoneBinding.getBone() != null &&
                    animationBoneBinding.getBone().Index == int1;
    keyframe.setIdentity();
    float float0 = 0.0F;
    bool boolean1 = true;
    float float1 = 1.0F;

    for (int int2 = int0 - 1; int2 >= 0 && float1 > 0.0F && !(float1 <= 0.001F);
         int2--) {
      ...
      if (!(float2 <= 0.001F)) {
        ...
        if (!(float3 <= 0.001F)) {
          ...
          this.getTrackTransform(int1, animationTrack, animationBoneBindingPair,
                                 vector3f0, quaternion, vector3f1);
          if (boolean0 && animationTrack.getUseDeferredRotation()) {
            if (DebugOptions.instance.Character.Debug.Animate
                    .ZeroCounterRotationBone.getValue()) {
              Vector3f vector3f2 =
                  AnimationPlayer.L_updateBoneAnimationTransform.rotAxis;
              Matrix4f matrix4f =
                  AnimationPlayer.L_updateBoneAnimationTransform.rotMat;
              ...
              Vector3f vector3f3 = HelperFunctions.ToEulerAngles(
                  quaternion,
                  AnimationPlayer.L_updateBoneAnimationTransform.rotEulers);
              HelperFunctions.ToQuaternion(vector3f3.x, vector3f3.y,
                                           (float)(Math.PI / 2), quaternion);
            }
          }
          ...
          if (boolean2) {
            Vector3f vector3f4 =
                animationTrack.getCurrentDeferredCounterPosition(
          ...
          if (boolean1) {
            ...
            float0 += float4;
            Vector3.addScaled(keyframe.Position, vector3f0, float4,
                              keyframe.Position);
            PZMath.slerp(keyframe.Rotation, keyframe.Rotation, quaternion,
                         float5);
          }
        }
      }
    ...
    if (float1 > 0.0F && !this.m_boneTransformsNeedFirstFrame) {
      ...
      Vector3.addScaled(keyframe.Position, vector3f0, float1,
                        keyframe.Position);
    ...
    this.m_boneTransforms[int1].set(keyframe.Position, keyframe.Rotation,
                                    keyframe.Scale);
...
private
  void getTrackTransform(int int0, AnimationTrack animationTrack,
                         AnimationBoneBindingPair animationBoneBindingPair,
                         Vector3f vector3f0, Quaternion quaternion,
                         Vector3f vector3f1) {
    if (animationBoneBindingPair == nullptr) {
      ...
      Matrix4f matrix4f1 =
          getUnweightedBoneTransform(animationTrack, skinningBone0.Index,
                                     AnimationPlayer.L_getTrackTransform.Pa);
      SkinningBone skinningBone1 = skinningBone0.Parent;
      SkinningBone skinningBone2 = animationBoneBindingPair.getBoneB();
      Matrix4f matrix4f2 = this.getBoneModelTransform(
          skinningBone1.Index, AnimationPlayer.L_getTrackTransform.mA);
      Matrix4f matrix4f3 =
          Matrix4f.invert(matrix4f2, AnimationPlayer.L_getTrackTransform.mAinv);
      Matrix4f matrix4f4 = this.getBoneModelTransform(
          skinningBone2.Index, AnimationPlayer.L_getTrackTransform.mB);
      Matrix4f matrix4f5 = this.getUnweightedModelTransform(
          animationTrack, skinningBone1.Index,
          AnimationPlayer.L_getTrackTransform.umA);
      Matrix4f matrix4f6 = this.getUnweightedModelTransform(
          animationTrack, skinningBone2.Index,
          AnimationPlayer.L_getTrackTransform.umB);
      Matrix4f matrix4f7 = Matrix4f.invert(
          matrix4f6, AnimationPlayer.L_getTrackTransform.umBinv);
  ...
  bool isBoneReparented(int boneIdx) {
    return PZArrayUtil.contains(
        this.m_reparentedBoneBindings,
        Lambda.predicate(boneIdx,
                         (animationBoneBindingPair, integer)
                                 ->animationBoneBindingPair.getBoneIdxA() ==
                             integer));
  }
  ...
  void updateMultiTrackBoneTransforms_DeferredMovementOnly() {
    ...
    if (this.parentPlayer == nullptr) {
      ...
      if (this.m_totalAnimBlendCount != 0) {
        int[] ints =
            AnimationPlayer.updateMultiTrackBoneTransforms_DeferredMovementOnly
                .boneIndices;
        int int0 = 0;
        std::vector list = this.m_multiTrack.getTracks();
        int int1 = list.size();
  ...
  AnimationPlayerRecorder getRecorder() { return this.m_recorder; }

  void dismember(int bone) { this.dismembered.add(bone); }

  void updateModelTransforms() {
    GameProfiler.getInstance().invokeAndMeasure(
        "updateModelTransforms", this,
        AnimationPlayer::updateModelTransformsInternal);
  }
  ...
  void updateModelTransformsInternal() {
    ...
    for (int int0 = 1; int0 < this.modelTransforms.length; int0++) {
      ...
      SkinningBone skinningBone1 = skinningBone0.Parent;
      BoneTransform.mul(this.m_boneTransforms[skinningBone0.Index],
                        this.modelTransforms[skinningBone1.Index],
                        this.modelTransforms[skinningBone0.Index]);
    }
  }
  ...
  Matrix4f getBoneModelTransform(int boneIdx, Matrix4f out_modelTransform) {
    ...
    for (SkinningBone skinningBone1 = skinningBone0; skinningBone1 != nullptr;
         skinningBone1 = skinningBone1.Parent) {
  ...
  /**
   * Get the bone's transform, in its local space.   That is, relative to its
   * parent bone.
   */
  ...
  Matrix4f getUnweightedModelTransform(AnimationTrack track, int boneIdx,
                                       Matrix4f out_modelTransform) {
    Matrix4f matrix4f =
        AnimationPlayer.L_getUnweightedModelTransform.boneTransform;
    matrix4f.setIdentity();
    out_modelTransform.setIdentity();
    SkinningBone skinningBone0 = this.m_skinningData.getBoneAt(boneIdx);
  ...
  static Matrix4f getUnweightedBoneTransform(AnimationTrack track, int boneIdx,
                                             Matrix4f out_boneTransform) {
    ...
    HelperFunctions.CreateFromQuaternionPositionScale(
        vector3f0, quaternion, vector3f1, out_boneTransform);
    return out_boneTransform;
  }

  void UpdateSkinTransforms() { this.resetSkinTransforms(); }

public
  Matrix4f[] getSkinTransforms(SkinningData skinningData) {
    if (skinningData == nullptr) {
      return this.modelTransforms;
    } else {
      AnimationPlayer.SkinTransformData skinTransformData =
          this.getSkinTransformData(skinningData);
      Matrix4f[] matrix4fs = skinTransformData.transforms;
      if (skinTransformData.dirty) {
        for (int int0 = 0; int0 < this.modelTransforms.length; int0++) {
          if (skinningData.BoneOffset != nullptr &&
              skinningData.BoneOffset.get(int0) != nullptr) {
            Matrix4f.mul(skinningData.BoneOffset.get(int0),
                         this.modelTransforms[int0], matrix4fs[int0]);
          } else {
  ...
  float getDeferredAngleDelta() { return this.m_deferredAngleDelta; }

  float getDeferredRotationWeight() { return this.m_deferredRotationWeight; }

  AnimationMultiTrack getMultiTrack() { return this.m_multiTrack; }

  void setRecording(bool val) { this.m_recorder.setRecording(val); }
  ...
  float getRenderedAngle() { return this.m_angle + (float)(Math.PI / 2); }
   ...
   */
  float getMaxTwistAngle() { return this.m_maxTwistAngle; }

  /**
   * Set the maximum twist angle, in radians
   */
  void setMaxTwistAngle(float radians) { this.m_maxTwistAngle = radians; }

  float getExcessTwistAngle() { return this.m_excessTwist; }

  float getTwistAngle() { return this.m_twistAngle; }

  float getShoulderTwistAngle() { return this.m_shoulderTwistAngle; }

  /**
   * The lookAt bearing, in radians. The difference between angle and
...
private
  static class L_applyTwistBone {
    ...
    static const Vector3f twistRotDiffTransAxis =
  ...
  private static final class L_setTwistBones {
    static final ArrayList<String> boneNames = std::make_unique<ArrayList<>>();
  }
  ...
  private static final class L_updateBoneAnimationTransform {
    ...
    static const Vector3f pos = new Vector3f();
    static const Vector3f scale = new Vector3f();
    static const Keyframe key = new Keyframe(
        new Vector3f(0.0F, 0.0F, 0.0F), new Quaternion(0.0F, 0.0F, 0.0F, 1.0F),
  ...
  public
    ...
    bool dirty;
  private
    AnimationPlayer.SkinTransformData m_next;
  private
    static Pool<AnimationPlayer.SkinTransformData> s_pool =
        new Pool<>(AnimationPlayer.SkinTransformData::new);

    void setSkinnedTo(SkinningData arg0) {
      if (this.m_skinnedTo != arg0) {
        this.dirty = true;
        this.m_skinnedTo = arg0;
        this.transforms = PZArrayUtil.newInstance(
            Matrix4f.class, this.transforms, arg0.numBones(), Matrix4f::new);
      }
    }

  public
    static AnimationPlayer.SkinTransformData alloc(SkinningData arg0) {
