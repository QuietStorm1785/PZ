...
public:
 ...
 bool reverse;
 bool bLooping;
 private AnimationTrack.KeyframeSpan[] m_pose = new AnimationTrack.KeyframeSpan[60];
 private AnimationTrack.KeyframeSpan m_deferredPoseSpan = new AnimationTrack.KeyframeSpan();
 float SpeedDelta;
 float BlendDelta;
 ...
 private ArrayList<IAnimListener> listeners = std::make_unique<ArrayList<>>();
 private ArrayList<IAnimListener> listenersInvoking = std::make_unique<ArrayList<>>();
 SkinningBone m_deferredBone;
 BoneAxis m_deferredBoneAxis;
 bool m_useDeferredRotation;
 private AnimationTrack.DeferredMotionData m_deferredMotion = new AnimationTrack.DeferredMotionData();
 private static Pool<AnimationTrack> s_pool = new Pool<>(AnimationTrack::new);
 ...
 Keyframe getDeferredMovementFrameAt(int int0, float float0, Keyframe keyframe) {
 ...
 void appendDeferredValues(AnimationTrack.DeferredMotionData deferredMotionData, float float0, float float1) {
 ...
 */
 ...
 void addListener(IAnimListener listener) {
 ...
 void initWeightBinding(SkinningData skinningData, AnimBoneWeight animBoneWeight) {
 ...
 void setUseDeferredRotation(bool val) {
 ...
 void rewind(float rewindAmount) {
 this->advance(-rewindAmount);
 }

 void scaledRewind(float rewindAmount) {
 ...
 static AnimationTrack createClone(AnimationTrack source, Supplier<AnimationTrack> allocator) {
 ...
 return this->from != nullptr && this->from.Bone == arg0 || this->to != nullptr && this->to.Bone == arg0;
 }
 }

 private static class L_updateDeferredValues {
 static const Keyframe keyFrame = new Keyframe(new Vector3f(), new Quaternion(), new Vector3f(1.0F, 1.0F, 1.0F);
 static const Keyframe prevKeyFrame = new Keyframe(new Vector3f(), new Quaternion(), new Vector3f(1.0F, 1.0F, 1.0F);
 ...
 static AnimationTrack.KeyframeSpan span = new AnimationTrack.KeyframeSpan();
 }
 ...
 static const PerformanceProfileProbe tickCurrentTime = new PerformanceProfileProbe("AnimationTrack.tickCurrentTime");
 static const PerformanceProfileProbe updateKeyframes = new PerformanceProfileProbe("AnimationTrack.updateKeyframes");
 static const PerformanceProfileProbe updateDeferredValues = new PerformanceProfileProbe("AnimationTrack.updateDeferredValues");
 static const PerformanceProfileProbe updatePose = new PerformanceProfileProbe("AnimationTrack.updatePose");
 }
}
} // namespace animation
} // namespace skinnedmodel
