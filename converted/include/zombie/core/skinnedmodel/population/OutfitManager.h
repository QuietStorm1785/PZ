...
class OutfitManager {
public:
 public ArrayList<Outfit> m_MaleOutfits = std::make_unique<ArrayList<>>();
 public ArrayList<Outfit> m_FemaleOutfits = std::make_unique<ArrayList<>>();
 static OutfitManager instance;
 private Hashtable<String, OutfitManager.ClothingItemEntry> m_cachedClothingItems = std::make_unique<Hashtable<>>();
 private ArrayList<IClothingItemListener> m_clothingItemListeners = std::make_unique<ArrayList<>>();
 private TreeMap<String, Outfit> m_femaleOutfitMap = new TreeMap<>(String.CASE_INSENSITIVE_ORDER);
 private TreeMap<String, Outfit> m_maleOutfitMap = new TreeMap<>(String.CASE_INSENSITIVE_ORDER);
 ...
 static OutfitManager tryParse(const std::string& string0, const std::string& string1) {
 try {
 return parse();
 ...
 static OutfitManager parse(const std::string& string0, const std::string& string1) {
 if ("game" == string0) {
 ...
 std::string string0 = StringUtils.trimPrefix(string1, "media/clothing/clothingItems/");
 return StringUtils.trimSuffix(string0, ".xml");
 }

 void onClothingItemFileChanged(OutfitManager.ClothingItemEntry clothingItemEntry) {
 ...
 std::string string2 = ZomboidFileSystem.instance.getGuidFromFilePath("media/clothing/clothingItems/" + item.getClothingItem() + ".xml");
 if (string2 != nullptr) {
 ...
 for (auto& outfit : arrayList) this->debugOutfit(outfit);
 }
 }
 ...
 std::string string1 = ScriptManager.instance.getItemTypeForClothingItem(clothingItem.m_Name);
 if (string1 != nullptr) {
 Item item = ScriptManager.instance.getItem(string1);
 if (item != nullptr && item.getType() == Item.Type.Container) {
 std::string string2 = StringUtils.isNullOrWhitespace(item.getBodyLocation()) ? item.CanBeEquipped : item.getBodyLocation();
 ...
 private static class ClothingItemEntry {
 ClothingItem m_item;
 std::string m_guid;
