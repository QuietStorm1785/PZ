...
public:
 ...
 private ArrayList<SteamWorkshopItem> stagedItems = std::make_unique<ArrayList<>>();
 private ArrayList<ISteamWorkshopCallback> callbacks = std::make_unique<ArrayList<>>();
 ...
 for (auto& path1 : directoryStream) std::string string = path1.toAbsolutePath().toString();
 ...
 if (steamWorkshopItem.getID() != nullptr && SteamUtils.isValidSteamID(steamWorkshopItem.getID())) {
 ...
 bool DownloadItem(long long0, bool boolean0, ISteamWorkshopCallback iSteamWorkshopCallback) {
 if (!this->callbacks.contains(iSteamWorkshopCallback) {
 this->callbacks.add(iSteamWorkshopCallback);
 }
 ...
 long CreateQueryUGCDetailsRequest(long[] longs, ISteamWorkshopCallback iSteamWorkshopCallback) {
 if (!this->callbacks.contains(iSteamWorkshopCallback) {
 this->callbacks.add(iSteamWorkshopCallback);
 }
 ...
 if (string0 != nullptr && string0.replace("\\", "/").contains("/workshop/content/108600/")) {
 ...
 private void n_Shutdown();

 private boolean n_CreateItem();

 private boolean n_StartItemUpdate(long var1);

 private boolean n_SetItemTitle(String var1);

 private boolean n_SetItemDescription(String var1);

 private boolean n_SetItemVisibility(int var1);

 private boolean n_SetItemTags(String[] var1);

 private boolean n_SetItemContent(String var1);

 private boolean n_SetItemPreview(String var1);

 private boolean n_SubmitItemUpdate(String var1);

 private boolean n_GetItemUpdateProgress(long[] var1);

 private String[] n_GetInstalledItemFolders();

 private long n_GetItemState(long var1);

 private boolean n_SubscribeItem(long var1);

 private boolean n_DownloadItem(long var1, boolean var3);

 private String n_GetItemInstallFolder(long var1);

 private long n_GetItemInstallTimeStamp(long var1);

 private boolean n_GetItemDownloadInfo(long var1, long[] var3);

 private long n_CreateQueryUGCDetailsRequest(long[] var1);

 private SteamUGCDetails n_GetQueryUGCResult(long var1, int var3);

 private long[] n_GetQueryUGCChildren(long var1, int var3);

 private boolean n_ReleaseQueryUGCRequest(long var1);
