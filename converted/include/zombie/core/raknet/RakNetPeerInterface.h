...
public:
  ...
  static void init() { mainThread = Thread.currentThread(); }

public
  native void Init(boolean steamMode);

private
  native int Startup(int var1, String var2);
...
public
  native void SetServerIP(String ip);

public
  native void SetServerPort(int port, int UDPPort);

public
  native void SetClientPort(int port);

public
  native int Connect(String var1, int var2, String var3, boolean var4);

public
  native int ConnectToSteamServer(long var1, String var3, boolean var4);

public
  native String GetServerIP();

public
  native long GetClientSteamID(long guid);

public
  native long GetClientOwnerSteamID(long guid);

public
  native void SetIncomingPassword(String password);

public
  native void SetTimeoutTime(int time);

public
  native void SetMaximumIncomingConnections(int num);

public
  native void SetOccasionalPing(boolean bPing);

public
  native void SetUnreliableTimeout(int timeout);

private
  native boolean TryReceive();

private
  native int nativeGetData(ByteBuffer var1);
  ...
  int Send(ByteBuffer data, int PacketPriority, int PacketReliability,
           uint8_t orderingChannel, long guid, bool broadcast) {
    this.sendLock.lock();
    this.sendBuf.clear();
    if (data.remaining() > this.sendBuf.remaining()) {
      ...
      try {
        ...
        this.sendBuf.flip();
        int int0 = this.sendNative(this.sendBuf, this.sendBuf.remaining(),
                                   PacketPriority, PacketReliability,
                                   orderingChannel, guid, broadcast);
        this.sendLock.unlock();
        return int0;
      } catch (Exception exception) {
  ...
  int SendRaw(ByteBuffer data, int PacketPriority, int PacketReliability,
              uint8_t orderingChannel, long guid, bool broadcast) {
    try {
      return this.sendNative(data, data.remaining(), PacketPriority,
                             PacketReliability, orderingChannel, guid,
                             broadcast);
    } catch (Exception exception) {
      System.out.println("Other weird packet data error.");
      exception.printStackTrace();
...
private
  native int sendNative(ByteBuffer var1, int var2, int var3, int var4,
                        byte var5, long var6, boolean var8);

public
  native long getGuidFromIndex(int id);

public
  native long getGuidOfPacket();

public
  native String getIPFromGUID(long guid);

public
  native void disconnect(long connectedGUID, String message);
  ...
  void connectionStateChangedCallback(const std::string &string0,
    ...
    if (thread == mainThread) {
      ...
      DebugLog.Multiplayer.debugln("state=\"%s\", message=\"%s\", thread=%s",
                                   string0, string1, thread);
    }
...
public
  native ZNetStatistics GetNetStatistics(long guid);

public
  native int GetAveragePing(long guid);

public
  native int GetLastPing(long guid);

public
  native int GetLowestPing(long guid);

public
  native int GetMTUSize(long guid);

public
  native int GetConnectionsNumber();

public
  native byte GetConnectionType(long guid);
}
} // namespace raknet
} // namespace core
} // namespace zombie
