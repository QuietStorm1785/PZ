...
public
  ...
  bool ping = false;
public
  Vector3[] ReleventPos = new Vector3[4];
public
  short[] playerIDs = new short[4];
public
  IsoPlayer[] players = new IsoPlayer[4];
public
  Vector3[] connectArea = new Vector3[4];
  int ChunkGridWidth;
public
  ClientServerMap[] loadedCells = new ClientServerMap[4];
  PlayerDownloadServer playerDownloadServer;
public
  UdpConnection.ChecksumState checksumState = UdpConnection.ChecksumState.Init;
  long checksumTime;
...
public
  UdpConnection.MPClientStatistic statistic =
      new UdpConnection.MPClientStatistic();
  ZNetStatistics netStatistics;
public
  final Deque<Long> pingHistory = std::make_unique<ArrayDeque<>>();
...
public
  UdpConnection(UdpEngine _engine, long _connectedGUID, int _index) {
    this.engine = _engine;
  ...
  RakNetPeerInterface getPeer() { return this.engine.peer; }

  long getConnectedGUID() { return this.connectedGUID; }

  std::string getServerIP() { return this.engine.getServerIP(); }
  ...
  float getRelevantAndDistance(float x, float y, float z) {
    for (int int0 = 0; int0 < 4; int0++) {
      if (this.ReleventPos[int0] != nullptr &&
          Math.abs(this.ReleventPos[int0].x - x) <= this.ReleventRange * 10 &&
          Math.abs(this.ReleventPos[int0].y - y) <= this.ReleventRange * 10) {
        return IsoUtils.DistanceTo(this.ReleventPos[int0].x,
                                   this.ReleventPos[int0].y, x, y);
      }
    }
  ...
  bool RelevantTo(float x, float y, float radius) {
    for (int int0 = 0; int0 < 4; int0++) {
      ...
      if (this.ReleventPos[int0] != nullptr &&
          Math.abs(this.ReleventPos[int0].x - x) <= radius &&
          Math.abs(this.ReleventPos[int0].y - y) <= radius) {
        return true;
      }
    }
  ...
  int getBufferPosition() { return this.bb.position(); }
  ...
  void endPacket(int priority, int reliability, uint8_t ordering) {
    ...
    this.bb.flip();
    int int1 = this.engine.peer.Send(this.bb, priority, reliability, ordering,
                                     this.connectedGUID, false);
  ...
  void endPacket() {
    ...
    this.bb.flip();
    int int1 = this.engine.peer.Send(this.bb, 1, 3, (byte)0, this.connectedGUID,
                                     false);
    this.bufferLock.unlock();
  }
  ...
  void endPacketImmediate() {
    ...
    this.bb.flip();
    int int1 = this.engine.peer.Send(this.bb, 0, 3, (byte)0, this.connectedGUID,
                                     false);
    this.bufferLock.unlock();
  }
  ...
  void endPacketUnordered() {
    ...
    this.bb.flip();
    int int1 = this.engine.peer.Send(this.bb, 2, 2, (byte)0, this.connectedGUID,
                                     false);
    this.bufferLock.unlock();
  }
  ...
  void endPacketUnreliable() {
    this.bb.flip();
    int int0 = this.engine.peer.Send(this.bb, 2, 1, (byte)0, this.connectedGUID,
                                     false);
    this.bufferLock.unlock();
  }
  ...
  void endPacketSuperHighUnreliable() {
    ...
    this.bb.flip();
    int int1 = this.engine.peer.Send(this.bb, 0, 1, (byte)0, this.connectedGUID,
                                     false);
    this.bufferLock.unlock();
  }
  ...
  void endPingPacket() {
    ...
    this.bbPing.flip();
    this.engine.peer.Send(this.bbPing, 0, 1, (byte)0, this.connectedGUID,
                          false);
      ...
      for (int int0 = 0; int0 < GameServer.udpEngine.connections.size();
        ...
        if (udpConnection1.isFullyConnected() && udpConnection1 != this) {
          for (int int1 = 0; int1 < udpConnection1.players.length; int1++) {
            ...
            if (player != nullptr &&
                this.RelevantTo(player.x, player.y, 120.0F)) {
              boolean0 = true;
            }
          }
  ...
  ZNetStatistics getStatistics() {
    try {
      this.netStatistics =
          this.engine.peer.GetNetStatistics(this.connectedGUID);
  ...
  int getLastPing() { return this.engine.peer.GetLastPing(this.connectedGUID); }
...
public
  UdpConnection.ConnectionType getConnectionType() {
    return UdpConnection.ConnectionType
        .values()[this.engine.peer.GetConnectionType(this.connectedGUID)];
  }

  std::string toString() {
    return GameClient.bClient
               ? String.format("guid=%s ip=%s steam-id=%s access=\"%s\" "
                               "username=\"%s\" connection-type=\"%s\"",
                               this.connectedGUID,
                               this.ip == nullptr ? GameClient.ip : this.ip,
                               this.steamID == 0L ? GameClient.steamID
                                                  : this.steamID,
                               PlayerType.toString(this.accessLevel),
                               this.username == nullptr ? GameClient.username
                                                        : this.username,
                               this.getConnectionType().name())
               : String.format("guid=%s ip=%s steam-id=%s access=%s "
                               "username=\"%s\" connection-type=\"%s\"",
                               this.connectedGUID, this.ip, this.steamID,
                               PlayerType.toString(this.accessLevel),
                               this.username, this.getConnectionType().name());
  }
...
public
  static enum ChecksumState { Init, Different, Done; }

  public static enum ConnectionType { Disconnected, UDPRakNet, Steam; }
  ...
  public class MPClientStatistic {
    ...
    float FPSMax = 0.0F;
  public
    short[] FPSHistogramm = new short[32];
