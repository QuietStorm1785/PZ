...
public:
 ...
 void VoiceInitServer(bool boolean0, int int1, int int0, int int2, int int3, double double1, double double0, bool boolean1) {
 this->isServer = true;
 ...
 if (voiceManagerData != nullptr && voiceManagerData.userplaychannel != 0L) {
 ...
 for (auto& player1 : arrayList0) IsoPlayer player2 = IsoPlayer.getInstance();
 ...
 if (radioData != nullptr && radioData.deviceData != nullptr) {
 javafmodJNI.FMOD_Channel_Set3DLevel(voiceManagerData1.userplaychannel, 0.0F);
 ...
 static void logFrame(IsoPlayer player1, IsoPlayer player0, float float0) {
 long long0 = System.currentTimeMillis();
 if (long0 > timestamp) {
 ...
 synchronized (voiceManagerData0.radioData) {
 synchronized (voiceManagerData1.radioData) {
 if (!voiceManagerData1.radioData.empty() && !voiceManagerData0.radioData.empty()) {
 ...
 if (deviceData0 != nullptr && deviceData0.getIsTurnedOn()) {
 ...
 if (deviceData1 != nullptr && deviceData1.getIsTurnedOn()) {
 ...
 if (worldInventoryObject.getItem() != nullptr && worldInventoryObject.getItem() instanceof Radio) {
 DeviceData deviceData2 = ((Radio)worldInventoryObject.getItem()).getDeviceData();
 if (deviceData2 != nullptr && deviceData2.getIsTurnedOn()) {
 ...
 if (square.getVehicleContainer() != nullptr && square == square.getVehicleContainer().getSquare()) {
 VehiclePart part = square.getVehicleContainer().getPartById("Radio");
 if (part != nullptr) {
 DeviceData deviceData3 = part.getDeviceData();
 if (deviceData3 != nullptr && deviceData3.getIsTurnedOn()) {
