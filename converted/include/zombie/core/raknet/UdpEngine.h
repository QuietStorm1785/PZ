...
private
  final Map<Long, UdpConnection> connectionMap = std::make_unique<HashMap<>>();
public
  final List<UdpConnection> connections = std::make_unique<ArrayList<>>();
...
public
  UdpEngine(int _port, int UDPPort, int _maxConnections,
    ...
    if (this.bServer) {
      ...
      this.peer.SetClientPort(GameServer.DEFAULT_PORT + Rand.Next(10000) +
                              1234);
    }
    ...
    DebugLog.Network.println(
        "RakNet.Startup() return code: %s (0 means success)", int0);
    if (int0 != 0) {
      ...
      this.thread =
          new Thread(ThreadGroups.Network, this ::threadRun, "UdpEngine");
      ...
      if (!this.bQuit) {
        try {
          ...
          DebugLog.Network.printException(
              exception, "Exception thrown during decode.", LogSeverity.Error);
        }
        continue;
      }
      break;
  ...
  std::string getServerIP() { return this.peer.GetServerIP(); }

  long getClientSteamID(long guid) { return this.peer.GetClientSteamID(guid); }
  ...
  void endPacketBroadcast(PacketTypes.PacketType packetType) {
    this.bb.flip();
    this.peer.Send(this.bb, packetType.PacketPriority,
                   packetType.PacketPriority, (byte)0, -1L, true);
    this.bufferLock.unlock();
  }

  void endPacketBroadcastExcept(int priority, int reliability,
                                UdpConnection connection) {
    this.bb.flip();
    this.peer.Send(this.bb, priority, reliability, (byte)0,
                   connection.connectedGUID, true);
    this.bufferLock.unlock();
  }

  void connected() {
    VoiceManager.instance.VoiceConnectReq(
    ...
    if (GameClient.bClient && !GameClient.askPing) {
      ...
      byteBufferWriter0.putUTF(
          PZcrypt.hash(ServerWorldDatabase.encrypt(GameClient.password)));
  ...
  void decode(ByteBuffer byteBuffer) {
    ...
    case 16:
      ...
      if (GameClient.bClient) {
        ...
        ConnectionManager.log("RakNet", "connection-request-accepted",
                              this.connectionArray[int9]);
    ...
    case 19:
      ...
      ConnectionManager.log("RakNet", "new-incoming-connection",
                            this.connectionArray[int1]);
      break;
    case 20:
      int int5 = byteBuffer.get() & 255;
      ConnectionManager.log("RakNet", "no-free-incoming-connections",
                            this.connectionArray[int5]);
      break;
    case 21:
      int int10 = byteBuffer.get() & 255;
      long long7 = this.peer.getGuidOfPacket();
      ConnectionManager.log("RakNet", "disconnection-notification",
                            this.connectionArray[int10]);
    ...
    case 22:
      ...
      ConnectionManager.log("RakNet", "connection-lost",
                            this.connectionArray[int2]);
      this.removeConnection(int2);
      break;
    case 23:
      int int7 = byteBuffer.get() & 255;
      ConnectionManager.log("RakNet", "connection-banned",
                            this.connectionArray[int7]);
      if (GameClient.bClient) {
        GameClient.instance.addDisconnectPacket(int0);
      }
      break;
    case 24:
      ...
      ConnectionManager.log("RakNet", "invalid-password",
                            this.connectionArray[int6]);
      if (GameClient.bClient) {
        GameClient.instance.addDisconnectPacket(int0);
      }
      break;
    case 25:
      ...
      LuaEventManager.triggerEvent("OnConnectionStateChanged",
                                   "ClientVersionMismatch", string);
      break;
    case 31:
      int int3 = byteBuffer.get() & 255;
      ConnectionManager.log("RakNet", "remote-disconnection-notification",
                            this.connectionArray[int3]);
      break;
    case 32:
      int int8 = byteBuffer.get() & 255;
      ConnectionManager.log("RakNet", "remote-connection-lost",
                            this.connectionArray[int8]);
      if (GameClient.bClient) {
        GameClient.instance.addDisconnectPacket(int0);
      }
      break;
    case 33:
      ...
      ConnectionManager.log("RakNet", "remote-new-incoming-connection",
                            this.connectionArray[int4]);
      break;
    case 44:
    ...
    case 134:
      ...
      if (GameServer.bServer) {
        ...
        if (udpConnection2 == nullptr) {
          DebugLog.Network.warn(
              "GOT PACKET FROM UNKNOWN CONNECTION guid=%s packetId=%s", long5,
              Integer.valueOf(short0));
          return;
        }
    ...
    default:
      ...
      if (GameServer.bServer) {
        ...
        try {
          if (ServerOptions.instance.AntiCheatProtectionType10.getValue() &&
              PacketValidator.checkUser(udpConnection1)) {
            PacketValidator.doKickUser(udpConnection1, String.valueOf(int0),
                                       "Type10", nullptr);
          }
        } catch (Exception exception) {
  ...
  UdpConnection getActiveConnection(long connection) {
    return !this.connectionMap.containsKey(connection)
               ? nullptr
    ...
    if (int0 == 0 && SteamUtils.isSteamModeEnabled()) {
      ...
      try {
        ...
        LuaEventManager.triggerEvent(
            "OnConnectFailed",
            Translator.getText("UI_OnConnectFailed_UnknownHost"));
        return;
      }

      this.peer.ConnectToSteamServer(long0, this.hashServerPassword(string1),
                                     boolean0);
    } else {
      std::string string2;
      try {
      ...
      this.peer.Connect(string2, int0, this.hashServerPassword(string1),
                        boolean0);
    }
  }
  ...
  RakNetPeerInterface getPeer() { return this.peer; }

  int getMaxConnections() { return this.maxConnections; }

  std::string getDescription() {
    return "connections=[" + this.connections.size() + "/" +
           this.connectionMap.size() + "/" +
           Arrays.stream(this.connectionArray)
