...
public
  static final SpriteRenderer.RingBuffer ringBuffer =
      new SpriteRenderer.RingBuffer();
  static const int NUM_RENDER_STATES = 3;
  const SpriteRendererStates m_states = new SpriteRendererStates();
private
  volatile boolean m_waitingForRenderState = false;
  static bool GL_BLENDFUNC_ENABLED = true;

  void create() { ringBuffer.create(); }

  void clearSprites() { this.m_states.getPopulating().clear(); }

  void glDepthMask(bool b) {
    this.m_states.getPopulatingActiveState().glDepthMask(b);
  }

  void renderflipped(Texture tex, float x, float y, float width, float height,
                     float r, float g, float b, float a,
                     Consumer<TextureDraw> texdModifier) {
    this.m_states.getPopulatingActiveState().renderflipped(
        tex, x, y, width, height, r, g, b, a, texdModifier);
  }
  ...
  void drawSkyBox(Shader shader, int playerIndex, int apiId, int bufferId) {
    this.m_states.getPopulatingActiveState().drawSkyBox(shader, playerIndex,
                                                        apiId, bufferId);
  }

  void drawWater(Shader shader, int playerIndex, int apiId, bool bShore) {
    this.m_states.getPopulatingActiveState().drawWater(shader, playerIndex,
                                                       apiId, bShore);
  }

  void drawPuddles(Shader shader, int playerIndex, int apiId, int z) {
    this.m_states.getPopulatingActiveState().drawPuddles(shader, playerIndex,
                                                         apiId, z);
  }

  void drawParticles(int playerIndex, int var1, int var2) {
    this.m_states.getPopulatingActiveState().drawParticles(playerIndex, var1,
                                                           var2);
  }
  ...
  void glEnable(int a) { this.m_states.getPopulatingActiveState().glEnable(a); }

  void glStencilMask(int a) {
    this.m_states.getPopulatingActiveState().glStencilMask(a);
  }

  void glClear(int a) { this.m_states.getPopulatingActiveState().glClear(a); }
...
public
  void render(Texture tex, double x1, double y1, double x2, double y2,
              double x3, double y3, double x4, double y4, float r, float g,
              float b, float a, Consumer<TextureDraw> texdModifier) {
    this.m_states.getPopulatingActiveState().render(
        tex, x1, y1, x2, y2, x3, y3, x4, y4, r, g, b, a, texdModifier);
  }

public
  void render(Texture tex, double x1, double y1, double x2, double y2,
              double x3, double y3, double x4, double y4, float r1, float g1,
              float b1, float a1, float r2, float g2, float b2, float a2,
              float r3, float g3, float b3, float a3, float r4, float g4,
              float b4, float a4, Consumer<TextureDraw> texdModifier) {
    this.m_states.getPopulatingActiveState().render(
        tex, x1, y1, x2, y2, x3, y3, x4, y4, r1, g1, b1, a1, r2, g2, b2, a2, r3,
        g3, b3, a3, r4, g4, b4, a4, texdModifier);
  }

public
  void renderdebug(Texture tex, float x1, float y1, float x2, float y2,
                   float x3, float y3, float x4, float y4, float r1, float g1,
                   float b1, float a1, float r2, float g2, float b2, float a2,
                   float r3, float g3, float b3, float a3, float r4, float g4,
                   float b4, float a4, Consumer<TextureDraw> texdModifier) {
    this.m_states.getPopulatingActiveState().renderdebug(
        tex, x1, y1, x2, y2, x3, y3, x4, y4, r1, g1, b1, a1, r2, g2, b2, a2, r3,
        g3, b3, a3, r4, g4, b4, a4, texdModifier);
  }

  void renderline(Texture tex, int x1, int y1, int x2, int y2, float r, float g,
                  float b, float a, int thickness) {
    this.m_states.getPopulatingActiveState().renderline(tex, x1, y1, x2, y2, r,
                                                        g, b, a, thickness);
  }
  ...
  void render(Texture tex, float x1, float y1, float x2, float y2, float x3,
              float y3, float x4, float y4, int c1, int c2, int c3, int c4) {
    this.m_states.getPopulatingActiveState().render(tex, x1, y1, x2, y2, x3, y3,
                                                    x4, y4, c1, c2, c3, c4);
  }

  void render(Texture tex, float x, float y, float width, float height, float r,
              float g, float b, float a, Consumer<TextureDraw> texdModifier) {
    float float0 = PZMath.floor(x);
    float float1 = PZMath.floor(y);
    float float2 = PZMath.ceil(x + width);
    float float3 = PZMath.ceil(y + height);
    this.m_states.getPopulatingActiveState().render(
        tex, float0, float1, float2 - float0, float3 - float1, r, g, b, a,
        texdModifier);
  }

  void renderi(Texture tex, int x, int y, int width, int height, float r,
               float g, float b, float a, Consumer<TextureDraw> texdModifier) {
    this.m_states.getPopulatingActiveState().render(tex, x, y, width, height, r,
                                                    g, b, a, texdModifier);
  }

public
  void renderClamped(Texture tex, int x, int y, int width, int height,
                     int clampX, int clampY, int clampW, int clampH, float r,
    ...
    if (int0 != int2 && int1 != int3) {
      ...
      if (int4 == 0 && int5 == 0 && int6 == 0 && int7 == 0) {
        ...
        height = int3 - int1;
        this.m_states.getPopulatingActiveState().render(
            tex, int0, int1, width, height, r, g, b, a, float0, float1, float2,
            float3, float4, float5, float6, float7, texdModifier);
      }
    }
  }

  void renderRect(int x, int y, int width, int height, float r, float g,
                  float b, float a) {
    this.m_states.getPopulatingActiveState().renderRect(x, y, width, height, r,
                                                        g, b, a);
  }

  void renderPoly(float x1, float y1, float x2, float y2, float x3, float y3,
                  float x4, float y4, float r, float g, float b, float a) {
    this.m_states.getPopulatingActiveState().renderPoly(x1, y1, x2, y2, x3, y3,
                                                        x4, y4, r, g, b, a);
  }

  void renderPoly(Texture tex, float x1, float y1, float x2, float y2, float x3,
                  float y3, float x4, float y4, float r, float g, float b,
                  float a) {
    this.m_states.getPopulatingActiveState().renderPoly(tex, x1, y1, x2, y2, x3,
                                                        y3, x4, y4, r, g, b, a);
  }
...
public
  void renderPoly(Texture tex, float x1, float y1, float x2, float y2, float x3,
                  float y3, float x4, float y4, float r, float g, float b,
                  float a, float u1, float v1, float u2, float v2, float u3,
                  float v3, float u4, float v4) {
    this.m_states.getPopulatingActiveState().renderPoly(
        tex, x1, y1, x2, y2, x3, y3, x4, y4, r, g, b, a, u1, v1, u2, v2, u3, v3,
        u4, v4);
  }

public
  void render(Texture tex, float x, float y, float width, float height, float r,
              float g, float b, float a, float u1, float v1, float u2, float v2,
              float u3, float v3, float u4, float v4) {
    this.m_states.getPopulatingActiveState().render(tex, x, y, width, height, r,
                                                    g, b, a, u1, v1, u2, v2, u3,
                                                    v3, u4, v4, nullptr);
  }
...
public
  void render(Texture tex, float x, float y, float width, float height, float r,
    ...
    this.m_states.getPopulatingActiveState().render(tex, x, y, width, height, r,
                                                    g, b, a, u1, v1, u2, v2, u3,
                                                    v3, u4, v4, texdModifier);
  }

  static void buildDrawBuffer(TextureDraw[] textureDraws, Style[] styles,
                              int int1, SpriteRenderer.RingBuffer ringBufferx) {
  ...
  void prePopulating() { this.m_states.getPopulating().prePopulating(); }

  void postRender() {
    SpriteRenderState spriteRenderState = this.m_states.getRendering();
    if (spriteRenderState.numSprites == 0 &&
        spriteRenderState.stateUI.numSprites == 0) {
      spriteRenderState.onRendered();
    } else {
      TextureFBO.reset();
      IsoPuddles.VBOs.startFrame();
      GameProfiler.getInstance().invokeAndMeasure(
          "buildStateUIDrawBuffer(UI)", this, spriteRenderState,
          SpriteRenderer::buildStateUIDrawBuffer);
      GameProfiler.getInstance().invokeAndMeasure(
          "buildStateDrawBuffer", this, spriteRenderState,
          SpriteRenderer::buildStateDrawBuffer);
      spriteRenderState.onRendered();
  ...
  void buildStateDrawBuffer(SpriteRenderState spriteRenderState) {
    ringBuffer.begin();
    buildDrawBuffer(spriteRenderState.sprite, spriteRenderState.style,
                    spriteRenderState.numSprites, ringBuffer);
  ...
  void buildStateUIDrawBuffer(SpriteRenderState spriteRenderState) {
    if (spriteRenderState.stateUI.numSprites > 0) {
      ...
      buildDrawBuffer(spriteRenderState.stateUI.sprite,
                      spriteRenderState.stateUI.style,
                      spriteRenderState.stateUI.numSprites, ringBuffer);
      ringBuffer.render();
    }
  ...
  void notifyRenderStateQueue() {
    synchronized(this.m_states) { this.m_states.notifyAll(); }
  }
  ...
  void EndShader() { this.m_states.getPopulatingActiveState().EndShader(); }
  ...
  void setUseVertColorsArray(uint8_t whichShader, int c0, int c1, int c2,
                             int c3) {
    this.m_states.getPopulatingActiveState().setUseVertColorsArray(
        whichShader, c0, c1, c2, c3);
  }
  ...
  setExtraWallShaderParams(SpriteRenderer.WallShaderTexRender wallTexRender) {
    this.m_states.getPopulatingActiveState().setExtraWallShaderParams(
        wallTexRender);
  }

  void ShaderUpdate1i(int shaderID, int uniform, int uniformValue) {
    this.m_states.getPopulatingActiveState().ShaderUpdate1i(shaderID, uniform,
                                                            uniformValue);
  }

  void ShaderUpdate1f(int shaderID, int uniform, float uniformValue) {
    this.m_states.getPopulatingActiveState().ShaderUpdate1f(shaderID, uniform,
                                                            uniformValue);
  }

  void ShaderUpdate2f(int shaderID, int uniform, float value1, float value2) {
    this.m_states.getPopulatingActiveState().ShaderUpdate2f(shaderID, uniform,
                                                            value1, value2);
  }

  void ShaderUpdate3f(int shaderID, int uniform, float value1, float value2,
                      float value3) {
    this.m_states.getPopulatingActiveState().ShaderUpdate3f(
        shaderID, uniform, value1, value2, value3);
  }

  void ShaderUpdate4f(int shaderID, int uniform, float value1, float value2,
                      float value3, float value4) {
    this.m_states.getPopulatingActiveState().ShaderUpdate4f(
        shaderID, uniform, value1, value2, value3, value4);
  }
  ...
  void glBind(int a) { this.m_states.getPopulatingActiveState().glBind(a); }
  ...
  void startOffscreenUI() {
    ...
    this.m_states.getPopulating().stateUI.defaultStyle =
        TransparentStyle.instance;
    GLState.startFrame();
  }
  ...
  bool waitForReadyStateInternal(BooleanSupplier booleanSupplier) {
    if (RenderThread.isRunning() && this.m_states.getReady() == nullptr) {
      if (!RenderThread.isWaitForRenderState() &&
          !this.isWaitingForRenderState()) {
        return false;
      } else {
  ...
  bool getDoAdditive() {
    return this.m_states.getPopulatingActiveState().defaultStyle ==
           AdditiveStyle.instance;
  }
  ...
  void setDoAdditive(bool bDoAdditive) {
    this.m_states.getPopulatingActiveState().defaultStyle =
        (AbstractStyle)(bDoAdditive ? AdditiveStyle.instance
                                    : TransparentStyle.instance);
  }

  void initFromIsoCamera(int nPlayer) {
    this.m_states.getPopulating().playerCamera[nPlayer].initFromIsoCamera(
        nPlayer);
  }
  ...
  SpriteRenderState getRenderingState() { return this.m_states.getRendering(); }
  ...
  bool isWaitingForRenderState() { return this.m_waitingForRenderState; }
...
public
  static final class RingBuffer {
    ...
    void create() {
      ...
      for (int int1 = 0; int1 < this.numBuffers; int1++) {
        this.vbo[int1] = new GLVertexBufferObject(
            this.bufferSize, GLVertexBufferObject.funcs.GL_ARRAY_BUFFER(),
            GLVertexBufferObject.funcs.GL_STREAM_DRAW());
        this.vbo[int1].create();
        this.ibo[int1] = new GLVertexBufferObject(
            this.indexBufferSize,
            GLVertexBufferObject.funcs.GL_ELEMENT_ARRAY_BUFFER(),
            GLVertexBufferObject.funcs.GL_STREAM_DRAW());
    ...
    void add(TextureDraw textureDraw0, TextureDraw textureDraw1, Style style) {
      if (style != nullptr) {
        if (this.vertexCursor + 4 > this.bufferSizeInVertices ||
            this.indexCursor + 6 > this.indexBufferSize) {
          this.render();
          this.next();
        }
        ...
        if (this.prepareCurrentRun(textureDraw0, textureDraw1, style)) {
          ...
          floatBuffer.put(Float.intBitsToFloat(
              textureDraw0.useAttribArray != -1 ? textureDraw0.tex1_col0 : 0));
          ...
          floatBuffer.put(Float.intBitsToFloat(
              textureDraw0.useAttribArray != -1 ? textureDraw0.tex1_col1 : 0));
          ...
          floatBuffer.put(Float.intBitsToFloat(
              textureDraw0.useAttribArray != -1 ? textureDraw0.tex1_col2 : 0));
          ...
          floatBuffer.put(Float.intBitsToFloat(
              textureDraw0.useAttribArray != -1 ? textureDraw0.tex1_col3 : 0));
    ...
    bool prepareCurrentRun(TextureDraw textureDraw0, TextureDraw textureDraw1,
                           Style style) {
      Texture texture0 = textureDraw0.tex;
      Texture texture1 = textureDraw0.tex1;
      uint8_t byte0 = textureDraw0.useAttribArray;
      if (this.isStateChanged(textureDraw0, textureDraw1, style, texture0,
                              texture1, byte0)) {
    ...
    bool isStateChanged(TextureDraw textureDraw0, TextureDraw textureDraw1,
                        Style style, Texture texture0, Texture texture1,
                        uint8_t byte0) {
      if (this.currentRun == nullptr) {
        ...
        if (textureDraw1 != nullptr) {
          ...
          if (textureDraw0.type == TextureDraw.Type.glDraw &&
              textureDraw1.type != TextureDraw.Type.glDraw) {
            return true;
          }
    ...
    void next() {
      ...
      if (this.vertices[this.sequence] == nullptr ||
      ...
      if (this.indices[this.sequence] == nullptr ||
    ...
    void growStateRuns() {
      SpriteRenderer.RingBuffer.StateRun[] stateRuns =
          new SpriteRenderer.RingBuffer
              .StateRun[(int)(this.stateRun.length * 1.5F)];
    ...
    void shaderChangedTexture1() { this.shaderChangedTexture1 = true; }
    ...
    void debugBoundTexture(Texture texture0, int _unit) {
      if (GL11.glGetInteger(34016) == _unit) {
        ...
        if (texture0 == nullptr && int0 != 0) {
          for (Asset asset0 :
               TextureAssetManager.instance.getAssetTable().values()) {
          ...
          DebugLog.General.error("SpriteRenderer.lastBoundTexture0=nullptr "
                                 "doesn't match OpenGL texture id=" +
                                 int0 + " " + string);
        } else if (texture0 != nullptr && texture0.getID() != -1 &&
                   int0 != texture0.getID()) {
          for (Asset asset1 :
               TextureAssetManager.instance.getAssetTable().values()) {
          ...
          DebugLog.General.error(
              "SpriteRenderer.lastBoundTexture0 id=" + texture0.getID() +
              " doesn't match OpenGL texture id=" + int0 + " " + string);
        }
      }
    }
  ...
  private
    class StateRun {
      ...
      int startIndex;
      int endIndex;
      final ArrayList<TextureDraw> ops = std::make_unique<ArrayList<>>();

      std::string toString() {
        std::string string = System.lineSeparator();
        return this.getClass().getSimpleName() + "{ " + string +
               "  ops:" + PZArrayUtil.arrayToString(this.ops, "{", "}", ", ") +
               string + "  texture0:" + this.texture0 + string +
               "  texture1:" + this.texture1 + string +
               "  useAttribArray:" + this.useAttribArray + string +
               "  style:" + this.style + string + "  start:" + this.start +
               string + "  length:" + this.length + string +
               "  indices:" + this.indices + string +
               "  startIndex:" + this.startIndex + string +
               "  endIndex:" + this.endIndex + string + "}";
      }
        ...
        if (this.style != nullptr) {
          ...
          if (int0 > 0) {
            ...
            if (this.style != RingBuffer.this.lastRenderedStyle) {
              if (RingBuffer.this.lastRenderedStyle != nullptr &&
                  (!SpriteRenderer.RingBuffer.IGNORE_STYLES ||
                   RingBuffer.this.lastRenderedStyle !=
                           AdditiveStyle.instance &&
                       RingBuffer.this.lastRenderedStyle !=
                           TransparentStyle.instance &&
                       RingBuffer.this.lastRenderedStyle !=
                           LightingStyle.instance)) {
            ...
            if (RingBuffer.this.lastRenderedTexture0 != nullptr &&
                RingBuffer.this.lastRenderedTexture0.getID() !=
                    Texture.lastTextureID) {
            ...
            if (DebugOptions.instance.Checks.BoundTextures.getValue()) {
              RingBuffer.this.debugBoundTexture(
                  RingBuffer.this.lastRenderedTexture0, 33984);
            }
            ...
            if (this.useAttribArray != RingBuffer.this.lastUseAttribArray) {
              if (this.useAttribArray != -1) {
                if (this.useAttribArray == 1) {
                  int int2 = IsoGridSquare.CircleStencilShader.instance
                                 .a_wallShadeColor;
                  if (int2 != -1) {
                ...
                if (this.useAttribArray == 2) {
                  int int3 = IsoGridSquare.NoCircleStencilShader.instance
                                 .a_wallShadeColor;
                  if (int3 != -1) {
                ...
                if (RingBuffer.this.lastUseAttribArray == 1) {
                  int int4 = IsoGridSquare.CircleStencilShader.instance
                                 .a_wallShadeColor;
                  if (int4 != -1) {
                ...
                if (RingBuffer.this.lastUseAttribArray == 2) {
                  int int5 = IsoGridSquare.NoCircleStencilShader.instance
                                 .a_wallShadeColor;
                  if (int5 != -1) {
            ...
            if (this.length != 0) {
              if (this.length == -1) {
                ...
                if (RingBuffer.this.restoreVBOs) {
                  ...
                  GL11.glEnableClientState(32888);
                  int int6 = IsoGridSquare.CircleStencilShader.instance
                                 .a_wallShadeColor;
                  if (int6 != -1) {
                    GL20.glVertexAttribPointer(int6, 4, 5121, true, 32, 24L);
                  }

                  int6 = IsoGridSquare.NoCircleStencilShader.instance
                             .a_wallShadeColor;
                  if (int6 != -1) {
                    GL20.glVertexAttribPointer(int6, 4, 5121, true, 32, 24L);
                  }
                ...
                assert GL11.glGetInteger(34964) ==
                    RingBuffer.this.vbo[RingBuffer.this.sequence].getID();

                if (this.useAttribArray == 1) {
                  RingBuffer.this.vbo[RingBuffer.this.sequence]
                      .enableVertexAttribArray(IsoGridSquare.CircleStencilShader
                                                   .instance.a_wallShadeColor);

                  assert GL20.glGetVertexAttribi(
                  ...
                  RingBuffer.this.vbo[RingBuffer.this.sequence]
                      .enableVertexAttribArray(
                          IsoGridSquare.NoCircleStencilShader.instance
                              .a_wallShadeColor);
                } else {
                  RingBuffer.this.vbo[RingBuffer.this.sequence]
                ...
                if (this.style.getRenderSprite()) {
                  GL12.glDrawRangeElements(4, this.start,
                                           this.start + this.length,
                                           this.endIndex - this.startIndex,
                                           5123, this.startIndex * 2L);
                } else {
  ...
  public static enum WallShaderTexRender {
    All, LeftOnly, RightOnly;
  }

  private static class s_performance {
    static const PerformanceProfileProbe waitForReadyState =
        new PerformanceProfileProbe("waitForReadyState");
    static const PerformanceProfileProbe waitForReadySlotToOpen =
        new PerformanceProfileProbe("waitForReadySlotToOpen");
  }
}
} // namespace core
} // namespace zombie
