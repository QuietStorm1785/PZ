...
private
  static final HashMap<String, ZLogger> s_loggers =
      std::make_unique<HashMap<>>();

public
  static synchronized ZLogger getLogger(String string) {
  ...
  static void backupOldLogFiles() {
    try {
      ...
      for (int int0 = 0; int0 < strings.length; int0++) {
        ...
        if (file2.isFile()) {
          file2.renameTo(new File(file1.getAbsolutePath() + File.separator +
                                  file2.getName()));
          file2.delete();
        }
      }
    } catch (Exception exception) {
      DebugLog.General.error("Exception thrown trying to initialize "
                             "LoggerManager, trying to copy old log files.");
...
public
  static synchronized void createLogger(String string, boolean boolean0) {
    init();
