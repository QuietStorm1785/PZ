...
public:
 ...
 private SoundManager.Music music = new SoundManager.Music();
 ...
 private HashSet<BaseSoundEmitter> emitters = std::make_unique<HashSet<>>();
 private static ArrayList<SoundManager.AmbientSoundEffect> ambientSoundEffects = std::make_unique<ArrayList<>>();
 static BaseSoundManager instance;
 std::string currentMusicName;
 std::string currentMusicLibrary;
 private FMOD_STUDIO_EVENT_CALLBACK musicEventCallback = std::make_unique<FMOD_STUDIO_EVENT_CALLBACK>() {
 ...
 void PlayAsMusic(const std::string& name, Audio musicTrack, float volume, bool bloop) {
 }
 ...
 long long0 = this->uiEmitter.playClip(gameSoundClip, nullptr);
 this->uiEmitter.tick();
 ...
 bool boolean0 = javafmodJNI.FMOD_Studio_EventInstance_GetPaused(this->eventInstList[int5]);
 if (!boolean0) {
 ...
 if (player != nullptr && !player.Traits.Deaf.isSet() && IsoUtils.DistanceToSquared(player.x, player.y, x, y) < range * range) {
 return true;
 }
 }
 ...
 GameSound gameSound = GameSounds.getSound(name);
 GameSoundClip gameSoundClip = nullptr;
 if (gameSound != nullptr && !gameSound.clips.empty()) {
 ...
 void PlayAsMusic(const std::string& name, Audio musicTrack, bool loop, float volume) {
 }
 ...
 Audio PlayMusic(const std::string& n, const std::string& name, bool loop, float maxGain) {
 return nullptr;
 }

 Audio PlaySound(const std::string& name, bool loop, float maxGain, float pitchVar) {
 return nullptr;
 }
 ...
 Audio PlaySoundWav(const std::string& name, bool loop, float maxGain, float pitchVar) {
 return nullptr;
 }
 ...
 Audio PlaySoundWav(const std::string& name, int variations, bool loop, float maxGain) {
 return nullptr;
 }
 ...
 Audio PlayWorldSound(const std::string& name, IsoGridSquare source, float pitchVar, float radius, float maxGain, bool ignoreOutside) {
 return this->PlayWorldSound(name, false, source, pitchVar, radius, maxGain, ignoreOutside);
 }

 Audio PlayWorldSound(const std::string& name, bool loop, IsoGridSquare source, float pitchVar, float radius, float maxGain, bool ignoreOutside) {
 ...
 Audio PlayWorldSoundImpl(const std::string& name, bool loop, int sx, int sy, int sz, float pitchVar, float radius, float maxGain, bool ignoreOutside) {
 BaseSoundEmitter baseSoundEmitter = IsoWorld.instance.getFreeEmitter(sx + 0.5F, sy + 0.5F, sz);
 long long0 = baseSoundEmitter.playSoundImpl(name, (IsoObject)nullptr);
 return new FMODAudio(baseSoundEmitter);
 }

 Audio PlayWorldSound(const std::string& name, IsoGridSquare source, float pitchVar, float radius, float maxGain, int choices, bool ignoreOutside) {
 return this->PlayWorldSound(name, source, pitchVar, radius, maxGain, ignoreOutside);
 }

 Audio PlayWorldSoundWav(const std::string& name, IsoGridSquare source, float pitchVar, float radius, float maxGain, bool ignoreOutside) {
 return this->PlayWorldSoundWav(name, false, source, pitchVar, radius, maxGain, ignoreOutside);
 }

 Audio PlayWorldSoundWav(const std::string& name, bool loop, IsoGridSquare source, float pitchVar, float radius, float maxGain, bool ignoreOutside) {
 if (!GameServer.bServer && source != nullptr) {
 if (GameClient.bClient) {
 ...
 Audio PlayWorldSoundWavImpl(const std::string& name, bool loop, IsoGridSquare source, float pitchVar, float radius, float maxGain, bool ignoreOutside) {
 ...
 void PlayWorldSoundWav(const std::string& name, IsoGridSquare source, float pitchVar, float radius, float maxGain, int choices, bool ignoreOutside) {
 int integer = Rand.Next(choices) + 1;
 this->PlayWorldSoundWav(name + integer.toString(), source, pitchVar, radius, maxGain, ignoreOutside);
 }

 Audio PrepareMusic(const std::string& name) {
 return nullptr;
 }
 ...
 public static class AmbientSoundEffect implements Audio {
 std::string name;
 long eventInstance;
 float gain;
 GameSoundClip clip;
 ...
 public AmbientSoundEffect(const std::string& _name) {
 ...
 void setName(const std::string& choice) {
 this->name = choice;
 }
