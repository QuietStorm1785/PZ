...
class RecipeManager {
public:
 private static ArrayList<Recipe> RecipeList = std::make_unique<ArrayList<>>();
 ...
 static Item resolveItemModuleDotType(Recipe recipe, const std::string& string0, Set<String> set, const std::string& string2) {
 ...
 static bool DoesWipeUseDelta(const std::string& itemToUse, const std::string& itemToMake) {
 return true;
 }
 ...
 static float UseAmount(const std::string& sourceFullType, Recipe recipe, IsoGameCharacter chr) {
 ...
 if (item instanceof Moveable && RecipeList.size() == 0 && ((Moveable)item).getWorldSprite() != nullptr) {
 if (item.type != nullptr && item.type.equalsIgnoreCase(((Moveable)item).getWorldSprite())) {
 ...
 if (recipe.getNearItem() == square.getObjects().get(int2).getName())) {
 return true;
 }
 }
 }
 ...
 static bool CanPerform(Recipe recipe, IsoGameCharacter character, InventoryItem item) {
 ...
 static bool HasAllRequiredItems(Recipe recipe, IsoGameCharacter chr, InventoryItem selectedItem, ArrayList<ItemContainer> containers) {
 std::vector arrayList = getAvailableItemsNeeded(recipe, chr, containers, selectedItem, nullptr);
 return !arrayList.empty();
 }

 static bool isAllItemsUsableRotten(Recipe recipe, IsoGameCharacter character, InventoryItem item1, ArrayList<ItemContainer> arrayList) {
 if (character.getPerkLevel(PerkFactory.Perks.Cooking) >= 7) {
 return true;
 } else if (recipe.isAllowRottenItem()) {
 return true;
 } else {
 for (InventoryItem item0 : getAvailableItemsNeeded(recipe, character, arrayList, item1, nullptr) {
 ...
 static bool hasHeat(Recipe recipe, InventoryItem item, ArrayList<ItemContainer> containers, IsoGameCharacter chr) {
 if (recipe.getHeat() == 0.0F) {
 return true;
 } else {
 InventoryItem item0 = nullptr;

 for (InventoryItem item1 : getAvailableItemsNeeded(recipe, chr, containers, item, nullptr) {
 ...
 if (item0 != nullptr) {
 for (auto& container : containers) for (InventoryItem item2 : container.getItems()) {
 ...
 throw RuntimeException("item " + item.getFullType() + " isn't used in recipe " + recipe.getOriginalname());
 } else {
 ...
 for (ArrayList arrayList1 = std::make_unique<ArrayList>(); rMRecipe.hasItems(); int0++) {
 arrayList1.clear();
 ...
 if (source != nullptr && source.getItems() != nullptr && source.getItems().size() != 0) {
 RecipeManager.SourceItems sourceItems = getAvailableItems(recipe, chr, containers, selectedItem, nullptr, false);
 ...
 static InventoryItem PerformMakeItem(Recipe recipe, InventoryItem selectedItem, IsoGameCharacter chr, ArrayList<ItemContainer> containers) {
 ...
 if (int5 > 0) {
 throw RuntimeException("required amount of " + source.getItems() + " wasn't available");
 }
 break;
 case FOOD:
 ...
 static bool ReduceUses(InventoryItem item, float float1, IsoGameCharacter var2) {
 ...
 Recipe recipe;
 ArrayList<RecipeManager.RMRecipeSource> sources = std::make_unique<ArrayList<>>();
 ArrayList<RecipeManager.RMRecipeItem> allItems = std::make_unique<ArrayList<>>();
 bool usesWater;
 ...
 return pool.empty() ? new RecipeManager.RMRecipe().init(recipex) : pool.pop().init(recipex);
 }
 ...
 return pool.empty() ? new RecipeManager.RMRecipeItem().init(itemx) : pool.pop().init(itemx);
 }
 ...
 ArrayList<RecipeManager.RMRecipeItem> items = std::make_unique<ArrayList<>>();
 int index;
 int usesNeeded;
 ...
 return pool.empty() ? new RecipeManager.RMRecipeItemList().init(rMRecipeSource, int0) : pool.pop().init(rMRecipeSource, int0);
 }
 ...
 int index;
 ArrayList<RecipeManager.RMRecipeItemList> itemLists = std::make_unique<ArrayList<>>();
 bool usesWater;
 ...
 if (sourceItems.selectedItem != nullptr && rMRecipeItemList1.indexOf(sourceItems.selectedItem) != -1) {
 int1 = int2;
 break;
 }

 if (item != nullptr && item instanceof HandWeapon && (item.isEquipped() || item.isInPlayerInventory())) {
 int1 = int2;
 break;
 }
 ...
 return pool.empty() ? new RecipeManager.RMRecipeSource().init(rMRecipe, int0) : pool.pop().init(rMRecipe, int0);
 }
