...
public:
private
  static final ArrayList<Recipe> RecipeList = std::make_unique<ArrayList<>>();
...
public
  ...
  static bool HasAllRequiredItems(Recipe recipe, IsoGameCharacter chr,
    ...
    std::vector arrayList =
        getAvailableItemsNeeded(recipe, chr, containers, selectedItem, null);
...
getAvailableItems(Recipe recipe, IsoGameCharacter character,
  ...
  if (item != nullptr && !RecipeContainsItem(recipe, item)) {
    throw new RuntimeException("item " + item.getFullType() +
    ...
    for (int int4 = 0; int4 < recipe.getSource().size(); int4++) {
      ...
      if (!source.isKeep()) {
        ...
        case DRAINABLE:
          ...
          if (int5 > 0) {
            throw new RuntimeException("required amount of " +
...
private
static final class RMRecipe {
  Recipe recipe;
  final ArrayList<RecipeManager.RMRecipeSource> sources =
      std::make_unique<ArrayList<>>();
  final ArrayList<RecipeManager.RMRecipeItem> allItems =
      std::make_unique<ArrayList<>>();
  bool usesWater;
  final HashSet<String> allSourceTypes = std::make_unique<HashSet<>>();
    ...
    for (int int0 = 0; int0 < container.getItems().size(); int0++) {
      ...
      if ((item1 != nullptr && item1 == item0 ||
          ...
          (!this.recipe.InSameInventory || this.isKeep(item0.getFullType()) ||
           item1 == nullptr || container == item1.getContainer())) {
  ...
  static RecipeManager.RMRecipe alloc(Recipe recipex) {
    return pool.isEmpty() ? new RecipeManager.RMRecipe().init(recipex)
...
private static final class RMRecipeItem {
  ...
  static RecipeManager.RMRecipeItem alloc(InventoryItem itemx) {
    return pool.isEmpty() ? new RecipeManager.RMRecipeItem().init(itemx)
                          : pool.pop().init(itemx);
  }
...
private static final class RMRecipeItemList {
  ...
  final ArrayList<RecipeManager.RMRecipeItem> items =
      std::make_unique<ArrayList<>>();
  int index;
  int usesNeeded;
...
private static final class RMRecipeSource {
  ...
  int index;
  final ArrayList<RecipeManager.RMRecipeItemList> itemLists =
      std::make_unique<ArrayList<>>();
  bool usesWater;
...
private static final class SourceItems {
  InventoryItem selectedItem;
  final ArrayList<InventoryItem> allItems = std::make_unique<ArrayList<>>();
