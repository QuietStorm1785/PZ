...
public:
 ...
 private static ThreadLocal<CompressIdenticalItems.PerThreadData> perThreadVars = new ThreadLocal<CompressIdenticalItems.PerThreadData>() {
 ...
 ByteBuffer byteBuffer2 = ensureCapacity(nullptr, byteBuffer0.capacity() + 1024);
 return byteBuffer2.put(byteBuffer0.array(), 0, byteBuffer0.position());
 }
 }

 static bool setCompareItem(CompressIdenticalItems.PerThreadData perThreadData, InventoryItem item) {
 ...
 static bool areItemsIdentical(CompressIdenticalItems.PerThreadData perThreadData, InventoryItem item0, InventoryItem item1) {
 ...
 if (!container0.getItems().empty() || !container1.getItems().empty()) {
 return false;
 }
 }
 ...
 ArrayList<String> types = std::make_unique<ArrayList<>>();
 HashMap<String, ArrayList<InventoryItem>> typeToItems = std::make_unique<HashMap<>>();
 ArrayDeque<ArrayList<InventoryItem>> itemLists = std::make_unique<ArrayDeque<>>();
 ArrayList<InventoryItem> savedItems = std::make_unique<ArrayList<>>();
 ...
 return this->itemLists.empty() ? std::make_unique<ArrayList<>>() : this->itemLists.pop();
 }
 }
