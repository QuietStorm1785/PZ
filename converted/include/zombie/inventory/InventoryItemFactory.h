...
public:
  ...
  static InventoryItem CreateItem(const std::string &string1, float float0,
    ...
    try {
      if (string1.startsWith("Moveables.") &&
          !string1.equalsIgnoreCase("Moveables.Moveable")) {
    ...
    if (item1 == nullptr) {
      ...
      if (string1 == Core.getInstance().getPoisonousBerry())) {
          ...
          ((Food)item0)
              .setUseForPoison(
                  new Float(Math.abs(((Food)item0).getHungChange()) * 100.0F)
                      .intValue());
        }
      ...
      if (boolean0) {
        ...
        if (item0 instanceof
            Moveable && !((Moveable)item0).ReadFromWorldSprite(string0) &&
                item0 instanceof
            Radio) {
          DebugLog.log("InventoryItemFactory -> Radio item = " +
                       (string1 != nullptr ? string1 : "unknown"));
        }
      }
  ...
  static InventoryItem CreateItem(short short0) {
    ...
    if (itemInfo != nullptr && itemInfo.isValid()) {
      ...
      if (string != nullptr) {
        ...
        DebugLog.log("InventoryItemFactory.CreateItem() unknown item type \"" +
                     (string != nullptr ? string : "unknown") +
                     "\", registry id = \"" + short0 +
                     "\". Make sure all mods used in save are installed.");
      } else {
        DebugLog.log("InventoryItemFactory.CreateItem() unknown item with "
                     "registry ID \"" +
                     short0 +
                     "\". Make sure all mods used in save are installed.");
      }
      ...
      DebugLog.log(
          "InventoryItemFactory.CreateItem() unknown item with registry ID \"" +
          short0 + "\". Make sure all mods used in save are installed.");
    } else {
      DebugLog.log("InventoryItemFactory.CreateItem() cannot create item: " +
                   itemInfo.ToString());
    }
