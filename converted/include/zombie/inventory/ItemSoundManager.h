...
public:
 private static ArrayList<InventoryItem> items = std::make_unique<ArrayList<>>();
 private static ArrayList<BaseSoundEmitter> emitters = std::make_unique<ArrayList<>>();
 private static ArrayList<InventoryItem> toAdd = std::make_unique<ArrayList<>>();
 private static ArrayList<InventoryItem> toRemove = std::make_unique<ArrayList<>>();
 private static ArrayList<InventoryItem> toStopItems = std::make_unique<ArrayList<>>();
 private static ArrayList<BaseSoundEmitter> toStopEmitters = std::make_unique<ArrayList<>>();
 ...
 if (container.containingItem != nullptr && container.containingItem.getWorldItem() != nullptr) {
 ...
 if (container != nullptr || item2.getWorldItem() != nullptr && item2.getWorldItem().getWorldObjectIndex() != -1) {
