...
public
  static final Key[] highlightDoor = new Key[4];
...
public
  ...
  int getSaveType() { return Item.Type.Key.ordinal(); }
  ...
  void takeKeyId() {
    if (this.getContainer() != nullptr &&
      ...
      this.setKeyId(
          this.getContainer().getSourceGrid().getBuilding().def.getKeyId());
    }
  }

  static void setHighlightDoors(int playerNum, InventoryItem item) {
    if (item instanceof
        Key && !((Key)item).isPadlock() && !((Key)item).isDigitalPadlock()) {
  ...
  bool isPadlock() { return this.padlock; }

  void setPadlock(bool _padlock) { this.padlock = _padlock; }

  int getNumberOfKey() { return this.numberOfKey; }

  void setNumberOfKey(int _numberOfKey) { this.numberOfKey = _numberOfKey; }

  bool isDigitalPadlock() { return this.digitalPadlock; }
