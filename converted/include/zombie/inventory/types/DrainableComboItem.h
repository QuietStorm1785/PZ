...
public:
  ...
  std::string ReplaceOnDepleteFullType = null;
public
...
public
  ...
  bool IsDrainable() { return true; }

  int getSaveType() { return Item.Type.Drainable.ordinal(); }

  bool CanStack(InventoryItem item) { return false; }

  float getUsedDelta() { return this.delta; }
  ...
  bool shouldUpdateInWorld() {
    if (!GameServer.bServer && this.Heat != 1.0F) {
      return true;
    } else if (this.canStoreWater() && this.isWaterSource() &&
               this.getUsedDelta() < 1.0F) {
  ...
  void update() {
    ...
    if (container != nullptr) {
        ...
        if (this.Heat > 1.6F) {
          ...
          if (int0 != this.LastCookMinute) {
            ...
            if (this.isTaintedWater() &&
                float2 > Math.min(this.MinutesToCook, 10.0F)) {
                  ...
                  if (item != nullptr) {
                    if (item instanceof DrainableComboItem) {
                      ((DrainableComboItem)item)
                          .setUsedDelta(this.getUsedDelta());
                    }
      ...
      } else if (container != nullptr && container.isMicrowave() &&
                 this.isTaintedWater() && this.Heat > 1.6F) {
    ...
    if (this.container == nullptr && this.Heat != 1.0F) {
      ...
      if (this.Heat < float4) {
        this.Heat =
            this.Heat + float4 / 1000.0F * GameTime.instance.getMultiplier();
        if (this.Heat > float4) {
          this.Heat = float4;
        }
      }
    ...
    if (this.bUseWhileUnequiped && this.delta > 0.0F &&
        (this.canBeActivated() && this.isActivated() ||
         !this.canBeActivated())) {
      this.ticks = this.ticks + GameTime.instance.getMultiplier();
    ...
    if (this.getWorldItem() != nullptr && this.canStoreWater() &&
        this.isWaterSource() && RainManager.isRaining() &&
        this.getRainFactor() > 0.0F) {
      IsoGridSquare square = this.getWorldItem().getSquare();
      if (square != nullptr && square.isOutside()) {
        this.setUsedDelta(this.getUsedDelta() +
                          0.001F * RainManager.getRainIntensity() *
  ...
  void Use() {
    if (this.getWorldItem() != nullptr) {
      ...
      if (this.uses > 1) {
        ...
        this.uses = 1;
        InventoryItem item0 =
            InventoryItemFactory.CreateItem(this.getFullType());
        item0.setUses(int0);
  ...
  void updateWeight() {
    if (this.getReplaceOnDeplete() != nullptr) {
      ...
      if (item != nullptr) {
        ...
        this.setActualWeight(
            (this.getScriptItem().getActualWeight() - item.getActualWeight()) *
                this.getUsedDelta() +
    ...
    if (this.getWeightEmpty() != 0.0F) {
      this.setCustomWeight(true);
      this.setActualWeight(
          (this.getScriptItem().getActualWeight() - this.WeightEmpty) *
              this.getUsedDelta() +
          this.WeightEmpty);
    }
  }
   ...
   */
  bool isUseWhileEquiped() { return this.bUseWhileEquiped; }
   ...
   */
  bool isUseWhileUnequiped() { return this.bUseWhileUnequiped; }
   ...
   */
  int getTicksPerEquipUse() { return this.ticksPerEquipUse; }
   ...
   * @return the ticks
   */
  float getTicks() { return this.ticks; }
  ...
  void setHeat(float heat) { this.Heat = PZMath.clamp(heat, 0.0F, 3.0F); }

  float getHeat() { return this.Heat; }

  float getInvHeat() { return (1.0F - this.Heat) / 3.0F; }

  bool finishupdate() {
    if (this.canStoreWater() && this.isWaterSource() &&
      ...
      if (this.container != nullptr) {
        if (this.Heat != this.container.getTemprature() ||
  ...
  void setRainFactor(float _rainFactor) { this.rainFactor = _rainFactor; }

  bool canConsolidate() { return this.canConsolidate; }
