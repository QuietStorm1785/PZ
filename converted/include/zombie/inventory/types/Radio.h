...
public
  ...
  void AddDeviceText(const std::string &line, float r, float g, float b,
    ...
    if (player != nullptr && this.deviceData != nullptr &&
        this.deviceData.getDeviceVolume() > 0.0F &&
        !player.Traits.Deaf.isSet()) {
      player.SayRadio(line, r, g, b, UIFont.Medium,
                      this.deviceData.getDeviceVolumeRange(),
                      this.deviceData.getChannel(), "radio");
      if (codes != nullptr) {
  ...
  bool ReadFromWorldSprite(const std::string &sprite) {
    if (StringUtils.isNullOrWhitespace(sprite)) {
      ...
      System.out.println("Warning: Radio worldsprite not valid, sprite = " +
                         (sprite == nullptr ? "nullptr" : sprite));
      return false;
    }
  }

  int getSaveType() { return Item.Type.Radio.ordinal(); }
  ...
  IsoGridSquare getSquare() {
    return this.container != nullptr && this.container.parent != nullptr &&
           this.container.parent instanceof
        IsoPlayer ? this.container.parent.getSquare() : nullptr;
  }
  ...
  IsoPlayer getPlayer() {
    return this.container != nullptr && this.container.parent != nullptr &&
           this.container.parent instanceof
        IsoPlayer ? (IsoPlayer)this.container.parent : nullptr;
  }
  ...
  std::string getSayLine() { return null; }

  std::string getTalkerType() { return "radio"; }
