...
public:
 ...
 private HashMap<Integer, String> customPages = nullptr;
 std::string lockedBy = nullptr;
 int pageToWrite;
 private List<String> teachedRecipes = nullptr;
 const int maxTextLength = 16384;

 public Literature(const std::string& module, const std::string& name, const std::string& itemType, const std::string& texName) {
 ...
 public Literature(const std::string& module, const std::string& name, const std::string& itemType, Item item) {
 ...
 std::string string3 = Translator.getText("Tooltip_literature_NotBeenRead");
 ...
 if (profession != nullptr && profession.getFreeRecipes().contains(string4) {
 int4++;
 }

 for (int int7 = 0; int7 < traitCollection.size(); int7++) {
 TraitFactory.Trait trait = TraitFactory.getTrait(traitCollection.get(int7);
 if (trait != nullptr && trait.getFreeRecipes().contains(string4) {
 int5++;
 }
 }
 }
 ...
 */
 ...
 void setBookName(const std::string& _bookName) {
 ...
 void setCanBeWrite(bool _canBeWrite) {
 ...
 void setLockedBy(const std::string& _lockedBy) {
 ...
 void setPageToWrite(int _pageToWrite) {
