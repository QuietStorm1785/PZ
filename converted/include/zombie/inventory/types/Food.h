...
public:
 ...
 bool Poison = false;
 private List<String> ReplaceOnCooked = nullptr;
 float baseHunger = 0.0F;
 public ArrayList<String> spices = nullptr;
 bool isSpice = false;
 ...
 public Food(const std::string& module, const std::string& name, const std::string& itemType, const std::string& texName) {
 ...
 public Food(const std::string& module, const std::string& name, const std::string& itemType, Item item) {
 ...
 if (player != nullptr && !player.isDead() && this->Chef == player.getFullName())) {
 ...
 if (!boolean0 && this->container != nullptr && this->container.getParent() != nullptr && this->container.getParent() instanceof IsoFireplace
 )
 {
 boolean0 = true;
 }

 if (this->container != nullptr && this->container.SourceGrid != nullptr && !boolean0) {
 ...
 this->m_cookingSound = emitter.playSoundImpl(this->getCookingSound(), (IsoObject)nullptr);
 this->setCookingParameter(emitter);
 } else {
 ...
 ItemContainer container = this->getOutermostContainer();
 return container != nullptr && container.getParent() != nullptr && container.getParent().getObjectIndex() != -1 && !(container.getTemprature() <= 1.6F)
 ...
 if (worldInventoryObject != nullptr && worldInventoryObject.getSquare() != nullptr) {
 ...
 if (container != nullptr && container.parent instanceof IsoCompost) {
 return;
 }
 ...
 bool boolean0 = false;
 if (container != nullptr && container.getSourceGrid() != nullptr && container.getSourceGrid().haveElectricity()) {
 boolean0 = true;
 }
 ...
 if (this->isFrozen()) {
 double0 *= 0.02F;
 } else if (container != nullptr && (container.getType() == "fridge") || container.getType() == "freezer"))) {
 if (boolean0) {
 double0 *= float1;
 ...
 bool boolean1 = !this->Burnt && this->OffAge < 1000000000 && this->Age < this->OffAge;
 bool boolean2 = !this->Burnt && this->OffAgeMax < 1000000000 && this->Age >= this->OffAgeMax;
 this->Age = (float)(this->Age + double0 * float3 / 24.0);
 this->LastAged = float0;
 bool boolean3 = !this->Burnt && this->OffAge < 1000000000 && this->Age < this->OffAge;
 bool boolean4 = !this->Burnt && this->OffAgeMax < 1000000000 && this->Age >= this->OffAgeMax;
 ...
 float float1 = float0;
 bool boolean0 = false;
 if (container != nullptr && container.getParent() != nullptr && container.getParent().getSprite() != nullptr) {
 boolean0 = container.getParent().getSprite().getProperties().Is("IsFridge");
 }

 if (container != nullptr && (boolean0 || container.getType() == "fridge") || container.getType() == "freezer"))) {
 ...
 if (this->isThawing()) {
 float float4 = float3;
 if (outermostContainer != nullptr && "fridge" == outermostContainer.getType()) && outermostContainer.isPowered()) {
 float4 = float3 * 2.0F;
 }

 if (outermostContainer != nullptr && outermostContainer.getTemprature() > 1.0F) {
 float4 /= 6.0F;
 }
 ...
 */
 ...
 string = string.trim();
 return string.empty() ? this->name : Translator.getText("IGUI_FoodNaming", string, this->name);
 }
 ...
 void setEnduranceChange(float _endChange) {
 ...
 */
 ...
 bool boolean0 = "Icecream".equals(this->getType()) || this->hasTag("GoodFrozen");
 ...
 */
 ...
 void setBadCold(bool _bBadCold) {
 ...
 void setGoodHot(bool _bGoodHot) {
 ...
 void setCookedInMicrowave(bool b) {
 ...
 void setEndChange(float _endChange) {
 this->endChange = _endChange;
 }
 ...
 void setHungChange(float _hungChange) {
 ...
 void setBaseHunger(float _baseHunger) {
 ...
 void setUseForPoison(int useForPoison) {
 ...
 void setPoisonPower(int poisonPower) {
 ...
 void setChef(const std::string& chef) {
 this->Chef = chef;
 }
 ...
 void setHerbalistType(const std::string& type) {
 ...
 */
 ...
 void setFluReduction(int _fluReduction) {
 ...
 void setPackaged(bool _packaged) {
 ...
 void setCanBeFrozen(bool _canBeFrozen) {
 ...
 void setCompostTime(float _compostTime) {
 ...
 void setOnEat(const std::string& _onEat) {
 this->onEat = _onEat;
 }
