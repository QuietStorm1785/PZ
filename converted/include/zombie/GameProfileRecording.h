...
public:
 long m_startTime;
 private GameProfileRecording.Row m_rootRow = new GameProfileRecording.Row();
 ...
 public GameProfileRecording(const std::string& string) {
 super(string, "_times");
 ...
 void setNumFramesPerSegment(int int0) {
 this->m_numFramesPerFile = int0;
 }

 void setStartTime(long long0) {
 ...
 std::string string = String.format("%s%s_%04d", this->m_fileKey, this->m_valuesFileNameSuffix, this->m_segmentFilePaths.size());
 ...
 void writeFrameTimeRow(StringBuilder stringBuilder, GameProfileRecording.Row row, int int0) {
 appendCell(stringBuilder, row.StartTime / 100L);
 appendCell(stringBuilder, row.EndTime / 100L);
 appendCell(stringBuilder, int0);
 }

 void writeSpan(StringBuilder stringBuilder, GameProfileRecording.Row row, GameProfileRecording.Span span0) {
 ...
 long StartTime;
 long EndTime;
 List<GameProfileRecording.Span> Spans = std::make_unique<ArrayList<>>();
