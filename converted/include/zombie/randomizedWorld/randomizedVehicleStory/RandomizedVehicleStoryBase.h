...
private
  static HashMap<RandomizedVehicleStoryBase, Integer> rvsMap =
  ...
  static void initAllRVSMapChance(IsoMetaGrid.Zone zone, IsoChunk chunk) {
    ...
    for (int int0 = 0;
         int0 < IsoWorld.instance.getRandomizedVehicleStoryList().size();
         int0++) {
      RandomizedVehicleStoryBase randomizedVehicleStoryBase =
          IsoWorld.instance.getRandomizedVehicleStoryList().get(int0);
      if (randomizedVehicleStoryBase.isValid(zone, chunk, false) &&
          randomizedVehicleStoryBase.isTimeValid(false)) {
        totalChance = totalChance + randomizedVehicleStoryBase.getChance();
        rvsMap.put(randomizedVehicleStoryBase,
                   randomizedVehicleStoryBase.getChance());
      }
    }
  }
      ...
      if (!chunk.vehicles.isEmpty()) {
        ...
        if (randomizedVehicleStoryBase == nullptr) {
          ...
          VehicleStorySpawnData vehicleStorySpawnData =
              randomizedVehicleStoryBase.initSpawnDataForChunk(zone, chunk);
  ...
  static RandomizedVehicleStoryBase getRandomStory() {
    ...
    while (iterator.hasNext()) {
      RandomizedVehicleStoryBase randomizedVehicleStoryBase =
          (RandomizedVehicleStoryBase)iterator.next();
  ...
  VehicleStorySpawnData initSpawnDataForChunk(IsoMetaGrid.Zone zone,
                                              IsoChunk chunk) {
    ...
    if (!this.getSpawnPoint(zone, chunk, floats)) {
      ...
      int[] ints = new int[4];
      VehicleStorySpawner.getInstance().getAABB(float0, float1, int0, int1,
                                                float2, ints);
      return new VehicleStorySpawnData(this, zone, float0, float1, float2,
                                       ints[0], ints[1], ints[2], ints[3]);
    }
  }

  bool getSpawnPoint(IsoMetaGrid.Zone zone, IsoChunk chunk, float[] floats) {
    return this.getRectangleSpawnPoint(zone, chunk, floats) ||
           this.getPolylineSpawnPoint(zone, chunk, floats);
  }

  bool getRectangleSpawnPoint(IsoMetaGrid.Zone zone, IsoChunk chunk,
                              float[] floats) {
    if (!zone.isRectangle()) {
      return false;
    } else {
      ...
      if (zone.w > 30 && zone.h < 15) {
        ...
        if (zone.getWidth() < int1) {
          this.debugLine =
              "Horizontal street is too small, w:" + zone.getWidth() +
              " h:" + zone.getHeight();
          return false;
          ...
          floats[0] = PZMath.clamp(chunk.wx * 10 + 5.0F, float0 + int1 / 2.0F,
                                   float1 - int1 / 2.0F);
          floats[1] = float2;
        ...
        if (zone.getWidth() < int0) {
          this.debugLine =
              "Vertical street is too small, w:" + zone.getWidth() +
              " h:" + zone.getHeight();
          return false;
          ...
          floats[0] = float5;
          floats[1] = PZMath.clamp(chunk.wy * 10 + 5.0F, float3 + int1 / 2.0F,
                                   float4 - int1 / 2.0F);
  ...
  bool getPolylineSpawnPoint(IsoMetaGrid.Zone zone, IsoChunk chunk,
                             float[] floats) {
    if (zone.isPolyline() && zone.polylineWidth > 0) {
      ...
      if (zone.polylineWidth < int0) {
        this.debugLine =
            "Polyline zone is too narrow, width:" + zone.polylineWidth;
        return false;
      } else {
        double[] doubles = new double[2];
        int int2 = zone.getClippedSegmentOfPolyline(
            chunk.wx * 10, chunk.wy * 10, (chunk.wx + 1) * 10,
            (chunk.wy + 1) * 10, doubles);
        if (int2 == -1) {
          return false;
        } else {
          ...
          if (float7 < int1) {
            ...
            if (floats == nullptr) {
              ...
              float float10 =
                  PZMath.min((float)double1 + float8, 1.0F - float8);
              ...
              if (Core.bDebug) {
                float15 =
                    (float)(System.currentTimeMillis() / 20L % 360L) / 360.0F;
              }
  ...
  bool isChunkLoaded(int wx, int wy) {
    IsoChunk chunk = GameServer.bServer
                         ? ServerMap.instance.getChunk(wx, wy)
                         : IsoWorld.instance.CurrentCell.getChunk(wx, wy);
  ...
  bool callVehicleStorySpawner(IsoMetaGrid.Zone zone, IsoChunk chunk,
    ...
    if (!this.getSpawnPoint(zone, chunk, floats)) {
      ...
      vehicleStorySpawner.spawn(floats[0], floats[1], 0.0F, ++float0,
                                this ::spawnElement);
      return true;
    }
  }
...
public
  BaseVehicle[] addSmashedOverlay(BaseVehicle v1, BaseVehicle v2, int xOffset,
                                  int yOffset, boolean _horizontalZone,
                                  boolean addBlood) {
  ...
  void setMaximumDays(int maximumDays) { this.maximumDays = maximumDays; }
