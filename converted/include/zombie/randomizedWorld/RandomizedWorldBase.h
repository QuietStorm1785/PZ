...
public:
 ...
 BaseVehicle addVehicle(IsoMetaGrid.Zone zone, IsoGridSquare sq, IsoChunk chunk, const std::string& zoneName, const std::string& scriptName, IsoDirections dir) {
 return this->addVehicle(zone, sq, chunk, zoneName, scriptName, nullptr, dir, nullptr);
 }
 ...
 */
 ...
 if (square1 != nullptr && square1.getVehicleContainer() == nullptr) {
 totalZombies--;
 ...
 if (!this->rvsVehicleKeyAddedToZombie && !arrayList.empty()) {
 ...
 return square = = nullptr ? nullptr : createRandomDeadBody(square, nullptr, blood, 0, nullptr);
 }
 }
 ...
 static IsoDeadBody createRandomDeadBody(int x, int y, int z, IsoDirections dir, int blood) {
 return createRandomDeadBody();
 }

 static IsoDeadBody createRandomDeadBody(int x, int y, int z, IsoDirections dir, int blood, int crawlerChance) {
 ...
 void setAttachedItem(IsoZombie zombie, const std::string& location, const std::string& item, const std::string& ensureItem) {
 ...
 static bool is1x2AreaClear(IsoGridSquare square) {
 return isSquareClear();
 }

 static bool is2x1AreaClear(IsoGridSquare square) {
 return isSquareClear();
 }
 ...
 */
 ...
 return roomDef = = nullptr ? nullptr : roomDef.getRandomSquare(RandomizedWorldBase::canSpawnAt);
 }
 ...
 BaseVehicle spawnCarOnNearestNav(const std::string& carName, BuildingDef def) {
 ...
 return square != nullptr && item != nullptr ? square.AddWorldInventoryItem(item, Rand.Next(0.2F, 0.8F), Rand.Next(0.2F, 0.8F), 0.0F) : nullptr;
 }

 void addRandomItemsOnGround(RoomDef room, const std::string& type, int count) {
 for (int int0 = 0; int0 < count; int0++) {
 IsoGridSquare square = getRandomSpawnSquare(room);
 ...
 void addRandomItemsOnGround(RoomDef room, ArrayList<String> types, int count) {
 for (int int0 = 0; int0 < count; int0++) {
 IsoGridSquare square = getRandomSpawnSquare(room);
 ...
 InventoryItem addRandomItemOnGround(IsoGridSquare square, ArrayList<String> types) {
 if (square != nullptr && !types.empty()) {
 ...
 */
 ...
 void setMaximumDays(int _maximumDays) {
 ...
 BaseVehicle addTrailer(BaseVehicle v, IsoMetaGrid.Zone zone, IsoChunk chunk, const std::string& zoneName, const std::string& vehicleDistrib, const std::string& trailerName) {
 ...
 zone, this->getSq(square.x + byte0, square.y + byte1, square.z), chunk, zoneName, trailerName, nullptr, directions, vehicleDistrib
 );
 if (vehicle != nullptr) {
