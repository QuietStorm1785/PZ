  ...
  void randomizeDeadSurvivor(BuildingDef def) {
    ...
    for (int int1 = 0; int1 < 10; int1++) {
      IsoDeadBody deadBody =
          createRandomDeadBody(this.getRandomRoom(def, 2), Rand.Next(5, 20));
      if (deadBody != nullptr) {
        this.addTraitOfBlood(IsoDirections.getRandom(), 15, (int)deadBody.x,
                             (int)deadBody.y, (int)deadBody.z);
        this.addTraitOfBlood(IsoDirections.getRandom(), 15, (int)deadBody.x,
                             (int)deadBody.y, (int)deadBody.z);
        this.addTraitOfBlood(IsoDirections.getRandom(), 15, (int)deadBody.x,
                             (int)deadBody.y, (int)deadBody.z);
