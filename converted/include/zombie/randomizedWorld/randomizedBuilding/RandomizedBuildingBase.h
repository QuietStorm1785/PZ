...
private
  static HashMap<RandomizedBuildingBase, Integer> rbMap =
      std::make_unique<HashMap<>>();
  static const int KBBuildingX = 10744;
  static const int KBBuildingY = 9409;
  bool alwaysDo = false;
private
  static HashMap<String, String> weaponsList = std::make_unique<HashMap<>>();

  void randomizeBuilding(BuildingDef def) { def.bAlarmed = false; }
  ...
  static void initAllRBMapChance() {
    for (int int0 = 0;
         int0 < IsoWorld.instance.getRandomizedBuildingList().size(); int0++) {
      totalChance =
          totalChance +
          IsoWorld.instance.getRandomizedBuildingList().get(int0).getChance();
      rbMap.put(
          IsoWorld.instance.getRandomizedBuildingList().get(int0),
          IsoWorld.instance.getRandomizedBuildingList().get(int0).getChance());
    }
  }
   ...
   * building have a bathroom, a kitchen and a bedroom  This is ignored for the
   * alwaysDo building (so i can do stuff in spiffo, pizzawhirled, etc..)
   */
  bool isValid(BuildingDef def, bool force) {
    ...
    if (GameClient.bClient) {
      ...
      if (!GameServer.bServer) {
        if (!force && IsoPlayer.getInstance().getSquare() != nullptr &&
            IsoPlayer.getInstance().getSquare().getBuilding() != nullptr &&
            IsoPlayer.getInstance().getSquare().getBuilding().def == def) {
          this.customizeStartingHouse(
              IsoPlayer.getInstance().getSquare().getBuilding().def);
          return false;
        }
      } else if (!force) {
  ...
  int getMinimumDays() { return this.minimumDays; }

  void setMinimumDays(int minimumDays) { this.minimumDays = minimumDays; }

  int getMinimumRooms() { return this.minimumRooms; }

  void setMinimumRooms(int minimumRooms) { this.minimumRooms = minimumRooms; }

  static void ChunkLoaded(IsoBuilding building) {
    if (!GameClient.bClient && building.def != nullptr && !building.def.seen &&
        building.def.isFullyStreamedIn()) {
      ...
      for (int int1 = 0;
           int1 < IsoWorld.instance.getRandomizedBuildingList().size();
           int1++) {
        RandomizedBuildingBase randomizedBuildingBase0 =
            IsoWorld.instance.getRandomizedBuildingList().get(int1);
        if (randomizedBuildingBase0.isAlwaysDo() &&
            randomizedBuildingBase0.isValid(building.def, false)) {
          arrayList.add(randomizedBuildingBase0);
        }
      }

      building.def.seen = true;
      if (building.def.x == 10744 && building.def.y == 9409 &&
          Rand.Next(100) < 31) {
      ...
      if (!arrayList.isEmpty()) {
        RandomizedBuildingBase randomizedBuildingBase1 =
            (RandomizedBuildingBase)arrayList.get(
      ...
      if (GameServer.bServer &&
          SpawnPoints.instance.isSpawnBuilding(building.getDef())) {
        return;
      }

      RandomizedBuildingBase randomizedBuildingBase2 =
          IsoWorld.instance.getRBBasic();
      ...
      try {
        ...
        if (randomizedBuildingBase2.isValid(building.def, false) &&
            randomizedBuildingBase2.isTimeValid(false)) {
  ...
  void setChance(int _chance) { this.chance = _chance; }

  bool isAlwaysDo() { return this.alwaysDo; }

  void setAlwaysDo(bool _alwaysDo) { this.alwaysDo = _alwaysDo; }
  ...
  static RandomizedBuildingBase getRandomStory() {
    ...
    while (iterator.hasNext()) {
      RandomizedBuildingBase randomizedBuildingBase =
          (RandomizedBuildingBase)iterator.next();
...
public
   ...
   * @param femaleChance force female zombies (if not set it'll be 50% chance,
   * you can set             it to 0 to exclude female from spawning, or 100 to
   * force only             female)
   * @param room force spawn zombies inside a certain room (not mandatory)
   */
public
  ArrayList<IsoZombie> addZombies(BuildingDef def, int totalZombies,
                                  String outfit, Integer femaleChance,
                                  RoomDef room) {
  ...
  HandWeapon addRandomRangedWeapon(ItemContainer container,
                                   bool addBulletsInGun, bool addBoxInContainer,
                                   bool attachPart) {
    if (weaponsList == nullptr || weaponsList.isEmpty()) {
      this.init();
    }
    ...
    if (weapon == nullptr) {
      ...
      if (addBoxInContainer) {
        container.addItem(
            InventoryItemFactory.CreateItem(weaponsList.get(string)));
      }

      if (attachPart) {
        KahluaTable table0 =
            (KahluaTable)LuaManager.env.rawget("WeaponUpgrades");
        if (table0 == nullptr) {
          return null;
        }
        ...
        for (int int1 = 1; int1 <= int0; int1++) {
          ...
          WeaponPart weaponPart = (WeaponPart)InventoryItemFactory.CreateItem(
              (String)table1.rawget(int2));
  ...
  void spawnItemsInContainers(BuildingDef def, const std::string &distribName,
    ...
    ItemPickerJava.ItemPickerRoom itemPickerRoom =
        ItemPickerJava.rooms.get(distribName);
  ...
  void removeAllZombies(BuildingDef buildingDef) {
    for (int int0 = buildingDef.x - 1;
         int0 < buildingDef.x + buildingDef.x2 + 1; int0++) {
      for (int int1 = buildingDef.y - 1;
           int1 < buildingDef.y + buildingDef.y2 + 1; int1++) {
        for (int int2 = 0; int2 < 8; int2++) {
    ...
    for (int int0 = 0; int0 < sq.getObjects().size(); int0++) {
      ...
      if (object instanceof IsoDoor) {
        ...
        IsoGridSquare square0 =
            sq.getRoom() == null ? sq : ((IsoDoor)object).getOppositeSquare();
        if (square0 != nullptr && square0.getRoom() == nullptr) {
          bool boolean0 = square0 != sq;
          IsoBarricade barricade0 =
              IsoBarricade.AddBarricadeToObject((IsoDoor)object, boolean0);
      ...
      if (object instanceof
          IsoWindow && ((IsoWindow)object).isBarricadeAllowed()) {
        IsoGridSquare square1 =
            sq.getRoom() == null ? sq : ((IsoWindow)object).getOppositeSquare();
        bool boolean1 = square1 != sq;
        IsoBarricade barricade1 =
            IsoBarricade.AddBarricadeToObject((IsoWindow)object, boolean1);
  ...
  InventoryItem addWorldItem(const std::string &item, IsoGridSquare sq,
    ...
    if (item != nullptr && sq != nullptr) {
      ...
      if (_item != nullptr) {
        _item.setAutoAge();
        return sq.AddWorldInventoryItem(_item, Rand.Next(0.3F, 0.9F),
                                        Rand.Next(0.3F, 0.9F), float0);
      } else {
        return null;
      }
    } else {
  ...
  bool isTableFor3DItems(IsoObject obj, IsoGridSquare sq) {
    return obj.getSurfaceOffsetNoTable() > 0.0F &&
           ...
           !obj.hasWater() && obj.getProperties().Val("BedType") == nullptr;
  }

public
  static final class HumanCorpse extends IsoGameCharacter implements
      IHumanVisual {
  ...
  public
    HumanCorpse(IsoCell cell, float float0, float float1, float float2) {
    ...
    HumanVisual getVisual() { return this.humanVisual; }
    ...
    bool isSkeleton() { return this.isSkeleton; }
  }
}
} // namespace randomizedBuilding
} // namespace randomizedWorld
