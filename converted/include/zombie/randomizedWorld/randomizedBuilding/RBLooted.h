...
/**
 * This building will be almost empty of loot, and lot of the doors/windows will
 * be broken
 */
        ...
        for (int int2 = 0; int2 < 8; int2++) {
          ...
          if (square != nullptr) {
            for (int int3 = 0; int3 < square.getObjects().size(); int3++) {
              ...
              if (Rand.Next(100) >= 85 && object instanceof
              ...
              if (object.getContainer() != nullptr &&
                  object.getContainer().getItems() != nullptr) {
                for (int int4 = 0;
                     int4 < object.getContainer().getItems().size(); int4++) {
                  if (Rand.Next(100) < 80) {
