...
public:
 ...
 int minZoneWidth = 0;
 int minZoneHeight = 0;
 public ArrayList<String> zoneType = std::make_unique<ArrayList<>>();
 private static HashMap<RandomizedZoneStoryBase, Integer> rzsMap = std::make_unique<HashMap<>>();

 static bool isValidForStory(IsoMetaGrid.Zone zone, bool force) {
 if (zone.pickedXForZoneStory > 0 && zone.pickedYForZoneStory > 0 && zone.pickedRZStory != nullptr && checkCanSpawnStory(zone, force) {
 ...
 */
 IsoGridSquare getRandomFreeSquare(RandomizedZoneStoryBase rzs, IsoMetaGrid.Zone zone) {
 ...
 IsoGridSquare getRandomFreeSquareFullZone(RandomizedZoneStoryBase rzs, IsoMetaGrid.Zone zone) {
 IsoGridSquare square = nullptr;
