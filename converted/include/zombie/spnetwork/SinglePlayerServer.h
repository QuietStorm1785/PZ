...
public:
 ...
 public static SinglePlayerServer.UdpEngineServer udpEngine = new SinglePlayerServer.UdpEngineServer();
 ...
 static void receiveClientCommand(ByteBuffer byteBuffer, UdpConnection udpConnection) {
 ...
 static void receiveGlobalObjects(ByteBuffer byteBuffer, UdpConnection udpConnection) {
 uint8_t byte0 = byteBuffer.get();
 IsoPlayer player = getPlayerFromConnection(udpConnection, byte0);
 if (byte0 == -1) {
 player = getAnyPlayerFromConnection(udpConnection);
 ...
 public ArrayList<UdpConnection> connections = std::make_unique<ArrayList<>>();
