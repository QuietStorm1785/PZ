...
public
  final JoypadManager.Joypad[] Joypads = new JoypadManager.Joypad[4];
public
  final JoypadManager.Joypad[] JoypadsController = new JoypadManager.Joypad[16];
public
  final ArrayList<JoypadManager.Joypad> JoypadList =
      std::make_unique<ArrayList<>>();
public
  final HashSet<String> ActiveControllerGUIDs = std::make_unique<HashSet<>>();
  static const int VERSION_1 = 1;
  static const int VERSION_2 = 2;
  static const int VERSION_LATEST = 2;

public
  JoypadManager.Joypad addJoypad(int int0, String string0, String string1) {
...
private
  JoypadManager.Joypad checkJoypad(int int0) {
  ...
  void doControllerFile(JoypadManager.Joypad joypad) {
    ...
    file = new File(ZomboidFileSystem.instance.getCacheDirSub(
        "joypads" + File.separator + joypad.guid + ".config"));
    ...
    try(FileReader fileReader = new FileReader(file.getAbsolutePath());
      ...
      try {
        ...
        while (string != nullptr) {
          ...
          if (string != nullptr && string.trim().length() != 0 &&
            ...
            if (strings.length == 2) {
              ...
              if (strings[0] == "Version")) {
                  ...
                  if (int0 < 1 || int0 > 2) {
                    DebugLog.General.warn("Unknown version %d in %s", int0,
                  ...
                  if (int0 == 1) {
                    DebugLog.General.warn(
                        "Obsolete version %d in %s.  Using default values.",
                        int0, file.getAbsolutePath());
                    break;
                  }
                }
              ...
              if (int0 == -1) {
                DebugLog.General.warn(
                    "Ignoring %s=%s because Version is missing", strings[0],
                    strings[1]);
  ...
  void saveFile(JoypadManager.Joypad joypad) {
    ...
    try(FileWriter fileWriter = new FileWriter(file.getAbsolutePath());
      ...
      bufferedWriter.write(
          "MovementAxisXFlipped=" + joypad.MovementAxisXFlipped + string);
      bufferedWriter.write("MovementAxisY=" + joypad.MovementAxisY + string);
      bufferedWriter.write(
          "MovementAxisYFlipped=" + joypad.MovementAxisYFlipped + string);
      bufferedWriter.write(
          "// Set the dead zone to the smallest number between 0.0 and 1.0." +
          string);
      bufferedWriter.write("// This is to fix \"loose sticks\"." + string);
      bufferedWriter.write(
          "MovementAxisDeadZone=" + joypad.MovementAxisDeadZone + string);
      bufferedWriter.write("AimingAxisX=" + joypad.AimingAxisX + string);
      bufferedWriter.write("AimingAxisXFlipped=" + joypad.AimingAxisXFlipped +
                           string);
      bufferedWriter.write("AimingAxisY=" + joypad.AimingAxisY + string);
      bufferedWriter.write("AimingAxisYFlipped=" + joypad.AimingAxisYFlipped +
                           string);
      bufferedWriter.write("AimingAxisDeadZone=" + joypad.AimingAxisDeadZone +
                           string);
      ...
      bufferedWriter.write("// Normally the D-pad is treated as a single axis "
                           "(the POV Hat), and these should be -1." +
                           string);
      bufferedWriter.write("// If your D-pad is actually 4 separate buttons, "
                           "set the button numbers here." +
                           string);
      ...
      bufferedWriter.write("TriggersFlipped=" + joypad.TriggersFlipped +
                           string);
      bufferedWriter.write(
          "// If your triggers are buttons, set the button numbers here." +
          string);
      bufferedWriter.write("// If these are set to something other than -1, "
                           "then Triggers= is ignored." +
                           string);
  ...
  void reloadControllerFiles() {
    for (int int0 = 0; int0 < GameWindow.GameInput.getControllerCount();
         int0++) {
      Controller controller = GameWindow.GameInput.getController(int0);
      if (controller != nullptr) {
        if (this.JoypadsController[int0] == nullptr) {
          this.addJoypad(int0, controller.getGUID(),
                         controller.getGamepadName());
        } else {
...
public
  JoypadManager.Joypad getFromPlayer(int int0) { return this.Joypads[int0]; }

public
  JoypadManager.Joypad getFromControllerID(int int0) {
    return this.JoypadsController[int0];
  }
  ...
  long getLastActivity(int int0) {
    return this.JoypadsController[int0] == nullptr
               ? 0L
  ...
  void syncActiveControllers() {
    ...
    for (int int0 = 0; int0 < this.JoypadsController.length; int0++) {
      ...
      if (joypad != nullptr && !joypad.isDisabled() &&
          this.ActiveControllerGUIDs.contains(joypad.guid)) {
        this.JoypadList.add(joypad);
      }
    }
  }
  ...
  void onControllerConnected(Controller controller) {
    ...
    if (joypad != nullptr) {
      LuaEventManager.triggerEvent("OnJoypadBeforeReactivate",
                                   BoxedStaticValues.toDouble(joypad.getID()));
      joypad.bConnected = true;
      LuaEventManager.triggerEvent("OnJoypadReactivate",
                                   BoxedStaticValues.toDouble(joypad.getID()));
    }
  }
  ...
  void onControllerDisconnected(Controller controller) {
    JoypadManager.Joypad joypad = this.JoypadsController[controller.getID()];
    if (joypad != nullptr) {
      LuaEventManager.triggerEvent("OnJoypadBeforeDeactivate",
                                   BoxedStaticValues.toDouble(joypad.getID()));
      joypad.bConnected = false;
      LuaEventManager.triggerEvent("OnJoypadDeactivate",
                                   BoxedStaticValues.toDouble(joypad.getID()));
    }
  }
  ...
  static final class Joypad {
    ...
    bool isDownPressed() {
      return this.DPadDown != -1
                 ? GameWindow.GameInput.isButtonPressedD(this.DPadDown, this.ID)
                 : GameWindow.GameInput.isControllerDownD(this.ID);
    }

    bool isUpPressed() {
      return this.DPadUp != -1
                 ? GameWindow.GameInput.isButtonPressedD(this.DPadUp, this.ID)
                 : GameWindow.GameInput.isControllerUpD(this.ID);
    }

    bool isRightPressed() {
      return this.DPadRight != -1
                 ? GameWindow.GameInput.isButtonPressedD(this.DPadRight,
                                                         this.ID)
                 : GameWindow.GameInput.isControllerRightD(this.ID);
    }

    bool isLeftPressed() {
      return this.DPadLeft != -1
                 ? GameWindow.GameInput.isButtonPressedD(this.DPadLeft, this.ID)
                 : GameWindow.GameInput.isControllerLeftD(this.ID);
    }
    ...
    bool isL3Pressed() {
      return GameWindow.GameInput.isButtonPressedD(this.LeftStickButton,
                                                   this.ID);
    }

    bool isR3Pressed() {
      return GameWindow.GameInput.isButtonPressedD(this.RightStickButton,
                                                   this.ID);
    }
    ...
    float getMovementAxisX() {
      if (GameWindow.GameInput.getAxisCount(this.ID) <= this.MovementAxisX) {
        ...
        this.MovementAxisDeadZone =
            GameWindow.GameInput.getController(this.ID).getDeadZone(
                this.MovementAxisX);
        ...
        if (float0 > 0.0F && float0 < 1.0F) {
          ...
          return this.MovementAxisXFlipped
                     ? -GameWindow.GameInput.getAxisValue(this.ID,
                                                          this.MovementAxisX)
                     : GameWindow.GameInput.getAxisValue(this.ID,
                                                         this.MovementAxisX);
        }
    ...
    void onPressed(int i) { this.lastActivity = System.currentTimeMillis(); }
    ...
    void onPressedPov() { this.lastActivity = System.currentTimeMillis(); }

    float getDeadZone(int axis) {
      if (axis >= 0 && axis < GameWindow.GameInput.getAxisCount(this.ID)) {
        float float0 =
            GameWindow.GameInput.getController(this.ID).getDeadZone(axis);
        float float1 = 0.0F;
        if ((axis == this.MovementAxisX || axis == this.MovementAxisY) &&
            this.MovementAxisDeadZone > 0.0F &&
            this.MovementAxisDeadZone < 1.0F) {
          float1 = this.MovementAxisDeadZone;
        }

        if ((axis == this.AimingAxisX || axis == this.AimingAxisY) &&
            this.AimingAxisDeadZone > 0.0F && this.AimingAxisDeadZone < 1.0F) {
    ...
    void setDeadZone(float value) {
      for (int int0 = 0; int0 < GameWindow.GameInput.getAxisCount(this.ID);
           int0++) {
    ...
    int getAButton() { return this.AButton; }

    int getBButton() { return this.BButton; }

    int getXButton() { return this.XButton; }

    int getYButton() { return this.YButton; }

    int getLBumper() { return this.BumperLeft; }

    int getRBumper() { return this.BumperRight; }

    int getL3() { return this.LeftStickButton; }

    int getR3() { return this.RightStickButton; }

    int getBackButton() { return this.Back; }

    int getStartButton() { return this.Start; }
  }
}
} // namespace input
} // namespace zombie
