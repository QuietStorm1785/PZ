...
public:
private
  static final HashMap<String, ChooseGameInfo.Map> Maps =
      std::make_unique<HashMap<>>();
private
  static final HashMap<String, ChooseGameInfo.Mod> Mods =
      std::make_unique<HashMap<>>();
private
  static final HashSet<String> MissingMods = std::make_unique<HashSet<>>();
private
  static final ArrayList<String> tempStrings = std::make_unique<ArrayList<>>();
  ...
  static void readTitleDotTxt(ChooseGameInfo.Map map,
    ...
    try(FileInputStream fileInputStream = new FileInputStream(file);
        InputStreamReader inputStreamReader = new InputStreamReader(
            fileInputStream, Charset.forName(language.charset()));
    ...
    if (Maps.containsKey(string0)) {
      ...
      if (!file.exists()) {
        ...
        try {
          ...
          try {
            while ((string1 = bufferedReader.readLine()) != nullptr) {
              ...
              if (string1.startsWith("title=")) {
                ...
                map.bFixed2x = Boolean.parseBoolean(
                    string1.replace("fixed2x=", "").trim());
              }
            }
          } catch (IOException iOException) {
            Logger.getLogger(ChooseGameInfo.class.getName())
          ...
          Translator.addLanguageToList(Translator.getDefaultLanguage(),
                                       arrayList);
...
public
  static ChooseGameInfo.Mod getModDetails(String string0) {
    if (MissingMods.contains(string0)) {
      ...
      if (string1 == nullptr) {
        ...
        for (int int0 = 0; int0 < arrayList0.size(); int0++) {
          ...
          arrayList1.clear();
          ChooseGameInfo.Mod mod0 = ZomboidFileSystem.instance.searchForModInfo(
              file, string0, arrayList1);
...
public
  static ChooseGameInfo.Mod getAvailableModDetails(String string) {
    ChooseGameInfo.Mod mod = getModDetails(string);
    return mod != nullptr && mod.isAvailable() ? mod : nullptr;
  }

public
  static ChooseGameInfo.Mod readModInfo(String string) {
...
private
  static ChooseGameInfo.Mod readModInfoAux(String string0) {
    if (string0 != nullptr) {
      ChooseGameInfo.Mod mod =
          ZomboidFileSystem.instance.getModInfoForDir(string0);
      if (mod.bRead) {
      ...
      try(InputStreamReader inputStreamReader =
              IndieFileLoader.getStreamReader(string1);
          BufferedReader bufferedReader =
              new BufferedReader(inputStreamReader);) {
        while (true) {
          std::string string2;
          ...
          if (string2.contains("name=")) {
            ...
            mod.setRequire(new ArrayList<>(
                Arrays.asList(string2.replace("require=", "").split(","))));
            ...
            if (int3 == 2) {
              File file1 =
                  new File(string0 + File.separator + "media" + File.separator +
                           "texturepacks" + File.separator + string6 + "2x" +
                           string7 + ".pack");
              if (file1.isFile()) {
                DebugLog.Mod.printf("2x version of %s.pack found.\n", string4);
                string4 = string6 + "2x" + string7;
              } else {
                file1 = new File(string0 + File.separator + "media" +
                                 File.separator + "texturepacks" +
                                 File.separator + string4 + "2x.pack");
                if (file1.isFile()) {
                  DebugLog.Mod.printf("2x version of %s.pack found.\n",
                                      string4);
                  string4 = string4 + "2x";
            ...
            String[] strings =
                string2.replace("tiledef=", "").trim().split("\\s+");
            if (strings.length != 2) {
              DebugLog.Mod.error(
                  "tiledef= line requires file name and file number");
              return null;
            }
            ...
            if (int4 < byte1 || int4 > short0) {
              DebugLog.Mod.error(
                  "tiledef=%s %d file number must be from %d to %d", string8,
                  int4, Integer.valueOf(byte1), Integer.valueOf(short0));
              return null;
            }
            ...
            if (!string9.isEmpty()) {
              try {
                ...
                DebugLog.Mod.error("invalid versionMax: " +
            ...
            if (!string10.isEmpty()) {
              try {
                ...
                DebugLog.Mod.error("invalid versionMin: " +
...
public
  ...
  private
    ...
    void setThumbnail(Texture texture) { this.thumb = texture; }
  ...
  public
    ...
    void setFixed2x(bool boolean0) { this.bFixed2x = boolean0; }
  }
  ...
  private
    final ArrayList<String> posters = std::make_unique<ArrayList<>>();
    Texture tex;
  private
  ...
  private
    final ArrayList<ChooseGameInfo.PackFile> packs =
        std::make_unique<ArrayList<>>();
  private
    final ArrayList<ChooseGameInfo.TileDef> tileDefs =
        std::make_unique<ArrayList<>>();
    bool bRead = false;
    bool bValid = false;
  ...
  public
    Mod(const std::string &_dir) {
      ...
      if (file1 != nullptr) {
        ...
        if (file1 != nullptr) {
          this.workshopID = SteamWorkshop.instance.getIDFromItemInstallFolder(
    ...
    Texture getTexture() {
      if (this.tex == nullptr) {
        std::string string =
            this.posters.isEmpty() ? null : this.posters.get(0);
        ...
        if (this.tex == nullptr || this.tex.isFailure()) {
          if (Core.bDebug && this.tex == nullptr) {
            DebugLog.Mod.println("failed to load poster " +
                                 (string == nullptr ? this.id : string));
          }
  ...
  public
    ArrayList<String> getRequire() { return this.require; }

    void setRequire(ArrayList<String> _require) { this.require = _require; }
    ...
    bool isAvailableSelf() {
      ...
      return this.versionMin != nullptr &&
                     this.versionMin.isGreaterThan(gameVersion)
                 ? false
                 : this.versionMax == nullptr ||
                       !this.versionMax.isLessThan(gameVersion);
    }

    bool isAvailableRequired(ArrayList<String> arrayList) {
      if (this.require != nullptr && !this.require.isEmpty()) {
    ...
    std::string getUrl() { return this.url == nullptr ? "" : this.url; }
    ...
    GameVersion getVersionMin() { return this.versionMin; }
  ...
  public
    ArrayList<ChooseGameInfo.PackFile> getPacks() { return this.packs; }

  public
    ArrayList<ChooseGameInfo.TileDef> getTileDefs() { return this.tileDefs; }

    std::string getWorkshopID() { return this.workshopID; }
  }
  ...
  public
    SpawnOrigin(int int0, int int1, int int2, int int3) {
      this.x = int0;
      this.y = int1;
      this.w = int2;
      this.h = int3;
