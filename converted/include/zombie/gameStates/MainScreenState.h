...
public:
 ...
 public ArrayList<MainScreenState.ScreenElement> Elements = new ArrayList<>(16);
 ...
 try {
 std::string string0 = ZomboidFileSystem.instance.getCacheDir() + File.separator + "console.txt";
 ...
 static void DrawTexture(Texture tex, int x, int y, int width, int height, float _alpha) {
 SpriteRenderer.instance.renderi(tex, x, y, width, height, 1.0F, 1.0F, 1.0F, _alpha, nullptr);
 }

 static void DrawTexture(Texture tex, int x, int y, int width, int height, Color col) {
 SpriteRenderer.instance.renderi(tex, x, y, width, height, col.r, col.g, col.b, col.a, nullptr);
 }
 ...
 SpriteRenderer.instance.renderi(nullptr, Core.getInstance().getScreenWidth() - int0 - 12, 12, int0, int2, 0.0F, 0.5F, 0.75F, 1.0F, nullptr);
 ...
 std::string string = System.getProperty("os.name").toUpperCase(Locale.ENGLISH);
 ...
 static double getIconRatio(BufferedImage bufferedImage1, BufferedImage bufferedImage0) {
 double double0 = 1.0;
 ...
 for (auto& file : files) System.out
 .println(
 ...
 static std::string wmic(const std::string& string2, String[] strings0) {
 std::string string0 = "";
 ...
 for (auto& string5 : strings0) string3 = string3.replaceAll(string5, "");
 }
