...
private
  ...
  static ClipperOffset m_clipperOffset = null;
  static ByteBuffer m_clipperBuffer;
  static const int VERSION = 1;
private
  final ArrayList<ConfigOption> options = std::make_unique<ArrayList<>>();
private
  DebugChunkState.BooleanDebugOption BuildingRect =
      new DebugChunkState.BooleanDebugOption("BuildingRect", true);
private
  DebugChunkState.BooleanDebugOption ChunkGrid =
      new DebugChunkState.BooleanDebugOption("ChunkGrid", true);
private
  DebugChunkState.BooleanDebugOption ClosestRoomSquare =
      new DebugChunkState.BooleanDebugOption("ClosestRoomSquare", true);
private
  DebugChunkState.BooleanDebugOption EmptySquares =
      new DebugChunkState.BooleanDebugOption("EmptySquares", true);
private
  DebugChunkState.BooleanDebugOption FlyBuzzEmitters =
      new DebugChunkState.BooleanDebugOption("FlyBuzzEmitters", true);
private
  DebugChunkState.BooleanDebugOption LightSquares =
      new DebugChunkState.BooleanDebugOption("LightSquares", true);
private
  DebugChunkState.BooleanDebugOption LineClearCollide =
      new DebugChunkState.BooleanDebugOption("LineClearCollide", true);
private
  DebugChunkState.BooleanDebugOption NearestWallsOpt =
      new DebugChunkState.BooleanDebugOption("NearestWalls", true);
private
  DebugChunkState.BooleanDebugOption ObjectPicker =
      new DebugChunkState.BooleanDebugOption("ObjectPicker", true);
private
  DebugChunkState.BooleanDebugOption RoomLightRects =
      new DebugChunkState.BooleanDebugOption("RoomLightRects", true);
private
  DebugChunkState.BooleanDebugOption VehicleStory =
      new DebugChunkState.BooleanDebugOption("VehicleStory", true);
private
  DebugChunkState.BooleanDebugOption RandomSquareInZone =
      new DebugChunkState.BooleanDebugOption("RandomSquareInZone", true);
private
  DebugChunkState.BooleanDebugOption ZoneRect =
      new DebugChunkState.BooleanDebugOption("ZoneRect", true);
  ...
  void enter() {
    ...
    if (this.m_selfUI.size() == 0) {
      ...
      this.m_luaEnv.caller.pcall(
          ...
          this.m_luaEnv.env.rawget("DebugChunkState_InitUI"), this);
...
public
  GameStateMachine.StateAction update() {
    return !this.bExit && !GameKeyboard.isKeyPressed(60)
               ? this.updateScene()
               : GameStateMachine.StateAction.Continue;
  }
  ...
  void renderScene() {
    ...
    if (this.LightSquares.getValue()) {
      ...
      for (int int0 = 0; int0 < stack.size(); int0++) {
        ...
        if (lightSource.z == this.m_z) {
          this.paintSquare(lightSource.x, lightSource.y, lightSource.z, 1.0F,
                           1.0F, 0.0F, 0.5F);
        }
      }
    }
    ...
    if (this.BuildingRect.getValue()) {
      IsoGridSquare square0 = IsoWorld.instance.getCell().getGridSquare(
          this.gridX, this.gridY, this.m_z);
      if (square0 != nullptr && square0.getBuilding() != nullptr) {
        BuildingDef buildingDef0 = square0.getBuilding().getDef();
        this.DrawIsoLine(buildingDef0.getX(), buildingDef0.getY(),
                         buildingDef0.getX2(), buildingDef0.getY(), 1.0F, 1.0F,
                         1.0F, 1.0F, 2);
        this.DrawIsoLine(buildingDef0.getX2(), buildingDef0.getY(),
                         buildingDef0.getX2(), buildingDef0.getY2(), 1.0F, 1.0F,
                         1.0F, 1.0F, 2);
        this.DrawIsoLine(buildingDef0.getX2(), buildingDef0.getY2(),
                         buildingDef0.getX(), buildingDef0.getY2(), 1.0F, 1.0F,
                         1.0F, 1.0F, 2);
        this.DrawIsoLine(buildingDef0.getX(), buildingDef0.getY2(),
                         buildingDef0.getX(), buildingDef0.getY(), 1.0F, 1.0F,
                         1.0F, 1.0F, 2);
      }
    }
    ...
    if (this.RoomLightRects.getValue()) {
      ...
      for (int int1 = 0; int1 < arrayList.size(); int1++) {
        ...
        if (roomLight.z == this.m_z) {
          this.DrawIsoRect(roomLight.x, roomLight.y, roomLight.width,
                           roomLight.height, 0.0F, 1.0F, 1.0F, 1.0F, 1);
        }
      }
    }
    ...
    if (this.ClosestRoomSquare.getValue()) {
      ...
      BuildingDef buildingDef1 =
          ((AmbientStreamManager)AmbientStreamManager.getInstance())
              .getNearestBuilding(float0, float1, vector2f);
      if (buildingDef1 != nullptr) {
        this.DrawIsoLine(float0, float1, vector2f.x, vector2f.y, 1.0F, 1.0F,
                         1.0F, 1.0F, 1);
      }
    }

    if (this.m_table != nullptr &&
        this.m_table.rawget("selectedSquare") != nullptr) {
      IsoGridSquare square1 = Type.tryCastTo(
          this.m_table.rawget("selectedSquare"), IsoGridSquare.class);
      if (square1 != nullptr) {
        this.DrawIsoRect(square1.x, square1.y, 1.0F, 1.0F, 1.0F, 1.0F, 1.0F,
                         1.0F, 2);
      }
    }
...
public
  GameStateMachine.StateAction updateScene() {
    ...
    if (GameKeyboard.isKeyDown(16)) {
      if (!keyQpressed) {
        IsoGridSquare square = IsoWorld.instance.getCell().getGridSquare(
    ...
    if (GameKeyboard.isKeyDown(19)) {
      if (!keyQpressed) {
        ...
        DebugLog.General.debugln(
            "IsoCell.newRender = %s",
            DebugOptions.instance.Terrain.RenderTiles.NewRender.getValue());
      }
    } else {
      keyQpressed = false;
    }
  ...
  void DrawIsoLine(float float2, float float3, float float6, float float7,
    ...
    LineDrawer.drawLine(float1, float4, float5, float8, float9, float10,
                        float11, float12, int0);
  }

  void DrawIsoRect(float float0, float float1, float float6, float float7,
                   float float2, float float3, float float4, float float5,
                   int int0) {
    this.DrawIsoLine(float0, float1, float0 + float6, float1, float2, float3,
                     float4, float5, int0);
    this.DrawIsoLine(float0 + float6, float1, float0 + float6, float1 + float7,
                     float2, float3, float4, float5, int0);
    this.DrawIsoLine(float0 + float6, float1 + float7, float0, float1 + float7,
                     float2, float3, float4, float5, int0);
    this.DrawIsoLine(float0, float1 + float7, float0, float1, float2, float3,
                     float4, float5, int0);
  }
  ...
  void drawGrid() {
    ...
    float float1 = IsoUtils.YToIso(
        Core.getInstance().getOffscreenWidth(int0) + 128, -256.0F, 0.0F);
    float float2 = IsoUtils.XToIso(
        Core.getInstance().getOffscreenWidth(int0) + 128,
        Core.getInstance().getOffscreenHeight(int0) + 256, 6.0F);
    float float3 = IsoUtils.YToIso(
        -128.0F, Core.getInstance().getOffscreenHeight(int0) + 256, 6.0F);
    int int1 = (int)float1;
    int int2 = (int)float3;
    int int3 = (int)float0;
    int int4 = (int)float2;
  ...
  void drawCursor() {
    ...
    float float0 = this.m_z;
    int int2 =
        (int)IsoUtils.XToScreenExact(this.gridX, this.gridY + 1, float0, 0);
    int int3 =
        (int)IsoUtils.YToScreenExact(this.gridX, this.gridY + 1, float0, 0);
    SpriteRenderer.instance.renderPoly(
        int2, int3, int2 + 32 * int1, int3 - 16 * int1, int2 + 64 * int1, int3,
        int2 + 32 * int1, int3 + 16 * int1, 0.0F, 0.0F, 1.0F, 0.5F);
    IsoChunkMap chunkMap = IsoWorld.instance.getCell().ChunkMap[int0];

    for (int int4 = chunkMap.getWorldYMinTiles();
         int4 < chunkMap.getWorldYMaxTiles(); int4++) {
      for (int int5 = chunkMap.getWorldXMinTiles();
           int5 < chunkMap.getWorldXMaxTiles(); int5++) {
        IsoGridSquare square0 = IsoWorld.instance.getCell().getGridSquare(
            (double)int5, (double)int4, (double)float0);
        if (square0 != nullptr) {
          if (square0 != chunkMap.getGridSquare(int5, int4, (int)float0)) {
            ...
            SpriteRenderer.instance.renderPoly(
                int2, int3, int2 + 32, int3 - 16, int2 + 64, int3, int2 + 32,
                int3 + 16, 1.0F, 0.0F, 0.0F, 0.8F);
          }

          if (square0 == nullptr || square0.getX() != int5 ||
              square0.getY() != int4 || square0.getZ() != float0 ||
              square0.e != nullptr && square0.e.w != nullptr &&
                  square0.e.w != square0 ||
              square0.w != nullptr && square0.w.e != nullptr &&
                  square0.w.e != square0 ||
              square0.n != nullptr && square0.n.s != nullptr &&
                  square0.n.s != square0 ||
              square0.s != nullptr && square0.s.n != nullptr &&
                  square0.s.n != square0 ||
              square0.nw != nullptr && square0.nw.se != nullptr &&
                  square0.nw.se != square0 ||
              square0.se != nullptr && square0.se.nw != nullptr &&
                  square0.se.nw != square0) {
            int2 = (int)IsoUtils.XToScreenExact(int5, int4 + 1, float0, 0);
            int3 = (int)IsoUtils.YToScreenExact(int5, int4 + 1, float0, 0);
            SpriteRenderer.instance.renderPoly(
                int2, int3, int2 + 32, int3 - 16, int2 + 64, int3, int2 + 32,
                int3 + 16, 1.0F, 0.0F, 0.0F, 0.5F);
          }

          if (square0 != nullptr) {
            IsoGridSquare square1 = square0.testPathFindAdjacent(null, -1, 0, 0)
                                        ? null
                                        : square0.nav[IsoDirections.W.index()];
            IsoGridSquare square2 = square0.testPathFindAdjacent(null, 0, -1, 0)
                                        ? null
                                        : square0.nav[IsoDirections.N.index()];
            IsoGridSquare square3 = square0.testPathFindAdjacent(null, 1, 0, 0)
                                        ? null
                                        : square0.nav[IsoDirections.E.index()];
            IsoGridSquare square4 = square0.testPathFindAdjacent(null, 0, 1, 0)
                                        ? null
                                        : square0.nav[IsoDirections.S.index()];
            IsoGridSquare square5 =
                square0.testPathFindAdjacent(null, -1, -1, 0)
                    ? null
                    : square0.nav[IsoDirections.NW.index()];
            IsoGridSquare square6 = square0.testPathFindAdjacent(null, 1, -1, 0)
                                        ? null
                                        : square0.nav[IsoDirections.NE.index()];
            IsoGridSquare square7 = square0.testPathFindAdjacent(null, -1, 1, 0)
                                        ? null
                                        : square0.nav[IsoDirections.SW.index()];
            IsoGridSquare square8 = square0.testPathFindAdjacent(null, 1, 1, 0)
                                        ? null
                                        : square0.nav[IsoDirections.SE.index()];
            if (square1 != square0.w || square2 != square0.n ||
                square3 != square0.e || square4 != square0.s ||
                square5 != square0.nw || square6 != square0.ne ||
                square7 != square0.sw || square8 != square0.se) {
              this.paintSquare(int5, int4, (int)float0, 1.0F, 0.0F, 0.0F, 0.5F);
            }
          }

          if (square0 != nullptr &&
              (square0.nav[IsoDirections.NW.index()] != nullptr &&
                   square0.nav[IsoDirections.NW.index()]
                           .nav[IsoDirections.SE.index()] != square0 ||
               square0.nav[IsoDirections.NE.index()] != nullptr &&
                   square0.nav[IsoDirections.NE.index()]
                           .nav[IsoDirections.SW.index()] != square0 ||
               square0.nav[IsoDirections.SW.index()] != nullptr &&
                   square0.nav[IsoDirections.SW.index()]
                           .nav[IsoDirections.NE.index()] != square0 ||
               square0.nav[IsoDirections.SE.index()] != nullptr &&
                   square0.nav[IsoDirections.SE.index()]
                           .nav[IsoDirections.NW.index()] != square0 ||
               square0.nav[IsoDirections.N.index()] != nullptr &&
                   square0.nav[IsoDirections.N.index()]
                           .nav[IsoDirections.S.index()] != square0 ||
               square0.nav[IsoDirections.S.index()] != nullptr &&
                   square0.nav[IsoDirections.S.index()]
                           .nav[IsoDirections.N.index()] != square0 ||
               square0.nav[IsoDirections.W.index()] != nullptr &&
                   square0.nav[IsoDirections.W.index()]
                           .nav[IsoDirections.E.index()] != square0 ||
               square0.nav[IsoDirections.E.index()] != nullptr &&
                   square0.nav[IsoDirections.E.index()]
                           .nav[IsoDirections.W.index()] != square0)) {
            int2 = (int)IsoUtils.XToScreenExact(int5, int4 + 1, float0, 0);
            int3 = (int)IsoUtils.YToScreenExact(int5, int4 + 1, float0, 0);
            SpriteRenderer.instance.renderPoly(
                int2, int3, int2 + 32, int3 - 16, int2 + 64, int3, int2 + 32,
          ...
          if (square0.getRoom() != nullptr && square0.isFree(false) &&
              !VirtualZombieManager.instance.canSpawnAt(int5, int4,
                                                        (int)float0)) {
    ...
    if (IsoCamera.CamCharacter.getCurrentSquare() != nullptr &&
      ...
      IsoGridSquare square9 = IsoWorld.instance.CurrentCell.getGridSquare(
          this.gridX, this.gridY, this.m_z);
      IsoObject object =
          IsoCamera.CamCharacter.getCurrentSquare().testCollideSpecialObjects(
              square9);
      if (object != nullptr) {
        object.getSprite().RenderGhostTileRed(
            (int)object.getX(), (int)object.getY(), (int)object.getZ());
      }
    }

    if (this.LineClearCollide.getValue()) {
      this.lineClearCached(IsoWorld.instance.CurrentCell, this.gridX,
                           this.gridY, (int)float0,
                           (int)IsoCamera.CamCharacter.getX(),
                           (int)IsoCamera.CamCharacter.getY(), this.m_z, false);
    }
  ...
  void drawZones() {
    std::vector arrayList = IsoWorld.instance.MetaGrid.getZonesAt(
        this.gridX, this.gridY, this.m_z, new ArrayList<>());
    ...
    for (int int0 = 0; int0 < arrayList.size(); int0++) {
      ...
      if (!zone1.isPolyline()) {
        if (!zone1.points.isEmpty()) {
          ...
          this.DrawIsoLine(zone1.x, zone1.y, zone1.x + zone1.w, zone1.y, 1.0F,
                           1.0F, 0.0F, 1.0F, 1);
          this.DrawIsoLine(zone1.x, zone1.y + zone1.h, zone1.x + zone1.w,
                           zone1.y + zone1.h, 1.0F, 1.0F, 0.0F, 1.0F, 1);
          this.DrawIsoLine(zone1.x, zone1.y, zone1.x, zone1.y + zone1.h, 1.0F,
                           1.0F, 0.0F, 1.0F, 1);
          this.DrawIsoLine(zone1.x + zone1.w, zone1.y, zone1.x + zone1.w,
                           zone1.y + zone1.h, 1.0F, 1.0F, 0.0F, 1.0F, 1);
        }
      }
    }

    arrayList = IsoWorld.instance.MetaGrid.getZonesIntersecting(
        this.gridX - 1, this.gridY - 1, this.m_z, 3, 3,
    ...
    for (int int5 = 0; int5 < arrayList.size(); int5++) {
      ...
      if (zone2 != nullptr && zone2.isPolyline() && !zone2.points.isEmpty()) {
        for (byte byte1 = 0; byte1 < zone2.points.size() - 2; byte1 += 2) {
          ...
          if (chunk != nullptr &&
              liangBarsky.lineRectIntersect(
                  int6, int7, float0, float1, chunk.wx * 10, chunk.wy * 10,
                  chunk.wx * 10 + 10, chunk.wy * 10 + 10, doubles)) {
            this.DrawIsoLine(int6 + (float)doubles[0] * float0,
                             int7 + (float)doubles[0] * float1,
                             int6 + (float)doubles[1] * float0,
                             int7 + (float)doubles[1] * float1, 0.0F, 1.0F,
                             0.0F, 1.0F, 1);
          }
        }
        ...
        if (zone2.polylineOutlinePoints != nullptr) {
          ...
          for (byte byte2 = 0; byte2 < floats0.length; byte2 += 2) {
            ...
            this.DrawIsoLine(float2, float3, float4, float5, 1.0F, 1.0F, 0.0F,
                             1.0F, 1);
          }
        }
      }
    ...
    IsoMetaGrid.VehicleZone vehicleZone =
        IsoWorld.instance.MetaGrid.getVehicleZoneAt(this.gridX, this.gridY,
                                                    this.m_z);
    if (vehicleZone != nullptr) {
      ...
      if (vehicleZone.isPolygon()) {
        for (byte byte3 = 0; byte3 < vehicleZone.points.size(); byte3 += 2) {
          ...
          int int12 =
              vehicleZone.points.get((byte3 + 2) % vehicleZone.points.size());
          int int13 =
              vehicleZone.points.get((byte3 + 3) % vehicleZone.points.size());
          this.DrawIsoLine(int10, int11, int12, int13, 1.0F, 1.0F, 0.0F, 1.0F,
                           1);
        }
      } else if (vehicleZone.isPolyline()) {
        for (byte byte4 = 0; byte4 < vehicleZone.points.size() - 2;
             byte4 += 2) {
          ...
          this.DrawIsoLine(int14, int15, int16, int17, 1.0F, 1.0F, 0.0F, 1.0F,
                           1);
        }
        ...
        if (vehicleZone.polylineOutlinePoints != nullptr) {
          ...
          for (byte byte5 = 0; byte5 < floats1.length; byte5 += 2) {
            ...
            this.DrawIsoLine(float10, float11, float12, float13, 1.0F, 1.0F,
                             0.0F, 1.0F, 1);
          }
        }
      } else {
        this.DrawIsoLine(vehicleZone.x, vehicleZone.y,
                         vehicleZone.x + vehicleZone.w, vehicleZone.y, float6,
                         float7, float8, float9, 1);
        this.DrawIsoLine(vehicleZone.x, vehicleZone.y + vehicleZone.h,
                         vehicleZone.x + vehicleZone.w,
                         vehicleZone.y + vehicleZone.h, float6, float7, float8,
                         float9, 1);
        this.DrawIsoLine(vehicleZone.x, vehicleZone.y, vehicleZone.x,
                         vehicleZone.y + vehicleZone.h, float6, float7, float8,
                         float9, 1);
        this.DrawIsoLine(vehicleZone.x + vehicleZone.w, vehicleZone.y,
                         vehicleZone.x + vehicleZone.w,
                         vehicleZone.y + vehicleZone.h, float6, float7, float8,
                         float9, 1);
      }
  ...
  void drawVehicleStory() {
    std::vector arrayList = IsoWorld.instance.MetaGrid.getZonesIntersecting(
        this.gridX - 1, this.gridY - 1, this.m_z, 3, 3, new ArrayList<>());
    if (!arrayList.isEmpty()) {
      ...
      if (chunk != nullptr) {
        for (int int0 = 0; int0 < arrayList.size(); int0++) {
          ...
          if ("Nav" == zone.type)) {
              VehicleStorySpawner vehicleStorySpawner =
                  VehicleStorySpawner.getInstance();
              RandomizedVehicleStoryBase randomizedVehicleStoryBase =
                  IsoWorld.instance.getRandomizedVehicleStoryByName(
                      this.m_vehicleStory);
              if (randomizedVehicleStoryBase != nullptr &&
                ...
                if (randomizedVehicleStoryBase.getSpawnPoint(zone, chunk,
                                                             floats)) {
                  float float0 = floats[0];
                  float float1 = floats[1];
                  float float2 = floats[2] + (float)(Math.PI / 2);
                  vehicleStorySpawner.spawn(float0, float1, 0.0F, float2,
                                            (var0, var1x)->{});
                  vehicleStorySpawner.render(float0, float1, 0.0F, int1, int2,
                                             floats[2]);
                }
              }
            }
      ...
      for (int int1 = -5; int1 <= 6; int1++) {
        for (int int2 = -5; int2 <= 6; int2++) {
          if (int2 >= int1 - 5 && int2 <= int1 + 5) {
            this.paintSquare(square.getX() + int2 + int0 * 3,
                             square.getY() + int1 + int0 * 3,
                             square.getZ() + int0, 1.0F, 1.0F, 0.0F, 0.25F);
          }
        }
      }
    }
  ...
  void paintSquare(int int4, int int3, int int2, float float0, float float1,
    ...
    SpriteRenderer.instance.renderPoly(
        int1, int5, int1 + 32 * int0, int5 - 16 * int0, int1 + 64 * int0, int5,
        int1 + 32 * int0, int5 + 16 * int0, float0, float1, float2, float3);
  }

  void drawModData() {
    int int0 = this.m_z;
    IsoGridSquare square0 =
        IsoWorld.instance.getCell().getGridSquare(this.gridX, this.gridY, int0);
    ...
    if (square0 != nullptr && square0.getModData() != nullptr) {
      ...
      int int4;
      this.DrawString(int1, int4 = int2 + int3,
                      "MOD DATA x,y,z=" + square0.getX() + "," +
                          square0.getY() + "," + square0.getZ());
      KahluaTableIterator kahluaTableIterator0 = table.iterator();

      while (kahluaTableIterator0.advance()) {
        this.DrawString(int1, int4 += int3,
                        kahluaTableIterator0.getKey().toString() + " = " +
                            kahluaTableIterator0.getValue().toString());
        if (kahluaTableIterator0.getValue() instanceof KahluaTable) {
          KahluaTableIterator kahluaTableIterator1 =
              ((KahluaTable)kahluaTableIterator0.getValue()).iterator();

          while (kahluaTableIterator1.advance()) {
            this.DrawString(int1 + 8, int4 += int3,
                            kahluaTableIterator1.getKey().toString() + " = " +
                                kahluaTableIterator1.getValue().toString());
          }
        }
      }
    ...
    if (square0 != nullptr) {
      ...
      if (!arrayList.isEmpty()) {
        this.DrawString(int1, int2 += int3,
                        "PROPERTIES x,y,z=" + square0.getX() + "," +
                            square0.getY() + "," + square0.getZ());
        Collections.sort(arrayList);

        for (auto &string : arrayList)
          this.DrawString(int1, int2 += int3,
                          string + " = \"" + propertyContainer.Val(string) +
                              "\"");
      }
    }
  ...
  if (square0 != nullptr) {
    ...
    if (square1 != nullptr) {
      int2 += int3;
      int int5;
      this.DrawString(int1, int5 = int2 + int3,
                      "EROSION x,y,z=" + square0.getX() + "," + square0.getY() +
                          "," + square0.getZ());
      this.DrawString(int1, int2 = int5 + int3, "init=" + square1.init);
      int int6;
      this.DrawString(int1, int6 = int2 + int3,
                      "doNothing=" + square1.doNothing);
      this.DrawString(int1, int2 = int6 + int3,
                      "chunk.init=" + square0.chunk.getErosionData().init);
    }
  }
}
  ...
  int int3;
  this.DrawString(int0, int3 = int1 + int2,
                  "bored = " + character.getBodyDamage().getBoredomLevel());
  this.DrawString(int0, int1 = int3 + int2,
                  "endurance = " + character.getStats().endurance);
  int int4;
  this.DrawString(int0, int4 = int1 + int2,
                  "fatigue = " + character.getStats().fatigue);
  this.DrawString(int0, int1 = int4 + int2,
                  "hunger = " + character.getStats().hunger);
  int int5;
  this.DrawString(int0, int5 = int1 + int2,
                  "pain = " + character.getStats().Pain);
  this.DrawString(int0, int1 = int5 + int2,
                  "panic = " + character.getStats().Panic);
  int int6;
  this.DrawString(int0, int6 = int1 + int2,
                  "stress = " + character.getStats().getStress());
  this.DrawString(int0, int1 = int6 + int2,
                  "clothingTemp = " +
                      ((IsoPlayer)character).getPlayerClothingTemperature());
  int int7;
  this.DrawString(int0, int7 = int1 + int2,
                  "temperature = " + character.getTemperature());
  this.DrawString(int0, int1 = int7 + int2,
                  "thirst = " + character.getStats().thirst);
  int int8;
  this.DrawString(int0, int8 = int1 + int2,
                  "foodPoison = " +
                      character.getBodyDamage().getFoodSicknessLevel());
  this.DrawString(int0, int1 = int8 + int2,
                  "poison = " + character.getBodyDamage().getPoisonLevel());
  int int9;
  this.DrawString(int0, int9 = int1 + int2,
                  "unhappy = " +
                      character.getBodyDamage().getUnhappynessLevel());
  this.DrawString(int0, int1 = int9 + int2,
                  "infected = " + character.getBodyDamage().isInfected());
  int int10;
  this.DrawString(int0, int10 = int1 + int2,
                  "InfectionLevel = " +
                      character.getBodyDamage().getInfectionLevel());
  this.DrawString(int0, int1 = int10 + int2,
                  "FakeInfectionLevel = " +
                      character.getBodyDamage().getFakeInfectionLevel());
  int1 += int2;
  int int11;
  this.DrawString(int0, int11 = int1 + int2, "WORLD");
  this.DrawString(int0, int1 = int11 + int2,
                  "globalTemperature = " +
                      IsoWorld.instance.getGlobalTemperature());
}

public
LosUtil.TestResults lineClearCached(IsoCell cell, int x1, int y1, int z1,
  ...
  if (int3 >= 0 && int4 >= 0 && int5 >= 0 && int3 < 200 && int4 < 200) {
    ...
    if (Math.abs(int1) > Math.abs(int0) && Math.abs(int1) > Math.abs(int2)) {
      ...
      while (x0 != x1) {
        ...
        if (square1 != nullptr && square0 != nullptr &&
            square1.testVisionAdjacent(square0.getX() - square1.getX(),
                                       square0.getY() - square1.getY(),
                                       square0.getZ() - square1.getZ(), true,
                                       bIgnoreDoors) ==
                LosUtil.TestResults.Blocked) {
          this.paintSquare(x0, (int)float0, (int)float1, 1.0F, 0.0F, 0.0F,
                           0.5F);
          this.paintSquare(square0.getX(), square0.getY(), square0.getZ(), 1.0F,
                           0.0F, 0.0F, 0.5F);
          byte0 = 4;
        }
      ...
      while (y0 != y1) {
        ...
        if (square2 != nullptr && square0 != nullptr &&
            square2.testVisionAdjacent(square0.getX() - square2.getX(),
                                       square0.getY() - square2.getY(),
                                       square0.getZ() - square2.getZ(), true,
                                       bIgnoreDoors) ==
                LosUtil.TestResults.Blocked) {
          this.paintSquare((int)float0, y0, (int)float1, 1.0F, 0.0F, 0.0F,
                           0.5F);
          this.paintSquare(square0.getX(), square0.getY(), square0.getZ(), 1.0F,
                           0.0F, 0.0F, 0.5F);
          byte0 = 4;
      ...
      while (z0 != z1) {
        ...
        if (square3 != nullptr && square0 != nullptr &&
            square3.testVisionAdjacent(square0.getX() - square3.getX(),
                                       square0.getY() - square3.getY(),
                                       square0.getZ() - square3.getZ(), true,
                                       bIgnoreDoors) ==
                LosUtil.TestResults.Blocked) {
          byte0 = 4;
        }
    ...
    if (byte0 == 1) {
      ...
      return byte0 = =
                 4 ? LosUtil.TestResults.Blocked : LosUtil.TestResults.Blocked;
    }
  } else {
...
void DrawString(int int3, int int2, const std::string &string) {
  ...
  SpriteRenderer.instance.renderi(nullptr, int3 - 1, int2, int0 + 2, int1, 0.0F,
                                  0.0F, 0.0F, 0.8F, nullptr);
  TextManager.instance.DrawString(this.FONT, int3, int2, string, 1.0, 1.0, 1.0,
                                  1.0);
}
...
void save() {
  std::string string = ZomboidFileSystem.instance.getCacheDir() +
                       File.separator + "debugChunkState-options.ini";
...
private
  Stack<IsoGridSquare> stack = std::make_unique<Stack<>>();
    ...
    for (int int2 = 0; int2 < 11; int2++) {
      for (int int3 = 0; int3 < 11; int3++) {
        if (this.visited.getValue(int3, int2)) {
          int int4 = (int)IsoUtils.XToScreenExact(int0 + int3, int1 + int2 + 1,
                                                  this.start.getZ(), 0);
          int int5 = (int)IsoUtils.YToScreenExact(int0 + int3, int1 + int2 + 1,
                                                  this.start.getZ(), 0);
          SpriteRenderer.instance.renderPoly(int4, int5, int4 + 32, int5 - 16,
                                             int4 + 64, int5, int4 + 32,
                                             int5 + 16, 1.0F, 1.0F, 0.0F, 0.5F);
        }
      }
    }
  }
