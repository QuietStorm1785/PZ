...
public:
  static ClientPlayerDB instance = null;
  static bool allow = false;
private
  ClientPlayerDB.NetworkCharacterProfile networkProfile = nullptr;
...
public
  static synchronized ClientPlayerDB getInstance() {
  ...
  void updateMain() { this.saveNetworkPlayersToDB(); }
  ...
  void saveNetworkPlayersToDB() {
    if (this.canSavePlayers &&
        (this.forceSavePlayers || this.saveToDBPeriod4Network.Check())) {
...
public
  ArrayList<IsoPlayer> getAllNetworkPlayers() {
  ...
  void clientSendNetworkPlayerInt(IsoPlayer player) {
    if (GameClient.connection != nullptr) {
      try {
        ...
        std::string string = player.getDescriptor().getForename() + " " +
        ...
        if (int0 > 2097152) {
          DebugLog.log("FATAL ERROR: The player " + player.getUsername() +
                       " cannot be saved");
  ...
  bool isAliveMainNetworkPlayer() { return !this.networkProfile.isDead[0]; }

  bool clientLoadNetworkPlayer() {
    if (this.networkProfile != nullptr && this.networkProfile.isLoaded &&
        this.networkProfile.username == GameClient.username)
            && this.networkProfile.server == GameClient.ip)) {
...
public
  byte[] getClientLoadNetworkPlayerData(int int0) {
    if (this.networkProfile != nullptr && this.networkProfile.isLoaded &&
        this.networkProfile.username == GameClient.username)
            && this.networkProfile.server == GameClient.ip)) {
              switch (int0) {
  ...
  bool loadNetworkPlayer() {
    try {
      ...
      if (bytes != nullptr) {
        ...
        IsoPlayer.getInstance().load(byteBuffer,
                                     this.networkProfile.worldVersion[0]);
        return true;
      }
    } catch (Exception exception) {
      ExceptionLogger.logException(exception);
    ...
    if (this.networkProfile != nullptr && this.networkProfile.isLoaded &&
        this.networkProfile.username == GameClient.username)
            ...
            && int0 <= this.networkProfile.playerCount) {
              int int1 = (int)(this.networkProfile.x[int0 - 1] / 10.0F) +
                         IsoWorld.saveoffsetx * 30;
              int int2 = (int)(this.networkProfile.y[int0 - 1] / 10.0F) +
                         IsoWorld.saveoffsety * 30;
        ...
        this.networkProfile.username == GameClient.username)
            && this.networkProfile.server == GameClient.ip)) {
              for (int int0 = 1; int0 < 4; int0++) {
                if (this.networkProfile.character[int0] == nullptr ||
                    this.networkProfile.isDead[int0]) {
                  return int0 + 1;
                }
              }
            }
...
private
  final class NetworkCharacterProfile {
    bool isLoaded = false;
