...
public:
 ...
 private PlayerDB.IPlayerStore m_store = new PlayerDB.SQLPlayerStore();
 const TIntHashSet m_usedIDs = new TIntHashSet();
 private ConcurrentLinkedQueue<PlayerDB.PlayerData> m_toThread = std::make_unique<ConcurrentLinkedQueue<>>();
 private ConcurrentLinkedQueue<PlayerDB.PlayerData> m_fromThread = std::make_unique<ConcurrentLinkedQueue<>>();
 ...
 for (PlayerDB.PlayerData playerData = this->m_toThread.poll(); playerData != nullptr; playerData = this->m_toThread.poll()) {
 try {
 ...
 std::string string = "INSERT INTO localPlayers(wx,wy,x,y,z,worldversion,data,isDead,name,id) VALUES(?,?,?,?,?,?,?,?,?,?)";
 ...
 if (this->m_conn != nullptr && playerData.m_sqlID >= 1) {
 std::string string = "UPDATE localPlayers SET wx = ?, wy = ?, x = ?, y = ?, z = ?, worldversion = ?, data = ?, isDead = ?, name = ? WHERE id=?";
