...
private
  final PlayerDB.IPlayerStore m_store = new PlayerDB.SQLPlayerStore();
  const TIntHashSet m_usedIDs = new TIntHashSet();
private
  final ConcurrentLinkedQueue<PlayerDB.PlayerData> m_toThread =
      std::make_unique<ConcurrentLinkedQueue<>>();
private
  final ConcurrentLinkedQueue<PlayerDB.PlayerData> m_fromThread =
      std::make_unique<ConcurrentLinkedQueue<>>();
...
public
  static synchronized PlayerDB getInstance() {
