...
public:
  static ServerPlayerDB instance = null;
  static bool allow = false;
  Connection conn = null;
private
  ConcurrentLinkedQueue<ServerPlayerDB.NetworkCharacterData> CharactersToSave;

  static void setAllow(bool boolean0) { allow = boolean0; }

  static bool isAllow() { return allow; }

public
  static synchronized ServerPlayerDB getInstance() {
...
public
  ...
  void create() {
    ...
    try {
      ...
      ResultSet resultSet =
          databaseMetaData.getColumns(null, null, "networkPlayers", "steamid");
      if (!resultSet.next()) {
        statement.executeUpdate(
            "ALTER TABLE 'networkPlayers' ADD 'steamid' STRING NULL");
      }
  ...
  void process() {
    if (!this.CharactersToSave.isEmpty()) {
      for (ServerPlayerDB.NetworkCharacterData networkCharacterData =
               this.CharactersToSave.poll();
  ...
  void serverUpdateNetworkCharacter(ByteBuffer byteBuffer,
                                    UdpConnection udpConnection) {
    this.CharactersToSave.add(
        new ServerPlayerDB.NetworkCharacterData(byteBuffer, udpConnection));
  }

  void serverUpdateNetworkCharacterInt(
      ServerPlayerDB.NetworkCharacterData networkCharacterData) {
    if (networkCharacterData.playerIndex >= 0 &&
        networkCharacterData.playerIndex < 4) {
      if (this.conn != nullptr) {
        std::string string0;
        if (GameServer.bCoop && SteamUtils.isSteamModeEnabled()) {
          string0 = "SELECT id FROM networkPlayers WHERE steamid=? AND world=? "
                    "AND playerIndex=?";
        } else {
          string0 = "SELECT id FROM networkPlayers WHERE username=? AND "
                    "world=? AND playerIndex=?";
        }

        std::string string1 =
            "INSERT INTO networkPlayers(world,username,steamid, "
            "playerIndex,name,x,y,z,worldversion,isDead,data) "
            "VALUES(?,?,?,?,?,?,?,?,?,?,?)";
        std::string string2 =
            "UPDATE networkPlayers SET x=?, y=?, z=?, worldversion = ?, isDead "
            "= ?, data = ?, name = ? WHERE id=?";

        try {
          try(PreparedStatement preparedStatement0 =
                  this.conn.prepareStatement(string0)) {
            if (GameServer.bCoop && SteamUtils.isSteamModeEnabled()) {
            ...
            if (resultSet.next()) {
              ...
              try(PreparedStatement preparedStatement1 =
                      this.conn.prepareStatement(string2)) {
                ...
                preparedStatement1.setString(7,
                                             networkCharacterData.playerName);
          ...
          try(PreparedStatement preparedStatement2 =
                  this.conn.prepareStatement(string1)) {
  ...
  void serverConvertNetworkCharacter(const std::string &string2,
    ...
    try {
      std::string string0 =
          ...
          "AND (steamid is null or steamid = '')";
      ...
      try(PreparedStatement preparedStatement =
        ...
        if (int0 > 0) {
          DebugLog.General.warn(
              "serverConvertNetworkCharacter: The steamid was set for the '" +
              string2 + "' for " + int0 + " players. ");
        }
  ...
  void serverLoadNetworkCharacter(ByteBuffer byteBuffer,
                                  UdpConnection udpConnection) {
    ...
    if (byte0 < 0 || byte0 >= 4) {
      ...
      if (GameServer.bCoop && SteamUtils.isSteamModeEnabled()) {
        this.serverConvertNetworkCharacter(udpConnection.username,
                                           udpConnection.idStr);
      }

      std::string string;
      if (GameServer.bCoop && SteamUtils.isSteamModeEnabled()) {
        string = "SELECT id, x, y, z, data, worldversion, isDead FROM "
                 "networkPlayers WHERE steamid=? AND world=? AND playerIndex=?";
      } else {
        string =
            "SELECT id, x, y, z, data, worldversion, isDead FROM "
            "networkPlayers WHERE username=? AND world=? AND playerIndex=?";
      }

      try(PreparedStatement preparedStatement =
              this.conn.prepareStatement(string)) {
        if (GameServer.bCoop && SteamUtils.isSteamModeEnabled()) {
...
private
  static final class NetworkCharacterData {
    byte[] buffer;
    std::string username;
    std::string steamid;
    int playerIndex;
  ...
  public
    NetworkCharacterData(ByteBuffer byteBuffer, UdpConnection udpConnection) {
