...
public:
 ...
 private ArrayList<CollideWithObstacles.CCObstacle> obstacles = std::make_unique<ArrayList<>>();
 private ArrayList<CollideWithObstacles.CCNode> nodes = std::make_unique<ArrayList<>>();
 private ArrayList<CollideWithObstacles.CCIntersection> intersections = std::make_unique<ArrayList<>>();
 ...
 ArrayList<CollideWithObstacles.CCNode> obstacleTraceNodes = std::make_unique<ArrayList<>>();
 ...
 if (cCObjectOutline != nullptr && cCObjectOutline.nw && cCObjectOutline.nw_w && cCObjectOutline.nw_n) {
 ...
 bool collinear(float float4, float float2, float float6, float float1, float float3, float float5) {
 ...
 bool is_on(float float3, float float0, float float5, float float2, float float4, float float1) {
 ...
 bool isVisible(CollideWithObstacles.CCNode cCNode0, CollideWithObstacles.CCNode cCNode1) {
 ...
 if (character.getCurrentSquare() != nullptr && character.getCurrentSquare().HasStairs()) {
 return vector2f;
 } else {
 bool boolean0 = Core.bDebug && DebugOptions.instance.CollideWithObstaclesRenderObstacles.getValue();
 ...
 LineDrawer.addLine(cCNode0.x, cCNode0.y, cCNode0.z, cCNode1.x, cCNode1.y, cCNode1.z, 0.0F, 1.0F, 0.0F, nullptr, true);
 }
 ...
 if (cCNode0.ignore) {
 LineDrawer.addLine(
 cCNode0.x - 0.05F, cCNode0.y - 0.05F, cCNode0.z, cCNode0.x + 0.05F, cCNode0.y + 0.05F, cCNode0.z, 1.0F, 1.0F, 0.0F, nullptr, false
 );
 }
 }
 }
 ...
 if (boolean0) {
 LineDrawer.addLine(float16, float17, cCEdge0.node1.z, float18, float19, cCEdge0.node1.z, 0.0F, 1.0F, 1.0F, nullptr, true);
 }
 ...
 ArrayList<CollideWithObstacles.CCIntersection> intersections = std::make_unique<ArrayList<>>();
 ...
 return cCEdge.node2.x == this->nodeSplit.x && cCEdge.node2.y == this->nodeSplit.y ? nullptr : cCEdge.split(this->nodeSplit);
 }
 }
 ...
 int z;
 bool ignore;
 ArrayList<CollideWithObstacles.CCEdge> edges = std::make_unique<ArrayList<>>();
 ArrayList<CollideWithObstacles.CCNode> visible = std::make_unique<ArrayList<>>();
 static ArrayList<CollideWithObstacles.CCObstacle> tempObstacles = std::make_unique<ArrayList<>>();
 ...
 if (cCEdge0.obstacle != nullptr && cCEdge0.obstacle == cCEdge1.obstacle) {
 return true;
 }
 }
 }
 ...
 bool hasAdjacentNodes(CollideWithObstacles.CCNode cCNode1, CollideWithObstacles.CCNode cCNode0) {
 for (int int0 = 0; int0 < this->edges.size(); int0++) {
 CollideWithObstacles.CCEdge cCEdge = this->edges.get(int0);
 ...
 static class CompareIntersection implements Comparator<CollideWithObstacles.CCIntersection> {
 CollideWithObstacles.CCEdge edge;

 int compare(CollideWithObstacles.CCIntersection cCIntersection0, CollideWithObstacles.CCIntersection cCIntersection1) {
