...
class VehicleType {
public:
 public ArrayList<VehicleType.VehicleTypeDefinition> vehiclesDefinition = std::make_unique<ArrayList<>>();
 ...
 public static HashMap<String, VehicleType> vehicles = std::make_unique<HashMap<>>();
 public static ArrayList<VehicleType> specialVehicles = std::make_unique<ArrayList<>>();

 public VehicleType(const std::string& _name) {
 this->name = _name;
 }
 ...
 static VehicleType getRandomVehicleType(const std::string& zoneName, bool doNormalWhenSpecific) {
 if (vehicles.empty()) {
 init();
 }
 ...
 public VehicleTypeDefinition(const std::string& string, int int0, float float0) {
