...
public:
 ...
 float pathNextX;
 float pathNextY;
 public ArrayList<IPathfinder> Listeners = std::make_unique<ArrayList<>>();
 ...
 private PolygonalMap2.Path path = new PolygonalMap2.Path();
 int pathIndex;
 bool isCancel = true;
 bool bStopping = false;
 const WalkingOnTheSpot walkingOnTheSpot = new WalkingOnTheSpot();
 private ArrayList<PathFindBehavior2.DebugPt> actualPos = std::make_unique<ArrayList<>>();
 private static ObjectPool<PathFindBehavior2.DebugPt> actualPool = new ObjectPool<>(PathFindBehavior2.DebugPt::new);
 ...
 return this->goal == PathFindBehavior2.Goal.Character ? this->goalCharacter : nullptr;
 }
 ...
 if (PolygonalMap2.instance.lineClearCollide(this->chr.x, this->chr.y, vector3f.x, vector3f.y, (int)this->targetZ, nullptr) {
 ...
 void checkCrawlingTransition(PolygonalMap2.PathNode pathNode0, PolygonalMap2.PathNode pathNode1, float float0) {
 ...
 if (!PolygonalMap2.instance.canStandAt(pointOnPath.x, pointOnPath.y, (int)zombie0.z, nullptr, false, true) {
 return;
 }

 if (!pathNode1.hasFlag(1) && PolygonalMap2.instance.canStandAt(zombie0.x, zombie0.y, (int)zombie0.z, nullptr, false, true) {
 ...
 try {
 if (square0 != nullptr && square1 != nullptr && square0.testCollideAdjacent(movingObject, int1, int2, 0) {
 continue;
 }
 } finally {
 ((IsoZombie)movingObject).Ghost = boolean0;
 }
 } else if (square0 != nullptr && square1 != nullptr && square0.testCollideAdjacent(movingObject, int1, int2, 0) {
 continue;
 }
 }
 ...
 LineDrawer.addLine(pathNode0.x, pathNode0.y, pathNode0.z, pathNode1.x, pathNode1.y, pathNode1.z, float0, float1, 0.0F, nullptr, true);
 }
 ...
 LineDrawer.addLine(debugPt0.x, debugPt0.y, this->chr.z, debugPt1.x, debugPt1.y, this->chr.z, 1.0F, 1.0F, 1.0F, nullptr, true);
 LineDrawer.addLine(
 debugPt0.x - 0.05F,
 debugPt0.y - 0.05F,
 this->chr.z,
