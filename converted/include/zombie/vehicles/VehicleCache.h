...
public:
 ...
 float y;
 float z;
 private static TShortObjectHashMap<VehicleCache> mapId = std::make_unique<TShortObjectHashMap<>>();
 private static TIntObjectHashMap<List<VehicleCache>> mapXY = std::make_unique<TIntObjectHashMap<>>();

 static void vehicleUpdate(short short0, float float0, float float1, float float2) {
