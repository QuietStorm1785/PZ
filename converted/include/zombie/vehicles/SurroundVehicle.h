...
public:
 private static ObjectPool<SurroundVehicle.Position> s_positionPool = new ObjectPool<>(SurroundVehicle.Position::new);
 ...
 float y4p;
 bool m_bMoved = false;
 private ArrayList<SurroundVehicle.Position> m_positions = std::make_unique<ArrayList<>>();
 long m_updateMS = 0L;
 ...
 void addPositions(float float5, float float3, float float7, float float12, SurroundVehicle.PositionSide positionSide) {
