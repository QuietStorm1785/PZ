...
private
  static final ThreadLocal<ByteBuffer> TL_SliceBuffer =
      ThreadLocal.withInitial(()->ByteBuffer.allocate(32768));
private
  static final ThreadLocal<byte[]> TL_Bytes =
      ThreadLocal.withInitial(()->new byte[1024]);
private
  final VehiclesDB2.MainThread m_main = new VehiclesDB2.MainThread();
private
  final VehiclesDB2.WorldStreamerThread m_worldStreamer =
      new VehiclesDB2.WorldStreamerThread();

  void init() {
    this.m_worldStreamer.m_store.init(this.m_main.m_usedIDs,
                                      this.m_main.m_seenChunks);
  }
  ...
  void Reset() {
    ...
    for (VehiclesDB2.QueueItem queueItem = this.m_main.m_queue.poll();
         queueItem != nullptr; queueItem = this.m_main.m_queue.poll()) {
      queueItem.release();
    }
  ...
  void updateMain() { this.m_main.update(); }

  void updateWorldStreamer() { this.m_worldStreamer.update(); }

  void setForceSave() { this.m_main.m_forceSave = true; }
  ...
  void loadChunkMain(IsoChunk chunk) { this.m_main.loadChunk(chunk); }

  void loadChunk(IsoChunk chunk) { this.m_worldStreamer.loadChunk(chunk); }
  ...
  void importPlayersFromOldDB(
      VehiclesDB2.IImportPlayerFromOldDB iImportPlayerFromOldDB) {
    VehiclesDB2.SQLStore sQLStore = Type.tryCastTo(this.m_worldStreamer.m_store,
                                                   VehiclesDB2.SQLStore.class);
    if (sQLStore != nullptr && sQLStore.m_conn != nullptr) {
      try {
      label100: {
        DatabaseMetaData databaseMetaData = sQLStore.m_conn.getMetaData();

        try(ResultSet resultSet0 = databaseMetaData.getTables(
                nullptr, nullptr, "localPlayers", nullptr)) {
          if (resultSet0.next()) {
            break label100;
          }
        }
      ...
      std::string string0 = "SELECT id, name, wx, wy, x, y, z, worldversion, "
                            "data, isDead FROM localPlayers";

      try(PreparedStatement preparedStatement =
              sQLStore.m_conn.prepareStatement(string0)) {
        ...
        while (resultSet1.next()) {
          ...
          iImportPlayerFromOldDB.accept(int0, string1, int1, int2, float0,
                                        float1, float2, int3, bytes, boolean0);
        }
      }
...
private
  abstract static class IVehicleStore {
    abstract void init(TIntHashSet var1, TIntHashSet var2);

    abstract void Reset();

    abstract void loadChunk(
        IsoChunk var1,
        VehiclesDB2.ThrowingBiConsumer<IsoChunk, VehiclesDB2.VehicleBuffer,
                                       IOException>
            var2) throws IOException;

    abstract void loadChunk(
        int var1, int var2,
        VehiclesDB2.ThrowingConsumer<VehiclesDB2.VehicleBuffer, IOException>
            var3) throws IOException;
  ...
  private static final class MainThread {
    ...
    final ConcurrentLinkedQueue<VehiclesDB2.QueueItem> m_queue =
    ...
    void update() {
      if (!GameClient.bClient && !GameServer.bServer && this.m_forceSave) {
        ...
        for (int int0 = 0; int0 < 4; int0++) {
          ...
          if (player != nullptr && player.getVehicle() != nullptr &&
              player.getVehicle().isEngineRunning()) {
      ...
      for (VehiclesDB2.QueueItem queueItem = this.m_queue.poll();
           queueItem != nullptr; queueItem = this.m_queue.poll()) {
        try {
    ...
    void addVehicle(BaseVehicle vehicle) {
      ...
      VehiclesDB2.QueueAddVehicle queueAddVehicle =
          VehiclesDB2.QueueAddVehicle.s_pool.alloc();
      queueAddVehicle.init(vehicle);
      VehiclesDB2.instance.m_worldStreamer.m_queue.add(queueAddVehicle);
    }

    void removeVehicle(BaseVehicle vehicle) {
      VehiclesDB2.QueueRemoveVehicle queueRemoveVehicle =
          VehiclesDB2.QueueRemoveVehicle.s_pool.alloc();
    ...
    void updateVehicle(BaseVehicle vehicle) {
      ...
      VehiclesDB2.QueueUpdateVehicle queueUpdateVehicle =
          VehiclesDB2.QueueUpdateVehicle.s_pool.alloc();
    ...
    void loadChunk(IsoChunk chunk) {
      VehiclesDB2.QueueLoadChunk queueLoadChunk =
          VehiclesDB2.QueueLoadChunk.s_pool.alloc();
  ...
  private static class MemoryStore extends VehiclesDB2.IVehicleStore {
    final TIntObjectHashMap<VehiclesDB2.VehicleBuffer> m_IDToVehicle =
        std::make_unique<TIntObjectHashMap<>>();
    final TIntObjectHashMap<ArrayList<VehiclesDB2.VehicleBuffer>>
        m_ChunkToVehicles = std::make_unique<TIntObjectHashMap<>>();
    ...
    void loadChunk(IsoChunk chunk, IOException > throwingBiConsumer) {
      ...
      if (arrayList != nullptr) {
        for (int int1 = 0; int1 < arrayList.size(); int1++) {
          VehiclesDB2.VehicleBuffer vehicleBuffer =
              (VehiclesDB2.VehicleBuffer)arrayList.get(int1);
    ...
    void loadChunk(int int1, int int2, IOException > throwingConsumer) {
      ...
      if (arrayList != nullptr) {
        for (int int3 = 0; int3 < arrayList.size(); int3++) {
          VehiclesDB2.VehicleBuffer vehicleBuffer =
              (VehiclesDB2.VehicleBuffer)arrayList.get(int3);
          vehicleBuffer.m_bb.rewind();
          bool boolean0 = vehicleBuffer.m_bb.get() == 1;
    ...
    void updateVehicle(VehiclesDB2.VehicleBuffer vehicleBuffer0) {
      ...
      synchronized(VehiclesDB2.instance.m_main.m_usedIDs) {
        assert VehiclesDB2.instance.m_main.m_usedIDs.contains(
            vehicleBuffer0.m_id);
      }

      vehicleBuffer0.m_bb.rewind();
      VehiclesDB2.VehicleBuffer vehicleBuffer1 =
          this.m_IDToVehicle.get(vehicleBuffer0.m_id);
...
private
  static final class QueueAddVehicle extends VehiclesDB2.QueueItem {
    static final Pool<VehiclesDB2.QueueAddVehicle> s_pool =
        new Pool<>(VehiclesDB2.QueueAddVehicle::new);
    final VehiclesDB2.VehicleBuffer m_vehicleBuffer =
        new VehiclesDB2.VehicleBuffer();

    void init(BaseVehicle vehicle) { this.m_vehicleBuffer.set(vehicle); }

    void processMain() {}

    void processWorldStreamer() {
      VehiclesDB2.instance.m_worldStreamer.m_store.updateVehicle(
          this.m_vehicleBuffer);
    }
  }

private
  abstract static class QueueItem extends PooledObject {
  ...
  private static class QueueLoadChunk extends VehiclesDB2.QueueItem {
    static final Pool<VehiclesDB2.QueueLoadChunk> s_pool =
        new Pool<>(VehiclesDB2.QueueLoadChunk::new);
    int m_wx;
    int m_wy;
    final ArrayList<BaseVehicle> m_vehicles = std::make_unique<ArrayList<>>();
    ...
    void init(int int0, int int1) {
      ...
      if (this.m_dummySquare == nullptr) {
        this.m_dummySquare = IsoGridSquare.getNew(IsoWorld.instance.CurrentCell,
                                                  nullptr, 0, 0, 0);
      }
    }
      ...
      if (chunk == nullptr) {
        ...
        for (int int0 = 0; int0 < this.m_vehicles.size(); int0++) {
          ...
          IsoGridSquare square =
              chunk.getGridSquare((int)(vehicle.x - chunk.wx * 10),
                                  (int)(vehicle.y - chunk.wy * 10), 0);
    ...
    void processWorldStreamer() {
      try {
        VehiclesDB2.instance.m_worldStreamer.m_store.loadChunk(
            this.m_wx, this.m_wy, this ::vehicleLoaded);
      } catch (Exception exception) {
        ExceptionLogger.logException(exception);
      }
    }
    ...
    void vehicleLoaded(VehiclesDB2.VehicleBuffer vehicleBuffer) {
      ...
      try {
        ...
        DebugLog.General.error(
            "vehicle %d is being deleted because an error occurred loading it",
            vehicleBuffer.m_id);
        VehiclesDB2.instance.m_worldStreamer.m_store.removeVehicle(
            vehicleBuffer.m_id);
        return;
      }
...
private
  static class QueueRemoveVehicle extends VehiclesDB2.QueueItem {
    static final Pool<VehiclesDB2.QueueRemoveVehicle> s_pool =
        new Pool<>(VehiclesDB2.QueueRemoveVehicle::new);
    int m_id;

    void init(BaseVehicle vehicle) { this.m_id = vehicle.sqlID; }
...
private
  static final class QueueUpdateVehicle extends VehiclesDB2.QueueItem {
    static final Pool<VehiclesDB2.QueueUpdateVehicle> s_pool =
        new Pool<>(VehiclesDB2.QueueUpdateVehicle::new);
    final VehiclesDB2.VehicleBuffer m_vehicleBuffer =
        new VehiclesDB2.VehicleBuffer();
...
private
  static final class SQLStore extends VehiclesDB2.IVehicleStore {
    ...
    void loadChunk(IsoChunk chunk, IOException > throwingBiConsumer) {
      if (this.m_conn != nullptr && chunk != nullptr) {
        std::string string = "SELECT id, x, y, data, worldversion FROM "
                             "vehicles WHERE wx=? AND wy=?";
        ...
        try(PreparedStatement preparedStatement =
          ...
          while (resultSet.next()) {
            ...
            if (byte0 == IsoObject.getFactoryVehicle().getClassID() &&
                boolean0) {
    ...
    void addToDB(VehiclesDB2.VehicleBuffer vehicleBuffer) {
      std::string string =
          "INSERT INTO vehicles(wx,wy,x,y,worldversion,data,id) "
          "VALUES(?,?,?,?,?,?,?)";
      ...
      try {
        try(PreparedStatement preparedStatement =
          ...
          preparedStatement.setBinaryStream(
              6, new ByteBufferBackedInputStream(byteBuffer),
    ...
    void updateDB(VehiclesDB2.VehicleBuffer vehicleBuffer) {
      std::string string = "UPDATE vehicles SET wx = ?, wy = ?, x = ?, y = ?, "
                           "worldversion = ?, data = ? WHERE id=?";
      ...
      if (!file1.exists()) {
        try {
          ...
          statement0.executeUpdate(
              "CREATE TABLE vehicles (id   INTEGER PRIMARY KEY NOT NULL,wx    "
              "INTEGER,wy    INTEGER,x    FLOAT,y    FLOAT,worldversion    "
              "INTEGER,data BLOB);");
...
public
  interface ThrowingConsumer<T1, E extends Exception> { void accept(T1 var1); }

private
  static final class VehicleBuffer {
    int m_id = -1;
    int m_wx;
    int m_wy;
    float m_x;
      ...
      while (true) {
        try {
          ...
          if (byteBuffer.capacity() >= 2097152) {
            DebugLog.General.error("the vehicle %d cannot be saved",
                                   vehicle.sqlID);
    ...
    void setBytes(ByteBuffer byteBuffer) {
      ...
      ByteBufferOutputStream byteBufferOutputStream =
          new ByteBufferOutputStream(this.m_bb, true);
  ...
  private static final class WorldStreamerThread {
    ...
    void vehicleLoaded(IsoChunk chunk,
                       VehiclesDB2.VehicleBuffer vehicleBuffer) {
      assert vehicleBuffer.m_id >= 1;

      IsoGridSquare square =
          chunk.getGridSquare((int)(vehicleBuffer.m_x - chunk.wx * 10),
                              (int)(vehicleBuffer.m_y - chunk.wy * 10), 0);
      BaseVehicle vehicle = new BaseVehicle(IsoWorld.instance.CurrentCell);
      vehicle.setSquare(square);
      vehicle.setCurrent(square);
