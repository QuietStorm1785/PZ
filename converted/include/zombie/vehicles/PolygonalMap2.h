...
public:
 ...
 private ArrayList<PolygonalMap2.VehicleCluster> clusters = std::make_unique<ArrayList<>>();
 private PolygonalMap2.ClosestPointOnEdge closestPointOnEdge = new PolygonalMap2.ClosestPointOnEdge();
 private TIntObjectHashMap<PolygonalMap2.Node> squareToNode = std::make_unique<TIntObjectHashMap<>>();
 private ArrayList<PolygonalMap2.Square> tempSquares = std::make_unique<ArrayList<>>();
 static const PolygonalMap2 instance = new PolygonalMap2();
 private ArrayList<PolygonalMap2.VisibilityGraph> graphs = std::make_unique<ArrayList<>>();
 Clipper clipperThread;
 const ByteBuffer xyBufferThread = ByteBuffer.allocateDirect(8192);
 private PolygonalMap2.AdjustStartEndNodeData adjustStartData = new PolygonalMap2.AdjustStartEndNodeData();
 private PolygonalMap2.AdjustStartEndNodeData adjustGoalData = new PolygonalMap2.AdjustStartEndNodeData();
 ...
 private PolygonalMap2.TestRequest testRequest = new PolygonalMap2.TestRequest();
 int testZ = 0;
 private PathFindBehavior2.PointOnPath pointOnPath = new PathFindBehavior2.PointOnPath();
 ...
 private ConcurrentLinkedQueue<PolygonalMap2.IChunkTask> chunkTaskQueue = std::make_unique<ConcurrentLinkedQueue<>>();
 private ConcurrentLinkedQueue<PolygonalMap2.SquareUpdateTask> squareTaskQueue = std::make_unique<ConcurrentLinkedQueue<>>();
 private ConcurrentLinkedQueue<PolygonalMap2.IVehicleTask> vehicleTaskQueue = std::make_unique<ConcurrentLinkedQueue<>>();
 private ArrayList<PolygonalMap2.Vehicle> vehicles = std::make_unique<ArrayList<>>();
 private HashMap<BaseVehicle, PolygonalMap2.Vehicle> vehicleMap = std::make_unique<HashMap<>>();
 int minX;
 int minY;
 int width;
 int height;
 private PolygonalMap2.Cell[][] cells;
 private HashMap<BaseVehicle, PolygonalMap2.VehicleState> vehicleState = std::make_unique<HashMap<>>();
 private TObjectProcedure<PolygonalMap2.Node> releaseNodeProc = new TObjectProcedure<PolygonalMap2.Node>() {
 bool execute(PolygonalMap2.Node node) {
 node.release();
 return true;
 }
 };
 bool rebuild;
 private PolygonalMap2.Path shortestPath = new PolygonalMap2.Path();
 ...
 private PolygonalMap2.RequestQueue requests = new PolygonalMap2.RequestQueue();
 private ConcurrentLinkedQueue<PolygonalMap2.PathFindRequest> requestToMain = std::make_unique<ConcurrentLinkedQueue<>>();
 private ConcurrentLinkedQueue<PolygonalMap2.PathRequestTask> requestTaskQueue = std::make_unique<ConcurrentLinkedQueue<>>();
 private HashMap<Mover, PolygonalMap2.PathFindRequest> requestMap = std::make_unique<HashMap<>>();
 ...
 private float[] tempFloats = new float[8];
 const CollideWithObstacles collideWithObstacles = new CollideWithObstacles();
 const CollideWithObstaclesPoly collideWithObstaclesPoly = new CollideWithObstaclesPoly();
 ...
 if (square1 != nullptr && square2 != nullptr && square3 != nullptr && square4 != nullptr && square5 != nullptr) {
 ...
 if (square1 != nullptr && !square1.isNonThumpableSolid() && !square1.has(504) && square1.has(512) {
 ...
 if (square2 != nullptr && !square2.isNonThumpableSolid() && !square2.has(504) && square2.has(512) {
 ...
 if (square5 != nullptr && (square5.has(2) || square5.has(504) || square5.isReallySolid())) {
 float2 -= 0.35000002F;
 }

 if (square7 != nullptr && (square7.has(4) || square7.has(504) || square7.isReallySolid())) {
 float3 -= 0.35000002F;
 }
 ...
 bool boolean0 = square2 != nullptr && (square2.has(504) || square2.isReallySolid());
 ...
 bool boolean1 = square4 != nullptr && (square4.has(2) || square4.has(504) || square4.isReallySolid());
 ...
 bool boolean2 = square8 != nullptr && (square8.has(4) || square8.has(504) || square8.isReallySolid());
 ...
 bool boolean3 = square6 != nullptr && (square6.has(2) || square6.has(4) || square6.has(504) || square6.isReallySolid());
 if (boolean3) {
 ...
 && this->getChunkFromSquarePos((int)pathFindRequest.targetX, (int)pathFindRequest.targetY) != nullptr) {
 ...
 std::vector arrayList = this->astar.shortestPath(pathFindRequest, searchNode0, searchNode1);
 if (arrayList != nullptr) {
 if (arrayList.size() == 1) {
 pathFindRequest.path.addNode(searchNode0);
 if (!boolean2
 && searchNode1.square != nullptr
 ...
 bool boolean0 = pathFindRequest.mover instanceof IsoPlayer && ((IsoPlayer)pathFindRequest.mover).isNPC();
 ...
 PolygonalMap2.PathNode pathNode4 = int8 < pathFindRequest.path.nodes.size() - 1 ? pathFindRequest.path.nodes.get(int8 + 1) : nullptr;
 if (pathNode3.hasFlag(1) {
 bool boolean3 = pathNode2 != nullptr && pathNode2.hasFlag(2) || pathNode4 != nullptr && pathNode4.hasFlag(2);
 if (!boolean3) {
 pathNode3.flags &= -4;
 }
 }
 ...
 && IsoPlayer.getInstance().getPath2() == nullptr;
 ...
 LineDrawer.addLine(IsoPlayer.getInstance().x, IsoPlayer.getInstance().y, int0, float2, float3, int0, 1, 1, 1, nullptr);
 int int1 = 9;
 int1 |= 2;
 if (this->lccMain.isNotClear(this, IsoPlayer.getInstance().x, IsoPlayer.getInstance().y, float2, float3, int0, nullptr, int1) {
 Vector2f vector2f = this->resolveCollision(IsoPlayer.getInstance(), float2, float3, PolygonalMap2.L_render.vector2f);
 LineDrawer.addLine(
 vector2f.x - 0.05F, vector2f.y - 0.05F, int0, vector2f.x + 0.05F, vector2f.y + 0.05F, int0, 1.0F, 1.0F, 0.0F, nullptr, false
 );
 }
 }
 ...
 LineDrawer.addLine(
 (int)float6 - 1, (int)float7 + int3, (int)float8, (int)float6 + 2, (int)float7 + int3, (int)float8, 0.3F, 0.3F, 0.3F, nullptr, false
 );
 }

 for (int int4 = -1; int4 <= 2; int4++) {
 LineDrawer.addLine(
 (int)float6 + int4, (int)float7 - 1, (int)float8, (int)float6 + int4, (int)float7 + 2, (int)float8, 0.3F, 0.3F, 0.3F, nullptr, false
 );
 }
 ...
 LineDrawer.addLine(
 (int)float6, (int)float7, (int)float8, (int)float6, (int)float7, (int)IsoPlayer.getInstance().getZ(), 0.3F, 0.3F, 0.3F, nullptr, true
 );
 ...
 if (this->findPath(pathFindRequest, boolean1) && !pathFindRequest.path.empty()) {
 ...
 LineDrawer.addLine(pathNode0.x, pathNode0.y, float12, pathNode1.x, pathNode1.y, float13, float14, float15, float16, nullptr, true);
 ...
 void* object = LuaManager.env.rawget("ISPathFindAction_pathToLocationF");
 if (object != nullptr) {
 ...
 LineDrawer.addLine(pathNode4.x, pathNode4.y, pathNode4.z, pathNode5.x, pathNode5.y, pathNode5.z, float20, float21, float22, nullptr, true);
 }
 ...
 LineDrawer.addLine(node0.x, node0.y, int9, node1.x, node1.y, int9, 1.0F, 0.0F, 0.0F, nullptr, true);
 }
 }
 }
 }
 ...
 int int2 = int0 / 300;
 int int3 = int1 / 300;
 return int2 < this->width && int3 < this->height ? this->cells[int2][int3] : nullptr;
 } else {
 return nullptr;
 }
 }
 ...
 bool boolean0 = square1 != nullptr && this->astar.canNotMoveBetween(square0, square1, false);
 bool boolean1 = square2 != nullptr && this->astar.canNotMoveBetween(square0, square2, false);
 bool boolean2 = square3 != nullptr && this->astar.canNotMoveBetween(square0, square3, false);
 bool boolean3 = square4 != nullptr && this->astar.canNotMoveBetween(square0, square4, false);
 ...
 for (PolygonalMap2.IChunkTask iChunkTask = this->chunkTaskQueue.poll(); iChunkTask != nullptr; iChunkTask = this->chunkTaskQueue.poll()) {
 iChunkTask.release();
 }
 ...
 for (PolygonalMap2.IVehicleTask iVehicleTask = this->vehicleTaskQueue.poll(); iVehicleTask != nullptr; iVehicleTask = this->vehicleTaskQueue.poll()) {
 ...
 for (PolygonalMap2.PathFindRequest pathFindRequest = this->requestToMain.poll(); pathFindRequest != nullptr; pathFindRequest = this->requestToMain.poll()) {
 ...
 float float3 = this->shortestPath.empty() ? Float.MAX_VALUE : this->shortestPath.length();
 ...
 bool lineClearCollide(float float0, float float1, float float2, float float3, int int0) {
 return this->lineClearCollide(float0, float1, float2, float3, int0, nullptr);
 }

 bool lineClearCollide(float float0, float float1, float float2, float float3, int int0, IsoMovingObject movingObject) {
 ...
 bool lineClearCollide(float float0, float float1, float float2, float float3, int int0, IsoMovingObject movingObject, int int1) {
 ...
 Vector2 getCollidepoint(float float0, float float1, float float2, float float3, int int0, IsoMovingObject movingObject, int int1) {
 BaseVehicle vehicle = nullptr;
 if (movingObject instanceof IsoGameCharacter) {
 vehicle = ((IsoGameCharacter)movingObject).getVehicle();
 } else if (movingObject instanceof BaseVehicle) {
 ...
 bool canStandAt(float float0, float float1, int int0, IsoMovingObject movingObject, bool boolean0, bool boolean1) {
 BaseVehicle vehicle = nullptr;
 if (movingObject instanceof IsoGameCharacter) {
 vehicle = ((IsoGameCharacter)movingObject).getVehicle();
 } else if (movingObject instanceof BaseVehicle) {
 ...
 bool canStandAt(float float0, float float1, int int1, BaseVehicle vehicle, int int0) {
 return this->lccMain.canStandAtOld(this, float0, float1, int1, vehicle, int0);
 }

 bool intersectLineWithVehicle(float float8, float float6, float float7, float float5, BaseVehicle vehicle, Vector2 vector) {
 ...
 static ArrayDeque<PolygonalMap2.Cell> pool = std::make_unique<ArrayDeque<>>();
 ...
 int0 -= this->cx * 30;
 int1 -= this->cy * 30;
 return int0 >= 0 && int0 < 30 && int1 >= 0 && int1 < 30 ? this->chunks[int0][int1] : nullptr;
 }
 }
 ...
 static ArrayDeque<PolygonalMap2.Chunk> pool = std::make_unique<ArrayDeque<>>();
 ...
 int0 -= this->wx * 10;
 int1 -= this->wy * 10;
 return int0 >= 0 && int0 < 10 && int1 >= 0 && int1 < 10 && int2 >= 0 && int2 < 8 ? this->squares[int0][int1][int2] : nullptr;
 }
 ...
 public ArrayList<PolygonalMap2.Obstacle> obstacles = std::make_unique<ArrayList<>>();
 public ArrayList<PolygonalMap2.Node> nodes = std::make_unique<ArrayList<>>();
 int z;
 static short EPOCH = 0;
 short epoch;
 public static ObjectPool<PolygonalMap2.ChunkDataZ> pool = new ObjectPool<>(PolygonalMap2.ChunkDataZ::new);
 ...
 PolygonalMap2.Obstacle obstacle = PolygonalMap2.Obstacle.alloc().init((IsoGridSquare)nullptr);
 ...
 void getEdgesFromBuffer(ByteBuffer byteBuffer, PolygonalMap2.Obstacle obstacle, bool boolean0) {
 short short0 = byteBuffer.getShort();
 if (short0 < 3) {
 ...
 int wx;
 int wy;
 static ArrayDeque<PolygonalMap2.ChunkRemoveTask> pool = std::make_unique<ArrayDeque<>>();
 ...
 static ArrayDeque<PolygonalMap2.ChunkUpdateTask> pool = std::make_unique<ArrayDeque<>>();
 ...
 bool innerCorner;
 bool start;
 static ArrayDeque<PolygonalMap2.ClusterOutline> pool = std::make_unique<ArrayDeque<>>();
 ...
 PolygonalMap2.ClusterOutline clusterOutline = this->get(int0, int1, int2);
 return clusterOutline != nullptr && clusterOutline.inner && clusterOutline.w && !clusterOutline.tw;
 }

 bool canTrace_N(int int0, int int1, int int2) {
 PolygonalMap2.ClusterOutline clusterOutline = this->get(int0, int1, int2);
 return clusterOutline != nullptr && clusterOutline.inner && clusterOutline.n && !clusterOutline.tn;
 }

 bool canTrace_E(int int0, int int1, int int2) {
 PolygonalMap2.ClusterOutline clusterOutline = this->get(int0, int1, int2);
 return clusterOutline != nullptr && clusterOutline.inner && clusterOutline.e && !clusterOutline.te;
 }

 bool canTrace_S(int int0, int int1, int int2) {
 PolygonalMap2.ClusterOutline clusterOutline = this->get(int0, int1, int2);
 return clusterOutline != nullptr && clusterOutline.inner && clusterOutline.s && !clusterOutline.ts;
 }

 bool isInner(int int0, int int1, int int2) {
 PolygonalMap2.ClusterOutline clusterOutline = this->get(int0, int1, int2);
 return clusterOutline != nullptr && (clusterOutline.start || clusterOutline.inner);
 }
 ...
 return square0 != nullptr && square1 != nullptr ? !this->isBlocked(square0, square1, false) : false;
 }
 } else {
 return false;
 }
 ...
 bool isBlocked(PolygonalMap2.Square square1, PolygonalMap2.Square square0, bool boolean5) {
 ...
 bool boolean4 = boolean2 && square1.has(4) && (square1.x != square0.x || boolean5 || !square1.has(16384);
 bool boolean6 = boolean0 && square1.has(2) && (square1.y != square0.y || boolean5 || !square1.has(8192);
 bool boolean7 = boolean3 && square0.has(4) && (square1.x != square0.x || boolean5 || !square0.has(16384);
 bool boolean8 = boolean1 && square0.has(2) && (square1.y != square0.y || boolean5 || !square0.has(8192);
 ...
 int flags;
 static ArrayDeque<PolygonalMap2.Connection> pool = std::make_unique<ArrayDeque<>>();
 ...
 ArrayList<PolygonalMap2.Intersection> intersections = std::make_unique<ArrayList<>>();
 ...
 static ArrayDeque<PolygonalMap2.EdgeRing> pool = std::make_unique<ArrayDeque<>>();
 ...
 if (int0 % 10 == 0 && PolygonalMap2.instance.getChunkFromSquarePos(int0 - 1, int1) == nullptr) {
 return true;
 } else if (int0 % 10 == 9 && PolygonalMap2.instance.getChunkFromSquarePos(int0 + 1, int1) == nullptr) {
 return true;
 } else {
 return int1 % 10 == 0 && PolygonalMap2.instance.getChunkFromSquarePos(int0, int1 - 1) == nullptr
 ? true
 : int1 % 10 == 9 && PolygonalMap2.instance.getChunkFromSquarePos(int0, int1 + 1) == nullptr;
 }
 } else {
 ...
 float w;
 float h;
 static ArrayDeque<PolygonalMap2.ImmutableRectF> pool = std::make_unique<ArrayDeque<>>();
 ...
 bool lineRectIntersect(float float0, float float1, float float2, float float3, float float4, float float5, float float6, float float7) {
 return this->lineRectIntersect(float0, float1, float2, float3, float4, float5, float6, float7, nullptr);
 }
 ...
 bool canStandAt(PolygonalMap2 polygonalMap2, float float4, float float3, float float2, float float1, float float0, PolygonalMap2.Vehicle vehicle1) {
 ...
 bool boolean0 = float2 >= int5 && float1 >= int4 && float2 < int5 + 1 && float1 < int4 + 1;
 bool boolean1 = false;
 ...
 bool isPointInPolygon_WindingNumber(float float1, float float0, PolygonalMap2.VehiclePoly vehiclePoly) {
 ...
 bool isNotClearOld(PolygonalMap2 polygonalMap2, float float1, float float0, float float2, float float3, int int1, int int0) {
 ...
 return true;
 } else if (!this->canStandAt(polygonalMap2, float1, float0, float2, float3, int1, nullptr) {
 return true;
 } else {
 ...
 if (square != nullptr && !square.isReallySolid() && !square.has(504) && square.has(512) {
 ...
 bool isNotClearClipper(PolygonalMap2 polygonalMap2, float float1, float float0, float float2, float float3, int int1, int int0) {
 bool boolean0 = (int0 & 4) != 0;
 PolygonalMap2.Square square = polygonalMap2.getSquare((int)float1, (int)float0, int1);
 if (square != nullptr && square.has(504) {
 return true;
 } else if (!this->canStandAtClipper(polygonalMap2, float1, float0, float2, float3, int1, nullptr, int0) {
 return true;
 } else {
 ...
 bool isNotClearCost(float float5, float float2, float float4, float float1, int int1) {
 ...
 bool isNotClear(PolygonalMap2 polygonalMap2, float float0, float float1, float float2, float float3, int int0, int int1) {
 ...
 bool canStandAtOld(PolygonalMap2 var1, float float0, float float1, float float2, BaseVehicle vehicle1, int int0) {
 ...
 bool boolean2 = float0 >= int6 && float1 >= int5 && float0 < int6 + 1 && float1 < int5 + 1;
 ...
 bool canStandAtClipper(PolygonalMap2 var1, float float0, float float1, float float2, BaseVehicle vehicle, int int0) {
 return PolygonalMap2.instance.collideWithObstaclesPoly.canStandAt(float0, float1, float2, vehicle, int0);
 }

 void drawCircle(float float0, float float1, float float2, float float3, float float4, float float5, float float6, float float7) {
 LineDrawer.DrawIsoCircle(float0, float1, float2, float3, 16, float4, float5, float6, float7);
 }

 bool isNotClearOld(PolygonalMap2 polygonalMap2, float float1, float float0, float float3, float float2, int int1, BaseVehicle vehicle0, int int0) {
 bool boolean0 = (int0 & 1) != 0;
 bool boolean1 = (int0 & 2) != 0;
 ...
 LineDrawer.addLine(point0.x, point0.y, int1, point0.x + 1.0F, point0.y + 1.0F, int1, 1.0F, 1.0F, 0.0F, nullptr, false);
 }
 }
 ...
 if (boolean2 && square != nullptr && PolygonalMap2.SquareUpdateTask.getCost(square) > 0) {
 return true;
 }
 ...
 bool isNotClearClipper(PolygonalMap2 polygonalMap2, float float1, float float0, float float3, float float2, int int1, BaseVehicle vehicle, int int0) {
 bool boolean0 = (int0 & 1) != 0;
 bool boolean1 = (int0 & 2) != 0;
 bool boolean2 = (int0 & 4) != 0;
 bool boolean3 = (int0 & 8) != 0;
 ...
 bool isNotClear(PolygonalMap2 polygonalMap2, float float0, float float1, float float2, float float3, int int0, BaseVehicle vehicle, int int1) {
 ...
 for (int int3 = 0; int3 < this->pts.size(); int3++) {
 PolygonalMap2.Point point0 = this->pts.get(int3);
 LineDrawer.addLine(point0.x, point0.y, int2, point0.x + 1.0F, point0.y + 1.0F, int2, 1.0F, 1.0F, 0.0F, nullptr, false);
 }
 }
 ...
 bool polyVehicleIntersect(PolygonalMap2.VehiclePoly vehiclePolyx, bool boolean1) {
 ...
 if (Line2D.linesIntersect(vector0.x, vector0.y, vector1.x, vector1.y, vector2.x, vector2.y, vector3.x, vector3.y) {
 if (boolean1) {
 LineDrawer.addLine(vector0.x, vector0.y, 0.0F, vector1.x, vector1.y, 0.0F, 1.0F, 0.0F, 0.0F, nullptr, true);
 LineDrawer.addLine(vector2.x, vector2.y, 0.0F, vector3.x, vector3.y, 0.0F, 1.0F, 0.0F, 0.0F, nullptr, true);
 }
 ...
 ArrayList<PolygonalMap2.Connection> visible = std::make_unique<ArrayList<>>();
 int flags = 0;
 static ArrayList<PolygonalMap2.Obstacle> tempObstacles = std::make_unique<ArrayList<>>();
 static ArrayDeque<PolygonalMap2.Node> pool = std::make_unique<ArrayDeque<>>();
 ...
 static ArrayDeque<PolygonalMap2.ObjectOutline> pool = std::make_unique<ArrayDeque<>>();
 ...
 ArrayList<PolygonalMap2.EdgeRing> inner = std::make_unique<ArrayList<>>();
 PolygonalMap2.ImmutableRectF bounds;
 PolygonalMap2.Node nodeCrawlFront;
 PolygonalMap2.Node nodeCrawlRear;
 ArrayList<PolygonalMap2.Node> crawlNodes = std::make_unique<ArrayList<>>();
 static ArrayDeque<PolygonalMap2.Obstacle> pool = std::make_unique<ArrayDeque<>>();
 ...
 bool lineSegmentIntersects(float float0, float float1, float float2, float float3) {
 ...
 LineDrawer.addLine(node0.x, node0.y, node0.z, node1.x, node1.y, node1.z, float0, float1, float2, nullptr, true);
 ...
 LineDrawer.addLine(node2.x - 0.1F, node2.y - 0.1F, node2.z, node2.x + 0.1F, node2.y + 0.1F, node2.z, 1.0F, 0.0F, 0.0F, nullptr, false);
 }
 }
 ...
 ArrayList<PolygonalMap2.PathNode> nodes = std::make_unique<ArrayList<>>();
 ArrayDeque<PolygonalMap2.PathNode> nodePool = std::make_unique<ArrayDeque<>>();
 ...
 PolygonalMap2.PathNode pathNode = this->nodePool.empty() ? new PolygonalMap2.PathNode() : this->nodePool.pop();
 ...
 bool bCanThump;
 ArrayList<KnownBlockedEdges> knownBlockedEdges = std::make_unique<ArrayList<>>();
 float startX;
 float startY;
 float startZ;
 float targetX;
 ...
 bool cancel = false;
 static ArrayDeque<PolygonalMap2.PathFindRequest> pool = std::make_unique<ArrayDeque<>>();
 ...
 std::vector arrayList = character.getMapKnowledge().getKnownBlockedEdges();
 ...
 static ArrayDeque<PolygonalMap2.PathRequestTask> pool = std::make_unique<ArrayDeque<>>();
 ...
 } else if (this->request.mover instanceof IsoZombie && ((IsoZombie)this->request.mover).target != nullptr) {
 ...
 ArrayDeque<PolygonalMap2.PathFindRequest> playerQ = std::make_unique<ArrayDeque<>>();
 ArrayDeque<PolygonalMap2.PathFindRequest> aggroZombieQ = std::make_unique<ArrayDeque<>>();
 ArrayDeque<PolygonalMap2.PathFindRequest> otherQ = std::make_unique<ArrayDeque<>>();
 ...
 static ArrayDeque<PolygonalMap2.SearchNode> pool = std::make_unique<ArrayDeque<>>();
 ...
 double double0 = 0.0;
 bool boolean0 = this->astar.mover instanceof IsoZombie && ((IsoZombie)this->astar.mover).bCrawling;
 bool boolean1 = !(this->astar.mover instanceof IsoZombie) || ((IsoZombie)this->astar.mover).bCrawling;
 ...
 if (square0 != nullptr && !this->astar.isSquareInCluster(square0) && !this->astar.canNotMoveBetween(this->square, square0, false) {
 ...
 if (square1 != nullptr && square1.has(64) && !this->astar.isSquareInCluster(square1) {
 ...
 if (square1 != nullptr && square1.has(8) && !this->astar.isSquareInCluster(square1) {
 ...
 int bits;
 short cost;
 static ArrayDeque<PolygonalMap2.Square> pool = std::make_unique<ArrayDeque<>>();
 ...
 PolygonalMap2.Square square1 = PolygonalMap2.instance.getSquare(this->x - 1, this->y, this->z);
 return square1 != nullptr && (square1.has(262144) || square1.has(448) || square1.isReallySolid());
 }
 }
 ...
 PolygonalMap2.Square square1 = PolygonalMap2.instance.getSquare(this->x, this->y - 1, this->z);
 return square1 != nullptr && (square1.has(524288) || square1.has(56) || square1.isReallySolid());
 }
 }
 ...
 int bits;
 short cost;
 static ArrayDeque<PolygonalMap2.SquareUpdateTask> pool = std::make_unique<ArrayDeque<>>();
 ...
 if (thumpable != nullptr && thumpable.isThumpable() && thumpable.isBlockAllTheSquare()) {
 return true;
 }
 }
 ...
 && !thumpable.canClimbThrough(nullptr)
 && !thumpable.canClimbOver(nullptr)
 ...
 ArrayList<PolygonalMap2.VisibilityGraph> graphs;
 ArrayList<PolygonalMap2.SearchNode> searchNodes = std::make_unique<ArrayList<>>();
 TIntObjectHashMap<PolygonalMap2.SearchNode> nodeMap = std::make_unique<TIntObjectHashMap<>>();
 PolygonalMap2.GoalNode goalNode = new PolygonalMap2.GoalNode();
 TIntObjectHashMap<PolygonalMap2.SearchNode> squareToNode = std::make_unique<TIntObjectHashMap<>>();
 Mover mover;
 bool bCanCrawl;
 bool bIgnoreCrawlCost;
 bool bCanThump;
 TIntObjectHashMap<KnownBlockedEdges> knownBlockedEdges = std::make_unique<TIntObjectHashMap<>>();
 PolygonalMap2.VGAStar.InitProc initProc = new PolygonalMap2.VGAStar.InitProc();
 ...
 bool canNotMoveBetween(PolygonalMap2.Square square1, PolygonalMap2.Square square0, bool boolean9) {
 assert Math.abs(square1.x - square0.x) <= 1;
 ...
 bool boolean6 = square1.isCanPathN() && (this->bCanThump || !square1.isThumpN());
 bool boolean7 = square1.isCanPathW() && (this->bCanThump || !square1.isThumpW());
 bool boolean8 = boolean3 && square1.isCollideN() && (square1.x != square0.x || boolean9 || !boolean6);
 bool boolean10 = boolean1 && square1.isCollideW() && (square1.y != square0.y || boolean9 || !boolean7);
 boolean6 = square0.isCanPathN() && (this->bCanThump || !square0.isThumpN());
 boolean7 = square0.isCanPathW() && (this->bCanThump || !square0.isThumpW());
 bool boolean11 = boolean4 && square0.has(131076) && (square1.x != square0.x || boolean9 || !boolean6);
 bool boolean12 = boolean2 && square0.has(131074) && (square1.y != square0.y || boolean9 || !boolean7);
 ...
 if (square0.x == square1.x + 1 && square0.y == square1.y + 1 && square2 != nullptr && square3 != nullptr) {
 ...
 if (square2 != nullptr && this->canNotMoveBetween(square1, square2, true) {
 return true;
 }

 if (square3 != nullptr && this->canNotMoveBetween(square1, square3, true) {
 return true;
 }

 if (square2 != nullptr && this->canNotMoveBetween(square0, square2, true) {
 return true;
 }

 if (square3 != nullptr && this->canNotMoveBetween(square0, square3, true) {
 return true;
 }
 }
 ...
 bool isKnownBlocked(PolygonalMap2.Square square1, PolygonalMap2.Square square0) {
 ...
 return knownBlockedEdges0 != nullptr && knownBlockedEdges0.isBlocked(square0.x, square0.y)
 ? true
 : knownBlockedEdges1 != nullptr && knownBlockedEdges1.isBlocked(square1.x, square1.y);
 }
 }
 ...
 float upVectorDot;
 static ArrayDeque<PolygonalMap2.Vehicle> pool = std::make_unique<ArrayDeque<>>();
 ...
 const TFloatArrayList crawlOffsets = new TFloatArrayList();
 float upVectorDot;
 static ArrayDeque<PolygonalMap2.VehicleAddTask> pool = std::make_unique<ArrayDeque<>>();
 ...
 int z;
 ArrayList<PolygonalMap2.VehicleRect> rects = std::make_unique<ArrayList<>>();
 static ArrayDeque<PolygonalMap2.VehicleCluster> pool = std::make_unique<ArrayDeque<>>();
 ...
 float y4;
 float z;
 public Vector2[] borders = new Vector2[4];
 ...
 static Vector2 lineIntersection(Vector2 vector2, Vector2 vector1, Vector2 vector4, Vector2 vector3) {
 ...
 int h;
 int z;
 static ArrayDeque<PolygonalMap2.VehicleRect> pool = std::make_unique<ArrayDeque<>>();
 ...
 PolygonalMap2 map;
 BaseVehicle vehicle;
 static ArrayDeque<PolygonalMap2.VehicleRemoveTask> pool = std::make_unique<ArrayDeque<>>();
 ...
 PolygonalMap2.VehiclePoly polyPlusRadius = new PolygonalMap2.VehiclePoly();
 static ArrayDeque<PolygonalMap2.VehicleState> pool = std::make_unique<ArrayDeque<>>();
 ...
 bool check() {
 bool boolean0 = this->x != this->vehicle.x || this->y != this->vehicle.y || (int)this->z != (int)this->vehicle.z;
 if (!boolean0) {
 ...
 PolygonalMap2.VehiclePoly polyPlusRadius = new PolygonalMap2.VehiclePoly();
 float upVectorDot;
 static ArrayDeque<PolygonalMap2.VehicleUpdateTask> pool = std::make_unique<ArrayDeque<>>();
 ...
 ArrayList<PolygonalMap2.Edge> edges = std::make_unique<ArrayList<>>();
 ArrayList<PolygonalMap2.Obstacle> obstacles = std::make_unique<ArrayList<>>();
 ArrayList<PolygonalMap2.Node> intersectNodes = std::make_unique<ArrayList<>>();
 ArrayList<PolygonalMap2.Node> perimeterNodes = std::make_unique<ArrayList<>>();
 ArrayList<PolygonalMap2.Edge> perimeterEdges = std::make_unique<ArrayList<>>();
 ArrayList<PolygonalMap2.Node> obstacleTraceNodes = std::make_unique<ArrayList<>>();
 ...
 private static PolygonalMap2.ClusterOutlineGrid clusterOutlineGrid = new PolygonalMap2.ClusterOutlineGrid();
 private static ArrayDeque<PolygonalMap2.VisibilityGraph> pool = std::make_unique<ArrayDeque<>>();
 ...
 bool intersects(PolygonalMap2.Node node1, PolygonalMap2.Node node0, PolygonalMap2.Edge edge) {
 ...
 if (objectOutline != nullptr && objectOutline.nw && objectOutline.nw_w && objectOutline.nw_n) {
 ...
 void getEdgesFromBuffer(ByteBuffer byteBuffer, PolygonalMap2.Obstacle obstacle, bool boolean0, int int2) {
 short short0 = byteBuffer.getShort();
 if (short0 < 3) {
 byteBuffer.position(byteBuffer.position() + short0 * 4 * 2);
 } else {
 ...
 if (square != nullptr && square.has(72) && !arrayList.contains(square) {
 ...
 for (int int1 = vehicleRect.left(); int1 < vehicleRect.right(); int1++) {
 PolygonalMap2.Square square = PolygonalMap2.instance.getSquare(int1, int0, this->cluster.z);
 if (square != nullptr && (square.isCanPathW() || square.isCanPathN()) && !arrayList.contains(square) {
 arrayList.add(square);
 }
 }
 }
 ...
 if (square != nullptr && obstacle0.isPointInside(int7 + 0.5F, int6 + 0.5F) {
 ...
 node8.flags |= 1;
 bool boolean0 = edge0.node1.isConnectedTo(edge0.node2);
 ...
 if (obstacle1.vehicle != nullptr && obstacle1.nodeCrawlFront != nullptr) {
 ...
 if (square != nullptr && !square.isNonThumpableSolid() && !square.has(504) {
 ...
 PolygonalMap2.Edge edge = PolygonalMap2.Edge.alloc().init(node1, node2, nullptr, nullptr);
 ...
 int getPointOutsideObstacles(float float2, float float3, float float4, PolygonalMap2.AdjustStartEndNodeData adjustStartEndNodeData) {
 ...
 if (closestPointOnEdge.edge != nullptr && closestPointOnEdge.distSq < double0) {
 ...
 LineDrawer.addLine(edge0.node1.x, edge0.node1.y, this->cluster.z, edge0.node2.x, edge0.node2.y, this->cluster.z, float0, 0.5F, 0.5F, nullptr, true);
 float0 = 1.0F - float0;
 }
 ...
 LineDrawer.addLine(
 edge1.node1.x, edge1.node1.y, this->cluster.z, edge1.node2.x, edge1.node2.y, this->cluster.z, float0, 0.5F, 0.5F, nullptr, true
 );
 float0 = 1.0F - float0;
 }
 ...
 LineDrawer.addLine(
 edge2.node1.x, edge2.node1.y, this->cluster.z, edge2.node2.x, edge2.node2.y, this->cluster.z, float0, 0.5F, 0.5F, nullptr, true
 );
 float0 = 1.0F - float0;
 }
 }
 ...
 LineDrawer.addLine(
 node0.x - 0.05F, node0.y - 0.05F, this->cluster.z, node0.x + 0.05F, node0.y + 0.05F, this->cluster.z, 0.5F, 1.0F, 0.5F, nullptr, false
 );

 for (PolygonalMap2.Connection connection0 : node0.visible) {
 PolygonalMap2.Node node1 = connection0.otherNode(node0);
 if (node1.hasFlag(1) {
 LineDrawer.addLine(node0.x, node0.y, this->cluster.z, node1.x, node1.y, this->cluster.z, 0.5F, 1.0F, 0.5F, nullptr, true);
 }
 }
 }
 }
 ...
 LineDrawer.addLine(node2.x, node2.y, this->cluster.z, node3.x, node3.y, this->cluster.z, 0.0F, 0.25F, 0.0F, nullptr, true);
 }
 }
 ...
 LineDrawer.addLine(
 node2.x - 0.05F, node2.y - 0.05F, this->cluster.z, node2.x + 0.05F, node2.y + 0.05F, this->cluster.z, float1, float2, float3, nullptr, false
 );
 }
 }
 ...
 LineDrawer.addLine(node4.x, node4.y, this->cluster.z, node5.x, node5.y, this->cluster.z, 0.0F, 1.0F, 0.0F, nullptr, true);
 }
 }
 }
 ...
 LineDrawer.addLine(
 node4.x - 0.05F, node4.y - 0.05F, this->cluster.z, node4.x + 0.05F, node4.y + 0.05F, this->cluster.z, 1.0F, 1.0F, 0.0F, nullptr, false
 );
 }
 }
 ...
 LineDrawer.addLine(
 node6.x - 0.1F, node6.y - 0.1F, this->cluster.z, node6.x + 0.1F, node6.y + 0.1F, this->cluster.z, 1.0F, 0.0F, 0.0F, nullptr, false
 );
 }
 }

 static class CompareIntersection implements Comparator<PolygonalMap2.Intersection> {
 PolygonalMap2.Edge edge;

 int compare(PolygonalMap2.Intersection intersection0, PolygonalMap2.Intersection intersection1) {
