...
public:
 private ArrayList<UI3DScene.SceneObject> m_objects = std::make_unique<ArrayList<>>();
 ...
 private UI3DScene.CharacterSceneModelCamera m_CharacterSceneModelCamera = new UI3DScene.CharacterSceneModelCamera();
 private UI3DScene.VehicleSceneModelCamera m_VehicleSceneModelCamera = new UI3DScene.VehicleSceneModelCamera();
 private static ObjectPool<UI3DScene.SetModelCamera> s_SetModelCameraPool = new ObjectPool<>(UI3DScene.SetModelCamera::new);
 private UI3DScene.StateData[] m_stateData = new UI3DScene.StateData[3];
 private UI3DScene.Gizmo m_gizmo;
 private UI3DScene.RotateGizmo m_rotateGizmo = new UI3DScene.RotateGizmo();
 private UI3DScene.ScaleGizmo m_scaleGizmo = new UI3DScene.ScaleGizmo();
 private UI3DScene.TranslateGizmo m_translateGizmo = new UI3DScene.TranslateGizmo();
 const Vector3f m_gizmoPos = new Vector3f();
 const Vector3f m_gizmoRotate = new Vector3f();
 private UI3DScene.SceneObject m_gizmoParent = nullptr;
 private UI3DScene.SceneObject m_gizmoOrigin = nullptr;
 private UI3DScene.SceneObject m_gizmoChild = nullptr;
 private UI3DScene.OriginAttachment m_originAttachment = new UI3DScene.OriginAttachment(this);
 private UI3DScene.OriginBone m_originBone = new UI3DScene.OriginBone(this);
 private UI3DScene.OriginGizmo m_originGizmo = new UI3DScene.OriginGizmo(this);
 float m_gizmoScale = 1.0F;
 std::string m_selectedAttachment = nullptr;
 private ArrayList<UI3DScene.PositionRotation> m_axes = std::make_unique<ArrayList<>>();
 private UI3DScene.OriginBone m_highlightBone = new UI3DScene.OriginBone(this);
 private static ObjectPool<UI3DScene.PositionRotation> s_posRotPool = new ObjectPool<>(UI3DScene.PositionRotation::new);
 private ArrayList<UI3DScene.AABB> m_aabb = std::make_unique<ArrayList<>>();
 private static ObjectPool<UI3DScene.AABB> s_aabbPool = new ObjectPool<>(UI3DScene.AABB::new);
 private ArrayList<UI3DScene.Box3D> m_box3D = std::make_unique<ArrayList<>>();
 private static ObjectPool<UI3DScene.Box3D> s_box3DPool = new ObjectPool<>(UI3DScene.Box3D::new);
 ...
 private static ThreadLocal<ObjectPool<UI3DScene.Ray>> TL_Ray_pool = ThreadLocal.withInitial(UI3DScene.RayObjectPool::new);
 private static ThreadLocal<ObjectPool<UI3DScene.Plane>> TL_Plane_pool = ThreadLocal.withInitial(UI3DScene.PlaneObjectPool::new);
 ...
 if (boolean0) {
 throw NullPointerException("scene object \"" + string + "\" not found");
 } else {
 return nullptr;
 }
 }
 ...
 static void releaseRay(UI3DScene.Ray ray) {
 ...
 if (animationMultiTrack1 != nullptr && !animationMultiTrack1.getTracks().empty()) {
 ...
 if (animationMultiTrack0 != nullptr && !animationMultiTrack0.getTracks().empty()) {
 ...
 if (sceneModel5.m_modelScript.getAttachmentById((String)arg1) != nullptr) {
 throw IllegalArgumentException("model script \"" + arg0 + "\" already has attachment named \"" + arg1 + "\"");
 }
 ...
 return nullptr;
 default:
 throw IllegalArgumentException(String.format("unhandled \"%s\" \"%s\" \"%s\" \"%s\"", func, arg0, arg1, arg2);
 }
 }

 void* fromLua4(const std::string& func, void* arg0, void* arg1, void* arg2, void* arg3) {
 switch (func) {
 case "setGizmoOrigin":
 std::string string1 = (String)arg0;
 uint8_t byte0 = -1;
 ...
 if (sceneObject1.m_parent != nullptr && sceneObject1.m_parent.m_parent == sceneObject1) {
 ...
 std::string string0 = "inside".equalsIgnoreCase(position.getId()) ? "player-vehicle" : "player-editor";
 ...
 void* fromLua6(const std::string& func, void* arg0, void* arg1, void* arg2, void* arg3, void* arg4, void* arg5) {
 switch (func) {
 case "addAABB":
 float float0 = ((Double)arg0).floatValue();
 float float1 = ((Double)arg1).floatValue();
 ...
 void* fromLua9(const std::string& func, void* arg0, void* arg1, void* arg2, void* arg3, void* arg4, void* arg5, void* arg6, void* arg7, void* arg8) {
 uint8_t byte0 = -1;
 ...
 Vector3f uiToScene(Matrix4f modelTransform, float uiX, float uiY, float uiZ, Vector3f out) {
 ...
 void renderAABB(float float6, float float7, float float8, float float1, float float3, float float5, float float10, float float11, float float12) {
 ...
 void renderAABB(float float0, float float1, float float2, Vector3f vector3f1, Vector3f vector3f0, float float4, float float5, float float6) {
 ...
 static int intersect_ray_plane(UI3DScene.Plane Pn, UI3DScene.Ray S, Vector3f out) {
 ...
 void init(UI3DScene.SceneCharacter sceneCharacter, UI3DScene.CharacterRenderData characterRenderData) {
 ...
 private static ObjectPool<UI3DScene.CharacterRenderData> s_pool = new ObjectPool<>(UI3DScene.CharacterRenderData::new);
 ...
 bool hitTestRect(float float17, float float16, float float1, float float2, float float3, float float6, float float7, float float8) {
 ...
 private static class ModelDrawer extends TextureDraw.GenericDrawer {
 UI3DScene.SceneModel m_model;
 UI3DScene.ModelRenderData m_renderData;
 bool bRendered;

 void init(UI3DScene.SceneModel sceneModel, UI3DScene.ModelRenderData modelRenderData) {
 ...
 private static ObjectPool<UI3DScene.ModelRenderData> s_pool = new ObjectPool<>(UI3DScene.ModelRenderData::new);
 ...
 if (sceneModel.m_model.isReady() && sceneModel.m_model.Mesh.m_transform != nullptr) {
 sceneModel.m_model.Mesh.m_transform.transpose();
 this->m_transform.mul(sceneModel.m_model.Mesh.m_transform);
 sceneModel.m_model.Mesh.m_transform.transpose();
 }

 if (sceneModel.m_modelScript != nullptr && sceneModel.m_modelScript.scale != 1.0F) {
 ...
 private static class OriginAttachment extends UI3DScene.SceneObject {
 ...
 private static class OriginBone extends UI3DScene.SceneObject {
 UI3DScene.SceneCharacter m_character;
 ...
 private static class OriginGizmo extends UI3DScene.SceneObject {
 ...
 public static class PlaneObjectPool extends ObjectPool<UI3DScene.Plane> {
 int allocated = 0;
 ...
 ArrayList<Vector3f> m_circlePointsMain = std::make_unique<ArrayList<>>();
 ArrayList<Vector3f> m_circlePointsRender = std::make_unique<ArrayList<>>();
 ...
 float hitTestCircle(UI3DScene.Ray ray1, ArrayList<Vector3f> arrayList, Vector2 vector) {
 ...
 private static class SceneCharacter extends UI3DScene.SceneObject {
 ...
 Matrix4f getAttachmentTransform(const std::string& string, Matrix4f matrix4f0) {
 matrix4f0.identity();
 ...
 private static class SceneModel extends UI3DScene.SceneObject {
 ...
 Matrix4f getAttachmentTransform(const std::string& var1, Matrix4f matrix4f) {
 matrix4f.identity();
 return matrix4f;
 }
 }
 ...
 private static ObjectPool<UI3DScene.SceneObjectRenderData> s_pool = new ObjectPool<>(UI3DScene.SceneObjectRenderData::new);
 ...
 private static class SetModelCamera extends TextureDraw.GenericDrawer {
 ...
 ArrayList<UI3DScene.SceneObjectRenderData> m_objectData = std::make_unique<ArrayList<>>();
 ...
 UI3DScene.TranslateGizmoRenderData m_translateGizmoRenderData = new UI3DScene.TranslateGizmoRenderData();
 ArrayList<UI3DScene.PositionRotation> m_axes = std::make_unique<ArrayList<>>();
 ...
 private static class TranslateGizmoRenderData {
 bool m_hideX;
 bool m_hideY;
 bool m_hideZ;
 }
 ...
 void init(UI3DScene.SceneVehicle sceneVehicle, UI3DScene.VehicleRenderData vehicleRenderData) {
 ...
 shader1.setLight(int1, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F, Float.NaN, 0.0F, 0.0F, 0.0F, nullptr);
 }
 ...
 shader1.End();
 } else if (shader1 != nullptr && model.Mesh != nullptr && model.Mesh.isReady()) {
 GL11.glDepthFunc(513);
 GL11.glDepthMask(true);
 GL11.glDepthRange(0.0, 1.0);
 GL11.glEnable(2929);
 ...
 shader1.setLight(int2, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F, Float.NaN, 0.0F, 0.0F, 0.0F, nullptr);
 }
 ...
 private static ObjectPool<UI3DScene.VehicleRenderData> s_pool = new ObjectPool<>(UI3DScene.VehicleRenderData::new);
 ...
 if (sceneVehicle.m_model.Mesh != nullptr && sceneVehicle.m_model.Mesh.isReady() && sceneVehicle.m_model.Mesh.m_transform != nullptr) {
 ...
 void initWheelModel(UI3DScene.SceneVehicle sceneVehicle, VehicleScript.Part part, Matrix4f matrix4f2) {
 VehicleScript vehicleScript = sceneVehicle.m_script;
 float float0 = vehicleScript.getModelScale();
 ...
 UI3DScene.releaseMatrix4f(matrix4f1);
 matrix4f2.mul(matrix4f0, matrix4f0);
 if (model1.Mesh != nullptr && model1.Mesh.isReady() && model1.Mesh.m_transform != nullptr) {
