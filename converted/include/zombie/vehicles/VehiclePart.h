...
public:
 ...
 void setLastUpdated(float hours) {
 ...
 void setGeneralCondition(InventoryItem _item, float baseQuality, float chanceToSpawnDamaged) {
 int int0 = 100;
 ...
 return chr = = nullptr ? this->getItemContainer().getCapacity() : this->getItemContainer().getEffectiveCapacity(chr);
 ...
 return this->scriptPart != nullptr && this->scriptPart.luaFunctions != nullptr ? this->scriptPart.luaFunctions.get(name) : nullptr;
 }
 ...
 return this->children != nullptr && index >= 0 && index < this->children.size() ? this->children.get(index) : nullptr;
 }
 ...
 std::string string = this->getItemType().get(Rand.Next(this->getItemType().size()));
 ...
 if (this->isContainer() && this->getItemContainer() == nullptr && float0 != this->getContainerCapacity()) {
