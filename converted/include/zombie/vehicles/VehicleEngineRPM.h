...
public:
 ...
 std::string m_name;
 public EngineRPMData[] m_rpmData = new EngineRPMData[8];
 ...
 if (int0 < 0 || int0 > 1) {
 throw RuntimeException(String.format("unknown vehicleEngineRPM VERSION \"%s\"", string1);
 }
 }
 }

 if (int0 == -1) {
 throw RuntimeException(String.format("unknown vehicleEngineRPM VERSION \"%s\"", block0.type);
 } else {
 int int1 = 0;

 for (ScriptParser.Block block1 : block0.children) {
 if (!"data" == block1.type) {
 throw RuntimeException(String.format("unknown block vehicleEngineRPM.%s", block1.type);
 }

 if (int1 == 8) {
 throw RuntimeException(String.format("too many vehicleEngineRPM.data blocks, max is %d", 8);
 }
 ...
 throw RuntimeException(String.format("unknown value vehicleEngineRPM.data.%s", value.string);
 }
 ...
 for (ScriptParser.Block block1 : block0.children) {
 if (!"xxx" == block1.type) {
 throw RuntimeException(String.format("unknown block vehicleEngineRPM.data.%s", block1.type);
 }
 }
 }
