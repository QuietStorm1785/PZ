...
public:
 ...
 int history;
 bool buffering;
 private static ArrayDeque<VehicleInterpolationData> pool = std::make_unique<ArrayDeque<>>();
 private static List<VehicleInterpolationData> outdated = std::make_unique<ArrayList<>>();
 ...
 VehicleInterpolationData vehicleInterpolationData = pool.empty() ? new VehicleInterpolationData() : pool.pop();
 ...
 void interpolationDataAdd(BaseVehicle vehicle, VehicleInterpolationData data) {
 VehicleInterpolationData vehicleInterpolationData = pool.empty() ? new VehicleInterpolationData() : pool.pop();
 ...
 void interpolationDataAdd(ByteBuffer bb, long time, float x, float y, float z, long currentTime) {
 VehicleInterpolationData vehicleInterpolationData = pool.empty() ? new VehicleInterpolationData() : pool.pop();
 ...
 } else if (vehicleInterpolationData1 != nullptr && Math.abs(vehicleInterpolationData0.time - vehicleInterpolationData1.time) >= 10L) {
