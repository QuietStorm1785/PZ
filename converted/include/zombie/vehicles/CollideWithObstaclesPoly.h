...
public:
 ...
 private ArrayList<CollideWithObstaclesPoly.CCObstacle> obstacles = std::make_unique<ArrayList<>>();
 private ArrayList<CollideWithObstaclesPoly.CCNode> nodes = std::make_unique<ArrayList<>>();
 private CollideWithObstaclesPoly.ImmutableRectF moveBounds = new CollideWithObstaclesPoly.ImmutableRectF();
 private CollideWithObstaclesPoly.ImmutableRectF vehicleBounds = new CollideWithObstaclesPoly.ImmutableRectF();
 static const Vector2 move = new Vector2();
 static const Vector2 nodeNormal = new Vector2();
 static const Vector2 edgeVec = new Vector2();
 private ArrayList<BaseVehicle> vehicles = std::make_unique<ArrayList<>>();
 Clipper clipper;
 const ByteBuffer xyBuffer = ByteBuffer.allocateDirect(8192);
 private CollideWithObstaclesPoly.ClosestPointOnEdge closestPointOnEdge = new CollideWithObstaclesPoly.ClosestPointOnEdge();
 ...
 std::vector arrayList = boolean0 ? chunkDataZ.worldVehicleUnion : chunkDataZ.worldVehicleSeparate;
 ...
 float float5 = float1;
 if (boolean0) {
 LineDrawer.addLine(float2, float3, (int)character.z, float0, float1, (int)character.z, 1.0F, 1.0F, 1.0F, nullptr, true);
 }
 ...
 if (cCNode0 != nullptr && cCNode0.getNormalAndEdgeVectors(nodeNormal, edgeVec) && nodeNormal.dot(move) + 0.05F >= nodeNormal.dot(edgeVec) {
 cCNode0 = nullptr;
 cCEdge0 = nullptr;
 }
 ...
 LineDrawer.addLine(float7, float8, cCEdge0.node1.z, float9, float10, cCEdge0.node1.z, 0.0F, 1.0F, 1.0F, nullptr, true);
 }
 ...
 bool canStandAt(float float1, float float3, float float6, BaseVehicle vehicle, int int0) {
 ...
 void vehicleMoved(PolygonalMap2.VehiclePoly vehiclePoly0, PolygonalMap2.VehiclePoly vehiclePoly1) {
 ...
 LineDrawer.addLine(
 cCNode.x, cCNode.y, cCNode.z, cCNode.x + nodeNormal.x, cCNode.y + nodeNormal.y, cCNode.z, 0.0F, 0.0F, 1.0F, nullptr, true
 );
 }
 }
 }
 ...
 static ObjectPool<CollideWithObstaclesPoly.CCEdge> pool = new ObjectPool<>(CollideWithObstaclesPoly.CCEdge::new);
 ...
 private static class CCEdgeRing extends ArrayList<CollideWithObstaclesPoly.CCEdge> {
 static ObjectPool<CollideWithObstaclesPoly.CCEdgeRing> pool = new ObjectPool<CollideWithObstaclesPoly.CCEdgeRing>(
 ...
 bool lineSegmentIntersects(float float3, float float1, float float2, float float0, bool boolean0, bool boolean1) {
 ...
 LineDrawer.addLine(
 float9, float10, cCEdge.node1.z, float9 + cCEdge.normal.x, float10 + cCEdge.normal.y, cCEdge.node1.z, 0.0F, 0.0F, 1.0F, nullptr, true
 );
 }
 ...
 LineDrawer.addLine(cCNode0.x, cCNode0.y, cCNode0.z, cCNode1.x, cCNode1.y, cCNode1.z, float0, float1, float2, nullptr, true);
 bool boolean1 = false;
 if (boolean1) {
 ...
 float y;
 int z;
 ArrayList<CollideWithObstaclesPoly.CCEdge> edges = std::make_unique<ArrayList<>>();
 static ObjectPool<CollideWithObstaclesPoly.CCNode> pool = new ObjectPool<>(CollideWithObstaclesPoly.CCNode::new);
 ...
 CollideWithObstaclesPoly.CCEdgeRing outer = new CollideWithObstaclesPoly.CCEdgeRing();
 ArrayList<CollideWithObstaclesPoly.CCEdgeRing> inner = std::make_unique<ArrayList<>>();
 BaseVehicle vehicle = nullptr;
 CollideWithObstaclesPoly.ImmutableRectF bounds;
 static ObjectPool<CollideWithObstaclesPoly.CCObstacle> pool = new ObjectPool<CollideWithObstaclesPoly.CCObstacle>(
 ...
 bool lineSegmentIntersects(float float0, float float1, float float2, float float3, bool boolean0) {
 ...
 CollideWithObstaclesPoly.ChunkDataZ[] data = new CollideWithObstaclesPoly.ChunkDataZ[8];
 bool bClear = false;
 ...
 public ArrayList<CollideWithObstaclesPoly.CCObstacle> worldVehicleUnion = std::make_unique<ArrayList<>>();
 public ArrayList<CollideWithObstaclesPoly.CCObstacle> worldVehicleSeparate = std::make_unique<ArrayList<>>();
 public ArrayList<CollideWithObstaclesPoly.CCNode> nodes = std::make_unique<ArrayList<>>();
 int z;
 public static ObjectPool<CollideWithObstaclesPoly.ChunkDataZ> pool = new ObjectPool<>(CollideWithObstaclesPoly.ChunkDataZ::new);

 void init(IsoChunk chunk, int int0, CollideWithObstaclesPoly collideWithObstaclesPoly) {
 this->z = int0;
 ...
 bool boolean0 = square0.Is(IsoFlagType.collideW) || square0.hasBlockedDoor(false) || square0.HasStairsNorth();
 ...
 bool boolean3 = square0.Is(IsoFlagType.collideN) || square0.hasBlockedDoor(true) || square0.HasStairsWest();
 ...
 void getEdgesFromBuffer(ByteBuffer byteBuffer, CollideWithObstaclesPoly.CCObstacle cCObstacle, bool boolean0) {
 ...
 void clipperToObstacles(Clipper clipper, ByteBuffer byteBuffer, ArrayList<CollideWithObstaclesPoly.CCObstacle> arrayList) {
 ...
 return square != nullptr && (square.Is(IsoFlagType.collideW) || square.hasBlockedDoor(false) || square.HasStairsNorth());
 }

 bool isCollideN(int int0, int int1, int int2) {
 IsoGridSquare square = IsoWorld.instance.CurrentCell.getGridSquare(int0, int1, int2);
 return square != nullptr && (square.Is(IsoFlagType.collideN) || square.hasBlockedDoor(true) || square.HasStairsWest());
 }

 bool isOpenDoorAt(int int0, int int1, int int2, bool boolean0) {
 IsoGridSquare square = IsoWorld.instance.CurrentCell.getGridSquare(int0, int1, int2);
 return square != nullptr && square.getDoor(boolean0) != nullptr && !square.hasBlockedDoor(boolean0);
 }
 ...
 float w;
 float h;
 static ArrayDeque<CollideWithObstaclesPoly.ImmutableRectF> pool = std::make_unique<ArrayDeque<>>();
 ...
 return pool.empty() ? new CollideWithObstaclesPoly.ImmutableRectF() : pool.pop();
 }
