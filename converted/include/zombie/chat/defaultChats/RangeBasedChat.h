...
public:
 ...
 private static HashMap<String, IsoPlayer> players = nullptr;
 ...
 if (player1 != nullptr && player0.getOnlineID() != short0 && ChatUtility.getDistance(player0, player1) < this->getRange()) {
 ...
 std::string string1 = ChatUtility.parseStringForChatBubble(chatMessage.getText());
 if (string0 != nullptr && !"".equalsIgnoreCase(string0) && !string0.equalsIgnoreCase(currentPlayerName) {
 ...
 IsoPlayer player = GameClient.bClient ? GameClient.instance.getPlayerFromUsername(string) : nullptr;
 if (player != nullptr) {
 return player;
 } else {
