...
public:
 ...
 protected ArrayList<Short> members;
 private ArrayList<Short> justAddedMembers = std::make_unique<ArrayList<>>();
 private ArrayList<Short> justRemovedMembers = std::make_unique<ArrayList<>>();
 ...
 */
 ...
 ChatMessage createMessage(const std::string& string1, const std::string& string0) {
 ...
 for (auto& string : players) object = ChatUtility.findPlayer(string);
 if (object != nullptr) {
 ...
 for (auto& short0 : arrayList0) this->addMember(short0);
 }
 ...
 for (auto& short2 : arrayList1) this->leaveMember(short2);
 }
 }
 }
 ...
 void sendChatMessageToPlayer(UdpConnection udpConnection, ChatMessage chatMessage) {
 ...
 void sendChatMessageFromPlayer(UdpConnection udpConnection, ChatMessage chatMessage) {
 ByteBufferWriter byteBufferWriter = udpConnection.startPacket();
