...
public:
 ...
 private static HashMap<String, String> allowedChatIcons = std::make_unique<HashMap<>>();
 private static HashMap<String, String> allowedChatIconsFull = std::make_unique<HashMap<>>();
 ...
 static float getScrambleValue(IsoObject object, IsoPlayer player, float float0) {
 return getScrambleValue();
 }

 static float getScrambleValue(float float5, float float6, float float1, IsoGridSquare square, IsoPlayer player, float float3) {
 float float0 = 1.0F;
 bool boolean0 = false;
 bool boolean1 = false;
 if (square != nullptr && player.getSquare() != nullptr) {
 if (player.getBuilding() != nullptr && square.getBuilding() != nullptr && player.getBuilding() == square.getBuilding()) {
 ...
 } else if (player.getBuilding() != nullptr || square.getBuilding() != nullptr) {
 ...
 static bool playerWithinBounds(IsoObject object1, IsoObject object0, float float0) {
 ...
 return player = = nullptr ? -1.0F : (float)Math.sqrt(Math.pow(object.getX() - player.x, 2.0) + Math.pow(object.getY() - player.y, 2.0);
 }

 static float getDistance(float float1, float float0, IsoPlayer player) {
 return player = = nullptr ? -1.0F : (float)Math.sqrt(Math.pow(float1 - player.x, 2.0) + Math.pow(float0 - player.y, 2.0);
 }
 ...
 if (udpConnection1.players[int1] != nullptr && udpConnection1.players[int1].username.equalsIgnoreCase(string) {
 udpConnection0 = udpConnection1;
 break;
 }
 }
 ...
 static std::string findPlayerName(int int0) {
 return findPlayer();
 }
 ...
 for (auto& string1 : strings) switch (string1) {
 case "s":
 ...
 static bool chatStreamEnabled(ChatType chatType) {
 ...
 std::string string3 = string1.equalsIgnoreCase("music") ? "Icon_music_notes" : allowedChatIconsFull.get(string1.toLowerCase());
