...
public:
 ...
 public BufferedRandomAccessFile(const std::string& filename, const std::string& mode, int bufsize) {
 super(filename, mode);
 this->invalidate();
 ...
 public BufferedRandomAccessFile(File file, const std::string& mode, int bufsize) {
 super(file, mode);
 this->invalidate();
 this->BUF_SIZE = bufsize;
 this->buffer = new byte[this->BUF_SIZE];
 ...
 return int2 = = -1 && stringBuilder.length() == 0 ? nullptr : stringBuilder.toString();
 } else {
