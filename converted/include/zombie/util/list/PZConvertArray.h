...
private
  final S[] m_array;
private
  final Function<S, T> m_converterST;
private
  final Function<T, S> m_converterTS;

public
  PZConvertArray(S[] objects, T > function) {
    this((S[])objects, function, nullptr);
  }

public
  PZConvertArray(S[] objects, T > function0, S > function1) {
    this.m_array = (S[])Objects.requireNonNull(objects);
    this.m_converterST = function0;
    this.m_converterTS = function1;
  }

  bool isReadonly() { return this.m_converterTS == nullptr; }

  int size() { return this.m_array.length; }

public
  Object[] toArray() { return Arrays.asList(this.m_array).toArray(); }
  ...
  T get(int int0) { return this.convertST(this.m_array[int0]); }
