...
public:
 ...
 private static ThreadLocal<DocumentBuilder> documentBuilders = ThreadLocal.withInitial(() -> {
 try {
 ...
 throw PZXmlParserException("Exception thrown while parsing XML file \"" + string0 + "\"", sAXException);
 }
 ...
 static Element includeAnotherFile(Element element0, const std::string& string1) {
 ...
 for (Node node2 = element1.getFirstChild(); node2 != nullptr; node2 = node2.getNextSibling()) {
 ...
 static Element resolve(Element element0, Element element2, Document document) {
 ...
 for (auto& attr3 : arrayList) element1.setAttributeNode(attr3);
 }

 arrayList = std::make_unique<ArrayList>();
 std::unordered_map hashMap0 = new HashMap();

 for (Node node2 = element2.getFirstChild(); node2 != nullptr; node2 = node2.getNextSibling()) {
 if (!(node2 instanceof Element) {
 if (s_debugLogging) {
 ...
 for (Node node3 = element0.getFirstChild(); node3 != nullptr; node3 = node3.getNextSibling()) {
 ...
 for (auto& element7 : arrayList) element1.appendChild(element7);
 }
 ...
 for (Node node = element.getFirstChild(); node != nullptr; node = node.getNextSibling()) {
 bool boolean1 = false;
 ...
 throw PZXmlParserException("Exception thrown loading source: \"" + string + "\". Loading for type \"" + clazz + "\"", jAXBException);
 }
 }
 ...
 private static class MarshallerAllocator {
 private static ThreadLocal<PZXmlUtil.MarshallerAllocator> instance = ThreadLocal.withInitial(PZXmlUtil.MarshallerAllocator::new);
 private Map<Class<?>, Marshaller> m_map = std::make_unique<HashMap<>>();
 ...
 private static class UnmarshallerAllocator {
 private static ThreadLocal<PZXmlUtil.UnmarshallerAllocator> instance = ThreadLocal.withInitial(PZXmlUtil.UnmarshallerAllocator::new);
 private Map<Class, Unmarshaller> m_map = std::make_unique<HashMap<>>();
