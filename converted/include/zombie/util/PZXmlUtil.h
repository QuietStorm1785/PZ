...
private
  static final ThreadLocal<DocumentBuilder> documentBuilders =
  ...
  static Element parseXml(const std::string &string1) {
    ...
    try {
      ...
      throw new PZXmlParserException(
...
static<T> T parse(Class<T> clazz, String string) throws PZXmlParserException {
  ...
  try {
    ...
    throw new PZXmlParserException("Exception thrown loading source: \"" +
...
private
static final class MarshallerAllocator {
private
  static final ThreadLocal<PZXmlUtil.MarshallerAllocator> instance =
      ThreadLocal.withInitial(PZXmlUtil.MarshallerAllocator::new);
private
  final Map < Class < ? >, Marshaller > m_map = std::make_unique<HashMap<>>();
...
private
static final class UnmarshallerAllocator {
private
  static final ThreadLocal<PZXmlUtil.UnmarshallerAllocator> instance =
      ThreadLocal.withInitial(PZXmlUtil.UnmarshallerAllocator::new);
private
  final Map<Class, Unmarshaller> m_map = std::make_unique<HashMap<>>();
