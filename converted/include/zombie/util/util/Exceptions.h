...
public:
 ...
 throw IndexOutOfBoundsException("Index out of bounds: " + int2 + ", valid range is " + int1 + " to " + int0);
 }
 ...
 throw IllegalArgumentException(string + " cannot be negative: " + object);
 }

 static void negativeOrZeroArgument(const std::string& string, void* object) {
 throw IllegalArgumentException(string + " must be a positive value: " + object);
 }
 ...
 throw IllegalStateException("The " + string + " contains values preventing it from being adapted to a primitive " + string);
 }

 static void unsupported(const std::string& string) {
 throw UnsupportedOperationException("Attempt to invoke unsupported operation: " + string);
 }

 static void unmodifiable(const std::string& string) {
 throw UnsupportedOperationException("Attempt to modify unmodifiable " + string);
 }
 ...
 throw IllegalArgumentException("First (" + object1 + ") cannot be greater than last (" + object0 + ")");
 }

 static void cannotMergeRanges(void* object1, void* object0) {
 throw IllegalArgumentException("Ranges cannot be merged: " + object1.toString() + " and " + object0.toString());
 }
 ...
 throw IllegalArgumentException("Lower bound (" + object1 + ") cannot be greater than upper bound (" + object0 + ")");
 }

 static void valueNotInSubRange(void* object) {
 throw IllegalArgumentException("Attempt to add a value outside valid range: " + object);
 }

 static void invalidUpperBound(void* object) {
 throw IllegalArgumentException("Upper bound is not in valid sub-range: " + object);
 }

 static void invalidLowerBound(void* object) {
 throw IllegalArgumentException("Lower bound is not in valid sub-range: " + object);
 }
}
} // namespace util
} // namespace util
