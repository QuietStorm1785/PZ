...
class MapGroups {
public:
 private ArrayList<MapGroups.MapGroup> groups = std::make_unique<ArrayList<>>();
 private ArrayList<MapGroups.MapDirectory> realDirectories = std::make_unique<ArrayList<>>();
 ...
 for (auto& path : directoryStream) arrayList.add(string + "/" + path.getFileName().toString());
 }
 ...
 static std::string addMissingVanillaDirectories(const std::string& mapName) {
 std::vector arrayList0 = getVanillaMapDirectories(false);
 bool boolean0 = false;
 String[] strings = mapName.split(";");

 for (auto& string0 : strings) string0 = string0.trim();
 ...
 for (auto& string2 : arrayList0) if (!arrayList1.contains(string2) {
 ...
 for (auto& string4 : arrayList1) if (!string3.empty()) {
 string3 = string3 + ";";
 }
 ...
 void createGroups(ActiveMods activeMods, bool includeVanilla, bool includeChallenges) {
 this->groups.clear();
 ...
 for (auto& string3 : arrayList0) this->handleMapDirectory(string3, string2 + File.separator + string3);
 }
 }
 ...
 void getDirsRecursively(MapGroups.MapDirectory mapDirectory0, ArrayList<MapGroups.MapDirectory> arrayList) {
 ...
 void handleMapDirectory(const std::string& string1, const std::string& string0) {
 ...
 public MapDirectory(const std::string& string0, const std::string& string1) {
 this->name = string0;
 this->path = string1;
 }

 public MapDirectory(const std::string& string0, const std::string& string1, ArrayList<String> arrayList) {
 this->name = string0;
 this->path = string1;
 ...
 void addDirectory(const std::string& string0, const std::string& string1, ArrayList<String> arrayList) {
 assert !this->hasDirectory(string0);
 ...
 for (auto& string : list) if (this->hasDirectory(string) {
 ...
 for (auto& string0 : arrayList0) if (!hashMap.containsKey(string0) {
 ...
 if (int0 != int1) {
 std::string string2 = Translator.getText("UI_MapConflict", mapDirectory1.name, arrayList1.get(int1), string1);
