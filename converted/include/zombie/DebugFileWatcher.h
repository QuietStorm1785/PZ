...
public:
 private HashMap<Path, String> m_watchedFiles = std::make_unique<HashMap<>>();
 private HashMap<WatchKey, Path> m_watchkeyMapping = std::make_unique<HashMap<>>();
 private ArrayList<PredicatedFileWatcher> m_predicateWatchers = std::make_unique<ArrayList<>>();
 private ArrayList<PredicatedFileWatcher> m_predicateWatchersInvoking = std::make_unique<ArrayList<>>();
 ...
 private ArrayList<String> m_modifiedFiles = std::make_unique<ArrayList<>>();
 ...
 for (WatchKey watchKey = this->m_watcher.poll(); watchKey != nullptr; watchKey = this->m_watcher.poll()) {
 try {
 ...
 std::string string0 = this->m_watchedFiles.getOrDefault(path2, path2.toString());
 ...
 std::string string1 = this->m_watchedFiles.getOrDefault(path4, path4.toString());
 this->m_modificationTime = System.currentTimeMillis();
