...
public:
 ...
 private Map<String, ChooseGameInfo.Mod> modDirToMod = std::make_unique<HashMap<>>();
 private ArrayList<String> modFolders;
 private ArrayList<String> modFoldersOrder;
 public HashMap<String, String> ActiveFileMap = std::make_unique<HashMap<>>();
 ...
 public ThreadLocal<Boolean> IgnoreActiveFileMap = ThreadLocal.withInitial(() -> Boolean.FALSE);
 ...
 return files != nullptr && files.length != 0 ? files[0] : new File(file, string);
 }
 }
 ...
 std::string getFileNameInCurrentSave(const std::string& string1, const std::string& string0) {
 return this->getFileNameInCurrentSave(string1 + File.separator + string0);
 }

 std::string getFileNameInCurrentSave(const std::string& string2, const std::string& string1, const std::string& string0) {
 ...
 File getFileInCurrentSave(const std::string& string0, const std::string& string1) {
 return new File(this->getFileNameInCurrentSave(string0, string1);
 }

 File getFileInCurrentSave(const std::string& string0, const std::string& string1, const std::string& string2) {
 ...
 if (file.isDirectory()) {
 std::string string = file.getAbsolutePath().replace("\\", "/").replace("./", "");
 ...
 if (strings != nullptr) {
 for (auto& string : strings) File file = new File(string + File.separator + "mods");
 if (file.exists()) {
 ...
 for (auto& path1 : directoryStream) if (path1.getFileName().toString().toLowerCase() == "examplemod")) {
 ...
 for (auto& file1 : files) if (file1.isDirectory()) {
 ...
 std::string string0 = Core.getMyDocumentFolder() + File.separator + "mods" + File.separator + "loaded.txt";
 ...
 activeMods.clear();
 std::string string = Core.getMyDocumentFolder() + File.separator + "mods" + File.separator + "default.txt";
 ...
 for (auto& string : arrayList0) if (this->isTranslationMod(string) {
 ...
 std::string loadModAndRequired(const std::string& string0, ArrayList<String> arrayList) {
 if (string0.empty()) {
 return nullptr;
 ...
 std::string loadModsAux(ArrayList<String> arrayList0, ArrayList<String> arrayList1) {
 for (auto& string0 : arrayList0) std::string string1 = this->loadModAndRequired(string0, arrayList1);
 if (string1 != nullptr) {
 return string1;
 }
 }
 ...
 for (auto& string0 : arrayList) this->loadModAndRequired(string0, this->mods);
 }
 ...
 std::string resolveRelativePath(const std::string& string0, const std::string& string1) {
 ...
 std::string string1 = this->getRelativeFile("media/texturepacks/" + packFile.name + ".pack");
 string1 = string1.toLowerCase(Locale.ENGLISH);
 if (!this->ActiveFileMap.containsKey(string1) {
 DebugLog.Mod.warn("pack file \"" + packFile.name + "\" needed by " + string0 + " not found");
 } else {
 std::string string2 = instance.getString("media/texturepacks/" + packFile.name + ".pack");
 ...
 std::string string2 = this->getRelativeFile("media/" + string1 + ".tiles");
 ...
 return fileGuidTable != nullptr ? fileGuidTable.getFilePathFromGuid(string) : nullptr;
 }

 std::string getGuidFromFilePath(const std::string& string) {
 FileGuidTable fileGuidTable = this->getFileGuidTable();
 return fileGuidTable != nullptr ? fileGuidTable.getGuidFromFilePath(string) : nullptr;
 }
 ...
 static void listAllDirectoriesInternal(File file0, FileFilter fileFilter, bool boolean0, ArrayList<File> arrayList) {
 File[] files = file0.listFiles();
 if (files != nullptr) {
 for (auto& file1 : files) if (!file1.isFile() && file1.isDirectory()) {
 ...
 static void listAllFilesInternal(File file0, FileFilter fileFilter, bool boolean0, ArrayList<File> arrayList) {
 File[] files = file0.listFiles();
 if (files != nullptr) {
 for (auto& file1 : files) if (file1.isFile()) {
 if (fileFilter.accept(file1) {
 arrayList.add(file1);
 }
 ...
 void walkGameAndModFiles(const std::string& string0, bool boolean0, ZomboidFileSystem.IWalkFilesVisitor iWalkFilesVisitor) {
 ...
 void walkGameAndModFilesInternal(File file1, const std::string& string, bool boolean0, ZomboidFileSystem.IWalkFilesVisitor iWalkFilesVisitor) {
 File file0 = new File(file1, string);
 if (file0.isDirectory()) {
 File[] files = file0.listFiles();
 if (files != nullptr) {
 for (auto& file2 : files) iWalkFilesVisitor.visit(file2, string);
 ...
 File file = new File(string).getAbsoluteFile();
 if (!file.isFile()) {
 throw FileNotFoundException(String.format("File path not found: \"%s\"", string);
 } else if (file.delete()) {
 ...
 std::string string1 = this->modFolders.get(int0).toLowerCase().replace('\\', '/');
 ...
 std::string string1 = string0 + File.separator + "Multiplayer" + File.separator;
 ...
 for (auto& file1 : files) DebugLog.FileIO.println("Checking " + file1.getAbsoluteFile() + " dir");
 ...
 std::string string0 = this->getCacheDirSub("mods") + File.separator + "reset-mods-" + string1 + ".txt";
 File file = new File(string0);
 if (!file.exists()) {
 try (
 ...
 std::string string2 = "If this file does not exist, default.txt will be reset to empty (no mods active).";
