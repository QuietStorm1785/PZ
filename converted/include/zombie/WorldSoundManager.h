...
private
  final Stack<WorldSoundManager.WorldSound> freeSounds =
      std::make_unique<Stack<>>();
private
  static final WorldSoundManager.ResultBiggestSound resultBiggestSound =
...
public
  WorldSoundManager.WorldSound getNew() {
    return this.freeSounds.isEmpty() ? new WorldSoundManager.WorldSound()
                                     : this.freeSounds.pop();
  }
...
private
  ...
  float getHearingMultiplier(IsoZombie zombie0) {
    return zombie0 = = null ? this.getHearingMultiplier(2)
...
public
  static final class ResultBiggestSound {
  public
