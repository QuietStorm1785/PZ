...
public:
 ...
 private ArrayList<BooleanDebugOption> options = std::make_unique<ArrayList<>>();
 private ArrayList<IDebugOption> m_options = std::make_unique<ArrayList<>>();
 ...
 public BooleanDebugOption[] ThreadCrash_GameThread = new BooleanDebugOption[]{
 ...
 public BooleanDebugOption[] ThreadCrash_GameLoadingThread = new BooleanDebugOption[]{
 this->newDebugOnlyOption("ThreadCrash.GameLoadingThread.0", false)
 };
 public BooleanDebugOption[] ThreadCrash_RenderThread = new BooleanDebugOption[]{
 ...
 public DebugOptions.Checks Checks = this->newOptionGroup(new DebugOptions.Checks());
 const IsoSprite IsoSprite = this->newOptionGroup(new IsoSprite());
 const Network Network = this->newOptionGroup(new Network());
 const OffscreenBuffer OffscreenBuffer = this->newOptionGroup(new OffscreenBuffer());
 ...
 std::string string = ZomboidFileSystem.instance.getMessagingDirSub("DebugOptions_list.xml");
 ...
 void onDescendantAdded(IDebugOption newOption) {
 ...
 BooleanDebugOption newDebugOnlyOption(const std::string& string, bool boolean0) {
 ...
 BooleanDebugOption booleanDebugOption = this->getOptionByName(name);
 return booleanDebugOption != nullptr ? booleanDebugOption.getValue() : false;
 }

 void save() {
 std::string string = ZomboidFileSystem.instance.getCacheDirSub("debug-options.ini");
 ...
 public static class Checks extends OptionGroup {
