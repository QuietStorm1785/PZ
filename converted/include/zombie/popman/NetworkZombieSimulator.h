...
public:
 ...
 private ArrayDeque<IsoZombie> SendQueue = std::make_unique<ArrayDeque<>>();
 private ArrayDeque<IsoZombie> ExtraSendQueue = std::make_unique<ArrayDeque<>>();
 ...
 this->authoriseZombiesLast.removeIf(short0 -> GameClient.getZombie(short0) == nullptr);
 ...
 zombie0.getStateMachine().changeState(ZombieOnGroundState.instance(), nullptr);
 ...
 if (player != nullptr && player.ReanimatedCorpseID == zombiePacketx.id && zombiePacketx.id != -1) {
