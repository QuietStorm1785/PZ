...
public:
 ...
 private static ConcurrentHashMap<Long, MPDebugInfo.MPSoundDebugInfo> debugSounds = std::make_unique<ConcurrentHashMap<>>();
 private ArrayList<MPDebugInfo.MPCell> loadedCells = std::make_unique<ArrayList<>>();
 private ObjectPool<MPDebugInfo.MPCell> cellPool = new ObjectPool<>(MPDebugInfo.MPCell::new);
 const LoadedAreas loadedAreas = new LoadedAreas(false);
 private ArrayList<MPDebugInfo.MPRepopEvent> repopEvents = std::make_unique<ArrayList<>>();
 private ObjectPool<MPDebugInfo.MPRepopEvent> repopEventPool = new ObjectPool<>(MPDebugInfo.MPRepopEvent::new);
 short repopEpoch = 0;
 long requestTime = 0L;
 ...
 private static boolean n_hasData(boolean var0);

 private static void n_requestData();

 private static int n_getLoadedCellsCount();

 private static int n_getLoadedCellsData(int var0, ByteBuffer var1);

 private static int n_getLoadedAreasCount();

 private static int n_getLoadedAreasData(int var0, ByteBuffer var1);

 private static int n_getRepopEventCount();

 private static int n_getRepopEventData(int var0, ByteBuffer var1);
 ...
 while (!this->repopEvents.empty() && this->repopEvents.get(0).worldAge + this->REPOP_DISPLAY_HOURS < float0) {
