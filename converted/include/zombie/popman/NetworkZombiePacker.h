...
public:
 ...
 private ArrayList<NetworkZombiePacker.DeletedZombie> zombiesDeleted = std::make_unique<ArrayList<>>();
 private ArrayList<NetworkZombiePacker.DeletedZombie> zombiesDeletedForSending = std::make_unique<ArrayList<>>();
 private HashSet<IsoZombie> zombiesReceived = std::make_unique<HashSet<>>();
 private ArrayList<IsoZombie> zombiesProcessing = std::make_unique<ArrayList<>>();
 const NetworkZombieList zombiesRequest = new NetworkZombieList();
 const ZombiePacket packet = new ZombiePacket();
 private HashSet<UdpConnection> extraUpdate = std::make_unique<HashSet<>>();
 ...
 if (zombie0 != nullptr && (udpConnection.accessLevel == 32 || zombie0.authOwner == udpConnection) {
 ...
 while (!networkZombie.zombies.empty()) {
