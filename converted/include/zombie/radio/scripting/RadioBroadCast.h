...
public:
 ...
 private ArrayList<RadioLine> lines = std::make_unique<ArrayList<>>();
 std::string ID = "";
 int startStamp = 0;
 int endStamp = 0;
 int lineCount = 0;
 ...
 public RadioBroadCast(const std::string& id, int startstamp, int endstamp) {
 this->ID = id;
 ...
 void setPreSegment(RadioBroadCast broadCast) {
 ...
 if (doChildren && !this->hasDonePreSegment && this->lineCount == 0 && this->preSegment != nullptr) {
 ...
 if (this->lineCount >= 0 && this->lineCount < this->lines.size()) {
 return this->lines.get(this->lineCount) != nullptr && this->lines.get(this->lineCount).getText() != nullptr
