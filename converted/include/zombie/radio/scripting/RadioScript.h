...
class RadioScript {
public:
 private ArrayList<RadioBroadCast> broadcasts = std::make_unique<ArrayList<>>();
 private ArrayList<RadioScript.ExitOption> exitOptions = std::make_unique<ArrayList<>>();
 std::string GUID;
 ...
 public RadioScript(const std::string& n, int loopmin, int loopmax) {
 this(n, loopmin, loopmax, UUID.randomUUID().toString());
 }

 public RadioScript(const std::string& n, int loopmin, int loopmax, const std::string& guid) {
 this->name = n;
 ...
 DebugLog.log(DebugType.Radio, "Error cannot add broadcast ID: '" + string + "' to script '" + this->name + "', nullptr or timestamp error");
 }
 }

 void AddExitOption(const std::string& scriptname, int chance, int startdelay) {
 int int0 = chance;
 ...
 public ExitOption(const std::string& name, int rollchance, int startdelay) {
