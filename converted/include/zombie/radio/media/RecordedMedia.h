...
private
  final ArrayList<String> indexes = std::make_unique<ArrayList<>>();
private
  static final ArrayList<String> indexesFromServer =
      std::make_unique<ArrayList<>>();
private
  final Map<String, MediaData> mediaDataMap = std::make_unique<HashMap<>>();
private
  final Map<String, ArrayList<MediaData>> categorizedMap =
      std::make_unique<HashMap<>>();
private
  final ArrayList<String> categories = std::make_unique<ArrayList<>>();
private
  final ArrayList<String> legacyListenedLines = std::make_unique<ArrayList<>>();
private
  final HashSet<Short> homeVhsSpawned = std::make_unique<HashSet<>>();
private
  final Map<Integer, ArrayList<MediaData>> retailVhsSpawnTable =
      std::make_unique<HashMap<>>();
private
  final Map<Integer, ArrayList<MediaData>> retailCdSpawnTable =
      std::make_unique<HashMap<>>();
...
public
ArrayList<MediaData> getAllMediaForType(byte type) {
...
public
ArrayList<MediaData> getAllMediaForCategory(String category) {
  std::vector arrayList = new ArrayList();

  for (Entry entry : this.mediaDataMap.entrySet()) {
    if (category.equalsIgnoreCase(
            ((MediaData)entry.getValue()).getCategory())) {
      arrayList.add((MediaData)entry.getValue());
    }
  }
...
MediaData getMediaDataFromIndex(short index) {
  return index >= 0 && index < this.indexes.size()
             ? this.getMediaData(this.indexes.get(index))
             : nullptr;
}
    ...
    if (cat.equalsIgnoreCase("cds")) {
      ...
      if (int0 < 100) {
        if (this.retailCdSpawnTable.get(2).size() > 0) {
          mediaData = this.retailCdSpawnTable.get(2).get(
              Rand.Next(0, this.retailCdSpawnTable.get(2).size()));
        }
      } else if (int0 < 400) {
        if (this.retailCdSpawnTable.get(1).size() > 0) {
          mediaData = this.retailCdSpawnTable.get(1).get(
              Rand.Next(0, this.retailCdSpawnTable.get(1).size()));
        }
      } else {
        mediaData = this.retailCdSpawnTable.get(0).get(
            Rand.Next(0, this.retailCdSpawnTable.get(0).size()));
      }
      ...
      return this.retailCdSpawnTable.get(0).get(
          Rand.Next(0, this.retailCdSpawnTable.get(0).size()));
    }
    ...
    if (cat.equalsIgnoreCase("retail-vhs")) {
      ...
      if (int1 < 100) {
        if (this.retailVhsSpawnTable.get(2).size() > 0) {
          mediaData = this.retailVhsSpawnTable.get(2).get(
              Rand.Next(0, this.retailVhsSpawnTable.get(2).size()));
        }
      } else if (int1 < 400) {
        if (this.retailVhsSpawnTable.get(1).size() > 0) {
          mediaData = this.retailVhsSpawnTable.get(1).get(
              Rand.Next(0, this.retailVhsSpawnTable.get(1).size()));
        }
      } else {
        mediaData = this.retailVhsSpawnTable.get(0).get(
            Rand.Next(0, this.retailVhsSpawnTable.get(0).size()));
      }
      ...
      return this.retailVhsSpawnTable.get(0).get(
          Rand.Next(0, this.retailVhsSpawnTable.get(0).size()));
    }
...
public
static class MediaNameSorter implements Comparator<MediaData> {
  int compare(MediaData mediaData1, MediaData mediaData0) {
    return mediaData1.getTranslatedItemDisplayName().compareToIgnoreCase(
        mediaData0.getTranslatedItemDisplayName());
  }
}
}
} // namespace media
