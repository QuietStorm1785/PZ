...
public:
 ...
 private HashMap<Integer, Boolean> state = std::make_unique<HashMap<>>();
 private ArrayList<StorySound> storySounds = std::make_unique<ArrayList<>>();
 int nextTick = 0;
 ...
 if (string0.lastIndexOf(".") != -1 && string0.lastIndexOf(".") != 0 && string0.substring(string0.lastIndexOf(".") + 1) == "ogg")) {
 std::string string1 = string0.substring(0, string0.lastIndexOf("."));
 ...
 void renderDebug() {
 if (DEBUG) {
 std::string string = Emitter.coordinate3D ? "3D coordinates, X-Z-Y" : "2D coordinates X-Y-Z";
 ...
 void renderLine(UIFont uIFont, const std::string& string, int int1, int int0) {
