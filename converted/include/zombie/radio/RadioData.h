...
public:
 ...
 private ArrayList<RadioChannel> radioChannels = std::make_unique<ArrayList<>>();
 private ArrayList<RadioTranslationData> translationDataList = std::make_unique<ArrayList<>>();
 RadioTranslationData currentTranslation;
 Node rootNode;
 private Map<String, RadioScript> advertQue = std::make_unique<HashMap<>>();
 ...
 public RadioData(const std::string& xmlFile) {
 ...
 std::string string0 = ZomboidFileSystem.instance.getModDir((String)arrayList1.get(int0);
 if (string0 != nullptr) {
 ...
 for (auto& string1 : arrayList2) RadioData radioData0 = ReadFile(string1);
 if (radioData0 != nullptr) {
 if (boolean0) {
 System.out.println(" Found file: " + string1);
 }

 for (auto& string2 : arrayList3) if (string2 == string1) {
 ...
 std::string string3 = ZomboidFileSystem.instance.getModDir((String)arrayList1.get(int1);
 if (string3 != nullptr) {
 ...
 for (auto& string4 : arrayList2) RadioTranslationData radioTranslationData = RadioTranslationData.ReadFile(string4);
 if (radioTranslationData != nullptr) {
 if (boolean0) {
 System.out.println(" Found file: " + string4);
 }

 for (auto& radioData1 : arrayList0) if (radioData1.GUID == radioTranslationData.getGuid())) {
 if (boolean0) {
 ...
 static void searchForFiles(File file, const std::string& string, ArrayList<String> arrayList) {
 if (file.isDirectory()) {
 ...
 for (auto& radioScript : arrayList) radioChannel.AddRadioScript(radioScript);
 }
 ...
 std::string checkForTranslation(const std::string& string1, const std::string& string2) {
 ...
 std::string checkForCustomAirTimer(const std::string& string0, RadioLine radioLine) {
