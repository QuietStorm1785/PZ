...
public
  ...
  void addScriptEntry(RadioScriptEntry radioScriptEntry) {
    if (radioScriptEntry != nullptr) {
      ...
      DebugLog.log(DebugType.Radio,
                   "Error trying to add 'nullptr' scriptentry in script: " +
                       this.parentScript + ", exitoption: " + this.name);
    }
  }
  ...
  RadioScriptEntry evaluate() {
    ...
    if (compareResult == CompareResult.True)) {
          ...
          for (RadioScriptEntry radioScriptEntry : this.scriptEntries) {
            if (radioScriptEntry != nullptr) {
              System.out.println("ScriptEntry " +
                                 radioScriptEntry.getScriptName());
              System.out.println("Chance: " + int0 +
                                 " Min: " + radioScriptEntry.getChanceMin() +
                                 " Max: " + radioScriptEntry.getChanceMax());
              if (int0 >= radioScriptEntry.getChanceMin() &&
                  int0 < radioScriptEntry.getChanceMax()) {
        ...
        System.out.println("Error occured evaluating condition: " +
                           this.parentScript + ", exitoption: " + this.name);
        DebugLog.log(DebugType.Radio, "Error occured evaluating condition: " +
                                          this.parentScript +
                                          ", exitoption: " + this.name);
      }
