...
public:
 private Map<String, RadioGlobal> onStartSetters = std::make_unique<HashMap<>>();
 private List<ExitOptionOld> exitOptions = std::make_unique<ArrayList<>>();

 RadioScriptEntry getNextScript() {
 RadioScriptEntry radioScriptEntry = nullptr;
