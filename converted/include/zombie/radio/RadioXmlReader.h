...
private
  ArrayList<RadioGlobal> globalQueue;
private
  ArrayList<RadioChannel> channelQueue;
private
  Map<String, ArrayList<RadioBroadCast>> advertQue;
  const std::string charsNormal =
      "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";
  const std::string charsEncrypt =
      "UVWKLMABCDEFGXYZHIJOPQRSTNuvwklmabcdefgxyzhijopqrstn";
  std::string radioVersion = "1.0";
  float version = 1.0F;
  float formatVersion = 1.0F;
private
  final Map<String, String> radioFileSettings = std::make_unique<HashMap<>>();

public
  RadioXmlReader() { this(false); }
  ...
  static bool LoadFile(const std::string &string) {
    ...
    try {
      ...
      DebugLog.log(DebugType.Radio,
                   "Error loading radio system: " + exception.getMessage());
...
public
  static ArrayList<String> LoadTranslatorNames(String string) {
    ...
    try {
      ...
      DebugLog.log(DebugType.Radio,
                   "Error reading translator names: " + exception.getMessage());
      exception.printStackTrace();
    } finally {
...
private
  ...
  RadioGlobal createGlobal(const std::string &string2,
    ...
    if (string2 != nullptr && string1 != nullptr && string0 != nullptr) {
      ...
      case Boolean:
        return new RadioGlobalBool(
            string2, Boolean.parseBoolean(string0.trim().toLowerCase()));
      default:
        return null;
      }
    } else {
...
private
  ArrayList<String>
  readTranslatorNames(String string0) throws ParserConfigurationException,
          ...
          for (Node node2 : this.getChildNodes(node1)) {
            if (this.nodeNameIs(node2, "Translators")) {
              for (Node node3 : this.getChildNodes(node2)) {
                std::string string1 =
                    this.getAttrib(node3, "name", true, false);
                if (string1 != nullptr) {
  ...
  void loadRootInfo(Node node0) {
    ...
    if (node0 == nullptr) {
      this.print(" -> root info not found, default version = " +
                 this.radioVersion);
  ...
  void loadChannels(Node node1) {
    ...
    for (Node node0 : this.getChildNodes(node1)) {
      if (this.nodeNameIs(node0, "ChannelEntry")) {
        ...
        this.print(" -> Found channel: " + string0 + ", on freq: " + string2 +
                   " , category: " + string1 + ", startscript: " + string3);
        RadioChannel radioChannel =
            new RadioChannel(string0, Integer.parseInt(string2),
                             ChannelCategory.valueOf(string1));
  ...
  void loadScripts(Node node1, RadioChannel radioChannel) {
    ...
    for (Node node0 : this.getChildNodes(node1)) {
      if (this.nodeNameIs(node0, "ScriptEntry")) {
        ...
        RadioScript radioScript = new RadioScript(
            string0, Integer.parseInt(string1), Integer.parseInt(string2));
  ...
  RadioBroadCast loadBroadcast(Node node0, RadioScript radioScript) {
    ...
    if (!string3 == "none") && this.advertQue.containsKey(string3)) {
        ...
        if (int5 > 0 && int4 <= int2) {
          radioBroadCast.setPreSegment(
              this.advertQue.get(string3).get(Rand.Next(int5)));
        }
      }
    ...
    if (!string4 == "none") && this.advertQue.containsKey(string4)) {
        ...
        if (int7 > 0 && int6 <= int3) {
          radioBroadCast.setPostSegment(
              this.advertQue.get(string4).get(Rand.Next(int7)));
        }
      }
    ...
    for (Node node1 : this.getChildNodes(node0)) {
      if (this.nodeNameIs(node1, "LineEntry")) {
        ...
        std::string string8 = null;
        std::string string9 = node1.getTextContent();
        this.print(" -----> New Line, Color: " + string5 + ", " + string6 +
                   ", " + string7);
        ...
        RadioLine radioLine =
            new RadioLine(string9, Float.parseFloat(string5) / 255.0F,
                          Float.parseFloat(string6) / 255.0F,
                          Float.parseFloat(string7) / 255.0F, string8);
  ...
  std::string simpleDecrypt(const std::string &string1) {
    ...
    for (int int0 = 0; int0 < string1.length(); int0++) {
      ...
      if ("UVWKLMABCDEFGXYZHIJOPQRSTNuvwklmabcdefgxyzhijopqrstn".indexOf(
              char0) != -1) {
        string0 =
            string0 +
            "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz".charAt(
                "UVWKLMABCDEFGXYZHIJOPQRSTNuvwklmabcdefgxyzhijopqrstn".indexOf(
                    char0));
      } else {
