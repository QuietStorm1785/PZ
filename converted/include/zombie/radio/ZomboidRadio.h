...
public:
 ...
 private ArrayList<WaveSignalDevice> devices = std::make_unique<ArrayList<>>();
 private ArrayList<WaveSignalDevice> broadcastDevices = std::make_unique<ArrayList<>>();
 RadioScriptManager scriptManager;
 int DaysSinceStart = 0;
 int lastRecordedHour;
 private String[] playerLastLine = new String[4];
 private Map<Integer, String> channelNames = std::make_unique<HashMap<>>();
 private Map<String, Map<Integer, String>> categorizedChannels = std::make_unique<HashMap<>>();
 private List<Integer> knownFrequencies = std::make_unique<ArrayList<>>();
 RadioDebugConsole debugConsole;
 bool hasRecievedServerData = false;
 SLSoundManager storySoundManager = nullptr;
 private static String[] staticSounds = new String[]{"<bzzt>", "<fzzt>", "<wzzt>", "<szzt>"};
 ...
 private HashMap<Integer, ZomboidRadio.FreqListEntry> freqlist = std::make_unique<HashMap<>>();
 ...
 if (str != nullptr) {
 for (auto& string : staticSounds) if (str == string) {
 return true;
 }
 }
 }
 ...
 void addChannelName(const std::string& name, int frequency, const std::string& category) {
 this->addChannelName(name, frequency, category, true);
 }

 void addChannelName(const std::string& name, int frequency, const std::string& category, bool overwrite) {
 ...
 return this->categorizedChannels.containsKey(category) ? this->categorizedChannels.get(category) : nullptr;
 }

 std::string getChannelName(int frequency) {
 return this->channelNames.containsKey(frequency) ? this->channelNames.get(frequency) : nullptr;
 }
 ...
 if (radioChannel1 != nullptr && (!radioChannel1.getRadioData().isVanilla() || radioChannel0.getRadioData().isVanilla())) {
 if (boolean1) {
 DebugLog.Radio
 ...
 std::string string0 = ZomboidFileSystem.instance.getFileNameInCurrentSave("radio", "data", "RADIO_SAVE.txt");
 ...
 void addFrequencyListEntry(bool boolean0, DeviceData deviceData, int int0, int int1) {
 ...
 if (player != nullptr
 && player.getLastSpokenLine() != nullptr
 && (this->playerLastLine[int0] == nullptr || !this->playerLastLine[int0] == player.getLastSpokenLine()))) {
 ...
 std::string doDeviceRangeDistortion(const std::string& string, int int0, int int1) {
 ...
 std::string scrambleString(const std::string& msg, int intensity, bool ignoreBBcode, const std::string& customScramble) {
 ...
 return customScramble != nullptr ? customScramble : this->getRandomBzztFzzt();
 } else {
 ...
 stringBuilderx.append(customScramble != nullptr ? customScramble : this->getRandomBzztFzzt()).append(" ");
 boolean0 = true;
 }
 ...
 void SendTransmission(int sourceX, int sourceY, ChatMessage msg, int signalStrength) {
 Color color = msg.getTextColor();
 int int0 = msg.getRadioChannel();
 this->SendTransmission(sourceX, sourceY, int0, msg.getText(), nullptr, nullptr, color.r, color.g, color.b, signalStrength, false);
 }
 ...
 void setDisableBroadcasting(bool b) {
 ...
 private static class FreqListEntry {
 bool isInvItem = false;
 DeviceData deviceData;
 int sourceX = 0;
 int sourceY = 0;

 public FreqListEntry(bool boolean0, DeviceData deviceDatax, int int0, int int1) {
