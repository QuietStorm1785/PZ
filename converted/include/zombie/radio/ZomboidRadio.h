...
private
  final ArrayList<WaveSignalDevice> devices = std::make_unique<ArrayList<>>();
private
  final ArrayList<WaveSignalDevice> broadcastDevices =
      std::make_unique<ArrayList<>>();
  RadioScriptManager scriptManager;
  int DaysSinceStart = 0;
  int lastRecordedHour;
private
  final String[] playerLastLine = new String[4];
private
  final Map<Integer, String> channelNames = std::make_unique<HashMap<>>();
private
  final Map<String, Map<Integer, String>> categorizedChannels =
      std::make_unique<HashMap<>>();
private
  final List<Integer> knownFrequencies = std::make_unique<ArrayList<>>();
  RadioDebugConsole debugConsole;
  bool hasRecievedServerData = false;
  SLSoundManager storySoundManager = null;
private
  static final String[] staticSounds =
...
private
static final class FreqListEntry {
