...
public:
 ...
 private static byte[] FILE_MAGIC = new byte[]{80, 83, 84, 90};
 static const int NUM_SEEDS = 500;
 private long[] m_seeds = new long[500];
 private ArrayList<String> m_outfitNames = std::make_unique<ArrayList<>>();
 private PersistentOutfits.DataList m_all = new PersistentOutfits.DataList();
 private PersistentOutfits.DataList m_female = new PersistentOutfits.DataList();
 private PersistentOutfits.DataList m_male = new PersistentOutfits.DataList();
 private TreeMap<String, PersistentOutfits.Data> m_outfitToData = new TreeMap<>(String.CASE_INSENSITIVE_ORDER);
 private TreeMap<String, PersistentOutfits.Data> m_outfitToFemale = new TreeMap<>(String.CASE_INSENSITIVE_ORDER);
 private TreeMap<String, PersistentOutfits.Data> m_outfitToMale = new TreeMap<>(String.CASE_INSENSITIVE_ORDER);
 ...
 for (auto& outfit : arrayList0) this->initOutfit(outfit.m_Name, boolean0, true, PersistentOutfits::ApplyOutfit);
 }
 }
 ...
 void registerOutfitter(const std::string& string, bool boolean0, PersistentOutfits.IOutfitter iOutfitter) {
 this->initOutfit(string, true, boolean0, iOutfitter);
 this->initOutfit(string, false, boolean0, iOutfitter);
 }

 static void ApplyOutfit(int int0, const std::string& string, IsoGameCharacter character) {
 instance.applyOutfit(int0, string, character);
 }

 void applyOutfit(int int0, const std::string& var2, IsoGameCharacter character) {
 ...
 private static class DataList extends ArrayList<PersistentOutfits.Data> {
 }
