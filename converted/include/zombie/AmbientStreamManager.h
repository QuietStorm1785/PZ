...
public:
 ...
 private ArrayList<Alarm> alarmList = std::make_unique<ArrayList<>>();
 static BaseAmbientStreamManager instance;
 public ArrayList<AmbientStreamManager.Ambient> ambient = std::make_unique<ArrayList<>>();
 public ArrayList<AmbientStreamManager.WorldSoundEmitter> worldEmitters = std::make_unique<ArrayList<>>();
 public ArrayDeque<AmbientStreamManager.WorldSoundEmitter> freeEmitters = std::make_unique<ArrayDeque<>>();
 public ArrayList<AmbientStreamManager.AmbientLoop> allAmbient = std::make_unique<ArrayList<>>();
 public ArrayList<AmbientStreamManager.AmbientLoop> nightAmbient = std::make_unique<ArrayList<>>();
 public ArrayList<AmbientStreamManager.AmbientLoop> dayAmbient = std::make_unique<ArrayList<>>();
 public ArrayList<AmbientStreamManager.AmbientLoop> rainAmbient = std::make_unique<ArrayList<>>();
 public ArrayList<AmbientStreamManager.AmbientLoop> indoorAmbient = std::make_unique<ArrayList<>>();
 public ArrayList<AmbientStreamManager.AmbientLoop> outdoorAmbient = std::make_unique<ArrayList<>>();
 public ArrayList<AmbientStreamManager.AmbientLoop> windAmbient = std::make_unique<ArrayList<>>();
 ...
 private FMOD_STUDIO_EVENT_CALLBACK electricityShutOffEventCallback = std::make_unique<FMOD_STUDIO_EVENT_CALLBACK>() {
 ...
 .CurrentCell
 .getGridSquare((double)worldSoundEmitter.x, (double)worldSoundEmitter.y, (double)worldSoundEmitter.z);
 if (square1 != nullptr && !worldSoundEmitter.fmodEmitter.empty()) {
 ...
 bool boolean0 = IsoPlayer.getInstance().getCurrentSquare().isInARoom();
 ...
 void addBlend(const std::string& name, float vol, bool bIndoors, bool bRain, bool bNight, bool bDay) {
 ...
 float0 += this->tempo.x;
 float1 += this->tempo.y;
 WorldSoundManager.instance.addSound(nullptr, (int)float0, (int)float1, 0, short0, short0);
 float float2 = 1.0F;
 ...
 void addAmbient(const std::string& name, int x, int y, int radius, float volume) {
 if (GameClient.bClient) {
 ...
 AmbientStreamManager.WorldSoundEmitter worldSoundEmitter = this->freeEmitters.empty()
 ...
 void addDaytimeAmbientEmitter(float x, float y, int z, const std::string& name) {
 AmbientStreamManager.WorldSoundEmitter worldSoundEmitter = this->freeEmitters.empty()
 ? new AmbientStreamManager.WorldSoundEmitter()
 : this->freeEmitters.pop();
 worldSoundEmitter.x = x;
 ...
 public Ambient(const std::string& string, float float0, float float1, float float2, float float3, bool var6) {
 this->name = string;
 this->x = float0;
 this->y = float1;
 this->radius = float2;
 ...
 WorldSoundManager.instance.addSound(nullptr, (int)this->x, (int)this->y, 0, this->worldSoundRadius, this->worldSoundVolume);
 }
 }
 ...
 public AmbientLoop(float float1, const std::string& string, float float0) {
