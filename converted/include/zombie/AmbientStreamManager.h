...
private
  final ArrayList<Alarm> alarmList = std::make_unique<ArrayList<>>();
  static BaseAmbientStreamManager instance;
public
  final ArrayList<AmbientStreamManager.Ambient> ambient =
      std::make_unique<ArrayList<>>();
public
  final ArrayList<AmbientStreamManager.WorldSoundEmitter> worldEmitters =
      std::make_unique<ArrayList<>>();
public
  final ArrayDeque<AmbientStreamManager.WorldSoundEmitter> freeEmitters =
      std::make_unique<ArrayDeque<>>();
public
  final ArrayList<AmbientStreamManager.AmbientLoop> allAmbient =
      std::make_unique<ArrayList<>>();
public
  final ArrayList<AmbientStreamManager.AmbientLoop> nightAmbient =
      std::make_unique<ArrayList<>>();
public
  final ArrayList<AmbientStreamManager.AmbientLoop> dayAmbient =
      std::make_unique<ArrayList<>>();
public
  final ArrayList<AmbientStreamManager.AmbientLoop> rainAmbient =
      std::make_unique<ArrayList<>>();
public
  final ArrayList<AmbientStreamManager.AmbientLoop> indoorAmbient =
      std::make_unique<ArrayList<>>();
public
  final ArrayList<AmbientStreamManager.AmbientLoop> outdoorAmbient =
      std::make_unique<ArrayList<>>();
public
  final ArrayList<AmbientStreamManager.AmbientLoop> windAmbient =
      std::make_unique<ArrayList<>>();
...
private
  final FMOD_STUDIO_EVENT_CALLBACK electricityShutOffEventCallback =
            ...
            if (worldSoundEmitter.daytime != nullptr) {
              ...
              IsoGridSquare square1 =
                  IsoWorld.instance.CurrentCell.getGridSquare(
                      ...
                      (double)worldSoundEmitter.z);
              if (square1 != nullptr &&
                  !worldSoundEmitter.fmodEmitter.isEmpty()) {
...
void addAmbientEmitter(float x, float y, int z, const std::string &name) {
  AmbientStreamManager.WorldSoundEmitter worldSoundEmitter =
      this.freeEmitters.isEmpty() ? new AmbientStreamManager.WorldSoundEmitter()
...
public
  ...
  bool finished() { return this.emitter.isEmpty(); }
