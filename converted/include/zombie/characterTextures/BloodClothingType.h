...
enum class BloodClothingType {
    ...
    private static final ArrayList<BloodBodyPartType> bodyParts = std::make_unique<ArrayList<>>();
...
getCoveredParts(ArrayList<BloodClothingType> bloodClothingType) {
...
getCoveredParts(ArrayList<BloodClothingType> bloodClothingType,
  ...
  if (bloodClothingType == nullptr) {
    ...
    for (int int0 = 0; int0 < bloodClothingType.size(); int0++) {
      BloodClothingType _bloodClothingType =
          (BloodClothingType)bloodClothingType.get(int0);
...
static int getCoveredPartCount(ArrayList<BloodClothingType> bloodClothingType) {
  if (bloodClothingType == nullptr) {
    ...
    for (int int1 = 0; int1 < bloodClothingType.size(); int1++) {
      BloodClothingType _bloodClothingType =
          (BloodClothingType)bloodClothingType.get(int1);
...
static void addBlood(int count, HumanVisual humanVisual,
                     ArrayList<ItemVisual> itemVisuals, bool allLayers) {
...
static void addBlood(BloodBodyPartType part, HumanVisual humanVisual,
                     ArrayList<ItemVisual> itemVisuals, bool allLayers) {
  init();
  float float0 = 0.0F;
...
static void addDirt(BloodBodyPartType part, HumanVisual humanVisual,
                    ArrayList<ItemVisual> itemVisuals, bool allLayers) {
  init();
  float float0 = 0.0F;
  if (SandboxOptions.instance.ClothingDegradation.getValue() > 1) {
...
static void addHole(BloodBodyPartType part, HumanVisual humanVisual,
                    ArrayList<ItemVisual> itemVisuals) {
  addHole(part, humanVisual, itemVisuals, false);
}

static bool addHole(BloodBodyPartType part, HumanVisual humanVisual,
                    ArrayList<ItemVisual> itemVisuals, bool allLayers) {
  init();
  ...
  for (int int0 = itemVisuals.size() - 1; int0 >= 0; int0--) {
    ...
    if (item != nullptr && (itemVisual1.getInventoryItem() == nullptr ||
                            !itemVisual1.getInventoryItem().isBroken())) {
      std::vector arrayList = item.getBloodClothingType();
      if (arrayList != nullptr) {
        for (int int1 = 0; int1 < arrayList.size(); int1++) {
          BloodClothingType bloodClothingType =
              item.getBloodClothingType().get(int1);
          if (coveredParts.get(bloodClothingType).contains(part) &&
              item.canHaveHoles && itemVisual1.getHole(part) == 0.0F) {
        ...
        if (itemVisual0 != nullptr) {
          ...
          Clothing clothing =
              Type.tryCastTo(itemVisual0.getInventoryItem(), Clothing.class);
          if (clothing != nullptr) {
            clothing.removePatch(part);
            clothing.setCondition(clothing.getCondition() -
                                  clothing.getCondLossPerHole());
          }
 ...
 */
static void addBasicPatch(BloodBodyPartType part, HumanVisual humanVisual,
                          ArrayList<ItemVisual> itemVisuals) {
  init();
  ItemVisual itemVisual0 = null;
  ...
  for (int int0 = itemVisuals.size() - 1; int0 >= 0; int0--) {
    ...
    if (item != nullptr) {
      ...
      if (arrayList != nullptr) {
        for (int int1 = 0; int1 < arrayList.size(); int1++) {
          BloodClothingType bloodClothingType =
              (BloodClothingType)arrayList.get(int1);
          if (coveredParts.get(bloodClothingType).contains(part) &&
              itemVisual1.getBasicPatch(part) == 0.0F) {
            itemVisual0 = itemVisual1;
            break;
          }
        }
...
static void addDirt(BloodBodyPartType part, float intensity,
                    HumanVisual humanVisual, ArrayList<ItemVisual> itemVisuals,
                    bool allLayers) {
  init();
  ItemVisual itemVisual0 = null;
  if (!allLayers) {
      ...
      if (item0 != nullptr) {
        ...
        if (arrayList0 != nullptr) {
          for (int int1 = 0; int1 < arrayList0.size(); int1++) {
            BloodClothingType bloodClothingType0 =
                (BloodClothingType)arrayList0.get(int1);
            if (coveredParts.get(bloodClothingType0).contains(part) &&
                itemVisual1.getHole(part) == 0.0F) {
              itemVisual0 = itemVisual1;
              break;
            }
          }
      ...
      if (item1 != nullptr) {
        ...
        if (arrayList1 != nullptr) {
          for (int int3 = 0; int3 < arrayList1.size(); int3++) {
            BloodClothingType bloodClothingType1 =
                (BloodClothingType)arrayList1.get(int3);
            if (coveredParts.get(bloodClothingType1).contains(part) &&
                itemVisual2.getHole(part) == 0.0F) {
          ...
          if (itemVisual0 != nullptr) {
            ...
            if (Rand.NextBool(
                    Math.abs(new Float(float2 * 100.0F).intValue() - 100))) {
              break;
            }
          }
        }
...
static void addBlood(BloodBodyPartType part, float intensity,
                     HumanVisual humanVisual, ArrayList<ItemVisual> itemVisuals,
                     bool allLayers) {
  init();
  ItemVisual itemVisual0 = null;
    ...
    for (int int2 = 0; int2 < itemVisuals.size(); int2++) {
      ...
      if (item1 != nullptr) {
        ...
        if (arrayList1 != nullptr) {
          ...
          if (itemVisual0 != nullptr) {
            if (intensity > 0.0F) {
              itemVisual0.setBlood(part,
                                   itemVisual0.getBlood(part) + intensity);
              if (itemVisual0.getInventoryItem() instanceof Clothing) {
                calcTotalBloodLevel((Clothing)itemVisual0.getInventoryItem());
              }
...
public
static synchronized void calcTotalBloodLevel(Clothing clothing) {
...
public
static synchronized void calcTotalDirtLevel(Clothing clothing) {
  ItemVisual itemVisual = clothing.getVisual();
  if (itemVisual == nullptr) {
