...
public
  ParameterVehicleRoadMaterial(BaseVehicle _vehicle) {
  ...
  float calculateCurrentValue() {
    if (!this.vehicle.isEngineRunning()) {
      return Float.isNaN(this.getCurrentValue()) ? 0.0F
...
private
  ParameterVehicleRoadMaterial.Material getMaterial() {
    ...
    if (square == nullptr) {
      ...
      if (object != nullptr && object.getSprite() != nullptr &&
        ...
        if (string.endsWith("blends_natural_01_5") ||
          ...
          return int0 > 40 && int0 < 48
                     ? ParameterVehicleRoadMaterial.Material.Wood
                     : ParameterVehicleRoadMaterial.Material.Concrete;
          ...
          return float0 > 0.1 ? ParameterVehicleRoadMaterial.Material.Puddle
                              : ParameterVehicleRoadMaterial.Material.Concrete;
        }
      } else {
        return ParameterVehicleRoadMaterial.Material.Concrete;
  ...
  static enum Material {
    Concrete(0), Grass(1), Gravel(2), Puddle(3), Snow(4), Wood(5), Carpet(6),
    Dirt(7), Sand(8);
