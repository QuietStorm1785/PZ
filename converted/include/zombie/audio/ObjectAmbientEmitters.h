...
public:
private
  final HashMap<String, ObjectAmbientEmitters.PowerPolicy> powerPolicyMap =
      std::make_unique<HashMap<>>();
  static ObjectAmbientEmitters instance = null;
  static const Vector2 tempVector2 = new Vector2();
private
  final HashMap<IsoObject, ObjectAmbientEmitters.ObjectWithDistance> m_added =
      std::make_unique<HashMap<>>();
private
  final ObjectPool<ObjectAmbientEmitters.ObjectWithDistance> m_objectPool =
      new ObjectPool<>(ObjectAmbientEmitters.ObjectWithDistance::new);
private
  final ArrayList<ObjectAmbientEmitters.ObjectWithDistance> m_objects =
      std::make_unique<ArrayList<>>();
private
  final ObjectAmbientEmitters.Slot[] m_slots;
private
  final Comparator<ObjectAmbientEmitters.ObjectWithDistance> comp =
      new Comparator<ObjectAmbientEmitters.ObjectWithDistance>(){int compare(
          ObjectAmbientEmitters.ObjectWithDistance objectWithDistance1,
          ObjectAmbientEmitters.ObjectWithDistance objectWithDistance0){
...
ObjectAmbientEmitters() {
  uint8_t byte0 = 16;
  this.m_slots = PZArrayUtil.newInstance(
      ObjectAmbientEmitters.Slot.class, byte0, ObjectAmbientEmitters.Slot::new);
  this.powerPolicyMap.put("FactoryMachineAmbiance",
                          ObjectAmbientEmitters.PowerPolicy.InteriorHydro);
  this.powerPolicyMap.put("HotdogMachineAmbiance",
                          ObjectAmbientEmitters.PowerPolicy.InteriorHydro);
  this.powerPolicyMap.put("PayPhoneAmbiance",
                          ObjectAmbientEmitters.PowerPolicy.ExteriorOK);
  this.powerPolicyMap.put("StreetLightAmbiance",
                          ObjectAmbientEmitters.PowerPolicy.ExteriorOK);
  this.powerPolicyMap.put("NeonLightAmbiance",
                          ObjectAmbientEmitters.PowerPolicy.ExteriorOK);
  this.powerPolicyMap.put("NeonSignAmbiance",
                          ObjectAmbientEmitters.PowerPolicy.ExteriorOK);
  this.powerPolicyMap.put("JukeboxAmbiance",
                          ObjectAmbientEmitters.PowerPolicy.InteriorHydro);
  this.powerPolicyMap.put("ControlStationAmbiance",
                          ObjectAmbientEmitters.PowerPolicy.InteriorHydro);
  this.powerPolicyMap.put("ClockAmbiance",
                          ObjectAmbientEmitters.PowerPolicy.InteriorHydro);
  this.powerPolicyMap.put("GasPumpAmbiance",
                          ObjectAmbientEmitters.PowerPolicy.ExteriorOK);
  this.powerPolicyMap.put("LightBulbAmbiance",
                          ObjectAmbientEmitters.PowerPolicy.InteriorHydro);
  this.powerPolicyMap.put("ArcadeMachineAmbiance",
                          ObjectAmbientEmitters.PowerPolicy.InteriorHydro);
}
    ...
    if (!this.m_added.containsKey(object)) {
      ...
      for (int int0 = 0; int0 < IsoPlayer.numPlayers; int0++) {
        ...
        if (player != nullptr && object.getObjectIndex() != -1) {
          uint8_t byte0 = 15;
          if (perObjectLogic instanceof
              ObjectAmbientEmitters.DoorLogic || perObjectLogic instanceof
              ObjectAmbientEmitters.WindowLogic) {
            byte0 = 10;
          }

          if ((object.square.z == PZMath.fastfloor(player.getZ()) ||
               !(perObjectLogic instanceof ObjectAmbientEmitters.DoorLogic) &&
                   !(perObjectLogic instanceof
                     ObjectAmbientEmitters.WindowLogic)) &&
              !(player.DistToSquared(object.square.x + 0.5F,
                                     object.square.y + 0.5F) > byte0 * byte0)) {
            boolean0 = true;
            break;
          }
        }
      ...
      if (boolean0) {
        ObjectAmbientEmitters.ObjectWithDistance objectWithDistance =
  ...
  if (!GameServer.bServer) {
    ...
    if (this.m_objects.isEmpty()) {
      ...
      for (int int1 = 0; int1 < this.m_objects.size(); int1++) {
        ...
        ObjectAmbientEmitters.PerObjectLogic perObjectLogic0 =
            this.m_objects.get(int1).logic;
        if (!this.shouldPlay(object0, perObjectLogic0)) {
          ...
          objectWithDistance.distSq = this.getClosestListener(
              tempVector2.x, tempVector2.y, object0.square.z);
        }
      }
      ...
      for (int int3 = 0; int3 < int2; int3++) {
        ...
        ObjectAmbientEmitters.PerObjectLogic perObjectLogic1 =
            this.m_objects.get(int3).logic;
      ...
      for (int int5 = 0; int5 < int2; int5++) {
        ...
        ObjectAmbientEmitters.PerObjectLogic perObjectLogic2 =
            this.m_objects.get(int5).logic;
    ...
    if (!chunkMap.ignore) {
      ...
      for (int int3 = -1; int3 <= 1; int3++) {
        for (int int4 = -1; int4 <= 1; int4++) {
          ...
          if (chunk != nullptr) {
            for (IsoObject object :
                 chunk.m_objectEmitterData.m_objects.keySet()) {
              this.addObject(object,
                             chunk.m_objectEmitterData.m_objects.get(object));
            }
          }
        }
      }
...
bool shouldPlay(IsoObject object,
                ObjectAmbientEmitters.PerObjectLogic perObjectLogic) {
  if (object == nullptr) {
    return false;
  } else {
    return object.getObjectIndex() == -1 ? false
                                         : perObjectLogic.shouldPlaySound();
  }
}
...
void render() {
  if (DebugOptions.instance.ObjectAmbientEmitterRender.getValue()) {
    IsoChunkMap chunkMap = IsoWorld.instance.CurrentCell
                               .ChunkMap[IsoCamera.frameState.playerIndex];
    if (!chunkMap.ignore) {
          ...
          if (chunk != nullptr) {
            for (IsoObject object0 :
              ...
              if (object0.square.z == (int)IsoCamera.frameState.CamCharacterZ) {
                ...
                LineDrawer.addLine(float0 - 0.45F, float1 - 0.45F, float2,
                                   float0 + 0.45F, float1 + 0.45F, float2, 0.5F,
    ...
    for (int int4 = 0; int4 < this.m_slots.length; int4++) {
      ...
      if (slot.playing) {
        ...
        LineDrawer.addLine(float3 - 0.45F, float4 - 0.45F, float5,
                           float3 + 0.45F, float4 + 0.45F, float5, 0.0F, 0.0F,
                           1.0F, nullptr, false);
      }
    }
  }
...
public
static final class AmbientSoundLogic extends
    ObjectAmbientEmitters.PerObjectLogic {
  ObjectAmbientEmitters.PowerPolicy powerPolicy =
      ObjectAmbientEmitters.PowerPolicy.NotRequired;
  bool bHasGeneratorParameter = false;

public
  ObjectAmbientEmitters.PerObjectLogic init(IsoObject object) {
    super.init(object);
    std::string string = this.getSoundName();
    this.powerPolicy =
        ObjectAmbientEmitters.getInstance().powerPolicyMap.getOrDefault(
            string, ObjectAmbientEmitters.PowerPolicy.NotRequired);
    if (this.powerPolicy != ObjectAmbientEmitters.PowerPolicy.NotRequired) {
      GameSound gameSound = GameSounds.getSound(string);
      this.bHasGeneratorParameter =
          gameSound != nullptr &&
          gameSound.numClipsUsingParameter("Generator") > 0;
    }
  ...
  bool shouldPlaySound() {
    if (this.powerPolicy == ObjectAmbientEmitters.PowerPolicy.InteriorHydro) {
      bool boolean0 = this.object.square.haveElectricity() ||
                      IsoWorld.instance.isHydroPowerOn() &&
                          this.object.square.getRoom() != null;
      if (!boolean0) {
        return false;
      }
    }
    ...
    if (this.powerPolicy == ObjectAmbientEmitters.PowerPolicy.ExteriorOK) {
      bool boolean1 = this.object.square.haveElectricity() ||
                      IsoWorld.instance.isHydroPowerOn();
      if (!boolean1) {
        return false;
      }
    ...
    if (this.powerPolicy != ObjectAmbientEmitters.PowerPolicy.NotRequired &&
        !IsoWorld.instance.isHydroPowerOn() && !this.bHasGeneratorParameter) {
      return false;
    } else {
  ...
  void checkParameters(BaseSoundEmitter baseSoundEmitter, long long0) {
    if (this.powerPolicy != ObjectAmbientEmitters.PowerPolicy.NotRequired) {
      this.setParameterValue1(baseSoundEmitter, long0, "Generator",
                              IsoWorld.instance.isHydroPowerOn() ? 0.0F : 1.0F);
    }
  }
}
...
static final class ChunkData {
  final HashMap<IsoObject, ObjectAmbientEmitters.PerObjectLogic> m_objects =
      std::make_unique<HashMap<>>();
  ...
  void removeObject(IsoObject object) { this.m_objects.remove(object); }

  void reset() { this.m_objects.clear(); }
}

public static final class DoorLogic extends
    ObjectAmbientEmitters.PerObjectLogic {
  bool shouldPlaySound() { return true; }

  std::string getSoundName() { return "DoorAmbiance"; }
...
public
static final class FridgeHumLogic extends ObjectAmbientEmitters.PerObjectLogic {
  bool shouldPlaySound() {
    ItemContainer container =
        this.object.getContainerByEitherType("fridge", "freezer");
    return container != nullptr && container.isPowered();
  }

  std::string getSoundName() { return "FridgeHum"; }
...
public
  ObjectAmbientEmitters.PerObjectLogic init(IsoObject _object) {
    this.object = _object;
    return this;
  }
  ...
  void setParameterValue1(BaseSoundEmitter baseSoundEmitter, long long0,
    ...
    if (float0 != this.parameterValue1) {
      ...
      FMOD_STUDIO_PARAMETER_DESCRIPTION fmod_studio_parameter_description =
          FMODManager.instance.getParameterDescription(string);
      baseSoundEmitter.setParameterValue(
          long0, fmod_studio_parameter_description, float0);
    }
  }

  void setParameterValue1(
      BaseSoundEmitter baseSoundEmitter, long long0,
      FMOD_STUDIO_PARAMETER_DESCRIPTION fmod_studio_parameter_description,
      float float0) {
    if (float0 != this.parameterValue1) {
      this.parameterValue1 = float0;
...
public
  abstract boolean shouldPlaySound();

public
  abstract String getSoundName();

public
  abstract void startPlaying(BaseSoundEmitter emitter, long instance);

public
  abstract void stopPlaying(BaseSoundEmitter emitter, long instance);

public
  abstract void checkParameters(BaseSoundEmitter emitter, long instance);
}

static enum PowerPolicy {
  NotRequired, InteriorHydro, ExteriorOK;
}
...
static final class Slot {
  ...
  void playSound(IsoObject objectx,
                 ObjectAmbientEmitters.PerObjectLogic perObjectLogic) {
    ...
    this.emitter.setPos(ObjectAmbientEmitters.tempVector2.getX(),
                        ObjectAmbientEmitters.tempVector2.getY(),
                        objectx.square.z);
    this.object = objectx;
    ...
    if (!this.emitter.isPlaying(string)) {
      ...
      FMODSoundEmitter fMODSoundEmitter =
          Type.tryCastTo(this.emitter, FMODSoundEmitter.class);
...
public static final class TentAmbianceLogic extends
    ObjectAmbientEmitters.PerObjectLogic {
  bool shouldPlaySound() {
    return this.object.sprite != nullptr &&
           this.object.sprite.getName() != nullptr &&
           this.object.sprite.getName().startsWith("camping_01") &&
           (this.object.sprite.tileSheetIndex == 0 ||
            this.object.sprite.tileSheetIndex == 3);
  }

  std::string getSoundName() { return "TentAmbiance"; }
...
public
static final class TreeAmbianceLogic extends
    ObjectAmbientEmitters.PerObjectLogic {
  bool shouldPlaySound() { return true; }

  std::string getSoundName() { return "TreeAmbiance"; }
...
public
static final class WaterDripLogic extends ObjectAmbientEmitters.PerObjectLogic {
  bool shouldPlaySound() {
    return this.object.sprite != nullptr &&
           this.object.sprite.getProperties().Is(IsoFlagType.waterPiped) &&
           this.object.getWaterAmount() > 0.0F;
  }

  std::string getSoundName() { return "WaterDrip"; }

  void startPlaying(BaseSoundEmitter baseSoundEmitter, long long0) {
    if (this.object.sprite != nullptr &&
        this.object.sprite.getProperties().Is("SinkType")) {
    ...
    float float0 = !window.IsOpen() && !window.isDestroyed() ? 0.0F :
                      1.0F;
                      this.setParameterValue1(baseSoundEmitter, long0,
                                              "DoorWindowOpen", float0);
                    }
    }
  }
} // namespace audio
