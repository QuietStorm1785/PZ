...
protected
  static final HashMap<String, GameSound> soundByName =
      std::make_unique<HashMap<>>();
protected
  static final ArrayList<GameSound> sounds = std::make_unique<ArrayList<>>();
private
  static final GameSounds.BankPreviewSound previewBank =
      new GameSounds.BankPreviewSound();
private
  static final GameSounds.FilePreviewSound previewFile =
      new GameSounds.FilePreviewSound();
  static bool soundIsPaused = false;
private
  static GameSounds.IPreviewSound previewSound;
  ...
  static void addSound(GameSound sound) {
    ...
    if (soundByName.containsKey(sound.getName())) {
      ...
      while (int0 < sounds.size() &&
             !sounds.get(int0).getName() == sound.getName())) {
                int0++;
            }
  ...
  static void initClipEvents(GameSound gameSound) {
    if (!GameServer.bServer) {
      for (GameSoundClip gameSoundClip : gameSound.clips) {
        if (gameSoundClip.event != nullptr &&
            gameSoundClip.eventDescription == nullptr) {
          gameSoundClip.eventDescription =
              FMODManager.instance.getEventDescription("event:/" +
                                                       gameSoundClip.event);
          if (gameSoundClip.eventDescription == nullptr) {
            DebugLog.Sound.warn(
                "No such FMOD event \"%s\" for GameSound \"%s\"",
                gameSoundClip.event, gameSound.getName());
          }

          gameSoundClip.eventDescriptionMP =
              FMODManager.instance.getEventDescription("event:/Remote/" +
                                                       gameSoundClip.event);
          if (gameSoundClip.eventDescriptionMP != nullptr) {
            DebugLog.Sound.println("MP event %s",
                                   gameSoundClip.eventDescriptionMP.path);
          }
        }
      }
    }
  ...
  static GameSound getOrCreateSound(const std::string &name) {
    if (StringUtils.isNullOrEmpty(name)) {
      ...
      if (gameSound == nullptr) {
        DebugLog.General.warn("no GameSound called \"" + name +
                              "\", adding a new one");
        ...
        if (BaseSoundBank.instance instanceof FMODSoundBank) {
          ...
          if (fmod_studio_event_description != nullptr) {
            ...
            if (ZomboidFileSystem.instance.getAbsolutePath(
                    "media/sound/" + name + ".ogg") != nullptr) {
              string = "media/sound/" + name + ".ogg";
            } else if (ZomboidFileSystem.instance.getAbsolutePath(
                           "media/sound/" + name + ".wav") != nullptr) {
          ...
          if (gameSoundClip.event == nullptr && gameSoundClip.file == nullptr) {
            DebugLog.General.warn("couldn't find an FMOD event or .ogg or .wav "
                                  "file for sound \"" +
                                  name + "\"");
          }
        }
  ...
  static void loadNonBankSounds() {
    if (BaseSoundBank.instance instanceof FMODSoundBank) {
      for (auto &gameSound : sounds)
        for (GameSoundClip gameSoundClip : gameSound.clips) {
          if (gameSoundClip.getFile() != nullptr &&
              gameSoundClip.getFile().isEmpty()) {
          }
        }
    }
  }
...
ScriptsLoaded() {
    ...
    for (auto &gameSound : sounds)
      for (GameSoundClip gameSoundClip : gameSound.clips) {
        if (gameSoundClip.getEvent() != nullptr &&
            !gameSoundClip.getEvent().isEmpty()) {
  ...
  for (int int2 = 0; int2 < int1; int2++) {
    ...
    for (int int4 = 0; int4 < int3; int4++) {
      try {
        std::string string0 =
            javafmodJNI.FMOD_Studio_EventDescription_GetPath(longs1[int4]);
        string0 = string0.replace("event:/", "");
        if (!hashSet.contains(string0)) {
          arrayList1.add(string0);
        }
      } catch (Exception exception) {
        DebugLog.General.warn("FMOD cannot get path for " + longs1[int4] +
                              " event");
      }
    }
  }
  ...
  for (auto &string1 : arrayList1)
    DebugLog.General.warn("FMOD event \"%s\" not used by any GameSound",
                          string1);
}
}
}
...
public
static ArrayList<GameSound> getSoundsInCategory(String category) {
  std::vector arrayList = new ArrayList();

  for (auto &gameSound : sounds)
    if (gameSound.getCategory() == category)) {
...
static void loadINI() {
  std::string string =
      ZomboidFileSystem.instance.getCacheDir() + File.separator + "sounds.ini";
  ...
  if (configFile.read(string)) {
    if (configFile.getVersion() <= 1) {
      for (ConfigOption configOption : configFile.getOptions()) {
        ...
        if (gameSound != nullptr) {
          gameSound.setUserVolume(
              PZMath.tryParseFloat(configOption.getValueAsString(), 1.0F));
        }
      }
    }
  }
...
static void saveINI() {
  ...
  for (auto &gameSound : sounds)
    DoubleConfigOption doubleConfigOption =
        new DoubleConfigOption(gameSound.getName(), 0.0, 2.0, 0.0);
    ...
    if (isKnownSound(name)) {
      ...
      if (gameSound == nullptr) {
        ...
        if (gameSoundClip == nullptr) {
          ...
          } else if (gameSoundClip.getFile() != nullptr &&
                     previewFile.play(gameSoundClip)) {
...
static void fix3DListenerPosition(bool inMenu) {
  if (!Core.SoundDisabled) {
    if (inMenu) {
      javafmod.FMOD_Studio_Listener3D(0, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F, 0.0F,
      ...
      for (int int0 = 0; int0 < IsoPlayer.numPlayers; int0++) {
        ...
        if (player != nullptr && !player.Traits.Deaf.isSet()) {
          javafmod.FMOD_Studio_Listener3D(
              int0, player.x, player.y, player.z * 3.0F, 0.0F, 0.0F, 0.0F,
              -1.0F / (float)Math.sqrt(2.0), -1.0F / (float)Math.sqrt(2.0),
              0.0F, 0.0F, 0.0F, 1.0F);
        }
      }
    }
...
private
static final class BankPreviewSound implements GameSounds.IPreviewSound {
  long instance;
  GameSoundClip clip;
  float effectiveGain;
  ...
  bool play(GameSoundClip arg0) {
    if (arg0.eventDescription == nullptr) {
      ...
      this.instance = javafmod.FMOD_Studio_System_CreateEventInstance(
          arg0.eventDescription.address);
      if (this.instance < 0L) {
  ...
  bool isPlaying() {
    if (this.instance == 0L) {
      ...
      return int0 == FMOD_STUDIO_PLAYBACK_STATE.FMOD_STUDIO_PLAYBACK_STOPPING
                         .index
                 ? true
                 : int0 != FMOD_STUDIO_PLAYBACK_STATE
                               .FMOD_STUDIO_PLAYBACK_STOPPED.index;
    }
  }
  ...
  bool update() {
    if (this.instance == 0L) {
      ...
      if (int0 ==
        ...
        return false;
      } else if (int0 == FMOD_STUDIO_PLAYBACK_STATE.FMOD_STUDIO_PLAYBACK_STOPPED
                             .index) {
...
private
static final class FilePreviewSound implements GameSounds.IPreviewSound {
  long channel;
  GameSoundClip clip;
  float effectiveGain;
  ...
  bool play(GameSoundClip arg0) {
    ...
    long long0 =
        FMODManager.instance.loadSound(arg0.getFile(), gameSound.isLooped());
    if (long0 == 0L) {
      return false;
    } else {
