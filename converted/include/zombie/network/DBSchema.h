...
class DBSchema {
public:
 private HashMap<String, HashMap<String, String>> schema = std::make_unique<HashMap<>>();
 ...
 ResultSet resultSet0 = databaseMetaData.getTables(nullptr, nullptr, nullptr, strings);

 while (resultSet0.next()) {
 std::string string0 = resultSet0.getString(3);
 if (!string0.startsWith("SQLITE_")) {
 ResultSet resultSet1 = databaseMetaData.getColumns(nullptr, nullptr, string0, nullptr);
 ...
 void setFullTable(KahluaTable table) {
