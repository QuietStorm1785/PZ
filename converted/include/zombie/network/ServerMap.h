...
public:
 ...
 public IsoObjectID<IsoZombie> ZombieMap = new IsoObjectID<>(IsoZombie.class);
 ...
 public ArrayList<ServerMap.ServerCell> LoadedCells = std::make_unique<ArrayList<>>();
 public ArrayList<ServerMap.ServerCell> ReleventNow = std::make_unique<ArrayList<>>();
 int width;
 int height;
 IsoMetaGrid grid;
 ArrayList<ServerMap.ServerCell> ToLoad = std::make_unique<ArrayList<>>();
 static ServerMap.DistToCellComparator distToCellComparator = new ServerMap.DistToCellComparator();
 private ArrayList<ServerMap.ServerCell> tempCells = std::make_unique<ArrayList<>>();
 long lastTick = 0L;
 Vector2 start;
 ...
 return !this->isValidCell(int0, int1) ? nullptr : this->cellMap[int1 * this->width + int0];
 }
 ...
 bool boolean1 = this->ReleventNow.contains(serverCell) || !this->outsidePlayerInfluence(serverCell);
 ...
 if (udpConnection.players[int1] != nullptr && udpConnection.players[int1].OnlineID == short0) {
 ...
 if (udpConnection.players[int5] != nullptr && udpConnection.players[int5].OnlineID == short0) {
 ...
 int0 -= this->getMinX();
 int1 -= this->getMinY();
 return this->ToLoad.contains(this->getCell(int0, int1) ? false : this->getCell(int0, int1) != nullptr;
 }
 ...
 return serverCell != nullptr && serverCell.bLoaded ? serverCell.chunks[int4][int5] : nullptr;
 } else {
 return nullptr;
 }
 }
 ...
 int compare(ServerMap.ServerCell serverCell0, ServerMap.ServerCell serverCell1) {
 ...
 float distToCell(float float1, float float3, ServerMap.ServerCell serverCell) {
 int int0 = serverCell.WX * 50;
 int int1 = serverCell.WY * 50;
 int int2 = int0 + 50;
 int int3 = int1 + 50;
 ...
 public IsoChunk[][] chunks = new IsoChunk[5][5];
 private HashSet<RoomDef> UnexploredRooms = std::make_unique<HashSet<>>();
 static const ServerChunkLoader chunkLoader = new ServerChunkLoader();
 private static ArrayList<ServerMap.ServerCell> loaded = std::make_unique<ArrayList<>>();
 bool startedLoading = false;
 bool bCancelLoading = false;
 bool bLoadingWasCancelled = false;
 private static ArrayList<ServerMap.ServerCell> loaded2 = std::make_unique<ArrayList<>>();
 ...
 if (square4 != nullptr) {
 if (square4.getRoom() != nullptr && !square4.getRoom().def.bExplored) {
