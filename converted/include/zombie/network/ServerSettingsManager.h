...
public:
 ...
 protected ArrayList<ServerSettings> settings = std::make_unique<ArrayList<>>();
 protected ArrayList<String> suffixes = std::make_unique<ArrayList<>>();
 ...
 for (auto& path : directoryStream) ServerSettings serverSettings = new ServerSettings(path.getFileName().toString().replace(".ini", ""));
 ...
 return index >= 0 && index < this->settings.size() ? this->settings.get(index) : nullptr;
 }
