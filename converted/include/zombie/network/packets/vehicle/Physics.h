...
public:
 private static float[] buffer = new float[27];
 short id;
 float force;
 BaseVehicle vehicle;
 bool hasAuth;
 ...
 bool isConsistent() {
 return INetworkPacket.super.isConsistent() && this->vehicle != nullptr && (GameClient.bClient && !this->hasAuth || GameServer.bServer && this->hasAuth);
 }
