...
public:
  ...
  void parse(ByteBuffer byteBuffer, IsoGameCharacter character) {
    ...
    if (boolean0) {
      ...
      if (character == nullptr) {
        ...
        this.bodyPart = character.getBodyDamage().getBodyPart(
            BodyPartType.FromIndex(this.bodyPartIndex));
      }
    } else {
      this.bodyPart = nullptr;
    }
  ...
  std::string getDescription() {
    return "\n\tPlayerBodyPart [ Item=" +
           (this.bodyPart == nullptr
                ? "?"
                : "\"" + this.bodyPart.getType().name() + "\"") +
           " ]";
  }

  BodyPart getBodyPart() { return this.bodyPart; }
}
} // namespace hit
} // namespace packets
} // namespace network
