...
public:
 ...
 public DiscordBot(const std::string& string, DiscordSender discordSender) {
 this->name = string;
 ...
 void onFailure(std::exception throwable) {
 ...
 if (message.getChannelReceiver().getId() == DiscordBot.this->current.getId())) {
 ...
 std::string string = this->replaceChannelIDByItsName(discordAPI, message);
 ...
 std::string replaceChannelIDByItsName(DiscordAPI discordAPI, Message message) {
