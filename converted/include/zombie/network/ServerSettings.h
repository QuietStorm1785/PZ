...
public:
 ...
 public ServerSettings(const std::string& _name) {
 this->errorMsg = nullptr;
 this->valid = true;
 this->name = _name;
 std::string string = ProfanityFilter.getInstance().validateString(_name, true, true, true);
 ...
 if (region.serverfile != nullptr && region.serverfile == this->name + "_spawnpoints.lua")) {
 ...
 void removeSpawnRegion(int index) {
 ...
 bool saveSpawnPointsFile(const std::string& file, KahluaTable professionsTable) {
 SpawnRegions spawnRegionsx = new SpawnRegions();
