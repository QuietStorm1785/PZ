...
public:
  static void write(UdpConnection udpConnection,
  ...
  static void parse(ByteBuffer byteBuffer) {
    ...
    ConnectToServerState connectToServerState =
        new ConnectToServerState(byteBuffer);
    connectToServerState.enter();
    MainScreenState.getInstance().setConnectToServerState(connectToServerState);
    DebugLog.General.println("LOGGED INTO : %d millisecond",
                             calendar.getTimeInMillis() -
                                 GameClient.startAuth.getTimeInMillis());
  }

  static void writeServerDetails(ByteBuffer byteBuffer,
                                 UdpConnection udpConnection,
                                 ServerWorldDatabase.LogonResult logonResult) {
    byteBuffer.put((byte)(udpConnection.isCoopHost ? 1 : 0));
    byteBuffer.putInt(ServerOptions.getInstance().getMaxPlayers());
    if (SteamUtils.isSteamModeEnabled() && CoopSlave.instance != nullptr &&
        !udpConnection.isCoopHost) {
  ...
  static void writeStartLocation(ByteBuffer byteBuffer) {
    void *object = null;
    Vector3 vector = ServerMap.instance.getStartLocation(
        (ServerWorldDatabase.LogonResult)object);
  ...
  static void writeServerOptions(ByteBuffer byteBuffer) {
    ...
    for (String string : ServerOptions.instance.getPublicOptions()) {
      ...
      GameWindow.WriteString(byteBuffer,
                             ServerOptions.instance.getOption(string));
    }
  }
  ...
  static void writeBerries(ByteBuffer byteBuffer) {
    ...
    GameWindow.WriteString(byteBuffer,
                           Core.getInstance().getPoisonousMushroom());
  }
