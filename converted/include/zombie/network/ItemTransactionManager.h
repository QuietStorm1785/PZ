...
public:
 private static HashSet<ItemTransactionManager.ItemRequest> requests = std::make_unique<HashSet<>>();
 ...
 static void receiveOnServer(ByteBuffer byteBuffer, UdpConnection udpConnection, short var2) {
 try {
 uint8_t byte0 = byteBuffer.get();
 int int0 = byteBuffer.getInt();
 int int1 = byteBuffer.getInt();
 ...
 static void sendItemTransaction(UdpConnection udpConnection, uint8_t byte0, int int0, int int1, int int2) {
