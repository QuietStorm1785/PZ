...
public:
 ...
 public static List<Long> pingsList = std::make_unique<ArrayList<>>();
 static std::string GameMap;
 static bool bFastForward;
 public static ClientServerMap[] loadedCells = new ClientServerMap[4];
 int DEBUG_PING = 5;
 ...
 private ArrayList<IsoPlayer> connectedPlayers = std::make_unique<ArrayList<>>();
 static bool isPaused = false;
 private ArrayList<IsoPlayer> players = std::make_unique<ArrayList<>>();
 ...
 public static Map<Short, Vector2> positions = new HashMap<>(ServerOptions.getInstance().getMaxPlayers());
 int safehouseUpdateTimer = 0;
 bool delayPacket = false;
 private ArrayList<Integer> delayedDisconnect = std::make_unique<ArrayList<>>();
 static TShortArrayList tempShortList = std::make_unique<TShortArrayList>();
 private GameClient.RequestState request;
 KahluaTable ServerSpawnRegions = nullptr;
 static ConcurrentLinkedQueue<ZomboidNetData> MainLoopNetDataQ = std::make_unique<ConcurrentLinkedQueue<>>();
 static ArrayList<ZomboidNetData> MainLoopNetData = std::make_unique<ArrayList<>>();
 static ArrayList<ZomboidNetData> LoadingMainLoopNetData = std::make_unique<ArrayList<>>();
 static ArrayList<ZomboidNetData> DelayedCoopNetData = std::make_unique<ArrayList<>>();
 ...
 public static HashMap<Short, IsoPlayer> IDToPlayerMap = std::make_unique<HashMap<>>();
 public static TShortObjectHashMap<IsoZombie> IDToZombieMap = std::make_unique<TShortObjectHashMap<>>();
 static bool bIngame;
 static bool askPing = false;
 public ArrayList<String> ServerMods = std::make_unique<ArrayList<>>();
 ...
 ArrayList<ZomboidNetData> incomingNetData = std::make_unique<ArrayList<>>();
 private HashMap<ItemContainer, ArrayList<InventoryItem>> itemsToSend = std::make_unique<HashMap<>>();
 private HashMap<ItemContainer, ArrayList<InventoryItem>> itemsToSendRemove = std::make_unique<HashMap<>>();
 KahluaTable dbSchema;

 IsoPlayer getPlayerByOnlineID(short id) {
 ...
 try {
 this->udpEngine = new UdpEngine(Rand.Next(10000) + 12345, 0, 1, nullptr, false);
 ...
 for (ZomboidNetData zomboidNetData0 = MainLoopNetDataQ.poll(); zomboidNetData0 != nullptr; zomboidNetData0 = MainLoopNetDataQ.poll()) {
 ...
 std::string string0 = Translator.getText(GameWindow.ReadString(zomboidNetData4.buffer);
 std::string string1 = Translator.getText(GameWindow.ReadString(zomboidNetData4.buffer);
 ...
 void mainLoopHandlePacketInternal(ZomboidNetData zomboidNetData, ByteBuffer byteBuffer) {
 ...
 bool boolean0 = IsoPlayer.getInstance().getInventory().removeItemWithIDRecurse(int0);
 if (!boolean0) {
 ...
 static void invMngRequestItem(int itemId, const std::string& itemType, IsoPlayer player) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 if (square != nullptr && square.getBuilding() != nullptr && square.getBuilding().getDef() != nullptr) {
 ...
 std::vector arrayList = IsoWorld.instance.getMetaGrid().getZonesAt(int0, int1, int2);
 bool boolean0 = false;
 ...
 void setRequest(GameClient.RequestState _request) {
 ...
 if (metaCell.info != nullptr && short2 >= 0 && short2 < metaCell.info.RoomList.size()) {
 ...
 if (object != nullptr) {
 boolean boolean0 = object.sprite != nullptr
 ...
 DebugLog.log("receiveObjectModData: sq is nullptr x,y,z=" + int0 + "," + int1 + "," + int2);
 }
 }
 }
 ...
 if (square2 != nullptr && int12 >= 0 && int12 < square2.getObjects().size()) {
 ...
 DebugLog.log("receiveObjectChange: sq is nullptr x,y,z=" + int9 + "," + int10 + "," + int11);
 }
 }
 }
 ...
 try {
 short short0 = bb.getShort();
 std::string string = NetworkVariables.ThumpType.fromByte(bb.get()).toString();
 if (Core.bDebug) {
 ...
 if (object instanceof IsoWorldInventoryObject || object.getContainer() != nullptr) {
 ...
 ItemContainer container1 = object1 != nullptr ? object1.getContainerByIndex(byte2) : nullptr;
 ...
 IsoObject object1 = square.getObjects().get(byte1);
 container = object1 != nullptr ? object1.getContainerByIndex(byte2) : nullptr;
 } else if (short0 == 3) {
 short short1 = byteBufferReader.getShort();
 uint8_t byte3 = byteBufferReader.getByte();
 BaseVehicle vehicle = VehicleManager.instance.getVehicleByID(short1);
 ...
 try {
 std::vector arrayList = CompressIdenticalItems.load(byteBufferReader.bb, 195, nullptr, nullptr);
 ...
 ((IsoMannequin)container.getParent()).wearItem(item, nullptr);
 }
 }
 }
 }
 ...
 int int5 = byteBuffer.getInt();
 if (square != nullptr && byte3 >= 0 && byte3 < square.getStaticMovingObjects().size()) {
 ...
 if (worldInventoryObject.getItem() != nullptr && worldInventoryObject.getItem().id == int6) {
 ...
 void sendPlayer(IsoPlayer isoPlayer) {
 ...
 static IsoZombie getZombie(short id) {
 ...
 if (boolean0) {
 std::string string3 = ServerOptions.getInstance().getOption("ServerWelcomeMessage");
 ...
 if (SteamUtils.isSteamModeEnabled()) {
 std::string string2 = SteamUtils.convertSteamIDToString(byteBuffer.getLong());
 ...
 getCustomModData();
 std::string string2 = ServerOptions.getInstance().getOption("ServerWelcomeMessage");
 ...
 return byte0 >= 0 && byte0 < square.getObjects().size() ? square.getObjects().get(byte0) : nullptr;
 }
 }
 ...
 ((IsoLightSwitch)square.getObjects().get(byte0).receiveSyncCustomizedSettings(bb, nullptr);
 } else {
 ...
 DebugLog.log("Sync Lightswitch custom settings: sq is nullptr x,y,z=" + int0 + "," + int1 + "," + int2);
 }
 }
 ...
 if (byte0 >= 0 && byte0 < square.getObjects().size()) {
 square.getObjects().get(byte0).syncIsoObject(true, byte2, nullptr, byteBuffer);
 } else {
 ...
 if (worldInventoryObject != nullptr && worldInventoryObject.getItem() instanceof AlarmClock && worldInventoryObject.getItem().id == int7) {
 ...
 void convertGameSaveWorldDirectory(const std::string& string0, const std::string& string1) {
 ...
 void doConnect(const std::string& string0, const std::string& string1, const std::string& string2, const std::string& string3, const std::string& string4, const std::string& string5, const std::string& string6, bool boolean0) {
 ...
 void sendWorldSound(void* source, int x, int y, int z, int radius, int volume, bool stressHumans, float zombieIgnoreDist, float stressMod) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 PacketTypes.PacketType.WorldSound.doPacket(byteBufferWriter);
 ...
 float float1 = byteBuffer.getFloat();
 bool boolean1 = byteBuffer.get() == 1;
 WorldSoundManager.instance.addSound(nullptr, int0, int1, int2, int3, int4, boolean0, float0, float1, boolean1, false, true);
 }
 ...
 void sendClientCommand(IsoPlayer player, const std::string& module, const std::string& command, KahluaTable args) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 void sendClientCommandV(IsoPlayer player, const std::string& module, const std::string& command, Object... objects) {
 if (objects.length == 0) {
 ...
 void sendClothing(IsoPlayer player, const std::string& location, InventoryItem item) {
 ...
 void sendAttachedItem(IsoPlayer player, const std::string& location, InventoryItem item) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 switch (zombieSound) {
 case Burned:
 std::string string4 = zombie0.isFemale() ? "FemaleZombieDeath" : "MaleZombieDeath";
 zombie0.getEmitter().playVocals(string4);
 break;
 case DeadCloseKilled:
 zombie0.getEmitter().playSoundImpl("HeadStab", nullptr);
 std::string string3 = zombie0.isFemale() ? "FemaleZombieDeath" : "MaleZombieDeath";
 ...
 std::string string2 = zombie0.isFemale() ? "FemaleZombieDeath" : "MaleZombieDeath";
 ...
 break;
 case Idle:
 std::string string1 = zombie0.isFemale() ? "FemaleZombieIdle" : "MaleZombieIdle";
 zombie0.getEmitter().playVocals(string1);
 break;
 case Lunge:
 std::string string0 = zombie0.isFemale() ? "FemaleZombieAttack" : "MaleZombieAttack";
 ...
 void sendBandage(int onlineID, int i, bool bandaged, float bandageLife, bool isAlcoholic, const std::string& bandageType) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 void sendStitch(IsoGameCharacter wielder, IsoGameCharacter target, BodyPart bodyPart, InventoryItem item, bool doIt) {
 Stitch stitch = new Stitch();
 ...
 void sendDisinfect(IsoGameCharacter wielder, IsoGameCharacter target, BodyPart bodyPart, InventoryItem alcohol) {
 Disinfect disinfect = new Disinfect();
 ...
 void sendSplint(int onlineID, int i, bool doIt, float factor, const std::string& splintItem) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 void sendRemoveGlass(IsoGameCharacter wielder, IsoGameCharacter target, BodyPart bodyPart, bool handPain) {
 RemoveGlass removeGlass = new RemoveGlass();
 ...
 void sendRemoveBullet(IsoGameCharacter wielder, IsoGameCharacter target, BodyPart bodyPart) {
 RemoveBullet removeBullet = new RemoveBullet();
 ...
 void sendCleanBurn(IsoGameCharacter _wielder, IsoGameCharacter _target, BodyPart bodyPart, InventoryItem _bandage) {
 CleanBurn cleanBurn = new CleanBurn();
 ...
 if (container.getContainingItem() != nullptr && container.getContainingItem().getWorldItem() != nullptr) {
 ...
 if (!SystemDisabler.doWorldSyncEnable && !this->itemsToSendRemove.empty() && boolean0) {
 ...
 if (container0.getContainingItem() != nullptr && container0.getContainingItem().getWorldItem() != nullptr) {
 ...
 if (container1.getContainingItem() != nullptr && container1.getContainingItem().getWorldItem() != nullptr) {
 ...
 try {
 CompressIdenticalItems.save(byteBufferWriter1.bb, arrayList1, nullptr);
 } catch (Exception exception1) {
 exception1.printStackTrace();
 }
 ...
 if (item != nullptr) {
 if (item.getWorldItem() != nullptr && item.getWorldItem().getWorldObjectIndex() != -1) {
 ...
 void StopSound(IsoMovingObject object, const std::string& soundName, bool trigger) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 std::string string2 = System.getProperty("java.home") + string0 + "bin" + string0 + "java";
 ...
 void sendCataplasm(int onlineID, int i, float plantainFactor, float comfreyFactor, float garlicFactor) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 static void sendPlayerListensChannel(int channel, bool listenmode, bool isTV) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 void addUserlog(const std::string& user, const std::string& type, const std::string& text) {
 if (canSeePlayerStats()) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 void removeUserlog(const std::string& user, const std::string& type, const std::string& text) {
 ...
 void addWarningPoint(const std::string& user, const std::string& reason, int amount) {
 if (canModifyPlayerStats()) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 static bool sendItemListNet(IsoPlayer sender, ArrayList<InventoryItem> items, IsoPlayer receiver, const std::string& sessionID, const std::string& custom) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 void tradingUISendAddItem(IsoPlayer you, IsoPlayer other, InventoryItem item) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 int sendInventoryPutItems(ByteBufferWriter byteBufferWriter, InventoryItem> linkedHashMap0, long long0) {
 ...
 for (int int1 = 0; int1 < int0; int1++) {
 KahluaTable table1 = LuaManager.platform.newTable();
 std::string string0 = GameWindow.ReadStringUTF(byteBuffer) + "." + GameWindow.ReadStringUTF(byteBuffer);
 long long0 = byteBuffer.getLong();
 ...
 static void sendZombieHelmetFall(IsoPlayer player, IsoGameCharacter zombie, InventoryItem item) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 static void sendSneezingCoughing(short playerId, int sneezingCoughing, uint8_t sneezeVar) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 static void rememberPlayerPosition(IsoPlayer player, float float0, float float1) {
 if (player != nullptr && !player.isLocalPlayer()) {
