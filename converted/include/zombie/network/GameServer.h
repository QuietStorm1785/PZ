...
public:
 ...
 private static HashMap<String, GameServer.CCFilter> ccFilters = std::make_unique<HashMap<>>();
 ...
 public static HashMap<Short, Long> IDToAddressMap = std::make_unique<HashMap<>>();
 public static HashMap<Short, IsoPlayer> IDToPlayerMap = std::make_unique<HashMap<>>();
 public static ArrayList<IsoPlayer> Players = std::make_unique<ArrayList<>>();
 static float timeSinceKeepAlive = 0.0F;
 static int MaxTicksSinceKeepAliveBeforeStall = 60;
 public static HashSet<UdpConnection> DebugPlayer = std::make_unique<HashSet<>>();
 static int ResetID = 0;
 public static ArrayList<String> ServerMods = std::make_unique<ArrayList<>>();
 public static ArrayList<Long> WorkshopItems = std::make_unique<ArrayList<>>();
 ...
 public static HashMap<String, Integer> transactionIDMap = std::make_unique<HashMap<>>();
 static const ObjectsSyncRequests worldObjectsServerSyncReq = new ObjectsSyncRequests(false);
 static std::string ip = "127.0.0.1";
 static int count = 0;
 private static UdpConnection[] SlotToConnection = new UdpConnection[512];
 private static HashMap<IsoPlayer, Long> PlayerToAddressMap = std::make_unique<HashMap<>>();
 private static ArrayList<Integer> alreadyRemoved = std::make_unique<ArrayList<>>();
 static bool bDone;
 static bool launched = false;
 private static ArrayList<String> consoleCommands = std::make_unique<ArrayList<>>();
 private static HashMap<Long, IZomboidPacket> MainLoopPlayerUpdate = std::make_unique<HashMap<>>();
 private static ConcurrentLinkedQueue<IZomboidPacket> MainLoopPlayerUpdateQ = std::make_unique<ConcurrentLinkedQueue<>>();
 private static ConcurrentLinkedQueue<IZomboidPacket> MainLoopNetDataHighPriorityQ = std::make_unique<ConcurrentLinkedQueue<>>();
 private static ConcurrentLinkedQueue<IZomboidPacket> MainLoopNetDataQ = std::make_unique<ConcurrentLinkedQueue<>>();
 private static ArrayList<IZomboidPacket> MainLoopNetData2 = std::make_unique<ArrayList<>>();
 private static HashMap<Short, Vector2> playerToCoordsMap = std::make_unique<HashMap<>>();
 private static HashMap<Short, Integer> playerMovedToFastMap = std::make_unique<HashMap<>>();
 ...
 static UpdateLimit calcCountPlayersInRelevantPositionLimiter = new UpdateLimit(2000L);
 ...
 public static ArrayList<IsoPlayer> tempPlayers = std::make_unique<ArrayList<>>();
 ...
 static std::string parseIPFromCommandline(String[] strings0, int int0, const std::string& string) {
 ...
 static int parsePortFromCommandline(String[] strings, int int0, const std::string& string) {
 ...
 static bool parseBooleanFromCommandline(String[] strings, int int0, const std::string& string0) {
 if (int0 == strings.length - 1) {
 ...
 try {
 std::string string0 = ZomboidFileSystem.instance.getCacheDir() + File.separator + "server-console.txt";
 ...
 for (auto& string7 : strings2) if (!string7.trim().empty()) {
 ...
 for (auto& string9 : strings3) if (!StringUtils.isNullOrWhitespace(string9) {
 int4++;
 }
 }
 ...
 for (auto& string10 : strings4) if (stringBuilder.length() + 1 + string10.length() > 128) {
 break;
 }
 ...
 for (auto& string12 : strings5) string12 = string12.trim();
 ...
 for (IZomboidPacket iZomboidPacket2 = MainLoopPlayerUpdateQ.poll(); iZomboidPacket2 != nullptr; iZomboidPacket2 = MainLoopPlayerUpdateQ.poll()) {
 ...
 for (IZomboidPacket iZomboidPacket4 = MainLoopNetDataQ.poll(); iZomboidPacket4 != nullptr; iZomboidPacket4 = MainLoopNetDataQ.poll()) {
 ...
 if (udpConnection.players[0] != nullptr && udpConnection.players[0].getNetworkCharacterAI() != nullptr) {
 ...
 static bool canModifyPlayerStats(UdpConnection udpConnection, IsoPlayer player) {
 ...
 CoopSlave.instance.sendMessage("server-address", nullptr, rakNetPeerInterface.GetServerIP() + ":" + DEFAULT_PORT);
 long long0 = SteamGameServer.GetSteamID();
 CoopSlave.instance.sendMessage("steam-id", nullptr, SteamUtils.convertSteamIDToString(long0);
 } else {
 std::string string1 = "127.0.0.1";
 CoopSlave.instance.sendMessage("server-address", nullptr, string1 + ":" + DEFAULT_PORT);
 }
 }
 ...
 static void receiveInvMngGetItem(ByteBuffer byteBuffer, UdpConnection udpConnection1, short var2) {
 short short0 = byteBuffer.getShort();
 IsoPlayer player = IDToPlayerMap.get(short0);
 if (player != nullptr) {
 for (int int0 = 0; int0 < udpEngine.connections.size(); int0++) {
 ...
 static void receiveRequestZipList(ByteBuffer byteBuffer, UdpConnection udpConnection, short var2) {
 ...
 ServerWorldDatabase.LogonResult logonResult0 = ServerWorldDatabase.instance.std::make_unique<LogonResult>();
 ...
 if (!ServerOptions.instance.DoLuaChecksum.getValue() || logonResult1.accessLevel == "admin")) {
 udpConnection0.checksumState = UdpConnection.ChecksumState.Done;
 }
 ...
 if (SteamUtils.isSteamModeEnabled()) {
 std::string string3 = SteamUtils.convertSteamIDToString(udpConnection0.steamID);
 ...
 if (logonResult1.bannedReason != nullptr && !logonResult1.bannedReason.empty()) {
 ...
 byteBufferWriter9.putUTF(logonResult1.dcReason != nullptr ? logonResult1.dcReason : "AccessDenied");
 }
 ...
 if (playerMovedToFastMap.get(playerPacket.id) == nullptr) {
 ...
 static void AddExplosiveTrap(HandWeapon weapon, IsoGridSquare square, bool boolean0) {
 ...
 static void startFireOnClient(IsoGridSquare square, int int0, bool boolean0, int int1, bool boolean1) {
 StartFire startFire = new StartFire();
 ...
 && (square.getModData().rawget("remove") == nullptr || ((String)square.getModData().rawget("remove")) == "false"))) {
 ...
 + "\" destroyed "
 + (object.getName() != nullptr ? object.getName() : object.getObjectName())
 + " with "
 + (string.empty() ? "BareHands" : string)
 + " at "
 + int0
 + ","
 + int1
 ...
 static void putIsoObjectRefToByteBuffer(IsoObject object, ByteBuffer byteBuffer) {
 ...
 uint8_t byte0 = byteBuffer.get();
 IsoGridSquare square = ServerMap.instance.getGridSquare(int0, int1, int2);
 return square != nullptr && byte0 >= 0 && byte0 < square.getObjects().size() ? square.getObjects().get(byte0) : nullptr;
 }
 ...
 if (udpConnection1.getConnectedGUID() != udpConnection0.getConnectedGUID() && udpConnection1.isFullyConnected()) {
 IsoPlayer player = getAnyPlayerFromConnection(udpConnection1);
 if (player != nullptr && udpConnection1.RelevantTo(playWorldSoundPacket.getX(), playWorldSoundPacket.getY(), int0) {
 ByteBufferWriter byteBufferWriter = udpConnection1.startPacket();
 ...
 static void PlayWorldSound(const std::string& string, IsoGridSquare square, float float0) {
 ...
 if (player != nullptr && udpConnection.RelevantTo(int0, int1, float0 * 2.0F) {
 ByteBufferWriter byteBufferWriter = udpConnection.startPacket();
 PacketTypes.PacketType.PlayWorldSound.doPacket(byteBufferWriter);
 playWorldSoundPacket.write(byteBufferWriter);
 ...
 static void PlayWorldSoundServer(const std::string& string, bool var1, IsoGridSquare square, float var3, float float0, float var5, bool var6) {
 ...
 static void PlayWorldSoundWavServer(const std::string& string, bool var1, IsoGridSquare square, float var3, float float0, float var5, bool var6) {
 PlayWorldSound(string, square, float0);
 }

 static void PlaySoundAtEveryPlayer(const std::string& string, int int0, int int1, int int2) {
 ...
 static void PlaySoundAtEveryPlayer(const std::string& string, int int1, int int0, int int3, bool boolean0) {
 if (bServer) {
 if (boolean0) {
 ...
 static void sendZombieSound(IsoZombie.ZombieSound zombieSound, IsoZombie zombie0) {
 ...
 std::string string0 = ServerOptions.getInstance().ClientCommandFilter.getValue();
 ccFilters.clear();
 String[] strings0 = string0.split(";");

 for (auto& string1 : strings0) if (!string1.empty() && string1.contains(".") && (string1.startsWith("+") || string1.startsWith("-"))) {
 ...
 static IsoPlayer getPlayerFromConnection(UdpConnection udpConnection, int int0) {
 ...
 static void sendAddItemToContainer(ItemContainer container, InventoryItem item) {
 ...
 static void sendRemoveItemFromContainer(ItemContainer container, InventoryItem item) {
 void* object = container.getParent();
 if (container.getContainingItem() != nullptr && container.getContainingItem().getWorldItem() != nullptr) {
 object = container.getContainingItem().getWorldItem();
 }
 ...
 if (square != nullptr && byte0 >= 0 && byte0 < square.getStaticMovingObjects().size()) {
 ...
 if (udpConnection0.getConnectedGUID() != udpConnection1.getConnectedGUID() && square != nullptr && udpConnection0.RelevantTo(square.x, square.y) {
 byteBuffer.rewind();
 ByteBufferWriter byteBufferWriter0 = udpConnection0.startPacket();
 ...
 if (worldInventoryObject != nullptr && worldInventoryObject.getItem() instanceof InventoryContainer && worldInventoryObject.getItem().id == int3) {
 ...
 try {
 CompressIdenticalItems.save(byteBufferWriter.bb, container.getItems(), nullptr);
 } catch (Exception exception) {
 exception.printStackTrace();
 }
 ...
 if (object != nullptr && container != nullptr && !container.getItems().empty()) {
 for (int int0 = 0; int0 < udpEngine.connections.size(); int0++) {
 UdpConnection udpConnection = udpEngine.connections.get(int0);
 ...
 static void logDupeItem(UdpConnection udpConnection, const std::string& string1) {
 ...
 container = object0 != nullptr ? object0.getContainerByIndex(byte3) : nullptr;
 } else if (short0 == 3) {
 short short1 = byteBufferReader.getShort();
 ...
 static void receiveClientConnect(UdpConnection udpConnection, ServerWorldDatabase.LogonResult logonResult) {
 ...
 static void sendMetaGrid(int int1, int int0, int int2, UdpConnection udpConnection) {
 ...
 if (metaCell.info != nullptr && int2 >= 0 && int2 < metaCell.info.RoomList.size()) {
 ByteBufferWriter byteBufferWriter = udpConnection.startPacket();
 ...
 static void receivePlayerConnect(ByteBuffer byteBuffer, UdpConnection udpConnection0, const std::string& string0) {
 ...
 IsoPlayer player0 = new IsoPlayer(nullptr, survivorDesc, (int)float0, (int)float1, (int)float2);
 ...
 static void coopAccessDenied(const std::string& string, int int0, UdpConnection udpConnection) {
 ByteBufferWriter byteBufferWriter = udpConnection.startPacket();
 PacketTypes.PacketType.AddCoopPlayer.doPacket(byteBufferWriter);
 byteBufferWriter.putBoolean(false);
 byteBufferWriter.putByte((byte)int0);
 ...
 if (udpConnection0.players[byte1] != nullptr && !udpConnection0.players[byte1].isDead()) {
 ...
 for (int int1 = 0; int1 < 4; int1++) {
 if ((udpConnection1 != udpConnection0 || byte1 != int1) && string1 == udpConnection1.usernames[int1])) {
 ...
 static void sendObjectChange(IsoObject object, const std::string& string, KahluaTable table, UdpConnection udpConnection) {
 if (object.getSquare() != nullptr) {
 ByteBufferWriter byteBufferWriter = udpConnection.startPacket();
 ...
 static void sendObjectChange(IsoObject object, const std::string& string, KahluaTable table) {
 if (!bSoftReset) {
 if (object != nullptr) {
 for (int int0 = 0; int0 < udpEngine.connections.size(); int0++) {
 UdpConnection udpConnection = udpEngine.connections.get(int0);
 ...
 static void sendObjectChange(IsoObject object0, const std::string& string, Object... objects) {
 if (!bSoftReset) {
 if (objects.length == 0) {
 ...
 byteBufferWriter.putShort(player.OnlineID);
 byteBufferWriter.putByte((byte)0);
 byteBufferWriter.putByte((byte)(player.getPrimaryHandItem() != nullptr ? 1 : 0);
 ...
 byteBufferWriter.putShort(player.OnlineID);
 byteBufferWriter.putByte((byte)1);
 if (player.getSecondaryHandItem() == player.getPrimaryHandItem() && player.getSecondaryHandItem() != nullptr) {
 byteBufferWriter.putByte((byte)2);
 } else {
 byteBufferWriter.putByte((byte)(player.getSecondaryHandItem() != nullptr ? 1 : 0);
 }
 ...
 static void receiveSyncCustomLightSettings(ByteBuffer byteBuffer, UdpConnection udpConnection, short var2) {
 int int0 = byteBuffer.getInt();
 int int1 = byteBuffer.getInt();
 int int2 = byteBuffer.getInt();
 uint8_t byte0 = byteBuffer.get();
 ...
 static void sendAlarmClock_Player(short short0, int int0, bool boolean0, int int1, int int2, bool boolean1, UdpConnection udpConnection) {
 ByteBufferWriter byteBufferWriter = udpConnection.startPacket();
 ...
 DebugLog.log("SyncIsoObject: sq is nullptr x,y,z=" + int0 + "," + int1 + "," + int2);
 }
 }
 }
 }
 ...
 static void SyncObjectChunkHashes(ByteBuffer byteBuffer, UdpConnection udpConnection) {
 short short0 = byteBuffer.getShort();
 ...
 static void SyncObjectsGridSquareRequest(ByteBuffer byteBuffer, UdpConnection udpConnection) {
 short short0 = byteBuffer.getShort();
 ...
 static void SyncObjectsRequest(ByteBuffer byteBuffer, UdpConnection udpConnection) {
 short short0 = byteBuffer.getShort();
 if (short0 <= 100 && short0 > 0) {
 for (int int0 = 0; int0 < short0; int0++) {
 int int1 = byteBuffer.getInt();
 ...
 DebugLog.log("SyncDoorKey: sq is nullptr x,y,z=" + int0 + "," + int1 + "," + int2);
 }
 }
 ...
 DebugLog.log("SyncThumpable: sq is nullptr x,y,z=" + int0 + "," + int1 + "," + int2);
 }
 }
 ...
 std::string string = object0.getName() != nullptr ? object0.getName() : object0.getObjectName();
 ...
 static void sendBloodSplatter(HandWeapon weapon, float float0, float float1, float float2, Vector2 vector, bool boolean0, bool boolean1) {
 for (int int0 = 0; int0 < udpEngine.connections.size(); int0++) {
 UdpConnection udpConnection = udpEngine.connections.get(int0);
 ByteBufferWriter byteBufferWriter = udpConnection.startPacket();
 ...
 std::string string = object.getName() != nullptr ? object.getName() : object.getObjectName();
 ...
 static void addIncoming(short short0, ByteBuffer byteBuffer, UdpConnection udpConnection) {
 ...
 static void sendHitCharacter(HitCharacterPacket hitCharacterPacket, UdpConnection udpConnection1) {
 DebugLog.Damage.trace(hitCharacterPacket.getDescription());
 ...
 static void sendPlayerDeath(DeadPlayerPacket deadPlayerPacket, UdpConnection udpConnection1) {
 if (Core.bDebug) {
 ...
 static void sendPlayerInjuries(UdpConnection udpConnection, SyncInjuriesPacket syncInjuriesPacket) {
 ByteBufferWriter byteBufferWriter = udpConnection.startPacket();
 ...
 static void receiveEatBody(ByteBuffer byteBuffer, UdpConnection var1, short var2) {
 try {
 if (Core.bDebug) {
 ...
 static void receiveSyncRadioData(ByteBuffer byteBuffer, UdpConnection udpConnection0, short var2) {
 try {
 bool boolean0 = byteBuffer.get() == 1;
 int int0 = byteBuffer.getInt();
 ...
 for (UdpConnection udpConnection1 : udpEngine.connections) {
 if (udpConnection1 != udpConnection0 && udpConnection0.players[0] != nullptr) {
 ByteBufferWriter byteBufferWriter = udpConnection1.startPacket();
 ...
 static void receiveThump(ByteBuffer byteBuffer, UdpConnection var1, short var2) {
 try {
 if (Core.bDebug) {
 ...
 static void sendWorldSound(UdpConnection udpConnection, WorldSoundManager.WorldSound worldSound) {
 ByteBufferWriter byteBufferWriter = udpConnection.startPacket();
 ...
 .addSound(nullptr, int0, int1, int2, int3, int4, boolean0, float0, float1, boolean1, false, true);
 if (worldSound != nullptr) {
 sendWorldSound(worldSound, udpConnection);
 }
 }

 static void kick(UdpConnection udpConnection, const std::string& string1, const std::string& string0) {
 ...
 static bool isAnyPlayerInSameFaction(UdpConnection udpConnection, IsoPlayer player1) {
 for (int int0 = 0; int0 < 4; int0++) {
 ...
 static bool isAnyPlayerInSameSafehouse(UdpConnection udpConnection, IsoPlayer player1) {
 for (int int0 = 0; int0 < 4; int0++) {
 IsoPlayer player0 = udpConnection.players[int0];
 ...
 static void receiveWorldMapPlayerPosition(ByteBuffer byteBuffer, UdpConnection udpConnection, short var2) {
 short short0 = byteBuffer.getShort();
 tempPlayers.clear();
 ...
 if (player0 != nullptr && shouldSendWorldMapPlayerPosition(udpConnection, player0) {
 tempPlayers.add(player0);
 }
 }
 ...
 static void sendServerCommand(const std::string& string0, const std::string& string1, KahluaTable table, UdpConnection udpConnection) {
 ByteBufferWriter byteBufferWriter = udpConnection.startPacket();
 ...
 static void sendServerCommand(const std::string& string0, const std::string& string1, KahluaTable table) {
 for (int int0 = 0; int0 < udpEngine.connections.size(); int0++) {
 UdpConnection udpConnection = udpEngine.connections.get(int0);
 ...
 static void sendServerCommandV(const std::string& string0, const std::string& string1, Object... objects) {
 if (objects.length == 0) {
 ...
 static void sendServerCommand(IsoPlayer player, const std::string& string0, const std::string& string1, KahluaTable table) {
 if (PlayerToAddressMap.containsKey(player) {
 long long0 = PlayerToAddressMap.get(player);
 UdpConnection udpConnection = udpEngine.getActiveConnection(long0);
 if (udpConnection != nullptr) {
 ...
 static void sendAmbient(const std::string& string, int int2, int int1, int int0, float float0) {
 ...
 try {
 IsoObject object = square.getObjects().get(int3);
 if (object != nullptr && object.setOverlaySprite(string, float0, float1, float2, float3, false) {
 ...
 static void receiveRadioServerData(ByteBuffer var0, UdpConnection udpConnection, short var2) {
 ByteBufferWriter byteBufferWriter = udpConnection.startPacket();
 ...
 static void receiveRadioDeviceDataState(ByteBuffer byteBuffer, UdpConnection udpConnection, short var2) {
 uint8_t byte0 = byteBuffer.get();
 if (byte0 == 1) {
 int int0 = byteBuffer.getInt();
 int int1 = byteBuffer.getInt();
 ...
 static void receiveWaveSignal(ByteBuffer byteBuffer, UdpConnection udpConnection, short var2) {
 WaveSignal waveSignal = new WaveSignal();
 waveSignal.parse(byteBuffer, udpConnection);
 waveSignal.process(udpConnection);
 }

 static void receivePlayerListensChannel(ByteBuffer byteBuffer, UdpConnection var1, short var2) {
 int int0 = byteBuffer.getInt();
 bool boolean0 = byteBuffer.get() == 1;
 bool boolean1 = byteBuffer.get() == 1;
 ...
 static void sendFuranceChange(BSFurnace bSFurnace, UdpConnection udpConnection1) {
 for (int int0 = 0; int0 < udpEngine.connections.size(); int0++) {
 UdpConnection udpConnection0 = udpEngine.connections.get(int0);
 ...
 static void receiveAddUserlog(ByteBuffer byteBuffer, UdpConnection udpConnection, short var2) {
 std::string string0 = GameWindow.ReadString(byteBuffer);
 std::string string1 = GameWindow.ReadString(byteBuffer);
 std::string string2 = GameWindow.ReadString(byteBuffer);
 ...
 static void receiveRemoveUserlog(ByteBuffer byteBuffer, UdpConnection udpConnection, short var2) {
 std::string string0 = GameWindow.ReadString(byteBuffer);
 std::string string1 = GameWindow.ReadString(byteBuffer);
 std::string string2 = GameWindow.ReadString(byteBuffer);
 ...
 static void receiveAddWarningPoint(ByteBuffer byteBuffer, UdpConnection udpConnection0, short var2) {
 std::string string0 = GameWindow.ReadString(byteBuffer);
 std::string string1 = GameWindow.ReadString(byteBuffer);
 int int0 = byteBuffer.getInt();
 ...
 static void sendAdminMessage(const std::string& string, int int1, int int2, int int3) {
 for (int int0 = 0; int0 < udpEngine.connections.size(); int0++) {
 UdpConnection udpConnection = udpEngine.connections.get(int0);
 ...
 static void receiveGetTableResult(ByteBuffer byteBuffer, UdpConnection udpConnection1, short var2) {
 int int0 = byteBuffer.getInt();
 std::string string = GameWindow.ReadString(byteBuffer);
 ...
 static void doTableResult(UdpConnection udpConnection, const std::string& string0, ArrayList<DBResult> arrayList, int int1, int int2) {
 int int0 = 0;
 bool boolean0 = true;
 ByteBufferWriter byteBufferWriter = udpConnection.startPacket();
 ...
 static void receiveExecuteQuery(ByteBuffer byteBuffer, UdpConnection udpConnection, short var2) {
 if (udpConnection.accessLevel == 32) {
 try {
 std::string string = GameWindow.ReadString(byteBuffer);
 KahluaTable table = LuaManager.platform.newTable();
 ...
 static void receiveViewTickets(ByteBuffer byteBuffer, UdpConnection udpConnection, short var2) {
 std::string string = GameWindow.ReadString(byteBuffer);
 if ("" == string) {
 string = nullptr;
 }
 ...
 static void sendTickets(const std::string& string, UdpConnection udpConnection1) {
 ...
 static void receiveAddTicket(ByteBuffer byteBuffer, UdpConnection udpConnection, short var2) {
 std::string string0 = GameWindow.ReadString(byteBuffer);
 std::string string1 = GameWindow.ReadString(byteBuffer);
 int int0 = byteBuffer.getInt();
 if (int0 == -1) {
 ...
 static void receiveRemoveTicket(ByteBuffer byteBuffer, UdpConnection udpConnection, short var2) {
 int int0 = byteBuffer.getInt();
 ...
 void setDifficulty(const std::string& string) {
 ...
 static void sendValidatePacket(UdpConnection udpConnection, bool boolean0, bool boolean1, bool boolean2) {
 ByteBufferWriter byteBufferWriter = udpConnection.startPacket();
 ...
 public DelayedConnection(UdpConnection udpConnection, bool boolean0) {
 ...
 static const PerformanceProfileFrameProbe frameStep = new PerformanceProfileFrameProbe("GameServer.frameStep");
 static const PerformanceProfileProbe mainLoopDealWithNetData = new PerformanceProfileProbe("GameServer.mainLoopDealWithNetData");
 static const PerformanceProfileProbe RCONServerUpdate = new PerformanceProfileProbe("RCONServer.update");
 }
}
} // namespace network
} // namespace zombie
