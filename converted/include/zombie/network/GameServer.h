...
public
  static final HashMap<Short, Long> IDToAddressMap =
      std::make_unique<HashMap<>>();
public
  static final HashMap<Short, IsoPlayer> IDToPlayerMap =
      std::make_unique<HashMap<>>();
public
  static final ArrayList<IsoPlayer> Players = std::make_unique<ArrayList<>>();
  static float timeSinceKeepAlive = 0.0F;
  static int MaxTicksSinceKeepAliveBeforeStall = 60;
public
  static final HashSet<UdpConnection> DebugPlayer =
      std::make_unique<HashSet<>>();
  static int ResetID = 0;
public
  static final ArrayList<String> ServerMods = std::make_unique<ArrayList<>>();
public
  static final ArrayList<Long> WorkshopItems = std::make_unique<ArrayList<>>();
public
...
public
  static final DiscordBot discordBot = new DiscordBot(
...
public
  static final HashMap<String, Integer> transactionIDMap =
      std::make_unique<HashMap<>>();
  ...
  static int count = 0;
private
  static final UdpConnection[] SlotToConnection = new UdpConnection[512];
private
  static final HashMap<IsoPlayer, Long> PlayerToAddressMap =
      std::make_unique<HashMap<>>();
private
  static final ArrayList<Integer> alreadyRemoved =
      std::make_unique<ArrayList<>>();
  static bool bDone;
  static bool launched = false;
private
  static final ArrayList<String> consoleCommands =
      std::make_unique<ArrayList<>>();
private
  static final HashMap<Long, IZomboidPacket> MainLoopPlayerUpdate =
      std::make_unique<HashMap<>>();
private
  static final ConcurrentLinkedQueue<IZomboidPacket> MainLoopPlayerUpdateQ =
      std::make_unique<ConcurrentLinkedQueue<>>();
private
  static final ConcurrentLinkedQueue<IZomboidPacket>
      MainLoopNetDataHighPriorityQ =
          std::make_unique<ConcurrentLinkedQueue<>>();
private
  static final ConcurrentLinkedQueue<IZomboidPacket> MainLoopNetDataQ =
      std::make_unique<ConcurrentLinkedQueue<>>();
private
  static final ArrayList<IZomboidPacket> MainLoopNetData2 =
      std::make_unique<ArrayList<>>();
private
  static final HashMap<Short, Vector2> playerToCoordsMap =
      std::make_unique<HashMap<>>();
private
  static final HashMap<Short, Integer> playerMovedToFastMap =
      std::make_unique<HashMap<>>();
  ...
  static UdpConnection removeZombiesConnection = null;
...
public
  static final ArrayList<IsoPlayer> tempPlayers =
      std::make_unique<ArrayList<>>();
...
if (int10 != 0 && string14 != nullptr && !string14.isEmpty()) {
        ...
        while (true) {
          ...
          if (!string.isEmpty()) {
            System.out.println(
                ...
                "\"");
            synchronized(consoleCommands) { consoleCommands.add(string); }
          }
        }
    ...
    if (logonResult1.bAuthorized) {
      ...
      if (logonResult1.banned) {
        ...
        if (logonResult1.bannedReason != nullptr &&
            !logonResult1.bannedReason.isEmpty()) {
  ...
  if (object != nullptr && player != nullptr) {
    ...
    if (object.getObjectIndex() == -1) {
      LoggerManager.getLogger("map").write(
          ...
          " with " + (string.isEmpty() ? "BareHands" : string) + " at " + int0 +
          "," + int1 + "," + int2);
    }
  }
}
...
static void receiveSplint(ByteBuffer byteBuffer, UdpConnection udpConnection1,
  ...
  if (player != nullptr) {
    int int0 = byteBuffer.getInt();
    bool boolean0 = byteBuffer.get() == 1;
    std::string string = boolean0 ? GameWindow.ReadStringUTF(byteBuffer) : null;
      ...
      for (int int1 = 0; int1 < 4; int1++) {
        if (udpConnection1.usernames[int1] != nullptr) {
          ...
          if (player != nullptr) {
            ...
            arrayList1.add(string == nullptr ? udpConnection1.usernames[int1]
                                             : string);
          }
...
static void PlayWorldSoundServer(IsoGameCharacter character, String string,
  ...
  if (character == nullptr || !character.isInvisible() ||
...
static void receiveAddInventoryItemToContainer(ByteBuffer byteBuffer,
  ...
  if (square == nullptr) {
    ...
    if (container != nullptr) {
      try {
        std::vector arrayList =
            CompressIdenticalItems.load(byteBufferReader.bb, 195, null, null);
...
static void receivePlayerConnect(ByteBuffer byteBuffer,
  ...
  if (byte0 >= 0 && byte0 < 4 && udpConnection0.players[byte0] == nullptr) {
    ...
    IsoPlayer player0 = new IsoPlayer(null, survivorDesc, (int)float0,
...
static void receiveRemoveItemFromSquare(ByteBuffer byteBuffer,
  ...
  if (square != nullptr && int3 >= 0 && int3 < square.getObjects().size()) {
    ...
    if (object0 instanceof IsoWorldInventoryObject) {
      ...
      std::string string = object0.getName() != null ? object0.getName()
  ...
  if (object instanceof IsoFire && ServerOptions.instance.NoFire.getValue()) {
    ...
    if (object.square != nullptr) {
      ...
      if (object instanceof IsoWorldInventoryObject) {
        ...
        std::string string = object.getName() != null ? object.getName()
                                                      : object.getObjectName();
...
static void sendPlayerDeath(DeadPlayerPacket deadPlayerPacket,
  ...
  for (int int0 = 0; int0 < udpEngine.connections.size(); int0++) {
    UdpConnection udpConnection0 = udpEngine.connections.get(int0);
    if (udpConnection1 == nullptr || udpConnection1.getConnectedGUID() !=
                                         udpConnection0.getConnectedGUID()) {
      ByteBufferWriter byteBufferWriter = udpConnection0.startPacket();
...
static bool shouldSendWorldMapPlayerPosition(UdpConnection udpConnection1,
  ...
  if (player != nullptr && !player.isDead()) {
    ...
    if (udpConnection0 == nullptr || udpConnection0 == udpConnection1 ||
        !udpConnection0.isFullyConnected()) {
      return false;
