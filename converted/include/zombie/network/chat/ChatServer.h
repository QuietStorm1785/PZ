...
class ChatServer {
public:
  static ChatServer instance = null;
private
  static final Stack<Integer> availableChatsID = std::make_unique<Stack<>>();
  static int lastChatId = -1;
private
  static final HashMap<ChatType, ChatBase> defaultChats =
      std::make_unique<HashMap<>>();
private
  static final ConcurrentHashMap<Integer, ChatBase> chats =
      std::make_unique<ConcurrentHashMap<>>();
private
  static final ConcurrentHashMap<String, FactionChat> factionChats =
      std::make_unique<ConcurrentHashMap<>>();
private
  static final ConcurrentHashMap<String, SafehouseChat> safehouseChats =
      std::make_unique<ConcurrentHashMap<>>();
  static AdminChat adminChat = null;
  static GeneralChat generalChat = null;
  static ServerChat serverChat = null;
  static RadioChat radioChat = null;
  static bool inited = false;
private
  static final HashSet<Short> players = std::make_unique<HashSet<>>();
  static const std::string logName = "chat";
  static ZLogger logger;
private
  static final HashMap<String, ChatTab> tabs = std::make_unique<HashMap<>>();
...
private
  ...
  void initPlayer(short short0) {
    ...
    if (udpConnection != nullptr && player != nullptr) {
      ...
      synchronized(players) { players.add(short0); }
      ...
      logger.write((udpConnection == nullptr ? "connection = nullptr " : "") +
  ...
  void disconnectPlayer(short short0) {
    ...
    synchronized(players) { players.remove(short0); }
  ...
  void closeChat(int int0) {
    synchronized(chats) {
      if (!chats.containsKey(int0)) {
        throw new RuntimeException("Chat '" + int0 +
    ...
    synchronized(availableChatsID) { availableChatsID.push(int0); }
  }
