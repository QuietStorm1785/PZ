...
public:
 ...
 void addSaveLoadedJob(IsoChunk chunk) {
 ...
 return chunk = = nullptr ? nullptr : chunk.getGridSquare(int0 % 10, int1 % 10, int2);
 } else {
 return nullptr;
 }
 }
 ...
 return int1 >= 0 && int1 < 50 ? this->cell.chunks[int0 / 10][int1 / 10] : nullptr;
 }
 }
 ...
 if (square1 != nullptr) {
 if (int16 > 0 && !square1.getObjects().empty()) {
 ...
 private ArrayList<ServerChunkLoader.SaveTask> toSaveChunk = std::make_unique<ArrayList<>>();
 private ArrayList<ServerChunkLoader.SaveTask> savedChunks = std::make_unique<ArrayList<>>();
 ...
 void quit() {
 this->toThread.add(ServerChunkLoader.this->std::make_unique<QuitThreadTask>());
