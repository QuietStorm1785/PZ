  ...
  static void receiveServerLoginQueueRequest(ByteBuffer var0,
    ...
    synchronized(LoginQueue) {
      if (!ServerOptions.getInstance().LoginQueueEnabled.getValue() ||
          !udpConnection.preferredInQueue && currentLoginQueue == nullptr &&
              PreferredLoginQueue.isEmpty() && LoginQueue.isEmpty() ||
          udpConnection.preferredInQueue && currentLoginQueue == nullptr &&
...
static void loadNextPlayer() {
  if (!PreferredLoginQueue.isEmpty() && currentLoginQueue == nullptr) {
  ...
  if (!LoginQueue.isEmpty() && currentLoginQueue == nullptr) {
