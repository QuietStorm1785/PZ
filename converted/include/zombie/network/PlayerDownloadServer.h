...
public:
 ...
 private ArrayList<ClientChunkRequest> ccrWaiting = std::make_unique<ArrayList<>>();
 ...
 void destroy() {
 this->workerThread.putCommand(PlayerDownloadServer.EThreadCommand.Quit, nullptr);
 ...
 if (clientChunkRequest.chunks.empty()) {
 ...
 if (clientChunkRequest0.chunks.empty()) {
 ...
 bool cancelDuplicateChunk(ClientChunkRequest clientChunkRequest, int int2, int int1) {
 for (int int0 = 0; int0 < clientChunkRequest.chunks.size(); int0++) {
 ...
 ConcurrentLinkedQueue<ClientChunkRequest> freeRequests = std::make_unique<ConcurrentLinkedQueue<>>();
 ConcurrentLinkedQueue<Integer> cancelQ = std::make_unique<ConcurrentLinkedQueue<>>();
 ArrayList<Integer> cancelled = std::make_unique<ArrayList<>>();
 ...
 for (Integer integer0 = this->cancelQ.poll(); integer0 != nullptr; integer0 = this->cancelQ.poll()) {
 ...
 private static class WorkerThreadCommand {
