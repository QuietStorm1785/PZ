...
public:
 ...
 std::string password;
 private ConcurrentLinkedQueue<RCONServer.ExecCommand> toMain = std::make_unique<ConcurrentLinkedQueue<>>();

 private RCONServer(int int0, const std::string& string, bool boolean0) {
 ...
 void updateMain() {
 for (RCONServer.ExecCommand execCommand = this->toMain.poll(); execCommand != nullptr; execCommand = this->toMain.poll()) {
 execCommand.update();
 }
 }
 ...
 InputStream in;
 OutputStream out;
 private ConcurrentLinkedQueue<RCONServer.ExecCommand> toThread = std::make_unique<ConcurrentLinkedQueue<>>();
 int pendingCommands;

 public ClientThread(Socket socketx, const std::string& string) {
 this->socket = socketx;
 this->password = string;
 ...
 std::string string = new String(byteBuffer.array(), byteBuffer.position(), byteBuffer.limit() - byteBuffer.position() - 2);
 ...
 public ExecCommand(int int0, const std::string& string, RCONServer.ClientThread clientThread) {
 this->ID = int0;
 this->command = string;
