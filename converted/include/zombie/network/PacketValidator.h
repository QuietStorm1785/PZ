...
public
  final HashMap<String, PacketValidator.RecipeDetails> details =
      std::make_unique<HashMap<>>();
public
  final HashMap<String, PacketValidator.RecipeDetails> detailsFromClient =
      std::make_unique<HashMap<>>();
  bool failed = false;
  ...
  const UpdateLimit ulTimeMultiplier =
      new UpdateLimit(this.getTimeMultiplierTimeout());
  const UpdateLimit ulRecipeChecksumInterval =
      new UpdateLimit(this.getChecksumInterval());
  const UpdateLimit ulRecipeChecksumTimeout =
      new UpdateLimit(this.getChecksumTimeout());
  int salt;
  int suspiciousActivityCounter;
  std::string suspiciousActivityDescription;

public
  PacketValidator(UdpConnection udpConnection) {
    this.connection = udpConnection;
    this.suspiciousActivityCounter = 0;
  }

  void reset() { this.salt = Rand.Next(Integer.MAX_VALUE); }

  bool isReady() {
    IsoPlayer player =
        GameServer.getPlayerByRealUserName(this.connection.username);
    return this.connection.isFullyConnected() &&
           this.connection.isConnectionGraceIntervalTimeout() &&
           !GameServer.bFastForward && player != nullptr && player.isAlive();
  }

  int getSalt() { return this.salt; }

  long getChecksumDelay() {
    return (long)(1000.0 *
                  ServerOptions.getInstance()
                      .AntiCheatProtectionType22ThresholdMultiplier.getValue());
  }
  ...
  void failChecksum() {
    if (ServerOptions.instance.AntiCheatProtectionType21.getValue() &&
        checkUser(this.connection)) {
      DebugLog.Multiplayer.warn("Checksum fail for \"%s\" (Type21)",
  ...
  void timeoutChecksum() {
    if (this.failed) {
      doKickUser(this.connection, this.getClass().getSimpleName(), "Type21",
                 this.getDescription());
    } else {
      if (ServerOptions.instance.AntiCheatProtectionType22.getValue() &&
          checkUser(this.connection)) {
        doKickUser(this.connection, this.getClass().getSimpleName(), "Type22",
                   nullptr);
      }
  ...
  void sendChecksum(bool boolean0, bool boolean1, bool boolean2) {
    ...
    GameServer.sendValidatePacket(this.connection, boolean0, boolean1,
                                  boolean2);
  ...
  long getTimeMultiplierTimeout() {
    return (long)(10000.0 *
                  ServerOptions.getInstance()
                      .AntiCheatProtectionType24ThresholdMultiplier.getValue());
  }

  void failTimeMultiplier(float float0) {
    if (ServerOptions.instance.AntiCheatProtectionType23.getValue() &&
        checkUser(this.connection)) {
      doKickUser(this.connection, this.getClass().getSimpleName(), "Type23",
                 String.valueOf(float0));
    }
  ...
  void timeoutTimeMultiplier() {
    if (ServerOptions.instance.AntiCheatProtectionType24.getValue() &&
        checkUser(this.connection)) {
      doKickUser(this.connection, this.getClass().getSimpleName(), "Type24",
                 nullptr);
    }
  ...
  static bool checkPVP(UdpConnection udpConnection, char character1,
    ...
    boolean boolean0 =
        ...
        SafetySystemManager.checkUpdateDelay(character1.getCharacter(),
                                             character0.getCharacter());
    if (!boolean0 &&
        ServerOptions.instance.AntiCheatProtectionType1.getValue() &&
        checkUser(udpConnection)) {
  ...
  static bool checkSpeed(UdpConnection udpConnection, IMovable iMovable,
    ...
    bool boolean0 =
        float0 <=
        double0 * ServerOptions.instance
                      .AntiCheatProtectionType2ThresholdMultiplier.getValue();
    if (!boolean0 &&
        ServerOptions.instance.AntiCheatProtectionType2.getValue() &&
        checkUser(udpConnection)) {
  ...
  static bool checkLongDistance(UdpConnection udpConnection,
    ...
    bool boolean0 =
        float0 <=
        udpConnection.ReleventRange * 10 *
            ServerOptions.instance.AntiCheatProtectionType3ThresholdMultiplier
                .getValue();
    if (!boolean0 &&
        ServerOptions.instance.AntiCheatProtectionType3.getValue() &&
        checkUser(udpConnection)) {
      if (udpConnection.validator.checkSuspiciousActivity("Type3")) {
        doKickUser(udpConnection, string, "Type3", String.valueOf(float0));
      } else {
        doLogUser(udpConnection, Userlog.UserlogType.SuspiciousActivity, string,
                  "Type3");
      }
    }
  ...
  static bool checkDamage(UdpConnection udpConnection, Hit hit,
    ...
    bool boolean0 =
        float0 <=
        101.0 * ServerOptions.instance
                    .AntiCheatProtectionType4ThresholdMultiplier.getValue();
    if (!boolean0 &&
        ServerOptions.instance.AntiCheatProtectionType4.getValue() &&
        checkUser(udpConnection)) {
  ...
  static bool checkOwner(UdpConnection udpConnection1, Zombie zombie1,
    ...
    bool boolean0 =
        ...
        System.currentTimeMillis() - zombie0.lastChangeOwner > 2000L;
    if (!boolean0 &&
        ServerOptions.instance.AntiCheatProtectionType5.getValue() &&
        checkUser(udpConnection1)) {
      if (udpConnection1.validator.checkSuspiciousActivity("Type5")) {
        doKickUser(udpConnection1, string, "Type5",
                   Optional.ofNullable(udpConnection0)
                       .map(udpConnection->udpConnection.username)
                       .orElse(""));
      } else {
        doLogUser(udpConnection1, Userlog.UserlogType.SuspiciousActivity,
                  string, "Type5");
      }
    }
  ...
  static bool checkTarget(UdpConnection udpConnection, Player player1,
    ...
    bool boolean0 = Arrays.stream(udpConnection.players)
                        .anyMatch(player1x->player1x.getOnlineID() ==
                                  player0.getOnlineID());
    if (!boolean0 &&
        ServerOptions.instance.AntiCheatProtectionType6.getValue() &&
        checkUser(udpConnection)) {
  ...
  static bool checkSafehouseAuth(UdpConnection udpConnection,
    ...
    if (!boolean0 &&
        ServerOptions.instance.AntiCheatProtectionType7.getValue() &&
        checkUser(udpConnection)) {
  ...
  static bool checkShortDistance(UdpConnection udpConnection,
    ...
    bool boolean0 =
        float0 <=
        10.0 * ServerOptions.instance
                   .AntiCheatProtectionType3ThresholdMultiplier.getValue();
    if (!boolean0 &&
        ServerOptions.instance.AntiCheatProtectionType3.getValue() &&
        checkUser(udpConnection)) {
      doKickUser(udpConnection, string, "Type3", String.valueOf(float0));
  ...
  static bool isUntouchable(UdpConnection udpConnection) {
    return !udpConnection.isFullyConnected() ||
           ...
           Arrays.stream(udpConnection.players)
               .filter(Objects::nonNull)
               .anyMatch(IsoGameCharacter::isGodMod);
  }
  ...
  bool checkSuspiciousActivity(const std::string &string) {
    if (this.suspiciousActivityCounter <= 4) {
      ...
      this.suspiciousActivityDescription = String.format(
          "player=\"%s\" type=\"%s\"", this.connection.username, string);
      DebugLog.Multiplayer.noise("SuspiciousActivity increase: counter=%d %s",
                                 this.suspiciousActivityCounter,
                                 this.suspiciousActivityDescription);
    }
  ...
  static void doKickUser(UdpConnection udpConnection,
    ...
    DebugLog.Multiplayer.warn(
        "Kick: player=\"%s\" type=\"%s\" issuer=\"%s\" description=\"%s\"",
        udpConnection.username, string0, string1, string2);
  ...
  static void doBanUser(UdpConnection udpConnection, const std::string &string0,
    ...
    ServerWorldDatabase.instance.addUserlog(udpConnection.username,
                                            ...
                                            "AntiCheat" + string1, 1);
    DebugLog.Multiplayer.warn("Ban: player=\"%s\" type=\"%s\" issuer=\"%s\"",
                              udpConnection.username, string0, string1);
    ServerWorldDatabase.instance.banUser(udpConnection.username, true);
    if (SteamUtils.isSteamModeEnabled()) {
      std::string string2 =
          SteamUtils.convertSteamIDToString(udpConnection.steamID);
      ServerWorldDatabase.instance.banSteamID(string2, string0, true);
    } else {
      ServerWorldDatabase.instance.banIp(udpConnection.ip,
                                         udpConnection.username, string0, true);
    }
  ...
  static bool checkPVP(IsoGameCharacter character,
    ...
    if (player1 != nullptr) {
      ...
      if (NonPvpZone.getNonPvpZone((int)movingObject.getX(),
                                   (int)movingObject.getY()) != nullptr) {
        return false;
      }

      if (player0 != nullptr &&
          NonPvpZone.getNonPvpZone((int)character.getX(),
                                   (int)character.getY()) != nullptr) {
        return false;
      }
  ...
  std::string getDescription() {
    ...
    if (GameServer.bServer) {
      Set set = this.details.entrySet()
                    .stream()
                    .filter(entry->this.detailsFromClient.get(entry.getKey()) !=
                                nullptr &&
                            this.detailsFromClient.get(entry.getKey()).crc ==
                                entry.getValue().crc)
                    .map(Entry::getKey)
      ...
      this.details.values().forEach(
          recipeDetails->stringBuilder.append(recipeDetails.getDescription()));
      stringBuilder.append("\nServer end\nClient start size=")
          .append(this.detailsFromClient.size());
      this.detailsFromClient.values().forEach(
          recipeDetails->stringBuilder.append(recipeDetails.getDescription()));
      stringBuilder.append("\nClient end");
    }
...
public
  static enum CheckState { None, Sent, Success; }
  ...
  private
    final ArrayList<PacketValidator.RecipeDetails.Skill> skills =
        std::make_unique<ArrayList<>>();
  private
    final ArrayList<String> items = std::make_unique<ArrayList<>>();
    const std::string type;
    const std::string module;
    const int count;

    long getCRC() { return this.crc; }

  public
    RecipeDetails(String string0, long long0, int int0,
                  ArrayList<Recipe.RequiredSkill> arrayList0,
                  ArrayList<Recipe.Source> arrayList1, String string1,
      ...
      if (arrayList0 != nullptr) {
        for (Recipe.RequiredSkill requiredSkill : arrayList0) {
          this.skills.add(new PacketValidator.RecipeDetails.Skill(
              requiredSkill.getPerk().getName(), requiredSkill.getLevel()));
        }
      }
  ...
  public
    RecipeDetails(ByteBuffer byteBuffer) {
    ...
    std::string getDescription() {
      return "\n\tRecipe: name=\"" + this.name + "\" crc=" + this.crc +
             " time=" + this.timeToMake + " type=\"" + this.type +
             "\" module=\"" + this.module + "\" count=" + this.count +
             " items=[" + String.join(",", this.items) + "] skills=[" +
             this.skills.stream()
