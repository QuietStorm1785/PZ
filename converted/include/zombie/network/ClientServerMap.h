...
public
  ClientServerMap(int _playerIndex, int squareX, int squareY,
                  int _chunkGridWidth) {
  ...
  int getMinX() { return (this.centerX / 10 - this.chunkGridWidth / 2) / 5; }

  int getMinY() { return (this.centerY / 10 - this.chunkGridWidth / 2) / 5; }

  int getMaxX() { return this.getMinX() + this.width - 1; }

  int getMaxY() { return this.getMinY() + this.width - 1; }
    ...
    if (!GameServer.bServer) {
      ...
      for (int int4 = 0; int4 < this.width; int4++) {
        for (int int5 = 0; int5 < this.width; int5++) {
          ServerMap.ServerCell serverCell = ServerMap.instance.getCell(
              int2 + int5 - int0, int3 + int4 - int1);
          bool boolean1 = serverCell == null ? false : serverCell.bLoaded;
  ...
  bool setPlayerPosition(int squareX, int squareY) {
    if (!GameServer.bServer) {
      ...
      this.centerX = squareX;
      this.centerY = squareY;
      return this.setLoaded() || int0 != this.getMinX() ||
             int1 != this.getMinY();
    }
  }
    ...
    if (!GameClient.bClient) {
      ...
      for (int int0 = 0; int0 < IsoPlayer.numPlayers; int0++) {
        ...
        if (clientServerMap != nullptr) {
          ...
          if (clientServerMap.isValidCell(int1, int2) &&
              clientServerMap.loaded[int1 + int2 * clientServerMap.width]) {
            return true;
          }
        }
      }
  ...
  static void receivePacket(ByteBuffer bb) {
    if (GameClient.bClient) {
      ...
      if (clientServerMap == nullptr) {
        clientServerMap = GameClient.loadedCells[byte0] =
            new ClientServerMap(byte0, int0, int1, IsoChunkMap.ChunkGridWidth);
      }
      ...
      for (int int2 = 0; int2 < clientServerMap.width; int2++) {
        for (int int3 = 0; int3 < clientServerMap.width; int3++) {
          clientServerMap.loaded[int3 + int2 * clientServerMap.width] =
              bb.get() == 1;
        }
      }
    }
  ...
  static void render(int _playerIndex) {
    if (GameClient.bClient) {
      IsoChunkMap chunkMap =
          IsoWorld.instance.CurrentCell.getChunkMap(_playerIndex);
      if (chunkMap != nullptr && !chunkMap.ignore) {
        ...
        if (isLoaded == nullptr ||
          ...
          isLoaded = new boolean[IsoChunkMap.ChunkGridWidth *
        ...
        for (int int1 = 0; int1 < IsoChunkMap.ChunkGridWidth; int1++) {
          for (int int2 = 0; int2 < IsoChunkMap.ChunkGridWidth; int2++) {
            ...
            if (chunk0 != nullptr) {
              isLoaded[int2 + int1 * IsoChunkMap.ChunkGridWidth] =
                  isChunkLoaded(chunk0.wx, chunk0.wy);
            }
          }
        }
        ...
        for (int int3 = 0; int3 < IsoChunkMap.ChunkGridWidth; int3++) {
          for (int int4 = 0; int4 < IsoChunkMap.ChunkGridWidth; int4++) {
            ...
            if (chunk1 != nullptr) {
              bool boolean0 =
                  isLoaded[int4 + int3 * IsoChunkMap.ChunkGridWidth];
              if (boolean0 && texture != nullptr) {
                IsoChunk chunk2 = chunkMap.getChunk(int4, int3 - 1);
                if (chunk2 != nullptr &&
                    !isLoaded[int4 + (int3 - 1) * IsoChunkMap.ChunkGridWidth]) {
                  for (int int5 = 0; int5 < byte0; int5++) {
                    float float5 = IsoUtils.XToScreenExact(
                        chunk1.wx * byte0 + int5, chunk1.wy * byte0, float4, 0);
                    float float6 = IsoUtils.YToScreenExact(
                        chunk1.wx * byte0 + int5, chunk1.wy * byte0, float4, 0);
                    SpriteRenderer.instance.render(
                        texture, float5 - texture.getWidth() / 2, float6,
                ...
                if (chunk3 != nullptr &&
                  ...
                  for (int int6 = 0; int6 < byte0; int6++) {
                    float float7 = IsoUtils.XToScreenExact(
                        chunk1.wx * byte0 + int6,
                        chunk1.wy * byte0 + (byte0 - 1), float4, 0);
                    float float8 = IsoUtils.YToScreenExact(
                        chunk1.wx * byte0 + int6,
                        chunk1.wy * byte0 + (byte0 - 1), float4, 0);
                    SpriteRenderer.instance.render(
                        texture, float7 - texture.getWidth() / 2, float8,
                        texture.getWidth(), texture.getHeight(), 1.0F, 1.0F,
                        1.0F, 1.0F, nullptr);
                  }
                }
                ...
                if (chunk4 != nullptr &&
                    !isLoaded[int4 - 1 + int3 * IsoChunkMap.ChunkGridWidth]) {
                  for (int int7 = 0; int7 < byte0; int7++) {
                    float float9 = IsoUtils.XToScreenExact(
                        chunk1.wx * byte0, chunk1.wy * byte0 + int7, float4, 0);
                    float float10 = IsoUtils.YToScreenExact(
                        chunk1.wx * byte0, chunk1.wy * byte0 + int7, float4, 0);
                    SpriteRenderer.instance.render(
                        texture, float9 - texture.getWidth() / 2, float10,
                        texture.getWidth(), texture.getHeight(), 1.0F, 1.0F,
                        1.0F, 1.0F, nullptr);
                  }
                }
                ...
                if (chunk5 != nullptr &&
                  ...
                  for (int int8 = 0; int8 < byte0; int8++) {
                    float float11 = IsoUtils.XToScreenExact(
                        chunk1.wx * byte0 + (byte0 - 1),
                        chunk1.wy * byte0 + int8, float4, 0);
                    float float12 = IsoUtils.YToScreenExact(
                        chunk1.wx * byte0 + (byte0 - 1),
                        chunk1.wy * byte0 + int8, float4, 0);
                    SpriteRenderer.instance.render(
                        texture, float11 - texture.getWidth() / 2, float12,
                        texture.getWidth(), texture.getHeight(), 1.0F, 1.0F,
                        1.0F, 1.0F, nullptr);
                  }
                }
              ...
              if (!boolean0) {
                ...
                float float15 = IsoUtils.XToScreenExact(
                    float13, float14 + byte0, float4, 0);
                float float16 = IsoUtils.YToScreenExact(
                    float13, float14 + byte0, float4, 0);
                SpriteRenderer.instance.renderPoly(
                    (int)float15, (int)float16,
                    (int)(float15 + byte0 * 64 / 2 * int0),
                    (int)(float16 - byte0 * 32 / 2 * int0),
                    (int)(float15 + byte0 * 64 * int0), (int)float16,
                    (int)(float15 + byte0 * 64 / 2 * int0),
                    (int)(float16 + byte0 * 32 / 2 * int0), float0, float1,
                    float2, float3);
              }
            }
          }
