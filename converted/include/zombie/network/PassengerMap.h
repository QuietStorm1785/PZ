...
public:
 ...
 private static PassengerMap.PassengerLocal[] perPlayerPngr = new PassengerMap.PassengerLocal[4];
 private static PassengerMap.DriverLocal[] perPlayerDriver = new PassengerMap.DriverLocal[4];

 static void updatePassenger(IsoPlayer player) {
 if (player != nullptr && player.getVehicle() != nullptr && !player.getVehicle().isDriver(player) {
 ...
 static void serverReceivePacket(ByteBuffer byteBuffer, UdpConnection udpConnection0) {
 ...
 bool boolean0 = (passengerRemote.loaded & 1L << int4 + int3 * 7) != 0L;
 if (!boolean0) {
 ...
 private static class DriverLocal {
 PassengerMap.PassengerRemote[] passengers = new PassengerMap.PassengerRemote[16];
 }
 ...
 bool boolean4 = int0 != this->wx || int1 != this->wy || long0 != this->loaded;
 if (boolean4) {
 this->wx = int0;
 this->wy = int1;
 this->loaded = long0;
