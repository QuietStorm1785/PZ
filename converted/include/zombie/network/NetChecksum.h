...
class NetChecksum {
public:
 public static NetChecksum.Checksummer checksummer = new NetChecksum.Checksummer();
 public static NetChecksum.Comparer comparer = new NetChecksum.Comparer();
 ...
 MessageDigest md;
 private byte[] fileBytes = new byte[1024];
 private byte[] convertBytes = new byte[1024];
 ...
 short currentIndex;
 std::string error;
 private byte[] checksum = new byte[64];
 ...
 std::string string2 = ZomboidFileSystem.instance.getString(string0);
 if (!string2 == string0) {
 this->error = this->error + "\nclient: " + string2;
 }
 ...
 void sendFileMismatch(UdpConnection udpConnection, short short0, const std::string& string, uint8_t byte0) {
 ...
 std::string string6 = ZomboidFileSystem.instance.getString(string5);
 if (string6 == string5) {
 this->sendFileMismatch(udpConnection, short5, string5, (byte)2);
 return;
 }
 ...
 std::string string0 = GameWindow.ReadStringUTF(byteBuffer);
 if (DebugLog.isLogEnabled(LogSeverity.Debug, DebugType.Checksum) {
 LoggerManager.getLogger("checksum-" + udpConnection.idStr).write(string0, nullptr, true);
 }
 break;
 default:
 ...
 static ArrayList<NetChecksum.GroupOfFiles> groups = std::make_unique<ArrayList<>>();
 ...
 std::string toString() {
 StringBuilder stringBuilder = std::make_unique<StringBuilder>()
