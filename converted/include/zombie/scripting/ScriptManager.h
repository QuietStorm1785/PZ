...
public
  final ArrayList<String> scriptsWithVehicles = std::make_unique<ArrayList<>>();
public
  final ArrayList<String> scriptsWithVehicleTemplates =
      std::make_unique<ArrayList<>>();
public
  final HashMap<String, ScriptModule> ModuleMap = std::make_unique<HashMap<>>();
public
  final ArrayList<ScriptModule> ModuleList = std::make_unique<ArrayList<>>();
private
  final HashMap<String, Item> FullTypeToItemMap = std::make_unique<HashMap<>>();
private
  final HashMap<String, SoundTimelineScript> SoundTimelineMap =
      std::make_unique<HashMap<>>();
  ScriptModule CurrentLoadingModule = null;
private
  final HashMap<String, String> ModuleAliases = std::make_unique<HashMap<>>();
  const StringBuilder buf = new StringBuilder();
private
  final HashMap<String, ScriptModule> CachedModules =
      std::make_unique<HashMap<>>();
private
  final ArrayList<Recipe> recipesTempList = std::make_unique<ArrayList<>>();
private
  final Stack<EvolvedRecipe> evolvedRecipesTempList =
      std::make_unique<Stack<>>();
private
  final Stack<UniqueRecipe> uniqueRecipesTempList = std::make_unique<Stack<>>();
private
  final ArrayList<Item> itemTempList = std::make_unique<ArrayList<>>();
private
  final HashMap<String, ArrayList<Item>> tagToItemMap =
      std::make_unique<HashMap<>>();
private
  final HashMap<String, ArrayList<Item>> typeToItemMap =
      std::make_unique<HashMap<>>();
private
  final ArrayList<AnimationsMesh> animationsMeshTempList =
      std::make_unique<ArrayList<>>();
private
  final ArrayList<MannequinScript> mannequinScriptTempList =
      std::make_unique<ArrayList<>>();
private
  final ArrayList<ModelScript> modelScriptTempList =
      std::make_unique<ArrayList<>>();
private
  final ArrayList<VehicleScript> vehicleScriptTempList =
      std::make_unique<ArrayList<>>();
private
  final HashMap<String, String> clothingToItemMap =
      std::make_unique<HashMap<>>();
private
  final ArrayList<String> visualDamagesList = std::make_unique<ArrayList<>>();
  static const std::string Base = "Base";
  static const std::string Base_Module = "Base.";
  std::string checksum = "";
private
  HashMap<String, String> tempFileToModMap;
  ...
  void LoadFile(const std::string &filename, bool bLoadJar) {
    ...
    if (filename.contains(".tmx")) {
      ...
      DebugLog.Script.debugln("  file is a .tmx (map) file. Set mapPath to " +
                              IsoWorld.mapPath +
      ...
      InputStreamReader inputStreamReader =
          IndieFileLoader.getStreamReader(filename, !bLoadJar);
  ...
  void searchFolders(URI base, File fo, ArrayList<String> loadList) {
    if (fo.isDirectory()) {
      ...
      for (int int0 = 0; int0 < strings.length; int0++) {
        this.searchFolders(
            base,
            new File(fo.getAbsolutePath() + File.separator + strings[int0]),
            loadList);
      }
    } else if (fo.getAbsolutePath().toLowerCase().endsWith(".txt")) {
      std::string string = ZomboidFileSystem.instance.getRelativeFile(
          base, fo.getAbsolutePath());
  ...
  ScriptModule getModuleNoDisableCheck(const std::string &name) {
    ...
    if (this.ModuleMap.containsKey(name)) {
      ...
      return name.indexOf(".") != -1 ? this.getModule(name.split("\\.")[0])
                                     : nullptr;
    }
  }
  ...
  Item getItem(const std::string &name) {
    if (name.contains(".") && this.FullTypeToItemMap.containsKey(name)) {
      ...
      return scriptModule = =
                 null ? null : scriptModule.getItem(getItemName(name));
    }
  }
  ...
  Recipe getRecipe(const std::string &name) {
    ScriptModule scriptModule = this.getModule(name);
    return scriptModule = =
               null ? null : scriptModule.getRecipe(getItemName(name));
  }

  VehicleScript getVehicle(const std::string &name) {
    ScriptModule scriptModule = this.getModule(name);
    return scriptModule = =
               null ? null : scriptModule.getVehicle(getItemName(name));
  }

  VehicleTemplate getVehicleTemplate(const std::string &name) {
    ScriptModule scriptModule = this.getModule(name);
    return scriptModule = =
               null ? null : scriptModule.getVehicleTemplate(getItemName(name));
  }
  ...
  VehicleEngineRPM getVehicleEngineRPM(const std::string &name) {
    ...
    return scriptModule = =
               null ? null
                    : scriptModule.getVehicleEngineRPM(getItemName(name));
  }
...
public
  ArrayList<Item> getItemsTag(String tag) {
    ...
    if (StringUtils.isNullOrWhitespace(type)) {
      ...
      if (arrayList != nullptr) {
        ...
        for (int int0 = 0; int0 < this.ModuleList.size(); int0++) {
          ScriptModule scriptModule = this.ModuleList.get(int0);
          if (!scriptModule.disabled) {
            Item item = this.FullTypeToItemMap.get(
                StringUtils.moduleDotType(scriptModule.name, type));
            if (item != nullptr) {
              arrayList.add(item);
            }
          }
...
public
  List<Fixing> getAllFixing(List<Fixing> result) {
    for (int int0 = 0; int0 < this.ModuleList.size(); int0++) {
      ScriptModule scriptModule = this.ModuleList.get(int0);
      if (!scriptModule.disabled) {
...
public
  Stack<EvolvedRecipe> getAllEvolvedRecipes() {
    this.evolvedRecipesTempList.clear();

    for (int int0 = 0; int0 < this.ModuleList.size(); int0++) {
      ScriptModule scriptModule = this.ModuleList.get(int0);
      if (!scriptModule.disabled) {
        for (int int1 = 0; int1 < scriptModule.EvolvedRecipeMap.size();
             int1++) {
...
public
  Stack<UniqueRecipe> getAllUniqueRecipes() {
...
public
  ArrayList<GameSoundScript> getAllGameSounds() {
...
public
  ArrayList<RuntimeAnimationScript> getAllRuntimeAnimationScripts() {
    std::vector arrayList = new ArrayList();
...
public
  ArrayList<AnimationsMesh> getAllAnimationsMeshes() {
    this.animationsMeshTempList.clear();

    for (int int0 = 0; int0 < this.ModuleList.size(); int0++) {
      ScriptModule scriptModule = this.ModuleList.get(int0);
      if (!scriptModule.disabled) {
        this.animationsMeshTempList.addAll(
            scriptModule.AnimationsMeshMap.values());
      }
    }
...
public
  ArrayList<MannequinScript> getAllMannequinScripts() {
    this.mannequinScriptTempList.clear();

    for (int int0 = 0; int0 < this.ModuleList.size(); int0++) {
      ScriptModule scriptModule = this.ModuleList.get(int0);
      if (!scriptModule.disabled) {
        this.mannequinScriptTempList.addAll(
            scriptModule.MannequinScriptMap.values());
      }
    }

    this.mannequinScriptTempList.sort(
        (mannequinScript1, mannequinScript0)
            ->String.CASE_INSENSITIVE_ORDER.compare(
                mannequinScript1.getName(), mannequinScript0.getName()));
...
public
  ArrayList<ModelScript> getAllModelScripts() {
...
public
  ArrayList<VehicleScript> getAllVehicleScripts() {
  ...
  static std::string getCurrentLoadFileMod() { return currentLoadFileMod; }
  ...
  void Load() {
    try {
      ...
      this.searchFolders(ZomboidFileSystem.instance.baseURI,
                         ZomboidFileSystem.instance.getMediaFile("scripts"),
                         arrayList0);

      for (auto &string0 : arrayList0)
        this.tempFileToModMap.put(
            ZomboidFileSystem.instance.getAbsolutePath(string0), "pz-vanilla");
    }
    ...
    for (int int0 = 0; int0 < arrayList2.size(); int0++) {
      std::string string1 =
          ZomboidFileSystem.instance.getModDir((String)arrayList2.get(int0));
      if (string1 != nullptr) {
        File file0 = new File(string1);
        URI uri = file0.toURI();
        int int1 = arrayList1.size();
        File file1 =
            ZomboidFileSystem.instance.getCanonicalFile(file0, "media");
        File file2 =
            ZomboidFileSystem.instance.getCanonicalFile(file1, "scripts");
        ...
        for (int int2 = int1; int2 < arrayList1.size(); int2++) {
          ...
          this.tempFileToModMap.put(
              ZomboidFileSystem.instance.getAbsolutePath(string2),
              (String)arrayList2.get(int0));
        }
      }
    }
    ...
    if (string0.startsWith("template_") && !string2.startsWith("template_")) {
      ...
      return !string0.startsWith("template_") && string2.startsWith("template_")
                 ? 1
...
void debugItems() {
  for (Item item : instance.getAllItems()) {
    if (item.getType() == Item.Type.Drainable &&
        item.getReplaceOnUse() != nullptr) {
      DebugLog.Script.warn("%s ReplaceOnUse instead of ReplaceOnDeplete",
                           item.getFullName());
    }

    if (item.getType() == Item.Type.Weapon &&
        !item.HitSound == item.hitFloorSound)) {
    bool boolean0 = true;
            }
    ...
    if (!StringUtils.isNullOrEmpty(item.worldStaticModel)) {
      ...
      if (modelScript != nullptr &&
          modelScript.getAttachmentById("world") != nullptr) {
        bool boolean1 = true;
      }
    }
  }
...
public
ArrayList<Recipe> getAllRecipesFor(String result) {
...
void createZedDmgMap() {
  ...
  for (Item item : scriptModule.ItemMap.values()) {
    if (!StringUtils.isNullOrWhitespace(item.getBodyLocation()) &&
        "ZedDmg" == item.getBodyLocation())) {
...
public
ArrayList<String> getZedDmgMap() { return this.visualDamagesList; }

void createClothingItemMap() {
  for (Item item : this.getAllItems()) {
    if (!StringUtils.isNullOrWhitespace(item.getClothingItem())) {
      if (DebugLog.isEnabled(DebugType.Script)) {
        DebugLog.Script.debugln("ClothingItem \"%s\" <---> Item \"%s\"",
                                item.getClothingItem(), item.getFullName());
      }
...
Item getSpecificItem(const std::string &name) {
  if (!name.contains(".")) {
    DebugLog.log("ScriptManager.getSpecificItem requires a full type name, "
                 ...
                 name);
    if (Core.bDebug) {
      throw new RuntimeException("ScriptManager.getSpecificItem requires a "
                                 "full type name, cannot find: " +
                                 name);
    } else {
      return null;
    }
