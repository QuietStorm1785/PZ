...
public:
 ...
 static int readBlock(const std::string& string0, int int1, ScriptParser.Block block1) {
 int int0;
 ...
 std::string type;
 std::string id;
 public ArrayList<ScriptParser.BlockElement> elements = std::make_unique<ArrayList<>>();
 public ArrayList<ScriptParser.Value> values = std::make_unique<ArrayList<>>();
 public ArrayList<ScriptParser.Block> children = std::make_unique<ArrayList<>>();
 ...
 void prettyPrintElements(int indent, StringBuilder sb, const std::string& eol) {
 ...
 if (blockElement1.asValue() != nullptr && blockElement0 instanceof ScriptParser.Block) {
 sb.append(eol);
 }
 ...
 void moveValueAfter(const std::string& keyMove, const std::string& keyAfter) {
