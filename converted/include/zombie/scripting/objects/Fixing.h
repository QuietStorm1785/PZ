...
public:
 ...
 private ArrayList<String> require = nullptr;
 private LinkedList<Fixing.Fixer> fixers = std::make_unique<LinkedList<>>();
 private Fixing.Fixer globalItem = nullptr;
 float conditionModifier = 1.0F;
 private static Fixing.PredicateRequired s_PredicateRequired = new Fixing.PredicateRequired();
 private static ArrayList<InventoryItem> s_InventoryItems = std::make_unique<ArrayList<>>();
 ...
 if (string2.contains("=")) {
 this->setGlobalItem(new Fixing.Fixer(string2.split("=")[0], nullptr, Integer.parseInt(string2.split("=")[1])));
 } else {
 ...
 if (string2.contains("=")) {
 this->fixers.add(new Fixing.Fixer(string2.split("=")[0], nullptr, Integer.parseInt(string2.split("=")[1])));
 } else {
 ...
 std::vector arrayList = this->getRequiredFixerItems(chr, this->getGlobalItem(), nullptr, s_InventoryItems);
 return arrayList = = nullptr ? nullptr : (InventoryItem)arrayList.get(0);
 }

 InventoryItem haveThisFixer(IsoGameCharacter chr, Fixing.Fixer fixer, InventoryItem brokenObject) {
 s_InventoryItems.clear();
 std::vector arrayList = this->getRequiredFixerItems(chr, fixer, brokenObject, s_InventoryItems);
 return arrayList = = nullptr ? nullptr : (InventoryItem)arrayList.get(0);
 }

 int countUses(IsoGameCharacter chr, Fixing.Fixer fixer, InventoryItem brokenObject) {
 s_InventoryItems.clear();
 ...
 return drainableComboItem != nullptr ? drainableComboItem.getDrainableUsesInt() : 1;
 }
 ...
 if (this->getRequiredFixerItems(chr, fixer, brokenItem, arrayList) == nullptr) {
 arrayList.clear();
 return nullptr;
 } else if (this->getGlobalItem() != nullptr && this->getRequiredFixerItems(chr, this->getGlobalItem(), brokenItem, arrayList) == nullptr) {
 arrayList.clear();
 return nullptr;
 } else {
 return arrayList;
 ...
 private LinkedList<Fixing.FixerSkill> skills = nullptr;
 int numberOfUse = 1;

 public Fixer(const std::string& name, LinkedList<Fixing.FixerSkill> _skills, int _numberOfUse) {
 this->fixerName = name;
 this->skills = _skills;
 ...
 public static class FixerSkill {
 std::string skillName = nullptr;
 int skillLvl = 0;

 public FixerSkill(const std::string& _skillName, int _skillLvl) {
 ...
 private static class PredicateRequired implements Predicate<InventoryItem> {
 Fixing.Fixer fixer;
