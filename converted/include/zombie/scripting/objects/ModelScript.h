...
public:
 ...
 bool invertX = false;
 Model loadedModel;
 public ArrayList<AnimBoneWeight> boneWeights = std::make_unique<ArrayList<>>();
 std::string animationsMesh = nullptr;
 private static HashSet<String> reported = std::make_unique<HashSet<>>();
 ...
 static void checkMesh(const std::string& string2, const std::string& string0) {
 ...
 static void checkTexture(const std::string& string2, const std::string& string0) {
 ...
 static void check(const std::string& string1, const std::string& string0, const std::string& string2) {
 if (!StringUtils.isNullOrWhitespace(string0) {
 ...
 if (recipe.getProp1() != nullptr && !recipe.getProp1().startsWith("Source=")) {
 check(recipe.getFullType(), recipe.getProp1());
 }

 if (recipe.getProp2() != nullptr && !recipe.getProp2().startsWith("Source=")) {
