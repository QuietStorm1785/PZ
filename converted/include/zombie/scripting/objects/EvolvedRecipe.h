...
public:
  static const DecimalFormat DECIMAL_FORMAT =
  ...
  int maxItems = 0;
public
  final Map<String, ItemRecipe> itemsList = std::make_unique<HashMap<>>();
  std::string resultItem = null;
  std::string baseItem = null;
  bool cookable = false;
  bool addIngredientIfCooked = false;
  bool canAddSpicesEmpty = false;
  std::string addIngredientSound = null;
  bool hidden = false;
...
public
    ...
    for (int int0 = 0; int0 < strings0.length; int0++) {
      if (!strings0[int0].trim().isEmpty() && strings0[int0].contains(":")) {
        ...
        if (string1 == "BaseItem")) {
            ...
            this.addIngredientSound =
                StringUtils.discardNullOrWhitespace(string2);
          }
  ...
  bool needToBeCooked(InventoryItem itemTest) {
    ...
    return itemRecipe = = null ? true
                               : itemRecipe.cooked == itemTest.isCooked() ||
                                     itemRecipe.cooked == itemTest.isBurnt() ||
                                     !itemRecipe.cooked;
  }

public
  ArrayList<InventoryItem>
  getItemsCanBeUse(IsoGameCharacter chr, InventoryItem _baseItem,
    ...
    while (iterator.hasNext()) {
      ...
      for (auto &container0 : containers)
  ...
  if (_baseItem.haveExtraItems() && _baseItem.getExtraItems().size() >= 3) {
    for (int int1 = 0; int1 < containers.size(); int1++) {
      ...
      for (int int2 = 0; int2 < container1.getItems().size(); int2++) {
        ...
        if (item instanceof
            Food && ((Food)item).getPoisonLevelForRecipe() != nullptr &&
                chr.isKnownPoison(item) && !arrayList.contains(item)) {
          arrayList.add(item);
        }
      }
    }
  ...
  for (int int0 = 0; int0 < arrayList0.size(); int0++) {
    ...
    if (item0 instanceof Food && this.itemsList.get(string).use != -1) {
      ...
      } else if ((!item1.haveExtraItems() ||
                  item1.extraItems.size() < this.maxItems) &&
                 (!food.isRotten() || int1 >= 7)) {
        boolean0 = true;
      }
...
InventoryItem addItem(InventoryItem _baseItem, InventoryItem usedItem,
  ...
  if (!this.isResultItem(_baseItem)) {
    ...
    if (item1 != nullptr) {
      if (_baseItem instanceof HandWeapon) {
        item1.getModData().rawset("condition:" + _baseItem.getType(),
                                  (double)_baseItem.getCondition() /
                                      _baseItem.getConditionMax());
      }
      ...
      if (item1 instanceof Food) {
        ...
        if (usedItem instanceof
            Food && ((Food)usedItem).getPoisonLevelForRecipe() != nullptr) {
        ...
        if (item2 instanceof Food && item2.getOffAgeMax() != 1000000000 &&
                                 item1.getOffAgeMax() != 1000000000) {
  ...
  if (this.itemsList.get(usedItem.getType()) != nullptr &&
      this.itemsList.get(usedItem.getType()).use > -1) {
    if (!(usedItem instanceof Food)) {
      usedItem.Use();
    } else {
      float float1 =
          this.itemsList.get(usedItem.getType()).use.intValue() / 100.0F;
      ...
      if (food0.isSpice() && _baseItem instanceof Food) {
        if (_baseItem instanceof Food && boolean0) {
          food1.setReduceFoodSickness(food1.getReduceFoodSickness() +
                                      food0.getReduceFoodSickness());
          food1.setPainReduction(food1.getPainReduction() +
                                 food0.getPainReduction());
          food1.setFluReduction(food1.getFluReduction() +
                                food0.getFluReduction());
          if (food0.getEnduranceChange() > 0.0F) {
            food1.setEnduranceChange(food1.getEnduranceChange() +
                                     food0.getEnduranceChange());
          }
        ...
        if (int0 == 7 || int0 == 8) {
          float1 = Float.parseFloat(
              decimalFormat0
                  .format(Math.abs(
                      food0.getBaseHunger() -
                      (food0.getBaseHunger() - 0.05F * food0.getBaseHunger())))
                  .replace(",", "."));
          ...
          float1 = Float.parseFloat(
              decimalFormat0
                  .format(Math.abs(
                      food0.getBaseHunger() -
                      (food0.getBaseHunger() - 0.1F * food0.getBaseHunger())))
                  .replace(",", "."));
        }
      ...
      if (Math.abs(food0.getHungerChange()) < float1) {
        ...
        float1 = Math.abs(Float.parseFloat(
            decimalFormat1.format(food0.getHungerChange()).replace(",", ".")));
        boolean1 = true;
      }
      ...
      if (_baseItem instanceof Food) {
        ...
        if (_baseItem.extraItems != nullptr &&
            _baseItem.extraItems.size() - 2 > int0) {
        ...
        _baseItem.setUnhappyChange(
            ((Food)_baseItem).getUnhappyChangeUnmodified() - (5 - int1 * 5));
        if (_baseItem.getUnhappyChange() > 25.0F) {
          _baseItem.setUnhappyChange(25.0F);
        }

        float float4 = int0 / 15.0F + 1.0F;
        food1.setCalories(food1.getCalories() +
                          food0.getCalories() * float4 * float3);
        food1.setProteins(food1.getProteins() +
                          food0.getProteins() * float4 * float3);
        food1.setCarbohydrates(food1.getCarbohydrates() +
                               food0.getCarbohydrates() * float4 * float3);
        food1.setLipids(food1.getLipids() +
                        food0.getLipids() * float4 * float3);
        ...
        food0.setCarbohydrates(food0.getCarbohydrates() -
                               food0.getCarbohydrates() * float3);
        ...
        if (food0.getFatigueChange() < 0.0F) {
          ...
          food0.setFatigueChange(food0.getFatigueChange() -
                                 food0.getFatigueChange() * float3);
        }

        if (food0.getPoisonPower() > 0) {
          food0.setPoisonPower((int)(food0.getPoisonPower() -
                                     food0.getPoisonPower() * float3 + 0.999));
          ((Food)_baseItem)
              .setPoisonPower((int)(food0.getPoisonPower() * float3 + 0.999));
        }
      }
    }

    _baseItem.addExtraItem(usedItem.getFullType());
  } else if (usedItem instanceof
             Food && ((Food)usedItem).getPoisonLevelForRecipe() != nullptr) {
    this.addPoison(usedItem, _baseItem, chr);
  }
      ...
      if (uniqueRecipe.getBaseRecipe() == item.getType())) {
          ...
          for (int int1 = 0; int1 < uniqueRecipe.getItems().size(); int1++) {
            ...
            for (int int2 = 0; int2 < food.getExtraItems().size(); int2++) {
              if (!arrayList.contains(int2) &&
                  food.getExtraItems().get(int2) ==
                      uniqueRecipe.getItems().get(int1))) {
                  boolean1 = true;
                  arrayList.add(int2);
                  break;
                }
          ...
          if (food.getExtraItems().size() == uniqueRecipe.getItems().size() &&
              boolean0) {
            ...
            food.setBaseHunger(food.getBaseHunger() -
                               uniqueRecipe.getHungerBonus() / 100.0F);
            food.setHungChange(food.getBaseHunger());
            food.setBoredomChange(food.getBoredomChangeUnmodified() -
                                  uniqueRecipe.getBoredomBonus());
            food.setUnhappyChange(food.getUnhappyChangeUnmodified() -
                                  uniqueRecipe.getHapinessBonus());
...
void addPoison(InventoryItem item0, InventoryItem item1,
               IsoGameCharacter character) {
  Food food0 = (Food)item0;
  if (item1 instanceof Food food1) {
    int int0 = food0.getPoisonLevelForRecipe() -
               character.getPerkLevel(PerkFactory.Perks.Cooking);
    if (int0 < 1) {
      int0 = 1;
    }
    ...
    int int1 = new Float(float0 * (food0.getPoisonPower() / 100.0F) * 100.0F)
                   .intValue();
...
void useSpice(Food food1, Food food0, float float1, int int0) {
  if (!this.isSpiceAdded(food0, food1)) {
      ...
      if (int0 == 7 || int0 == 8) {
        float1 = Float.parseFloat(
            decimalFormat
                .format(Math.abs(
                    food1.getBaseHunger() -
                    (food1.getBaseHunger() - 0.05F * food1.getBaseHunger())))
                .replace(",", "."));
      } else if (int0 == 9 || int0 == 10) {
        float1 = Float.parseFloat(
            decimalFormat
                .format(Math.abs(
                    food1.getBaseHunger() -
                    (food1.getBaseHunger() - 0.1F * food1.getBaseHunger())))
                .replace(",", "."));
      }
    }
    ...
    food0.setUnhappyChange(food0.getUnhappyChangeUnmodified() -
                           float1 * 200.0F);
    food0.setBoredomChange(food0.getBoredomChangeUnmodified() -
                           float1 * 200.0F);
    food0.setCalories(food0.getCalories() +
                      food1.getCalories() * float3 * float2);
    food0.setProteins(food0.getProteins() +
                      food1.getProteins() * float3 * float2);
    food0.setCarbohydrates(food0.getCarbohydrates() +
                           food1.getCarbohydrates() * float3 * float2);
...
std::string getUntranslatedName() { return this.name; }

std::string getBaseItem() { return this.baseItem; }

public
Map<String, ItemRecipe> getItemsList() { return this.itemsList; }

public
ArrayList<ItemRecipe> getPossibleItems() {
  std::vector arrayList = new ArrayList();
...
std::string getResultItem() {
  return !this.resultItem.contains(".") ? this.resultItem
                                        : this.resultItem.split("\\.")[1];
}

std::string getFullResultItem() { return this.resultItem; }

bool isCookable() { return this.cookable; }
...
bool isSpiceAdded(InventoryItem _baseItem, InventoryItem spiceItem) {
  if (!this.isResultItem(_baseItem)) {
    ...
    return arrayList = =
               null ? false : arrayList.contains(spiceItem.getFullType());
  }
}

std::string getAddIngredientSound() { return this.addIngredientSound; }

void setIsHidden(bool hide) { this.hidden = hide; }

bool isHidden() { return this.hidden; }

bool isAllowFrozenItem() { return this.allowFrozenItem; }

void setAllowFrozenItem(bool allow) { this.allowFrozenItem = allow; }

static { DECIMAL_FORMAT.applyPattern("#.##"); }
}
} // namespace objects
} // namespace scripting
} // namespace zombie
