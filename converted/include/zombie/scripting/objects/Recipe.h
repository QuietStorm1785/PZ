...
public:
  ...
  std::string Prop2;
public
  final ArrayList<Recipe.Source> Source = std::make_unique<ArrayList<>>();
public
...
public
  ArrayList<Recipe.RequiredSkill> skillRequired = nullptr;
  ...
  bool isCanBeDoneFromFloor() { return this.canBeDoneFromFloor; }
...
public
  ...
  int FindIndexOf(InventoryItem a) { return -1; }

public
  ArrayList<Recipe.Source> getSource() { return this.Source; }
  ...
  float getTimeToMake() { return this.TimeToMake; }
      ...
      if (!strings0[int0].trim().isEmpty()) {
        if (strings0[int0].contains(":")) {
          ...
          if (string1 == "CanBeDoneFromFloor")) {
              this.setCanBeDoneFromFloor(
                  string2.trim().equalsIgnoreCase("true"));
            }
          ...
          if (string1 == "SkillRequired")) {
              ...
              for (int int1 = 0; int1 < strings2.length; int1++) {
                ...
                PerkFactory.Perk perk =
                    PerkFactory.Perks.FromString(strings3[0]);
                if (perk == PerkFactory.Perks.MAX) {
                  DebugLog.Recipe.warn("Unknown skill \"%s\" in recipe \"%s\"",
                                       strings3, this.name);
                } else {
                  int int2 = PZMath.tryParseInt(strings3[1], 1);
                  Recipe.RequiredSkill requiredSkill =
                      new Recipe.RequiredSkill(perk, int2);
...
public
  Recipe.Result getResult() { return this.Result; }
  ...
  bool needToBeLearn() { return this.needToBeLearn; }
...
public
  Recipe.RequiredSkill getRequiredSkill(int index) {
    return this.skillRequired != nullptr && index >= 0 &&
                   index < this.skillRequired.size()
               ? this.skillRequired.get(index)
               : nullptr;
  }
...
public
  Recipe.Source findSource(String sourceFullType) {
  ...
  bool isDestroy(const std::string &sourceFullType) {
    ...
    if (source != nullptr) {
      ...
      throw new RuntimeException("recipe " + this.getOriginalname() +
                                 " doesn't use item " + sourceFullType);
    }
  }
  ...
  bool noBrokenItems() { return !this.AllowDestroyedItem; }

  bool isAllowDestroyedItem() { return this.AllowDestroyedItem; }

  void setAllowDestroyedItem(bool allow) { this.AllowDestroyedItem = allow; }

  bool isAllowFrozenItem() { return this.AllowFrozenItem; }

  void setAllowFrozenItem(bool allow) { this.AllowFrozenItem = allow; }

  bool isAllowRottenItem() { return this.AllowRottenItem; }

  void setAllowRottenItem(bool boolean0) { this.AllowRottenItem = boolean0; }
  ...
  bool isRemoveResultItem() { return this.removeResultItem; }
  ...
  void setStopOnWalk(bool stop) { this.stopOnWalk = stop; }

  bool isStopOnWalk() { return this.stopOnWalk; }

  void setStopOnRun(bool stop) { this.stopOnRun = stop; }

  bool isStopOnRun() { return this.stopOnRun; }

  void setIsHidden(bool hide) { this.hidden = hide; }

  bool isHidden() { return this.hidden; }

  bool isInSameInventory() { return this.InSameInventory; }

public
  static final class RequiredSkill {
  private
    final PerkFactory.Perk perk;
    const int level;

  public
    RequiredSkill(PerkFactory.Perk _perk, int _level) {
      this.perk = _perk;
      this.level = _level;
    }
  ...
  private
    ...
    bool isDestroy() { return this.destroy; }

    void setDestroy(bool _destroy) { this.destroy = _destroy; }

    bool isKeep() { return this.keep; }
    ...
    float getUse() { return this.use; }

    void setUse(float _use) { this.use = _use; }

  public
    ArrayList<String> getItems() { return this.items; }
