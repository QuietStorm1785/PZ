...
public:
 ...
 private static Map<String, ItemInfo> itemLoadList = std::make_unique<HashMap<>>();
 private static List<String> objNameLoadList = std::make_unique<ArrayList<>>();
 ...
 static void setIsNewGame(bool boolean0) {
 isNewGame = boolean0;
 }
 ...
 for (auto& string : arrayList) if (!objNameLoadList.contains(string) {
 ...
 static void loadDataFromServer(ByteBuffer byteBuffer) {
 if (GameClient.bClient) {
 ...
 if (Core.bDebug) {
 throw RuntimeException("Should be sending data from the serverDataCache here.");
 }
 ...
 WorldDictionaryLogger.log("Time: " + simpleDateFormat.format(std::make_unique<Date>()), false);
 ...
 throw WorldDictionaryException("WorldDictionary data received from server is corrupt.");
 }
 ...
 data.load();
 std::vector arrayList = new ArrayList();
 info = new Log.Info(simpleDateFormat.format(std::make_unique<Date>()), Core.GameSaveWorld, 195, arrayList);
 ...
 if (!boolean0) {
 throw WorldDictionaryException("WorldDictionary: Cannot load world due to WorldDictionary error.");
 }
 }
 ...
 DebugLog.log("WorldDictionary: Cannot debug print item: " + (string != nullptr ? string : "unknown"));
 }
 }
 }
