...
public:
 protected Map<Short, ItemInfo> itemIdToInfoMap = std::make_unique<HashMap<>>();
 protected Map<String, ItemInfo> itemTypeToInfoMap = std::make_unique<HashMap<>>();
 protected Map<String, Integer> spriteNameToIdMap = std::make_unique<HashMap<>>();
 protected Map<Integer, String> spriteIdToNameMap = std::make_unique<HashMap<>>();
 protected Map<String, Byte> objectNameToIdMap = std::make_unique<HashMap<>>();
 protected Map<Byte, String> objectIdToNameMap = std::make_unique<HashMap<>>();
 protected ArrayList<String> unsetObject = std::make_unique<ArrayList<>>();
 protected ArrayList<String> unsetSprites = std::make_unique<ArrayList<>>();
 short NextItemID = 0;
 ...
 if (sprite != nullptr && sprite.ID >= 0 && sprite.ID != 20000000 && sprite.name == string) {
 return sprite.ID;
 }
 }
 ...
 long long0 = Instant.now().getEpochSecond();
 std::string string = ZomboidFileSystem.instance.getFileNameInCurrentSave("WD_ERROR_" + long0) + File.separator;
 ...
 void load() {
 if (!Core.getInstance().isNoSave()) {
 std::string string = ZomboidFileSystem.instance.getFileNameInCurrentSave("WorldDictionary.bin");
 ...
 exception.printStackTrace();
 throw WorldDictionaryException("Error loading WorldDictionary.", exception);
 }
 }
 }
 }
 ...
 exception.printStackTrace();
 throw WorldDictionaryException("Error saving WorldDictionary.", exception);
 }
 }
 }
 ...
 for (auto& string0 : arrayList0) GameWindow.WriteString(byteBuffer, string0);
 }
 ...
 for (auto& string1 : arrayList1) GameWindow.WriteString(byteBuffer, string1);
 }
 ...
 std::string string0 = ZomboidFileSystem.instance.getFileNameInCurrentSave(string1);
 ...
 exception.printStackTrace();
 throw WorldDictionaryException("Error saving WorldDictionary as text.", exception);
 }
 }
 }
 }
