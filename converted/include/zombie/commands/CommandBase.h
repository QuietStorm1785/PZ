...
public:
 ...
 for (auto& clazz : childrenClasses) if (!isDisabled(clazz) {
 CommandName[] commandNames = clazz.getAnnotationsByType(CommandName.class);

 for (auto& commandName : commandNames) Pattern pattern = Pattern.compile("^" + commandName.name() + "\\b", 2);
 ...
 protected CommandBase(const std::string& string0, const std::string& string2, const std::string& string1, UdpConnection udpConnection) {
 ...
 + this->playerType
 + "\" "
 + (udpConnection != nullptr ? "guid=\"" + udpConnection.getConnectedGUID() + "\" id=\"" + udpConnection.idStr : "unknown connection")
 + "\"";
 }
 ...
 return this->commandArgs != nullptr && integer >= 0 && integer < this->commandArgs.length ? this->commandArgs[integer] : nullptr;
 }

 bool hasOptionalArg(int integer) {
 return this->commandArgs != nullptr && integer >= 0 && integer < this->commandArgs.length;
 }
 ...
 bool boolean0 = commandArgss.length != 0 && this->commandArgs.length != 0 || commandArgss.length == 0 && this->commandArgs.length == 0;
 std::vector arrayList = new ArrayList();

 for (auto& commandArgsx : commandArgss) arrayList.clear();
 this->message = "";
 int int0 = 0;
 boolean0 = true;
