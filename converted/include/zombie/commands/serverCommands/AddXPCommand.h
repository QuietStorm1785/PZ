...
class AddXPCommand : public CommandBase {
    public:
    public
      AddXPCommand(const std::string &string0, const std::string &string1,
      ...
      std::string Command() {
        ...
        if (player0 == nullptr) {
          ...
          if (strings.length != 2) {
            ...
            if (PerkFactory.Perks.FromString((String)object) ==
                PerkFactory.Perks.MAX) {
              std::string string3 = this.connection == null ? "\n" : " LINE ";
              StringBuilder stringBuilder = new StringBuilder();

              for (int int1 = 0; int1 < PerkFactory.PerkList.size(); int1++) {
                if (PerkFactory.PerkList.get(int1) !=
                    PerkFactory.Perks.Passiv) {
              ...
              return "List of available perks :" + string3 +
              ...
              IsoPlayer player1 =
                  GameServer.getPlayerByUserNameForCommand(string2);
              if (player1 != nullptr) {
                string2 = player1.getDisplayName();
                UdpConnection udpConnection =
                    GameServer.getConnectionFromPlayer(player1);
                if (udpConnection != nullptr) {
                  AddXp addXp = new AddXp();
                  addXp.set(player0,
                            PerkFactory.Perks.FromString((String)object), int0);
                  ...
                  LoggerManager.getLogger("admin").write(
                      this.getExecutorUsername() + " added " + int0 + " " +
                      object + " xp's to " + string2);
