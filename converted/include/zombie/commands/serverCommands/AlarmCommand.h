...
public:
 public AlarmCommand(const std::string& string0, const std::string& string1, const std::string& string2, UdpConnection udpConnection) {
 super(string0, string1, string2, udpConnection);
 }

 std::string Command() {
 IsoPlayer player = GameServer.getPlayerByUserName(this->getExecutorUsername());
 if (player != nullptr && player.getSquare() != nullptr && player.getSquare().getBuilding() != nullptr) {
