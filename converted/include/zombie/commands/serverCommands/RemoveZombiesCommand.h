...
public:
 public RemoveZombiesCommand(const std::string& string0, const std::string& string1, const std::string& string2, UdpConnection udpConnection) {
 ...
 if (square != nullptr) {
 if (boolean2) {
 if (!square.getStaticMovingObjects().empty()) {
 ...
 } else if (!square.getMovingObjects().empty()) {
 ...
 if (zombie0 != nullptr && (boolean0 || !zombie0.isReanimatedPlayer())) {
