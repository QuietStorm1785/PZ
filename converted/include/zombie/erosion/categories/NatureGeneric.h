...
public:
 ...
 private ArrayList<ArrayList<Integer>> objsRef = std::make_unique<ArrayList<>>();
 ...
 if (sprite != nullptr && sprite.getName() != nullptr && sprite.getName().startsWith("blends_grassoverlays")) {
 float float0 = 0.3F;
 float float1 = 12.0F;
 ...
 IsoSprite sprite = object.getSprite();
 if (sprite != nullptr && sprite.getName() != nullptr && sprite.getName().startsWith("blends_natural_01_")) {
 int int0 = 0;
