...
private
  int[][] objsRef = new int[3][2];
private
  HashMap<String, Integer> spriteToObj = std::make_unique<HashMap<>>();
private
  HashMap<String, Integer> spriteToStage = std::make_unique<HashMap<>>();
private
      ...
      if (object.AttachedAnimSprite != nullptr) {
        for (int int2 = 0; int2 < object.AttachedAnimSprite.size(); int2++) {
          ...
          if (sprite != nullptr && sprite.getName() != nullptr &&
            ...
            WallVines.CategoryData categoryData =
                (WallVines.CategoryData)this.setCatModData(square1);
            ...
            if (object.AttachedAnimSprite != nullptr &&
                int2 < object.AttachedAnimSprite.size()) {
              object.AttachedAnimSprite.remove(int2);
            }
  ...
  bool validateSpawn(IsoGridSquare square1, ErosionData.Square square0,
    ...
    if (!boolean0) {
      ...
      if (int1 == 0) {
        ...
        WallVines.CategoryData categoryData0 =
            (WallVines.CategoryData)this.setCatModData(square0);
        categoryData0.gameObj = this.objsRef[byte0][square0.rand(
            square1.x, square1.y, this.objsRef[byte0].length)];
        categoryData0.maxStage =
            int0 > 65 ? 3 : (int0 > 60 ? 2 : (int0 > 55 ? 1 : 0));
        ...
        if (categoryData0.maxStage == 3) {
          ...
          if (square2 != nullptr) {
            ...
            ErosionObjOverlay erosionObjOverlay =
                this.objs.get(categoryData0.gameObj);
            if (object2 != nullptr && erosionObjOverlay != nullptr) {
              WallVines.CategoryData categoryData1 =
                  new WallVines.CategoryData();
              categoryData1.gameObj = this.objsRef[byte0][square0.rand(
                  square1.x, square1.y, this.objsRef[byte0].length)];
              categoryData1.maxStage = int0 > 75 ? 2 : (int0 > 70 ? 1 : 0);
              categoryData1.stage = 0;
              categoryData1.spawnTime =
                  categoryData0.spawnTime +
                  (int)(erosionObjOverlay.cycleTime /
                        (categoryData0.maxStage + 1.0F) * 4.0F);
  ...
  void update(IsoGridSquare square0, ErosionData.Square square2,
    ...
    if (int0 >= categoryData.spawnTime && !categoryData.doNothing) {
      if (categoryData.gameObj >= 0 &&
        ...
        int int2 =
            (int)Math.floor((int0 - categoryData.spawnTime) /
                            (erosionObjOverlay.cycleTime / (int1 + 1.0F)));
        ...
        if (int2 == 3 && categoryData.hasTop != nullptr &&
            categoryData.hasTop.spawnTime > int0) {
          int2 = 2;
        }
        ...
        if (int2 != categoryData.stage || categoryData.dispSeason != int3) {
          ...
          if (object0 != nullptr) {
            ...
            int int5 =
                erosionObjOverlay.setOverlay(object0, int4, int2, int3, 0.0F);
            if (int5 >= 0) {
  ...
  void init() {
    ...
    for (int int1 = 0; int1 < ints1.length; int1++) {
      ErosionObjOverlaySprites erosionObjOverlaySprites =
          new ErosionObjOverlaySprites(4, "WallVines");
      ...
      for (int int2 = 0; int2 <= 3; int2++) {
        for (int int3 = 0; int3 <= 2; int3++) {
          ...
          if (int3 == 2) {
            erosionObjOverlaySprites.setSprite(int2, string + int4,
                                               ints0[int3 + 1]);
          }
...
protected
  ErosionCategory.Data allocData() { return new WallVines.CategoryData(); }
...
private
  static final class CategoryData extends ErosionCategory.Data {
    int gameObj;
    int maxStage;
    int spawnTime;
    int curID = -999999;
  public
    WallVines.CategoryData hasTop;
