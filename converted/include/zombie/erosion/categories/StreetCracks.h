...
private
  ArrayList<ErosionObjOverlay> crackObjs = std::make_unique<ArrayList<>>();
private
  ...
  bool validateSpawn(IsoGridSquare square1, ErosionData.Square square0,
    ...
    if (int1 == 0) {
      ...
      StreetCracks.CategoryData categoryData =
          (StreetCracks.CategoryData)this.setCatModData(square0);
      categoryData.gameObj =
          square0.rand(square1.x, square1.y, this.crackObjs.size());
  ...
  void update(IsoGridSquare square0, ErosionData.Square square1,
    ...
    if (int0 >= categoryData.spawnTime && !categoryData.doNothing) {
      ...
      if (categoryData.gameObj >= 0 &&
          categoryData.gameObj < this.crackObjs.size() && object != nullptr) {
        ErosionObjOverlay erosionObjOverlay =
            this.crackObjs.get(categoryData.gameObj);
        ...
        if (categoryData.hasGrass) {
          ...
          if (erosionObj != nullptr) {
            ...
            bool boolean0 = false;
            bool boolean1 = false;
            this.updateObj(square1, data, square0, erosionObj, boolean0, int2,
                           int5, boolean1);
          }
        }
      } else {
  ...
  void init() {
    for (int int0 = 0; int0 < 100; int0++) {
      this.spawnChance[int0] =
          int0 >= 40 ? (int)this.clerp((int0 - 40) / 60.0F, 0.0F, 60.0F) : 0;
    }
    ...
    for (int int1 = 0; int1 <= 7; int1++) {
      ErosionObjOverlaySprites erosionObjOverlaySprites =
          new ErosionObjOverlaySprites(3, "StreeCracks");
      ErosionObjSprites erosionObjSprites =
          new ErosionObjSprites(3, "CrackGrass", false, false, false);
      ...
      this.crackObjs.add(
          new ErosionObjOverlay(erosionObjOverlaySprites, 60, false));
      this.objs.add(new ErosionObj(erosionObjSprites, 60, 0.0F, 0.0F, false));
    }
  }

protected
  ErosionCategory.Data allocData() { return new StreetCracks.CategoryData(); }
