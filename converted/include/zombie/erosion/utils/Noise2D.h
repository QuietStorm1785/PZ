...
public:
private
  ArrayList<Noise2D.Layer> layers = new ArrayList<>(3);
private
  ...
  float fade(float float0) {
    return float0 * float0 * float0 *
           (float0 * (float0 * 6.0F - 15.0F) + 10.0F);
  }
  ...
  float noise(float float0, float float1, int[] ints) {
    ...
    return this.lerp(float3,
                     this.lerp(float2, perm[ints[int2]], perm[ints[int4]]),
                     this.lerp(float2, perm[ints[int3]], perm[ints[int5]]));
  }
  ...
  float layeredNoise(float _x, float _y) {
    ...
    for (int int0 = 0; int0 < this.layers.size(); int0++) {
      ...
      float1 += layer.amp;
      float0 +=
          this.noise(_x * layer.freq, _y * layer.freq, layer.p) * layer.amp;
    }
  ...
  void addLayer(int _seed, float _freq, float _amp) {
    ...
    for (int int1 = 0; int1 < 256; int1++) {
      int int2 = (int)Math.floor(int0 + int1 -
                                 Math.floor((int0 + int1) / 256.0F) * 256.0);
