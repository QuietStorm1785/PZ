...
public:
private
  static final ArrayList<ZombiesZoneDefinition.ZZDZone> s_zoneList =
      std::make_unique<ArrayList<>>();
private
  static final HashMap<String, ZombiesZoneDefinition.ZZDZone> s_zoneMap =
      std::make_unique<HashMap<>>();
  static bool bDirty = true;
private
  static final ZombiesZoneDefinition.PickDefinition pickDef =
      new ZombiesZoneDefinition.PickDefinition();
private
  static final HashMap<String, ZombiesZoneDefinition.ZZDOutfit>
      s_customOutfitMap = std::make_unique<HashMap<>>();
  ...
  static void init() {
    s_zoneList.clear();
    s_zoneMap.clear();
    KahluaTableImpl kahluaTableImpl0 = Type.tryCastTo(
        LuaManager.env.rawget("ZombiesZoneDefinition"), KahluaTableImpl.class);
    if (kahluaTableImpl0 != nullptr) {
      ...
      while (kahluaTableIterator.advance()) {
        ...
        if (kahluaTableImpl1 != nullptr) {
          ZombiesZoneDefinition.ZZDZone zZDZone = initZone(
              kahluaTableIterator.getKey().toString(), kahluaTableImpl1);
          if (zZDZone != nullptr) {
...
private
  static ZombiesZoneDefinition.ZZDZone
  initZone(String string, KahluaTableImpl kahluaTableImpl0) {
    ...
    while (kahluaTableIterator.advance()) {
      KahluaTableImpl kahluaTableImpl1 =
          Type.tryCastTo(kahluaTableIterator.getValue(), KahluaTableImpl.class);
      if (kahluaTableImpl1 != nullptr) {
        ZombiesZoneDefinition.ZZDOutfit zZDOutfit =
            initOutfit(kahluaTableImpl1);
...
private
  static ZombiesZoneDefinition.ZZDOutfit
  initOutfit(KahluaTableImpl kahluaTableImpl) {
    ZombiesZoneDefinition.ZZDOutfit zZDOutfit =
        new ZombiesZoneDefinition.ZZDOutfit();
    ...
    zZDOutfit.femaleHairStyles =
        initStringChance(kahluaTableImpl.rawgetStr("femaleHairStyles"));
    zZDOutfit.maleHairStyles =
        initStringChance(kahluaTableImpl.rawgetStr("maleHairStyles"));
    zZDOutfit.beardStyles =
        initStringChance(kahluaTableImpl.rawgetStr("beardStyles"));
    return zZDOutfit;
  }

private
  static ArrayList<ZombiesZoneDefinition.StringChance>
  initStringChance(String string0) {
    if (StringUtils.isNullOrWhitespace(string0)) {
      ...
      ZombiesZoneDefinition.StringChance stringChance =
          new ZombiesZoneDefinition.StringChance();
...
static void
dressInRandomOutfit(IsoZombie zombie0) {
  if (!zombie0.isSkeleton()) {
    IsoGridSquare square = zombie0.getCurrentSquare();
    if (square != nullptr) {
      ZombiesZoneDefinition.PickDefinition pickDefinition =
          pickDefinition(square.x, square.y, square.z, zombie0.isFemale());
      if (pickDefinition == nullptr) {
        std::string string =
            square.getRoom() == null ? null : square.getRoom().getName();
        ...
        applyDefinition(zombie0, pickDefinition.zone, pickDefinition.table,
                        pickDefinition.bFemale);
        UnderwearDefinition.addRandomUnderwear(zombie0);
      }
    }
  }
...
public
static IsoMetaGrid.Zone getDefinitionZoneAt(int int0, int int1, int int2) {
  std::vector arrayList =
      IsoWorld.instance.MetaGrid.getZonesAt(int0, int1, int2);

  for (int int3 = arrayList.size() - 1; int3 >= 0; int3--) {
    IsoMetaGrid.Zone zone = (IsoMetaGrid.Zone)arrayList.get(int3);
    if ("ZombiesType".equalsIgnoreCase(zone.type) ||
        s_zoneMap.containsKey(zone.type)) {
      return zone;
    }
  }
...
public
static ZombiesZoneDefinition.PickDefinition
pickDefinition(int int0, int int1, int int2, boolean boolean0) {
  ...
  if (square == nullptr) {
    ...
    if (zone == nullptr) {
      ...
      std::string string1 =
          StringUtils.isNullOrEmpty(zone.name) ? zone.type : zone.name;
      ...
      if (zZDZone == nullptr) {
        ...
        if (zZDZone.chanceToSpawn != -1) {
          ...
          std::vector arrayList0 =
              IsoWorld.instance.getSpawnedZombieZone().get(zone.getName());
          if (arrayList0 == nullptr) {
            arrayList0 = std::make_unique<ArrayList>();
            IsoWorld.instance.getSpawnedZombieZone().put(zone.getName(),
                                                         arrayList0);
          }
          ...
          if (int4 == -1 ||
              zone.spawnSpecialZombies == nullptr && arrayList0.size() < int4) {
        ...
        if (zone == nullptr) {
          ...
          for (int int7 = 0; int7 < zZDZone.outfits.size(); int7++) {
            ZombiesZoneDefinition.ZZDOutfit zZDOutfit0 =
                zZDZone.outfits.get(int7);
            std::string string2 = zZDOutfit0.gender;
            std::string string3 = zZDOutfit0.room;
            if ((string3 == nullptr ||
                 string0 != nullptr && string3.contains(string0)) &&
...
static void applyDefinition(IsoZombie zombie0, IsoMetaGrid.Zone zone,
                            ZombiesZoneDefinition.ZZDOutfit zZDOutfit,
                            bool boolean0) {
...
static Outfit getRandomDefaultOutfit(bool boolean0,
  ...
  for (int int0 = 0; int0 < zZDZone.outfits.size(); int0++) {
    ...
    if ((string1 == nullptr ||
         string2 != nullptr && string1.contains(string2)) &&
        (string0 == nullptr || "male".equalsIgnoreCase(string0) && !boolean0 ||
         "female".equalsIgnoreCase(string0) && boolean0)) {
      arrayList.add(zZDOutfit0);
    }
  }

  ZombiesZoneDefinition.ZZDOutfit zZDOutfit1 =
      getRandomOutfitInSetList(arrayList, false);
  Outfit outfit = null;
  if (zZDOutfit1 != nullptr) {
    if (boolean0) {
...
public
static ZombiesZoneDefinition.ZZDOutfit
getRandomOutfitInSetList(ArrayList<ZombiesZoneDefinition.ZZDOutfit> arrayList,
                         boolean boolean0) {
  float float0 = 0.0F;

  for (int int0 = 0; int0 < arrayList.size(); int0++) {
    ZombiesZoneDefinition.ZZDOutfit zZDOutfit0 =
        (ZombiesZoneDefinition.ZZDOutfit)arrayList.get(int0);
  ...
  for (int int1 = 0; int1 < arrayList.size(); int1++) {
    ZombiesZoneDefinition.ZZDOutfit zZDOutfit1 =
        (ZombiesZoneDefinition.ZZDOutfit)arrayList.get(int1);
...
static std::string
getRandomHairOrBeard(ArrayList<ZombiesZoneDefinition.StringChance> arrayList) {
  float float0 = OutfitRNG.Next(0.0F, 100.0F);
  float float1 = 0.0F;

  for (int int0 = 0; int0 < arrayList.size(); int0++) {
    ZombiesZoneDefinition.StringChance stringChance =
        (ZombiesZoneDefinition.StringChance)arrayList.get(int0);
...
static void registerCustomOutfits() {
  ...
  for (ZombiesZoneDefinition.ZZDZone zZDZone : s_zoneList) {
    for (ZombiesZoneDefinition.ZZDOutfit zZDOutfit : zZDZone.outfits) {
      PersistentOutfits.instance.registerOutfitter(
          zZDOutfit.customName, true, ZombiesZoneDefinition::ApplyCustomOutfit);
...
static void ApplyCustomOutfit(int int0, const std::string &string,
  ...
  if (zombie0 == nullptr) {
    ...
    if (boolean1 && zZDOutfit.femaleHairStyles != nullptr) {
      zombie0.getHumanVisual().setHairModel(
          getRandomHairOrBeard(zZDOutfit.femaleHairStyles));
    }

    if (!boolean1 && zZDOutfit.maleHairStyles != nullptr) {
      zombie0.getHumanVisual().setHairModel(
          getRandomHairOrBeard(zZDOutfit.maleHairStyles));
    }

    if (!boolean1 && zZDOutfit.beardStyles != nullptr) {
      zombie0.getHumanVisual().setBeardModel(
          getRandomHairOrBeard(zZDOutfit.beardStyles));
    }
...
static int pickPersistentOutfit(IsoGridSquare square) {
  if (!GameServer.bServer) {
    ...
    ZombiesZoneDefinition.PickDefinition pickDefinition =
        pickDefinition(square.x, square.y, square.z, boolean0);
    Outfit outfit;
    if (pickDefinition == nullptr) {
      std::string string0 =
          square.getRoom() == null ? null : square.getRoom().getName();
...
public
static final class PickDefinition {
...
private static final class ZZDZone {
  ...
  int chanceToSpawn;
  int toSpawn;
  final ArrayList<ZombiesZoneDefinition.ZZDOutfit> outfits =
      std::make_unique<ArrayList<>>();
}
}
} // namespace characters
