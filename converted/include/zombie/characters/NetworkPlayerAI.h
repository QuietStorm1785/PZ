...
public:
public
  final LinkedList<EventPacket> events = std::make_unique<LinkedList<>>();
  IsoPlayer player;
  PathFindBehavior2 pfb2 = null;
...
public
  NetworkPlayerAI(IsoGameCharacter character) {
    super(character);
  ...
  void needToUpdate() { this.needUpdate = true; }

  void setBlockUpdate(bool value) { this.blockUpdate = value; }
  ...
  bool isNeedToUpdate() {
    ...
    if ((!this.timer.check() && int0 == this.lastBooleanVariables &&
    ...
    if (!this.player.isPlayerMoving()) {
      ...
      if (SystemDisabler.useNetworkCharacter) {
        NetworkCharacter.Transform transform =
            this.player.networkCharacter.predict(
                int1, int0, this.player.x, this.player.y, vector0.x, vector0.y);
      ...
      if (this.player.z == this.pfb2.getTargetZ() &&
          !PolygonalMap2.instance.lineClearCollide(
              this.player.x, this.player.y, vector1.x, vector1.y,
              (int)this.player.z, nullptr)) {
        playerPacket.x = vector1.x;
        playerPacket.y = vector1.y;
        playerPacket.z = (byte)this.pfb2.getTargetZ();
      } else {
        Vector2 vector2 = PolygonalMap2.instance.getCollidepoint(
            this.player.x, this.player.y, vector1.x, vector1.y,
            (int)this.player.z, null, 2);
        playerPacket.collidePointX = vector2.x;
        playerPacket.collidePointY = vector2.y;
        playerPacket.x =
            vector2.x +
            (this.player.dir != IsoDirections.N &&
                     this.player.dir != IsoDirections.S
                 ? (this.player.dir.index() >= IsoDirections.NW.index() &&
                            this.player.dir.index() <= IsoDirections.SW.index()
                        ? -1.0F
                        : 1.0F)
                 : 0.0F);
        playerPacket.y =
            vector2.y +
            (this.player.dir != IsoDirections.W &&
                     this.player.dir != IsoDirections.E
                 ? (this.player.dir.index() >= IsoDirections.SW.index() &&
                            this.player.dir.index() <= IsoDirections.SE.index()
                        ? 1.0F
                        : -1.0F)
                 : 0.0F);
        playerPacket.z = (byte)this.player.z;
  ...
  bool set(PlayerPacket packet) {
    ...
    bool boolean1 = this.player.playerMoveDir.getLength() < 0.01 &&
                    this.lastPlayerMoveDirLen > 0.01F;
    this.lastPlayerMoveDirLen = this.player.playerMoveDir.getLength();
    packet.booleanVariables =
        NetworkPlayerVariables.getBooleanVariables(this.player);
    ...
    bool boolean3 =
        Math.abs(this.lastForwardDirection - packet.direction) > 0.2F;
    this.lastForwardDirection = packet.direction;
    packet.footstepSoundRadius = this.footstepSoundRadius;
    return boolean0 || boolean2 || boolean3 || this.player.isJustMoved() ||
           boolean1;
  }
  ...
  void parse(PlayerPacket packet) {
    if (!this.player.isTeleporting()) {
      ...
      this.direction.set((float)Math.cos(packet.direction),
                         (float)Math.sin(packet.direction));
      ...
      BaseVehicle vehicle =
          VehicleManager.instance.getVehicleByID(packet.VehicleID);
      NetworkPlayerVariables.setBooleanVariables(this.player,
                                                 packet.booleanVariables);
      ...
      if (this.player.getVehicle() == nullptr) {
        if (vehicle != nullptr) {
          if (packet.VehicleSeat >= 0 &&
              packet.VehicleSeat < vehicle.getMaxPassengers()) {
            IsoGameCharacter character0 =
                vehicle.getCharacter(packet.VehicleSeat);
            if (character0 == nullptr) {
              if (GameServer.bDebug) {
                DebugLog.log(this.player.getUsername() + " got in vehicle " +
                             vehicle.VehicleID + " seat " + packet.VehicleSeat);
              }

              vehicle.enterRSync(packet.VehicleSeat, this.player, vehicle);
            } else if (character0 != this.player) {
              DebugLog.log(this.player.getUsername() + " got in same seat as " +
                           ((IsoPlayer)character0).getUsername());
              this.player.sendObjectChange("exitVehicle");
            }
          } else {
            DebugLog.log(this.player.getUsername() + " invalid seat vehicle " +
                         vehicle.VehicleID + " seat " + packet.VehicleSeat);
          }
        }
      } else if (vehicle != nullptr) {
        if (vehicle == this.player.getVehicle() &&
            this.player.getVehicle().getSeat(this.player) != -1) {
          ...
          if (character1 == nullptr) {
            ...
            DebugLog.log(this.player.getUsername() +
                         " switched to same seat as " +
                         ((IsoPlayer)character1).getUsername());
            this.player.sendObjectChange("exitVehicle");
          }
        } else {
          DebugLog.log(this.player.getUsername() + " vehicle/seat remote " +
                       vehicle.VehicleID + "/" + packet.VehicleSeat +
                       " local " + this.player.getVehicle().VehicleID + "/" +
  ...
  bool isPressedMovement() { return this.pressedMovement; }
  ...
  bool isPressedCancelAction() { return this.pressedCancelAction; }
  ...
  void update() {
    if (DebugOptions.instance.MultiplayerHotKey.getValue() &&
        GameKeyboard.isKeyPressed(45) && GameKeyboard.isKeyDown(56)) {
  ...
  bool isDismantleAllowed() { return true; }
}
} // namespace characters
} // namespace zombie
