...
private
  static final ObjectPool<EquippedTextureCreator> pool =
      new ObjectPool<>(EquippedTextureCreator::new);
  ...
  void init(ModelInstance _modelInstance, float _bloodLevel) {
    ...
    if (this.modelInstance != nullptr) {
      ...
      texture = Texture.getSharedTexture(
    ...
    if (modelInstancex != nullptr) {
      ...
      if (itemSmartTexture != nullptr) {
        itemSmartTexture.setBlood(
            "media/textures/BloodTextures/BloodOverlayWeapon.png",
            "media/textures/BloodTextures/BloodOverlayWeaponMask.png", float0,
            300);
  ...
  static EquippedTextureCreator alloc() { return pool.alloc(); }
}
} // namespace characters
} // namespace zombie
