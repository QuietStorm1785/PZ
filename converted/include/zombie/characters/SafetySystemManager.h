...
public:
 private static LinkedHashMap<String, Float> playerCooldown = std::make_unique<LinkedHashMap<>>();
 private static LinkedHashMap<String, Boolean> playerSafety = std::make_unique<LinkedHashMap<>>();
 private static LinkedHashMap<String, Long> playerDelay = std::make_unique<LinkedHashMap<>>();
 ...
 bool boolean0 = NonPvpZone.getNonPvpZone(PZMath.fastfloor(player.getX()), PZMath.fastfloor(player.getY())) != nullptr;
 if (!boolean0) {
 ...
 static bool checkUpdateDelay(IsoGameCharacter character0, IsoGameCharacter character1) {
 bool boolean0 = false;
