...
public:
 ...
 uint8_t byte0 = (byte)((this->zombie.getVariableBoolean("bMoving") ? 1 : 0) | (this->zombie.getVariableBoolean("bPathfind") ? 2 : 0);
 if (this->flags != byte0) {
 this->flags = byte0;
 this->extraUpdate();
 }

 uint8_t byte1 = (byte)IsoDirections.fromAngleActual(this->zombie.getForwardDirection()).index();
 ...
 static std::string getPredictionDebug(IsoGameCharacter character, ZombiePacket packet, int currentTime, long next) {
