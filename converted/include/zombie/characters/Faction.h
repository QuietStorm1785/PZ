...
public:
  ...
  ColorInfo tagColor;
private
  final ArrayList<String> players = std::make_unique<ArrayList<>>();
public
  static ArrayList<Faction> factions = std::make_unique<ArrayList<>>();
...
public
  static ArrayList<Faction> getFactions() { return factions; }
  ...
  static bool canCreateFaction(IsoPlayer player) {
    ...
    if (boolean0 &&
        ...
        player.getHoursSurvived() / 24.0 <
            ServerOptions.instance.FactionDaySurvivedToCreate.getValue()) {
      boolean0 = false;
    }
  ...
  void load(ByteBuffer input, int WorldVersion) {
    ...
    if (input.get() == 1) {
      ...
      this.setTagColor(new ColorInfo(Rand.Next(0.3F, 1.0F),
                                     Rand.Next(0.3F, 1.0F),
                                     Rand.Next(0.3F, 1.0F), 1.0F));
    }
    ...
    if (ChatServer.isInited()) {
      FactionChat factionChat =
          ChatServer.getInstance().createFactionChat(this.getName());
      ChatSettings chatSettings = FactionChat.getDefaultSettings();
      chatSettings.setFontColor(this.tagColor.r, this.tagColor.g,
                                this.tagColor.b, this.tagColor.a);
