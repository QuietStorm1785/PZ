...
public
  ...
  void extrapolate(int int0) {
    ...
    if (float1 > this.minMovement || float2 > this.minMovement ||
        -float1 > this.minMovement || -float2 > this.minMovement) {
  ...
  void extrapolateInternal(int int0, float float2, float float4) {
    ...
    if (float1 > this.minMovement || float3 > this.minMovement ||
        -float1 > this.minMovement || -float3 > this.minMovement) {
      this.transform.moving = true;
      this.transform.rotation.x = this.movement.x;
      this.transform.rotation.y = this.movement.y;
      this.transform.rotation.normalize();
...
public
  NetworkCharacter.Transform predict(int dt, int t, float px, float py,
                                     float rx, float ry) {
...
public
  NetworkCharacter.Transform reconstruct(int t, float px, float py, float rx,
                                         float ry) {
    this.transform.moving = false;
    this.transform.operation = NetworkCharacter.Operation.NONE;
    if (this.d2.t != 0) {
...
public
  static enum Operation { INTERPOLATION, EXTRAPOLATION, NONE; }
  ...
  public
    NetworkCharacter.Operation operation = NetworkCharacter.Operation.NONE;
    bool moving = false;
    int time = 0;
  }
}
