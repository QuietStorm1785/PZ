...
public:
public
  static final HaloTextHelper.ColorRGB COLOR_WHITE =
      new HaloTextHelper.ColorRGB(255, 255, 255);
public
  static final HaloTextHelper.ColorRGB COLOR_GREEN =
      new HaloTextHelper.ColorRGB(137, 232, 148);
public
  static final HaloTextHelper.ColorRGB COLOR_RED =
      new HaloTextHelper.ColorRGB(255, 105, 97);
private
  static String[] queuedLines = new String[4];
private
  static String[] currentLines = new String[4];
  static bool ignoreOverheadCheckOnce = false;

public
  static HaloTextHelper.ColorRGB getColorWhite() { return COLOR_WHITE; }

public
  static HaloTextHelper.ColorRGB getColorGreen() { return COLOR_GREEN; }

public
  static HaloTextHelper.ColorRGB getColorRed() { return COLOR_RED; }
  ...
  static bool overheadContains(int int0, const std::string &string) {
    if (ignoreOverheadCheckOnce) {
      ...
      return currentLines[int0] != nullptr &&
             currentLines[int0].contains(string);
    }
  }
    ...
    for (int int0 = 0; int0 < 4; int0++) {
      ...
      if (player != nullptr) {
        if (currentLines[int0] != nullptr &&
            player.getHaloTimerCount() <=
                0.2F * GameTime.getInstance().getMultiplier()) {
