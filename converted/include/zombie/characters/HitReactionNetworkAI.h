...
public
  HitReactionNetworkAI(IsoGameCharacter _character) {
  ...
  bool isStarted() { return this.startTime > 0L; }

  void start() {
    if (this.isSetup() && !this.isStarted()) {
      this.startTime = GameTime.getServerTimeMills();
      if (this.startPosition.x != this.character.x ||
          this.startPosition.y != this.character.y) {
        DebugLog.Multiplayer.warn("HitReaction start shifted");
      }

      DebugLog.Damage.trace(
          "id=%d: %s / %s => %s", this.character.getOnlineID(),
          this.getActualDescription(), this.getStartDescription(),
          this.getFinalDescription());
    }
  }
  ...
  void setup(float dropPositionX, float dropPositionY, uint8_t dropPositionZ,
             float angle) {
    ...
    Vector2 vector =
        new Vector2().set(this.finalPosition.x - this.startPosition.x,
                          this.finalPosition.y - this.startPosition.y);
    if (angle == nullptr) {
      vector.normalize();
      angle = vector.dot(this.character.getForwardDirection());
      PZMath.lerp(this.finalDirection, vector,
                  this.character.getForwardDirection(), Math.abs(angle));
      IsoMovingObject.getVectorFromDirection(
          this.finalDirection, IsoDirections.fromAngle(this.finalDirection));
    } else {
  ...
  void moveInternal(float float0, float float1, float float2, float float3) {
    ...
    this.character.getAnimationPlayer().SetForceDir(
        this.character.getForwardDirection());
  }

  void moveFinal() {
    this.moveInternal(this.finalPosition.x, this.finalPosition.y,
                      this.finalDirection.x, this.finalDirection.y);
    this.character.lx = this.character.nx = this.character.x =
        this.finalPosition.x;
    this.character.ly = this.character.ny = this.character.y =
        this.finalPosition.y;
    this.character.setCurrent(IsoWorld.instance.CurrentCell.getGridSquare(
        (double)((int)this.finalPosition.x),
        (double)((int)this.finalPosition.y), (double)this.character.z));
    DebugLog.Damage.trace("id=%d: %s / %s => %s", this.character.getOnlineID(),
                          this.getActualDescription(),
                          this.getStartDescription(),
                          this.getFinalDescription());
  }
  ...
  void move() {
    if (this.finalPositionZ != (byte)this.character.z) {
      DebugLog.Damage.trace(
          "HitReaction interrupt id=%d: z-final:%d z-current=%d",
          this.character.getOnlineID(), this.finalPositionZ,
          (byte)this.character.z);
      this.finish();
    } else {
      float float0 =
          Math.min(1.0F, Math.max(0.0F, (float)(GameTime.getServerTimeMills() -
                                                this.startTime) /
                                            600.0F));
      if (this.startPosition.x == this.finalPosition.x &&
          this.startPosition.y == this.finalPosition.y) {
        float0 = 1.0F;
      }
  ...
  bool isDoSkipMovement() {
    if (this.character instanceof IsoZombie) {
      return this.character.isCurrentState(ZombieFallDownState.instance()) ||
             this.character.isCurrentState(ZombieOnGroundState.instance());
    } else {
      return !(this.character instanceof IsoPlayer)
                 ? false
                 : this.character.isCurrentState(
                       PlayerFallDownState.instance()) ||
                       this.character.isCurrentState(
                           PlayerKnockedDown.instance()) ||
                       this.character.isCurrentState(
                           PlayerOnGroundState.instance());
    }
  }

  std::string getStartDescription() {
    return String.format("start=[ pos=( %f ; %f ) dir=( %f ; %f ) angle=%f ]",
                         this.startPosition.x, this.startPosition.y,
                         this.startDirection.x, this.startDirection.y,
                         this.startAngle);
  }

  std::string getFinalDescription() {
    return String.format("final=[ pos=( %f ; %f ) dir=( %f ; %f ) angle=%f ]",
                         this.finalPosition.x, this.finalPosition.y,
                         this.finalDirection.x, this.finalDirection.y,
                         this.finalAngle);
  }

  std::string getActualDescription() {
    return String.format("actual=[ pos=( %f ; %f ) dir=( %f ; %f ) angle=%f ]",
  ...
  std::string getDescription() {
    return String.format(
        "start=%d | (x=%f,y=%f;a=%f;l=%f)", this.startTime,
        this.finalPosition.x, this.finalPosition.y, this.finalAngle,
        IsoUtils.DistanceTo(this.startPosition.x, this.startPosition.y,
                            this.finalPosition.x, this.finalPosition.y));
  }

  static void CalcHitReactionWeapon(IsoGameCharacter wielder,
                                    IsoGameCharacter target,
                                    HandWeapon weapon) {
    HitReactionNetworkAI hitReactionNetworkAI =
        target.getHitReactionNetworkAI();
    if (target.isOnFloor()) {
      hitReactionNetworkAI.setup(target.x, target.y, (byte)target.z,
                                 target.getAnimAngleRadians());
    } else {
      Vector2 vector = new Vector2();
      float float0 = target.calcHitDir(wielder, weapon, vector);
      if (target instanceof IsoPlayer) {
        vector.x =
            (vector.x + target.x + ((IsoPlayer)target).networkAI.targetX) *
            0.5F;
        vector.y =
            (vector.y + target.y + ((IsoPlayer)target).networkAI.targetY) *
            0.5F;
      } else {
      ...
      if (PolygonalMap2.instance.lineClearCollide(target.x, target.y, vector.x,
                                                  vector.y, (int)target.z,
                                                  nullptr, false, true)) {
        vector.x = target.x;
        vector.y = target.y;
      }
  ...
  static void CalcHitReactionVehicle(IsoGameCharacter target,
                                     BaseVehicle vehicle) {
    HitReactionNetworkAI hitReactionNetworkAI =
        target.getHitReactionNetworkAI();
    if (!hitReactionNetworkAI.isStarted()) {
      if (target.isOnFloor()) {
        ...
        if (PolygonalMap2.instance.lineClearCollide(
                target.x, target.y, vector.x, vector.y, (int)target.z, vehicle,
                false, true)) {
          vector.x = target.x;
          vector.y = target.y;
        }
  ...
  void process(float dropPositionX, float dropPositionY, float dropPositionZ,
               float dropDirection) {
    this.setup(dropPositionX, dropPositionY, (byte)dropPositionZ,
               dropDirection);
    this.start();
