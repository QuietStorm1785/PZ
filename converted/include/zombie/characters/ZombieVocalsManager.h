...
public:
 ...
 private HashSet<IsoZombie> m_added = std::make_unique<HashSet<>>();
 private ObjectPool<ZombieVocalsManager.ObjectWithDistance> m_objectPool = new ObjectPool<>(ZombieVocalsManager.ObjectWithDistance::new);
 private ArrayList<ZombieVocalsManager.ObjectWithDistance> m_objects = std::make_unique<ArrayList<>>();
 private ZombieVocalsManager.Slot[] m_slots;
 long m_updateMS = 0L;
 private Comparator<ZombieVocalsManager.ObjectWithDistance> comp = new Comparator<ZombieVocalsManager.ObjectWithDistance>() {
 int compare(ZombieVocalsManager.ObjectWithDistance objectWithDistance1, ZombieVocalsManager.ObjectWithDistance objectWithDistance0) {
 ...
 if (this->character != nullptr && this->character != zombie0 && this->character.vocalEvent != 0L) {
 ...
 std::string string = zombie0.isFemale() ? "FemaleZombieCombined" : "MaleZombieCombined";
