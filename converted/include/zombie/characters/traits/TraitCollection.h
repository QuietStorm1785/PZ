...
public:
 private List<String> m_activeTraitNames = std::make_unique<ArrayList<>>();
 private List<TraitCollection.TraitSlot> m_traits = std::make_unique<ArrayList<>>();
 ...
 return StringUtils.isNullOrWhitespace(name) ? nullptr : this->getOrCreateSlotInternal(name);
 }
 ...
 private TraitSlot(const std::string& string) {
 this->Name = string;
 this->m_isSet = false;
 }
