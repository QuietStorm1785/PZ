...
class TraitFactory {
public:
 public static LinkedHashMap<String, TraitFactory.Trait> TraitMap = std::make_unique<LinkedHashMap<>>();
 ...
 for (auto& entry : linkedList) linkedHashMap.put((String)entry.getKey(), (TraitFactory.Trait)entry.getValue());
 }
 ...
 private List<String> freeRecipes = std::make_unique<ArrayList<>>();
 public ArrayList<String> MutuallyExclusive = new ArrayList<>(0);
 public HashMap<PerkFactory.Perk, Integer> XPBoostMap = std::make_unique<HashMap<>>();
 ...
 public Trait(const std::string& tr, const std::string& _name, int _cost, const std::string& desc, bool _prof, bool _removeInMP) {
 this->traitID = tr;
 this->name = _name;
 this->cost = _cost;
