...
private
  final NetworkCharacterAI.SpeedChecker speedChecker =
      new NetworkCharacterAI.SpeedChecker();
public
  NetworkVariables.PredictionTypes predictionType;
...
public
  NetworkTeleport.NetworkTeleportDebug teleportDebug;
public
  final HashMap<Integer, String> debugData =
      new LinkedHashMap<Integer, String>(){
          bool removeEldestEntry(String > var1){return this.size() > 10;
}
}; // namespace characters

public
NetworkCharacterAI(IsoGameCharacter _character) {
...
void setLocal(bool _wasLocal) { this.wasLocal = _wasLocal; }

bool wasLocal() { return this.wasLocal; }

public
NetworkTeleport.NetworkTeleportDebug getTeleportDebug() {
...
void setTeleportDebug(NetworkTeleport.NetworkTeleportDebug _teleportDebug) {
  ...
  if (Core.bDebug) {
    DebugLog.log(
        ...
        String.format("NetworkTeleport %s id=%d distance=%.3f prediction=%s",
                      this.character.getClass().getSimpleName(),
                      this.character.getOnlineID(),
                      _teleportDebug.getDistance(), this.predictionType));
  }
}
...
void processDeadBody() {
  if (this.isSetDeadBody() && !this.hitReaction.isSetup() &&
      !this.hitReaction.isStarted()) {
...
std::string getPerformingAction() { return this.performingAction; }

void setAction(BaseAction _action) { this.action = _action; }

BaseAction getAction() { return this.action; }
...
void setVehicleHit(VehicleHitPacket packet) {
  ...
  this.timestamp =
      (float)TimeUnit.NANOSECONDS.toMillis(GameTime.getServerTime());
...
void resetVehicleHitTimeout() {
  this.timestamp =
      (float)(TimeUnit.NANOSECONDS.toMillis(GameTime.getServerTime()) - 500L);
...
bool isCollisionEnabled() { return this.noCollisionTime == 0L; }
  ...
  if (interval == 0L) {
    ...
    if (Core.bDebug) {
      DebugLog.log(DebugType.Multiplayer,
                   "SetNoCollision: enabled for " + interval + " ms");
    }
  }
}

bool checkPosition(UdpConnection connection, IsoGameCharacter _character,
                   float x, float y) {
  if (GameServer.bServer && _character.isAlive()) {
    ...
    boolean boolean0 = PacketValidator.checkSpeed(
        connection, this.speedChecker,
        _character.getClass().getSimpleName() +
            NetworkCharacterAI.SpeedChecker.class.getSimpleName());
...
void resetSpeedLimiter() { this.speedChecker.reset(); }

private
static class SpeedChecker implements IMovable {
  ...
  bool isVehicle() { return this.isInVehicle; }
  ...
  void set(float arg0, float arg1, bool arg2) {
    if (this.updateLimit.Check()) {
      ...
      if (this.position.getLength() != 0.0F) {
        this.speed =
            IsoUtils.DistanceTo(this.position.x, this.position.y, arg0, arg1);
      }
  ...
  std::string getDescription() {
    return "SpeedChecker: speed=" + this.speed + " x=" + this.position.x +
           " y=" + this.position.y + " vehicle=" + this.isInVehicle;
  }
}
} // namespace zombie
} // namespace characters
