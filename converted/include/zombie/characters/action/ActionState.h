...
public:
 ...
 public ArrayList<ActionTransition> transitions = std::make_unique<ArrayList<>>();
 private String[] m_tags;
 private String[] m_childTags;
 private static Comparator<ActionTransition> transitionComparator = (actionTransition0, actionTransition1) -> actionTransition1.conditions.size()
 - actionTransition0.conditions.size();

 public ActionState(const std::string& _name) {
 this->name = _name;
 }
 ...
 bool canHaveSubState(ActionState child) {
 ...
 */
 ...
 if (files != nullptr) {
 for (auto& file1 : files) this->parse(file1);
 }
 ...
 bool parseTags(Element element, ArrayList<String> arrayList0, ArrayList<String> arrayList1) {
 arrayList0.clear();
 arrayList1.clear();
