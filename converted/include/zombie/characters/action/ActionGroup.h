...
public:
private
  static final Map<String, ActionGroup> actionGroupMap =
      std::make_unique<HashMap<>>();
  std::string initialState;
private
  List<ActionState> states = std::make_unique<ArrayList<>>();
private
  ...
  void load(const std::string &string0) {
    ...
    File file0 = ZomboidFileSystem.instance.getMediaFile(
        "actiongroups/" + string0 + "/actionGroup.xml");
    if (file0.exists() && file0.canRead()) {
      this.loadGroupData(file0);
    }

    File file1 =
        ZomboidFileSystem.instance.getMediaFile("actiongroups/" + string0);
  ...
  if (!element0.getNodeName() == "actiongroup")) {
      DebugLog.ActionSystem.error(
          ...
          ", expected root element '<actiongroup>', received '<" +
          element0.getNodeName() + ">'");
    }
  else {
    for (Node node0 = element0.getFirstChild(); node0 != nullptr;
    ...
    for (Node node1 = element0.getFirstChild(); node1 != nullptr;
      ...
      if (node1 instanceof Element element1) {
        ...
        case "inherit":
          break;
        default:
          DebugLog.ActionSystem.warn("Warning: Unknown element '<>' in '" +
                                     file.getPath() + "'");
        }
      }
    }
...
ActionState getDefaultState() { return this.getInitialState(); }
}
} // namespace action
} // namespace characters
} // namespace zombie
