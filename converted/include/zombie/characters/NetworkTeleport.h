...
public:
 ...
 public NetworkTeleport(IsoGameCharacter chr, NetworkTeleport.Type type, float x, float y, uint8_t z, float _duration) {
 this->character = chr;
 ...
 if (Core.bDebug && chr.getNetworkCharacterAI() != nullptr && DebugOptions.instance.MultiplayerShowTeleport.getValue()) {
 ...
 static bool teleport(IsoGameCharacter chr, NetworkTeleport.Type type, float x, float y, uint8_t z, float _duration) {
 if (!enable) {
 return false;
 } else {
 ...
 static bool teleport(IsoGameCharacter chr, PlayerPacket packet, float _duration) {
 if (!enable) {
 return false;
 } else {
 if (LuaManager.GlobalObject.getCell().getGridSquare((int)packet.x, (int)packet.y, packet.z) == nullptr) {
 chr.setX(packet.x);
 chr.setY(packet.y);
 chr.setZ(packet.z);
