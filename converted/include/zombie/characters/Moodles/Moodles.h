...
private
  Stack<Moodle> MoodleList = std::make_unique<Stack<>>();
...
public
  ...
  std::string getMoodleDisplayString(int MoodleIndex) {
    return MoodleType.getDisplayName(
        this.MoodleList.get(MoodleIndex).Type,
        this.MoodleList.get(MoodleIndex).getLevel());
  }

  std::string getMoodleDescriptionString(int MoodleIndex) {
    return MoodleType.getDescriptionText(
        this.MoodleList.get(MoodleIndex).Type,
        this.MoodleList.get(MoodleIndex).getLevel());
  }
  ...
  int getNumMoodles() { return this.MoodleList.size(); }
