...
public:
 ...
 public DeadBodyAtlas.BodyTexture atlasTex = nullptr;
 ...
 int sight = -1;
 int hearing = -1;
 private ArrayList<InventoryItem> itemsToSpawnAtDeath = nullptr;
 float soundReactDelay = 0.0F;
 private IsoGameCharacter.Location delayedSound = new IsoGameCharacter.Location(-1, -1, -1);
 ...
 const ParameterCharacterInside parameterCharacterInside = new ParameterCharacterInside(this);
 ...
 const ParameterPlayerDistance parameterPlayerDistance = new ParameterPlayerDistance(this);
 ...
 const ParameterZombieState parameterZombieState = new ParameterZombieState(this);
 bool scratch = false;
 ...
 static const SharedSkeleAnimationRepository m_sharedSkeleRepo = new SharedSkeleAnimationRepository();
 int palette = 0;
 ...
 short OnlineID = -1;
 private static ArrayList<IsoDeadBody> tempBodies = std::make_unique<ArrayList<>>();
 ...
 bool bCanWalk = true;
 bool bRemote;
 private static IsoZombie.FloodFill floodFill = new IsoZombie.FloodFill();
 ...
 if (this->target instanceof IsoGameCharacter && ((IsoGameCharacter)this->target).ReanimatedCorpse != nullptr) {
 this->setTarget(nullptr);
 }
 ...
 return this->target != nullptr || this->soundSourceTarget != nullptr && !(this->soundSourceTarget instanceof IsoZombie);
 });
 ...
 IsoGridSquare square0 = this->getCurrentSquare();
 IsoGridSquare square1 = this->target.getCurrentSquare();
 if (square1 != nullptr && square1.isSomethingTo(square0) && this->getThumpTarget() != nullptr) {
 return false;
 ...
 return !(float0 > 3.5F)
 || float0 <= 4.0F && this->target instanceof IsoGameCharacter && ((IsoGameCharacter)this->target).getVehicle() != nullptr
 ...
 () -> this->target != nullptr && this->target instanceof IsoPlayer && ((IsoPlayer)this->target).getVehicle() != nullptr
 ...
 if (object.getSquare() == nullptr || this->DistToSquared(object.getX() + 0.5F, object.getY() + 0.5F) > 9.0F) {
 ...
 bool boolean0 = CrawlingZombieTurnState.calculateDir(this, directions0);
 ...
 bool boolean1 = CrawlingZombieTurnState.calculateDir(this, directions1);
 ...
 if (short0 >= 0 && short0 < arrayList.size() && this->wornItems.getBodyLocationGroup().getLocation(string) != nullptr) {
 ...
 } else if ((!(obj instanceof IsoDoor) || !((IsoDoor)obj).isHoppable()) && obj != nullptr && obj.getThumpableFor(this) != nullptr && boolean0) {
 boolean boolean1 = (
 this->isCurrentState(PathFindState.instance())
 || this->isCurrentState(WalkTowardState.instance())
 || this->isCurrentState(WalkTowardNetworkState.instance())
 ...
 void* object = obj;
 if (obj instanceof IsoWindow && obj.getThumpableFor(this) != nullptr && obj.isDestroyed()) {
 ...
 float Hit(HandWeapon weapon, IsoGameCharacter wielder, float damageSplit, bool bIgnoreDamage, float modDelta, bool _bRemote) {
 ...
 bool boolean0 = this->isCurrentState(WalkTowardState.instance()) || this->isCurrentState(PathFindState.instance());
 ...
 if (resultBiggestSound.sound != nullptr && (this->soundAttractTimeout == 0.0F || this->soundAttract * 2.0F < resultBiggestSound.attract) {
 worldSound = resultBiggestSound.sound;
 float1 = resultBiggestSound.attract;
 object = worldSound.source;
 }

 if (worldSound != nullptr && worldSound.bRepeating && worldSound.z == (int)this->z) {
 ...
 if (this->aggroList[int0] != nullptr && this->aggroList[int0].getAggro() <= 0.0F) {
 ...
 for (int int0 = 0; int0 < this->aggroList.length; int0++) {
 if (this->aggroList[int0] != nullptr && this->aggroList[int0].obj == other) {
 ...
 if (this->aggroList[int0] != nullptr && float0 < this->aggroList[int0].getAggro()) {
 ...
 float0 = 50.0F;
 if (other.getCurrentSquare().getRoom() != nullptr && this->getCurrentSquare().getRoom() == nullptr
 || other.getCurrentSquare().getRoom() == nullptr && this->getCurrentSquare().getRoom() != nullptr) {
 float0 = 20.0F;
 ...
 if (this->target instanceof IsoGameCharacter && ((IsoGameCharacter)this->target).getVehicle() != nullptr) {
 if ((
 ...
 if (player != nullptr && player.DistToSquared(this) < 225.0F) {
 ...
 bool boolean1 = SoundManager.instance.isListenerInRange(this->getX(), this->getY(), 15.0F);
 if (boolean1) {
 ...
 if (animationPlayer0 != nullptr && animationPlayer0.isReady() && animationPlayer1 != nullptr && animationPlayer1.isReady()) {
 ...
 if (IsoPlayer.players[int0] != nullptr && IsoPlayer.players[int0].DistTo(this) < float0 && !IsoPlayer.players[int0].isDead()) {
 ...
 if (!GameServer.bServer) {
 bool boolean0 = SoundManager.instance.isListenerInRange(this->getX(), this->getY(), 20.0F);
 ...
 && !PolygonalMap2.instance
 .lineClearCollide(this->getX(), this->getY(), deadBody.x, deadBody.y, (int)this->getZ(), nullptr, false, true) {
 ...
 return true;
 } else if (thumpable != nullptr && thumpable.getThumpableFor(this) != nullptr
 || window != nullptr && window.getThumpableFor(this) != nullptr
 ...
 if (window != nullptr && window.getThumpableFor(this) != nullptr) {
 ...
 return true;
 } else if (object != nullptr && IsoWindowFrame.isWindowFrame(object) {
 ...
 if (building != nullptr && building.getDef() != nullptr && building.getDef().getKeyId() != -1 && Rand.Next(4) == 0) {
 if (Rand.Next(10) != 1) {
 ...
 void setWalkType(const std::string& _walkType) {
 ...
 float Hit(BaseVehicle vehicle, float speed, bool isHitFromBehind, Vector2 hitDir) {
 float float0 = 0.0F;
 ...
 std::unordered_map hashMap = this->StateMachineParams.get(ClimbOverFenceState.instance());
 uint8_t byte0 = 3;
 uint8_t byte1 = 4;
 ...
 std::unordered_map hashMap = this->StateMachineParams.get(ClimbThroughWindowState.instance());
 uint8_t byte0 = 12;
 uint8_t byte1 = 13;
 int int0 = (Integer)hashMap.get(Integer.valueOf(byte0);
 int int1 = (Integer)hashMap.get(Integer.valueOf(byte1);
 ...
 void setWasFakeDead(bool wasFakeDead) {
 this->bWasFakeDead = wasFakeDead;
 }

 void setCrawler(bool crawling) {
 ...
 void setBecomeCrawler(bool crawler) {
 ...
 if (this->vocalEvent != 0L) {
 std::string string = female ? "FemaleZombieCombined" : "MaleZombieCombined";
 if (this->getEmitter() != nullptr && !this->getEmitter().isPlaying(string) {
 this->getEmitter().stopSoundLocal(this->vocalEvent);
 this->vocalEvent = 0L;
 }
 }
 ...
 */
 ...
 if (outfit != nullptr && !outfit.empty()) {
 ...
 void setCanCrawlUnderVehicle(bool b) {
 ...
 void setCanWalk(bool bCanStand) {
 ...
 && PolygonalMap2.instance.lineClearCollide(this->x, this->y, float0, float1, (int)this->z, nullptr) {
 return false;
 }
 }
 ...
 ? false
 : PolygonalMap2.instance.canStandAt(this->x, this->y, (int)this->z, nullptr, false, true);
 }
 }
 ...
 if (player != nullptr && player.isAlive() && !player.isOnFloor() && !player.isCurrentState(PlayerHitReactionState.instance())) {
 this->setEatBodyTarget(nullptr, false);
 }
 ...
 void setCrawlerType(int _crawlerType) {
 ...
 */
 ...
 std::string string = OutfitRNG.pickRandom(ScriptManager.instance.getZedDmgMap());
 ...
 void setThumpTimer(int _thumpTimer) {
 ...
 void setTargetSeenTime(float seconds) {
 ...
 void setBodyToEat(IsoDeadBody body) {
 this->bodyToEat = body;
 }
 ...
 void setOnlyJawStab(bool _onlyJawStab) {
 ...
 void setStaggerBack(bool _bStaggerBack) {
 ...
 void setKnifeDeath(bool _bKnifeDeath) {
 ...
 Mover mover = nullptr;
 private ArrayList<IsoGridSquare> choices = new ArrayList<>(121);
 ...
 int radius;
 private static IsoZombie.ZombieSound[] values = values();
 ...
 static const PerformanceProfileProbe update = new PerformanceProfileProbe("IsoZombie.update");
 static const PerformanceProfileProbe postUpdate = new PerformanceProfileProbe("IsoZombie.postUpdate");
 }
}
} // namespace characters
} // namespace zombie
