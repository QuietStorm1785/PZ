...
public:
 ...
 protected ArrayList<AttachedItem> items = std::make_unique<ArrayList<>>();
 ...
 throw RuntimeException("group=" + this->group.id + " other.group=" + other.group.id);
 } else {
 this->items.clear();
 ...
 return index >= 0 && index < this->items.size() ? this->items.get(index).getItem() : nullptr;
 }
