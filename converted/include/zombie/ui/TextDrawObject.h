...
private
  String[] validImages =
      new String[]{"Icon_music_notes", "media/ui/CarKey.png",
                   "media/ui/ArrowUp.png", "media/ui/ArrowDown.png"};
private
  String[] validFonts =
      new String[]{"Small", "Dialogue", "Medium", "Code", "Large", "Massive"};
private
  final ArrayList<TextDrawObject.DrawLine> lines =
      std::make_unique<ArrayList<>>();
  int width = 0;
  int height = 0;
  int maxCharsLine = -1;
  UIFont defaultFontEnum = UIFont.Dialogue;
  AngelCodeFont defaultFont = null;
  std::string original = "";
  std::string unformatted = "";
private
  TextDrawObject.DrawLine currentLine;
private
  TextDrawObject.DrawElement currentElement;
  ...
  int drawMode = 0;
private
  static ArrayList<TextDrawObject.RenderBatch> renderBatch =
      std::make_unique<ArrayList<>>();
private
  static ArrayDeque<TextDrawObject.RenderBatch> renderBatchPool =
      std::make_unique<ArrayDeque<>>();
  std::string elemText;

public
  TextDrawObject() { this(255, 255, 255, true, true, true, true, true, false); }

public
  TextDrawObject(int r, int g, int b, bool _allowBBcode) {
    this(r, g, b, _allowBBcode, true, true, true, true, false);
  }

public
  TextDrawObject(int r, int g, int b, boolean _allowBBcode,
                 boolean _allowImages, boolean _allowChatIcons,
                 boolean _allowColors, boolean _allowFonts,
                 boolean _equalizeLineHeights) {
    this.setSettings(_allowBBcode, _allowImages, _allowChatIcons, _allowColors,
                     _allowFonts, _equalizeLineHeights);
  ...
  void setVisibleRadius(int radius) { this.visibleRadius = radius; }

  int getVisibleRadius() { return this.visibleRadius; }

  void setDrawBackground(bool draw) { this.drawBackground = draw; }
...
public
  void setSettings(boolean _allowBBcode, boolean _allowImages,
                   boolean _allowChatIcons, boolean _allowColors,
                   boolean _allowFonts, boolean _equalizeLineHeights) {
  ...
  void setValidImages(String[] strings) { this.validImages = strings; }

  void setValidFonts(String[] strings) { this.validFonts = strings; }
  ...
  void setHorizontalAlign(TextDrawHorizontal horz) { this.defaultHorz = horz; }

  TextDrawHorizontal getHorizontalAlign() { return this.defaultHorz; }

  std::string getOriginal() { return this.original; }
  ...
  UIFont getDefaultFontEnum() { return this.defaultFontEnum; }

  bool isNullOrZeroLength() {
    return this.original == nullptr || this.original.length() == 0;
  }

  float getInternalClock() { return this.internalClock; }
  ...
  float updateInternalTickClock() {
    return this.updateInternalTickClock(1.25F *
  ...
  float getScrambleVal() { return this.scrambleVal; }
  ...
  void addWord(const std::string &string) {
    if (this.maxCharsLine > 0 &&
      ...
      for (int int0 = 0; int0 < string.length() / this.maxCharsLine + 1;
        ...
        int int2 = int1 + this.maxCharsLine < string.length()
                       ? int1 + this.maxCharsLine
                       : string.length();
    ...
    if (this.allowFonts && string0 == "fnt")) {
        ...
        if (string1 != nullptr && this.isValidFont(string1)) {
          ...
          this.currentElement.font =
              TextManager.instance.getFontFromEnum(this.currentElement.f);
          this.hasOpened = true;
        ...
        if (string2 != nullptr && string2.trim().length() > 0) {
          ...
          if (this.allowChatIcons &&
              this.isValidImage(this.currentElement.text)) {
            this.currentElement.tex =
                Texture.getSharedTexture(this.currentElement.text);
            this.currentElement.isTextImage = true;
          } else if (this.allowImages) {
            this.currentElement.tex =
                Texture.getSharedTexture("Item_" + this.currentElement.text);
            if (this.currentElement.tex == nullptr) {
              this.currentElement.tex = Texture.getSharedTexture(
                  "media/ui/Container_" + this.currentElement.text);
            }
      ...
      for (int int0 = 0; int0 < chars.length; int0++) {
        ...
        if (this.allowBBcode && char0 == '[') {
          ...
          if (int0 + 4 < chars.length) {
            std::string string1 =
                ("" + chars[int0 + 1] + chars[int0 + 2] + chars[int0 + 3])
                    .toLowerCase();
          ...
          if (this.hasOpened && int0 + 2 < chars.length &&
              chars[int0 + 1] == '/' && chars[int0 + 2] == ']') {
            this.hasOpened = false;
            this.addNewElement();
            int0 += 2;
            continue;
        ...
        if (Character.isWhitespace(char0) && int0 > 0 &&
  ...
  void Draw(double x, double y) {
    this.Draw(this.defaultHorz, x, y, this.defaultR, this.defaultG,
              this.defaultB, this.defaultA, false);
  }

  void Draw(double x, double y, bool drawOutlines) {
    this.Draw(this.defaultHorz, x, y, this.defaultR, this.defaultG,
              this.defaultB, this.defaultA, drawOutlines);
  }

  void Draw(double x, double y, bool drawOutlines, float alpha) {
    this.Draw(this.defaultHorz, x, y, this.defaultR, this.defaultG,
              this.defaultB, alpha, drawOutlines);
  }
  ...
  void Draw(TextDrawHorizontal horz, double x, double y, double r, double g,
            double b, double a, bool drawOutlines) {
    this.DrawRaw(horz, x, y, (float)r, (float)g, (float)b, (float)a,
                 drawOutlines);
  }

  void AddBatchedDraw(double x, double y) {
    this.AddBatchedDraw(this.defaultHorz, x, y, this.defaultR, this.defaultG,
                        this.defaultB, this.defaultA, false);
  }
  ...
  void AddBatchedDraw(double x, double y, double r, double g, double b,
                      double a, bool drawOutlines) {
    this.AddBatchedDraw(this.defaultHorz, x, y, r, g, b, a, drawOutlines);
  }

  void AddBatchedDraw(TextDrawHorizontal horz, double x, double y, double r,
                      double g, double b, double a, bool drawOutlines) {
    if (!GameServer.bServer) {
      TextDrawObject.RenderBatch renderBatchx =
          renderBatchPool.isEmpty() ? new TextDrawObject.RenderBatch()
                                    : renderBatchPool.pop();
    ...
    if (renderBatch.size() > 0) {
      for (int int0 = 0; int0 < renderBatch.size(); int0++) {
        ...
        if (renderBatchx.playerNum == playerNum) {
          renderBatchx.element.DrawRaw(
              renderBatchx.horz, renderBatchx.x, renderBatchx.y, renderBatchx.r,
              renderBatchx.g, renderBatchx.b, renderBatchx.a,
  ...
  void DrawRaw(TextDrawHorizontal horz, double x, double y, float r, float g,
               float b, float a, bool drawOutlines) {
    double double0 = x;
    double double1 = y;
    double double2 = 0.0;
    ...
    if (!(double0 - byte0 >= int0) &&
        !(double0 + this.getWidth() + byte0 <= 0.0) && !(y - byte0 >= int1) &&
        !(y + this.getHeight() + byte0 <= 0.0)) {
      if (this.drawBackground && ChatElement.backdropTexture != nullptr) {
        ChatElement.backdropTexture.renderInnerBased(
            (int)double0, (int)y, this.getWidth(), this.getHeight(), 0.0F, 0.0F,
            0.0F, 0.4F * a);
      }
      ...
      for (int int2 = 0; int2 < this.lines.size(); int2++) {
        ...
        for (int int3 = 0; int3 < drawLine.elements.size(); int3++) {
          ...
          this.elemText = this.scrambleVal > 0.0F ? drawElement.scrambleText
                                                  : drawElement.text;
          if (drawElement.isImage && drawElement.tex != nullptr) {
            if (drawOutlines && drawElement.isTextImage) {
              SpriteRenderer.instance.renderi(
                  drawElement.tex, (int)(double0 - 1.0),
                  (int)(double1 + double2 - 1.0), drawElement.w, drawElement.h,
                  this.outlineR, this.outlineG, this.outlineB, float0, nullptr);
              SpriteRenderer.instance.renderi(
                  drawElement.tex, (int)(double0 + 1.0),
                  (int)(double1 + double2 + 1.0), drawElement.w, drawElement.h,
                  this.outlineR, this.outlineG, this.outlineB, float0, nullptr);
            ...
            if (drawElement.useColor) {
              SpriteRenderer.instance.renderi(
                  drawElement.tex, (int)double0, (int)(double1 + double2),
                  drawElement.w, drawElement.h, drawElement.R, drawElement.G,
                  drawElement.B, a, nullptr);
            } else if (drawElement.isTextImage) {
              SpriteRenderer.instance.renderi(
                  drawElement.tex, (int)double0, (int)(double1 + double2),
                  drawElement.w, drawElement.h, r, g, b, a, nullptr);
            } else {
              SpriteRenderer.instance.renderi(
                  drawElement.tex, (int)double0, (int)(double1 + double2),
                  drawElement.w, drawElement.h, 1.0F, 1.0F, 1.0F, a, nullptr);
            }
          } else if (drawElement.useFont && drawElement.font != nullptr) {
            if (drawOutlines) {
              drawElement.font.drawString((float)(double0 - 1.0),
                                          (float)(double1 + double2 - 1.0),
                                          this.elemText, this.outlineR,
                                          this.outlineG, this.outlineB, float0);
              drawElement.font.drawString((float)(double0 + 1.0),
                                          (float)(double1 + double2 + 1.0),
                                          this.elemText, this.outlineR,
                                          this.outlineG, this.outlineB, float0);
            ...
            drawElement.font.drawString((float)double0,
                                        (float)(double1 + double2),
                                        this.elemText, r, g, b, a);
          } else if (this.defaultFont != nullptr) {
            if (drawOutlines) {
              this.defaultFont.drawString((float)(double0 - 1.0),
                                          (float)(double1 + double2 - 1.0),
                                          this.elemText, this.outlineR,
                                          this.outlineG, this.outlineB, float0);
              this.defaultFont.drawString((float)(double0 + 1.0),
            ...
            if (drawElement.useColor) {
              this.defaultFont.drawString(
                  (float)double0, (float)(double1 + double2), this.elemText,
                  drawElement.R, drawElement.G, drawElement.B, a);
            } else {
              this.defaultFont.drawString((float)double0,
                                          (float)(double1 + double2),
                                          this.elemText, r, g, b, a);
            }
          }
...
private
  static final class DrawElement {
    std::string text = "";
...
private
  TextDrawObject.DrawElement softclone() {
...
private
  final ArrayList<TextDrawObject.DrawElement> elements =
      std::make_unique<ArrayList<>>();
  int h = 0;
  int w = 0;
  int charW = 0;
