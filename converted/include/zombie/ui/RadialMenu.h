...
public:
 int outerRadius = 200;
 int innerRadius = 100;
 protected ArrayList<RadialMenu.Slice> slices = std::make_unique<ArrayList<>>();
 int highlight = -1;
 int joypad = -1;
 ...
 return int0 >= 0 && int0 < this->slices.size() ? this->slices.get(int0) : nullptr;
 }
