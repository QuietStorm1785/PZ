...
public:
 ...
 Texture mask;
 Texture border;
 private ArrayList<RadarPanel.ZombiePos> zombiePos = std::make_unique<ArrayList<>>();
 ...
 void renderRect(float float1, float float3, float float5, float float7, float float8, float float9, float float10, float float11) {
 ...
 private static class ZombiePos {
 float x;
 float y;
 ...
 private ArrayDeque<RadarPanel.ZombiePos> pool = std::make_unique<ArrayDeque<>>();

 public RadarPanel.ZombiePos alloc(float arg0, float arg1) {
 return this->pool.empty() ? new RadarPanel.ZombiePos(arg0, arg1) : this->pool.pop().set(arg0, arg1);
 }
