...
public:
  ...
  std::string clickedValue = null;
public
  final ArrayList<UIElement> Controls = std::make_unique<ArrayList<>>();
  ...
  bool enabled = true;
private
  final ArrayList<UIElement> toTop = new ArrayList<>(0);
...
public
  UIElement(KahluaTable _table) { this.table = _table; }

  double getMaxDrawHeight() {
    return BoxedStaticValues.toDouble(this.maxDrawHeight);
  }

  void setMaxDrawHeight(double _height) { this.maxDrawHeight = (int)_height; }

  void clearMaxDrawHeight() { this.maxDrawHeight = -1; }

  double getXScroll() { return this.xScroll; }

  void setXScroll(double _x) { this.xScroll = _x; }

  double getYScroll() { return this.yScroll; }

  void setYScroll(double _y) { this.yScroll = _y; }

  void setAlwaysOnTop(bool b) { this.alwaysOnTop = b; }

  void backMost() { this.alwaysBack = true; }
  ...
  void setScrollHeight(double h) { this.scrollHeight = (int)h; }

  bool isConsumeMouseEvents() { return this.bConsumeMouseEvents; }
  ...
  void DrawTextUntrimmed(UIFont font, const std::string &text, double _x,
                         ...
                         double alpha) {
    if (text != nullptr) {
      TextManager.instance.DrawStringUntrimmed(
          font, _x + this.getAbsoluteX() + this.xScroll,
          _y + this.getAbsoluteY() + this.yScroll, text, r, g, b, alpha);
    }
  }
  ...
  void DrawTextureAngle(Texture tex, double centerX, double centerY,
                        double angle, double r, double g, double b, double a) {
    if (this.isVisible()) {
      ...
      SpriteRenderer.instance.render(
          tex, double5, double6, double7, double8, double9, double10, double11,
          double12, (float)r, (float)g, (float)b, (float)a, (float)r, (float)g,
          (float)b, (float)a, (float)r, (float)g, (float)b, (float)a, (float)r,
...
public
  void DrawTexture(Texture tex, double tlx, double tly, double trx, double try2,
                   double brx, double bry, double blx, double bly, double r,
                   double g, double b, double a) {
    SpriteRenderer.instance.render(
        tex, tlx, tly, trx, try2, brx, bry, blx, bly, (float)r, (float)g,
        (float)b, (float)a, (float)r, (float)g, (float)b, (float)a, (float)r,
        (float)g, (float)b, (float)a, (float)r, (float)g, (float)b, (float)a,
        nullptr);
  }
  ...
  void DrawTexture(Texture tex, double _x, double _y, double alpha) {
    if (this.isVisible()) {
      ...
      if (int0 + tex.getHeight() >= 0 && int0 <= 4096) {
        SpriteRenderer.instance.renderi(tex, (int)(double0 + this.xScroll),
                                        (int)(double1 + this.yScroll),
                                        tex.getWidth(), tex.getHeight(), 1.0F,
                                        1.0F, 1.0F, (float)alpha, nullptr);
      }
    }
  }
    ...
    if (this.isVisible()) {
      ...
      if (int2 + int1 >= 0 && int2 <= 4096) {
        SpriteRenderer.instance.renderi(
            tex, (int)(double0 + this.xScroll), (int)(double1 + this.yScroll),
            int0, int1, col.r, col.g, col.b, col.a, nullptr);
      }
    }
  }

  void DrawTextureScaled(Texture tex, double _x, double _y, double _width,
                         double _height, double alpha) {
    if (this.isVisible()) {
      double double0 = _x + this.getAbsoluteX();
      double double1 = _y + this.getAbsoluteY();
      SpriteRenderer.instance.renderi(
          tex, (int)(double0 + this.xScroll), (int)(double1 + this.yScroll),
          (int)_width, (int)_height, 1.0F, 1.0F, 1.0F, (float)alpha, nullptr);
    }
  }

  void DrawTextureScaledUniform(Texture tex, double _x, double _y, double scale,
                                double r, double g, double b, double alpha) {
    if (this.isVisible() && tex != nullptr) {
      ...
      SpriteRenderer.instance.renderi(
          tex, (int)(double0 + this.xScroll), (int)(double1 + this.yScroll),
          (int)(tex.getWidth() * scale), (int)(tex.getHeight() * scale),
          (float)r, (float)g, (float)b, (float)alpha, nullptr);
    }
  }

  void DrawTextureScaledAspect(Texture tex, double _x, double _y, double _width,
                               double _height, double r, double g, double b,
                               double alpha) {
    if (this.isVisible() && tex != nullptr) {
      double double0 = _x + this.getAbsoluteX();
      double double1 = _y + this.getAbsoluteY();
      if (tex.getWidth() > 0 && tex.getHeight() > 0 && _width > 0.0 &&
          _height > 0.0) {
        double double2 = Math.min(_width / tex.getWidthOrig(),
                                  _height / tex.getHeightOrig());
      ...
      SpriteRenderer.instance.renderi(
          tex, (int)(double0 + this.xScroll), (int)(double1 + this.yScroll),
          (int)_width, (int)_height, (float)r, (float)g, (float)b, (float)alpha,
          nullptr);
    }
  }

  void DrawTextureScaledAspect2(Texture tex, double _x, double _y,
                                double _width, double _height, double r,
                                double g, double b, double alpha) {
    if (this.isVisible() && tex != nullptr) {
      double double0 = _x + this.getAbsoluteX();
      double double1 = _y + this.getAbsoluteY();
      if (tex.getWidth() > 0 && tex.getHeight() > 0 && _width > 0.0 &&
          _height > 0.0) {
        double double2 =
            Math.min(_width / tex.getWidth(), _height / tex.getHeight());
        double double3 = _width;
        double double4 = _height;
        _width = tex.getWidth() * double2;
        _height = tex.getHeight() * double2;
      ...
      SpriteRenderer.instance.render(tex, (int)(double0 + this.xScroll),
                                     (int)(double1 + this.yScroll), (int)_width,
                                     (int)_height, (float)r, (float)g, (float)b,
                                     (float)alpha, nullptr);
    }
  }

  void DrawTextureScaledCol(Texture tex, double _x, double _y, double _width,
                            double _height, double r, double g, double b,
                            double a) {
    if (tex != nullptr) {
      bool boolean0 = false;
    ...
    if (this.isVisible()) {
      ...
      if (!(int0 + _height < 0.0) && int0 <= 4096) {
        SpriteRenderer.instance.renderi(tex, (int)(double0 + this.xScroll),
                                        (int)(double1 + this.yScroll),
                                        (int)_width, (int)_height, (float)r,
                                        (float)g, (float)b, (float)a, nullptr);
      }
    }
  }
  ...
  void DrawTextureScaledCol(Texture tex, double _x, double _y, double _width,
    ...
    if (this.isVisible()) {
      ...
      SpriteRenderer.instance.render(
          tex, (int)(double0 + this.xScroll), (int)(double1 + this.yScroll),
          (int)_width, (int)_height, col.r, col.g, col.b, col.a, nullptr);
    }
  }

  void DrawTextureScaledColor(Texture tex, double _x, double _y, double _width,
                              double _height, double r, double g, double b,
                              double a) {
    this.DrawTextureScaledCol(tex, _x, _y, _width, _height, r, g, b, a);
  }

  void DrawTextureColor(Texture tex, double _x, double _y, double r, double g,
                        double b, double a) {
    tempcol.r = (float)r;
    tempcol.g = (float)g;
    tempcol.b = (float)b;
...
public
  void DrawSubTextureRGBA(Texture tex, double subX, double subY, double subW,
                          double subH, double _x, double _y, double w, double h,
                          double r, double g, double b, double a) {
    if (tex != nullptr && this.isVisible() && !(subW <= 0.0) &&
        !(subH <= 0.0) && !(w <= 0.0) && !(h <= 0.0)) {
  ...
  void DrawTextureTiled(Texture tex, double _x, double _y, double w, double h,
                        double r, double g, double b, double a) {
    if (tex != nullptr && this.isVisible() && !(w <= 0.0) && !(h <= 0.0)) {
      for (double double0 = _y; double0 < _y + h; double0 += tex.getHeight()) {
        for (double double1 = _x; double1 < _x + w; double1 += tex.getWidth()) {
          ...
          this.DrawSubTextureRGBA(tex, 0.0, 0.0, double2, double3, double1,
                                  double0, double2, double3, r, g, b, a);
        }
      }
    }
  }

  void DrawTextureTiledX(Texture tex, double _x, double _y, double w, double h,
                         double r, double g, double b, double a) {
    if (tex != nullptr && this.isVisible() && !(w <= 0.0) && !(h <= 0.0)) {
      for (double double0 = _x; double0 < _x + w; double0 += tex.getWidth()) {
        ...
        this.DrawSubTextureRGBA(tex, 0.0, 0.0, double1, double2, double0, _y,
                                double1, double2, r, g, b, a);
      }
    }
  }

  void DrawTextureTiledY(Texture tex, double _x, double _y, double w, double h,
                         double r, double g, double b, double a) {
    if (tex != nullptr && this.isVisible() && !(w <= 0.0) && !(h <= 0.0)) {
      for (double double0 = _y; double0 < _y + h; double0 += tex.getHeight()) {
        double double1 = tex.getWidth();
        ...
        this.DrawSubTextureRGBA(tex, 0.0, 0.0, double1, double2, _x, double0,
                                double1, double2, r, g, b, a);
      }
    }
  }

  void DrawTextureIgnoreOffset(Texture tex, double _x, double _y, int _width,
                               int _height, Color col) {
    if (this.isVisible()) {
      double double0 = _x + this.getAbsoluteX();
      double double1 = _y + this.getAbsoluteY();
      SpriteRenderer.instance.render(
          tex, (int)(double0 + this.xScroll), (int)(double1 + this.yScroll),
          _width, _height, col.r, col.g, col.b, col.a, nullptr);
    }
  }

  void DrawTexture_FlippedX(Texture tex, double _x, double _y, int _width,
                            int _height, Color col) {
    if (this.isVisible()) {
      double double0 = _x + this.getAbsoluteX();
      double double1 = _y + this.getAbsoluteY();
      SpriteRenderer.instance.renderflipped(
          tex, (float)(double0 + this.xScroll), (float)(double1 + this.yScroll),
          _width, _height, col.r, col.g, col.b, col.a, nullptr);
    }
  }

  void DrawTexture_FlippedXIgnoreOffset(Texture tex, double _x, double _y,
                                        int _width, int _height, Color col) {
    if (this.isVisible()) {
      double double0 = _x + this.getAbsoluteX();
      double double1 = _y + this.getAbsoluteY();
      SpriteRenderer.instance.renderflipped(
...
public
  void DrawUVSliceTexture(Texture tex, double _x, double _y, double _width,
                          double _height, Color col, double xStart,
    ...
    if (this.isVisible()) {
      ...
      SpriteRenderer.instance.render(
          tex, (float)double0 + this.getXScroll().intValue(),
          (float)double1 + this.getYScroll().intValue(),
          (float)(double12 - double0), (float)(double13 - double1), col.r,
          col.g, col.b, col.a, (float)double2, (float)double3, (float)double4,
          (float)double3, (float)double4, (float)double5, (float)double2,
          (float)double5);
    }
  }
  ...
  void setScrollChildren(bool bScroll) { this.bScrollChildren = bScroll; }

  bool getScrollWithParent() {
    return this.bScrollWithParent ? Boolean.TRUE : Boolean.FALSE;
  }

  void setScrollWithParent(bool bScroll) { this.bScrollWithParent = bScroll; }

  void setRenderClippedChildren(bool b) { this.bRenderClippedChildren = b; }
    ...
    if (this.getParent() != nullptr) {
      return this.getParent().getScrollChildren() && this.getScrollWithParent()
                 ? BoxedStaticValues.toDouble(
                       ...
                       this.getX().intValue() +
                       this.getParent().getXScroll().intValue())
                 : BoxedStaticValues.toDouble(this.getParent().getAbsoluteX() +
                                              this.getX().intValue());
    } else {
  ...
  double getAbsoluteY() {
    if (this.getParent() != nullptr) {
      return this.getParent().getScrollChildren() && this.getScrollWithParent()
                 ? BoxedStaticValues.toDouble(
                       this.getParent().getAbsoluteY() +
                       this.getY().intValue() +
                       this.getParent().getYScroll().intValue())
                 : BoxedStaticValues.toDouble(this.getParent().getAbsoluteY() +
                                              this.getY().intValue());
    } else {
  ...
  std::string getClickedValue() { return this.clickedValue; }
  ...
  void onRightMouseUpOutside(double double1, double double0) {
    if (this.getTable() != nullptr &&
        this.getTable().rawget("onRightMouseUpOutside") != nullptr) {
      LuaManager.caller.protectedCallVoid(
          UIManager.getDefaultThread(),
          this.getTable().rawget("onRightMouseUpOutside"), this.table,
    ...
    for (int int0 = this.getControls().size() - 1; int0 >= 0; int0--) {
      ...
      uIElement1.onRightMouseUpOutside(
          double1 - uIElement1.getXScrolled(this).intValue(),
          double0 - uIElement1.getYScrolled(this).intValue());
    }
  }

  void onRightMouseDownOutside(double double1, double double0) {
    if (this.getTable() != nullptr &&
        this.getTable().rawget("onRightMouseDownOutside") != nullptr) {
      LuaManager.caller.protectedCallVoid(
          UIManager.getDefaultThread(),
          this.getTable().rawget("onRightMouseDownOutside"), this.table,
          BoxedStaticValues.toDouble(double1 - this.xScroll),
          BoxedStaticValues.toDouble(double0 - this.yScroll));
    }
  ...
  void onMouseUpOutside(double _x, double _y) {
    if (this.getTable() != nullptr &&
        this.getTable().rawget("onMouseUpOutside") != nullptr) {
      LuaManager.caller.protectedCallVoid(
          UIManager.getDefaultThread(),
          this.getTable().rawget("onMouseUpOutside"), this.table,
    ...
    for (int int0 = this.getControls().size() - 1; int0 >= 0; int0--) {
      ...
      uIElement.onMouseUpOutside(_x - uIElement.getXScrolled(this).intValue(),
                                 _y - uIElement.getYScrolled(this).intValue());
    }
  }

  void onMouseDownOutside(double double1, double double0) {
    if (this.getTable() != nullptr &&
        this.getTable().rawget("onMouseDownOutside") != nullptr) {
      LuaManager.caller.protectedCallVoid(
          UIManager.getDefaultThread(),
          this.getTable().rawget("onMouseDownOutside"), this.table,
          BoxedStaticValues.toDouble(double1 - this.xScroll),
          BoxedStaticValues.toDouble(double0 - this.yScroll));
    }
    ...
    for (int int0 = this.getControls().size() - 1; int0 >= 0; int0--) {
      UIElement uIElement1 = this.getControls().get(int0);
      uIElement1.onMouseDownOutside(double1 - uIElement1.getX().intValue(),
                                    double0 - uIElement1.getY().intValue());
    }
  }

  bool onMouseDown(double _x, double _y) {
    if (this.clicked &&
        UIManager.isDoubleClick((int)this.clickX, (int)this.clickY, (int)_x,
                                (int)_y, this.leftDownTime) &&
      ...
      if (this.Parent != nullptr && this.Parent.maxDrawHeight != -1 &&
        ...
        if (this.getTable() != nullptr &&
            this.getTable().rawget("onFocus") != nullptr) {
          LuaManager.caller.protectedCallVoid(
              UIManager.getDefaultThread(), this.getTable().rawget("onFocus"),
              this.table, BoxedStaticValues.toDouble(_x - this.xScroll),
              BoxedStaticValues.toDouble(_y - this.yScroll));
        }
        ...
        for (int int0 = this.getControls().size() - 1; int0 >= 0; int0--) {
          ...
          if (!boolean0 &&
            ...
            if (uIElement.onMouseDown(
                    _x - uIElement.getXScrolled(this).intValue(),
                    _y - uIElement.getYScrolled(this).intValue())) {
              boolean0 = true;
            }
          } else if (uIElement.getTable() != nullptr &&
        ...
        if (this.getTable() != nullptr) {
          if (boolean0) {
            if (this.getTable().rawget("onMouseDownOutside") != nullptr) {
              Boolean boolean1 = LuaManager.caller.protectedCallBoolean(
                  UIManager.getDefaultThread(),
                  this.getTable().rawget("onMouseDownOutside"), this.table,
                  BoxedStaticValues.toDouble(_x - this.xScroll),
                  BoxedStaticValues.toDouble(_y - this.yScroll));
            ...
            Boolean boolean2 = LuaManager.caller.protectedCallBoolean(
                UIManager.getDefaultThread(),
                this.getTable().rawget("onMouseDown"), this.table,
                BoxedStaticValues.toDouble(_x - this.xScroll),
                BoxedStaticValues.toDouble(_y - this.yScroll));
            if (boolean2 == nullptr) {
              return Boolean.TRUE;
  ...
  bool onMouseDoubleClick(double double1, double double0) {
    if (this.Parent != nullptr && this.Parent.maxDrawHeight != -1 &&
      ...
      if (this.getTable().rawget("onMouseDoubleClick") != nullptr) {
        Boolean boolean0 = LuaManager.caller.protectedCallBoolean(
            UIManager.getDefaultThread(),
            this.getTable().rawget("onMouseDoubleClick"), this.table,
            BoxedStaticValues.toDouble(double1 - this.xScroll),
            BoxedStaticValues.toDouble(double0 - this.yScroll));
        if (boolean0 == nullptr) {
          return Boolean.TRUE;
  ...
  bool onMouseWheel(double del) {
    ...
    if (this.getTable() != nullptr &&
        this.getTable().rawget("onMouseWheel") != nullptr) {
      Boolean boolean0 = LuaManager.caller.protectedCallBoolean(
          UIManager.getDefaultThread(), this.getTable().rawget("onMouseWheel"),
          this.table, BoxedStaticValues.toDouble(del));
      if (boolean0 == Boolean.TRUE) {
        return Boolean.TRUE;
      }
    }
  ...
  bool onMouseMove(double dx, double dy) {
    ...
    if (this.Parent != nullptr && this.Parent.maxDrawHeight != -1 &&
        this.Parent.maxDrawHeight <= this.y) {
      return Boolean.FALSE;
    } else if (this.maxDrawHeight != -1 &&
               this.maxDrawHeight <= int1 - this.getAbsoluteY()) {
      return Boolean.FALSE;
    } else if (!this.visible) {
      return Boolean.FALSE;
    } else {
      if (this.getTable() != nullptr &&
        ...
        LuaManager.caller.protectedCallVoid(
            UIManager.getDefaultThread(), this.getTable().rawget("onMouseMove"),
            this.table, BoxedStaticValues.toDouble(dx),
      ...
      for (int int2 = this.getControls().size() - 1; int2 >= 0; int2--) {
        UIElement uIElement = this.getControls().get(int2);
        if ((!(int0 >= uIElement.getAbsoluteX()) ||
             !(int1 >= uIElement.getAbsoluteY()) ||
             !(int0 < uIElement.getAbsoluteX() + uIElement.getWidth()) ||
             !(int1 < uIElement.getAbsoluteY() + uIElement.getHeight())) &&
            !uIElement.isCapture()) {
  ...
  void onMouseMoveOutside(double dx, double dy) {
    if (this.getTable() != nullptr &&
      ...
      LuaManager.caller.protectedCallVoid(
          UIManager.getDefaultThread(),
          this.getTable().rawget("onMouseMoveOutside"), this.table,
          BoxedStaticValues.toDouble(dx), BoxedStaticValues.toDouble(dy));
    }
    ...
    if (this.Parent != nullptr && this.Parent.maxDrawHeight != -1 &&
      ...
      if (this.getTable() != nullptr) {
        if (boolean0) {
          ...
          Boolean boolean2 = LuaManager.caller.protectedCallBoolean(
              UIManager.getDefaultThread(), this.getTable().rawget("onMouseUp"),
              this.table, BoxedStaticValues.toDouble(_x - this.xScroll),
              BoxedStaticValues.toDouble(_y - this.yScroll));
          if (boolean2 == nullptr) {
            return Boolean.TRUE;
  ...
  void onResize() {
    ...
    if (this.getTable() != nullptr &&
        this.getTable().rawget("onResize") != nullptr) {
      LuaManager.caller.pcallvoid(
          UIManager.getDefaultThread(), this.getTable().rawget("onResize"),
          this.table, this.getWidth(), this.getHeight());
    }
  ...
  bool onRightMouseDown(double _x, double _y) {
    if (!this.isVisible()) {
      return Boolean.FALSE;
    } else if (this.Parent != nullptr && this.Parent.maxDrawHeight != -1 &&
               this.Parent.maxDrawHeight <= _y) {
      return Boolean.FALSE;
    } else if (this.maxDrawHeight != -1 && this.maxDrawHeight <= _y) {
      return Boolean.FALSE;
      ...
      if (this.getTable() != nullptr) {
        if (boolean0) {
          ...
          Boolean boolean2 = LuaManager.caller.protectedCallBoolean(
              UIManager.getDefaultThread(),
              this.getTable().rawget("onRightMouseDown"), this.table,
              BoxedStaticValues.toDouble(_x - this.xScroll),
              BoxedStaticValues.toDouble(_y - this.yScroll));
          if (boolean2 == nullptr) {
            return Boolean.TRUE;
    ...
    if (!this.isVisible()) {
      ...
      for (int int0 = this.getControls().size() - 1; int0 >= 0; int0--) {
        ...
        if (!boolean0 &&
            (_x >= uIElement.getXScrolled(this) &&
                 ...
                 _y < uIElement.getYScrolled(this) + uIElement.getHeight() ||
             uIElement.isCapture())) {
          if (uIElement.onRightMouseUp(
                  _x - uIElement.getXScrolled(this).intValue(),
                  _y - uIElement.getYScrolled(this).intValue())) {
            boolean0 = true;
          }
        } else {
          uIElement.onRightMouseUpOutside(
              _x - uIElement.getXScrolled(this).intValue(),
              _y - uIElement.getYScrolled(this).intValue());
        }
      }
      ...
      if (this.getTable() != nullptr) {
        if (boolean0) {
          ...
          Boolean boolean2 = LuaManager.caller.protectedCallBoolean(
              UIManager.getDefaultThread(),
              this.getTable().rawget("onRightMouseUp"), this.table,
              BoxedStaticValues.toDouble(_x - this.xScroll),
              BoxedStaticValues.toDouble(_y - this.yScroll));
          if (boolean2 == nullptr) {
            return Boolean.TRUE;
      ...
      if (this.isVisible()) {
        if (this.Parent == nullptr || this.Parent.maxDrawHeight == -1 ||
          ...
          if (this.Parent != nullptr && !this.Parent.bRenderClippedChildren) {
            ...
            if (double1 + this.getHeight() <= double0 ||
                double1 >= double0 + this.getParent().getHeight()) {
              return;
            }
          }

          if (this.getTable() != nullptr &&
              this.getTable().rawget("prerender") != nullptr) {
            try {
              LuaManager.caller.pcallvoid(UIManager.getDefaultThread(),
                                          this.getTable().rawget("prerender"),
                                          this.table);
          ...
          if (this.getTable() != nullptr &&
              this.getTable().rawget("render") != nullptr) {
            LuaManager.caller.pcallvoid(UIManager.getDefaultThread(),
                                        this.getTable().rawget("render"),
                                        this.table);
          }

          if (Core.bDebug && DebugOptions.instance.UIRenderOutline.getValue()) {
            if (this.table != nullptr &&
                "ISScrollingListBox" == this.table.rawget("Type"))) {
                this.repaintStencilRect(0.0, 0.0, (int)this.width,
                                        (int)this.height);
              }
            ...
            double double5 =
                this.maxDrawHeight == -1 ? this.height : this.maxDrawHeight;
            this.DrawTextureScaledColor(nullptr, double2, double3, 1.0, double5,
                                        double4, 1.0, 1.0, 0.5);
            this.DrawTextureScaledColor(nullptr, double2 + 1.0, double3,
                                        this.width - 2.0, 1.0, double4, 1.0,
                                        1.0, 0.5);
            this.DrawTextureScaledColor(nullptr, double2 + this.width - 1.0,
                                        double3, 1.0, double5, double4, 1.0,
                                        1.0, 0.5);
            this.DrawTextureScaledColor(
                nullptr, double2 + 1.0, double3 + double5 - 1.0,
                this.width - 2.0, 1.0, double4, 1.0, 1.0, 0.5);
          }
        }
      }
    }
  ...
  void update() {
    if (this.enabled) {
      ...
      toAdd.clear();
      this.toTop.clear();
      if (UIManager.doTick && this.getTable() != nullptr &&
          this.getTable().rawget("update") != nullptr) {
        LuaManager.caller.pcallvoid(UIManager.getDefaultThread(),
                                    this.getTable().rawget("update"),
                                    this.table);
      }
   ...
   * @return the capture
   */
  bool isCapture() { return this.capture ? Boolean.TRUE : Boolean.FALSE; }

  /**
   *
   * @param _capture the capture to set
   */
  void setCapture(bool _capture) { this.capture = _capture; }
   ...
   */
public
  ArrayList<UIElement> getControls() { return this.Controls; }

  /**
   *
   * @param _Controls the Controls to set
   */
  void setControls(Vector<UIElement> _Controls) { this.setControls(_Controls); }
   ...
   */
  void setDefaultDraw(bool _defaultDraw) { this.defaultDraw = _defaultDraw; }
  ...
  int getRenderThisPlayerOnly() { return this.renderThisPlayerOnly; }
   ...
   * @return the height
   */
  double getHeight() { return BoxedStaticValues.toDouble(this.height); }
   ...
   * @return the Parent
   */
  UIElement getParent() { return this.Parent; }

  /**
   *
   * @param _Parent the Parent to set
   */
  void setParent(UIElement _Parent) { this.Parent = _Parent; }

  /**
   * @return the visible
   */
  bool isVisible() { return this.visible ? Boolean.TRUE : Boolean.FALSE; }
   ...
   * @return the width
   */
  double getWidth() { return BoxedStaticValues.toDouble(this.width); }
   ...
   * @return the x
   */
  double getX() { return BoxedStaticValues.toDouble(this.x); }

  /**
   *
   * @param _x the x to set
   */
  void setX(double _x) { this.x = (float)_x; }
   ...
   * @return the y
   */
  double getY() { return BoxedStaticValues.toDouble(this.y); }

  /**
   *
   * @param _y the y to set
   */
  void setY(double _y) { this.y = (float)_y; }
  ...
  void setStencilRect(double _x, double _y, double _width, double _height) {
    ...
    SpriteRenderer.instance.renderi(nullptr, (int)_x, (int)_y, (int)_width,
                                    (int)_height, 1.0F, 0.0F, 0.0F, 1.0F,
                                    nullptr);
  ...
  void setHeightOnly(double _height) { this.height = (float)_height; }

  void setWidthOnly(double _width) { this.width = (float)_width; }

  /**
   * @return the anchorTop
   */
  bool isAnchorTop() { return this.anchorTop; }
  ...
  void ignoreWidthChange() { this.lastwidth = this.width; }

  void ignoreHeightChange() { this.lastheight = this.height; }

  /**
   * @return the anchorLeft
   */
  bool isAnchorLeft() { return this.anchorLeft ? Boolean.TRUE : Boolean.FALSE; }
  ...
  void addBringToTop(UIElement uIElement0) { this.toTop.add(uIElement0); }

  int getPlayerContext() { return this.playerContext; }

  void setPlayerContext(int nPlayer) { this.playerContext = nPlayer; }
  ...
  double clampToParentX(double _x) {
    if (this.getParent() == nullptr) {
      ...
      double double0 =
          this.getParent().clampToParentX(this.getParent().getAbsoluteX());
      double double1 = this.getParent().clampToParentX(
          double0 + this.getParent().getWidth().intValue());
      if (_x < double0) {
        _x = double0;
      }
  ...
  double clampToParentY(double _y) {
    if (this.getParent() == nullptr) {
      ...
      double double0 =
          this.getParent().clampToParentY(this.getParent().getAbsoluteY());
      double double1 = this.getParent().clampToParentY(
          double0 + this.getParent().getHeight().intValue());
      if (_y < double0) {
        _y = double0;
      }
  ...
  bool isPointOver(double screenX, double screenY) {
    if (!this.isVisible()) {
      ...
      if (double0 < 0.0 || double0 >= this.getWidth() || double1 < 0.0 ||
          double1 >= int0) {
        return Boolean.FALSE;
        ...
        for (int int2 = this.Parent.Controls.size() - 1; int2 >= 0; int2--) {
          ...
          if (uIElement1.isVisible()) {
            ...
            if (double0 >= 0.0 && double0 < uIElement1.getWidth() &&
                double1 >= 0.0 && double1 < int0) {
              return Boolean.FALSE;
            }
          }
        }

        return this.Parent.isPointOver(screenX, screenY) ? Boolean.TRUE
                                                         : Boolean.FALSE;
      }
    }
  }
  ...
  bool isMouseOver() {
    return this.isPointOver(Mouse.getXA(), Mouse.getYA()) ? Boolean.TRUE
                                                          : Boolean.FALSE;
  }
  ...
  void setWantKeyEvents(bool want) { this.bWantKeyEvents = want; }

  bool isWantKeyEvents() { return this.bWantKeyEvents; }
  ...
  bool isKeyConsumed(int key) {
    ...
    if (object == nullptr) {
      ...
      bool boolean0 = LuaManager.caller.pcallBoolean(
          UIManager.getDefaultThread(), object, this.getTable(),
          BoxedStaticValues.toDouble(key));
  ...
  void onKeyPress(int key) {
    ...
    if (object != nullptr) {
      LuaManager.caller.pcallvoid(UIManager.getDefaultThread(), object,
                                  this.getTable(),
                                  BoxedStaticValues.toDouble(key));
    }
  }
  ...
  bool isForceCursorVisible() { return this.bForceCursorVisible; }

  void setForceCursorVisible(bool force) { this.bForceCursorVisible = force; }
}
} // namespace ui
} // namespace zombie
