...
public:
 ...
 if (chr.getCurrentSquare() == nullptr || chr.getCurrentSquare().getZone() == nullptr || !chr.getCurrentSquare().getZone().haveConstruction) {
 bool boolean0 = false;
 ...
 if (square != nullptr) {
 bool boolean1 = square.haveElectricity() || IsoWorld.instance.isHydroPowerOn();
 if (boolean1) {
 ...
 && (
 object1.getContainer().getType() == "fridge")
 ...
 if (door != nullptr && door.isExteriorDoor(nullptr) && door.IsOpen()) {
 int0 += 25;
 ...
 if (barricade0 != nullptr && (barricade0.getNumPlanks() > 4 || barricade0.isMetal())) {
 byte0 -= 20;
 }
 ...
 if (barricade1 != nullptr && (barricade1.getNumPlanks() > 3 || barricade1.isMetal())) {
 byte0 -= 5;
 }
 ...
 return false;
 } else if (character.getBed() != nullptr && "Tent" == character.getBed().getName())) {
 return false;
 } else {
 BaseVehicle vehicle = character.getVehicle();
 return vehicle = = nullptr || !vehicle.hasRoof(vehicle.getSeat(character);
 }
 }
 ...
 if (sleepingEventData.weakestWindow != nullptr && sleepingEventData.weakestWindow.getZ() == 0.0F) {
 if (!sleepingEventData.weakestWindow.north) {
 if (sleepingEventData.weakestWindow.getSquare().getRoom() == nullptr) {
 ...
 } else if (sleepingEventData.weakestWindow.getSquare().getRoom() == nullptr) {
 square = sleepingEventData.weakestWindow.getSquare();
 } else {
 square = sleepingEventData.weakestWindow
 .getSquare()
