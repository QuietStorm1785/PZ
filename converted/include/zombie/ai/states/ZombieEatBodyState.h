...
public:
  ...
  static ZombieEatBodyState instance() { return _instance; }
  ...
  void enter(IsoGameCharacter character) {
    ...
    } else if (zombie0.getEatBodyTarget() instanceof
               IsoPlayer && GameClient.bClient && zombie0.isLocal()) {
      GameClient.sendEatBody(zombie0, zombie0.getEatBodyTarget());
    }
  }
  ...
  void execute(IsoGameCharacter character) {
    ...
    } else if (!GameServer.bServer && !Core.SoundDisabled &&
  ...
  void exit(IsoGameCharacter character) {
    ...
    if (zombie0.parameterZombieState.isState(
            ParameterZombieState.State.Eating)) {
