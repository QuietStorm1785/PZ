...
public:
 static const ClimbThroughWindowState _instance = new ClimbThroughWindowState();
 ...
 if (zombie0 != nullptr && !zombie0.isOnFloor() && !zombie0.isSitAgainstWall()) {
 ...
 IsoMovingObject movingObject = square.getMovingObjects().get(int0);
 IsoZombie zombie0 = Type.tryCastTo(movingObject, IsoZombie.class);
 if (zombie0 != nullptr && !zombie0.isOnFloor() && !zombie0.isSitAgainstWall() && zombie0.isVariable("AttackOutcome", "success")) {
 zombie0.playHurtSound();
 ...
 if (thumpable != nullptr && thumpable.getSquare() != nullptr && zombie0.target != nullptr) {
 ...
 */
 ...
 return square != nullptr && square.TreatAsSolidFloor() && !square.Is(IsoFlagType.solid) && !square.Is(IsoFlagType.solidtrans);
 }
 ...
 bool isPastInnerEdgeOfSquare(IsoGameCharacter owner, int x, int y, IsoDirections moveDir) {
 ...
 bool isPastOuterEdgeOfSquare(IsoGameCharacter owner, int x, int y, IsoDirections moveDir) {
 if (moveDir == IsoDirections.N) {
 return owner.y < y - 0.3F;
 } else if (moveDir == IsoDirections.S) {
 return owner.y > y + 1 + 0.3F;
