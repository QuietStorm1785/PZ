...
public:
 ...
 } else if (weapon != nullptr && !StringUtils.isNullOrWhitespace(weapon.getSwingSound())) {
 ...
 if (window.getObjectIndex() == -1 || window.isDestroyed() && !"true" == owner.getVariableString("OwnerSmashedIt"))) {
 owner.setVariable("bSmashWindow", false);
 return;
 }
 ...
 if (vehicleWindow.isDestroyed() && !"true" == owner.getVariableString("OwnerSmashedIt"))) {
 owner.setVariable("bSmashWindow", false);
 return;
 }
 }
