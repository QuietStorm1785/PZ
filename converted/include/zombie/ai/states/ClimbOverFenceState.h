...
public:
  ...
  static ClimbOverFenceState instance() { return _instance; }
  ...
  void enter(IsoGameCharacter owner) {
    ...
    if (hashMap.get(PARAM_RUN) == Boolean.TRUE) {
      ...
      stats0.endurance = (float)(stats0.endurance -
                                 ZomboidGlobals.RunningEnduranceReduce * 300.0);
    } else if (hashMap.get(PARAM_SPRINT) == Boolean.TRUE) {
      owner.setVariable("VaultOverSprint", true);
      Stats stats1 = owner.getStats();
      stats1.endurance = (float)(stats1.endurance -
                                 ZomboidGlobals.RunningEnduranceReduce * 700.0);
    }
    ...
    if ((owner.getVariableBoolean("VaultOverRun") ||
         owner.getVariableBoolean("VaultOverSprint")) &&
        this.shouldFallAfterVaultOver(owner)) {
  ...
  void execute(IsoGameCharacter owner) {
    ...
    IsoDirections directions =
        Type.tryCastTo(hashMap.get(PARAM_DIR), IsoDirections.class);
    ...
    if (!"lunge" == string)) {
        ...
        if (directions == IsoDirections.N || directions == IsoDirections.S) {
          owner.x = owner.nx =
              PZMath.clamp(owner.x, int0 + float0, int0 + 1 - float0);
        } else if (directions == IsoDirections.W ||
                   directions == IsoDirections.E) {
          owner.y = owner.ny =
              PZMath.clamp(owner.y, int1 + float0, int1 + 1 - float0);
        }
      }

    if (owner.getVariableBoolean("ClimbFenceStarted") && !"back" == string)
            && !"fallback" == string)
            ...
            && !"obstacleEnd" == string)) {
              ...
              if ((int)owner.x != (int)float1 &&
                  (directions == IsoDirections.W ||
                   directions == IsoDirections.E)) {
                this.slideX(owner, float1);
              }

              if ((int)owner.y != (int)float2 &&
  ...
  void exit(IsoGameCharacter owner) {
    std::unordered_map hashMap = owner.getStateMachineParams(this);
    if (owner instanceof
        IsoPlayer && "fall" == owner.getVariableString("ClimbFenceOutcome"))) {
  ...
  void animEvent(IsoGameCharacter owner, AnimEvent event) {
    ...
    if (event.m_EventName.equalsIgnoreCase("OnFloor")) {
      hashMap.put(PARAM_ZOMBIE_ON_FLOOR,
      ...
      if (Boolean.parseBoolean(event.m_ParameterValue)) {
        ...
        if (this.countZombiesClimbingOver(object0) >= 2) {
          object0.Damage =
              (short)(object0.Damage -
                      Rand.Next(7, 12) / (this.isMetalFence(object0) ? 2 : 1));
          if (object0.Damage <= 0) {
            IsoDirections directions =
                Type.tryCastTo(hashMap.get(PARAM_DIR), IsoDirections.class);
    ...
    if (event.m_EventName.equalsIgnoreCase("PlayFenceSound")) {
      ...
      if (owner instanceof IsoPlayer) {
        ParameterCharacterMovementSpeed parameterCharacterMovementSpeed0 =
            ((IsoPlayer)owner).getParameterCharacterMovementSpeed();
        owner.getEmitter().setParameterValue(
            long0, parameterCharacterMovementSpeed0.getParameterDescription(),
            parameterCharacterMovementSpeed0.calculateCurrentValue());
      }

      owner.getEmitter().setParameterValue(
          long0, FMODManager.instance.getParameterDescription("FenceTypeLow"),
          int0);
    }
    ...
    if (event.m_EventName.equalsIgnoreCase("PlayTripSound")) {
      ...
      long long1 =
          owner.getEmitter().playSoundImpl(event.m_ParameterValue, null);
      ParameterCharacterMovementSpeed parameterCharacterMovementSpeed1 =
          ((IsoPlayer)owner).getParameterCharacterMovementSpeed();
      owner.getEmitter().setParameterValue(
          long1, parameterCharacterMovementSpeed1.getParameterDescription(),
          parameterCharacterMovementSpeed1.calculateCurrentValue());
      owner.getEmitter().setParameterValue(
          long1,
  ...
  void getDeltaModifiers(IsoGameCharacter owner, MoveDeltaModifiers modifiers) {
    bool boolean0 = owner.getPath2() != null;
    ...
    int int4 = int1;
        switch (dir) {
            case N:
                    ...
                    bool boolean0 = false;
                    bool boolean1 =
                        square != null && square.Is(IsoFlagType.solidtrans);
                    bool boolean2 =
                        square != null && square.TreatAsSolidFloor();
                    bool boolean3 =
                        square != null && owner.canClimbDownSheetRope(square);
                    ...
                    hashMap.put(PARAM_SCRATCH,
                                boolean0 ? Boolean.TRUE : Boolean.FALSE);
                    hashMap.put(PARAM_COUNTER,
                                boolean1 ? Boolean.TRUE : Boolean.FALSE);
                    hashMap.put(PARAM_SOLID_FLOOR,
                                boolean2 ? Boolean.TRUE : Boolean.FALSE);
                    hashMap.put(PARAM_SHEET_ROPE,
                                boolean3 ? Boolean.TRUE : Boolean.FALSE);
                    hashMap.put(PARAM_RUN, owner.isRunning() ? Boolean.TRUE
                                                             : Boolean.FALSE);
                    hashMap.put(PARAM_SPRINT, owner.isSprinting()
                                                  ? Boolean.TRUE
                                                  : Boolean.FALSE);
