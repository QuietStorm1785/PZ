...
public:
  ...
  static AttackNetworkState instance() { return s_instance; }
  ...
  void execute(IsoGameCharacter character0) {
    ...
    if (character1 == nullptr ||
        !"Chainsaw" == character1.getVariableString("ZombieHitReaction"))) {
        ...
        if (character1 != nullptr && hashMap.get(0) == Boolean.FALSE &&
            !"started" == string)
  ...
  void animEvent(IsoGameCharacter character, AnimEvent animEvent) {
    ...
    if (GameClient.bClient && zombie0.isRemoteZombie()) {
      ...
      if (animEvent.m_EventName.equalsIgnoreCase("EatBody")) {
        ...
        ((IsoZombie)character)
            .setEatBodyTarget(((IsoZombie)character).target, true);
