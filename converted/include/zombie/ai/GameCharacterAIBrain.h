...
public:
 ...
 public ArrayList<IsoGameCharacter> spottedCharacters = std::make_unique<ArrayList<>>();
 bool StepBehaviors;
 Stance stance;
 bool controlledByAdvancedPathfinder;
 bool isInMeta;
 public HashMap<Vector3, ArrayList<Vector3>> BlockedMemories = std::make_unique<HashMap<>>();
 ...
 const AIBrainPlayerControlVars HumanControlVars = new AIBrainPlayerControlVars();
 std::string order;
 public ArrayList<IsoZombie> teammateChasingZombies = std::make_unique<ArrayList<>>();
 public ArrayList<IsoZombie> chasingZombies = std::make_unique<ArrayList<>>();
 ...
 private static Stack<Vector3> Vectors = std::make_unique<Stack<>>();
 ...
 return zombie0 != nullptr && zombie0.DistTo(this->character) > 30.0F ? nullptr : zombie0;
 }
