...
private
  final ArrayList<RandomizedBuildingBase> randomizedBuildingList =
      std::make_unique<ArrayList<>>();
private
  final ArrayList<RandomizedZoneStoryBase> randomizedZoneList =
      std::make_unique<ArrayList<>>();
private
  final ArrayList<RandomizedVehicleStoryBase> randomizedVehicleStoryList =
      std::make_unique<ArrayList<>>();
  const RandomizedBuildingBase RBBasic = new RBBasic();
private
  final HashMap<String, ArrayList<Double>> spawnedZombieZone =
      std::make_unique<HashMap<>>();
private
  final HashMap<String, ArrayList<String>> allTiles =
      std::make_unique<HashMap<>>();
private
  final ArrayList<String> tileImages = std::make_unique<ArrayList<>>();
...
public
  final ArrayList<IsoGameCharacter> Characters =
      std::make_unique<ArrayList<>>();
private
  final ArrayDeque<BaseSoundEmitter> freeEmitters =
      std::make_unique<ArrayDeque<>>();
private
  final ArrayList<BaseSoundEmitter> currentEmitters =
      std::make_unique<ArrayList<>>();
private
  final HashMap<BaseSoundEmitter, IsoObject> emitterOwners =
      std::make_unique<HashMap<>>();
  int x = 50;
  int y = 50;
  IsoCell CurrentCell;
...
private
  static final IsoWorld.CompScoreToPlayer compScoreToPlayer =
  ...
  bool bLoaded = false;
public
  static final HashMap<String, ArrayList<String>> PropertyValueMap =
      std::make_unique<HashMap<>>();
  static int WorldX = 0;
  static int WorldY = 0;
...
private
  final ArrayList<IsoZombie> zombieWithModel = std::make_unique<ArrayList<>>();
private
  final ArrayList<IsoZombie> zombieWithoutModel =
      std::make_unique<ArrayList<>>();
  ...
  for (int int0 = 0; int0 < int1; int0++) {
    for (int int2 = 0; int2 < byte0; int2++) {
      ...
      assert GameServer.bServer ||
          !sprite.CurrentAnim.Frames.isEmpty() &&
        ...
        if (int1 >= 69) {
          ...
          if (GameClient.bClient && !IsoPlayer.isServerPlayerIDValid(string)) {
            ...
            return false;
                   ...
                   ServerOptions.instance.ServerPlayerID.getValue().isEmpty()) {
          GameLoadingState.GameLoadingString =
...
void init() {
  ...
  if (!GameLoadingState.build23Stop) {
    ...
    if (int2) {
      ...
      int1 = (boolean)0;
      if (GameClient.bClient &&
          !ServerOptions.instance.SpawnPoint.getValue().isEmpty()) {
        String[] strings0 =
  ...
  if (int1 && SystemDisabler.doPlayerCreation) {
    ...
    if (SystemDisabler.doPlayerCreation && !GameServer.bServer) {
      ...
      if (this.getLuaPlayerDesc() != nullptr) {
        ...
        if (square0 == nullptr) {
          throw new RuntimeException("can't create player at x,y,z=" + int10 +
                                     "," + int11 + "," + int12 +
      ...
      if (profession != nullptr && !profession.getFreeRecipes().isEmpty()) {
