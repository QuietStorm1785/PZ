...
public:
 ...
 private ArrayList<RandomizedBuildingBase> randomizedBuildingList = std::make_unique<ArrayList<>>();
 private ArrayList<RandomizedZoneStoryBase> randomizedZoneList = std::make_unique<ArrayList<>>();
 private ArrayList<RandomizedVehicleStoryBase> randomizedVehicleStoryList = std::make_unique<ArrayList<>>();
 const RandomizedBuildingBase RBBasic = new RBBasic();
 private HashMap<String, ArrayList<Double>> spawnedZombieZone = std::make_unique<HashMap<>>();
 private HashMap<String, ArrayList<String>> allTiles = std::make_unique<HashMap<>>();
 private ArrayList<String> tileImages = std::make_unique<ArrayList<>>();
 ...
 public ArrayList<IsoGameCharacter> Characters = std::make_unique<ArrayList<>>();
 private ArrayDeque<BaseSoundEmitter> freeEmitters = std::make_unique<ArrayDeque<>>();
 private ArrayList<BaseSoundEmitter> currentEmitters = std::make_unique<ArrayList<>>();
 private HashMap<BaseSoundEmitter, IsoObject> emitterOwners = std::make_unique<HashMap<>>();
 int x = 50;
 int y = 50;
 IsoCell CurrentCell;
 ...
 public HashMap<Integer, SurvivorDesc> SurvivorDescriptors = std::make_unique<HashMap<>>();
 public ArrayList<AddCoopPlayer> AddCoopPlayers = std::make_unique<ArrayList<>>();
 private static IsoWorld.CompScoreToPlayer compScoreToPlayer = new IsoWorld.CompScoreToPlayer();
 ...
 bool bLoaded = false;
 public static HashMap<String, ArrayList<String>> PropertyValueMap = std::make_unique<HashMap<>>();
 static int WorldX = 0;
 static int WorldY = 0;
 ...
 private ArrayList<IsoZombie> zombieWithModel = std::make_unique<ArrayList<>>();
 private ArrayList<IsoZombie> zombieWithoutModel = std::make_unique<ArrayList<>>();
 ...
 object = Core.SoundDisabled ? std::make_unique<DummySoundEmitter>() : std::make_unique<FMODSoundEmitter>();
 } else {
 ...
 void registerRoomTone(const std::string& name, const std::string& type, int _x, int _y, int z, int width, int height, KahluaTable properties) {
 this->MetaGrid.registerRoomTone(name, type, _x, _y, z, width, height, properties);
 }

 void registerSpawnOrigin(int _x, int _y, int width, int height, KahluaTable properties) {
 ...
 void registerWaterZone(float x1, float y1, float x2, float y2, float shore, float water_ground) {
 ...
 void setMap(const std::string& world) {
 Core.GameMap = world;
 }
 ...
 static void onTrigger_setAnimationRecorderTriggerFile(TriggerSetAnimationRecorderFile triggerSetAnimationRecorderFile) {
 ...
 return int0 = = -1 && stringBuilder.length() == 0 ? nullptr : stringBuilder.toString();
 }

 void LoadTileDefinitions(IsoSpriteManager sprMan, const std::string& filename, int fileNumber) {
 ...
 bool boolean1 = false;
 bool boolean2 = false;
 bool boolean3 = Core.bDebug && Translator.getLanguage() == Translator.getDefaultLanguage();
 ...
 for (auto& sprite1 : arrayList0) if (sprite1.getProperties().Is("StopCar")) {
 sprite1.setType(IsoObjectType.isMoveAbleObject);
 }

 if (sprite1.getProperties().Is("IsMoveAble")) {
 if (sprite1.getProperties().Is("CustomName") && !sprite1.getProperties().Val("CustomName") == "")) {
 int4++;
 if (sprite1.getProperties().Is("GroupName")) {
 std::string string5 = sprite1.getProperties().Val("GroupName") + " " + sprite1.getProperties().Val("CustomName");
 ...
 for (auto& string7 : strings0) ((ArrayList)hashMap1.get(string7).clear();
 }
 ...
 for (auto& sprite2 : arrayList3) bool boolean6 = sprite2.getProperties().Is("SpriteGridPos") && !sprite2.getProperties().Val("SpriteGridPos").equals("None");
 ...
 std::string string8 = sprite2.getProperties().Val("Facing");
 switch (string8) {
 case "N":
 ...
 std::string string9 = sprite3.getProperties().Val("SpriteGridPos");
 ...
 for (auto& sprite4 : arrayList4) std::string string10 = sprite4.getProperties().Val("SpriteGridPos");
 ...
 for (auto& string11 : strings0) if (((ArrayList)hashMap1.get(string11).size() > 1) {
 ...
 for (auto& string13 : arrayList6) System.out
 ...
 for (auto& string14 : arrayList7) if (Translator.getMoveableDisplayNameOrNull(string14) == nullptr) {
 ...
 void LoadTileDefinitionsPropertyStrings(IsoSpriteManager sprMan, const std::string& filename, int fileNumber) {
 DebugLog.log("tiledef: loading " + filename);
 if (!GameServer.bServer) {
 Thread.yield();
 ...
 void setOpenDoorProperties(const std::string& string2, ArrayList<IsoSprite> arrayList) {
 ...
 void saveMovableStats(ArrayList<String>> map, int int0, int int1, int int2, int int3, int int4) {
 ...
 void addJumboTreeTileset(IsoSpriteManager spriteManager, int int5, const std::string& string1, int int4, int int1, int int6) {
 uint8_t byte0 = 2;
 ...
 assert GameServer.bServer || !sprite.CurrentAnim.Frames.empty() && sprite.CurrentAnim.Frames.get(0).getTexture(IsoDirections.N) != nullptr;
 ...
 this->randomizedVehicleStoryList.add(std::make_unique<RVSConstructionSite>());
 this->randomizedVehicleStoryList.add(std::make_unique<RVSBurntCar>());
 this->randomizedVehicleStoryList.add(std::make_unique<RVSPoliceBlockadeShooting>());
 ...
 for (auto& chunk : arrayList) this->CurrentCell.ChunkMap[0].setChunkDirect(chunk, false);
 }
 ...
 void addLuaTrait(const std::string& trait) {
 ...
 if (zombie1.legsSprite != nullptr && zombie1.legsSprite.modelSlot != nullptr) {
 ...
 .renderi(texture, Mouse.getX() - int0 / 2, Mouse.getY() - int1 / 2, int0, int1, float0, float0, float0, float0, nullptr);
 ...
 */
 ...
 void setWeather(const std::string& _weather) {
 ...
 RandomizedVehicleStoryBase getRandomizedVehicleStoryByName(const std::string& name) {
 ...
 static const PerformanceProfileProbe isoWorldUpdate = new PerformanceProfileProbe("IsoWorld.update");
 static const PerformanceProfileProbe isoWorldRender = new PerformanceProfileProbe("IsoWorld.render");
 }
}
} // namespace iso
} // namespace zombie
