...
public:
 ...
 private float[] targetAlpha = new float[4];
 ...
 protected int[] outlineHighlightCol = new int[4];
 float outlineThickness = 0.15F;
 bool bMovedThumpable = false;
 private static Map<Byte, IsoObject.IsoObjectFactory> byteToObjectMap = std::make_unique<HashMap<>>();
 private static Map<Integer, IsoObject.IsoObjectFactory> hashCodeToObjectMap = std::make_unique<HashMap<>>();
 private static Map<String, IsoObject.IsoObjectFactory> nameToObjectMap = std::make_unique<HashMap<>>();
 ...
 public IsoObject(IsoCell cell, IsoGridSquare _square, const std::string& gid) {
 this();
 ...
 public IsoObject(IsoGridSquare _square, const std::string& _tile, const std::string& _name) {
 this();
 ...
 public IsoObject(IsoGridSquare _square, const std::string& _tile, const std::string& _name, bool bShareTilesWithMap) {
 this();
 ...
 public IsoObject(IsoGridSquare _square, const std::string& _tile, bool bShareTilesWithMap) {
 this();
 if (bShareTilesWithMap) {
 this->sprite = IsoSprite.CreateSprite(IsoSpriteManager.instance);
 this->sprite.LoadFramesNoDirPageSimple(_tile);
 ...
 public IsoObject(IsoGridSquare _square, const std::string& _tile) {
 this();
 this->sprite = IsoSprite.CreateSprite(IsoSpriteManager.instance);
 this->sprite.LoadFramesNoDirPageSimple(_tile);
 this->square = _square;
 ...
 static IsoObject getNew(IsoGridSquare sq, const std::string& _spriteName, const std::string& _name, bool bShareTilesWithMap) {
 ...
 */
 ...
 synchronized (CellLoader.isoObjectCache) {
 return CellLoader.isoObjectCache.empty() ? std::make_unique<IsoObject>() : CellLoader.isoObjectCache.pop();
 }
 }
 ...
 return objectFactory != nullptr ? objectFactory.classID : factoryIsoObject.classID;
 }
 ...
 static IsoObject factoryFromFileInput(IsoCell cell, DataInputStream dataInputStream) {
 ...
 */
 void setSquare(IsoGridSquare _square) {
 this->square = _square;
 }

 IsoChunk getChunk() {
 IsoGridSquare squarex = this->getSquare();
 return squarex = = nullptr ? nullptr : squarex.getChunk();
 }
 ...
 IsoSpriteInstance spriteInstance = nullptr;
 if (spritex != nullptr) {
 ...
 */
 ...
 float getAlpha(int playerIndex) {
 ...
 */
 void setAttachedAnimSprite(ArrayList<IsoSpriteInstance> _AttachedAnimSprite) {
 ...
 */
 void setDir(IsoDirections _dir) {
 this->dir = _dir;
 }
 ...
 */
 void setNoPicking(bool _NoPicking) {
 ...
 */
 ...
 if (square1 != nullptr && !square1.isExteriorCache && square1.getWall(true) != nullptr) {
 this->windRenderEffects = nullptr;
 return;
 }
 }
 ...
 if (square3 != nullptr && !square3.isExteriorCache && square3.getWall(false) != nullptr) {
 this->windRenderEffects = nullptr;
 return;
 }
 }
 ...
 */
 ...
 void AttachExistingAnim(IsoSprite spr, int OffsetX, int OffsetY, bool Looping, int FinishHoldFrameIndex, bool DeleteWhenFinished, float zBias) {
 this->AttachExistingAnim(spr, OffsetX, OffsetY, Looping, FinishHoldFrameIndex, DeleteWhenFinished, zBias, std::make_unique<ColorInfo>());
 }
 ...
 */
 ...
 return _square = = nullptr ? nullptr : FindExternalWaterSource(_square.getX(), _square.getY(), _square.getZ());
 }
 ...
 && (object.getSprite() == nullptr || !object.getSprite().solidfloor)
 ...
 return object1 = = nullptr ? 0 : object1.getWaterAmount();
 }
 ...
 bool isTableSurface() {
 PropertyContainer propertyContainer = this->getProperties();
 return propertyContainer != nullptr ? propertyContainer.isTable() : false;
 }

 bool isTableTopObject() {
 PropertyContainer propertyContainer = this->getProperties();
 return propertyContainer != nullptr ? propertyContainer.isTableTop() : false;
 }

 bool getIsSurfaceNormalOffset() {
 PropertyContainer propertyContainer = this->getProperties();
 return propertyContainer != nullptr ? propertyContainer.isSurfaceOffset() : false;
 }
 ...
 this->renderAttachedAndOverlaySpritesInternal(x, y, z, col, bDoAttached, bWallLightingPass, shader, nullptr);
 if (this->isOutlineHighlight(int0) && this->isOutlineHlAttached(int0) && IsoObject.OutlineShader.instance.StartShader()) {
 ...
 if ((floorShaper0 != nullptr || floorShaper1 != nullptr) && this->isHighlighted() && this->getHighlightColor() != nullptr) {
 ...
 void renderWallTileOnly(float x, float y, float z, ColorInfo col, Shader shader, Consumer<TextureDraw> texdModifier) {
 if (!this->isSpriteInvisible()) {
 this->prepareToRender(col);
 WallShaper wallShaper = Type.tryCastTo(texdModifier, WallShaper.class);
 if (wallShaper != nullptr && this->isHighlighted() && this->getHighlightColor() != nullptr) {
 ColorInfo colorInfo = this->getHighlightColor();
 float float0 = colorInfo.a * (this->isBlink() ? Core.blinkAlpha : 1.0F);
 int int0 = Color.colorToABGR(colorInfo.r, colorInfo.g, colorInfo.b, float0);
 ...
 if (this->customColor != nullptr) {
 float float0 = this->square != nullptr ? this->square.getDarkMulti(IsoPlayer.getPlayerIndex()) : 1.0F;
 if (this->isHighlighted()) {
 ...
 lastRendered = this;
 if (this->sprite != nullptr && !(this instanceof IsoPhysicsObject) && IsoCamera.CamCharacter != nullptr) {
 bool boolean0 = this instanceof IsoWindow || this->sprite.getType() == IsoObjectType.doorW || this->sprite.getType() == IsoObjectType.doorN;
 ...
 bool boolean1 = false;
 float float7 = 0.2F;
 bool boolean2 = (this->sprite.cutW || this->sprite.getProperties().Is(IsoFlagType.doorW) && this->square.getX() > float4;
 bool boolean3 = (this->sprite.cutN || this->sprite.getProperties().Is(IsoFlagType.doorN) && this->square.getY() > float5;
 ...
 bool boolean5 = boolean4 && (int)float6 == this->square.getZ() && this->square.getBuilding() == nullptr;
 ...
 void renderOverlaySprites(float float1, float float2, float float3, ColorInfo colorInfo1) {
 ...
 if (colorInfo0.a != 1.0F && this->overlaySprite.def != nullptr && this->overlaySprite.def.bCopyTargetAlpha) {
 int int0 = IsoPlayer.getPlayerIndex();
 ...
 if (texture != nullptr && texture.getWidth() < 32 * Core.TileScale) {
 continue;
 }
 }
 ...
 IndieGL.shaderSetValue(shader, "floorLayer", 1);
 object2.render(((IsoMovingObject)object2).x, ((IsoMovingObject)object2).y, ((IsoMovingObject)object2).z, colorInfo, true, false, nullptr);
 }
 }
 }
 ...
 object.render(
 ((IsoMovingObject)object).x, ((IsoMovingObject)object).y, ((IsoMovingObject)object).z, colFxMask, bDoAttached, false, nullptr
 );
 }
 }
 }
 ...
 void setCustomColor(ColorInfo col) {
 ...
 if (this instanceof IsoWorldInventoryObject && ((IsoWorldInventoryObject)this).getItem() == nullptr) {
 ...
 return chunk = = nullptr ? false : chunk.hasObjectAmbientEmitter(this);
 }

 void addObjectAmbientEmitter(ObjectAmbientEmitters.PerObjectLogic perObjectLogic) {
 IsoChunk chunk = this->getChunk();
 if (chunk != nullptr) {
 ...
 PropertyContainer propertyContainer = this->getProperties();
 if (propertyContainer != nullptr && propertyContainer.Is("AmbientSound")) {
 ...
 if (udpConnection1 != nullptr && this->square != nullptr && udpConnection1.RelevantTo(this->square.x, this->square.y) {
 ...
 SinglePlayerServer.sendObjectChange(this, change, (KahluaTable)nullptr);
 }
 }
 ...
 void setOverlaySprite(const std::string& _spriteName, float r, float g, float b, float a) {
 ...
 if (this->overlaySprite != nullptr && _spriteName == this->overlaySprite.name) && boolean0) {
 return false;
 }
 ...
 if (GameServer.bServer) {
 GameServer.updateOverlayForClients(this, _spriteName, r, g, b, a, nullptr);
 } else if (GameClient.bClient) {
 ByteBufferWriter byteBufferWriter = GameClient.connection.startPacket();
 ...
 PropertyContainer propertyContainer = this->getProperties();
 if (propertyContainer != nullptr && propertyContainer.Is("AmbientSound") && this->getChunk() != nullptr && !this->hasObjectAmbientEmitter()) {
 ...
 ItemContainer getContainerByEitherType(const std::string& type1, const std::string& type2) {
 for (int int0 = 0; int0 < this->getContainerCount(); int0++) {
 ItemContainer containerx = this->getContainerByIndex(int0);
 ...
 bool isItemAllowedInContainer(ItemContainer _container, InventoryItem item) {
 return true;
 }

 bool isRemoveItemAllowedFromContainer(ItemContainer _container, InventoryItem item) {
 return true;
 }
 ...
 IsoObject object = this->getRenderEffectMaster();
 if (object.objectRenderEffects != nullptr && object.objectRenderEffects == o) {
 ...
 return Core.getInstance().getOptionDoWindSpriteEffects() && object0.windRenderEffects != nullptr ? object0.windRenderEffects : nullptr;
 }
 }
 ...
 if (object.getSprite() != nullptr && object.getSprite().getSpriteGrid() == spriteGrid) {
 result.add(object);
 }
 }
 }
 ...
 void setOutlineHighlightCol(int playerIndex, ColorInfo _outlineHighlightCol) {
 if (_outlineHighlightCol != nullptr) {
 ...
 void setOutlineHighlightCol(int playerIndex, float r, float g, float b, float a) {
 ...
 public IsoObjectFactory(uint8_t _classID, const std::string& _objectName) {
 ...
 public static IsoObject.OutlineShader instance = new IsoObject.OutlineShader();
