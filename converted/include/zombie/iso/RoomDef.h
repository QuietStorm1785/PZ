...
class RoomDef {
public:
 private static ArrayList<IsoGridSquare> squareChoices = std::make_unique<ArrayList<>>();
 ...
 public ArrayList<MetaObject> objects = new ArrayList<>(0);
 int x = 100000;
 int y = 100000;
 int x2 = -10000;
 int y2 = -10000;
 int area;
 private HashMap<String, Integer> proceduralSpawnedContainer = std::make_unique<HashMap<>>();
 bool roofFixed = false;
 long metaID;

 public RoomDef(int _ID, const std::string& _name) {
 this->ID = _ID;
 this->name = _name;
 }
 ...
 void setBuilding(BuildingDef def) {
 this->building = def;
 }
 ...
 float getClosestPointOnEdge(float float3, float float1, float float2, float float0, float float5, float float4, float float6, Vector2f vector2f) {
