...
private
  final HashMap<String, String> itemsPowered = std::make_unique<HashMap<>>();
  float totalPowerUsing = 0.0F;
private
  static final ArrayList<IsoGenerator> AllGenerators =
      std::make_unique<ArrayList<>>();
  static const int GENERATOR_RADIUS = 20;

public
  IsoGenerator(IsoCell cell) { super(cell); }

public
  IsoGenerator(InventoryItem item, IsoCell cell, IsoGridSquare sq) {
    super(cell, sq,
          IsoSpriteManager.instance.getSprite("appliances_misc_01_0"));
    if (item != nullptr) {
...
public
  ...
  void update() {
    ...
    if (this.isActivated()) {
      if (!GameServer.bServer && (this.emitter == nullptr ||
                                  !this.emitter.isPlaying("GeneratorLoop"))) {
      ...
      WorldSoundManager.instance.addSoundRepeating(
          this, (int)this.getX(), (int)this.getY(), (int)this.getZ(), 20, 1,
          false);
      if ((int)GameTime.getInstance().getWorldAgeHours() != this.lastHour) {
        if (!this.getSquare().getProperties().Is(IsoFlagType.exterior) &&
            this.getSquare().getBuilding() != nullptr) {
          this.getSquare().getBuilding().setToxic(false);
          this.getSquare().getBuilding().setToxic(this.isActivated());
        }

        int int0 =
            (int)GameTime.getInstance().getWorldAgeHours() - this.lastHour;
        float float0 = 0.0F;
        int int1 = 0;

        for (int int2 = 0; int2 < int0; int2++) {
          float float1 = this.totalPowerUsing;
          float1 = (float)(float1 * SandboxOptions.instance
                                        .GeneratorFuelConsumption.getValue());
          float0 += float1;
          if (Rand.Next(30) == 0) {
            int1 += Rand.Next(2) + 1;
          }
  ...
  void setSurroundingElectricity() {
    ...
    if (this.square != nullptr && this.square.chunk != nullptr) {
        ...
        for (int int3 = -2; int3 <= 2; int3++) {
          IsoChunk chunk =
              GameServer.bServer
                  ...
                  : IsoWorld.instance.CurrentCell.getChunk(int0 + int3,
                                                           int1 + int2);
          if (chunk != nullptr && this.touchesChunk(chunk)) {
            if (this.isActivated()) {
              chunk.addGeneratorPos(this.square.x, this.square.y,
                                    this.square.z);
            } else {
              chunk.removeGeneratorPos(this.square.x, this.square.y,
                                       this.square.z);
            }
          }
        }
      ...
      bool boolean0 =
          SandboxOptions.getInstance().AllowExteriorGenerator.getValue();
      ...
      for (int int10 = int8; int10 < int9; int10++) {
        for (int int11 = int4; int11 <= int5; int11++) {
          for (int int12 = int6; int12 <= int7; int12++) {
            if (!(IsoUtils.DistanceToSquared(int11 + 0.5F, int12 + 0.5F,
                                             this.getSquare().getX() + 0.5F,
                                             this.getSquare().getY() + 0.5F) >
                  400.0F)) {
              IsoGridSquare square =
                  this.getCell().getGridSquare(int11, int12, int10);
              if (square != nullptr) {
                ...
                for (int int13 = 0; int13 < square.getObjects().size();
                     int13++) {
                  ...
                  if (object != nullptr &&
                    ...
                    if (object instanceof
                        IsoClothingDryer &&
                            ((IsoClothingDryer)object).isActivated()) {
                      this.addPoweredItem(object, 0.09F);
                    }

                    if (object instanceof
                        IsoClothingWasher &&
                            ((IsoClothingWasher)object).isActivated()) {
                      this.addPoweredItem(object, 0.09F);
                    }

                    if (object instanceof
                        IsoCombinationWasherDryer &&
                            ((IsoCombinationWasherDryer)object).isActivated()) {
                      this.addPoweredItem(object, 0.09F);
                    }

                    if (object instanceof
                        IsoStackedWasherDryer stackedWasherDryer) {
                      float float0 = 0.0F;
                    ...
                    if (object instanceof
                        IsoTelevision && ((IsoTelevision)object)
                                             .getDeviceData()
                                             .getIsTurnedOn()) {
                    ...
                    if (object instanceof
                        IsoStove && ((IsoStove)object).Activated()) {
                      this.addPoweredItem(object, 0.09F);
                    }

                    bool boolean2 = object.getContainerByType("fridge") != null;
                    bool boolean3 =
                        object.getContainerByType("freezer") != null;
                    ...
                    if (object instanceof
                        IsoLightSwitch && ((IsoLightSwitch)object).Activated) {
  ...
  void addPoweredItem(IsoObject object, float float0) {
    ...
    if (propertyContainer != nullptr && propertyContainer.Is("CustomName")) {
      ...
      if (propertyContainer.Is("CustomName")) {
        if (propertyContainer.Is("GroupName")) {
          string1 = propertyContainer.Val("GroupName") + " " +
    ...
    this.itemsPowered.put(string0 + " x" + int0,
                          String.format(" (%.2f L/h)", float0 * int0));
    if (int0 == 1) {
      ...
      for (int int6 = int4; int6 < int5; int6++) {
        for (int int7 = int0; int7 <= int1; int7++) {
          for (int int8 = int2; int8 <= int3; int8++) {
            if (IsoUtils.DistanceToSquared(int7, int8, this.square.x,
                                           this.square.y) <= 400.0F) {
              IsoGridSquare square =
                  this.getCell().getGridSquare(int7, int8, int6);
              if (square != nullptr) {
  ...
  std::string getObjectName() { return "IsoGenerator"; }
      ...
      for (int int1 = -2; int1 <= 2; int1++) {
        if (int1 != 0 || int0 != 0) {
          IsoChunk _chunk = GameServer.bServer
                                ? ServerMap.instance.getChunk(chunk.wx + int1,
                                                              chunk.wy + int0)
                                : IsoWorld.instance.CurrentCell.getChunk(
                                      chunk.wx + int1, chunk.wy + int0);
          if (_chunk != nullptr) {
      ...
      for (int int0 = 0; int0 < AllGenerators.size(); int0++) {
        ...
        if (generator.getSquare() != nullptr) {
          float float0 = IsoUtils.DistanceToSquared(
              sq.x + 0.5F, sq.y + 0.5F, generator.getSquare().getX() + 0.5F,
              generator.getSquare().getY() + 0.5F);
          if (float0 <= 400.0F) {
            generator.updateSurrounding = true;
          }
        }
  ...
  static bool isPoweringSquare(int generatorX, int generatorY, int generatorZ,
                               int x, int y, int z) {
    int int0 = Math.max(0, generatorZ - 3);
    int int1 = Math.min(8, generatorZ + 3);
    return z >= int0 && z < int1
               ? IsoUtils.DistanceToSquared(generatorX + 0.5F,
                                            generatorY + 0.5F, x + 0.5F,
                                            y + 0.5F) <= 400.0F
               : false;
  }

public
  ArrayList<String> getItemsPowered() {
  ...
  float getTotalPowerUsing() { return this.totalPowerUsing; }
