...
public:
 ...
 void setHasBatteryRaw(bool b) {
 this->hasBattery = b;
 }

 void addBattery(IsoGameCharacter chr, InventoryItem battery) {
 if (this->canBeModified && this->useBattery && !this->hasBattery && battery != nullptr && battery.getFullType() == "Base.Battery")) {
 ...
 void setBulbItemRaw(const std::string& item) {
 this->bulbItem = item;
 }

 void addLightBulb(IsoGameCharacter chr, InventoryItem bulb) {
 if (!this->hasLightBulb() && bulb != nullptr && bulb.getType().startsWith("LightBulb")) {
 ...
 bool boolean1 = boolean0 ? this->square.getRoom() != nullptr || this->bStreetLight : this->square.haveElectricity();
 ...
 if (square != nullptr && (boolean0 && square.getRoom() != nullptr || square.haveElectricity())) {
 boolean1 = true;
 break;
 }
 }
 ...
 void writeLightSwitchObjectHeader(ByteBufferWriter byteBufferWriter, uint8_t byte0) {
 ...
 bool boolean3 = boolean2 ? this->square.getRoom() != nullptr : this->square.haveElectricity();
 bool boolean4 = this->useBattery && this->hasBattery && this->power > 0.0F;
 ...
 if (room0 != nullptr && room0.hasLightSwitches() && !arrayList.contains(room0) {
 arrayList.add(room0);
 }
 }
 }
