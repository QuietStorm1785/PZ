...
public
  IsoTrap(HandWeapon _weapon, IsoCell cell, IsoGridSquare sq) {
    this.square = sq;
  ...
  void initSprite(HandWeapon weaponx) {
    if (weaponx != nullptr) {
      std::string string;
      if (weaponx.getPlacedSprite() != nullptr &&
          !weaponx.getPlacedSprite().isEmpty()) {
        string = weaponx.getPlacedSprite();
      } else if (weaponx.getTex() != nullptr &&
                 weaponx.getTex().getName() != nullptr) {
      ...
      if (string.startsWith("Item_") && texture != nullptr) {
        if (weaponx.getScriptItem() == nullptr) {
          ...
          float float1 =
              weaponx.getScriptItem().ScaleWorldIcon * (float0 / 2.0F);
  ...
  void updateSounds() {
    if (this.noiseStartTime > 0.0F) {
      ...
      float float2 = 60.0F;
      if (float0 - this.noiseStartTime >
          this.getNoiseDuration() / float2 * float1) {
        ...
        if (!GameServer.bServer &&
            (this.emitter == nullptr ||
             !this.emitter.isPlaying(this.getExplosionSound()))) {
        ...
        if (float0 - this.lastWorldSoundTime > 1.0F / float2 * float1 &&
            this.getObjectIndex() != -1) {
          this.lastWorldSoundTime = float0;
          WorldSoundManager.instance.addSoundRepeating(
              nullptr, this.getSquare().getX(), this.getSquare().getY(),
              this.getSquare().getZ(), this.getNoiseRange(), 1, true);
        }
      }
    }
    ...
    if (this.sprite.CurrentAnim != nullptr &&
      ...
      if (texture != nullptr) {
        if (texture.getName().startsWith("Item_")) {
          ...
          this.offsetY = 0.0F;
          this.sx = 0.0F;
          this.sprite.render(this, x + 0.5F, y + 0.5F, z, this.dir,
                             this.offsetX + float0, this.offsetY + float1, col,
                             true);
        } else {
  ...
  void triggerExplosion(bool sensor) {
    ...
    if (sensor) {
      if (this.getSensorRange() > 0) {
        ...
        this.square.drawCircleExplosion(this.getSensorRange(), this,
                                        IsoTrap.ExplosionMode.Sensor);
      }
    } else {
      ...
      if (this.getNoiseRange() > 0) {
        WorldSoundManager.instance.addSound(nullptr, (int)this.getX(),
                                            (int)this.getY(), (int)this.getZ(),
                                            this.getNoiseRange(), 1);
      ...
      if (this.getExplosionRange() > 0) {
        this.square.drawCircleExplosion(this.getExplosionRange(), this,
                                        IsoTrap.ExplosionMode.Explosion);
      }

      if (this.getFireRange() > 0) {
        this.square.drawCircleExplosion(this.getFireRange(), this,
                                        IsoTrap.ExplosionMode.Fire);
      }

      if (this.getSmokeRange() > 0) {
        this.square.drawCircleExplosion(this.getSmokeRange(), this,
                                        IsoTrap.ExplosionMode.Smoke);
      }
      ...
      if (this.getObjectIndex() != -1) {
        ...
        if (!GameServer.bServer) {
          ...
          if (!this.emitter.isPlaying(this.getExplosionSound())) {
            this.emitter.playSoundImpl(this.getExplosionSound(),
                                       (IsoObject) nullptr);
          }
        }
      }
  ...
  void save(ByteBuffer output, bool IS_DEBUG_SAVE) {
    ...
    output.putInt(this.timerBeforeExplosion > 1
                      ? Math.max(this.timerBeforeExplosion / this.FPS, 1)
                      : 0);
  ...
  int getTimerBeforeExplosion() { return this.timerBeforeExplosion; }
  ...
  std::string getObjectName() { return "IsoTrap"; }
  ...
  InventoryItem getItem() { return this.weapon; }
          ...
          if (square != nullptr) {
            for (int int8 = square.getObjects().size() - 1; int8 >= 0; int8--) {
              ...
              if (object instanceof
                  IsoTrap &&
                      ((IsoTrap)object).getRemoteControlID() == remoteID) {
  ...
  bool isInstantExplosion() { return this.instantExplosion; }

public
  static enum ExplosionMode { Explosion, Fire, Smoke, Sensor; }
}
} // namespace objects
} // namespace iso
} // namespace zombie
