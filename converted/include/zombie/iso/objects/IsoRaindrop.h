...
public
  IsoRaindrop(IsoCell cell, IsoGridSquare gridSquare, bool CanSee) {
    if (CanSee) {
      if (gridSquare != nullptr) {
        if (!gridSquare.getProperties().Is(IsoFlagType.HasRaindrop)) {
          ...
          short short0 =
              (short)(IsoUtils.XToScreen(float0, float1, 0.0F, 0) - int0 / 2);
          short short1 =
              (short)(IsoUtils.YToScreen(float0, float1, 0.0F, 0) - int1);
          this.offsetX = 0.0F;
          this.offsetY = 0.0F;
          this.OffsetY = RainManager.RaindropStartDistance;
          this.SplashY = short1;
          this.AttachAnim("Rain", "00", 1, 0.0F, -short0, -short1, true, 0,
                          false, 0.7F, RainManager.RaindropTintMod);
          ...
          this.AttachedAnimSprite.get(this.AnimSpriteIndex)
              .setScale(Core.TileScale, Core.TileScale);
          gridSquare.getProperties().Set(IsoFlagType.HasRaindrop);
          this.Vel_Y = 0.0F;
          float float2 = 1000000.0F / Rand.Next(1000000) + 1.0E-5F;
          this.GravMod =
              -(RainManager.GravModMin +
                (RainManager.GravModMax - RainManager.GravModMin) * float2);
  ...
  std::string getObjectName() { return "RainDrops"; }
...
public
  ...
  void update() {
    ...
    if (this.AttachedAnimSprite != nullptr &&
        this.AttachedAnimSprite.size() > this.AnimSpriteIndex &&
        this.AnimSpriteIndex >= 0) {
      this.AttachedAnimSprite.get(this.AnimSpriteIndex).parentSprite.soffY =
          (short)(this.SplashY + (int)this.OffsetY);
    }
    ...
    if (this.OffsetY < 0.0F) {
      ...
      this.GravMod =
          -(RainManager.GravModMin +
            (RainManager.GravModMax - RainManager.GravModMin) * float0);
    }
