...
public
  IsoCarBatteryCharger(InventoryItem _item, IsoCell cell,
    ...
    if (_item == nullptr) {
      throw new NullPointerException("item is nullptr");
    } else {
      this.item = _item;
    }
  }
    ...
    if (this.battery == nullptr) {
      ...
      bool boolean0 =
          ...
          (this.square.haveElectricity() ||
           IsoWorld.instance.isHydroPowerOn() && this.square.getRoom() != null);
      if (!boolean0) {
        this.activated = false;
      }
  ...
  void render(float x, float y, float z, ColorInfo col, bool bDoChild,
    ...
    if (this.chargerSprite.CurrentAnim != nullptr &&
        !this.chargerSprite.CurrentAnim.Frames.isEmpty()) {
      Texture texture =
      ...
      if (texture != nullptr) {
        ...
        if (this.battery != nullptr) {
          ...
          if (this.batterySprite != nullptr &&
              ...
              !this.batterySprite.CurrentAnim.Frames.isEmpty()) {
            this.sx = 0.0F;
