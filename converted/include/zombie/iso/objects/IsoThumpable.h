...
public:
  ...
  std::string lightSourceFuel = null;
  float lifeLeft = -1.0F;
  ...
  bool hasModData() {
    return this.modData != nullptr && !this.modData.isEmpty();
  }
...
public
  ...
  void save(ByteBuffer output, bool IS_DEBUG_SAVE) {
    ...
    if (this.modData != nullptr && !this.modData.isEmpty()) {
...
public
    ...
    if (this.getObjectIndex() != -1) {
      if (!GameServer.bServer) {
        ...
        if (this.lightSourceFuel != nullptr &&
            !this.lightSourceFuel.isEmpty() && this.lightSource == nullptr &&
            this.square != nullptr) {
          uint8_t byte0 = 0;
  ...
  void setLightSourceFuel(const std::string &_lightSourceFuel) {
    if (_lightSourceFuel != nullptr && _lightSourceFuel.isEmpty()) {
