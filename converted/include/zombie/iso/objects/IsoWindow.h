...
public:
 ...
 return square1 = = nullptr ? false : square0.isInARoom() != square1.isInARoom();
 }
 ...
 if (sprite != nullptr && sprite.getProperties().Is("AttachedToGlass")) {
 ...
 public IsoWindow(IsoCell cell, IsoGridSquare gridSquare, IsoSprite gid, bool _north) {
 ...
 if (thumper instanceof IsoZombie && this->getThumpableFor((IsoZombie)thumper) != nullptr) {
 ...
 if (sq.nav[IsoDirections.N.index()] != nullptr && sq.nav[IsoDirections.N.index()].Is(IsoFlagType.climbSheetTopS) {
 return true;
 }
 } else {
 ...
 if (sq.nav[IsoDirections.W.index()] != nullptr && sq.nav[IsoDirections.W.index()].Is(IsoFlagType.climbSheetTopE) {
 return true;
 }
 }
 ...
 static bool addSheetRope(IsoPlayer player, IsoGridSquare sq, bool _north, const std::string& itemType) {
 bool boolean0 = false;
 int int0 = 0;
 uint8_t byte0 = 0;
 if (_north) {
 ...
 if (square.nav[IsoDirections.N.index()] == nullptr || !square.nav[IsoDirections.N.index()].Is(IsoFlagType.climbSheetTopS) {
 return false;
 }
 ...
 if (square.nav[IsoDirections.W.index()] == nullptr || !square.nav[IsoDirections.W.index()].Is(IsoFlagType.climbSheetTopE) {
 return false;
 }
 ...
 if (!this->isDestroyed() && this->Health == 0) {
 bool boolean0 = !(chr instanceof IsoZombie) || SandboxOptions.getInstance().Lore.TriggerHouseAlarm.getValue();
 ...
 void setSmashedSprite(IsoSprite sprite) {
 this->smashedSprite = sprite;
 }

 void setPermaLocked(bool permaLock) {
 ...
 static bool canClimbThroughHelper(IsoGameCharacter chr, IsoGridSquare sq, IsoGridSquare oppositeSq, bool _north) {
 ...
 && !PolygonalMap2.instance.canStandAt(square.x + float0, square.y + float1, square.z, nullptr, 19)
 ? !square.TreatAsSolidFloor()
 : !GameClient.bClient
 || !(chr instanceof IsoPlayer)
 || SafeHouse.isSafeHouse(square, ((IsoPlayer)chr).getUsername(), true) == nullptr
 ...
 return false;
 } else if (chr != nullptr && !canClimbThroughHelper(chr, this->getSquare(), this->getOppositeSquare(), this->north) {
 return false;
 } else {
 IsoGameCharacter character = this->getFirstCharacterClosing();
 if (character != nullptr && character.isVariable("CloseWindowOutcome", "success")) {
 return false;
 } else {
 ...
 return character != nullptr ? character : this->getFirstCharacterClimbingThrough(this->getOppositeSquare());
 }
 ...
 return character != nullptr ? character : this->getFirstCharacterClosing(this->getOppositeSquare());
 }
 ...
 IsoGridSquare square = this->square.getRoom() == nullptr ? this->square : this->getOppositeSquare();
 if (this->getZ() == 0.0F && square != nullptr && square.getRoom() == nullptr) {
 bool boolean0 = square != this->square;
 IsoBarricade barricade = IsoBarricade.AddBarricadeToObject(this, boolean0);
 if (barricade != nullptr) {
