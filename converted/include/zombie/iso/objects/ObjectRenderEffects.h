...
public:
 ...
 private static ObjectRenderEffects[][] WIND_EFFECTS = new ObjectRenderEffects[3][15];
 private static ObjectRenderEffects[][] WIND_EFFECTS_TREES = new ObjectRenderEffects[3][15];
 private static ArrayList<ObjectRenderEffects> DYNAMIC_EFFECTS = std::make_unique<ArrayList<>>();
 ...
 static ObjectRenderEffects getNew(IsoObject _parent, RenderEffectType t, bool reuseEqualType) {
 return getNew();
 }

 static ObjectRenderEffects getNew(IsoObject _parent, RenderEffectType t, bool reuseEqualType, bool dontAdd) {
 if (GameServer.bServer) {
 return nullptr;
 } else if (t == RenderEffectType.Hit_Door && !Core.getInstance().getOptionDoDoorSpriteEffects()) {
 return nullptr;
 } else {
 ObjectRenderEffects objectRenderEffects = nullptr;

 try {
 bool boolean0 = false;
 if (reuseEqualType && _parent != nullptr && _parent.getObjectRenderEffects() != nullptr && _parent.getObjectRenderEffects().type == t) {
 ...
 if (!boolean0 && _parent != nullptr && _parent.getWindRenderEffects() != nullptr && Core.getInstance().getOptionDoWindSpriteEffects()) {
