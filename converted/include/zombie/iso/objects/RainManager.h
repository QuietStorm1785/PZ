...
public
  static IsoGridSquare[] PlayerLocation = new IsoGridSquare[4];
public
  static IsoGridSquare[] PlayerOldLocation = new IsoGridSquare[4];
  static bool PlayerMoved = true;
  static int RainRadius = 18;
  static Audio RainAmbient;
  static Audio ThunderAmbient = null;
...
public
  static ArrayList<IsoRainSplash> RainSplashStack = new ArrayList<>(1600);
public
  static ArrayList<IsoRaindrop> RaindropStack = new ArrayList<>(1600);
public
  static Stack<IsoRainSplash> RainSplashReuseStack =
      std::make_unique<Stack<>>();
public
  static Stack<IsoRaindrop> RaindropReuseStack = std::make_unique<Stack<>>();
  ...
  static void AddSplashes() {
    if (AddNewSplashesTimer > 0) {
      ...
      AddNewSplashesTimer = (int)(AddNewSplashesDelay *
      ...
      if (!stopRain) {
        ...
        for (int int6 = 0; int6 < IsoPlayer.numPlayers; int6++) {
          if (IsoPlayer.players[int6] != nullptr &&
              PlayerLocation[int6] != nullptr) {
            for (int int7 = 0; int7 < int5; int7++) {
              int int8 = Rand.Next(-RainRadius, RainRadius);
              int int9 = Rand.Next(-RainRadius, RainRadius);
              object =
                  cell.getGridSquare(PlayerLocation[int6].getX() + int8,
                                     PlayerLocation[int6].getY() + int9, 0);
              if (object != nullptr && ((IsoGridSquare)object).isSeen(int6) &&
                  !((IsoGridSquare)object)
                       .getProperties()
                       .Is(IsoFlagType.vegitation) &&
                  ((IsoGridSquare)object)
                      .getProperties()
                      .Is(IsoFlagType.exterior)) {
  ...
  static void SetPlayerLocation(int playerIndex,
  ...
  static bool isRaining() { return ClimateManager.getInstance().isRaining(); }
  ...
  static void StartRaindrop(IsoCell cell, IsoGridSquare gridSquare,
                            bool CanSee) {
  ...
  static void StartRainSplash(IsoCell cell, IsoGridSquare gridSquare,
                              bool CanSee) {}

  static void Update() {
    IsRaining = ClimateManager.getInstance().isRaining();
    RainIntensity =
        IsRaining ? ClimateManager.getInstance().getPrecipitationIntensity()
                  : 0.0F;
  ...
  static void setRandRainMin(int pRandRainMin) { randRainMin = pRandRainMin; }
    ...
    if (sq == nullptr) {
      ...
      for (int int0 = 0; int0 < IsoPlayer.numPlayers; int0++) {
        ...
        if (player != nullptr && PlayerLocation[int0] != nullptr) {
          if (sq.getX() < PlayerLocation[int0].getX() - RainRadius ||
              sq.getX() >= PlayerLocation[int0].getX() + RainRadius) {
            return true;
          }

          if (sq.getY() < PlayerLocation[int0].getY() - RainRadius ||
              sq.getY() >= PlayerLocation[int0].getY() + RainRadius) {
            return true;
          }
        }
      }
  ...
  static bool interruptSleep(IsoPlayer player) {
    if (player.isAsleep() && player.isOutside() && player.getBed() != nullptr &&
        !player.getBed().getName() == "Tent")) {
        IsoObject object = player.getBed();
        if (object.getCell().getGridSquare(
                (double)object.getX(), (double)object.getY(),
                (double)(object.getZ() + 1.0F)) == nullptr ||
            object.getCell()
                    .getGridSquare((double)object.getX(), (double)object.getY(),
                                   (double)(object.getZ() + 1.0F))
                    .getFloor() == nullptr) {
          return true;
        }
      }
