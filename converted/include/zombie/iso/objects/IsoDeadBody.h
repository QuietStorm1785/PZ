...
public:
 ...
 private static IsoObjectID<IsoDeadBody> Bodies = new IsoObjectID<>(IsoDeadBody.class);
 ...
 short onlineID = -1;
 short objectID = -1;
 private static ThreadLocal<IsoZombie> tempZombie = std::make_unique<ThreadLocal<IsoZombie>>() {
 IsoZombie initialValue() {
 return new IsoZombie(nullptr);
 }
 };
 ...
 for (auto& deadBody : Bodies) if (deadBody.onlineID == id && float0 - deadBody.deathTime < 0.1F) {
 return true;
 }
 }
 ...
 public IsoDeadBody(IsoGameCharacter died, bool wasCorpseAlready) {
 super(died.getCell(), false);
 IsoZombie zombie0 = Type.tryCastTo(died, IsoZombie.class);
 this->setFallOnFront(died.isFallOnFront());
 if (!GameClient.bClient && !GameServer.bServer && zombie0 != nullptr && zombie0.bCrawling) {
 ...
 void setCrawling(bool crawling) {
 ...
 if (short1 >= 0 && short1 < arrayList.size() && this->wornItems.getBodyLocationGroup().getLocation(string0) != nullptr) {
 ...
 if (short2 >= 0 && short2 < arrayList.size() && this->attachedItems.getGroup().getLocation(string1) != nullptr) {
 ...
 col = inf;
 if (!boolean0 && PerformanceSettings.LightingFrameSkip < 3 && this->getCurrentSquare() != nullptr) {
 ...
 LineDrawer.addLine(float8, float14, 0.0F, float9, float15, 0.0F, float16, float17, float18, nullptr, true);
 LineDrawer.addLine(float8, float14, 0.0F, float10, float12, 0.0F, float16, float17, float18, nullptr, true);
 LineDrawer.addLine(float9, float15, 0.0F, float11, float13, 0.0F, float16, float17, float18, nullptr, true);
 LineDrawer.addLine(float10, float12, 0.0F, float11, float13, 0.0F, float16, float17, float18, nullptr, true);
 }
 ...
 static void renderShadow(float x, float y, float z, Vector3f forward, float w, float fm, float bm, ColorInfo lightInfo, float alpha) {
 ...
 LineDrawer.addLine(float5, float11, z, float6, float12, z, 1, 1, 1, nullptr);
 LineDrawer.addLine(float6, float12, z, float8, float10, z, 1, 1, 1, nullptr);
 ...
 if (object0.getName() != nullptr && "burnedCorpse" == object0.getName())) {
 boolean0 = false;
 break;
 }
 }
 ...
 */
 ...
 for (auto& deadBody : tempBodies) if (deadBody.getHumanVisual() != nullptr) {
 ...
 bool boolean0 = this->square != nullptr && ServerLOS.instance.isCouldSee(player0, this->square);
 ...
 if (!GameClient.bClient) {
 std::vector arrayList = IsoWorld.instance.CurrentCell.getStaticUpdaterObjectList();
 ...
 if (boolean0) {
 try {
 std::vector arrayList = this->getContainer().load(b, IsoWorld.getWorldVersion());
 ...
 if (short0 >= 0 && short0 < arrayList.size() && this->getWornItems().getBodyLocationGroup().getLocation(string1) != nullptr) {
 ...
 if (short1 >= 0 && short1 < arrayList.size() && this->getAttachedItems().getGroup().getLocation(string2) != nullptr) {
 ...
 void setObjectID(short _objectID) {
 ...
 void setOnlineID(short _onlineID) {
 ...
 static IsoDeadBody getDeadBody(short id) {
