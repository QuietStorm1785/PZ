...
public:
 ...
 private IsoMannequin.PerPlayer[] perPlayer = new IsoMannequin.PerPlayer[4];
 bool bAnimate = false;
 AnimatedModel animatedModel = nullptr;
 private IsoMannequin.Drawer[] drawers = nullptr;
 float screenX;
 float screenY;
 private static IsoMannequin.StaticPerPlayer[] staticPerPlayer = new IsoMannequin.StaticPerPlayer[4];
 ...
 if (perPlayerx.renderDirection != nullptr && perPlayerx.renderDirection != IsoDirections.Max) {
 ...
 static void renderMoveableObject(IsoMannequin mannequin, int x, int y, int z, IsoDirections dir) {
 ...
 public static class MannequinZone extends IsoMetaGrid.Zone {
 int bFemale = -1;
 ...
 public MannequinZone(const std::string& string0, const std::string& string1, int int0, int int1, int int2, int int3, int int4, KahluaTable table) {
 ...
 private static class PerPlayer {
 private DeadBodyAtlas.BodyTexture atlasTex = nullptr;
 IsoDirections renderDirection = nullptr;
 bool bWasRenderDirection = false;
 }

 private static class StaticPerPlayer {
 ...
 this->_mannequin.render(int0, int1, int2, IsoMannequin.inf, false, false, nullptr);
 }
 }
 }
