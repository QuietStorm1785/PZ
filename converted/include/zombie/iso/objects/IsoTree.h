...
public:
 ...
 bool bRenderFlag;
 float fadeAlpha;
 private static IsoGameCharacter.Location[] s_chopTreeLocation = new IsoGameCharacter.Location[4];
 private static ArrayList<IsoGridSquare> s_chopTreeIndicators = std::make_unique<ArrayList<>>();
 ...
 public IsoTree(IsoGridSquare sq, const std::string& gid) {
 super(sq, gid, false);
 this->initTree();
 }
 ...
 if ("parkranger" == ((IsoGameCharacter)chr).getDescriptor().getProfession())) {
 float0 = 1.5F;
 }

 if ("lumberjack" == ((IsoGameCharacter)chr).getDescriptor().getProfession())) {
 float0 = 1.2F;
 }
 }
 ...
 void renderInner(float float4, float float5, float float6, ColorInfo colorInfo, bool var5, bool boolean0) {
 ...
 */
 ...
 SpriteRenderer.instance.render(texture, float3, float4, 64 * Core.TileScale, 128 * Core.TileScale, 0.0F, 0.5F, 0.0F, 0.75F, nullptr);
 }
 }

 public static class TreeShader {
 public static IsoTree.TreeShader instance = new IsoTree.TreeShader();
