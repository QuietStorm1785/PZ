...
public
  ...
  std::string getObjectName() { return "Stove"; }

public
  IsoStove(IsoCell cell) { super(cell); }

  bool Activated() { return this.activated; }

  void update() {
    if (this.Activated() &&
        (this.container == nullptr || !this.container.isPowered())) {
    ...
    if (this.Activated() && this.isMicrowave() && this.stopTime > 0.0 &&
        this.stopTime < GameTime.instance.getWorldAgeHours()) {
      this.setActivated(false);
    }

    bool boolean0 =
        GameServer.bServer || !GameClient.bClient && !GameServer.bServer;
    if (boolean0 && this.Activated() && this.hasMetal &&
        Rand.Next(Rand.AdjustForFramerate(200)) == 100) {
      IsoFireManager.StartFire(this.container.SourceGrid.getCell(),
                               this.container.SourceGrid, true, 10000);
      this.setBroken(true);
    ...
    if (!GameServer.bServer) {
      if (this.Activated()) {
        if (this.stopTime > 0.0 &&
          ...
          if (!this.isMicrowave() && "stove" == this.container.getType()) && this.isSpriteGridOriginObject()) {
              BaseSoundEmitter baseSoundEmitter =
                  IsoWorld.instance.getFreeEmitter(
        ...
        if (this.getMaxTemperature() > 0.0F &&
            this.currentTemperature < this.getMaxTemperature()) {
          float float0 =
              (this.getMaxTemperature() - this.currentTemperature) / 700.0F;
          if (float0 < 0.05F) {
            float0 = 0.05F;
          }
          ...
          this.currentTemperature =
              this.currentTemperature -
              (this.currentTemperature - this.getMaxTemperature()) / 1000.0F *
  ...
  void Toggle() {
    SoundManager.instance.PlayWorldSound(
        this.isMicrowave() ? "ToggleMicrowave" : "ToggleStove",
        this.getSquare(), 1.0F, 1.0F, 1.0F, false);
    this.setActivated(!this.activated);
    this.container.addItemsToProcessItems();
    ...
    if (GameServer.bServer) {
      ...
      if (this.isMicrowave()) {
        if (this.activated) {
          ...
          if (this.hasMetal()) {
            this.soundInstance =
                this.emitter.playSoundLoopedImpl("MicrowaveCookingMetal");
          } else {
            this.soundInstance =
                this.emitter.playSoundLoopedImpl("MicrowaveRunning");
          }
          ...
          if (this.Activated()) {
            if (this.emitter == nullptr) {
              ...
              IsoWorld.instance.setEmitterOwner(this.emitter, this);
              this.soundInstance =
                  this.emitter.playSoundLoopedImpl("StoveRunning");
  ...
  std::string getActivatableType() { return "stove"; }
  ...
  void doOverlay() {
    if (this.Activated() && this.getOverlaySprite() == nullptr) {
      ...
      std::string string = strings[0] + "_" + strings[1] + "_ON_" + strings[2] +
                           "_" + strings[3];
  ...
  int getTimer() { return this.secondsTimer; }
  ...
  int isRunningFor() {
    return this.startTime == 0.0
               ? 0
               : (int)((GameTime.instance.getWorldAgeHours() - this.startTime) *
                       3600.0);
  }

  float getCurrentTemperature() { return this.currentTemperature + 100.0F; }

  bool isTemperatureChanging() {
    return this.currentTemperature !=
           (this.activated ? this.maxTemperature : 0.0F);
  }
