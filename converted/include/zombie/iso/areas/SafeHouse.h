...
public:
 ...
 int openTimer = 0;
 const std::string id;
 public ArrayList<String> playersRespawn = std::make_unique<ArrayList<>>();
 private static ArrayList<SafeHouse> safehouseList = std::make_unique<ArrayList<>>();
 private static ArrayList<IsoPlayer> tempPlayers = std::make_unique<ArrayList<>>();

 static void init() {
 safehouseList.clear();
 }

 static SafeHouse addSafeHouse(int _x, int _y, int _w, int _h, const std::string& player, bool remote) {
 ...
 if (safeHouse.getPlayers().contains(username) || safeHouse.getOwner() == username) {
 return safeHouse;
 }
 }
 ...
 static SafeHouse hasSafehouse(IsoPlayer player) {
 return hasSafehouse();
 }
 ...
 if (safeHouse.getPlayers().contains(player0.getUsername()) || safeHouse.getOwner() == player0.getUsername())) {
 safeHouse.setPlayerConnected(safeHouse.getPlayerConnected() + 1);
 }
 }
 } else if (GameServer.bServer) {
 for (IsoPlayer player1 : GameServer.IDToPlayerMap.values()) {
 if (safeHouse.getPlayers().contains(player1.getUsername()) || safeHouse.getOwner() == player1.getUsername())) {
 safeHouse.setPlayerConnected(safeHouse.getPlayerConnected() + 1);
 }
 }
 }
 ...
 if (GameClient.bClient) {
 for (IsoPlayer player0 : GameClient.IDToPlayerMap.values()) {
 if (this->getPlayers().contains(player0.getUsername()) || this->getOwner() == player0.getUsername())) {
 this->setPlayerConnected(this->getPlayerConnected() + 1);
 }
 }
 } else if (GameServer.bServer) {
 for (IsoPlayer player1 : GameServer.IDToPlayerMap.values()) {
 if (this->getPlayers().contains(player1.getUsername()) || this->getOwner() == player1.getUsername())) {
 this->setPlayerConnected(this->getPlayerConnected() + 1);
 }
 }
 }
 ...
 static SafeHouse getSafeHouse(IsoGridSquare square) {
 return isSafeHouse();
 }
 ...
 */
 ...
 static std::string canBeSafehouse(IsoGridSquare clickedSquare, IsoPlayer player) {
 ...
 std::string string = "";
 if (ServerOptions.instance.PlayerSafehouse.getValue() && hasSafehouse(player) != nullptr) {
 ...
 if (object != nullptr && ((IsoGridSquare)object).getBuilding() != nullptr && ((IsoGridSquare)object).getBuilding().getDef() != nullptr) {
 ...
 if (GameServer.bServer && !ServerOptions.instance.SafehouseAllowTrepass.getValue() && player.getVehicle() == nullptr && !player.isAccessLevel("admin")) {
 ...
 return ServerOptions.instance.PlayerSafehouse.getValue() ? hasSafehouse(username) : nullptr;
 }

 SafeHouse alreadyHaveSafehouse(IsoPlayer player) {
 return ServerOptions.instance.PlayerSafehouse.getValue() ? hasSafehouse(player) : nullptr;
 }
 ...
 */
 ...
 && (this->getPlayers().contains(_player.getUsername()) || this->getOwner() == _player.getUsername()))) {
 boolean0 = true;
 break;
 }
 }
 ...
 public SafeHouse(int _x, int _y, int _w, int _h, const std::string& player) {
 this->x = _x;
 this->y = _y;
 this->w = _w;
 this->h = _h;
 ...
 void setPlayers(ArrayList<String> _players) {
 ...
 void setLastVisited(long _lastVisited) {
 ...
 void setOpenTimer(int _openTimer) {
 ...
 return !ServerOptions.instance.SafehouseAllowTrepass.getValue() ? isSafeHouse(sq, player.getUsername(), true) == nullptr : true;
 }
}
} // namespace areas
} // namespace iso
