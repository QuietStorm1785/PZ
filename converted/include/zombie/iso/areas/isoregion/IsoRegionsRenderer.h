...
public:
private
  final List<DataChunk> tempChunkList = std::make_unique<ArrayList<>>();
private
  final List<String> debugLines = std::make_unique<ArrayList<>>();
  float xPos;
  float yPos;
  float offx;
  float offy;
...
private
  final IsoRegionsRenderer.BooleanDebugOption CellGrid =
      new IsoRegionsRenderer.BooleanDebugOption(this.options, "CellGrid", true);
private
  final IsoRegionsRenderer.BooleanDebugOption MetaGridBuildings =
      new IsoRegionsRenderer.BooleanDebugOption(this.options,
                                                "MetaGrid.Buildings", true);
private
  final IsoRegionsRenderer.BooleanDebugOption IsoRegionRender =
      new IsoRegionsRenderer.BooleanDebugOption(this.options,
                                                "IsoRegion.Render", true);
private
  final IsoRegionsRenderer.BooleanDebugOption IsoRegionRenderChunks =
      new IsoRegionsRenderer.BooleanDebugOption(
          this.options, "IsoRegion.RenderChunks", false);
private
  final IsoRegionsRenderer.BooleanDebugOption IsoRegionRenderChunksPlus =
      new IsoRegionsRenderer.BooleanDebugOption(
                    ...
                    if (int22 > 0 && int22 == int12) {
                      ...
                      IsoWorldRegion worldRegion2 =
                          chunkRegion1 != null
                              ? chunkRegion1.getIsoWorldRegion()
                              : null;
                      bool boolean0 = chunkRegion1 == null ||
                                      worldRegion2 == null ||
