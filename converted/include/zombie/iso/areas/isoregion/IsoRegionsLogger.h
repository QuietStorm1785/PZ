...
public:
 private ConcurrentLinkedQueue<IsoRegionsLogger.IsoRegionLog> pool = std::make_unique<ConcurrentLinkedQueue<>>();
 private ConcurrentLinkedQueue<IsoRegionsLogger.IsoRegionLog> loggerQueue = std::make_unique<ConcurrentLinkedQueue<>>();
 const bool consolePrint;
 private ArrayList<IsoRegionsLogger.IsoRegionLog> logs = std::make_unique<ArrayList<>>();
 const int maxLogs = 100;
 bool isDirtyUI = false;

 public IsoRegionsLogger(bool doConsolePrint) {
 ...
 void update() {
 if (Core.bDebug) {
 for (IsoRegionsLogger.IsoRegionLog regionLog0 = this->loggerQueue.poll(); regionLog0 != nullptr; regionLog0 = this->loggerQueue.poll()) {
