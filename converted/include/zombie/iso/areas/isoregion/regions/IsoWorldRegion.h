...
public:
 ...
 Color color;
 bool enclosed = true;
 private ArrayList<IsoChunkRegion> isoChunkRegions = std::make_unique<ArrayList<>>();
 int squareSize = 0;
 int roofCnt = 0;
 bool isDirtyEnclosed = false;
 bool isDirtyRoofed = false;
 private ArrayList<IsoWorldRegion> neighbors = std::make_unique<ArrayList<>>();
 ...
 if (chunkRegion1.getIsoWorldRegion() != nullptr && chunkRegion1.getIsoWorldRegion() != this) {
