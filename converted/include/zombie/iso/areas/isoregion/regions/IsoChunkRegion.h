...
private
  final boolean[] enclosed = new boolean[4];
  bool enclosedCache = true;
private
  final ArrayList<IsoChunkRegion> connectedNeighbors =
      std::make_unique<ArrayList<>>();
private
  final ArrayList<IsoChunkRegion> allNeighbors =
      std::make_unique<ArrayList<>>();
...
protected
  ...
  IsoChunkRegion reset() {
    ...
    if (worldRegion != nullptr && worldRegion.size() <= 0) {
      if (Core.bDebug) {
        throw new RuntimeException("ChunkRegion.reset IsoChunkRegion has "
