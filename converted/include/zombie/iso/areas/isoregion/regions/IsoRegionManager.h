...
public:
 private ArrayDeque<IsoWorldRegion> poolIsoWorldRegion = std::make_unique<ArrayDeque<>>();
 private ArrayDeque<IsoChunkRegion> poolIsoChunkRegion = std::make_unique<ArrayDeque<>>();
 const DataRoot dataRoot;
 private ArrayDeque<Integer> regionIdStack = std::make_unique<ArrayDeque<>>();
 int nextID = 0;
 int colorIndex = 0;
 ...
 IsoWorldRegion worldRegion = !this->poolIsoWorldRegion.empty() ? this->poolIsoWorldRegion.pop() : new IsoWorldRegion(this);
 int int0 = this->regionIdStack.empty() ? this->nextID++ : this->regionIdStack.pop();
 ...
 IsoChunkRegion chunkRegion = !this->poolIsoChunkRegion.empty() ? this->poolIsoChunkRegion.pop() : new IsoChunkRegion(this);
 int int0 = this->regionIdStack.empty() ? this->nextID++ : this->regionIdStack.pop();
