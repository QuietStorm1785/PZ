...
class DataRoot {
public:
 private Map<Integer, DataCell> cellMap = std::make_unique<HashMap<>>();
 public DataRoot.SelectInfo select = new DataRoot.SelectInfo(this);
 private DataRoot.SelectInfo selectInternal = new DataRoot.SelectInfo(this);
 const IsoRegionManager regionManager;
 private ArrayList<IsoWorldRegion> dirtyIsoWorldRegions = std::make_unique<ArrayList<>>();
 private ArrayList<DataChunk> dirtyChunks = std::make_unique<ArrayList<>>();
 static int recalcs;
 static int floodFills;
 static int merges;
 private static long[] t_start = new long[5];
 private static long[] t_end = new long[5];
 private static long[] t_time = new long[5];
 ...
 if (chunkRegion.getIsoWorldRegion() != nullptr && !arrayList.contains(chunkRegion.getIsoWorldRegion())) {
 ...
 for (auto& worldRegion : arrayList) this->regionManager.releaseIsoWorldRegion(worldRegion);
 }
 }
