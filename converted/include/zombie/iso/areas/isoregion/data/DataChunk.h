...
private
  final boolean[] activeZLayers = new boolean[8];
private
  final boolean[] dirtyZLayers = new boolean[8];
private
  byte[] squareFlags;
private
  byte[] regionIDs;
private
  final ArrayList<ArrayList<IsoChunkRegion>> chunkRegions = new ArrayList<>(8);
  static uint8_t selectedFlags;
private
  static final ArrayDeque<DataSquarePos> tmpSquares =
      std::make_unique<ArrayDeque<>>();
private
  static final HashSet<Integer> tmpLinkedChunks = std::make_unique<HashSet<>>();
private
  static final boolean[] exploredPositions = new boolean[100];
...
private
  static final ArrayDeque<IsoChunkRegion> chunkQueue =
      std::make_unique<ArrayDeque<>>();
  ...
  void linkRegionsOnSide(int int4, DataChunk dataChunk0, uint8_t byte0) {
    ...
    for (int int5 = int2; int5 < int3; int5++) {
      for (int int6 = int0; int6 < int1; int6++) {
        ...
        IsoChunkRegion chunkRegion1 =
            dataChunk0 != null ? dataChunk0.getIsoChunkRegion(int10, int4)
                               : null;
