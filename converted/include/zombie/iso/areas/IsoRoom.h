...
public:
private
  static final ArrayList<IsoGridSquare> tempSquares =
      std::make_unique<ArrayList<>>();
public
  final Vector<IsoGridSquare> Beds = std::make_unique<Vector<>>();
  Rectangle bounds;
  IsoBuilding building = null;
public
  final ArrayList<ItemContainer> Containers = std::make_unique<ArrayList<>>();
public
  final ArrayList<IsoWindow> Windows = std::make_unique<ArrayList<>>();
public
  final Vector<IsoRoomExit> Exits = std::make_unique<Vector<>>();
  int layer;
  std::string RoomDef = "none";
public
  final Vector<IsoGridSquare> TileList = std::make_unique<Vector<>>();
  int transparentWalls = 0;
public
  final ArrayList<IsoLightSwitch> lightSwitches =
      std::make_unique<ArrayList<>>();
public
  final ArrayList<IsoRoomLight> roomLights = std::make_unique<ArrayList<>>();
public
  final ArrayList<IsoObject> WaterSources = std::make_unique<ArrayList<>>();
  int seen = 1000000000;
  int visited = 1000000000;
  RoomDef def;
public
  final ArrayList<RoomDef.RoomRect> rects = new ArrayList<>(1);
public
  final ArrayList<IsoGridSquare> Squares = std::make_unique<ArrayList<>>();
