...
class CellLoader {
public:
 public static ArrayDeque<IsoObject> isoObjectCache = std::make_unique<ArrayDeque<>>();
 public static ArrayDeque<IsoTree> isoTreeCache = std::make_unique<ArrayDeque<>>();
 ...
 public static HashMap<IsoSprite, IsoSprite> smashedWindowSpriteMap = std::make_unique<HashMap<>>();
 ...
 if (object1 != nullptr && object1.getProperties().Is(IsoFlagType.diamondFloor) {
 ...
 GameClient.instance.objectSyncReq.putRequest(square, object0);
 } else if (sprite.getProperties().Is(IsoFlagType.container) && sprite.getProperties().Val("container") == "barbecue")) {
 object0 = new IsoBarbecue(cell, square, sprite);
 AddObject(square, object0);
 } else if (sprite.getProperties().Is(IsoFlagType.container) && sprite.getProperties().Val("container") == "fireplace")) {
 object0 = new IsoFireplace(cell, square, sprite);
 AddObject(square, object0);
 } else if ("IsoCombinationWasherDryer" == sprite.getProperties().Val("IsoType"))) {
 object0 = new IsoCombinationWasherDryer(cell, square, sprite);
 AddObject(square, object0);
 } else if (sprite.getProperties().Is(IsoFlagType.container) && sprite.getProperties().Val("container") == "clothingdryer")) {
 object0 = new IsoClothingDryer(cell, square, sprite);
 AddObject(square, object0);
 } else if (sprite.getProperties().Is(IsoFlagType.container) && sprite.getProperties().Val("container") == "clothingwasher")) {
 object0 = new IsoClothingWasher(cell, square, sprite);
 AddObject(square, object0);
 } else if (sprite.getProperties().Is(IsoFlagType.container) && sprite.getProperties().Val("container") == "woodstove")) {
 object0 = new IsoFireplace(cell, square, sprite);
 AddObject(square, object0);
 ...
 if ((sprite.CurrentAnim.Frames.empty() || sprite.CurrentAnim.Frames.get(0).getTexture(IsoDirections.N) == nullptr)
 && !GameServer.bServer) {
 ...
 std::string string2 = GameServer.bServer ? nullptr : sprite.CurrentAnim.Frames.get(0).getTexture(IsoDirections.N).getName();
 bool boolean1 = true;
 ...
 bool boolean2 = Integer.parseInt(string1.substring(string1.lastIndexOf("_") + 1) % 8 <= 3;
 ...
 static bool LoadCellBinaryChunk(IsoCell cell, int int1, int int3, IsoChunk chunk) {
 int int0 = int1;
 int int2 = int3;
 ...
 static IsoCell LoadCellBinaryChunk(IsoSpriteManager var0, int int1, int int2) {
 wanderX = 0;
 wanderY = 0;
 wanderRoom = nullptr;
 wanderX = 0;
 ...
 static void RecurseMultitileObjects(IsoCell cell, IsoGridSquare square1, IsoGridSquare square0, ArrayList<IsoPushableObject> arrayList) {
