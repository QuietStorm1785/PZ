...
public:
 ...
 private static ArrayList<ContainerOverlays.ContainerOverlayEntry> tempEntries = std::make_unique<ArrayList<>>();
 private THashMap<String, ContainerOverlays.ContainerOverlay> overlayMap = std::make_unique<THashMap<>>();
 ...
 if (int0 != 1) {
 throw RuntimeException("unknown overlayMap.VERSION " + int0);
 }
 ...
 return obj != nullptr && obj.sprite != nullptr && obj.sprite.name != nullptr && this->overlayMap.containsKey(obj.sprite.name);
 }
 ...
 if (obj.sprite != nullptr && obj.sprite.name != nullptr && container != nullptr && container.getItems() != nullptr && !container.empty()) {
 ...
 if (containerOverlayEntry.fewItems != nullptr && container.getItems().size() < 7) {
 ...
 private static class ContainerOverlay {
 std::string name;
 public ArrayList<ContainerOverlays.ContainerOverlayEntry> entries = std::make_unique<ArrayList<>>();

 void getEntries(const std::string& string, ArrayList<ContainerOverlays.ContainerOverlayEntry> arrayList) {
 arrayList.clear();
 ...
 private static class ContainerOverlayEntry {
 std::string room;
