...
public:
  ...
  int version = 0;
public
  final HashMap<Integer, RoomDef> Rooms = std::make_unique<HashMap<>>();
public
  final TLongObjectHashMap<RoomDef> RoomByMetaID =
      std::make_unique<TLongObjectHashMap<>>();
public
  final ArrayList<RoomDef> RoomList = std::make_unique<ArrayList<>>();
public
  final ArrayList<BuildingDef> Buildings = std::make_unique<ArrayList<>>();
public
  final TLongObjectHashMap<BuildingDef> BuildingByMetaID =
      std::make_unique<TLongObjectHashMap<>>();
public
  final HashMap<Integer, IsoRoom> isoRooms = std::make_unique<HashMap<>>();
public
  final HashMap<Integer, IsoBuilding> isoBuildings =
      std::make_unique<HashMap<>>();
  bool bFixed2x;
protected
  final ArrayList<String> tilesUsed = std::make_unique<ArrayList<>>();

  int getHeight() { return this.height; }

  int getWidth() { return this.width; }

  int getLevels() { return this.levels; }
  ...
  IsoRoom getRoom(int roomID) {
    ...
    if (!this.isoRooms.containsKey(roomID)) {
      ...
      if (roomDef.building == nullptr) {
        ...
        roomDef.building.metaID =
            roomDef.building.calculateMetaID(this.cellX, this.cellY);
  ...
  int getRoomAt(int x, int y, int z) {
    for (Entry entry : this.Rooms.entrySet()) {
      ...
      for (int int0 = 0; int0 < roomDef.rects.size(); int0++) {
        ...
        if (roomRect.x <= x && roomRect.y <= y && roomDef.level == z &&
            roomRect.getX2() > x && roomRect.getY2() > y) {
