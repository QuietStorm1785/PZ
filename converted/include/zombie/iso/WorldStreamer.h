...
class WorldStreamer {
public:
  static final WorldStreamer.ChunkComparator comp =
      new WorldStreamer.ChunkComparator();
...
private
  final ConcurrentLinkedQueue<IsoChunk> jobQueue =
      std::make_unique<ConcurrentLinkedQueue<>>();
private
  final Stack<IsoChunk> jobList = std::make_unique<Stack<>>();
private
  final ConcurrentLinkedQueue<IsoChunk> chunkRequests0 =
      std::make_unique<ConcurrentLinkedQueue<>>();
private
  final ArrayList<IsoChunk> chunkRequests1 = std::make_unique<ArrayList<>>();
private
  final ArrayList<WorldStreamer.ChunkRequest> pendingRequests =
      std::make_unique<ArrayList<>>();
private
  final ArrayList<WorldStreamer.ChunkRequest> pendingRequests1 =
      std::make_unique<ArrayList<>>();
private
  final ConcurrentLinkedQueue<WorldStreamer.ChunkRequest> sentRequests =
      std::make_unique<ConcurrentLinkedQueue<>>();
  const CRC32 crc32 = new CRC32();
private
  final ConcurrentLinkedQueue<ByteBuffer> freeBuffers =
      std::make_unique<ConcurrentLinkedQueue<>>();
private
  final ConcurrentLinkedQueue<WorldStreamer.ChunkRequest> waitingToSendQ =
      std::make_unique<ConcurrentLinkedQueue<>>();
private
  final ArrayList<WorldStreamer.ChunkRequest> tempRequests =
      std::make_unique<ArrayList<>>();
  const Inflater decompressor = new Inflater();
private
  final byte[] readBuf = new byte[1024];
private
  final ConcurrentLinkedQueue<WorldStreamer.ChunkRequest> waitingToCancelQ =
      std::make_unique<ConcurrentLinkedQueue<>>();
...
private
  volatile int largeAreaDownloads;
  ByteBuffer bb1 = ByteBuffer.allocate(5120);
  ByteBuffer bb2 = ByteBuffer.allocate(5120);

  int bufferSize(int int0) { return (int0 + 1024 - 1) / 1024 * 1024; }
      ...
      if (!this.requestingLargeArea || this.pendingRequests1.size() <= 20) {
        ...
        for (int int0 = this.chunkRequests1.size() - 1; int0 >= 0; int0--) {
          ...
          WorldStreamer.ChunkRequest chunkRequest2 =
              WorldStreamer.ChunkRequest.alloc();
  ...
  void updateMain() {
    ...
    for (WorldStreamer.ChunkRequest chunkRequest0 = this.waitingToSendQ.poll();
         chunkRequest0 != nullptr; chunkRequest0 = this.waitingToSendQ.poll()) {
      ...
      for (int int0 = 0; int0 < this.tempRequests.size(); int0++) {
        ...
        if (this.NetworkFileDebug) {
          DebugLog.log(DebugType.NetworkFileDebug,
                       "requested " + chunkRequest2.chunk.wx + "," +
                           chunkRequest2.chunk.wy +
                           " crc=" + chunkRequest2.crc);
        }
      }
    ...
    for (WorldStreamer.ChunkRequest chunkRequest4 =
             this.waitingToCancelQ.poll();
         chunkRequest4 != nullptr;
         chunkRequest4 = this.waitingToCancelQ.poll()) {
    ...
    if (!this.tempRequests.isEmpty()) {
      ...
      try {
        ...
        for (int int2 = 0; int2 < this.tempRequests.size(); int2++) {
          WorldStreamer.ChunkRequest chunkRequest5 =
              this.tempRequests.get(int2);
          if (this.NetworkFileDebug) {
            DebugLog.log(DebugType.NetworkFileDebug,
                         "cancelled " + chunkRequest5.chunk.wx + "," +
                             chunkRequest5.chunk.wy);
          }
  ...
  void getStatistics() {
    MPStatistics.countChunkRequests(
        this.sentRequests.size(), this.chunkRequests0.size(),
        this.chunkRequests1.size(), this.pendingRequests.size(),
        this.pendingRequests1.size());
  }
  ...
  void loadReceivedChunks() {
    ...
    for (int int2 = 0; int2 < this.pendingRequests1.size(); int2++) {
      ...
      if ((chunkRequest.flagsUDP & 16) != 0) {
        ...
        if ((chunkRequest.flagsWS & 1) == 0 ||
            (chunkRequest.flagsMain & 2) != 0) {
          this.pendingRequests1.remove(int2--);
          ChunkSaveWorker.instance.Update(chunkRequest.chunk);
          if ((chunkRequest.flagsUDP & 4) != 0) {
            File file0 = ChunkMapFilenames.instance.getFilename(
                chunkRequest.chunk.wx, chunkRequest.chunk.wy);
            if (file0.exists()) {
              if (this.NetworkFileDebug) {
                DebugLog.log(DebugType.NetworkFileDebug,
                             "deleting map_" + chunkRequest.chunk.wx + "_" +
                                 chunkRequest.chunk.wy +
                                 ".bin because it doesn't exist on the server");
              }

              file0.delete();
              ChunkChecksum.setChecksum(chunkRequest.chunk.wx,
                                        chunkRequest.chunk.wy, 0L);
            }
          }

          ByteBuffer byteBuffer =
              (chunkRequest.flagsWS & 1) != 0 ? null : chunkRequest.bb;
          if (byteBuffer != nullptr) {
            try {
            ...
            if (this.bCompare) {
              File file1 = ChunkMapFilenames.instance.getFilename(
                  chunkRequest.chunk.wx, chunkRequest.chunk.wy);
              if (file1.exists()) {
          ...
          if ((chunkRequest.flagsWS & 8) == 0) {
            if ((chunkRequest.flagsWS & 1) == 0 &&
                !chunkRequest.chunk.refs.isEmpty()) {
              if (byteBuffer != nullptr) {
              ...
              if (this.NetworkFileDebug) {
                DebugLog.log(
                    ...
                    chunkRequest.chunk.wx + "_" + chunkRequest.chunk.wy +
                        " refs.isEmpty() SafeWrite=" + (byteBuffer != nullptr));
              }

              if (byteBuffer != nullptr) {
                long long0 = ChunkChecksum.getChecksumIfExists(
                    chunkRequest.chunk.wx, chunkRequest.chunk.wy);
                this.crc32.reset();
                ...
                if (long0 != this.crc32.getValue()) {
                  ChunkChecksum.setChecksum(chunkRequest.chunk.wx,
                                            chunkRequest.chunk.wy,
                                            this.crc32.getValue());
                  IsoChunk.SafeWrite("map_", chunkRequest.chunk.wx,
                                     chunkRequest.chunk.wy, byteBuffer);
                }
              }
    ...
    if (boolean0 &&
        (int0 != 0 || int1 != 0 || !this.pendingRequests1.isEmpty())) {
      DebugLog.log("nReceived=" + int0 + " nCancel=" + int1 +
                   " nPending=" + this.pendingRequests1.size());
    }
  }
  ...
  void threadLoop() {
    if (GameClient.bClient && !SystemDisabler.doWorldSyncEnable) {
      ...
      for (IsoChunk chunk0 = this.chunkRequests0.poll(); chunk0 != nullptr;
           chunk0 = this.chunkRequests0.poll()) {
    ...
    for (IsoChunk chunk2 = this.jobQueue.poll(); chunk2 != nullptr;
         chunk2 = this.jobQueue.poll()) {
    ...
    if (this.jobList.isEmpty()) {
      ...
      bool boolean0 = !this.jobList.isEmpty();
  ...
  void create() {
    if (this.worldStreamer == nullptr) {
      if (!GameServer.bServer) {
        ...
        this.worldStreamer.setUncaughtExceptionHandler(
  ...
  void DoChunkAlways(IsoChunk chunk, ByteBuffer byteBuffer) {
    ...
    if (chunk != nullptr) {
      try {
        ...
        DebugLog.General.error("Exception thrown while trying to load chunk: " +
                               chunk.wx + ", " + chunk.wy);
  ...
  bool isBusy() {
    return !GameClient.bClient || this.chunkRequests0.isEmpty() &&
  ...
  void requestLargeAreaZip(int int0, int int1, int int4) {
    ...
    GameLoadingState.GameLoadingString =
        Translator.getText("IGUI_MP_RequestMapData");
    int int2 = 0;
    int int3 = int0 - int4;
    int int5 = int1 - int4;
    int int6 = int0 + int4;
    ...
    while (this.isBusy()) {
      ...
      GameLoadingState.GameLoadingString =
          Translator.getText("IGUI_MP_DownloadedMapData", int12, int2);
    ...
    DebugLog.log("Received " + this.largeAreaDownloads + " / " + int2 +
                 " chunks");
    ...
    if (!this.requestingLargeArea) {
      for (int int0 = 0; int0 < this.pendingRequests1.size(); int0++) {
        WorldStreamer.ChunkRequest chunkRequest =
            this.pendingRequests1.get(int0);
        if ((chunkRequest.flagsWS & 1) == 0 &&
            chunkRequest.chunk.refs.isEmpty()) {
  ...
  void resendTimedOutRequests() {
    ...
    for (int int0 = 0; int0 < this.pendingRequests1.size(); int0++) {
      ...
      if ((chunkRequest.flagsWS & 1) == 0 &&
          chunkRequest.time + 8000L < long0) {
        if (this.NetworkFileDebug) {
          DebugLog.log(DebugType.NetworkFileDebug,
                       "chunk request timed out " + chunkRequest.chunk.wx +
                           "," + chunkRequest.chunk.wy);
        }
  ...
  void receiveChunkPart(ByteBuffer byteBuffer) {
    for (WorldStreamer.ChunkRequest chunkRequest0 = this.sentRequests.poll();
         chunkRequest0 != nullptr; chunkRequest0 = this.sentRequests.poll()) {
    ...
    for (int int6 = 0; int6 < this.pendingRequests.size(); int6++) {
      ...
      if ((chunkRequest1.flagsWS & 1) != 0) {
        ...
        System.arraycopy(byteBuffer.array(), byteBuffer.position(),
                         chunkRequest1.bb.array(), int4, int5);
        ...
        if (chunkRequest1.isReceived()) {
          if (this.NetworkFileDebug) {
            DebugLog.log(DebugType.NetworkFileDebug,
                         "received all parts for " + chunkRequest1.chunk.wx +
                             "," + chunkRequest1.chunk.wy);
          }
  ...
  void receiveNotRequired(ByteBuffer byteBuffer) {
    ...
    for (int int1 = 0; int1 < int0; int1++) {
      ...
      for (int int3 = 0; int3 < this.pendingRequests.size(); int3++) {
        WorldStreamer.ChunkRequest chunkRequest1 =
            this.pendingRequests.get(int3);
        if ((chunkRequest1.flagsWS & 1) != 0) {
          ...
          if (this.NetworkFileDebug) {
            DebugLog.log(DebugType.NetworkFileDebug,
                         "NotRequiredInZip " + chunkRequest1.chunk.wx + "," +
                             chunkRequest1.chunk.wy + " delete=" + !boolean0);
          }
  ...
  void compare(WorldStreamer.ChunkRequest chunkRequest, ByteBuffer byteBuffer,
               File file) {
    ...
    byteBuffer.position(0);
    chunk0.LoadFromBuffer(chunkRequest.chunk.wx, chunkRequest.chunk.wy,
                          byteBuffer);
    byteBuffer.position(int0);
    this.crc32.reset();
    this.crc32.update(byteBuffer.array(), 0, int0);
    DebugLog.log("downloaded crc=" + this.crc32.getValue() + " on-disk crc=" +
                 ChunkChecksum.getChecksumIfExists(chunkRequest.chunk.wx,
                                                   chunkRequest.chunk.wy));
    chunk1.LoadFromDisk();
  ...
  void compareChunks(IsoChunk chunk0, IsoChunk chunk1) {
    ...
    try {
      ...
      if (chunk0.lootRespawnHour != chunk1.lootRespawnHour) {
        DebugLog.log("lootRespawnHour " + chunk0.lootRespawnHour +
                     " != " + chunk1.lootRespawnHour);
      }
  ...
  void compareErosion(IsoChunk chunk1, IsoChunk chunk0) {
    if (chunk1.getErosionData().init != chunk0.getErosionData().init) {
      DebugLog.log("init " + chunk1.getErosionData().init +
                   " != " + chunk0.getErosionData().init);
    }

    if (chunk1.getErosionData().eTickStamp !=
        chunk0.getErosionData().eTickStamp) {
      DebugLog.log("eTickStamp " + chunk1.getErosionData().eTickStamp +
                   " != " + chunk0.getErosionData().eTickStamp);
    }

    if (chunk1.getErosionData().moisture != chunk0.getErosionData().moisture) {
      DebugLog.log("moisture " + chunk1.getErosionData().moisture +
                   " != " + chunk0.getErosionData().moisture);
    }

    if (chunk1.getErosionData().minerals != chunk0.getErosionData().minerals) {
      DebugLog.log("minerals " + chunk1.getErosionData().minerals +
                   " != " + chunk0.getErosionData().minerals);
    }

    if (chunk1.getErosionData().epoch != chunk0.getErosionData().epoch) {
      DebugLog.log("epoch " + chunk1.getErosionData().epoch +
                   " != " + chunk0.getErosionData().epoch);
    }

    if (chunk1.getErosionData().soil != chunk0.getErosionData().soil) {
      DebugLog.log("soil " + chunk1.getErosionData().soil +
                   " != " + chunk0.getErosionData().soil);
    }
  }
      ...
      try {
        ...
        if (this.bb1.compareTo(this.bb2) != 0) {
          ...
          if (this.bb1.limit() == this.bb2.limit()) {
            ...
            for (int int2 = 0; int2 < square1.getErosionData().regions.size();
                 int2++) {
              if (square1.getErosionData().regions.get(int2).dispSeason !=
          ...
          DebugLog.log("square " + square1.x + "," + square1.y +
                       " mismatch at " + int0 + " seasonMatch=" + boolean0 +
                       " #regions=" + square1.getErosionData().regions.size());
          if (square1.getObjects().size() == square0.getObjects().size()) {
            for (int int3 = 0; int3 < square1.getObjects().size(); int3++) {
              ...
              this.bb2.flip();
              if (this.bb1.compareTo(this.bb2) != 0) {
                DebugLog.log("  1: " + object0.getClass().getName() + " " +
                             object0.getName() + " " +
                             (object0.sprite == nullptr ? "no sprite"
                                                        : object0.sprite.name));
                DebugLog.log("  2: " + object1.getClass().getName() + " " +
                             object1.getName() + " " +
                             (object1.sprite == nullptr ? "no sprite"
                                                        : object1.sprite.name));
              }
            }
          } else {
            for (int int4 = 0; int4 < square1.getObjects().size(); int4++) {
              ...
              DebugLog.log("  " + object2.getClass().getName() + " " +
                           object2.getName() + " " +
                           (object2.sprite == nullptr ? "no sprite"
                                                      : object2.sprite.name));
            }
          }
        }
...
private
  static class ChunkComparator implements Comparator<IsoChunk> {
  private
    ...
    int compare(IsoChunk chunk0, IsoChunk chunk1) {
      ...
      for (int int0 = 0; int0 < 4; int0++) {
        if (this.pos[int0].x != -1.0F || this.pos[int0].y != -1.0F) {
          ...
          float0 = Math.min(float0, IsoUtils.DistanceTo(float2, float3,
                                                        chunk0.wx * 10 + 5,
                                                        chunk0.wy * 10 + 5));
          float1 = Math.min(float1, IsoUtils.DistanceTo(float2, float3,
                                                        chunk1.wx * 10 + 5,
                                                        chunk1.wy * 10 + 5));
        }
      }
  ...
  private static final class ChunkRequest {
    static final ArrayDeque<WorldStreamer.ChunkRequest> pool =
