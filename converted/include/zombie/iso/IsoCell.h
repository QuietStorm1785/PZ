...
/**
 * Loaded area 'reality bubble' around the player(s). Don't confuse this with
 * map cells - the name is a relic from when it did actually represent these.
 * Only one instance should ever exist. Instantiating this class during gameplay
 * will likely immediately crash.
 */
class IsoCell {
public:
...
public
  ArrayList<IsoGridSquare> Trees = std::make_unique<ArrayList<>>();
  static final ArrayList<IsoGridSquare> stchoices =
      std::make_unique<ArrayList<>>();
public
  final IsoChunkMap[] ChunkMap = new IsoChunkMap[4];
public
  final ArrayList<IsoBuilding> BuildingList = std::make_unique<ArrayList<>>();
private
  final ArrayList<IsoWindow> WindowList = std::make_unique<ArrayList<>>();
private
  final ArrayList<IsoMovingObject> ObjectList = std::make_unique<ArrayList<>>();
private
  final ArrayList<IsoPushableObject> PushableObjectList =
      std::make_unique<ArrayList<>>();
private
  final HashMap<Integer, BuildingScore> BuildingScores =
      std::make_unique<HashMap<>>();
private
  final ArrayList<IsoRoom> RoomList = std::make_unique<ArrayList<>>();
private
  final ArrayList<IsoObject> StaticUpdaterObjectList =
      std::make_unique<ArrayList<>>();
private
  final ArrayList<IsoZombie> ZombieList = std::make_unique<ArrayList<>>();
private
  final ArrayList<IsoGameCharacter> RemoteSurvivorList =
      std::make_unique<ArrayList<>>();
private
  final ArrayList<IsoMovingObject> removeList = std::make_unique<ArrayList<>>();
private
  final ArrayList<IsoMovingObject> addList = std::make_unique<ArrayList<>>();
private
  final ArrayList<IsoObject> ProcessIsoObject = std::make_unique<ArrayList<>>();
private
  final ArrayList<IsoObject> ProcessIsoObjectRemove =
      std::make_unique<ArrayList<>>();
private
  final ArrayList<InventoryItem> ProcessItems = std::make_unique<ArrayList<>>();
private
  final ArrayList<InventoryItem> ProcessItemsRemove =
      std::make_unique<ArrayList<>>();
private
  final ArrayList<IsoWorldInventoryObject> ProcessWorldItems =
      std::make_unique<ArrayList<>>();
public
  final ArrayList<IsoWorldInventoryObject> ProcessWorldItemsRemove =
      std::make_unique<ArrayList<>>();
private
  final IsoGridSquare[][] gridSquares =
      new IsoGridSquare[4][IsoChunkMap.ChunkWidthInTiles *
                           IsoChunkMap.ChunkWidthInTiles * 8];
  ...
  bool safeToAdd = true;
private
  final Stack<IsoLightSource> LamppostPositions = std::make_unique<Stack<>>();
public
  final ArrayList<IsoRoomLight> roomLights = std::make_unique<ArrayList<>>();
private
  final ArrayList<IsoHeatSource> heatSources = std::make_unique<ArrayList<>>();
public
  final ArrayList<BaseVehicle> addVehicles = std::make_unique<ArrayList<>>();
public
...
private
  final IsoZombie[] nearestVisibleZombie = new IsoZombie[4];
private
  final float[] nearestVisibleZombieDistSqr = new float[4];
private
  static Stack<BuildingScore> buildingscores = std::make_unique<Stack<>>();
...
private
  static final ArrayList<IsoGridSquare> ShadowSquares = new ArrayList<>(1000);
private
  static final ArrayList<IsoGridSquare> MinusFloorCharacters =
      new ArrayList<>(1000);
private
  static final ArrayList<IsoGridSquare> SolidFloor = new ArrayList<>(5000);
private
  static final ArrayList<IsoGridSquare> ShadedFloor = new ArrayList<>(5000);
private
  static final ArrayList<IsoGridSquare> VegetationCorpses =
      new ArrayList<>(5000);
public
  static final IsoCell.PerPlayerRender[] perPlayerRender =
      new IsoCell.PerPlayerRender[4];
private
  final int[] StencilXY = new int[]{0,  0,  -1, 0,  0,  -1, -1, -1, -2, -1,
                                    -1, -2, -2, -2, -3, -2, -2, -3, -3, -3};
private
  final int[] StencilXY2z =
      new int[]{0,  0,  -1, 0,  0,  -1, -1, -1, -2, -1, -1, -2, -2,
                -2, -3, -2, -2, -3, -3, -3, -4, -3, -3, -4, -4, -4,
                -5, -4, -4, -5, -5, -5, -6, -5, -5, -6, -6, -6};
  int StencilX1;
  int StencilY1;
  int StencilX2;
  int StencilY2;
  Texture m_stencilTexture = null;
  const DiamondMatrixIterator diamondMatrixIterator =
      new DiamondMatrixIterator(123);
  const Vector2i diamondMatrixPos = new Vector2i();
  int DeferredCharacterTick = 0;
  bool hasSetupSnowGrid = false;
private
  IsoCell.SnowGridTiles snowGridTiles_Square;
private
  IsoCell.SnowGridTiles[] snowGridTiles_Strip;
private
  IsoCell.SnowGridTiles[] snowGridTiles_Edge;
private
  IsoCell.SnowGridTiles[] snowGridTiles_Cove;
private
  IsoCell.SnowGridTiles snowGridTiles_Enclosed;
  int m_snowFirstNonSquare = -1;
  Noise2D snowNoise2D = new Noise2D();
private
  IsoCell.SnowGrid snowGridCur;
private
  IsoCell.SnowGrid snowGridPrev;
  ...
  final ArrayList<IsoSurvivor> SurvivorList = std::make_unique<ArrayList<>>();
...
private
  int[] rainY = new int[4];
private
  Texture[] rainTextures = new Texture[5];
private
  long[] rainFileTime = new long[5];
  float rainAlphaMax = 0.6F;
private
  float[] rainAlpha = new float[4];
  int rainIntensity = 0;
  int rainSpeed = 6;
  ...
  Thread LightInfoUpdate = null;
private
  final Stack<IsoRoom> SpottedRooms = std::make_unique<Stack<>>();
  IsoZombie fakeZombieForHit;

  /**
   * @return the MaxHeight
   */
  static int getMaxHeight() { return MaxHeight; }

  LotHeader getCurrentLotHeader() {
    IsoChunk chunk = this.getChunkForGridSquare((int)IsoCamera.CamCharacter.x,
                                                (int)IsoCamera.CamCharacter.y,
                                                (int)IsoCamera.CamCharacter.z);
    return chunk.lotheader;
  }

  IsoChunkMap getChunkMap(int pl) { return this.ChunkMap[pl]; }
   ...
   */
public
  static Stack<BuildingScore> getBuildings() { return buildingscores; }
  ...
  IsoChunk getChunk(int _wx, int _wy) {
    for (int int0 = 0; int0 < IsoPlayer.numPlayers; int0++) {
      ...
      if (!chunkMap.ignore) {
        IsoChunk chunk = chunkMap.getChunk(_wx - chunkMap.getWorldXMin(),
                                           _wy - chunkMap.getWorldYMin());
        if (chunk != nullptr) {
          return chunk;
        }
      }
...
public
  IsoCell(int _width, int _height) {
  ...
  short getStencilValue(int x, int y, int z) {
    short[][][] shorts0 =
        perPlayerRender[IsoCamera.frameState.playerIndex].StencilValues;
    int int0 = 0;
    int int1 = 0;
    ...
    for (byte byte0 = 0; byte0 < this.StencilXY.length; byte0 += 2) {
      ...
      if (int3 >= this.minX && int3 < this.maxX && int4 >= this.minY &&
          int4 < this.maxY) {
  ...
  void setStencilValue(int x, int y, int z, int stencil) {
    ...
    for (byte byte0 = 0; byte0 < this.StencilXY.length; byte0 += 2) {
      ...
      if (int1 >= this.minX && int1 < this.maxX && int2 >= this.minY &&
          int2 < this.maxY) {
  ...
  void CalculateVertColoursForTile(IsoGridSquare sqThis, int x, int y, int zz,
    ...
    IsoGridSquare square0 =
        !IsoGridSquare.getMatrixBit(sqThis.visionMatrix, 0, 0, 1)
            ? sqThis.nav[IsoDirections.NW.index()]
            : null;
    IsoGridSquare square1 =
        !IsoGridSquare.getMatrixBit(sqThis.visionMatrix, 1, 0, 1)
            ? sqThis.nav[IsoDirections.N.index()]
            : null;
    IsoGridSquare square2 =
        !IsoGridSquare.getMatrixBit(sqThis.visionMatrix, 2, 0, 1)
            ? sqThis.nav[IsoDirections.NE.index()]
            : null;
    IsoGridSquare square3 =
        !IsoGridSquare.getMatrixBit(sqThis.visionMatrix, 2, 1, 1)
            ? sqThis.nav[IsoDirections.E.index()]
            : null;
    IsoGridSquare square4 =
        !IsoGridSquare.getMatrixBit(sqThis.visionMatrix, 2, 2, 1)
            ? sqThis.nav[IsoDirections.SE.index()]
            : null;
    IsoGridSquare square5 =
        !IsoGridSquare.getMatrixBit(sqThis.visionMatrix, 1, 2, 1)
            ? sqThis.nav[IsoDirections.S.index()]
            : null;
    IsoGridSquare square6 =
        !IsoGridSquare.getMatrixBit(sqThis.visionMatrix, 0, 2, 1)
            ? sqThis.nav[IsoDirections.SW.index()]
            : null;
    IsoGridSquare square7 =
        !IsoGridSquare.getMatrixBit(sqThis.visionMatrix, 0, 1, 1)
            ? sqThis.nav[IsoDirections.W.index()]
            : null;
  ...
  Texture getStencilTexture() {
    if (this.m_stencilTexture == nullptr) {
      this.m_stencilTexture =
          Texture.getSharedTexture("media/mask_circledithernew.png");
    }
  ...
  void DrawStencilMask() {
    ...
    if (texture != nullptr) {
      ...
      texture.renderstrip(int0 - (int)IsoCamera.getRightClickOffX(),
                          int1 - (int)IsoCamera.getRightClickOffY(),
                          texture.getWidth() * Core.TileScale,
                          texture.getHeight() * Core.TileScale, 1.0F, 1.0F,
                          1.0F, 1.0F, nullptr);
  ...
  void RenderTiles(int _MaxHeight) {
    IsoCell.s_performance.isoCellRenderTiles.invokeAndMeasure(
        this, _MaxHeight, IsoCell::renderTilesInternal);
  }
  ...
  void renderTilesInternal(int int1) {
    if (DebugOptions.instance.Terrain.RenderTiles.Enable.getValue()) {
      ...
      player.dirtyRecalcGridStackTime =
          player.dirtyRecalcGridStackTime -
          GameTime.getInstance().getMultiplier() / 4.0F;
      IsoCell.PerPlayerRender perPlayerRenderx =
          this.getPerPlayerRenderAt(int0);
      perPlayerRenderx.setSize(this.maxX - this.minX + 1,
      ...
      if (this.SetCutawayRoomsForPlayer() || boolean0) {
        ...
        for (int int2 = 0; int2 < int1 + 1; int2++) {
          ...
          for (int int3 = 0; int3 < GridStack.size(); int3++) {
            ...
            square.setPlayerCutawayFlag(
                int0, this.IsCutawaySquare(square, long0), long0);
          }
        }
      }
...
private
  IsoCell.PerPlayerRender getPerPlayerRenderAt(int int0) {
  ...
  void recalculateAnyGridStacks(IsoCell.PerPlayerRender perPlayerRenderx,
      ...
      for (int int4 = int5; int4 >= 0; int4--) {
        ...
        if (int4 < this.maxZ) {
          if (DebugOptions.instance.Terrain.RenderTiles.NewRender.getValue()) {
            DiamondMatrixIterator diamondMatrixIteratorx =
                this.diamondMatrixIterator.reset(this.maxX - this.minX);
            ...
            while (diamondMatrixIteratorx.next(vector2i)) {
              if (vector2i.y < this.maxY - this.minY + 1) {
                square0 = this.ChunkMap[int0].getGridSquare(
                    vector2i.x + this.minX, vector2i.y + this.minY, int4);
                if (int4 == 0) {
                ...
                if (square0 == nullptr) {
                  WeatherFxMask.addMaskLocation(nullptr, vector2i.x + this.minX,
                                                vector2i.y + this.minY, int4);
                } else {
                  IsoChunk chunk0 = square0.getChunk();
                  if (chunk0 != nullptr && square0.IsOnScreen(true)) {
                    WeatherFxMask.addMaskLocation(square0,
                                                  vector2i.x + this.minX,
                                                  vector2i.y + this.minY, int4);
            ...
            for (int int6 = this.minY; int6 < this.maxY; int6++) {
              int int7 = this.minX;
              IsoGridSquare square1 =
                  this.ChunkMap[int0].getGridSquare(int7, int6, int4);
              ...
              while (int7 < this.maxX) {
                ...
                int int9 =
                    this.ChunkMap[int0].WorldX - IsoChunkMap.ChunkGridWidth / 2;
                chunkMap0 = this.ChunkMap[int0];
                int int10 = int7 - int9 * 10;
                chunkMap0 = this.ChunkMap[int0];
                int9 =
                    this.ChunkMap[int0].WorldY - IsoChunkMap.ChunkGridWidth / 2;
                chunkMap0 = this.ChunkMap[int0];
                ...
                if (int10 != int1 || int11 != int2) {
                  IsoChunk chunk1 =
                      this.ChunkMap[int0].getChunkForGridSquare(int7, int6);
                  if (chunk1 != nullptr) {
                ...
                if (int3 < int4) {
                  ...
                  if (chunk2 != nullptr && square1.IsOnScreen(true)) {
                    WeatherFxMask.addMaskLocation(square1, square1.x, square1.y,
                                                  int4);
      ...
      if (!(vehicle.getAlpha(int3) <= 0.0F)) {
        for (int int4 = -2; int4 < 5; int4++) {
          for (int int5 = -2; int5 < 5; int5++) {
            ...
            if (int6 >= this.minX && int6 <= this.maxX && int7 >= this.minY &&
                int7 <= this.maxY) {
  ...
  void performRenderTiles(IsoCell.PerPlayerRender perPlayerRenderx, int int1,
                          int int3, long long0) {
    IsoGridStack gridStack = perPlayerRenderx.GridStacks;
    boolean[][] booleans = perPlayerRenderx.FlattenGrassEtc;
    Shader shader0;
    Shader shader1;
    if (Core.bDebug &&
        !DebugOptions.instance.Terrain.RenderTiles.UseShaders.getValue()) {
      shader0 = nullptr;
      shader1 = nullptr;
    } else {
    ...
    for (int int0 = 0; int0 < int1 + 1; int0++) {
      IsoCell.s_performance.renderTiles
          .PperformRenderTilesLayer pperformRenderTilesLayer =
          IsoCell.s_performance.renderTiles.performRenderTilesLayers.start(
              int0);
      ...
      for (int int2 = 0; int2 < GridStack.size(); int2++) {
        ...
        if (square0.chunk == nullptr ||
            !square0.chunk.bLightingNeverDone[int3]) {
          square0.bFlattenGrassEtc =
              int0 == 0 &&
              booleans[square0.x - this.minX][square0.y - this.minY];
      ...
      WorldMarkers.instance.renderGridSquareMarkers(perPlayerRenderx, int0,
                                                    int3);
      ...
      if (DebugOptions.instance.Terrain.RenderTiles.VegetationCorpses
              .getValue()) {
        ...
        for (int int6 = 0; int6 < VegetationCorpses.size(); int6++) {
          ...
          square1.renderMinusFloor(this.maxZ, false, true, false, false, false,
                                   shader1);
      ...
      if (DebugOptions.instance.Terrain.RenderTiles.MinusFloorCharacters
              .getValue()) {
        ...
        for (int int7 = 0; int7 < MinusFloorCharacters.size(); int7++) {
          ...
          bool boolean1 =
              square3 != null && square3.getPlayerCutawayFlag(int3, long0);
          bool boolean2 = square2.getPlayerCutawayFlag(int3, long0);
          bool boolean3 =
              square4 != null && square4.getPlayerCutawayFlag(int3, long0);
          this.currentLY = square2.getY() - this.minY;
          this.currentLZ = int0;
          ImprovedFog.renderRowsBehind(square2);
          bool boolean4 = square2.renderMinusFloor(
              this.maxZ, false, false, boolean1, boolean2, boolean3, shader1);
          ...
          if (boolean4) {
            square2.renderMinusFloor(this.maxZ, true, false, boolean1, boolean2,
                                     boolean3, shader1);
          }
        }
      ...
      IsoMarkers.instance.renderIsoMarkersDeferred(perPlayerRenderx, int0,
                                                   int3);
    ...
    for (int int0 = 0; int0 < ShadowSquares.size(); int0++) {
      ...
      for (int int1 = 0; int1 < square.getMovingObjects().size(); int1++) {
        ...
        IsoGameCharacter character =
            Type.tryCastTo(movingObject0, IsoGameCharacter.class);
        if (character != nullptr) {
          character.renderShadow(character.getX(), character.getY(),
                                 character.getZ());
        } else {
          BaseVehicle vehicle =
              Type.tryCastTo(movingObject0, BaseVehicle.class);
          if (vehicle != nullptr) {
      ...
      if (boolean0) {
        for (int int2 = 0; int2 < square.getStaticMovingObjects().size();
             int2++) {
          IsoMovingObject movingObject1 =
              square.getStaticMovingObjects().get(int2);
    ...
    if (Core.bDebug && DebugOptions.instance.LightingRender.getValue()) {
      ...
      for (int int0 = 0; int0 < int1 + 1; int0++) {
        ...
        for (int int2 = 0; int2 < GridStack.size(); int2++) {
          ...
          float float0 =
              IsoUtils.XToScreenExact(square.x + 0.3F, square.y, 0.0F, 0);
          float float1 =
              IsoUtils.YToScreenExact(square.x + 0.3F, square.y, 0.0F, 0);
          float float2 =
              IsoUtils.XToScreenExact(square.x + 0.6F, square.y, 0.0F, 0);
          float float3 =
              IsoUtils.YToScreenExact(square.x + 0.6F, square.y, 0.0F, 0);
          float float4 =
              IsoUtils.XToScreenExact(square.x + 1, square.y + 0.3F, 0.0F, 0);
          float float5 =
              IsoUtils.YToScreenExact(square.x + 1, square.y + 0.3F, 0.0F, 0);
          float float6 =
              IsoUtils.XToScreenExact(square.x + 1, square.y + 0.6F, 0.0F, 0);
          float float7 =
              IsoUtils.YToScreenExact(square.x + 1, square.y + 0.6F, 0.0F, 0);
          float float8 =
              IsoUtils.XToScreenExact(square.x + 0.6F, square.y + 1, 0.0F, 0);
          float float9 =
              IsoUtils.YToScreenExact(square.x + 0.6F, square.y + 1, 0.0F, 0);
          float float10 =
              IsoUtils.XToScreenExact(square.x + 0.3F, square.y + 1, 0.0F, 0);
          float float11 =
              IsoUtils.YToScreenExact(square.x + 0.3F, square.y + 1, 0.0F, 0);
          float float12 =
              IsoUtils.XToScreenExact(square.x, square.y + 0.6F, 0.0F, 0);
          float float13 =
              IsoUtils.YToScreenExact(square.x, square.y + 0.6F, 0.0F, 0);
          float float14 =
              IsoUtils.XToScreenExact(square.x, square.y + 0.3F, 0.0F, 0);
          float float15 =
              IsoUtils.YToScreenExact(square.x, square.y + 0.3F, 0.0F, 0);
          if (IsoGridSquare.getMatrixBit(square.visionMatrix, 0, 0, byte0)) {
            LineDrawer.drawLine(float0, float1, float2, float3, 1.0F, 0.0F,
                                0.0F, 1.0F, 0);
          }

          if (IsoGridSquare.getMatrixBit(square.visionMatrix, 0, 1, byte0)) {
            LineDrawer.drawLine(float2, float3, float4, float5, 1.0F, 0.0F,
                                0.0F, 1.0F, 0);
          }

          if (IsoGridSquare.getMatrixBit(square.visionMatrix, 0, 2, byte0)) {
            LineDrawer.drawLine(float4, float5, float6, float7, 1.0F, 0.0F,
                                0.0F, 1.0F, 0);
          }

          if (IsoGridSquare.getMatrixBit(square.visionMatrix, 1, 2, byte0)) {
            LineDrawer.drawLine(float6, float7, float8, float9, 1.0F, 0.0F,
                                0.0F, 1.0F, 0);
          }

          if (IsoGridSquare.getMatrixBit(square.visionMatrix, 2, 2, byte0)) {
            LineDrawer.drawLine(float8, float9, float10, float11, 1.0F, 0.0F,
                                0.0F, 1.0F, 0);
          }

          if (IsoGridSquare.getMatrixBit(square.visionMatrix, 2, 1, byte0)) {
            LineDrawer.drawLine(float10, float11, float12, float13, 1.0F, 0.0F,
                                0.0F, 1.0F, 0);
          }

          if (IsoGridSquare.getMatrixBit(square.visionMatrix, 2, 0, byte0)) {
            LineDrawer.drawLine(float12, float13, float14, float15, 1.0F, 0.0F,
                                0.0F, 1.0F, 0);
          }

          if (IsoGridSquare.getMatrixBit(square.visionMatrix, 1, 0, byte0)) {
            LineDrawer.drawLine(float14, float15, float0, float1, 1.0F, 0.0F,
                                0.0F, 1.0F, 0);
          }
        }
      }
  ...
  void CullFullyOccludedSquares(IsoGridStack gridStack, boolean[][][] booleans1,
                                boolean[][] booleans0) {
    int int0 = 0;
      ...
      for (int int3 = MaxHeight; int3 >= 0; int3--) {
        ...
        for (int int4 = GridStack.size() - 1; int4 >= 0; int4--) {
          ...
          if (int5 < 0 || int5 >= booleans0.length) {
              ...
              if (boolean0) {
                ...
                if (int5 > 2) {
                  if (int6 > 2) {
                    ...
                    boolean0 = !booleans1[int5 - 1][int6][0] ||
                               !booleans1[int5][int6][0] ||
                               !booleans1[int5][int6][1];
                  }
                } else if (int5 > 1) {
                  ...
                  boolean0 = !booleans1[int5][int6 - 1][1] ||
                             !booleans1[int5][int6][0] ||
                             !booleans1[int5][int6][1];
                } else if (int6 > 1) {
                  boolean0 = !booleans1[int5][int6 - 1][1] ||
                  ...
                  boolean0 =
                      !booleans1[int5][int6][0] || !booleans1[int5][int6][1];
                }
              }
            ...
            int2++;
            bool boolean1 =
                IsoGridSquare.getMatrixBit(square.visionMatrix, 0, 1, 1) &&
                square.getProperties().Is(IsoFlagType.cutW);
            bool boolean2 =
                IsoGridSquare.getMatrixBit(square.visionMatrix, 1, 0, 1) &&
                square.getProperties().Is(IsoFlagType.cutN);
            bool boolean3 = false;
            if (boolean1 || boolean2) {
              boolean3 = (square.x > IsoCamera.frameState.CamCharacterX ||
                          square.y > IsoCamera.frameState.CamCharacterY) &&
                         square.z >= (int)IsoCamera.frameState.CamCharacterZ;
              if (boolean3) {
                int int7 =
                    (int)(square.CachedScreenX - IsoCamera.frameState.OffX);
                int int8 =
                    (int)(square.CachedScreenY - IsoCamera.frameState.OffY);
  ...
  void RenderFloorShading(int zza) {
    if (DebugOptions.instance.Terrain.RenderTiles.IsoGridSquare.Floor
            .LightingOld.getValue() &&
        !DebugOptions.instance.Terrain.RenderTiles.IsoGridSquare.Floor.Lighting
             .getValue()) {
          ...
          if (texture != nullptr) {
            ...
            for (int int3 = 0; int3 < ShadedFloor.size(); int3++) {
              ...
              if (square.getProperties().Is(IsoFlagType.solidfloor)) {
                ...
                float float1 = 0.0F;
                float float2 = 0.0F;
                if (square.getProperties().Is(
                        IsoFlagType.FloorHeightOneThird)) {
                  float1 = -1.0F;
                  float0 = -1.0F;
                } else if (square.getProperties().Is(
                ...
                float float3 =
                    IsoUtils.XToScreen(square.getX() + float0,
                                       square.getY() + float1, zza + float2, 0);
                float float4 =
                    IsoUtils.YToScreen(square.getX() + float0,
                                       square.getY() + float1, zza + float2, 0);
                float3 -= int1;
                float4 -= int2;
                ...
                texture.renderdiamond(float3 - 32 * Core.TileScale,
                                      float4 + 16 * Core.TileScale,
                                      64 * Core.TileScale, 32 * Core.TileScale,
                                      int7, int4, int5, int6);
              }
            }
          }
  ...
  int GetEffectivePlayerRoomId() {
    ...
    if (IsoPlayer.players[int0] != nullptr &&
        IsoPlayer.players[int0].isClimbing()) {
      int1 = -1;
    }
    ...
    if (square0 == nullptr) {
      ...
      if (int1 == -1) {
        if (this.playerWindowPeekingRoomId[int0] != -1) {
          this.tempPlayerCutawayRoomIDs.add(
              this.playerWindowPeekingRoomId[int0]);
        } else {
        ...
        for (int int6 = int2; int6 <= int4; int6++) {
          for (int int7 = int3; int7 <= int5; int7++) {
            IsoGridSquare square1 =
                this.getGridSquare(int6, int7, square0.getZ());
            if (square1 != nullptr) {
              int int8 = square1.getRoomID();
              if (square1.getCanSee(int0) && int8 != -1 &&
                  !this.tempPlayerCutawayRoomIDs.contains(int8)) {
                this.tempCutawaySqrVector.set(
                    square1.getX() + 0.5F - player.getX(),
                    square1.getY() + 0.5F - player.getY());
                if (square0 == square1 ||
                    player.getForwardDirection().dot(
                        this.tempCutawaySqrVector) > 0.0F) {
      ...
      if (square1.getZ() != square0.getZ()) {
        ...
        if (this.tempPlayerCutawayRoomIDs.isEmpty()) {
          ...
          if (this.IsCollapsibleBuildingSquare(square0)) {
            ...
            if (square0.getBuilding() != nullptr &&
                (square1.getX() < square0.getBuilding().def.x ||
                 square1.getY() < square0.getBuilding().def.y)) {
              return true;
            }
          ...
          if (square2 != nullptr && square2.getRoomID() == -1 &&
              square3 != nullptr && square3.getRoomID() == -1) {
            return this.DoesSquareHaveValidCutaways(square1, square0, int0,
                                                    long0);
          }
        } else {
          ...
          for (int int2 = 0; int2 < 8; int2++) {
            if (square0.nav[int2] != nullptr &&
                square0.nav[int2].getRoomID() != square0.getRoomID()) {
              boolean0 = true;
              break;
            }
          }
          ...
          if (boolean0 || boolean1 || square0.getWall() != nullptr) {
            ...
            for (int int3 = 0; int3 < 3; int3++) {
              ...
              if (square13.getRoomID() != -1 &&
                  this.tempPlayerCutawayRoomIDs.contains(
                      square13.getRoomID())) {
          ...
          if (square5 != nullptr && square8 != nullptr &&
          ...
          if (square1.getRoomID() == -1 &&
            ...
            if (int4 < 0 && int5 < 0) {
              if (int4 >= -3) {
                ...
                if (square5 != nullptr && square7 != nullptr &&
  ...
  bool DoesSquareHaveValidCutaways(IsoGridSquare square6, IsoGridSquare square1,
                                   int int1, long long0) {
    ...
    if (square0 != nullptr && square0.nav[IsoDirections.W.index()] != nullptr &&
        square0.nav[IsoDirections.W.index()].getRoomID() ==
            square0.getRoomID()) {
      object3 = square0.getWall(false);
    }

    if (square4 != nullptr && square4.nav[IsoDirections.N.index()] != nullptr &&
        square4.nav[IsoDirections.N.index()].getRoomID() ==
            square4.getRoomID()) {
      object2 = square4.getWall(true);
    }

    if (object1 != nullptr || object0 != nullptr || object3 != nullptr ||
        object2 != nullptr) {
      IsoGridSquare square5 = square1.nav[IsoDirections.NW.index()];

      for (int int0 = 0; int0 < 2 && square5 != nullptr &&
                         square5.getRoomID() == square6.getRoomID();
           int0++) {
        IsoGridSquare square7 = square5.nav[IsoDirections.S.index()];
        IsoGridSquare square8 = square5.nav[IsoDirections.E.index()];
        if (square7 != nullptr && square7.getBuilding() != nullptr ||
            square8 != nullptr && square8.getBuilding() != nullptr) {
          break;
        }

        if (square5.isCanSee(int1) && square5.isCouldSee(int1) &&
            square5.DistTo(square6) <= 6 - (int0 + 1)) {
          return true;
        }
    ...
    if ((object0 == nullptr || !object0.sprite.name.contains("fencing")) &&
        (object1 == nullptr || !object1.sprite.name.contains("fencing"))) {
      if (square1.DistTo(square6) <= 6.0F &&
          square1.nav[IsoDirections.NW.index()] != nullptr &&
          square1.nav[IsoDirections.NW.index()].getRoomID() ==
              square1.getRoomID() &&
          (square1.getWall(true) == nullptr ||
           square1.getWall(true) == object0) &&
          (square1.getWall(false) == nullptr ||
           square1.getWall(false) == object1)) {
        if (square3 != nullptr && square0 != nullptr && int3 != 0) {
          if (int3 > 0 && object1 != nullptr &&
    ...
    if (square1 == square6 &&
        ...
        square1.nav[IsoDirections.NW.index()].getRoomID() ==
            square1.getRoomID()) {
      if (object0 != nullptr && square0 != nullptr &&
          square0.getWall(false) == nullptr && square0.isCanSee(int1) &&
          square0.isCouldSee(int1)) {
        return true;
      }

      if (object1 != nullptr && square4 != nullptr &&
          square4.getWall(true) != nullptr && square4.isCanSee(int1) &&
          square4.isCouldSee(int1)) {
        return true;
      }
    }

    return square0 != nullptr && square4 != nullptr && int2 != 0 && int3 != 0 &&
                   object3 != nullptr && object2 != nullptr &&
                   square0.getPlayerCutawayFlag(int1, long0) &&
                   square4.getPlayerCutawayFlag(int1, long0)
               ? true
               : int2 < 0 && int2 >= -6 && int3 < 0 && int3 >= -6 &&
                     (object1 != nullptr && square1.getWall(true) == nullptr ||
                      object0 != nullptr && square1.getWall(false) == nullptr);
  }
  ...
  bool IsCollapsibleBuildingSquare(IsoGridSquare square) {
    if (square.getProperties().Is(IsoFlagType.forceRender)) {
      ...
      for (int int0 = 0; int0 < 4; int0++) {
        ...
        for (int int1 = 0;
             int1 < short0 && this.playerOccluderBuildingsArr[int0] != nullptr;
             int1++) {
          ...
          if (this.collapsibleBuildingSquareAlgorithm(
                  buildingDef0, square, IsoPlayer.players[int0].getSquare())) {
            return true;
          }

          if (square.getY() - buildingDef0.getY2() == 1 &&
              square.getWall(true) != nullptr) {
            return true;
          }

          if (square.getX() - buildingDef0.getX2() == 1 &&
              square.getWall(false) != nullptr) {
            return true;
          }
        }
      }
      ...
      if (player.getVehicle() != nullptr) {
        ...
        for (int int3 = 0;
             int3 < 500 && this.zombieOccluderBuildingsArr[int2] != nullptr;
             int3++) {
          ...
          if (this.collapsibleBuildingSquareAlgorithm(buildingDef1, square,
                                                      player.getSquare())) {
            return true;
          }
        }
        ...
        for (int int4 = 0;
             int4 < 500 && this.otherOccluderBuildingsArr[int2] != nullptr;
             int4++) {
          ...
          if (this.collapsibleBuildingSquareAlgorithm(buildingDef2, square,
                                                      player.getSquare())) {
            return true;
          }
        }
  ...
  bool collapsibleBuildingSquareAlgorithm(BuildingDef buildingDef,
                                          IsoGridSquare square2,
                                          IsoGridSquare square0) {
    ...
    if (square0.getRoomID() == -1 &&
        this.buildingRectTemp.contains(square0.getX(), square0.getY())) {
      ...
      if (square4 != nullptr && square1 != nullptr && square3 != nullptr) {
        ...
        return this.buildingRectTemp.contains(square2.getX(), square2.getY()) &&
               (square0.getZ() < square2.getZ() ||
                ...
                !boolean0 && (boolean3 || boolean1 == boolean2 ||
                              boolean1 && boolean5 || boolean2 && boolean4));
      } else {
        return false;
      }
    } else {
    ...
    if (player.current == nullptr) {
      ...
      if (square0.getZ() >= square1.getZ()) {
        ...
        if (square1.getZ() > this.hidesOrphanStructuresAbove) {
          ...
          for (int int1 = square1.getZ() - 1; int1 >= 0 && building == nullptr;
               int1--) {
            IsoGridSquare square2 =
                this.getGridSquare(square1.x, square1.y, int1);
            if (square2 != nullptr) {
  ...
  void updateSnow(int int0) {
    if (this.snowGridCur == nullptr) {
      ...
      if (int0 != this.snowGridCur.frac) {
        ...
        DebugLog.log("snow from " + this.snowGridPrev.frac + " to " +
                     this.snowGridCur.frac);
      }
    }
  }
    ...
    if (square != nullptr) {
      if (!square.getProperties().Is(IsoFlagType.solidfloor)) {
        ...
        return false;
                 ...
                 square.room == nullptr && !square.isInARoom()) {
      ...
      if (snowGrid0 != nullptr) {
        ...
        if (snowGrid0.frac > 0 || snowGrid1.frac > 0) {
          ...
          for (int int2 = 0; int2 < SolidFloor.size(); int2++) {
            ...
            if (square.room == nullptr &&
                square.getProperties().Is(IsoFlagType.exterior) &&
                square.getProperties().Is(IsoFlagType.solidfloor)) {
              int int3;
              ...
              float float3 =
                  IsoUtils.XToScreen(square.getX(), square.getY(), int6, 0);
              float float4 =
                  IsoUtils.YToScreen(square.getX(), square.getY(), int6, 0);
              float3 -= int0;
              float4 -= int1;
              ...
              FloorShaperAttachedSprites.instance.setVertColors(int8, int9,
                                                                int10, int11);
              FloorShaperDiamond.instance.setVertColors(int8, int9, int10,
                                                        int11);

              for (int int12 = 0; int12 < 2; int12++) {
                if (float1 > float0) {
                  this.renderSnowTileGeneral(snowGrid0, float0, square, int3,
                                             int4, int5, (int)float3,
                                             (int)float4, int12);
                  this.renderSnowTileGeneral(snowGrid1, float1, square, int3,
                                             int4, int5, (int)float3,
                                             (int)float4, int12);
                } else {
                  this.renderSnowTileGeneral(snowGrid1, float1, square, int3,
...
private
  void renderSnowTileGeneral(IsoCell.SnowGrid snowGrid, float float0,
                             IsoGridSquare square, int int3, int int2, int int1,
                             int int4, int int5, int int0) {
    if (!(float0 <= 0.0F)) {
      Texture texture = snowGrid.grid[int2][int1][int0];
      if (texture != nullptr) {
        if (int0 == 0) {
          this.renderSnowTile(snowGrid, int2, int1, int0, square, int3, texture,
                              int4, int5, float0);
        } else if (int3 == 0) {
          uint8_t byte0 = snowGrid.gridType[int2][int1][int0];
          this.renderSnowTileBase(texture, int4, int5, float0,
                                  byte0 < this.m_snowFirstNonSquare);
        }
      }
    }
  }

  void renderSnowTileBase(Texture texture, int int1, int int0, float float0,
                          bool boolean0) {
    void *object = boolean0 ? FloorShaperDiamond.instance
                            : FloorShaperAttachedSprites.instance;
    ((FloorShaper)object).setAlpha4(float0);
    texture.render(int1, int0, texture.getWidth(), texture.getHeight(), 1.0F,
                   1.0F, 1.0F, float0, (Consumer<TextureDraw>)object);
  }

private
  void renderSnowTile(IsoCell.SnowGrid snowGrid, int int3, int int2, int int1,
                      IsoGridSquare square, int int0, Texture texture, int int4,
                      int int5, float float0) {
    if (int0 == 0) {
      ...
      bool boolean1 =
          (int0 & 1) == 1 && (boolean0 || snowGrid.check(int3, int2 - 1));
      bool boolean2 =
          (int0 & 2) == 2 && (boolean0 || snowGrid.check(int3 + 1, int2));
      bool boolean3 =
          (int0 & 4) == 4 && (boolean0 || snowGrid.check(int3, int2 + 1));
      bool boolean4 =
          (int0 & 8) == 8 && (boolean0 || snowGrid.check(int3 - 1, int2));
      if (boolean1) {
        int6++;
      }
  ...
  static bool isSnowShore(IsoGridSquare square1, int int1, int int0) {
    IsoGridSquare square0 = IsoWorld.instance.getCell().getGridSquare(
        square1.getX() + int1, square1.getY() + int0, 0);
  ...
  IsoBuilding getClosestBuildingExcept(IsoGameCharacter chr, IsoRoom except) {
    ...
    for (int int0 = 0; int0 < this.BuildingList.size(); int0++) {
      ...
      for (int int1 = 0; int1 < building1.Exits.size(); int1++) {
        float float1 = chr.DistTo(building1.Exits.get(int1).x,
                                  building1.Exits.get(int1).y);
        if (float1 < float0 &&
            (except == nullptr || except.building != building1)) {
          building0 = building1;
          float0 = float1;
        }
      }
  ...
  void ObjectDeletionAddition() {
    for (int int0 = 0; int0 < this.removeList.size(); int0++) {
      ...
      if (!(movingObject0 instanceof IsoPlayer) ||
          ((IsoPlayer)movingObject0).isDead()) {
        ...
        if (movingObject0.getCurrentSquare() != nullptr) {
          movingObject0.getCurrentSquare().getMovingObjects().remove(
              movingObject0);
        }

        if (movingObject0.getLastSquare() != nullptr) {
          movingObject0.getLastSquare().getMovingObjects().remove(
              movingObject0);
        }
      }
    }
  ...
  void ProcessItems(Iterator<InventoryItem> var1) {
    ...
    for (int int2 = 0; int2 < int0; int2++) {
      IsoWorldInventoryObject worldInventoryObject =
          this.ProcessWorldItems.get(int2);
  ...
  void DoGridNav(IsoGridSquare newSquare, IsoGridSquare.GetSquare getter) {
    ...
    newSquare.nav[IsoDirections.N.index()] =
        getter.getGridSquare(int0, int1 - 1, int2);
    newSquare.nav[IsoDirections.NW.index()] =
        getter.getGridSquare(int0 - 1, int1 - 1, int2);
    newSquare.nav[IsoDirections.W.index()] =
        getter.getGridSquare(int0 - 1, int1, int2);
    newSquare.nav[IsoDirections.SW.index()] =
        getter.getGridSquare(int0 - 1, int1 + 1, int2);
    newSquare.nav[IsoDirections.S.index()] =
        getter.getGridSquare(int0, int1 + 1, int2);
    newSquare.nav[IsoDirections.SE.index()] =
        getter.getGridSquare(int0 + 1, int1 + 1, int2);
    newSquare.nav[IsoDirections.E.index()] =
        getter.getGridSquare(int0 + 1, int1, int2);
    newSquare.nav[IsoDirections.NE.index()] =
        getter.getGridSquare(int0 + 1, int1 - 1, int2);
    if (newSquare.nav[IsoDirections.N.index()] != nullptr) {
      newSquare.nav[IsoDirections.N.index()].nav[IsoDirections.S.index()] =
          newSquare;
    }

    if (newSquare.nav[IsoDirections.NW.index()] != nullptr) {
      newSquare.nav[IsoDirections.NW.index()].nav[IsoDirections.SE.index()] =
          newSquare;
    }

    if (newSquare.nav[IsoDirections.W.index()] != nullptr) {
      newSquare.nav[IsoDirections.W.index()].nav[IsoDirections.E.index()] =
          newSquare;
    }

    if (newSquare.nav[IsoDirections.SW.index()] != nullptr) {
      newSquare.nav[IsoDirections.SW.index()].nav[IsoDirections.NE.index()] =
          newSquare;
    }

    if (newSquare.nav[IsoDirections.S.index()] != nullptr) {
      newSquare.nav[IsoDirections.S.index()].nav[IsoDirections.N.index()] =
          newSquare;
    }

    if (newSquare.nav[IsoDirections.SE.index()] != nullptr) {
      newSquare.nav[IsoDirections.SE.index()].nav[IsoDirections.NW.index()] =
          newSquare;
    }

    if (newSquare.nav[IsoDirections.E.index()] != nullptr) {
      newSquare.nav[IsoDirections.E.index()].nav[IsoDirections.W.index()] =
          newSquare;
    }

    if (newSquare.nav[IsoDirections.NE.index()] != nullptr) {
      newSquare.nav[IsoDirections.NE.index()].nav[IsoDirections.SW.index()] =
          newSquare;
    }
  }
  ...
  IsoGridSquare ConnectNewSquare(IsoGridSquare newSquare, bool bDoSurrounds) {
    for (int int0 = 0; int0 < IsoPlayer.numPlayers; int0++) {
      if (!this.ChunkMap[int0].ignore) {
        this.ChunkMap[int0].setGridSquare(newSquare, newSquare.getX(),
                                          newSquare.getY(), newSquare.getZ());
      }
    }
    ...
    for (int int1 = sx; int1 < sx + lot.info.width; int1++) {
      for (int int2 = sy; int2 < sy + lot.info.height; int2++) {
        for (int int3 = sz; int3 < int0; int3++) {
          ...
          int int5 = int2 - sy;
          int int6 = int3 - sz;
          if (int1 < this.width && int2 < this.height && int1 >= 0 &&
              int2 >= 0 && int3 >= 0) {
            ...
            if (int8 != -1) {
              ...
              if (int9 > 0) {
                ...
                for (int int10 = 0; int10 < int9; int10++) {
                  std::string string = lot.info.tilesUsed.get(
                      lot.m_data.getQuick(int8 + 1 + int10));
                  IsoSprite sprite =
                      IsoSpriteManager.instance.NamedMap.get(string);
                  if (sprite == nullptr) {
                    Logger.getLogger(GameWindow.class.getName())
                        .log(Level.SEVERE,
                             "Missing tile definition: " + string);
                  } else {
                    IsoGridSquare square = this.getGridSquare(int1, int2, int3);
                    if (square == nullptr) {
                      if (IsoGridSquare.loadGridSquareCache != nullptr) {
                        square = IsoGridSquare.getNew(
                            IsoGridSquare.loadGridSquareCache, this, nullptr,
                            int1, int2, int3);
                      } else {
                        square = IsoGridSquare.getNew(this, nullptr, int1, int2,
                                                      int3);
                      }

                      this.ChunkMap[IsoPlayer.getPlayerIndex()].setGridSquare(
                          square, int1, int2, int3);
                    } else {
                      if (bClearExisting && int10 == 0 &&
                          sprite.getProperties().Is(IsoFlagType.solidfloor) &&
                          (!sprite.Properties.Is(IsoFlagType.hidewalls) ||
                           int9 > 1)) {
                        boolean0 = true;
                      }
                    ...
                    CellLoader.DoTileObjectCreation(sprite, sprite.getType(),
                                                    square, this, int1, int2,
                                                    int3, string);
                  }
                }
              }
  ...
  void PlaceLot(IsoLot lot, int sx, int sy, int sz, IsoChunk ch, int WX,
                int WY) {
    WX *= 10;
    WY *= 10;
      ...
      for (int int1 = WX + sx; int1 < WX + sx + 10; int1++) {
        for (int int2 = WY + sy; int2 < WY + sy + 10; int2++) {
          for (int int3 = sz; int3 < int0; int3++) {
            ...
            int int5 = int2 - WY - sy;
            int int6 = int3 - sz;
            if (int1 < WX + 10 && int2 < WY + 10 && int1 >= WX && int2 >= WY &&
                int3 >= 0) {
              int int7 = int4 + int5 * 10 + int6 * 100;
              int int8 = lot.m_offsetInData[int7];
              if (int8 != -1) {
                ...
                if (int9 > 0) {
                  IsoGridSquare square0 =
                      ch.getGridSquare(int1 - WX, int2 - WY, int3);
                  if (square0 == nullptr) {
                    if (IsoGridSquare.loadGridSquareCache != nullptr) {
                      ...
                      square0 =
                          IsoGridSquare.getNew(this, nullptr, int1, int2, int3);
                    }
                  ...
                  for (int int10 = -1; int10 <= 1; int10++) {
                    for (int int11 = -1; int11 <= 1; int11++) {
                      if ((int10 != 0 || int11 != 0) &&
                          int10 + int1 - WX >= 0 && int10 + int1 - WX < 10 &&
                          int11 + int2 - WY >= 0 && int11 + int2 - WY < 10) {
                        IsoGridSquare square1 = ch.getGridSquare(
                            int1 + int10 - WX, int2 + int11 - WY, int3);
                        if (square1 == nullptr) {
                          square1 = IsoGridSquare.getNew(
                              this, nullptr, int1 + int10, int2 + int11, int3);
                          ch.setSquare(int1 + int10 - WX, int2 + int11 - WY,
                                       int3, square1);
                        }
                      }
                    }
                  ...
                  if (roomDef != nullptr) {
                    ...
                    square0.roofHideBuilding =
                        room == nullptr ? nullptr : room.building;
                  }
    ...
    if (player >= 0 && player < 4) {
      if (this.drag[player] != nullptr && this.drag[player] != item) {
        ...
        if (object instanceof JavaFunction || object instanceof LuaClosure) {
          LuaManager.caller.pcallvoid(LuaManager.thread, object,
                                      this.drag[player]);
        }
      }
  ...
  bool doBuildingInternal(int int0, bool boolean0) {
    if (UIManager.getPickedTile() != nullptr && this.drag[int0] != nullptr &&
        JoypadManager.instance.getFromPlayer(int0) == nullptr) {
      if (!IsoWorld.instance.isValidSquare(
      ...
      IsoGridSquare square = this.getGridSquare(
          (int)UIManager.getPickedTile().x, (int)UIManager.getPickedTile().y,
          (int)IsoCamera.CamCharacter.getZ());
      if (!boolean0) {
        if (square == nullptr) {
          square = this.createNewGridSquare((int)UIManager.getPickedTile().x,
                                            (int)UIManager.getPickedTile().y,
                                            (int)IsoCamera.CamCharacter.getZ(),
                                            true);
          if (square == nullptr) {
      ...
      LuaEventManager.triggerEvent("OnDoTileBuilding2", this.drag[int0],
                                   boolean0, (int)UIManager.getPickedTile().x,
                                   (int)UIManager.getPickedTile().y,
                                   (int)IsoCamera.CamCharacter.getZ(), square);
    }

    if (this.drag[int0] != nullptr &&
        JoypadManager.instance.getFromPlayer(int0) != nullptr) {
      LuaEventManager.triggerEvent("OnDoTileBuilding3", this.drag[int0],
                                   boolean0,
  ...
  float DistanceFromSupport(int x, int y, int z) { return 0.0F; }
   ...
   */
public
  ArrayList<IsoBuilding> getBuildingList() { return this.BuildingList; }

public
  ArrayList<IsoWindow> getWindowList() { return this.WindowList; }
   ...
   */
public
  ArrayList<IsoMovingObject> getObjectList() { return this.ObjectList; }
   ...
   */
public
  ArrayList<IsoPushableObject> getPushableObjectList() {
   ...
   */
public
  HashMap<Integer, BuildingScore> getBuildingScores() {
   ...
   */
public
  ArrayList<IsoRoom> getRoomList() { return this.RoomList; }

  /**
   * @return the StaticUpdaterObjectList
   */
public
  ArrayList<IsoObject> getStaticUpdaterObjectList() {
   ...
   */
public
  ArrayList<IsoZombie> getZombieList() { return this.ZombieList; }
   ...
   */
public
  ArrayList<IsoGameCharacter> getRemoteSurvivorList() {
   ...
   */
public
  ArrayList<IsoMovingObject> getRemoveList() { return this.removeList; }

  /**
   * @return the addList
   */
public
  ArrayList<IsoMovingObject> getAddList() { return this.addList; }

  void addMovingObject(IsoMovingObject o) { this.addList.add(o); }

  /**
   * @return the ProcessItems
   */
public
  ArrayList<InventoryItem> getProcessItems() { return this.ProcessItems; }

public
  ArrayList<IsoWorldInventoryObject> getProcessWorldItems() {
    return this.ProcessWorldItems;
  }

public
  ArrayList<IsoObject> getProcessIsoObjects() { return this.ProcessIsoObject; }

  /**
   * @return the ProcessItemsRemove
   */
public
  ArrayList<InventoryItem> getProcessItemsRemove() {
    return this.ProcessItemsRemove;
  }

public
  ArrayList<BaseVehicle> getVehicles() { return this.vehicles; }
   ...
   * @return the worldX
   */
  int getWorldX() { return this.worldX; }

  /**
   *
   * @param _worldX the worldX to set
   */
  void setWorldX(int _worldX) { this.worldX = _worldX; }

  /**
   * @return the worldY
   */
  int getWorldY() { return this.worldY; }

  /**
   *
   * @param _worldY the worldY to set
   */
  void setWorldY(int _worldY) { this.worldY = _worldY; }

  /**
   * @return the safeToAdd
   */
  bool isSafeToAdd() { return this.safeToAdd; }

  /**
   *
   * @param _safeToAdd the safeToAdd to set
   */
  void setSafeToAdd(bool _safeToAdd) { this.safeToAdd = _safeToAdd; }

  /**
   * @return the LamppostPositions
   */
public
  Stack<IsoLightSource> getLamppostPositions() {
  ...
  IsoLightSource getLightSourceAt(int x, int y, int z) {
    for (int int0 = 0; int0 < this.LamppostPositions.size(); int0++) {
      ...
      if (lightSource.getX() == x && lightSource.getY() == y &&
          lightSource.getZ() == z) {
        return lightSource;
      }
    }
  ...
  IsoLightSource addLamppost(int x, int y, int z, float r, float g, float b,
                             int rad) {
   ...
   */
  int getCurrentLightX() { return this.currentLX; }

  /**
   *
   * @param _currentLX the currentLX to set
   */
  void setCurrentLightX(int _currentLX) { this.currentLX = _currentLX; }

  /**
   * @return the currentLY
   */
  int getCurrentLightY() { return this.currentLY; }

  /**
   *
   * @param _currentLY the currentLY to set
   */
  void setCurrentLightY(int _currentLY) { this.currentLY = _currentLY; }

  /**
   * @return the currentLZ
   */
  int getCurrentLightZ() { return this.currentLZ; }

  /**
   *
   * @param _currentLZ the currentLZ to set
   */
  void setCurrentLightZ(int _currentLZ) { this.currentLZ = _currentLZ; }
   ...
   */
  void setMinX(int _minX) { this.minX = _minX; }
   ...
   */
  void setMaxX(int _maxX) { this.maxX = _maxX; }
   ...
   */
  void setMinY(int _minY) { this.minY = _minY; }
   ...
   */
  void setMaxY(int _maxY) { this.maxY = _maxY; }

  /**
   * @return the minZ
   */
  int getMinZ() { return this.minZ; }

  /**
   *
   * @param _minZ the minZ to set
   */
  void setMinZ(int _minZ) { this.minZ = _minZ; }
   ...
   */
  void setMaxZ(int _maxZ) { this.maxZ = _maxZ; }

  /**
   * @return the dangerUpdate
   */
  OnceEvery getDangerUpdate() { return this.dangerUpdate; }
   ...
   */
  Thread getLightInfoUpdate() { return this.LightInfoUpdate; }
...
public
  ArrayList<IsoSurvivor> getSurvivorList() { return this.SurvivorList; }

  static int getRComponent(int col) { return col & 0xFF; }

  static int getGComponent(int col) { return (col & 0xFF00) >> 8; }

  static int getBComponent(int col) { return (col & 0xFF0000) >> 16; }

  static int toIntColor(float r, float g, float b, float a) {
    return (int)(r * 255.0F) << 0 | (int)(g * 255.0F) << 8 |
           (int)(b * 255.0F) << 16 | (int)(a * 255.0F) << 24;
  }
  ...
  IsoGridSquare getRandomOutdoorTile() {
    ...
    do {
      square = this.getGridSquare(
          this.ChunkMap[IsoPlayer.getPlayerIndex()].getWorldXMin() * 10 +
              Rand.Next(this.width),
          this.ChunkMap[IsoPlayer.getPlayerIndex()].getWorldYMin() * 10 +
              Rand.Next(this.height),
          0);
      if (square != nullptr) {
        square.setCachedIsFree(false);
      }
    } while (square == nullptr || !square.isFree(false) ||
             square.getRoom() != nullptr);

    return square;
  }

  static void InsertAt(int int1, BuildingScore buildingScore,
  ...
  static void Place(BuildingScore buildingScore, BuildingScore[] buildingScores,
    ...
    for (int int0 = 0; int0 < buildingScores.length; int0++) {
      if (buildingScores[int0] != nullptr) {
        ...
        if (buildingScores[int0] == nullptr) {
          ...
          case General:
            if (buildingScores[int0].food + buildingScores[int0].defense +
                    buildingScores[int0].size + buildingScores[int0].weapons <
                buildingScore.food + buildingScore.defense +
                    buildingScore.size + buildingScore.weapons) {
              boolean0 = true;
            }
            break;
          case Food:
...
public
  Stack<BuildingScore> getBestBuildings(IsoCell.BuildingSearchCriteria criteria,
                                        int count) {
  ...
  bool blocked(Mover mover, int x, int y, int z, int lx, int ly, int lz) {
    ...
    if (square == nullptr) {
      ...
      if (mover instanceof IsoMovingObject) {
        if (square.testPathFindAdjacent((IsoMovingObject)mover, x - lx, y - ly,
                                        z - lz)) {
          return true;
        }
  ...
  void Dispose() {
    ...
    IsoGridSquare getGridSquare(double x, double y, double z) {
          return GameServer.bServer
                     ? ServerMap.instance.getGridSquare((int)x, (int)y, (int)z)
                     : this.getGridSquare((int)x, (int)y, (int)z);
        }
    ...
    IsoGridSquare getOrCreateGridSquare(double x, double y, double z) {
          if (GameServer.bServer) {
            ...
            if (square0 == nullptr) {
              square0 =
                  IsoGridSquare.getNew(this, nullptr, (int)x, (int)y, (int)z);
        ...
        void setCacheGridSquare(int x, int y, int z, IsoGridSquare square) {
          assert square = = null || x == square.getX() && y == square.getY() &&
                                        z == square.getZ();
          ...
          if (!GameServer.bServer) {
            ...
            for (int int1 = 0; int1 < IsoPlayer.numPlayers; int1++) {
              if (!this.ChunkMap[int1].ignore) {
                ...
                if (z < 8 && z >= 0 && int2 >= 0 && int2 < int0 && int3 >= 0 &&
                    int3 < int0) {
                  this.gridSquares[int1][int2 + int3 * int0 + z * int0 * int0] =
                      square;
                }
              }
            }
        ...
        void setCacheChunk(IsoChunk chunk, int playerIndex) {
          if (!GameServer.bServer) {
            ...
            if (!chunkMap.ignore) {
              ...
              if (int1 >= 0 && int1 < IsoChunkMap.ChunkGridWidth && int2 >= 0 &&
                  ...
                  for (int int4 = 0; int4 < 10; int4++) {
                    for (int int5 = 0; int5 < 10; int5++) {
                      IsoGridSquare square =
                          chunk.squares[int3][int5 + int4 * 10];
        ...
        void setCacheGridSquareLocal(int x, int y, int z, IsoGridSquare square,
          ...
          if (!GameServer.bServer) {
            ...
            if (z < 8 && z >= 0 && x >= 0 && x < int0 && y >= 0 && y < int0) {
              this.gridSquares[playerIndex][x + y * int0 + z * int0 * int0] =
                  square;
            }
          }
        }
            ...
            for (int int1 = 0; int1 < IsoPlayer.numPlayers; int1++) {
              if (!this.ChunkMap[int1].ignore) {
                ...
                if (z < 8 && z >= 0 && int2 >= 0 && int2 < int0 && int3 >= 0 &&
                    int3 < int0) {
                  IsoGridSquare square =
                      this.gridSquares[int1]
                                      [int2 + int3 * int0 + z * int0 * int0];
                  if (square != nullptr) {
                    return square;
                  }
                }
        ...
        void DeleteAllMovingObjects() { this.ObjectList.clear(); }
    ...
    return 8;
        ...
        int CalculateColor(IsoGridSquare square1, IsoGridSquare square2,
          ...
          if (square0 == nullptr) {
            ...
            float float4 = 0.0F;
            bool boolean0 = true;
            if (square1 != nullptr && square0.room == square1.room &&
                square1.getChunk() != nullptr) {
              float4++;
            ...
            if (square2 != nullptr && square0.room == square2.room &&
                square2.getChunk() != nullptr) {
              float4++;
            ...
            if (square3 != nullptr && square0.room == square3.room &&
                square3.getChunk() != nullptr) {
              float4++;
            ...
            if (square0 != nullptr) {
              square0.setVertLight(
                  int1,
                  (int)(float0 * 255.0F) << 0 | (int)(float1 * 255.0F) << 8 |
                      (int)(float2 * 255.0F) << 16 | 0xFF000000,
                  int0);
              square0.setVertLight(
                  int1 + 4,
                  (int)(float0 * 255.0F) << 0 | (int)(float1 * 255.0F) << 8 |
        ...
        static IsoCell getInstance() { return instance; }

        void render() {
          IsoCell.s_performance.isoCellRender.invokeAndMeasure(
              this, IsoCell::renderInternal);
        }
        ...
        void renderInternal() {
          ...
          if (!PerformanceSettings.NewRoofHiding) {
            IsoGridSquare square0 =
                IsoCamera.CamCharacter == null
                    ? null
                    : IsoCamera.CamCharacter.getCurrentSquare();
            if (square0 != nullptr) {
              IsoGridSquare square1 = this.getGridSquare(
                  (double)Math.round(IsoCamera.CamCharacter.getX()),
                  (double)Math.round(IsoCamera.CamCharacter.getY()),
                  (double)IsoCamera.CamCharacter.getZ());
              if (square1 != nullptr && this.IsBehindStuff(square1)) {
                this.bHideFloors[int0] = true;
              }

              if (!this.bHideFloors[int0] &&
                      square0.getProperties().Is(IsoFlagType.hidewalls) ||
            ...
            if (IsoCamera.CamCharacter != nullptr &&
              ...
              if (this.nearestVisibleZombie[int0] != nullptr) {
                if (this.nearestVisibleZombie[int0].isDead()) {
                  ...
                  float float4 = this.nearestVisibleZombie[int0].x -
                                 IsoCamera.CamCharacter.x;
                  float float5 = this.nearestVisibleZombie[int0].y -
                                 IsoCamera.CamCharacter.y;
                  this.nearestVisibleZombieDistSqr[int0] =
                      float4 * float4 + float5 * float5;
                }
              }
              ...
              for (int int7 = 0; int7 < int6; this.zombieScanCursor++) {
                ...
                if (zombie0 != nullptr) {
                  ...
                  if (square4 != nullptr && square3.z == square4.z &&
                      square4.getCanSee(int0)) {
                    if (this.nearestVisibleZombie[int0] == nullptr) {
                      this.nearestVisibleZombie[int0] = zombie0;
                      float float6 = this.nearestVisibleZombie[int0].x -
                                     IsoCamera.CamCharacter.x;
                      float float7 = this.nearestVisibleZombie[int0].y -
                                     IsoCamera.CamCharacter.y;
                      this.nearestVisibleZombieDistSqr[int0] =
                          float6 * float6 + float7 * float7;
                    } else {
              ...
              for (int int8 = 0; int8 < 4; int8++) {
                ...
                if (player1 != nullptr &&
                    player1.getCurrentSquare() != nullptr) {
                  ...
                  if (this.lastPlayerAngle[int8] == nullptr) {
                    this.lastPlayerAngle[int8] =
                        new Vector2(player1.getForwardDirection());
                    this.playerCutawaysDirty[int8] = true;
                  } else if (player1.getForwardDirection().dot(
                                 this.lastPlayerAngle[int8]) < 0.98F) {
                    this.lastPlayerAngle[int8].set(
                        player1.getForwardDirection());
                    this.playerCutawaysDirty[int8] = true;
                  }

                  IsoDirections directions0 =
                      IsoDirections.fromAngle(player1.getForwardDirection());
                  if (this.lastPlayerSquare[int8] != square5 ||
                    ...
                    this.GetBuildingsInFrontOfCharacter(
                        this.playerOccluderBuildings.get(int8), square5,
                        boolean0);
                    if (this.playerOccluderBuildingsArr[int0] == nullptr) {
                      this.playerOccluderBuildingsArr[int0] =
                          new IsoBuilding[500];
                    }

                    this.playerHidesOrphanStructures[int8] =
                        this.bOccludedByOrphanStructureFlag;
                    if (building0 == nullptr && !player1.bRemote) {
                      building0 =
                          this.GetPeekedInBuilding(square5, directions0);
                      if (building0 != nullptr) {
                        this.playerWindowPeekingRoomId[int8] =
                            this.playerPeekedRoomId;
                      }
                    }

                    if (building0 != nullptr) {
                      this.AddUniqueToBuildingList(
                          this.playerOccluderBuildings.get(int8), building0);
                    }

                    std::vector arrayList0 =
                        this.playerOccluderBuildings.get(int8);
                    ...
                    this.playerOccluderBuildingsArr[int0][arrayList0.size()] =
                        nullptr;
                  }
                  ...
                  if (int8 == int0 && square2 != nullptr) {
                    ...
                    this.GetSquaresAroundPlayerSquare(
                        player1, square2, this.gridSquaresTempLeft,
                        this.gridSquaresTempRight);

                    for (int int10 = 0; int10 < this.gridSquaresTempLeft.size();
                         int10++) {
                      IsoGridSquare square6 =
                          this.gridSquaresTempLeft.get(int10);
                      if (square6.getCanSee(int0) &&
                          (square6.getBuilding() == nullptr ||
                           square6.getBuilding() == square2.getBuilding())) {
                        std::vector arrayList1 =
                            this.GetBuildingsInFrontOfMustSeeSquare(
                                square6,
                                IsoGridOcclusionData.OcclusionFilter.Right);

                        for (int int11 = 0; int11 < arrayList1.size();
                             int11++) {
                          this.AddUniqueToBuildingList(
                              this.otherOccluderBuildings.get(int0),
                              (IsoBuilding)arrayList1.get(int11));
                        }

                        this.playerHidesOrphanStructures[int0] =
                            this.playerHidesOrphanStructures[int0] |
                            this.bOccludedByOrphanStructureFlag;
                      }
                    }
                    ...
                    for (int int12 = 0;
                         int12 < this.gridSquaresTempRight.size(); int12++) {
                      IsoGridSquare square7 =
                          this.gridSquaresTempRight.get(int12);
                      if (square7.getCanSee(int0) &&
                          (square7.getBuilding() == nullptr ||
                           square7.getBuilding() == square2.getBuilding())) {
                        std::vector arrayList2 =
                            this.GetBuildingsInFrontOfMustSeeSquare(
                                square7,
                                IsoGridOcclusionData.OcclusionFilter.Left);

                        for (int int13 = 0; int13 < arrayList2.size();
                             int13++) {
                          this.AddUniqueToBuildingList(
                              this.otherOccluderBuildings.get(int0),
                              (IsoBuilding)arrayList2.get(int13));
                        }
                    ...
                    std::vector arrayList3 =
                        this.otherOccluderBuildings.get(int0);
                    if (this.otherOccluderBuildingsArr[int0] == nullptr) {
                      this.otherOccluderBuildingsArr[int0] =
                          new IsoBuilding[500];
                    }

                    for (int int14 = 0; int14 < arrayList3.size(); int14++) {
                      IsoBuilding building2 =
                          (IsoBuilding)arrayList3.get(int14);
                      this.otherOccluderBuildingsArr[int0][int14] = building2;
                    }

                    this.otherOccluderBuildingsArr[int0][arrayList3.size()] =
                        nullptr;
                  }

                  if (this.playerHidesOrphanStructures[int8] &&
              ...
              bool boolean1 = false;
              if (this.nearestVisibleZombie[int0] != nullptr &&
                  this.nearestVisibleZombieDistSqr[int0] < 150.0F) {
                IsoGridSquare square8 =
                    this.nearestVisibleZombie[int0].getCurrentSquare();
                if (square8 != nullptr && square8.getCanSee(int0)) {
                  double double2 =
                      this.nearestVisibleZombie[int0].x -
                      Math.floor(this.nearestVisibleZombie[int0].x);
                  double double3 =
                      this.nearestVisibleZombie[int0].y -
                      Math.floor(this.nearestVisibleZombie[int0].y);
                  bool boolean2 = double2 > double3;
                  boolean1 = true;
                  if (this.lastZombieSquare[int0] != square8 ||
                      this.lastZombieSquareHalf[int0] != boolean2) {
                    this.lastZombieSquare[int0] = square8;
                    this.lastZombieSquareHalf[int0] = boolean2;
                    this.GetBuildingsInFrontOfCharacter(
                        this.zombieOccluderBuildings.get(int0), square8,
                        boolean2);
                    std::vector arrayList4 =
                        this.zombieOccluderBuildings.get(int0);
                    if (this.zombieOccluderBuildingsArr[int0] == nullptr) {
                      this.zombieOccluderBuildingsArr[int0] =
                          new IsoBuilding[500];
                    }

                    for (int int15 = 0; int15 < arrayList4.size(); int15++) {
                      IsoBuilding building3 =
                          (IsoBuilding)arrayList4.get(int15);
                      this.zombieOccluderBuildingsArr[int0][int15] = building3;
                    }

                    this.zombieOccluderBuildingsArr[int0][arrayList4.size()] =
                        nullptr;
                  }
                }
              }
              ...
              if (player2 != nullptr) {
                ...
                if (building4 == nullptr) {
                  IsoDirections directions1 =
                      IsoDirections.fromAngle(player2.getForwardDirection());
                  building4 = this.GetPeekedInBuilding(
                      player2.getCurrentSquare(), directions1);
                  if (building4 != nullptr) {
                    this.playerWindowPeekingRoomId[int17] =
                        this.playerPeekedRoomId;
                  }
                }
              }
          ...
          if (IsoCamera.CamCharacter != nullptr &&
              IsoCamera.CamCharacter.getCurrentSquare() != nullptr &&
              IsoCamera.CamCharacter.getCurrentSquare().getProperties().Is(
                  IsoFlagType.hidewalls)) {
            this.maxZ = (int)IsoCamera.CamCharacter.getZ() + 1;
          }
          ...
          for (int int19 = 0; int19 < this.StaticUpdaterObjectList.size();
               int19++) {
        ...
        IsoWeatherFX getWeatherFX() { return this.weatherFX; }

        void renderRain() {}

        void setRainAlpha(int alpha) { this.rainAlphaMax = alpha / 100.0F; }

        void setRainIntensity(int intensity) { this.rainIntensity = intensity; }

        void setRainSpeed(int speed) { this.rainSpeed = speed; }

        void reloadRainTextures() {}

        void GetBuildingsInFrontOfCharacter(ArrayList<IsoBuilding> arrayList,
                                            IsoGridSquare square,
                                            bool boolean0) {
          arrayList.clear();
          ...
          if (square != nullptr) {
            ...
            int int1 = square.getY();
            int int2 = square.getZ();
            this.GetBuildingsInFrontOfCharacterSquare(int0, int1, int2,
                                                      boolean0, arrayList);
            if (int2 < MaxHeight) {
              this.GetBuildingsInFrontOfCharacterSquare(
                  int0 - 1 + 3, int1 - 1 + 3, int2 + 1, boolean0, arrayList);
              this.GetBuildingsInFrontOfCharacterSquare(
                  int0 - 2 + 3, int1 - 2 + 3, int2 + 1, boolean0, arrayList);
              if (boolean0) {
                this.GetBuildingsInFrontOfCharacterSquare(
                    int0 + 3, int1 - 1 + 3, int2 + 1, !boolean0, arrayList);
                this.GetBuildingsInFrontOfCharacterSquare(
                    int0 - 1 + 3, int1 - 2 + 3, int2 + 1, !boolean0, arrayList);
              } else {
                this.GetBuildingsInFrontOfCharacterSquare(
                    int0 - 1 + 3, int1 + 3, int2 + 1, !boolean0, arrayList);
                this.GetBuildingsInFrontOfCharacterSquare(
                    int0 - 2 + 3, int1 - 1 + 3, int2 + 1, !boolean0, arrayList);
              }
            }
          }
        }

        void GetBuildingsInFrontOfCharacterSquare(
          ...
          if (square == nullptr) {
            if (int2 < MaxHeight) {
              this.GetBuildingsInFrontOfCharacterSquare(
                  int0 + 3, int1 + 3, int2 + 1, boolean0, arrayList0);
            }
          } else {
            IsoGridOcclusionData gridOcclusionData =
                square.getOrCreateOcclusionData();
            IsoGridOcclusionData.OcclusionFilter occlusionFilter =
                boolean0 ? IsoGridOcclusionData.OcclusionFilter.Right
                         : IsoGridOcclusionData.OcclusionFilter.Left;
            this.bOccludedByOrphanStructureFlag =
                this.bOccludedByOrphanStructureFlag |
                gridOcclusionData.getCouldBeOccludedByOrphanStructures(
                    occlusionFilter);
            std::vector arrayList1 =
                gridOcclusionData.getBuildingsCouldBeOccluders(occlusionFilter);

            for (int int3 = 0; int3 < arrayList1.size(); int3++) {
              this.AddUniqueToBuildingList(arrayList0,
                                           (IsoBuilding)arrayList1.get(int3));
            }
          }
        }

      private
        ArrayList<IsoBuilding> GetBuildingsInFrontOfMustSeeSquare(
            IsoGridSquare square,
            IsoGridOcclusionData.OcclusionFilter occlusionFilter) {
          IsoGridOcclusionData gridOcclusionData =
              square.getOrCreateOcclusionData();
          this.bOccludedByOrphanStructureFlag =
              gridOcclusionData.getCouldBeOccludedByOrphanStructures(
                  IsoGridOcclusionData.OcclusionFilter.All);
          return gridOcclusionData.getBuildingsCouldBeOccluders(
              occlusionFilter);
        }

        IsoBuilding GetPeekedInBuilding(IsoGridSquare square0,
          ...
          if (square0 == nullptr) {
            ...
            if ((directions == IsoDirections.NW ||
                ...
                LosUtil.lineClear(this, square0.x, square0.y, square0.z,
            ...
            if ((directions == IsoDirections.SW ||
                ...
                LosUtil.lineClear(this, square0.x, square0.y, square0.z,
                                  square0.x - 1, square0.y, square0.z,
                                  false) != LosUtil.TestResults.Blocked) {
        ...
        void GetSquaresAroundPlayerSquare(IsoPlayer player,
                                          IsoGridSquare square0,
                                          ArrayList<IsoGridSquare> arrayList0,
                                          ArrayList<IsoGridSquare> arrayList1) {
          ...
          for (int int3 = int1; int3 < int1 + 10; int3++) {
            for (int int4 = int0; int4 < int0 + 10; int4++) {
              if ((int4 >= (int)player.x || int3 >= (int)player.y) &&
                  (int4 != (int)player.x || int3 != (int)player.y)) {
        ...
        bool IsBehindStuff(IsoGridSquare square0) {
          if (!square0.getProperties().Is(IsoFlagType.exterior)) {
            ...
            for (int int0 = 1; int0 < 8 && square0.getZ() + int0 < MaxHeight;
                 int0++) {
              for (int int1 = -5; int1 <= 6; int1++) {
                for (int int2 = -5; int2 <= 6; int2++) {
                  if (int2 >= int1 - 5 && int2 <= int1 + 5) {
                    IsoGridSquare square1 =
                        this.getGridSquare(square0.getX() + int2 + int0 * 3,
                                           square0.getY() + int1 + int0 * 3,
                                           square0.getZ() + int0);
                    if (square1 != nullptr && !square1.getObjects().isEmpty()) {
                      ...
                      if (object.sprite == nullptr ||
                          ...
                          !object.sprite.name.startsWith("lighting_outdoor")) {
                        return true;
                      }
                    }
                  }
        ...
        void update() {
          IsoCell.s_performance.isoCellUpdate.invokeAndMeasure(
              this, IsoCell::updateInternal);
        }

        void updateInternal() {
          MovingObjectUpdateScheduler.instance.startFrame();
          IsoSprite.alphaStep =
              0.075F * (GameTime.getInstance().getMultiplier() / 1.6F);
            ...
            for (int int0 = 0; int0 < IsoPlayer.numPlayers; int0++) {
              if (IsoPlayer.players[int0] != nullptr &&
                  (!IsoPlayer.players[int0].isDead() ||
                   IsoPlayer.players[int0].ReanimatedCorpse != nullptr)) {
          ...
          if (GameClient.bClient &&
              (NetworkZombieSimulator.getInstance().anyUnknownZombies() &&
                   GameClient.instance.sendZombieRequestsTimer.Check() ||
          ...
          IsoGridSquare.setRecalcLightTime(IsoGridSquare.getRecalcLightTime() -
                                           1);
          if (GameServer.bServer) {
            this.LamppostPositions.clear();
          ...
          if (!GameTime.isGamePaused()) {
            this.rainScroll = this.rainScroll +
                              this.rainSpeed / 10.0F * 0.075F *
                                  (30.0F / PerformanceSettings.getLockFPS());
        ...
        IsoGridSquare getRandomFreeTile() {
          ...
          do {
            boolean0 = true;
            square = this.getGridSquare(Rand.Next(this.width),
                                        Rand.Next(this.height), 0);
            if (square == nullptr) {
              boolean0 = false;
        ...
        IsoGridSquare getRandomFreeTileInRoom() {
          ...
          for (int int0 = 0; int0 < this.RoomList.size(); int0++) {
            if (this.RoomList.get(int0).TileList.size() > 9 &&
                ...
                this.RoomList.get(int0).TileList.get(0).getProperties().Is(
          ...
          synchronized(this.SpottedRooms) {
            for (int int0 = 0; int0 < this.SpottedRooms.size(); int0++) {
              ...
              if (!room.def.bDoneSpawn) {
                ...
                if (!GameClient.bClient && !Core.bLastStand &&
                        ("shed" == room.def.name) ||
                    "garagestorage" == room.def.name) || "storageunit" == room.def.name))) {
                    uint8_t byte0 = 7;
                    ...
                    switch (
                        SandboxOptions.instance.GeneratorSpawning.getValue()) {
                    case 1:
                      byte0 += 3;
                      break;
                    case 2:
                    ...
                    if (Rand.Next((int)byte0) == 0) {
                      ...
                      if (square != nullptr) {
                        IsoGenerator generator = new IsoGenerator(
                            InventoryItemFactory.CreateItem("Base.Generator"),
                            this, square);
                        if (GameServer.bServer) {
        ...
        bool LoadPlayer(int WorldVersion) {
          if (GameClient.bClient) {
            ...
            File file =
                ZomboidFileSystem.instance.getFileInCurrentSave("map_p.bin");
            if (!file.exists()) {
              ...
              synchronized(SliceY.SliceBufferLock) {
                ...
                instance.ChunkMap[IsoPlayer.getPlayerIndex()].WorldX =
                    SliceY.SliceBuffer.getInt() + IsoWorld.saveoffsetx * 30;
                instance.ChunkMap[IsoPlayer.getPlayerIndex()].WorldY =
                    SliceY.SliceBuffer.getInt() + IsoWorld.saveoffsety * 30;
        ...
        IsoGridSquare getGridSquareDirect(int x, int y, int z,
                                          int playerIndex) {
          int int0 = IsoChunkMap.ChunkWidthInTiles;
      ...
      public
        ArrayList<IsoObject> getProcessIsoObjectRemove() {
        ...
        void addHeatSource(IsoHeatSource heatSource) {
          if (!GameServer.bServer) {
            if (this.heatSources.contains(heatSource)) {
              DebugLog.log(
                  "ERROR addHeatSource called again with the same HeatSource");
            } else {
        ...
        int getHeatSourceTemperature(int x, int y, int z) {
          ...
          for (int int1 = 0; int1 < this.heatSources.size(); int1++) {
            ...
            if (heatSource.getZ() == z) {
              float float0 = IsoUtils.DistanceToSquared(x, y, heatSource.getX(),
                                                        heatSource.getY());
              if (float0 < heatSource.getRadius() * heatSource.getRadius()) {
                LosUtil.TestResults testResults = LosUtil.lineClear(
                    this, heatSource.getX(), heatSource.getY(),
                    heatSource.getZ(), x, y, z, false);
                if (testResults == LosUtil.TestResults.Clear ||
                    testResults == LosUtil.TestResults.ClearThroughOpenDoor) {
                  int0 = (int)(int0 + heatSource.getTemperature() *
                                          (1.0 - Math.sqrt(float0) /
                                                     heatSource.getRadius()));
                }
              }
            }
          }
        ...
        float getHeatSourceHighestTemperature(float surroundingAirTemperature,
                                              int x, int y, int z) {
          ...
          for (int int0 = 0; int0 < this.heatSources.size(); int0++) {
            IsoHeatSource heatSource = this.heatSources.get(int0);
            if (heatSource.getZ() == z) {
              float float4 = IsoUtils.DistanceToSquared(x, y, heatSource.getX(),
                                                        heatSource.getY());
              object = this.getGridSquare(heatSource.getX(), heatSource.getY(),
                                          heatSource.getZ());
              float3 = 0.0F;
              if (object != nullptr) {
              ...
              float2 = ClimateManager.lerp(
                  (float)(1.0 - Math.sqrt(float4) / heatSource.getRadius()),
                  float0, heatSource.getTemperature() + float3);
              if (!(float2 <= float1) &&
                  float4 < heatSource.getRadius() * heatSource.getRadius()) {
                LosUtil.TestResults testResults = LosUtil.lineClear(
                    this, heatSource.getX(), heatSource.getY(),
                    heatSource.getZ(), x, y, z, false);
                if (testResults == LosUtil.TestResults.Clear ||
          ...
          if (chr != nullptr && chr.savedVehicleSeat != -1) {
            ...
            for (int int4 = int1; int4 <= int3; int4++) {
              for (int int5 = int0; int5 <= int2; int5++) {
                IsoChunk chunk = this.getChunkForGridSquare(
                    int5 * 10, int4 * 10, (int)chr.getZ());
                if (chunk != nullptr) {
                  for (int int6 = 0; int6 < chunk.vehicles.size(); int6++) {
                    ...
                    if ((int)vehicle.getZ() == (int)chr.getZ() &&
                        IsoUtils.DistanceToSquared(
                            vehicle.getX(), vehicle.getY(), chr.savedVehicleX,
                            chr.savedVehicleY) < 0.010000001F) {
                      if (vehicle.VehicleID == -1) {
                        return;
                      }

                      VehicleScript.Position position =
                          vehicle.getPassengerPosition(chr.savedVehicleSeat,
                                                       "inside");
                      if (position != nullptr &&
                          !vehicle.isSeatOccupied(chr.savedVehicleSeat)) {
                        vehicle.enter(chr.savedVehicleSeat, chr,
                                      position.offset);
                        LuaEventManager.triggerEvent("OnEnterVehicle", chr);
                        if (vehicle.getCharacter(chr.savedVehicleSeat) == chr &&
                            chr.savedVehicleRunning) {
                ...
                if (chunk != nullptr) {
                  for (int int6 = 0; int6 < chunk.vehicles.size(); int6++) {
                    ...
                    if (vehicle.lightbarSirenMode.isEnable()) {
                      vehicle.setLightbarSirenMode(
                          vehicle.lightbarSirenMode.get());
                    }
                  }
                }
              }
        ...
        void AddUniqueToBuildingList(ArrayList<IsoBuilding> arrayList,
      ...
      public
        static enum BuildingSearchCriteria {
          Food, Defense, Wood, Weapons, General;
        }

        public static final class PerPlayerRender {
          const IsoGridStack GridStacks = new IsoGridStack(9);
        public
          boolean[][][] VisiOccludedFlags;
        public
          boolean[][] VisiCulledFlags;
        public
          short[][][] StencilValues;
        public
          boolean[][] FlattenGrassEtc;
          int minX;
          int minY;
          int maxX;
          int maxY;
          ...
          void setSize(int w, int h) {
            if (this.VisiOccludedFlags == nullptr ||
                this.VisiOccludedFlags.length < w ||
                this.VisiOccludedFlags[0].length < h) {
        ...
        public
          final Texture[][][] grid = new Texture[this.w][this.h][2];
        public
          final byte[][][] gridType = new byte[this.w][this.h][2];

        public
          SnowGrid(int arg1) { this.init(arg1); }

        public
          IsoCell.SnowGrid init(int arg0) {
            if (!IsoCell.this.hasSetupSnowGrid) {
              ...
              uint8_t byte0 = 0;
              IsoCell.this.snowGridTiles_Square =
                  IsoCell.this.new SnowGridTiles(byte0++);
              uint8_t byte1 = 40;

              for (int int0 = 0; int0 < 4; int0++) {
                IsoCell.this.snowGridTiles_Square.add(Texture.getSharedTexture(
                    "e_newsnow_ground_1_" + (byte1 + int0)));
              }

              IsoCell.this.snowGridTiles_Enclosed =
                  IsoCell.this.new SnowGridTiles(byte0++);
              byte1 = 0;

              for (int int1 = 0; int1 < 4; int1++) {
                IsoCell.this.snowGridTiles_Enclosed.add(
                    Texture.getSharedTexture("e_newsnow_ground_1_" +
                                             (byte1 + int1)));
              }
              ...
              for (int int2 = 0; int2 < 4; int2++) {
                IsoCell.this.snowGridTiles_Cove[int2] =
                    IsoCell.this.new SnowGridTiles(byte0++);
                if (int2 == 0) {
                  byte1 = 7;
                }
                ...
                for (int int3 = 0; int3 < 3; int3++) {
                  IsoCell.this.snowGridTiles_Cove[int2].add(
                      Texture.getSharedTexture("e_newsnow_ground_1_" +
                                               (byte1 + int3 * 4)));
                }
              }
              ...
              for (int int4 = 0; int4 < 4; int4++) {
                IsoCell.this.snowGridTiles_Edge[int4] =
                    IsoCell.this.new SnowGridTiles(byte0++);
                if (int4 == 0) {
                  byte1 = 16;
                }
                ...
                for (int int5 = 0; int5 < 3; int5++) {
                  IsoCell.this.snowGridTiles_Edge[int4].add(
                      Texture.getSharedTexture("e_newsnow_ground_1_" +
                                               (byte1 + int5 * 4)));
                }
              }
              ...
              for (int int6 = 0; int6 < 4; int6++) {
                IsoCell.this.snowGridTiles_Strip[int6] =
                    IsoCell.this.new SnowGridTiles(byte0++);
                if (int6 == 0) {
                  byte1 = 28;
                }
                ...
                for (int int7 = 0; int7 < 3; int7++) {
                  IsoCell.this.snowGridTiles_Strip[int6].add(
                      Texture.getSharedTexture("e_newsnow_ground_1_" +
                                               (byte1 + int7 * 4)));
                }
              }
            ...
            for (int int9 = 0; int9 < this.h; int9++) {
              for (int int10 = 0; int10 < this.w; int10++) {
                ...
                if (noise2D.layeredNoise(int10 / 10.0F, int9 / 10.0F) <=
                    arg0 / 100.0F) {
                  this.grid[int10][int9][0] =
                      IsoCell.this.snowGridTiles_Square.getNext();
                  this.gridType[int10][int9][0] =
                      IsoCell.this.snowGridTiles_Square.ID;
                }
              }
            }
              ...
              for (int int13 = 0; int13 < this.w; int13++) {
                ...
                if (texture == nullptr) {
                  ...
                  if (int14 != 0) {
                    if (int14 == 1) {
                      if (boolean0) {
                        this.set(int13, int12, 0,
                                 IsoCell.this.snowGridTiles_Strip[0]);
                      } else if (boolean1) {
                        this.set(int13, int12, 0,
                                 IsoCell.this.snowGridTiles_Strip[1]);
                      } else if (boolean3) {
                        this.set(int13, int12, 0,
                                 IsoCell.this.snowGridTiles_Strip[3]);
                      } else if (boolean2) {
                        this.set(int13, int12, 0,
                                 IsoCell.this.snowGridTiles_Strip[2]);
                      }
                    } else if (int14 == 2) {
                      if (boolean0 && boolean1) {
                        this.set(int13, int12, 0,
                                 IsoCell.this.snowGridTiles_Strip[0]);
                        this.set(int13, int12, 1,
                                 IsoCell.this.snowGridTiles_Strip[1]);
                        ...
                        this.set(
                            int13, int12, 0,
                            IsoCell.this.snowGridTiles_Edge[boolean2 ? 0 : 3]);
                      } else if (boolean1) {
                        this.set(
                            int13, int12, 0,
                            IsoCell.this.snowGridTiles_Edge[boolean2 ? 2 : 1]);
                      } else if (boolean2) {
                        this.set(
                            int13, int12, 0,
                            IsoCell.this.snowGridTiles_Edge[boolean0 ? 0 : 2]);
                      } else if (boolean3) {
                        this.set(
                            int13, int12, 0,
                            IsoCell.this.snowGridTiles_Edge[boolean0 ? 3 : 1]);
                      }
                    } else if (int14 == 3) {
                      if (!boolean0) {
                        this.set(int13, int12, 0,
                                 IsoCell.this.snowGridTiles_Cove[1]);
                      } else if (!boolean1) {
                        this.set(int13, int12, 0,
                                 IsoCell.this.snowGridTiles_Cove[0]);
                      } else if (!boolean3) {
                        this.set(int13, int12, 0,
                                 IsoCell.this.snowGridTiles_Cove[2]);
                      } else if (!boolean2) {
                        this.set(int13, int12, 0,
                                 IsoCell.this.snowGridTiles_Cove[3]);
                      }
                    } else if (int14 == 4) {
                      this.set(int13, int12, 0,
                               IsoCell.this.snowGridTiles_Enclosed);
          ...
          bool checkAny(int arg0, int arg1) {
            ...
            if (arg0 < 0 || arg0 >= this.w) {
              ...
              return arg1 >= 0 && arg1 < this.h
                         ? this.grid[arg0][arg1][0] != nullptr
                         : false;
            }
          }
            ...
            for (int int0 = 0; int0 < this.h; int0++) {
              for (int int1 = 0; int1 < this.w; int1++) {
                for (int int2 = 0; int2 < 2; int2++) {
                  if (arg0.gridType[int1][int0][int2] ==
                      this.gridType[int1][int0][int2]) {
        ...
        protected class SnowGridTiles {
          ...
          int counter = -1;
        private
          final ArrayList<Texture> textures = std::make_unique<ArrayList<>>();

        public
          SnowGridTiles(uint8_t arg1) { this.ID = arg1; }

          void add(Texture texture) { this.textures.add(texture); }
          ...
          Texture get(int int0) { return this.textures.get(int0); }

          int size() { return this.textures.size(); }

          Texture getRand() { return this.textures.get(Rand.Next(4)); }
        ...
        private static class s_performance {
          static const PerformanceProfileProbe isoCellUpdate =
              new PerformanceProfileProbe("IsoCell.update");
          static const PerformanceProfileProbe isoCellRender =
              new PerformanceProfileProbe("IsoCell.render");
          static const PerformanceProfileProbe isoCellRenderTiles =
              new PerformanceProfileProbe("IsoCell.renderTiles");
          static const PerformanceProfileProbe isoCellDoBuilding =
              new PerformanceProfileProbe("IsoCell.doBuilding");

          static class renderTiles {
            static const PerformanceProfileProbe performRenderTiles =
                new PerformanceProfileProbe("performRenderTiles");
            static const PerformanceProfileProbe recalculateAnyGridStacks =
                new PerformanceProfileProbe("recalculateAnyGridStacks");
            static const PerformanceProfileProbe flattenAnyFoliage =
                new PerformanceProfileProbe("flattenAnyFoliage");
            static const PerformanceProfileProbe renderDebugPhysics =
                new PerformanceProfileProbe("renderDebugPhysics");
            static const PerformanceProfileProbe renderDebugLighting =
                new PerformanceProfileProbe("renderDebugLighting");
            static PerformanceProfileProbeList<
                IsoCell.s_performance.renderTiles.PperformRenderTilesLayer>
                performRenderTilesLayers =
                    PerformanceProfileProbeList.construct(
                        "performRenderTiles", 8,
                        IsoCell.s_performance.renderTiles
                            .PperformRenderTilesLayer.class,
                        IsoCell.s_performance.renderTiles
                            .PperformRenderTilesLayer::new);

            static class PperformRenderTilesLayer extends
                PerformanceProfileProbe {
              const PerformanceProfileProbe renderIsoWater =
                  new PerformanceProfileProbe("renderIsoWater");
              const PerformanceProfileProbe renderFloor =
                  new PerformanceProfileProbe("renderFloor");
              const PerformanceProfileProbe renderPuddles =
                  new PerformanceProfileProbe("renderPuddles");
              const PerformanceProfileProbe renderShore =
                  new PerformanceProfileProbe("renderShore");
              const PerformanceProfileProbe renderSnow =
                  new PerformanceProfileProbe("renderSnow");
              const PerformanceProfileProbe renderBlood =
                  new PerformanceProfileProbe("renderBlood");
              const PerformanceProfileProbe vegetationCorpses =
                  new PerformanceProfileProbe("vegetationCorpses");
              const PerformanceProfileProbe renderFloorShading =
                  new PerformanceProfileProbe("renderFloorShading");
              const PerformanceProfileProbe renderShadows =
                  new PerformanceProfileProbe("renderShadows");
              const PerformanceProfileProbe luaOnPostFloorLayerDraw =
                  new PerformanceProfileProbe("luaOnPostFloorLayerDraw");
              const PerformanceProfileProbe minusFloorCharacters =
                  new PerformanceProfileProbe("minusFloorCharacters");

              PperformRenderTilesLayer(String string) { super(string); }
            }
          }
        }
}
