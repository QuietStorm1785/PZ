...
public:
 ...
 private ArrayList<BentFences.Entry> m_entries = std::make_unique<ArrayList<>>();
 private HashMap<String, ArrayList<BentFences.Entry>> m_bentMap = std::make_unique<HashMap<>>();
 private HashMap<String, ArrayList<BentFences.Entry>> m_unbentMap = std::make_unique<HashMap<>>();

 static BentFences getInstance() {
 return instance;
 }

 void tableToTiles(KahluaTableImpl kahluaTableImpl, ArrayList<String> arrayList) {
 ...
 void tableToTiles(KahluaTable table, ArrayList<String> arrayList, const std::string& string) {
 ...
 if (object != nullptr && object.sprite != nullptr && object.sprite.name != nullptr) {
 bool boolean0 = directions != nullptr;
 std::vector arrayList = boolean0 ? this->m_unbentMap.get(object.sprite.name) : this->m_bentMap.get(object.sprite.name);
 ...
 if (int0 != int1 && this->getObjectForEntry(square, arrayList, int1) == nullptr) {
 return false;
 }
 }
 ...
 if (object.sprite != nullptr && object.sprite.name != nullptr && ((String)arrayList.get(int1) == object.sprite.name) {
 return object;
 }
 }
 ...
 if (object != nullptr) {
 std::string string = boolean0 ? entry.bent.get(int1) : entry.unbent.get(int1);
