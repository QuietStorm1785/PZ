...
public
  PlayerCamera(int _playerIndex) { this.playerIndex = _playerIndex; }
  ...
  void center() {
    ...
    if (IsoCamera.CamCharacter != nullptr) {
      ...
      float0 = IsoUtils.XToScreen(character.x + this.DeferedX,
                                  character.y + this.DeferedY, character.z, 0);
      float1 = IsoUtils.YToScreen(character.x + this.DeferedX,
                                  character.y + this.DeferedY, character.z, 0);
  ...
  void update() {
    ...
    PAN_SPEED = 110.0F;
    float float2 = this.panTime < 0L
                       ? 1.0F
                       : (float)(long0 - this.panTime) / 1000.0F * PAN_SPEED;
    float2 = 1.0F / float2;
    this.panTime = long0;
    IsoPlayer player = IsoPlayer.players[this.playerIndex];
    bool boolean0 = GameWindow.ActivatedJoyPad != null && player != null &&
                    player.JoypadBind != -1;
    ...
    if (Core.getInstance().getOptionPanCameraWhileDriving() &&
        vehicle != nullptr && vehicle.getCurrentSpeedKmHour() > 1.0F) {
      float float3 = Core.getInstance().getZoom(this.playerIndex);
      float float4 = vehicle.getCurrentSpeedKmHour() *
                     BaseVehicle.getFakeSpeedModifier() / 10.0F;
      float4 *= float3;
      Vector3f vector3f =
          vehicle.getForwardVector(BaseVehicle.TL_vector3f_pool.get().alloc());
      float float5 = this.m_lastVehicleForwardDirection.angle(vector3f) *
      ...
      this.RightClickTargetX = (int)IsoUtils.XToScreen(
          vector3f.x * float4, vector3f.z * float4, player.z, 0);
      this.RightClickTargetY = (int)IsoUtils.YToScreen(
          vector3f.x * float4, vector3f.z * float4, player.z, 0);
      ...
      this.RightClickTargetX =
          (int)PZMath.clamp(float7 + this.RightClickTargetX, float9,
                            int0 - float9) -
          float7;
      this.RightClickTargetY =
          (int)PZMath.clamp(float8 + this.RightClickTargetY, float9,
                            int1 - float9) -
          float8;
      if (Math.abs(float4) < 5.0F) {
        ...
        float2 /= 0.5F * float3;
        float float11 =
            IsoUtils.XToScreenExact(player.x, player.y, player.z, 0);
        float float12 =
            IsoUtils.YToScreenExact(player.x, player.y, player.z, 0);
        if (float11 < float9 / 2.0F || float11 > int0 - float9 / 2.0F ||
            float12 < float9 / 2.0F || float12 > int1 - float9 / 2.0F) {
          float2 /= 4.0F;
        }

        this.RightClickX_f = PZMath.step(
            this.RightClickX_f, this.RightClickTargetX,
            1.875F * PZMath.sign(this.RightClickTargetX - this.RightClickX_f) /
                float2);
        this.RightClickY_f = PZMath.step(
            this.RightClickY_f, this.RightClickTargetY,
            1.875F * PZMath.sign(this.RightClickTargetY - this.RightClickY_f) /
                float2);
      ...
      if ((player.IsAiming() || player.isLookingWhileInVehicle()) &&
        ...
        this.RightClickTargetX =
            JoypadManager.instance.getAimingAxisX(player.JoypadBind) * 1500.0F;
        this.RightClickTargetY =
            JoypadManager.instance.getAimingAxisY(player.JoypadBind) * 1500.0F;
        float2 /= 0.5F * Core.getInstance().getZoom(this.playerIndex);
        this.RightClickX_f = PZMath.step(
            this.RightClickX_f, this.RightClickTargetX,
            (this.RightClickTargetX - this.RightClickX_f) / (80.0F * float2));
        this.RightClickY_f = PZMath.step(
            this.RightClickY_f, this.RightClickTargetY,
            (this.RightClickTargetY - this.RightClickY_f) / (80.0F * float2));
        this.RightClickX = (int)this.RightClickX_f;
        this.RightClickY = (int)this.RightClickY_f;
    ...
    } else if (this.playerIndex == 0 && player != nullptr &&
               !player.isBlockMovement() &&
               GameKeyboard.isKeyDown(Core.getInstance().getKey("PanCamera"))) {
      ...
      offVec.setLength(
          Math.min(offVec.getLength(), Math.min(int2, int3) / 2.0F));
      ...
      IsoSprite.globalOffsetX = -1.0F;
               ...
               Core.getInstance().getOptionPanCameraWhileAiming()) {
      bool boolean1 = !GameServer.bServer;
      bool boolean2 = !UIManager.isMouseOverInventory() && player != null &&
                      player.isAiming();
      bool boolean3 = !boolean0 && player != null && !player.isDead();
      if (boolean1 && boolean2 && boolean3) {
        ...
        if (offVec.getLength() < float20) {
          ...
          offVec.setLength(
              Math.min(offVec.getLength(), Math.min(int6, int7) / 2.0F) -
              float20);
  ...
  void returnToCenter(float float0) {
    ...
    if (this.RightClickTargetX != this.RightClickX ||
        this.RightClickTargetY != this.RightClickY) {
      this.RightClickX_f =
          PZMath.step(this.RightClickX_f, this.RightClickTargetX,
                      (this.RightClickTargetX - this.RightClickX_f) * float0);
      this.RightClickY_f =
          PZMath.step(this.RightClickY_f, this.RightClickTargetY,
                      (this.RightClickTargetY - this.RightClickY_f) * float0);
      this.RightClickX = (int)this.RightClickX_f;
      this.RightClickY = (int)this.RightClickY_f;
  ...
  float getOffX() { return (int)(this.OffX + this.RightClickX); }

  float getOffY() { return (int)(this.OffY + this.RightClickY); }
  ...
  float getLastOffX() { return (int)(this.lastOffX + this.RightClickX); }

  float getLastOffY() { return (int)(this.lastOffY + this.RightClickY); }
