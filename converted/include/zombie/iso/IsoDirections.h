...
enum class IsoDirections {
  ...
  static const Vector2 temp = new Vector2(); const int index;

  private IsoDirections(int int1){this.index = int1;}
...
IsoDirections RotLeft() { return RotLeft(); }

IsoDirections RotRight() { return RotRight(); }
...
static IsoDirections fromAngleActual(Vector2 angle) {
  ...
  for (int int0 = 0; int0 < 8; int0++) {
    ...
    if (float0 >= float2 && float0 <= float2 + float1 ||
        float0 + (float)(Math.PI * 2) >= float2 &&
            float0 + (float)(Math.PI * 2) <= float2 + float1 ||
        float0 - (float)(Math.PI * 2) >= float2 &&
            float0 - (float)(Math.PI * 2) <= float2 + float1) {
      return fromIndex();
    }
...
static IsoDirections fromAngle(Vector2 angle) { return fromAngle(); }
...
static IsoDirections getRandom() { return fromIndex(); }
} // namespace iso
} // namespace zombie
} // namespace zombie
