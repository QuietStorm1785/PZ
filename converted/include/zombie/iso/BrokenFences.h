...
public:
 ...
 private THashMap<String, BrokenFences.Tile> s_unbrokenMap = std::make_unique<THashMap<>>();
 private THashMap<String, BrokenFences.Tile> s_brokenLeftMap = std::make_unique<THashMap<>>();
 private THashMap<String, BrokenFences.Tile> s_brokenRightMap = std::make_unique<THashMap<>>();
 private THashMap<String, BrokenFences.Tile> s_allMap = std::make_unique<THashMap<>>();
 ...
 void damageAdjacent(IsoGridSquare square1, IsoDirections directions0, IsoDirections directions1) {
 IsoGridSquare square0 = square1.getAdjacentSquare(directions0);
 if (square0 != nullptr) {
 bool boolean0 = directions0 == IsoDirections.W || directions0 == IsoDirections.E;
 IsoObject object = this->getBreakableObject(square0, boolean0);
 if (object != nullptr) {
 bool boolean1 = directions0 == IsoDirections.N || directions0 == IsoDirections.E;
 bool boolean2 = directions0 == IsoDirections.S || directions0 == IsoDirections.W;
 ...
 return object != nullptr && object.sprite != nullptr && object.sprite.name != nullptr ? this->s_unbrokenMap.contains(object.sprite.name) : false;
 }

 bool isBrokenLeft(IsoObject object) {
 return object != nullptr && object.sprite != nullptr && object.sprite.name != nullptr ? this->s_brokenLeftMap.contains(object.sprite.name) : false;
 }

 bool isBrokenRight(IsoObject object) {
 return object != nullptr && object.sprite != nullptr && object.sprite.name != nullptr ? this->s_brokenRightMap.contains(object.sprite.name) : false;
 }

 bool isBreakableObject(IsoObject obj) {
 return obj != nullptr && obj.sprite != nullptr && obj.sprite.name != nullptr ? this->s_allMap.containsKey(obj.sprite.name) : false;
 }
 ...
 if (string != nullptr && square != nullptr && square.TreatAsSolidFloor()) {
