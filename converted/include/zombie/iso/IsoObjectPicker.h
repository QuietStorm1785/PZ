...
public:
  ...
  static final ArrayList<IsoObjectPicker.ClickObject> choices =
      std::make_unique<ArrayList<>>();
  static const Vector2 tempo = new Vector2();
  static const Vector2 tempo2 = new Vector2();
  static final Comparator<IsoObjectPicker.ClickObject> comp =
      new Comparator<IsoObjectPicker.ClickObject>(){
          int compare(IsoObjectPicker.ClickObject clickObject0,
  ...
  if (int0 > int1) {
    ...
    return clickObject0.tile != nullptr &&
               ...
               ? clickObject0.tile.getObjectIndex() -
                     clickObject1.tile.getObjectIndex()
               : 0;
  }
}
}; // namespace iso
...
IsoObjectPicker.ClickObject[] ClickObjectStore =
    new IsoObjectPicker.ClickObject[15000];
int count = 0;
int counter = 0;
int maxcount = 0;
public
final ArrayList<IsoObjectPicker.ClickObject> ThisFrame =
    std::make_unique<ArrayList<>>();
bool dirty = true;
...
IsoObjectPicker getInstance() { return Instance; }

void Add(int x, int y, int width, int height, IsoGridSquare gridSquare,
         IsoObject tile, bool flip, float scaleX, float scaleY) {
  if (!(x + width <= this.lx - 32.0F) && !(x >= this.lx + 32.0F) &&
      !(y + height <= this.ly - 32.0F) && !(y >= this.ly + 32.0F)) {
            ...
            if (x + width >= 0) {
              if (y + height >= 0) {
                IsoObjectPicker.ClickObject clickObject =
                    this.ClickObjectStore[this.ThisFrame.size()];
...
public
IsoObjectPicker.ClickObject ContextPick(int screenX, int screenY) {
  ...
  for (int int0 = this.ThisFrame.size() - 1; int0 >= 0; int0--) {
    ...
    if ((!(clickObject0.tile instanceof IsoPlayer) ||
         clickObject0.tile != IsoPlayer.players[0]) &&
        (clickObject0.tile.sprite == nullptr ||
         clickObject0.tile.getTargetAlpha(0) != 0.0F &&
             (!clickObject0.tile.sprite.Properties.Is(IsoFlagType.cutW) &&
                      !clickObject0.tile.sprite.Properties.Is(
                          IsoFlagType.cutN) ||
                  clickObject0.tile instanceof
              IsoWindow || !(clickObject0.tile.getTargetAlpha(0) < 1.0F)))) {
      if (clickObject0.tile != nullptr && clickObject0.tile.sprite != nullptr) {
      }

      if (float0 > clickObject0.x && float1 > clickObject0.y &&
        ...
        if (clickObject0.tile instanceof IsoPlayer) {
          if (clickObject0.tile.sprite == nullptr ||
              ...
              clickObject0.tile.sprite.def.Frame >=
                  clickObject0.tile.sprite.CurrentAnim.Frames.size()) {
            continue;
          }

          int int1 = (int)(float0 - clickObject0.x);
          int int2 = (int)(float1 - clickObject0.y);
          Texture texture = clickObject0.tile.sprite.CurrentAnim.Frames
                                .get((int)clickObject0.tile.sprite.def.Frame)
                                .directions[clickObject0.tile.dir.index()];
          ...
          if (int1 >= 0 && int2 >= 0 && int1 <= texture.getWidth() * int3 &&
              int2 <= texture.getHeight() * int3) {
        ...
        if (clickObject0.scaleX == 1.0F && clickObject0.scaleY == 1.0F) {
          if (clickObject0.tile.isMaskClicked((int)(float0 - clickObject0.x),
                                              (int)(float1 - clickObject0.y),
                                              clickObject0.flip)) {
          ...
          float float2 =
              clickObject0.x + (float0 - clickObject0.x) / clickObject0.scaleX;
          float float3 =
              clickObject0.y + (float1 - clickObject0.y) / clickObject0.scaleY;
          if (clickObject0.tile.isMaskClicked((int)(float2 - clickObject0.x),
                                              (int)(float3 - clickObject0.y),
                                              clickObject0.flip)) {
            if (clickObject0.tile.rerouteMask != nullptr) {
              clickObject0.tile = clickObject0.tile.rerouteMask;
            }
...
public
IsoObjectPicker.ClickObject Pick(int xx, int yy) {
  float float0 = xx;
  float float1 = yy;
  ...
  for (int int0 = this.ThisFrame.size() - 1; int0 >= 0; int0--) {
    ...
    if (!(clickObject.tile instanceof IsoPlayer) &&
        (clickObject.tile.sprite == nullptr ||
         clickObject.tile.getTargetAlpha(0) != 0.0F)) {
      if (clickObject.tile != nullptr && clickObject.tile.sprite != nullptr) {
      }

      if (float0 > clickObject.x && float1 > clickObject.y &&
          float0 <= clickObject.x + clickObject.width &&
          float1 <= clickObject.y + clickObject.height) {
        if (clickObject.tile instanceof IsoSurvivor) {
          bool boolean0 = false;
        } else if (clickObject.tile.isMaskClicked((int)(float0 - clickObject.x),
                                                  (int)(float1 - clickObject.y),
                                                  clickObject.flip)) {
...
IsoMovingObject PickTarget(int xx, int yy) {
  ...
  for (int int0 = this.ThisFrame.size() - 1; int0 >= 0; int0--) {
    ...
    if (clickObject.tile != IsoPlayer.getInstance() &&
        (clickObject.tile.sprite == nullptr ||
         clickObject.tile.getTargetAlpha() != 0.0F)) {
      if (clickObject.tile != nullptr && clickObject.tile.sprite != nullptr) {
      }
      ...
      if (float0 > clickObject.x && float1 > clickObject.y &&
              float0 <= clickObject.x + clickObject.width &&
              float1 <= clickObject.y + clickObject.height &&
              clickObject.tile instanceof
          IsoMovingObject &&
              clickObject.tile.isMaskClicked((int)(float0 - clickObject.x),
                                             (int)(float1 - clickObject.y),
                                             clickObject.flip)) {
        if (clickObject.tile.rerouteMask != nullptr) {
        }
...
IsoObject PickDoor(int screenX, int screenY, bool bTransparent) {
  ...
  for (int int1 = this.ThisFrame.size() - 1; int1 >= 0; int1--) {
    ...
    if (clickObject.tile instanceof
        IsoDoor && clickObject.tile.getTargetAlpha(int0) != 0.0F &&
            bTransparent == clickObject.tile.getTargetAlpha(int0) < 1.0F &&
            float0 >= clickObject.x && float1 >= clickObject.y &&
            float0 < clickObject.x + clickObject.width &&
            float1 < clickObject.y + clickObject.height) {
...
IsoObject PickWindow(int screenX, int screenY) {
  ...
  for (int int0 = this.ThisFrame.size() - 1; int0 >= 0; int0--) {
    IsoObjectPicker.ClickObject clickObject = this.ThisFrame.get(int0);
    if ((clickObject.tile instanceof IsoWindow || clickObject.tile instanceof
         IsoCurtain) &&
        (clickObject.tile.sprite == nullptr ||
         clickObject.tile.getTargetAlpha() != 0.0F) &&
        float0 >= clickObject.x && float1 >= clickObject.y &&
...
IsoObject PickCorpse(int screenX, int screenY) {
  ...
  for (int int0 = this.ThisFrame.size() - 1; int0 >= 0; int0--) {
    ...
    if (float0 >= clickObject.x && float1 >= clickObject.y &&
      ...
      if (clickObject.tile.isMaskClicked((int)(float0 - clickObject.x),
                                         (int)(float1 - clickObject.y),
                                         clickObject.flip) &&
      ...
      if (clickObject.tile instanceof
          IsoDeadBody &&
              ((IsoDeadBody)clickObject.tile).isMouseOver(float0, float1)) {
        return clickObject.tile;
      }
    }
  }
...
BaseVehicle PickVehicle(int screenX, int screenY) {
  ...
  for (int int0 = 0; int0 < IsoWorld.instance.CurrentCell.getVehicles().size();
       int0++) {
...
public
static final class ClickObject {
  int height;
  ...
  int calculateScore() {
      ...
      if (object instanceof IsoWindow) {
        ...
        if (object instanceof IsoCurtain) {
          ...
          float0 += 20.0F;
                   ...
                   thumpable.getLightSource() != nullptr) {
          float0 += 3.0F;
          ...
          float0 -= 100.0F;
        } else if (sprite.Properties.Is(IsoFlagType.cutW) ||
                   sprite.Properties.Is(IsoFlagType.cutN)) {
          float0 -= 2.0F;
        }
      }
    } else {
      float0 += 6.0F;
      if (door != nullptr && door.isAdjacentToSquare(square0) ||
          thumpable != nullptr && thumpable.isAdjacentToSquare(square0)) {
        float0++;
      }
    ...
    float float2 =
        IsoUtils.DistanceManhatten(square1.getX() + 0.5F, square1.getY() + 0.5F,
