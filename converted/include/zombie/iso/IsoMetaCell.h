...
class IsoMetaCell {
public:
 public ArrayList<IsoMetaGrid.VehicleZone> vehicleZones = std::make_unique<ArrayList<>>();
 public IsoMetaChunk[] ChunkMap = new IsoMetaChunk[900];
 LotHeader info = nullptr;
 public ArrayList<IsoMetaGrid.Trigger> triggers = std::make_unique<ArrayList<>>();
 int wx = 0;
 int wy = 0;
 public ArrayList<IsoMannequin.MannequinZone> mannequinZones = std::make_unique<ArrayList<>>();
 public ArrayList<IsoMetaGrid.RoomTone> roomTones = std::make_unique<ArrayList<>>();
 ...
 void addTrigger(BuildingDef def, int triggerRange, int zombieExclusionRange, const std::string& type) {
 ...
 return y < 30 && x < 30 && x >= 0 && y >= 0 ? this->ChunkMap[y * 30 + x] : nullptr;
 }
