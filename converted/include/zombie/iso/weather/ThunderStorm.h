...
public:
 ...
 private ThunderStorm.PlayerLightningInfo[] lightningInfos = new ThunderStorm.PlayerLightningInfo[4];
 ...
 void startThunderCloud(float str, float angle, float radius, float eventFreq, float thunderRatio, double duration, bool targetRandomPlayer) {
 ...
 if (((IsoPlayer)arrayList.get(int4).getCurrentSquare() == nullptr) {
 ...
 void applyLightningForPlayer(RenderSettings.PlayerRenderSettings renderSettings, int plrIndex, IsoPlayer player) {
 ...
 void triggerThunderEvent(int x, int y, bool doStrike, bool doLightning, bool doRumble) {
 if (GameServer.bServer) {
 ...
 this->climateManager.transmitClimatePacket(ClimateManager.ClimateNetAuth.ServerOnly, (byte)2, nullptr);
 ...
 void enqueueThunderEvent(int x, int y, bool doStrike, bool doLightning, bool doRumble) {
 ...
 if (player1 != nullptr && this->lightningInfos[int3].distance < 7500.0F) {
 ...
 */
 ...
 ClimateColorInfo lightningColor = new ClimateColorInfo(1.0F, 1.0F, 1.0F, 1.0F);
