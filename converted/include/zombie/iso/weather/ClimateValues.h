...
private
  ClimateManager.DayInfo testCurrentDay;
private
  ClimateManager.DayInfo testNextDay;
...
public
  ClimateValues(ClimateManager _clim) {
  ...
  void print() {
    ...
    if (this.testCurrentDay == nullptr) {
      GregorianCalendar gregorianCalendar =
          new GregorianCalendar(this.cacheYear, this.cacheMonth, this.cacheDay);
      DebugLog.log("Printing climate values for: " +
                   new SimpleDateFormat("yyyy MM dd")
                       .format(gregorianCalendar.getTime()));
    } else {
      DebugLog.log("Printing climate values for: " +
                   new SimpleDateFormat("yyyy MM dd")
                       .format(this.testCurrentDay.calendar.getTime()));
    }
  ...
  void pollDate(GregorianCalendar calendar) {
    ...
    this.clim.setDayInfo(this.testCurrentDay, calendar.get(5), calendar.get(2),
                         calendar.get(1), 0);
    this.clim.setDayInfo(this.testNextDay, calendar.get(5), calendar.get(2),
                         calendar.get(1), 1);
    GregorianCalendar gregorianCalendar = new GregorianCalendar(
        this.gt.getYear(), this.gt.getMonth(), this.gt.getDayPlusOne(),
        this.gt.getHour(), this.gt.getMinutes());
    double double1 = ChronoUnit.MINUTES.between(gregorianCalendar.toInstant(),
                                                calendar.toInstant());
    double1 /= 60.0;
  ...
  void updateValues(double double0, float float4,
                    ClimateManager.DayInfo dayInfo0,
                    ClimateManager.DayInfo dayInfo1) {
    if (dayInfo0.year != this.cacheYear || dayInfo0.month != this.cacheMonth ||
        dayInfo0.day != this.cacheDay) {
      ...
      if (this.dayDoFog) {
        ...
        float float2 = (float)SimplexNoise.noise(
            this.simplexOffsetA,
            (double0 + 12.0 - 48.0) /
                this.clim.getAirMassNoiseFrequencyMod(
    ...
    float float6 =
        ClimateManager.lerp(float5, dayInfo0.season.getCurDayPercent(),
                            dayInfo1.season.getCurDayPercent());
    this.airMassNoiseFrequencyMod = this.clim.getAirMassNoiseFrequencyMod(
        SandboxOptions.instance.getRainModifier());
    this.noiseAirmass = (float)SimplexNoise.noise(
        this.simplexOffsetA, double0 / this.airMassNoiseFrequencyMod);
    float float7 = (float)SimplexNoise.noise(
        this.simplexOffsetC, double0 / this.airMassNoiseFrequencyMod);
    this.airMassTemperature = (float)SimplexNoise.noise(
        this.simplexOffsetA, (double0 - 48.0) / this.airMassNoiseFrequencyMod);
    double double1 = Math.floor(double0) + 12.0;
    this.airFrontAirmass = (float)SimplexNoise.noise(
        this.simplexOffsetA, double1 / this.airMassNoiseFrequencyMod);
    float float8 =
        ClimateManager.clerp(float5, dayInfo0.season.getDayTemperature(),
                             dayInfo1.season.getDayTemperature());
    float float9 =
        ClimateManager.clerp(float5, dayInfo0.season.getDayMeanTemperature(),
                             dayInfo1.season.getDayMeanTemperature());
    ...
    this.dayLightLagged =
        this.clim.getTimeLerpHours(float4, this.dawn + float10, float11, true);
    float float12 = 5.0F * (1.0F - this.dayLightLagged);
    this.nightLagged =
        this.clim.getTimeLerpHours(float4, float11, this.dawn + float10, true);
    ...
    float float17 =
        (float)SimplexNoise.noise(double0 / 40.0, this.simplexOffsetA);
    float float18 = (float17 + 1.0F) * 0.5F;
    float18 *= float15 * float16;
    float18 *= 0.65F;
    this.windIntensity = float18;
    float float19 =
        (float)SimplexNoise.noise(double0 / 80.0, this.simplexOffsetB);
    this.windAngleIntensity = float19;
    float float20 =
        (float)SimplexNoise.noise(double0 / 40.0, this.simplexOffsetD);
    float20 = (float20 + 1.0F) * 0.5F;
    this.windAngleDegrees = 360.0F * float20;
    this.lerpNight =
        this.clim.getTimeLerpHours(float4, this.dusk, this.dawn, true);
    ...
    this.cloudyT = 1.0F - ClimateManager.clamp01(
                              (this.airMassTemperature + 0.8F) * 0.625F);
    this.cloudyT *= 0.8F;
  ...
  float getNoon() { return this.noon; }

  double getAirMassNoiseFrequencyMod() { return this.airMassNoiseFrequencyMod; }

  float getNoiseAirmass() { return this.noiseAirmass; }

  float getAirMassTemperature() { return this.airMassTemperature; }

  float getBaseTemperature() { return this.baseTemperature; }

  float getDayLightLagged() { return this.dayLightLagged; }

  float getNightLagged() { return this.nightLagged; }

  float getTemperature() { return this.temperature; }

  bool isTemperatureIsSnow() { return this.temperatureIsSnow; }

  float getHumidity() { return this.humidity; }

  float getWindIntensity() { return this.windIntensity; }

  float getWindAngleIntensity() { return this.windAngleIntensity; }

  float getWindAngleDegrees() { return this.windAngleDegrees; }

  float getNightStrength() { return this.nightStrength; }

  float getDayLightStrength() { return this.dayLightStrength; }

  float getAmbient() { return this.ambient; }

  float getDesaturation() { return this.desaturation; }

  float getDayLightStrengthBase() { return this.dayLightStrengthBase; }

  float getLerpNight() { return this.lerpNight; }

  float getCloudyT() { return this.cloudyT; }

  float getCloudIntensity() { return this.cloudIntensity; }

  float getAirFrontAirmass() { return this.airFrontAirmass; }

  double getCacheWorldAgeHours() { return this.cacheWorldAgeHours; }

  int getCacheYear() { return this.cacheYear; }

  int getCacheMonth() { return this.cacheMonth; }

  int getCacheDay() { return this.cacheDay; }

  float getDayMeanTemperature() { return this.dayMeanTemperature; }

  bool isDayDoFog() { return this.dayDoFog; }

  float getDayFogStrength() { return this.dayFogStrength; }

  float getDayFogDuration() { return this.dayFogDuration; }
}
} // namespace weather
} // namespace iso
} // namespace zombie
