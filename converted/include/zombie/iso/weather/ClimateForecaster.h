...
public:
 ...
 int int0 = 10 + offset;
 return int0 >= 0 && int0 < this->forecasts.length ? this->forecasts[int0] : nullptr;
 }
 ...
 void sampleDay(ClimateManager climateManager, ClimateForecaster.DayForecast dayForecast1, int int3) {
 ...
 float float0 = dayForecast2 != nullptr ? dayForecast2.weatherPeriod.getTotalStrength() : -1.0F;
 if (float0 < 0.1F) {
 ...
 bool boolean1 = int4 > this->climateValues.getDawn() && int4 <= this->climateValues.getDusk();
 ...
 if (dayForecast1.weatherStarts || dayForecast1.weatherOverlap != nullptr) {
 ...
 } else if (dayForecast1.weatherOverlap != nullptr && int4 < dayForecast1.weatherEndTime) {
 ...
 private ArrayList<Integer> weatherStages = std::make_unique<ArrayList<>>();
