...
public:
 ...
 private static ArrayList<WorldFlares.Flare> flares = std::make_unique<ArrayList<>>();
 ...
 static void launchFlare(float lifetime, int x, int y, int range, float windSpeed, float r, float g, float b, float ri, float gi, float bi) {
 if (flares.size() > 100) {
 flares.remove(0);
 }
 ...
 static void applyFlaresForPlayer(RenderSettings.PlayerRenderSettings renderSettings, int plrIndex, IsoPlayer player) {
 for (int int0 = flares.size() - 1; int0 >= 0; int0--) {
 if (flares.get(int0).hasLaunched) {
 ...
 ClimateColorInfo color = new ClimateColorInfo(1.0F, 0.0F, 0.0F, 1.0F, 1.0F, 0.0F, 0.0F, 1.0F);
 bool hasLaunched = false;
 SteppedUpdateFloat intensity = new SteppedUpdateFloat(0.0F, 0.01F, 0.0F, 1.0F);
 float maxLifeTime;
 float lifeTime;
 ...
 float getIntensityPlayer(int index) {
 return this->infos[index].intensity;
 }

 float getLerpPlayer(int index) {
 return this->infos[index].lerp;
 }

 float getDistModPlayer(int index) {
 ...
 * (float)Math.cos(ClimateManager.getInstance().getWindAngleRadians())
 ...
 void applyFlare(RenderSettings.PlayerRenderSettings playerRenderSettings, int int0, IsoPlayer var3) {
 WorldFlares.PlayerFlareLightInfo playerFlareLightInfo = this->infos[int0];
