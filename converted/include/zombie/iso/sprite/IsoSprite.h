...
public:
 ...
 private static HashMap<String, Object[]> AnimNameSet = std::make_unique<HashMap<>>();
 int firerequirement;
 ...
 public HashMap<String, IsoAnim> AnimMap = new HashMap<>(2);
 public ArrayList<IsoAnim> AnimStack = new ArrayList<>(1);
 std::string name;
 ...
 static IsoSprite CreateSpriteUsingCache(const std::string& objectName, const std::string& animName, int numFrames) {
 ...
 static IsoSprite getSprite(IsoSpriteManager manager, const std::string& _name, int offset) {
 IsoSprite sprite = manager.NamedMap.get(_name);
 std::string string0 = sprite.name.substring(0, sprite.name.lastIndexOf(95);
 std::string string1 = sprite.name.substring(sprite.name.lastIndexOf(95) + 1);
 ...
 */
 ...
 void LoadFramesNoDirPage(const std::string& ObjectName, const std::string& AnimName, int nFrames) {
 this->CurrentAnim = std::make_unique<IsoAnim>();
 this->AnimMap.put(AnimName, this->CurrentAnim);
 this->CurrentAnim.ID = this->AnimStack.size();
 this->AnimStack.add(this->CurrentAnim);
 ...
 void LoadFramesNoDirPageDirect(const std::string& ObjectName, const std::string& AnimName, int nFrames) {
 this->CurrentAnim = std::make_unique<IsoAnim>();
 this->AnimMap.put(AnimName, this->CurrentAnim);
 this->CurrentAnim.ID = this->AnimStack.size();
 this->AnimStack.add(this->CurrentAnim);
 ...
 this->render(spriteInstance, nullptr, x, y, z, IsoDirections.N, 32 * Core.TileScale, 96 * Core.TileScale, IsoGridSquare.getDefColorInfo(), true);
 }
 ...
 void RenderGhostTileColor(int x, int y, int z, float r, float g, float b, float a) {
 this->RenderGhostTileColor(x, y, z, 0.0F, 0.0F, r, g, b, a);
 }

 void RenderGhostTileColor(int x, int y, int z, float offsetX, float offsetY, float r, float g, float b, float a) {
 IsoSpriteInstance spriteInstance = IsoSpriteInstance.get(this);
 ...
 this->render(spriteInstance, nullptr, x, y, z, IsoDirections.N, 32 * int0 + offsetX, 96 * int0 + offsetY, IsoGridSquare.getDefColorInfo(), true);
 }
 ...
 void render(IsoObject obj, float x, float y, float z, IsoDirections dir, float offsetX, float offsetY, ColorInfo info2, bool bDoRenderPrep) {
 this->render(obj, x, y, z, dir, offsetX, offsetY, info2, bDoRenderPrep, nullptr);
 }
 ...
 this->render(inst, obj, x, y, z, dir, offsetX, offsetY, info2, bDoRenderPrep, nullptr);
 }
 ...
 if (object instanceof IsoMovingObject && this->CurrentAnim != nullptr && this->CurrentAnim.FramesArray[int0].getTexture(directions) != nullptr) {
 ...
 if (DebugOptions.instance.IsoSprite.DropShadowEdges.getValue() && StringUtils == texture.getName(), "dropshadow")) {
 this->renderSpriteOutline(float4, float5, texture, float2, float3);
 }

 if (!this->hideForWaterRender || !IsoWater.getInstance().getShaderEnable()) {
 if (object != nullptr && object.getObjectRenderEffectsToApply() != nullptr) {
 ...
 && object != nullptr) {
 bool boolean0 = directions == IsoDirections.W || directions == IsoDirections.SW || directions == IsoDirections.S;
 ...
 void renderSpriteOutline(float float0, float float1, Texture texture, float float3, float float2) {
 ...
 info.b = info2.b;
 info.a = info2.a;
 this->CurrentAnim.Frames.get(0).render(float0, float1, IsoDirections.N, info, false, nullptr);
 } catch (Exception exception) {
 Logger.getLogger(GameWindow.class.getName()).log(Level.SEVERE, nullptr, exception);
 }
 }
 ...
 void renderObjectPicker(IsoSpriteInstance _def, IsoObject obj, IsoDirections dir) {
 if (this->CurrentAnim != nullptr) {
 if (_def != nullptr) {
 ...
 if (this->CurrentAnim.Frames.get((int)_def.Frame).getTexture(dir) != nullptr) {
 ...
 bool boolean0 = dir == IsoDirections.W || dir == IsoDirections.SW || dir == IsoDirections.S;
 if (_def.Flip) {
 boolean0 = !boolean0;
 }
 ...
 IsoSprite setFromCache(const std::string& objectName, const std::string& animName, int numFrames) {
 ...
 void setTintMod(ColorInfo _info) {
 ...
 void setSpriteGrid(IsoSpriteGrid sGrid) {
