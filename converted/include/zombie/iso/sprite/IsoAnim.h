...
public:
public
  static final HashMap<String, IsoAnim> GlobalAnimMap =
      std::make_unique<HashMap<>>();
  short FinishUnloopedOnFrame = 0;
  short FrameDelay = 0;
  short LastFrame = 0;
public
  final ArrayList<IsoDirectionFrame> Frames = new ArrayList<>(8);
  std::string name;
  bool looped = true;
  int ID = 0;
private
  static final ThreadLocal<StringBuilder> tlsStrBuf =
      std::make_unique<ThreadLocal<StringBuilder>>(){
          StringBuilder initialValue(){return std::make_unique<StringBuilder>();
}
}; // namespace sprite
public
IsoDirectionFrame[] FramesArray = new IsoDirectionFrame[0];
...
void LoadExtraFrame(const std::string &string2, const std::string &string0,
  ...
  IsoDirectionFrame directionFrame = new IsoDirectionFrame(
      Texture.getSharedTexture(string1 + "8" + string3 + integer.toString() +
                               ".png"),
      Texture.getSharedTexture(string1 + "9" + string3 + integer.toString() +
                               ".png"),
      Texture.getSharedTexture(string1 + "6" + string3 + integer.toString() +
                               ".png"),
      Texture.getSharedTexture(string1 + "3" + string3 + integer.toString() +
                               ".png"),
      Texture.getSharedTexture(string1 + "2" + string3 + integer.toString() +
                               ".png"));
  ...
  if (texture != nullptr) {
    for (int int3 = 0; int3 < nFrames; int3++) {
      ...
      if (texture == nullptr) {
        ...
        object = new IsoDirectionFrame(Texture.getSharedTexture(string1),
        ...
        object = new IsoDirectionFrame(Texture.getSharedTexture(string6),
    ...
    for (int int3 = 0; int3 < nFrames; int3++) {
      ...
      if (texture != nullptr) {
        ...
        for (int int5 = 0; int5 < string0.length(); int5++) {
          try {
            stringBuilder.setCharAt(int1 + int5,
                                    integer.toString().charAt(int5));
        ...
        stringBuilder.setCharAt(int0, '2');
        std::string string13 = stringBuilder.toString();
        object = new IsoDirectionFrame(Texture.getSharedTexture(string9),
                                       Texture.getSharedTexture(string10),
                                       Texture.getSharedTexture(string11),
                                       Texture.getSharedTexture(string12),
                                       Texture.getSharedTexture(string13));
...
void LoadFramesUseOtherFrame(const std::string &ObjectName,
  ...
  for (int int0 = 0; int0 < 1; int0++) {
    ...
    IsoDirectionFrame directionFrame = new IsoDirectionFrame(
        Texture.getSharedTexture(string0 + "8" + string1 + integer.toString() +
                                 string2 + ".png"),
        Texture.getSharedTexture(string0 + "9" + string1 + integer.toString() +
                                 string2 + ".png"),
        Texture.getSharedTexture(string0 + "6" + string1 + integer.toString() +
                                 string2 + ".png"),
        Texture.getSharedTexture(string0 + "3" + string1 + integer.toString() +
                                 string2 + ".png"),
        Texture.getSharedTexture(string0 + "2" + string1 + integer.toString() +
                                 string2 + ".png"));
    this.Frames.add(directionFrame);
  }
...
void LoadFramesBitRepeatFrame(const std::string &ObjectName,
  ...
  IsoDirectionFrame directionFrame = new IsoDirectionFrame(
      Texture.getSharedTexture(AnimName + "8" + string0 + integer.toString() +
                               string1 + ".png"),
      Texture.getSharedTexture(AnimName + "9" + string0 + integer.toString() +
                               string1 + ".png"),
      Texture.getSharedTexture(AnimName + "6" + string0 + integer.toString() +
                               string1 + ".png"),
      Texture.getSharedTexture(AnimName + "3" + string0 + integer.toString() +
                               string1 + ".png"),
      Texture.getSharedTexture(AnimName + "2" + string0 + integer.toString() +
                               string1 + ".png"));
  this.Frames.add(directionFrame);
  this.FinishUnloopedOnFrame = (short)(this.Frames.size() - 1);
  this.FramesArray = this.Frames.toArray(this.FramesArray);
...
void LoadFramesNoDir(const std::string &string2, const std::string &string0,
  ...
  for (int int0 = 0; int0 < int1; int0++) {
    int integer = new Integer(int0);
    IsoDirectionFrame directionFrame =
        new IsoDirectionFrame(Texture.getSharedTexture(
            string1 + string3 + integer.toString() + ".png"));
    this.Frames.add(directionFrame);
  }
...
void LoadFramesNoDirPage(const std::string &string2, const std::string &string0,
  ...
  for (int int0 = 0; int0 < int1; int0++) {
    int integer = new Integer(int0);
    IsoDirectionFrame directionFrame = new IsoDirectionFrame(
        Texture.getSharedTexture(string1 + string3 + integer.toString()));
    this.Frames.add(directionFrame);
  }
...
void LoadFramesNoDirPage(const std::string &string1) {
  ...
  for (int int0 = 0; int0 < 1; int0++) {
    IsoDirectionFrame directionFrame =
        new IsoDirectionFrame(Texture.getSharedTexture(string0));
    this.Frames.add(directionFrame);
  }
...
void LoadFramesPageSimple(const std::string &NObjectName,
  ...
  for (int int0 = 0; int0 < 1; int0++) {
    new Integer();
    IsoDirectionFrame directionFrame =
        new IsoDirectionFrame(Texture.getSharedTexture(NObjectName),
...
void LoadFramesNoDirPalette(const std::string &string2,
  ...
  for (int int0 = 0; int0 < int1; int0++) {
    ...
    IsoDirectionFrame directionFrame =
        new IsoDirectionFrame(Texture.getSharedTexture(
            string1 + string3 + integer.toString() + ".pcx", string4));
    this.Frames.add(directionFrame);
  }
