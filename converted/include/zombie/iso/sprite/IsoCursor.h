...
class IsoCursor {
public:
  static IsoCursor instance = null;
  ...
  IsoCursor() {
    ...
    if (this.m_shader != nullptr) {
      this.m_shader.m_textureCursor =
          Texture.getSharedTexture("media/ui/isocursor.png");
    }
  }

  void createShader() { this.m_shader = new IsoCursor.IsoCursorShader(); }
      ...
      if (player != nullptr && !player.isDead() && player.isAiming() &&
        ...
        if (!GameTime.isGamePaused()) {
          if (this.m_shader != nullptr && this.m_shader.isCompiled()) {
            ...
            int int1 = (int)(this.m_shader.m_textureCursor.getWidth() *
                             Core.TileScale / 2.0F * float0);
            int int2 = (int)(this.m_shader.m_textureCursor.getHeight() *
                             Core.TileScale / 2.0F * float0);
            ...
            SpriteRenderer.instance.renderClamped(
                this.m_shader.m_textureCursor, this.m_shader.m_screenX,
                this.m_shader.m_screenY, int1, int2, int3, int4, int5, int6,
                1.0F, 1.0F, 1.0F, 1.0F, this.m_shader);
...
private
  static class IsoCursorShader extends Shader implements Consumer<TextureDraw> {
    float m_alpha = 1.0F;
    ...
    IsoCursorShader() { super("isocursor"); }
    ...
    void accept(TextureDraw textureDraw) {
      ...
      float float2 = IsoCamera.getScreenTop(byte0) +
                     IsoCamera.getScreenHeight(byte0) -
                     (this.m_screenY + this.height);
