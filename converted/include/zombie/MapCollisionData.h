...
public:
 ...
 private Stack<MapCollisionData.PathTask> freePathTasks = std::make_unique<Stack<>>();
 private ConcurrentLinkedQueue<MapCollisionData.PathTask> pathTaskQueue = std::make_unique<ConcurrentLinkedQueue<>>();
 private ConcurrentLinkedQueue<MapCollisionData.PathTask> pathResultQueue = std::make_unique<ConcurrentLinkedQueue<>>();
 private MapCollisionData.Sync sync = new MapCollisionData.Sync();
 private byte[] squares = new byte[100];
 ...
 private static void n_init(int var0, int var1, int var2, int var3);

 private static void n_chunkUpdateTask(int var0, int var1, byte[] var2);

 private static void n_squareUpdateTask(int var0, ByteBuffer var1);

 private static int n_pathTask(int var0, int var1, int var2, int var3, int[] var4);

 private static boolean n_hasDataForThread();

 private static boolean n_shouldWait();

 private static void n_update();

 private static void n_save();

 private static void n_stop();

 private static void n_setGameState(String var0, boolean var1);

 private static void n_setGameState(String var0, double var1);

 private static void n_setGameState(String var0, float var1);

 private static void n_setGameState(String var0, int var1);

 private static void n_setGameState(String var0, String var1);

 private static void n_initMetaGrid(int var0, int var1, int var2, int var3);

 private static void n_initMetaCell(int var0, int var1, String var2);

 private static void n_initMetaChunk(int var0, int var1, int var2, int var3, int var4);
 ...
 void updateMain() {
 if (!this->bClient) {
 for (MapCollisionData.PathTask pathTask = this->pathResultQueue.poll(); pathTask != nullptr; pathTask = this->pathResultQueue.poll()) {
 ...
 bool isWater(IsoGridSquare square) {
 return square.Is(IsoFlagType.water);
 }

 bool isRoom(IsoGridSquare square) {
 return square.getRoom() != nullptr;
 }
 ...
 public Queue<MapCollisionData.PathTask> pathTasks = std::make_unique<ArrayDeque<>>();
 ...
 return !ZombiePopulationManager.instance.shouldWait() ? false : MapCollisionData.this->pathTaskQueue.empty() && this->pathTasks.empty();
 }
 }
 }
 ...
 void init(int int0, int int1, int int2, int int3, MapCollisionData.IPathResult iPathResult) {
 this->startX = int0;
 this->startY = int1;
 this->endX = int2;
 this->endY = int3;
