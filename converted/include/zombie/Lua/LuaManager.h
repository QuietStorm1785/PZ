...
public
  static LuaManager.Exposer exposer;
public
  static ArrayList<String> loaded = std::make_unique<ArrayList<>>();
private
  static final HashSet<String> loading = std::make_unique<HashSet<>>();
public
  static HashMap<String, Object> loadedReturn = std::make_unique<HashMap<>>();
  static bool checksumDone = false;
public
  static ArrayList<String> loadList = std::make_unique<ArrayList<>>();
  static ArrayList<String> paths = std::make_unique<ArrayList<>>();
private
  static final HashMap<String, Object> luaFunctionMap =
      std::make_unique<HashMap<>>();
private
  static final HashSet<KahluaTable> s_wiping = std::make_unique<HashSet<>>();
  ...
  static void LoadDirBase(const std::string &string1, bool boolean0) {
    ...
    for (int int0 = 0; int0 < arrayList1.size(); int0++) {
      std::string string2 =
          ZomboidFileSystem.instance.getModDir((String)arrayList1.get(int0));
      if (string2 != nullptr) {
        File file1 = new File(string2);
        URI uri = file1.getCanonicalFile().toURI();
        File file2 =
            ZomboidFileSystem.instance.getCanonicalFile(file1, "media");
        File file3 = ZomboidFileSystem.instance.getCanonicalFile(file2, "lua");
        File file4 =
            ZomboidFileSystem.instance.getCanonicalFile(file3, string1);
        File file5 = file4;
    ...
    for (auto &string3 : loadList)
      if (!hashSet.contains(string3)) {
        hashSet.add(string3);
        std::string string4 =
            ZomboidFileSystem.instance.getAbsolutePath(string3);
        if (string4 == nullptr) {
        ...
        if (!checksumDone && !string3.contains("SandboxVars.lua") &&
...
static void searchFolders(URI uri, File file) {
  if (file.isDirectory()) {
    ...
    for (int int0 = 0; int0 < strings.length; int0++) {
      searchFolders(uri, new File(file.getCanonicalFile().getAbsolutePath() +
                                  File.separator + strings[int0]));
    }
  } else if (file.getAbsolutePath().toLowerCase().endsWith(".lua")) {
    std::string string =
        ZomboidFileSystem.instance.getRelativeFile(uri, file.getAbsolutePath());
...
static std::string getLuaCacheDir() {
  std::string string =
      ZomboidFileSystem.instance.getCacheDir() + File.separator + "Lua";
...
static std::string getSandboxCacheDir() {
  std::string string = ZomboidFileSystem.instance.getCacheDir() +
                       File.separator + "Sandbox Presets";
  File file = new File(string);
  if (!file.exists()) {
    file.mkdir();
  }
...
static void transferItem(IsoGameCharacter character, InventoryItem item,
                         ItemContainer container0, ItemContainer container1) {
...
static void dropItem(InventoryItem item) {
  LuaClosure luaClosure =
      getDotDelimitedClosure("ISInventoryPaneContextMenu.dropItem");
  caller.pcall(thread, luaClosure, item);
}

static IsoGridSquare AdjacentFreeTileFinder(IsoGridSquare square,
                                            IsoPlayer player) {
...
static void *RunLuaInternal(const std::string &string, bool boolean0) {
  ...
  if (loaded.contains(string)) {
    ...
    if (DebugLog.isEnabled(DebugType.Lua)) {
      DebugLog.Lua.println("Loading: " +
                           ZomboidFileSystem.instance.getRelativeFile(string));
    }
    ...
    try(BufferedReader bufferedReader = new BufferedReader(inputStreamReader)) {
      luaClosure = LuaCompiler.loadis(
          bufferedReader, string.substring(string.lastIndexOf(47) + 1), env);
    }
    catch (Exception exception) {
      Logger.getLogger(LuaManager.class.getName())
          .log(Level.SEVERE, "Error found in LUA file: " + string, nullptr);
    ...
    if (!luaReturn.isSuccess()) {
      Logger.getLogger(IsoWorld.class.getName())
          .log(Level.SEVERE, luaReturn.getErrorString(), nullptr);
      if (luaReturn.getJavaException() != nullptr) {
        Logger.getLogger(IsoWorld.class.getName())
            .log(Level.SEVERE, luaReturn.getJavaException().toString(),
                 nullptr);
      }

      Logger.getLogger(IsoWorld.class.getName())
          .log(Level.SEVERE, luaReturn.getLuaStackTrace(), nullptr);
    }

    loaded.add(string);
    void *object = luaReturn.isSuccess() && luaReturn.size() > 0
                       ? luaReturn.getFirst()
                       : null;
    if (object != nullptr) {
  ...
  if (string != nullptr && !string.isEmpty()) {
    ...
    if (object != nullptr) {
      ...
      if (string.contains(".")) {
        ...
        for (int int0 = 0; int0 < strings.length - 1; int0++) {
          KahluaTable table1 =
              Type.tryCastTo(table0.rawget(strings[int0]), KahluaTable.class);
          if (table1 == nullptr) {
        ...
        if (Modifier.isStatic(field.getModifiers()) &&
            ...
            field.getType() == int.class)
                        ...
                        && !field.getName().endsWith("WIN")) {
                          table0.rawset(field.getName(),
                                        (double)field.getInt(nullptr));
                        }
    }
  }
  catch (Exception exception) {
    ...
    try {
      for (auto &field : fields)
        if (Modifier.isStatic(field.getModifiers()) &&
            ...
            Modifier.isFinal(field.getModifiers()) &&
            field.getType() == int.class)) {
            table.rawset(field.getName(),
                         BoxedStaticValues.toDouble(field.getInt(nullptr)));
          }
    }
  }
  catch (Exception exception) {
...
static KahluaTable copyTable(KahluaTable table) { return copyTable(); }
...
static KahluaTable copyTable(KahluaTable table0, KahluaTable table1) {
  ...
  if (table1 != nullptr && !table1.isEmpty()) {
...
public
static final class Exposer extends LuaJavaClassExposer {
private
  final HashSet < Class < ? >> exposed = std::make_unique<HashSet<>>();

public
  Exposer(KahluaConverterManager manager, Platform platform,
  ...
  void exposeAll() {
    ...
    if (Core.bDebug) {
      try {
        this.exposeMethod(Class.class, Class.class.getMethod("getName"),
                          LuaManager.env);
        this.exposeMethod(Class.class, Class.class.getMethod("getSimpleName"),
                          LuaManager.env);
...
/**
 * Object containing global Lua functions. The methods in this class can be
 * called from Lua using ``methodName()``, instead of qualifying them with the
 * class name, even if they are not static: the instance is state of the Lua
 * environment.
 */
...
private
  static final LuaManager.GlobalObject.TimSortComparator timSortComparator =
      new LuaManager.GlobalObject.TimSortComparator();
    ...
    static Model loadZomboidModel(const std::string& name, const std::string& mesh, const std::string& tex, const std::string& shader, bool bStatic) {
              try {
                ...
                Model model = ModelManager.instance.tryGetLoadedModel(
                    mesh, tex, bStatic, shader, false);
                if (model != nullptr) {
                  return model;
                } else {
                  ModelManager.instance.setModelMetaData(name, mesh, tex,
                                                         shader, bStatic);
                  Model.ModelAssetParams modelAssetParams =
                      new Model.ModelAssetParams();
                  ...
                  modelAssetParams.textureFlags =
                      ModelManager.instance.getTextureFlags();
                  model = (Model)ModelAssetManager.instance.load(
                      new AssetPath(name), modelAssetParams);
                  if (model != nullptr) {
                    ModelManager.instance.putLoadedModel(mesh, tex, bStatic,
                                                         shader, model);
                  }
                ...
                DebugLog.General.error("LuaManager.loadZomboidModel> Exception "
                                       "thrown loading model: " +
                                       name + " mesh:" + mesh + " tex:" + tex +
                                       " shader:" + shader +
                                       " isStatic:" + bStatic);
    ...
    static void setModelMetaData(const std::string& name, const std::string& mesh, const std::string& tex, const std::string& shader, bool bStatic) {
              ...
              ModelManager.instance.setModelMetaData(name, mesh, tex, shader,
                                                     bStatic);
            }
    ...
    static RadioAPI getRadioAPI() {
        ...
        public static ArrayList<String> getTranslatorCredits(Language language) {
              File file = new File(ZomboidFileSystem.instance.getString(
                  "media/lua/shared/Translate/" + language.name() +
                  "/credits.txt"));

              try {
                std::vector arrayList0;
                try(FileReader fileReader = new FileReader(
                        file, Charset.forName(language.charset()));
    ...
    static void setPuddles(float initialPuddles) {
              ...
              puddlesFloat.setEnableAdmin(true);
              puddlesFloat.setAdminValue(
                  PZMath.clamp_01(initialPuddles * 1.2F));
            }
    ...
    static ZomboidRadio getZomboidRadio() {
              return ZomboidRadio.hasInstance() ? ZomboidRadio.getInstance()
                                                : nullptr;
            }
    ...
    bool boolean0 = false;
        ...
        public static void serverConnect(
            String string0, String string1, String string2, String string3, String string4, String string5, String string6, boolean boolean0
              ...
              GameClient.instance.doConnect(string0, string1, string2, string3,
    ...
    static void sendPing() {
              if (GameClient.bClient) {
                ByteBufferWriter byteBufferWriter =
                    GameClient.connection.startPingPacket();
    ...
    static KahluaTable getPacketCounts(int category) {
              return GameClient.bClient ? PacketTypes.getPacketCounts(category)
                                        : nullptr;
            }
    ...
    static float isoToScreenX(int player, float x, float y, float z) {
              float float0 = IsoUtils.XToScreen(x, y, z, 0) -
                             IsoCamera.cameras[player].getOffX();
    ...
    static float isoToScreenY(int player, float x, float y, float z) {
              float float0 = IsoUtils.YToScreen(x, y, z, 0) -
                             IsoCamera.cameras[player].getOffY();
              float0 /= Core.getInstance().getZoom(player);
    ...
    static float screenToIsoX(int player, float x, float y, float z) {
              ...
              return IsoCamera.cameras[player].XToIso(x * float0, y * float0,
                                                      z);
            }
    ...
    static float screenToIsoY(int player, float x, float y, float z) {
              ...
              x -= IsoCamera.getScreenLeft(player);
              y -= IsoCamera.getScreenTop(player);
              return IsoCamera.cameras[player].YToIso(x * float0, y * float0,
                                                      z);
            }
    ...
    static void setActivePlayer(int id) {
            ...
            /**
             * Gets the current player. To support splitscreen,
             * getSpecificPlayer() should be preferred instead.
    ...
    static IsoPlayer getPlayerByOnlineID(int id) {
              if (GameServer.bServer) {
                ...
                return GameClient.bClient
                           ? GameClient.IDToPlayerMap.get((short)id)
                           : nullptr;
              }
            }
    ...
    static void initUISystem() {
              UIManager.init();
              LuaEventManager.triggerEvent("OnCreatePlayer", 0,
                                           IsoPlayer.players[0]);
            }
    ...
    static void AddWorldSound(IsoPlayer player, int radius, int volume) {
              WorldSoundManager.instance.addSound(
                  nullptr, (int)player.getX(), (int)player.getY(),
                  (int)player.getZ(), radius, volume, false);
            }
    ...
    static KahluaTable getServerSpawnRegions() {
              return !GameClient.bClient
                         ? nullptr
                         : GameClient.instance.getServerSpawnRegions();
            }
    ...
    static std::string getServerIP() {
              if (GameServer.bServer) {
                return GameServer.IPCommandline == nullptr
                           ? GameServer.ip
                           : GameServer.IPCommandline;
              } else {
    ...
    static KahluaTable getLatestSave() {
              ...
              try {
                bufferedReader = new BufferedReader(new FileReader(
                    new File(ZomboidFileSystem.instance.getCacheDir() +
                             File.separator + "latestSave.ini")));
              } catch (FileNotFoundException fileNotFoundException) {
                return table;
              }
              ...
              try {
                ...
                for (int int0 = 1;
                     (object = bufferedReader.readLine()) != nullptr; int0++) {
    ...
    static bool isCurrentExecutionPoint(const std::string& file, int line) {
              ...
              LuaCallFrame luaCallFrame =
                  LuaManager.thread.currentCoroutine.getCallFrame(int0);
    ...
    static std::string getAccessLevel() {
        ...
        public static ArrayList<IsoPlayer> getOnlinePlayers() {
              if (GameServer.bServer) {
                ...
                return GameClient.bClient ? GameClient.instance.getPlayers()
                                          : nullptr;
              }
            }
    ...
    static float getCameraOffY() {
            ...
            /**
             * Create a dynamic table containing all spawnpoints.lua we find in
             * vanilla  folder + in loaded mods
             */
    ...
    static KahluaTable createRegionFile() {
              ...
              if (string0 == "DEFAULT")) {
                  ...
                  if (mapGroups.getNumberOfGroups() != 1) {
                    throw new RuntimeException(
                        "GameMap is DEFAULT but there are multiple worlds to "
                        "choose from");
                  }
              ...
              if (!string1.isEmpty()) {
                File file = new File(ZomboidFileSystem.instance.getString(
                    "media/maps/" + string1 + "/spawnpoints.lua"));
                if (file.exists()) {
                  ...
                  if (strings != nullptr) {
                    for (int int2 = 0; int2 < strings.length; int2++) {
                      ...
                      ChooseGameInfo.Map map =
                          ChooseGameInfo.getMapDetails(string2);
                      if (map.getLotDirectories() != nullptr &&
                          !map.getLotDirectories().isEmpty() &&
                          !string2 == "challengemaps")) {
    ...
    static void deleteSave(const std::string& file) {
              if (StringUtils.containsDoubleDot(file)) {
                ...
                File file0 =
                    new File(ZomboidFileSystem.instance.getSaveDirSub(file));
                String[] strings = file0.list();
                if (strings != nullptr) {
                  for (int int0 = 0; int0 < strings.length; int0++) {
                    File file1 =
                        new File(ZomboidFileSystem.instance.getSaveDirSub(
                            file + File.separator + strings[int0]));
    ...
    static std::string getServerListFile() {
              return SteamUtils.isSteamModeEnabled() ? "ServerListSteam.txt"
                                                     : "ServerList.txt";
            }
    ...
    static KahluaTable getServerList() {
              ...
              try {
                File file = new File(LuaManager.getLuaCacheDir() +
                                     File.separator + getServerListFile());
                if (!file.exists()) {
                  file.createNewFile();
                }
                ...
                while ((object = bufferedReader.readLine()) != nullptr) {
                  if (object.startsWith("name=")) {
                    ...
                    server0.setDescription(
                        object.replaceFirst("description=", ""));
                    ...
                    server0.setServerPassword(
                        object.replaceFirst("serverpassword=", ""));
                  } else if (object.startsWith("usesteamrelay=")) {
                    server0.setUseSteamRelay(Boolean.parseBoolean(
                        object.replaceFirst("usesteamrelay=", "")));
                  }
                }
    ...
    static KahluaTable transformIntoKahluaTable(Object> map) {
        ...
        public static ArrayList<File> getSaveDirectory(String folder) {
              ...
              if (strings == nullptr) {
                ...
                for (int int0 = 0; int0 < strings.length; int0++) {
                  File file1 =
                      new File(folder + File.separator + strings[int0]);
                  if (file1.isDirectory()) {
                    arrayList.add(file1);
                  }
                }
    ...
    static KahluaTable getFullSaveDirectoryTable() {
              KahluaTable table = LuaManager.platform.newTable();
              File file0 = new File(ZomboidFileSystem.instance.getSaveDir() +
                                    File.separator);
              if (!file0.exists()) {
                file0.mkdir();
              }
              ...
              if (strings == nullptr) {
                ...
                for (int int0 = 0; int0 < strings.length; int0++) {
                  File file1 =
                      new File(ZomboidFileSystem.instance.getSaveDir() +
                               File.separator + strings[int0]);
                  if (file1.isDirectory() && !"Multiplayer" == strings[int0])) {
                      std::vector arrayList1 = getSaveDirectory(
    ...
    int compare(File file0, File file1) {
            ...
            static std::string getSaveName(File file) {
              String[] strings =
                  file.getAbsolutePath().split("\\" + File.separator);
              return strings[strings.length - 2] + File.separator +
                     file.getName();
            }
    ...
    static void deleteAllGameModeSaves(const std::string& gameMode) {
              ...
              Path path =
                  Paths.get(ZomboidFileSystem.instance.getGameModeCacheDir());
    ...
    FileVisitResult preVisitDirectory(Path var1, BasicFileAttributes var2) {
                ...
                FileVisitResult visitFileFailed(
                    Path var1, std::ios_base::failure iOExceptionx) {
                  iOExceptionx.printStackTrace();
                  return FileVisitResult.CONTINUE;
                }

                FileVisitResult postVisitDirectory(
                    Path path, std::ios_base::failure var2) {
                  Files.delete(path);
                  return FileVisitResult.CONTINUE;
                }
              });
    ...
    static void createHordeFromTo(float spawnX, float spawnY, float targetX, float targetY, int count) {
              ZombiePopulationManager.instance.createHordeFromTo(
                  (int)spawnX, (int)spawnY, (int)targetX, (int)targetY, count);
            }
              ...
              for (int int0 = 0; int0 < count; int0++) {
                ...
                IsoGridSquare square =
                    IsoWorld.instance.CurrentCell.getGridSquare(
                        (double)Rand.Next(x, x2), (double)Rand.Next(y, y2),
                        (double)z);
                if (square != nullptr) {
              ...
              if (depth <= 1) {
                if (param instanceof KahluaTable table) {
                  ...
                  do {
                    ...
                    if (object1 != nullptr) {
                      if (object2 != nullptr) {
                        DebugLog.Lua.debugln(object0 + object1 + " : " +
                                             object2.toString());
                      }
    ...
    static void sendItemsInContainer(IsoObject obj, ItemContainer container) {
              GameServer.sendItemsInContainer(
                  obj, container == nullptr ? obj.getContainer() : container);
            }
        ...
        public static ArrayList<String> getMapFoldersForMod(String modID) {
              try {
                ...
                if (mod == nullptr) {
                  ...
                  std::string string = mod.getDir() + File.separator + "media" +
                                       File.separator + "maps";
                  File file = new File(string);
                    ...
                    try(DirectoryStream directoryStream =
                      ...
                      for (auto &path : directoryStream)
                        if (Files.isDirectory(path)) {
                          file = new File(string + File.separator +
                                          path.getFileName().toString() +
                                          File.separator + "map.info");
                          if (file.exists()) {
    ...
    static bool spawnpointsExistsForMod(const std::string& modID, const std::string& mapFolder) {
              try {
                ...
                if (mod == nullptr) {
                  ...
                  String string = mod.getDir() + File.separator + "media" +
            ...
            /**
             * Returns the OS-defined file separator. It is not generally needed
             * to use this, as most functions that expect a filepath string will
             * parse them in an OS-independent way.
    ...
    static bool fileExists(const std::string& filename) {
              ...
              File file =
                  new File(ZomboidFileSystem.instance.getString(string));
              return file.exists();
            }
    ...
    static bool serverFileExists(const std::string& filename) {
              ...
              File file =
                  new File(ZomboidFileSystem.instance.getCacheDir() +
                           File.separator + "Server" + File.separator + string);
              return file.exists();
            }
    ...
    static void* require(const std::string& f) {
              ...
              for (int int0 = 0; int0 < LuaManager.paths.size(); int0++) {
                ...
                std::string string2 =
                    ZomboidFileSystem.instance.getAbsolutePath(string1 +
                                                               string0);
                if (string2 != nullptr) {
                  return LuaManager.RunLua(
                      ZomboidFileSystem.instance.getString(string2));
                }
              }
    ...
    static SandboxOptions getSandboxOptions() {
             ...
             *
             * @param filename Path, relative to the Lua cache root, to write
             * to. '..' is not allowed.
    ...
    static DataOutputStream getFileOutput(const std::string& filename) {
              if (StringUtils.containsDoubleDot(filename)) {
                ...
                std::string string0 =
                    LuaManager.getLuaCacheDir() + File.separator + filename;
                ...
                try {
                  ...
                  Logger.getLogger(LuaManager.class.getName())
                      .log(Level.SEVERE, nullptr, fileNotFoundException);
                }
        ...
        public static List<String> getLastStandPlayerFileNames() throws IOException {
              ...
              for (File file1 : file0.listFiles()) {
                if (!file1.isDirectory() && file1.getName().endsWith(".txt")) {
                  arrayList.add(getLastStandPlayersDirectory() +
    ...
    static void deleteSandboxPreset(const std::string& name) {
             ...
             *
             * @param filename Path, relative to the Lua cache root, to read
             * from. '..' is not allowed.
             * @param createIfNull Whether to create the file if it does not
             * exist. The created file will be empty.
    ...
    static BufferedReader getFileReader(const std::string& filename, bool createIfNull) {
             ...
             *
             * @param modId ID of the target mod. If null, the path will be
             * relative to the local mods directory.
             * @param filename Path, relative to the mod's common folder, to
             * read from. '..' is not allowed.
             * @param createIfNull Whether to create the file if it does not
             * exist. The created file will be empty.
             * @return The file reader, or null if the path or mod was not
             * valid.
             */
    ...
    static BufferedReader getModFileReader(const std::string& modId, const std::string& filename, bool createIfNull) {
            ...
            static void refreshAnimSets(bool reload) {
              try {
                if (reload) {
                  ...
                  for (Asset asset :
                       AnimNodeAssetManager.instance.getAssetTable().values()) {
            ...
            /**
             * Gets a file writer for a file in a mod's directory. Note: it is
             * generally unwise to write to a mod's lua or scripts directories,
             * as this will change the checksum.
             *
             * @param modId ID of the target mod. If null, the path will be
             * relative to the local mods directory.
             * @param filename Path, relative to the mod's common folder, to
             * write to. '..' is not allowed.
             * @param createIfNull Whether to create the file if it does not
             * exist. The created file will be empty.
             * @param append Whether to open the file in append mode. If true,
             * the writer will write after the file's current contents. If
             * false, the current contents of the file will be erased.
             * @return The file writer, or null if the path or mod was not
             * valid.
             */
        ...
        public static LuaManager.GlobalObject.LuaFileWriter getModFileWriter(String modId, String filename, boolean createIfNull, boolean append) {
              if (!filename.isEmpty() &&
                ...
                if (mod == nullptr) {
                  ...
                  std::string string0 =
                      mod.getDir() + File.separator + filename;
                  string0 = string0.replace("/", File.separator);
                  string0 = string0.replace("\\", File.separator);
                  std::string string1 =
                      string0.substring(0, string0.lastIndexOf(File.separator));
                  ...
                  if (!file1.exists() && createIfNull) {
                    try {
                      ...
                      Logger.getLogger(LuaManager.class.getName())
                          .log(Level.SEVERE, nullptr, iOException0);
                    }
                  }
                  ...
                  try {
                    FileOutputStream fileOutputStream =
                        new FileOutputStream(file1, append);
                    ...
                    Logger.getLogger(LuaManager.class.getName())
                        .log(Level.SEVERE, nullptr, iOException1);
                  }
    ...
    static bool isControllerConnected(int index) {
              return index >= 0 &&
                             index <= GameWindow.GameInput.getControllerCount()
                         ? GameWindow.GameInput.getController(index) != nullptr
                         : false;
            }
    ...
    static std::string getControllerGUID(int joypad) {
              if (joypad >= 0 &&
                  joypad < GameWindow.GameInput.getControllerCount()) {
                Controller controller =
                    GameWindow.GameInput.getController(joypad);
    ...
    static int getJoypadAButton(int joypad) {
              JoypadManager.Joypad _joypad =
                  JoypadManager.instance.getFromControllerID(joypad);
    ...
    static bool wasMouseActiveMoreRecentlyThanJoypad() {
              if (IsoPlayer.players[0] == nullptr) {
                ...
                return joypad != nullptr && !joypad.isDisabled()
                           ? JoypadManager.instance.getLastActivity(
                                 joypad.getID()) < Mouse.lastActivity
                           : true;
              } else {
                int int0 = IsoPlayer.players[0].getJoypadBind();
                return int0 = = -1 ? true
                                   : JoypadManager.instance.getLastActivity(
                                         int0) < Mouse.lastActivity;
              }
            }
    ...
    static void activateJoypadOnSteamDeck() {
              if (GameWindow.ActivatedJoyPad == nullptr) {
                ...
                JoypadManager.Joypad joypad =
                    JoypadManager.instance.JoypadList.get(0);
                GameWindow.ActivatedJoyPad = joypad;
              }

              if (IsoPlayer.getInstance() != nullptr) {
                LuaEventManager.triggerEvent(
                    "OnJoypadActivate", GameWindow.ActivatedJoyPad.getID());
              } else {
                LuaEventManager.triggerEvent(
                    "OnJoypadActivateUI", GameWindow.ActivatedJoyPad.getID());
              }
            }
    ...
    static bool isJoypadConnected(int index) {
            ...
            static void addPlayerToWorld(int int0, IsoPlayer player,
                                         bool boolean0) {
              ...
              if (GameClient.bClient && int0 != 0 &&
                  player.serverPlayerIndex != 1) {
                ClientPlayerDB.getInstance().forgetPlayer(
              ...
              if (GameClient.bClient && int0 != 0 &&
                  player.serverPlayerIndex == 1) {
                player.serverPlayerIndex =
                    ClientPlayerDB.getInstance().getNextServerPlayerIndex();
              }
              ...
              if (boolean0) {
                ...
                if (profession != nullptr &&
                    !profession.getFreeRecipes().isEmpty()) {
                ...
                for (String string1 : IsoWorld.instance.getLuaTraits()) {
                  ...
                  if (trait != nullptr && !trait.getFreeRecipes().isEmpty()) {
                ...
                LuaEventManager.triggerEvent("OnNewGame", player,
    ...
    static void setPlayerJoypad(int player, int joypad, IsoPlayer playerObj, const std::string& username) {
              if (IsoPlayer.players[player] == nullptr ||
                ...
                bool boolean0 = playerObj == null;
                if (playerObj == nullptr) {
                  IsoPlayer _player = IsoPlayer.getInstance();
                  IsoWorld world = IsoWorld.instance;
                  int int0 =
                      world.getLuaPosX() + 300 * world.getLuaSpawnCellX();
                  int int1 =
                      world.getLuaPosY() + 300 * world.getLuaSpawnCellY();
                  int int2 = world.getLuaPosZ();
                  DebugLog.Lua.debugln("coop player spawning at " + int0 + "," +
                                       int1 + "," + int2);
                  playerObj =
                      new IsoPlayer(world.CurrentCell, world.getLuaPlayerDesc(),
                                    int0, int1, int2);
    ...
    static void setDebugToggleControllerPluggedIn(int index) {
        ...
        public static LuaManager.GlobalObject.LuaFileWriter getSandboxFileWriter(String filename, boolean createIfNull, boolean append) {
              if (StringUtils.containsDoubleDot(filename)) {
                ...
                std::string string0 =
                    LuaManager.getSandboxCacheDir() + File.separator + filename;
                string0 = string0.replace("/", File.separator);
                string0 = string0.replace("\\", File.separator);
                std::string string1 =
                    string0.substring(0, string0.lastIndexOf(File.separator));
    ...
    static void createWorld(const std::string& worldName) {
              if (worldName == nullptr || worldName.isEmpty()) {
                worldName = "blah";
              }
    ...
    static double ZombRand(double max) {
              if (max == 0.0) {
                ...
                return max < 0.0 ? -Rand.Next(-((long)max), Rand.randlua)
                                 : Rand.Next((long)max, Rand.randlua);
              }
            }

            /**
             * Returns a pseudorandom integer between min and max - 1. No
             * difference from ZombRand(min, max).
             *
    ...
    static bool isSoundPlaying(void* sound) {
              return sound instanceof
                  Double ? FMODManager.instance.isPlaying(
                               ((Double)sound).longValue())
                         : false;
            }
    ...
    static std::string getFilenameOfCallframe(LuaCallFrame c) {
              return c.closure == nullptr ? nullptr
                                          : c.closure.prototype.filename;
            }
    ...
    static int getLocalVarCount(Coroutine c) {
              ...
              return luaCallFrame = =
                         null ? 0 : luaCallFrame.LocalVarNames.size();
            }
    ...
    static void showFolderInDesktop(const std::string& folder) {
            ...
            /**
             * Gets the list of currently activated mods. Remember that in B42+,
             * mod ids are prefixed with a \ character.
             */
    ...
    static void saveModsFile() {
            ...
            static void deleteSavefileFilesMatching(File file,
              ...
              try(DirectoryStream directoryStream =
                      Files.newDirectoryStream(file.toPath(), filter)) {
                for (auto &path : directoryStream)
                  ...
                  if (file.exists() && file.isDirectory()) {
                    ...
                    case "DeleteChunkDataXYBin":
                      deleteSavefileFilesMatching(
                          file, "chunkdata_[0-9]+_[0-9]+\\.bin");
                      break;
                    case "DeleteMapXYBin":
                      deleteSavefileFilesMatching(file,
                                                  "map_[0-9]+_[0-9]+\\.bin");
                      break;
                    ...
                    case "WriteModsDotTxt":
                      ...
                      activeModsFile.write(string + File.separator + "mods.txt",
                                           activeMods);
                      break;
                    default:
    ...
    static int getLocalVarStack(Coroutine c, int n) {
              LuaCallFrame luaCallFrame = c.currentCallFrame();
              return (Integer)luaCallFrame.LocalVarToStackMap.get(
                  luaCallFrame.LocalVarNames.get(n));
            }
              ...
              synchronized(IsoSpriteManager.instance.NamedMap) {
                ...
                if (sprite != nullptr) {
                  ...
                  CellLoader.DoTileObjectCreation(
                      sprite, sprite.getType(), square,
                      IsoWorld.instance.CurrentCell, int0, int1, int2, tile);
                }
              }
            }
    ...
    static void drawOverheadMap(UIElement ui, float zoom, float xpos, float ypos) {
              IngameState.renderDebugOverhead2(
                  getCell(), 0, zoom, ui.getAbsoluteX().intValue(),
                  ui.getAbsoluteY().intValue(), xpos, ypos,
                  ui.getWidth().intValue(), ui.getHeight().intValue());
            }
    ...
    static int getGameSpeed() {
              return UIManager.getSpeedControls() != nullptr
    ...
    static KahluaTable getSaveInfo(const std::string& saveDir) {
              ...
              file = new File(ZomboidFileSystem.instance.getSaveDir() +
                              File.separator + saveDir + File.separator +
                              "map_ver.bin");
              if (file.exists()) {
              ...
              std::string string3 = ZomboidFileSystem.instance.getSaveDir() +
                                    File.separator + saveDir + File.separator +
                                    "mods.txt";
              ...
              std::string string4 = ZomboidFileSystem.instance.getSaveDir() +
                                    File.separator + saveDir;
              table0.rawset("playerAlive",
                            PlayerDBHelper.isPlayerAlive(string4, 1));
              KahluaTable table1 = LuaManager.platform.newTable();
    ...
    static bool renameSaveFile(const std::string& gameMode, const std::string& oldName, const std::string& newName) {
              if (gameMode == nullptr || gameMode.contains("/") ||
                ...
                if (string == newName) && !string.startsWith(".") && !string.endsWith(".")) {
                    if (!new File(
                             ZomboidFileSystem.instance.getSaveDirSub(gameMode))
                             .exists()) {
                      return false;
                    } else {
                      Path path0 = FileSystems.getDefault().getPath(
                          ZomboidFileSystem.instance.getSaveDirSub(
                              gameMode + File.separator + oldName));
                      Path path1 = FileSystems.getDefault().getPath(
                          ZomboidFileSystem.instance.getSaveDirSub(
                              gameMode + File.separator + string));
    ...
    static KahluaTable getZombieInfo(IsoZombie zombie) {
              ...
              if (zombie == nullptr) {
                ...
                table.rawset("PathLength",
                             zombie.getPathFindBehavior2().getPathLength());
                table.rawset(
                    "TargetLength",
                    Math.sqrt(
                        (zombie.x -
                         zombie.getPathFindBehavior2().getTargetX()) *
                            (zombie.x -
                             zombie.getPathFindBehavior2().getTargetX()) +
                        (zombie.y -
                         zombie.getPathFindBehavior2().getTargetY()) *
                            (zombie.y -
                             zombie.getPathFindBehavior2().getTargetY())));
                table.rawset("clientActionState", zombie.getActionStateName());
                table.rawset("clientAnimationState",
                             zombie.getAnimationStateName());
                table.rawset("finderProgress",
                             zombie.getFinder().progress.name());
                table.rawset("usePathFind",
                             Boolean.toString(zombie.networkAI.usePathFind));
    ...
    static KahluaTable getPlayerInfo(IsoPlayer player) {
              ...
              if (player == nullptr) {
                ...
                table.rawset("PathLength",
                             player.getPathFindBehavior2().getPathLength());
                table.rawset(
                    "TargetLength",
                    Math.sqrt(
                        (player.x -
                         player.getPathFindBehavior2().getTargetX()) *
                            (player.x -
                             player.getPathFindBehavior2().getTargetX()) +
                        (player.y -
                         player.getPathFindBehavior2().getTargetY()) *
                            (player.y -
                             player.getPathFindBehavior2().getTargetY())));
                table.rawset("clientActionState", player.getActionStateName());
                table.rawset("clientAnimationState",
                             player.getAnimationStateName());
                table.rawset("finderProgress",
                             player.getFinder().progress.name());
                table.rawset("usePathFind",
                             Boolean.toString(player.networkAI.usePathFind));
                return table;
              }
            }
    ...
    static KahluaTable getVehicleInfo(BaseVehicle vehicle) {
              if (vehicle == nullptr) {
                ...
                table.rawset("frontEndDurability",
                             Integer.toString(vehicle.frontEndDurability));
                table.rawset("rearEndDurability",
                             Integer.toString(vehicle.rearEndDurability));
                table.rawset(
                    "currentFrontEndDurability",
                    Integer.toString(vehicle.currentFrontEndDurability));
                table.rawset(
                    "currentRearEndDurability",
                    Integer.toString(vehicle.currentRearEndDurability));
                ...
                table.rawset(
                    "gas_capacity",
                    vehicle.getPartById("GasTank").getContainerCapacity());
                ...
                if (part0 != nullptr) {
                  ...
                  table.rawset("doorleft_lockbroken",
                               part0.getDoor().isLockBroken());
                  ...
                  if (vehicleWindow0 != nullptr) {
                    ...
                    table.rawset("windowleft_health",
                                 vehicleWindow0.getHealth());
                  }
                }
                ...
                if (part0 != nullptr) {
                  ...
                  table.rawset("doorright_lockbroken",
                               part1.getDoor().isLockBroken());
                  ...
                  if (vehicleWindow1 != nullptr) {
                    ...
                    table.rawset("windowright_health",
                                 vehicleWindow1.getHealth());
                  }
                }
                ...
                if (vehicle.getPartById("Heater") != nullptr) {
                  ...
                  void *object =
                      vehicle.getPartById("Heater").getModData().rawget(
                          "active");
                  if (object == nullptr) {
        ...
        public static ArrayList<String> getLotDirectories() {
              return IsoWorld.instance.MetaGrid != nullptr
                         ? IsoWorld.instance.MetaGrid.getLotDirectories()
                         : nullptr;
            }
    ...
    static bool isXBOXController() {
              for (int int0 = 0;
                   int0 < GameWindow.GameInput.getControllerCount(); int0++) {
                Controller controller =
                    GameWindow.GameInput.getController(int0);
                if (controller != nullptr &&
                    controller.getGamepadName().contains("XBOX 360")) {
                  return true;
                }
              }
            ...
            /**
             * Sends a command to the server, triggering the OnClientCommand
             * event on the server. Does nothing if called on the server.
             *
             * @param module Module of the command. It is conventional to use
             * the name of your mod as the module for all of your commands.
             * @param command Name of the command.
             * @param args Arguments to pass to the server. Non-POD elements of
    ...
    static void sendClientCommand(const std::string& module, const std::string& command, KahluaTable args) {
              if (GameClient.bClient && GameClient.bIngame) {
                ...
                if (GameServer.bServer) {
                  throw new IllegalStateException(
                      "can't call this function on the server");
                }

                SinglePlayerClient.sendClientCommand(nullptr, module, command,
                                                     args);
              }
            }
             ...
             * event on the server. Does nothing if called on the server.
             *
             * @param player The local player to associate the command with. If
             * the player is not local, no command will be sent.
             * @param module Module of the command. It is conventional to use
             * the name of your mod as the module for all of your commands.
             * @param command Name of the command.
             * @param args Arguments to pass to the server. Non-POD elements of
    ...
    static void sendClientCommand(IsoPlayer player, const std::string& module, const std::string& command, KahluaTable args) {
              if (player != nullptr && player.isLocalPlayer()) {
                if (GameClient.bClient && GameClient.bIngame) {
                  ...
                  SinglePlayerClient.sendClientCommand(player, module, command,
                                                       args);
                }
              }
            }
            ...
            /**
             * Sends a command to all clients, triggering the OnServerCommand
             * event on every client. Does nothing if called on the client.
             *
             * @param module Module of the command. It is conventional to use
             * the name of your mod as the module for all of your commands.
             * @param command Name of the command.
             * @param args Arguments to pass to the clients. Non-POD elements of
             * the table will be lost.
             */
    ...
    static void sendServerCommand(const std::string& module, const std::string& command, KahluaTable args) {
            ...
            /**
             * Sends a command to a specific client, triggering the
             * OnServerCommand event on the client. Does nothing if called on
             * the client.
             *
             * @param player The player to send the command to. Only that
             * player's client will receive the command.
             * @param module Module of the command. It is conventional to use
             * the name of your mod as the module for all of your commands.
             * @param command Name of the command.
             * @param args Arguments to pass to the client. Non-POD elements of
             * the table will be lost.
             */
            name = "sendServerCommand",
            global = true
    ...
    static bool canModifyPlayerScoreboard() {
              return GameClient.bClient &&
                     GameClient.connection.accessLevel != 1;
            }
              ...
              if (GameClient.bClient) {
                return GameClient.connection.accessLevel == 1
                           ...
                           : GameClient.connection.accessLevel ==
                                 PlayerType.fromString(accessLevel);
              } else {
                return false;
              }
            }
    ...
    static void sendCataplasm(int onlineID, int i, float plantainFactor, float comfreyFactor, float garlicFactor) {
              GameClient.instance.sendCataplasm(onlineID, i, plantainFactor,
                                                comfreyFactor, garlicFactor);
            }
    ...
    static void sendStitch(IsoGameCharacter wielder, IsoGameCharacter target, BodyPart bodyPart, InventoryItem item, bool doIt) {
              GameClient.instance.sendStitch(wielder, target, bodyPart, item,
                                             doIt);
            }
    ...
    static void sendDisinfect(IsoGameCharacter wielder, IsoGameCharacter target, BodyPart bodyPart, InventoryItem alcohol) {
              GameClient.instance.sendDisinfect(wielder, target, bodyPart,
                                                alcohol);
            }
    ...
    static void sendRemoveGlass(IsoGameCharacter wielder, IsoGameCharacter target, BodyPart bodyPart, bool handPain) {
              GameClient.instance.sendRemoveGlass(wielder, target, bodyPart,
                                                  handPain);
            }
    ...
    static void sendCleanBurn(IsoGameCharacter wielder, IsoGameCharacter target, BodyPart bodyPart, InventoryItem bandage) {
              GameClient.instance.sendCleanBurn(wielder, target, bodyPart,
                                                bandage);
            }
    ...
    static bool isCoopHost() {
              return GameClient.connection != nullptr &&
    ...
    static bool canInviteFriends() {
              return GameClient.bClient && SteamUtils.isSteamModeEnabled()
                         ? CoopMaster.instance.isRunning() ||
                               !GameClient.bCoopInvite
                         : false;
            }
    ...
    static bool isValidSteamID(const std::string& s) {
              return s != nullptr && !s.isEmpty() ? SteamUtils.isValidSteamID(s)
                                                  : false;
            }
    ...
    static std::string getCurrentUserSteamID() {
              return SteamUtils.isSteamModeEnabled() && !GameServer.bServer
    ...
    static void activateSteamOverlayToWorkshop() {
              if (SteamUtils.isOverlayEnabled()) {
                SteamFriends.ActivateGameOverlayToWebPage(
                    "steam://url/SteamWorkshopPage/108600");
              }
            }
    ...
    static void activateSteamOverlayToWorkshopUser() {
              if (SteamUtils.isOverlayEnabled()) {
                SteamFriends.ActivateGameOverlayToWebPage(
                    "steam://url/SteamIDCommunityFilesPage/" +
                    SteamUser.GetSteamIDString() + "/108600");
              }
            }
    ...
    static void activateSteamOverlayToWorkshopItem(const std::string& itemID) {
              if (SteamUtils.isOverlayEnabled() &&
                ...
                SteamFriends.ActivateGameOverlayToWebPage(
                    "steam://url/CommunityFilePage/" + itemID);
              }
            }
    ...
    static Texture getSteamAvatarFromUsername(const std::string& username) {
        ...
        public static ArrayList<SteamWorkshopItem> getSteamWorkshopStagedItems() {
              return SteamUtils.isSteamModeEnabled()
                         ? SteamWorkshop.instance.loadStagedItems()
                         : nullptr;
            }
        ...
        public static ArrayList<String> getSteamWorkshopItemIDs() {
              if (SteamUtils.isSteamModeEnabled()) {
                std::vector arrayList = new ArrayList();
                String[] strings =
                    SteamWorkshop.instance.GetInstalledItemFolders();
                if (strings == nullptr) {
                  return arrayList;
                } else {
                  for (int int0 = 0; int0 < strings.length; int0++) {
                    std::string string =
                        SteamWorkshop.instance.getIDFromItemInstallFolder(
                            strings[int0]);
                    if (string != nullptr) {
    ...
    static bool isFloatingGamepadTextInputVisible() {
              return SteamUtils.isSteamModeEnabled()
                         ? SteamUtils.isFloatingGamepadTextInputVisible()
                         : false;
            }
    ...
    static void querySteamWorkshopItemDetails(ArrayList<String> itemIDs, LuaClosure functionObj, void* arg1) {
              if (itemIDs == nullptr || functionObj == nullptr) {
                ...
                if (arg1 == nullptr) {
                  LuaManager.caller.pcall(LuaManager.thread, functionObj,
                                          "Completed",
                                          std::make_unique<ArrayList>());
                } else {
                  LuaManager.caller.pcall(LuaManager.thread, functionObj, arg1,
                                          "Completed",
                                          std::make_unique<ArrayList>());
                }
              } else {
                new LuaManager.GlobalObject.ItemQuery(itemIDs, functionObj,
                                                      arg1);
              }
            }
    ...
    static KahluaTable getPublicServersList() {
              ...
              if (!SteamUtils.isSteamModeEnabled() &&
                ...
                return table;
              } else if (System.currentTimeMillis() - timeLastRefresh <
                         60000L) {
                return table;
              } else {
                std::vector arrayList = new ArrayList();
                ...
                try {
                  if (getSteamModeActive()) {
                    ...
                    for (auto &gameServerDetails : list)
                      Server server0 = new Server();
                    server0.setName(gameServerDetails.name);
                    server0.setDescription(gameServerDetails.gameDescription);
                    server0.setSteamId(
                        Long.toString(gameServerDetails.steamId));
                    server0.setPing(Integer.toString(gameServerDetails.ping));
                    server0.setPlayers(
                        Integer.toString(gameServerDetails.numPlayers));
                    server0.setMaxPlayers(
                        Integer.toString(gameServerDetails.maxPlayers));
                  ...
                  for (int int0 = 0; int0 < nodeList0.getLength(); int0++) {
                    ...
                    if (node.getNodeType() == 1) {
                      ...
                      server1.setName(element.getElementsByTagName("name")
                                          .item(0)
                                          .getTextContent());
                      if (element.getElementsByTagName("desc").item(0) !=
                              nullptr &&
                          !"" == element.getElementsByTagName("desc")
                                     .item(0)
                                     .getTextContent())) {
                          server1.setDescription(
                              element.getElementsByTagName("desc")
                                  .item(0)
                                  .getTextContent());
                        }

                      server1.setIp(element.getElementsByTagName("ip")
                                        .item(0)
                                        .getTextContent());
                      server1.setPort(element.getElementsByTagName("port")
                                          .item(0)
                                          .getTextContent());
                      server1.setPlayers(element.getElementsByTagName("players")
                                             .item(0)
                                             .getTextContent());
                      server1.setMaxPlayers(
                          element.getElementsByTagName("maxPlayers")
                              .item(0)
                              .getTextContent());
                      if (element.getElementsByTagName("version") != nullptr &&
                          element.getElementsByTagName("version").item(0) !=
                              nullptr) {
                        server1.setVersion(
                            element.getElementsByTagName("version")
                                .item(0)
                                .getTextContent());
                      }
                                ...
                                int integer = Integer.parseInt(
                                    element.getElementsByTagName("lastUpdate")
                                        .item(0)
                                        .getTextContent());
                                if (element.getElementsByTagName("mods").item(
                                        0) != nullptr &&
                                    !"" == element.getElementsByTagName("mods")
                                               .item(0)
                                               .getTextContent())) {
                                    server1.setMods(
                                        element.getElementsByTagName("mods")
                                ...
                                server1.setLastUpdate(
                                    new Double(Math.floor((getTimestamp() -
                                                           integer.intValue()) /
                                                          60L))
                                        .intValue());
                                NodeList nodeList1 =
    ...
    static Server steamGetInternetServerDetails(int index) {
              if (!ServerBrowser.IsRefreshing()) {
                ...
                GameServerDetails gameServerDetails =
                    ServerBrowser.GetServerDetails(index);
                if (gameServerDetails == nullptr) {
                  return null;
                } else if (gameServerDetails.tags.contains("hidden") ||
                           gameServerDetails.tags.contains("hosted")) {
                  return null;
                } else if (!gameServerDetails.tags.contains("hidden") &&
                           !gameServerDetails.tags.contains("hosted")) {
                  ...
                  server.setPasswordProtected(
                      gameServerDetails.passwordProtected);
                  return server;
                } else {
                  return null;
                }
    ...
    static void testSound() {
              ...
              float float2 = 50.0F;
              float float3 = 1.0F;
              AmbientStreamManager.Ambient ambient =
                  new AmbientStreamManager.Ambient("Meta/House Alarm", int1,
                                                   int2, float2, float3);
              ambient.trackMouse = true;
              ((AmbientStreamManager)AmbientStreamManager.instance)
                  .ambient.add(ambient);
            }
    ...
    static void renderIsoCircle(float x, float y, float z, float radius, float r, float g, float b, float a, int thickness) {
              ...
              for (double double1 = 0.0; double1 < Math.PI * 2;
                   double1 += double0) {
                ...
                LineDrawer.drawLine(float4, float5, float6, float7, r, g, b, a,
                                    thickness);
              }
            }
    ...
    static void sendSwitchSeat(BaseVehicle vehicle, IsoGameCharacter chr, int seatFrom, int seatTo) {
              if (GameClient.bClient) {
                VehicleManager.instance.sendSwitchSeat(
                    GameClient.connection, vehicle, chr, seatFrom, seatTo);
              }
            }
    ...
    static IsoDeadBody createRandomDeadBody(IsoGridSquare square, int blood) {
              if (square == nullptr) {
                ...
                ItemPickerJava.fillContainerType(
                    itemPickerRoom, deadBody.getContainer(),
                    humanCorpse.isFemale() ? "inventoryfemale"
                                           : "inventorymale",
                    nullptr);
                return deadBody;
              }
    ...
    void addZombieSitting(int x, int y, int z) {
              IsoGridSquare square =
                  IsoCell.getInstance().getGridSquare(x, y, z);
              if (square != nullptr) {
                VirtualZombieManager.instance.choices.clear();
                VirtualZombieManager.instance.choices.add(square);
                IsoZombie zombie0 =
                ...
                ZombiePopulationManager.instance.sitAgainstWall(zombie0,
                                                                square);
              }
            }
    ...
    void addZombiesEating(int x, int y, int z, int totalZombies, bool skeletonBody) {
              ...
              if (square != nullptr) {
                ...
                VirtualZombieManager.instance.createEatingZombies(deadBody,
                                                                  totalZombies);
              }
            }
        ...
        public ArrayList<IsoZombie> addZombiesInOutfitArea(int x1, int y1, int x2, int y2, int z, int totalZombies, String outfit, Integer femaleChance) {
              ...
              for (int int0 = 0; int0 < totalZombies; int0++) {
                arrayList.addAll(addZombiesInOutfit(Rand.Next(x1, x2),
                                                    Rand.Next(y1, y2), z, 1,
                                                    outfit, femaleChance));
              }
              ...
              if (IsoWorld.getZombiesDisabled()) {
                ...
                if (square == nullptr) {
                  ...
                  ZombieSpawnRecorder.instance.record(
                      arrayList, LuaManager.GlobalObject.class.getSimpleName());
                  return arrayList;
                }
              }
            }
        ...
        public ArrayList<IsoZombie> addZombiesInBuilding(BuildingDef def, int totalZombies, String outfit, RoomDef room, Integer femaleChance) {
              ...
              if (IsoWorld.getZombiesDisabled()) {
                ...
                for (int int3 = 0; int3 < int2; int3++) {
                  IsoGridSquare square =
                      RandomizedBuildingBase.getRandomSpawnSquare(room);
                  if (square == nullptr) {
                    break;
                  }
                ...
                ZombieSpawnRecorder.instance.record(
                    arrayList, this.getClass().getSimpleName());
                return arrayList;
              }
            }
    ...
    static BaseVehicle addVehicleDebug(const std::string& scriptName, IsoDirections dir, int skinIndex, IsoGridSquare sq) {
              ...
              BaseVehicle vehicle =
                  new BaseVehicle(IsoWorld.instance.CurrentCell);
              ...
              vehicle.setDir(dir);
              float float0 =
                  dir.toAngle() + (float)Math.PI + Rand.Next(-0.2F, 0.2F);
                ...
                if (arrayList.isEmpty()) {
                  ...
                  if (StringUtils.isNullOrWhitespace(script)) {
                    VehicleScript vehicleScript =
                        PZArrayUtil.pickRandom(arrayList);
                  ...
                  if (IsoChunk.doSpawnedVehiclesInInvalidPosition(vehicle)) {
                    ...
                    DebugLog.Lua.error(
                        "ERROR: I can not spawn the vehicle. Invalid position. "
    ...
    static void attachTrailerToPlayerVehicle(int playerIndex) {
              ...
              if (vehicle0 == nullptr) {
                vehicle0 =
                    addVehicleDebug("Base.OffRoad", IsoDirections.N, 0, square);
                vehicle0.repair();
                player.getInventory().AddItem(vehicle0.createVehicleKey());
              }

              square = IsoWorld.instance.CurrentCell.getGridSquare(
                  square.x, square.y + 5, square.z);
              BaseVehicle vehicle1 =
                  addVehicleDebug("Base.Trailer", IsoDirections.N, 0, square);
              vehicle1.repair();
              vehicle0.addPointConstraint(player, vehicle1, "trailer",
                                          "trailer");
            }
    ...
    static void addAllVehicles() {
              addAllVehicles(
                  vehicleScript->!vehicleScript.getName().contains("Smashed") &&
    ...
    static void addAllBurntVehicles() {
              addAllVehicles(
                  vehicleScript->vehicleScript.getName().contains("Burnt"));
            }
    ...
    static void addAllSmashedVehicles() {
            ...
            static void addAllVehicles(Predicate<VehicleScript> predicate) {
              std::vector arrayList =
                  ScriptManager.instance.getAllVehicleScripts();
              Collections.sort(arrayList,
                               Comparator.comparing(VehicleScript::getName));
              float float0 = IsoWorld.instance.CurrentCell.ChunkMap[0]
                                 .getWorldXMinTiles() +
                             5;
              ...
              for (int int0 = 0; int0 < arrayList.size(); int0++) {
                VehicleScript vehicleScript =
                    (VehicleScript)arrayList.get(int0);
                if (vehicleScript.getModel() != nullptr &&
                  ...
                  if (IsoChunk.doSpawnedVehiclesInInvalidPosition(vehicle)) {
                    ...
                    DebugLog.Lua.warn(vehicleScript.getName() +
                                      " not spawned, position invalid");
                  }

                  float0 += 4.0F;
                  if (float0 > IsoWorld.instance.CurrentCell.ChunkMap[0]
                                       .getWorldXMaxTiles() -
                                   5) {
                    float0 = IsoWorld.instance.CurrentCell.ChunkMap[0]
                                 .getWorldXMinTiles() +
                             5;
                    float1 += 8.0F;
                  }
    ...
    static BaseVehicle addPhysicsObject() {
              ...
              int int0 = Bullet.addPhysicsObject(getPlayer().getX(),
                                                 getPlayer().getY());
              ...
              WorldSimulation.instance.physicsObjectMap.put(int0,
                                                            pushableObject);
              return null;
            }
    ...
    static void reloadSoundFiles() {
              try {
                for (String string :
                     ZomboidFileSystem.instance.ActiveFileMap.keySet()) {
    ...
    static void reloadVehicles() {
              try {
                for (String string0 :
                     ScriptManager.instance.scriptsWithVehicleTemplates) {
                  ScriptManager.instance.LoadFile(string0, true);
                }

                for (String string1 :
                     ScriptManager.instance.scriptsWithVehicles) {
                  ScriptManager.instance.LoadFile(string1, true);
                }

                BaseVehicle.LoadAllVehicleTextures();

                for (BaseVehicle vehicle :
                     IsoWorld.instance.CurrentCell.vehicles) {
    ...
    static void reloadEngineRPM() {
              try {
                ScriptManager.instance.LoadFile(
                    ZomboidFileSystem.instance.getString(
                        "media/scripts/vehicles/engine_rpm.txt"),
                    true);
              } catch (Exception exception) {
                exception.printStackTrace();
              }
    ...
    static std::string proceedPM(const std::string& command) {
              ...
              if (matcher.matches()) {
                ...
                ChatManager.getInstance().sendWhisperMessage(string,
                                                             (String)object);
                return string;
              } else {
                ChatManager.getInstance().addMessage(
                    "Error", getText("IGUI_Commands_Whisper"));
                return "";
              }
            }
    ...
    static void showWrongChatTabMessage(int actualTabID, int rightTabID, const std::string& chatCommand) {
              ...
              std::string string1 =
                  ChatManager.getInstance().getTabName((short)rightTabID);
              std::string string2 = Translator.getText(
                  "UI_chat_wrong_tab", string0, string1, chatCommand);
    ...
    static void reloadVehicleTextures(const std::string& scriptName) {
              ...
              if (vehicleScript == nullptr) {
                ...
                for (int int0 = 0; int0 < vehicleScript.getSkinCount();
                     int0++) {
                  ...
                  if (skin.textureRust != nullptr) {
                    Texture.reload("media/textures/" + skin.textureRust +
                                   ".png");
                  }

                  if (skin.textureMask != nullptr) {
                    Texture.reload("media/textures/" + skin.textureMask +
                                   ".png");
                  }

                  if (skin.textureLights != nullptr) {
                    Texture.reload("media/textures/" + skin.textureLights +
                                   ".png");
                  }

                  if (skin.textureDamage1Overlay != nullptr) {
                    Texture.reload("media/textures/" +
                                   skin.textureDamage1Overlay + ".png");
                  }

                  if (skin.textureDamage1Shell != nullptr) {
                    Texture.reload("media/textures/" +
                                   skin.textureDamage1Shell + ".png");
                  }

                  if (skin.textureDamage2Overlay != nullptr) {
                    Texture.reload("media/textures/" +
                                   skin.textureDamage2Overlay + ".png");
                  }

                  if (skin.textureDamage2Shell != nullptr) {
                    Texture.reload("media/textures/" +
                                   skin.textureDamage2Shell + ".png");
                  }

                  if (skin.textureShadow != nullptr) {
                    Texture.reload("media/textures/" + skin.textureShadow +
                                   ".png");
                  }
                }
              }
    ...
    static ErosionMain getErosion() {
        ...
        public static ArrayList<String> getAllOutfits(boolean female) {
              ...
              if (OutfitManager.instance == nullptr) {
                ...
                for (Outfit outfit :
                     female ? OutfitManager.instance.m_FemaleOutfits
                            : OutfitManager.instance.m_MaleOutfits) {
        ...
        public static ArrayList<String> getAllHairStyles(boolean female) {
              ...
              if (HairStyles.instance == nullptr) {
                ...
                std::vector arrayList1 =
                    new ArrayList<>(female ? HairStyles.instance.m_FemaleStyles
                                           : HairStyles.instance.m_MaleStyles);
                arrayList1.sort((hairStyle0, hairStyle1)->{
                  if (hairStyle0.name.isEmpty()) {
                    ...
                    std::string string0 =
                        getText("IGUI_Hair_" + hairStyle0.name);
                    std::string string1 =
                        getText("IGUI_Hair_" + hairStyle1.name);
                    return string0.compareTo(string1);
                  }
                });

                for (auto &hairStyle : arrayList1)
    ...
    static BeardStyles getBeardStylesInstance() {
        ...
        public static ArrayList<String> getAllBeardStyles() {
              ...
              if (BeardStyles.instance == nullptr) {
                ...
                std::vector arrayList1 =
                    new ArrayList<>(BeardStyles.instance.m_Styles);
                arrayList1.sort((beardStyle0, beardStyle1)->{
                  if (beardStyle0.name.isEmpty()) {
                    ...
                    std::string string0 =
                        getText("IGUI_Beard_" + beardStyle0.name);
                    std::string string1 =
                        getText("IGUI_Beard_" + beardStyle1.name);
                    return string0.compareTo(string1);
                  }
                });

                for (auto &beardStyle : arrayList1)
              ...
              if (item != nullptr && ClothingDecals.instance != nullptr) {
                ...
                if (clothingItem == nullptr) {
                  ...
                  if (StringUtils.isNullOrWhitespace(string)) {
                    ...
                    ClothingDecalGroup clothingDecalGroup =
                        ClothingDecals.instance.FindGroup(string);
    ...
    void addSound(IsoObject source, int x, int y, int z, int radius, int volume) {
              WorldSoundManager.instance.addSound(source, x, y, z, radius,
                                                  volume);
            }
    ...
    std::string checkServerName(const std::string& name) {
              ...
              return !StringUtils.isNullOrEmpty(string)
                         ? Translator.getText("UI_BadWordCheck", string)
                         : nullptr;
            }

            /**
             * Draws an item's model in the world. Only works when certain
             * render state is set.
             *
    ...
    void Render3DItem(InventoryItem item, IsoGridSquare sq, float xoffset, float yoffset, float zoffset, float rotation) {
              WorldItemModelDrawer.renderMain(item, sq, xoffset, yoffset,
                                              zoffset, 0.0F, rotation);
            }
    ...
    double getAverageFSP() {
              ...
              if (!PerformanceSettings.isUncappedFPS()) {
                float0 =
                    Math.min(float0, (float)PerformanceSettings.getLockFPS());
              }
    ...
    static void createItemTransaction(InventoryItem item, ItemContainer src, ItemContainer dst) {
              if (GameClient.bClient && item != nullptr) {
                int int0 = Optional.ofNullable(src)
                               .map(ItemContainer::getContainingItem)
                               .map(InventoryItem::getID)
                               .orElse(-1);
                int int1 = Optional.ofNullable(dst)
                               .map(ItemContainer::getContainingItem)
                               .map(InventoryItem::getID)
                               .orElse(-1);
                ItemTransactionManager.createItemTransaction(item.getID(), int0,
                                                             int1);
              }
            }
    ...
    static void removeItemTransaction(InventoryItem item, ItemContainer src, ItemContainer dst) {
              if (GameClient.bClient && item != nullptr) {
                ...
                int int1 = Optional.ofNullable(dst)
                               ...
                               .map(InventoryItem::getID)
                               .orElse(-1);
                ItemTransactionManager.removeItemTransaction(item.getID(), int0,
                                                             int1);
              }
            }
    ...
    static bool isItemTransactionConsistent(InventoryItem item, ItemContainer src, ItemContainer dst) {
              if (GameClient.bClient && item != nullptr) {
                ...
                int int1 = Optional.ofNullable(dst)
                               ...
                               .map(InventoryItem::getID)
                               .orElse(-1);
                return ItemTransactionManager.isConsistent(item.getID(), int0,
                                                           int1);
              } else {
                return true;
              }
    ...
    static void checkModsNeedUpdate(UdpConnection udpConnection) {
              ...
              if (SteamUtils.isSteamModeEnabled() && isServer()) {
                ...
                new LuaManager.GlobalObject.ItemQueryJava(arrayList,
                                                          udpConnection);
              }
            }
    ...
    static void timSort(KahluaTable table, void* functionObject) {
              KahluaTableImpl kahluaTableImpl =
                  Type.tryCastTo(table, KahluaTableImpl.class);
              if (kahluaTableImpl != nullptr && kahluaTableImpl.len() >= 2 &&
          ...
          private
            static final class ItemQuery implements ISteamWorkshopCallback {
              LuaClosure functionObj;
              void *arg1;
              long handle;

            public
              ItemQuery(ArrayList<String> arrayList, LuaClosure luaClosure,
                        void *object) {
                ...
                for (int int1 = 0; int1 < arrayList.size(); int1++) {
                  long long0 = SteamUtils.convertStringToSteamID(
                ...
                this.handle =
                    SteamWorkshop.instance.CreateQueryUGCDetailsRequest(longs,
                                                                        this);
                if (this.handle == 0L) {
                  SteamWorkshop.instance.RemoveCallback(this);
                  if (object == nullptr) {
                    LuaManager.caller.pcall(LuaManager.thread, luaClosure,
                                            "NotCompleted");
                  } else {
                    LuaManager.caller.pcall(LuaManager.thread, luaClosure,
                                            object, "NotCompleted");
                ...
                if (long0 == this.handle) {
                  ...
                  for (int int0 = 0; int0 < int1; int0++) {
                    SteamUGCDetails steamUGCDetails =
                        SteamWorkshop.instance.GetQueryUGCResult(long0, int0);
                  ...
                  if (this.arg1 == nullptr) {
                    LuaManager.caller.pcall(LuaManager.thread, this.functionObj,
                                            "Completed", arrayList);
                  } else {
                    LuaManager.caller.pcall(LuaManager.thread, this.functionObj,
                                            this.arg1, "Completed", arrayList);
                  }
                }
              }
              ...
              void onItemQueryNotCompleted(long long0, int var3) {
                if (long0 == this.handle) {
                  ...
                  if (this.arg1 == nullptr) {
                    ...
                    LuaManager.caller.pcall(LuaManager.thread, this.functionObj,
                                            this.arg1, "NotCompleted");
                  }
                }
              }
            }

            private static final class ItemQueryJava implements
                ISteamWorkshopCallback {
              long handle;
              UdpConnection connection;

            public
              ItemQueryJava(ArrayList<String> arrayList,
                  ...
                  for (int int0 = 0; int0 < int1; int0++) {
                    ...
                    if (steamUGCDetails != nullptr) {
                      ...
                      if (SteamWorkshopItem.ItemState.Installed.and (long2) &&
                          ...
                          steamUGCDetails.getTimeUpdated() !=
                              SteamWorkshop.instance.GetItemInstallTimeStamp(
                                  long1)) {
                        long2 |=
                            SteamWorkshopItem.ItemState.NeedsUpdate.getValue();
                      }
            ...
            private static final class TimSortComparator implements
                Comparator<Object> {
              void *comp;
    ...
    return 0;
                  ...
                  bool boolean0 =
                      LuaManager.thread.pcallBoolean(this.comp, arg0, arg1);
