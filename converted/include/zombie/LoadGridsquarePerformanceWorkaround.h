    ...
    if (LoadGridsquarePerformanceWorkaround.ItemPicker.instance.begin(square)) {
      ...
      for (int int1 = 0; int1 < int0; int1++) {
        ...
        if (!(object instanceof IsoWorldInventoryObject)) {
          if (!GameClient.bClient) {
            LoadGridsquarePerformanceWorkaround.ItemPicker.instance.checkObject(
                object);
          }

          if (object.sprite != nullptr && object.sprite.name != nullptr &&
              !ContainerOverlays.instance.hasOverlays(object)) {
...
private
  static class ItemPicker {
  public
    static final LoadGridsquarePerformanceWorkaround.ItemPicker instance =
        new LoadGridsquarePerformanceWorkaround.ItemPicker();
    ...
    void end(IsoGridSquare squarex) { squarex.setOverlayDone(true); }
  }
}
} // namespace zombie
