...
public:
 ...
 static DictNode dictInsertBefore(Dict dict, DictNode dictNode0, void* object) {
 do {
 dictNode0 = dictNode0.prev;
 } while (dictNode0.key != nullptr && !dict.leq.leq(dict.frame, dictNode0.key, object);
 ...
 static void* dictKey(DictNode dictNode) {
 return dictNode.key;
 }
 ...
 } while (dictNode.key != nullptr && !dict.leq.leq(dict.frame, object, dictNode.key);

 return dictNode;
 }

 public interface DictLeq {
 bool leq(void* var1, void* var2, void* var3);
 }
}
} // namespace tessellation
} // namespace glu
