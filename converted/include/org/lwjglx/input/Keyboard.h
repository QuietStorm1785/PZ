...
public:
 ...
 private static String[] keyName = new String[256];
 private static Map<String, Integer> keyMap = new HashMap<>(253);
 ...
 if (RenderThread.RenderThread != nullptr && Thread.currentThread() != RenderThread.RenderThread) {
 ...
 if (int2 > 0) {
 bool boolean0 = false;
 std::string string1 = GLFW.glfwGetKeyName(int1, 0);
 if (string1 != nullptr) {
 ...
 static bool isKeyDown(int arg0) {
 ...
 static void enableRepeatEvents(bool arg0) {
 repeatEvents = arg0;
 }

 static bool areRepeatEventsEnabled() {
 return repeatEvents;
 }

 static bool isRepeatEvent() {
 return repeatEvents;
 }
 ...
 static std::string getKeyName(int arg0) {
 return keyName[arg0];
 }
