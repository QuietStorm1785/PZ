...
private
  static final Controller[] controllers = new Controller[16];
  static bool isCreated = false;
private
  static Consumer<Integer> controllerConnectedCallback = nullptr;
private
  static Consumer<Integer> controllerDisconnectedCallback = nullptr;
  ...
  static void readGameControllerDB() {
    ...
    std::string string = ZomboidFileSystem.instance.getCacheDirSub(
        "joypads" + File.separator + "gamecontrollerdb.txt");
  ...
  static int getControllerCount() {
    if (!isCreated()) {
      throw new RuntimeException("Before calling 'getJoypadCount()' you should "
                                 "call 'create()' method");
    } else {
  ...
  static Controller getController(int int0) {
    if (!isCreated()) {
      throw new RuntimeException(
          "Before calling 'getJoypad(int)' you should call 'create()' method");
    } else {
  ...
  static void poll(GamepadState[] gamepadStates) {
    if (!isCreated()) {
      throw new RuntimeException(
          "Before calling 'poll()' you should call 'create()' method");
    } else {
      if (Core.bDebug && debugToggleControllerPluggedIn >= 0 &&
          debugToggleControllerPluggedIn < 16) {
