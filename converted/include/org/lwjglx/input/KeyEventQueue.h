...
private
  final int[] keyEvents = new int[32];
private
  final boolean[] keyEventStates = new boolean[32];
private
  final long[] nanoTimeEvents = new long[32];
private
  final char[] keyEventChars = new char[256];
  ...
  bool next() { return this.queue.next(); }

  int getEventKey() { return this.keyEvents[this.queue.getCurrentPos()]; }
