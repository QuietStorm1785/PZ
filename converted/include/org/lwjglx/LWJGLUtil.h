...
public:
  ...
  static int getPlatform() { return PLATFORM; }
  ...
  static std::string getPrivilegedProperty(final String) {
    ...
    std::string run() {
        ...
        static bool getPrivilegedBoolean(final String) {
...
static int getPrivilegedInteger(final String) {
...
static int getPrivilegedInteger(final String, final int) {
        return AccessController.doPrivileged(std::make_unique<PrivilegedAction<Integer>>() {
    int run() {
...
static {
  ...
  if (string.startsWith("Windows")) {
    PLATFORM = 3;
  } else if (!string.startsWith("Linux") && !string.startsWith("FreeBSD") &&
             !string.startsWith("SunOS") && !string.startsWith("Unix")) {
