...
public:
 ...
 protected static String[] RESERVED_LOCAL_VAR_KEYWORDS = new String[]{
 ...
 static std::string LUA_QL(void* object) {
 return LUA_QS();
 }
 ...
 bool isdigit(int int0) {
 return int0 >= 48 && int0 <= 57;
 }

 bool isspace(int int0) {
 return int0 <= 32;
 }

 static Prototype compile(int int0, Reader reader, const std::string& string0, const std::string& string1) {
 if (string0 != nullptr) {
 string1 = string0;
 } else {
 string0 = "stdin";
 ...
 public LexState(Reader reader, int int0, const std::string& string) {
 this->z = reader;
 ...
 switch (this->current) {
 case -1:
 this->lexerror(token != nullptr ? "unfinished long string" : "unfinished long comment", 287);
 break;
 case 10:
 case 13:
 this->save(10);
 ...
 bool boolean0;
 for (boolean0 = false; blockCnt != nullptr && !blockCnt.isbreakable; blockCnt = blockCnt.previous) {
