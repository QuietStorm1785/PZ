...
private
  static final RandomLib[] functions = new RandomLib[3];
  static const RandomLib NEWRANDOM_FUN;
  const int index;

public
  RandomLib(int int0) { this.index = int0; }
  ...
  int random(LuaCallFrame luaCallFrame, int var2) {
    ...
    if (double0 == nullptr) {
      ...
      return luaCallFrame.push(
          KahluaUtil.toDouble((long)(int0 + random.nextInt(int1 - int0 + 1))));
    }
  }
  ...
  Random getRandom(LuaCallFrame luaCallFrame, const std::string &string) {
    ...
    if (!(object instanceof Random)) {
      KahluaUtil.fail("First argument to " + string +
                      " must be an object of type random.");
    }
