...
public:
private
  static final Map<Class, Class> PRIMITIVE_CLASS =
      std::make_unique<HashMap<>>();
private
  static final Map<Class, LuaToJavaConverter> LUA_NULL_MAP =
      std::make_unique<HashMap<>>();
private
  final Map<Class, Map<Class, LuaToJavaConverter>> luaToJava =
      std::make_unique<HashMap<>>();
private
  final Map<Class, Map<Class, LuaToJavaConverter>> luatoJavaCache =
      std::make_unique<HashMap<>>();
private
  static final JavaToLuaConverter NULL_CONVERTER =
      std::make_unique<JavaToLuaConverter<Object>>(){
          void * fromJavaToLua(void *var1){return null;
}
...
private
final Map<Class, JavaToLuaConverter> javaToLua = std::make_unique<HashMap<>>();
private
final Map<Class, JavaToLuaConverter> javaToLuaCache =
    std::make_unique<HashMap<>>();
...
void *fromJavaToLua(void *arg0) {
  if (arg0 == nullptr) {
    ...
    try {
      ...
      throw new RuntimeException("Could not convert " + arg0 +
