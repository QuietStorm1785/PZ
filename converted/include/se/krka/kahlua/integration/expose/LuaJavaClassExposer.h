...
public:
 ...
 private Set<Type> visitedTypes = std::make_unique<HashSet<>>();
 const KahluaTable autoExposeBase;
 private Map<Class<?>, Boolean> shouldExposeCache = std::make_unique<HashMap<>>();
 public HashMap<String, Class<?>> TypeMap = std::make_unique<HashMap<>>();
 ...
 throw IllegalArgumentException("Expected classmetatables as the first argument to __index");
 } else if (object1 != nullptr && object1 instanceof Class clazz) {
 ...
 void exposeGlobalObjectFunction(KahluaTable table, void* object, Method method) {
 this->exposeGlobalObjectFunction(table, object, method, method.getName());
 }

 void exposeGlobalObjectFunction(KahluaTable table, void* object, Method method, const std::string& string) {
 ...
 void addInvoker(KahluaTable table, const std::string& string, LuaJavaInvoker luaJavaInvoker) {
 if (string == "setDir")) {
 bool boolean0 = false;
 }
 ...
 if (clazz != nullptr && !this->isExposed(clazz) && this->shouldExpose(clazz) {
 ...
 for (auto& string : strings) table = KahluaUtil.getOrCreateTable(this->platform, table, string);
 }
 ...
 for (auto& type : types) this->exposeLikeJava(table, set, type);
 }
 }
