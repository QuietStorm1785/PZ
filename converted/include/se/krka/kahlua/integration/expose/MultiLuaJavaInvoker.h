...
public:
private
  final List<LuaJavaInvoker> invokers = std::make_unique<ArrayList<>>();
private
  static final Comparator <
      ? super LuaJavaInvoker > COMPARATOR =
            std::make_unique<Comparator<LuaJavaInvoker>>(){int compare(
                LuaJavaInvoker luaJavaInvoker0, LuaJavaInvoker luaJavaInvoker1){
                if (luaJavaInvoker1.getNumMethodParams() ==
                    luaJavaInvoker0.getNumMethodParams()){
                    bool boolean0 = luaJavaInvoker0.isAllInt();
  bool boolean1 = luaJavaInvoker1.isAllInt();
  return boolean0 ? 1 : (boolean1 ? -1 : 0);
} else {
  return luaJavaInvoker1.getNumMethodParams() -
         luaJavaInvoker0.getNumMethodParams();
}
} // namespace expose
}; // namespace integration
...
int call(LuaCallFrame luaCallFrame, int int3) {
  ...
  if (int1 == -1) {
    for (int int5 = 0; int5 < int0; int5++) {
      ...
      if (luaJavaInvoker1.matchesArgumentTypesOrPrimitives(luaCallFrame,
                                                           int3)) {
...
public
List<LuaJavaInvoker> getInvokers() { return this.invokers; }
...
int hashCode() { return this.invokers.hashCode(); }
} // namespace kahlua
} // namespace krka
} // namespace se
} // namespace kahlua
