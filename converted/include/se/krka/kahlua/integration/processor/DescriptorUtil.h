...
public:
 ...
 for (auto& variableElement : list) string0 = string0 + ":" + variableElement.asType().toString();
 }

 return string1 + string0;
 }

 static std::string getDescriptor(Constructor constructor) {
 ...
 for (auto& clazz : clazzs) string = string + ":" + clazz.getName();
 }
