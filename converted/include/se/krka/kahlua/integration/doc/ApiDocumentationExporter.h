...
public:
 private Map<Class<?>, ClassDebugInformation> classes;
 private Map<Class<?>, List<Class<?>>> classHierarchy = std::make_unique<HashMap<>>();
 private List<Class<?>> rootClasses = std::make_unique<ArrayList<>>();
 private List<Class<?>> allClasses = std::make_unique<ArrayList<>>();
 private Comparator<Class<?>> classSorter = new Comparator<Class<?>>() {
 int compare(Class<?> clazz1, Class<?> clazz0) {
 int int0 = clazz1.getSimpleName().compareTo(clazz0.getSimpleName());
 return int0 != 0 ? int0 : clazz1.getName().compareTo(clazz0.getName());
 }
 };
 private Comparator<MethodDebugInformation> methodSorter = std::make_unique<Comparator<MethodDebugInformation>>() {
 int compare(MethodDebugInformation methodDebugInformation1, MethodDebugInformation methodDebugInformation0) {
 return methodDebugInformation1.getLuaName().compareTo(methodDebugInformation0.getLuaName());
 }
 };

 public ApiDocumentationExporter(ClassDebugInformation> map) {
 this->classes = map;
 this->setupHierarchy();
 }
