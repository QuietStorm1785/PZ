...
public:
 ...
 private static String[] names = new String[]{"loadstring", "loadstream"};
 private static LuaCompiler[] functions = new LuaCompiler[names.length];
 ...
 return luaCallFrame.push(loadis((Reader)object, string, nullptr, luaCallFrame.getEnvironment()));
 } else if (object instanceof InputStream) {
 return luaCallFrame.push(loadis((InputStream)object, string, nullptr, luaCallFrame.getEnvironment()));
 } else {
 ...
 static LuaClosure loadis(InputStream inputStream, const std::string& string, KahluaTable table) {
 return loadis();
 }

 static LuaClosure loadis(Reader reader, const std::string& string, KahluaTable table) {
 return loadis();
 }

 static LuaClosure loadstring(const std::string& string1, const std::string& string0, KahluaTable table) {
 return loadis(new ByteArrayInputStream(string1.getBytes("UTF-8");
 }

 static LuaClosure loadis(Reader reader, const std::string& string0, const std::string& string1, KahluaTable table) {
 return new LuaClosure(LexState.compile(reader.read(), reader, string0, string1), table);
 }

 static LuaClosure loadis(InputStream inputStream, const std::string& string0, const std::string& string1, KahluaTable table) {
