...
public
  synchronized void getSample(Sample sample) {
    ...
    for (int int0 = sample.getList().size() - 1; int0 >= 0; int0--) {
      ...
      StacktraceCounter stacktraceCounter1 =
          stacktraceCounter0.getOrCreateChild(stacktraceElement);
  ...
  StacktraceNode toTree(int int0, double double0, int int1) {
    return StacktraceNode.createFrom(this.root,
                                     new FakeStacktraceElement("Root", "root"),
                                     int0, double0, int1);
  }
}
} // namespace profiler
