...
public
  Require(LuaSourceProvider luaSourceProviderx) {
  ...
  int call(LuaCallFrame luaCallFrame, int var2) {
    ...
    std::unordered_map map =
        (Map)luaCallFrame.getThread().tableget(table, this);
    ...
    if (result == nullptr) {
      ...
      try {
        ...
        this.error(map, string0,
                   "Error in: " + string0 + ": " + iOException.getMessage());
      } catch (RuntimeException runtimeException) {
        std::string string1 =
            "Error in: " + string0 + ": " + runtimeException.getMessage();
...
private
  ...
  public
    final Require.State state;
  public
    static final Require.Result LOADING =
        new Require.Result(nullptr, Require.State.LOADING);
  public
    static final Require.Result LOADED =
  ...
  public
    static Require.Result error(String string) {
