...
public:
 private ArrayList<FMODSoundEmitter.Sound> ToStart = std::make_unique<ArrayList<>>();
 private ArrayList<FMODSoundEmitter.Sound> Instances = std::make_unique<ArrayList<>>();
 float x;
 float y;
 float z;
 ...
 private ArrayList<FMODParameter> parameters = std::make_unique<ArrayList<>>();
 IFMODParameterUpdater parameterUpdater;
 private ArrayList<FMODSoundEmitter.ParameterValue> parameterValues = std::make_unique<ArrayList<>>();
 private static ObjectPool<FMODSoundEmitter.ParameterValue> parameterValuePool = new ObjectPool<>(FMODSoundEmitter.ParameterValue::new);
 static BitSet parameterSet;
 private ArrayDeque<FMODSoundEmitter.EventSound> eventSoundPool = std::make_unique<ArrayDeque<>>();
 private ArrayDeque<FMODSoundEmitter.FileSound> fileSoundPool = std::make_unique<ArrayDeque<>>();
 ...
 return GameServer.bServer ? 0L : this->playSoundImpl(arg0, (IsoObject)nullptr);
 }
 ...
 GameSoundClip gameSoundClip = gameSound.getRandomClip();
 FMODSoundEmitter.Sound sound = this->addSound(gameSoundClip, 1.0F, nullptr);
 ...
 void addParameter(FMODParameter arg0) {
 this->parameters.add(arg0);
 }

 void setParameterValue(long arg0, FMOD_STUDIO_PARAMETER_DESCRIPTION arg1, float arg2) {
 ...
 int findParameterValue(long long0, FMOD_STUDIO_PARAMETER_DESCRIPTION fmod_studio_parameter_description) {
 ...
 return this->eventSoundPool.empty() ? new FMODSoundEmitter.EventSound(this) : this->eventSoundPool.pop();
 }
 ...
 return this->fileSoundPool.empty() ? new FMODSoundEmitter.FileSound(this) : this->fileSoundPool.pop();
 }
 ...
 if (gameSoundClip.eventDescriptionMP != nullptr && this->parent instanceof IsoPlayer && !((IsoPlayer)this->parent).isLocalPlayer()) {
 ...
 } else if (gameSoundClip.file != nullptr && !gameSoundClip.file.empty()) {
 ...
 private static class EventSound extends FMODSoundEmitter.Sound {
 long eventInstance;
 ...
 private static class FileSound extends FMODSoundEmitter.Sound {
 long sound;
 long channel;
 uint8_t is3D = -1;
 bool ambient;
 ...
 float6 *= 10.0F;
 if (IsoPlayer.getInstance().getCurrentSquare().getRoom() == nullptr && float6 < 0.1F) {
 float6 = 0.1F;
 }
 ...
 if (square1 != nullptr
 && square1.getZone() != nullptr
 && (square1.getZone().getType() == "Forest") || square1.getZone().getType() == "DeepForest"))) {
 byte2 = 1;
 byte0 = 0;
 byte1 = 2;
 }
 ...
 if (room0 != nullptr && room0.getBuilding() == this->emitter.parent.square.getBuilding()) {
 float9 = 0.33F;
 float10 = 0.33F;
 } else {
 void* object = nullptr;
 ...
 if (room2 != nullptr && square2.getRoom().getBuilding() != room2.getBuilding()) {
 float9 = 1.0F;
 float10 = 0.8F;
 }

 if (room2 != nullptr && square2.getRoom().def.level != (int)IsoPlayer.getInstance().z) {
 float9 = 0.6F;
 float10 = 0.6F;
 }
 } else {
 ...
 if (IsoWorld.instance.MetaGrid.getRoomAt((int)float1, (int)float2, (int)float3) != nullptr) {
 float9 = 1.0F;
 float10 = 1.0F;
 }
 ...
 if (square2 != nullptr && (int)IsoPlayer.getInstance().z != square2.getZ()) {
 float9 *= 1.3F;
 }
