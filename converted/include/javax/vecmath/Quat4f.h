...
public
  Quat4f(float float4, float float3, float float2, float float1) {
    float float0 = (float)(1.0 / Math.sqrt(float4 * float4 + float3 * float3 +
                                           float2 * float2 + float1 * float1));
...
public
  Quat4f(float[] floats) {
    float float0 =
        (float)(1.0 / Math.sqrt(floats[0] * floats[0] + floats[1] * floats[1] +
                                floats[2] * floats[2] + floats[3] * floats[3]));
...
public
  Quat4f(Quat4f quat4f1) { super(quat4f1); }

public
  Quat4f(Quat4d quat4d) { super(quat4d); }

public
  Quat4f(Tuple4f tuple4f) {
    float float0 =
        (float)(1.0 / Math.sqrt(tuple4f.x * tuple4f.x + tuple4f.y * tuple4f.y +
...
public
  ...
  void mul(Quat4f quat4f2, Quat4f quat4f1) {
    if (this != quat4f2 && this != quat4f1) {
      this.w = quat4f2.w * quat4f1.w - quat4f2.x * quat4f1.x -
               quat4f2.y * quat4f1.y - quat4f2.z * quat4f1.z;
      this.x = quat4f2.w * quat4f1.x + quat4f1.w * quat4f2.x +
               quat4f2.y * quat4f1.z - quat4f2.z * quat4f1.y;
      this.y = quat4f2.w * quat4f1.y + quat4f1.w * quat4f2.y -
               quat4f2.x * quat4f1.z + quat4f2.z * quat4f1.x;
      this.z = quat4f2.w * quat4f1.z + quat4f1.w * quat4f2.z +
               quat4f2.x * quat4f1.y - quat4f2.y * quat4f1.x;
    } else {
      float float0 = quat4f2.w * quat4f1.w - quat4f2.x * quat4f1.x -
                     quat4f2.y * quat4f1.y - quat4f2.z * quat4f1.z;
      float float1 = quat4f2.w * quat4f1.x + quat4f1.w * quat4f2.x +
                     quat4f2.y * quat4f1.z - quat4f2.z * quat4f1.y;
      float float2 = quat4f2.w * quat4f1.y + quat4f1.w * quat4f2.y -
                     quat4f2.x * quat4f1.z + quat4f2.z * quat4f1.x;
      this.z = quat4f2.w * quat4f1.z + quat4f1.w * quat4f2.z +
               quat4f2.x * quat4f1.y - quat4f2.y * quat4f1.x;
      this.w = float0;
  ...
  void mul(Quat4f quat4f0) {
    float float0 = this.w * quat4f0.w - this.x * quat4f0.x -
                   this.y * quat4f0.y - this.z * quat4f0.z;
    float float1 = this.w * quat4f0.x + quat4f0.w * this.x +
                   this.y * quat4f0.z - this.z * quat4f0.y;
    float float2 = this.w * quat4f0.y + quat4f0.w * this.y -
                   this.x * quat4f0.z + this.z * quat4f0.x;
    this.z = this.w * quat4f0.z + quat4f0.w * this.z + this.x * quat4f0.y -
             this.y * quat4f0.x;
    this.w = float0;
    this.x = float1;
    this.y = float2;
  ...
  void inverse(Quat4f quat4f0) {
    float float0 = 1.0F / (quat4f0.w * quat4f0.w + quat4f0.x * quat4f0.x +
                           quat4f0.y * quat4f0.y + quat4f0.z * quat4f0.z);
  ...
  void inverse() {
    float float0 = 1.0F / (this.w * this.w + this.x * this.x + this.y * this.y +
                           this.z * this.z);
    this.w *= float0;
    this.x *= -float0;
    this.y *= -float0;
  ...
  void normalize(Quat4f quat4f0) {
    float float0 = quat4f0.x * quat4f0.x + quat4f0.y * quat4f0.y +
                   quat4f0.z * quat4f0.z + quat4f0.w * quat4f0.w;
    if (float0 > 0.0F) {
  ...
  void set(Matrix4f matrix4f) {
    float float0 =
        0.25F * (matrix4f.m00 + matrix4f.m11 + matrix4f.m22 + matrix4f.m33);
    if (float0 >= 0.0F) {
      if (float0 >= 1.0E-30) {
  ...
  void set(AxisAngle4d axisAngle4d) {
    float float0 = (float)(1.0 / Math.sqrt(axisAngle4d.x * axisAngle4d.x +
  ...
  void interpolate(Quat4f quat4f0, float float0) {
    double double0 = this.x * quat4f0.x + this.y * quat4f0.y +
                     this.z * quat4f0.z + this.w * quat4f0.w;
    if (double0 < 0.0) {
      quat4f0.x = -quat4f0.x;
      quat4f0.y = -quat4f0.y;
      quat4f0.z = -quat4f0.z;
  ...
  void interpolate(Quat4f quat4f0, Quat4f quat4f1, float float0) {
    double double0 = quat4f1.x * quat4f0.x + quat4f1.y * quat4f0.y +
                     quat4f1.z * quat4f0.z + quat4f1.w * quat4f0.w;
    if (double0 < 0.0) {
      quat4f0.x = -quat4f0.x;
      quat4f0.y = -quat4f0.y;
      quat4f0.z = -quat4f0.z;
