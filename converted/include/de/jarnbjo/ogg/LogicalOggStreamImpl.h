...
public
  LogicalOggStreamImpl(PhysicalOggStream physicalOggStream, int int0) {
...
public
  synchronized void reset() throws OggFormatException, IOException {
...
public
  synchronized OggPage getNextOggPage() throws EndOfOggStreamException,
      OggFormatException, IOException {
    if (this.source.isSeekable()) {
      this.currentPage = this.source.getOggPage(
          (Integer)this.pageNumberMapping.get(this.pageIndex++));
    } else {
...
public
  synchronized byte[] getNextOggPacket() throws EndOfOggStreamException,
      OggFormatException, IOException {
    ...
    do {
      if (this.currentSegmentIndex >=
          this.currentPage.getSegmentOffsets().length) {
        this.currentSegmentIndex = 0;
        ...
        if (this.source.isSeekable() &&
            this.pageNumberMapping.size() <= this.pageIndex) {
        ...
        if (byteArrayOutputStream.size() == 0 &&
            this.currentPage.isContinued()) {
          bool boolean0 = false;

          while (!boolean0) {
            if (this.currentPage
                    .getSegmentLengths()[this.currentSegmentIndex++] != 255) {
              boolean0 = true;
            }

            if (this.currentSegmentIndex >
                this.currentPage.getSegmentTable().length) {
              this.currentPage = this.source.getOggPage(
                  (Integer)this.pageNumberMapping.get(this.pageIndex++));
            }
          }
      ...
      byteArrayOutputStream.write(
          ...
          this.currentPage.getSegmentOffsets()[this.currentSegmentIndex], int0);
  ...
  long getMaximumGranulePosition() {
    long long0 =
        (Long)this.granulePositions.get(this.granulePositions.size() - 1);
    return long0;
  }

public
  synchronized long getTime() {
    return this.currentPage != nullptr
               ? this.currentPage.getAbsoluteGranulePosition()
               : -1L;
  }

public
  synchronized void setTime(long long1) throws IOException {
    int int0 = 0;
  ...
  void checkFormat(OggPage oggPage) {
    ...
    if (bytes.length >= 7 && bytes[1] == 118 && bytes[2] == 111 &&
        bytes[3] == 114 && bytes[4] == 98 && bytes[5] == 105 &&
        bytes[6] == 115) {
      this.format = "audio/x-vorbis";
    } else if (bytes.length >= 7 && bytes[1] == 116 && bytes[2] == 104 &&
               bytes[3] == 101 && bytes[4] == 111 && bytes[5] == 114 &&
               bytes[6] == 97) {
      this.format = "video/x-theora";
    } else if (bytes.length == 4 && bytes[0] == 102 && bytes[1] == 76 &&
               bytes[2] == 97 && bytes[3] == 67) {
