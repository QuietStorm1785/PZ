...
private
  int[] highNeighbours;
private
  static final int[] RANGES = new int[]{256, 128, 86, 64};

private
  Floor1() {}

protected
  Floor1(BitInputStream bitInputStream, SetupHeader setupHeader) {
    this.maximumClass = -1;
    ...
    for (int int3 = 0; int3 <= this.maximumClass; int3++) {
      ...
      if (this.classDimensions[int3] > setupHeader.getCodeBooks().length ||
          this.classSubclasses[int3] > setupHeader.getCodeBooks().length) {
        throw new VorbisFormatException(
            "There is a class dimension or class subclasses entry higher than "
            "the number of codebooks in the setup header.");
      }
    ...
    for (int int5 = 0; int5 < int0; int5++) {
      for (int int6 = 0;
           int6 < this.classDimensions[this.partitionClassList[int5]]; int6++) {
  ...
  Floor decodeFloor(VorbisStream vorbisStream, BitInputStream bitInputStream) {
    if (!bitInputStream.getBit()) {
      ...
      for (int int2 = 0; int2 < this.partitionClassList.length; int2++) {
        ...
        int int7 = 0;
        if (int5 > 0) {
          int7 = bitInputStream.getInt(
              vorbisStream.getSetupHeader()
                  .getCodeBooks()[this.classMasterbooks[int3]]
                  .getHuffmanRoot());
        }
        ...
        for (int int8 = 0; int8 < int4; int8++) {
          ...
          int7 >> >= int5;
          if (int9 >= 0) {
            floor10.yList[int8 + int1] =
                bitInputStream.getInt(vorbisStream.getSetupHeader()
                                          .getCodeBooks()[int9]
                                          .getHuffmanRoot());
          } else {
  ...
  void computeFloor(float[] floats0) {
    ...
    for (int int3 = 2; int3 < int1; int3++) {
      ...
      int int6 =
          Util.renderPoint(this.xList[int4], this.xList[int5], this.yList[int4],
                           this.yList[int5], this.xList[int3]);
      ...
      if (int7 != 0) {
        ...
        if (int7 >= int9) {
          this.yList[int3] =
              int8 > int6 ? int7 - int6 + int6 : -int7 + int8 + int6 - 1;
        } else {
          this.yList[int3] =
              (int7 & 1) == 1 ? int6 - (int7 + 1 >> 1) : int6 + (int7 >> 1);
        }
      } else {
        booleans[int3] = false;
  ...
  static void sort(int[] ints0, int[] ints1, boolean[] booleans) {
    ...
    for (int int2 = byte0; int2 < int1; int2++) {
      for (int int3 = int2; int3 > byte0 && ints0[int3 - 1] > ints0[int3];
           int3--) {
        int int4 = ints0[int3];
