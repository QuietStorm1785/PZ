...
import zombie.world.logger.Log.ModIDChangedItem;
import zombie.world.logger.Log.ObsoleteItem;
import zombie.world.logger.Log.RegisterItem;
import zombie.world.logger.Log.RegisterObject;
import zombie.world.logger.Log.ReinstateItem;
import zombie.world.logger.Log.RemovedItem;
...
public class DictionaryData {
      ...
      for (int var2 = 0; var2 < var1.size(); var2++) {
         ...
         if (!this.objectNameToIdMap.containsKey(var3)) {
            if (this.NextObjectNameID >= 127) {
               ...
               byte var4 = this.NextObjectNameID++;
   ...
   protected void createErrorBackups() {
      if (!Core.getInstance().isNoSave()) {
         try {
            ...
            String var3 = ZomboidFileSystem.instance.getFileNameInCurrentSave("WD_ERROR_" + var1) + File.separator;
   ...
   protected void load() throws IOException, WorldDictionaryException {
      if (!Core.getInstance().isNoSave()) {
         String var1 = ZomboidFileSystem.instance.getFileNameInCurrentSave("WorldDictionary.bin");
   ...
   protected void saveAsText(String var1) throws IOException, WorldDictionaryException {
      if (!Core.getInstance().isNoSave()) {
         ...
         if (var2.exists() && var2.isDirectory()) {
            String var3 = ZomboidFileSystem.instance.getFileNameInCurrentSave(var1);
            File var4 = new File(var3);
