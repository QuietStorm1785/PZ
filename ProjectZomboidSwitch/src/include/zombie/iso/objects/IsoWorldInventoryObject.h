#pragma once
#include <string>
#include <vector>
#include <memory>
#include <unordered_map>
#include <unordered_set>
#include <cstdint>
#include "se/krka/kahlua/vm/KahluaTable.h"
#include "zombie/GameTime.h"
#include "zombie/Lua/LuaEventManager.h"
#include "zombie/core/Core.h"
#include "zombie/core/Rand.h"
#include "zombie/core/logger/ExceptionLogger.h"
#include "zombie/core/math/PZMath.h"
#include "zombie/core/opengl/Shader.h"
#include "zombie/core/skinnedmodel/model/WorldItemModelDrawer.h"
#include "zombie/core/textures/ColorInfo.h"
#include "zombie/core/textures/Texture.h"
#include "zombie/debug/DebugOptions.h"
#include "zombie/debug/LineDrawer.h"
#include "zombie/input/Mouse.h"
#include "zombie/inventory/InventoryItem.h"
#include "zombie/inventory/InventoryItemFactory.h"
#include "zombie/inventory/ItemContainer.h"
#include "zombie/inventory/ItemSoundManager.h"
#include "zombie/inventory/types/DrainableComboItem.h"
#include "zombie/inventory/types/InventoryContainer.h"
#include "zombie/iso/IsoCamera.h"
#include "zombie/iso/IsoCell.h"
#include "zombie/iso/IsoGridSquare.h"
#include "zombie/iso/IsoObject.h"
#include "zombie/iso/IsoUtils.h"
#include "zombie/iso/IsoWorld.h"
#include "zombie/iso/PlayerCamera.h"
#include "zombie/iso/sprite/IsoDirectionFrame.h"
#include "zombie/iso/sprite/IsoSprite.h"
#include "zombie/iso/sprite/IsoSpriteManager.h"
#include "zombie/network/GameServer.h"
#include "zombie/network/ServerGUI.h"
#include "zombie/scripting/ScriptManager.h"
#include "zombie/scripting/objects/Item.h"
#include "zombie/scripting/objects/Item/Type.h"
#include "zombie/ui/ObjectTooltip.h"
#include "zombie/util/io/BitHeader.h"
#include "zombie/util/io/BitHeader/HeaderSize.h"
#include "zombie/util/io/BitHeaderRead.h"
#include "zombie/util/io/BitHeaderWrite.h"
#include <filesystem>
#include <fstream>
#include <iostream>

namespace zombie {
namespace iso {
namespace objects {


class IsoWorldInventoryObject : public IsoObject {
public:
    InventoryItem item;
    float xoff;
    float yoff;
    float zoff;
    bool removeProcess = false;
    double dropTime = -1.0;
    bool ignoreRemoveSandbox = false;

    public IsoWorldInventoryObject(InventoryItem var1, IsoGridSquare var2, float var3, float var4, float var5) {
      this.OutlineOnMouseover = true;
      if (var1.worldZRotation < 0) {
         var1.worldZRotation = Rand.Next(0, 360);
      }

      var1.setContainer(nullptr);
      this.xoff = var3;
      this.yoff = var4;
      this.zoff = var5;
      if (this.xoff == 0.0F) {
         this.xoff = Rand.Next(1000) / 1000.0F;
      }

      if (this.yoff == 0.0F) {
         this.yoff = Rand.Next(1000) / 1000.0F;
      }

      this.item = var1;
      this.sprite = IsoSprite.CreateSprite(IsoSpriteManager.instance);
      this.updateSprite();
      this.square = var2;
      this.offsetY = 0.0F;
      this.offsetX = 0.0F;
      this.dropTime = GameTime.getInstance().getWorldAgeHours();
   }

    public IsoWorldInventoryObject(IsoCell var1) {
      super(var1);
      this.offsetY = 0.0F;
      this.offsetX = 0.0F;
   }

    void swapItem(InventoryItem var1) {
      if (var1 != nullptr) {
         if (this.getItem() != nullptr) {
            IsoWorld.instance.CurrentCell.addToProcessItemsRemove(this.getItem());
            ItemSoundManager.removeItem(this.getItem());
            this.getItem().setWorldItem(nullptr);
            var1.setID(this.getItem().getID());
            var1.worldScale = this.getItem().worldScale;
            var1.worldZRotation = this.getItem().worldZRotation;
         }

         this.item = var1;
         if (var1.getWorldItem() != nullptr) {
            throw IllegalArgumentException("newItem.getWorldItem() != nullptr");
         } else {
            this.getItem().setWorldItem(this);
            this.setKeyId(this.getItem().getKeyId());
            this.setName(this.getItem().getName());
            if (this.getItem().shouldUpdateInWorld()) {
               IsoWorld.instance.CurrentCell.addToProcessWorldItems(this);
            }

            IsoWorld.instance.CurrentCell.addToProcessItems(var1);
            this.updateSprite();
            LuaEventManager.triggerEvent("OnContainerUpdate");
            if (GameServer.bServer) {
               this.sendObjectChange("swapItem");
            }
         }
      }
   }

    void saveChange(const std::string& var1, KahluaTable var2, ByteBuffer var3) {
      if ("swapItem" == var1)) {
         if (this.getItem() == nullptr) {
            return;
         }

         try {
            this.getItem().saveWithSize(var3, false);
         } catch (Exception var5) {
            ExceptionLogger.logException(var5);
         }
      } else {
         super.saveChange(var1, var2, var3);
      }
   }

    void loadChange(const std::string& var1, ByteBuffer var2) {
      if ("swapItem" == var1)) {
         try {
    InventoryItem var3 = InventoryItem.loadItem(var2, 195);
            if (var3 != nullptr) {
               this.swapItem(var3);
            }
         } catch (Exception var4) {
            ExceptionLogger.logException(var4);
         }
      } else {
         super.loadChange(var1, var2);
      }
   }

    bool isWaterSource() {
      if (this.item == nullptr) {
    return false;
      } else {
         if (this.item.isBroken()) {
         }

         if (!this.item.canStoreWater()) {
    return false;
         } else if (this.item.isWaterSource() && this.dynamic_cast<DrainableComboItem*>(item) != nullptr) {
            return ((DrainableComboItem)this.item).getRainFactor() > 0.0F;
         } else {
            if (this.item.hasReplaceType("WaterSource")) {
    Item var1 = ScriptManager.instance.getItem(this.item.getReplaceType("WaterSource"));
               if (var1 != nullptr && var1.getType() == Type.Drainable) {
                  return var1.getCanStoreWater() && var1.getRainFactor() > 0.0F;
               }
            }

    return false;
         }
      }
   }

    int getWaterAmount() {
      if (this.isWaterSource()) {
         return this.dynamic_cast<DrainableComboItem*>(item) != nullptr ? ((DrainableComboItem)this.item).getRemainingUses() : 0;
      } else {
    return 0;
      }
   }

    void setWaterAmount(int var1) {
      if (this.isWaterSource()) {
    DrainableComboItem var2 = (DrainableComboItem)zombie.util.Type.tryCastTo(this.item, DrainableComboItem.class);
         if (var2 != nullptr) {
            var2.setUsedDelta(var1 * var2.getUseDelta());
            if (var1 == 0 && var2.getReplaceOnDeplete() != nullptr) {
    InventoryItem var3 = InventoryItemFactory.CreateItem(var2.getReplaceOnDepleteFullType());
               if (var3 != nullptr) {
                  var3.setCondition(this.getItem().getCondition());
                  var3.setFavorite(this.getItem().isFavorite());
                  this.swapItem(var3);
               }
            }
         } else if (var1 > 0 && this.getItem().hasReplaceType("WaterSource")) {
    InventoryItem var5 = InventoryItemFactory.CreateItem(this.getItem().getReplaceType("WaterSource"));
            if (var5 != nullptr) {
               var5.setCondition(this.getItem().getCondition());
               var5.setFavorite(this.getItem().isFavorite());
               var5.setTaintedWater(this.getItem().isTaintedWater());
               var2 = (DrainableComboItem)zombie.util.Type.tryCastTo(var5, DrainableComboItem.class);
               if (var2 != nullptr) {
                  var2.setUsedDelta(var1 * var2.getUseDelta());
               }

               this.swapItem(var5);
            }
         }
      }
   }

    int getWaterMax() {
      if (this.isWaterSource()) {
    float var1;
         if (this.dynamic_cast<DrainableComboItem*>(item) != nullptr) {
            var1 = 1.0F / ((DrainableComboItem)this.item).getUseDelta();
         } else {
            if (!this.getItem().hasReplaceType("WaterSource")) {
    return 0;
            }

    Item var2 = ScriptManager.instance.getItem(this.getItem().getReplaceType("WaterSource"));
            if (var2 == nullptr) {
    return 0;
            }

            var1 = 1.0F / var2.getUseDelta();
         }

         return var1 - (int)var1 > 0.99F ? (int)var1 + 1 : (int)var1;
      } else {
    return 0;
      }
   }

    bool isTaintedWater() {
      return this.isWaterSource() ? this.getItem().isTaintedWater() : false;
   }

    void setTaintedWater(bool var1) {
      if (this.isWaterSource()) {
         this.getItem().setTaintedWater(var1);
      }
   }

    void update() {
    IsoCell var1 = IsoWorld.instance.getCell();
      if (!this.removeProcess && this.item != nullptr && this.item.shouldUpdateInWorld()) {
         var1.addToProcessItems(this.item);
      }
   }

    void updateSprite() {
      this.sprite.setTintMod(std::make_shared<ColorInfo>(this.item.col.r, this.item.col.g, this.item.col.b, this.item.col.a));
      if (!GameServer.bServer || ServerGUI.isCreated()) {
    std::string var1 = this.item.getTex().getName();
         if (this.item.isUseWorldItem()) {
            var1 = this.item.getWorldTexture();
         }

         try {
    Texture var2 = Texture.getSharedTexture(var1);
            if (var2 == nullptr) {
               var1 = this.item.getTex().getName();
            }
         } catch (Exception var4) {
            var1 = "media/inventory/world/WItem_Sack.png";
         }

    Texture var5 = this.sprite.LoadFrameExplicit(var1);
         if (this.item.getScriptItem() == nullptr) {
            this.sprite.def.scaleAspect(var5.getWidthOrig(), var5.getHeightOrig(), 16 * Core.TileScale, 16 * Core.TileScale);
         } else {
    float var10001 = Core.TileScale;
    float var3 = this.item.getScriptItem().ScaleWorldIcon * (var10001 / 2.0F);
            this.sprite.def.setScale(var3, var3);
         }
      }
   }

    bool finishupdate() {
      return this.removeProcess || this.item == nullptr || !this.item.shouldUpdateInWorld();
   }

    void load(ByteBuffer var1, int var2, bool var3) {
      this.xoff = var1.getFloat();
      this.yoff = var1.getFloat();
      this.zoff = var1.getFloat();
    float var4 = var1.getFloat();
    float var5 = var1.getFloat();
      this.sprite = IsoSprite.CreateSprite(IsoSpriteManager.instance);
      this.item = InventoryItem.loadItem(var1, var2);
      if (this.item == nullptr) {
         var1.getDouble();
         if (var2 >= 193) {
    BitHeaderRead var13 = BitHeader.allocRead(HeaderSize.uint8_t, var1);
            var13.release();
         }
      } else {
         this.item.setWorldItem(this);
         this.sprite.getTintMod().r = this.item.getR();
         this.sprite.getTintMod().g = this.item.getG();
         this.sprite.getTintMod().b = this.item.getB();
         if (var2 >= 108) {
            this.dropTime = var1.getDouble();
         } else {
            this.dropTime = GameTime.getInstance().getWorldAgeHours();
         }

         if (var2 >= 193) {
    BitHeaderRead var6 = BitHeader.allocRead(HeaderSize.uint8_t, var1);
            this.ignoreRemoveSandbox = var6.hasFlags(1);
            var6.release();
         }

         if (!GameServer.bServer || ServerGUI.isCreated()) {
    std::string var12 = this.item.getTex().getName();
            if (this.item.isUseWorldItem()) {
               var12 = this.item.getWorldTexture();
            }

            try {
    Texture var7 = Texture.getSharedTexture(var12);
               if (var7 == nullptr) {
                  var12 = this.item.getTex().getName();
               }
            } catch (Exception var9) {
               var12 = "media/inventory/world/WItem_Sack.png";
            }

    Texture var14 = this.sprite.LoadFrameExplicit(var12);
            if (var14 != nullptr) {
               if (var2 < 33) {
                  var4 -= var14.getWidthOrig() / 2;
                  var5 -= var14.getHeightOrig();
               }

               if (this.item.getScriptItem() == nullptr) {
                  this.sprite.def.scaleAspect(var14.getWidthOrig(), var14.getHeightOrig(), 16 * Core.TileScale, 16 * Core.TileScale);
               } else {
    float var10001 = Core.TileScale;
    float var8 = this.item.getScriptItem().ScaleWorldIcon * (var10001 / 2.0F);
                  this.sprite.def.setScale(var8, var8);
               }
            }
         }
      }
   }

    bool Serialize() {
    return true;
   }

    void save(ByteBuffer var1, bool var2) {
      var1.put((byte)(this.Serialize() ? 1 : 0));
      if (this.Serialize()) {
         var1.put(IsoObject.factoryGetClassID(this.getObjectName()));
         var1.putFloat(this.xoff);
         var1.putFloat(this.yoff);
         var1.putFloat(this.zoff);
         var1.putFloat(this.offsetX);
         var1.putFloat(this.offsetY);
         this.item.saveWithSize(var1, false);
         var1.putDouble(this.dropTime);
    BitHeaderWrite var3 = BitHeader.allocWrite(HeaderSize.uint8_t, var1);
         if (this.ignoreRemoveSandbox) {
            var3.addFlags(1);
         }

         var3.write();
         var3.release();
      }
   }

    void softReset() {
      this.square.removeWorldObject(this);
   }

    std::string getObjectName() {
      return "WorldInventoryItem";
   }

    void DoTooltip(ObjectTooltip var1) {
      this.item.DoTooltip(var1);
   }

    bool HasTooltip() {
    return false;
   }

    bool onMouseLeftClick(int var1, int var2) {
    return false;
   }

    void debugDrawLocation(float var1, float var2, float var3) {
      if (Core.bDebug && DebugOptions.instance.ModelRenderAxis.getValue()) {
         var1 += this.xoff;
         var2 += this.yoff;
         var3 += this.zoff;
         LineDrawer.DrawIsoLine(var1 - 0.25F, var2, var3, var1 + 0.25F, var2, var3, 1.0F, 1.0F, 1.0F, 0.5F, 1);
         LineDrawer.DrawIsoLine(var1, var2 - 0.25F, var3, var1, var2 + 0.25F, var3, 1.0F, 1.0F, 1.0F, 0.5F, 1);
      }
   }

    void debugHitTest() {
    int var1 = IsoCamera.frameState.playerIndex;
    float var2 = Core.getInstance().getZoom(var1);
    float var3 = Mouse.getXA();
    float var4 = Mouse.getYA();
      var3 -= IsoCamera.getScreenLeft(var1);
      var4 -= IsoCamera.getScreenTop(var1);
      var3 *= var2;
      var4 *= var2;
    float var5 = this.getScreenPosX(var1) * var2;
    float var6 = this.getScreenPosY(var1) * var2;
    float var7 = IsoUtils.DistanceTo2D(var5, var6, var3, var4);
    uint8_t var8 = 48;
      if (var7 < var8) {
         LineDrawer.drawCircle(var5, var6, var8, 16, 1.0F, 1.0F, 1.0F);
      }
   }

    void render(float var1, float var2, float var3, ColorInfo var4, bool var5, bool var6, Shader var7) {
      if (Core.bDebug) {
      }

      if (this.getItem().getScriptItem().isWorldRender()) {
         if (WorldItemModelDrawer.renderMain(this.getItem(), this.getSquare(), this.getX() + this.xoff, this.getY() + this.yoff, this.getZ() + this.zoff, 0.0F)
            )
          {
            this.debugDrawLocation(var1, var2, var3);
         } else if (this.sprite.CurrentAnim != nullptr && !this.sprite.CurrentAnim.Frames.empty()) {
    Texture var8 = ((IsoDirectionFrame)this.sprite.CurrentAnim.Frames.get(0)).getTexture(this.dir);
            if (var8 != nullptr) {
    float var9 = var8.getWidthOrig() * this.sprite.def.getScaleX() / 2.0F;
    float var10 = var8.getHeightOrig() * this.sprite.def.getScaleY() * 3.0F / 4.0F;
    int var11 = IsoCamera.frameState.playerIndex;
    float var12 = this.getAlpha(var11);
    float var13 = this.getTargetAlpha(var11);
    float var14 = PZMath.min(getSurfaceAlpha(this.square, this.zoff), var12);
               this.setAlphaAndTarget(var11, var14);
               this.sprite.render(this, var1 + this.xoff, var2 + this.yoff, var3 + this.zoff, this.dir, this.offsetX + var9, this.offsetY + var10, var4, true);
               this.setAlpha(var11, var12);
               this.setTargetAlpha(var11, var13);
               this.debugDrawLocation(var1, var2, var3);
            }
         }
      }
   }

    void renderObjectPicker(float var1, float var2, float var3, ColorInfo var4) {
      if (this.sprite != nullptr) {
         if (this.sprite.CurrentAnim != nullptr && !this.sprite.CurrentAnim.Frames.empty()) {
    Texture var5 = ((IsoDirectionFrame)this.sprite.CurrentAnim.Frames.get(0)).getTexture(this.dir);
            if (var5 != nullptr) {
    float var6 = var5.getWidthOrig() / 2;
    float var7 = var5.getHeightOrig();
               this.sprite.renderObjectPicker(this.sprite.def, this, this.dir);
            }
         }
      }
   }

    InventoryItem getItem() {
      return this.item;
   }

    void addToWorld() {
      if (this.item != nullptr && this.item.shouldUpdateInWorld() && !IsoWorld.instance.CurrentCell.getProcessWorldItems().contains(this)) {
         IsoWorld.instance.CurrentCell.getProcessWorldItems().push_back(this);
      }

      if (this.dynamic_cast<InventoryContainer*>(item) != nullptr) {
    ItemContainer var1 = ((InventoryContainer)this.item).getInventory();
         if (var1 != nullptr) {
            var1.addItemsToProcessItems();
         }
      }

      super.addToWorld();
   }

    void removeFromWorld() {
      this.removeProcess = true;
      IsoWorld.instance.getCell().getProcessWorldItems().remove(this);
      if (this.item != nullptr) {
         IsoWorld.instance.CurrentCell.addToProcessItemsRemove(this.item);
         ItemSoundManager.removeItem(this.item);
         this.item.atlasTexture = nullptr;
      }

      if (this.dynamic_cast<InventoryContainer*>(item) != nullptr) {
    ItemContainer var1 = ((InventoryContainer)this.item).getInventory();
         if (var1 != nullptr) {
            var1.removeItemsFromProcessItems();
         }
      }

      super.removeFromWorld();
   }

    void removeFromSquare() {
      if (this.square != nullptr) {
         this.square.getWorldObjects().remove(this);
         this.square.chunk.recalcHashCodeObjects();
      }

      super.removeFromSquare();
   }

    float getScreenPosX(int var1) {
    float var2 = IsoUtils.XToScreen(this.getX() + this.xoff, this.getY() + this.yoff, this.getZ() + this.zoff, 0);
    PlayerCamera var3 = IsoCamera.cameras[var1];
      return (var2 - var3.getOffX()) / Core.getInstance().getZoom(var1);
   }

    float getScreenPosY(int var1) {
    Texture var2 = this.sprite == nullptr ? nullptr : this.sprite.getTextureForCurrentFrame(this.dir);
    float var3 = var2 == nullptr ? 0.0F : var2.getHeightOrig() * this.sprite.def.getScaleY() * 1.0F / 4.0F;
    float var4 = IsoUtils.YToScreen(this.getX() + this.xoff, this.getY() + this.yoff, this.getZ() + this.zoff, 0);
    PlayerCamera var5 = IsoCamera.cameras[var1];
      return (var4 - var5.getOffY() - var3) / Core.getInstance().getZoom(var1);
   }

    void setIgnoreRemoveSandbox(bool var1) {
      this.ignoreRemoveSandbox = var1;
   }

    bool isIgnoreRemoveSandbox() {
      return this.ignoreRemoveSandbox;
   }

    float getWorldPosX() {
      return this.getX() + this.xoff;
   }

    float getWorldPosY() {
      return this.getY() + this.yoff;
   }

    float getWorldPosZ() {
      return this.getZ() + this.zoff;
   }

    static float getSurfaceAlpha(IsoGridSquare var0, float var1) {
      if (var0 == nullptr) {
         return 1.0F;
      } else {
    int var2 = IsoCamera.frameState.playerIndex;
    float var3 = 1.0F;
         if (var1 > 0.01F) {
    bool var4 = false;

            for (int var5 = 0; var5 < var0.getObjects().size(); var5++) {
    IsoObject var6 = (IsoObject)var0.getObjects().get(var5);
               if (var6.getSurfaceOffsetNoTable() > 0.0F) {
                  if (!var4) {
                     var4 = true;
                     var3 = 0.0F;
                  }

                  var3 = PZMath.max(var3, var6.getAlpha(var2));
               }
            }
         }

    return var3;
      }
   }
}
} // namespace objects
} // namespace iso
} // namespace zombie
