...
public:
    ...
    IGLBufferObject var1 = GLVertexBufferObject.funcs;
      this.m_vbo = std::make_shared<GLVertexBufferObject>(7168L, var1.GL_ARRAY_BUFFER(), var1.GL_STREAM_DRAW());
      this.m_vbo.create();
      this.m_ibo = std::make_shared<GLVertexBufferObject>(512L, var1.GL_ELEMENT_ARRAY_BUFFER(), var1.GL_STREAM_DRAW());
      this.m_ibo.create();
   }
   ...
   public void addTriangle(
    ...
    void addQuad(float var1, float var2, float var3, float var4, float var5, float var6, float var7, float var8, float var9) {
      this.reserve(6);
    ...
    return false;
         ...
         return this.m_mode == 4 && this.m_elements.position() % 84 == 0 && this.m_elements.position() + 84 > 7168 ? true : this.m_elements.position() == 7168;
      }
   }
    ...
    bool hasRoomFor(int var1) {
      return this.m_elements == nullptr || this.m_elements.position() / 28 + var1 <= 256;
   }
    ...
    int var4 = this.m_indices.limit() / 2;
         GL12.glDrawRangeElements(this.m_mode, var5, var5 + var2, var4 - var3, 5123, var3 * 2);
         this.m_vbo.bindNone();
         this.m_ibo.bindNone();
