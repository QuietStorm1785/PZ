...
public:
   ...
   private const std::vector<Alarm> alarmList = std::make_unique<std::vector<>>();
    static BaseAmbientStreamManager instance;
   public const std::vector<Ambient> ambient = std::make_unique<std::vector<>>();
   public const std::vector<WorldSoundEmitter> worldEmitters = std::make_unique<std::vector<>>();
   public const ArrayDeque<WorldSoundEmitter> freeEmitters = std::make_unique<ArrayDeque<>>();
   public const std::vector<AmbientLoop> allAmbient = std::make_unique<std::vector<>>();
   public const std::vector<AmbientLoop> nightAmbient = std::make_unique<std::vector<>>();
   public const std::vector<AmbientLoop> dayAmbient = std::make_unique<std::vector<>>();
   public const std::vector<AmbientLoop> rainAmbient = std::make_unique<std::vector<>>();
   public const std::vector<AmbientLoop> indoorAmbient = std::make_unique<std::vector<>>();
   public const std::vector<AmbientLoop> outdoorAmbient = std::make_unique<std::vector<>>();
   public const std::vector<AmbientLoop> windAmbient = std::make_unique<std::vector<>>();
    bool initialized = false;
    FMODSoundEmitter electricityShutOffEmitter = nullptr;
    long electricityShutOffEvent = 0L;
    int electricityShutOffState = -1;
    const ParameterFogIntensity parameterFogIntensity = std::make_shared<ParameterFogIntensity>();
    const ParameterRainIntensity parameterRainIntensity = std::make_shared<ParameterRainIntensity>();
    const ParameterSeason parameterSeason = std::make_shared<ParameterSeason>();
    const ParameterSnowIntensity parameterSnowIntensity = std::make_shared<ParameterSnowIntensity>();
    const ParameterStorm parameterStorm = std::make_shared<ParameterStorm>();
    const ParameterTimeOfDay parameterTimeOfDay = std::make_shared<ParameterTimeOfDay>();
    const ParameterTemperature parameterTemperature = std::make_shared<ParameterTemperature>();
    const ParameterWeatherEvent parameterWeatherEvent = std::make_shared<ParameterWeatherEvent>();
    const ParameterWindIntensity parameterWindIntensity = std::make_shared<ParameterWindIntensity>();
    const ParameterZone parameterZoneDeepForest = std::make_shared<ParameterZone>("ZoneDeepForest", "DeepForest");
    const ParameterZone parameterZoneFarm = std::make_shared<ParameterZone>("ZoneFarm", "Farm");
    const ParameterZone parameterZoneForest = std::make_shared<ParameterZone>("ZoneForest", "Forest");
    const ParameterZone parameterZoneNav = std::make_shared<ParameterZone>("ZoneNav", "Nav");
    const ParameterZone parameterZoneTown = std::make_shared<ParameterZone>("ZoneTown", "TownZone");
    const ParameterZone parameterZoneTrailerPark = std::make_shared<ParameterZone>("ZoneTrailerPark", "TrailerPark");
    const ParameterZone parameterZoneVegetation = std::make_shared<ParameterZone>("ZoneVegetation", "Vegitation");
    const ParameterZoneWaterSide parameterZoneWaterSide = std::make_shared<ParameterZoneWaterSide>();
    const ParameterCameraZoom parameterCameraZoom = std::make_shared<ParameterCameraZoom>();
    const ParameterClosestWallDistance parameterClosestWallDistance = std::make_shared<ParameterClosestWallDistance>();
    const ParameterHardOfHearing parameterHardOfHearing = std::make_shared<ParameterHardOfHearing>();
    const ParameterInside parameterInside = std::make_shared<ParameterInside>();
    const ParameterMoodlePanic parameterMoodlePanic = std::make_shared<ParameterMoodlePanic>();
    const ParameterPowerSupply parameterPowerSupply = std::make_shared<ParameterPowerSupply>();
    const ParameterRoomSize parameterRoomSize = std::make_shared<ParameterRoomSize>();
    const ParameterRoomType parameterRoomType = std::make_shared<ParameterRoomType>();
    const ParameterWaterSupply parameterWaterSupply = std::make_shared<ParameterWaterSupply>();
    const Vector2 tempo = std::make_shared<Vector2>();
    const FMOD_STUDIO_EVENT_CALLBACK electricityShutOffEventCallback = std::make_shared<1>(this);
    ...
    IsoGridSquare var4 = IsoWorld.instance.CurrentCell.getGridSquare(var3.x, var3.y, var3.z);
                        if (var4 == nullptr) {
                           ...
                           this.freeEmitters.push_back(var3);
                           var2--;
                        } else {
                           if (var1 > var3.dawn && var1 < var3.dusk) {
                              if (var3.fmodEmitter.empty()) {
                                 var3.channel = var3.fmodEmitter.playAmbientLoopedImpl(var3.daytime);
                              }
                           } else if (!var3.fmodEmitter.empty()) {
                              var3.fmodEmitter.stopSound(var3.channel);
                              var3.channel = 0L;
                           }

                           if (!var3.fmodEmitter.empty() && (IsoWorld.instance.emitterUpdate || var3.fmodEmitter.hasSoundsToStart())) {
    ...
    IsoGridSquare var8 = IsoWorld.instance.CurrentCell.getGridSquare(var3.x, var3.y, var3.z);
                        if (var8 != nullptr && !var3.fmodEmitter.empty()) {
    ...
    Ambient var8 = std::make_shared<Ambient>(var10, var4, var5, 50.0F, Rand.Next(0.2F, 0.5F));
                  this.ambient.push_back(var8);
               }
            }
         }
      }
    ...
    AmbientLoop var7 = std::make_shared<AmbientLoop>(0.0F, var1, var2);
      this.allAmbient.push_back(var7);
      if (var3) {
         this.indoorAmbient.push_back(var7);
      } else {
         this.outdoorAmbient.push_back(var7);
      }

      if (var4) {
         this.rainAmbient.push_back(var7);
      }

      if (var5) {
         this.nightAmbient.push_back(var7);
      }

      if (var6) {
         this.dayAmbient.push_back(var7);
      }
   }
    ...
    float var9 = 1.0F;
    Ambient var10 = std::make_shared<Ambient>(var12, var4, var5, 700.0F, var9);
         this.ambient.push_back(var10);
      }
   }
    ...
    void doAlarm(RoomDef var1) {
      if (var1 != nullptr && var1.building != nullptr && var1.building.bAlarmed) {
         ...
         this.alarmList.push_back(std::make_shared<Alarm>(var1.x + var1.getW() / 2, var1.y + var1.getH() / 2));
      }
   }
    ...
    void addAmbient(const std::string& var1, int var2, int var3, int var4, float var5) {
      if (GameClient.bClient) {
    Ambient var6 = std::make_shared<Ambient>(var1, var2, var3, var4, var5, true);
         this.ambient.push_back(var6);
      }
   }

    void addAmbientEmitter(float var1, float var2, int var3, const std::string& var4) {
    WorldSoundEmitter var5 = this.freeEmitters.empty() ? std::make_shared<WorldSoundEmitter>() : this.freeEmitters.pop();
      var5.x = var1;
      var5.y = var2;
      var5.z = var3;
      ...
      this.worldEmitters.push_back(var5);
   }
    ...
    Vector2f var2 = std::make_shared<Vector2f>();
    ...
    BuildingDef var7 = nullptr;
    float var8 = float.MAX_VALUE;
      var3.set(0.0F);
    Vector2f var9 = std::make_shared<Vector2f>();
