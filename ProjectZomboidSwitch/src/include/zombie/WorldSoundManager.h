...
public:
    static const WorldSoundManager instance = std::make_shared<WorldSoundManager>();
   public const std::vector<WorldSound> SoundList = std::make_unique<std::vector<>>();
   private const std::stack<WorldSound> freeSounds = std::make_unique<std::stack<>>();
    static const ResultBiggestSound resultBiggestSound = std::make_shared<ResultBiggestSound>();
    ...
    WorldSound getNew() {
      return this.freeSounds.empty() ? std::make_shared<WorldSound>(this) : this.freeSounds.pop();
   }
   ...
   public WorldSound addSound(
    ...
    WorldSound var13;
         /* synchronized - TODO: add std::mutex */ (this.SoundList) {
    ...
    IsoChunk var23 = IsoWorld.instance.CurrentCell.getChunk(var21, var22);
                     if (var23 != nullptr) {
                        var23.SoundList.push_back(var13);
                     }
                  }
               }
            }

            this.SoundList.push_back(var13);
    ...
    WorldSound var7 = nullptr;
    IsoChunk var8 = nullptr;
      if (var5 != nullptr) {
    ...
    return var1 = = nullptr ? this.getHearingMultiplier(2) : this.getHearingMultiplier(var1.hearing);
   }
