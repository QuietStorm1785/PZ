...
public:
   ...
   private static const Map<std::string, ItemInfo> itemLoadList = std::make_unique<std::unordered_map<>>();
   private static const List<std::string> objNameLoadList = std::make_unique<std::vector<>>();
   ...
   private static byte[] clientRemoteData;
    ...
    ItemInfo var1 = itemLoadList.get(var0.getFullName());
         ...
         if (var1.modID != nullptr && !var0.getModID() == var1.modID)) {
            if (var1.modOverrides == nullptr) {
               var1.modOverrides = std::make_unique<std::vector>();
            }

            if (!var1.modOverrides.contains(var1.modID)) {
               var1.modOverrides.push_back(var1.modID);
            } else {
    ...
    for (auto& var5 : var0)                  if (!objNameLoadList.contains(var5)) {
                     objNameLoadList.push_back(var5);
                  }
               }
            }
         }
    ...
    SimpleDateFormat var1 = std::make_shared<SimpleDateFormat>("yyyy/MM/dd HH:mm:ss");
    ...
    std::vector var7 = new std::vector();
         var2 = std::make_shared<Info>(var1.format(std::make_unique<Date>()), Core.GameSaveWorld, 195, var7);
