...
public:
    int outerRadius = 200;
    int innerRadius = 100;
   protected std::vector<Slice> slices = std::make_unique<std::vector<>>();
    int highlight = -1;
    int joypad = -1;
    UITransition transition = std::make_shared<UITransition>();
    UITransition select = std::make_shared<UITransition>();
    UITransition deselect = std::make_shared<UITransition>();
    int selectIndex = -1;
    int deselectIndex = -1;
    ...
    float var15 = 0.0F + this.height / 2.0F + (var2 + (var3 - var2) / 2.0F) * (float)Math.sin(var47);
            ...
            if (var7 != nullptr && !StringUtils.isNullOrWhitespace(var7.text)) {
    ...
    void formatTextInsideCircle(const std::string& var1) {
    ...
    void addSlice(const std::string& var1, Texture var2) {
    Slice var3 = std::make_shared<Slice>();
      var3.text = var1;
      var3.texture = var2;
      this.slices.push_back(var3);
   }

    Slice getSlice(int var1) {
      return var1 >= 0 && var1 < this.slices.size() ? this.slices.get(var1) : nullptr;
   }

    void setSliceText(int var1, const std::string& var2) {
    ...
    void setSliceTexture(int var1, Texture var2) {
    Slice var3 = this.getSlice(var1);
      if (var3 != nullptr) {
         var3.texture = var2;
      }
    ...
    int getSliceIndexFromMouse(int var1, int var2) {
    ...
    int getSliceIndexFromJoypad(int var1) {
