...
#include "zombie/ui/RadialProgressBar/RadSegment.h"
...
public:
   ...
   private static const RadSegment[] segments = new RadSegment[8];
    ...
    float var1 = (float)(this.x + this.xScroll + this.getAbsoluteX() + this.radialTexture.offsetX);
    float var2 = (float)(this.y + this.yScroll + this.getAbsoluteY() + this.radialTexture.offsetY);
    ...
    Vector2 var11 = std::make_shared<Vector2>((float)Math.cos(var10), (float)Math.sin(var10));
    float var12;
    float var13;
    ...
    RadSegment var20 = segments[var19];
                     if (var20 != nullptr && var19 <= var18) {
                        if (var19 != var18) {
                           SpriteRenderer.instance
                              .renderPoly(
                                 this.radialTexture,
                                 var1 + var20.vertex[0].x * this.radialTexture.getWidth(),
                                 var2 + var20.vertex[0].y * this.radialTexture.getHeight(),
                                 var1 + var20.vertex[1].x * this.radialTexture.getWidth(),
                                 var2 + var20.vertex[1].y * this.radialTexture.getHeight(),
                                 var1 + var20.vertex[2].x * this.radialTexture.getWidth(),
                                 var2 + var20.vertex[2].y * this.radialTexture.getHeight(),
                                 var1 + var20.vertex[2].x * this.radialTexture.getWidth(),
                                 var2 + var20.vertex[2].y * this.radialTexture.getHeight(),
                                 1.0F,
                                 1.0F,
                                 1.0F,
                                 1.0F,
                                 var3 + var20.uv[0].x * var5,
                                 var4 + var20.uv[0].y * var6,
                                 var3 + var20.uv[1].x * var5,
                                 var4 + var20.uv[1].y * var6,
                                 var3 + var20.uv[2].x * var5,
                                 var4 + var20.uv[2].y * var6,
                                 var3 + var20.uv[2].x * var5,
                                 var4 + var20.uv[2].y * var6
                              );
                        } else {
    ...
    void printTexture(Texture var1) {
