...
public:
   public static const std::unordered_map<std::string, WorldMapData> s_fileNameToData = std::make_unique<std::unordered_map<>>();
    std::string m_relativeFileName;
   public const std::vector<WorldMapCell> m_cells = std::make_unique<std::vector<>>();
   public const std::unordered_map<int, WorldMapCell> m_cellLookup = std::make_unique<std::unordered_map<>>();
    int m_minX;
    int m_minY;
    int m_maxX;
    int m_maxY;
    static const AssetType ASSET_TYPE = std::make_shared<AssetType>("WorldMapData");
    ...
    WorldMapData var1 = s_fileNameToData.get(var0);
      if (var1 == nullptr && Files.exists(Paths.get(var0))) {
         var1 = (WorldMapData)WorldMapDataAssetManager.instance.load(std::make_shared<AssetPath>(var0));
    ...
    public WorldMapData(AssetPath var1, AssetManager var2, AssetParams var3) {
      super(var1, var2);
   }
    ...
    void onLoaded() {
      this.m_minX = int.MAX_VALUE;
      this.m_minY = int.MAX_VALUE;
      this.m_maxX = int.MIN_VALUE;
      this.m_maxY = int.MIN_VALUE;
      this.m_cellLookup.clear();
