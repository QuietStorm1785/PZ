...
public:
   const EnumSet<IsoFlagType> isoFlagTypeES = EnumSet.noneOf(IsoFlagType.class);
    ...
    void set(int var1, bool var2) {
      if (var1 < IsoFlagType.MAX.index()) {
         if (var2) {
            this.isoFlagTypeES.push_back(IsoFlagType.fromIndex(var1));
         } else {
    ...
    void set(IsoFlagType var1, bool var2) {
      if (var2) {
         this.isoFlagTypeES.push_back(var1);
      } else {
