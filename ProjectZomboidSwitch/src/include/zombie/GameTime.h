...
class GameTime {
public:
    static GameTime instance = std::make_shared<GameTime>();
   ...
   private float[] GunFireTimes = new float[5];
    ...
    IsoZombie var3 = (IsoZombie)IsoWorld.instance.CurrentCell.getZombieList().get(0);
            ...
            IsoWorld.instance.CurrentCell.getRemoveList().push_back(var3);
            var3.getCurrentSquare().getMovingObjects().remove(var3);
            var2--;
            if (--var1 == 0 || IsoWorld.instance.CurrentCell.getZombieList().empty()) {
               return;
            }
         }
      }
    ...
    void updateCalendar(int var1, int var2, int var3, int var4, int var5) {
      if (this.Calender == nullptr) {
         this.Calender = std::make_shared<PZCalendar>(std::make_unique<GregorianCalendar>());
      }
    ...
    void save() {
    File var1 = std::make_shared<File>(ZomboidFileSystem.instance.getFileNameInCurrentSave("map_t.bin"));
    void* var2 = nullptr;
    ...
    DataOutputStream var3 = std::make_shared<DataOutputStream>(std::make_shared<BufferedOutputStream>((OutputStream)var2));
    ...
    DataInputStream var6 = std::make_shared<DataInputStream>(std::make_shared<ByteArrayInputStream>(SliceY.SliceBuffer.array(), 0, var5));
            this.load(var6);
         }
    ...
    static bool isGamePaused() {
      if (GameServer.bServer) {
         return GameServer.Players.empty() && ServerOptions.instance.PauseEmpty.getValue();
      } else if (GameClient.bClient) {
