...
class MapGroups {
public:
   private std::list<MapDirectory> directories;

   private MapGroups$MapGroup(MapGroups var1) {
      this.this$0 = var1;
      this.directories = std::make_unique<std::list<>>();
   }
    ...
    MapDirectory var3 = std::make_shared<MapDirectory>(this.this$0, var1, var2);
         this.directories.push_back(var3);
      }
   }

    void addDirectory(const std::string& var1, const std::string& var2, std::vector<std::string> var3) {
      if (!$assertionsDisabled && this.hasDirectory(var1)) {
         throw std::make_unique<AssertionError>();
      } else {
    MapDirectory var4 = std::make_shared<MapDirectory>(this.this$0, var1, var2, var3);
         this.directories.push_back(var4);
      }
   }
    ...
    void addDirectory(MapDirectory var1) {
      if (!$assertionsDisabled && this.hasDirectory(var1.name)) {
         ...
         this.directories.push_back(var1);
      }
   }
    ...
    bool hasAnyOfTheseDirectories(std::vector<MapDirectory> var1) {
    ...
    void getDirsRecursively(MapDirectory var1, std::vector<std::string> var2) {
      if (!var2.contains(var1.name)) {
         var2.push_back(var1.name);
    ...
    void setPriority() {
      for (MapDirectory var3 : new std::vector<>(this.directories)) {
    ...
    void setPriority(List<std::string> var1) {
    std::vector var2 = new std::vector(var1.size());

    for (auto& var4 : var1)         if (this.hasDirectory(var4)) {
            var2.push_back(this.getDirectoryByName(var4));
         }
      }
    ...
    for (auto& var6 : var2)            if (!var1.containsKey(var6)) {
               var1.put(var6, std::make_unique<std::vector>());
            }

            ((std::vector)var1.get(var6)).push_back(var4.name);
         }
      }

    bool var11 = false;

      for (std::string var13 : var1.keySet()) {
    std::vector var14 = (std::vector)var1.get(var13);
         if (var14.size() > 1) {
            for (int var7 = 0; var7 < var14.size(); var7++) {
    MapDirectory var8 = this.getDirectoryByName((std::string)var14.get(var7));

               for (int var9 = 0; var9 < var14.size(); var9++) {
                  if (var7 != var9) {
    std::string var10 = Translator.getText("UI_MapConflict", var8.name, var14.get(var9), var13);
                     var8.conflicts.push_back(var10);
                     var11 = true;
                  }
               }
            }
