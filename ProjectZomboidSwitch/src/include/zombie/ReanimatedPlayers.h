...
public:
    static ReanimatedPlayers instance = std::make_shared<ReanimatedPlayers>();
   private const std::vector<IsoZombie> Zombies = std::make_unique<std::vector<>>();
    ...
    IsoGridSquare var8 = var1.getGridSquare((int)var7.getX() - var2, (int)var7.getY() - var3, (int)var7.getZ());
            if (var8 != nullptr) {
               ...
               IsoWorld.instance.CurrentCell.getObjectList().push_back(var7);
               IsoWorld.instance.CurrentCell.getZombieList().push_back(var7);
    ...
    FileOutputStream var11 = std::make_shared<FileOutputStream>(var9);
    BufferedOutputStream var6 = std::make_shared<BufferedOutputStream>(var11);
