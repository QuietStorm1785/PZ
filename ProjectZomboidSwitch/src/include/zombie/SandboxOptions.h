...
public:
    static const SandboxOptions instance = std::make_shared<SandboxOptions>();
    int Speed = 3;
   ...
   protected const std::vector<SandboxOption> options = std::make_unique<std::vector<>>();
   protected const std::unordered_map<std::string, SandboxOption> optionByName = std::make_unique<std::unordered_map<>>();
    const std::unordered_map Map = std::make_shared<Map>(this);
    const ZombieLore Lore = std::make_shared<ZombieLore>(this);
    const ZombieConfig zombieConfig = std::make_shared<ZombieConfig>(this);
    const int FIRST_YEAR = 1993;
    const int SANDBOX_VERSION = 5;
   private const std::vector<SandboxOption> m_customOptions = std::make_unique<std::vector<>>();
    ...
    public SandboxOptions() {
      ...
      this.WaterShutModifier = (IntegerSandboxOption)this.newIntegerOption("WaterShutModifier", -1, int.MAX_VALUE, 14).setTranslation("WaterShut");
      this.ElecShutModifier = (IntegerSandboxOption)this.newIntegerOption("ElecShutModifier", -1, int.MAX_VALUE, 14).setTranslation("ElecShut");
      ...
      this.SeenHoursPreventLootRespawn = this.newIntegerOption("SeenHoursPreventLootRespawn", 0, int.MAX_VALUE, 0);
      ...
      this.DaysForRottenFoodRemoval = this.newIntegerOption("DaysForRottenFoodRemoval", -1, int.MAX_VALUE, -1);
    ...
    void toLua() {
   // Refactored: Assume Lua table is on the stack at given index
   // Example: lua_getglobal(L, "SandboxVars");

      for (int var2 = 0; var2 < this.options.size(); var2++) {
         this.options.get(var2)->toTable(L, -1);
      }
   }
   ...
   // Refactored: Assume Lua table is on the stack at given index
      ...
      for (int var2 = 0; var2 < this.options.size(); var2++) {
         this.options.get(var2)->fromTable(L, -1);
      }
   ...
   private static std::string[] parseName(std::string var0) {
      std::string[] var1 = new std::string[]{nullptr, var0};
      if (var0.contains(".")) {
    ...
    SandboxOptions var1 = std::make_shared<SandboxOptions>();
    ...
    ConfigFile var3 = std::make_shared<ConfigFile>();
      if (!var3.read(var1)) {
    return false;
      } else {
    ...
    void* var3 = LuaManager.RunLua(var2.getAbsolutePath());
            if (!(dynamic_cast<KahluaTable*>(var3) != nullptr)) {
    ...
    void* var16 = LuaManager.env.rawget("SandboxVars");
            if (var16 != nullptr) {
               if (dynamic_cast<KahluaTable*>(var16) != nullptr var17) {
    int var8 = 0;
    void* var9 = var17.rawget("VERSION");
                  if (var9 != nullptr) {
                     if (dynamic_cast<double*>(var9) != nullptr) {
    ...
    std::vector var6 = new std::vector();
            var5.put("", std::make_unique<std::vector>());

            for (SandboxOption var8 : this.options) {
               if (var8.getTableName() == nullptr) {
                  ((std::vector)var5.get("")).push_back(var8);
               } else {
                  if (var5.get(var8.getTableName()) == nullptr) {
                     var5.put(var8.getTableName(), std::make_unique<std::vector>());
                     var6.push_back(var8.getTableName());
                  }

                  ((std::vector)var5.get(var8.getTableName())).push_back(var8);
               }
            }
    ...
    std::string var20 = System.lineSeparator();
            ...
            for (SandboxOption var9 : (std::vector)var5.get("")) {
               if (!var2) {
    ...
    for (auto& var23 : var6)               var4.write("    " + var23 + " = {" + var20);
               ...
               for (SandboxOption var27 : (std::vector)var5.get(var23)) {
                  if (!var2) {
    std::string var28 = var27.asConfigOption().getTooltip();
                     ...
                     if (dynamic_cast<EnumSandboxOption*>(var27) != nullptr) {
    ...
    KahluaTableIterator var5 = var2.iterator();
      ...
      while (var5.advance()) {
         if (!(var5.getKey() instanceof std::string)) {
            throw IllegalStateException("expected a std::string key");
         }
    ...
    std::string var7 = this.upgradeOptionValue(var8, var5.getValue(), var3);
    ...
    CustomBooleanSandboxOption var2 = (CustomBooleanSandboxOption)Type.tryCastTo(var1, CustomBooleanSandboxOption.class);
      if (var2 != nullptr) {
         this.addCustomOption(std::make_shared<BooleanSandboxOption>(this, var2.m_id, var2.defaultValue), var1);
      } else {
    CustomDoubleSandboxOption var3 = (CustomDoubleSandboxOption)Type.tryCastTo(var1, CustomDoubleSandboxOption.class);
         if (var3 != nullptr) {
            this.addCustomOption(std::make_shared<DoubleSandboxOption>(this, var3.m_id, var3.min, var3.max, var3.defaultValue), var1);
         } else {
    CustomEnumSandboxOption var4 = (CustomEnumSandboxOption)Type.tryCastTo(var1, CustomEnumSandboxOption.class);
            if (var4 != nullptr) {
    EnumSandboxOption var7 = std::make_shared<EnumSandboxOption>(this, var4.m_id, var4.numValues, var4.defaultValue);
    ...
    CustomIntegerSandboxOption var5 = (CustomIntegerSandboxOption)Type.tryCastTo(var1, CustomIntegerSandboxOption.class);
               if (var5 != nullptr) {
                  this.addCustomOption(std::make_shared<IntegerSandboxOption>(this, var5.m_id, var5.min, var5.max, var5.defaultValue), var1);
               } else {
    CustomStringSandboxOption var6 = (CustomStringSandboxOption)Type.tryCastTo(var1, CustomStringSandboxOption.class);
                  if (var6 != nullptr) {
                     this.addCustomOption(std::make_shared<StringSandboxOption>(this, var6.m_id, var6.defaultValue, -1), var1);
                  } else {
    ...
    void addCustomOption(SandboxOption var1, CustomSandboxOption var2) {
      ...
      this.m_customOptions.push_back(var1);
   }
