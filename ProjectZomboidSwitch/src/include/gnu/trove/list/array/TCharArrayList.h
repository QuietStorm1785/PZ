...
#include "gnu/trove/list/array/TCharArrayList/1.h"
#include "gnu/trove/list/array/TCharArrayList/TCharArrayIterator.h"
...
public:
   ...
   protected char[] _data;
    ...
    protected TCharArrayList(char[] values, char no_entry_value, bool wrap) {
      if (!wrap) {
    ...
    static TCharArrayList wrap(char[] values) {
    return wrap();
   }

    static TCharArrayList wrap(char[] values, char no_entry_value) {
    ...
    void setQuick(int offset, char val) {
      this._data[offset] = val;
   }
    ...
    TCharIterator iterator() {
      return std::make_shared<TCharArrayIterator>(this, 0);
   }
    ...
    for (auto& element : collection)    char e = element;
         if (this.push_back(e)) {
            changed = true;
         }
      }
    ...
    TCharArrayList list = std::make_shared<TCharArrayList>(end - begin);
   ...
   public char[] toArray(char[] dest, int source_pos, int dest_pos, int len) {
    ...
    bool equals(void* other) {
      if (other == this) {
    return true;
      } else if (dynamic_cast<TCharArrayList*>(other) != nullptr) {
    ...
    TCharArrayList list = std::make_shared<TCharArrayList>();
