...
#include "gnu/trove/set/hash/TShortHashSet/TShortHashIterator.h"
#include <algorithm>
#include <fstream>
#include <iostream>
...
class TShortHashSet : public TShortHash {
public:
    ...
    public TShortHashSet(Collection<? extends) {
    ...
    public TShortHashSet(TShortCollection collection) {
      this(Math.max(collection.size(), 10));
      if (dynamic_cast<TShortHashSet*>(collection) != nullptr) {
    ...
    TShortIterator iterator() {
      return std::make_shared<TShortHashIterator>(this, this);
   }
