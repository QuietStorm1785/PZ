...
class TCharByteHash : public TPrimitiveHash {
public:
   ...
   public transient char[] _set;
    ...
    public TCharByteHash(int initialCapacity, float loadFactor, char no_entry_key, uint8_t no_entry_value) {
      super(initialCapacity, loadFactor);
    ...
    int indexRehashed(char key, int index, int hash, uint8_t state) {
    ...
    int insertKeyRehash(char val, int index, int hash, uint8_t state) {
    int length = this._set.length;
    int probe = 1 + hash % (length - 2);
    int loopIndex = index;
    int firstRemoved = -1;
