...
class TIntDoubleHash : public TPrimitiveHash {
public:
   ...
   public transient int[] _set;
    ...
    public TIntDoubleHash(int initialCapacity, float loadFactor, int no_entry_key, double no_entry_value) {
      super(initialCapacity, loadFactor);
    ...
    int indexRehashed(int key, int index, int hash, uint8_t state) {
    ...
    int insertKeyRehash(int val, int index, int hash, uint8_t state) {
    int length = this._set.length;
    int probe = 1 + hash % (length - 2);
    int loopIndex = index;
    int firstRemoved = -1;
