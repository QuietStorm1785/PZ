...
class TShortShortHash : public TPrimitiveHash {
public:
   ...
   public transient short[] _set;
    ...
    public TShortShortHash(int initialCapacity, float loadFactor, short no_entry_key, short no_entry_value) {
      super(initialCapacity, loadFactor);
    ...
    int indexRehashed(short key, int index, int hash, uint8_t state) {
    ...
    int insertKeyRehash(short val, int index, int hash, uint8_t state) {
    int length = this._set.length;
    int probe = 1 + hash % (length - 2);
    int loopIndex = index;
    int firstRemoved = -1;
