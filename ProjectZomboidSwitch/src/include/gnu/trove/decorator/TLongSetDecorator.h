...
#include <cstdint>
#include "gnu/trove/decorator/TLongSetDecorator/1.h"
...
class TLongSetDecorator : public AbstractSet {
public:
    ...
    int i = that.size();
            ...
            while (i-- > 0) {
    void* val = it.next();
               if (!(dynamic_cast<int64_t*>(val) != nullptr)) {
    return false;
               }
    ...
    bool remove(void* value) {
      return dynamic_cast<int64_t*>(value) != nullptr && this._set.remove((int64_t)value);
   }

   public Iterator<int64_t> iterator() {
    ...
    bool contains(void* o) {
      return !(dynamic_cast<int64_t*>(o) != nullptr) ? false : this._set.contains((int64_t)o);
   }
