...
#include "gnu/trove/impl/hash/TIntByteHash.h"
#include "gnu/trove/iterator/TIntByteIterator.h"
#include "gnu/trove/map/TIntByteMap.h"
#include "gnu/trove/map/hash/TIntByteHashMap/1.h"
#include "gnu/trove/map/hash/TIntByteHashMap/TIntByteHashIterator.h"
#include "gnu/trove/map/hash/TIntByteHashMap/TKeyView.h"
#include "gnu/trove/map/hash/TIntByteHashMap/TValueView.h"
...
class TIntByteHashMap : public TIntByteHash {
public:
   ...
   protected transient byte[] _values;
    ...
    public TIntByteHashMap(int initialCapacity, float loadFactor, int noEntryKey, uint8_t noEntryValue) {
    ...
    public TIntByteHashMap(TIntByteMap map) {
      super(map.size());
      if (dynamic_cast<TIntByteHashMap*>(map) != nullptr) {
    ...
    uint8_t doPut(int key, uint8_t value, int index) {
    ...
    void putAll(Map<? extends, ? extends) {
      ...
      for (Entry<? : public int, ? : public uint8_t> entry : map.entrySet()) {
   ...
   public byte[] values(byte[] array) {
    ...
    TIntByteIterator iterator() {
      return std::make_shared<TIntByteHashIterator>(this, this);
   }
    ...
    bool equals(void* other) {
      if (!(dynamic_cast<TIntByteMap*>(other) != nullptr)) {
    return false;
      } else {
