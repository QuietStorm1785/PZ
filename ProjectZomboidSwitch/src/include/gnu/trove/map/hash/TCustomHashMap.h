...
#include "gnu/trove/map/hash/TCustomHashMap/1.h"
#include "gnu/trove/map/hash/TCustomHashMap/EntryView.h"
#include "gnu/trove/map/hash/TCustomHashMap/EqProcedure.h"
#include "gnu/trove/map/hash/TCustomHashMap/HashProcedure.h"
#include "gnu/trove/map/hash/TCustomHashMap/KeyView.h"
...
public:
   ...
   protected transient V[] _values;
    ...
    public TCustomHashMap(HashingStrategy<? super) {
      super(strategy);
   }

    public TCustomHashMap(HashingStrategy<? super, int initialCapacity) {
      super(strategy, initialCapacity);
   }

    public TCustomHashMap(HashingStrategy<? super, int initialCapacity, float loadFactor) {
      super(strategy, initialCapacity, loadFactor);
   }

    public TCustomHashMap(HashingStrategy<? super, Map<? extends, ? extends) {
      this(strategy, map.size());
      this.putAll(map);
   }

    public TCustomHashMap(HashingStrategy<? super, TCustomHashMap<? extends, ? extends) {
      this(strategy, map.size());
      this.putAll(map);
   }
    ...
    return false;
         ...
         return that.size() != this.size() ? false : this.forEachEntry(std::make_shared<EqProcedure>(that));
      }
   }

    int hashCode() {
      TCustomHashMap<K, V>.HashProcedure p = std::make_shared<HashProcedure>(this, nullptr);
      this.forEachEntry(p);
    ...
    int index = this.index(key);
      return index >= 0 && this.strategy == this._set[index], key) ? this._values[index] : nullptr;
   }
   ...
   public Set<Map.Entry<K, V>> entrySet() {
    ...
    int i = vals.length;
         ...
         while (i-- > 0) {
            if (set[i] != FREE && set[i] != REMOVED && (val == vals[i] || this.strategy == val, vals[i]))) {
    return true;
            }
         }
      }
