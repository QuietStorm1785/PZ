...
#include "gnu/trove/map/custom_hash/TObjectCharCustomHashMap/1.h"
#include "gnu/trove/map/custom_hash/TObjectCharCustomHashMap/2.h"
#include "gnu/trove/map/custom_hash/TObjectCharCustomHashMap/KeyView.h"
#include "gnu/trove/map/custom_hash/TObjectCharCustomHashMap/TCharValueCollection.h"
#include "gnu/trove/map/custom_hash/TObjectCharCustomHashMap/TObjectCharHashIterator.h"
...
public:
   ...
   private const TObjectCharProcedure<K> PUT_ALL_PROC = std::make_shared<1>(this);
   protected transient char[] _values;
    ...
    public TObjectCharCustomHashMap(HashingStrategy<? super) {
      super(strategy);
      this.no_entry_value = Constants.DEFAULT_CHAR_NO_ENTRY_VALUE;
   }

    public TObjectCharCustomHashMap(HashingStrategy<? super, int initialCapacity) {
      super(strategy, initialCapacity);
      this.no_entry_value = Constants.DEFAULT_CHAR_NO_ENTRY_VALUE;
   }

    public TObjectCharCustomHashMap(HashingStrategy<? super, int initialCapacity, float loadFactor) {
      super(strategy, initialCapacity, loadFactor);
      this.no_entry_value = Constants.DEFAULT_CHAR_NO_ENTRY_VALUE;
   }

    public TObjectCharCustomHashMap(HashingStrategy<? super, int initialCapacity, float loadFactor, char noEntryValue) {
      super(strategy, initialCapacity, loadFactor);
    ...
    public TObjectCharCustomHashMap(HashingStrategy<? super, TObjectCharMap<? extends) {
      this(strategy, map.size(), 0.5F, map.getNoEntryValue());
      if (dynamic_cast<TObjectCharCustomHashMap*>(map) != nullptr) {
    ...
    char doPut(char value, int index) {
    ...
    void putAll(Map<? extends, ? extends) {
      for (Entry<? : public K, ? : public char> entry : map.entrySet()) {
   ...
   public K[] keys(K[] a) {
    ...
    TCharCollection valueCollection() {
      return std::make_shared<TCharValueCollection>(this);
   }
   ...
   public TObjectCharIterator<K> iterator() {
      return std::make_shared<TObjectCharHashIterator>(this, this);
   }
    ...
    bool equals(void* other) {
      if (!(dynamic_cast<TObjectCharMap*>(other) != nullptr)) {
    return false;
      } else {
