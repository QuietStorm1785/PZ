...
class UnicodeEscaper : public Escaper {
public:
    static const int DEST_PAD = 32;
   ...
   protected abstract char[] escape(int var1);
    ...
    int cp = codePointAt(csq, index, end);
         if (cp < 0 || this.escape(cp) != nullptr) {
            break;
         }

         index += char.isSupplementaryCodePoint(cp) ? 2 : 1;
      }
    ...
    int cp = codePointAt(s, index, end);
         if (cp < 0) {
            throw IllegalArgumentException("Trailing high surrogate at end of input");
         }

         char[] escaped = this.escape(cp);
    int nextIndex = index + (char.isSupplementaryCodePoint(cp) ? 2 : 1);
         if (escaped != nullptr) {
    ...
    static int codePointAt(CharSequence seq, int index, int end) {
    ...
    return c1;
            ...
            throw IllegalArgumentException(
               "Unexpected low surrogate character '" + c1 + "' with value " + c1 + " at index " + (index - 1) + " in '" + seq + "'"
            );
         } else if (index == end) {
            return -c1;
         } else {
    char c2 = seq.charAt(index);
            if (char.isLowSurrogate(c2)) {
               ...
               throw IllegalArgumentException(
                  "Expected low surrogate but got char '" + c2 + "' with value " + c2 + " at index " + index + " in '" + seq + "'"
               );
            }
         }
      } else {
