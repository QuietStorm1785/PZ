...
#include "com/google/common/cache/LocalCache/WeightedWeakValueReference.h"
...
enum class LocalCache {
   <K, V> ValueReference<K, V> referenceValue(Segment<K, V> segment, ReferenceEntry<K, V> entry, V value, int weight) {
      return (ValueReference<K, V>)(weight == 1
         ? std::make_shared<WeakValueReference>(segment.valueReferenceQueue, value, entry)
         : std::make_shared<WeightedWeakValueReference>(segment.valueReferenceQueue, value, entry, weight));
   }
