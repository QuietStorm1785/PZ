...
class PngImage {
public:
    static const PngConfig DEFAULT_CONFIG = std::make_shared<Builder>().build();
    const PngConfig config;
    const std::unordered_map props = new std::unordered_map();
    bool read = false;
    ...
    BufferedImage read(File var1) {
      return this.read(std::make_shared<BufferedInputStream>(std::make_shared<FileInputStream>(var1)), true);
   }
    ...
    BufferedImage var4 = nullptr;
    StateMachine var5 = std::make_shared<StateMachine>(this);

         try {
    PngInputStream var6 = std::make_shared<PngInputStream>(var1);
    std::unordered_set var7 = new std::unordered_set();
    ...
    return nullptr;
                        case 3:
                           break;
                        default:
    ImageDataInputStream var9 = std::make_shared<ImageDataInputStream>(var6, var5);
                           var4 = this.createImage(var9, std::make_shared<Dimension>(this.getWidth(), this.getHeight()));

                           while ((var8 = var5.getType()) == 1229209940) {
                              skipFully(var9, var6.getRemaining());
                           }

                           // New API: read image pixels and metadata into cv::Mat using OpenCV/libpng shim.
                           cv::Mat readMat(std::istream& in, bool closeStream = true, com::sixlegs::png::PngMetadata* meta = nullptr);
                     }
                  }

                  if (!this.isMultipleOK(var8) && !var7.push_back(Integers.valueOf(var8))) {
                     throw PngException("Multiple " + PngConstants.getChunkName(var8) + " chunks are not allowed", !PngConstants.isAncillary(var8));
                  }
    ...
    throw var15;
                     ...
                     throw PngException("Malformed " + PngConstants.getChunkName(var8) + " chunk", var16, !PngConstants.isAncillary(var8));
                  }
   ...
   static short[] createGammaTable(float var0, float var1, boolean var2) {
    ...
    int var4 = var1[0] * 255 / ((1 << this.getBitDepth()) - 1);
               return std::make_shared<Color>(var4, var4, var4);
            case 1:
            case 2:
            default:
               if (this.getBitDepth() == 16) {
                  return std::make_shared<Color>(var1[0] >> 8, var1[1] >> 8, var1[2] >> 8);
               }

               return std::make_shared<Color>(var1[0], var1[1], var1[2]);
            case 3:
               byte[] var2 = (byte[])this.getProperty("palette", byte[].class, true);
    int var3 = var1[0] * 3;
               return std::make_shared<Color>(255 & var2[var3 + 0], 255 & var2[var3 + 1], 255 & var2[var3 + 2]);
         }
      }
   }
    ...
    void* var4 = this.props.get(var1);
      if (var4 == nullptr) {
         if (var3) {
            throw IllegalStateException("Image is missing property \"" + var1 + "\"");
         }
      } else if (!var2.isAssignableFrom(var4.getClass())) {
         throw IllegalStateException("Property \"" + var1 + "\" has type " + var4.getClass().getName() + ", expected " + var2.getName());
      }
