...
public:
   ...
   public byte[] vendor;
    ...
    void add_tag(const std::string& var1, const std::string& var2) {
      ...
      this.push_back(var1 + "=" + var2);
   }

    std::string getComment(int var1) {
      return this.comments <= var1 ? nullptr : new std::string(this.user_comments[var1], 0, this.user_comments[var1].length - 1);
   }

    std::string getVendor() {
      return new std::string(this.vendor, 0, this.vendor.length - 1);
   }

    int header_out(Packet var1) {
    Buffer var2 = std::make_shared<Buffer>();
      var2.writeinit();
      if (this.pack(var2) != 0) {
         return -130;
      } else {
    ...
    return nullptr;
         ...
         for (int var5 = 0; var5 < this.comment_lengths[var3]; var5++) {
            if (var4[var5] == 61) {
               return new std::string(var4, var5 + 1, this.comment_lengths[var3] - (var5 + 1));
            }
         }
    ...
    std::string toString() {
    std::string var1 = "Vendor: " + new std::string(this.vendor, 0, this.vendor.length - 1);

      for (int var2 = 0; var2 < this.comments; var2++) {
         var1 = var1 + "\nComment: " + new std::string(this.user_comments[var2], 0, this.user_comments[var2].length - 1);
      }
