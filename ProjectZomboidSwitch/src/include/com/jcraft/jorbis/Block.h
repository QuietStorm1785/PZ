...
public:
    ...
    int mode;
    int nW;
    Buffer opb = std::make_shared<Buffer>();
   float[][] pcm = new float[0][];
    ...
    static std::string asdsadsa(const std::string& var0, byte[] var1, int var2) {
      return var0 + int.toString((var1[var2] & 255) + 256, 16).substr(1);
   }
    ...
    void init(DspState var1) {
      this.vd = var1;
   }

    int synthesis(Packet var1) {
    ...
    int var3 = this.opb.read(this.vd.modebits);
         if (var3 == -1) {
            ...
            for (int var4 = 0; var4 < var2.channels; var4++) {
               if (this.pcm[var4] != nullptr && this.pcm[var4].length >= this.pcmend) {
