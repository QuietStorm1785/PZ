...
public:
    ...
    VorbisFile var1 = nullptr;

      try {
         if (var0.length > 0) {
            var1 = std::make_shared<VorbisFile>(var0[0]);
         } else {
            var1 = std::make_shared<VorbisFile>(System.in, nullptr, -1);
         }
    ...
    Info var3 = var1.getInfo(var2);
         ...
         DebugLog.log(
            "\t\t" + var3.rate + "Hz " + var3.channels + " channels bitrate " + var1.bitrate(var2) / 1000 + "kbps serial number=" + var1.serialnumber(var2)
         );
