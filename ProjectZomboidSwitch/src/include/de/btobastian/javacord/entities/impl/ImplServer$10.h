...
#include "de/btobastian/javacord/entities/impl/ImplServer/10/1.h"
...
class ImplServer {
public:
   ImplServer$10(ImplServer var1, std::string var2) {
      this.this$0 = var1;
      this.val$userId = var2;
   }

    Void call() {
      ImplServer.access$000().debug("Trying to kick an user from server {} (user id: {})", this.this$0);
      HttpResponse<JsonNode> response = Unirest.delete("https://discordapp.com/api/v6/guilds/" + this.this$0.getId() + "/members/" + this.val$userId)
    ...
    User user = (User)ImplServer.access$100(this.this$0).getUserById(this.val$userId).get();
      ...
      ImplServer.access$000().info("Kicked an user from server {} (user id: {})", this.this$0);
