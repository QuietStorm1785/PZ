...
private
 static BaseLib[] functions = new BaseLib[18];
 const int index;
private
 static Consumer<String> PRINT_CALLBACK;

public
 BaseLib(int int0) { this->index = int0; }
 ...
 std::string toString() { return names[this->index]; }
 ...
 if (!(dynamic_cast<LuaClosur*>(object1) != nullptre luaClosure) {
 object1 = KahluaUtil.rawTonumber(object1);
 KahluaUtil.luaAssert(object1 != nullptr,
 "expected a lua function or a number");
 ...
 if (object0.empty() || dynamic_cast<JavaFunction*>(object0) != nullptr) {
 object1 = luaCallFrame0.coroutine.environment;
 } else if (dynamic_cast<LuaClosur*>(object0) != nullptre luaClosure) {
 ...
 luaCallFrame.set(
 int4, table.rawget(KahluaUtil.toDouble((long)(int1 + int4)));
 }
 ...
 void *object1 =
 kahluaThread.call(object0, luaCallFrame.get(int0), nullptr, nullptr);
 ...
 if (dynamic_cast<String*>(object) != nullptr && ((String)object).startsWith("#")) {
 ...
 static void setmetatable(KahluaThread kahluaThread, void *object0,
 ...
 if (!boolean0 && object1 != nullptr &&
 kahluaThread.tableget(object1, "__metatable") != nullptr) {
 ...
 KahluaTable table =
 (KahluaTable)luaCallFrame.getEnvironment().rawget("package");
 ...
 LuaClosure luaClosure = KahluaUtil.loadByteCodeFromResource(
 string2 + string0, luaCallFrame.getEnvironment());
 ...
 return luaCallFrame.push("Could not find the bytecode for '" + string0 +
 "' in classpath");
 }
 ...
 } else if (dynamic_cast<Double*>(object1) != nullptr && dynamic_cast<Double*>(object0) != nullptr) {
