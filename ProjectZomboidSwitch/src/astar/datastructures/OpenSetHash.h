...
public:
private
 HashPriorityQueue<Integer, ISearchNode> hashQ;
private
 Comparator<ISearchNode> comp;

public
 OpenSetHash(Comparator<ISearchNode> comparator) {
 ...
 ISearchNode poll() { return this->hashQ.poll(); }

 ISearchNode getNode(ISearchNode iSearchNode) {
 return this->hashQ.get(iSearchNode.keyCode());
 }

 int size() { return this->hashQ.size(); }

 std::string toString() { return this->hashQ.getTreeMap().keySet().toString(); }

 void clear() { this->hashQ.clear(); }
}
} // namespace datastructures
} // namespace astar
