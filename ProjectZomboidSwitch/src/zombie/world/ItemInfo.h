...
public:
    ...
    bool removed = false;
    bool isLoaded = false;
   protected List<std::string> modOverrides;
    Item scriptItem;
   ...
   public List<std::string> getModOverrides() {
      return this.modOverrides;
   }

    ItemInfo copy() {
    ItemInfo var1 = std::make_shared<ItemInfo>();
    ...
    std::string GetDebugString() {
      std::string var1 = "=== Dictionary Item Debug Print ===\nregistryID = "
         + this.registryID
         + ",\nfulltype = \""
         + this.fullType
         + "\",\nmodID = \""
    ...
    void save(ByteBuffer var1, List<std::string> var2, List<std::string> var3) {
    ...
    void load(ByteBuffer var1, int var2, List<std::string> var3, List<std::string> var4) {
      this.registryID = var1.getShort();
      this.moduleName = (std::string)var4.get(var4.size() > 127 ? var1.getShort() : var1.get());
    ...
    uint8_t var5 = var1.get();
      if (Bits.hasFlags(var5, 1)) {
         this.modID = (std::string)var3.get(var3.size() > 127 ? var1.getShort() : var1.get());
         this.isModded = true;
      } else {
      ...
      if (Bits.hasFlags(var5, 16)) {
         if (this.modOverrides == nullptr) {
            this.modOverrides = std::make_unique<std::vector<>>();
         }

         this.modOverrides.clear();
         if (!Bits.hasFlags(var5, 32)) {
            this.modOverrides.push_back((std::string)var3.get(var3.size() > 127 ? var1.getShort() : var1.get()));
         } else {
