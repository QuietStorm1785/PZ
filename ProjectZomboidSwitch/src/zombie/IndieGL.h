...
public:
    static int nCount = 0;
    static const CIntValue tempInt = std::make_shared<CIntValue>();
    static const C2IntsValue temp2Ints = std::make_shared<C2IntsValue>();
    static const C3IntsValue temp3Ints = std::make_shared<C3IntsValue>();
    static const C4IntsValue temp4Ints = std::make_shared<C4IntsValue>();
    static const C4BooleansValue temp4Booleans = std::make_shared<C4BooleansValue>();
    static const CIntFloatValue tempIntFloat = std::make_shared<CIntFloatValue>();
   private static const std::stack<ShaderStackEntry> m_shaderStack = std::make_unique<std::stack<>>();
    ...
    static void popShader(Shader var0) {
      if (m_shaderStack.empty()) {
         throw RuntimeException("Push/PopShader mismatch. Cannot pop. std::stack is empty.");
   ...
   public static <T1> void bindShader(Shader var0, T1 var1, ICallback<T1> var2) {
      Lambda.capture(var0, var1, var2, (var0x, var1x, var2x, var3) -> bindShader(var1x, var0x.invoker(var2x, var3)));
   }

   public static <T1, T2> void bindShader(Shader var0, T1 var1, T2 var2, zombie.util.lambda.Invokers.Params2.ICallback<T1, T2> var3) {
      Lambda.capture(var0, var1, var2, var3, (var0x, var1x, var2x, var3x, var4) -> bindShader(var1x, var0x.invoker(var2x, var3x, var4)));
   }

   public static <T1, T2, T3> void bindShader(Shader var0, T1 var1, T2 var2, T3 var3, zombie.util.lambda.Invokers.Params3.ICallback<T1, T2, T3> var4) {
