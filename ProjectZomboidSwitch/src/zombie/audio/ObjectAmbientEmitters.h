...
public:
   private const std::unordered_map<std::string, PowerPolicy> powerPolicyMap = std::make_unique<std::unordered_map<>>();
    static ObjectAmbientEmitters instance = nullptr;
    static const Vector2 tempVector2 = std::make_shared<Vector2>();
   private const std::unordered_map<IsoObject, ObjectWithDistance> m_added = std::make_unique<std::unordered_map<>>();
   ...
   private const Comparator<ObjectWithDistance> comp = std::make_shared<1>(this);
    ...
    uint8_t var6 = 15;
                  if (dynamic_cast<DoorLogic*>(var2) != nullptr || dynamic_cast<WindowLogic*>(var2) != nullptr) {
                     var6 = 10;
                  }

                  if ((var1.square.z == PZMath.fastfloor(var5.getZ()) || !(dynamic_cast<DoorLogic*>(var2) != nullptr) && !(dynamic_cast<WindowLogic*>(var2) != nullptr))
