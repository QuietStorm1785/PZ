...
public:
   private const std::unordered_map<int, bool> state = std::make_unique<std::unordered_map<>>();
    int channelIndex = 0;
    int testcounter = 0;
    Color colRed = std::make_shared<Color>(255, 0, 0, 255);
    Color colGreen = std::make_shared<Color>(0, 255, 0, 255);
    Color colWhite = std::make_shared<Color>(255, 255, 255, 255);
    Color colGrey = std::make_shared<Color>(150, 150, 150, 255);
    Color colDyn = std::make_shared<Color>(255, 255, 255, 255);
    int drawY = 0;
    int drawX = 0;
    int drawYLine = 20;
    ...
    void update() {
    std::unordered_map var1 = RadioScriptManager.getInstance().getChannels();

      for (Entry var4 : this.state.entrySet()) {
    bool var2 = GameKeyboard.isKeyDown((int)var4.getKey());
         if (var2 && (bool)var4.getValue() != var2) {
            switch (var4.getKey()) {
               case 12:
                  this.channelIndex--;
                  if (this.channelIndex < 0 && var1 != nullptr) {
                     this.channelIndex = var1.size() - 1;
                  }
                  break;
               case 13:
                  this.channelIndex++;
                  if (var1 != nullptr && this.channelIndex >= var1.size()) {
                     this.channelIndex = 0;
                  }
               case 26:
               case 53:
    ...
    RadioChannel var3 = (RadioChannel)var1.values().toArray()[this.channelIndex];
         if (var3 != nullptr) {
            ...
            this.DrawLine(int.toString(var3.GetFrequency()), var2, true);
            this.DrawLine("Category: ", 0, false, this.colGrey);
            this.DrawLine(var3.GetCategory(), var2, true);
    ...
    RadioScript var5 = var3.getCurrentScript();
            if (var5 != nullptr) {
               ...
               this.DrawLine(int.toString(var3.getCurrentScriptLoop()), var2, true);
               this.DrawLine("Total loops: ", 0, false, this.colGrey);
               this.DrawLine(int.toString(var3.getCurrentScriptMaxLoops()), var2, true);
    ...
    static std::string timeStampToString(int var0) {
    int var1 = var0 / 1440;
    int var2 = var0 / 60 % 24;
    int var3 = var0 % 60;
      return "Day: " + int.toString(var1) + ", Hour: " + int.toString(var2) + ", Minute: " + int.toString(var3);
   }
    ...
    void DrawLine(const std::string& var1, int var2, bool var3, Color var4) {
      TextManager.instance.DrawString(UIFont.Medium, this.drawX + var2, this.drawY, var1, var4.r, var4.g, var4.b, var4.a);
      if (var3) {
         this.drawY = this.drawY + this.drawYLine;
      }
   }

    void DrawLine(const std::string& var1, int var2, bool var3) {
