#include <string>
#include "zombie\iso\areas/IsoArea.h"

namespace zombie {
namespace iso {
namespace areas {

new IsoArea::FileInputStream() {
    // TODO: Implement FileInputStream
    return nullptr;
}

std::string IsoArea::Ardo(const std::string& var0) {
    // TODO: Implement Ardo
    return "";
}

bool IsoArea::Thigglewhat2(const std::string& var0, const std::string& var1) {
    // TODO: Implement Thigglewhat2
    return false;
}

std::string IsoArea::Thigglewhat22(const std::string& var0) {
    // TODO: Implement Thigglewhat22
    return "";
}

bool IsoArea::Thigglewhat() {
    // TODO: Implement Thigglewhat
    return false;
}

} // namespace areas
} // namespace iso
} // namespace zombie
