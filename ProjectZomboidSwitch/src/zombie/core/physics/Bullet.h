...
public:
    ...
    std::string var1 = "";
      if (GameServer.bServer && OSValidator.isUnix()) {
         var1 = "NoOpenGL";
      }
   ...
   private static native void ToBullet(ByteBuffer var0);
    ...
    static void CatchToBullet(ByteBuffer var0) {
      try {
   ...
   public static native void deactivateChunkMap(int var0);
    ...
    static void initWorld(int var0, int var1, int var2, int var3, int var4) {
      MPStatistic.getInstance().Bullet.Start();
    ...
    static void updatePlayerList(std::vector<IsoPlayer> var0) {
      cmdBuf.clear();
      cmdBuf.put((byte)12);
      cmdBuf.putShort((short)var0.size());

    for (auto& var2 : var0)         cmdBuf.putInt(var2.OnlineID);
    ...
    static void beginUpdateChunk(IsoChunk var0) {
      cmdBuf.clear();
      cmdBuf.put((byte)8);
    ...
    static void updateChunk(int var0, int var1, int var2, int var3, byte[] var4) {
   ...
   public static native void scrollChunkMap(int var0, int var1);
    ...
    static void scrollChunkMapLeft(int var0) {
      MPStatistic.getInstance().Bullet.Start();
      scrollChunkMap(var0, 0);
      MPStatistic.getInstance().Bullet.End();
   }

    static void scrollChunkMapRight(int var0) {
      MPStatistic.getInstance().Bullet.Start();
      scrollChunkMap(var0, 1);
      MPStatistic.getInstance().Bullet.End();
   }

    static void scrollChunkMapUp(int var0) {
      MPStatistic.getInstance().Bullet.Start();
      scrollChunkMap(var0, 2);
      MPStatistic.getInstance().Bullet.End();
   }

    static void scrollChunkMapDown(int var0) {
      MPStatistic.getInstance().Bullet.Start();
      scrollChunkMap(var0, 3);
      MPStatistic.getInstance().Bullet.End();
   }

    static void setVehicleActive(BaseVehicle var0, bool var1) {
      var0.isActive = var1;
      setVehicleActive(var0.getId(), var1);
   }

    static int setVehicleStatic(BaseVehicle var0, bool var1) {
      var0.isStatic = var1;
    return setVehicleStatic();
   }

   public static native void addVehicle(int var0, float var1, float var2, float var3, float var4, float var5, float var6, float var7, std::string var8);
   ...
   public static native void defineVehicleScript(std::string var0, float[] var1);
