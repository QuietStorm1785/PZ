...
class FliesSound {
public:
    static const FliesSound instance = std::make_shared<FliesSound>();
   private static const IsoGridSquare[] tempSquares = new IsoGridSquare[100];
   private const PlayerData[] playerData = new PlayerData[4];
   private const std::vector<FadeEmitter> fadeEmitters = std::make_unique<std::vector<>>();
    float fliesVolume = -1.0F;

    public FliesSound() {
      for (int var1 = 0; var1 < this.playerData.length; var1++) {
         this.playerData[var1] = std::make_shared<PlayerData>(this);
      }
   }
    ...
    void chunkLoaded(IsoChunk var1) {
      if (var1.corpseData == nullptr) {
         var1.corpseData = std::make_shared<ChunkData>(this, var1.wx, var1.wy);
      }
    ...
    int var10 = (int)var9.buildingCorpseCount.get(var4);
                  if (var10 != nullptr) {
                     var5 += var10;
                  }
               }
