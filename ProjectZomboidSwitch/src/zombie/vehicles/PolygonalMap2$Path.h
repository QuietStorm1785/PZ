...
class PolygonalMap2 {
public:
   const std::vector<PathNode> nodes = std::make_unique<std::vector<>>();
   const ArrayDeque<PathNode> nodePool = std::make_unique<ArrayDeque<>>();
    ...
    PathNode var5 = this.nodePool.empty() ? std::make_shared<PathNode>() : this.nodePool.pop();
    ...
    PathNode addNode(SearchNode var1) {
      return this.addNode(var1.getX(), var1.getY(), var1.getZ(), var1.vgNode == nullptr ? 0 : var1.vgNode.flags);
   }
    ...
    bool crossesSquare(int var1, int var2, int var3) {
