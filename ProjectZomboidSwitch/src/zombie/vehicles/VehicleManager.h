...
#include "zombie/vehicles/VehicleManager/1.h"
...
public:
   ...
   private const std::vector<BaseVehicle> sendReliable = std::make_unique<std::vector<>>();
   private const std::vector<BaseVehicle> sendUnreliable = std::make_unique<std::vector<>>();
    const TShortArrayList vehiclesWaitUpdates = std::make_shared<TShortArrayList>(128);
    const TShortShortHashMap towedVehicleMap = std::make_shared<TShortShortHashMap>();
   public static std::unordered_map<uint8_t, std::string> vehiclePacketTypes = std::make_unique<std::unordered_map<>>();
   public const UdpConnection[] connected = new UdpConnection[512];
   private const float[] tempFloats = new float[27];
   private const float[] engineSound = new float[2];
    const PosUpdateVars posUpdateVars = std::make_shared<PosUpdateVars>();
    const UpdateLimit vehiclesWaitUpdatesFrequency = std::make_shared<UpdateLimit>(1000L);
    BaseVehicle tempVehicle;
   private const std::vector<ModelInfo> oldModels = std::make_unique<std::vector<>>();
   private const std::vector<ModelInfo> curModels = std::make_unique<std::vector<>>();
    const UpdateLimit sendRequestGetPositionFrequency = std::make_shared<UpdateLimit>(500L);
    const UpdateLimit VehiclePhysicSyncPacketLimit = std::make_shared<UpdateLimit>(500L);

    void registerVehicle(BaseVehicle var1) {
      this.IDToVehicle.put(var1.VehicleID, var1);
      this.idMapDirty = true;
   }

    void unregisterVehicle(BaseVehicle var1) {
      this.IDToVehicle.remove(var1.VehicleID);
      this.idMapDirty = true;
   }

    BaseVehicle getVehicleByID(short var1) {
   ...
   public std::vector<BaseVehicle> getVehicles() {
    ...
    void removeFromWorld(BaseVehicle var1) {
      if (var1.VehicleID != -1) {
    ...
    UdpConnection var3 = (UdpConnection)GameServer.udpEngine.connections.get(var2);
               if (var1.connectionState[var3.index] != nullptr) {
    ...
    std::vector var1 = IsoWorld.instance.CurrentCell.getVehicles();
      ...
      for (int var2 = 0; var2 < this.connected.length; var2++) {
         if (this.connected[var2] != nullptr && !GameServer.udpEngine.connections.contains(this.connected[var2])) {
            DebugLog.Vehicle.trace("vehicles: dropped connection %d", var2);

            for (int var8 = 0; var8 < var1.size(); var8++) {
               ((BaseVehicle)var1.get(var8)).connectionState[var2] = nullptr;
            }

            this.connected[var2] = nullptr;
         } else {
            for (int var3 = 0; var3 < var1.size(); var3++) {
               if (((BaseVehicle)var1.get(var3)).connectionState[var2] != nullptr) {
    ...
    void sendVehicles(UdpConnection var1, short var2) {
      if (var1.isFullyConnected()) {
         this.sendReliable.clear();
         this.sendUnreliable.clear();
    std::vector var3 = IsoWorld.instance.CurrentCell.getVehicles();
    ...
    BaseVehicle var5 = (BaseVehicle)var3.get(var4);
            ...
            if (var1.RelevantTo(var5.x, var5.y)) {
               if (var5.connectionState[var1.index] == nullptr) {
                  var5.connectionState[var1.index] = std::make_unique<ServerVehicleState>();
               }
    ...
    ServerVehicleState var6 = var5.connectionState[var1.index];
               if (var6.shouldSend(var5)) {
                  if (!var5.isReliable && PacketType.Vehicles.getId() != var2) {
                     this.sendUnreliable.push_back(var5);
                  } else {
                     this.sendReliable.push_back(var5);
                  }
               }
            }
         }
    ...
    void sendVehiclesInternal(UdpConnection var1, std::vector<BaseVehicle> var2, PacketType var3) {
      if (!var2.empty()) {
    ...
    InventoryItem var14 = var34.getInventoryItem();
                           if (dynamic_cast<DrainableComboItem*>(var14) != nullptr) {
                              var5.put((byte)var26);
    ...
    VehicleAuthorizationPacket var21 = std::make_shared<VehicleAuthorizationPacket>();
    ...
    uint8_t var33 = var1.get();
            DebugLog.Vehicle.trace("Vehicle enter vid=%d pid=%d seat=%d", var18, var26, int.valueOf(var33));
    ...
    uint8_t var32 = var1.get();
            DebugLog.Vehicle.trace("Vehicle exit vid=%d pid=%d seat=%d", var17, var25, int.valueOf(var32));
    ...
    uint8_t var31 = var1.get();
    uint8_t var38 = var1.get();
            DebugLog.Vehicle.trace("Vehicle switch seat vid=%d pid=%d seats=%d=>%d", var16, var24, int.valueOf(var31), int.valueOf(var38));
    ...
    IsoPlayer var11 = (IsoPlayer)Type.tryCastTo(var43.getCharacter(var38), IsoPlayer.class);
         ...
         default:
            ...
            break;
         case 9:
    Physics var15 = std::make_shared<Physics>();
    ...
    BaseVehicle var36 = this.IDToVehicle.get(var29);
               if (var36 != nullptr) {
                  if (var36.connectionState[var2.index] == nullptr) {
                     var36.connectionState[var2.index] = std::make_unique<ServerVehicleState>();
                  }
    ...
    void serverSendInitialWorldState(UdpConnection var1) {
    ...
    void vehiclePosUpdate(BaseVehicle var1, float[] var2) {
    int var3 = 0;
    ...
    std::vector var9 = this.getVehicles();
    ...
    void clientUpdateVehiclePos(BaseVehicle var1, float var2, float var3, float var4, IsoGridSquare var5) {
      var1.setX(var2);
      var1.setY(var3);
      var1.setZ(0.0F);
      var1.square = var5;
    ...
    void clientReceiveUpdateFull(ByteBuffer var1, short var2, float var3, float var4, float var5) {
    ...
    BaseVehicle var10 = this.IDToVehicle.get(var2);
         ...
         if (var10.physics != nullptr && (var10.getDriver() == nullptr || !var10.getDriver().isLocal())) {
    ...
    BaseVehicle var12 = std::make_shared<BaseVehicle>(IsoWorld.instance.CurrentCell);
         ...
         if (var9 != nullptr) {
            ...
            var12.chunk.vehicles.push_back(var12);
            var12.addToWorld();
         }
    ...
    IsoPlayer var14 = IsoPlayer.players[var13];
            if (var14 != nullptr && !var14.isDead() && var14.getVehicle() == nullptr) {
               IsoWorld.instance.CurrentCell.putInVehicle(var14);
            }
         }

         DebugLog.Vehicle.trace("added vehicle id=%d %s", var12.VehicleID, var9 == nullptr ? " (delayed)" : "");
      }
   }

   // $VF: Unable to simplify switch on enum
   // Please report this to the Vineflower issue tracker, at https://github.com/Vineflower/vineflower/issues with a copy of the class file (if you have the rights to distribute it!)
    void clientReceiveUpdate(ByteBuffer var1) {
    ...
    bool var19 = var1.get() == 1;
    bool var11 = var1.get() == 1;
    InventoryItem var12 = nullptr;
            if (var1.get() == 1) {
               try {
    ...
    bool var21 = var1.get() == 1;
    bool var41 = var1.get() == 1;
    uint8_t var53 = var1.get();
    ...
    InventoryItem var61 = var54.getInventoryItem();
               if (dynamic_cast<DrainableComboItem*>(var61) != nullptr) {
    ...
    InventoryItem var62;
                  ...
                  var45.setInventoryItem(nullptr);
               }
    ...
    uint8_t var56 = var1.get();
    uint8_t var64 = var1.get();
    ...
    ModelInfo var16 = var9.setModelVisible(var14, var15, true);
               this.curModels.push_back(var16);
            }
    ...
    uint8_t var34 = var1.get();
    std::string var42 = GameWindow.ReadString(var1);
    ...
    uint8_t var41 = var1.get();
            DebugLog.Vehicle.trace("Vehicle enter vid=%d pid=%d seat=%d", var23, var33, int.valueOf(var41));
    ...
    uint8_t var40 = var1.get();
            DebugLog.Vehicle.trace("Vehicle exit vid=%d pid=%d seat=%d", var22, var32, int.valueOf(var40));
    ...
    uint8_t var39 = var1.get();
    uint8_t var46 = var1.get();
            DebugLog.Vehicle.trace("Vehicle switch seat vid=%d pid=%d seats=%d=>%d", var21, var31, int.valueOf(var39), int.valueOf(var46));
    ...
    IsoPlayer var9 = (IsoPlayer)Type.tryCastTo(var51.getCharacter(var46), IsoPlayer.class);
         ...
         case 5:
            if (this.tempVehicle == nullptr || this.tempVehicle.getCell() != IsoWorld.instance.CurrentCell) {
               this.tempVehicle = std::make_shared<BaseVehicle>(IsoWorld.instance.CurrentCell);
            }
    ...
    BaseVehicle var29 = this.IDToVehicle.get(var19);
            ...
            break;
         case 9:
    Physics var18 = std::make_shared<Physics>();
    ...
    Vector3f var28 = std::make_shared<Vector3f>();
    Vector3f var38 = std::make_shared<Vector3f>();
    ...
    short var26 = var1.getShort();
    std::string var36 = GameWindow.ReadString(var1);
    std::string var44 = GameWindow.ReadString(var1);
    ...
    BaseVehicle var8 = this.IDToVehicle.get(var26);
            if (var7 != nullptr && var8 != nullptr) {
               var7.addPointConstraint(nullptr, var8, var36, var44);
            }
            break;
         case 18:
    short var14 = var1.getShort();
    ...
    void sendCollide(BaseVehicle var1, IsoGameCharacter var2, bool var3) {
    short var4 = var2 == nullptr ? -1 : ((IsoPlayer)var2).OnlineID;
    ...
    static void sendSound(BaseVehicle var0, uint8_t var1) {
    ...
    static void sendSoundFromServer(BaseVehicle var0, uint8_t var1) {
      for (int var2 = 0; var2 < GameServer.udpEngine.connections.size(); var2++) {
    UdpConnection var3 = (UdpConnection)GameServer.udpEngine.connections.get(var2);
    ByteBufferWriter var4 = var3.startPacket();
         PacketType.Vehicles.doPacket(var4);
    ...
    void sendPassengerPosition(BaseVehicle var1, int var2, const std::string& var3) {
    ...
    void sendPassengerPosition(BaseVehicle var1, int var2, const std::string& var3, UdpConnection var4) {
    ...
    void sendRequestGetFull(short var1, PacketType var2) {
    ...
    ByteBufferWriter var3 = GameClient.connection.startPacket();
         ...
         PacketType.Vehicles.send(GameClient.connection);
         this.vehiclesWaitUpdates.push_back(var1);
      }
   }

    void sendRequestGetFull(List<VehicleCache> var1) {
    ...
    ByteBufferWriter var2 = GameClient.connection.startPacket();
         ...
         for (int var3 = 0; var3 < var1.size(); var3++) {
            ...
            this.vehiclesWaitUpdates.push_back(((VehicleCache)var1.get(var3)).id);
         }
    ...
    void sendRequestGetPosition(short var1, PacketType var2) {
    ...
    void sendAddImpulse(BaseVehicle var1, Vector3f var2, Vector3f var3) {
    UdpConnection var4 = nullptr;

      for (int var5 = 0; var5 < GameServer.udpEngine.connections.size() && var4 == nullptr; var5++) {
    UdpConnection var6 = (UdpConnection)GameServer.udpEngine.connections.get(var5);

         for (int var7 = 0; var7 < var6.players.length; var7++) {
    IsoPlayer var8 = var6.players[var7];
            if (var8 != nullptr && var8.getVehicle() != nullptr && var8.getVehicle().VehicleID == var1.VehicleID) {
               var4 = var6;
               break;
            }
         }
    ...
    void sendSwitchSeat(UdpConnection var1, BaseVehicle var2, IsoGameCharacter var3, int var4, int var5) {
    ...
    void sendSwitchSeat(BaseVehicle var1, IsoGameCharacter var2, int var3, int var4) {
      for (UdpConnection var6 : GameServer.udpEngine.connections) {
         this.sendSwitchSeat(var6, var1, var2, var3, var4);
      }
   }

    void sendEnter(UdpConnection var1, BaseVehicle var2, IsoGameCharacter var3, int var4) {
    ...
    void sendEnter(BaseVehicle var1, IsoGameCharacter var2, int var3) {
      for (UdpConnection var5 : GameServer.udpEngine.connections) {
         this.sendEnter(var5, var1, var2, var3);
      }
   }

    void sendExit(UdpConnection var1, BaseVehicle var2, IsoGameCharacter var3, int var4) {
    ByteBufferWriter var5 = var1.startPacket();
      PacketType.Vehicles.doPacket(var5);
      var5.bb.put((byte)3);
      var5.bb.putShort(var2.getId());
    ...
    void sendExit(BaseVehicle var1, IsoGameCharacter var2, int var3) {
      for (UdpConnection var5 : GameServer.udpEngine.connections) {
         this.sendExit(var5, var1, var2, (byte)var3);
      }
   }

    void sendPhysic(BaseVehicle var1) {
    ByteBufferWriter var2 = GameClient.connection.startPacket();
    PacketType var3 = var1.isReliable ? PacketType.Vehicles : PacketType.VehiclesUnreliable;
      var3.doPacket(var2);
      var2.bb.put((byte)9);
    Physics var4 = std::make_shared<Physics>();
      if (var4.set(var1)) {
         var4.write(var2);
    ...
    void sendTowing(UdpConnection var1, BaseVehicle var2, BaseVehicle var3, const std::string& var4, const std::string& var5) {
    ...
    void sendTowing(BaseVehicle var1, BaseVehicle var2, const std::string& var3, const std::string& var4) {
    ...
    void sendDetachTowing(UdpConnection var1, BaseVehicle var2, BaseVehicle var3) {
    ByteBufferWriter var4 = var1.startPacket();
      PacketType.Vehicles.doPacket(var4);
      var4.bb.put((byte)18);
      var4.bb.putShort(var2 == nullptr ? -1 : var2.VehicleID);
      var4.bb.putShort(var3 == nullptr ? -1 : var3.VehicleID);
      PacketType.Vehicles.send(var1);
   }

    void sendDetachTowing(BaseVehicle var1, BaseVehicle var2) {
      if (var1 != nullptr && this.towedVehicleMap.containsKey(var1.VehicleID)) {
         this.towedVehicleMap.remove(var1.VehicleID);
      }

      if (var2 != nullptr && this.towedVehicleMap.containsKey(var2.VehicleID)) {
    ...
    short getTowedVehicleID(short var1) {
      return this.towedVehicleMap.containsKey(var1) ? this.towedVehicleMap.get(var1) : -1;
   }

    void writePositionOrientation(ByteBuffer var1, BaseVehicle var2) {
