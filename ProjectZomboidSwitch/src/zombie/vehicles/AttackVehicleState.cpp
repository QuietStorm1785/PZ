#include "zombie/vehicles/AttackVehicleState.h"

namespace zombie {
namespace vehicles {

AttackVehicleState AttackVehicleState::instance() {
    // TODO: Implement instance
    return nullptr;
}

void AttackVehicleState::enter(IsoGameCharacter var1) {
    // TODO: Implement enter
}

void AttackVehicleState::execute(IsoGameCharacter var1) {
    // TODO: Implement execute
}

void AttackVehicleState::exit(IsoGameCharacter var1) {
    // TODO: Implement exit
}

void AttackVehicleState::animEvent(IsoGameCharacter var1, AnimEvent var2) {
    // TODO: Implement animEvent
}

bool AttackVehicleState::isAttacking(IsoGameCharacter var1) {
    // TODO: Implement isAttacking
    return false;
}

bool AttackVehicleState::isPassengerExposed(IsoGameCharacter var1) {
    // TODO: Implement isPassengerExposed
    return false;
}

} // namespace vehicles
} // namespace zombie
