...
public:
   ...
   protected static const std::unordered_map<std::string, GameSound> soundByName = std::make_unique<std::unordered_map<>>();
   protected static const std::vector<GameSound> sounds = std::make_unique<std::vector<>>();
    static const BankPreviewSound previewBank = std::make_shared<BankPreviewSound>();
    static const FilePreviewSound previewFile = std::make_shared<FilePreviewSound>();
    ...
    GameSound var1 = soundByName.get(var0);
         if (var1 == nullptr) {
            ...
            var1.name = var0;
            var1.category = "AUTO";
    GameSoundClip var2 = std::make_shared<GameSoundClip>(var1);
            var1.clips.push_back(var2);
            sounds.push_back(var1);
            soundByName.put(var0.replace(".wav", "").replace(".ogg", ""), var1);
            if (BaseSoundBank.dynamic_cast<FMODSoundBank*>(instance) != nullptr) {
    ...
    for (auto& var1 : sounds)            for (GameSoundClip var3 : var1.clips) {
               if (var3.getFile() != nullptr && var3.getFile().empty()) {
               }
            }
         }
      }
    ...
    GameSoundScript var2 = (GameSoundScript)var0.get(var1);
      ...
      loadNonBankSounds();
      loadINI();
      if (Core.bDebug && BaseSoundBank.dynamic_cast<FMODSoundBank*>(instance) != nullptr) {
    std::unordered_set var12 = new std::unordered_set();

    for (auto& var3 : sounds)            for (GameSoundClip var5 : var3.clips) {
               if (var5.getEvent() != nullptr && !var5.getEvent().empty()) {
                  var12.push_back(var5.getEvent());
               }
            }
         }
    ...
    FMODSoundBank var14 = (FMODSoundBank)BaseSoundBank.instance;
         ...
         for (FMODFootstep var18 : var14.footstepMap.values()) {
            var12.push_back(var18.wood);
            var12.push_back(var18.concrete);
            var12.push_back(var18.grass);
            var12.push_back(var18.upstairs);
            var12.push_back(var18.woodCreak);
         }

         for (FMODVoice var19 : var14.voiceMap.values()) {
            var12.push_back(var19.sound);
         }
    ...
    std::string var10 = javafmodJNI.FMOD_Studio_EventDescription_GetPath(var21[var9]);
         ...
         var17.sort(std::string::compareTo);
    ...
    GameSoundScript var3 = (GameSoundScript)var1.get(var2);
            ...
            } else if (!var3.gameSound.clips.empty()) {
   ...
   public static std::vector<std::string> getCategories() {
    std::unordered_set var0 = new std::unordered_set();

    for (auto& var2 : sounds)         var0.push_back(var2.getCategory());
      }

    std::vector var3 = new std::vector(var0);
      Collections.sort(var3);
    return var3;
   }

   public static std::vector<GameSound> getSoundsInCategory(std::string var0) {
    ...
    for (auto& var2 : sounds)    DoubleConfigOption var3 = std::make_shared<DoubleConfigOption>(var2.getName(), 0.0, 2.0, 0.0);
         var3.setValue(var2.getUserVolume());
         var0.push_back(var3);
      }

    std::string var4 = ZomboidFileSystem.instance.getCacheDir() + File.separator + "sounds.ini";
    ConfigFile var5 = std::make_shared<ConfigFile>();
