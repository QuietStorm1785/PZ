...
public:
    static const MovingObjectUpdateScheduler instance = std::make_shared<MovingObjectUpdateScheduler>();
    const MovingObjectUpdateSchedulerUpdateBucket fullSimulation = std::make_shared<MovingObjectUpdateSchedulerUpdateBucket>(1);
    const MovingObjectUpdateSchedulerUpdateBucket halfSimulation = std::make_shared<MovingObjectUpdateSchedulerUpdateBucket>(2);
    const MovingObjectUpdateSchedulerUpdateBucket quarterSimulation = std::make_shared<MovingObjectUpdateSchedulerUpdateBucket>(4);
    const MovingObjectUpdateSchedulerUpdateBucket eighthSimulation = std::make_shared<MovingObjectUpdateSchedulerUpdateBucket>(8);
    const MovingObjectUpdateSchedulerUpdateBucket sixteenthSimulation = std::make_shared<MovingObjectUpdateSchedulerUpdateBucket>(16);
    long frameCounter;
    bool isEnabled = true;
    ...
    IsoMovingObject var3 = (IsoMovingObject)var1.get(var2);
         if (!GameServer.bServer || !(dynamic_cast<IsoZombie*>(var3) != nullptr)) {
    bool var4 = false;
    bool var5 = false;
    float var6 = 1.0E8F;
    bool var7 = false;
    ...
    int var11 = 3;
            ...
            if (dynamic_cast<BaseVehicle*>(var3) != nullptr) {
               var11 = 3;
            }
            ...
            if (var11 == 3) {
               this.fullSimulation.push_back(var3);
            }

            if (var11 == 2) {
               this.halfSimulation.push_back(var3);
            }

            if (var11 == 1) {
               this.quarterSimulation.push_back(var3);
            }

            if (var11 == 0) {
               this.eighthSimulation.push_back(var3);
            }

            if (var11 < 0) {
               this.sixteenthSimulation.push_back(var3);
            }
         }
      }
   }
   ...
   public std::vector<IsoMovingObject> getBucket() {
