...
public
 ByteArrayBitInputStream(byte[] bytes) { this(bytes, 0); }

public
 ByteArrayBitInputStream(byte[] bytes, int int0) {
 this->endian = int0;
 this->source = bytes;
 ...
 int1 = (this->currentByte & 255 & (1 << this->bitIndex + 1) - 1)
 << int0 - this->bitIndex - 1;
 int0 -= this->bitIndex + 1;

 for (this->currentByte = this->source[++this->byteIndex]; int0 >= 8;
 this->currentByte = this->source[++this->byteIndex]) {
 int0 -= 8;
 ...
 huffmanNode = (this->currentByte & 1 << this->bitIndex++) != 0
 ? huffmanNode.o1
 : huffmanNode.o0;
 }
 ...
 "ByteArrayBitInputStream.readSignedRice() is only supported in big "
 "endian mode");
 } else {
 ...
 int1 =
 (this->source[this->byteIndex] & 255 & (1 << this->bitIndex + 1) - 1)
 << int3 - this->bitIndex - 1;
...
public
 byte[] getSource() { return this->source; }
}
} // namespace io
} // namespace util
} // namespace jarnbjo
