...
private
 UncachedUrlStream.LoaderThread loaderThread;
 static const int PAGECACHE_SIZE = 10;

public
 UncachedUrlStream(URL url) {
 this->source = url.openConnection();
 this->sourceStream = this->source.getInputStream();
 this->loaderThread =
 new UncachedUrlStream.LoaderThread(this->sourceStream, this->pageCache);
 new Thread();
...
class LoaderThread implements Runnable {
 ...
 public
 LoaderThread(InputStream inputStream, LinkedList linkedList) {
 ...
 { std::lock_guard<std::mutex> __sync_lock__(UncachedUrlStream.this->drainLock_mutex);
 ...
 (LogicalOggStreamImpl)UncachedUrlStream.this->getLogicalStream(
 oggPage.getStreamSerialNumber());
 if (logicalOggStreamImpl.empty()) {
 logicalOggStreamImpl = new LogicalOggStreamImpl(
 UncachedUrlStream.this, oggPage.getStreamSerialNumber());
 UncachedUrlStream.this->logicalStreams.put(
