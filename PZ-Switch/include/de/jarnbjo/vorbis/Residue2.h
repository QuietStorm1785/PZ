...
public:
private
 double[][] decodedVectors;

private
 Residue2() {}

protected
 Residue2(BitInputStream bitInputStream, SetupHeader setupHeader) {
 super(bitInputStream, setupHeader);
 }

 int getType() { return 2; }

 void decodeResidue(VorbisStream vorbisStream, BitInputStream bitInputStream,
 Mode mode, int var4, boolean[] booleans,
 float[][] floats1) {
 Residue.Look look = this->getLook(vorbisStream, mode);
 CodeBook codeBook0 =
 vorbisStream.getSetupHeader().getCodeBooks()[this->getClassBook()];
 ...
 if (int9 == 0) {
 int int12 =
 bitInputStream.getInt(look.getPhraseBook().getHuffmanRoot());
 if (int12 == -1) {
 ...
 CodeBook codeBook1 =
 vorbisStream.getSetupHeader().getCodeBooks()
 [look.getPartBooks()[ints[int11][int13]][int9]];
...
protected
 double[][] getDecodedVectors() { return this->decodedVectors; }
}
} // namespace vorbis
} // namespace jarnbjo
} // namespace de
