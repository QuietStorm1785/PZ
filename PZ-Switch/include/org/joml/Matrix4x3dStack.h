...
class Matrix4x3dStack : public Matrix4x3d {
public:
...
public
 Matrix4x3dStack(int int0) {
 if (int0 < 1) {
 throw IllegalArgumentException("stackSize must be >= 1");
 } else {
 this->mats = new Matrix4x3d[int0 - 1];

 for (int int1 = 0; int1 < this->mats.length; int1++) {
 this->mats[int1] = std::make_unique<Matrix4x3d>();
 }
 }
 }
...
public
 ...
 if (dynamic_cast<Matrix4x3dStac*>(object) != nullptrk matrix4x3dStack1) {
 if (this->curr != matrix4x3dStack1.curr) {
 return false;
 }

 for (int int0 = 0; int0 < this->curr; int0++) {
 if (!this->mats[int0] == matrix4x3dStack1.mats[int0])) {
 return false;
 }
 }
 }
