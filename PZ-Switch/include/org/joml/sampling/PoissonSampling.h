...
public
 ...
 const float diskRadiusSquared;
 const float minDist;
 const float minDistSquared;
 const float cellSize;
 const int numCells;
 const Random rnd;
 const std::vector processList;

 public
 Disk(long long0, float float0, float float1, int int0,
 ...
 this->processList = std::make_unique<ArrayList>();
 this->compute(int0, callback2d);
 }

 void compute(int int2, Callback2d callback2d) {
 float float0;
 float float1;
 do {
 ...
 float0 = (float)(float3 *
 Math.sin_roquen_9(float2 + (java.lang.Math.PI / 2));
 float1 = (float)(float3 * Math.sin_roquen_9(float2);
 float0 += vector2f1.x;
 float1 += vector2f1.y;
 if (!(float0 * float0 + float1 * float1 > this->diskRadiusSquared) &&
 !this->searchNeighbors(float0, float1) {
 boolean0 = true;
