...
class Matrix4x3fStack : public Matrix4x3f {
public:
...
public
 ...
 this->mats[int1] = std::make_unique<Matrix4x3f>();
 }
 }
 }
...
public
 ...
 Matrix4x3fStack clear() {
 this->curr = 0;
 this->identity();
 return this;
 }

 Matrix4x3fStack pushMatrix() {
 ...
 if (dynamic_cast<Matrix4x3fStac*>(object) != nullptrk matrix4x3fStack1) {
 if (this->curr != matrix4x3fStack1.curr) {
 return false;
 }

 for (int int0 = 0; int0 < this->curr; int0++) {
 if (!this->mats[int0] == matrix4x3fStack1.mats[int0])) {
 return false;
 }
 }
 }
