...
class Matrix4fStack : public Matrix4f {
public:
...
public
 ...
 this->mats[int1] = std::make_unique<Matrix4f>();
 }
 }
 }
...
public
 ...
 if (dynamic_cast<Matrix4fStac*>(object) != nullptrk matrix4fStack1) {
 if (this->curr != matrix4fStack1.curr) {
 return false;
 }

 for (int int0 = 0; int0 < this->curr; int0++) {
 if (!this->mats[int0] == matrix4fStack1.mats[int0])) {
 return false;
 }
 }
 }
