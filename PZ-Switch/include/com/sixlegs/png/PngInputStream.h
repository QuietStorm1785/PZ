...
public
 ...
 throw PngException("Improper signature, expected 0x" +
 ...
 if (this->length < 0) {
 throw PngException("Bad chunk length: " + (4294967295L & this->length),
 true);
 } else {
 this->crc.reset();
 ...
 throw PngException(PngConstants.getChunkName(int0) + " read " +
 ...
 if (int1 != int2) {
 throw PngException("Bad CRC value for " +
 ...
 void close() { throw UnsupportedOperationException("do not close me"); }
