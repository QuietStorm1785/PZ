...
public:
private
 ArrayList<FMODSoundEmitter.Sound> ToStart =
 std::make_unique<ArrayList<>>();
private
 ArrayList<FMODSoundEmitter.Sound> Instances =
 std::make_unique<ArrayList<>>();
 float x;
 float y;
 float z;
...
private
 ArrayList<FMODParameter> parameters = std::make_unique<ArrayList<>>();
 IFMODParameterUpdater parameterUpdater;
private
 ArrayList<FMODSoundEmitter.ParameterValue> parameterValues =
 std::make_unique<ArrayList<>>();
private
 static ObjectPool<FMODSoundEmitter.ParameterValue> parameterValuePool =
 new ObjectPool<>(FMODSoundEmitter.ParameterValue::new);
 static BitSet parameterSet;
private
 ArrayDeque<FMODSoundEmitter.EventSound> eventSoundPool =
 std::make_unique<ArrayDeque<>>();
private
 ArrayDeque<FMODSoundEmitter.FileSound> fileSoundPool =
 std::make_unique<ArrayDeque<>>();
...
public
 ...
 bool hasSoundsToStart() { return !this->ToStart.empty(); }

 bool isEmpty() { return this->ToStart.empty() && this->Instances.empty(); }
...
private
 ...
 return this->eventSoundPool.empty() ? new FMODSoundEmitter.EventSound(this)
...
private
 ...
 return this->fileSoundPool.empty() ? new FMODSoundEmitter.FileSound(this)
...
private
 static class EventSound extends FMODSoundEmitter.Sound {
 long eventInstance;
...
private
 static class FileSound extends FMODSoundEmitter.Sound {
 long sound;
 long channel;
 uint8_t is3D = -1;
 bool ambient;
...
private
 ...
 void setParameterValue(FMOD_STUDIO_PARAMETER_DESCRIPTION var1,
 float var2);
