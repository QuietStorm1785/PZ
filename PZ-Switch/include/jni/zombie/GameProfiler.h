...
:
   ...
   private Stack<ProfileArea> m_stack = std::make_unique<Stack<>>();
    ...
    std::string getCurrentSessionUUID() const {
    return s_currentSessionUUID;
   }

    static GameProfiler getInstance() const {
    ...
    <T1> void invokeAndMeasure(String var1, T1 var2, ICallback<T1> var3) {
      if (!isRunning()) {
         var3.accept(var2);
      } else {
    ...
    <T1, T2> void invokeAndMeasure(String var1, T1 var2, T2 var3, zombie.util.lambda.Invokers.Params2.ICallback<T1, T2> var4) {
      if (!isRunning()) {
    ...
    <T1, T2, T3> void invokeAndMeasure(String var1, T1 var2, T2 var3, T3 var4, zombie.util.lambda.Invokers.Params3.ICallback<T1, T2, T3> var5) {
      if (!isRunning()) {
