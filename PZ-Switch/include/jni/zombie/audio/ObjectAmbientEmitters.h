...
:
   private HashMap<String, PowerPolicy> powerPolicyMap = std::make_unique<HashMap<>>();
    static ObjectAmbientEmitters instance = nullptr;
    static const Vector2 tempVector2 = new Vector2();
   private HashMap<IsoObject, ObjectWithDistance> m_added = std::make_unique<HashMap<>>();
   private ObjectPool<ObjectWithDistance> m_objectPool = new ObjectPool(ObjectWithDistance::new);
   private ArrayList<ObjectWithDistance> m_objects = std::make_unique<ArrayList<>>();
   private Slot[] m_slots;
   private Comparator<ObjectWithDistance> comp = new 1(this);

    static ObjectAmbientEmitters getInstance() const {
      if (instance == nullptr) {
