...
:
   ...
   private int[] snowFracOnDay;
    ...
    static ErosionMain getInstance() const {
    return instance;
   }

     ErosionMain(IsoSpriteManager var1, bool var2) {
      instance = this;
      this.sprMngr = var1;
      this.debug = var2;
      this.start();
    ...
    ErosionConfig getConfig() const {
      return this.cfg;
   }

    ErosionSeason getSeasons() const {
      return this.Season;
   }
    ...
    IsoSpriteManager getSpriteManager() const {
      return this.sprMngr;
   }
    ...
    int var1 = SandboxOptions.instance.ErosionDays.getValue();
         if (this.debug) {
            ...
            this.ticks++;
            this.eTicks = (int)(this.ticks / 144.0F / var1 * 100.0F);
         } else {
            this.ticks++;
    ...
    IsoChunk var7 = var4.chunks[var6][var5];
                        if (var7 != nullptr) {
    Chunk var8 = var7.getErosionData();
    ...
    int getSnowFraction() const {
      return this.snowFrac;
   }

    int getSnowFractionYesterday() const {
    ...
    void sendState(ByteBuffer var1) {
      if (GameServer.bServer) {
    ...
    void receiveState(ByteBuffer var1) {
      if (GameClient.bClient) {
    ...
    void loadGridsquare(IsoGridSquare var1) {
      if (var1 != nullptr && var1.chunk != nullptr && var1.getZ() == 0) {
         this.getChunk(var1);
    Square var2 = var1.getErosionData();
         if (!var2.init) {
    ...
    void initGridSquare(IsoGridSquare var1, Square var2) {
    int var3 = var1.getX();
    int var4 = var1.getY();
    float var5 = this.noiseMain.layeredNoise(var3 / 10.0F, var4 / 10.0F);
      var2.noiseMainByte = Bits.packFloatUnitToByte(var5);
      var2.noiseMain = var5;
      var2.noiseMainInt = (int)Math.floor(var2.noiseMain * 100.0F);
      var2.noiseKudzu = this.noiseKudzu.layeredNoise(var3 / 10.0F, var4 / 10.0F);
      var2.soil = this.chunkModData.soil;
    float var6 = var2.rand(var3, var4, 100) / 100.0F;
    ...
    void getChunk(IsoGridSquare var1) {
    ...
    void initChunk(IsoChunk var1, Chunk var2) {
      var2.set(var1);
    float var3 = var2.x / 5.0F;
    float var4 = var2.y / 5.0F;
    float var5 = this.noiseMoisture.layeredNoise(var3, var4);
    float var6 = this.noiseMinerals.layeredNoise(var3, var4);
    int var7 = var5 < 1.0F ? (int)Math.floor(var5 * 10.0F) : 9;
    int var8 = var6 < 1.0F ? (int)Math.floor(var6 * 10.0F) : 9;
      var2.init = true;
      var2.eTickStamp = -1;
      var2.epoch = -1;
      var2.moisture = var5;
    ...
    return true;
         ...
         var2 = new File(ZomboidFileSystem.instance.getCacheDir() + File.separator + var1);
    ...
    int var9 = SandboxOptions.instance.getErosionSpeed();
         switch (var9) {
            case 1:
    ...
    float var4 = this.cfg.time.tickunit * 100 / 144.0F;
    float var5 = (SandboxOptions.instance.TimeSinceApo.getValue() - 1) * 30;
         this.cfg.time.eticks = (int)Math.floor(Math.min(1.0F, var5 / var4) * 100.0F);
    int var6 = SandboxOptions.instance.ErosionDays.getValue();
         if (var6 > 0) {
            this.cfg.time.tickunit = 144;
            this.cfg.time.eticks = (int)Math.floor(Math.min(1.0F, var5 / var6) * 100.0F);
         }
    ...
    Chunk var2 = var1.getErosionData();
      if (!var2.init) {
    ...
    IsoChunk var5 = var2.getChunk(var4, var3);
                     if (var5 != nullptr) {
    Chunk var6 = var5.getErosionData();
