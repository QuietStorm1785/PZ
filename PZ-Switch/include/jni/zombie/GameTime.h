...
:
   ...
   private float[] GunFireTimes = new float[5];
    ...
    static GameTime getInstance() const {
    return instance;
   }
    ...
    static long getServerTime() const {
      if (isUTest) {
    ...
    static long getServerTimeMills() const {
      return TimeUnit.NANOSECONDS.toMillis(getServerTime());
   }

    static bool getServerTimeShiftIsSet() const {
    ...
    float getRealworldSecondsSinceLastUpdate() const {
      return 0.016666668F * this.FPSMultiplier;
   }

    float getMultipliedSecondsSinceLastUpdate() const {
      return 0.016666668F * this.getUnmoddedMultiplier();
   }

    float getGameWorldSecondsSinceLastUpdate() const {
    ...
    return var2 = = 1 ? Translator.getText("IGUI_Gametime_zombieCount", var2) : nullptr;
      }
   }

    std::string getGameModeText() const {
    ...
    float getDeltaMinutesPerDay() const {
      return this.MinutesPerDayStart / this.MinutesPerDay;
   }

    float getNightMin() const {
    ...
    long getMinutesStamp() const {
      return this.minutesStamp;
   }

    bool getThunderStorm() const {
    ...
    static float getAnimSpeedFix() const {
      return 0.8F;
   }
    ...
    float getServerMultiplier() const {
    ...
    float getUnmoddedMultiplier() const {
    ...
    float getInvMultiplier() const {
      return 1.0F / this.getMultiplier();
   }

    float getTrueMultiplier() const {
    ...
    int getHelicopterDay1() const {
      return this.HelicopterDay1;
   }

    int getHelicopterDay() const {
      return this.HelicopterDay1;
   }
    ...
    int getHelicopterStartHour() const {
    ...
    int getHelicopterEndHour() const {
