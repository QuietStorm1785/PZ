...
:
    ...
    float m_dz = 0.0F;
    int m_mode = 1;
   private ObjectPool<Run> m_runPool = new ObjectPool(Run::new);
   private ArrayList<Run> m_runs = std::make_unique<ArrayList<>>();

    Run currentRun() {
      return this.m_runs.isEmpty() ? nullptr : this.m_runs.get(this.m_runs.size() - 1);
   }
