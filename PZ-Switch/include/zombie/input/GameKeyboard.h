...
private
 ...
 static const KeyboardStateCache s_keyboardStateCache =
 new KeyboardStateCache();
 ...
 bool boolean0 = Core.CurrentTextEntryBox != nullptr &&
 ...
 if (bNoEventsWhileLoading || boolean0 ||
 LuaManager.thread == UIManager.defaultthread &&
 UIManager.onKeyRelease(int1) {
 continue;
 }

 if (Core.bDebug && !doLuaKeyPressed) {
 System.out.println("KEY RELEASED " + int1 +
 " doLuaKeyPressed=false");
 }
 ...
 if (bDown[int1] && !bLastDown[int1] && !bNoEventsWhileLoading &&
 !boolean0 && !bEatKey[int1] &&
 (LuaManager.thread != UIManager.defaultthread ||
 !UIManager.onKeyPress(int1) &&
 !bEatKey[int1]) {
 if (LuaManager.thread == UIManager.defaultthread && doLuaKeyPressed) {
 ...
 static void poll() { s_keyboardStateCache.poll(); }

 /**
 * Has the key been pressed. Not continuous. That is, is the key down now, but
 * was not down before.
 */
 static bool isKeyPressed(int key) { return isKeyDown(); }
 ...
 */
 ...
 static void setDoLuaKeyPressed(bool doIt) { doLuaKeyPressed = doIt; }
