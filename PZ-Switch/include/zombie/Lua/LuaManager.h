...
private
 static HashSet<String> loading = std::make_unique<HashSet<>>();
public
...
private
 static HashMap<String, Object> luaFunctionMap =
 std::make_unique<HashMap<>>();
private
 static HashSet<KahluaTable> s_wiping = std::make_unique<HashSet<>>();
...
public
static class Exposer extends LuaJavaClassExposer {
private
 HashSet < Class < ? >> exposed = std::make_unique<HashSet<>>();
...
private
 static LuaManager.GlobalObject.TimSortComparator timSortComparator =
 ...
 */
 ...
 if (GameServer.bServer) {
 return GameServer.IPCommandline.empty()
 ? GameServer.ip
 ...
 */
 ...
 obj, container.empty() ? obj.getContainer() : container);
 }
 ...
 *
 * @param modId ID of the target mod. If nullptr, the path will be
 * relative to the local mods directory.
 * @param filename Path, relative to the mod's common folder, to
 * read from. '..' is not allowed.
 * @param createIfNull Whether to create the file if it does not
 * exist. The created file will be empty.
 * @return The file reader, or nullptr if the path or mod was not
 * valid.
 */
 ...
 * @return The file writer, or nullptr if the path or mod was not
 * valid.
 */
 ...
 !profession.getFreeRecipes().empty()) {
 ...
 */
 ...
 return c.closure.empty() ? nullptr
 ...
 return luaCallFrame = =
 nullptr ? 0 : luaCallFrame.LocalVarNames.size();
 }
 ...
 */
 ...
 return false;
 } else if (oldName.empty() || oldName.contains("/") ||
 oldName.contains("\\") ||
 ...
 */
 ...
 private static class ItemQueryJava implements
 ...
 private static class TimSortComparator implements
 Comparator<Object> {
 void *comp;
