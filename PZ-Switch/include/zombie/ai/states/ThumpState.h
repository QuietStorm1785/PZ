...
public:
 ...
 static ThumpState instance() { return _instance; }
 ...
 bool boolean0 = GameServer.bServer && GameServer.bFastForward ||
 !GameServer.bServer && IsoPlayer.allPlayersAsleep();
 ...
 for (int int1 = 0; int1 < int0 &&
 this->isThumpTargetValid(owner, owner.getThumpTarget());
 int1++) {
 owner.getThumpTarget().Thump(owner);
 }

 Thumpable thumpable1 =
 owner.getThumpTarget() == nullptr
 ? nullptr
 : owner.getThumpTarget().getThumpableFor(owner);
 bool boolean1 =
 GameServer.bServer ||
 SoundManager.instance.isListenerInRange(owner.x, owner.y, 20.0F);
 ...
 Type.tryCastTo(thumpable1, IsoBarricade.class);
 if (barricade.empty() ||
 !barricade.isMetal() && !barricade.isMetalBar()) {
 ...
 owner.setTimeThumping(0);
 if (thumpable0 instanceof
 IsoWindow && ((IsoWindow)thumpable0).canClimbThrough(owner) {
 owner.climbThroughWindow((IsoWindow)thumpable0);
 } else {
 if (thumpable0 instanceof IsoDoor && (((IsoDoor)thumpable0).open ||
 thumpable0.isDestroyed())) {
 ...
 if (thumpable0 instanceof
 IsoThumpable && ((IsoThumpable)thumpable0).isDoor &&
 (((IsoThumpable)thumpable0).open || thumpable0.isDestroyed())) {
 ...
 IsoWorld.instance.CurrentCell.getGridSquare(int3, int0, int6);
 if (square != nullptr) {
 ...
 IsoPlayer && !((IsoPlayer)movingObject).isGhostMode()) {
 ...
 bool lungeThroughDoor(IsoZombie zombie0, IsoGridSquare square1,
 ...
 square2 = square0;
 if (boolean0) {
 player = this->findPlayer(square0.getX() - 1, square0.getX() + 1,
 square0.getY() - 1, square0.getY(),
 square0.getZ());
 } else {
 player = this->findPlayer(square0.getX() - 1, square0.getX(),
 square0.getY() - 1, square0.getY() + 1,
 square0.getZ());
 }
 } else if (zombie0.getCurrentSquare() == square0) {
 square2 = square1;
 if (boolean0) {
 player = this->findPlayer(square1.getX() - 1, square1.getX() + 1,
 square1.getY(), square1.getY() + 1,
 square1.getZ());
 } else {
 player = this->findPlayer(square1.getX(), square1.getX() + 1,
 square1.getY() - 1, square1.getY() + 1,
 square1.getZ());
 }
 }

 if (player != nullptr &&
 !LosUtil.lineClearCollide(square2.getX(), square2.getY(),
 square2.getZ(), (int)player.getX(),
 ...
 if (GameServer.bServer) {
 return (
 int)(GameServer.bFastForward
 ? ServerOptions.instance.FastForwardMultiplier.getValue() /
 gameTime.getDeltaMinutesPerDay()
 : 1.0);
 } else if (GameClient.bClient) {
 return (
 int)(GameClient.bFastForward
 ? ServerOptions.instance.FastForwardMultiplier.getValue() /
 gameTime.getDeltaMinutesPerDay()
 : 1.0);
 } else {
 return IsoPlayer.allPlayersAsleep()
 ? (int)(200.0F * (30.0F / PerformanceSettings.getLockFPS()) /
 1.6F)
 : (int)gameTime.getTrueMultiplier();
 }
 }
 ...
 return chunk = =
 nullptr ? false : thumpable.getThumpableFor(character) != nullptr;
 }
 }
 }
}
