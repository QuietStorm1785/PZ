...
public:
 ...
 static FakeDeadAttackState instance() { return _instance; }
 ...
 if (zombie0.target instanceof IsoPlayer && !((IsoPlayer)zombie0.target)
 .getCharacterTraits()
 .Desensitized.isSet()) {
 ...
 owner.isAlive() && zombie0.isTargetInCone(1.5F, 0.9F) &&
 zombie0.target instanceof
 IsoGameCharacter character &&
 (character.getVehicle() == nullptr ||
 character.getVehicle().couldCrawlerAttackPassenger(character)) {
 character.getBodyDamage().AddRandomDamageFromZombie((IsoZombie)owner,
 nullptr);
 }
