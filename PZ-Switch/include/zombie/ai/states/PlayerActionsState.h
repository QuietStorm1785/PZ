...
public:
 ...
 static PlayerActionsState instance() { return _instance; }
 ...
 IsoPlayer && owner.isLocal() &&
 !owner.getCharacterActions().empty() &&
 owner.getNetworkCharacterAI().getAction() == nullptr) {
 owner.getNetworkCharacterAI().setAction(
 owner.getCharacterActions().get(0);
 ...
 (owner.getNetworkCharacterAI().getAction() !=
 owner.getCharacterActions().get(0) ||
 string != nullptr &&
 !string == owner.getNetworkCharacterAI().getPerformingAction()))) {
 GameClient.sendAction(owner.getNetworkCharacterAI().getAction(),
 false);
 owner.getNetworkCharacterAI().setAction(
 owner.getCharacterActions().get(0);
 GameClient.sendAction(owner.getNetworkCharacterAI().getAction(),
 ...
 if (GameClient.bClient && event != nullptr && owner instanceof
 IsoPlayer && owner.getNetworkCharacterAI().getAction() != nullptr &&
 !owner.isLocal() &&
 ...
 owner.getNetworkCharacterAI().setOverride(true, event.m_ParameterValue,
 nullptr);
 }
 }
 }
