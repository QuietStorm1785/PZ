...
public:
 ...
 static ClimbSheetRopeState instance() { return _instance; }
 ...
 float float0 = 0.0F;
 float float1 = 0.0F;
 if (owner.getCurrentSquare().getProperties().Is(IsoFlagType.climbSheetN) ||
 owner.getCurrentSquare().getProperties().Is(
 ...
 if (owner.getCurrentSquare().getProperties().Is(IsoFlagType.climbSheetS) ||
 owner.getCurrentSquare().getProperties().Is(
 ...
 if (owner.getCurrentSquare().getProperties().Is(IsoFlagType.climbSheetW) ||
 owner.getCurrentSquare().getProperties().Is(
 ...
 if (owner.getCurrentSquare().getProperties().Is(IsoFlagType.climbSheetE) ||
 owner.getCurrentSquare().getProperties().Is(
 ...
 if (thumpable != nullptr) {
 if (!IsoWindow.canClimbThroughHelper(owner, square0, square1,
 owner.dir == IsoDirections.N ||
 owner.dir ==
 IsoDirections.S) {
 owner.climbDownSheetRope();
 return;
