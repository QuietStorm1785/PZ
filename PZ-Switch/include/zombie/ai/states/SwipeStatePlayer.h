...
private
 static ArrayList<HitInfo> HitList2 = std::make_unique<ArrayList<>>();
 static const Vector2 tempVector2_1 = new Vector2();
 static const Vector2 tempVector2_2 = new Vector2();
private
 ArrayList<Float> dotList = std::make_unique<ArrayList<>>();
 bool bHitOnlyTree;
public
 ObjectPool<HitInfo> hitInfoPool = new ObjectPool<>(HitInfo::new);
private
 static SwipeStatePlayer.CustomComparator Comparator =
 new SwipeStatePlayer.CustomComparator();
...
private
 SwipeStatePlayer.WindowVisitor windowVisitor =
 new SwipeStatePlayer.WindowVisitor();

 static SwipeStatePlayer instance() { return _instance; }

 static void WeaponLowerCondition(HandWeapon weapon, IsoGameCharacter owner) {
 if (weapon.getUses() > 1) {
 weapon.Use();
 InventoryItem item =
 InventoryItemFactory.CreateItem(weapon.getFullType());
 ...
 if (object.empty() || !(object instanceof HandWeapon) || boolean0 ||
 attackVars.bDoShove) {
 ...
 if (player.PlayerIndex == 0 && player.JoypadBind == -1 &&
 UIManager.getPicked() != nullptr &&
 (!GameClient.bClient || player.isLocalPlayer())) {
 if (UIManager.getPicked().tile instanceof IsoMovingObject) {
 player.setAttackTargetSquare(
 ((IsoMovingObject)UIManager.getPicked().tile).getCurrentSquare());
 } else {
 ...
 this->doAttack(player, 2.0F, owner.isForceShove(), owner.getClickSound(),
 player.attackVars);
 ...
 IsoWorld.instance.CurrentCell.setDrag(nullptr,
 ((IsoPlayer)owner).PlayerIndex);
 }
 ...
 player.targetOnGround =
 (IsoGameCharacter)player.attackVars.targetOnGround.getMovingObject();
 if (!player.bDoShove && !boolean0 && player.getClickSound() == nullptr &&
 weapon.getPhysicsObject() == nullptr && !weapon.isRanged()) {
 }
 ...
 if (!player.bDoShove && !boolean0 && !weapon.isRanged() &&
 ...
 void execute(IsoGameCharacter owner) { owner.StopAllActionQueue(); }

 int DoSwingCollisionBoneCheck(IsoGameCharacter character0, HandWeapon weapon,
 IsoGameCharacter character1, int int0,
 float float2) {
 movingStatic.clear();
 float float0 = weapon.WeaponLength;
 float0 += 0.5F;
 if (character0.isAimAtFloor() &&
 ((IsoLivingCharacter)character0).bDoShove) {
 float0 = 0.3F;
 }
 ...
 tempVector3_1.x = tempVector3_1.x +
 character0.getForwardDirection().x * float0 * float1;
 tempVector3_1.y = tempVector3_1.y +
 character0.getForwardDirection().y * float0 * float1;
 ...
 event.m_EventName.equalsIgnoreCase("NonLoopedAnimFadeOut")) {
 ...
 if (owner instanceof
 IsoPlayer && "charge" == ((IsoPlayer)owner).getAttackType())) {
 int0 = (int)(int0 / 1.5);
 }

 if (Rand.Next(int0 + owner.getMaintenanceMod() * 2) == 0) {
 WeaponLowerCondition(weapon0, owner);
 } else if (Rand.NextBool(2) && !weapon0.isRanged() &&
 WeaponType.getWeaponType(owner) != WeaponType.barehand) {
 if (weapon0.isTwoHandWeapon() &&
 (owner.getPrimaryHandItem() != weapon0 ||
 owner.getSecondaryHandItem() != weapon0) &&
 Rand.NextBool(3) {
 return;
 }
 ...
 owner.setVariable("ShoveAnim",
 Boolean.parseBoolean(event.m_ParameterValue);
 }

 if (event.m_EventName.equalsIgnoreCase("StompAnim")) {
 owner.setVariable("StompAnim",
 Boolean.parseBoolean(event.m_ParameterValue);
 }

 HandWeapon weapon1 = GetWeapon(owner);
 if (event.m_EventName.equalsIgnoreCase("AttackCollisionCheck") &&
 hashMap.get(PARAM_ATTACKED) == Boolean.FALSE && owner instanceof
 IsoPlayer && ((IsoPlayer)owner).isLocalPlayer()) {
 this->ConnectSwing(owner, weapon1);
 }

 if (event.m_EventName.equalsIgnoreCase("BlockMovement") &&
 SandboxOptions.instance.AttackBlockMovements.getValue()) {
 owner.setVariable("SlowingMovement",
 Boolean.parseBoolean(event.m_ParameterValue);
 }

 if (event.m_EventName.equalsIgnoreCase("WeaponEmptyCheck") &&
 owner.getClickSound() != nullptr) {
 ...
 if (event.m_EventName.equalsIgnoreCase("ShotDone") && weapon1 != nullptr &&
 weapon1.isRackAfterShoot()) {
 owner.setVariable("ShotDone", true);
 }

 if (event.m_EventName.equalsIgnoreCase("SetVariable") &&
 event.m_ParameterValue.startsWith("ShotDone=")) {
 owner.setVariable("ShotDone", owner.getVariableBoolean("ShotDone") &&
 weapon1 != nullptr &&
 weapon1.isRackAfterShoot());
 }
 ...
 .AddDamage(Rand.Next(5, 10);
 owner.getBodyDamage()
 .getBodyPart(BodyPartType.Foot_R)
 .setAdditionalPain(owner.getBodyDamage()
 .getBodyPart(BodyPartType.Foot_R)
 .getAdditionalPain() +
 Rand.Next(5, 10);
 } else {
 ...
 if (weapon != nullptr &&
 (weapon.getCondition() <= 0 || boolean0 && weapon.isUseSelf())) {
 owner.removeFromHands(weapon);
 if (DebugOptions.instance.MultiplayerAutoEquip.getValue() &&
 weapon.getPhysicsObject() != nullptr) {
 owner.setPrimaryHandItem(
 owner.getInventory().getItemFromType(weapon.getType()));
 }
 ...
 bool boolean0 = owner.isAttackAnim() ||
 owner.getVariableBoolean("ShoveAnim") ||
 owner.getVariableBoolean("StompAnim");
 ...
 vars.recoilDelay = 0;
 if (vars.getWeapon(owner) == owner.bareHands || vars.bDoShove ||
 owner.isForceShove()) {
 vars.bDoShove = true;
 vars.bAimAtFloor = false;
 ...
 this->calcValidTargets(owner, vars.getWeapon(owner), true,
 vars.targetsProne, vars.targetsStanding);
 HitInfo hitInfo0 =
 vars.targetsStanding.empty() ? nullptr : vars.targetsStanding.get(0);
 HitInfo hitInfo1 =
 vars.targetsProne.empty() ? nullptr : vars.targetsProne.get(0);
 ...
 } else if (hitInfo1 != nullptr &&
 (Core.OptionAutoProneAtk || owner.bDoShove) {
 if (!boolean0) {
 ...
 if (!(float0 >= vars.getWeapon(owner).getMinRange() *
 vars.getWeapon(owner).getMinRange()) &&
 (hitInfo0.empty() ||
 !this->isWindowBetween(owner, hitInfo0.getObject()))) {
 if (owner.getStats().NumChasingZombies <= 1 &&
 WeaponType.getWeaponType(owner) == WeaponType.knife) {
 ...
 float float1 =
 int2 *
 (1.0F - owner.getPerkLevel(PerkFactory.Perks.Aiming) / 30.0F);
 vars.recoilDelay = float1.intValue();
 owner.setVariable(
 "singleShootSpeed",
 (0.8F + owner.getPerkLevel(PerkFactory.Perks.Aiming) / 10.0F) *
...
public
 void calcValidTargets(IsoLivingCharacter owner, HandWeapon weapon,
 boolean extraRange, ArrayList<HitInfo> targetsProne,
 ArrayList<HitInfo> targetsStanding) {
 ...
 HitInfo hitInfo =
 this->calcValidTarget(owner, weapon, movingObject, float2);
 if (hitInfo != nullptr) {
 ...
 if (!targetsProne.empty() &&
 this->shouldIgnoreProneZombies(owner, targetsStanding, float0) {
 this->hitInfoPool.release(targetsProne);
 targetsProne.clear();
 }
 ...
 if (weapon.isRanged()) {
 float3 -= weapon.getAimingPerkMinAngleModifier() *
 (owner.getPerkLevel(PerkFactory.Perks.Aiming) / 2.0F);
 }

 this->removeUnhittableTargets(owner, weapon, float3, float4, extraRange,
 targetsStanding);
 ...
 bool shouldIgnoreProneZombies(IsoGameCharacter character,
 ArrayList<HitInfo> arrayList, float float0) {
 if (float0 <= 0.0F) {
 return false;
 } else {
 bool boolean0 = character.isInvisible() || character instanceof
 IsoPlayer && ((IsoPlayer)character).isGhostMode();

 for (int int0 = 0; int0 < arrayList.size(); int0++) {
 HitInfo hitInfo = (HitInfo)arrayList.get(int0);
 IsoZombie zombie0 =
 Type.tryCastTo(hitInfo.getObject(), IsoZombie.class);
 if ((zombie0.empty() || zombie0.target != nullptr || boolean0) &&
 !(hitInfo.distSq > float0 * float0) {
 int int1 = (int)character.z;
 boolean boolean1 = PolygonalMap2.instance.lineClearCollide(
 character.x, character.y, hitInfo.getObject().x,
 hitInfo.getObject().y, int1, character, false, true);
 if (!boolean1) {
 return true;
 }
 }
 ...
 bool isUnhittableTarget(IsoGameCharacter character, HandWeapon weapon,
 float float1, float float0, HitInfo hitInfo,
 bool boolean0) {
 if (!(hitInfo.dot < float1) && !(hitInfo.dot > float0) {
 Vector3 vector = tempVectorBonePos.set(hitInfo.x, hitInfo.y, hitInfo.z);
 return !character.IsAttackRange(weapon, hitInfo.getObject(), vector,
 boolean0);
 } else {
 return true;
 }
...
private
 void removeUnhittableTargets(IsoGameCharacter character, HandWeapon weapon,
 float float0, float float1, boolean boolean0,
 ArrayList<HitInfo> arrayList) {
 for (int int0 = arrayList.size() - 1; int0 >= 0; int0--) {
 HitInfo hitInfo = (HitInfo)arrayList.get(int0);
 if (this->isUnhittableTarget(character, weapon, float0, float1, hitInfo,
 boolean0) {
 ...
 bool getNearestTargetPosAndDot(IsoGameCharacter character0, HandWeapon weapon,
 IsoMovingObject movingObject, bool boolean0,
 Vector4f vector4f) {
 ...
 if (isStanding(movingObject) {
 if (weapon.isRanged()) {
 float1 -= weapon.getAimingPerkMinAngleModifier() *
 (character0.getPerkLevel(PerkFactory.Perks.Aiming) / 2.0F);
 }
 } else {
 float1 /= 1.5F;
 }
 ...
 Vector3 vector =
 tempVectorBonePos.set(vector4f.x, vector4f.y, vector4f.z);
 ...
 void getNearestTargetPosAndDot(IsoGameCharacter character, Vector3 vector0,
 Vector2 vector1, Vector4f vector4f) {
 ...
 character.x, character.y, (int)character.z * 3, vector0.x, vector0.y,
 (int)Math.max(vector0.z, 0.0F) * 3);
 if (float1 < vector1.x) {
 vector1.x = float1;
 ...
 void getNearestTargetPosAndDot(IsoGameCharacter character1,
 IsoMovingObject movingObject,
 Vector4f vector4f) {
 ...
 Type.tryCastTo(movingObject, IsoGameCharacter.class);
 if (character0.empty()) {
 this->getNearestTargetPosAndDot(character1, movingObject, (String) nullptr,
 vector0, vector4f);
 } else {
 ...
 this->getNearestTargetPosAndDot(character1, movingObject, "Bip01_Pelvis",
 vector0, vector4f);
 Vector3 vector2 = tempVectorBonePos.set(
 movingObject.getX(), movingObject.getY(), movingObject.getZ());
 this->getNearestTargetPosAndDot(character1, vector2, vector0, vector4f);
 } else {
 this->getNearestTargetPosAndDot(character1, vector1, vector0, vector4f);
 this->getNearestTargetPosAndDot(character1, movingObject, "Bip01_Pelvis",
 vector0, vector4f);
 this->getNearestTargetPosAndDot(
 character1, movingObject, "Bip01_DressFrontNub", vector0, vector4f);
 }
 }
 }

 HitInfo calcValidTarget(IsoLivingCharacter livingCharacter, HandWeapon weapon,
 IsoMovingObject movingObject, float float3) {
 ...
 return nullptr;
 } else if (character.getHitReaction() != nullptr &&
 character.getHitReaction().contains("Death")) {
 return nullptr;
 } else {
 ...
 livingCharacter.x, livingCharacter.y, (int)livingCharacter.z * 3,
 vector4f.x, vector4f.y, (int)vector4f.z * 3);
 if (float1 < 0.0F) {
 return nullptr;
 ...
 livingCharacter.getCell(), (int)livingCharacter.getX(),
 (int)livingCharacter.getY(), (int)livingCharacter.getZ(),
 (int)character.getX(), (int)character.getY(),
 (int)character.getZ(), false);
 return testResults != LosUtil.TestResults.Blocked &&
 testResults !=
 LosUtil.TestResults.ClearThroughClosedDoor
 ? this->hitInfoPool.alloc().init(character, float1,
 float2, vector4f.x,
 vector4f.y, vector4f.z)
 : nullptr;
 }
 }
 }
 ...
 static bool isStanding(IsoMovingObject obj) { return !isProne(obj); }

 bool isProneTargetBetter(IsoGameCharacter owner, HitInfo bestStanding,
 HitInfo bestProne) {
 ...
 boolean boolean0 = PolygonalMap2.instance.lineClearCollide(
 owner.x, owner.y, bestStanding.getObject().x,
 bestStanding.getObject().y, int0, nullptr, false, true);
 if (!boolean0) {
 return false;
 } else {
 int0 = (int)owner.z;
 boolean boolean1 = PolygonalMap2.instance.lineClearCollide(
 owner.x, owner.y, bestProne.getObject().x, bestProne.getObject().y,
 int0, nullptr, false, true);
 return !boolean1;
 }
 }
 }
 ...
 if (player1.isGodMod() || !ServerOptions.instance.PVP.getValue() ||
 ServerOptions.instance.SafetySystem.getValue() &&
 owner.getSafety().isEnabled() &&
 ((IsoGameCharacter)obj).getSafety().isEnabled()) {
 return false;
 }

 if (NonPvpZone.getNonPvpZone((int)obj.getX(), (int)obj.getY()) !=
 nullptr) {
 return false;
 }

 if (player0 != nullptr &&
 NonPvpZone.getNonPvpZone((int)owner.getX(), (int)owner.getY()) !=
 nullptr) {
 return false;
 }
 ...
 void CalcHitListShove(IsoGameCharacter character0, bool boolean1,
 AttackVars attackVars, ArrayList<HitInfo> arrayList1) {
 ...
 IsoGameCharacter character1 =
 Type.tryCastTo(movingObject, IsoGameCharacter.class);
 if (character1 != nullptr && !character1.isGodMod() &&
 !character1.isDead()) {
 IsoZombie zombie0 = Type.tryCastTo(movingObject, IsoZombie.class);
 if ((zombie0.empty() ||
 !zombie0.isCurrentState(FakeDeadZombieState.instance())) &&
 checkPVP(character0, movingObject) {
 boolean boolean0 =
 movingObject == attackVars.targetOnGround.getMovingObject() ||
 movingObject.isShootable() && isStanding(movingObject) &&
 !attackVars.bAimAtFloor ||
 movingObject.isShootable() && isProne(movingObject) &&
 attackVars.bAimAtFloor;
 if (boolean0) {
 Vector4f vector4f = this->tempVector4f;
 if (this->getNearestTargetPosAndDot(
 character0, weapon, movingObject, boolean1, vector4f) {
 float float0 = vector4f.w;
 float float1 = IsoUtils.DistanceToSquared(
 character0.x, character0.y, (int)character0.z * 3,
 vector4f.x, vector4f.y, (int)vector4f.z * 3);
 LosUtil.TestResults testResults = LosUtil.lineClear(
 character0.getCell(), (int)character0.getX(),
 (int)character0.getY(), (int)character0.getZ(),
 (int)movingObject.getX(), (int)movingObject.getY(),
 (int)movingObject.getZ(), false);
 ...
 movingObject.getCurrentSquare() ==
 character0.getCurrentSquare() ||
 !movingObject.getCurrentSquare().isWindowBlockedTo(
 character0.getCurrentSquare())) &&
 movingObject.getSquare().getTransparentWallTo(
 character0.getSquare()) == nullptr) {
 HitInfo hitInfo = this->hitInfoPool.alloc().init(
 movingObject, float0, float1, vector4f.x, vector4f.y,
 vector4f.z);
 if (attackVars.targetOnGround.getMovingObject() ==
 movingObject) {
 arrayList1.clear();
 ...
 void CalcHitListWeapon(IsoGameCharacter character0, bool boolean1,
 AttackVars attackVars, ArrayList<HitInfo> arrayList1) {
 HandWeapon weapon = attackVars.getWeapon((IsoLivingCharacter)character0);
 std::vector arrayList0 = IsoWorld.instance.CurrentCell.getObjectList();
 ...
 IsoGameCharacter character1 =
 Type.tryCastTo(movingObject, IsoGameCharacter.class);
 if ((character1.empty() || !character1.isGodMod()) &&
 (character1.empty() || !character1.isDead())) {
 IsoZombie zombie0 = Type.tryCastTo(movingObject, IsoZombie.class);
 if ((zombie0.empty() ||
 !zombie0.isCurrentState(FakeDeadZombieState.instance())) &&
 checkPVP(character0, movingObject) {
 ...
 VehiclePart part = ((BaseVehicle)movingObject)
 .getNearestBodyworkPart(character0);
 if (part.empty()) {
 continue;
 }

 float float0 = character0.getDotWithForwardDirection(
 movingObject.x, movingObject.y);
 if (float0 < 0.8F) {
 continue;
 }

 vector4f.set(movingObject.x, movingObject.y, movingObject.z,
 float0);
 } else if (character1.empty() ||
 !this->getNearestTargetPosAndDot(character0, weapon,
 movingObject, boolean1,
 vector4f) {
 continue;
 }
 ...
 character0.x, character0.y, (int)character0.z * 3,
 vector4f.x, vector4f.y, (int)vector4f.z * 3);
 if (movingObject.getSquare().getTransparentWallTo(
 character0.getSquare()) != nullptr &&
 character0 instanceof
 IsoPlayer) {
 if (WeaponType.getWeaponType(character0) ==
 WeaponType.spear) {
 ((IsoPlayer)character0).setAttackType("spearStab");
 } else if (WeaponType.getWeaponType(character0) !=
 WeaponType.knife) {
 continue;
 }
 }
 ...
 IsoWindow window =
 this->getWindowBetween(character0, movingObject);
 if (window.empty() || !window.isBarricaded()) {
 HitInfo hitInfo = this->hitInfoPool.alloc().init(
 movingObject, float1, float2, vector4f.x, vector4f.y,
 vector4f.z);
 ...
 void CalcHitListWindow(IsoGameCharacter character, HandWeapon weapon,
 ...
 if ((int)window.getZ() == (int)character.z &&
 this->windowVisitor.isHittable(window) {
 ...
 character.x, character.y, character.x + vector.x,
 character.y + vector.y, float0, float1, float2, float3) {
 IsoGridSquare square = window.getAddSheetSquare(character);
 if (square != nullptr &&
 !LosUtil.lineClearCollide((int)character.x, (int)character.y,
 (int)character.z, square.x, square.y,
 square.z, false) {
 float float4 = IsoUtils.DistanceToSquared(
 character.x, character.y, float0 + (float2 - float0) / 2.0F,
 float1 + (float3 - float1) / 2.0F);
 ...
 void CalcHitList(IsoGameCharacter owner, bool extraRange,
 AttackVars attackVars, ArrayList<HitInfo> hitList) {
 if (!GameClient.bClient || owner.isLocal()) {
 ...
 if (!weapon.isRanged() &&
 !SandboxOptions.instance.MultiHitZombies.getValue()) {
 int0 = 1;
 }

 if (weapon == ((IsoPlayer)owner).bareHands &&
 !(owner.getPrimaryHandItem() instanceof HandWeapon) {
 int0 = 1;
 }

 if (weapon == ((IsoPlayer)owner).bareHands &&
 attackVars.targetOnGround.getMovingObject() != nullptr) {
 int0 = 1;
 }
 ...
 if (hitList.size() == 1 &&
 ((HitInfo)hitList.get(0).getObject() == nullptr) {
 return;
 }
 ...
 this->hitInfoPool.release(
 (HitInfo)hitList.remove(hitList.size() - 1);
 }
 }
 }
 ...
 int CalcHitChance(IsoGameCharacter owner, HandWeapon weapon,
 HitInfo hitInfo) {
 ...
 Vector3f vector3f = vehicle.getForwardVector(
 BaseVehicle.TL_vector3f_pool.get().alloc());
 ...
 vehicle.getScript().getAreaById(vehicle.getPassengerArea(int0);
 uint8_t byte0 = -90;
 if (area.x > 0.0F) {
 byte0 = 90;
 }
 ...
 int1 = (int)(int1 + weapon.getAimingPerkHitChanceModifier() *
 owner.getPerkLevel(PerkFactory.Perks.Aiming);
 if (int1 > 95) {
 int1 = 95;
 }
 ...
 int1 = (int)(int1 +
 (weapon.getMaxRange() * weapon.getRangeMod(owner) - float1) *
 float2);
 if (weapon.getMinRangeRanged() > 0.0F) {
 if (float1 < weapon.getMinRangeRanged()) {
 int1 -= 50;
 }
 } else if (float1 < 1.7 && weapon.isRanged() &&
 !(movingObject instanceof IsoPlayer) {
 int1 += 35;
 }

 if (weapon.isRanged() &&
 owner.getBeenMovingFor() >
 weapon.getAimingTime() +
 owner.getPerkLevel(PerkFactory.Perks.Aiming) {
 int1 = (int)(int1 - (owner.getBeenMovingFor() -
 (weapon.getAimingTime() +
 owner.getPerkLevel(PerkFactory.Perks.Aiming)));
 }
 ...
 int1 =
 (int)(int1 -
 Math.abs(owner.getVehicle().getCurrentSpeedKmHour()) * 2.0F);
 }
 ...
 for (int int2 = BodyPartType.ToIndex(BodyPartType.Hand_L);
 int2 <= BodyPartType.ToIndex(BodyPartType.UpperArm_R); int2++) {
 ...
 owner.attackVars.targetOnGround.setMovingObject(
 livingCharacter.targetOnGround);
 ...
 (owner.getPrimaryHandItem() != weapon ||
 owner.getSecondaryHandItem() != weapon) {
 float0 = weapon.getWeight() / 1.5F / 10.0F;
 }

 if (int0 <= 0 && !owner.isForceShove()) {
 float float1 =
 (weapon.getWeight() * 0.18F * weapon.getFatigueMod(owner) *
 owner.getFatigueMod() * weapon.getEnduranceMod() * 0.3F +
 float0) *
 0.04F;
 float float2 = 1.0F;
 ...
 int int1 = 1;
 this->dotList.clear();
 if (owner.hitList.empty() && owner.getClickSound() != nullptr &&
 ...
 IsoZombie zombie0 = Type.tryCastTo(movingObject, IsoZombie.class);
 if (hitInfo.getObject() == nullptr &&
 hitInfo.window.getObject() != nullptr) {
 ...
 ParameterMeleeHitSurface.Material.Body);
 }
 ...
 owner, GetWeapon(owner), (IsoGameCharacter)movingObject,
 ((IsoGameCharacter)movingObject)
 .getAnimationPlayer()
 .getSkinningBoneIndex("Bip01_Head", -1),
 0.28F);
 if (int5 == -1) {
 ...
 ((IsoGameCharacter)movingObject)
 .getAnimationPlayer()
 .getSkinningBoneIndex("Bip01_Spine", -1),
 0.28F);
 if (int5 == -1) {
 int5 = this->DoSwingCollisionBoneCheck(
 owner, GetWeapon(owner), (IsoGameCharacter)movingObject,
 ((IsoGameCharacter)movingObject)
 .getAnimationPlayer()
 .getSkinningBoneIndex("Bip01_L_Calf", -1),
 0.13F);
 if (int5 == -1) {
 int5 = this->DoSwingCollisionBoneCheck(
 owner, GetWeapon(owner), (IsoGameCharacter)movingObject,
 ((IsoGameCharacter)movingObject)
 .getAnimationPlayer()
 .getSkinningBoneIndex("Bip01_R_Calf", -1),
 0.13F);
 }
 ...
 ((IsoGameCharacter)movingObject)
 .getAnimationPlayer()
 .getSkinningBoneIndex("Bip01_L_Foot", -1),
 0.23F);
 }
 ...
 ((IsoGameCharacter)movingObject)
 .getAnimationPlayer()
 .getSkinningBoneIndex("Bip01_R_Foot", -1),
 0.23F);
 }
 ...
 !livingCharacter.bDoShove && movingObject instanceof
 IsoGameCharacter &&
 ...
 if (weapon.isRanged()) {
 long0 = ((IsoGameCharacter)movingObject)
 .playSound(weapon.getZombieHitSound());
 } else {
 owner.getEmitter().stopSoundByName(weapon.getSwingSound());
 long0 = owner.playSound(weapon.getZombieHitSound());
 }
 ...
 if (zombie0 != nullptr &&
 zombie0.isCurrentState(ZombieOnGroundState.instance())) {
 zombie0.setReanimateTimer(zombie0.getReanimateTimer() +
 Rand.Next(10);
 }

 if (zombie0 != nullptr &&
 zombie0.isCurrentState(ZombieGetUpState.instance())) {
 ...
 for (int int6 = BodyPartType.ToIndex(BodyPartType.UpperLeg_L);
 int6 <= BodyPartType.ToIndex(BodyPartType.Foot_R); int6++) {
 float10 +=
 owner.getBodyDamage().getBodyParts().get(int6).getPain();
 }
 ...
 for (int int7 = BodyPartType.ToIndex(BodyPartType.Hand_L);
 int7 <= BodyPartType.ToIndex(BodyPartType.UpperArm_R);
 int7++) {
 float11 +=
 owner.getBodyDamage().getBodyParts().get(int7).getPain();
 }
 ...
 if (weapon.isRanged() &&
 owner.getPerkLevel(PerkFactory.Perks.Aiming) < 6 &&
 owner.getMoodles().getMoodleLevel(MoodleType.Panic) > 2) {
 float12 -=
 owner.getMoodles().getMoodleLevel(MoodleType.Panic) * 0.2F;
 MoodlesUI.getInstance().wiggle(MoodleType.Panic);
 }

 if (!weapon.isRanged() &&
 owner.getMoodles().getMoodleLevel(MoodleType.Panic) > 1) {
 float12 -=
 owner.getMoodles().getMoodleLevel(MoodleType.Panic) * 0.1F;
 MoodlesUI.getInstance().wiggle(MoodleType.Panic);
 }

 if (owner.getMoodles().getMoodleLevel(MoodleType.Stress) > 1) {
 float12 -=
 owner.getMoodles().getMoodleLevel(MoodleType.Stress) * 0.1F;
 ...
 if (owner.isAimAtFloor() && livingCharacter.bDoShove) {
 float12 = Rand.Next(0.7F, 1.0F) +
 owner.getPerkLevel(PerkFactory.Perks.Strength) * 0.2F;
 Clothing clothing = (Clothing)owner.getWornItem("Shoes");
 if (clothing.empty()) {
 float12 *= 0.5F;
 } else {
 ...
 if (Rand.Next(3) != 0 &&
 (!owner.isAimAtFloor() || !livingCharacter.bDoShove) {
 if (Rand.Next(7) == 0) {
 ...
 zombie0.addBlood(BloodBodyPartType.Torso_Upper, true, false,
 false);
 zombie0.addBlood(BloodBodyPartType.UpperArm_L, true, false,
 false);
 zombie0.addBlood(BloodBodyPartType.UpperArm_R, true, false,
 false);
 float12 *= 3.0F;
 }
 ...
 int int8 = 0;
 if (byte0 > 0) {
 int8 = Rand.Next(BodyPartType.ToIndex(BodyPartType.Head),
 BodyPartType.ToIndex(BodyPartType.Neck) + 1);
 } else if (boolean5) {
 int8 = Rand.Next(BodyPartType.ToIndex(BodyPartType.Groin),
 BodyPartType.ToIndex(BodyPartType.Foot_R) + 1);
 } else {
 int8 = Rand.Next(BodyPartType.ToIndex(BodyPartType.Hand_L),
 BodyPartType.ToIndex(BodyPartType.Neck) + 1);
 }

 float float15 = zombie0.getBodyPartClothingDefense(
 int8, false, weapon.isRanged()) /
 2.0F;
 float15 += zombie0.getBodyPartClothingDefense(int8, true,
 weapon.isRanged());
 if (float15 > 70.0F) {
 float15 = 70.0F;
 }
 ...
 GameClient.bClient && owner instanceof
 IsoPlayer && ((IsoPlayer)owner).isLocalPlayer()) {
 ...
 int int9 =
 owner.getPerkLevel(PerkFactory.Perks.SmallBlade) + 1;
 ...
 if (vehicle.empty() && movingObject.getSquare() != nullptr &&
 owner.getSquare() != nullptr) {
 movingObject.setCloseKilled(owner.attackVars.bCloseKill);
 if (((IsoPlayer)owner).isLocalPlayer() || owner.isNPC()) {
 float17 =
 movingObject.Hit(weapon, owner, float12, boolean0, float14);
 this->setParameterCharacterHitResult(owner, zombie0, long0);
 }

 LuaEventManager.triggerEvent("OnWeaponHitXp", owner, weapon,
 movingObject, float12);
 if ((!livingCharacter.bDoShove || owner.isAimAtFloor()) &&
 ...
 int int11 = this->calcDamageToVehicle(
 (int)float12 * 10, weapon.getDoorDamage(), true);
 vehicleWindow.damage(int11);
 owner.playSound("HitVehicleWindowWithWeapon");
 } else {
 int int12 = this->calcDamageToVehicle(
 (int)float12 * 10, weapon.getDoorDamage(), false);
 ...
 owner, (IsoGameCharacter)movingObject, weapon);
 }

 boolean4 = GameClient.sendHitCharacter(
 owner, movingObject, weapon, float17, boolean0, float14,
 boolean9, boolean2, byte0 > 0);
 }
 }
 }
 }
 ...
 GameClient.sendHitCharacter(owner, nullptr, weapon, 0.0F, boolean0, 1.0F,
 owner.isCriticalHit(), false, false);
 }

 if (!boolean1 && boolean3) {
 bool boolean10 = this->bHitOnlyTree &&
 weapon.getScriptItem().Categories.contains("Axe");
 int int13 = boolean10 ? 2 : 1;
 if (Rand.Next(weapon.getConditionLowerChance() * int13 +
 owner.getMaintenanceMod() * 2) == 0) {
 boolean1 = true;
 ...
 float float0 = int0;
 float float1 =
 PZMath.clamp(float0 / (boolean0 ? 10.0F : 40.0F), 0.0F, 1.0F);
 int int1 = (int)(int2 * float1);
 return PZMath.clamp(int1, 1, int2);
 }
 }

 static void splash(IsoMovingObject obj, HandWeapon weapon,
 IsoGameCharacter owner) {
 IsoGameCharacter character = (IsoGameCharacter)obj;
 if (weapon != nullptr &&
 SandboxOptions.instance.BloodLevel.getValue() > 1) {
 ...
 IsoZombie &&
 (((IsoZombie)character).bCrawling ||
 character.getCurrentState() == ZombieOnGroundState.instance())) {
 float0 = 0.2F;
 }
 ...
 IsoZombieGiblets.GibletType.A, character.getCell(),
 character.getX(), character.getY(), character.getZ() + float0,
 character.getHitDir().x * float1, character.getHitDir().y * float2);
 }
 }
 }

 bool checkObjectHit(IsoGameCharacter character, HandWeapon weapon,
 IsoGridSquare square, bool boolean1, bool boolean0) {
 if (square.empty()) {
 return false;
 } else {
 for (int int0 = square.getSpecialObjects().size() - 1; int0 >= 0;
 int0--) {
 ...
 if (door != nullptr &&
 (boolean1 && door.north || boolean0 && !door.north) {
 ...
 if (!thumpable0.isDoor() && !thumpable0.isWindow() &&
 thumpable0.isBlockAllTheSquare()) {
 Thumpable thumpable2 = thumpable0.getThumpableFor(character);
 if (thumpable2 != nullptr) {
 thumpable2.WeaponHit(character, weapon);
 return true;
 }
 } else if (boolean1 && thumpable0.north ||
 boolean0 && !thumpable0.north) {
 ...
 if (window != nullptr &&
 (boolean1 && window.north || boolean0 && !window.north) {
 ...
 IsoDirections.fromAngle(character.getForwardDirection());
 int int2 = 0;
 int int3 = 0;
 if (directions == IsoDirections.NE || directions == IsoDirections.N ||
 directions == IsoDirections.NW) {
 int3--;
 }

 if (directions == IsoDirections.SE || directions == IsoDirections.S ||
 directions == IsoDirections.SW) {
 int3++;
 }

 if (directions == IsoDirections.NW || directions == IsoDirections.W ||
 directions == IsoDirections.SW) {
 int2--;
 }

 if (directions == IsoDirections.NE || directions == IsoDirections.E ||
 ...
 square0.getX() + int2, square0.getY() + int3, square0.getZ());
 if (square1 != nullptr) {
 ...
 if (directions == IsoDirections.SE || directions == IsoDirections.E ||
 directions == IsoDirections.NE) {
 IsoGridSquare square3 = cell.getGridSquare(
 square0.getX() + 1, square0.getY(), square0.getZ());
...
private
 LosUtil.TestResults los(int int5, int int2, int int4, int int1, int int7,
 SwipeStatePlayer.LOSVisitor lOSVisitor) {
 IsoCell cell = IsoWorld.instance.CurrentCell;
 int int0 = int1 - int2;
 int int3 = int4 - int5;
 int int6 = int7 - int7;
 ...
 cell.getGridSquare(int5, (int)float0, (int)float1);
 ...
 cell.getGridSquare((int)float0, int2, (int)float1);
 ...
 IsoWindow getWindowBetween(IsoMovingObject movingObject0,
 IsoMovingObject movingObject1) {
 return this->getWindowBetween((int)movingObject0.x, (int)movingObject0.y,
 (int)movingObject1.x, (int)movingObject1.y,
 (int)movingObject0.z);
 }

 bool isWindowBetween(IsoMovingObject movingObject0,
 IsoMovingObject movingObject1) {
 return this->getWindowBetween(movingObject0, movingObject1) != nullptr;
 }

 void smashWindowBetween(IsoGameCharacter character,
 IsoMovingObject movingObject, HandWeapon weapon) {
 ...
 if (item != weapon && item instanceof
 HandWeapon && item.getType() == weapon.getType() &&
 item.getCondition() > 0) {
 if (owner.getPrimaryHandItem() == weapon &&
 owner.getSecondaryHandItem() == weapon) {
 owner.setPrimaryHandItem(item);
 ...
 HandWeapon _weapon =
 (HandWeapon)owner.getInventory().getBestWeapon(owner.getDescriptor());
 owner.setPrimaryHandItem(nullptr);
 if (owner.getSecondaryHandItem() == weapon) {
 owner.setSecondaryHandItem(nullptr);
 }

 if (_weapon != nullptr && _weapon != owner.getPrimaryHandItem() &&
 _weapon.getCondition() > 0) {
 owner.setPrimaryHandItem(_weapon);
 if (_weapon.isTwoHandWeapon() &&
 ...
 void setParameterCharacterHitResult(IsoGameCharacter character,
 IsoZombie zombie0, long long0) {
 if (long0 != 0L) {
 uint8_t byte0 = 0;
 if (zombie0 != nullptr) {
 if (zombie0.isDead()) {
 ...
 long0,
 FMODManager.instance.getParameterDescription("CharacterHitResult"),
 byte0);
 }
 }

public
 static class CustomComparator implements Comparator<HitInfo> {
...
private
 static class WindowVisitor implements SwipeStatePlayer.LOSVisitor {
 ...
 square0.getX() - square1.getX(), square0.getY() - square1.getY(),
 square0.getZ() - square1.getZ(), boolean0, boolean1);
 if (testResults == LosUtil.TestResults.ClearThroughWindow) {
 IsoWindow windowx = square1.getWindowTo(square0);
 if (this->isHittable(windowx) &&
 windowx.TestVision(square1, square0) ==
 IsoObject.VisionResult.Unblocked) {
 this->window = windowx;
 return true;
 }
 }
 ...
 this->test != LosUtil.TestResults.ClearThroughOpenDoor) {
 if (testResults == LosUtil.TestResults.ClearThroughClosedDoor &&
 this->test == LosUtil.TestResults.ClearThroughOpenDoor) {
 ...
 public
 LosUtil.TestResults getResult() { return this->test; }

 bool isHittable(IsoWindow windowx) {
 if (windowx.empty()) {
 return false;
 } else {
 return windowx.isBarricaded()
 ? true
 : !windowx.isDestroyed() && !windowx.IsOpen();
 }
 }
 }
}
