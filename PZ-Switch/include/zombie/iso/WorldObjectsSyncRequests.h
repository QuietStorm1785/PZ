...
public
 ArrayList<WorldObjectsSyncRequests.SyncData> requests =
 std::make_unique<ArrayList<>>();
 long timeout = 1000L;

 void putRequest(IsoChunk chunk) {
 WorldObjectsSyncRequests.SyncData syncData =
 new WorldObjectsSyncRequests.SyncData();
 syncData.x = chunk.wx;
 syncData.y = chunk.wy;
 syncData.hashCodeWorldObjects = chunk.getHashCodeObjects();
 syncData.reqTime = 0L;
 syncData.reqCount = 0;
 { std::lock_guard<std::mutex> __sync_lock__(requests_mutex); this->requests.add(syncData); }
 }
 ...
 int int1 = 0;
 { std::lock_guard<std::mutex> __sync_lock__(requests_mutex);
