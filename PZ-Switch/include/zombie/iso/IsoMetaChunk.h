...
private
 ...
 float getZombieIntensity() { return this->getZombieIntensity(true); }
 ...
 int getUnadjustedZombieIntensity() { return this->ZombieIntensity; }
...
public
 IsoMetaGrid.Zone getZone(int index) {
...
public
 ...
 int numZones() { return this->zonesSize; }
 ...
 if (roomRect.x <= x && roomRect.y <= y && x < roomRect.getX2() &&
 y < roomRect.getY2()) {
 return roomDef;
 }
 }
 ...
 int getNumRooms() { return this->roomsSize; }

 void getRoomsIntersecting(int x, int y, int w, int h,
 ArrayList<RoomDef> result) {
 for (int int0 = 0; int0 < this->roomsSize; int0++) {
 RoomDef roomDef = this->rooms[int0];
 if (!roomDef.isEmptyOutside() && !result.contains(roomDef) &&
