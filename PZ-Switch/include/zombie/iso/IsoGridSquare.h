...
public
 IsoGridSquare[] nav = new IsoGridSquare[8];
 int collideMatrix = -1;
 int pathMatrix = -1;
 int visionMatrix = -1;
...
private
 boolean[] playerCutawayFlags = new boolean[4];
private
 long[] playerCutawayFlagLockUntilTimes = new long[4];
private
 boolean[] targetPlayerCutawayFlags = new boolean[4];
private
 boolean[] playerIsDissolvedFlags = new boolean[4];
private
 long[] playerIsDissolvedFlagLockUntilTimes = new long[4];
private
 boolean[] targetPlayerIsDissolvedFlags = new boolean[4];
 IsoWaterGeometry water = nullptr;
 IsoPuddlesGeometry puddles = nullptr;
 float puddlesCacheSize = -1.0F;
 float puddlesCacheLevel = -1.0F;
public
 IsoGridSquare.ILighting[] lighting = new IsoGridSquare.ILighting[4];
 int x;
 int y;
 int z;
...
private
 ArrayList<IsoGameCharacter> DeferedCharacters =
 std::make_unique<ArrayList<>>();
 int DeferredCharacterTick = -1;
private
 ArrayList<IsoMovingObject> StaticMovingObjects = new ArrayList<>(0);
private
 ArrayList<IsoMovingObject> MovingObjects = new ArrayList<>(0);
protected
 PZArrayList<IsoObject> Objects = new PZArrayList<>(IsoObject.class, 2);
protected
 PZArrayList<IsoObject> localTemporaryObjects =
 new PZArrayList<>(IsoObject.class, 2);
private
 ArrayList<IsoWorldInventoryObject> WorldObjects =
 std::make_unique<ArrayList<>>();
...
private
 ArrayList<IsoObject> SpecialObjects = new ArrayList<>(0);
 bool haveRoof = false;
 bool burntOut = false;
 bool bHasFlies = false;
 IsoGridOcclusionData OcclusionDataCache = nullptr;
private
 static PZArrayList<IsoWorldInventoryObject> tempWorldInventoryObjects =
 new PZArrayList<>(IsoWorldInventoryObject.class, 16);
public
 static ConcurrentLinkedQueue<IsoGridSquare> isoGridSquareCache =
...
public
 static ArrayList<IsoGridSquare> choices =
 std::make_unique<ArrayList<>>();
...
public
 static IsoGridSquare.CellGetSquare cellGetSquare =
...
private
 static Comparator<IsoMovingObject> comp =
 ...
 int splashFrameNum;
private
 ColorInfo[] lightInfo = new ColorInfo[4];
...
private
 static int[] SURFACE_OFFSETS = new int[8];
 ...
 try {
 { std::lock_guard<std::mutex> __sync_lock__(IsoPuddlesGeometry.pool_mutex);
 ...
 */
 ...
 !object.AttachedAnimSprite.empty()) {
 bool boolean0 = false;
...
private
 ...
 bool shouldSave() { return !this->Objects.empty(); }
 ...
 */
 ...
 if (square0.empty() || this->isBlockedTo(square0) ||
 square1.empty() || this->isBlockedTo(square1) {
 return nullptr;
...
public
 ...
 return this->getBuilding() == nullptr && this->roofHideBuilding.empty()
 ? this->getFloor()
 : nullptr;
 }
 ...
 */
 ...
 if (square2.empty() || square3.empty() ||
 ...
 if (square4.empty() || square5.empty() ||
...
int renderFloorInternal(Shader shader) {
 ...
 bool boolean1 = GameClient.bClient && IsoPlayer.players[int0] != nullptr &&
 IsoPlayer.players[int0].isSeeNonPvpZone();
 bool boolean2 = Core.bDebug && GameClient.bClient &&
 SafeHouse.isSafeHouse(this, nullptr, true) != nullptr;
 bool boolean3 = true;
 float float1 = 1.0F;
 float float2 = 1.0F;
 if (square1 != nullptr) {
 ...
 if (!boolean5) {
 bool boolean6 = object0.sprite != nullptr &&
...
bool RenderMinusFloorFxMask(int maxZ, bool doSE, bool vegitationRender) {
 ...
 bool boolean5 =
 square0 != nullptr && square0.getPlayerCutawayFlag(int4, long0);
 bool boolean6 = this->getPlayerCutawayFlag(int4, long0);
 bool boolean7 =
 square1 != nullptr && square1.getPlayerCutawayFlag(int4, long0);
...
boolean renderMinusFloor(PZArrayList<IsoObject> pZArrayList, int int8,
 ...
 if (this->z >= int8 &&
 (object1.sprite.empty() || !object1.sprite.alwaysDraw) {
 boolean12 = false;
 }

 bool boolean14 = object1.sprite != nullptr &&
...
public
static class CircleStencilShader extends Shader {
public
 static IsoGridSquare.CircleStencilShader instance =
...
public
static class Lighting implements IsoGridSquare.ILighting {
private
 int[] lightverts = new int[8];
...
public
static class NoCircleStencilShader {
public
 static IsoGridSquare.NoCircleStencilShader instance =
