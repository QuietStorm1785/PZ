...
public
 IsoBarbecue(IsoCell cell) { super(cell); }

public
 IsoBarbecue(IsoCell cell, IsoGridSquare sq, IsoSprite gid) {
 super(cell, sq, gid);
 ...
 this->noTankSprite =
 IsoSprite.getSprite(IsoSpriteManager.instance, this->sprite, byte0);
 } else if (isSpriteWithoutPropaneTank(this->sprite) {
 uint8_t byte1 = -8;
 this->normalSprite =
 IsoSprite.getSprite(IsoSpriteManager.instance, this->sprite, byte1);
 this->noTankSprite = this->sprite;
 }
 }

 std::string getObjectName() { return "Barbecue"; }
 ...
 bool hasPropaneTank() { return this->isPropaneBBQ() && this->bHasPropaneTank; }

 bool isPropaneBBQ() {
 return this->getSprite() != nullptr &&
 this->getProperties().Is("propaneTank");
 }
 ...
 uint8_t byte0 = 8;
 IsoSprite _sprite =
 IsoSprite.getSprite(IsoSpriteManager.instance, sprite, byte0);
 ...
 this->FuelAmount = (int)(this->FuelAmount *
 ((DrainableComboItem)tank).getUsedDelta());
 }
 }
 }
 ...
 bool isSmouldering() { return this->bIsSmouldering; }
 ...
 void toggle() { this->setLit(!this->isLit()); }
 ...
 std::string string =
 this->isPropaneBBQ() ? "BBQPropaneRunning" : "BBQRegularRunning";
 ...
 "IsoBarbecue burned " + int0 + " minutes (" +
 ...
 int int1 = Math.min(int0, SMOULDER_MINUTES -
 this->MinutesSinceExtinguished);
 DebugLog.log(DebugType.Fireplace,
 "IsoBarbecue smoldered " + int1 + " minutes (" +
 this->getFuelAmount() + " remaining)");
 ...
 if (!this->isLit() || this->AttachedAnimSprite != nullptr &&
 !this->AttachedAnimSprite.empty()) {
 if (!this->isLit() && this->AttachedAnimSprite != nullptr &&
 !this->AttachedAnimSprite.empty()) {
 ...
 this->AttachAnim("Smoke", "01", 4, IsoFireManager.SmokeAnimDelay, -14, 58,
 true, 0, false, 0.7F, colorInfo);
 this->AttachedAnimSprite.get(0).alpha =
 ...
 */
 ...
 this->noTankSprite =
 IsoSprite.getSprite(IsoSpriteManager.instance, newsprite, byte0);
 } else if (isSpriteWithoutPropaneTank(newsprite) {
 uint8_t byte1 = -8;
 this->normalSprite =
 IsoSprite.getSprite(IsoSpriteManager.instance, newsprite, byte1);
