...
public
 IsoCombinationWasherDryer(IsoCell cell) { super(cell); }

public
 IsoCombinationWasherDryer(IsoCell cell, IsoGridSquare sq, IsoSprite gid) {
 super(cell, sq, gid);
 }

 std::string getObjectName() { return "CombinationWasherDryer"; }

 void load(ByteBuffer input, int WorldVersion, bool IS_DEBUG_SAVE) {
 super.load(input, WorldVersion, IS_DEBUG_SAVE);
 this->m_logic = (IClothingWasherDryerLogic)(input.get() == 0 ? this->m_washer
 : this->m_dryer);
 ...
 bool isModeWasher() { return this->m_logic == this->m_washer; }

 bool isModeDryer() { return this->m_logic == this->m_dryer; }
}
} // namespace objects
} // namespace iso
} // namespace zombie
