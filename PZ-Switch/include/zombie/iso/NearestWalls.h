...
public:
 ...
 if (square0 != nullptr && (square0.Is(IsoFlagType.WallW) ||
 ...
 if (square1 != nullptr && (square1.Is(IsoFlagType.WallN) ||
 ...
 return (int2 - chunk.wx * 10) * 4 + (int1 - chunk.wy * 10) * 40 +
 int0 * 10 * 40;
 }

 static int getNearestWallOnSameChunk(IsoChunk chunk, int int1, int int2,
 int int3, int int4) {
 ...
 IsoChunk chunk1 = IsoWorld.instance.CurrentCell.getChunk(
 chunk0.wx + int4 * byte1, chunk0.wy + int4 * byte2);
 if (chunk1.empty()) {
 break;
 }
 ...
 int0 = hasWall(chunk1, int5, int2, int3, 0)
 ? 0
 : getNearestWallOnSameChunk(chunk1, int5, int2, int3, byte0);
 if (int0 != -1) {
 return int5 + int0;
 }
 }
 ...
 static int getNearestWallNorth(IsoChunk chunk0, int int1, int int2,
 int int3) {
 uint8_t byte0 = 2;
 uint8_t byte1 = 0;
 uint8_t byte2 = -1;
 ...
 static int getNearestWallSouth(IsoChunk chunk0, int int1, int int2,
 int int3) {
 uint8_t byte0 = 3;
 uint8_t byte1 = 0;
 uint8_t byte2 = 1;
 ...
 int0 = hasWall(chunk1, int1, int5, int3, 2)
 ? 0
 : getNearestWallOnSameChunk(chunk1, int1, int5, int3, byte0);
 if (int0 != -1) {
 return int5 + int0;
 }
 }
 ...
 renderY = int1;
 renderZ = int2;
 System.out.println("ClosestWallDistance=" +
 ClosestWallDistance(chunk, int0, int1, int2);
 }

 int int3 = getNearestWallWest(chunk, int0, int1, int2);
 if (int3 != -1) {
 DrawIsoLine(int3, int1 + 0.5F, int0 + 0.5F, int1 + 0.5F, int2, 1.0F,
 1.0F, 1.0F, 1.0F, 1);
 DrawIsoLine(int3, int1, int3, int1 + 1, int2, 1.0F, 1.0F, 1.0F, 1.0F,
 1);
 }
 ...
 if (int4 != -1) {
 DrawIsoLine(int0 + 0.5F, int4, int0 + 0.5F, int1 + 0.5F, int2, 1.0F,
 1.0F, 1.0F, 1.0F, 1);
 DrawIsoLine(int0, int4, int0 + 1, int4, int2, 1.0F, 1.0F, 1.0F, 1.0F,
 1);
 }
...
private
 ...
 return square != nullptr && square.chunk != nullptr
 ? ClosestWallDistance(square.chunk, square.x, square.y, square.z)
 : 127;
 }
 ...
 int int0 =
 int3 - chunk.wx * 10 + (int2 - chunk.wy * 10) * 10 + int1 * 10 * 10;
 ...
 int3 - chunk.wx * 10, int2 - chunk.wy * 10, int1);
 ...
 return bytes[int0] = (byte)Math.min(
 int4, Math.min(int5, Math.min(int6, int7));
 } else {
 return bytes[int0] = (byte)127;
 }
 }
