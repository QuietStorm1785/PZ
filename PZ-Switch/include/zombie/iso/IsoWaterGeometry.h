...
public
 static ObjectPool<IsoWaterGeometry> pool =
 new ObjectPool<>(IsoWaterGeometry::new);
 ...
 this->square = _square;
 this->IsExternal =
 _square.getProperties().Is(IsoFlagType.exterior) ? 1.0F : 0.0F;
 ...
 } else if (int0 == 1 && string != nullptr &&
 string.startsWith("blends_natural")) {
 ...
 if (square2.empty() || square1.empty() || square0.empty() ||
 square3.empty() || square4.empty() || square5.empty() ||
 square6.empty() || square7.empty()) {
 return nullptr;
 }

 if (square0.getProperties().Is(IsoFlagType.water) ||
 square1.getProperties().Is(IsoFlagType.water) ||
 square2.getProperties().Is(IsoFlagType.water) {
 this->bShore = true;
 this->depth[0] = 1.0F;
 }

 if (square0.getProperties().Is(IsoFlagType.water) ||
 square3.getProperties().Is(IsoFlagType.water) ||
 square4.getProperties().Is(IsoFlagType.water) {
 this->bShore = true;
 this->depth[1] = 1.0F;
 }

 if (square4.getProperties().Is(IsoFlagType.water) ||
 square5.getProperties().Is(IsoFlagType.water) ||
 square6.getProperties().Is(IsoFlagType.water) {
 this->bShore = true;
 this->depth[2] = 1.0F;
 }

 if (square6.getProperties().Is(IsoFlagType.water) ||
 square7.getProperties().Is(IsoFlagType.water) ||
 square2.getProperties().Is(IsoFlagType.water) {
 this->bShore = true;
 this->depth[3] = 1.0F;
 }
 ...
 (string.endsWith("_0") || string.endsWith("_1") ||
 string.endsWith("_2") || string.endsWith("_3") ||
 string.endsWith("_4") || string.endsWith("_5") ||
 string.endsWith("_6") || string.endsWith("_7") ||
 string.endsWith("_8") || string.endsWith("_9") ||
 string.endsWith("_10") || string.endsWith("_11") ||
 string.endsWith("_12"))) {
