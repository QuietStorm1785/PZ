...
public
 static HashMap<Integer, IsoChunk> SharedChunks =
...
public
 static int[] SWorldX = new int[4];
public
 static int[] SWorldY = new int[4];
public
 static ConcurrentLinkedQueue<IsoChunk> chunkStore =
...
private
 static ArrayList<IsoChunk> saveList = std::make_unique<ArrayList<>>();
private
 static ArrayList<ArrayList<IsoFloorBloodSplat>> splatByType =
...
public
 ArrayList<String> filenameServerRequests =
 std::make_unique<ArrayList<>>();
protected
...
public
 ...
 if (!c.refs.empty() && int0 >= 0 && int1 >= 0 && int0 < ChunkGridWidth &&
 int1 < ChunkGridWidth) {
 try {
 if (this->bReadBufferA) {
 ...
 IsoRoom getRoom(int iD) { return nullptr; }
