...
public:
 ...
 static std::mutex SpottedRooms_mutex;
...
public
 ...
 static ArrayList<IsoGridSquare> stchoices =
 std::make_unique<ArrayList<>>();
public
 IsoChunkMap[] ChunkMap = new IsoChunkMap[4];
public
 ArrayList<IsoBuilding> BuildingList = std::make_unique<ArrayList<>>();
private
 ArrayList<IsoWindow> WindowList = std::make_unique<ArrayList<>>();
private
 ArrayList<IsoMovingObject> ObjectList = std::make_unique<ArrayList<>>();
private
 ArrayList<IsoPushableObject> PushableObjectList =
 std::make_unique<ArrayList<>>();
private
 HashMap<Integer, BuildingScore> BuildingScores =
 std::make_unique<HashMap<>>();
private
 ArrayList<IsoRoom> RoomList = std::make_unique<ArrayList<>>();
private
 ArrayList<IsoObject> StaticUpdaterObjectList =
 std::make_unique<ArrayList<>>();
private
 ArrayList<IsoZombie> ZombieList = std::make_unique<ArrayList<>>();
private
 ArrayList<IsoGameCharacter> RemoteSurvivorList =
 std::make_unique<ArrayList<>>();
private
 ArrayList<IsoMovingObject> removeList = std::make_unique<ArrayList<>>();
private
 ArrayList<IsoMovingObject> addList = std::make_unique<ArrayList<>>();
private
 ArrayList<IsoObject> ProcessIsoObject = std::make_unique<ArrayList<>>();
private
 ArrayList<IsoObject> ProcessIsoObjectRemove =
 std::make_unique<ArrayList<>>();
private
 ArrayList<InventoryItem> ProcessItems = std::make_unique<ArrayList<>>();
private
 ArrayList<InventoryItem> ProcessItemsRemove =
 std::make_unique<ArrayList<>>();
private
 ArrayList<IsoWorldInventoryObject> ProcessWorldItems =
 std::make_unique<ArrayList<>>();
public
 ArrayList<IsoWorldInventoryObject> ProcessWorldItemsRemove =
 std::make_unique<ArrayList<>>();
private
 IsoGridSquare[][] gridSquares =
 ...
 bool safeToAdd = true;
private
 Stack<IsoLightSource> LamppostPositions = std::make_unique<Stack<>>();
public
 ArrayList<IsoRoomLight> roomLights = std::make_unique<ArrayList<>>();
private
 ArrayList<IsoHeatSource> heatSources = std::make_unique<ArrayList<>>();
public
 ArrayList<BaseVehicle> addVehicles = std::make_unique<ArrayList<>>();
public
...
private
 IsoZombie[] nearestVisibleZombie = new IsoZombie[4];
private
 float[] nearestVisibleZombieDistSqr = new float[4];
private
...
private
 static ArrayList<IsoGridSquare> ShadowSquares = new ArrayList<>(1000);
private
 static ArrayList<IsoGridSquare> MinusFloorCharacters =
 new ArrayList<>(1000);
private
 static ArrayList<IsoGridSquare> SolidFloor = new ArrayList<>(5000);
private
 static ArrayList<IsoGridSquare> ShadedFloor = new ArrayList<>(5000);
private
 static ArrayList<IsoGridSquare> VegetationCorpses =
 new ArrayList<>(5000);
public
 static IsoCell.PerPlayerRender[] perPlayerRender =
 new IsoCell.PerPlayerRender[4];
private
 int[] StencilXY = new int[]{0, 0, -1, 0, 0, -1, -1, -1, -2, -1,
 -1, -2, -2, -2, -3, -2, -2, -3, -3, -3};
private
 int[] StencilXY2z =
...
private
 ...
 ArrayList<ArrayList<IsoBuilding>> playerOccluderBuildings =
 new ArrayList<>(4);
 ...
 ArrayList<ArrayList<IsoBuilding>> zombieOccluderBuildings =
 new ArrayList<>(4);
 ...
 ArrayList<ArrayList<IsoBuilding>> otherOccluderBuildings =
 new ArrayList<>(4);
...
private
 Stack<IsoRoom> SpottedRooms = std::make_unique<Stack<>>();
...
private
 ...
 bool boolean1 =
 square3 != nullptr && square3.getPlayerCutawayFlag(int3, long0);
 bool boolean2 = square2.getPlayerCutawayFlag(int3, long0);
 bool boolean3 =
 square4 != nullptr && square4.getPlayerCutawayFlag(int3, long0);
 ...
 if ((object0.empty() || !object0.sprite.name.contains("fencing")) &&
 (object1.empty() || !object1.sprite.name.contains("fencing"))) {
 ...
 for (int int1 = square1.getZ() - 1; int1 >= 0 && building.empty();
 int1--) {
 IsoGridSquare square2 =
 ...
 square.room.empty() && !square.isInARoom()) {
...
private
 ...
 if (float1 < float0 &&
 (except.empty() || except.building != building1) {
 building0 = building1;
 float0 = float1;
 }
 }
 ...
 room.empty() ? nullptr : room.building;
 }
...
public
 ...
 } while (square.empty() || !square.isFree(false) ||
...
public
 ...
 assert square = = nullptr || x == square.getX() && y == square.getY() &&
 z == square.getZ();
 ...
 { std::lock_guard<std::mutex> __sync_lock__(SpottedRooms_mutex);
 ...
 if (this->VisiOccludedFlags.empty() ||
 ...
 public
 Texture[][][] grid = new Texture[this->w][this->h][2];
 public
 byte[][][] gridType = new byte[this->w][this->h][2];
