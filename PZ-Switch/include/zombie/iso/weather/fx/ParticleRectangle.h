...
public
 ...
 int6 =
 this->height - (int)Math.abs(IsoCamera.frameState.OffY) % this->height;
 }
 ...
 LineDrawer.drawRect(int11, int12, this->width, this->height, 0.0F, 1.0F,
 0.0F, 1.0F, 1);
 }
 ...
 if (!(weatherParticle.renderAlpha <= 0.0F) &&
 weatherParticle.isOnScreen(int11, int12) {
 weatherParticle.render(int11, int12);
 if (this->DEBUG_BOUNDS || IsoWeatherFX.DEBUG_BOUNDS) {
 LineDrawer.drawRect(int11 + weatherParticle.bounds.getX(),
 int12 + weatherParticle.bounds.getY(),
 weatherParticle.bounds.getWidth(),
 weatherParticle.bounds.getHeight(), 0.0F,
 0.0F, 1.0F, 0.5F, 1);
 }
 }
 }
