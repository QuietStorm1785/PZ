...
protected
 ...
 this->setTarget(
 (IsoGameCharacter)arrayList.get(Rand.Next(arrayList.size())));
 }
 }
 ...
 if (IsoUtils.DistanceToSquared(this->x, this->y, this->targetX,
 this->targetY) < 4.0F) {
 ...
 this->timeSinceChopperSawPlayer =
 this->timeSinceChopperSawPlayer +
 ...
 this->targetY) < 1.0F) {
 this->targetX = this->target.x +
 (Rand.Next(RADIUS_HOVER * 2) - RADIUS_HOVER);
 this->targetY = this->target.y +
 (Rand.Next(RADIUS_HOVER * 2) - RADIUS_HOVER);
 this->move.x = this->targetX - this->x;
 this->move.y = this->targetY - this->y;
 ...
 this->timeSinceChopperSawPlayer = 0.0F;
 this->changeState(Helicopter.State.Hovering);
 } else if (IsoUtils.DistanceToSquared(this->x, this->y, this->targetX,
 this->targetY) < 1.0F) {
 this->targetX = this->target.x +
 (Rand.Next(RADIUS_SEARCH * 2) - RADIUS_SEARCH);
 this->targetY = this->target.y +
 (Rand.Next(RADIUS_SEARCH * 2) - RADIUS_SEARCH);
 this->move.x = this->targetX - this->x;
 this->move.y = this->targetY - this->y;
 ...
 if (IsoUtils.DistanceToSquared(this->x, this->y, player0.getX(),
 player0.getY()) < 1000000.0F) {
 boolean0 = true;
 break;
 }
 }
 ...
 if (player1 != nullptr &&
 IsoUtils.DistanceToSquared(this->x, this->y, player1.getX(),
 player1.getY()) < 1000000.0F) {
 boolean0 = true;
 break;
 }
 }
 ...
 WorldSoundManager.instance.addSound(nullptr, (int)this->x, (int)this->y,
 0, 500, 500);
 }

 float float1 =
 this->move.x * (GameTime.getInstance().getMultiplier() / 1.6F);
 float float2 =
 this->move.y * (GameTime.getInstance().getMultiplier() / 1.6F);
 if (this->state != Helicopter.State.Leaving &&
 IsoUtils.DistanceToSquared(this->x + float1, this->y + float2,
 this->targetX, this->targetY) >
 IsoUtils.DistanceToSquared(this->x, this->y, this->targetX,
 this->targetY) {
 this->x = this->targetX;
 this->y = this->targetY;
 } else {
 ...
 inst = javafmod.FMOD_Studio_System_CreateEventInstance(
 event.address);
 }
 }
 ...
 javafmod.FMOD_Studio_EventInstance3D(inst, this->x, this->y,
 200.0F);
 float float1 = 0.0F;
 ...
 return zone = = nullptr ? true
 : !"Forest".equals(zone.getType()) &&
...
private
 static enum State { Arriving, Hovering, Searching, Leaving; }
}
} // namespace iso
} // namespace zombie
