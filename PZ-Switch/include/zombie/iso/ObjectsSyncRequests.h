...
public:
 ...
 std::mutex requestsSyncIsoChunk_mutex;
 std::mutex requestsSyncIsoGridSquare_mutex;
 std::mutex requestsSyncIsoObject_mutex;
...
public
 ArrayList<ObjectsSyncRequests.SyncIsoChunk> requestsSyncIsoChunk;
public
 ArrayList<ObjectsSyncRequests.SyncIsoGridSquare> requestsSyncIsoGridSquare;
public
 ArrayList<ObjectsSyncRequests.SyncIsoObject> requestsSyncIsoObject;
 long timeout = 1000L;

public
 ObjectsSyncRequests(bool boolean0) {
 if (boolean0) {
 this->requestsSyncIsoChunk = std::make_unique<ArrayList<>>();
 this->requestsSyncIsoGridSquare = std::make_unique<ArrayList<>>();
 this->requestsSyncIsoObject = std::make_unique<ArrayList<>>();
 } else {
 this->requestsSyncIsoGridSquare = std::make_unique<ArrayList<>>();
 }
 }
 ...
 void putRequestSyncIsoChunk(IsoChunk chunk) {
 if (!GameClient.bClient || SystemDisabler.doWorldSyncEnable) {
 ObjectsSyncRequests.SyncIsoChunk syncIsoChunk =
 new ObjectsSyncRequests.SyncIsoChunk();
 ...
 { std::lock_guard<std::mutex> __sync_lock__(requestsSyncIsoChunk_mutex);
 this->requestsSyncIsoChunk.add(syncIsoChunk);
 }
 }
 }

 void putRequestSyncItemContainer(ItemContainer container) {
 if (container != nullptr && container.parent != nullptr &&
 container.parent.square != nullptr) {
 this->putRequestSyncIsoGridSquare(container.parent.square);
 }
 }

 void putRequestSyncIsoGridSquare(IsoGridSquare square) {
 if (square != nullptr) {
 ObjectsSyncRequests.SyncIsoGridSquare syncIsoGridSquare =
 new ObjectsSyncRequests.SyncIsoGridSquare();
 ...
 { std::lock_guard<std::mutex> __sync_lock__(requestsSyncIsoGridSquare_mutex);
 if (!this->requestsSyncIsoGridSquare.contains(square) {
 this->requestsSyncIsoGridSquare.add(syncIsoGridSquare);
 } else {
 DebugLog.log("Warning: [putRequestSyncIsoGridSquare] Tryed to add "
 "dublicate object.");
 }
 }
 }
 ...
 if (this->requestsSyncIsoChunk != nullptr &&
 this->requestsSyncIsoChunk.size() != 0) {
 ...
 int int1 = 0;
 { std::lock_guard<std::mutex> __sync_lock__(requestsSyncIsoChunk_mutex);
 for (int int2 = this->requestsSyncIsoChunk.size() - 1; int2 >= 0;
 int2--) {
 ObjectsSyncRequests.SyncIsoChunk syncIsoChunk =
 this->requestsSyncIsoChunk.get(int2);
 ...
 if (System.currentTimeMillis() - syncIsoChunk.reqTime >=
 this->timeout) {
 syncIsoChunk.reqTime = System.currentTimeMillis();
 int1++;
 byteBuffer0.putInt(syncIsoChunk.x);
 ...
 if (this->requestsSyncIsoGridSquare != nullptr &&
 this->requestsSyncIsoGridSquare.size() != 0) {
 ...
 int int5 = 0;
 { std::lock_guard<std::mutex> __sync_lock__(requestsSyncIsoGridSquare_mutex);
 for (int int6 = 0; int6 < this->requestsSyncIsoGridSquare.size();
 int6++) {
 ObjectsSyncRequests.SyncIsoGridSquare syncIsoGridSquare =
 this->requestsSyncIsoGridSquare.get(int6);
 ...
 if (System.currentTimeMillis() - syncIsoGridSquare.reqTime >=
 this->timeout) {
 syncIsoGridSquare.reqTime = System.currentTimeMillis();
 int5++;
 byteBuffer1.putInt(syncIsoGridSquare.x);
 ...
 if (this->requestsSyncIsoObject != nullptr &&
 this->requestsSyncIsoObject.size() != 0) {
 ...
 int int9 = 0;
 { std::lock_guard<std::mutex> __sync_lock__(requestsSyncIsoObject_mutex);
 for (int int10 = 0; int10 < this->requestsSyncIsoObject.size();
 int10++) {
 ObjectsSyncRequests.SyncIsoObject syncIsoObject =
 this->requestsSyncIsoObject.get(int10);
 ...
 if (System.currentTimeMillis() - syncIsoObject.reqTime >=
 this->timeout) {
 syncIsoObject.reqTime = System.currentTimeMillis();
 int9++;
 byteBuffer2.putInt(syncIsoObject.x);
 ...
 void receiveSyncIsoChunk(int int2, int int1) {
 { std::lock_guard<std::mutex> __sync_lock__(requestsSyncIsoChunk_mutex);
 for (int int0 = 0; int0 < this->requestsSyncIsoChunk.size(); int0++) {
 ObjectsSyncRequests.SyncIsoChunk syncIsoChunk =
 this->requestsSyncIsoChunk.get(int0);
 ...
 void receiveSyncIsoGridSquare(int int3, int int2, int int1) {
 { std::lock_guard<std::mutex> __sync_lock__(requestsSyncIsoGridSquare_mutex);
 for (int int0 = 0; int0 < this->requestsSyncIsoGridSquare.size(); int0++) {
 ObjectsSyncRequests.SyncIsoGridSquare syncIsoGridSquare =
 this->requestsSyncIsoGridSquare.get(int0);
 if (syncIsoGridSquare.x == int3 && syncIsoGridSquare.y == int2 &&
 syncIsoGridSquare.z == int1) {
 ...
 void receiveSyncIsoObject(int int3, int int2, int int1, long long0) {
 { std::lock_guard<std::mutex> __sync_lock__(requestsSyncIsoObject_mutex);
 for (int int0 = 0; int0 < this->requestsSyncIsoObject.size(); int0++) {
 ObjectsSyncRequests.SyncIsoObject syncIsoObject =
 this->requestsSyncIsoObject.get(int0);
 if (syncIsoObject.x == int3 && syncIsoObject.y == int2 &&
 syncIsoObject.z == int1 && syncIsoObject.hash == long0) {
 ...
 void receiveGridSquareHashes(ByteBuffer byteBuffer) {
 ...
 IsoWorld.instance.CurrentCell.getGridSquare(int2, int3, byte0);
 if (square != nullptr) {
 ...
 void receiveGridSquareObjectHashes(ByteBuffer byteBuffer) {
 short short0 = byteBuffer.getShort();
 ...
 IsoGridSquare square =
 IsoWorld.instance.CurrentCell.getGridSquare(int1, int2, byte0);
 if (square.empty()) {
 return;
 }
 ...
 if (booleans1[int9]) {
 ObjectsSyncRequests.SyncIsoObject syncIsoObject =
 new ObjectsSyncRequests.SyncIsoObject();
 ...
 DebugLog.log("ERROR: receiveGridSquareObjects " +
 ...
 void receiveObject(ByteBuffer byteBuffer) {
 ...
 IsoGridSquare square =
 IsoWorld.instance.CurrentCell.getGridSquare(int0, int1, byte0);
 if (square != nullptr) {
 uint8_t byte1 = byteBuffer.get();
 ...
 if (int5 == -1) {
 DebugLog.log("ERROR: ObjectsSyncRequest.receiveObject OBJECT EXIST (" +
 int0 + ", " + int1 + ", " + byte0 + ") hash=" + long0);
 } else {
 ...
 square.RecalcAllWithNeighbours(true);
 IsoWorld.instance.CurrentCell.checkHaveRoof(square.getX(),
 square.getY());
 LuaEventManager.triggerEvent("OnContainerUpdate");
 }
 }
 ...
 void serverSendRequests(UdpEngine udpEngine) {
 ...
 void serverSendRequests(UdpConnection udpConnection) {
 ...
 ObjectsSyncRequests.SyncIsoGridSquare syncIsoGridSquare =
 this->requestsSyncIsoGridSquare.get(int2);
 if (udpConnection.RelevantTo(syncIsoGridSquare.x, syncIsoGridSquare.y,
 100.0F) {
 IsoGridSquare square = ServerMap.instance.getGridSquare(
 syncIsoGridSquare.x, syncIsoGridSquare.y, syncIsoGridSquare.z);
 if (square != nullptr) {
 int1++;
...
class SyncIsoChunk {
 ...
 int hashCode() { return this->x + this->y + this->z; }
 }
 ...
 int hashCode() { return (int)(this->x + this->y + this->z + this->hash); }
 }
}
} // namespace iso
} // namespace zombie
