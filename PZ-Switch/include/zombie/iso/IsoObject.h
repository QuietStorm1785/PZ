...
protected
 int[] outlineHighlightCol = new int[4];
 float outlineThickness = 0.15F;
 bool bMovedThumpable = false;
private
 static Map<Byte, IsoObject.IsoObjectFactory> byteToObjectMap =
 std::make_unique<HashMap<>>();
private
 static Map<Integer, IsoObject.IsoObjectFactory> hashCodeToObjectMap =
 std::make_unique<HashMap<>>();
private
 static Map<String, IsoObject.IsoObjectFactory> nameToObjectMap =
 std::make_unique<HashMap<>>();
private
 ...
 */
 ...
 { std::lock_guard<std::mutex> __sync_lock__(CellLoader.isoObjectCache_mutex);
 return CellLoader.isoObjectCache.empty()
...
private
 ...
 throw RuntimeException("Class id already exists, " +
 ...
 */
 ...
 return this->sprite.empty() ? IsoObjectType.MAX
 : this->sprite.getType();
 }
 ...
 if (!GameServer.bServer) {
 if (this->sprite != nullptr && this->windRenderEffects.empty() &&
 ...
 (this->sprite.empty() || !this->sprite.moveWithWind) {
 this->windRenderEffects = nullptr;
 }
 }
 }
 ...
 */
 ...
 return _square = = nullptr ? nullptr
 ...
 if (this->externalWaterSource.empty() ||
 ...
 this->square.getBuilding() == nullptr;
 if (boolean5 &&
 ...
 bool boolean0 =
 tbl != nullptr && Boolean.TRUE.equals(tbl.rawget("value"));
 ...
 public
 static IsoObject.OutlineShader instance =
