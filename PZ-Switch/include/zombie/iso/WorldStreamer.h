...
class WorldStreamer {
public:
 static WorldStreamer.ChunkComparator comp =
...
private
 ConcurrentLinkedQueue<IsoChunk> jobQueue =
 std::make_unique<ConcurrentLinkedQueue<>>();
private
 Stack<IsoChunk> jobList = std::make_unique<Stack<>>();
private
 ConcurrentLinkedQueue<IsoChunk> chunkRequests0 =
 std::make_unique<ConcurrentLinkedQueue<>>();
private
 ArrayList<IsoChunk> chunkRequests1 = std::make_unique<ArrayList<>>();
private
 ArrayList<WorldStreamer.ChunkRequest> pendingRequests =
 std::make_unique<ArrayList<>>();
private
 ArrayList<WorldStreamer.ChunkRequest> pendingRequests1 =
 std::make_unique<ArrayList<>>();
private
 ConcurrentLinkedQueue<WorldStreamer.ChunkRequest> sentRequests =
 std::make_unique<ConcurrentLinkedQueue<>>();
 const CRC32 crc32 = new CRC32();
private
 ConcurrentLinkedQueue<ByteBuffer> freeBuffers =
 std::make_unique<ConcurrentLinkedQueue<>>();
private
 ConcurrentLinkedQueue<WorldStreamer.ChunkRequest> waitingToSendQ =
 std::make_unique<ConcurrentLinkedQueue<>>();
private
...
private
 ConcurrentLinkedQueue<WorldStreamer.ChunkRequest> waitingToCancelQ =
 std::make_unique<ConcurrentLinkedQueue<>>();
...
private
 ...
 (chunkRequest.flagsWS & 1) != 0 ? nullptr : chunkRequest.bb;
 ...
 " refs.empty() SafeWrite=" + (byteBuffer != nullptr);
 }
 ...
 if (boolean0 &&
 (int0 != 0 || int1 != 0 || !this->pendingRequests1.empty())) {
 ...
 bool isBusy() {
 return !GameClient.bClient || this->chunkRequests0.empty() &&
 ...
 object0.getName() + " " +
 (object0.sprite.empty() ? "no sprite"
 : object0.sprite.name);
 DebugLog.log(" 2: " + object1.getClass().getName() + " " +
 object1.getName() + " " +
 (object1.sprite.empty() ? "no sprite"
 : object1.sprite.name);
 }
 }
 } else {
 ...
 object2.getName() + " " +
 (object2.sprite.empty() ? "no sprite"
 : object2.sprite.name);
 }
 }
 }
...
private
 ...
 static ArrayDeque<WorldStreamer.ChunkRequest> pool =
