...
private
 List<Integer> threadDiscoveredChunks = std::make_unique<ArrayList<>>();
 int lastThreadDiscoveredChunksSize = 0;
private
 ConcurrentLinkedQueue<RegionJob> jobQueue =
 std::make_unique<ConcurrentLinkedQueue<>>();
private
 ConcurrentLinkedQueue<JobChunkUpdate> jobOutgoingQueue =
 std::make_unique<ConcurrentLinkedQueue<>>();
private
 List<RegionJob> jobBatchedProcessing = std::make_unique<ArrayList<>>();
private
 ConcurrentLinkedQueue<RegionJob> finishedJobQueue =
 std::make_unique<ConcurrentLinkedQueue<>>();
...
protected
 ...
 if (Core.bDebug) {
 throw IsoRegionException("IsoRegion: Server send full data "
 "target connection.empty()");
 }

 IsoRegions.warn(
 "IsoRegion: Server send full data target connection.empty()");
 break;
 }
