...
private
 boolean[] activeZLayers = new boolean[8];
private
 boolean[] dirtyZLayers = new boolean[8];
private
 byte[] squareFlags;
private
 byte[] regionIDs;
private
 ArrayList<ArrayList<IsoChunkRegion>> chunkRegions = new ArrayList<>(8);
 static uint8_t selectedFlags;
private
 static ArrayDeque<DataSquarePos> tmpSquares =
 std::make_unique<ArrayDeque<>>();
private
 static HashSet<Integer> tmpLinkedChunks = std::make_unique<HashSet<>>();
private
 static boolean[] exploredPositions = new boolean[100];
 static IsoChunkRegion lastCurRegion;
 static IsoChunkRegion lastOtherRegionFullConnect;
private
 static ArrayList<IsoChunkRegion> oldList = std::make_unique<ArrayList<>>();
private
 static ArrayDeque<IsoChunkRegion> chunkQueue =
 std::make_unique<ArrayDeque<>>();

protected
 DataChunk(int int1, int int2, DataCell dataCell, int int0) {
 this->cell = dataCell;
 ...
 int getHashId() { return this->hashId; }

 int getChunkX() { return this->chunkX; }

 int getChunkY() { return this->chunkY; }

protected
 ArrayList<IsoChunkRegion> getChunkRegions(int int0) {
 return this->chunkRegions.get(int0);
 }

 long getLastUpdateStamp() { return this->lastUpdateStamp; }
 ...
 return int2 >= 0 && int2 < 10 && int1 >= 0 && int1 < 10 && int0 >= 0 &&
 int0 < this->highestZ + 1;
 }
 ...
 if (this->squareFlags != nullptr &&
 (ignoreCoordCheck || this->validCoords(x, y, z)) {
 return this->activeZLayers[z] ? this->squareFlags[this->getCoord1D(x, y, z)]
 : -1;
 } else {
 return -1;
 }
 ...
 */
 ...
 uint8_t squareGetFlags(int int0) { return this->squareFlags[int0]; }
 ...
 return this->squareCanConnect(this->getCoord1D(int1, int2, int0), int0,
 byte0);
 }
 ...
 IsoChunkRegion chunkRegion0 =
 this->cell.dataRoot.regionManager.allocIsoChunkRegion(int0);
 ...
 int int3 =
 this->getCoord1D(dataSquarePos0.x, dataSquarePos0.y, dataSquarePos0.z);
 ...
 DataSquarePos dataSquarePos1 =
 this->getNeighbor(dataSquarePos0, byte1);
 if (dataSquarePos1 != nullptr) {
 int int4 = this->getCoord1D(dataSquarePos1.x, dataSquarePos1.y,
 dataSquarePos1.z);
 ...
 this->squareCanConnect(int4, dataSquarePos1.z,
 IsoRegions.GetOppositeDir(byte1)) {
 ...
 this->getIsoChunkRegion(int4, dataSquarePos1.z);
 ...
 void clearExploredPositions() { Arrays.fill(exploredPositions, false); }
 ...
 return int0 >= 0 && int0 < 10 && int1 >= 0 && int1 < 10
 ? DataSquarePos.alloc(int0, int1, dataSquarePos.z)
 : nullptr;
 }

 void link(DataChunk dataChunk1, DataChunk dataChunk2, DataChunk dataChunk3,
 ...
 IsoChunkRegion chunkRegion1 =
 dataChunk0 != nullptr ? dataChunk0.getIsoChunkRegion(int10, int4)
 : nullptr;
 if (chunkRegion0.empty()) {
 IsoRegions.warn(
 "ds.getRegion()==nullptr, shouldnt happen at this point.");
 } else {
 if (lastCurRegion != nullptr && lastCurRegion != chunkRegion0) {
 lastOtherRegionFullConnect = nullptr;
 }

 if (lastCurRegion.empty() || lastCurRegion != chunkRegion0 ||
 chunkRegion1.empty() ||
 lastOtherRegionFullConnect != chunkRegion1) {
 if (dataChunk0 != nullptr && chunkRegion1 != nullptr) {
 if (this->squareCanConnect(int9, int4, byte0) &&
 dataChunk0.squareCanConnect(
 int10, int4, IsoRegions.GetOppositeDir(byte0)) {
 ...
 chunkRegion1.setEnclosed(IsoRegions.GetOppositeDir(byte0),
 true);
 }
 ...
 for (int int1 = 0; int1 < arrayList.size(); int1++) {
 IsoChunkRegion chunkRegion0 = (IsoChunkRegion)arrayList.get(int1);
 if (chunkRegion0.getzLayer() == int0 &&
 chunkRegion0.getIsoWorldRegion() == nullptr) {
 if (chunkRegion0.getConnectedNeighbors().size() == 0) {
 IsoWorldRegion worldRegion0 =
 this->cell.dataRoot.regionManager.allocIsoWorldRegion();
 this->cell.dataRoot.EnqueueDirtyIsoWorldRegion(worldRegion0);
 worldRegion0.addIsoChunkRegion(chunkRegion0);
 } else {
 IsoChunkRegion chunkRegion1 =
 chunkRegion0.getConnectedNeighborWithLargestIsoWorldRegion();
 ...
 this->cell.dataRoot.regionManager.releaseIsoWorldRegion(
 worldRegion2);
 IsoWorldRegion worldRegion3 =
 this->cell.dataRoot.regionManager.allocIsoWorldRegion();
 ...
 void floodFillExpandWorldRegion(IsoChunkRegion chunkRegion0,
 ...
 for (int int0 = 0; int0 < chunkRegion1.getConnectedNeighbors().size();
 int0++) {
 IsoChunkRegion chunkRegion2 =
 chunkRegion1.getConnectedNeighbors().get(int0);
 ...
 if (byte0 > 0) {
 boolean0 =
 boolean0 || this->squareHasFlags(int4, int3, int5, (byte)32);
 if (boolean0) {
 IsoChunkRegion chunkRegion1 =
 this->getIsoChunkRegion(int4, int3, int5);
 if (chunkRegion1 != nullptr) {
 chunkRegion1.addRoof();
 if (chunkRegion1.getIsoWorldRegion() != nullptr &&
 !chunkRegion1.getIsoWorldRegion().isEnclosed()) {
 boolean0 = false;
 }
 } else {
 boolean0 = false;
