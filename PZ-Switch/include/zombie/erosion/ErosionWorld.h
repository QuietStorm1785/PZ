...
public:
 ...
 void validateSpawn(IsoGridSquare _sq, ErosionData.Square _sqErosionData,
 ErosionData.Chunk _chunkData) {
 bool boolean0 = _sq.Is(IsoFlagType.exterior);
 bool boolean1 = _sq.Has(IsoObjectType.wall);
 IsoObject object = _sq.getFloor();
 std::string string0 = object != nullptr && object.getSprite() != nullptr
 ...
 bool boolean3 = erosionCategory.replaceExistingObject(
 _sq, _sqErosionData, _chunkData, boolean0, boolean1);
 if (!boolean3) {
 boolean3 = erosionCategory.validateSpawn(
 _sq, _sqErosionData, _chunkData, boolean0, boolean1, false);
 }
 ...
 void update(IsoGridSquare _sq, ErosionData.Square _sqErosionData,
 ErosionData.Chunk _chunkData, int _eTick) {
 ...
 ErosionRegions.getCategory(data.regionID, data.categoryID);
