...
public:
 ...
 bool isLoaded = false;
protected
 List<String> modOverrides;
 Item scriptItem;

 std::string getFullType() { return this->fullType; }

 short getRegistryID() { return this->registryID; }

 bool isExistsAsVanilla() { return this->existsAsVanilla; }

 bool isModded() { return this->isModded; }

 std::string getModID() { return this->modID; }

 bool isObsolete() { return this->obsolete; }

 bool isRemoved() { return this->removed; }
...
public
 List<String> getModOverrides() { return this->modOverrides; }
 ...
 bool isValid() { return !this->obsolete && !this->removed && this->isLoaded; }

 void DebugPrint() { DebugLog.log(this->GetDebugString()); }

 std::string GetDebugString() {
 String string =
 "=== Dictionary Item Debug Print ===\nregistryID = " + this->registryID +
 ...
 std::string ToString() {
 return "registryID = " + this->registryID + ",fulltype = \"" +
 this->fullType + "\",modID = \"" + this->modID +
 "\",existsAsVanilla = " + this->existsAsVanilla +
 ",isModded = " + this->isModded + ",obsolete = " + this->obsolete +
 ",removed = " + this->removed + ",modOverrides = " +
 (this->modOverrides != nullptr ? this->modOverrides.size() : 0) + ",";
 }
 ...
 System.lineSeparator());
 fileWriter.write(string0 + "modID = \"" + this->modID + "\"," +
 System.lineSeparator());
 fileWriter.write(string0 + "existsAsVanilla = " + this->existsAsVanilla +
 "," + System.lineSeparator());
 fileWriter.write(string0 + "isModded = " + this->isModded + "," +
 System.lineSeparator());
 fileWriter.write(string0 + "obsolete = " + this->obsolete + "," +
 System.lineSeparator());
 fileWriter.write(string0 + "removed = " + this->removed + "," +
 System.lineSeparator());
 if (this->modOverrides != nullptr) {
 ...
 if (list1.size() > 127) {
 byteBuffer.putShort(
 (short)list1.indexOf(this->modOverrides.get(int1));
 } else {
 ...
 void load(ByteBuffer byteBuffer, int var2, List<String> list1,
 List<String> list0) {
 this->registryID = byteBuffer.getShort();
 this->moduleName = (String)list0.get(
 list0.size() > 127 ? byteBuffer.getShort() : byteBuffer.get());
 ...
 this->modID = (String)list1.get(list1.size() > 127 ? byteBuffer.getShort()
 : byteBuffer.get());
 this->isModded = true;
 } else {
 ...
 this->modOverrides.add((String)list1.get(
 list1.size() > 127 ? byteBuffer.getShort() : byteBuffer.get()));
 } else {
