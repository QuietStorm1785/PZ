...
private
 Map<String, KahluaTable> modData = std::make_unique<HashMap<>>();
 static const int BLOCK_SIZE = 524288;
 static int LAST_BLOCK_SIZE = -1;

 KahluaTable createModDataTable() { return LuaManager.platform.newTable(); }

public
 GlobalModData() { this->reset(); }

 void init() {
 this->reset();
 this->load();
 LuaEventManager.triggerEvent("OnInitGlobalModData",
 WorldDictionary.isIsNewGame());
 }
 ...
 DebugLog.log("GlobalModData -> cannot transmit moddata not found: " +
 string);
 }
 }
 ...
 if (table.empty()) {
 DebugLog.log(
 "GlobalModData -> received request for non-existing table, table: " +
 string);
 }
 ...
 LAST_BLOCK_SIZE == -1 ? 1048576 : LAST_BLOCK_SIZE);
