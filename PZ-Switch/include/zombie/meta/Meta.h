...
public:
 ...
 static std::mutex SquaresSeen_mutex;
 static const Meta instance = new Meta();
 ArrayList<IsoGridSquare> SquaresProcessing =
 std::make_unique<ArrayList<>>();
private
 ArrayList<IsoGridSquare> SquaresSeen = new ArrayList<>(2000);
 const TIntHashSet SquaresSeenSet = new TIntHashSet();

 void dealWithSquareSeen(IsoGridSquare square) {
 if (!GameClient.bClient) {
 if (square.hourLastSeen !=
 (int)GameTime.getInstance().getWorldAgeHours()) {
 { std::lock_guard<std::mutex> __sync_lock__(SquaresSeen_mutex);
