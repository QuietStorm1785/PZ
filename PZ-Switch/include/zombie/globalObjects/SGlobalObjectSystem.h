...
protected
 HashSet<String> modDataKeys = std::make_unique<HashSet<>>();
protected
 HashSet<String> objectModDataKeys = std::make_unique<HashSet<>>();
protected
...
public
 ...
 if (int1 < 134) {
 throw IOException("invalid WorldVersion " + int1 + ": " +
 file.getAbsolutePath());
 }

 if (int1 > 195) {
 throw IOException("file is from a newer version " + int1 +
