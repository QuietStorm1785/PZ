...
public:
public
 CGlobalObjectSystem(std::string_view name) { super(name); }

 GlobalObject makeObject(int int0, int int1, int int2) {
 return new CGlobalObject(this, int0, int1, int2);
 }

 void sendCommand(std::string_view command, IsoPlayer player,
 KahluaTable args) {
 CGlobalObjectNetwork.sendClientCommand(player, this->name, command, args);
 }

 void receiveServerCommand(std::string_view command, KahluaTable args) {
