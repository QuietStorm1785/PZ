...
public
 Stack<StoryEmitter.Sound> SoundStack = std::make_unique<Stack<>>();
public
 ArrayList<StoryEmitter.Sound> Instances = std::make_unique<ArrayList<>>();
public
 ArrayList<StoryEmitter.Sound> ToStart = std::make_unique<ArrayList<>>();
 ...
 sound.y = y;
 sound.z = z;
 sound.volume = SoundManager.instance.getSoundVolume() * baseVolume *
 this->volumeMod;
 sound.sound = long0;
 sound.channel = javafmod.FMOD_System_PlaySound(long0, true);
 this->ToStart.add(sound);
 javafmod.FMOD_Channel_Set3DAttributes(
 sound.channel, sound.x - IsoPlayer.getInstance().x,
 sound.y - IsoPlayer.getInstance().y,
 sound.z - IsoPlayer.getInstance().z, 0.0F, 0.0F, 0.0F);
 ...
 sound1.x, sound1.y, IsoPlayer.getInstance().x,
 IsoPlayer.getInstance().y, sound1.z,
 ...
 javafmod.FMOD_Channel_Set3DAttributes(
 sound1.channel, Math.abs(sound1.x - IsoPlayer.getInstance().x),
 Math.abs(sound1.y - IsoPlayer.getInstance().y),
 Math.abs(sound1.z - IsoPlayer.getInstance().z), 0.0F, 0.0F, 0.0F);
 } else {
 javafmod.FMOD_Channel_Set3DAttributes(
 sound1.channel, Math.abs(sound1.x - IsoPlayer.getInstance().x),
 Math.abs(sound1.z - IsoPlayer.getInstance().z),
 Math.abs(sound1.y - IsoPlayer.getInstance().y), 0.0F, 0.0F, 0.0F);
 }

 javafmod.FMOD_System_SetReverbDefault(
 0, FMODManager.FMOD_PRESET_MOUNTAINS);
 javafmod.FMOD_Channel_SetReverbProperties(sound1.channel, 0, 1.0F);
 javafmod.FMOD_Channel_Set3DMinMaxDistance(
 sound1.channel, sound1.minRange, sound1.maxRange);
 float float1 = 0.0F;
 float float2 = 0.0F;
 IsoGridSquare square = IsoPlayer.getInstance().getCurrentSquare();
 this->soundVect.set(sound1.x, sound1.y);
 this->playerVect.set(IsoPlayer.getInstance().x,
 IsoPlayer.getInstance().y);
 float float3 =
 (float)Math.toDegrees(this->playerVect.angleTo(this->soundVect);
 float float4 = (float)Math.toDegrees(
 IsoPlayer.getInstance().getForwardDirection().getDirectionNeg());
