...
public:
private
 ArrayDeque<IsoZombie> ReusableZombies =
 std::make_unique<ArrayDeque<>>();
private
 HashSet<IsoZombie> ReusableZombieSet = std::make_unique<HashSet<>>();
private
 ArrayList<IsoZombie> ReusedThisFrame = std::make_unique<ArrayList<>>();
private
 ArrayList<IsoZombie> RecentlyRemoved = std::make_unique<ArrayList<>>();
 static VirtualZombieManager instance = new VirtualZombieManager();
 int MaxRealZombies = 1;
private
 ArrayList<IsoZombie> m_tempZombies = std::make_unique<ArrayList<>>();
public
 ArrayList<IsoGridSquare> choices = std::make_unique<ArrayList<>>();
private
 ArrayList<IsoGridSquare> bestchoices = std::make_unique<ArrayList<>>();
 ...
 if (square.empty() || !this->canSpawnAt(square.x, square.y, square.z) ||
 square.HasStairs()) {
 return nullptr;
 } else {
 ...
 (object.empty() || !((IsoGridSquare)object).isFree(false));
