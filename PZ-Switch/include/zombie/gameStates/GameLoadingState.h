...
public:
 ...
 std::mutex assetLock1_mutex;
 std::mutex assetLock2_mutex;
...
private
 boolean bWaitForAssetLoadingToFinish1 = false;
private
 boolean bWaitForAssetLoadingToFinish2 = false;
 ...
 this->text =
 Translator.getText("UI_DirectConnectionPortWarning",
 ServerOptions.getInstance().UDPPort.getValue());
 this->width =
 TextManager.instance.MeasureStringX(UIFont.NewMedium, this->text) + 8;
 }
 ...
 for (Core.GameMode = "Multiplayer"; GameClient.instance.ID == -1;
 ...
 LuaManager.GlobalObject.createWorld("clienttest" +
 GameClient.instance.ID);
 }

 if (Core.GameSaveWorld.empty()) {
 DebugLog.log("No savefile directory was specified. It's a bug.");
 GameWindow.DoLoadingText("No savefile directory was specified. The game "
 "will now close. Sorry!");
 ...
 GameWindow.DoLoadingText("The savefile directory doesn't exist. The "
 "game will now close. Sorry!");
 ...
 try {
 if (!GameClient.bClient && !GameServer.bServer && !Core.bTutorial &&
 !Core.isLastStand() && !"Multiplayer" == Core.GameMode) {
 FileWriter fileWriter =
 new FileWriter(new File(ZomboidFileSystem.instance.getCacheDir() +
 ...
 LosUtil.init(IsoChunkMap.ChunkGridWidth * 10,
 IsoChunkMap.ChunkGridWidth * 10);
 this->Time = 0.0F;
 this->Stage = 0;
 ...
 { std::lock_guard<std::mutex> __sync_lock__(GameLoadingState.this->assetLock1_mutex);
 ...
 bool boolean0 = new File(ZomboidFileSystem.instance.getGameModeCacheDir() +
 File.separator)
 .mkdir();
 ...
 { std::lock_guard<std::mutex> __sync_lock__(GameLoadingState.this->assetLock2_mutex);
...
static void SendDone() {
 DebugLog.log("game loading took " +
 (System.currentTimeMillis() - startTime + 999L) / 1000L +
 " seconds");
 if (!GameClient.bClient) {
 bDone = true;
 GameKeyboard.bNoEventsWhileLoading = false;
 } else {
 GameClient.instance.sendLoginQueueDone2(System.currentTimeMillis() -
 startTime);
 }
}
...
GameState redirectState() { return std::make_unique<IngameState>(); }
...
void exit() {
 ...
 IsoPlayer.getInstance().getCurrentSquare(), GameClient.username, true);
 ...
 std::string string =
 ZomboidFileSystem.instance.getFileNameInCurrentSave("mods.txt");
...
void render() {
 ...
 int int2 =
 TextManager.instance.getFontFromEnum(UIFont.Medium).getLineHeight();
 int int3 = int1 - int2 / 2;
 std::string string0 = Translator.getText("UI_GameLoad_MapDownloadFailed");
 TextManager.instance.DrawStringCentre(UIFont.Medium, int0, int3, string0,
 0.8, 0.1, 0.1, 1.0);
 UIManager.render();
 ...
 UIFont.Medium, int7, int9,
 Translator.getText("UI_GameLoad_UnexpectedError1"), 0.8, 0.1, 0.1, 1.0);
 TextManager.instance.DrawStringCentre(
 UIFont.Small, int7, int9 + int4 + byte0,
 Translator.getText("UI_GameLoad_UnexpectedError2"), 1.0, 1.0, 1.0, 1.0);
 std::string string1 = ZomboidFileSystem.instance.getCacheDir() +
 File.separator + "console.txt";
 TextManager.instance.DrawStringCentre(UIFont.Small, int7,
 int9 + int4 + byte0 + int5 + byte1,
 string1, 1.0, 1.0, 1.0, 1.0);
 UIManager.render();
 Core.getInstance().EndFrameUI();
 ...
 TextManager.instance.DrawStringCentre(UIFont.Medium, int10, int13, string2,
 0.8, 0.1, 0.1, 1.0);
 UIManager.render();
 Core.getInstance().EndFrameUI();
 } else {
 if (build23Stop) {
 TextManager.instance.DrawStringCentre(
 UIFont.Small, Core.getInstance().getScreenWidth() / 2,
 Core.getInstance().getScreenHeight() - 100,
 "This save is incompatible. Please switch to Steam branch "
 "\"build23\" to continue this save.",
 0.8, 0.1, 0.1, 1.0);
 } else if (convertingWorld) {
 TextManager.instance.DrawStringCentre(
 UIFont.Small, Core.getInstance().getScreenWidth() / 2,
 Core.getInstance().getScreenHeight() - 100,
 Translator.getText("UI_ConvertWorld"), 0.5, 0.5, 0.5, 1.0);
 if (convertingFileMax != -1) {
 TextManager.instance.DrawStringCentre(
 UIFont.Small, Core.getInstance().getScreenWidth() / 2,
 Core.getInstance().getScreenHeight() - 100 +
 TextManager.instance.getFontFromEnum(UIFont.Small)
 .getLineHeight() +
 8,
 convertingFileCount + " / " + convertingFileMax, 0.5, 0.5, 0.5,
 1.0);
 }
 }
 ...
 TextManager.instance.DrawStringCentre(UIFont.Medium, int14, int17,
 string3, 0.8, 0.1, 0.1, 1.0);
 UIManager.render();
 Core.getInstance().EndFrameUI();
 } else {
 if (GameClient.bClient) {
 ...
 TextManager.instance.DrawStringCentre(
 UIFont.Small, Core.getInstance().getScreenWidth() / 2,
 Core.getInstance().getScreenHeight() - 60, string4, 0.5, 0.5, 0.5,
 1.0);
 if (GameClient.connection.getConnectionType() ==
 UdpConnection.ConnectionType.Steam) {
 SpriteRenderer.instance.render(
 nullptr,
 (Core.getInstance().getScreenWidth() - this->width) / 2.0F,
 Core.getInstance().getScreenHeight() - 32, this->width, 18.0F,
 1.0F, 0.4F, 0.35F, 0.8F, nullptr);
 TextManager.instance.DrawStringCentre(
 UIFont.Medium, Core.getInstance().getScreenWidth() / 2,
 Core.getInstance().getScreenHeight() - 32, this->text, 0.1, 0.1,
 0.1, 1.0);
 }
 ...
 Translator.getText("UI_Loading").replace(".", ""), 0.5, 0.5, 0.5,
 1.0);
 ...
 2 +
 1,
 Core.getInstance().getScreenHeight() - 60, this->loadingDot, 0.5,
 0.5, 0.5, 1.0);
 }
 ...
 TextManager.instance.getFontFromEnum(UIFont.Intro).getLineHeight() /
 2;
 TextManager.instance.DrawStringCentre(UIFont.Intro, int18, int19,
 Translator.getText("UI_Intro1"),
 1.0, 1.0, 1.0, float0);
 }
 ...
 TextManager.instance.getFontFromEnum(UIFont.Intro).getLineHeight() /
 2;
 TextManager.instance.DrawStringCentre(UIFont.Intro, int20, int21,
 Translator.getText("UI_Intro2"),
 1.0, 1.0, 1.0, float1);
 }
 ...
 TextManager.instance.getFontFromEnum(UIFont.Intro).getLineHeight() /
 2;
 TextManager.instance.DrawStringCentre(UIFont.Intro, int22, int23,
 Translator.getText("UI_Intro3"),
 1.0, 1.0, 1.0, float2);
 }
 ...
 if (bDone && playerCreated &&
 (!newGame || this->Time >= this->TotalTime || Core.isLastStand() ||
 "Tutorial" == Core.GameMode)) {
 if (this->clickToSkipFadeIn) {
 this->clickToSkipAlpha =
 this->clickToSkipAlpha +
 GameTime.getInstance().getMultiplier() / 1.6F / 30.0F;
 ...
 !JoypadManager.instance.JoypadList.empty()) {
 Texture texture =
 Texture.getSharedTexture("media/ui/xbox/XBOX_A.png");
 if (texture != nullptr) {
 int int24 = TextManager.instance.getFontFromEnum(UIFont.Small)
 .getLineHeight();
 SpriteRenderer.instance.renderi(
 texture,
 Core.getInstance().getScreenWidth() / 2 -
 ...
 2 -
 8 - texture.getWidth(),
 Core.getInstance().getScreenHeight() - 60 + int24 / 2 -
 texture.getHeight() / 2,
 texture.getWidth(), texture.getHeight(), 1.0F, 1.0F, 1.0F,
 this->clickToSkipAlpha, nullptr);
 }

 TextManager.instance.DrawStringCentre(
 UIFont.Small, Core.getInstance().getScreenWidth() / 2,
 Core.getInstance().getScreenHeight() - 60,
 Translator.getText("UI_PressAToStart"), 1.0, 1.0, 1.0,
 this->clickToSkipAlpha);
 } else {
 TextManager.instance.DrawStringCentre(
 UIFont.NewLarge, Core.getInstance().getScreenWidth() / 2,
 Core.getInstance().getScreenHeight() - 60,
 Translator.getText("UI_ClickToSkip"), 1.0, 1.0, 1.0,
 this->clickToSkipAlpha);
 }
 }
...
GameStateMachine.StateAction update() {
 if (this->bWaitForAssetLoadingToFinish1 &&
 !OutfitManager.instance.isLoadingClothingItems()) {
 if (Core.bDebug) {
 OutfitManager.instance.debugOutfits();
 }

 { std::lock_guard<std::mutex> __sync_lock__(assetLock1_mutex);
 this->bWaitForAssetLoadingToFinish1 = false;
 this->assetLock1.notifyAll();
 }
 }

 if (this->bWaitForAssetLoadingToFinish2 &&
 !ModelManager.instance.isLoadingAnimations() &&
 !GameWindow.fileSystem.hasWork()) {
 { std::lock_guard<std::mutex> __sync_lock__(assetLock2_mutex);
 this->bWaitForAssetLoadingToFinish2 = false;
 this->assetLock2.notifyAll();

 for (RuntimeAnimationScript runtimeAnimationScript :
 ScriptManager.instance.getAllRuntimeAnimationScripts()) {
