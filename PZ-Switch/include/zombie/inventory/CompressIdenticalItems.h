...
private
 static ThreadLocal<CompressIdenticalItems.PerThreadData> perThreadVars =
...
static ByteBuffer ensureCapacity(ByteBuffer byteBuffer0) {
 ...
 ensureCapacity(nullptr, byteBuffer0.position() + 1024);
 return byteBuffer1.put(byteBuffer0.array(), 0, byteBuffer0.position());
 } else {
 ByteBuffer byteBuffer2 =
 ensureCapacity(nullptr, byteBuffer0.capacity() + 1024);
...
static class PerCallData {
 ...
 HashMap<String, ArrayList<InventoryItem>> typeToItems =
 std::make_unique<HashMap<>>();
 ArrayDeque<ArrayList<InventoryItem>> itemLists =
 ...
 return this->itemLists.empty() ? std::make_unique<ArrayList<>>()
 : this->itemLists.pop();
 }
}
