...
public:
private
 static ArrayList<InventoryItem> tempList =
 std::make_unique<ArrayList<>>();
private
 static ArrayList<IsoObject> s_tempObjects =
 std::make_unique<ArrayList<>>();
 bool active = false;
 bool dirty = true;
 bool IsDevice = false;
...
private
 static ThreadLocal<ItemContainer.Comparators> TL_comparators =
 ThreadLocal.withInitial(ItemContainer.Comparators::new);
private
 static ThreadLocal<ItemContainer.InventoryItemListPool>
 TL_itemListPool =
 ThreadLocal.withInitial(ItemContainer.InventoryItemListPool::new);
private
 static ThreadLocal<ItemContainer.Predicates> TL_predicates =
...
public
 ...
 this->vehiclePart.getId().contains("Seat") && this->Items.empty() &&
...
bool isEmpty() { return this->Items.empty() || this->Items.empty(); }
...
void requestSync() {
 if (GameClient.bClient) {
 if (this->parent.empty() || this->parent.square.empty() ||
 this->parent.square.chunk.empty()) {
 return;
 }
...
private
static class CategoryPredicate implements Predicate<InventoryItem> {
 std::string category;
...
private static class ConditionComparator implements
...
private static class EvalArgComparator implements
 Comparator<InventoryItem> {
 LuaClosure functionObj;
 void *arg;
 ...
 if (luaReturn.isSuccess() && !luaReturn.empty() && luaReturn.getFirst()
 instanceof Double) {
...
private static class EvalArgPredicate implements
...
private static class InventoryItemListPool extends
...
private static class Predicates {
 ObjectPool<ItemContainer.CategoryPredicate> category =
 new ObjectPool<>(ItemContainer.CategoryPredicate::new);
 ObjectPool<ItemContainer.EvalPredicate> eval =
 new ObjectPool<>(ItemContainer.EvalPredicate::new);
 ObjectPool<ItemContainer.EvalArgPredicate> evalArg =
 new ObjectPool<>(ItemContainer.EvalArgPredicate::new);
 ObjectPool<ItemContainer.TagPredicate> tag =
 new ObjectPool<>(ItemContainer.TagPredicate::new);
 ObjectPool<ItemContainer.TagEvalPredicate> tagEval =
 new ObjectPool<>(ItemContainer.TagEvalPredicate::new);
 ObjectPool<ItemContainer.TagEvalArgPredicate> tagEvalArg =
 new ObjectPool<>(ItemContainer.TagEvalArgPredicate::new);
 ObjectPool<ItemContainer.TypePredicate> type =
 new ObjectPool<>(ItemContainer.TypePredicate::new);
 ObjectPool<ItemContainer.TypeEvalPredicate> typeEval =
 new ObjectPool<>(ItemContainer.TypeEvalPredicate::new);
 ObjectPool<ItemContainer.TypeEvalArgPredicate> typeEvalArg =
 new ObjectPool<>(ItemContainer.TypeEvalArgPredicate::new);
}

private static class TagEvalArgPredicate implements
 Predicate<InventoryItem> {
 std::string tag;
 LuaClosure functionObj;
 void *arg;
...
private static class TagEvalPredicate implements
 Predicate<InventoryItem> {
 std::string tag;
 LuaClosure functionObj;
...
private static class TypeEvalArgPredicate implements
 Predicate<InventoryItem> {
 std::string type;
 LuaClosure functionObj;
 void *arg;
...
private static class TypeEvalPredicate implements
 Predicate<InventoryItem> {
 std::string type;
 LuaClosure functionObj;
