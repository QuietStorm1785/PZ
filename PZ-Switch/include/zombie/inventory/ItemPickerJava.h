...
public
 static ArrayList<String> NoContainerFillRooms =
 std::make_unique<ArrayList<>>();
public
 static ArrayList<ItemPickerJava.ItemPickerUpgradeWeapons>
 WeaponUpgrades = std::make_unique<ArrayList<>>();
public
 static HashMap<String, ItemPickerJava.ItemPickerUpgradeWeapons>
 WeaponUpgradeMap = std::make_unique<HashMap<>>();
public
 static THashMap<String, ItemPickerJava.ItemPickerRoom> rooms =
 std::make_unique<THashMap<>>();
public
 static THashMap<String, ItemPickerJava.ItemPickerContainer> containers =
 std::make_unique<THashMap<>>();
public
 static THashMap<String, ItemPickerJava.ItemPickerContainer>
 ProceduralDistributions = std::make_unique<THashMap<>>();
public
 static THashMap<String, ItemPickerJava.VehicleDistribution>
 VehicleDistributions = std::make_unique<THashMap<>>();
 ...
 (KahluaTableImpl)LuaManager.env.rawget("NoContainerFillRooms");
 ...
 (KahluaTableImpl)LuaManager.env.rawget("WeaponUpgrades");

 for (Entry entry1 : kahluaTableImpl1.delegate.entrySet()) {
 std::string string1 = entry1.getKey().toString();
 ItemPickerJava.ItemPickerUpgradeWeapons itemPickerUpgradeWeapons =
 new ItemPickerJava.ItemPickerUpgradeWeapons();
 ...
 !string1.empty() &&
 kahluaTableImpl2.delegate.containsKey("rolls") &&
 kahluaTableImpl2.delegate.containsKey("items")) {
 ItemPickerJava.ItemPickerContainer itemPickerContainer1 =
 ExtractContainersFromLua(kahluaTableImpl2);
 itemPickerRoom.Containers.put(string1, itemPickerContainer1);
 } else {
 DebugLog.log("ERROR: SuburbsDistributions[\"" + string0 +
 "\"] is broken");
 }
 }
 }
 ...
 KahluaTableImpl kahluaTableImpl0 =
 (KahluaTableImpl)LuaManager.env.rawget("VehicleDistributions");
 if (kahluaTableImpl0 != nullptr && kahluaTableImpl0.rawget(1) instanceof
 KahluaTableImpl) {
 kahluaTableImpl0 = (KahluaTableImpl)kahluaTableImpl0.rawget(1);

 for (Entry entry0 : kahluaTableImpl0.delegate.entrySet()) {
 if (entry0.getKey() instanceof String && entry0.getValue() instanceof
 KahluaTableImpl) {
 KahluaTableImpl kahluaTableImpl1 = (KahluaTableImpl)entry0.getValue();
 ItemPickerJava.VehicleDistribution vehicleDistribution =
 new ItemPickerJava.VehicleDistribution();
 if (kahluaTableImpl1.rawget("Normal") instanceof KahluaTableImpl) {
 KahluaTableImpl kahluaTableImpl2 =
 (KahluaTableImpl)kahluaTableImpl1.rawget("Normal");
 ...
 itemPickerRoom0.Containers.put(
 string0,
 ExtractContainersFromLua((KahluaTableImpl)entry1.getValue()));
 }
 ...
 (KahluaTableImpl)kahluaTableImpl1.rawget("Specific");

 for (int int0 = 1; int0 <= kahluaTableImpl3.len(); int0++) {
 KahluaTableImpl kahluaTableImpl4 =
 (KahluaTableImpl)kahluaTableImpl3.rawget(int0);
 ...
 itemPickerRoom1.Containers.put(
 string1, ExtractContainersFromLua(
 (KahluaTableImpl)entry2.getValue()));
 }
 }
 ...
 VehicleDistributions.put((String)entry0.getKey(),
 vehicleDistribution);
 }
 }
 }
 ...
 KahluaTableImpl kahluaTableImpl0 =
 Type.tryCastTo(LuaManager.env.rawget("ProceduralDistributions"),
 KahluaTableImpl.class);
 if (kahluaTableImpl0 != nullptr) {
 KahluaTableImpl kahluaTableImpl1 = Type.tryCastTo(
 kahluaTableImpl0.rawget("list"), KahluaTableImpl.class);
...
private
 ...
 ExtractContainersFromLua(KahluaTableImpl kahluaTableImpl0) {
 ItemPickerJava.ItemPickerContainer itemPickerContainer =
 new ItemPickerJava.ItemPickerContainer();
 if (kahluaTableImpl0.delegate.containsKey("procedural")) {
 itemPickerContainer.procedural =
 kahluaTableImpl0.rawgetBool("procedural");
 ...
 itemPickerContainer.noAutoAge =
 kahluaTableImpl0.rawgetBool("noAutoAge");
 }
 ...
 itemPickerContainer.stashChance =
 kahluaTableImpl0.rawgetInt("stashChance");
 }

 if (kahluaTableImpl0.delegate.containsKey("dontSpawnAmmo")) {
 itemPickerContainer.dontSpawnAmmo =
 kahluaTableImpl0.rawgetBool("dontSpawnAmmo");
 }

 if (kahluaTableImpl0.delegate.containsKey("ignoreZombieDensity")) {
 itemPickerContainer.ignoreZombieDensity =
 kahluaTableImpl0.rawgetBool("ignoreZombieDensity");
 }

 double double0 = (Double)kahluaTableImpl0.delegate.get("rolls");
 if (kahluaTableImpl0.delegate.containsKey("junk")) {
 itemPickerContainer.junk = ExtractContainersFromLua(
 (KahluaTableImpl)kahluaTableImpl0.rawget("junk"));
 }

 itemPickerContainer.rolls = (int)double0;
 KahluaTableImpl kahluaTableImpl1 =
 (KahluaTableImpl)kahluaTableImpl0.delegate.get("items");
 std::vector arrayList = new ArrayList();
 int int0 = kahluaTableImpl1.len();

 for (byte byte0 = 0; byte0 < int0; byte0 += 2) {
 std::string string =
 Type.tryCastTo(kahluaTableImpl1.delegate.get(
 KahluaUtil.toDouble((long)(byte0 + 1)),
 String.class);
 double double1 =
 Type.tryCastTo(kahluaTableImpl1.delegate.get(
 KahluaUtil.toDouble((long)(byte0 + 2)),
 Double.class);
 if (string != nullptr && double1 != nullptr) {
 Item item = ScriptManager.instance.FindItem(string);
 if (item != nullptr && !item.OBSOLETE) {
 ItemPickerJava.ItemPickerItem itemPickerItem =
 new ItemPickerJava.ItemPickerItem();
 itemPickerItem.itemName = string;
 itemPickerItem.chance = double1.floatValue();
 arrayList.add(itemPickerItem);
 } else if (Core.bDebug) {
 DebugLog.General.warn(
 "ignoring invalid ItemPicker item type \"%s\"", string);
 }
 }
 }
...
private
 static ArrayList<ItemPickerJava.ProceduralItem>
 ExtractProcList(KahluaTableImpl kahluaTableImpl1) {
 std::vector arrayList = new ArrayList();
 KahluaTableImpl kahluaTableImpl0 =
 (KahluaTableImpl)kahluaTableImpl1.rawget("procList");
 ...
 new ItemPickerJava.ProceduralItem();
 ...
 OtherLootModifier =
 doSandboxSettings(SandboxOptions.getInstance().OtherLoot.getValue());
 FoodLootModifier =
 doSandboxSettings(SandboxOptions.getInstance().FoodLoot.getValue());
 WeaponLootModifier =
 doSandboxSettings(SandboxOptions.getInstance().WeaponLoot.getValue());
 RangedWeaponLootModifier = doSandboxSettings(
 SandboxOptions.getInstance().RangedWeaponLoot.getValue());
 AmmoLootModifier =
 doSandboxSettings(SandboxOptions.getInstance().AmmoLoot.getValue());
 CannedFoodLootModifier = doSandboxSettings(
 SandboxOptions.getInstance().CannedFoodLoot.getValue());
 LiteratureLootModifier = doSandboxSettings(
 SandboxOptions.getInstance().LiteratureLoot.getValue());
 SurvivalGearsLootModifier = doSandboxSettings(
 SandboxOptions.getInstance().SurvivalGearsLoot.getValue());
 MedicalLootModifier =
 doSandboxSettings(SandboxOptions.getInstance().MedicalLoot.getValue());
 MechanicsLootModifier = doSandboxSettings(
 SandboxOptions.getInstance().MechanicsLoot.getValue());
 }
 ...
 ItemPickerJava.ItemPickerContainer itemPickerContainer0 =
 nullptr;
 if (itemPickerRoom1.Containers.containsKey(
 container.getType())) {
 itemPickerContainer0 =
 itemPickerRoom1.Containers.get(container.getType());
 }

 if (itemPickerContainer0.empty() &&
 itemPickerRoom1.Containers.containsKey("other")) {
 itemPickerContainer0 =
 itemPickerRoom1.Containers.get("other");
 }

 if (itemPickerContainer0.empty() &&
 itemPickerRoom1.Containers.containsKey("all")) {
 itemPickerContainer0 =
 itemPickerRoom1.Containers.get("all");
 string0 = "all";
 }

 if (itemPickerContainer0.empty()) {
 fillContainerType(itemPickerRoom0, container, string0,
 _player);
 LuaEventManager.triggerEvent("OnFillContainer", string0,
 container.getType(),
 container);
 } else {
 ...
 fillContainerType(itemPickerRoom0, container,
 room.getName(), _player);
 LuaEventManager.triggerEvent(
 "OnFillContainer", room.getName(),
 ...
 fillContainerType(itemPickerRoom0, container, (String)object,
 _player);
 LuaEventManager.triggerEvent("OnFillContainer", object,
 container.getType(), container);
 }
 }
 else {
 std::string string1 = container.getType();
 if (container.getParent() != nullptr && container.getParent()
 instanceof IsoDeadBody) {
 string1 = ((IsoDeadBody)container.getParent()).getOutfitName();
 }

 for (int int0 = 0; int0 < container.getItems().size(); int0++) {
 if (container.getItems().get(int0) instanceof
 InventoryContainer) {
 ItemPickerJava.ItemPickerContainer itemPickerContainer1 =
 containers.get(container.getItems().get(int0).getType());
 if (itemPickerContainer1 != nullptr &&
 Rand.Next(itemPickerContainer1.fillRand) == 0) {
 rollContainerItem(
 (InventoryContainer)container.getItems().get(int0),
 nullptr,
 containers.get(
 container.getItems().get(int0).getType()));
 }
 }
 }

 ItemPickerJava.ItemPickerContainer itemPickerContainer2 =
 rooms.get("all").Containers.get("Outfit_" + string1);
 if (itemPickerContainer2.empty()) {
 itemPickerContainer2 =
 rooms.get("all").Containers.get(container.getType());
 }
 ...
 rollItem((ItemPickerJava.ItemPickerContainer)object, container, boolean0,
 character, roomDist);
 }
 ...
 container.getContainingItem().getContainer();
 if (_container != nullptr &&
 !_container.hasRoomFor(nullptr, float0) {
 return nullptr;
 }
 }
...
private
 static void
 rollProceduralItem(ArrayList<ItemPickerJava.ProceduralItem> arrayList0,
 ItemContainer container, float float0,
 IsoGameCharacter character,
 ItemPickerJava.ItemPickerRoom itemPickerRoom) {
 if (container.getSourceGrid() != nullptr &&
 container.getSourceGrid().getRoom() != nullptr) {
 std::unordered_map hashMap0 = container.getSourceGrid()
 .getRoom()
 .getRoomDef()
 ...
 ItemPickerJava.ProceduralItem proceduralItem =
 (ItemPickerJava.ProceduralItem)arrayList0.get(int0);
 ...
 if (list0 != nullptr) {
 for (int int4 = container.getSourceGrid().getRoom().getRoomDef().x;
 int4 < container.getSourceGrid().getRoom().getRoomDef().x2;
 int4++) {
 for (int int5 = container.getSourceGrid().getRoom().getRoomDef().y;
 int5 < container.getSourceGrid().getRoom().getRoomDef().y2;
 int5++) {
 IsoGridSquare square0 =
 container.getSourceGrid().getCell().getGridSquare(
 int4, int5, container.getSourceGrid().z);
 if (square0 != nullptr) {
 ...
 list2.contains(object1.getSprite().getName())) {
 hashMap1.clear();
 hashMap1.put(string0, -1);
 break;
 }
 ...
 if (square2.getBuilding().getRandomRoom(
 (String)list3.get(int8) != nullptr) {
 hashMap1.clear();
 hashMap1.put(string0, -1);
 break;
 }
 ...
 std::vector arrayList1 = IsoWorld.instance.MetaGrid.getZonesAt(
 container.getSourceGrid().x, container.getSourceGrid().y, 0);

 for (int int9 = 0; int9 < arrayList1.size(); int9++) {
 if ((Integer)hashMap0.get(string0) < int2 &&
 (list1.contains(
 ((IsoMetaGrid.Zone)arrayList1.get(int9).type) ||
 list1.contains(
 ((IsoMetaGrid.Zone)arrayList1.get(int9).name)) {
 hashMap1.clear();
 hashMap1.put(string0, -1);
 break;
 }
 ...
 if (list0.empty() && list1.empty() && list2.empty() &&
 list3.empty()) {
 ...
 if (string1 != nullptr) {
 ItemPickerJava.ItemPickerContainer itemPickerContainer =
 ProceduralDistributions.get(string1);
 if (itemPickerContainer != nullptr) {
 if (itemPickerContainer.junk != nullptr) {
 doRollItem(itemPickerContainer.junk, container, float0, character,
 true, true, itemPickerRoom);
 }

 doRollItem(itemPickerContainer, container, float0, character, true,
 false, itemPickerRoom);
...
public
 static void rollItem(ItemPickerJava.ItemPickerContainer containerDist,
 ItemContainer container, boolean doItemContainer,
 IsoGameCharacter character,
 ...
 metaChunk = IsoWorld.instance.getMetaChunk((int)player.getX() / 10,
 (int)player.getY() / 10);
 } else if (container.getSourceGrid() != nullptr) {
 metaChunk = IsoWorld.instance.getMetaChunk(
 container.getSourceGrid().getX() / 10,
 container.getSourceGrid().getY() / 10);
 }
 ...
 rollProceduralItem(containerDist.proceduralItems, container, float0,
 character, roomDist);
 } else {
 if (containerDist.junk != nullptr) {
 doRollItem(containerDist.junk, container, float0, character,
 doItemContainer, true, roomDist);
 }

 doRollItem(containerDist, container, float0, character, doItemContainer,
 false, roomDist);
 }
 }
 }
...
public
 static void doRollItem(ItemPickerJava.ItemPickerContainer containerDist,
 ItemContainer container, float zombieDensity,
 IsoGameCharacter character, boolean doItemContainer,
 boolean isJunk,
 ItemPickerJava.ItemPickerRoom roomDist) {
 bool boolean0 = false;
 bool boolean1 = false;
 ...
 if (Rand.Next(10000) <=
 float0 * 100.0F * float1 + zombieDensity * 10.0F) {
 InventoryItem item0 =
 tryAddItemToContainer(container, string0, containerDist);
 if (item0.empty()) {
 return;
 }
 ...
 int int2 = container.getSourceGrid()
 .getBuilding()
 .getDef()
 .getKeySpawned();
 if (int2 < 2) {
 container.getSourceGrid().getBuilding().getDef().setKeySpawned(
 int2 + 1);
 } else {
 ...
 ZomboidRadio.getInstance().getRecordedMedia();
 ...
 if (!boolean2 && Rand.Next(100) < 40 && item0 instanceof
 DrainableComboItem) {
 float float2 = 1.0F / ((DrainableComboItem)item0).getUseDelta();
 ((DrainableComboItem)item0)
 .setUsedDelta(Rand.Next(1.0F, float2 - 1.0F) *
 ((DrainableComboItem)item0).getUseDelta());
 }
 ...
 if (item0 instanceof HandWeapon && !containerDist.dontSpawnAmmo &&
 Rand.Next(100) < 90) {
 int int3 = 30;
 HandWeapon weapon = (HandWeapon)item0;
 if (Core.getInstance().getOptionReloadDifficulty() > 1 &&
 !StringUtils.isNullOrEmpty(weapon.getMagazineType()) &&
 Rand.Next(100) < 90) {
 if (Rand.NextBool(3) {
 InventoryItem item1 =
 container.AddItem(weapon.getMagazineType());
 if (Rand.NextBool(5) {
 ...
 if (!StringUtils.isNullOrEmpty(weapon.getAmmoBox()) &&
 Rand.Next(100) < int3) {
 container.AddItem(weapon.getAmmoBox());
 } else if (!StringUtils.isNullOrEmpty(weapon.getAmmoType()) &&
 Rand.Next(100) < 50) {
 ...
 if (item0 instanceof
 InventoryContainer && containers.containsKey(item0.getType())) {
 ItemPickerJava.ItemPickerContainer itemPickerContainer =
 containers.get(item0.getType());
 if (doItemContainer &&
 Rand.Next(itemPickerContainer.fillRand) == 0) {
 rollContainerItem((InventoryContainer)item0, character,
 containers.get(item0.getType()));
 }
 }
 }
 ...
 ItemPickerJava.ItemPickerContainer itemPickerContainer) {
 if (itemPickerContainer.stashChance > 0 && item instanceof
 MapItem && !StringUtils.isNullOrEmpty(((MapItem)item).getMapID())) {
 ...
 static void
 rollContainerItem(InventoryContainer bag, IsoGameCharacter character,
 ItemPickerJava.ItemPickerContainer containerDist) {
 ...
 InventoryItem item =
 tryAddItemToContainer(container, string, containerDist);
 if (item.empty()) {
 return;
 }
 ...
 if (mapItem0 != nullptr &&
 !StringUtils.isNullOrEmpty(mapItem0.getMapID()) &&
 containerDist.maxMap > 0) {
 int int2 = 0;

 for (int int3 = 0; int3 < container.getItems().size(); int3++) {
 MapItem mapItem1 = Type.tryCastTo(
 container.getItems().get(int3), MapItem.class);
 if (mapItem1 != nullptr &&
 !StringUtils.isNullOrEmpty(mapItem1.getMapID())) {
 int2++;
 }
 }
 ...
 container.getSourceGrid().getBuilding() != nullptr &&
 container.getSourceGrid().getBuilding().getDef() != nullptr) {
 int int4 = container.getSourceGrid()
 .getBuilding()
 .getDef()
 .getKeySpawned();
 if (int4 < 2) {
 container.getSourceGrid()
 .getBuilding()
 .getDef()
 .setKeySpawned(int4 + 1);
 } else {
 container.Remove(item);
 }
 }
 ...
 ItemPickerJava.ItemPickerUpgradeWeapons itemPickerUpgradeWeapons =
 WeaponUpgradeMap.get(item0.getType());
 ...
 std::string string =
 PZArrayUtil.pickRandom(itemPickerUpgradeWeapons.Upgrades);
 ...
 item.getType() == Item.Type.Normal &&
 !StringUtils.isNullOrEmpty(item.getAmmoType())) {
 ...
 !object.getContainer().isExplored()) {
...
public
 static ItemPickerJava.ItemPickerContainer
 getItemContainer(String room, String container, String proceduralName,
 boolean junk) {
 ...
 ItemPickerJava.ItemPickerContainer itemPickerContainer0 =
 itemPickerRoom.Containers.get(container);
 ...
 ItemPickerJava.ProceduralItem proceduralItem =
 (ItemPickerJava.ProceduralItem)arrayList.get(int0);
 if (proceduralName == proceduralItem.name) {
 ItemPickerJava.ItemPickerContainer itemPickerContainer1 =
 ProceduralDistributions.get(proceduralName);
...
public
 static class ItemPickerContainer {
 public
 ItemPickerJava.ItemPickerItem[] Items =
 new ItemPickerJava.ItemPickerItem[0];
 float rolls;
 bool noAutoAge;
 int fillRand;
 int maxMap;
 int stashChance;
 public
 ItemPickerJava.ItemPickerContainer junk;
 bool procedural;
 ...
 public
 THashMap<String, ItemPickerJava.ItemPickerContainer> Containers =
 std::make_unique<THashMap<>>();
 int fillRand;
 bool isShop;
 ...
 std::string name;
 public
 ArrayList<String> Upgrades = std::make_unique<ArrayList<>>();
 }
 ...
 int max;
 public
 List<String> forceForItems;
 public
 List<String> forceForZones;
 public
 List<String> forceForTiles;
 public
 List<String> forceForRooms;
 int weightChance;
 }
 ...
 public
 ArrayList<ItemPickerJava.ItemPickerRoom> Specific =
 std::make_unique<ArrayList<>>();
 }
}
} // namespace inventory
