...
private
 static PolygonalMap2.VehiclePoly tempPoly =
...
private
 static float[] wheelParams = new float[24];
private
 static float[] physicsParams = new float[27];
...
private
 static float[] lowRiderParam = new float[4];
private
 BaseVehicle.VehicleImpulse impulseFromServer =
 new BaseVehicle.VehicleImpulse();
private
 BaseVehicle.VehicleImpulse[] impulseFromSquishedZombie =
 new BaseVehicle.VehicleImpulse[4];
private
 ArrayList<BaseVehicle.VehicleImpulse> impulseFromHitZombie =
 std::make_unique<ArrayList<>>();
 const int netPlayerTimeoutMax = 30;
 const Vector4f tempVector4f = new Vector4f();
public
 ArrayList<BaseVehicle.ModelInfo> models =
 std::make_unique<ArrayList<>>();
 IsoChunk chunk;
 bool polyDirty = true;
...
public
 ...
 float collideY = -1.0F;
public
 PolygonalMap2.VehiclePoly shadowCoord = new PolygonalMap2.VehiclePoly();
public
...
public
 BaseVehicle.WheelInfo[] wheelInfo = new BaseVehicle.WheelInfo[4];
 bool skidding = false;
 long skidSound;
 long ramSound;
 long ramSoundTime;
...
public
 long[] new_EngineSoundId = new long[8];
...
protected
 ArrayList<VehiclePart> parts = std::make_unique<ArrayList<>>();
 VehiclePart battery;
 int engineQuality;
 int engineLoudness;
 int enginePower;
...
protected
 ArrayList<VehiclePart> lights = std::make_unique<ArrayList<>>();
...
private
 HashMap<String, String> choosenParts = std::make_unique<HashMap<>>();
 std::string type = "";
...
private
 ArrayList<IsoObject> brekingObjectsList =
 std::make_unique<ArrayList<>>();
...
private
 static HashMap<String, Integer> s_PartToMaskMap =
 std::make_unique<HashMap<>>();
 static const uint8_t BYTE_ZERO = (byte)0;
private
 HashMap<String, Byte> bloodIntensity = std::make_unique<HashMap<>>();
...
public
 static ThreadLocal<BaseVehicle.Vector2ObjectPool> TL_vector2_pool =
 ThreadLocal.withInitial(BaseVehicle.Vector2ObjectPool::new);
public
 static ThreadLocal<BaseVehicle.Vector2fObjectPool> TL_vector2f_pool =
 ThreadLocal.withInitial(BaseVehicle.Vector2fObjectPool::new);
public
 static ThreadLocal<BaseVehicle.Vector3fObjectPool> TL_vector3f_pool =
 ThreadLocal.withInitial(BaseVehicle.Vector3fObjectPool::new);
public
 static ThreadLocal<BaseVehicle.Matrix4fObjectPool> TL_matrix4f_pool =
 ThreadLocal.withInitial(BaseVehicle.Matrix4fObjectPool::new);
public
 static ThreadLocal<BaseVehicle.QuaternionfObjectPool>
 TL_quaternionf_pool =
...
protected
 BaseVehicle.HitVars hitVars = new BaseVehicle.HitVars();
...
public
 ...
 _part == nullptr ? nullptr : _part.getWindow();
 ...
 if (part.bCreated && arrayList != nullptr && !arrayList.empty() &&
 part.getInventoryItem() == nullptr &&
 part.getTable("install") == nullptr) {
 part.bCreated = false;
 } else if ((arrayList.empty() || arrayList.empty()) &&
 part.getInventoryItem() != nullptr) {
 part.item = nullptr;
 }
 ...
 bool boolean1 = this->loaded && this->parts.empty() ||
 ...
 part0.container.contentType.empty()) {
...
public
 ...
 BaseVehicle.Passenger passenger = this->getPassenger(seat);
 return passenger = = nullptr ? nullptr
...
private
 ...
 !part.getItemContainer().getItems().empty()
 ? true
 ...
 int int0 =
 vehicleType == nullptr ? 70 : vehicleType.getChanceToSpawnKey();
 ...
 if (vector1.empty() && object.sprite != nullptr &&
 ...
 part0.getCondition() < 50 && !part0.container.Items.empty()) {
 ...
 vehicleImpulsex->vehicleImpulsex != nullptr && vehicleImpulsex.enable);
 if (boolean0) {
 Vector3f vector3f0 =
 TL_vector3f_pool.get().alloc().set(0.0F, 0.0F, 0.0F);
 Vector3f vector3f1 =
...
public
 ...
 : nullptr;
 float float7 =
 vehicleImpulse != nullptr && vehicleImpulse.enable ? 0.05F : 0.0F;
 ...
 */
 ...
 if (this->removedFromWorld || this->current.empty() ||
 ...
 bool boolean0 = IsoCamera.CamCharacter != nullptr &&
...
private
 ...
 std::string string = (player == nullptr ? "@server" : player.getUsername()) +
 " ( " + this->netPlayerId + " )";
 float float8;
 ...
 */
 ...
 !vehicleDistribution.Specific.empty()) {
 itemPickerRoom0 =
 ...
 */
 ...
 Thumpable getThumpableFor(IsoGameCharacter chr) { return nullptr; }
...
public
 ...
 private static HashMap<Byte, BaseVehicle.Authorization>
...
public static class QuaternionfObjectPool extends
...
private static class VehicleImpulse {
 static ArrayDeque<BaseVehicle.VehicleImpulse> pool =
