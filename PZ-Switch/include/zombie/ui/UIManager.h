...
public
 ...
 static Clock clock;
public
 static ArrayList<UIElement> UI = std::make_unique<ArrayList<>>();
...
public
 static MoodlesUI[] MoodleUI = new MoodlesUI[4];
 static bool bFadeBeforeUI = false;
public
 static ActionProgressBar[] ProgressBar = new ActionProgressBar[4];
...
public
 static ArrayList<String> DoneTutorials =
...
private
 static ArrayList<UIElement> tutorialStack =
 std::make_unique<ArrayList<>>();
public
 static ArrayList<UIElement> toTop = std::make_unique<ArrayList<>>();
...
private
 static UIManager.FadeInfo[] playerFadeInfo = new UIManager.FadeInfo[4];
 ...
 if (useUIFBO &&
 (UIFBO.empty() || UIFBO.getTexture().getWidth() != width ||
