...
private
 static Stack<ObjectTooltip.Layout> freeLayouts = std::make_unique<Stack<>>();
...
public
 ...
 lineSpacing =
 TextManager.instance.getFontFromEnum(font).getLineHeight();
 }
 }
 ...
 void DrawTextureScaled(Texture tex, double x, double y, double width,
 double height, double _alpha) {
 if (!this->measureOnly) {
 super.DrawTextureScaled(tex, x, y, width, height, _alpha);
 }
 }

 void DrawTextureScaledAspect(Texture tex, double x, double y, double width,
 ...
 void DrawProgressBar(int x, int y, int w, int h, float f, double r, double g,
 double b, double a) {
 if (!this->measureOnly) {
 if (f < 0.0F) {
 f = 0.0F;
 ...
 this->DrawTextureScaledColor(nullptr, (double)x, (double)y, (double)int0,
 3.0, r, g, b, a);
 this->DrawTextureScaledColor(nullptr, (double)x + int0, (double)y,
 (double)w - int0, 3.0, 0.25, 0.25, 0.25, 1.0);
 }
 }
 ...
 if (this->isVisible()) {
 if (!(this->alpha <= 0.0F) {
 if (!this->bIsItem && this->Object != nullptr &&
 this->Object.haveSpecialTooltip()) {
...
public
 ObjectTooltip.Layout beginLayout() {
 ...
 void setCharacter(IsoGameCharacter chr) { this->character = chr; }

 IsoGameCharacter getCharacter() { return this->character; }

 void setMeasureOnly(bool b) { this->measureOnly = b; }

 bool isMeasureOnly() { return this->measureOnly; }

 float getWeightOfStack() { return this->weightOfStack; }

 void setWeightOfStack(float weight) { this->weightOfStack = weight; }

public
 static class Layout {
 public
 ArrayList<ObjectTooltip.LayoutItem> items = std::make_unique<ArrayList<>>();
 int minLabelWidth;
 int minValueWidth;
 public
 ObjectTooltip.Layout next;
 int nextPadY;
 private
 static Stack<ObjectTooltip.LayoutItem> freeItems =
 std::make_unique<Stack<>>();

 public
 ObjectTooltip.LayoutItem addItem() {
 ...
 void setMinLabelWidth(int minWidth) { this->minLabelWidth = minWidth; }

 void setMinValueWidth(int minWidth) { this->minValueWidth = minWidth; }
 ...
 int5 = Math.max(int5,
 Math.max(layoutItem0.labelWidth, this->minLabelWidth) +
 byte0);
 int4 = Math.max(int4,
 int0 + byte0 + Math.max(Math.max(int1, int2), int3);
 } else {
 int0 = Math.max(int0, layoutItem0.labelWidth);
 ...
 ? this->next.render(left, top + this->next.nextPadY, ui)
 : top;
 }
 ...
 this->labelWidth =
 TextManager.instance.MeasureStringX(ObjectTooltip.font, this->label);
 }

 if (this->hasValue) {
 if (this->value != nullptr) {
 int int0 = TextManager.instance.MeasureStringX(ObjectTooltip.font,
 this->value);
 ...
 if (this->label != nullptr) {
 ui.DrawText(ObjectTooltip.font, this->label, x, y, this->r0, this->g0,
 this->b0, this->a0);
 }

 if (this->value != nullptr) {
 if (this->rightJustify) {
 ui.DrawTextRight(ObjectTooltip.font, this->value, x + mid + right, y,
 this->r1, this->g1, this->b1, this->a1);
 } else {
 ui.DrawText(ObjectTooltip.font, this->value, x + mid, y, this->r1,
 this->g1, this->b1, this->a1);
 }
 }

 if (this->progressFraction != -1.0F) {
 ui.DrawProgressBar(x + mid, y + ObjectTooltip.lineSpacing / 2 - 1,
 this->progressWidth, 2, this->progressFraction,
 this->r1, this->g1, this->b1, this->a1);
 }
 }
 }
