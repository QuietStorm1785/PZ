...
public:
 ...
 std::mutex fpsGraph_mutex;
 std::mutex lpsGraph_mutex;
 static FPSGraph instance;
 static const int NUM_BARS = 30;
 static const int BAR_WID = 8;
private
 FPSGraph.Graph fpsGraph = new FPSGraph.Graph();
private
 FPSGraph.Graph upsGraph = new FPSGraph.Graph();
private
 FPSGraph.Graph lpsGraph = new FPSGraph.Graph();
private
 FPSGraph.Graph uiGraph = new FPSGraph.Graph();
...
public
 ...
 { std::lock_guard<std::mutex> __sync_lock__(fpsGraph_mutex); this->fpsGraph.add(long0); }
 }

 void addUpdate(long long0) { this->upsGraph.add(long0); }

 void addLighting(long long0) {
 { std::lock_guard<std::mutex> __sync_lock__(lpsGraph_mutex); this->lpsGraph.add(long0); }
 }
 ...
 { std::lock_guard<std::mutex> __sync_lock__(fpsGraph_mutex);
 ...
 { std::lock_guard<std::mutex> __sync_lock__(lpsGraph_mutex);
...
class Graph {
 private
 ArrayList<Long> times = std::make_unique<ArrayList<>>();
 private
 ArrayList<Integer> bars = std::make_unique<ArrayList<>>();
