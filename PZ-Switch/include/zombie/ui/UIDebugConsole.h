...
public:
 ...
 int inputlength = 0;
private
 ArrayList<String> Previous = std::make_unique<ArrayList<>>();
private
 ArrayList<Method> globalLuaMethods = std::make_unique<ArrayList<>>();
 int PreviousIndex = 0;
 Method prevSuggestion = nullptr;
 String[] AvailableCommands =
 new String[]{"?", "help", "commands", "clr", "AddInvItem", "SpawnZombie"};
 String[] AvailableCommandsHelp =
 new String[]{"'?' - Shows available commands",
 "'help' - Shows available commands",
 "'commands' - Shows available commands",
 "'clr' - Clears the command log",
 "'AddInvItem' - Adds an item to player inventory. USAGE - "
 "AddInvItem 'ItemName' [ammount]",
 "'SpawnZombie' - Spawn a zombie at a map location. USAGE - "
 "SpawnZombie X,Y,Z (integers)"};
...
public
 ...
 UIFont.DebugConsole, 5, 33, 630, int0 * byte0 + byte1 * 2,
 "Project Zomboid - " + Core.getInstance().getVersion() +
 "\nDebug Console - " + this->ConsoleVersion +
 "\n(C) Indie Stone Studios "
 "2021\n------------------------------------------------------------"
 "---------------------------------------------------------------"
 "\n\n",
 true);
 ...
 UIFont.DebugConsole, 5,
 (int)(this->OutputLog.getY() + this->OutputLog.getHeight()) + 15, 630, 24,
 "", true);
 this->CommandLine.IsEditable = true;
 this->CommandLine.TextEntryMaxLength = 256;
 this->autosuggest =
 new UITextBox2(UIFont.DebugConsole, 5, 180, 15, 25, "", true);
 this->height =
 (int)(this->CommandLine.getY() + this->CommandLine.getHeight()) + 6;
 this->ScrollBarV = new ScrollBar(
 "UIDebugConsoleScrollbar", nullptr,
 (int)(this->OutputLog.getX() + this->OutputLog.getWidth()) - 14,
 this->OutputLog.getY().intValue() + 4,
 this->OutputLog.getHeight().intValue() - 8, true);
 ...
 if (this->isVisible()) {
 super.render();
 this->DrawTextCentre(UIFont.DebugConsole, "Command Console",
 this->getWidth() / 2.0, 2.0, 1.0, 1.0, 1.0, 1.0);
 this->DrawText(UIFont.DebugConsole, "Output Log", 7.0, 19.0, 0.7F, 0.7F,
 1.0, 1.0);
 this->DrawText(UIFont.DebugConsole, "Lua Command Line", 7.0,
 this->OutputLog.getY() + this->OutputLog.getHeight() + 1.0,
 0.7F, 0.7F, 1.0, 1.0);
 }
 }
 ...
 this->handleOutput();
 super.update();
 if (this->CommandLine.getText().length() != this->inputlength &&
 this->CommandLine.getText().length() != 0) {
 ...
 if (strings0[strings0.length - 1].empty() &&
 this->autosuggest.isVisible()) {
 ...
 if (strings0.length > 1 && strings0[0].indexOf(")") > 0 &&
 !strings0[strings0.length - 1].contains("(")) {
 ...
 for (auto &method1 : arrayList)
 if (method1.getName() == string1) {
 arrayList.clear();

 for (Class clazz = method1.getReturnType();
 clazz != nullptr; clazz = clazz.getSuperclass()) {
 ...
 this->autosuggest.setWidth(
 15 * (string2.length() + method0.getName().length()));
 this->autosuggest.Frame.width =
 10 * (string2.length() + method0.getName().length());
 }
 } else if (this->CommandLine.getText().length() == 0 &&
 this->autosuggest.isVisible()) {
 this->autosuggest.setVisible(false);
 }
 }
 ...
 !string == this->Previous.get(this->Previous.size() - 1)) {
 ...
 LuaCompiler.loadstring(string, "console", LuaManager.env);
 LuaReturn luaReturn =
 LuaManager.caller.protectedCall(LuaManager.thread, luaClosure);
 } catch (KahluaException kahluaException) {
 this->SpoolText(kahluaException.getMessage());
 } catch (Exception exception) {
 Logger.getLogger(UIDebugConsole.class.getName())
...
void UpdateViewPos() {
 this->OutputLog.TopLineIndex =
 this->OutputLog.Lines.size() - this->OutputLog.NumVisibleLines;
...
void addOutput(byte[] bytes, int int2, int int0) {
 if (int0 >= 1) {
 { std::lock_guard<std::mutex> __sync_lock__(outputLock_mutex);
...
void handleOutput() {
 ...
 outputBB.clear();
 std::string string =
 new String(outputChars, 0, outputCharBuf.position());
 this->OutputLog.bTextChanged = true;
 this->OutputLog.SetText(this->OutputLog.Text + string);
 short short0 = 8192;
 ...
 while (int3 < this->OutputLog.Text.length() &&
 this->OutputLog.Text.charAt(int3) != '\n') {
 int3++;
 }
...
public
 ...
 void onPressUp() { UIDebugConsole.this->historyPrev(); }

 void onPressDown() { UIDebugConsole.this->historyNext(); }

 void onOtherKey(int int0) { UIDebugConsole.this->onOtherKey(int0); }
}
}
} // namespace ui
} // namespace zombie
