...
private
 static ThreadLocal<FakeClientManager.StringUTF> stringUTF =
 ThreadLocal.withInitial(FakeClientManager.StringUTF::new);
 static int logLevel = 0;
 static long startTime = System.currentTimeMillis();
private
 static HashSet<FakeClientManager.Player> players =
...
private
 ...
 private
 HashMap<Integer, FakeClientManager.Client.Request> requests =
 ...
 private
 HashMap<Integer, FakeClientManager.Client> createdClients =
 std::make_unique<HashMap<>>();
 private
 HashMap<Long, FakeClientManager.Client> connectedClients =
 std::make_unique<HashMap<>>();
 ...
 int connected = -1;
 private
 static HashMap<Integer, String> systemPacketTypeNames =
 std::make_unique<HashMap<>>();
 ...
 PacketTypes.PacketType packetType = PacketTypes.packetTypes.get(short0);
 std::string string =
 packetType == nullptr ? "unknown user packet" : packetType.name();
 ...
 private
 ArrayList<FakeClientManager.Player.Clothes> clothes;
 ...
 float float0 = Float.POSITIVE_INFINITY;
 { std::lock_guard<std::mutex> __sync_lock__(playerManager.players_mutex);
 ...
 public
 HashMap<Integer, FakeClientManager.PlayerManager.RemotePlayer>
 ...
 { std::lock_guard<std::mutex> __sync_lock__(players_mutex);
 FakeClientManager.PlayerManager.RemotePlayer remotePlayer =
 ...
 short short0 = byteBuffer.getShort();
 { std::lock_guard<std::mutex> __sync_lock__(players_mutex); this->players.remove(short0); }
 ...
 private
 HashMap<Integer, FakeClientManager.Zombie> zombies =
 std::make_unique<HashMap<>>();
 private
 ArrayDeque<FakeClientManager.Zombie> zombies4Add =
 std::make_unique<ArrayDeque<>>();
 private
 ArrayDeque<FakeClientManager.Zombie> zombies4Delete =
 std::make_unique<ArrayDeque<>>();
 private
 HashSet<Short> authoriseZombies = std::make_unique<HashSet<>>();
 private
 ArrayDeque<FakeClientManager.Zombie> SendQueue =
 std::make_unique<ArrayDeque<>>();
 ...
 if (canChangeTarget) {
 { std::lock_guard<std::mutex> __sync_lock__(player.playerManager.players_mutex);
