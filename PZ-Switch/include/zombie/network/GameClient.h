...
public:
 ...
 std::mutex delayedDisconnect_mutex;
 static const GameClient instance = new GameClient();
...
public
 static List<Long> pingsList = std::make_unique<ArrayList<>>();
 static std::string GameMap;
 static bool bFastForward;
public
 static ClientServerMap[] loadedCells = new ClientServerMap[4];
 int DEBUG_PING = 5;
 IsoObjectSyncRequests objectSyncReq = new IsoObjectSyncRequests();
 ObjectsSyncRequests worldObjectsSyncReq = new ObjectsSyncRequests(true);
 static bool bCoopInvite;
private
 ArrayList<IsoPlayer> connectedPlayers = std::make_unique<ArrayList<>>();
 static bool isPaused = false;
private
 ArrayList<IsoPlayer> players = std::make_unique<ArrayList<>>();
 bool idMapDirty = true;
 static const int sendZombieWithoutNeighbor = 4000;
 static const int sendZombieWithNeighbor = 200;
 const UpdateLimit sendZombieTimer = new UpdateLimit(4000L);
 const UpdateLimit sendZombieRequestsTimer = new UpdateLimit(200L);
 const UpdateLimit UpdateChannelsRoamingLimit = new UpdateLimit(3010L);
 long disconnectTime = System.currentTimeMillis();
 static const long disconnectTimeLimit = 10000L;
 static long steamID = 0L;
public
 static Map<Short, Vector2> positions =
 new HashMap<>(ServerOptions.getInstance().getMaxPlayers());
 int safehouseUpdateTimer = 0;
 bool delayPacket = false;
private
 ArrayList<Integer> delayedDisconnect = std::make_unique<ArrayList<>>();
 static TShortArrayList tempShortList = std::make_unique<TShortArrayList>();
private
 GameClient.RequestState request;
 KahluaTable ServerSpawnRegions = nullptr;
 static ConcurrentLinkedQueue<ZomboidNetData> MainLoopNetDataQ =
 std::make_unique<ConcurrentLinkedQueue<>>();
 static ArrayList<ZomboidNetData> MainLoopNetData =
 std::make_unique<ArrayList<>>();
 static ArrayList<ZomboidNetData> LoadingMainLoopNetData =
 std::make_unique<ArrayList<>>();
 static ArrayList<ZomboidNetData> DelayedCoopNetData =
 std::make_unique<ArrayList<>>();
...
public
 static TShortObjectHashMap<IsoZombie> IDToZombieMap =
 std::make_unique<TShortObjectHashMap<>>();
 static bool bIngame;
 static bool askPing = false;
public
 ArrayList<String> ServerMods = std::make_unique<ArrayList<>>();
 ...
 ArrayList<ZomboidNetData> incomingNetData =
 std::make_unique<ArrayList<>>();
private
 HashMap<ItemContainer, ArrayList<InventoryItem>> itemsToSend =
 std::make_unique<HashMap<>>();
private
 HashMap<ItemContainer, ArrayList<InventoryItem>> itemsToSendRemove =
 std::make_unique<HashMap<>>();
 KahluaTable dbSchema;

 IsoPlayer getPlayerByOnlineID(short id) { return IDToPlayerMap.get(id); }
 ...
 try {
 this->udpEngine =
 new UdpEngine(Rand.Next(10000) + 12345, 0, 1, nullptr, false);
 if (CoopMaster.instance != nullptr && CoopMaster.instance.isRunning()) {
 this->udpEngine.Connect("127.0.0.1",
 ...
 DebugLog.Network.printException(
 exception, "Exception thrown during GameClient.startClient.",
 LogSeverity.Error);
 }
 }
 }
 ...
 if (player.getVehicle() == nullptr && !playerPacket.usePathFinder &&
 (player.networkAI.distance.getLength() > 7.0F ||
 IsoUtils.DistanceTo(playerPacket.x, playerPacket.y, playerPacket.z,
 player.x, player.y, player.z) > 1.0F &&
 ...
 (double)playerPacket.x, (double)playerPacket.y,
 (double)playerPacket.z);
 if (square != nullptr) {
 if (player.isAlive() &&
 !IsoWorld.instance.CurrentCell.getObjectList().contains(player) {
 IsoWorld.instance.CurrentCell.getObjectList().add(player);
 player.setCurrent(square);
 }
 } else if (IsoWorld.instance.CurrentCell.getObjectList().contains(
 player) {
 ...
 for (ZomboidNetData zomboidNetData0 = MainLoopNetDataQ.poll();
 zomboidNetData0 != nullptr;
 zomboidNetData0 = MainLoopNetDataQ.poll()) {
 MainLoopNetData.add(zomboidNetData0);
 }

 { std::lock_guard<std::mutex> __sync_lock__(delayedDisconnect_mutex);
 ...
 "OnConnectFailed",
 Translator.getText("UI_OnConnectFailed_AlreadyConnected"));
 case 19:
 case 20:
 case 22:
 case 25:
 ...
 LuaEventManager.triggerEvent(
 "OnConnectFailed",
 Translator.getText("UI_OnConnectFailed_Banned"));
 break;
 case 24:
 LuaEventManager.triggerEvent(
 "OnConnectFailed",
 Translator.getText("UI_OnConnectFailed_InvalidServerPassword"));
 break;
 case 32:
 LuaEventManager.triggerEvent(
 "OnConnectFailed",
 Translator.getText("UI_OnConnectFailed_ConnectionLost"));
 }
 }
 }
 ...
 for (int int3 = 0;
 int3 < IsoWorld.instance.CurrentCell.getObjectList().size();
 int3++) {
 IsoMovingObject movingObject =
 IsoWorld.instance.CurrentCell.getObjectList().get(int3);
 if (movingObject instanceof
 IsoPlayer && !((IsoPlayer)movingObject).isLocalPlayer() &&
 !this->getPlayers().contains(movingObject) {
 if (Core.bDebug) {
 DebugLog.log("Disconnected/Distant player " +
 ((IsoPlayer)movingObject).username +
 " in CurrentCell.getObjectList() removed");
 }
 ...
 std::string string0 = Translator.getText(
 GameWindow.ReadString(zomboidNetData4.buffer);
 std::string string1 = Translator.getText(
 GameWindow.ReadString(zomboidNetData4.buffer);
 ...
 void smashWindow(IsoWindow isoWindow, int action) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 (byte)isoWindow.square.getObjects().indexOf(isoWindow);
 ...
 bool boolean0 = byteBuffer.get() == 1;
 std::string string =
 boolean0 ? GameWindow.ReadStringUTF(byteBuffer) : nullptr;
 ...
 void delayPacket(int int1, int int2, int var3) {
 if (IsoWorld.instance != nullptr) {
 for (int int0 = 0; int0 < IsoWorld.instance.AddCoopPlayers.size();
 int0++) {
 AddCoopPlayer addCoopPlayer =
 IsoWorld.instance.AddCoopPlayers.get(int0);
 ...
 void mainLoopDealWithNetData(ZomboidNetData zomboidNetData) {
 ...
 DebugLog.Network.printException(
 exception, "Error with packet of type: " + zomboidNetData.type,
 LogSeverity.Error);
 }
 ...
 void mainLoopHandlePacketInternal(ZomboidNetData zomboidNetData,
 ...
 static void receivePlayerDamageFromCarCrash(ByteBuffer byteBuffer,
 short var1) {
 float float0 = byteBuffer.getFloat();
 if (IsoPlayer.getInstance().getVehicle() == nullptr) {
 DebugLog.Multiplayer.error(
 "Receive damage from car crash, can't find vehicle");
 } else {
 IsoPlayer.getInstance().getVehicle().addRandomDamageFromCrash(
 IsoPlayer.getInstance(), float0);
 LuaEventManager.triggerEvent("OnPlayerGetDamage", IsoPlayer.getInstance(),
 "CARCRASHDAMAGE", float0);
 }
 }
 ...
 static bool IsClientPaused() { return isPaused; }
 ...
 int int0 = byteBuffer.getInt();
 InventoryItem item =
 IsoPlayer.getInstance().getInventory().getItemWithIDRecursiv(int0);
 if (item.empty()) {
 ...
 LuaEventManager.triggerEvent("OnClothingUpdated",
 ...
 bool boolean0 =
 IsoPlayer.getInstance().getInventory().removeItemWithIDRecurse(int0);
 if (!boolean0) {
 DebugLog.log("ERROR: GameClient.invMngRemoveItem can not remove item " +
 int0);
 }
 }
 }

 static void receiveInvMngGetItem(ByteBuffer byteBuffer, short var1) {
 short short0 = byteBuffer.getShort();
 ...
 static void receiveInvMngReqItem(ByteBuffer byteBuffer, short var1) {
 int int0 = 0;
 ...
 IsoPlayer.getInstance().getInventory().removeItemWithIDRecurse(
 item.getID());
 } else {
 IsoPlayer.getInstance().getInventory().RemoveOneOf(
 string.split("\\.")[1]);
 }
 ...
 static void invMngRequestRemoveItem(int itemId, IsoPlayer player) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 int int1 = byteBuffer.getInt();
 int int2 = byteBuffer.getInt();
 DebugLog.log(DebugType.Sound, "sound: received " + string + " at " + int0 +
 "," + int1 + "," + int2);
 if (!Core.SoundDisabled) {
 FMOD_STUDIO_EVENT_DESCRIPTION fmod_studio_event_description =
 FMODManager.instance.getEventDescription(string);
 ...
 long0, Core.getInstance().getOptionAmbientVolume() / 20.0F);
 ...
 HandWeapon weapon = item != nullptr ? (HandWeapon)item : nullptr;
 ...
 DebugLog.log("syncThumpable: index=" + byte0 +
 " is invalid x,y,z=" + int0 + "," + int1 + "," + int2);
 }
 }
 }
 ...
 DebugLog.log("SyncDoorKey: index=" + byte0 +
 " is invalid x,y,z=" + int0 + "," + int1 + "," + int2);
 }
 }
 }
 ...
 for (int int1 = 0; int1 < IsoWorld.instance.AddCoopPlayers.size();
 int1++) {
 ...
 void sendAddXp(IsoPlayer otherPlayer, PerkFactory.Perk perk, int amount) {
 ...
 void sendSyncXp(IsoPlayer player) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 void sendTransactionID(IsoPlayer player) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 string, Userlog.UserlogType.fromIndex(byteBuffer.getInt()).toString(),
 GameWindow.ReadString(byteBuffer), GameWindow.ReadString(byteBuffer),
 byteBuffer.getInt(), GameWindow.ReadString(byteBuffer));
 }
 ...
 std::string string0 = GameWindow.ReadString(byteBuffer);
 std::string string1 = byteBuffer.getInt() - 1 + "/" + byteBuffer.getInt();
 ...
 safetyPacket.log(nullptr, "ReceiveChangeSafety");
 safetyPacket.process();
 } catch (Exception exception) {
 DebugLog.Multiplayer.printException(
 exception, "ReceiveChangeSafety: failed", LogSeverity.Error);
 }
 }
 ...
 std::string string2 = Translator.getText(string0);
 if (!StringUtils.isNullOrEmpty(string1) {
 string2 =
 string2 + " " + Translator.getText("UI_ValidationFailed_" + string1);
 }
 ...
 void addDisconnectPacket(int packet) {
 { std::lock_guard<std::mutex> __sync_lock__(delayedDisconnect_mutex); this->delayedDisconnect.add(packet); }

 ConnectionManager.log("disconnect", String.valueOf(packet), nullptr);
 }
 ...
 if (int0 > 100) {
 ChatManager.getInstance().showServerChatMessage(
 ServerOptions.clientOptionsList.get("roll"));
 return;
 }
 } catch (Exception exception) {
 ChatManager.getInstance().showServerChatMessage(
 ...
 if (!IsoPlayer.getInstance().getInventory().contains("Dice") &&
 connection.accessLevel == 1) {
 ChatManager.getInstance().showServerChatMessage(
 ServerOptions.clientOptionsList.get("roll"));
 return;
 }
 ...
 connection.accessLevel == 1) {
 ChatManager.getInstance().showServerChatMessage(
 ServerOptions.clientOptionsList.get("card"));
 ...
 std::string string = ChatManager.getInstance().getFocusTab().getTitleID();
 if ("UI_chat_admin_tab_title_id" == string) {
 ...
 else if ("UI_chat_main_tab_title_id" == string) {
 ...
 ChatManager.getInstance().showServerChatMessage(
 String.format("Client \"%s\" log level is \"%s\"",
 ...
 ChatManager.getInstance().showServerChatMessage(
 Translator.getText("UI_ServerOptionDesc_SetLogLevel",
 debugType.empty()
 ? "\"type\""
 : debugType.name().toLowerCase(),
 logSeverity.empty()
 ...
 static void sendServerPing(long timestamp) {
 ...
 if (timestamp == -1L) {
 DebugLog.Multiplayer.debugln("Player \"%s\" toggled lua debugger",
 connection.username);
 }
 }
 }

 bool gameLoadingDealWithNetData(ZomboidNetData zomboidNetData) {
 ByteBuffer byteBuffer = zomboidNetData.buffer;
 ...
 for (int int1 = 0; int1 < int0; int1++) {
 ServerOptions.instance.putOption(GameWindow.ReadString(byteBuffer),
 ...
 WorldMapRemotePlayers.instance.getOrCreatePlayerByID(short1);
 if (boolean0) {
 short short2 = byteBuffer.getShort();
 std::string string0 = GameWindow.ReadStringUTF(byteBuffer);
 std::string string1 = GameWindow.ReadStringUTF(byteBuffer);
 std::string string2 = GameWindow.ReadStringUTF(byteBuffer);
 std::string string3 = GameWindow.ReadStringUTF(byteBuffer);
 float float0 = byteBuffer.getFloat();
 float float1 = byteBuffer.getFloat();
 bool boolean1 = byteBuffer.get() == 1;
 worldMapRemotePlayer.setFullData(short2, string0, string1, string2,
 string3, float0, float1, boolean1);
 ...
 static void receiveGlobalObjects(ByteBuffer byteBuffer, short var1) {
 CGlobalObjectNetwork.receive(byteBuffer);
 }

 void setRequest(GameClient.RequestState _request) { this->request = _request; }
 ...
 DebugLog.Multiplayer.printException(
 interruptedException, "GameLoadingRequestData sleep error",
 LogSeverity.Error);
 }
 }
 }
 ...
 if (short0 >= metaGrid.getMinX() && short0 <= metaGrid.getMaxX() &&
 short1 >= metaGrid.getMinY() && short1 <= metaGrid.getMaxY()) {
 IsoMetaCell metaCell = metaGrid.getCellData(short0, short1);
 if (metaCell.info != nullptr && short2 >= 0 &&
 short2 < metaCell.info.RoomList.size()) {
 ...
 if (object != nullptr) {
 object.setCustomColor(
 new ColorInfo(byteBuffer.getFloat(), byteBuffer.getFloat(),
 ...
 IsoObject object = square.getObjects().get(int3);
 if (object != nullptr) {
 object.setOverlaySprite(string, float0, float1, float2, float3,
 false);
 }
 } catch (Exception exception) {
 }
 ...
 KahluaTable copyTable(KahluaTable table1) {
 ...
 IsoWorld.instance.CurrentCell.getChunkForGridSquare(int0, int1, int2) !=
 nullptr) {
 square = IsoGridSquare.getNew(IsoWorld.instance.getCell(), nullptr, int0,
 int1, int2);
 }
 ...
 square0.getWorldObjects().get(int4);
 if (worldInventoryObject.getItem() != nullptr &&
 worldInventoryObject.getItem().getID() == int3) {
 ...
 if (Core.bDebug) {
 DebugLog.log("receiveObjectChange: itemID=" + int3 +
 " is invalid x,y,z=" + int0 + "," + int1 + "," + int2);
 }
 } else if (byte0 == 4) {
 int int5 = byteBuffer.getInt();
 ...
 IsoWorld.instance.CurrentCell.getGridSquare(int5, int6, int7);
 if (square1.empty()) {
 ...
 DebugLog.log("receiveObjectChange: index=" + int8 +
 " is invalid x,y,z=" + int5 + "," + int6 + "," + int7);
 }
 } else {
 ...
 std::string string4 = GameWindow.ReadString(byteBuffer);
 if (Core.bDebug) {
 DebugLog.log("receiveObjectChange " + string4);
 }

 IsoGridSquare square2 =
 IsoWorld.instance.CurrentCell.getGridSquare(int9, int10, int11);
 if (square2.empty()) {
 instance.delayPacket(int9, int10, int11);
 return;
 }

 if (square2 != nullptr && int12 >= 0 &&
 int12 < square2.getObjects().size()) {
 IsoObject object1 = square2.getObjects().get(int12);
 object1.loadChange(string4, byteBuffer);
 } else if (square2 != nullptr) {
 if (Core.bDebug) {
 DebugLog.log("receiveObjectChange: index=" + int12 +
 " is invalid x,y,z=" + int9 + "," + int10 + "," + int11);
 }
 } else if (Core.bDebug) {
 DebugLog.log("receiveObjectChange: sq is nullptr x,y,z=" + int9 + "," +
 int10 + "," + int11);
 }
 }
 }
 ...
 static void receiveRemoveContestedItemsFromInventory(ByteBuffer byteBuffer,
 short var1) {
 int int0 = byteBuffer.getInt();
 ...
 "Server shut down safely. Players and map data saved.";
 GameWindow.bServerDisconnected = true;
...
public
 static boolean sendHitCharacter(IsoGameCharacter wielder,
 IsoMovingObject target, HandWeapon weapon,
 float damage, boolean ignoreDamage,
 float range, boolean isCriticalHit,
 boolean helmetFall, boolean hitHead) {
 bool boolean0 = false;
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 bool boolean1 = ((IsoPlayer)target).isLocalPlayer();
 bool boolean2 = !((IsoZombie)wielder).isRemoteZombie();
 if (boolean2 && boolean1) {
 ZombieHitPlayerPacket zombieHitPlayerPacket =
 new ZombieHitPlayerPacket();
 zombieHitPlayerPacket.set((IsoZombie)wielder, (IsoPlayer)target);
 object = zombieHitPlayerPacket;
 } else {
 DebugLog.Multiplayer.warn(
 String.format("SendHitCharacter: Wielder or target is not "
 "local (wielder=%b, target=%b)",
 boolean2, boolean1);
 }
 } else {
 DebugLog.Multiplayer.warn(String.format(
 "SendHitCharacter: unknown target type (wielder=%s, target=%s)",
 wielder.getClass().getName(), target.getClass().getName()));
 }
 } else if (wielder instanceof IsoPlayer) {
 if (target.empty()) {
 PlayerHitSquarePacket playerHitSquarePacket =
 new PlayerHitSquarePacket();
 ...
 PlayerHitPlayerPacket playerHitPlayerPacket =
 new PlayerHitPlayerPacket();
 playerHitPlayerPacket.set((IsoPlayer)wielder, (IsoPlayer)target,
 weapon, damage, ignoreDamage, range,
 isCriticalHit, hitHead);
 object = playerHitPlayerPacket;
 } else if (target instanceof IsoZombie) {
 PlayerHitZombiePacket playerHitZombiePacket =
 new PlayerHitZombiePacket();
 playerHitZombiePacket.set((IsoPlayer)wielder, (IsoZombie)target,
 weapon, damage, ignoreDamage, range,
 isCriticalHit, helmetFall, hitHead);
 object = playerHitZombiePacket;
 } else if (target instanceof BaseVehicle) {
 PlayerHitVehiclePacket playerHitVehiclePacket =
 new PlayerHitVehiclePacket();
 playerHitVehiclePacket.set((IsoPlayer)wielder, (BaseVehicle)target,
 weapon, isCriticalHit);
 ...
 DebugLog.Multiplayer.warn(String.format(
 "SendHitCharacter: unknown wielder type (wielder=%s, target=%s)",
 wielder.getClass().getName(), target.getClass().getName()));
 }
 ...
 connection.cancelPacket();
 DebugLog.Multiplayer.printException(exception, "SendHitCharacter: failed",
 LogSeverity.Error);
 }
...
public
 static void sendHitVehicle(IsoPlayer wielder, IsoGameCharacter target,
 BaseVehicle vehicle, float damage,
 boolean isTargetHitFromBehind, int vehicleDamage,
 float vehicleSpeed,
 ...
 new VehicleHitPlayerPacket();
 vehicleHitPlayerPacket.set(wielder, (IsoPlayer)target, vehicle, damage,
 isTargetHitFromBehind, vehicleDamage,
 vehicleSpeed, isVehicleHitFromBehind);
 object = vehicleHitPlayerPacket;
 } else if (target instanceof IsoZombie) {
 VehicleHitZombiePacket vehicleHitZombiePacket =
 new VehicleHitZombiePacket();
 vehicleHitZombiePacket.set(wielder, (IsoZombie)target, vehicle, damage,
 isTargetHitFromBehind, vehicleDamage,
 vehicleSpeed, isVehicleHitFromBehind);
 object = vehicleHitZombiePacket;
 } else {
 DebugLog.Multiplayer.warn(String.format(
 "SendHitVehicle: unknown target type (wielder=%s, target=%s)",
 wielder.getClass().getName(), target.getClass().getName()));
 }
 ...
 connection.cancelPacket();
 DebugLog.Multiplayer.printException(exception, "SendHitVehicle: failed",
 LogSeverity.Error);
 }
 }
 ...
 DebugLog.Multiplayer.printException(
 exception, "ReceiveZombieDeath: failed", LogSeverity.Error);
 }
 }

 static void sendZombieDeath(IsoZombie zombie) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 deadPlayerPacket.getDeathDescription());
 }
 } catch (Exception exception) {
 DebugLog.Multiplayer.printException(
 exception, "ReceivePlayerDeath: failed", LogSeverity.Error);
 }
 }

 static void sendPlayerDeath(IsoPlayer player) {
 ...
 connection.cancelPacket();
 DebugLog.Multiplayer.printException(exception, "SendPlayerDeath: failed",
 LogSeverity.Error);
 }
 }
 ...
 static void sendPlayerDamage(IsoPlayer player) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 if (Core.bDebug) {
 DebugLog.Multiplayer.debugln(
 "SendPlayerDamage: \"%s\" %f", player.getUsername(),
 player.getBodyDamage().getOverallBodyHealth());
 }
 } catch (Exception exception) {
 connection.cancelPacket();
 DebugLog.Multiplayer.printException(exception, "SendPlayerDamage: failed",
 LogSeverity.Error);
 }
 }
 ...
 static void sendPlayerInjuries(IsoPlayer player) {
 SyncInjuriesPacket syncInjuriesPacket = new SyncInjuriesPacket();
 ...
 connection.cancelPacket();
 DebugLog.Multiplayer.printException(
 exception, "SendPlayerInjuries: failed", LogSeverity.Error);
 }
 }
 ...
 connection.cancelPacket();
 DebugLog.Multiplayer.printException(exception, "SendEvent: failed",
 LogSeverity.Error);
 }
 }
 ...
 static void sendAction(BaseAction action, bool operation) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 connection.cancelPacket();
 DebugLog.Multiplayer.printException(exception, "SendAction: failed",
 LogSeverity.Error);
 }
 }
 ...
 static void sendEatBody(IsoZombie zombie, IsoMovingObject target) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 DebugLog.Multiplayer.printException(exception, "SendEatBody: failed",
 LogSeverity.Error);
 connection.cancelPacket();
 }
 }

 static void receiveEatBody(ByteBuffer bb, short packetType) {
 try {
 ...
 String.format("ReceiveEatBody: zombie=%d type=%d", short0, byte0);
 }
 ...
 IsoGridSquare square = IsoWorld.instance.CurrentCell.getGridSquare(
 (double)float2, (double)float3, (double)float4);
 if (square.empty()) {
 ...
 (IsoDeadBody)square.getStaticMovingObjects().get(int0);
 if (deadBody != nullptr) {
 ...
 DebugLog.Multiplayer.error("ReceiveEatBody: no corpse with index " +
 int0 + " on square");
 }
 } else {
 ...
 static void sendThump(IsoGameCharacter zombie, Thumpable thumpable) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 PacketTypes.PacketType.Thump.doPacket(byteBufferWriter);

 try {
 short short0 = zombie.getOnlineID();
 std::string string = zombie.getVariableString("ThumpType");
 byteBufferWriter.putShort(short0);
 byteBufferWriter.putByte(
 (byte)NetworkVariables.ThumpType.fromString(string).ordinal());
 ...
 if (Core.bDebug) {
 DebugLog.log(DebugType.Multiplayer,
 String.format("SendThump: zombie=%d type=%s target=%s",
 short0, string,
 thumpable.empty()
 ? "nullptr"
 : thumpable.getClass().getSimpleName()));
 }

 PacketTypes.PacketType.Thump.send(connection);
 } catch (Exception exception) {
 DebugLog.Multiplayer.printException(exception, "SendThump: failed",
 LogSeverity.Error);
 connection.cancelPacket();
 }
 }

 static void receiveSyncRadioData(ByteBuffer bb, short packetType) {
 short short0 = bb.getShort();
 ...
 new VoiceManagerData.RadioData(int1, int2, int3, int4);
 }
 }
 }

 static void receiveThump(ByteBuffer bb, short packetType) {
 try {
 short short0 = bb.getShort();
 std::string string =
 NetworkVariables.ThumpType.fromByte(bb.get()).toString();
 if (Core.bDebug) {
 DebugLog.log(
 DebugType.Multiplayer,
 String.format("ReceiveThump: zombie=%d type=%s", short0, string);
 }
 ...
 float float2 = bb.getFloat();
 IsoGridSquare square = IsoWorld.instance.CurrentCell.getGridSquare(
 (double)float0, (double)float1, (double)float2);
 if (square.empty()) {
 ...
 DebugLog.Multiplayer.error("ReceiveThump: no thumpable with index " +
 int0 + " on square");
 }
 } catch (Exception exception) {
 DebugLog.Multiplayer.printException(exception, "ReceiveThump: failed",
 LogSeverity.Error);
 }
 }

 void sendWorldSound(WorldSoundManager.WorldSound sound) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 if (square != nullptr && int3 >= 0 &&
 int3 < square.getObjects().size()) {
 IsoObject object = square.getObjects().get(int3);
 square.RemoveTileObject(object);
 if (object instanceof
 IsoWorldInventoryObject || object.getContainer() != nullptr) {
 ...
 ClientPlayerDB.getInstance().clientLoadNetworkCharacter(byteBuffer,
 connection);
 }
 ...
 void sendLoginQueueDone2(long dt) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 static void receiveRemoveInventoryItemFromContainer(ByteBuffer byteBuffer,
 short var1) {
 if (IsoWorld.instance.CurrentCell != nullptr) {
 ...
 DebugLog.log(
 "ERROR: removeItemFromContainer: invalid corpse index");
 return;
 }
 ...
 IsoWorldInventoryObject worldInventoryObject =
 square.getWorldObjects().get(int8);
 ...
 if (container0.empty()) {
 DebugLog.log(
 "ERROR removeItemFromContainer can't find world item with id=" +
 int6);
 return;
 }
 ...
 DebugLog.log(
 "ERROR: removeItemFromContainer: invalid object index");
 return;
 }
 ...
 if (container2.empty()) {
 DebugLog.log("ERROR: removeItemFromContainer: part " +
 part.getId() + " has no container");
 return;
 }
 ...
 if (container.empty()) {
 DebugLog.log(
 "ERROR: sendItemsToContainer: can't find world item with id=" +
 int3);
 return;
 }
 } else if (short0 == 2) {
 ...
 if (container.empty()) {
 DebugLog.log("ERROR: sendItemsToContainer: part " + part.getId() +
 " has no container");
 return;
 }
 } else {
 ...
 try {
 std::vector arrayList = CompressIdenticalItems.load(
 byteBufferReader.bb, 195, nullptr, nullptr);
 ...
 ((IsoMannequin)container.getParent())
 .wearItem(item, nullptr);
 }
 }
 }
 }
 ...
 void readItemStats(ByteBuffer byteBuffer, InventoryItem item) {
 int int0 = byteBuffer.getInt();
 float float0 = byteBuffer.getFloat();
 bool boolean0 = byteBuffer.get() == 1;
 item.setUses(int0);
 ...
 IsoMovingObject movingObject =
 square.getStaticMovingObjects().get(byte3);
 ...
 int int4 = byteBuffer.getInt();
 if (square != nullptr && byte1 >= 0 &&
 byte1 < square.getObjects().size()) {
 ...
 static bool canSeePlayerStats() { return connection.accessLevel != 1; }

 static bool canModifyPlayerStats() {
 return (connection.accessLevel & 56) != 0;
 }

 void sendPersonalColor(IsoPlayer player) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 void sendChangedPlayerStats(IsoPlayer otherPlayer) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 void writePlayerConnectData(ByteBufferWriter b, IsoPlayer player) {
 ...
 void sendPlayerConnect(IsoPlayer player) {
 ...
 void sendPlayerSave(IsoPlayer player) {
 if (connection != nullptr) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 void sendPlayer2(IsoPlayer isoPlayer) {
 if (bClient && isoPlayer.isLocalPlayer() &&
 isoPlayer.networkAI.isNeedToUpdate()) {
 ...
 void sendPlayer(IsoPlayer isoPlayer) { isoPlayer.networkAI.needToUpdate(); }

 void heartBeat() { count++; }

 static IsoZombie getZombie(short id) { return IDToZombieMap.get(id); }
 ...
 try {
 File file0 = ZomboidFileSystem.instance.getFileInCurrentSave(
 (String)arrayList.get(int0);
 if (file0.exists()) {
 File file1 = new File(ZomboidFileSystem.instance.getCacheDir() +
 File.separator + (String)arrayList.get(int0);
 if (file1.exists()) {
 file1.delete();
 }
 ...
 DebugLog.log("server was reset, deleting " + Core.GameMode +
 File.separator + Core.GameSaveWorld);
 LuaManager.GlobalObject.deleteSave(Core.GameMode + File.separator +
 Core.GameSaveWorld);
 LuaManager.GlobalObject.createWorld(Core.GameSaveWorld);

 for (int int1 = 0; int1 < arrayList.size(); int1++) {
 try {
 File file2 = ZomboidFileSystem.instance.getFileInCurrentSave(
 (String)arrayList.get(int1);
 File file3 = new File(ZomboidFileSystem.instance.getCacheDir() +
 File.separator + (String)arrayList.get(int1);
 if (file3.exists()) {
 ...
 GameTime.getInstance().setMinutesPerDay(
 SandboxOptions.instance.getDayLengthMinutes());
 ...
 try {
 player = new IsoPlayer(IsoWorld.instance.CurrentCell, survivorDesc,
 ...
 connection.getConnectedGUID(), connection.getMTUSize(),
 ...
 player.setTagColor(new ColorInfo(byteBuffer.getFloat(),
 byteBuffer.getFloat(),
 byteBuffer.getFloat(), 1.0F);
 ...
 player.setSpeakColour(new Color(byteBuffer.getFloat(),
 byteBuffer.getFloat(),
 byteBuffer.getFloat(), 1.0F);
 ...
 if (Core.bDebug) {
 DebugLog.log(DebugType.Network,
 "Player Connect received for player " + username + " id " +
 short0 + (boolean0 ? " (local)" : " (remote)"));
 }
 ...
 if (boolean0) {
 std::string string3 =
 ServerOptions.getInstance().getOption("ServerWelcomeMessage");
 if (string3 != nullptr && !string3 == "")) {
 ...
 int int0 = byteBuffer.getInt();
 instance.connectedPlayers = std::make_unique<ArrayList<>>();
 ...
 if (SteamUtils.isSteamModeEnabled()) {
 std::string string2 =
 SteamUtils.convertSteamIDToString(byteBuffer.getLong());
 arrayList2.add(string2);
 }
 }

 LuaEventManager.triggerEvent("OnScoreboardUpdate", arrayList0, arrayList1,
 arrayList2);
 }

 bool receivePlayerConnectWhileLoading(ByteBuffer bb) {
 bool boolean0 = false;
 short short0 = bb.getShort();
 uint8_t byte0 = -1;
 if (short0 != -1) {
 ...
 try {
 survivorDesc.load(bb, 195, nullptr);
 } catch (IOException iOException1) {
 iOException1.printStackTrace();
 }
 ...
 player.setOnlineID(short0);
 if (Core.bDebug) {
 DebugLog.log(DebugType.Network, "Player Connect received for player " +
 username + " id " + short0 +
 (boolean0 ? " (me)" : " (not me)"));
 }
 ...
 for (int int1 = 0; int1 < int0; int1++) {
 ServerOptions.instance.putOption(GameWindow.ReadString(bb),
 GameWindow.ReadString(bb);
 }
 ...
 std::string string2 =
 ServerOptions.getInstance().getOption("ServerWelcomeMessage");
 if (boolean0 && string2 != nullptr && !string2 == "")) {
 ChatManager.getInstance().showServerChatMessage(string2);
 }
...
public
 ArrayList<IsoPlayer> getPlayers() {
 if (!this->idMapDirty) {
 return this->players;
 } else {
 this->players.clear();
 ...
 IsoObject getIsoObjectRefFromByteBuffer(ByteBuffer byteBuffer) {
 int int0 = byteBuffer.getInt();
 int int1 = byteBuffer.getInt();
 int int2 = byteBuffer.getInt();
 uint8_t byte0 = byteBuffer.get();
 ...
 void sendWeaponHit(IsoPlayer player, HandWeapon weapon, IsoObject object) {
 if (player != nullptr && object != nullptr && player.isLocalPlayer()) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 byteBufferWriter.putShort((short)player.getPlayerNum());
 byteBufferWriter.putUTF(weapon != nullptr ? weapon.getFullType() : "");
 PacketTypes.PacketType.WeaponHit.send(connection);
 }
 }

 static void receiveSyncCustomLightSettings(ByteBuffer bb, short packetType) {
 int int0 = bb.getInt();
 int int1 = bb.getInt();
 int int2 = bb.getInt();
 uint8_t byte0 = bb.get();
 ...
 .receiveSyncCustomizedSettings(bb, nullptr);
 } else {
 ...
 IsoGridSquare square =
 IsoWorld.instance.CurrentCell.getGridSquare(int1 * 10, int2 * 10, 0);
 if (square.empty()) {
 return;
 }
 ...
 if (byte0 >= 0 && byte0 < square.getObjects().size()) {
 square.getObjects().get(byte0).syncIsoObject(true, byte2, nullptr,
 byteBuffer);
 } else {
 DebugLog.Network.warn("SyncIsoObject: index=" + byte0 +
 " is invalid x,y,z=" + int0 + "," + int1 + "," +
 int2);
 }
 }
 }
 ...
 uint8_t byte0 = boolean0 ? 0 : byteBuffer.get();
 IsoPlayer player = IDToPlayerMap.get(short1);
 if (player != nullptr) {
 for (int int3 = 0; int3 < player.getInventory().getItems().size();
 int3++) {
 ...
 uint8_t byte1 = boolean1 ? 0 : byteBuffer.get();
 IsoGridSquare square =
 IsoWorld.instance.CurrentCell.getGridSquare(int4, int5, int6);
 if (square != nullptr) {
 ...
 String string1 =
 strings.length > 0
 ? Translator.getText("UI_OnConnectFailed_" + strings[0],
 strings.length > 1 ? strings[1] : nullptr,
 strings.length > 2 ? strings[2] : nullptr,
 strings.length > 3 ? strings[3] : nullptr)
 : nullptr;
 ...
 static void receivePlayerTimeout(short playerID) {
 ...
 if (player != nullptr) {
 DebugLog.log("Received timeout for player " + player.username + " id " +
 player.OnlineID);
 ...
 player.getVehicle().VehicleID, PacketTypes.PacketType.Vehicles);
 }
 ...
 void addIncoming(short id, ByteBuffer bb) {
 if (connection != nullptr) {
 ...
 void removeZombieFromCache(IsoZombie z) {
 ...
 if (player != nullptr && byte0 == 1 && byte1 == 2) {
 ...
 void equip(IsoPlayer player, int i) {
 InventoryItem item = nullptr;
 if (i == 0) {
 item = player.getPrimaryHandItem();
 } else {
 ...
 bool boolean0 = file0.renameTo(file1);
 if (boolean0) {
 DebugLog.log("CONVERT: The GameSaveWorld directory was renamed from " +
 string0 + " to " + string1);
 } else {
 DebugLog.log("ERROR: The GameSaveWorld directory cannot rename from " +
 string0 + " to " + string1);
 }
 }
 }
 ...
 instance.init();
 Core.GameSaveWorld =
 ip + "_" + port + "_" + ServerWorldDatabase.encrypt(string0);
 this->convertGameSaveWorldDirectory(
 ZomboidFileSystem.instance.getGameModeCacheDir() + File.separator + ip +
 "_" + port + "_" + string0,
 ZomboidFileSystem.instance.getCurrentSaveDir());
 if (CoopMaster.instance != nullptr && CoopMaster.instance.isRunning()) {
 Core.GameSaveWorld = CoopMaster.instance.getPlayerSaveFolder(
 CoopMaster.instance.getServerName());
 }
 }
 ...
 void sendWorldSound(void *source, int x, int y, int z, int radius, int volume,
 bool stressHumans, float zombieIgnoreDist,
 float stressMod) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 PacketTypes.PacketType.WorldSound.doPacket(byteBufferWriter);
 ...
 float float1 = byteBuffer.getFloat();
 bool boolean1 = byteBuffer.get() == 1;
 WorldSoundManager.instance.addSound(nullptr, int0, int1, int2, int3, int4,
 boolean0, float0, float1, boolean1,
 false, true);
 }
 ...
 int int2 = byteBuffer.getInt();
 float float0 = byteBuffer.getFloat();
 DebugLog.log(DebugType.Sound, "ambient: received " + string + " at " +
 int0 + "," + int1 + " radius=" + int2);
 ...
 DebugLog.log(
 "ERROR: sendClientCommand called with wrong number of arguments (" +
 module + " " + command + ")");
 } else {
 ...
 player.setAttachedItem(string0, nullptr);
 }
 }
 }

 void sendVisual(IsoPlayer player) {
 if (player != nullptr && player.OnlineID != -1) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 IsoGridSquare square =
 cell.getGridSquare((double)float0, (double)float1, (double)float2);
 if (square.empty()) {
 ...
 if (int0 != 0 || int1 != 0) {
 new IsoZombieGiblets(IsoZombieGiblets.GibletType.A, cell, float0,
 float1, float2, int0 * Rand.Next(0.25F, 0.5F),
 int1 * Rand.Next(0.25F, 0.5F);
 }
 }
 }

 new IsoZombieGiblets(float3 *, float4 *);
 } else {
 ...
 square.getChunk().addBloodSplat(float0, float1, (int)float2,
 Rand.Next(20);
 new IsoZombieGiblets(float3 *, float4 *);
 }
 ...
 "sound: received " + byte0 + " for zombie " + short0);
 IsoZombie zombie0 = IDToZombieMap.get(short0);
 ...
 case Burned:
 std::string string4 =
 zombie0.isFemale() ? "FemaleZombieDeath" : "MaleZombieDeath";
 zombie0.getEmitter().playVocals(string4);
 break;
 case DeadCloseKilled:
 zombie0.getEmitter().playSoundImpl("HeadStab", nullptr);
 std::string string3 =
 zombie0.isFemale() ? "FemaleZombieDeath" : "MaleZombieDeath";
 ...
 zombie0.getEmitter().playSoundImpl("HeadSmash", nullptr);
 std::string string2 =
 zombie0.isFemale() ? "FemaleZombieDeath" : "MaleZombieDeath";
 ...
 case Idle:
 std::string string1 =
 zombie0.isFemale() ? "FemaleZombieIdle" : "MaleZombieIdle";
 zombie0.getEmitter().playVocals(string1);
 break;
 case Lunge:
 std::string string0 =
 zombie0.isFemale() ? "FemaleZombieAttack" : "MaleZombieAttack";
 ...
 DebugLog.log(DebugType.Combat,
 "slowTimer=" + float0 + " slowFactor=" + float1);
 }
 }

 void sendCustomColor(IsoObject item) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 void sendStitch(IsoGameCharacter wielder, IsoGameCharacter target,
 ...
 void sendWoundInfection(int onlineID, int i, bool infected) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 void sendDisinfect(IsoGameCharacter wielder, IsoGameCharacter target,
 BodyPart bodyPart, InventoryItem alcohol) {
 Disinfect disinfect = new Disinfect();
 ...
 void sendAdditionalPain(int onlineID, int i, float level) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 void sendRemoveGlass(IsoGameCharacter wielder, IsoGameCharacter target,
 BodyPart bodyPart, bool handPain) {
 RemoveGlass removeGlass = new RemoveGlass();
 ...
 void sendRemoveBullet(IsoGameCharacter wielder, IsoGameCharacter target,
 BodyPart bodyPart) {
 RemoveBullet removeBullet = new RemoveBullet();
 ...
 void sendCleanBurn(IsoGameCharacter _wielder, IsoGameCharacter _target,
 BodyPart bodyPart, InventoryItem _bandage) {
 CleanBurn cleanBurn = new CleanBurn();
 ...
 void eatFood(IsoPlayer player, Food food, float percentage) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 void drink(IsoPlayer player, float drink) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 void addToItemSendBuffer(IsoObject parent, ItemContainer container,
 InventoryItem item) {
 if (parent instanceof IsoWorldInventoryObject) {
 InventoryItem _item = ((IsoWorldInventoryObject)parent).getItem();
 if (item.empty() || _item.empty() ||
 !(_item instanceof InventoryContainer) ||
 container != ((InventoryContainer)_item).getInventory()) {
 DebugLog.log("ERROR: addToItemSendBuffer parent=" + parent +
 " item=" + item);
 if (Core.bDebug) {
 ...
 container.vehiclePart.getVehicle() != parent) {
 DebugLog.log("ERROR: addToItemSendBuffer parent=" + parent +
 " item=" + item);
 if (Core.bDebug) {
 throw std::make_unique<IllegalStateException>();
 ...
 } else if (parent.empty() || item.empty() ||
 parent.getContainerIndex(container) == -1) {
 DebugLog.log("ERROR: addToItemSendBuffer parent=" + parent +
 " item=" + item);
 if (Core.bDebug) {
 throw std::make_unique<IllegalStateException>();
 ...
 std::vector arrayList0 = this->itemsToSendRemove.get(container);
 ...
 void addToItemRemoveSendBuffer(IsoObject parent, ItemContainer container,
 InventoryItem item) {
 if (parent instanceof IsoWorldInventoryObject) {
 InventoryItem _item = ((IsoWorldInventoryObject)parent).getItem();
 if (item.empty() || _item.empty() ||
 !(_item instanceof InventoryContainer) ||
 container != ((InventoryContainer)_item).getInventory()) {
 DebugLog.log("ERROR: addToItemRemoveSendBuffer parent=" + parent +
 " item=" + item);
 if (Core.bDebug) {
 throw std::make_unique<IllegalStateException>();
 } else {
 ...
 if (!SystemDisabler.doWorldSyncEnable) {
 if (this->itemsToSend.containsKey(container) {
 std::vector arrayList0 = this->itemsToSend.get(container);
 if (arrayList0.remove(item) {
 if (arrayList0.empty()) {
 ...
 void sendAddedRemovedItems(bool force) {
 bool boolean0 = force || this->itemSendFrequency.Check();
 if (!SystemDisabler.doWorldSyncEnable &&
 !this->itemsToSendRemove.empty() && boolean0) {
 for (Entry entry0 : this->itemsToSendRemove.entrySet()) {
 ItemContainer container0 = (ItemContainer)entry0.getKey();
 std::vector arrayList0 = (ArrayList)entry0.getValue();
 void *object0 = container0.getParent();
 if (container0.getContainingItem() != nullptr &&
 container0.getContainingItem().getWorldItem() != nullptr) {
 object0 = container0.getContainingItem().getWorldItem();
 }

 if (object0 != nullptr && ((IsoObject)object0).square != nullptr) {
 try {
 ByteBufferWriter byteBufferWriter0 = connection.startPacket();
 PacketTypes.PacketType.RemoveInventoryItemFromContainer.doPacket(
 byteBufferWriter0);
 ...
 (byte)((IsoObject)object0).getStaticMovingObjectIndex());
 } else if (object0 instanceof IsoWorldInventoryObject) {
 byteBufferWriter0.putShort((short)1);
 byteBufferWriter0.putInt(((IsoObject)object0).square.getX());
 byteBufferWriter0.putInt(((IsoObject)object0).square.getY());
 byteBufferWriter0.putInt(((IsoObject)object0).square.getZ());
 byteBufferWriter0.putInt(
 ((IsoWorldInventoryObject)object0).getItem().id);
 ...
 byteBufferWriter0.putByte(
 (byte)container0.vehiclePart.getIndex());
 } else {
 byteBufferWriter0.putShort((short)2);
 byteBufferWriter0.putInt(((IsoObject)object0).square.getX());
 byteBufferWriter0.putInt(((IsoObject)object0).square.getY());
 byteBufferWriter0.putInt(((IsoObject)object0).square.getZ());
 byteBufferWriter0.putByte((byte)((IsoObject)object0)
 .square.getObjects()
 .indexOf(object0);
 byteBufferWriter0.putByte(
 (byte)((IsoObject)object0).getContainerIndex(container0);
 }
 ...
 DebugLog.log("sendAddedRemovedItems: itemsToSendRemove container:" +
 container0 + "." + object0 + " items:" + arrayList0);
 if (arrayList0 != nullptr) {
 for (int int1 = 0; int1 < arrayList0.size(); int1++) {
 if (arrayList0.get(int1) == nullptr) {
 DebugLog.log("item:nullptr");
 } else {
 DebugLog.log("item:" +
 ((InventoryItem)arrayList0.get(int1).getName());
 }
 }
 ...
 if (!this->itemsToSend.empty() && boolean0) {
 for (Entry entry1 : this->itemsToSend.entrySet()) {
 ItemContainer container1 = (ItemContainer)entry1.getKey();
 std::vector arrayList1 = (ArrayList)entry1.getValue();
 void *object1 = container1.getParent();
 if (container1.getContainingItem() != nullptr &&
 container1.getContainingItem().getWorldItem() != nullptr) {
 object1 = container1.getContainingItem().getWorldItem();
 }

 if (object1 != nullptr && ((IsoObject)object1).square != nullptr) {
 try {
 ByteBufferWriter byteBufferWriter1 = connection.startPacket();
 ...
 (byte)((IsoObject)object1).getStaticMovingObjectIndex());

 try {
 CompressIdenticalItems.save(byteBufferWriter1.bb, arrayList1,
 nullptr);
 } catch (Exception exception1) {
 exception1.printStackTrace();
 }
 ...
 byteBufferWriter1.putInt(((IsoObject)object1).square.getY());
 byteBufferWriter1.putInt(((IsoObject)object1).square.getZ());
 byteBufferWriter1.putInt(
 ((IsoWorldInventoryObject)object1).getItem().id);
 ...
 byteBufferWriter1.putByte(
 (byte)container1.vehiclePart.getIndex());
 ...
 byteBufferWriter1.putInt(((IsoObject)object1).square.getY());
 byteBufferWriter1.putInt(((IsoObject)object1).square.getZ());
 byteBufferWriter1.putByte((byte)((IsoObject)object1)
 .square.getObjects()
 .indexOf(object1);
 byteBufferWriter1.putByte(
 (byte)((IsoObject)object1).getContainerIndex(container1);
 ...
 DebugLog.log("sendAddedRemovedItems: itemsToSend container:" +
 container1 + "." + object1 + " items:" + arrayList1);
 if (arrayList1 != nullptr) {
 for (int int2 = 0; int2 < arrayList1.size(); int2++) {
 if (arrayList1.get(int2) == nullptr) {
 DebugLog.log("item:nullptr");
 } else {
 DebugLog.log("item:" +
 ((InventoryItem)arrayList1.get(int2).getName());
 }
 }
 ...
 void checkAddedRemovedItems(IsoObject aboutToRemove) {
 if (aboutToRemove != nullptr) {
 if (!this->itemsToSend.empty() || !this->itemsToSendRemove.empty()) {
 if (aboutToRemove instanceof IsoDeadBody) {
 if (this->itemsToSend.containsKey(aboutToRemove.getContainer()) ||
 this->itemsToSendRemove.containsKey(
 aboutToRemove.getContainer())) {
 this->sendAddedRemovedItems(true);
 }
 } else if (aboutToRemove instanceof IsoWorldInventoryObject) {
 InventoryItem item =
 ((IsoWorldInventoryObject)aboutToRemove).getItem();
 if (item instanceof InventoryContainer) {
 ItemContainer container0 =
 ((InventoryContainer)item).getInventory();
 if (this->itemsToSend.containsKey(container0) ||
 this->itemsToSendRemove.containsKey(container0) {
 this->sendAddedRemovedItems(true);
 }
 }
 ...
 if (this->itemsToSend.containsKey(container1) ||
 this->itemsToSendRemove.containsKey(container1) {
 this->sendAddedRemovedItems(true);
 return;
 }
 }
 ...
 void sendReplaceOnCooked(InventoryItem item) {
 ...
 byteBufferWriter.putByte(
 (byte)object.square.getObjects().indexOf(object);
 byteBufferWriter.putByte(
 (byte)object.getContainerIndex(item.getContainer()));
 byteBufferWriter.putInt(item.getID());
 PacketTypes.PacketType.ReplaceOnCooked.send(connection);
 }
 }

 void writeItemStats(ByteBufferWriter byteBufferWriter, InventoryItem item) {
 byteBufferWriter.putInt(item.id);
 byteBufferWriter.putInt(item.getUses());
 byteBufferWriter.putFloat(
 item instanceof
 DrainableComboItem ? ((DrainableComboItem)item).getUsedDelta() : 0.0F);
 ...
 void sendItemStats(InventoryItem item) {
 if (item != nullptr) {
 if (item.getWorldItem() != nullptr &&
 item.getWorldItem().getWorldObjectIndex() != -1) {
 ...
 } else if (item.getContainer() == nullptr) {
 DebugLog.log("ERROR: sendItemStats(): item is neither in a container "
 "nor on the ground");
 if (Core.bDebug) {
 throw std::make_unique<IllegalStateException>();
 }
 ...
 if (!(_item instanceof InventoryContainer) ||
 container != ((InventoryContainer)_item).getInventory()) {
 DebugLog.log("ERROR: sendItemStats() parent=" + object +
 " item=" + item);
 if (Core.bDebug) {
 throw std::make_unique<IllegalStateException>();
 } else {
 ...
 ((IsoObject)object).getContainerIndex(container) == -1) {
 DebugLog.log("ERROR: sendItemStats() parent=" + object +
 " item=" + item);
 if (Core.bDebug) {
 throw std::make_unique<IllegalStateException>();
 ...
 std::string string0 = System.getProperty("file.separator");
 std::string string1 = System.getProperty("java.class.path");
 std::string string2 = System.getProperty("java.home") + string0 +
 "bin" + string0 + "java";
 ProcessBuilder processBuilder = new ProcessBuilder(
 string2, "-Xms2048m", "-Xmx2048m",
 "-Djava.library.path=../natives/", "-cp",
 ...
 static void registerZone(IsoMetaGrid.Zone zone, bool transmitToOthers) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 bool boolean0 = byteBuffer.get() == 1;
 if (IsoWorld.instance != nullptr &&
 IsoWorld.instance.helicopter != nullptr) {
 ...
 static void sendSafehouse(SafeHouse safehouse, bool remove) {
 ...
 static void sendKickOutOfSafehouse(IsoPlayer player) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 static void destroy(IsoObject obj) {
 ...
 static void sendTeleport(IsoPlayer player, float x, float y, float z) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 static void sendStopFire(IsoGridSquare sq) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 static void sendStopFire(IsoGameCharacter chr) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 PacketTypes.PacketType.StopFire.doPacket(byteBufferWriter);
 ...
 void sendCataplasm(int onlineID, int i, float plantainFactor,
 ...
 static void receiveRadioDeviceDataState(ByteBuffer bb, short packetType) {
 uint8_t byte0 = bb.get();
 if (byte0 == 1) {
 int int0 = bb.getInt();
 int int1 = bb.getInt();
 ...
 try {
 deviceData0.receiveDeviceDataStatePacket(bb, nullptr);
 } catch (Exception exception0) {
 ...
 try {
 deviceData1.receiveDeviceDataStatePacket(bb, nullptr);
 } catch (Exception exception2) {
 ...
 static void receiveRadioServerData(ByteBuffer bb, short packetType) {
 ...
 static void receiveRadioPostSilence(ByteBuffer bb, short packetType) {
 ZomboidRadio.POST_RADIO_SILENCE = bb.get() == 1;
 }

public
 static void sendIsoWaveSignal(int sourceX, int sourceY, int channel,
 String msg, String guid, String codes, float r,
 float g, float b, int signalStrength,
 boolean isTV) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 waveSignal.set(sourceX, sourceY, channel, msg, guid, codes, r, g, b,
 signalStrength, isTV);
 ...
 connection.cancelPacket();
 DebugLog.Multiplayer.printException(
 exception, "SendIsoWaveSignal: failed", LogSeverity.Error);
 }
 }

 static void receiveWaveSignal(ByteBuffer bb, short packetType) {
 ...
 static void sendPlayerListensChannel(int channel, bool listenmode,
 bool isTV) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 if (bSFurnace.empty()) {
 DebugLog.log("receiveFurnaceChange: furnace is nullptr x,y,z=" +
 int0 + "," + int1 + "," + int2);
 return;
 }
 ...
 static void sendFurnaceChange(BSFurnace furnace) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 static void sendCompost(IsoCompost isoCompost) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 void wakeUpPlayer(IsoPlayer chr) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
...
public
 ArrayList<IsoPlayer> getConnectedPlayers() { return this->connectedPlayers; }

 static void sendNonPvpZone(NonPvpZone nonPvpZone, bool remove) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 static void sendFaction(Faction faction, bool remove) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 for (int int1 = 0; int1 < int0; int1++) {
 DBTicket dBTicket0 =
 new DBTicket(GameWindow.ReadString(byteBuffer),
 GameWindow.ReadString(byteBuffer), byteBuffer.getInt());
 arrayList.add(dBTicket0);
 if (byteBuffer.get() == 1) {
 DBTicket dBTicket1 = new DBTicket(GameWindow.ReadString(byteBuffer),
 GameWindow.ReadString(byteBuffer),
 byteBuffer.getInt());
 ...
 byteBufferWriter.putByte((byte)(sender != nullptr ? 1 : 0);
 if (sender != nullptr) {
 byteBufferWriter.putShort(sender.getOnlineID());
 }

 GameWindow.WriteString(byteBufferWriter.bb, sessionID);
 byteBufferWriter.putByte((byte)(custom != nullptr ? 1 : 0);
 if (custom != nullptr) {
 GameWindow.WriteString(byteBufferWriter.bb, custom);
 }

 try {
 CompressIdenticalItems.save(byteBufferWriter.bb, items, nullptr);
 } catch (Exception exception) {
 exception.printStackTrace();
 connection.cancelPacket();
 return false;
 ...
 short short0 = byteBuffer.getShort();
 std::vector arrayList = new ArrayList(short0);
 ...
 void requestTrading(IsoPlayer you, IsoPlayer other) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 void acceptTrading(IsoPlayer you, IsoPlayer other, bool accept) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 PacketTypes.PacketType.RequestTrading.doPacket(byteBufferWriter);
 byteBufferWriter.putShort(other.OnlineID);
 byteBufferWriter.putShort(you.OnlineID);
 ...
 void tradingUISendAddItem(IsoPlayer you, IsoPlayer other,
 InventoryItem item) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 void tradingUISendRemoveItem(IsoPlayer you, IsoPlayer other, int index) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 void tradingUISendUpdateState(IsoPlayer you, IsoPlayer other, int state) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 int sendInventoryPutItems(ByteBufferWriter byteBufferWriter,
 InventoryItem > linkedHashMap0, long long0) {
 ...
 byteBufferWriter.putBoolean(item.getWorker() != nullptr && item.getWorker() == "inInv"));
 if (item instanceof
 InventoryContainer &&
 ...
 .getItems()
 .empty()) {
 LinkedHashMap linkedHashMap1 = ((InventoryContainer)item)
 .getItemContainer()
 .getItems4Admin();
 int0 += linkedHashMap1.size();
 this->sendInventoryPutItems(byteBufferWriter, linkedHashMap1,
 item.getID());
 }
 }
 ...
 byteBufferWriter.putFloat(
 IsoPlayer.getInstance().getInventory().getCapacityWeight());
 byteBufferWriter.putFloat(IsoPlayer.getInstance().getMaxWeight());
 LinkedHashMap linkedHashMap =
 IsoPlayer.getInstance().getInventory().getItems4Admin();
 int int1 =
 instance.sendInventoryPutItems(byteBufferWriter, linkedHashMap, -1L);
 ...
 static void sendGetItemInvMng(long itemId) {}
 ...
 static void receivePlayerConnectLoading(ByteBuffer byteBuffer) {
 int int0 = byteBuffer.position();
 ...
 try {
 climateManager.receiveClimatePacket(byteBuffer, nullptr);
 } catch (Exception exception) {
 exception.printStackTrace();
 }
 }
 ...
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 PacketTypes.PacketType.IsoRegionClientRequestFullUpdate.doPacket(
 byteBufferWriter);
 PacketTypes.PacketType.IsoRegionClientRequestFullUpdate.send(connection);
 }

 void sendSandboxOptionsToServer(SandboxOptions options) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 static void sendZombieHelmetFall(IsoPlayer player, IsoGameCharacter zombie,
 InventoryItem item) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 IsoZombie zombie0 = IDToZombieMap.get(short0);
 std::string string = GameWindow.ReadString(byteBuffer);
 if (zombie0 != nullptr && !StringUtils.isNullOrEmpty(string) {
 zombie0.helmetFall(true, string);
 }
 }

 static void sendPerks(IsoPlayer player) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 static void sendWeight(IsoPlayer player) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 LuaEventManager.triggerEvent("AcceptedSafehouseInvite",
 safeHouse.getTitle(), string1);
 }

 static void sendEquippedRadioFreq(IsoPlayer plyr) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 static void sendSneezingCoughing(short playerId, int sneezingCoughing,
 uint8_t sneezeVar) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 player.Say(Translator.getText("IGUI_PlayerText_" +
 (boolean0 ? "Sneeze" : "Cough") +
 (boolean1 ? "Muffled" : "")));
 player.reportEvent("EventDoExt");
 }
 }

 static void sendBurnCorpse(short playerId, short objectId) {
 ByteBufferWriter byteBufferWriter = connection.startPacket();
 ...
 static void rememberPlayerPosition(IsoPlayer player, float float0,
 float float1) {
 if (player != nullptr && !player.isLocalPlayer()) {
 ...
 WorldMapRemotePlayer worldMapRemotePlayer =
 WorldMapRemotePlayers.instance.getPlayerByID(player.getOnlineID());
 ...
 static void sendValidatePacket(ValidatePacket packet) {
...
public
 static enum RequestState { Start, Loading, Complete; }
}
} // namespace network
} // namespace zombie
