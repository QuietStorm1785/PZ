...
class LoaderThread extends Thread {
 private
 LinkedBlockingQueue<ServerMap.ServerCell> toThread =
 std::make_unique<LinkedBlockingQueue<>>();
 private
 LinkedBlockingQueue<ServerMap.ServerCell> fromThread =
 std::make_unique<LinkedBlockingQueue<>>();
...
class RecalcAllThread extends Thread {
 ...
 std::make_unique<LinkedBlockingQueue<>>();
 private
 ServerChunkLoader.GetSquare serverCellGetSquare =
 ...
 private
 LinkedBlockingQueue<ServerChunkLoader.SaveTask> toThread =
 std::make_unique<LinkedBlockingQueue<>>();
 private
 LinkedBlockingQueue<ServerChunkLoader.SaveTask> fromThread =
 std::make_unique<LinkedBlockingQueue<>>();
 bool quit = false;
 const CRC32 crc32 = new CRC32();
 const ClientChunkRequest ccr = new ClientChunkRequest();
 private
 ArrayList<ServerChunkLoader.SaveTask> toSaveChunk =
 std::make_unique<ArrayList<>>();
 private
 ArrayList<ServerChunkLoader.SaveTask> savedChunks =
 std::make_unique<ArrayList<>>();
