...
public:
 ...
 this->playerFlags = 0;
 this->playerFlags =
 (short)(this->playerFlags | (short)(player1.isAimAtFloor() ? 1 : 0);
 this->playerFlags =
 (short)(this->playerFlags | (short)(player1.isDoShove() ? 2 : 0);
 this->playerFlags = (short)(this->playerFlags |
 (short)(player1.isAttackFromBehind() ? 4 : 0);
 ...
 if (player1 != nullptr) {
 this->player =
 new IsoPlayer(player1.getCell(), std::make_unique<SurvivorDesc>(),
 (int)player1.x, (int)player1.y, (int)player1.z);
 }
 }
 ...
 bool isConsistent() { return super.isConsistent() && this->player != nullptr; }
 ...
 " | charge=" + this->charge + " | perkAiming=" + this->perkAiming +
 " | combatSpeed=" + this->combatSpeed + " | attackType=\"" +
 this->attackType +
 "\" | isAimAtFloor=" + ((this->playerFlags & 1) != 0) +
 " | isDoShove=" + ((this->playerFlags & 2) != 0) +
 " | isAttackFromBehind=" + ((this->playerFlags & 4) != 0) +
 " | isCriticalHit=" + ((this->playerFlags & 8) != 0) +
 " | _bodyDamage=" +
 (this->player.empty() ? "?"
 : this->player.getBodyDamage().getHealth()) +
 this->attackVars.getDescription() + "\n\t hitList=[" + string +
 "](count=" + this->hitList.size() + ") ]";
 }
 ...
 if (this->player.isAttackStarted() && weapon.isRanged() &&
 !this->player.isDoShove()) {
 ...
 } else if (GameServer.bServer && boolean0 &&
 !this->player.getSafety().isEnabled()) {
 this->player.getSafety().setCooldown(
 this->player.getSafety().getCooldown() +
 ServerOptions.getInstance().SafetyCooldownTimer.getValue());
 GameServer.sendChangeSafety(this->player.getSafety());
 }
 }

 IsoGameCharacter getCharacter() { return this->player; }
