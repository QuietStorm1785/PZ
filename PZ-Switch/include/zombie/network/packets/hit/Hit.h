...
public:
 ...
 return "\n\tHit [ ignore=" + this->ignore + " | damage=" + this->damage +
 " | force=" + this->hitForce + " | dir=( " + this->hitDirectionX +
 " ; " + this->hitDirectionY + " ) ]";
 }
 ...
 if (GameServer.bServer && character0 instanceof
 IsoZombie && character1 instanceof IsoPlayer) {
 ((IsoZombie)character0).addAggro(character1, this->damage);
 DebugLog.Damage.noise("AddAggro zombie=%d player=%d ( \"%s\" ) damage=%f",
 character0.getOnlineID(), character1.getOnlineID(),
 ((IsoPlayer)character1).getUsername(), this->damage);
 }
