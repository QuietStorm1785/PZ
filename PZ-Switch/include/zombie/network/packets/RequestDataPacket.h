...
public:
 ...
 void setPartData(RequestDataPacket.RequestID requestID,
 ...
 void sendConnectingDetails(UdpConnection udpConnection,
 ...
 DebugLog.Multiplayer.debugln("%s %db", this->id.name(),
 large_file_bb.position());
 ...
 exception, "RequestData packet parse failed", LogSeverity.Error);
 ...
 large_file_bb.put(byteBuffer.array(), byteBuffer.position(),
 this->partSize);
 this->buffer = large_file_bb;
 }
 }
 ...
 byteBufferWriter.bb.put(this->buffer.array(), this->dataSent,
 this->partSize);
 }
 }
 }
 ...
 RequestDataManager.getInstance().ACKWasReceived(this->id, udpConnection,
 this->dataSent);
 }
 }
 ...
 RequestDataManager.getInstance().putDataForTransmit(
 this->id, udpConnection, large_file_bb);
 }
 }
 ...
 SharedDescriptors.getPlayerZombieDescriptors();
 int int0 = 0;
 ...
 DebugLog.Multiplayer.printException(
 exception2, "map_meta.bin could not be saved", LogSeverity.Error);
 GameServer.kick(udpConnection,
 "You have been kicked from this server because "
 "map_meta.bin could not be saved.",
 nullptr);
 ...
 DebugLog.Multiplayer.printException(
 exception3, "map_zone.bin could not be saved", LogSeverity.Error);
 GameServer.kick(udpConnection,
 "You have been kicked from this server because "
 "map_zone.bin could not be saved.",
 nullptr);
 ...
 large_file_bb.clear();
 ZomboidRadio.getInstance().getRecordedMedia().sendRequestData(
 large_file_bb);
 this->doSendRequest(udpConnection);
 }
 ...
 ByteBuffer byteBuffer0 = RequestDataManager.getInstance().receiveClientData(
 this->id, byteBuffer1, this->dataSize, this->dataSent);
 ...
 DebugLog.Multiplayer.printException(
 iOException, "PersistentOutfits loading IO error",
 LogSeverity.Error);
 ExceptionLogger.logException(iOException);
 } catch (Exception exception0) {
 DebugLog.Multiplayer.printException(
 exception0, "PersistentOutfits loading error", LogSeverity.Error);
 }
 }
 ...
 DebugLog.Multiplayer.printException(
 exception1, "Player zombie descriptors loading error",
 LogSeverity.Error);
 ...
 DebugLog.Multiplayer.printException(
 exception2, "Radio data loading error", LogSeverity.Error);
 ...
 DebugLog.Multiplayer.printException(
 iOException, "Save to the " + string + " file error",
 LogSeverity.Error);
 }
 }
 ...
 DebugLog.Multiplayer.debugln("received " + short0 +
 " player-zombie descriptors");
 ...
 for (int int1 = 0;
 int1 < Math.min(15, this->buffer.limit() - this->buffer.position());
 int1++) {
...
public
 static enum RequestID {
 ConnectionDetails, Descriptors, MetaGrid, MapZone, PlayerZombieDescriptors,
 RadioData;
...
static enum RequestType {
 None, Request, FullData, PartData, PartDataACK;
}
} // namespace packets
} // namespace network
} // namespace zombie
