...
private
 ArrayList<ClientChunkRequest> ccrWaiting =
...
public
 ...
 if (this->workerThread.cancelQ.empty() &&
 !this->workerThread.cancelled.empty()) {
...
private
 ...
 bool bQuit;
 boolean bReady = true;
 LinkedBlockingQueue<PlayerDownloadServer.WorkerThreadCommand>
 commandQ = std::make_unique<LinkedBlockingQueue<>>();
 ConcurrentLinkedQueue<ClientChunkRequest> freeRequests =
 std::make_unique<ConcurrentLinkedQueue<>>();
 ConcurrentLinkedQueue<Integer> cancelQ =
 std::make_unique<ConcurrentLinkedQueue<>>();
