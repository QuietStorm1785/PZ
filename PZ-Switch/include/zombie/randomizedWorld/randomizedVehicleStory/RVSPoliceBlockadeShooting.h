...
/**
 * Police barricading a road, 2 police cars, some zombies police with
...
public
 ...
 "barricade", 0.0F, boolean0 ? -float2 - 2.5F : float2 + 2.5F,
 IsoDirections.N.ToVector().getDirection(), this->zoneWidth, 1.0F);
 int int0 = Rand.Next(7, 15);

 for (int int1 = 0; int1 < int0; int1++) {
 vehicleStorySpawner.addElement(
 "corpse",
 Rand.Next(-this->zoneWidth / 2.0F + 1.0F,
 this->zoneWidth / 2.0F - 1.0F),
 boolean0 ? Rand.Next(-7, -4) - float2 : Rand.Next(5, 8) + float2,
 IsoDirections.getRandom().ToVector().getDirection(), 1.0F, 2.0F);
 }
 ...
 IsoCell.getInstance().getGridSquare(int2, int3, zone.z);
 if (square1 != nullptr) {
 if (int3 != int0 && int3 != int1) {
 square1.AddTileObject(IsoObject.getNew(
 square1, "construction_01_9", nullptr, false);
 } else {
 square1.AddTileObject(IsoObject.getNew(
 square1, "street_decoration_01_26", nullptr, false);
 }
 }
 }
 } else {
 ...
 IsoCell.getInstance().getGridSquare(int7, int6, zone.z);
 if (square2 != nullptr) {
 if (int7 != int4 && int7 != int5) {
 square2.AddTileObject(IsoObject.getNew(
 square2, "construction_01_8", nullptr, false);
 } else {
 square2.AddTileObject(IsoObject.getNew(
 square2, "street_decoration_01_26", nullptr, false);
 }
 }
 }
 }
 ...
 if (vehicle1 != nullptr) {
 createRandomDeadBody(element.position.x, element.position.y, zone.z,
 element.direction, false, 10, 10, nullptr);
 IsoDirections directions =
 this->horizontalZone
 ? (element.position.x < vehicle1.x ? IsoDirections.W
 : IsoDirections.E)
 : (element.position.y < vehicle1.y ? IsoDirections.N
 : IsoDirections.S);
 ...
 case "vehicle1":
 case "vehicle2":
 BaseVehicle vehicle0 = this->addVehicle(
 zone, element.position.x, element.position.y, float0,
 element.direction, nullptr, string0, nullptr, nullptr);
 if (vehicle0 != nullptr) {
