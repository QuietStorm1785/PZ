...
/**
 * Van with a sewer hole & road cones around it, some construction worker and a
 * foreman + some tools in ground
 */
...
public
 ...
 vehicleStorySpawner.addElement("vehicle1", -int0 * 2.0F, 0.0F,
 vector.getDirection(), 2.0F, 5.0F);
 float float1 = 0.0F;
 vehicleStorySpawner.addElement("manhole", int0 * 1.5F, 1.5F, float1, 3.0F,
 3.0F);
 int int1 = Rand.Next(0, 3);

 for (int int2 = 0; int2 < int1; int2++) {
 float1 = 0.0F;
 vehicleStorySpawner.addElement("tool", int0 * Rand.Next(0.0F, 3.0F),
 -Rand.Next(0.7F, 2.3F), float1, 1.0F,
 1.0F);
 }
 ...
 switch (string0) {
 case "manhole":
 square0.AddTileObject(IsoObject.getNew(
 square0, "street_decoration_01_15", nullptr, false);
 ...
 break;
 case "tool":
 std::string string1 = this->tools.get(Rand.Next(this->tools.size()));
 square0.AddWorldInventoryItem(string1, float0, float1, float2);
 break;
 case "vehicle1":
 vehicle =
 this->addVehicle(zone, element.position.x, element.position.y,
 float3, element.direction, nullptr,
 "Base.PickUpTruck", nullptr, "ConstructionWorker");
 if (vehicle != nullptr) {
 this->addZombiesOnVehicle(Rand.Next(2, 5), "ConstructionWorker", 0,
 vehicle);
