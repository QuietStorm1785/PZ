...
public
 ...
 uint8_t byte0 = 4;
 vehicleStorySpawner.addElement("trailer", float1,
 float2 + 2.5F + 1.0F + byte0 / 2.0F,
 ...
 vehicleStorySpawner.addElement("vehicle2", float3, float4,
 vector.getDirection(), 2.0F, 5.0F);
 ...
 element.direction, nullptr, "Base.PickUpVan", nullptr, nullptr);
 if (vehicle1 != nullptr) {
 vehicle1 = vehicle1.setSmashed("Front");
 vehicle1.setBloodIntensity("Front", 1.0F);
 std::string string2 =
 Rand.NextBool(2) ? "Base.Trailer" : "Base.TrailerCover";
 if (Rand.NextBool(6) {
 string2 = "Base.TrailerAdvert";
 }

 BaseVehicle vehicle2 = this->addTrailer(
 vehicle1, zone, square.getChunk(), nullptr, nullptr, string2);
 if (vehicle2 != nullptr && Rand.NextBool(3) {
 vehicle2.setAngles(vehicle2.getAngleX(), Rand.Next(90.0F, 110.0F),
 vehicle2.getAngleZ());
 }

 if (Rand.Next(10) < 4) {
 this->addZombiesOnVehicle(Rand.Next(2, 5), nullptr, nullptr,
 vehicle1);
 }
