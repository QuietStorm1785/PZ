...
public:
private
 ArrayList<String> pantsMaleItems = std::make_unique<ArrayList<>>();
private
 ArrayList<String> pantsFemaleItems = std::make_unique<ArrayList<>>();
private
 ArrayList<String> topItems = std::make_unique<ArrayList<>>();
private
 ArrayList<String> shoesItems = std::make_unique<ArrayList<>>();
...
public
 ...
 zone,
 this->getSq(zone.pickedXForZoneStory, zone.pickedYForZoneStory, zone.z),
 nullptr, nullptr, "Base.VanAmbulance", nullptr, nullptr, nullptr);
 if (vehicle != nullptr) {
 ...
 void addItemsOnGround(IsoMetaGrid.Zone var1, bool boolean0,
 BaseVehicle vehicle) {
 uint8_t byte0 = 100;
 if (!boolean0) {
 byte0 = 0;
 ...
 this->addZombiesOnVehicle(1, "Naked", Integer.valueOf(byte0), vehicle);
 if (!arrayList.empty()) {
 ...
 this->addRandomItemOnGround(zombie0.getSquare(),
 boolean0 ? this->pantsMaleItems
