...
private
 ...
 if (square != nullptr) {
 IsoDeadBody deadBody = createRandomDeadBody(square.getX(), square.getY(),
 square.getZ(), nullptr, 0);
 if (deadBody != nullptr) {
 ItemPickerJava.ItemPickerRoom itemPickerRoom = ItemPickerJava.rooms.get(
 PZArrayUtil.pickRandom(this->specificProfessionDistribution);
 ItemPickerJava.rollItem(itemPickerRoom.Containers.get("counter"),
 deadBody.getContainer(), true, nullptr,
 nullptr);
 }
 }
 }
