...
public:
private
 static byte[] SIGNATURE = new byte[]{-119, 80, 78, 71, 13, 10, 26, 10};
...
public
 PNGDecoder(InputStream inputStream, bool boolean0) {
 ...
 bool hasAlphaChannel() { return this->colorType == 6 || this->colorType == 4; }

 bool hasAlpha() {
 return this->hasAlphaChannel() || this->paletteA != nullptr ||
 this->transPixel != nullptr;
 }
 ...
 void overwriteTRNS(uint8_t byte0, uint8_t byte1, uint8_t byte2) {
 ...
 if (bytes[byte3] != byte0 || bytes[byte3 + 1] != byte1 ||
 bytes[byte3 + 2] != byte2) {
...
public
 PNGDecoder.Format decideTextureFormat(PNGDecoder.Format format) {
 ...
 void decode(ByteBuffer byteBuffer, int int3, PNGDecoder.Format format) {
 ...
 byte[] bytes2 = this->bitdepth < 8 ? new byte[this->width + 1] : nullptr;
 this->maskM = 0;
 ...
 void decodeFlipped(ByteBuffer byteBuffer, int int0,
 ...
 void copy(ByteBuffer byteBuffer, byte[] bytes) {
 if (this->bDoMask) {
 uint8_t byte0 = 1;

 for (int int0 = bytes.length; byte0 < int0; byte0 += 4) {
 if (bytes[byte0 + 3] % 255 != 0) {
 this->mask.setValue(this->maskM % this->width, this->maskM / this->width,
 true);
 }
 ...
 void copyRGBtoABGR(ByteBuffer byteBuffer, byte[] bytes) {
 if (this->transPixel != nullptr) {
 uint8_t byte0 = this->transPixel[1];
 uint8_t byte1 = this->transPixel[3];
 uint8_t byte2 = this->transPixel[5];
 uint8_t byte3 = 1;

 for (int int0 = bytes.length; byte3 < int0; byte3 += 3) {
 uint8_t byte4 = bytes[byte3];
 uint8_t byte5 = bytes[byte3 + 1];
 uint8_t byte6 = bytes[byte3 + 2];
 uint8_t byte7 = -1;
 ...
 void copyRGBtoRGBA(ByteBuffer byteBuffer, byte[] bytes) {
 if (this->transPixel != nullptr) {
 uint8_t byte0 = this->transPixel[1];
 uint8_t byte1 = this->transPixel[3];
 uint8_t byte2 = this->transPixel[5];
 ...
 this->mask.setValue(this->maskID % this->width, this->maskID / this->width,
 true);
 this->maskID++;
 }
 ...
 void copyRGBtoBGRA(ByteBuffer byteBuffer, byte[] bytes) {
 if (this->transPixel != nullptr) {
 uint8_t byte0 = this->transPixel[1];
 uint8_t byte1 = this->transPixel[3];
 uint8_t byte2 = this->transPixel[5];
 ...
 void copyRGBAtoABGR(ByteBuffer byteBuffer, byte[] bytes) {
 uint8_t byte0 = 1;

 for (int int0 = bytes.length; byte0 < int0; byte0 += 4) {
 byteBuffer.put(bytes[byte0 + 3])
 .put(bytes[byte0 + 2])
 .put(bytes[byte0 + 1])
 .put(bytes[byte0]);
 }
 ...
 void copyRGBAtoBGRA(ByteBuffer byteBuffer, byte[] bytes) {
 uint8_t byte0 = 1;

 for (int int0 = bytes.length; byte0 < int0; byte0 += 4) {
 byteBuffer.put(bytes[byte0 + 2])
 .put(bytes[byte0 + 1])
 .put(bytes[byte0])
 .put(bytes[byte0 + 3]);
 }
 ...
 void copyRGBAtoRGB(ByteBuffer byteBuffer, byte[] bytes) {
 uint8_t byte0 = 1;
 ...
 void copyPALtoABGR(ByteBuffer byteBuffer, byte[] bytes) {
 if (this->paletteA != nullptr) {
 int int0 = 1;

 for (int int1 = bytes.length; int0 < int1; int0++) {
 int int2 = bytes[int0] & 255;
 uint8_t byte0 = this->palette[int2 * 3 + 0];
 uint8_t byte1 = this->palette[int2 * 3 + 1];
 uint8_t byte2 = this->palette[int2 * 3 + 2];
 uint8_t byte3 = this->paletteA[int2];
 ...
 uint8_t byte4 = this->palette[int5 * 3 + 0];
 uint8_t byte5 = this->palette[int5 * 3 + 1];
 uint8_t byte6 = this->palette[int5 * 3 + 2];
 uint8_t byte7 = -1;
 byteBuffer.put(byte7).put(byte6).put(byte5).put(byte4);
 }
 }
 ...
 void copyPALtoRGBA(ByteBuffer byteBuffer, byte[] bytes) {
 if (this->paletteA != nullptr) {
 int int0 = 1;
 ...
 void copyPALtoBGRA(ByteBuffer byteBuffer, byte[] bytes) {
 if (this->paletteA != nullptr) {
 int int0 = 1;
 ...
 void copyGREYtoRGBA(ByteBuffer byteBuffer, byte[] bytes) {
 int int0 = 1;
 ...
 void copyGREYALPHAtoRGBA(ByteBuffer byteBuffer, byte[] bytes) {
 uint8_t byte0 = 1;

 for (int int0 = bytes.length; byte0 < int0; byte0 += 2) {
 uint8_t byte1 = bytes[byte0];
 uint8_t byte2 = bytes[byte0 + 1];
 byteBuffer.put(byte1).put(byte1).put(byte1).put(byte2);
 }
 }

 void expand4(byte[] bytes1, byte[] bytes0) {
 uint8_t byte0 = 1;
 ...
 void expand2(byte[] bytes1, byte[] bytes0) {
 uint8_t byte0 = 1;
 ...
 void expand1(byte[] bytes1, byte[] bytes0) {
 uint8_t byte0 = 1;
 ...
 void unfilter(byte[] bytes0, byte[] bytes1) {
 switch (bytes0[0]) {
 case 0:
 break;
 case 1:
 ...
 break;
 default:
 throw IOException("invalide filter type in scanline: " + bytes0[0]);
 }
 }

 void unfilterSub(byte[] bytes) {
 ...
 void unfilterUp(byte[] bytes0, byte[] bytes1) {
 int int0 = this->bytesPerPixel;
 int int1 = 1;
 ...
 void unfilterAverage(byte[] bytes1, byte[] bytes0) {
 int int0 = this->bytesPerPixel;
 ...
 bytes1[int1] +=
 (byte)((bytes0[int1] & 255) + (bytes1[int1 - int0] & 255) >>> 1);
 }
 }

 void unfilterPaeth(byte[] bytes1, byte[] bytes0) {
 int int0 = this->bytesPerPixel;
 ...
 void refillInflater(Inflater inflater) {
 ...
 void readChunkUnzip(Inflater inflater, byte[] bytes, int int1, int int2) {
 ...
 throw IOException("Can't inflate " + int2 + " bytes");
 }
 ...
 return bytes[int0] << 24 | (bytes[int0 + 1] & 0xFF) << 16 |
 (bytes[int0 + 2] & 0xFF) << 8 | bytes[int0 + 3] & 0xFF;
 }
...
public
 static enum Format {
 ALPHA(1, true), LUMINANCE(1, false), LUMINANCE_ALPHA(2, true),
 RGB(3, false), RGBA(4, true), BGRA(4, true), ABGR(4, true);

 const int numComponents; const bool hasAlpha;

 private Format(int int1, bool boolean0){this->numComponents = int1;
 this->hasAlpha = boolean0;}

 int getNumComponents() { return this->numComponents; }

 bool isHasAlpha() { return this->hasAlpha; }
}
} // namespace textures
} // namespace core
} // namespace zombie
