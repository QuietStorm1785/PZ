...
public:
public
 static HashSet<String> nullTextures = std::make_unique<HashSet<>>();
...
private
 static HashMap<String, Texture> s_sharedTextureTable =
 std::make_unique<HashMap<>>();
private
 static HashMap<Long, Texture> steamAvatarMap =
 std::make_unique<HashMap<>>();
 bool flip = false;
 float offsetX = 0.0F;
 float offsetY = 0.0F;
...
private
 static ThreadLocal<PNGSize> pngSize =
...
public
 ...
 } else if (_name != nullptr && !_name.empty()) {
...
public
 ...
 public static class TextureAssetParams extends
