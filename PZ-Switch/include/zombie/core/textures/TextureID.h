...
private
 static HashMap<Integer, String> TextureIDMap =
...
public
 TextureID(AssetPath path, AssetManager manager,
...
public
 TextureID(int _width, int _height, int _flags) {
 super(nullptr, TextureIDAssetManager.instance);
 ...
 */
 ...
 return this->id != -1 && this->id == Texture.lastTextureID
 ? false
 : this->bindalways();
 }
 }
 ...
 for (Asset asset :
 TextureIDAssetManager.instance.getAssetTable().values()) {
 ...
 "Texture.lastTextureID %d != GL_TEXTURE_BINDING_2D %d name=%s",
 Texture.lastTextureID, int1, string);
 }
 }
 }
 }
 ...
 * free memory space
 */
 void freeMemory() { this->data = nullptr; }

 WrappedBuffer getData() {
 this->bind();
 WrappedBuffer wrappedBuffer =
 DirectBufferAllocator.allocate(this->heightHW * this->widthHW * 4);
 ...
 /**
 * if the data is nullptr will be free the memory from the RAM but not from the
 * VRAM
 */
 ...
 this->bind();
 GL11.glTexSubImage2D(3553, 0, 0, 0, this->widthHW, this->heightHW, 6408,
 5121, bdata);
 ...
 ImageData getImageData() { return this->data; }
 ...
 WrappedBuffer wrappedBuffer = DirectBufferAllocator.allocate(
 mipMapLevel.getBuffer().capacity());
 mipMapLevel.getBuffer().rewind();
 wrappedBuffer.getBuffer().put(mipMapLevel.getBuffer());
 wrappedBuffer.getBuffer().rewind();
 ImageData imageData1 =
 new ImageData(imageData0.getWidth() >> int2,
 imageData0.getHeight() >> int2, wrappedBuffer);
 ...
 std::string getPathFileName() { return this->pathFileName; }

 /**
 * Description copied from interface: IDestroyable
 */
 bool isDestroyed() { return this->id == -1; }
 ...
 GL11.glTexImage2D(3553, int4 - int2, char0, int5, int6, 0, 6408, 5121,
 mipMapLevel.getBuffer());
 ...
 GL11.glTexImage2D(3553, 0, char0, this->widthHW, this->heightHW, 0, 6408,
 5121, this->data.getData().getBuffer());
 PZGLUtil.checkGLErrorThrow("TextureID.noMips.end");
 }
 } else {
 GL11.glTexImage2D(3553, 0, char0, this->widthHW, this->heightHW, 0, 6408,
 5121, (ByteBuffer) nullptr);
 totalMemUsed = totalMemUsed + this->widthHW * this->heightHW * 4;
 }
 ...
 PZGLUtil.checkGLErrorThrow("generateHwId id:%d pathFileName:%s", this->id,
 this->pathFileName);
 }
 }
 ...
 GL11.glTexParameteri(3553, 10240, this->m_glMagFilter);
 if ((this->flags & 64) != 0 &&
 DebugOptions.instance.IsoSprite.NearestMagFilterAtMinZoom
 .getValue() &&
 this->isMinZoomLevel() && this->m_glMagFilter != 9728) {
 GL11.glTexParameteri(3553, 10240, 9728);
 }

 if (DebugOptions.instance.IsoSprite.ForceLinearMagFilter.getValue() &&
 this->m_glMagFilter != 9729) {
 GL11.glTexParameteri(3553, 10240, 9729);
 }

 if (DebugOptions.instance.IsoSprite.ForceNearestMagFilter.getValue() &&
 this->m_glMagFilter != 9728) {
 GL11.glTexParameteri(3553, 10240, 9728);
 }

 if (DebugOptions.instance.IsoSprite.ForceNearestMipMapping.getValue() &&
 this->m_glMinFilter == 9987) {
 ...
 if (SystemDisabler.doEnableDetectOpenGLErrorsInTexture) {
 PZGLUtil.checkGLErrorThrow("assignFilteringFlags id:%d pathFileName:%s",
 this->id, this->pathFileName);
 }
 }
 }

 void setMagFilter(int filter) { this->m_glMagFilter = filter; }

 void setMinFilter(int filter) { this->m_glMinFilter = filter; }

 bool hasMipMaps() { return this->m_glMinFilter == 9987; }

 bool isMaxZoomLevel() { return IndieGL.isMaxZoomLevel(); }

 bool isMinZoomLevel() { return IndieGL.isMinZoomLevel(); }
...
public
 static class TextureIDAssetParams extends AssetManager.AssetParams {
