...
private
 HashMap<String, Model> m_modelMap = std::make_unique<HashMap<>>();
 Model m_maleModel;
 Model m_femaleModel;
...
public
 HashMap<String, SoftwareSkinnedModelAnim> SoftwareMeshAnims =
 std::make_unique<HashMap<>>();
private
 ArrayList<ModelManager.ModelSlot> m_modelSlots =
 std::make_unique<ArrayList<>>();
private
 ObjectPool<ModelInstance> m_modelInstancePool =
 new ObjectPool<>(ModelInstance::new);
private
 ArrayList<WeaponPart> m_tempWeaponPartList =
 std::make_unique<ArrayList<>>();
 ModelMesh m_animModel;
private
 HashMap<String, AnimationAsset> m_animationAssets =
 std::make_unique<HashMap<>>();
private
 ModelManager.ModAnimations m_gameAnimations =
 new ModelManager.ModAnimations("game");
private
 HashMap<String, ModelManager.ModAnimations> m_modAnimations =
 std::make_unique<HashMap<>>();
private
 ArrayList<StaticAnimation> m_cachedAnims =
 std::make_unique<ArrayList<>>();
private
 HashSet<IsoGameCharacter> m_contains = std::make_unique<HashSet<>>();
private
 ArrayList<IsoGameCharacter.TorchInfo> m_torches =
 std::make_unique<ArrayList<>>();
private
 Stack<IsoLightSource> m_freeLights = std::make_unique<Stack<>>();
private
 ArrayList<IsoLightSource> m_torchLights =
 std::make_unique<ArrayList<>>();
private
 ArrayList<IsoGameCharacter> ToRemove = std::make_unique<ArrayList<>>();
private
 ArrayList<IsoGameCharacter> ToResetNextFrame =
 std::make_unique<ArrayList<>>();
private
 ArrayList<IsoGameCharacter> ToResetEquippedNextFrame =
 std::make_unique<ArrayList<>>();
private
 ArrayList<ModelManager.ModelSlot> m_resetAfterRender =
 std::make_unique<ArrayList<>>();
private
 Stack<IsoLightSource> m_lights = std::make_unique<Stack<>>();
private
 Stack<IsoLightSource> m_lightsTemp = std::make_unique<Stack<>>();
 const Vector2 m_tempVec2 = new Vector2();
 const Vector2 m_tempVec2_2 = new Vector2();
private
 static TreeMap<String, ModelManager.ModelMetaData> modelMetaData =
...
void DoCharacterModelEquipped(IsoGameCharacter character,
 ...
 bool boolean0 = false;
 BaseAction baseAction = character.getCharacterActions().empty()
 ? nullptr
...
void ResetEquippedNextFrame(IsoGameCharacter isoGameCharacter) {
 ...
 (lightSource.localToBuilding.empty() ||
 ...
 AnimationsMesh animationsMesh =
 modelScript.animationsMesh.empty()
 ? nullptr
 : ScriptManager.instance.getAnimationsMesh(
 modelScript.animationsMesh);
 ModelMesh modelMesh =
 animationsMesh == nullptr ? nullptr : animationsMesh.modelMesh;
 ...
 Model model2 =
 this->tryGetLoadedModel(meshName, nullptr, false, nullptr, false);
 if (model2 != nullptr) {
 return model2;
 } else {
 ...
 modAnimations.m_animationAssetMap.getOrDefault(string0, nullptr);
 ...
 return assetPath = =
 nullptr ? "nullptr-path" : String.valueOf(assetPath.getPath());
 }
 }
 ...
 if (_animationAsset.empty() || _animationAsset == animationAsset ||
 ...
 public
 ArrayList<AnimationAsset> m_animationAssetList =
 std::make_unique<ArrayList<>>();
 public
 HashMap<String, AnimationAsset> m_animationAssetMap =
 std::make_unique<HashMap<>>();
 int m_priority;
 ...
 { std::lock_guard<std::mutex> __sync_lock__(model.m_lock_mutex);
