...
private
 ArrayList<AnimationBoneBindingPair> m_reparentedBoneBindings =
 std::make_unique<ArrayList<>>();
private
 List<AnimationBoneBinding> m_twistBones =
 std::make_unique<ArrayList<>>();
 AnimationBoneBinding m_counterRotationBone = nullptr;
public
 ArrayList<Integer> dismembered = std::make_unique<ArrayList<>>();
 const float m_minimumValidAnimWeight = 0.001F;
 const int m_animBlendIndexCacheSize = 32;
private
 int[] m_animBlendIndices = new int[32];
private
 float[] m_animBlendWeights = new float[32];
private
 int[] m_animBlendLayers = new int[32];
private
 int[] m_animBlendPriorities = new int[32];
 const int m_maxLayers = 4;
private
 int[] m_layerBlendCounts = new int[4];
private
 float[] m_layerWeightTotals = new float[4];
...
private
 static AnimationTrack[] tempTracks = new AnimationTrack[0];
 static const Vector2 tempo = new Vector2();
private
 static Pool<AnimationPlayer> s_pool = new Pool<>(AnimationPlayer::new);
...
private
 ...
 bool boolean0 = animationBoneBinding != nullptr &&
 animationBoneBinding.getBone() != nullptr &&
