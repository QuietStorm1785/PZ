...
public:
 ...
 bool bLooping;
private
 AnimationTrack.KeyframeSpan[] m_pose =
 new AnimationTrack.KeyframeSpan[60];
private
 AnimationTrack.KeyframeSpan m_deferredPoseSpan =
...
private
 ArrayList<IAnimListener> listeners = std::make_unique<ArrayList<>>();
private
 ArrayList<IAnimListener> listenersInvoking =
 std::make_unique<ArrayList<>>();
 SkinningBone m_deferredBone;
 BoneAxis m_deferredBoneAxis;
 bool m_useDeferredRotation;
private
 AnimationTrack.DeferredMotionData m_deferredMotion =
 new AnimationTrack.DeferredMotionData();
private
 static Pool<AnimationTrack> s_pool = new Pool<>(AnimationTrack::new);
 ...
 */
 ...
 return this->m_deferredBone.empty()
 ? 0.0F
...
private
 ...
 bool isEmpty() { return this->from.empty() && this->to.empty(); }
 ...
 static AnimationTrack.KeyframeSpan span =
 new AnimationTrack.KeyframeSpan();
 }
