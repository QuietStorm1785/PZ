...
public:
private
 static ObjectPool<WorldItemModelDrawer> s_modelDrawerPool =
 new ObjectPool<>(WorldItemModelDrawer::new);
private
 static ObjectPool<WorldItemModelDrawer.WeaponPartParams>
 s_weaponPartParamPool =
 new ObjectPool<>(WorldItemModelDrawer.WeaponPartParams::new);
private
 static ArrayList<WeaponPart> s_tempWeaponPartList =
 ...
 Model m_model;
private
 ArrayList<WorldItemModelDrawer.WeaponPartParams> m_weaponParts;
 float m_hue;
 float m_tintR;
 float m_tintG;
 float m_tintB;
 ...
 static bool renderMain(InventoryItem item, IsoGridSquare square, float float0,
 ...
 return false;
 } else if (renderAtlasTexture(item, square, float0, float1, float2,
 float3, float4) {
 return true;
 } else {
 if (!StringUtils.isNullOrEmpty(item.getWorldStaticItem())) {
 ModelScript modelScript0 =
 ScriptManager.instance.getModelScript(item.getWorldStaticItem());
 ...
 string0, string1, boolean0, string2, true);
 if (model0.empty()) {
 ModelManager.instance.loadAdditionalModel(string0, string1,
 boolean0, string2);
 }

 model0 = ModelManager.instance.getLoadedModel(string0, string1,
 boolean0, string2);
 if (model0 != nullptr && model0.isReady() &&
 model0.Mesh != nullptr && model0.Mesh.isReady()) {
 WorldItemModelDrawer worldItemModelDrawer0 =
 s_modelDrawerPool.alloc();
 worldItemModelDrawer0.init(item, square, float0, float1, float2,
 model0, modelScript0, float5,
 immutableColor0, float3, false);
 ...
 (!((Clothing)item).isCosmetic() ||
 "Eyes" == item.getBodyLocation()))) {
 bool boolean1 = false;
 ...
 string3, string4, boolean2, string5, false);
 if (model1.empty()) {
 ModelManager.instance.loadAdditionalModel(string3, string4,
 boolean2, string5);
 }

 model1 = ModelManager.instance.getLoadedModel(
 string3, string4, boolean2, string5);
 if (model1 != nullptr && model1.isReady() &&
 model1.Mesh != nullptr && model1.Mesh.isReady()) {
 WorldItemModelDrawer worldItemModelDrawer1 =
 s_modelDrawerPool.alloc();
 float float6 = itemVisual1.getHue(clothingItem1);
 ImmutableColor immutableColor1 =
 itemVisual1.getTint(clothingItem1);
 worldItemModelDrawer1.init(item, square, float0, float1,
 float2, model1, nullptr, float6,
 immutableColor1, float3, false);
 if (NEW_WAY) {
 ...
 worldItemModelDrawer1.m_transform.translate(-0.08F, 0.0F,
 0.05F);
 }
 ...
 ScriptManager.instance.getModelScript(item.getStaticModel());
 ...
 string6, string7, boolean3, string8, false);
 if (model2.empty()) {
 ModelManager.instance.loadAdditionalModel(string6, string7,
 boolean3, string8);
 }

 model2 = ModelManager.instance.getLoadedModel(string6, string7,
 boolean3, string8);
 if (model2 != nullptr && model2.isReady() &&
 model2.Mesh != nullptr && model2.Mesh.isReady()) {
 WorldItemModelDrawer worldItemModelDrawer2 =
 s_modelDrawerPool.alloc();
 float float7 = 1.0F;
 ImmutableColor immutableColor2 = ImmutableColor.white;
 worldItemModelDrawer2.init(item, square, float0, float1, float2,
 model2, modelScript4, float7,
 immutableColor2, float3, true);
 ...
 if (!worldItemModelDrawer2.initWeaponParts((HandWeapon)item,
 modelScript4) {
 ...
 static bool renderAtlasTexture(InventoryItem item, IsoGridSquare square,
 ...
 if (item.atlasTexture != nullptr &&
 !item.atlasTexture.isStillValid(item) {
 ...
 float float7 = IsoWorldInventoryObject.getSurfaceAlpha(
 square, float5 - (int)float5);
 item.atlasTexture.render(float2, float6, tempColorInfo.r,
 tempColorInfo.g, tempColorInfo.b, float7);
...
private
 void init(InventoryItem item, IsoGridSquare square, float float1,
 float float2, float float3, Model model, ModelScript modelScript,
 float float0, ImmutableColor immutableColor, float float4,
 boolean boolean0) {
 this->m_model = model;
 ...
 ModelInstanceRenderData.makeAttachmentTransform(modelAttachment,
 s_attachmentXfrm);
 ...
 this->alpha =
 IsoWorldInventoryObject.getSurfaceAlpha(square, float3 - (int)float3);
 }
 ...
 if (model.empty()) {
 ModelManager.instance.loadAdditionalModel(string1, string2, boolean0,
 string3);
 }

 model = ModelManager.instance.getLoadedModel(string1, string2, boolean0,
 string3);
 if (model != nullptr && model.isReady() && model.Mesh != nullptr &&
 model.Mesh.isReady()) {
 WorldItemModelDrawer.WeaponPartParams weaponPartParams =
 s_weaponPartParamPool.alloc();
 ...
 this->m_transform.mul(weaponPartParams.m_transform,
 ...
 Core.getInstance().DoPushIsoStuff(this->m_x, this->m_y, this->m_z, 0.0F,
 false);
 ...
 WorldItemModelDrawer.WeaponPartParams weaponPartParams =
 this->m_weaponParts.get(int0);
 this->renderModel(weaponPartParams.m_model,
 weaponPartParams.m_transform);
 }
 }
 ...
 shader.setAmbient(this->m_ambientR * 0.4F, this->m_ambientG * 0.4F,
 this->m_ambientB * 0.4F);
 ...
 this->m_ambientB / 4.0F * float0, 5000.0F, Float.NaN,
