...
public:
 ...
 int bodyHair = -1;
private
 byte[] blood = new byte[BloodBodyPartType.MAX.index()];
private
 byte[] dirt = new byte[BloodBodyPartType.MAX.index()];
private
 byte[] holes = new byte[BloodBodyPartType.MAX.index()];
...
private
 static ArrayList<String> itemVisualLocations =
...
public
 ...
 throw IllegalArgumentException("expected HumanVisual, got " +
 other_);
 } else {
