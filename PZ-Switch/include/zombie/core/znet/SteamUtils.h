...
private
 static List<IJoinRequestCallback> m_joinRequestCallbacks;
 ...
 DebugLog.log(
 " Your system may be missing a DLL needed by the game's DLL.");
 DebugLog.log(" You may need to install the Microsoft Visual C++ "
 "Redistributable 2013.");
 File file = new File("../_CommonRedist/vcredist/");
 if (file.exists()) {
 DebugLog.log(" This file is provided in " +
 ...
 { std::lock_guard<std::mutex> __sync_lock__(RenderThread.m_contextLock_mutex);
 if (!m_steamEnabled) {
 ...
 static bool isSteamModeEnabled() { return m_steamEnabled; }
 ...
 static void
 addJoinRequestCallback(IJoinRequestCallback iJoinRequestCallback) {
 m_joinRequestCallbacks.add(iJoinRequestCallback);
 }

 static void
 removeJoinRequestCallback(IJoinRequestCallback iJoinRequestCallback) {
 ...
 static bool showFloatingGamepadTextInput(bool boolean0, int int0, int int1,
 int int2, int int3) {
 if (m_floatingGamepadTextInputVisible) {
 return true;
 } else {
 m_floatingGamepadTextInputVisible = n_ShowFloatingGamepadTextInput(
 boolean0 ? 1 : 0, int0, int1, int2, int3);
...
private
 static boolean n_Init(boolean var0);

private
 static void n_Shutdown();

private
 static void n_RunLoop();

private
 static boolean n_IsOverlayEnabled();

private
 static boolean n_IsSteamRunningOnSteamDeck();

private
 static boolean n_ShowGamepadTextInput(int var0, int var1, String var2,
 int var3, String var4);

private
 static boolean n_ShowFloatingGamepadTextInput(int var0, int var1,
 int var2, int var3,
 int var4);
...
static int
clientInitiateConnectionCallback(long long0) {
 if (CoopSlave.instance.empty()) {
 ServerWorldDatabase.LogonResult logonResult =
 ServerWorldDatabase.instance.authClient(long0);
 return logonResult.bAuthorized ? 0 : 1;
 } else {
 return !CoopSlave.instance.isHost(long0) &&
 !CoopSlave.instance.isInvited(long0)
 ? 2
 : 0;
 }
}
...
static int validateOwnerCallback(long long0, long long1) {
 ...
 ServerWorldDatabase.LogonResult logonResult =
 ServerWorldDatabase.instance.authOwner(long0, long1);
 return logonResult.bAuthorized ? 0 : 1;
 }
}
