...
private
 ...
 bool bHasMap;
private
 ArrayList<String> modIDs = std::make_unique<ArrayList<>>();
private
 ArrayList<String> mapFolders = std::make_unique<ArrayList<>>();
...
std::string validateMapDotInfo(Path var1) { return nullptr; }
...
std::string validateBuildingsFolder(Path var1) { return nullptr; }

std::string validateCreativeFolder(Path var1) { return nullptr; }
...
bool readWorkshopTxt() {
 ...
 if (!string1.empty() && !string1.startsWith("#") &&
 !string1.startsWith("//")) {
 if (string1.startsWith("id=")) {
...
bool writeWorkshopTxt() {
 ...
 "id=" + (this->PublishedFileId.empty() ? "" : this->PublishedFileId);
 bufferedWriter.newLine();
