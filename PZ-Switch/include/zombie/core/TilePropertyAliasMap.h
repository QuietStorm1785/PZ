...
public
 HashMap<String, Integer> PropertyToID = std::make_unique<HashMap<>>();
public
 ArrayList<TilePropertyAliasMap.TileProperty> Properties =
 ...
 TilePropertyAliasMap.TileProperty tileProperty = this->Properties.get(int0);
 return tileProperty.possibleValues.empty()
 ? ""
 : tileProperty.possibleValues.get(int1);
 }

public
 static class TileProperty {
 std::string propertyName;
 public
 ArrayList<String> possibleValues = std::make_unique<ArrayList<>>();
 public
 HashMap<String, Integer> idMap = std::make_unique<HashMap<>>();
 }
}
} // namespace core
} // namespace zombie
