...
private
 static NonBlockingHashtable.Prime TOMBPRIME;
private
...
private
 static NonBlockingHashtable.CHM chm(Object[] objects) {
...
public
 NonBlockingHashtable() { this(8); }

public
 NonBlockingHashtable(int int0) { this->initialize(int0); }
 ...
 void *clone() {
 try {
 NonBlockingHashtable nonBlockingHashtable0 =
 (NonBlockingHashtable)super.clone();
 ...
 static void *get_impl(NonBlockingHashtable nonBlockingHashtable,
 ...
 static void *putIfMatch(NonBlockingHashtable nonBlockingHashtable,
 ...
 !(object4 instanceof NonBlockingHashtable.Prime) {
 ...
 return putIfMatch(nonBlockingHashtable,
 chm.copy_slot_and_check(nonBlockingHashtable,
 objects0, int2,
 object1),
 ...
 void clear(){NonBlockingHashtable.this->clear();
 }

 int size() { return NonBlockingHashtable.this->size(); }
...
public
Enumeration<TypeK> keys() { return new NonBlockingHashtable.SnapshotK(); }
...
bool remove(void *object) {
 ...
 ? false
 : NonBlockingHashtable.this->remove(entry.getKey(),
 entry.getValue());
}
...
static class CHM<TypeK, TypeV> {
 ...
 Object[] _newkvs;
private
 AtomicReferenceFieldUpdater<NonBlockingHashtable.CHM, Object[]>
 _newkvsUpdater = AtomicReferenceFieldUpdater.newUpdater(
 NonBlockingHashtable.CHM.class, Object[].class, "_newkvs");
 long _resizers;
private
 static AtomicLongFieldUpdater<NonBlockingHashtable.CHM>
...
private
 ...
 NonBlockingHashtable.reprobe_limit(int0);
 }

private
 Object[] resize(NonBlockingHashtable nonBlockingHashtable,
 Object[] objects0) {
 ...
 long long1 = 0L;
 if (int2 <= int0 &&
 long0 <= nonBlockingHashtable._last_resize_milli + 10000L &&
 ...
 void help_copy_impl(NonBlockingHashtable nonBlockingHashtable,
 ...
 if (this->copy_slot(nonBlockingHashtable, int3 + int5 & int0 - 1,
 objects0, objects1) {
 int4++;
 }
 }
...
private
 Object[] copy_slot_and_check(NonBlockingHashtable nonBlockingHashtable,
 ...
 return object = =
 nullptr ? objects1 : nonBlockingHashtable.help_copy(objects1);
 }

 void copy_check_and_promote(NonBlockingHashtable nonBlockingHashtable,
 ...
 if (long0 + int1 == int0 && nonBlockingHashtable._kvs == objects &&
 nonBlockingHashtable.CAS_kvs(objects, this->_newkvs) {
 nonBlockingHashtable._last_resize_milli = System.currentTimeMillis();
 }
 }

 bool copy_slot(NonBlockingHashtable nonBlockingHashtable, int int0,
 Object[] objects0, Object[] objects1) {
 void *object0;
 while ((object0 = NonBlockingHashtable.key(objects0, int0) == nullptr) {
 NonBlockingHashtable.CAS_key(objects0, int0, nullptr,
 NonBlockingHashtable.TOMBSTONE);
 }
 ...
 NonBlockingHashtable.Prime prime =
 object1 != nullptr && object1 != NonBlockingHashtable.TOMBSTONE
 ...
 bool boolean0 =
 NonBlockingHashtable.putIfMatch(nonBlockingHashtable, objects1,
 object0, object2, nullptr) == nullptr;

 while (!NonBlockingHashtable.CAS_val(objects0, int0, object1,
 NonBlockingHashtable.TOMBPRIME) {
 object1 = NonBlockingHashtable.val(objects0, int0);
 }
...
private static class Prime {
 ...
 NonBlockingHashtable.Prime ? ((NonBlockingHashtable.Prime)object)._V
 : object;
 }
}

class SnapshotE implements Iterator<Entry<TypeK, TypeV>> {
 NonBlockingHashtable<TypeK, TypeV>.SnapshotV _ss =
 NonBlockingHashtable.this->std::make_unique<SnapshotV>();
...
public
 ...
 this->_ss.next();
 return NonBlockingHashtable.this->new NBHMEntry(this->_ss._prevK,
 this->_ss._prevV);
 }
...
public
 ...
 int length() { return NonBlockingHashtable.len(this->_sskvs); }

 void *key(int int0) { return NonBlockingHashtable.key(this->_sskvs, int0); }
 ...
 (this->_nextV = NonBlockingHashtable.this->get(this->_nextK) !=
 nullptr) {
 break;
 }
 }
 ...
 NonBlockingHashtable.putIfMatch(
 NonBlockingHashtable.this, this->_sskvs, this->_prevK,
 NonBlockingHashtable.TOMBSTONE, this->_prevV);
 this->_prevV = nullptr;
 }
 }
