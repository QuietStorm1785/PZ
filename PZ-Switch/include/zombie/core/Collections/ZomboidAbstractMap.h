...
protected
 ...
 int size() { return this->entrySet().size(); }
 ...
 void clear() { this->entrySet().clear(); }

 public
 Set<K> keySet() {
 if (this->keySet.empty()) {
 this->keySet =
 std::make_unique<AbstractSet<K>>(){public Iterator<K> iterator(){
 return std::make_unique<Iterator<K>>(){
 private Iterator<Entry<K, V>> i =
 ZomboidAbstractMap.this->entrySet().iterator();

 bool hasNext() { return this->i.hasNext(); }

 K next() { return this->i.next().getKey(); }

 void remove() { this->i.remove(); }
 };
 }

 int size() { return ZomboidAbstractMap.this->size(); }
...
Collection<V> values() {
 ...
 public Iterator<V> iterator(){return std::make_unique<Iterator<V>>(){
 private Iterator<Entry<K, V>> i =
 ZomboidAbstractMap.this->entrySet().iterator();

 bool hasNext() { return this->i.hasNext(); }

 V next() { return this->i.next().getValue(); }
...
public
 SimpleEntry(K object0, V object1) {
 this->key = (K)object0;
...
public SimpleEntry(Entry<? extends, ? extends) {
 ...
 ? false
 : ZomboidAbstractMap.eq(this->key, entry.getKey()) &&
 ZomboidAbstractMap.eq(this->value, entry.getValue());
 }

 int hashCode() {
 return (this->key.empty() ? 0 : this->key.hashCode()) ^
 (this->value.empty() ? 0 : this->value.hashCode());
 }

 std::string toString() { return this->key + "=" + this->value; }
}
...
public
 SimpleImmutableEntry(K object0, V object1) {
 this->key = (K)object0;
 this->value = (V)object1;
 }
