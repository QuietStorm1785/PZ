...
public:
 ...
 this->m_vbo =
 new GLVertexBufferObject(7168L, iGLBufferObject.GL_ARRAY_BUFFER(),
 iGLBufferObject.GL_STREAM_DRAW());
 this->m_vbo.create();
 this->m_ibo = new GLVertexBufferObject(
 512L, iGLBufferObject.GL_ELEMENT_ARRAY_BUFFER(),
 iGLBufferObject.GL_STREAM_DRAW());
 this->m_ibo.create();
 }
...
public
 void addTriangle(float float0, float float1, float float2, float float7,
 float float8, float float9, float float10, float float11,
 float float12, float float3, float float4, float float5,
 float float6) {
 this->reserve(3);
 this->addElement(float0, float1, float2, float3, float4, float5, float6);
 this->addElement(float7, float8, float9, float3, float4, float5, float6);
 ...
 float float8) {
 this->reserve(6);
 this->addTriangle(float0, float1, float2, float3, float1, float2, float0,
 float4, float2, float5, float6, float7, float8);
 this->addTriangle(float3, float1, float2, float3, float4, float2, float0,
 float4, float2, float5, float6, float7, float8);
 }
 ...
 return this->m_mode == 4 && this->m_elements.position() % 84 == 0 &&
 this->m_elements.position() + 84 > 7168
 ? true
 ...
 return this->m_elements.empty() ||
 this->m_elements.position() / 28 + int0 <= 256;
 }
 ...
 GL12.glDrawRangeElements(this->m_mode, byte0, byte0 + int1, int2 - byte1,
 5123, byte1 * 2);
 this->m_vbo.bindNone();
 this->m_ibo.bindNone();
