...
private
 ArrayList<ShaderUnit> m_vertexUnits = std::make_unique<ArrayList<>>();
private
 ArrayList<ShaderUnit> m_fragmentUnits = std::make_unique<ArrayList<>>();
private
 HashMap<String, PredicatedFileWatcher> m_fileWatchers =
 std::make_unique<HashMap<>>();
 bool m_sourceFilesChanged = false;
 bool m_compileFailed = false;
private
 HashMap<String, ShaderProgram.Uniform> uniformsByName =
 std::make_unique<HashMap<>>();
private
 ArrayList<IShaderProgramListener> m_onCompiledListeners =
 std::make_unique<ArrayList<>>();
private
 int[] m_uvScaleUniforms = new int[10];
 ...
 * @param isStatic
 * @param compile
 * @return The created shader program. Non-nullptr, but it may or may not have
...
public
 ShaderProgram.Uniform getUniform(String loc, int type) {
