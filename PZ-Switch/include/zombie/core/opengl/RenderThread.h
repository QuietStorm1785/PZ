...
public:
 ...
 std::mutex invokeOnRenderQueue_mutex;
 std::mutex m_contextLock_mutex;
 std::mutex m_initLock_mutex;
...
private
 static ArrayList<RenderContextQueueItem> invokeOnRenderQueue =
 std::make_unique<ArrayList<>>();
private
 static ArrayList<RenderContextQueueItem> invokeOnRenderQueue_Invoking =
 std::make_unique<ArrayList<>>();
 static bool m_isInitialized = false;
 static const void *m_initLock = "RenderThread Initialization Lock";
private
 static boolean m_isCloseRequested = false;
private
 static int m_displayWidth;
private
 static int m_displayHeight;
private
 static boolean m_renderingEnabled = true;
private
 static boolean m_waitForRenderState = false;
private
 static boolean m_hasContext = false;
 static bool m_cursorVisible = true;

 static void init() {
 { std::lock_guard<std::mutex> __sync_lock__(m_initLock_mutex);
 if (!m_isInitialized) {
 ...
 MainThread = Thread.currentThread();
 RenderThread = new Thread(ThreadGroups.Main, RenderThread::renderLoop,
 ...
 { std::lock_guard<std::mutex> __sync_lock__(m_contextLock_mutex);
 if (!m_hasContext) {
 acquireContextReentrant();
 }
 ...
 RenderThread.s_performance.renderStep.invokeAndMeasure(
 ...
 " GameThread failed to exit within time limit.");
 break;
 }
 ...
 { std::lock_guard<std::mutex> __sync_lock__(invokeOnRenderQueue_mutex);
 invokeOnRenderQueue_Invoking.addAll(invokeOnRenderQueue);
 invokeOnRenderQueue.clear();
 }

 PZArrayUtil.forEach(invokeOnRenderQueue_Invoking,
 RenderContextQueueItem::notifyWaitingListeners);
 }
 }
 }
 ...
 DebugLog.General.error("Thrown an " + exception.getClass().getTypeName() +
 ...
 SpriteRenderer.instance.acquireStateForRendering(
 RenderThread::waitForRenderStateCallback);
 ...
 notifyRenderStateQueue();
 if (!m_waitForRenderState ||
 LuaManager.thread != nullptr && LuaManager.thread.bStep) {
 RenderThread.s_performance.displayUpdate.invokeAndMeasure(
 ()->Display.processMessages());
 }
 ...
 (m_waitForRenderState ||
 SpriteRenderer.instance.isWaitingForRenderState());
 }

 static bool isWaitForRenderState() { return m_waitForRenderState; }
 ...
 RenderContextQueueItem renderContextQueueItem0 =
 invokeOnRenderQueue_Invoking.remove(0);
 ...
 for (int int0 = invokeOnRenderQueue_Invoking.size() - 1; int0 >= 0;
 int0--) {
 RenderContextQueueItem renderContextQueueItem1 =
 invokeOnRenderQueue_Invoking.get(int0);
 if (renderContextQueueItem1.isWaiting()) {
 while (int0 >= 0) {
 RenderContextQueueItem renderContextQueueItem2 =
 invokeOnRenderQueue_Invoking.remove(0);
 ...
 DebugLog.General.error("OpenGLException thrown: " +
 openGLException.getMessage());

 for (int int0 = GL11.glGetError(); int0 != 0; int0 = GL11.glGetError()) {
 std::string string = Util.translateGLErrorString(int0);
 DebugLog.General.error(" Also detected error: " + string +
 " ( code:" + int0 + ")");
 }
 ...
 { std::lock_guard<std::mutex> __sync_lock__(m_contextLock_mutex); acquireContextReentrantInternal(); }
 }

 static void releaseContextReentrant() {
 { std::lock_guard<std::mutex> __sync_lock__(m_contextLock_mutex); releaseContextReentrantInternal(); }
 }

 static void acquireContextReentrantInternal() {
 Thread thread = Thread.currentThread();
 if (ContextThread != nullptr && ContextThread != thread) {
 throw RuntimeException("Context thread mismatch: " + ContextThread +
 ", " + thread);
 } else {
 ...
 DebugLog.General.error(
 "Exception thrown trying to gain GL context.");
 ...
 throw RuntimeException("Context thread release overflow: 0: " +
 ContextThread + ", " + thread);
 } else {
 m_contextLockReentrantDepth--;
 ...
 DebugLog.General.error(
 "Exception thrown trying to release GL context.");
 lWJGLException.printStackTrace();
 }
 }
 ...
 RenderContextQueueItem.alloc(runnable);
 ...
 DebugLog.General.error(
 "Thread Interrupted while waiting for queued item to finish:" +
...
public
 static<T1> void
 invokeOnRenderContext(T1 object, Invokers.Params1.ICallback<T1> iCallback) {
 Lambda.capture(
 object, iCallback,
 (genericStack, objectx, iCallbackx)
 ->invokeOnRenderContext(genericStack.invoker(objectx, iCallbackx));
 }

public
 static<T1, T2> void
 invokeOnRenderContext(T1 object0, T2 object1,
 Invokers.Params2.ICallback<T1, T2> iCallback) {
 Lambda.capture(object0, object1, iCallback,
 (genericStack, object0x, object1x, iCallbackx)
 ->invokeOnRenderContext(genericStack.invoker(
 object0x, object1x, iCallbackx));
 }

public
 static<T1, T2, T3> void
 invokeOnRenderContext(T1 object0, T2 object1, T3 object2,
 Invokers.Params3.ICallback<T1, T2, T3> iCallback) {
 Lambda.capture(object0, object1, object2, iCallback,
 (genericStack, object0x, object1x, object2x, iCallbackx)
 ->invokeOnRenderContext(genericStack.invoker(
 object0x, object1x, object2x, iCallbackx));
 }

public
 static<T1, T2, T3, T4> void
 invokeOnRenderContext(T1 object0, T2 object1, T3 object2, T4 object3,
 Invokers.Params4.ICallback<T1, T2, T3, T4> iCallback) {
 Lambda.capture(
 object0, object1, object2, object3, iCallback,
 (genericStack, object0x, object1x, object2x, object3x, iCallbackx)
 ->invokeOnRenderContext(genericStack.invoker(
 object0x, object1x, object2x, object3x, iCallbackx));
 }
 ...
 static bool isRunning() { return m_isInitialized; }

 static void startRendering() { m_renderingEnabled = true; }
 ...
 static bool isCursorVisible() { return m_cursorVisible; }

private
 static class s_performance {
 static const PerformanceProfileFrameProbe renderStep =
 new PerformanceProfileFrameProbe("RenderThread.renderStep");
 static const PerformanceProfileProbe displayUpdate =
 new PerformanceProfileProbe("Display.update(true)");
 static const PerformanceProfileProbe spriteRendererPostRender =
 new PerformanceProfileProbe("SpriteRenderer.postRender");
 }
}
} // namespace opengl
} // namespace core
