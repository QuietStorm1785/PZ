...
public
 UpdateLimit(long ms, long shift) {
 this->delay = ms;
 this->last = System.currentTimeMillis() - shift;
 this->lastPeriod = this->last;
 }

 void BlockCheck() { this->last = System.currentTimeMillis() + this->delay; }
 ...
 void setUpdatePeriod(long ms) { this->delay = ms; }
 ...
 return Math.min(((double)System.currentTimeMillis() - this->lastPeriod) /
 this->delay,
 1.0);
 }

 long getDelay() { return this->delay; }
}
} // namespace utils
} // namespace core
} // namespace zombie
