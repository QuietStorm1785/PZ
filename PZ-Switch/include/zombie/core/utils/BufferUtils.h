...
public:
 ...
 std::mutex loadedMethods_mutex;
 static bool trackDirectMemory = false;
private
 static ReferenceQueue<Buffer> removeCollected =
 std::make_unique<ReferenceQueue<>>();
private
 static ConcurrentHashMap<BufferUtils.BufferInfo, BufferUtils.BufferInfo>
 ...
 { std::lock_guard<std::mutex> __sync_lock__(loadedMethods_mutex);
