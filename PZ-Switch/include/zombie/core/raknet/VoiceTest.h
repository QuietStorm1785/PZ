...
public:
 ...
 System.out.println("RakNet.Startup() return code: " + int1 +
 " (0 means success)");
 }
 ...
 rnclientPeer.SetClientPort(GameServer.DEFAULT_PORT + Rand.Next(10000) +
 1234);
 rnclientPeer.SetOccasionalPing(true);
 int int0 = rnclientPeer.Startup(byte0);
 System.out.println("RakNet.Startup() return code: " + int0 +
 " (0 means success)");
 }
 ...
 void run(){while (!VoiceTest.bQuit && !VoiceTest.bQuit){
