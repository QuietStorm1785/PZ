...
public:
 ...
 if ((!GameClient.bClient || GameClient.connection.empty()) &&
 ...
 void UpdateVMClient() throws InterruptedException {
 ...
 { std::lock_guard<std::mutex> __sync_lock__(voiceManagerData0.radioData_mutex);
 for (int int0 = 1; int0 < voiceManagerData0.radioData.size();
 int0++) {
 { std::lock_guard<std::mutex> __sync_lock__(voiceManagerData1.radioData_mutex);
 ...
 { std::lock_guard<std::mutex> __sync_lock__(voiceManagerData0.radioData_mutex);
 { std::lock_guard<std::mutex> __sync_lock__(voiceManagerData1.radioData_mutex);
 if (!voiceManagerData1.radioData.empty() &&
 !voiceManagerData0.radioData.empty()) {
 ...
 nullptr ? 0 : (int)FMODReceiveBuffer.get_loudness();
 }

 bool getMicVolumeError() {
 return FMODReceiveBuffer = =
 nullptr ? true : FMODReceiveBuffer.get_interror();
 }
