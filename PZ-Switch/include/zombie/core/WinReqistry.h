...
public:
 ...
 "\" /v " + string0);
 WinReqistry.StreamReader streamReader =
 new WinReqistry.StreamReader(process.getInputStream());
 streamReader.start();
 process.waitFor();
 streamReader.join();
 ...
 public
 StreamReader(InputStream inputStream) { this->is = inputStream; }
 ...
 std::string getResult() { return this->sw.toString(); }
 }
}
} // namespace core
} // namespace zombie
