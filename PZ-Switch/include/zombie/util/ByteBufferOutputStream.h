...
public
 ByteBufferOutputStream(ByteBuffer byteBuffer, bool boolean0) {
 ...
 ByteBuffer getWrappedBuffer() { return this->wrappedBuffer; }

 void clear() { this->wrappedBuffer.clear(); }

 void flip() { this->wrappedBuffer.flip(); }

 void growTo(int int2) {
 ...
 int int1 =
 Math.max(this->wrappedBuffer.capacity() * 2, int0 + bytes.length);
 this->growTo(int1);
 this->write(bytes);
 }
 }

 void write(byte[] bytes, int int1, int int2) {
 int int0 = 0;
