...
public
 GameSoundClip(GameSound _gameSound) {
 ...
 float getMinDistance() { return this->distanceMin; }

 float getMaxDistance() { return this->distanceMax; }
 ...
 if (gameSoundClip2.event != nullptr &&
 gameSoundClip2.event == this->event) {
 ...
 return this->eventDescription != nullptr &&
 this->eventDescription.bHasSustainPoints;
 }

 bool hasParameter(FMOD_STUDIO_PARAMETER_DESCRIPTION parameterDescription) {
 return this->eventDescription != nullptr &&
 this->eventDescription.hasParameter(parameterDescription);
 }
}
} // namespace audio
} // namespace zombie
