...
private
 IsoGameCharacter.Location delayedSound =
...
private
 ...
 bool bRemote;
private
 static IsoZombie.FloodFill floodFill = new IsoZombie.FloodFill();
 ...
 bool isRemoteZombie() { return this->authOwner.empty(); }
...
public
 ...
 if (vehicle != nullptr) {
 if (this->target.empty() && vehicle.hasLightbar() &&
 ...
 if (!this->bCrawling && this->target.empty() &&
 this->eatBodyTarget.empty()) {
 ...
 !this->itemsToSpawnAtDeath.empty()) {
 ...
 if (!GameClient.bClient || this->target.empty() ||
 wielder == this->target ||
 ...
 _sharedDesc.empty() ? 0 : _sharedDesc.getPersistentOutfitID();
...
private
 ...
 Mover mover = nullptr;
 private
 ArrayList<IsoGridSquare> choices = new ArrayList<>(121);
 ...
 int radius; private static IsoZombie.ZombieSound[] values = values();
