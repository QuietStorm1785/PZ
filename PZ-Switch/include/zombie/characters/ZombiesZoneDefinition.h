...
public:
private
 static ArrayList<ZombiesZoneDefinition.ZZDZone> s_zoneList =
 std::make_unique<ArrayList<>>();
private
 static HashMap<String, ZombiesZoneDefinition.ZZDZone> s_zoneMap =
 std::make_unique<HashMap<>>();
 static bool bDirty = true;
private
 static ZombiesZoneDefinition.PickDefinition pickDef =
 new ZombiesZoneDefinition.PickDefinition();
private
 static HashMap<String, ZombiesZoneDefinition.ZZDOutfit>
...
dressInRandomOutfit(IsoZombie zombie0) {
 ...
 std::string string =
 square.getRoom() == nullptr ? nullptr : square.getRoom().getName();
...
pickDefinition(int int0, int int1, int int2, boolean boolean0) {
 ...
 if (int4 == -1 ||
 zone.spawnSpecialZombies.empty() && arrayList0.size() < int4) {
...
static Outfit getRandomDefaultOutfit(bool boolean0,
 ...
 (string0.empty() || "male".equalsIgnoreCase(string0) && !boolean0 ||
...
private static class ZZDZone {
 ...
 int chanceToSpawn;
 int toSpawn;
 ArrayList<ZombiesZoneDefinition.ZZDOutfit> outfits =
 std::make_unique<ArrayList<>>();
}
}
} // namespace characters
