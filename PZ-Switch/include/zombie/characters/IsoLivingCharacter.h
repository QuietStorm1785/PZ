...
public
 ...
 super(cell, x, y, z);
 this->bareHands =
 (HandWeapon)InventoryItemFactory.CreateItem("Base.BareHands");
 }

 bool isCollidedWithPushableThisFrame() { return this->bCollidedWithPushable; }
 ...
 if (this->useHandWeapon.isCantAttackWithLowestEndurance() &&
 int0 == 4) {
 return false;
 }
 ...
 if (this instanceof IsoSurvivor && this->useHandWeapon.isRanged() &&
 int1 < this->useHandWeapon.getMaxHitCount()) {
 for (int int3 = 0; int3 < this->getCell().getObjectList().size();
 int3++) {
 IsoMovingObject movingObject =
 this->getCell().getObjectList().get(int3);
 if (movingObject != this && movingObject.isShootable() &&
 this->IsAttackRange(movingObject.getX(), movingObject.getY(),
 movingObject.getZ())) {
 float float1 = 1.0F;
 if (float1 > 0.0F) {
 Vector2 vector0 = new Vector2(this->getX(), this->getY());
 Vector2 vector1 =
 new Vector2(movingObject.getX(), movingObject.getY());
 ...
 if (float2 >= this->useHandWeapon.getMinAngle() &&
 float2 <= this->useHandWeapon.getMaxAngle()) {
 int1++;
 }
 ...
 ((IsoMovingObject)UIManager.getPicked().tile)
 .getCurrentSquare();
 }
 }

 if (this->useHandWeapon.getAmmoType() != nullptr &&
 !this->inventory.contains(this->useHandWeapon.getAmmoType())) {
 return false;
 }
 ...
 this->useHandWeapon.getOtherHandRequire())) {
 if (!this->useHandWeapon.isRanged()) {
 this->getEmitter().playSound(this->useHandWeapon.getSwingSound(),
 this);
 ...
 IsoPlayer.getInstance().inventory.RemoveOneOf(
 this->useHandWeapon.getAmmoType());
 } else {
 this->inventory.RemoveOneOf(this->useHandWeapon.getAmmoType());
 }
 }

 if (this->useHandWeapon.isUseSelf() &&
 this->leftHandItem != nullptr) {
 this->leftHandItem.Use();
 }

 if (this->useHandWeapon.isOtherHandUse() &&
 this->rightHandItem != nullptr) {
 ...
 bool isDoShove() { return this->bDoShove; }

 void setDoShove(bool _bDoShove) { this->bDoShove = _bDoShove; }
}
} // namespace characters
} // namespace zombie
