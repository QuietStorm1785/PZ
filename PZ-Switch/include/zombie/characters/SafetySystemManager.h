...
public:
private
 static LinkedHashMap<String, Float> playerCooldown =
 std::make_unique<LinkedHashMap<>>();
private
 static LinkedHashMap<String, Boolean> playerSafety =
 std::make_unique<LinkedHashMap<>>();
private
 static LinkedHashMap<String, Long> playerDelay =
 std::make_unique<LinkedHashMap<>>();
 ...
 PZMath.fastfloor(player.getY())) != nullptr;
 if (!boolean0) {
