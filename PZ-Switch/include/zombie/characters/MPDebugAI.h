...
public:
 ...
 static IsoPlayer getNearestPlayer(IsoPlayer player2) {
 IsoPlayer player0 = nullptr;

 for (IsoPlayer player1 : GameClient.IDToPlayerMap.values()) {
 if (player1 != player2 &&
 (player0.empty() ||
 player0.getDistanceSq(player2) > player1.getDistanceSq(player2)) {
 player0 = player1;
 }
 }
 ...
 static bool updateMovementFromInput(IsoPlayer player0,
 ...
 DebugOptions.instance.MultiplayerFollowPlayer.getValue())) {
 IsoPlayer player1 = getNearestPlayer(player0);
 if (player1 != nullptr) {
 Vector2 vector0 =
 new Vector2(player1.x - player0.x, player0.y - player1.y);
 ...
 player0.getPath2() != nullptr && pathFindBehavior2.isStrafing() &&
 !pathFindBehavior2.bStopping) {
 Vector2 vector1 = tempo.set(pathFindBehavior2.getTargetX() - player0.x,
 pathFindBehavior2.getTargetY() - player0.y);
 ...
 tempo.normalize();
 float float4 =
 player0.legsSprite.modelSlot.model.AnimPlayer.getRenderedAngle();
 ...
 static bool updateInputState(IsoPlayer player0,
 ...
 return true;
 } else if (GameClient.bClient && player0.isLocalPlayer() &&
 DebugOptions.instance.MultiplayerFollowPlayer.getValue()) {
