...
public:
 static const AttachedWeaponDefinitions instance =
 new AttachedWeaponDefinitions();
 bool m_dirty = true;
 int m_chanceOfAttachedWeapon;
public
 ArrayList<AttachedWeaponDefinition> m_definitions =
 std::make_unique<ArrayList<>>();
public
 ArrayList<AttachedWeaponCustomOutfit> m_outfitDefinitions =
 std::make_unique<ArrayList<>>();
 ...
 AttachedWeaponDefinitions.L_addRandomAttachedWeapon.definitions;
 arrayList.clear();
 int int0 = 1;
 ...
 int0 = attachedWeaponCustomOutfit.maxitem > -1
 ? attachedWeaponCustomOutfit.maxitem
 : 1;
 break;
 }
 ...
 this->pickRandomInList(arrayList, zombie0);
 ...
 OutfitRNG.Next(100) >= attachedWeaponCustomOutfit.chance) {
 return;
 }
 }
 }
 ...
 void addAttachedWeapon(AttachedWeaponDefinition attachedWeaponDefinition,
 IsoZombie zombie0) {
 ...
 item0.setCondition(OutfitRNG.Next(
 Math.max(2, item0.getConditionMax() - 5), item0.getConditionMax()));
 zombie0.setAttachedItem(
 OutfitRNG.pickRandom(attachedWeaponDefinition.weaponLocation), item0);
 if (attachedWeaponDefinition.ensureItem != nullptr &&
 !this->outfitHasItem(zombie0, attachedWeaponDefinition.ensureItem) {
 Item item1 = ScriptManager.instance.FindItem(
 attachedWeaponDefinition.ensureItem);
 if (item1 != nullptr && item1.getClothingItemAsset() != nullptr) {
 zombie0.getHumanVisual().addClothingItem(zombie0.getItemVisuals(),
 item1);
 } else {
 zombie0.addItemToSpawnAtDeath(InventoryItemFactory.CreateItem(
 attachedWeaponDefinition.ensureItem);
 }
 }
 ...
 for (int int0 = 0;
 int0 < attachedWeaponDefinition.bloodLocations.size(); int0++) {
 BloodBodyPartType bloodBodyPartType =
 attachedWeaponDefinition.bloodLocations.get(int0);
 ...
 pickRandomInList(ArrayList<AttachedWeaponDefinition> arrayList1,
 IsoZombie zombie0) {
 AttachedWeaponDefinition attachedWeaponDefinition0 = nullptr;
 int int0 = 0;
 std::vector arrayList0 =
 AttachedWeaponDefinitions.L_addRandomAttachedWeapon.possibilities;
 arrayList0.clear();

 for (int int1 = 0; int1 < arrayList1.size(); int1++) {
 AttachedWeaponDefinition attachedWeaponDefinition1 =
 (AttachedWeaponDefinition)arrayList1.get(int1);
 if (attachedWeaponDefinition1.daySurvived > 0) {
 if (IsoWorld.instance.getWorldAgeDays() >
 attachedWeaponDefinition1.daySurvived) {
 ...
 if (zombie0.getHumanVisual().getOutfit() != nullptr &&
 attachedWeaponDefinition1.outfit.contains(
 zombie0.getHumanVisual().getOutfit().m_Name) {
 int0 += attachedWeaponDefinition1.chance;
 arrayList0.add(attachedWeaponDefinition1);
 }
 } else {
 ...
 (AttachedWeaponDefinition)arrayList0.get(int4);
 ...
 (KahluaTableImpl)LuaManager.env.rawget("AttachedWeaponDefinitions");
 if (kahluaTableImpl0 != nullptr) {
 this->m_chanceOfAttachedWeapon =
 kahluaTableImpl0.rawgetInt("chanceOfAttachedWeapon");
 ...
 AttachedWeaponDefinition attachedWeaponDefinition =
 this->init((String)entry0.getKey(), kahluaTableImpl1);
 ...
 Collections.sort(this->m_definitions,
 (attachedWeaponDefinition1, attachedWeaponDefinition0)
 ->attachedWeaponDefinition1.id.compareTo(
 attachedWeaponDefinition0.id);
 }
 }
 ...
 this->tableToArrayList(kahluaTableImpl0, "outfit",
 attachedWeaponDefinition.outfit);
 this->tableToArrayList(kahluaTableImpl0, "weaponLocation",
 attachedWeaponDefinition.weaponLocation);
 KahluaTableImpl kahluaTableImpl1 =
 (KahluaTableImpl)kahluaTableImpl0.rawget("bloodLocations");
 if (kahluaTableImpl1 != nullptr) {
 KahluaTableIterator kahluaTableIterator = kahluaTableImpl1.iterator();

 while (kahluaTableIterator.advance()) {
 BloodBodyPartType bloodBodyPartType = BloodBodyPartType.FromString(
 kahluaTableIterator.getValue().toString());
 ...
 attachedWeaponDefinition.daySurvived =
 kahluaTableImpl0.rawgetInt("daySurvived");
 attachedWeaponDefinition.ensureItem =
 kahluaTableImpl0.rawgetStr("ensureItem");
 this->tableToArrayList(kahluaTableImpl0, "weapons",
 attachedWeaponDefinition.weapons);
...
private
 static class L_addRandomAttachedWeapon {
 static ArrayList<AttachedWeaponDefinition> possibilities =
 std::make_unique<ArrayList<>>();
 static ArrayList<AttachedWeaponDefinition> definitions =
 std::make_unique<ArrayList<>>();
 }
}
} // namespace AttachedItems
