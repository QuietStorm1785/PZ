...
public
 Safety(IsoGameCharacter _character) {
 ...
 void *getCharacter() { return this->character; }
 ...
 void setLast(bool _last) { this->last = _last; }

 float getCooldown() { return this->cooldown; }

 void setCooldown(float _cooldown) { this->cooldown = _cooldown; }

 float getToggle() { return this->toggle; }

 void setToggle(float _toggle) { this->toggle = _toggle; }

 bool isToggleAllowed() {
 return ServerOptions.getInstance().PVP.getValue() &&
 NonPvpZone.getNonPvpZone(PZMath.fastfloor(this->character.getX()),
 PZMath.fastfloor(this->character.getY())) ==
 nullptr &&
 (!ServerOptions.getInstance().SafetySystem.getValue() ||
 this->getCooldown() == 0.0F && this->getToggle() == 0.0F);
 }
 ...
 this->setToggle(
 ServerOptions.getInstance().SafetyToggleTimer.getValue());
 ...
 return "enabled=" + this->enabled + " last=" + this->last +
 " cooldown=" + this->cooldown + " toggle=" + this->toggle;
 }
}
} // namespace characters
} // namespace zombie
