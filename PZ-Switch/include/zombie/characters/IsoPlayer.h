...
public
 static IsoPlayer[] players = new IsoPlayer[4];
...
private
 static Stack<String> StaticTraits = std::make_unique<Stack<>>();
...
private
 static PredicatedFileWatcher m_isoPlayerTriggerWatcher =
...
protected
 ...
 int followID = 0;
protected
 Stack<IsoGameCharacter> FollowCamStack = std::make_unique<Stack<>>();
 bool bSeenThisFrame = false;
 bool bCouldBeSeenThisFrame = false;
 float AsleepTime = 0.0F;
protected
 Stack<IsoMovingObject> spottedList = std::make_unique<Stack<>>();
...
private
 IsoPlayer.InputState inputState = new IsoPlayer.InputState();
...
private
 HashMap<Long, Long> mechanicsItem = std::make_unique<HashMap<>>();
 ...
 long steamID;
private
 IsoPlayer.VehicleContainerData vehicleContainerData =
...
private
 static ArrayList<IsoPlayer> RecentlyRemoved =
 std::make_unique<ArrayList<>>();
 bool pathfindRun = false;
private
 static IsoPlayer.MoveVars s_moveVars = new IsoPlayer.MoveVars();
 int atkTimer = 0;
private
 static ArrayList<HitInfo> s_targetsProne =
 std::make_unique<ArrayList<>>();
private
 static ArrayList<HitInfo> s_targetsStanding =
 std::make_unique<ArrayList<>>();
 ...
 * the IsoPlayer.instance is non-nullptr. Performs this in a thread-safe
 ...
 */
 ...
 { std::lock_guard<std::mutex> __sync_lock__(instanceLock_mutex);
 if (instance != nullptr) {
 callback.run();
 }
 }
 ...
 { std::lock_guard<std::mutex> __sync_lock__(instanceLock_mutex); instance = newInstance; }
 }
...
public
 ...
 return IsoWorld.instance.empty() ||
 IsoWorld.instance.AddCoopPlayers.empty();
 }
...
public
 ...
 SafeHouse safeHouse =
 SafeHouse.isSafeHouse(this->getCurrentSquare(), nullptr, false);
 ...
 if (this->isAiming() &&
 (GameWindow.ActivatedJoyPad.empty() ||
 this->JoypadBind == -1) {
 ...
 HitInfo hitInfo0 =
 s_targetsStanding.empty() ? nullptr : s_targetsStanding.get(0);
 HitInfo hitInfo1 =
 s_targetsProne.empty() ? nullptr : s_targetsProne.get(0);
 ...
 IsoZombie && ((IsoZombie)character).target.empty()) {
 int2 += 30;
 } else {
 int2 += 5;
 }
...
void updateLOS() {
 ...
 long long0 =
 this->getEmitter().playSoundImpl("ZombieSurprisedPlayer", nullptr);
...
void updateHeartSound() {
 ...
 bool boolean0 = gameSound != nullptr && gameSound.getUserVolume() > 0.0F &&
...
void checkVehicleContainers() {
 ...
 this->vehicleContainerData.freeContainers.empty()
