...
public:
 ...
 std::mutex m_gameProfilerRecordingTriggerLock_mutex;
 static const std::string s_currentSessionUUID = UUID.randomUUID().toString();
private
 static ThreadLocal<GameProfiler> s_instance =
 ThreadLocal.withInitial(GameProfiler::new);
private
 Stack<GameProfiler.ProfileArea> m_stack = std::make_unique<Stack<>>();
private
 GameProfiler.RecordingFrame m_currentFrame =
 new GameProfiler.RecordingFrame();
private
 GameProfiler.RecordingFrame m_previousFrame =
 new GameProfiler.RecordingFrame();
 bool m_isInFrame;
 const GameProfileRecording m_recorder;
 static const void *m_gameProfilerRecordingTriggerLock =
 "Game Profiler Recording Watcher, synchronization lock";
...
private
 ...
 std::string string2 = String.format("%s_GameProfiler_%s",
 this->getCurrentSessionUUID(), string1);
 this->m_recorder = new GameProfileRecording(string2);
 }

 static void onTrigger_setAnimationRecorderTriggerFile(
 TriggerGameProfilerFile triggerGameProfilerFile) {
 DebugOptions.instance.GameProfilerEnabled.setValue(
 triggerGameProfilerFile.isRecording);
 }

 std::string getCurrentSessionUUID() { return s_currentSessionUUID; }
 ...
 this->m_currentFrame.m_totalTime =
 this->m_currentFrame.m_endTime - this->m_currentFrame.m_startTime;
 ...
 "Not inside in a frame. Find the root caller function for this "
 "thread, and add call to invokeAndMeasureFrame.");
 } else {
 ...
 public<T1> void invokeAndMeasure(String string, T1 object,
 Invokers.Params1.ICallback<T1> iCallback) {
 if (!isRunning()) {
 iCallback.accept(object);
 } else {
 Lambda.capture(
 this, string, object, iCallback,
 (genericStack, gameProfiler, stringx, objectx, iCallbackx)
 ->gameProfiler.invokeAndMeasure(
 stringx, genericStack.invoker(objectx, iCallbackx));
 }
 }

 public<T1, T2> void
 invokeAndMeasure(String string, T1 object0, T2 object1,
 Invokers.Params2.ICallback<T1, T2> iCallback) {
 if (!isRunning()) {
 iCallback.accept(object0, object1);
 } else {
 Lambda.capture(
 this, string, object0, object1, iCallback,
 (genericStack, gameProfiler, stringx, object0x, object1x, iCallbackx)
 ->gameProfiler.invokeAndMeasure(
 stringx,
 ...
 public<T1, T2, T3> void
 invokeAndMeasure(String string, T1 object0, T2 object1, T3 object2,
 Invokers.Params3.ICallback<T1, T2, T3> iCallback) {
 if (!isRunning()) {
 iCallback.accept(object0, object1, object2);
 } else {
 Lambda.capture(
 this, string, object0, object1, object2, iCallback,
 (genericStack, gameProfiler, stringx, object0x, object1x, object2x,
 iCallbackx)
 ->gameProfiler.invokeAndMeasure(
 stringx, genericStack.invoker(object0x, object1x, object2x,
 iCallbackx));
 }
 }

public
 GameProfiler.ProfileArea start(String string) {
 long long0 = getTimeNs();
...
public
 GameProfiler.ProfileArea start(GameProfiler.ProfileArea profileArea) {
 long long0 = getTimeNs();
 return this->start(profileArea, long0);
 }

public
 GameProfiler.ProfileArea start(GameProfiler.ProfileArea profileArea0,
 long long0) {
 ...
 "Incorrect exit. ProfileArea " + profileArea +
 " is not at the top of the stack: " + this->m_stack.peek());
 } else {
 this->m_stack.pop();
 if (this->m_stack.empty()) {
 ...
 TextManager.instance.DrawString(int1, int0, string, float3, float2, float1,
 1.0);
 TextManager.instance.DrawString(int1 + 300, int0, float0 + "%", float3,
 float2, float1, 1.0);
 }

 void render(int int0, int int1) {
 this->renderPercent(this->m_previousFrame.m_frameInvokerKey,
 this->m_previousFrame.m_totalTime, int0, int1, 1.0F, 1.0F,
 1.0F);
 }

 static long getTimeNs() { return System.nanoTime(); }

 static void init() { initTriggerWatcher(); }

 static void initTriggerWatcher() {
 if (m_gameProfilerRecordingTriggerWatcher.empty()) {
 { std::lock_guard<std::mutex> __sync_lock__(m_gameProfilerRecordingTriggerLock_mutex);
 if (m_gameProfilerRecordingTriggerWatcher.empty()) {
 m_gameProfilerRecordingTriggerWatcher = new PredicatedFileWatcher(
 ZomboidFileSystem.instance.getMessagingDirSub(
 "Trigger_PerformanceProfiler.xml"),
...
public
 static class ProfileArea extends PooledObject {
 std::string Key;
 long StartTime;
 long EndTime;
 long Total;
 ...
 float b = 1.0F;
 public
 List<GameProfiler.ProfileArea> Children =
 std::make_unique<ArrayList<>>();
 private
 static Pool<GameProfiler.ProfileArea> s_pool =
 new Pool<>(GameProfiler.ProfileArea::new);
 ...
 public
 static GameProfiler.ProfileArea alloc() { return s_pool.alloc(); }
 }
