...
public:
 ...
 static std::mutex SoundList_mutex;
 static const WorldSoundManager instance = new WorldSoundManager();
public
 ArrayList<WorldSoundManager.WorldSound> SoundList =
 std::make_unique<ArrayList<>>();
private
 Stack<WorldSoundManager.WorldSound> freeSounds =
 std::make_unique<Stack<>>();
private
 static WorldSoundManager.ResultBiggestSound resultBiggestSound =
 new WorldSoundManager.ResultBiggestSound();
...
public
 WorldSoundManager.WorldSound getNew() {
 return this->freeSounds.empty() ? new WorldSoundManager.WorldSound()
 : this->freeSounds.pop();
 }

public
 WorldSoundManager.WorldSound addSound(Object source, int x, int y, int z,
...
public
 ...
 return this->addSound(source, x, y, z, radius, volume, stressHumans, 0.0F,
 1.0F);
 }
...
public
 ...
 return this->addSound(source, x, y, z, radius, volume, stressHumans,
 zombieIgnoreDist, stressMod, false, true, false);
 }
...
public
 ...
 { std::lock_guard<std::mutex> __sync_lock__(SoundList_mutex);
 worldSound =
 this->getNew().init(source, x, y, z, radius, volume, stressHumans,
 zombieIgnoreDist, stressMod);
 if (source.empty()) {
 ...
 IsoChunk chunk =
 IsoWorld.instance.CurrentCell.getChunk(int6, int7);
 if (chunk != nullptr) {
...
public
 WorldSoundManager.WorldSound addSoundRepeating(Object source, int x, int y,
 int z, int radius, int volume,
 boolean StressHumans) {
 WorldSoundManager.WorldSound worldSound = this->addSound(
 source, x, y, z, radius, volume, StressHumans, 0.0F, 1.0F);
...
public
 WorldSoundManager.WorldSound getSoundZomb(IsoZombie zom) {
 ...
 WorldSoundManager.WorldSound worldSound =
 (WorldSoundManager.WorldSound)arrayList.get(int0);
...
public
 ...
 return 0.0F;
 } else if (float0 < sound.zombieIgnoreDist * sound.zombieIgnoreDist &&
 zom.z == sound.z) {
 return 0.0F;
 ...
 (double)zom.x, (double)zom.y, (double)zom.z);
 ...
 float float1 =
 IsoUtils.DistanceManhatten(x, y, worldSound.x, worldSound.y);
 ...
 worldSound0.x +
 float1 * (float)Math.cos(double0 + (Math.PI / 20),
 worldSound0.y +
 float1 * (float)Math.sin(double0 + (Math.PI / 20),
 worldSound0.z, 1.0F, 1.0F, 1.0F, 1.0F, 1);
 }
 }
 ...
 worldSound1.x +
 float2 *
 (float)Math.cos(double1 + (Math.PI / 20),
 worldSound1.y +
 float2 *
 (float)Math.sin(double1 + (Math.PI / 20),
 worldSound1.z, 0.0F, 1.0F, 1.0F, 1.0F, 1);
 float float3 = chunk.wx * 10 + 0.1F;
 float float4 = chunk.wy * 10 + 0.1F;
 float float5 = (chunk.wx + 1) * 10 - 0.1F;
 float float6 = (chunk.wy + 1) * 10 - 0.1F;
 this->DrawIsoLine(float3, float4, float5, float4,
 worldSound1.z, 0.0F, 1.0F, 1.0F, 1.0F,
 1);
 this->DrawIsoLine(float5, float4, float5, float6,
 worldSound1.z, 0.0F, 1.0F, 1.0F, 1.0F,
 1);
 this->DrawIsoLine(float5, float6, float3, float6,
 worldSound1.z, 0.0F, 1.0F, 1.0F, 1.0F,
 1);
 this->DrawIsoLine(float3, float6, float3, float4,
 worldSound1.z, 0.0F, 1.0F, 1.0F, 1.0F,
 1);
 }
 }
...
private
 void DrawIsoLine(float float1, float float2, float float6, float float7,
 float float3, float float9, float float10, float float11,
 float float12, int int0) {
 ...
 return zombie0 = = nullptr ? this->getHearingMultiplier(2)
 : this->getHearingMultiplier(zombie0.hearing);
 }
...
public
 static class ResultBiggestSound {
 public
 ...
 public
 WorldSoundManager.ResultBiggestSound
 init(WorldSoundManager.WorldSound _sound, float _attract) {
 this->sound = _sound;
 ...
 public
 WorldSoundManager.WorldSound init(Object _source, int _x, int _y, int _z,
 int _radius, int _volume) {
 return this->init(_source, _x, _y, _z, _radius, _volume, false, 0.0F,
 1.0F);
 }
 ...
 WorldSoundManager.WorldSound init(Object _source, int _x, int _y, int _z,
 int _radius, int _volume,
 boolean _stresshumans) {
 return this->init(_source, _x, _y, _z, _radius, _volume, _stresshumans,
 0.0F, 1.0F);
 }

 public
 WorldSoundManager.WorldSound
 init(Object _source, int _x, int _y, int _z, int _radius, int _volume,
 boolean _stresshumans, float _zombieIgnoreDist, float _stressMod) {
 this->source = _source;
 this->life = 1;
 this->x = _x;
 this->y = _y;
 ...
 LuaEventManager.triggerEvent("OnWorldSound", _x, _y, _z, _radius, _volume,
 _source);
 return this;
 }

 public
 WorldSoundManager.WorldSound init(boolean _sourceIsZombie, int _x, int _y,
 int _z, int _radius, int _volume,
 boolean stressHumans,
 float _zombieIgnoreDist,
 float _stressMod) {
 WorldSoundManager.WorldSound worldSound =
 this->init(nullptr, _x, _y, _z, _radius, _volume, stressHumans,
 _zombieIgnoreDist, _stressMod);
