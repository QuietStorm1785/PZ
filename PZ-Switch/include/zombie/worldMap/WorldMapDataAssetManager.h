...
public:
 static const WorldMapDataAssetManager instance =
 new WorldMapDataAssetManager();
 ...
 object = new FileTask_LoadWorldMapBinary(
 worldMapData, string + ".bin", fileSystem,
 objectx->this->loadCallback(worldMapData, objectx);
 } else {
 object = new FileTask_LoadWorldMapXML(
 worldMapData, string, fileSystem,
 objectx->this->loadCallback(worldMapData, objectx);
 }

 ((FileTask)object).setPriority(4);
 AssetTask_RunFileTask assetTask_RunFileTask =
 new AssetTask_RunFileTask((FileTask)object, asset);
 ...
 assetPath.getPath(), var3x->this->reload(worldMapData, assetParams));
 return worldMapData;
 }
