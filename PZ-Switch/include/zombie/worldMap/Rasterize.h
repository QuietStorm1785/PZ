...
class Rasterize {
public:
 Rasterize.Edge edge1 = new Rasterize.Edge();
 Rasterize.Edge edge2 = new Rasterize.Edge();
 Rasterize.Edge edge3 = new Rasterize.Edge();

 void scanLine(int int1, int int2, int int3, Integer > biConsumer) {
 for (int int0 = int1; int0 < int2; int0++) {
 biConsumer.accept(int0, int3);
 }
 }

 void scanSpan(Rasterize.Edge edge1x, Rasterize.Edge edge0, int int1, int int3,
 Integer > biConsumer) {
 ...
 double double4 =
 double0 * Math.max(0.0, Math.min((double)edge1x.dy,
 int4 + double2 - edge1x.y0) +
 edge1x.x0;
 double double5 =
 double1 * Math.max(0.0, Math.min((double)edge0.dy,
 int4 + double3 - edge0.y0) +
 edge0.x0;
 this->scanLine((int)Math.floor(double5), (int)Math.ceil(double4), int4,
 biConsumer);
 }
 }

 void scanTriangle(float float0, float float1, float float2, float float3,
 float float4, float float5, int int0, int int1,
...
private
 ...
 Rasterize.Edge init(float float3, float float0, float float2,
 float float1) {
 if (float0 > float1) {
 this->x0 = float2;
 this->y0 = float1;
