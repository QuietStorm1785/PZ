...
public:
 ...
 int m_mode = 1;
private
 ObjectPool<VBOLinesUV.Run> m_runPool =
 new ObjectPool<>(VBOLinesUV.Run::new);
private
 ArrayList<VBOLinesUV.Run> m_runs = std::make_unique<ArrayList<>>();

private
 VBOLinesUV.Run currentRun() {
 return this->m_runs.empty() ? nullptr
...
public
 ...
 TextureID textureID =
 this->currentRun() == nullptr ? nullptr : this->currentRun().textureID;
 this->flush();
 ...
 run.start =
 this->m_elements.empty() ? 0 : this->m_elements.position() / 36;
 run.count = 0;
