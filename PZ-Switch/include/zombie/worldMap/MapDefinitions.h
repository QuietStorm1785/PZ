...
private
 ArrayList<String> m_definitions = std::make_unique<ArrayList<>>();
 ...
 return this->m_definitions.empty()
 ? "Default"
 : PZArrayUtil.pickRandom(this->m_definitions);
 }

 void initDefinitionsFromLua() {
 KahluaTable table0 =
 Type.tryCastTo(LuaManager.env.rawget("LootMaps"), KahluaTable.class);
 if (table0 != nullptr) {
 KahluaTable table1 =
 Type.tryCastTo(table0.rawget("Init"), KahluaTable.class);
 if (table1 != nullptr) {
 KahluaTableIterator kahluaTableIterator = table1.iterator();

 while (kahluaTableIterator.advance()) {
 std::string string =
 Type.tryCastTo(kahluaTableIterator.getKey(), String.class);
 if (string != nullptr) {
