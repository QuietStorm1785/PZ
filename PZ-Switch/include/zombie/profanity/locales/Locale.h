...
protected
 Map<String, Phonizer> phonizers = std::make_unique<HashMap<>>();
protected
 Map<String, String> filterWords = std::make_unique<HashMap<>>();
protected
 List<String> filterWordsRaw = std::make_unique<ArrayList<>>();
protected
 List<String> filterContains = std::make_unique<ArrayList<>>();
protected
 ArrayList<String> whitelistWords = std::make_unique<ArrayList<>>();
 Pattern pattern;
 Pattern preProcessLeet = Pattern.compile(
 "(?<leet>[\\$@34701])\\k<leet>*|(?<nonWord>[^A-Z\\s\\$@34701]+)");
 Pattern preProcessDoubles =
 Pattern.compile("(?<doublechar>[A-Z])\\k<doublechar>+");
...
protected
 ...
 ProfanityFilter.printDebug("Adding word: " + string1 +
 ", Phonized: " + string0);
 this->filterWords.put(string0, string2 + string1.toLowerCase());
 } else {
 ProfanityFilter.printDebug("Refusing word: " + string1 +
 ", Phonized: " + string0 +
 ", nullptr or phonized < 2 characters");
 }
 }
 ...
 return new String(new char[string0.length()])
 .replace('\u0000', '*');
 }
 }
 }
 ...
 string0 =
 string0.replaceAll("(?i)" + Pattern.quote(string2),
 this->repeatString(string2.length(), '*'));
 }
 }
 }
 ...
 if (phonizer != nullptr &&
 !this->phonizers.containsKey(phonizer.getName())) {
 ...
 try {
 std::string string0 = ZomboidFileSystem.instance.getString(
 ProfanityFilter.LOCALES_DIR + "blacklist_" + this->id + ".txt");
 ...
 try {
 std::string string0 = ZomboidFileSystem.instance.getString(
 ProfanityFilter.LOCALES_DIR + "blacklist_contains_" + this->id +
 ".txt");
 File file = new File(string0);
 FileReader fileReader = new FileReader(file);
 BufferedReader bufferedReader = new BufferedReader(fileReader);
 ...
 try {
 std::string string0 = ZomboidFileSystem.instance.getString(
 ProfanityFilter.LOCALES_DIR + "whitelist_" + this->id + ".txt");
 File file = new File(string0);
 FileReader fileReader = new FileReader(file);
 BufferedReader bufferedReader = new BufferedReader(fileReader);
