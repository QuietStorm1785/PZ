...
public:
 ...
 static Vector2 axis = std::make_unique<Vector2>();
 static Polygon polygonA = std::make_unique<Polygon>();
 static Polygon polygonB = std::make_unique<Polygon>();
 float minA = 0.0F;
 float minB = 0.0F;
 float maxA = 0.0F;
 float maxB = 0.0F;
 CollisionManager.PolygonCollisionResult result =
 new CollisionManager.PolygonCollisionResult();
public
 ArrayList<CollisionManager.Contact> ContactMap =
 std::make_unique<ArrayList<>>();
 Long[] longArray = new Long[1000];
 Stack<CollisionManager.Contact> contacts = std::make_unique<Stack<>>();
 static const CollisionManager instance = new CollisionManager();

 void ProjectPolygonA(Vector2 vector, Polygon polygon) {
 ...
 void ProjectPolygonB(Vector2 vector, Polygon polygon) {
 float float0 = vector.dot(polygon.points.get(0);
 this->minB = float0;
 this->maxB = float0;
...
public
 CollisionManager.PolygonCollisionResult PolygonCollision(Vector2 vector2) {
 ...
 if (this->IntervalDistance(this->minA, this->maxA, this->minB, this->maxB) >
 0.0F) {
 ...
 float float2 =
 this->IntervalDistance(this->minA, this->maxA, this->minB, this->maxB);
 if (float2 > 0.0F) {
 ...
 float IntervalDistance(float float0, float float3, float float2,
 float float1) {
 ...
 this->ContactMap.get(int1).a = nullptr;
 this->ContactMap.get(int1).b = nullptr;
 ...
 void AddContact(IsoMovingObject movingObject1,
 IsoMovingObject movingObject0) {
 if (!(movingObject1 instanceof IsoSurvivor) &&
 !(movingObject0 instanceof IsoSurvivor) ||
 !(movingObject1 instanceof IsoPushableObject) &&
 !(movingObject0 instanceof IsoPushableObject) {
 ...
 CollisionManager.Contact contact(IsoMovingObject movingObject0,
 IsoMovingObject movingObject1) {
 if (this->contacts.empty()) {
 for (int int0 = 0; int0 < 50; int0++) {
 this->contacts.push(new CollisionManager.Contact(nullptr, nullptr);
 }
 }
 ...
 CollisionManager.s_performance.profile_ResolveContacts.invokeAndMeasure(
 this, CollisionManager::resolveContactsInternal);
 }
 ...
 std::vector list = CollisionManager.l_ResolveContacts.pushables;
 std::vector arrayList0 =
 IsoWorld.instance.CurrentCell.getPushableObjectList();
 int int0 = arrayList0.size();

 for (int int1 = 0; int1 < int0; int1++) {
 IsoPushableObject pushableObject0 =
 (IsoPushableObject)arrayList0.get(int1);
 if (pushableObject0.getImpulsex() != 0.0F ||
 pushableObject0.getImpulsey() != 0.0F) {
 if (pushableObject0.connectList != nullptr) {
 list.add(pushableObject0);
 } else {
 pushableObject0.setNx(pushableObject0.getNx() +
 pushableObject0.getImpulsex());
 pushableObject0.setNy(pushableObject0.getNy() +
 pushableObject0.getImpulsey());
 pushableObject0.setImpulsex(pushableObject0.getNx() -
 pushableObject0.getX());
 pushableObject0.setImpulsey(pushableObject0.getNy() -
 pushableObject0.getY());
 ...
 if (vector0.x != 0.0F || vector0.y != 0.0F || vector1.x != 0.0F ||
 vector1.y != 0.0F || contact.a.getImpulsex() != 0.0F ||
 ...
 CollisionManager.PolygonCollisionResult polygonCollisionResult =
 this->PolygonCollision(vector0);
 ...
 1.0F -
 contact.a.getWeight(
 polygonCollisionResult.MinimumTranslationVector.x,
 polygonCollisionResult.MinimumTranslationVector.y) /
 (contact.a.getWeight(
 polygonCollisionResult.MinimumTranslationVector.x,
 polygonCollisionResult.MinimumTranslationVector.y) +
 contact.b.getWeight(
 polygonCollisionResult.MinimumTranslationVector.x,
 polygonCollisionResult.MinimumTranslationVector.y);
 if (contact.a instanceof IsoPushableObject && contact.b instanceof
 IsoSurvivor) {
 ((IsoSurvivor)contact.b).bCollidedWithPushable = true;
 ((IsoSurvivor)contact.b).collidePushable =
 (IsoPushableObject)contact.a;
 } else if (contact.b instanceof
 IsoPushableObject && contact.a instanceof IsoSurvivor) {
 ((IsoSurvivor)contact.a).bCollidedWithPushable = true;
 ((IsoSurvivor)contact.a).collidePushable =
 (IsoPushableObject)contact.b;
 }

 if (contact.a instanceof IsoPushableObject) {
 std::vector arrayList1 =
 ((IsoPushableObject)contact.a).connectList;
 if (arrayList1 != nullptr) {
 int int9 = arrayList1.size();

 for (int int10 = 0; int10 < int9; int10++) {
 IsoPushableObject pushableObject2 =
 (IsoPushableObject)arrayList1.get(int10);
 pushableObject2.setImpulsex(
 pushableObject2.getImpulsex() +
 polygonCollisionResult.MinimumTranslationVector.x *
 float10);
 pushableObject2.setImpulsey(
 pushableObject2.getImpulsey() +
 polygonCollisionResult.MinimumTranslationVector.y *
 float10);
 }
 }
 } else {
 contact.a.setImpulsex(
 contact.a.getImpulsex() +
 polygonCollisionResult.MinimumTranslationVector.x * float10);
 contact.a.setImpulsey(
 contact.a.getImpulsey() +
 polygonCollisionResult.MinimumTranslationVector.y * float10);
 }

 if (contact.b instanceof IsoPushableObject) {
 std::vector arrayList2 =
 ((IsoPushableObject)contact.b).connectList;
 if (arrayList2 != nullptr) {
 int int11 = arrayList2.size();

 for (int int12 = 0; int12 < int11; int12++) {
 IsoPushableObject pushableObject3 =
 (IsoPushableObject)arrayList2.get(int12);
 pushableObject3.setImpulsex(
 pushableObject3.getImpulsex() -
 polygonCollisionResult.MinimumTranslationVector.x *
 (1.0F - float10);
 pushableObject3.setImpulsey(
 pushableObject3.getImpulsey() -
 polygonCollisionResult.MinimumTranslationVector.y *
 (1.0F - float10);
 }
 }
 } else {
 contact.b.setImpulsex(
 contact.b.getImpulsex() -
 polygonCollisionResult.MinimumTranslationVector.x *
 (1.0F - float10);
 contact.b.setImpulsey(
 contact.b.getImpulsey() -
 polygonCollisionResult.MinimumTranslationVector.y *
 (1.0F - float10);
 }
 }
 ...
 std::vector arrayList3 = IsoWorld.instance.CurrentCell.getObjectList();
...
class Contact {
 ...
 public
 Contact(IsoMovingObject movingObject0, IsoMovingObject movingObject1) {
 ...
 static List<IsoPushableObject> pushables =
 std::make_unique<ArrayList<>>();
 static IsoMovingObject[] objectListInvoking = new IsoMovingObject[1024];
 }

 private static class s_performance {
 static const PerformanceProfileProbe profile_ResolveContacts =
 new PerformanceProfileProbe("CollisionManager.ResolveContacts");
 static const PerformanceProfileProbe profile_MovingObjectPostUpdate =
 new PerformanceProfileProbe("IsoMovingObject.postupdate");
 }
}
} // namespace zombie
