...
public:
 ...
 std::string name;
private
 ArrayList<VehicleScript.Model> models = std::make_unique<ArrayList<>>();
public
...
private
 ArrayList<VehicleScript.PhysicsShape> m_physicsShapes =
 std::make_unique<ArrayList<>>();
private
 ArrayList<VehicleScript.Wheel> wheels = std::make_unique<ArrayList<>>();
private
 ArrayList<VehicleScript.Passenger> passengers =
 std::make_unique<ArrayList<>>();
...
public
 float[] gearRatio = new float[9];
private
 VehicleScript.Skin textures = new VehicleScript.Skin();
private
 ArrayList<VehicleScript.Skin> skins = std::make_unique<ArrayList<>>();
private
 ArrayList<VehicleScript.Area> areas = std::make_unique<ArrayList<>>();
private
 ArrayList<VehicleScript.Part> parts = std::make_unique<ArrayList<>>();
 bool hasSiren = false;
private
 VehicleScript.LightBar lightbar = new VehicleScript.LightBar();
private
 VehicleScript.Sounds sound = new VehicleScript.Sounds();
...
private
 ...
 block1, object instanceof KahluaTable ? (KahluaTable)object : nullptr);
...
VehicleScript.Container LoadContainer(ScriptParser.Block block,
 ...
 container1.empty() ? new VehicleScript.Container() : container1;
...
HashMap<String, String> LoadLuaFunctions(ScriptParser.Block block) {
 ...
 throw RuntimeException("expected \"key = value\", got \"" +
...
public static class Passenger {
 std::string id;
public
 ArrayList<VehicleScript.Anim> anims = std::make_unique<ArrayList<>>();
public
 ArrayList<VehicleScript.Passenger.SwitchSeat> switchSeats =
 std::make_unique<ArrayList<>>();
 bool hasRoof = true;
