...
public:
 ...
 IsoUtils.DistanceToSquared(zombie0.x, zombie0.y,
 zombie0.target.x,
 zombie0.target.y) < 16.0F) {
 return false;
 } else {
 State state = zombie0.getCurrentState();
 return state.empty() || state == ZombieIdleState.instance() ||
 ...
 if (System.currentTimeMillis() - zombie0.lastChangeOwner >= 2000L ||
 zombie0.authOwner.empty()) {
 if (SystemDisabler.zombiesSwitchOwnershipEachUpdate &&
 ...
 int int1 =
 GameServer.udpEngine.connections.indexOf(zombie0.authOwner) + 1;

 for (int int2 = 0; int2 < GameServer.udpEngine.connections.size();
 int2++) {
 UdpConnection udpConnection1 =
 GameServer.udpEngine.connections.get(
 (int2 + int1) % GameServer.udpEngine.connections.size());
 ...
 GameServer.getConnectionFromPlayer((IsoPlayer)zombie0.target);
 if (udpConnection2 != nullptr &&
 udpConnection2.isFullyConnected()) {
 float float0 =
 ((IsoPlayer)zombie0.target)
 .getRelevantAndDistance(zombie0.x, zombie0.y,
 udpConnection2.ReleventRange - 2);
 if (!Float.isInfinite(float0) {
 this->moveZombie(zombie0, udpConnection2,
 (IsoPlayer)zombie0.target);
 if (Core.bDebug) {
 DebugLog.log(
 DebugType.Ownership,
 String.format(
 "Zombie (%d) owner (\"%s\"): zombie has target",
 zombie0.getOnlineID(),
 ((IsoPlayer)zombie0.target).getUsername()));
 }
 ...
 float1 = udpConnection3.getRelevantAndDistance(zombie0.x, zombie0.y,
 zombie0.z);
 }

 for (int int3 = 0; int3 < GameServer.udpEngine.connections.size();
 int3++) {
 UdpConnection udpConnection4 =
 GameServer.udpEngine.connections.get(int3);
 if (udpConnection4 != udpConnection3) {
 for (IsoPlayer player1 : udpConnection4.players) {
 if (player1 != nullptr && player1.isAlive()) {
 float float2 = player1.getRelevantAndDistance(
 zombie0.x, zombie0.y, udpConnection4.ReleventRange - 2);
 if (!Float.isInfinite(float2) &&
 (udpConnection3.empty() ||
 float1 > float2 * 1.618034F) {
 ...
 if (Core.bDebug && player0 != nullptr &&
 player0 != zombie0.authOwnerPlayer) {
 DebugLog.log(
 DebugType.Ownership,
 String.format("Zombie (%d) owner (\"%s\"): zombie is closer",
 zombie0.getOnlineID(), player0.getUsername()));
 }

 if (udpConnection3.empty() && zombie0.isReanimatedPlayer()) {
 for (int int4 = 0; int4 < GameServer.udpEngine.connections.size();
 int4++) {
 UdpConnection udpConnection5 =
 ...
 player2.ReanimatedCorpse == zombie0) {
 udpConnection3 = udpConnection5;
 player0 = player2;
 if (Core.bDebug) {
 DebugLog.log(DebugType.Ownership,
 String.format("Zombie (%d) owner (\"%s\"): "
 "zombie is reanimated",
 zombie0.getOnlineID(),
 player2.getUsername()));
 }
 }
 ...
 if (udpConnection3 != nullptr &&
 !udpConnection3.RelevantTo(zombie0.x, zombie0.y,
 (udpConnection3.ReleventRange - 2) *
 10) {
 ...
 void moveZombie(IsoZombie zombie0, UdpConnection udpConnection,
 IsoPlayer player) {
 if (zombie0.isDead()) {
 if (zombie0.authOwner.empty() && zombie0.authOwnerPlayer.empty()) {
 zombie0.becomeCorpse();
 } else {
 { std::lock_guard<std::mutex> __sync_lock__(owns.lock_mutex);
 ...
 DebugLog.log(DebugType.Ownership,
 String.format(
 "Zombie (%d) owner (\"%s\"): zombie owner is driver",
 zombie0.getOnlineID(),
 player.empty() ? "" : player.getUsername()));
 }
 }
 ...
 NetworkZombieList.NetworkZombie networkZombie0 =
 this->owns.getNetworkZombie(zombie0.authOwner);
 if (networkZombie0 != nullptr &&
 !networkZombie0.zombies.remove(zombie0) {
 ...
 this->owns.getNetworkZombie(udpConnection);
...
public
 LinkedList<IsoZombie> getZombieList(UdpConnection udpConnection) {
 NetworkZombieList.NetworkZombie networkZombie =
 this->owns.getNetworkZombie(udpConnection);
 ...
 for (int int6 = square.getMovingObjects().size() - 1; int6 >= 0;
 int6--) {
 IsoZombie zombie0 = Type.tryCastTo(
 square.getMovingObjects().get(int6), IsoZombie.class);
 if (zombie0 != nullptr) {
 ...
 networkZombie.zombies.removeIf(zombie0->zombie0.authOwner !=
 udpConnection);
 }
 }
 }
