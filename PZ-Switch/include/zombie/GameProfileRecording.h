...
public:
 long m_startTime;
private
 GameProfileRecording.Row m_rootRow = new GameProfileRecording.Row();
private
 HashMap<String, Integer> m_keyValueTable =
 std::make_unique<HashMap<>>();
 PrintStream m_outSegment = nullptr;
 long m_firstFrameNo = -1L;
private
 List<String> m_segmentFilePaths = std::make_unique<ArrayList<>>();
...
public
 ...
 long StartTime;
 long EndTime;
 List<GameProfileRecording.Span> Spans =
 ...
 long StartTime;
 long EndTime;
 List<GameProfileRecording.Span> Children =
 std::make_unique<ArrayList<>>();
 private
 static Pool<GameProfileRecording.Span> s_pool =
