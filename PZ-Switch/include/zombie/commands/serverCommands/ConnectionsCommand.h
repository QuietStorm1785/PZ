...
class ConnectionsCommand : public CommandBase {
 public:
 public
 ConnectionsCommand(const std::string &string0,
 ...
 string0 =
 string0 + "connection=" + (int0 + 1) + "/" +
 GameServer.udpEngine.connections.size() + " " +
 udpConnection.idStr + " player=" + (int1 + 1) +
 "/4 id=" + udpConnection.playerIDs[int1] +
 " username=\"" + udpConnection.usernames[int1] +
 "\" fullyConnected=" + udpConnection.isFullyConnected() +
 string1;
 }
 }
 }
