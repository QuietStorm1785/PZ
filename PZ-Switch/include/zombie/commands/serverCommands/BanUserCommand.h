...
class BanUserCommand : public CommandBase {
 ...
 static const std::string banWithReasonIP =
 "Ban User And IP And Supply Reason";

 public
 BanUserCommand(const std::string &string0, const std::string &string1,
 ...
 string0, Userlog.UserlogType.Banned, this->reason,
 this->getExecutorUsername(), 1);
 LoggerManager.getLogger("admin").write(
 this->getExecutorUsername() + " banned user " + string0 +
 ...
 LoggerManager.getLogger("admin").write(
 this->getExecutorUsername() + " banned steamid " +
 udpConnection.steamID + "(" + udpConnection.username +
 ")" + (this->reason != nullptr ? this->reason : ""),
 "IMPORTANT");
 std::string string2 =
 SteamUtils.convertSteamIDToString(udpConnection.steamID);
 ServerWorldDatabase.instance.banSteamID(string2, this->reason,
 true);
 }

 if (boolean0) {
 LoggerManager.getLogger("admin").write(
 this->getExecutorUsername() + " banned ip " +
 udpConnection.ip + "(" + udpConnection.username +
 ")" + (this->reason != nullptr ? this->reason : ""),
 "IMPORTANT");
 ServerWorldDatabase.instance.banIp(udpConnection.ip, string0,
 this->reason, true);
 }
 ...
 if (boolean1 &&
 ServerOptions.instance.BanKickGlobalSound.getValue()) {
