...
class AddItemCommand : public CommandBase {
 ...
 public
 AddItemCommand(const std::string &string0, const std::string &string1,
 ...
 this->getCommandArg(this->getCommandArgsCount() - 1);
 }
 }

 std::string string0;
 if (this->argsName == "add item to player")) {
 IsoPlayer player0 = GameServer.getPlayerByUserNameForCommand(
 this->getCommandArg(0);
 if (player0.empty()) {
 return "No such user";
 }
 ...
 IsoPlayer player1 =
 GameServer.getPlayerByRealUserName(this->getExecutorUsername());
 if (player1.empty()) {
 return "No such user";
 }
 ...
 IsoPlayer player2 =
 GameServer.getPlayerByUserNameForCommand(string0);
 if (player2 != nullptr) {
 string0 = player2.getDisplayName();
 UdpConnection udpConnection =
 GameServer.getConnectionByPlayerOnlineID(player2.OnlineID);
 if (udpConnection != nullptr) {
 ByteBufferWriter byteBufferWriter0 =
 udpConnection.startPacket();
 PacketTypes.PacketType.AddItemInInventory.doPacket(
 byteBufferWriter0);
 ...
 this->getExecutorUsername() + " added item " + string1 +
 " in " + string0 + "'s inventory");
 ByteBufferWriter byteBufferWriter1 =
 udpConnection.startPacket();
 PacketTypes.PacketType.RequestInventory.doPacket(
 byteBufferWriter1);
 byteBufferWriter1.putShort(player2.OnlineID);
 PacketTypes.PacketType.RequestInventory.send(udpConnection);
 return "Item " + string1 + " Added in " + string0 +
 "'s inventory.";
 }
 }
