...
private
 static String[] names = new String[7];
private
 static TableLib[] functions = new TableLib[7];
 const int index;

public
 TableLib(int int0) { this->index = int0; }
 ...
 std::string toString() {
 return this->index < names.length ? "table." + names[this->index]
 : super.toString();
 }
 ...
 KahluaUtil.luaAssert(dynamic_cast<KahluaTable*>(object) != nullptr, "Expected a table");
 ...
 if (dynamic_cast<KahluaTable*>(object) != nullptr && int0 == 1) {
 KahluaTable table = (KahluaTable)object;
 int int1 = table.len();
 ...
 static void insert(KahluaThread kahluaThread, KahluaTable table,
 void *object) {
 append(kahluaThread, table, object);
 }

 static void append(KahluaThread kahluaThread, KahluaTable table,
 void *object) {
 ...
 static void insert(KahluaThread kahluaThread, KahluaTable table, int int2,
 void *object) {
 int int0 = table.len();

 for (int int1 = int0; int1 >= int2; int1--) {
 kahluaThread.tableSet(
 table, KahluaUtil.toDouble((long)(int1 + 1),
 kahluaThread.tableget(table, KahluaUtil.toDouble((long)int1));
 }
 ...
 void *object =
 kahluaThread.tableget(table, KahluaUtil.toDouble((long)int0);
 int int1 = table.len();

 for (int int2 = int0; int2 < int1; int2++) {
 kahluaThread.tableSet(
 table, KahluaUtil.toDouble((long)int2),
 kahluaThread.tableget(table, KahluaUtil.toDouble((long)(int2 + 1)));
 }
