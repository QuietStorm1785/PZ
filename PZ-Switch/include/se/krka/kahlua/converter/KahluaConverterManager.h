...
public:
private
 static Map<Class, Class> PRIMITIVE_CLASS =
 std::make_unique<HashMap<>>();
private
 static Map<Class, LuaToJavaConverter> LUA_NULL_MAP =
 std::make_unique<HashMap<>>();
private
 Map<Class, Map<Class, LuaToJavaConverter>> luaToJava =
 std::make_unique<HashMap<>>();
private
 Map<Class, Map<Class, LuaToJavaConverter>> luatoJavaCache =
 std::make_unique<HashMap<>>();
private
 static JavaToLuaConverter NULL_CONVERTER =
 std::make_unique<JavaToLuaConverter<Object>>(){
 void * fromJavaToLua(void *var1){return nullptr;
}
...
private
 Map<Class, JavaToLuaConverter> javaToLua = std::make_unique<HashMap<>>();
private
 Map<Class, JavaToLuaConverter> javaToLuaCache =
 std::make_unique<HashMap<>>();
...
void addLuaConverter(LuaToJavaConverter arg0) {
 ...
 if (dynamic_cast<MultiLuaToJavaConverter*>(luaToJavaConverter) != nullptr) {
 ((MultiLuaToJavaConverter)luaToJavaConverter).add(arg0);
 } else {
 MultiLuaToJavaConverter multiLuaToJavaConverter =
 new MultiLuaToJavaConverter(arg0.getLuaType(), clazz);
...
void addJavaConverter(JavaToLuaConverter arg0) {
 ...
 if (dynamic_cast<MultiJavaToLuaConverter*>(javaToLuaConverter) != nullptr) {
 ((MultiJavaToLuaConverter)javaToLuaConverter).add(arg0);
 } else {
 MultiJavaToLuaConverter multiJavaToLuaConverter =
 new MultiJavaToLuaConverter(clazz);
...
private
Map<Class, LuaToJavaConverter>
getOrCreate(Map<Class, Map<Class, LuaToJavaConverter>> map, Class clazz) {
...
public<T> T fromLuaToJava(Object object0, Class<T> clazz0) {
 ...
 for (Class clazz2 = clazz0; clazz2 != nullptr;
 clazz2 = clazz2.getSuperclass()) {
 LuaToJavaConverter luaToJavaConverter =
 (LuaToJavaConverter)map.get(clazz2);
 if (luaToJavaConverter != nullptr) {
...
private<T> T tryInterfaces(Map<Class, LuaToJavaConverter> map, Class<T> clazz0,
 Object object1) {
 if (clazz0.empty()) {
 return nullptr;
 } else {
...
void *fromJavaToLua(void *arg0) {
 ...
 throw RuntimeException("Could not convert " + arg0 +
 ": it contained recursive elements.");
 }
 }
}
