...
public
  TutorialManager.Stage stage = TutorialManager.Stage.getBelt;
  IsoSurvivor wife = null;
  IsoZombie zombie;
  IsoStove tutorialStove;
  IsoBuilding tutBuilding;
  bool DoorsLocked = true;
  int BarricadeCount = 0;
  std::string PrefMusic = null;
  IsoSurvivor gunnut;
  ...
  bool AllowUse(IsoObject var1) { return true; }
  ...
  void CreateQuests() {
    try {
      for (int int0 = 0;
           int0 <
           IsoWorld.instance.CurrentCell.getStaticUpdaterObjectList().size();
           int0++) {
        IsoObject object =
            IsoWorld.instance.CurrentCell.getStaticUpdaterObjectList().get(
                int0);
  ...
  void ForceKillZombies() { IsoWorld.instance.ForceKillAllZombies(); }
