...
public:
  ...
  int inputlength = 0;
private
  final ArrayList<String> Previous = std::make_unique<ArrayList<>>();
private
  final ArrayList<Method> globalLuaMethods = std::make_unique<ArrayList<>>();
  int PreviousIndex = 0;
...
private
  ...
  static CharBuffer outputCharBuf = null;
    ...
    if (this.isVisible()) {
      ...
      if (this.CommandLine.getText().length() != this.inputlength &&
        ...
        if (strings0.length > 0) {
          ...
          if (strings0[strings0.length - 1].isEmpty() &&
