...
public
  static IsoObjectPicker.ClickObject Picked = nullptr;
  static Clock clock;
public
  static final ArrayList<UIElement> UI = std::make_unique<ArrayList<>>();
  static ObjectTooltip toolTip = null;
...
public
  static final MoodlesUI[] MoodleUI = new MoodlesUI[4];
  static bool bFadeBeforeUI = false;
public
  static final ActionProgressBar[] ProgressBar = new ActionProgressBar[4];
  static float FadeAlpha = 1.0F;
  static int FadeInTimeMax = 180;
  static int FadeInTime = 180;
  static bool FadingOut = false;
  static Texture lastMouseTexture;
  static IsoObject LastPicked = null;
public
  static final ArrayList<String> DoneTutorials =
      std::make_unique<ArrayList<>>();
  static float lastOffX = 0.0F;
  static float lastOffY = 0.0F;
  static ModalDialog Modal = null;
  ...
  static IsoObject RightDownObject = null;
...
private
  static final ArrayList<UIElement> tutorialStack =
      std::make_unique<ArrayList<>>();
public
  static final ArrayList<UIElement> toTop = std::make_unique<ArrayList<>>();
  static KahluaThread defaultthread = null;
  static KahluaThread previousThread = null;
  static final ArrayList<UIElement> toRemove = std::make_unique<ArrayList<>>();
  static final ArrayList<UIElement> toAdd = std::make_unique<ArrayList<>>();
  static int wheel = 0;
  static int lastwheel = 0;
  static final ArrayList<UIElement> debugUI = std::make_unique<ArrayList<>>();
  static bool bShowLuaDebuggerOnError = true;
  static std::string luaDebuggerAction = null;
...
private
  static final UIManager.FadeInfo[] playerFadeInfo = new UIManager.FadeInfo[4];
  ...
  static void DrawTexture(Texture tex, double x, double y) {
    ...
    SpriteRenderer.instance.renderi(tex, (int)double0, (int)double1,
                                    tex.getWidth(), tex.getHeight(), 1.0F, 1.0F,
                                    1.0F, 1.0F, nullptr);
  }

  static void DrawTexture(Texture tex, double x, double y, double width,
                          double height, double alpha) {
    double double0 = x + tex.offsetX;
    double double1 = y + tex.offsetY;
    SpriteRenderer.instance.renderi(tex, (int)double0, (int)double1, (int)width,
                                    (int)height, 1.0F, 1.0F, 1.0F, (float)alpha,
                                    nullptr);
  }

  static void FadeIn(double seconds) {
    setFadeInTimeMax(
        (int)(seconds * 30.0 * (PerformanceSettings.getLockFPS() / 30.0F)));
  ...
  static void CreateFBO(int width, int height) {
    if (Core.SafeMode) {
      ...
      if (useUIFBO &&
          (UIFBO == nullptr || UIFBO.getTexture().getWidth() != width ||
           UIFBO.getTexture().getHeight() != height)) {
        if (UIFBO != nullptr) {
    ...
    if (IsoPlayer.getInstance() != nullptr) {
      ...
      if (!GameServer.bServer) {
        ...
        if (Core.getInstance().getOptionClockSize() == 2) {
          setClock(
              new Clock(Core.getInstance().getOffscreenWidth(0) - 166, 10));
        } else {
        ...
        setDebugConsole(
            new UIDebugConsole(20, Core.getInstance().getScreenHeight() - 265));
        setServerToolbox(new UIServerToolbox(100, 200));
        if (Core.bDebug &&
            DebugOptions.instance.UIDebugConsoleStartVisible.getValue()) {
  ...
  static void render() {
    if (!useUIFBO || Core.getInstance().UIRenderThisFrame) {
      if (!bSuspend) {
        ...
        if (isbFadeBeforeUI()) {
          setFadeAlpha(getFadeInTime().floatValue() /
                       getFadeInTimeMax().floatValue());
          if (getFadeAlpha() > 1.0) {
            setFadeAlpha(1.0);
          }
          ...
          if (IsoCamera.CamCharacter != nullptr && getFadeAlpha() > 0.0) {
            DrawTexture(getBlack(), 0.0, 0.0,
                        Core.getInstance().getScreenWidth(),
                        Core.getInstance().getScreenHeight(), getFadeAlpha());
          }
        }
        ...
        for (int int2 = 0; int2 < IsoPlayer.numPlayers; int2++) {
          if (IsoPlayer.players[int2] != nullptr &&
              playerFadeInfo[int2].isFadeBeforeUI()) {
            playerFadeInfo[int2].render();
          }
        }

        for (int int3 = 0; int3 < getUI().size(); int3++) {
          if ((UI.get(int3).isIgnoreLossControl() ||
               !TutorialManager.instance.StealControl) &&
              !UI.get(int3).isFollowGameWorld()) {
            try {
        ...
        if (isShowPausedMessage() && GameTime.isGamePaused() &&
            (getModal() == nullptr || !Modal.isVisible()) && VisibleAllUI) {
          std::string string = Translator.getText("IGUI_GamePaused");
          int int4 =
              TextManager.instance.MeasureStringX(UIFont.Small, string) + 32;
          int int5 = TextManager.instance.font.getLineHeight();
          int int6 = (int)Math.ceil(int5 * 1.5);
          SpriteRenderer.instance.renderi(
              nullptr, Core.getInstance().getScreenWidth() / 2 - int4 / 2,
              Core.getInstance().getScreenHeight() / 2 - int6 / 2, int4, int6,
              0.0F, 0.0F, 0.0F, 0.75F, nullptr);
          TextManager.instance.DrawStringCentre(
              Core.getInstance().getScreenWidth() / 2,
              Core.getInstance().getScreenHeight() / 2 - int5 / 2, string, 1.0,
              1.0, 1.0, 1.0);
        }
        ...
        for (int int7 = 0; int7 < IsoPlayer.numPlayers; int7++) {
          if (IsoPlayer.players[int7] != nullptr &&
              !playerFadeInfo[int7].isFadeBeforeUI()) {
  ...
  static void resize() {
    ...
    if (getClock() != nullptr) {
      ...
      for (int int0 = 0; int0 < 4; int0++) {
        ...
        MoodleUI[int0].setVisible(VisibleAllUI &&
  ...
  static Vector2 getTileFromMouse(double mx, double my, double z) {
    PickedTile.x =
        IsoUtils.XToIso((float)(mx - 0.0), (float)(my - 0.0), (float)z);
    PickedTile.y =
        IsoUtils.YToIso((float)(mx - 0.0), (float)(my - 0.0), (float)z);
    ...
    if (!bSuspend) {
      ...
      if (Mouse.isLeftPressed()) {
        ...
        for (int int9 = UI.size() - 1; int9 >= 0; int9--) {
          ...
          if ((getModal() == nullptr || getModal() == uIElement6 ||
               !getModal().isVisible()) &&
              (uIElement6.isIgnoreLossControl() ||
               !TutorialManager.instance.StealControl) &&
              uIElement6.isVisible()) {
            if ((!(int1 >= uIElement6.getX()) || !(int2 >= uIElement6.getY()) ||
                 !(int1 < uIElement6.getX() + uIElement6.getWidth()) ||
                 !(int2 < uIElement6.getY() + uIElement6.getHeight())) &&
                !uIElement6.isCapture()) {
              uIElement6.onMouseDownOutside(int1 - uIElement6.getX().intValue(),
                                            int2 -
                                                uIElement6.getY().intValue());
            } else if (uIElement6.onMouseDown(
                           int1 - uIElement6.getX().intValue(),
                           int2 - uIElement6.getY().intValue())) {
              boolean0 = true;
              break;
            }
          }
        ...
        if (checkPicked() && !boolean0) {
          LuaEventManager.triggerEvent("OnObjectLeftMouseButtonDown",
                                       Picked.tile,
      ...
      if (Mouse.isLeftReleased()) {
        ...
        for (int int10 = UI.size() - 1; int10 >= 0; int10--) {
          ...
          if ((uIElement7.isIgnoreLossControl() ||
               !TutorialManager.instance.StealControl) &&
              uIElement7.isVisible() &&
              (getModal() == nullptr || getModal() == uIElement7 ||
               !getModal().isVisible())) {
            if ((!(int1 >= uIElement7.getX()) || !(int2 >= uIElement7.getY()) ||
                 !(int1 < uIElement7.getX() + uIElement7.getWidth()) ||
                 !(int2 < uIElement7.getY() + uIElement7.getHeight())) &&
                !uIElement7.isCapture()) {
              uIElement7.onMouseUpOutside(int1 - uIElement7.getX().intValue(),
                                          int2 - uIElement7.getY().intValue());
            } else if (uIElement7.onMouseUp(int1 - uIElement7.getX().intValue(),
                                            int2 -
                                                uIElement7.getY().intValue())) {
              boolean3 = true;
              break;
            }
          }
        ...
        if (!boolean3) {
          LuaEventManager.triggerEvent("OnMouseUp",
                                       BoxedStaticValues.toDouble(int1),
                                       BoxedStaticValues.toDouble(int2));
          if (checkPicked() && !boolean0) {
            LuaEventManager.triggerEvent("OnObjectLeftMouseButtonUp",
                                         Picked.tile,
                                         BoxedStaticValues.toDouble(int1),
                                         BoxedStaticValues.toDouble(int2));
          }
      ...
      if (Mouse.isRightPressed()) {
        for (int int11 = UI.size() - 1; int11 >= 0; int11--) {
          ...
          if (uIElement8.isVisible() &&
              (getModal() == nullptr || getModal() == uIElement8 ||
               !getModal().isVisible())) {
            if ((!(int1 >= uIElement8.getX()) || !(int2 >= uIElement8.getY()) ||
                 !(int1 < uIElement8.getX() + uIElement8.getWidth()) ||
                 !(int2 < uIElement8.getY() + uIElement8.getHeight())) &&
                !uIElement8.isCapture()) {
              uIElement8.onRightMouseDownOutside(
                  int1 - uIElement8.getX().intValue(),
                  int2 - uIElement8.getY().intValue());
            } else if (uIElement8.onRightMouseDown(
                           int1 - uIElement8.getX().intValue(),
                           int2 - uIElement8.getY().intValue())) {
              boolean1 = true;
              break;
        ...
        if (!boolean1) {
          LuaEventManager.triggerEvent("OnRightMouseDown",
                                       BoxedStaticValues.toDouble(int1),
                                       BoxedStaticValues.toDouble(int2));
          if (checkPicked() && !boolean1) {
            LuaEventManager.triggerEvent("OnObjectRightMouseButtonDown",
                                         Picked.tile,
                                         BoxedStaticValues.toDouble(int1),
                                         BoxedStaticValues.toDouble(int2));
          }
        ...
        if (IsoWorld.instance.CurrentCell != nullptr &&
            getPicked() != nullptr && getSpeedControls() != nullptr &&
            !IsoPlayer.getInstance().isAiming() &&
            !IsoPlayer.getInstance().isAsleep() && !GameTime.isGamePaused()) {
      ...
      if (Mouse.isRightReleased()) {
        ...
        for (int int12 = UI.size() - 1; int12 >= 0; int12--) {
          ...
          if ((uIElement9.isIgnoreLossControl() ||
               !TutorialManager.instance.StealControl) &&
              uIElement9.isVisible() &&
              (getModal() == nullptr || getModal() == uIElement9 ||
               !getModal().isVisible())) {
            if ((!(int1 >= uIElement9.getX()) || !(int2 >= uIElement9.getY()) ||
                 !(int1 < uIElement9.getX() + uIElement9.getWidth()) ||
                 !(int2 < uIElement9.getY() + uIElement9.getHeight())) &&
                !uIElement9.isCapture()) {
              uIElement9.onRightMouseUpOutside(
                  int1 - uIElement9.getX().intValue(),
                  int2 - uIElement9.getY().intValue());
            } else if (uIElement9.onRightMouseUp(
                           int1 - uIElement9.getX().intValue(),
                           int2 - uIElement9.getY().intValue())) {
              boolean5 = true;
              break;
        ...
        if (!boolean5) {
          LuaEventManager.triggerEvent("OnRightMouseUp",
                                       BoxedStaticValues.toDouble(int1),
                                       BoxedStaticValues.toDouble(int2));
          if (checkPicked()) {
            bool boolean6 = true;
            if (GameClient.bClient && Picked.tile.getSquare() != nullptr) {
              SafeHouse safeHouse = SafeHouse.isSafeHouse(
                  Picked.tile.getSquare(),
                  IsoPlayer.getInstance().getUsername(), true);
            ...
            if (boolean6) {
              LuaEventManager.triggerEvent("OnObjectRightMouseButtonUp",
                                           Picked.tile,
                                           BoxedStaticValues.toDouble(int1),
                                           BoxedStaticValues.toDouble(int2));
            }
      ...
      if (wheel != lastwheel) {
        ...
        for (int int14 = UI.size() - 1; int14 >= 0; int14--) {
          ...
          if ((uIElement10.isIgnoreLossControl() ||
               !TutorialManager.instance.StealControl) &&
              uIElement10.isVisible() &&
              (uIElement10.isPointOver(int1, int2) ||
               uIElement10.isCapture()) &&
      ...
      if (getLastMouseX() != int1 || getLastMouseY() != int2) {
        for (int int15 = UI.size() - 1; int15 >= 0; int15--) {
          ...
          if ((uIElement11.isIgnoreLossControl() ||
               !TutorialManager.instance.StealControl) &&
              uIElement11.isVisible()) {
            if ((!(int1 >= uIElement11.getX()) ||
              ...
              uIElement11.onMouseMoveOutside(int1 - getLastMouseX(),
                                             int2 - getLastMouseY());
            } else if (!boolean2 &&
                       uIElement11.onMouseMove(int1 - getLastMouseX(),
                                               int2 - getLastMouseY())) {
              boolean2 = true;
            }
          }
      ...
      if (!boolean2 && IsoPlayer.players[0] != nullptr) {
        ...
        if (IsoCamera.CamCharacter != nullptr) {
          setPickedTile(
              getTileFromMouse(int3, int4, (int)IsoPlayer.players[0].getZ()));
        }

        LuaEventManager.triggerEvent(
            "OnMouseMove", BoxedStaticValues.toDouble(int1),
            BoxedStaticValues.toDouble(int2), BoxedStaticValues.toDouble(int3),
  ...
  static bool checkPicked() {
    return Picked != nullptr && Picked.tile != nullptr &&
           Picked.tile.getObjectIndex() != -1;
  }

  static void handleZoomKeys() {
    bool boolean0 = true;
    if (Core.CurrentTextEntryBox != nullptr &&
        Core.CurrentTextEntryBox.IsEditable &&
   ...
   */
public
  static IsoObjectPicker.ClickObject getPicked() { return Picked; }
   ...
   * @return the clock
   */
  static Clock getClock() { return clock; }

  /**
   *
   * @param aClock the clock to set
   */
  static void setClock(Clock aClock) { clock = aClock; }

  /**
   * @return the UI
   */
public
  static ArrayList<UIElement> getUI() { return UI; }

  /**
   *
   * @param aUI the UI to set
   */
  static void setUI(ArrayList<UIElement> aUI) { PZArrayUtil.copy(UI, aUI); }

  /**
   * @return the toolTip
   */
  static ObjectTooltip getToolTip() { return toolTip; }

  /**
   *
   * @param aToolTip the toolTip to set
   */
  static void setToolTip(ObjectTooltip aToolTip) { toolTip = aToolTip; }

  /**
   * @return the mouseArrow
   */
  static Texture getMouseArrow() { return mouseArrow; }

  /**
   *
   * @param aMouseArrow the mouseArrow to set
   */
  static void setMouseArrow(Texture aMouseArrow) { mouseArrow = aMouseArrow; }

  /**
   * @return the mouseExamine
   */
  static Texture getMouseExamine() { return mouseExamine; }
   ...
   */
  static Texture getMouseAttack() { return mouseAttack; }
   ...
   */
  static Texture getMouseGrab() { return mouseGrab; }

  /**
   *
   * @param aMouseGrab the mouseGrab to set
   */
  static void setMouseGrab(Texture aMouseGrab) { mouseGrab = aMouseGrab; }

  /**
   * @return the speedControls
   */
  static SpeedControls getSpeedControls() { return speedControls; }
   ...
   */
  static UIDebugConsole getDebugConsole() { return DebugConsole; }
   ...
   */
  static UIServerToolbox getServerToolbox() { return ServerToolbox; }
   ...
   * @return the MoodleUI
   */
  static MoodlesUI getMoodleUI(double index) { return MoodleUI[(int)index]; }
   ...
   */
  static bool isbFadeBeforeUI() { return bFadeBeforeUI; }
   ...
   */
  static double getFadeAlpha() { return BoxedStaticValues.toDouble(FadeAlpha); }

  /**
   *
   * @param aFadeAlpha the FadeAlpha to set
   */
  static void setFadeAlpha(double aFadeAlpha) { FadeAlpha = (float)aFadeAlpha; }
   ...
   */
  static bool isFadingOut() { return FadingOut ? Boolean.TRUE : Boolean.FALSE; }

  /**
   *
   * @param aFadingOut the FadingOut to set
   */
  static void setFadingOut(bool aFadingOut) { FadingOut = aFadingOut; }

  /**
   * @return the lastMouseTexture
   */
  static Texture getLastMouseTexture() { return lastMouseTexture; }
   ...
   */
  static IsoObject getLastPicked() { return LastPicked; }

  /**
   *
   * @param aLastPicked the LastPicked to set
   */
  static void setLastPicked(IsoObject aLastPicked) { LastPicked = aLastPicked; }

  /**
   * @return the DoneTutorials
   */
public
  static ArrayList<String> getDoneTutorials() { return DoneTutorials; }
   ...
   * @return the lastOffX
   */
  static float getLastOffX() { return lastOffX; }

  /**
   *
   * @param aLastOffX the lastOffX to set
   */
  static void setLastOffX(float aLastOffX) { lastOffX = aLastOffX; }

  /**
   * @return the lastOffY
   */
  static float getLastOffY() { return lastOffY; }

  /**
   *
   * @param aLastOffY the lastOffY to set
   */
  static void setLastOffY(float aLastOffY) { lastOffY = aLastOffY; }

  /**
   * @return the Modal
   */
  static ModalDialog getModal() { return Modal; }

  /**
   *
   * @param aModal the Modal to set
   */
  static void setModal(ModalDialog aModal) { Modal = aModal; }

  /**
   * @return the black
   */
  static Texture getBlack() { return black; }

  /**
   *
   * @param aBlack the black to set
   */
  static void setBlack(Texture aBlack) { black = aBlack; }

  /**
   * @return the lastAlpha
   */
  static float getLastAlpha() { return lastAlpha; }

  /**
   *
   * @param aLastAlpha the lastAlpha to set
   */
  static void setLastAlpha(float aLastAlpha) { lastAlpha = aLastAlpha; }

  /**
   * @return the PickedTileLocal
   */
  static Vector2 getPickedTileLocal() { return PickedTileLocal; }
   ...
   */
  static Vector2 getPickedTile() { return PickedTile; }
   ...
   */
  static IsoObject getRightDownObject() { return RightDownObject; }
  ...
  static void pushToTop(UIElement uIElement) { toTop.add(uIElement); }

  static bool isShowPausedMessage() { return showPausedMessage; }
    ...
    if (bShowLuaDebuggerOnError) {
      ...
      if (!GameServer.bServer) {
        if (!(GameWindow.states.current instanceof GameLoadingState)) {
          ...
          if (debugUI.size() == 0) {
            LuaManager.debugcaller.pcall(LuaManager.debugthread,
                                         LuaManager.env.rawget("DoLuaDebugger"),
                                         filename, pc);
          } else {
            UI.addAll(debugUI);
            LuaManager.debugcaller.pcall(
                LuaManager.debugthread,
                LuaManager.env.rawget("DoLuaDebuggerOnBreak"), filename, pc);
          }
          ...
          while (!boolean1) {
            ...
            if (!GameWindow.bLuaDebuggerKeyDown &&
                GameKeyboard.isKeyDown(
                    Core.getInstance().getKey("Toggle Lua Debugger"))) {
            ...
            if ("StepOver".equalsIgnoreCase(string)) {
              LuaManager.thread.bStep = true;
              LuaManager.thread.bStepInto = false;
              LuaManager.thread.lastCallFrame =
                  LuaManager.thread.getCurrentCoroutine().getCallframeTop();
              executeGame(arrayList, boolean0, int0);
              return;
            }
              ...
              if (boolean2) {
                if (!booleans[int2]) {
                  ...
                  if (int2 + 1 == 6) {
                    ...
                    LuaManager.thread.bStepInto = false;
                    LuaManager.thread.lastCallFrame =
                        LuaManager.thread.getCurrentCoroutine()
                            .getCallframeTop();
                    executeGame(arrayList, boolean0, int0);
                    return;
                  }
            ...
            Core.getInstance().EndFrameUI();
            resize();
            if (!GameKeyboard.isKeyDown(
                    Core.getInstance().getKey("Toggle Lua Debugger"))) {
              GameWindow.bLuaDebuggerKeyDown = false;
            }

            sync.endFrame();
            Core.getInstance().setScreenSize(RenderThread.getDisplayWidth(),
                                             RenderThread.getDisplayHeight());
          }
        }
      }
    }
  ...
  static void executeGame(ArrayList<UIElement> arrayList, bool boolean0,
                          int int0) {
    debugUI.clear();
    debugUI.addAll(UI);
    UI.clear();
  ...
  static double getDoubleClickDist() { return BoxedStaticValues.toDouble(5.0); }

  static bool isDoubleClick(double x1, double y1, double x2, double y2,
                            double clickTime) {
    if (Math.abs(x2 - x1) > getDoubleClickDist()) {
      ...
      return System.currentTimeMillis() - clickTime > getDoubleClickInterval()
                 ? Boolean.FALSE
                 : Boolean.TRUE;
    }
  }
  ...
  static void updateTooltip(double double1, double double0) {
    ...
    for (int int0 = getUI().size() - 1; int0 >= 0; int0--) {
      ...
      if (uIElement1 != toolTip && uIElement1.isVisible() &&
          double1 >= uIElement1.getX() && double0 >= uIElement1.getY() &&
          double1 < uIElement1.getX() + uIElement1.getWidth() &&
          double0 < uIElement1.getY() + uIElement1.getHeight() &&
          (uIElement1.maxDrawHeight == -1 ||
           double0 < uIElement1.getY() + uIElement1.maxDrawHeight)) {
      ...
      if (object != getLastPicked() && toolTip != nullptr) {
        ...
        if (object.haveSpecialTooltip()) {
          if (getToolTip().Object != object) {
            getToolTip().show(object, (double)((int)double1 + 8),
                              (double)((int)double0 + 16));
    ...
    if (toolTip != nullptr &&
        (object == nullptr ||
         toolTip.alpha <= 0.0F && toolTip.targetAlpha <= 0.0F)) {
      toolTip.hide();
    }
  }

  static void setPlayerInventory(int playerIndex, UIElement inventory,
                                 UIElement loot) {
    if (playerIndex == 0) {
  ...
  static void setPlayerInventoryTooltip(int playerIndex, UIElement inventory,
                                        UIElement loot) {
    if (playerIndex == 0) {
  ...
  static bool isMouseOverInventory() {
    if (playerInventoryTooltip != nullptr &&
        playerInventoryTooltip.isMouseOver()) {
      return true;
    } else if (playerLootTooltip != nullptr &&
               playerLootTooltip.isMouseOver()) {
      return true;
    } else if (playerInventoryUI != nullptr && playerLootUI != nullptr) {
      return playerInventoryUI.getMaxDrawHeight() == -1.0 &&
                     playerInventoryUI.isMouseOver()
                 ? true
                 : playerLootUI.getMaxDrawHeight() == -1.0 &&
                       playerLootUI.isMouseOver();
    } else {
      return false;
    }
  }
  ...
  static void setVisibleAllUI(bool visible) { VisibleAllUI = visible; }
  ...
  static bool isFBOActive() { return useUIFBO; }

  static double getMillisSinceLastUpdate() { return uiUpdateIntervalMS; }

  static double getSecondsSinceLastUpdate() {
    return uiUpdateIntervalMS / 1000.0;
  }

  static double getMillisSinceLastRender() { return uiRenderIntervalMS; }
  ...
  static bool isForceCursorVisible() {
    for (int int0 = UI.size() - 1; int0 >= 0; int0--) {
      ...
      if (uIElement.isVisible() &&
          (uIElement.isForceCursorVisible() || uIElement.isMouseOver())) {
        return true;
      }
    }
...
private
  ...
  public
    FadeInfo(int arg0) { this.playerIndex = arg0; }

    bool isFadeBeforeUI() { return this.bFadeBeforeUI; }

    void setFadeBeforeUI(bool arg0) { this.bFadeBeforeUI = arg0; }

    float getFadeAlpha() { return this.FadeAlpha; }

    void setFadeAlpha(float arg0) { this.FadeAlpha = arg0; }

    int getFadeTime() { return this.FadeTime; }

    void setFadeTime(int arg0) { this.FadeTime = arg0; }

    int getFadeTimeMax() { return this.FadeTimeMax; }

    void setFadeTimeMax(int arg0) { this.FadeTimeMax = arg0; }

    bool isFadingOut() { return this.FadingOut; }

    void setFadingOut(bool arg0) { this.FadingOut = arg0; }

    void FadeIn(int arg0) {
      this.setFadeTimeMax(
          (int)(arg0 * 30 * (PerformanceSettings.getLockFPS() / 30.0F)));
    ...
    void update() { this.setFadeTime(this.getFadeTime() - 1); }
    ...
    void render() {
      ...
      if (!(this.getFadeAlpha() <= 0.0F)) {
        ...
        UIManager.DrawTexture(UIManager.getBlack(), int0, int1, int2, int3,
                              this.getFadeAlpha());
      }
    }
  }
