...
private
  final ArrayList<String> incomingConnections = std::make_unique<ArrayList<>>();
  DialogButton buttonAccept;
  DialogButton buttonReject;
  std::string externalAddress = null;
  std::string steamID = null;
  bool autoAccept = false;

public
  UIServerToolbox(int int0, int int1) {
    ...
    uint8_t byte1 = 37;
    this.OutputLog =
        new UITextBox2(UIFont.Small, 5, 33, 330, 260,
                       Translator.getText("IGUI_ServerToolBox_Status"), true);
    this.OutputLog.multipleLine = true;
    this.ScrollBarV = new ScrollBar(
        "ServerToolboxScrollbar", this,
    ...
    this.buttonAccept = new DialogButton(
        this, 30, 225, Translator.getText("IGUI_ServerToolBox_acccept"),
        "accept");
    this.buttonReject = new DialogButton(
        this, 80, 225, Translator.getText("IGUI_ServerToolBox_reject"),
        "reject");
    ...
    void OnCoopServerMessage(const std::string& var1, const std::string& var2, const std::string& string0) {
            ...
            UIServerToolbox.this.PrintLine(Translator.getText(
                "IGUI_ServerToolBox_ServerAddress", string1));
    ...
    void OnCoopServerMessage(const std::string& var1, const std::string& var2, const std::string& string) {
                ...
                void render() {
                  if (this.isVisible()) {
                    super.render();
                    this.DrawTextCentre(
                        Translator.getText("IGUI_ServerToolBox_Title"),
                        this.getWidth() / 2.0, 2.0, 1.0, 1.0, 1.0, 1.0);
                    std::string string0 =
                        "null".equals(this.externalAddress)
                            ? Translator.getText("IGUI_ServerToolBox_IPUnknown")
                            : this.externalAddress;
                    this.DrawText(Translator.getText(
                                      "IGUI_ServerToolBox_ExternalIP", string0),
                                  7.0, 19.0, 0.7F, 0.7F, 1.0, 1.0);
                    if (!this.incomingConnections.isEmpty()) {
                      std::string string1 = this.incomingConnections.get(0);
                      if (string1 != nullptr) {
                        this.DrawText(
                            Translator.getText(
                                "IGUI_ServerToolBox_UserConnecting", string1),
                            10.0, 205.0, 0.7F, 0.7F, 1.0, 1.0);
                      }
                    }
                  }
                }
                ...
                void UpdateViewPos() {
                  this.OutputLog.TopLineIndex = this.OutputLog.Lines.size() -
                                                this.OutputLog.NumVisibleLines;
              ...
              public
                synchronized void OnCoopServerMessage(String string0,
                                                      String var2,
                                                      String string1) {
                  if (Objects == string0, "login-attempt")) {
                      ...
                      if (this.autoAccept) {
                        ...
                        CoopMaster.instance.sendMessage("approve-login-attempt",
                                                        string1);
                      } else {
                ...
                void Selected(const std::string &string0, int var2, int var3) {
                  ...
                  if (Objects == string0, "reject")) {
                      ...
                      CoopMaster.instance.sendMessage("reject-login-attempt",
                                                      string2);
                    }
                }
                }
