...
public:
private
  final ArrayList<String> specificProfessionDistribution =
      std::make_unique<ArrayList<>>();
private
  final Map<String, String> specificProfessionRoomDistribution =
      std::make_unique<HashMap<>>();
private
  static final HashMap<Integer, String> kitchenSinkItems =
      std::make_unique<HashMap<>>();
private
  static final HashMap<Integer, String> kitchenCounterItems =
      std::make_unique<HashMap<>>();
private
  static final HashMap<Integer, String> kitchenStoveItems =
      std::make_unique<HashMap<>>();
private
  static final HashMap<Integer, String> bathroomSinkItems =
      std::make_unique<HashMap<>>();
private
  final ArrayList<String> coldFood = std::make_unique<ArrayList<>>();
private
  final Map<String, String> plankStash = std::make_unique<HashMap<>>();
private
  final ArrayList<RandomizedDeadSurvivorBase> deadSurvivorsStory =
      std::make_unique<ArrayList<>>();
  int totalChanceRDS = 0;
private
  static final HashMap<RandomizedDeadSurvivorBase, Integer> rdsMap =
      std::make_unique<HashMap<>>();
private
  static final ArrayList<String> uniqueRDSSpawned =
      std::make_unique<ArrayList<>>();
private
  ArrayList<IsoObject> tablesDone = std::make_unique<ArrayList<>>();
  ...
  void randomizeBuilding(BuildingDef def) {
    ...
    std::string string = this.specificProfessionDistribution.get(
        Rand.Next(0, this.specificProfessionDistribution.size()));
    ItemPickerJava.ItemPickerRoom itemPickerRoom =
        ItemPickerJava.rooms.get(string);
    ...
    for (int int0 = def.x - 1; int0 < def.x2 + 1; int0++) {
      for (int int1 = def.y - 1; int1 < def.y2 + 1; int1++) {
        for (int int2 = 0; int2 < 8; int2++) {
          ...
          if (square != nullptr) {
            if (boolean1 && square.getFloor() != nullptr &&
                this.plankStash.containsKey(
                    square.getFloor().getSprite().getName())) {
              IsoThumpable thumpable = new IsoThumpable(
                  square.getCell(), square,
                  this.plankStash.get(square.getFloor().getSprite().getName()),
                  false, nullptr);
              thumpable.setIsThumpable(false);
              thumpable.container =
                  new ItemContainer("plankstash", square, thumpable);
            ...
            for (int int3 = 0; int3 < square.getObjects().size(); int3++) {
              ...
              if (Rand.Next(100) <= 65 && object instanceof
                  IsoDoor && !((IsoDoor)object).isExteriorDoor(nullptr)) {
                ((IsoDoor)object).ToggleDoorSilent();
                ((IsoDoor)object)
                    .syncIsoObject(true, (byte)1, nullptr, nullptr);
              }
              ...
              if (boolean0 && Rand.Next(100) <= 70 &&
                  object.getContainer() != nullptr &&
                  square.getRoom() != nullptr &&
                  square.getRoom().getName() != nullptr &&
                  this.specificProfessionRoomDistribution.get(string).contains(
                      square.getRoom().getName()) &&
                  itemPickerRoom.Containers.containsKey(
                      object.getContainer().getType())) {
                object.getContainer().clear();
                arrayList.add(object.getContainer());
                object.getContainer().setExplored(true);
              }

              if (Rand.Next(100) < 15 && object.getContainer() != nullptr &&
                  object.getContainer().getType() == "stove")) {
                  InventoryItem item = object.getContainer().AddItem(
                      this.coldFood.get(Rand.Next(0, this.coldFood.size())));
                  item.setCooked(true);
                  item.setAutoAge();
                }

              if (!this.tablesDone.contains(object) &&
                  object.getProperties().isTable() &&
                  object.getContainer() == nullptr && !this.doneTable) {
                this.checkForTableSpawn(def, object);
              }
            }

            if (square.getRoom() != nullptr &&
                "kitchen" == square.getRoom().getName())) {
                            this.doKitchenStuff(square);
                        }

            if (square.getRoom() != nullptr &&
                "bathroom" == square.getRoom().getName())) {
                            this.doBathroomStuff(square);
                        }

            if (square.getRoom() != nullptr &&
                "bedroom" == square.getRoom().getName())) {
                            this.doBedroomStuff(square);
                        }

            if (square.getRoom() != nullptr &&
                "livingroom" == square.getRoom().getName())) {
    ...
    if (!boolean0 && object0 != nullptr) {
      ...
      if (directions != nullptr) {
        if (directions == IsoDirections.E) {
          this.addWorldItem(string, square, 0.4F, Rand.Next(0.34F, 0.74F),
                            object0.getSurfaceOffsetNoTable() / 96.0F);
        }

        if (directions == IsoDirections.W) {
          this.addWorldItem(string, square, 0.64F, Rand.Next(0.34F, 0.74F),
                            object0.getSurfaceOffsetNoTable() / 96.0F);
        }

        if (directions == IsoDirections.N) {
          this.addWorldItem(string, square, Rand.Next(0.44F, 0.64F), 0.67F,
                            object0.getSurfaceOffsetNoTable() / 96.0F);
        }

        if (directions == IsoDirections.S) {
          this.addWorldItem(string, square, Rand.Next(0.44F, 0.64F), 0.42F,
                            object0.getSurfaceOffsetNoTable() / 96.0F);
        }
      }
    }
  ...
  void doBedroomStuff(IsoGridSquare square) {
    for (int int0 = 0; int0 < square.getObjects().size(); int0++) {
      ...
      if (Rand.NextBool(7) &&
          object.getSprite().getName().contains("bedding") &&
          object.getProperties().Val("BedType") != nullptr) {
        int int1 = Rand.Next(0, 14);
        switch (int1) {
        case 0:
          this.addWorldItem("Shirt_FormalTINT", square, 0.6F, 0.6F,
                            object.getSurfaceOffsetNoTable() / 96.0F);
          break;
        case 1:
          this.addWorldItem("Shirt_FormalWhite_ShortSleeveTINT", square, 0.6F,
                            0.6F, object.getSurfaceOffsetNoTable() / 96.0F);
          break;
        case 2:
          this.addWorldItem("Tshirt_DefaultDECAL_TINT", square, 0.6F, 0.6F,
                            object.getSurfaceOffsetNoTable() / 96.0F);
          break;
        case 3:
          this.addWorldItem("Tshirt_PoloStripedTINT", square, 0.6F, 0.6F,
                            object.getSurfaceOffsetNoTable() / 96.0F);
          break;
        case 4:
          this.addWorldItem("Tshirt_PoloTINT", square, 0.6F, 0.6F,
                            object.getSurfaceOffsetNoTable() / 96.0F);
          break;
        case 5:
          this.addWorldItem("Jacket_WhiteTINT", square, 0.6F, 0.6F,
                            object.getSurfaceOffsetNoTable() / 96.0F);
          break;
        case 6:
          this.addWorldItem("Jumper_DiamondPatternTINT", square, 0.6F, 0.6F,
                            object.getSurfaceOffsetNoTable() / 96.0F);
          break;
        case 7:
          this.addWorldItem("Jumper_TankTopDiamondTINT", square, 0.6F, 0.6F,
                            object.getSurfaceOffsetNoTable() / 96.0F);
          break;
        case 8:
          this.addWorldItem("HoodieDOWN_WhiteTINT", square, 0.6F, 0.6F,
                            object.getSurfaceOffsetNoTable() / 96.0F);
          break;
        case 9:
          this.addWorldItem("Trousers_DefaultTEXTURE_TINT", square, 0.6F, 0.6F,
                            object.getSurfaceOffsetNoTable() / 96.0F);
          break;
        case 10:
          this.addWorldItem("Trousers_WhiteTINT", square, 0.6F, 0.6F,
                            object.getSurfaceOffsetNoTable() / 96.0F);
          break;
        case 11:
          this.addWorldItem("Trousers_Denim", square, 0.6F, 0.6F,
                            object.getSurfaceOffsetNoTable() / 96.0F);
          break;
        case 12:
          this.addWorldItem("Trousers_Padded", square, 0.6F, 0.6F,
                            object.getSurfaceOffsetNoTable() / 96.0F);
          break;
        case 13:
          this.addWorldItem("TrousersMesh_DenimLight", square, 0.6F, 0.6F,
                            object.getSurfaceOffsetNoTable() / 96.0F);
        }
      }

      if (Rand.NextBool(7) && object.getContainer() != nullptr &&
          "sidetable" == object.getContainer().getType())) {
          ...
          if (directions != nullptr) {
            if (directions == IsoDirections.E) {
              this.addWorldItem(string, square, 0.42F, Rand.Next(0.34F, 0.74F),
                                object.getSurfaceOffsetNoTable() / 96.0F);
            }
  ...
  void doKitchenStuff(IsoGridSquare square) {
    ...
    for (int int0 = 0; int0 < square.getObjects().size(); int0++) {
      ...
      if (!boolean0 && object0.getSprite().getName().contains("sink") &&
          Rand.NextBool(4)) {
        IsoDirections directions0 = this.getFacing(object0.getSprite());
        if (directions0 != nullptr) {
          this.generateSinkClutter(directions0, object0, square,
                                   kitchenSinkItems);
          boolean0 = true;
        }
      } else if (!boolean1 && object0.getContainer() != nullptr &&
                 "counter" == object0.getContainer().getType()) && Rand.NextBool(6)) {
          bool boolean2 = true;
          ...
          for (int int1 = 0; int1 < square.getObjects().size(); int1++) {
            ...
            if (object1.getSprite() != nullptr &&
                        ...
                        object1.getSprite().getName().contains("sink") ||
                    object1 instanceof
                IsoStove || object1 instanceof IsoRadio) {
              boolean2 = false;
              break;
            }
          }
          ...
          if (boolean2) {
            ...
            if (directions1 != nullptr) {
              this.generateCounterClutter(directions1, object0, square,
                                          kitchenCounterItems);
              boolean1 = true;
            }
          }
               ...
               IsoStove && object0.getContainer() != nullptr &&
                   "stove" == object0.getContainer().getType()) && Rand.NextBool(4)) {
  ...
  void doBathroomStuff(IsoGridSquare square) {
    ...
    for (int int0 = 0; int0 < square.getObjects().size(); int0++) {
      ...
      if (!boolean0 && !boolean1 &&
          object0.getSprite().getName().contains("sink") && Rand.NextBool(5) &&
          object0.getSurfaceOffsetNoTable() > 0.0F) {
        IsoDirections directions0 = this.getFacing(object0.getSprite());
        if (directions0 != nullptr) {
          this.generateSinkClutter(directions0, object0, square,
                                   bathroomSinkItems);
      ...
      } else if (!boolean0 && !boolean1 && object0.getContainer() != nullptr &&
                 "counter" == object0.getContainer().getType()) && Rand.NextBool(5)) {
          bool boolean2 = true;
  ...
  void generateKitchenStoveClutter(IsoDirections directions, IsoObject object,
                                   IsoGridSquare square) {
    int int0 = Rand.Next(1, 3);
    std::string string =
        kitchenStoveItems.get(Rand.Next(1, kitchenStoveItems.size()));
    if (directions == IsoDirections.W) {
      switch (int0) {
      case 1:
        this.addWorldItem(string, square, 0.5703125F, 0.8046875F,
                          object.getSurfaceOffsetNoTable() / 96.0F);
        break;
      case 2:
        this.addWorldItem(string, square, 0.5703125F, 0.2578125F,
                          object.getSurfaceOffsetNoTable() / 96.0F);
      }
    }
    ...
    if (directions == IsoDirections.E) {
      switch (int0) {
      case 1:
        this.addWorldItem(string, square, 0.5F, 0.7890625F,
                          object.getSurfaceOffsetNoTable() / 96.0F);
        break;
      case 2:
        this.addWorldItem(string, square, 0.5F, 0.1875F,
                          object.getSurfaceOffsetNoTable() / 96.0F);
      }
    }
    ...
    if (directions == IsoDirections.S) {
      switch (int0) {
      case 1:
        this.addWorldItem(string, square, 0.3125F, 0.53125F,
                          object.getSurfaceOffsetNoTable() / 96.0F);
        break;
      case 2:
        this.addWorldItem(string, square, 0.875F, 0.53125F,
                          object.getSurfaceOffsetNoTable() / 96.0F);
      }
    }
    ...
    if (directions == IsoDirections.N) {
      switch (int0) {
      case 1:
        this.addWorldItem(string, square, 0.3203F, 0.523475F,
                          object.getSurfaceOffsetNoTable() / 96.0F);
        break;
      case 2:
        this.addWorldItem(string, square, 0.8907F, 0.523475F,
                          object.getSurfaceOffsetNoTable() / 96.0F);
      }
    }
  }

  void generateCounterClutter(IsoDirections directions, IsoObject object,
    ...
    for (int int4 = 0; int4 < arrayList0.size(); int4++) {
      ...
      if (directions == IsoDirections.S) {
        switch (int5) {
        case 1:
          this.addWorldItem(string, square, 0.138F, Rand.Next(0.2F, 0.523F),
                            object.getSurfaceOffsetNoTable() / 96.0F);
          break;
        case 2:
          this.addWorldItem(string, square, 0.383F, Rand.Next(0.2F, 0.523F),
                            object.getSurfaceOffsetNoTable() / 96.0F);
          break;
        case 3:
          this.addWorldItem(string, square, 0.633F, Rand.Next(0.2F, 0.523F),
                            object.getSurfaceOffsetNoTable() / 96.0F);
          break;
        case 4:
          this.addWorldItem(string, square, 0.78F, Rand.Next(0.2F, 0.523F),
                            object.getSurfaceOffsetNoTable() / 96.0F);
        }
      }
      ...
      if (directions == IsoDirections.N) {
        ...
        case 1:
          square.AddWorldInventoryItem(
              string, 0.133F, Rand.Next(0.53125F, 0.9375F),
              object.getSurfaceOffsetNoTable() / 96.0F);
          break;
        case 2:
          square.AddWorldInventoryItem(
              string, 0.38F, Rand.Next(0.53125F, 0.9375F),
              object.getSurfaceOffsetNoTable() / 96.0F);
          break;
        case 3:
          square.AddWorldInventoryItem(
              string, 0.625F, Rand.Next(0.53125F, 0.9375F),
              object.getSurfaceOffsetNoTable() / 96.0F);
          break;
        case 4:
          square.AddWorldInventoryItem(
              string, 0.92F, Rand.Next(0.53125F, 0.9375F),
              object.getSurfaceOffsetNoTable() / 96.0F);
        }
      }
      ...
      if (directions == IsoDirections.E) {
        switch (int5) {
        case 1:
          square.AddWorldInventoryItem(string, Rand.Next(0.226F, 0.593F), 0.14F,
                                       object.getSurfaceOffsetNoTable() /
                                           96.0F);
          break;
        case 2:
          square.AddWorldInventoryItem(string, Rand.Next(0.226F, 0.593F), 0.33F,
                                       object.getSurfaceOffsetNoTable() /
                                           96.0F);
          break;
        case 3:
          square.AddWorldInventoryItem(string, Rand.Next(0.226F, 0.593F), 0.64F,
                                       object.getSurfaceOffsetNoTable() /
                                           96.0F);
          break;
        case 4:
          square.AddWorldInventoryItem(string, Rand.Next(0.226F, 0.593F), 0.92F,
                                       object.getSurfaceOffsetNoTable() /
                                           96.0F);
        }
      }
      ...
      if (directions == IsoDirections.W) {
        ...
        case 1:
          square.AddWorldInventoryItem(
              string, Rand.Next(0.5859375F, 0.9F), 0.21875F,
              object.getSurfaceOffsetNoTable() / 96.0F);
          break;
        case 2:
          square.AddWorldInventoryItem(
              string, Rand.Next(0.5859375F, 0.9F), 0.421875F,
              object.getSurfaceOffsetNoTable() / 96.0F);
          break;
        case 3:
          square.AddWorldInventoryItem(
              string, Rand.Next(0.5859375F, 0.9F), 0.71F,
              object.getSurfaceOffsetNoTable() / 96.0F);
          break;
        case 4:
          square.AddWorldInventoryItem(
              string, Rand.Next(0.5859375F, 0.9F), 0.9175F,
              object.getSurfaceOffsetNoTable() / 96.0F);
        }
      }
    }
  ...
  void generateSinkClutter(IsoDirections directions, IsoObject object,
                           IsoGridSquare square, String > hashMap) {
    int int0 = Math.min(5, hashMap.size() + 1);
    int int1 = Rand.Next(1, int0);
    std::vector arrayList0 = new ArrayList();
    ...
    for (int int4 = 0; int4 < arrayList0.size(); int4++) {
      ...
      if (directions == IsoDirections.S) {
        switch (int5) {
        case 1:
          this.addWorldItem(string, square, 0.71875F, 0.125F,
                            object.getSurfaceOffsetNoTable() / 96.0F);
          break;
        case 2:
          this.addWorldItem(string, square, 0.0935F, 0.21875F,
                            object.getSurfaceOffsetNoTable() / 96.0F);
          break;
        case 3:
          this.addWorldItem(string, square, 0.1328125F, 0.589375F,
                            object.getSurfaceOffsetNoTable() / 96.0F);
          break;
        case 4:
          this.addWorldItem(string, square, 0.7890625F, 0.589375F,
                            object.getSurfaceOffsetNoTable() / 96.0F);
        }
      }
      ...
      if (directions == IsoDirections.N) {
        switch (int5) {
        case 1:
          this.addWorldItem(string, square, 0.921875F, 0.921875F,
                            object.getSurfaceOffsetNoTable() / 96.0F);
          break;
        case 2:
          this.addWorldItem(string, square, 0.1640625F, 0.8984375F,
                            object.getSurfaceOffsetNoTable() / 96.0F);
          break;
        case 3:
          this.addWorldItem(string, square, 0.021875F, 0.5F,
                            object.getSurfaceOffsetNoTable() / 96.0F);
          break;
        case 4:
          this.addWorldItem(string, square, 0.8671875F, 0.5F,
                            object.getSurfaceOffsetNoTable() / 96.0F);
        }
      }
      ...
      if (directions == IsoDirections.E) {
        switch (int5) {
        case 1:
          this.addWorldItem(string, square, 0.234375F, 0.859375F,
                            object.getSurfaceOffsetNoTable() / 96.0F);
          break;
        case 2:
          this.addWorldItem(string, square, 0.59375F, 0.875F,
                            object.getSurfaceOffsetNoTable() / 96.0F);
          break;
        case 3:
          this.addWorldItem(string, square, 0.53125F, 0.125F,
                            object.getSurfaceOffsetNoTable() / 96.0F);
          break;
        case 4:
          this.addWorldItem(string, square, 0.210937F, 0.1328125F,
                            object.getSurfaceOffsetNoTable() / 96.0F);
        }
      }
      ...
      if (directions == IsoDirections.W) {
        switch (int5) {
        case 1:
          this.addWorldItem(string, square, 0.515625F, 0.109375F,
                            object.getSurfaceOffsetNoTable() / 96.0F);
          break;
        case 2:
          this.addWorldItem(string, square, 0.578125F, 0.890625F,
                            object.getSurfaceOffsetNoTable() / 96.0F);
          break;
        case 3:
          this.addWorldItem(string, square, 0.8828125F, 0.8984375F,
                            object.getSurfaceOffsetNoTable() / 96.0F);
          break;
        case 4:
          this.addWorldItem(string, square, 0.8671875F, 0.1653125F,
                            object.getSurfaceOffsetNoTable() / 96.0F);
        }
      }
    }
  ...
  void checkForTableSpawn(BuildingDef buildingDef, IsoObject object) {
    if (Rand.NextBool(10)) {
      RBTableStoryBase rBTableStoryBase =
          RBTableStoryBase.getRandomStory(object.getSquare(), object);
    ...
    if (!this.tablesDone.contains(object0) && square != nullptr) {
      for (int int0 = 0; int0 < square.getObjects().size(); int0++) {
        ...
        if (!this.tablesDone.contains(object1) &&
            ...
            object1.getContainer() == nullptr && object1 != object0) {
          return object1;
        }
      }
  ...
  void addRandomDeadSurvivorStory(BuildingDef buildingDef) {
    ...
    while (iterator.hasNext()) {
      RandomizedDeadSurvivorBase randomizedDeadSurvivorBase =
          (RandomizedDeadSurvivorBase)iterator.next();
  ...
  void initRDSMap(BuildingDef buildingDef) {
    ...
    for (int int0 = 0; int0 < this.deadSurvivorsStory.size(); int0++) {
      RandomizedDeadSurvivorBase randomizedDeadSurvivorBase =
          this.deadSurvivorsStory.get(int0);
      if (randomizedDeadSurvivorBase.isValid(buildingDef, false) &&
          ...
          (randomizedDeadSurvivorBase.isUnique() &&
               !getUniqueRDSSpawned().contains(
                   randomizedDeadSurvivorBase.getName()) ||
           !randomizedDeadSurvivorBase.isUnique())) {
        this.totalChanceRDS =
            this.totalChanceRDS + this.deadSurvivorsStory.get(int0).getChance();
        rdsMap.put(this.deadSurvivorsStory.get(int0),
                   this.deadSurvivorsStory.get(int0).getChance());
      }
    }
  }

  void doRandomDeadSurvivorStory(BuildingDef buildingDef,
...
public
  RBBasic() {
    ...
    this.plankStash.put("floors_interior_tilesandwood_01_40",
                        "floors_interior_tilesandwood_01_56");
    this.plankStash.put("floors_interior_tilesandwood_01_41",
                        "floors_interior_tilesandwood_01_57");
    this.plankStash.put("floors_interior_tilesandwood_01_42",
                        "floors_interior_tilesandwood_01_58");
    this.plankStash.put("floors_interior_tilesandwood_01_43",
                        "floors_interior_tilesandwood_01_59");
    this.plankStash.put("floors_interior_tilesandwood_01_44",
                        "floors_interior_tilesandwood_01_60");
    this.plankStash.put("floors_interior_tilesandwood_01_45",
                        "floors_interior_tilesandwood_01_61");
    this.plankStash.put("floors_interior_tilesandwood_01_46",
                        "floors_interior_tilesandwood_01_62");
    this.plankStash.put("floors_interior_tilesandwood_01_47",
                        "floors_interior_tilesandwood_01_63");
    this.plankStash.put("floors_interior_tilesandwood_01_52",
                        "floors_interior_tilesandwood_01_68");
...
public
  ArrayList<RandomizedDeadSurvivorBase> getSurvivorStories() {
    return this.deadSurvivorsStory;
  }

public
  ArrayList<String> getSurvivorProfession() {
    return this.specificProfessionDistribution;
  }

public
  static ArrayList<String> getUniqueRDSSpawned() { return uniqueRDSSpawned; }
}
} // namespace randomizedBuilding
} // namespace randomizedWorld
} // namespace zombie
