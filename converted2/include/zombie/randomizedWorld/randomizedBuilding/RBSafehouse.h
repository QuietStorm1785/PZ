...
/**
 * This building will be barricaded, have a lot of canned food but also lot of
 * zombies inside it
 */
...
public:
  void randomizeBuilding(BuildingDef def) {
    ...
    ItemPickerJava.ItemPickerRoom itemPickerRoom =
        ItemPickerJava.rooms.get("SafehouseLoot");
          ...
          if (square0 != nullptr) {
            for (int int3 = 0; int3 < square0.getObjects().size(); int3++) {
              ...
              if (object instanceof
                  IsoDoor && ((IsoDoor)object).isBarricadeAllowed() &&
                      !SpawnPoints.instance.isSpawnBuilding(def)) {
                IsoGridSquare square1 =
                    square0.getRoom() == null
                        ? square0
                        : ((IsoDoor)object).getOppositeSquare();
                if (square1 != nullptr && square1.getRoom() == nullptr) {
                  bool boolean0 = square1 != square0;
                  IsoBarricade barricade0 = IsoBarricade.AddBarricadeToObject(
                      (IsoDoor)object, boolean0);
              ...
              if (object instanceof IsoWindow) {
                IsoGridSquare square2 =
                    square0.getRoom() == null
                        ? square0
                        : ((IsoWindow)object).getOppositeSquare();
                if (((IsoWindow)object).isBarricadeAllowed() && int2 == 0 &&
                    square2 != nullptr && square2.getRoom() == nullptr) {
                  bool boolean1 = square2 != square0;
                  IsoBarricade barricade1 = IsoBarricade.AddBarricadeToObject(
                      (IsoWindow)object, boolean1);
              ...
              if (object.getContainer() != nullptr &&
                ...
                ItemPickerJava.fillContainerType(
                    itemPickerRoom, object.getContainer(), "", nullptr);
  ...
  void addZombies(BuildingDef buildingDef) {
    ...
    if (Rand.Next(100) <= 60) {
      RandomizedBuildingBase.createRandomDeadBody(
          this.getLivingRoomOrKitchen(buildingDef), Rand.Next(3, 7));
    }
