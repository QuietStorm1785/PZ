...
public:
public
  static ArrayList<RBTableStoryBase> allStories =
      std::make_unique<ArrayList<>>();
  static int totalChance = 0;
  int chance = 0;
protected
  ArrayList<String> rooms = std::make_unique<ArrayList<>>();
  bool need2Tables = false;
  bool ignoreAgainstWall = false;
  IsoObject table2 = null;
  IsoObject table1 = null;
  bool westTable = false;
private
  static final HashMap<RBTableStoryBase, Integer> rbtsmap =
      std::make_unique<HashMap<>>();
private
  static final ArrayList<IsoObject> tableObjects =
      std::make_unique<ArrayList<>>();
public
  ...
  static void initStories(IsoGridSquare square, IsoObject object) {
    ...
    for (int int0 = 0; int0 < allStories.size(); int0++) {
      ...
      if (rBTableStoryBase.isValid(square, object, false) &&
          rBTableStoryBase.isTimeValid(false)) {
  ...
  static RBTableStoryBase getRandomStory(IsoGridSquare square,
                                         IsoObject object) {
  ...
  bool isValid(IsoGridSquare square, IsoObject object, bool boolean0) {
    if (boolean0) {
      return true;
    } else if (this.rooms != nullptr && square.getRoom() != nullptr &&
               !this.rooms.contains(square.getRoom().getName())) {
      return false;
    } else {
      if (this.need2Tables) {
  ...
  IsoObject getSecondTable(IsoObject object0) {
    ...
    if (this.ignoreAgainstWall && square0.getWallFull()) {
      ...
      return object1 != nullptr && this.ignoreAgainstWall &&
                     square1.getWallFull()
                 ? nullptr
                 : object1;
    }
