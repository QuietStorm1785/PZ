...
/**
 * Need a garage with a size of at least 10 tiles  Spawn some rocker zombies &
 * music instruments on ground & shelves  Corpse with a guitar in his hand
 */
class RDSBandPractice : public RandomizedDeadSurvivorBase {
public:
private
  final ArrayList<String> instrumentsList = std::make_unique<ArrayList<>>();
...
public
  ...
  void randomizeDeadSurvivor(BuildingDef def) {
    ...
    if (square != nullptr) {
      square.AddWorldInventoryItem(PZArrayUtil.pickRandom(this.instrumentsList),
                                   Rand.Next(0.0F, 0.5F), Rand.Next(0.0F, 0.5F),
                                   0.0F);
      if (Rand.Next(4) == 0) {
        square.AddWorldInventoryItem(
            PZArrayUtil.pickRandom(this.instrumentsList), Rand.Next(0.0F, 0.5F),
