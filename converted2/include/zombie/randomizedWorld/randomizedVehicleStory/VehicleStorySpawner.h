...
private
  static final ObjectPool<VehicleStorySpawner.Element> s_elementPool =
      new ObjectPool<>(VehicleStorySpawner.Element::new);
private
  static final int[] s_AABB = new int[4];
public
  final ArrayList<VehicleStorySpawner.Element> m_elements =
      std::make_unique<ArrayList<>>();
public
  final HashMap<String, Object> m_storyParams = std::make_unique<HashMap<>>();

  static VehicleStorySpawner getInstance() { return instance; }
...
public
  VehicleStorySpawner.Element addElement(String id, float x, float y,
                                         float direction, float width,
                                         float height) {
    VehicleStorySpawner.Element element =
        s_elementPool.alloc().init(id, x, y, direction, width, height);
  ...
  void spawn(float worldX, float worldY, float worldZ, float angleRadians,
    ...
    for (int int0 = 0; int0 < this.m_elements.size(); int0++) {
      ...
      Vector2 vector =
          s_vector2_1.setLengthAndDirection(element.direction, 1.0F);
      ...
      element.direction = Vector2.getDirection(vector.x - element.position.x,
                                               vector.y - element.position.y);
      element.z = worldZ;
      element.square = IsoWorld.instance.CurrentCell.getGridSquare(
          (double)element.position.x, (double)element.position.y,
          (double)worldZ);
  ...
  Vector2 rotate(float centerX, float centerY, Vector2 v, float angleRadians) {
    float float0 = v.x;
    float float1 = v.y;
    v.x = centerX + (float)(float0 * Math.cos(angleRadians) -
                            float1 * Math.sin(angleRadians));
    v.y = centerY + (float)(float0 * Math.sin(angleRadians) +
                            float1 * Math.cos(angleRadians));
    return v;
  }

  void getAABB(float float4, float float6, float float2, float float1,
               float float0, int[] ints) {
    ...
    float float17 =
        PZMath.min(float9, PZMath.min(float11, PZMath.min(float13, float15)));
    float float18 =
        PZMath.max(float9, PZMath.max(float11, PZMath.max(float13, float15)));
    float float19 =
        PZMath.min(float10, PZMath.min(float12, PZMath.min(float14, float16)));
    float float20 =
        PZMath.max(float10, PZMath.max(float12, PZMath.max(float14, float16)));
  ...
  void render(float centerX, float centerY, float z, float width, float height,
              float angleRadians) {
    LineDrawer.DrawIsoRectRotated(centerX, centerY, z, width, height,
                                  angleRadians, 0.0F, 0.0F, 1.0F, 1.0F);
    float float0 = 1.0F;
    float float1 = 1.0F;
    float float2 = 1.0F;
    float float3 = 1.0F;
    ...
    for (VehicleStorySpawner.Element element : this.m_elements) {
      Vector2 vector =
          s_vector2_1.setLengthAndDirection(element.direction, 1.0F);
      LineDrawer.DrawIsoLine(element.position.x, element.position.y, z,
                             element.position.x + vector.x,
                             element.position.y + vector.y, z, float0, float1,
                             float2, float3, 1);
      LineDrawer.DrawIsoRectRotated(
          element.position.x, element.position.y, z, element.width,
          element.height, element.direction, float0, float1, float2, float3);
      this.getAABB(element.position.x, element.position.y, element.width,
                   element.height, element.direction, s_AABB);
...
public
  static final class Element {
    ...
    VehicleStorySpawner.Element init(String string, float float0, float float1,
  ...
  public interface IElementSpawner {
    void spawn(VehicleStorySpawner spawner,
               VehicleStorySpawner.Element element);
  }
}
} // namespace randomizedVehicleStory
