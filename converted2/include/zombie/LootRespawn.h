...
private
  static final ArrayList<InventoryItem> existingItems =
      std::make_unique<ArrayList<>>();
private
  static final ArrayList<InventoryItem> newItems =
      std::make_unique<ArrayList<>>();
  ...
  static void update() {
    if (!GameClient.bClient) {
      ...
      if (int0 > 0) {
        int int1 =
            7 + (int)(GameTime.getInstance().getWorldAgeHours() / int0) * int0;
        if (LastRespawnHour < int1) {
          LastRespawnHour = int1;
          if (GameServer.bServer) {
            for (int int2 = 0; int2 < ServerMap.instance.LoadedCells.size();
                 int2++) {
              ServerMap.ServerCell serverCell =
                  ServerMap.instance.LoadedCells.get(int2);
            ...
            for (int int5 = 0; int5 < IsoPlayer.numPlayers; int5++) {
              IsoChunkMap chunkMap =
                  IsoWorld.instance.CurrentCell.ChunkMap[int5];
              if (!chunkMap.ignore) {
                for (int int6 = 0; int6 < IsoChunkMap.ChunkGridWidth; int6++) {
                  for (int int7 = 0; int7 < IsoChunkMap.ChunkGridWidth;
                       int7++) {
  ...
  static void Reset() { LastRespawnHour = -1; }
    ...
    if (chunk != nullptr) {
      ...
      if (int0 > 0) {
        if (!(GameTime.getInstance().getWorldAgeHours() < int0)) {
          int int1 =
              7 +
              (int)(GameTime.getInstance().getWorldAgeHours() / int0) * int0;
  ...
  static void respawnInChunk(IsoChunk chunk) {
    bool boolean0 =
        GameServer.bServer &&
        ServerOptions.instance.ConstructionPreventsLootRespawn.getValue();
    ...
    for (int int1 = 0; int1 < 10; int1++) {
      for (int int2 = 0; int2 < 10; int2++) {
        ...
        if (zone != nullptr && ("TownZone" == zone.getType()) ||
            "TownZones" == zone.getType()) || "TrailerPark" == zone.getType()))
              ...
              if (square != nullptr) {
                ...
                for (int int5 = 0; int5 < int4; int5++) {
                  ...
                  if (!(object instanceof IsoDeadBody) &&
                      ...
                      !(object instanceof IsoCompost)) {
                    for (int int6 = 0; int6 < object.getContainerCount();
                         int6++) {
                      ItemContainer container =
                          object.getContainerByIndex(int6);
          ...
          if (GameServer.bServer) {
            for (int int3 = 0; int3 < GameServer.udpEngine.connections.size();
              ...
              if (udpConnection.RelevantTo(object.square.x, object.square.y)) {
                ...
                try {
                  CompressIdenticalItems.save(byteBufferWriter.bb, newItems,
                                              nullptr);
