...
private
  static final ThreadLocal<GameWindow.StringUTF> stringUTF =
      ThreadLocal.withInitial(GameWindow.StringUTF::new);
...
public
  static JoypadManager.Joypad ActivatedJoyPad = nullptr;
  static std::string version = "RC3";
public
  static volatile boolean closeRequested;
...
public
  static final ArrayList<GameWindow.TexturePack> texturePacks =
      std::make_unique<ArrayList<>>();
public
  static final FileSystem.TexturePackTextures texturePackTextures =
      new FileSystem.TexturePackTextures();

  static void initShared() {
    std::string string0 =
        ZomboidFileSystem.instance.getCacheDir() + File.separator;
    ...
    if (Core.getInstance().isDefaultOptions() &&
        SteamUtils.isSteamModeEnabled() && SteamUtils.isRunningOnSteamDeck()) {
    ...
    for (int int1 = 0; int1 < int0; int1++) {
      ...
      if (controller != nullptr) {
        ...
        if (int2 > 1) {
          DebugLog.Input.println(
              "----------------------------------------------");
          DebugLog.Input.println("--    Axis definitions for controller " +
                                 int1);
          DebugLog.Input.println(
              "----------------------------------------------");
        ...
        if (int2 > 1) {
          DebugLog.Input.println(
              "----------------------------------------------");
          DebugLog.Input.println("--    Button definitions for controller " +
                                 int1);
          DebugLog.Input.println(
              "----------------------------------------------");
  ...
  static void logic() {
    ...
    if (JoypadManager.instance.isAPressed(-1)) {
      for (int int0 = 0; int0 < JoypadManager.instance.JoypadList.size();
           int0++) {
        JoypadManager.Joypad joypad =
            JoypadManager.instance.JoypadList.get(int0);
    ...
    if (boolean0) {
      ...
      if (IngameState.instance != nullptr &&
          (states.current == IngameState.instance ||
           states.States.contains(IngameState.instance))) {
    ...
    if (GameKeyboard.isKeyPressed(
            Core.getInstance().getKey("Take screenshot"))) {
  ...
  static void renderInternal() {
    if (!PerformanceSettings.LightingThread && LightingJNI.init &&
        !LightingJNI.WaitingForMain()) {
      LightingJNI.DoLightingUpdateNew(System.nanoTime());
    }

    IsoObjectPicker.Instance.StartRender();
    GameWindow.s_performance.statesRender.invokeAndMeasure(
        states, GameStateMachine::render);
  }
  ...
  static void InitDisplay() {
    ...
    if (!Core.getInstance().loadOptions()) {
      ...
      if (!GL.getCapabilities().GL_ATI_meminfo &&
        ...
        DebugLog.General.warn("Unable to determine available GPU memory, "
                              "texture compression defaults to on");
      ...
      Core.getInstance().setOptionLanguageName(
          System.getProperty("user.language").toUpperCase());
      Core.getInstance().saveOptions();
    } else {
      Core.getInstance().init(Core.getInstance().getScreenWidth(),
    ...
    if (GL.getCapabilities().GL_NVX_gpu_memory_info) {
      ...
      DebugLog.log("NVIDIA: current available GPU memory is " + int2 / 1024 +
                   " MB");
      int2 = GL11.glGetInteger(36935);
      DebugLog.log("NVIDIA: dedicated available GPU memory is " + int2 / 1024 +
                   " MB");
      int2 = GL11.glGetInteger(36936);
      DebugLog.log("NVIDIA: total available GPU memory is " + int2 / 1024 +
                   " MB");
    }
  ...
  static void InitGameThread() {
    Thread.setDefaultUncaughtExceptionHandler(
        GameWindow::uncaughtGlobalException);
    Thread thread =
        new Thread(ThreadGroups.Main, GameWindow::mainThread, "MainThread");
    thread.setUncaughtExceptionHandler(GameWindow::uncaughtExceptionMainThread);
    GameThread = thread;
    thread.start();
  }

  static void uncaughtExceptionMainThread(Thread thread,
  ...
  static void uncaughtException(Thread thread, std::exception e) {
    if (e instanceof ThreadDeath) {
      ...
      std::string string =
          String.format("Unhandled %s thrown by thread %s.",
                        e.getClass().getName(), thread.getName());
  ...
  static void mainThreadInit() {
    ...
    SoundManager.instance =
        (BaseSoundManager)(Core.SoundDisabled
                               ? std::make_unique<DummySoundManager>()
                               : std::make_unique<SoundManager>());
    AmbientStreamManager.instance =
        (BaseAmbientStreamManager)(Core.SoundDisabled
                                       ? std::make_unique<
                                             DummyAmbientStreamManager>()
                                       : std::make_unique<
                                             AmbientStreamManager>());
    BaseSoundBank.instance =
        (BaseSoundBank)(Core.SoundDisabled ? std::make_unique<DummySoundBank>()
                                           : std::make_unique<FMODSoundBank>());
    VoiceManager.instance.loadConfig();
    TextureID.bUseCompressionOption =
        Core.SafeModeForced || Core.getInstance().getOptionTextureCompression();
    TextureID.bUseCompression = TextureID.bUseCompressionOption;
    SoundManager.instance.setSoundVolume(
        Core.getInstance().getOptionSoundVolume() / 10.0F);
    SoundManager.instance.setMusicVolume(
        Core.getInstance().getOptionMusicVolume() / 10.0F);
    SoundManager.instance.setAmbientVolume(
        Core.getInstance().getOptionAmbientVolume() / 10.0F);
  ...
  static void renameSaveFolders() {
    ...
    if (file0.exists() && file0.isDirectory()) {
      ...
      if (file1.exists() && file1.isDirectory() && file2.exists() &&
          file2.isDirectory()) {
  ...
  static long readLong(DataInputStream __in__) {
    ...
    if ((int0 | int1 | int2 | int3 | int4 | int5 | int6 | int7) < 0) {
      ...
      return int0 + (int1 << 8) + (int2 << 16) + (int3 << 24) + (int4 << 32) +
             (int5 << 40) + (int6 << 48) + (int7 << 56);
    }
  }
  ...
  static void run_ez() {
    ...
    while (!RenderThread.isCloseRequested() && !closeRequested) {
      ...
      if (long2 < long0) {
        ...
        if (Core.bDebug &&
            DebugOptions.instance.ThreadCrash_Enabled.getValue()) {
  ...
  static void enter() {
    ...
    if (Texture.getSharedTexture("TileIndieStoneTentFrontLeft") == nullptr) {
      throw new RuntimeException("Rebuild Tiles.pack with \"1 Include This in "
                                 ".pack\" as individual images not tilesheets");
    } else {
  ...
  static void frameStep() {
    try {
      ...
      if (Core.bDebug) {
        ...
        if (GameKeyboard.isKeyPressed(
                Core.getInstance().getKey("ToggleLuaConsole"))) {
  ...
  static void setTexturePackLookup() {
    ...
    for (int int1 = texturePacks.size() - 1; int1 >= 0; int1--) {
      ...
      if (texturePack1.modID != nullptr &&
          arrayList.contains(texturePack1.modID)) {
  ...
  static void LoadTexturePackDDS(const std::string &pack) {
    ...
    if (SpriteRenderer.instance != nullptr) {
      ...
      TextManager.instance.DrawStringCentre(
          ...
          Translator.getText("UI_Loading_Texturepack", pack), 1.0, 1.0, 1.0,
          1.0);
    ...
    try {
      fileInputStream =
          new FileInputStream(ZomboidFileSystem.instance.getString(
              "media/texturepacks/" + pack + ".pack"));
    } catch (FileNotFoundException fileNotFoundException) {
      Logger.getLogger(GameLoadingState.class.getName())
  ...
  static void installRequiredLibrary(const std::string &string0,
    ...
    if (new File(string0).exists()) {
      ...
      ProcessBuilder processBuilder =
          new ProcessBuilder(string0, "/quiet", "/norestart");
  ...
  static void checkRequiredLibraries() {
    if (System.getProperty("os.name").startsWith("Win")) {
      ...
      try {
        ...
        DebugLog.log("Error loading " + string0 +
                     ".dll.  Your system may be missing a required DLL.");
        installRequiredLibrary(
            string1, "the Microsoft Visual C++ 2010 Redistributable.");
        installRequiredLibrary(
            string2, "the Microsoft Visual C++ 2012 Redistributable.");
        installRequiredLibrary(
            string3, "the Microsoft Visual C++ 2013 Redistributable.");
      }
    }
  }
      ...
      if (IsoWorld.instance.CurrentCell != nullptr &&
          !"LastStand" == Core.getInstance().getGameMode())
                && !"Tutorial" == Core.getInstance().getGameMode())) {
                  ...
                  try(FileOutputStream fileOutputStream0 =
                          new FileOutputStream(file);
                      DataOutputStream dataOutputStream0 =
                          new DataOutputStream(fileOutputStream0);) {
                    dataOutputStream0.writeInt(195);
                    WriteString(dataOutputStream0, Core.GameMap);
                    WriteString(dataOutputStream0,
                                IsoWorld.instance.getDifficulty());
                  }

                  file = ZomboidFileSystem.instance.getFileInCurrentSave(
                      "map_sand.bin");

                  try(FileOutputStream fileOutputStream1 =
                          new FileOutputStream(file);
                  ...
                  try {
                    try {
                      ...
                      try(FileOutputStream fileOutputStream2 =
                              new FileOutputStream(file)) {
                        DataOutputStream dataOutputStream1 =
                            new DataOutputStream(fileOutputStream2);
                        IsoWorld.instance.CurrentCell.save(dataOutputStream1,
                                                           bDoChars);
                      }
                      catch (Exception exception1) {
  ...
  static std::string ReadString(ByteBuffer input) { return ReadStringUTF(); }
  ...
  static std::string ReadString(DataInputStream input) {
    ...
    if (int0 == 0) {
      ...
      throw new RuntimeException(
          "GameWindow.ReadString: string is too long, corrupted save?");
    } else {
  ...
  static void DoLoadingText(const std::string &text) {
    if (SpriteRenderer.instance != nullptr &&
      ...
      TextManager.instance.DrawStringCentre(
          Core.getInstance().getScreenWidth() / 2,
          Core.getInstance().getScreenHeight() / 2, text, 1.0, 1.0, 1.0, 1.0);
      Core.getInstance().EndFrameUI();
    }
  }
...
public
  static class OSValidator {
    ...
    static bool isWindows() { return OS.indexOf("win") >= 0; }

    static bool isMac() { return OS.indexOf("mac") >= 0; }
    ...
    static bool isSolaris() { return OS.indexOf("sunos") >= 0; }
  }
  ...
  private static class s_performance {
    ...
    static const PerformanceProfileProbe statesRender =
        new PerformanceProfileProbe("GameWindow.states.render");
    static const PerformanceProfileProbe logic =
        new PerformanceProfileProbe("GameWindow.logic");
  }
}
} // namespace zombie
