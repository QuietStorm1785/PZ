...
public:
private
  ArrayList<Path.Step> steps = std::make_unique<ArrayList<>>();
  float cost = 0.0F;
public
  static Stack<Path.Step> stepstore = std::make_unique<Stack<>>();
   ...
   */
  int getLength() { return this.steps.size(); }
   ...
   */
public
  Path.Step getStep(int index) { return this.steps.get(index); }
   ...
   */
  int getX(int index) { return this.getStep(index).x; }
   ...
   */
  int getY(int index) { return this.getStep(index).y; }

  int getZ(int index) { return this.getStep(index).z; }
...
public
  ...
  public
    ...
    bool equals(void *other) {
      return !(other instanceof Path.Step step)
                 ? false
                 : step.x == this.x && step.y == this.y && step.z == this.z;
    }
