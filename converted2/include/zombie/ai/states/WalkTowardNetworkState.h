...
public:
  static WalkTowardNetworkState _instance =
      std::make_unique<WalkTowardNetworkState>();
  static const int PARAM_TICK_COUNT = 2;

  static WalkTowardNetworkState instance() { return _instance; }
  ...
  void execute(IsoGameCharacter character) {
    ...
    if (zombie0.z != zombie0.networkAI.targetZ ||
        zombie0.networkAI.predictionType !=
                NetworkVariables.PredictionTypes.Thump &&
            zombie0.networkAI.predictionType !=
                NetworkVariables.PredictionTypes.Climb) {
      if (zombie0.z == zombie0.networkAI.targetZ &&
          !PolygonalMap2.instance.lineClearCollide(
              zombie0.x, zombie0.y, zombie0.networkAI.targetX,
              zombie0.networkAI.targetY, zombie0.networkAI.targetZ, nullptr)) {
        ...
        pathFindBehavior2.moveToPoint(zombie0.networkAI.targetX,
                                      zombie0.networkAI.targetY, 1.0F);
        zombie0.setVariable("bMoving", IsoUtils.DistanceManhatten(
                                           zombie0.networkAI.targetX,
                                           zombie0.networkAI.targetY,
                                           zombie0.nx, zombie0.ny) > 0.5F);
      } else {
        if (!zombie0.networkAI.usePathFind) {
          pathFindBehavior2.pathToLocationF(zombie0.networkAI.targetX,
                                            zombie0.networkAI.targetY,
                                            zombie0.networkAI.targetZ);
          pathFindBehavior2.walkingOnTheSpot.reset(zombie0.x, zombie0.y);
      ...
      pathFindBehavior2.moveToPoint(zombie0.networkAI.targetX,
                                    zombie0.networkAI.targetY, 1.0F);
      zombie0.setVariable(
          "bMoving", IsoUtils.DistanceManhatten(zombie0.networkAI.targetX,
                                                zombie0.networkAI.targetY,
                                                zombie0.nx, zombie0.ny) > 0.5F);
    }
    ...
    if (zombie0.target instanceof
        IsoGameCharacter &&
            ((IsoGameCharacter)zombie0.target).getVehicle() != nullptr &&
            ((IsoGameCharacter)zombie0.target)
                .getVehicle()
                .isCharacterAdjacentTo(character)) {
      boolean0 = false;
    }

    if (character.isCollidedThisFrame() || boolean0) {
      zombie0.AllowRepathDelay = 0.0F;
      zombie0.pathToLocation(character.getPathTargetX(),
