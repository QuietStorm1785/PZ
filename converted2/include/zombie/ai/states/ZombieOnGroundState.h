...
public:
  ...
  static ZombieOnGroundState instance() { return _instance; }
  ...
  void execute(IsoGameCharacter owner) {
    ...
    if (owner.isDead() || zombie0.isFakeDead()) {
      ...
      if (owner.getReanimateTimer() <= 2.0F) {
        ...
        } else if (owner.isBeingSteppedOn() &&
                   zombie0.getReanimatedPlayer() == nullptr) {
          owner.setReanimateTimer(Rand.Next(60) + 30);
        }
      }
    }
  ...
  static bool isCharacterStandingOnOther(IsoGameCharacter chrStanding,
                                         IsoGameCharacter chrProne) {
    AnimationPlayer animationPlayer = chrProne.getAnimationPlayer();
    int int0 = DoCollisionBoneCheck(
        chrStanding, chrProne,
        animationPlayer.getSkinningBoneIndex("Bip01_Spine", -1), 0.32F);
    if (int0 == -1) {
      int0 = DoCollisionBoneCheck(
          chrStanding, chrProne,
          animationPlayer.getSkinningBoneIndex("Bip01_L_Calf", -1), 0.18F);
    }

    if (int0 == -1) {
      int0 = DoCollisionBoneCheck(
          chrStanding, chrProne,
          animationPlayer.getSkinningBoneIndex("Bip01_R_Calf", -1), 0.18F);
    }

    if (int0 == -1) {
      int0 = DoCollisionBoneCheck(
          chrStanding, chrProne,
          animationPlayer.getSkinningBoneIndex("Bip01_Head", -1), 0.28F);
    }

    return int0 > -1;
  }

  static int DoCollisionBoneCheck(IsoGameCharacter character1,
                                  IsoGameCharacter character0, int int0,
                                  float float2) {
    float float0 = 0.3F;
    ...
    for (int int1 = 1; int1 <= 10; int1++) {
      ...
      tempVector.x =
          tempVector.x + character1.getForwardDirection().x * float0 * float1;
      tempVector.y =
          tempVector.y + character1.getForwardDirection().y * float0 * float1;
