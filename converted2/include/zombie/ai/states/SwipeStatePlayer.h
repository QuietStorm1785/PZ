...
private
  static final ArrayList<HitInfo> HitList2 = std::make_unique<ArrayList<>>();
  static const Vector2 tempVector2_1 = new Vector2();
  static const Vector2 tempVector2_2 = new Vector2();
private
  final ArrayList<Float> dotList = std::make_unique<ArrayList<>>();
  bool bHitOnlyTree;
public
  final ObjectPool<HitInfo> hitInfoPool = new ObjectPool<>(HitInfo::new);
private
  static final SwipeStatePlayer.CustomComparator Comparator =
...
private
  final SwipeStatePlayer.WindowVisitor windowVisitor =
  ...
  void doAttack(IsoPlayer player, float float0, bool boolean0,
    ...
    if (object == nullptr || !(object instanceof HandWeapon) || boolean0 ||
        attackVars.bDoShove) {
  ...
  void CalcAttackVars(IsoLivingCharacter owner, AttackVars vars) {
    ...
    if (!GameClient.bClient || owner.isLocal()) {
      ...
      HitInfo hitInfo0 =
          vars.targetsStanding.isEmpty() ? null : vars.targetsStanding.get(0);
      HitInfo hitInfo1 =
          vars.targetsProne.isEmpty() ? null : vars.targetsProne.get(0);
  ...
  bool shouldIgnoreProneZombies(IsoGameCharacter character,
    ...
    if (float0 <= 0.0F) {
      ...
      for (int int0 = 0; int0 < arrayList.size(); int0++) {
        ...
        if ((zombie0 == nullptr || zombie0.target != nullptr || boolean0) &&
...
private
    ...
    for (int int0 = 0; int0 < arrayList0.size(); int0++) {
      ...
      if (movingObject != character0) {
        IsoGameCharacter character1 =
            Type.tryCastTo(movingObject, IsoGameCharacter.class);
        if ((character1 == nullptr || !character1.isGodMod()) &&
            (character1 == nullptr || !character1.isDead())) {
          IsoZombie zombie0 = Type.tryCastTo(movingObject, IsoZombie.class);
          if ((zombie0 == nullptr ||
               !zombie0.isCurrentState(FakeDeadZombieState.instance())) &&
              checkPVP(character0, movingObject)) {
  ...
  void ConnectSwing(IsoGameCharacter owner, HandWeapon weapon) {
    ...
    int int1 = 1;
    this.dotList.clear();
    if (owner.hitList.isEmpty() && owner.getClickSound() != nullptr &&
      ...
      if (hitInfo.getObject() == nullptr &&
        ...
        if (!this.isWindowBetween(owner, movingObject)) {
          ...
          if (boolean6) {
            ...
            if (vehicle == nullptr && movingObject.getSquare() != nullptr &&
...
private
  static final class WindowVisitor implements SwipeStatePlayer.LOSVisitor {
