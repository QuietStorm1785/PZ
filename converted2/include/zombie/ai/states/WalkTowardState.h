...
public:
  ...
  static WalkTowardState instance() { return _instance; }
  ...
  void enter(IsoGameCharacter owner) {
    ...
    if (hashMap.get(PARAM_IGNORE_OFFSET) == Boolean.TRUE &&
        System.currentTimeMillis() - (Long)hashMap.get(PARAM_IGNORE_TIME) >
            3000L) {
      hashMap.put(PARAM_IGNORE_OFFSET, Boolean.FALSE);
      hashMap.put(PARAM_IGNORE_TIME, 0L);
    }
  ...
  void execute(IsoGameCharacter owner) {
    ...
    IsoGameCharacter character =
        Type.tryCastTo(zombie0.target, IsoGameCharacter.class);
      ...
      if (zombie0.isTargetLocationKnown()) {
        if (character != nullptr) {
          ...
          if (character.getVehicle() != nullptr &&
              zombie0.DistToSquared(zombie0.target) < 16.0F) {
            Vector3f vector3f = character.getVehicle().chooseBestAttackPosition(
                character, zombie0, this.worldPos);
            if (vector3f == nullptr) {
            ...
            if (Math.abs(owner.x -
                         zombie0.getPathFindBehavior2().getTargetX()) > 0.1F ||
                Math.abs(owner.y -
                         zombie0.getPathFindBehavior2().getTargetY()) > 0.1F) {
              ...
              return;
                 ...
                 !owner.getPathFindBehavior2().isTargetLocation(
                     zombie0.LastTargetSeenX + 0.5F,
                     zombie0.LastTargetSeenY + 0.5F, zombie0.LastTargetSeenZ)) {
        owner.pathToLocation(zombie0.LastTargetSeenX, zombie0.LastTargetSeenY,
                             zombie0.LastTargetSeenZ);
      }
    }

    if (owner.getPathTargetX() == (int)owner.getX() &&
        owner.getPathTargetY() == (int)owner.getY()) {
      if (zombie0.target == nullptr) {
    ...
    if (character != nullptr && character.getVehicle() != nullptr &&
        character.getVehicle().isCharacterAdjacentTo(owner)) {
      boolean0 = false;
    }
    ...
    if (!boolean1 && !boolean0) {
      ...
      if (float3 < 0.025F) {
        ...
        if (!GameServer.bServer && !zombie0.bCrawling &&
            hashMap.get(PARAM_IGNORE_OFFSET) == Boolean.FALSE) {
          float float4 = Math.min(float3 / 2.0F, 4.0F);
          float float5 = (owner.getID() + zombie0.ZombieID) % 20 / 10.0F - 1.0F;
          float float6 =
              (zombie0.getID() + zombie0.ZombieID) % 20 / 10.0F - 1.0F;
          this.temp.x = this.temp.x + zombie0.getX();
          this.temp.y = this.temp.y + zombie0.getY();
        ...
        if (owner.getPathFindBehavior2().walkingOnTheSpot.check(owner.x,
                                                                owner.y)) {
        ...
        if (IngameState.instance.numberTicks - long0 == 2L) {
          zombie0.parameterZombieState.setState(
              ParameterZombieState.State.Idle);
        }
      }
    } else {
      zombie0.AllowRepathDelay = 0.0F;
      zombie0.pathToLocation(owner.getPathTargetX(), owner.getPathTargetY(),
                             owner.getPathTargetZ());
  ...
  bool isPathClear(IsoGameCharacter character, float float0, float float1,
                   float float2) {
    ...
    if (chunk != nullptr) {
      ...
      return !PolygonalMap2.instance.lineClearCollide(
          character.getX(), character.getY(), float0, float1, (int)float2,
          character.getPathFindBehavior2().getTargetChar(), int2);
    } else {
      return false;
    }
  }
  ...
  bool calculateTargetLocation(IsoZombie zomb, Vector2 location) {
    ...
    if (float0 < 0.025F) {
      return false;
    } else if (!GameServer.bServer && !zomb.bCrawling &&
               hashMap.get(PARAM_IGNORE_OFFSET) == Boolean.FALSE) {
