    ...
    if (!file1.exists()) {
      try {
        ...
        statement0.executeUpdate(
            "CREATE TABLE localPlayers (id   INTEGER PRIMARY KEY NOT NULL,name "
            "STRING,wx    INTEGER,wy    INTEGER,x    FLOAT,y    FLOAT,z    "
            "FLOAT,worldversion    INTEGER,data BLOB,isDead BOOLEAN);");
        statement0.executeUpdate(
            "CREATE TABLE networkPlayers (id   INTEGER PRIMARY KEY NOT "
            "NULL,world TEXT,username TEXT,playerIndex   INTEGER,name "
            "STRING,steamid STRING,x    FLOAT,y    FLOAT,z    "
            "FLOAT,worldversion    INTEGER,data BLOB,isDead BOOLEAN);");
        statement0.executeUpdate(
            "CREATE INDEX inpusername ON networkPlayers (username);");
        statement0.close();
        ...
        try {
          ...
          if (!file1.exists()) {
            ...
            try(Connection connection =
                    PZSQLUtils.getConnection(file1.getAbsolutePath())) {
              std::string string1 =
                  "SELECT isDead FROM localPlayers WHERE id=?";

              try(PreparedStatement preparedStatement =
                      connection.prepareStatement(string1)) {
...
public
  static ArrayList<Object> getPlayers(String string0) throws SQLException {
    ...
    if (Core.getInstance().isNoSave()) {
      ...
      if (!file.exists()) {
        ...
        try(Connection connection =
                PZSQLUtils.getConnection(file.getAbsolutePath())) {
