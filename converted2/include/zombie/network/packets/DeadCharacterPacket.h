...
public:
    ...
    if (this.character != nullptr) {
      ...
      if (this.character.getCurrentSquare() != square) {
        DebugLog.Multiplayer.warn(String.format(
            ...
            this.character.getClass().getSimpleName(), this.id,
            this.character.x, this.character.y, this.x, this.y));
      ...
      if (this.character.getAnimAngleRadians() - this.angle > 1.0E-4F) {
        DebugLog.Multiplayer.warn(
            String.format("Corpse %s(%d) teleport: direction (%f) => (%f)",
                          this.character.getClass().getSimpleName(), this.id,
                          this.character.getAnimAngleRadians(), this.angle));
        if (this.character.hasAnimationPlayer() &&
            this.character.getAnimationPlayer().isReady() &&
            !this.character.getAnimationPlayer()
      ...
      if (boolean0 != this.character.isFallOnFront()) {
        DebugLog.Multiplayer.warn(
            String.format("Corpse %s(%d) teleport: pose (%s) => (%s)",
                          this.character.getClass().getSimpleName(), this.id,
    ...
    if (square != nullptr) {
      for (IsoMovingObject movingObject : square.getStaticMovingObjects()) {
        if (movingObject instanceof
            IsoDeadBody &&
                ((IsoDeadBody)movingObject).getOnlineID() == this.id) {
    ...
    if (humanVisual != nullptr) {
      try {
        ...
        DebugLog.Multiplayer.printException(
            exception, "Parse dead body HumanVisuals failed",
            LogSeverity.Error);
      }
    }
  }
  ...
  void parseCharacterInventory(ByteBuffer byteBuffer) {
    if (this.character != nullptr) {
      ...
      this.character.getInventory().setSourceGrid(
          this.character.getCurrentSquare());
  ...
  void writeCharacterHumanVisuals(ByteBufferWriter byteBufferWriter) {
    if (this.character != nullptr) {
      ...
      try {
        ...
        DebugLog.Multiplayer.printException(
            exception, "Write character HumanVisuals failed",
            LogSeverity.Error);
      }
    }
  }
  ...
  void parseCharacterHumanVisuals(ByteBuffer byteBuffer) {
    ...
    if (this.character != nullptr && byte0 != 0) {
      try {
        ...
        DebugLog.Multiplayer.printException(
            exception, "Parse character HumanVisuals failed",
            LogSeverity.Error);
      }
    }
  }
  ...
  std::string getDescription() {
    ...
    if (this.character != nullptr) {
      ...
      if (this.character.getBodyDamage() != nullptr) {
        string = string + " | bodyDamage=" +
                 this.character.getBodyDamage().getOverallBodyHealth();
      }
  ...
  std::string getDeathDescription() {
    return this.getClass().getSimpleName() + " id(" + this.id + ") | killer=" +
           (this.killer == nullptr
                ? "Null"
                : this.killer.getClass().getSimpleName() + "(" +
                      this.killer.getOnlineID() + ")") +
           " | pos=(x=" + this.x + ",y=" + this.y + ",z=" + this.z +
           ";a=" + this.angle + ") | dir=" + this.direction.name() +
           " | isFallOnFront=" + ((this.characterFlags & 1) != 0);
  }

  bool isConsistent() { return this.character != nullptr; }
}
} // namespace packets
} // namespace network
} // namespace zombie
