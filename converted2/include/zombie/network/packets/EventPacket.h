...
public:
  ...
  IsoPlayer player;
private
  EventPacket.EventType event;
  long timestamp;

  std::string getDescription() {
    return "[ player=" + this.id + " \"" +
           (this.player == nullptr ? "?" : this.player.getUsername()) +
           "\" | name=\"" + (this.event == nullptr ? "?" : this.event.name()) +
           "\" | pos=( " + this.x + " ; " + this.y + " ; " + this.z +
           " ) | type1=\"" + this.type1 + "\" | type2=\"" + this.type2 +
           "\" | type3=\"" + this.type3 + "\" | type4=\"" + this.type4 +
           "\" | flags=" + this.flags +
           "\" | variables=" + this.booleanVariables + " ]";
  }

  bool isConsistent() {
    bool boolean0 = this.player != null && this.event != null;
    if (!boolean0 && Core.bDebug) {
      DebugLog.log(DebugType.Multiplayer,
                   "[Event] is not consistent " + this.getDescription());
    }
  ...
  void parse(ByteBuffer byteBuffer, UdpConnection var2) {
    ...
    if (this.eventID >= 0 &&
        this.eventID < EventPacket.EventType.values().length) {
  ...
  IsoWindow getWindow(IsoPlayer playerx) {
    for (IsoDirections directions : IsoDirections.values()) {
      IsoObject object =
          playerx.getContextDoorOrWindowOrWindowFrame(directions);
  ...
  IsoObject getObject(IsoPlayer playerx) {
    for (IsoDirections directions : IsoDirections.values()) {
      IsoObject object =
          playerx.getContextDoorOrWindowOrWindowFrame(directions);
      if (object instanceof IsoWindow || object instanceof
          IsoThumpable || IsoWindowFrame.isWindowFrame(object)) {
        return object;
      }
    }
  ...
  IsoDirections checkCurrentIsEventGridSquareFence(IsoPlayer playerx) {
    IsoGridSquare square0 = playerx.getCell().getGridSquare(
        (double)this.x, (double)this.y, (double)this.z);
    IsoGridSquare square1 = playerx.getCell().getGridSquare(
        (double)this.x, (double)(this.y + 1.0F), (double)this.z);
    IsoGridSquare square2 = playerx.getCell().getGridSquare(
  ...
  bool isTimeout() { return System.currentTimeMillis() > this.timestamp; }
  ...
  bool process(IsoPlayer playerx) {
    ...
    if (this.isConsistent()) {
      ...
      if (playerx.getCurrentSquare() ==
                  playerx.getCell().getGridSquare(
                      (double)this.x, (double)this.y, (double)this.z) &&
              !playerx.isPlayerMoving() ||
          !this.requireNonMoving()) {
        switch (this.event) {
        case EventSetActivatedPrimary:
          if (playerx.getPrimaryHandItem() != nullptr &&
              playerx.getPrimaryHandItem().canEmitLight()) {
            playerx.getPrimaryHandItem().setActivatedRemote((this.flags & 16) !=
                                                            0);
            boolean0 = true;
          }
          break;
        case EventSetActivatedSecondary:
          if (playerx.getSecondaryHandItem() != nullptr &&
              playerx.getSecondaryHandItem().canEmitLight()) {
            playerx.getSecondaryHandItem().setActivatedRemote(
                (this.flags & 16) != 0);
            boolean0 = true;
          }
          break;
        ...
        case EventClimbFence:
          IsoDirections directions0 =
              this.checkCurrentIsEventGridSquareFence(playerx);
        ...
        case EventSmashWindow:
          if ((this.flags & 2048) != 0) {
            BaseVehicle vehicle = VehicleManager.instance.getVehicleByID(
                Short.parseShort(this.type1));
            if (vehicle != nullptr) {
        ...
        case wasBumped:
          ...
          if (!StringUtils.isNullOrEmpty(this.type3) &&
              !StringUtils.isNullOrEmpty(this.type4)) {
            IsoGameCharacter character = null;
            if ((this.flags & 256) != 0) {
              character =
                  GameClient.IDToZombieMap.get(Short.parseShort(this.type3));
            } else {
              character =
                  GameClient.IDToPlayerMap.get(Short.parseShort(this.type3));
            }
        ...
        case EventOverrideItem:
          if (playerx.getNetworkCharacterAI().getAction() != nullptr) {
            playerx.getNetworkCharacterAI().setOverride(true, this.type1,
                                                        this.type2);
          }
        ...
        case Unknown:
        default:
          DebugLog.Multiplayer.warn("[Event] unknown: " +
                                    this.getDescription());
          boolean0 = true;
        }
      }
    ...
    for (EventPacket.EventType eventType : EventPacket.EventType.values()) {
      if (eventType.name() == string)) {
          ...
          case EventSetActivatedPrimary:
            this.flags =
                (short)(this.flags |
                        (playerx.getPrimaryHandItem().isActivated() ? 16 : 0));
            break;
          case EventSetActivatedSecondary:
            this.flags =
                (short)(this.flags |
                        (playerx.getSecondaryHandItem().isActivated() ? 16
                                                                      : 0));
          case EventFallClimb:
          case EventSitOnGround:
          case EventClimbRope:
          ...
          case EventUpdateFitness:
            ...
            if (playerx.getSecondaryHandItem() != nullptr &&
                playerx.getSecondaryHandItem() !=
                    playerx.getPrimaryHandItem()) {
              this.type4 = playerx.getSecondaryHandItem().getStaticModel();
            }

            this.flags =
                (short)(this.flags |
                        (playerx.getVariableBoolean("FitnessFinished") ? 4096
                                                                       : 0));
            break;
          case EventEmote:
          ...
          case EventClimbFence:
            ...
            break;
          case EventClimbWall:
            this.flags = (short)(this.flags |
                                 (playerx.isClimbOverWallSuccess() ? 32 : 0));
            this.flags = (short)(this.flags |
                                 (playerx.isClimbOverWallStruggle() ? 64 : 0));
            break;
          case EventSmashWindow:
            std::unordered_map hashMap =
                playerx.getStateMachineParams(SmashWindowState.instance());
            if (hashMap.get(1) instanceof BaseVehicle && hashMap.get(2)
                                              instanceof VehiclePart) {
          ...
          case wasBumped:
            ...
            this.flags =
                (short)(this.flags | (playerx.isBumpStaggered() ? 8 : 0));
            if (playerx.getBumpedChr() != nullptr) {
              ...
              this.flags =
                  (short)(this.flags | (playerx.isHitFromBehind() ? 128 : 0));
          ...
          case EventOverrideItem:
            ...
            this.type2 =
                baseAction.getSecondaryHandItem() == nullptr
          ...
          case Update:
            this.flags =
                (short)(this.flags |
                        (playerx.networkAI.isPressedMovement() ? 512 : 0));
            this.flags =
                (short)(this.flags |
                        (playerx.networkAI.isPressedCancelAction() ? 1024 : 0));
            break;
          default:
            DebugLog.Multiplayer.warn("[Event] unknown " +
                                      this.getDescription());
