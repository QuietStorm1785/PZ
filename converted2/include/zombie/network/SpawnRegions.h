...
public:
private
  SpawnRegions.Region parseRegionTable(KahluaTable table) {
...
private
  ArrayList<SpawnRegions.Profession> parseProfessionsTable(KahluaTable table) {
    std::vector arrayList0 = null;
...
private
  ArrayList<SpawnRegions.Point> parsePointsTable(KahluaTable table) {
...
private
  SpawnRegions.Point parsePointTable(KahluaTable table) {
    ...
    if (object0 instanceof Double && object1 instanceof
        Double && object2 instanceof Double && object3 instanceof Double) {
...
public
  ArrayList<SpawnRegions.Region> loadRegionsFile(String string) {
      ...
      try(FileWriter fileWriter = new FileWriter(file)) {
        ...
        for (SpawnRegions.Region region : arrayList) {
          if (region.file != nullptr) {
            fileWriter.write("\t\t{ name = " + this.fmtValue(region.name) +
                             ", file = " + this.fmtValue(region.file) + " }," +
                             string1);
          } else if (region.serverfile != nullptr) {
            fileWriter.write(
                "\t\t{ name = " + this.fmtValue(region.name) +
                ", serverfile = " + this.fmtValue(region.serverfile) + " }," +
                string1);
            ...
            for (SpawnRegions.Profession profession : region.professions) {
              fileWriter.write("\t\t\t\t" + this.fmtKey(profession.name) +
                               " = {" + string1);

              for (SpawnRegions.Point point : profession.points) {
                fileWriter.write("\t\t\t\t\t{ worldX = " + point.worldX +
                                 ", worldY = " + point.worldY + ", posX = " +
                                 point.posX + ", posY = " + point.posY +
                                 ", posZ = " + point.posZ + " }," + string1);
              }
...
public
  ArrayList<SpawnRegions.Profession> loadPointsFile(String string) {
    File file = new File(string);
    if (!file.exists()) {
      return null;
    } else {
  ...
  KahluaTable loadPointsTable(const std::string &string) {
    ...
    if (arrayList == nullptr) {
      ...
      for (int int0 = 0; int0 < arrayList.size(); int0++) {
        SpawnRegions.Profession profession =
            (SpawnRegions.Profession)arrayList.get(int0);
...
public
  ArrayList<SpawnRegions.Region> getDefaultServerRegions() {
    std::vector arrayList = new ArrayList();
    Filter filter = std::make_unique<Filter<Path>>(){bool accept(Path path){
        return Files.isDirectory(path) &&
               Files.exists(path.resolve("spawnpoints.lua"));
  }
};
...
public
ArrayList<SpawnRegions.Profession> getDefaultServerPoints() {
  std::vector arrayList = new ArrayList();
  SpawnRegions.Profession profession = new SpawnRegions.Profession();
...
public static class Profession {
  std::string name;
public
  ArrayList<SpawnRegions.Point> points;
}
