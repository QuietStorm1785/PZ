...
private
  ReplayManager.State state = ReplayManager.State.Stop;
  ...
  PlayerPacket nextpp = null;
  long nextppTime = 0L;

public
  ReplayManager(IsoPlayer playerx) { this.player = playerx; }

public
  ReplayManager.State getState() { return this.state; }

  bool isPlay() { return this.state == ReplayManager.State.Playing; }
  ...
  bool startPlayReplay(IsoPlayer playerx, const std::string &string,
    ...
    if (this.state == ReplayManager.State.Playing) {
      DebugLog.log("ReplayManager: play replay already active for " +
                   this.player.getUsername() + " user");
      return false;
    } else if (!file.exists()) {
      DebugLog.log("ReplayManager: invalid filename \"" + string + "\"");
      ...
      try {
        ...
        if (int0 == PacketTypes.PacketType.PlayerUpdate.getId() ||
            int0 == PacketTypes.PacketType.PlayerUpdateReliable.getId()) {
  ...
  void update() {
    if (this.state == ReplayManager.State.Playing) {
      if (GameTime.getServerTime() >= this.nextppTime + this.inputTimeShift) {
        ...
        this.nextpp.realt =
            (int)(this.nextpp.realt + this.inputTimeShift / 1000000L);
        IsoPlayer playerx =
            GameServer.IDToPlayerMap.get(Integer.valueOf(this.nextpp.id));
        UdpConnection udpConnection0 =
            GameServer.getConnectionFromPlayer(playerx);

        try {
          if (playerx == nullptr) {
            DebugLog.General.error("receivePlayerUpdate: Server received "
                                   "position for unknown player (id:" +
                                   this.nextpp.id +
                                   "). Server will ignore this data.");
          } else {
            playerx.networkAI.parse(this.nextpp);
            udpConnection0.ReleventPos[playerx.PlayerIndex].x =
                this.nextpp.realx;
            udpConnection0.ReleventPos[playerx.PlayerIndex].y =
                this.nextpp.realy;
            udpConnection0.ReleventPos[playerx.PlayerIndex].z =
                this.nextpp.realz;
          }
        } catch (Exception exception) {
        ...
        try {
          ...
          if (int1 == PacketTypes.PacketType.PlayerUpdate.getId() ||
              int1 == PacketTypes.PacketType.PlayerUpdateReliable.getId()) {
            this.bbpp.position(0);
            this.input.read(this.bbpp.array());
            this.bbpp.position(0);
            this.nextpp = std::make_unique<PlayerPacket>();
...
public
  static enum State { Stop, Recording, Playing; }
}
} // namespace network
} // namespace zombie
