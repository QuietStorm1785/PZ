...
private
  static final ThreadLocal<FakeClientManager.StringUTF> stringUTF =
      ThreadLocal.withInitial(FakeClientManager.StringUTF::new);
  static int logLevel = 0;
  static long startTime = System.currentTimeMillis();
private
  static final HashSet<FakeClientManager.Player> players =
...
private
  static HashMap<Integer, FakeClientManager.Movement> load(String string1) {
    ...
    try {
      ...
      if (jSONObject3.has("serverHost")) {
        FakeClientManager.Client.connectionServerHost =
            jSONObject3.getString("serverHost");
      }

      FakeClientManager.Client.connectionInterval =
          jSONObject3.getLong("interval");
      FakeClientManager.Client.connectionTimeout =
          jSONObject3.getLong("timeout");
      ...
      FakeClientManager.Client.statisticsClientID =
          Math.max(jSONObject4.getInt("id"), -1);
      if (jSONObject2.has("checksum")) {
        ...
        FakeClientManager.Client.scriptChecksum =
            jSONObject5.getString("script");
      }
      ...
      if (jSONObject1.has("zombies")) {
        ...
        FakeClientManager.ZombieSimulator.Behaviour behaviour =
            FakeClientManager.ZombieSimulator.Behaviour.Normal;
        if (jSONObject3.has("behaviour")) {
          behaviour = FakeClientManager.ZombieSimulator.Behaviour.valueOf(
              jSONObject3.getString("behaviour"));
        }

        FakeClientManager.ZombieSimulator.behaviour = behaviour;
        if (jSONObject3.has("maxZombiesPerUpdate")) {
          FakeClientManager.ZombieSimulator.maxZombiesPerUpdate =
              jSONObject3.getInt("maxZombiesPerUpdate");
        }

        if (jSONObject3.has("deleteZombieDistance")) {
          int int0 = jSONObject3.getInt("deleteZombieDistance");
          FakeClientManager.ZombieSimulator.deleteZombieDistanceSquared =
              int0 * int0;
        }

        if (jSONObject3.has("forgotZombieDistance")) {
          int int1 = jSONObject3.getInt("forgotZombieDistance");
          FakeClientManager.ZombieSimulator.forgotZombieDistanceSquared =
              int1 * int1;
        }

        if (jSONObject3.has("canSeeZombieDistance")) {
          int int2 = jSONObject3.getInt("canSeeZombieDistance");
          FakeClientManager.ZombieSimulator.canSeeZombieDistanceSquared =
              int2 * int2;
        }

        if (jSONObject3.has("seeZombieDistance")) {
          int int3 = jSONObject3.getInt("seeZombieDistance");
          FakeClientManager.ZombieSimulator.seeZombieDistanceSquared =
              int3 * int3;
        }

        if (jSONObject3.has("canChangeTarget")) {
          FakeClientManager.ZombieSimulator.canChangeTarget =
              jSONObject3.getBoolean("canChangeTarget");
        }
      }
      ...
      if (jSONObject3.has("damage")) {
        FakeClientManager.Player.damage =
            (float)jSONObject3.getDouble("damage");
      }
      ...
      for (int int4 = 0; int4 < jSONArray.length(); int4++) {
        ...
        FakeClientManager.Movement.Motion motion =
            Math.random() > 0.8F ? FakeClientManager.Movement.Motion.Vehicle
                                 : FakeClientManager.Movement.Motion.Pedestrian;
        if (jSONObject4.has("motion")) {
          motion = FakeClientManager.Movement.Motion.valueOf(
              jSONObject4.getString("motion"));
        }
        ...
        if (jSONObject4.has("speed")) {
          ...
          case Pedestrian:
            int8 = (int)Math.round(
                Math.random() *
                    (FakeClientManager.Movement.pedestrianSpeedMax -
                     FakeClientManager.Movement.pedestrianSpeedMin) +
                FakeClientManager.Movement.pedestrianSpeedMin);
            break;
          case Vehicle:
            int8 = (int)Math.round(
                Math.random() * (FakeClientManager.Movement.vehicleSpeedMax -
        ...
        FakeClientManager.Movement.Type type =
            FakeClientManager.Movement.Type.Line;
        if (jSONObject4.has("type")) {
          type = FakeClientManager.Movement.Type.valueOf(
        ...
        IsoDirections directions =
            IsoDirections.fromIndex((int)Math.round(Math.random() * 7.0));
        if (jSONObject4.has("direction")) {
          directions =
              IsoDirections.valueOf(jSONObject4.getString("direction"));
        }
        ...
        if (jSONObject4.has("createHorde")) {
          ...
          if (long4 != 0L) {
            hordeCreator =
                new FakeClientManager.HordeCreator(int13, int12, long4);
          }
        }
        ...
        if (jSONObject4.has("makeSound")) {
          ...
          if (int14 != 0) {
            soundMaker =
                new FakeClientManager.SoundMaker(int14, int15, string3);
          }
        }

        FakeClientManager.Movement movement = new FakeClientManager.Movement(
            int5, string2, int6, int7, motion, int8, type, int9, int10, int11,
            directions, boolean0, long0, long1, long2, long3, hordeCreator,
            soundMaker);
    ...
    if (logLevel >= 0) {
      System.out.print(
          String.format("%5s : %s , [%2d] > %s\n", "INFO",
                        logDateFormat.format(Calendar.getInstance().getTime()),
                        int0, string));
    }
  }
  ...
  static bool isVOIPEnabled() {
    return FakeClientManager.Player.isVOIPEnabled && getOnlineID() != -1L &&
           getConnectedGUID() != -1L;
  }

  static long getConnectedGUID() {
    return players.isEmpty() ? -1L
                             : players.iterator().next().client.connectionGUID;
  }
  ...
  static void main(String[] strings) {
    ...
    if (network.isStarted()) {
      int int3 = 0;
      if (int0 != -1) {
        FakeClientManager.Movement movement0 =
            (FakeClientManager.Movement)hashMap.get(int0);
        if (movement0 != nullptr) {
          players.add(
              new FakeClientManager.Player(movement0, network, int3, int2));
        } else {
        ...
        for (FakeClientManager.Movement movement1 : hashMap.values()) {
          players.add(
              new FakeClientManager.Player(movement1, network, int3++, int2));
        }
      }
...
private
  ...
  private
    final HashMap<Integer, FakeClientManager.Client.Request> requests =
        std::make_unique<HashMap<>>();
  private
    final FakeClientManager.Player player;
  private
    final FakeClientManager.Network network;
  ...
  private
    Client(FakeClientManager.Player playerx, FakeClientManager.Network networkx,
           int int0, int int1) {
      ...
      try {
        this.host =
            InetAddress.getByName(connectionServerHost).getHostAddress();
        this.state = FakeClientManager.Client.State.CONNECT;
        Thread thread = new Thread(ThreadGroups.Workers, this ::updateThread,
    ...
    void updateThread() {
      FakeClientManager.info(
          ...
          String.format("Start client (%d) %s:%d => %s:%d / \"%s\"",
                        this.connectionIndex, "0.0.0.0", this.port, this.host,
                        16261, this.player.movement.description));
      ...
      FakeClientManager.info(
          this.player.movement.id,
          String.format("Stop client (%d) %s:%d => %s:%d / \"%s\"",
                        this.connectionIndex, "0.0.0.0", this.port, this.host,
                        16261, this.player.movement.description));
    }

    void updateTime() { this.stateTime = System.currentTimeMillis(); }
    ...
    void changeState(FakeClientManager.Client.State statex) {
      this.updateTime();
      FakeClientManager.log(this.player.movement.id,
                            String.format("%s >> %s", this.state, statex));
      if (FakeClientManager.Client.State.RUN == statex)) {
          ...
          if (this.player.teleportLimiter == nullptr) {
            this.player.teleportLimiter =
                new UpdateLimit(this.player.movement.teleportDelay);
          }
    ...
    void update() {
      ...
      case PLAYER_EXTRA_INFO:
        this.sendPlayerExtraInfo(this.player.movement.ghost,
                                 this.player.movement.hordeCreator != nullptr ||
                                     FakeClientManager.Player.isVOIPEnabled);
        this.sendEquip();
        this.changeState(FakeClientManager.Client.State.WAIT);
        break;
      case LOAD:
        ...
        break;
      case RUN:
        if (this.player.movement.doDisconnect() &&
            this.player.movement.checkDisconnect()) {
      ...
      case DISCONNECT:
        if (this.network.isConnected()) {
          this.player.movement.timestamp = System.currentTimeMillis();
          this.network.disconnect(this.connectionGUID, this.player.movement.id,
                                  this.host);
        }

        if (this.player.movement.doReconnect() &&
                this.player.movement.checkReconnect() ||
            !this.player.movement.doReconnect() &&
    ...
    void receive(short short0, ByteBuffer byteBuffer) {
      ...
      case ExtraInfo:
        ...
        break;
      case SentChunk:
        if (this.state == FakeClientManager.Client.State.WAIT &&
            this.receiveChunkPart(byteBuffer)) {
          this.updateTime();
      ...
      case NotRequiredInZip:
        if (this.state == FakeClientManager.Client.State.WAIT &&
            this.receiveNotRequired(byteBuffer)) {
          this.updateTime();
          if (this.allChunkPartsReceived()) {
            this.changeState(FakeClientManager.Client.State.PLAYER_CONNECT);
          }
    ...
    void sendPlayer(NetworkCharacter.Transform transform, int int0,
                    Vector2 vector) {
      ...
      playerPacket.realdir =
          (byte)IsoDirections.fromAngleActual(this.player.direction).index();
      playerPacket.realt = int0;
      playerPacket.collidePointX = -1.0F;
      playerPacket.collidePointY = -1.0F;
      ByteBuffer byteBuffer = this.network.startPacket();
      this.doPacket(PacketTypes.PacketType.PlayerUpdateReliable.getId(),
                    byteBuffer);
    ...
    bool allChunkPartsReceived() { return this.requests.size() == 0; }

    void addChunkRequest(int int0, int int1, int var3, int var4) {
      FakeClientManager.Client.Request request =
          new FakeClientManager.Client.Request(int0, int1, this.requestId);
      this.requestId++;
    ...
    void requestLargeAreaZip() {
      ByteBuffer byteBuffer = this.network.startPacket();
      this.doPacket(PacketTypes.PacketType.RequestLargeAreaZip.getId(),
                    byteBuffer);
      ...
      for (int int4 = int1; int4 <= int3; int4++) {
        for (int int5 = int0; int5 <= int2; int5++) {
          FakeClientManager.Client.Request request =
              new FakeClientManager.Client.Request(int5, int4, this.requestId);
          this.requestId++;
          this.requests.put(request.id, request);
        }
      }
    ...
    void requestFullUpdate() {
      ByteBuffer byteBuffer = this.network.startPacket();
      this.doPacket(
          PacketTypes.PacketType.IsoRegionClientRequestFullUpdate.getId(),
          byteBuffer);
      this.network.endPacketImmediate(this.connectionGUID);
    }
    ...
    void requestChunkObjectState() {
      for (FakeClientManager.Client.Request request : this.requests.values()) {
        ByteBuffer byteBuffer = this.network.startPacket();
        this.doPacket(PacketTypes.PacketType.ChunkObjectState.getId(),
                      byteBuffer);
    ...
    void sendStatisticsEnable(int int0) {
      ByteBuffer byteBuffer = this.network.startPacket();
      this.doPacket(PacketTypes.PacketType.StatisticRequest.getId(),
                    byteBuffer);
    ...
    void receiveStatistics(ByteBuffer byteBuffer) {
      ...
      FakeClientManager.info(
          this.player.movement.id,
          String.format("ServerStats: con=[%2d] fps=[%2d] tps=[%2d] "
                        "upt=[%4d-%4d/%4d], c1=[%d] c2=[%d] c3=[%d]",
                        long5, long3, long4, long0, long1, long2, long6, long7,
                        long8));
    }
    ...
    void receiveTimeSync(ByteBuffer byteBuffer) {
      ...
      if (!serverTimeShiftIsSet) {
        ...
        serverTimeShift = (long)((float)serverTimeShift +
                                 (float)(long4 - serverTimeShift) * 0.05F);
      }
    ...
    void receiveSyncClock(ByteBuffer var1) {
      FakeClientManager.trace(
          this.player.movement.id,
          String.format("Player %3d sync clock", this.player.OnlineID));
    }
    ...
    void receiveKicked(ByteBuffer byteBuffer) {
      ...
      FakeClientManager.info(
          this.player.movement.id,
          String.format("Client kicked. Reason: %s", string));
    }

    void receiveChecksum(ByteBuffer byteBuffer) {
      FakeClientManager.trace(
          this.player.movement.id,
          String.format("Player %3d receive Checksum", this.player.OnlineID));
      short short0 = byteBuffer.getShort();
      ...
      if (short0 != 1 || !boolean0 || !boolean1) {
        FakeClientManager.info(
            this.player.movement.id,
            String.format("checksum lua: %b, script: %b", boolean0, boolean1));
      }
    ...
    void receiveKillZombie(ByteBuffer byteBuffer) {
      FakeClientManager.trace(
          this.player.movement.id,
          String.format("Player %3d receive KillZombie", this.player.OnlineID));
      short short0 = byteBuffer.getShort();
      FakeClientManager.Zombie zombie0 =
          this.player.simulator.zombies.get(Integer.valueOf(short0));
      if (zombie0 != nullptr) {
    ...
    void receiveTeleport(ByteBuffer byteBuffer) {
      ...
      FakeClientManager.info(this.player.movement.id,
                             String.format("Player %3d teleport to (%d, %d)",
                                           this.player.OnlineID, (int)float0,
                                           (int)float1));
    ...
    void receiveZombieSimulation(ByteBuffer byteBuffer) {
      ...
      for (short short1 = 0; short1 < short0; short1++) {
        ...
        FakeClientManager.Zombie zombie0 =
            this.player.simulator.zombies.get(Integer.valueOf(short2));
        this.player.simulator.zombies4Delete.add(zombie0);
      }
    ...
    void sendChecksum() {
      if (!luaChecksum.isEmpty()) {
        FakeClientManager.trace(
            this.player.movement.id,
            String.format("Player %3d sendChecksum", this.player.OnlineID));
        ByteBuffer byteBuffer = this.network.startPacket();
    ...
    void sendWorldSound4Player(int int0, int int1, int int2, int int3,
                               int int4) {
      ByteBuffer byteBuffer = this.network.startPacket();
  ...
  private
    ...
    private
      Request(int int1, int int2, int int0) {
        this.id = int0;
        this.wx = int1;
        this.wy = int2;
  ...
  public
    HordeCreator(int int0, int int1, long long0) {
      this.radius = int0;
      this.count = int1;
      this.interval = long0;
    ...
    std::string getCommand(int int2, int int1, int int0) {
      return String.format("/createhorde2 -x %d -y %d -z %d -count %d -radius "
                           "%d -crawler false -isFallOnFront false -isFakeDead "
                           "false -knockedDown false -health 1 -outfit",
                           int2, int1, int0, this.count, this.radius);
    }
  }
  ...
  public
    Movement(int int0, String string, int int2, int int1,
             FakeClientManager.Movement.Motion motionx, int int3,
             FakeClientManager.Movement.Type typex, int int4, int int6,
             int int5, IsoDirections directions, boolean boolean0, long long0,
    ...
    void connect(int int0) {
      ...
      if (this.disconnectDelay != 0L) {
        FakeClientManager.info(
            this.id,
            String.format("Player %3d connect in %.3fs, disconnect in %.3fs",
                          int0, (float)(long0 - this.timestamp) / 1000.0F,
                          (float)this.disconnectDelay / 1000.0F));
      } else {
        FakeClientManager.info(
            this.id, String.format("Player %3d connect in %.3fs", int0,
    ...
    void disconnect(int int0) {
      ...
      if (this.reconnectDelay != 0L) {
        FakeClientManager.info(
            this.id,
            String.format("Player %3d disconnect in %.3fs, reconnect in %.3fs",
                          int0, (float)(long0 - this.timestamp) / 1000.0F,
                          (float)this.reconnectDelay / 1000.0F));
      } else {
        FakeClientManager.info(
            this.id, String.format("Player %3d disconnect in %.3fs", int0,
                                   (float)(long0 - this.timestamp) / 1000.0F));
      }
    ...
    bool doTeleport() { return this.teleportDelay != 0L; }

    bool doDisconnect() { return this.disconnectDelay != 0L; }

    bool checkDisconnect() {
      return System.currentTimeMillis() - this.timestamp > this.disconnectDelay;
    }

    bool doReconnect() { return this.reconnectDelay != 0L; }
  ...
  private
    static enum Motion {
      Aim, Sneak, Walk, SneakRun, Run, Sprint, Pedestrian, Vehicle;
    }
  ...
  private
    final HashMap<Integer, FakeClientManager.Client> createdClients =
        std::make_unique<HashMap<>>();
  private
    final HashMap<Long, FakeClientManager.Client> connectedClients =
        std::make_unique<HashMap<>>();
    ...
    int connected = -1;
  private
    static final HashMap<Integer, String> systemPacketTypeNames =
        std::make_unique<HashMap<>>();

    bool isConnected() { return this.connected == 0; }

    bool isStarted() { return this.started == 0; }

  private
    Network(int int1, int int0) {
      ...
      if (this.started == 0) {
        Thread thread = new Thread(ThreadGroups.Network, this ::receiveThread,
                                   "PeerInterfaceReceive");
        thread.setDaemon(true);
        thread.start();
        FakeClientManager.log(-1, "Network start ok");
      } else {
        FakeClientManager.error(
            -1, String.format("Network start failed: %d", this.started));
      }
    }
    ...
    void connect(int int0, const std::string &string) {
      ...
      if (this.connected == 0) {
        FakeClientManager.log(
            int0, String.format("Client connected to %s:%d", string, 16261));
      } else {
        FakeClientManager.error(
            int0, String.format("Client connection to %s:%d failed: %d", string,
                                16261, this.connected));
      }
    }
    ...
    void disconnect(long long0, int int0, const std::string &string) {
      ...
      if (this.connected == -1) {
        FakeClientManager.log(
            int0,
            String.format("Client disconnected from %s:%d", string, 16261));
      } else {
        FakeClientManager.log(
            int0, String.format("Client disconnection from %s:%d failed: %d",
                                string, 16261, long0));
      }
    }
    ...
    void cancelPacket() { this.wb.clear(); }
    ...
    static void logUserPacket(int int0, short short0) {
      PacketTypes.PacketType packetType = PacketTypes.packetTypes.get(short0);
      std::string string =
          packetType == null ? "unknown user packet" : packetType.name();
      FakeClientManager.trace(int0,
                              String.format("## %s (%d)", string, short0));
    }

    static void logSystemPacket(int int1, int int0) {
      std::string string =
          systemPacketTypeNames.getOrDefault(int0, "unknown system packet");
    ...
    void decode(ByteBuffer byteBuffer) {
      ...
      case 16:
        ...
        FakeClientManager.log(
            -1, String.format("Connected clients: %d (connection index %d)",
                              this.connectedClients.size(), int1));
        break;
      case 17:
      case 18:
      case 23:
  ...
  private
    final FakeClientManager.Client client;
  private
    final FakeClientManager.Movement movement;
  private
    final ArrayList<FakeClientManager.Player.Clothes> clothes;
    ...
    float angle;
  private
    FakeClientManager.ZombieSimulator simulator;
  private
  ...
  private
    Player(FakeClientManager.Movement movementx,
           FakeClientManager.Network network, int int0, int int1) {
      ...
      this.clothes.add(new FakeClientManager.Player.Clothes(
          (byte)11, (byte)0, "Shirt_FormalWhite"));
      this.clothes.add(
          new FakeClientManager.Player.Clothes((byte)13, (byte)3, "Tie_Full"));
      this.clothes.add(new FakeClientManager.Player.Clothes((byte)11, (byte)0,
                                                            "Socks_Ankle"));
      this.clothes.add(new FakeClientManager.Player.Clothes((byte)13, (byte)0,
                                                            "Trousers_Suit"));
      this.clothes.add(new FakeClientManager.Player.Clothes((byte)13, (byte)0,
                                                            "Suit_Jacket"));
      this.clothes.add(new FakeClientManager.Player.Clothes((byte)11, (byte)0,
    ...
    float getDistance(float float0) { return float0 / 3.6F / fps; }
    ...
    void teleportMovement() {
      ...
      FakeClientManager.info(
          this.movement.id,
          String.format("Player %3d teleport (%9.3f,%9.3f) => (%9.3f,%9.3f) / "
                        "%9.3f, next in %.3fs",
                        this.OnlineID, this.x, this.y, float0, float1,
                        Math.sqrt(Math.pow(float0 - this.x, 2.0) +
                                  Math.pow(float1 - this.y, 2.0)),
    ...
    void lineMovement() {
      ...
      this.direction.set(this.movement.destination.x - this.x,
                         this.movement.destination.y - this.y);
      this.direction.normalize();
      float float0 = this.x + distance * this.direction.x;
      float float1 = this.y + distance * this.direction.y;
      if (this.x < this.movement.destination.x &&
              float0 > this.movement.destination.x ||
          this.x > this.movement.destination.x &&
              float0 < this.movement.destination.x ||
          this.y < this.movement.destination.y &&
    ...
    void circleMovement() {
      this.angle =
          (this.angle +
           (float)(2.0 * Math.asin(this.getDistance(this.movement.speed) /
                                   2.0F / this.movement.radius))) %
          360.0F;
      float float0 = this.movement.spawn.x +
                     (float)(this.movement.radius * Math.sin(this.angle));
      float float1 = this.movement.spawn.y +
                     (float)(this.movement.radius * Math.cos(this.angle));
      this.x = float0;
      this.y = float1;
    }

  private
    FakeClientManager.Zombie getNearestZombie() {
      ...
      for (FakeClientManager.Zombie zombie1 : this.simulator.zombies.values()) {
        float float1 =
            IsoUtils.DistanceToSquared(this.x, this.y, zombie1.x, zombie1.y);
        if (float1 < float0) {
          zombie0 = zombie1;
          float0 = float1;
        }
    ...
    FakeClientManager.Zombie getNearestZombie(
      ...
      for (FakeClientManager.Zombie zombie1 : this.simulator.zombies.values()) {
        float float1 = IsoUtils.DistanceToSquared(
            remotePlayer.x, remotePlayer.y, zombie1.x, zombie1.y);
        if (float1 < float0) {
          zombie0 = zombie1;
          float0 = float1;
        }
  ...
  private
    FakeClientManager.PlayerManager.RemotePlayer getNearestPlayer() {
      FakeClientManager.PlayerManager.RemotePlayer remotePlayer0 = nullptr;
      float float0 = Float.POSITIVE_INFINITY;
      synchronized(this.playerManager.players) {
        for (FakeClientManager.PlayerManager.RemotePlayer remotePlayer1 :
             this.playerManager.players.values()) {
          float float1 = IsoUtils.DistanceToSquared(
              this.x, this.y, remotePlayer1.x, remotePlayer1.y);
          if (float1 < float0) {
    ...
    void aiNormalMovement() {
      ...
      if (remotePlayer == nullptr) {
        ...
        float float1 = IsoUtils.DistanceToSquared(
            this.x, this.y, remotePlayer.x, remotePlayer.y);
        if (float1 > 36.0F) {
        ...
        if (zombie0 != nullptr) {
          float2 =
              IsoUtils.DistanceToSquared(this.x, this.y, zombie0.x, zombie0.y);
        }

        FakeClientManager.Zombie zombie1 = this.getNearestZombie(remotePlayer);
        float float3 = Float.POSITIVE_INFINITY;
        if (zombie1 != nullptr) {
          float3 = IsoUtils.DistanceToSquared(remotePlayer.x, remotePlayer.y,
                                              zombie1.x, zombie1.y);
        }
    ...
    void checkRequestChunks() {
      ...
      if (Math.abs(int0 - this.WorldX) < 13 &&
          Math.abs(int1 - this.WorldY) < 13) {
        if (int0 != this.WorldX) {
          if (int0 < this.WorldX) {
            for (int int2 = -6; int2 <= 6; int2++) {
              this.client.addChunkRequest(this.WorldX - 6, this.WorldY + int2,
                                          0, int2 + 6);
            }
          } else {
            for (int int3 = -6; int3 <= 6; int3++) {
              this.client.addChunkRequest(this.WorldX + 6, this.WorldY + int3,
                                          12, int3 + 6);
            }
          }
          ...
          if (int1 < this.WorldY) {
            for (int int4 = -6; int4 <= 6; int4++) {
              this.client.addChunkRequest(this.WorldX + int4, this.WorldY - 6,
                                          int4 + 6, 0);
            }
          } else {
            for (int int5 = -6; int5 <= 6; int5++) {
              this.client.addChunkRequest(this.WorldX + int5, this.WorldY + 6,
                                          int5 + 6, 12);
            }
          }
        }
        ...
        for (int int10 = int6; int10 <= int8; int10++) {
          for (int int11 = int7; int11 <= int9; int11++) {
            this.client.addChunkRequest(int10, int11, int10 - int6,
                                        int11 - int7);
          }
        }
      }
    ...
    void hit() {
      FakeClientManager.info(this.movement.id,
                             String.format("Player %3d hit", this.OnlineID));
    }
    ...
    void run() {
      ...
      if (this.updateLimiter.Check()) {
        ...
        if (this.predictLimiter.Check()) {
          ...
          NetworkCharacter.Transform transform = this.networkCharacter.predict(
              predictInterval, int0, this.x, this.y, this.direction.x,
              this.direction.y);
        ...
        if (this.movement.hordeCreator != nullptr &&
            this.movement.hordeCreator.hordeCreatorLimiter.Check()) {
          this.client.sendCommand(this.movement.hordeCreator.getCommand(
              (int)this.x, (int)this.y, (int)this.z));
        }

        if (this.movement.soundMaker != nullptr &&
            this.movement.soundMaker.soundMakerLimiter.Check()) {
          this.client.sendWorldSound4Player(
              (int)this.x, (int)this.y, (int)this.z,
              this.movement.soundMaker.radius, this.movement.soundMaker.radius);
          this.client.sendChatMessage(this.movement.soundMaker.message);
          this.client.sendEventPacket(this.OnlineID, (int)this.x, (int)this.y,
                                      (int)this.z, (byte)4, "shout");
        }
      }
    }
  ...
  private
    FakeClientManager.Player player = nullptr;
    const PlayerPacket playerPacket = new PlayerPacket();
  public
    final HashMap<Integer, FakeClientManager.PlayerManager.RemotePlayer>
        players = std::make_unique<HashMap<>>();

  public
    PlayerManager(FakeClientManager.Player playerx) { this.player = playerx; }
    ...
    void parsePlayer(ByteBuffer byteBuffer) {
      ...
      synchronized(this.players) {
        FakeClientManager.PlayerManager.RemotePlayer remotePlayer =
            this.players.get(playerPacketx.id);
        if (remotePlayer == nullptr) {
          remotePlayer = new FakeClientManager.PlayerManager.RemotePlayer(
              playerPacketx.id);
          this.players.put(Integer.valueOf(playerPacketx.id), remotePlayer);
          FakeClientManager.trace(
              this.player.movement.id,
              String.format("New player %s", remotePlayer.OnlineID));
        }
    ...
    void parsePlayerTimeout(ByteBuffer byteBuffer) {
      short short0 = byteBuffer.getShort();
      synchronized(this.players) { this.players.remove(short0); }

      FakeClientManager.trace(this.player.movement.id,
                              String.format("Remove player %s", short0));
    }
  ...
  private
    ...
    int encode(const std::string &string) {
      ...
      if (this.ce == nullptr) {
        this.ce = StandardCharsets.UTF_8.newEncoder()
    ...
    std::string decode(int int1) {
      if (this.cd == nullptr) {
        this.cd = StandardCharsets.UTF_8.newDecoder()
                      .onMalformedInput(CodingErrorAction.REPLACE)
                      .onUnmappableCharacter(CodingErrorAction.REPLACE);
      }
      ...
      if (short0 <= 0) {
        ...
        if (byteBufferx.remaining() < short0) {
          DebugLog.General.error(
              "GameWindow.StringUTF.load> numBytes:" + short0 +
              " is higher than the remaining bytes in the buffer:" +
              byteBufferx.remaining());
        }
  ...
  private static class Zombie {
    ...
    ZombiePacket zombiePacket = null;
    IsoDirections dir = IsoDirections.N;
    float health = 1.0F;
    uint8_t walkType =
        (byte)Rand.Next(NetworkVariables.WalkType.values().length);
    float dropPositionX;
    float dropPositionY;
    bool isMoving = false;
  ...
  public
    static FakeClientManager.ZombieSimulator.Behaviour behaviour =
        FakeClientManager.ZombieSimulator.Behaviour.Stay;
  ...
  private
    final ArrayList<Short> unknownZombies = std::make_unique<ArrayList<>>();
  private
    final HashMap<Integer, FakeClientManager.Zombie> zombies =
        std::make_unique<HashMap<>>();
  private
    final ArrayDeque<FakeClientManager.Zombie> zombies4Add =
        std::make_unique<ArrayDeque<>>();
  private
    final ArrayDeque<FakeClientManager.Zombie> zombies4Delete =
        std::make_unique<ArrayDeque<>>();
  private
    final HashSet<Short> authoriseZombies = std::make_unique<HashSet<>>();
  private
    final ArrayDeque<FakeClientManager.Zombie> SendQueue =
        std::make_unique<ArrayDeque<>>();
    static Vector2 tmpDir = new Vector2();

  public
    ZombieSimulator(FakeClientManager.Player playerx) { this.player = playerx; }
    ...
    void clear() {
      ...
      this.authoriseZombiesLast.removeIf(
          short0->this.zombies.get(Integer.valueOf(short0)) == nullptr);
    ...
    void parseZombie(ByteBuffer byteBuffer) {
      ...
      FakeClientManager.Zombie zombie0 =
          this.zombies.get(Integer.valueOf(zombiePacketx.id));
      if (!this.authoriseZombies.contains(zombiePacketx.id) ||
          zombie0 == nullptr) {
        if (zombie0 == nullptr) {
          ...
          FakeClientManager.trace(
              this.player.movement.id,
              String.format("New zombie %s", zombie0.OnlineID));
        }
    ...
    void process() {
      for (Short short0 : Sets.difference(this.authoriseZombiesCurrent,
        ...
        FakeClientManager.Zombie zombie0 =
            this.zombies.get(Integer.valueOf(short0));
        if (zombie0 != nullptr) {
      ...
      for (Short short1 : Sets.difference(this.authoriseZombiesLast,
        ...
        FakeClientManager.Zombie zombie1 =
            this.zombies.get(Integer.valueOf(short1));
        if (zombie1 != nullptr) {
    ...
    void send() {
      if (this.authoriseZombies.size() != 0 ||
          this.unknownZombies.size() != 0) {
        ...
        if (int3 > 0 || int0 > 0) {
          ...
          this.player.client.doPacket(
              PacketTypes.PacketType.ZombieSimulation.getId(), byteBuffer);
          byteBuffer.put(this.bb.array(), 0, this.bb.position());
          this.player.client.network.endPacketSuperHighUnreliable(
              this.player.client.connectionGUID);
        }
      }
    }

    void simulate(int var1, FakeClientManager.Zombie zombie0) {
      float float0 = IsoUtils.DistanceToSquared(this.player.x, this.player.y,
                                                zombie0.x, zombie0.y);
      if (!(float0 > deleteZombieDistanceSquared) &&
          (zombie0.localOwnership ||
           zombie0.lastUpdate + 5000L >= System.currentTimeMillis())) {
        ...
        if (canChangeTarget) {
          synchronized(this.player.playerManager.players) {
            for (FakeClientManager.PlayerManager.RemotePlayer remotePlayer :
                 this.player.playerManager.players.values()) {
              float float2 = IsoUtils.DistanceToSquared(
                  remotePlayer.x, remotePlayer.y, zombie0.x, zombie0.y);
        ...
        if (behaviour == FakeClientManager.ZombieSimulator.Behaviour.Stay) {
          ...
          if (float0 < canSeeZombieDistanceSquared &&
              (Rand.Next(100) < 1 ||
               zombie0.dir == IsoDirections.fromAngle(tmpDir))) {
        ...
        if (zombie0.isMoving) {
          ...
          zombie0.zombiePacket.x =
              zombie0.x + vector.x * (float)long0 * 0.001F * float3;
          zombie0.zombiePacket.y =
              zombie0.y + vector.y * (float)long0 * 0.001F * float3;
          zombie0.zombiePacket.z = (byte)zombie0.z;
          zombie0.zombiePacket.moveType =
              NetworkVariables.PredictionTypes.Moving;
        } else {
          zombie0.zombiePacket.x = zombie0.x;
          zombie0.zombiePacket.y = zombie0.y;
          zombie0.zombiePacket.z = (byte)zombie0.z;
          zombie0.zombiePacket.moveType =
              NetworkVariables.PredictionTypes.Static;
        }

        zombie0.zombiePacket.booleanVariables = 0;
        if (float0 < 100.0F) {
          zombie0.zombiePacket.booleanVariables =
              (short)(zombie0.zombiePacket.booleanVariables | 2);
        }
        ...
        zombie0.zombiePacket.walkType =
            NetworkVariables.WalkType.values()[zombie0.walkType];
        ...
        zombie0.zombiePacket.realState =
            NetworkVariables.ZombieState.fromString(
                "fakezombie-" + behaviour.toString().toLowerCase());
        if (zombie0.isMoving) {
    ...
    void writeHitInfoToZombie(ByteBuffer byteBuffer, short short0, float float0,
    ...
    void sendHitCharacter(FakeClientManager.Zombie zombie0, float float0) {
      bool boolean0 = false;
      ByteBuffer byteBuffer = this.player.client.network.startPacket();
      this.player.client.doPacket(PacketTypes.PacketType.HitCharacter.getId(),
                                  byteBuffer);
      byteBuffer.put((byte)3);
      byteBuffer.putShort(this.player.OnlineID);
      byteBuffer.putShort((short)0);
      ...
      FakeClientManager.WriteStringUTF(byteBuffer, "");
      FakeClientManager.WriteStringUTF(byteBuffer, "");
      byteBuffer.putShort((short)((this.player.weapon_isBareHeads ? 2 : 0) +
                                  (boolean0 ? 8 : 0)));
      byteBuffer.putFloat(1.0F);
      byteBuffer.putFloat(1.0F);
      byteBuffer.putFloat(1.0F);
      ...
      for (int int0 = 0; int0 < byte1; int0++) {
        this.writeHitInfoToZombie(byteBuffer, zombie0.OnlineID, zombie0.x,
                                  zombie0.y, float0);
      }
      ...
      this.player.client.network.endPacketImmediate(
          this.player.client.connectionGUID);
    }

    void sendSendDeadZombie(FakeClientManager.Zombie zombie0) {
      ByteBuffer byteBuffer = this.player.client.network.startPacket();
      this.player.client.doPacket(PacketTypes.PacketType.ZombieDeath.getId(),
                                  byteBuffer);
      byteBuffer.putShort(zombie0.OnlineID);
      byteBuffer.putFloat(zombie0.x);
      byteBuffer.putFloat(zombie0.y);
  ...
  private
    static enum Behaviour { Stay, Normal, Attack; }
  }
}
} // namespace network
} // namespace zombie
