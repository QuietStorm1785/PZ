...
public
  final IsoObjectID<IsoZombie> ZombieMap = new IsoObjectID<>(IsoZombie.class);
...
public
  ...
  static final ServerMap.DistToCellComparator distToCellComparator =
      new ServerMap.DistToCellComparator();
private
  final ArrayList<ServerMap.ServerCell> tempCells =
...
private
  ...
  public
    final IsoChunk[][] chunks = new IsoChunk[5][5];
  private
    final HashSet<RoomDef> UnexploredRooms = std::make_unique<HashSet<>>();
    static const ServerChunkLoader chunkLoader = new ServerChunkLoader();
  private
    static final ArrayList<ServerMap.ServerCell> loaded =
        std::make_unique<ArrayList<>>();
    bool startedLoading = false;
    bool bCancelLoading = false;
    bool bLoadingWasCancelled = false;
  private
    static final ArrayList<ServerMap.ServerCell> loaded2 =
        std::make_unique<ArrayList<>>();
