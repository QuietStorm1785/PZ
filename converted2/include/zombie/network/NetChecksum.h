...
public:
public
  static final NetChecksum.Checksummer checksummer =
      new NetChecksum.Checksummer();
public
  static final NetChecksum.Comparer comparer = new NetChecksum.Comparer();
...
public
  static final class Checksummer {
    MessageDigest md;
  private
    final byte[] fileBytes = new byte[1024];
  private
    final byte[] convertBytes = new byte[1024];
  ...
  private
    ...
    std::string error;
  private
    final byte[] checksum = new byte[64];
...
public static final class GroupOfFiles {
  ...
  static final ArrayList<NetChecksum.GroupOfFiles> groups =
