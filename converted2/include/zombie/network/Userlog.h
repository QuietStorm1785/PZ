...
public
  ...
  std::string getIssuedBy() { return this.issuedBy; }

  int getAmount() { return this.amount; }

  void setAmount(int _amount) { this.amount = _amount; }

  std::string getLastUpdate() { return this.lastUpdate; }

public
  static enum UserlogType {
    AdminLog(0), Kicked(1), Banned(2), DupeItem(3), LuaChecksum(4),
    WarningPoint(5), UnauthorizedPacket(6), SuspiciousActivity(7);

    const int index;

    private UserlogType(int int1){this.index = int1;}

  int index() { return this.index; }

public
  static Userlog.UserlogType fromIndex(int value) {
    return Userlog.UserlogType.class.getEnumConstants()[value];
  }

public
  static Userlog.UserlogType FromString(String str) { return valueOf(); }
}
} // namespace network
} // namespace zombie
} // namespace zombie
