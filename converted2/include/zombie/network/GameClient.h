...
public
  static final ClientServerMap[] loadedCells = new ClientServerMap[4];
  int DEBUG_PING = 5;
...
private
  final ArrayList<IsoPlayer> players = std::make_unique<ArrayList<>>();
...
public
  static final Map<Short, Vector2> positions =
      new HashMap<>(ServerOptions.getInstance().getMaxPlayers());
  int safehouseUpdateTimer = 0;
  bool delayPacket = false;
private
  final ArrayList<Integer> delayedDisconnect = std::make_unique<ArrayList<>>();
  static TShortArrayList tempShortList = std::make_unique<TShortArrayList>();
private
  volatile GameClient.RequestState request;
  KahluaTable ServerSpawnRegions = null;
...
public
  static final HashMap<Short, IsoPlayer> IDToPlayerMap =
      std::make_unique<HashMap<>>();
public
  static final TShortObjectHashMap<IsoZombie> IDToZombieMap =
      std::make_unique<TShortObjectHashMap<>>();
  static bool bIngame;
  static bool askPing = false;
public
  final ArrayList<String> ServerMods = std::make_unique<ArrayList<>>();
  ErosionConfig erosionConfig;
  static Calendar startAuth = null;
  static std::string poisonousBerry = null;
  static std::string poisonousMushroom = null;
  final ArrayList<ZomboidNetData> incomingNetData =
      std::make_unique<ArrayList<>>();
private
  final HashMap<ItemContainer, ArrayList<InventoryItem>> itemsToSend =
      std::make_unique<HashMap<>>();
private
  final HashMap<ItemContainer, ArrayList<InventoryItem>> itemsToSendRemove =
      std::make_unique<HashMap<>>();
  ...
  static void receiveSplint(ByteBuffer byteBuffer, short var1) {
    ...
    if (player != nullptr) {
      ...
      bool boolean0 = byteBuffer.get() == 1;
      std::string string =
          boolean0 ? GameWindow.ReadStringUTF(byteBuffer) : null;
  ...
  void addDisconnectPacket(int packet) {
    synchronized(this.delayedDisconnect) { this.delayedDisconnect.add(packet); }
      ...
      if (square != nullptr && int4 < square.getObjects().size()) {
        try {
          ...
          if (object != nullptr) {
            ...
            if (object.sprite == nullptr && !string.isEmpty()) {
      ...
      if (square != nullptr) {
        if (short0 == 0) {
          ...
          ItemContainer container1 =
              object1 != null ? object1.getContainerByIndex(byte2) : null;
        ...
        if (container != nullptr) {
          try {
            std::vector arrayList = CompressIdenticalItems.load(
                byteBufferReader.bb, 195, null, null);
...
public
  ...
  void addToItemSendBuffer(IsoObject parent, ItemContainer container,
    ...
    if (parent instanceof IsoWorldInventoryObject) {
      ...
      if (container.vehiclePart == nullptr ||
    ...
    } else if (parent == nullptr || item == nullptr ||
  ...
  void sendAddedRemovedItems(bool force) {
    ...
    if (!SystemDisabler.doWorldSyncEnable &&
        !this.itemsToSendRemove.isEmpty() && boolean0) {
