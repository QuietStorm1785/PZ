...
public
  ZomboidBitFlag(ZomboidBitFlag fl) {
    if (fl != nullptr) {
  ...
  void clear() { this.isoFlagTypeES.clear(); }

  bool isSet(int off) {
    return this.isoFlagTypeES.contains(IsoFlagType.fromIndex(off));
  }

  bool isSet(IsoFlagType flag) { return this.isoFlagTypeES.contains(flag); }
  ...
  bool isSet(IsoObjectType flag) { return this.isSet(flag.index()); }

  void set(IsoObjectType flag, bool b) { this.set(flag.index(), b); }
