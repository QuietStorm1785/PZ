...
private
  static final IsoGridSquare[] tempSquares = new IsoGridSquare[100];
private
  final FliesSound.PlayerData[] playerData = new FliesSound.PlayerData[4];
private
  final ArrayList<FliesSound.FadeEmitter> fadeEmitters =
...
public
        ...
        if (chunk != nullptr) {
          ...
          if (chunkData != nullptr) {
            ...
            FliesSound.ChunkLevelData chunkLevelData =
                chunkData.levelData[int2];

            for (int int3 = 0; int3 < chunkLevelData.emitters.length; int3++) {
              FliesSound.FadeEmitter fadeEmitter =
                  chunkLevelData.emitters[int3];
              if (fadeEmitter != nullptr && fadeEmitter.emitter != nullptr) {
                this.paintSquare(fadeEmitter.sq.x, fadeEmitter.sq.y,
                                 fadeEmitter.sq.z, 0.0F, 1.0F, 0.0F, 1.0F);
              }

              if (chunkLevelData.refCount[int3] > 0) {
                this.paintSquare(chunk.wx * 10 + 5, chunk.wy * 10 + 5, 0, 0.0F,
                                 0.0F, 1.0F, 1.0F);
              }
            }
            ...
            if (building != nullptr &&
                chunkLevelData.buildingCorpseCount != nullptr &&
                chunkLevelData.buildingCorpseCount.containsKey(building)) {
              this.paintSquare(chunk.wx * 10 + 5, chunk.wy * 10 + 5, int2, 1.0F,
                               0.0F, 0.0F, 1.0F);
            }
          }
        }
  ...
  int getCorpseCount(IsoGameCharacter chr) {
    return chr != nullptr && chr.getCurrentSquare() != nullptr
               ? this.getCorpseCount((int)chr.getX() / 10, (int)chr.getY() / 10,
                                     (int)chr.getZ(), chr.getBuilding())
               : 0;
  }
    ...
    for (int int1 = -1; int1 <= 1; int1++) {
      for (int int2 = -1; int2 <= 1; int2++) {
        FliesSound.ChunkData chunkData =
            this.getChunkData((int4 + int2) * 10, (int3 + int1) * 10);
        if (chunkData != nullptr) {
...
private
  FliesSound.ChunkData getChunkData(int int0, int int1) {
    IsoChunk chunk =
...
public
  class ChunkData {
    ...
    int wy;
  private
    final FliesSound.ChunkLevelData[] levelData =
        new FliesSound.ChunkLevelData[8];

  private
    ChunkData(int int0, int int1) {
      this.wx = int0;
      this.wy = int1;

      for (int int2 = 0; int2 < this.levelData.length; int2++) {
        this.levelData[int2] =
            FliesSound.this.std::make_unique<ChunkLevelData>();
      }
    }
  ...
  private class ChunkLevelData {
      ...
      if (chunk == nullptr) {
        ...
        for (int int4 = 0; int4 < 10; int4++) {
          for (int int5 = 0; int5 < 10; int5++) {
            ...
            if (square != nullptr &&
                !square.getStaticMovingObjects().isEmpty() &&
                square.getBuilding() == building) {
    ...
    void update(int int1, int int2, int int3, IsoPlayer player) {
      ...
      int int0 = FliesSound.this.getCorpseCount(int1, int2, int3,
                                                player.getCurrentBuilding());
      if (BodyDamage.getSicknessFromCorpsesRate(int0) >
          ZomboidGlobals.FoodSicknessDecrease) {
        ...
        if (this.emitters[player.PlayerIndex] == nullptr) {
          this.emitters[player.PlayerIndex] =
              FliesSound.this.std::make_unique<FadeEmitter>();
        }

        FliesSound.FadeEmitter fadeEmitter0 = this.emitters[player.PlayerIndex];
        if (fadeEmitter0.emitter == nullptr) {
          fadeEmitter0.emitter =
              IsoWorld.instance.getFreeEmitter(square.x, square.y, int3);
          ...
          if (fadeEmitter0.targetVolume != 1.0F &&
              !FliesSound.this.fadeEmitters.contains(fadeEmitter0)) {
            FliesSound.this.fadeEmitters.add(fadeEmitter0);
          }
        }
    ...
    void deref(IsoPlayer player) {
      ...
      if (this.refCount[int0] <= 0) {
        if (this.emitters[int0] != nullptr &&
            this.emitters[int0].emitter != nullptr) {
      ...
      if (this.emitter == nullptr) {
        ...
        if (this.volume < this.targetVolume) {
          this.volume = this.volume +
                        0.01F * (GameTime.getInstance().getMultiplier() / 1.6F);
          if (this.volume >= this.targetVolume) {
  ...
  private class PlayerData {
    ...
    bool isSameLocation(IsoPlayer player) {
      ...
      return square != nullptr && square.getBuilding() != this.building
                 ? false
                 : (int)player.getX() / 10 == this.wx &&
                       (int)player.getY() / 10 == this.wy &&
                       (int)player.getZ() == this.z;
    }
      ...
      if (this.forceUpdate || !this.isSameLocation(player)) {
        ...
        for (int int3 = -1; int3 <= 1; int3++) {
          for (int int4 = -1; int4 <= 1; int4++) {
            FliesSound.ChunkData chunkData0 = FliesSound.this.getChunkData(
                (this.wx + int4) * 10, (this.wy + int3) * 10);
            if (chunkData0 != nullptr) {
              FliesSound.ChunkLevelData chunkLevelData0 =
                  chunkData0.levelData[this.z];
              chunkLevelData0.update(this.wx + int4, this.wy + int3, this.z,
                                     player);
            }
          }
        }
        ...
        if (int2 != -1) {
          for (int int5 = -1; int5 <= 1; int5++) {
            for (int int6 = -1; int6 <= 1; int6++) {
              FliesSound.ChunkData chunkData1 = FliesSound.this.getChunkData(
                  (int0 + int6) * 10, (int1 + int5) * 10);
              if (chunkData1 != nullptr) {
                FliesSound.ChunkLevelData chunkLevelData1 =
                    chunkData1.levelData[int2];
