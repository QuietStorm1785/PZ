...
public:
  ...
  static CustomIntegerSandboxOption parse(ScriptParser.Block block) {
    ...
    if (int0 != Integer.MIN_VALUE && int1 != Integer.MIN_VALUE &&
        int2 != Integer.MIN_VALUE) {
      CustomIntegerSandboxOption customIntegerSandboxOption =
          new CustomIntegerSandboxOption(block.id, int0, int1, int2);
      return !customIntegerSandboxOption.parseCommon(block)
                 ? nullptr
