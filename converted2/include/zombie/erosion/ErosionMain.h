...
public:
  ...
  IsoChunk chunk;
private
  ErosionData.Chunk chunkModData;
  Noise2D noiseMain;
  ...
  int epoch = 0;
private
  static final int[][] soilTable = new int[][]{
      {1, 1, 1, 1, 1, 4, 4, 4, 4, 4},      {1, 1, 1, 1, 2, 5, 4, 4, 4, 4},
      {1, 1, 1, 2, 2, 5, 5, 4, 4, 4},      {1, 1, 2, 2, 3, 6, 5, 5, 4, 4},
      {1, 2, 2, 3, 3, 6, 6, 5, 5, 4},      {7, 8, 8, 9, 9, 12, 12, 11, 11, 10},
      {7, 7, 8, 8, 9, 12, 11, 11, 10, 10}, {7, 7, 7, 8, 8, 11, 11, 10, 10, 10},
      {7, 7, 7, 7, 8, 11, 10, 10, 10, 10}, {7, 7, 7, 7, 7, 10, 10, 10, 10, 10}};
  int snowFrac = 0;
  int snowFracYesterday = 0;
private
  int[] snowFracOnDay;

  static ErosionMain getInstance() { return instance; }

public
  ErosionMain(IsoSpriteManager _sprMngr, bool _debug) {
    instance = this;
  ...
  ErosionConfig getConfig() { return this.cfg; }

  ErosionSeason getSeasons() { return this.Season; }

  int getEtick() { return this.eTicks; }

  IsoSpriteManager getSpriteManager() { return this.sprMngr; }
  ...
  void mainTimer() {
    if (GameClient.bClient) {
      ...
      if (gameTime.getDay() != this.day || gameTime.getMonth() != this.month ||
          gameTime.getYear() != this.year) {
      ...
      if (GameServer.bServer) {
        for (int int1 = 0; int1 < ServerMap.instance.LoadedCells.size();
             int1++) {
          ServerMap.ServerCell serverCell =
              ServerMap.instance.LoadedCells.get(int1);
  ...
  int getSnowFraction() { return this.snowFrac; }

  int getSnowFractionYesterday() { return this.snowFracYesterday; }

  bool isSnow() { return this.isSnow; }
  ...
  void loadGridsquare(IsoGridSquare square0) {
    if (square0 != nullptr && square0.chunk != nullptr && square0.getZ() == 0) {
      ...
      if (this.chunkModData.eTickStamp >= this.eTicks &&
          this.chunkModData.epoch == this.epoch) {
        return;
      }
  ...
  void initGridSquare(IsoGridSquare square0, ErosionData.Square square1) {
    ...
    square1.noiseKudzu =
        this.noiseKudzu.layeredNoise(int0 / 10.0F, int1 / 10.0F);
  ...
  bool initConfig() {
    ...
    if (GameClient.bClient) {
      ...
      this.cfgPath =
          ZomboidFileSystem.instance.getFileNameInCurrentSave(string);
      return true;
    } else {
      ...
      file0 = new File(ZomboidFileSystem.instance.getCacheDir() +
                       File.separator + string);
      if (!file0.exists() && !Core.getInstance().isNoSave()) {
        File file1 = ZomboidFileSystem.instance.getMediaFile(
            "data" + File.separator + string);
        if (file1.exists()) {
          try {
            DebugLog.log("erosion: copying " + file1.getAbsolutePath() +
                         " to " + file0.getAbsolutePath());
      ...
      this.cfg.time.eticks =
          (int)Math.floor(Math.min(1.0F, float1 / float0) * 100.0F);
      int int1 = SandboxOptions.instance.ErosionDays.getValue();
      if (int1 > 0) {
        this.cfg.time.tickunit = 144;
        this.cfg.time.eticks =
            (int)Math.floor(Math.min(1.0F, float1 / int1) * 100.0F);
      }
  ...
  void start() {
    if (this.initConfig()) {
      ...
      this.Season.init(season.lat, int1, int0, season.tempDiff,
                       season.seasonLag, season.noon, season.seedA,
                       season.seedB, season.seedC);
      this.Season.setRain(season.jan, season.feb, season.mar, season.apr,
  ...
  void DebugUpdateMapNow() { this.updateMapNow(); }
              ...
              if (chunk0 != nullptr) {
                ...
                if (chunk1.eTickStamp != this.eTicks ||
                  ...
                  for (int int3 = 0; int3 < 10; int3++) {
                    for (int int4 = 0; int4 < 10; int4++) {
                      IsoGridSquare square =
                          chunk0.getGridSquare(int4, int3, 0);
                      if (square != nullptr) {
                        this.loadGridsquare(square);
                      }
                    }
  ...
  static void ChunkLoaded(IsoChunk _chunk) { instance.loadChunk(_chunk); }

  static void EveryTenMinutes() { instance.mainTimer(); }
