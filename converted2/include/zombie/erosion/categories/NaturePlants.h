...
private
  final int[][] soilRef =
      new int[][]{{17, 17, 17, 17, 17, 17, 17, 17, 17, 1, 2, 8, 8},
...
private
  final NaturePlants.PlantInit[] plants = new NaturePlants.PlantInit[]{
    ...
    for (int int1 = int0 - 1; int1 >= 1; int1--) {
      ...
      if (sprite != nullptr && sprite.getName() != nullptr) {
        if (sprite.getName().startsWith("d_plants_1_")) {
          int int2 =
              Integer.parseInt(sprite.getName().replace("d_plants_1_", ""));
          NaturePlants.CategoryData categoryData0 =
              (NaturePlants.CategoryData)this.setCatModData(square1);
          categoryData0.gameObj =
              int2 < 32 ? int2 % 8 : (int2 < 48 ? int2 % 8 + 8 : int2 % 8 + 16);
        ...
        if (!"vegetation_groundcover_01_16" == sprite.getName()) && !"vegetation_groundcover_01_17" == sprite.getName())) {
            ...
            NaturePlants.CategoryData categoryData1 =
                (NaturePlants.CategoryData)this.setCatModData(square1);
            ...
            while (--int1 > 0) {
              ...
              if (sprite != nullptr && sprite.getName() != nullptr &&
                  sprite.getName().startsWith("vegetation_groundcover_01_")) {
                square0.RemoveTileObjectErosionNoRecalc(object);
              }
            }
  ...
  bool validateSpawn(IsoGridSquare square0, ErosionData.Square square1,
    ...
    if (square0.getObjects().size() > (boolean0 ? 2 : 1)) {
      ...
      if (square1.rand(square0.x, square0.y, 101) < this.spawnChance[int0]) {
        NaturePlants.CategoryData categoryData =
            (NaturePlants.CategoryData)this.setCatModData(square1);
  ...
  void update(IsoGridSquare square1, ErosionData.Square square0,
    ...
    if (int0 >= categoryData.spawnTime && !categoryData.doNothing) {
      if (categoryData.gameObj >= 0 &&
        ...
        this.updateObj(square0, data, square1, erosionObj, boolean0, byte0,
                       int1, boolean1);
      } else {
  ...
  void init() {
    for (int int0 = 0; int0 < 100; int0++) {
      if (int0 >= 20 && int0 < 50) {
        this.spawnChance[int0] =
            (int)this.clerp((int0 - 20) / 30.0F, 0.0F, 8.0F);
      } else if (int0 >= 50 && int0 < 80) {
        this.spawnChance[int0] =
            (int)this.clerp((int0 - 50) / 30.0F, 8.0F, 0.0F);
      }
    }
    ...
    for (int int3 = 0; int3 < this.plants.length; int3++) {
      ...
      ErosionObjSprites erosionObjSprites = new ErosionObjSprites(
          1, plantInit.name, false, plantInit.hasFlower, false);
      ...
      ErosionObj erosionObj =
          new ErosionObj(erosionObjSprites, 30, float0, float1, false);
      this.objs.add(erosionObj);
    }
  }

protected
  ErosionCategory.Data allocData() { return new NaturePlants.CategoryData(); }
