...
private
  final NatureTrees.TreeInit[] trees = new NatureTrees.TreeInit[]{
      ...
      new NatureTrees.TreeInit("Cockspur Hawthorn", "e_cockspurhawthorn_1",
                               false),
      new NatureTrees.TreeInit("Dogwood", "e_dogwood_1", false),
      new NatureTrees.TreeInit("Carolina Silverbell", "e_carolinasilverbell_1",
                               false),
    ...
    for (int int1 = int0 - 1; int1 >= 1; int1--) {
      ...
      if (sprite != nullptr && sprite.getName() != nullptr) {
        if (sprite.getName().startsWith("jumbo_tree_01")) {
          ...
          NatureTrees.CategoryData categoryData0 =
              (NatureTrees.CategoryData)this.setCatModData(square1);
          categoryData0.gameObj =
              ints0[square1.rand(square0.x, square0.y, ints0.length)] - 1;
        ...
        if (sprite.getName().startsWith("vegetation_trees")) {
          ...
          NatureTrees.CategoryData categoryData1 =
              (NatureTrees.CategoryData)this.setCatModData(square1);
          categoryData1.gameObj =
              ints1[square1.rand(square0.x, square0.y, ints1.length)] - 1;
        ...
        for (int int6 = 0; int6 < this.trees.length; int6++) {
          if (sprite.getName().startsWith(this.trees[int6].tile)) {
            NatureTrees.CategoryData categoryData2 =
                (NatureTrees.CategoryData)this.setCatModData(square1);
  ...
  void init() {
    for (int int0 = 0; int0 < 100; int0++) {
      this.spawnChance[int0] =
          int0 >= 50 ? (int)this.clerp((int0 - 50) / 50.0F, 0.0F, 90.0F) : 0;
    }
    ...
    for (int int1 = 0; int1 < this.trees.length; int1++) {
      ...
      ErosionObjSprites erosionObjSprites =
          new ErosionObjSprites(6, string1, true, false, boolean0);
        ...
        for (int int3 = 0; int3 < ints.length; int3++) {
          if (int2 > 3) {
            ...
            if (int3 == 0) {
              ...
              erosionIceQueen.addSprite(
                  string0, string2.replace("_1", "JUMBO_1") + "_" + int4);
            } else if (boolean0) {
              erosionObjSprites.setChildSprite(
                  int2, string2.replace("_1", "JUMBO_1") + "_" + int4,
                  ints[int3]);
            }
          } else {
            ...
            if (int3 == 0) {
              ...
              erosionObjSprites.setChildSprite(int2, string2 + "_" + int5,
                                               ints[int3]);
            }
          }
        }
      ...
      ErosionObj erosionObj =
          new ErosionObj(erosionObjSprites, 60, 0.0F, 0.0F, true);
      this.objs.add(erosionObj);
    }
  }

protected
  ErosionCategory.Data allocData() { return new NatureTrees.CategoryData(); }
