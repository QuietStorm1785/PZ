...
private
  final ArrayList<ErosionIceQueen.Sprite> sprites =
  ...
  void addSprite(const std::string &_sprite, const std::string &_winterSprite) {
    ...
    if (sprite0 != nullptr && sprite1 != nullptr) {
      ...
      this.sprites.add(
          new ErosionIceQueen.Sprite(sprite0, _sprite, _winterSprite));
    }
  }
  ...
  void setSnow(bool _isSnow) {
    if (this.snowState != _isSnow) {
      ...
      for (int int0 = 0; int0 < this.sprites.size(); int0++) {
        ...
        sprite.sprite.ReplaceCurrentAnimFrames(this.snowState ? sprite.winter
                                                              : sprite.normal);
      }
    }
  }
...
public
  ErosionIceQueen(IsoSpriteManager _sprMngr) {
    instance = this;
    this.SprMngr = _sprMngr;
    this.setRoofSnow();

    for (int int0 = 0; int0 < 10; int0++) {
      this.addSprite("vegetation_ornamental_01_" + int0,
                     "f_bushes_2_" + (int0 + 10));
