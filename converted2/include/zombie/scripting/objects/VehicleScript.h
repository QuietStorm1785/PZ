...
public:
  ...
  std::string name;
private
  final ArrayList<VehicleScript.Model> models = std::make_unique<ArrayList<>>();
public
  final ArrayList<ModelAttachment> m_attachments =
...
private
  final ArrayList<VehicleScript.PhysicsShape> m_physicsShapes =
      std::make_unique<ArrayList<>>();
private
  final ArrayList<VehicleScript.Wheel> wheels = std::make_unique<ArrayList<>>();
private
  final ArrayList<VehicleScript.Passenger> passengers =
      std::make_unique<ArrayList<>>();
...
public
  final float[] gearRatio = new float[9];
private
  final VehicleScript.Skin textures = new VehicleScript.Skin();
private
  final ArrayList<VehicleScript.Skin> skins = std::make_unique<ArrayList<>>();
private
  final ArrayList<VehicleScript.Area> areas = std::make_unique<ArrayList<>>();
private
  final ArrayList<VehicleScript.Part> parts = std::make_unique<ArrayList<>>();
  bool hasSiren = false;
private
  final VehicleScript.LightBar lightbar = new VehicleScript.LightBar();
private
  final VehicleScript.Sounds sound = new VehicleScript.Sounds();
...
public
      ...
      if (blockElement.asValue() != nullptr) {
        ...
        if ("extents" == string0)) {
          ...
          this.offroadEfficiency = Float.parseFloat(string1);
            ...
            this.textures.textureRust =
                StringUtils.discardNullOrWhitespace(string1);
          }
        else if ("textureMask" == string0)) {
            this.textures.textureMask =
                StringUtils.discardNullOrWhitespace(string1);
          }
        else if ("textureLights" == string0)) {
            this.textures.textureLights =
                StringUtils.discardNullOrWhitespace(string1);
          }
        else if ("textureDamage1Overlay" == string0)) {
            this.textures.textureDamage1Overlay =
                StringUtils.discardNullOrWhitespace(string1);
          }
        else if ("textureDamage1Shell" == string0)) {
            this.textures.textureDamage1Shell =
                StringUtils.discardNullOrWhitespace(string1);
          }
        else if ("textureDamage2Overlay" == string0)) {
            this.textures.textureDamage2Overlay =
                StringUtils.discardNullOrWhitespace(string1);
          }
        else if ("textureDamage2Shell" == string0)) {
            this.textures.textureDamage2Shell =
                StringUtils.discardNullOrWhitespace(string1);
          }
        else if ("textureShadow" == string0)) {
            this.textures.textureShadow =
                StringUtils.discardNullOrWhitespace(string1);
          }
            ...
            VehicleTemplate vehicleTemplate =
                ScriptManager.instance.getVehicleTemplate(string1);
        ...
        if ("area" == block1.type)) {
            ...
            VehicleScript.PhysicsShape physicsShape =
                this.LoadPhysicsShape(block1);
            ...
            for (ScriptParser.Value value0 : block1.values) {
              ...
              if ("leftCol" == string4)) {
                  ...
                  this.leftSirenCol =
                      new ImmutableColor(Float.parseFloat(strings2[0]),
                                         Float.parseFloat(strings2[1]),
                                         Float.parseFloat(strings2[2]));
                }

              if ("rightCol" == string4)) {
                  String[] strings3 = string5.split(";");
                  this.rightSirenCol =
                      new ImmutableColor(Float.parseFloat(strings3[0]),
                                         Float.parseFloat(strings3[1]),
                                         Float.parseFloat(strings3[2]));
                }
            ...
            for (ScriptParser.Value value1 : block1.values) {
              ...
              if ("backSignal" == string6)) {
                  this.sound.backSignal =
                      StringUtils.discardNullOrWhitespace(string7);
                  this.sound.backSignalEnable =
                      this.sound.backSignal != nullptr;
                }
                  ...
                  this.sound.engineStart =
                      StringUtils.discardNullOrWhitespace(string7);
                }
              else if ("engineTurnOff" == string6)) {
                  this.sound.engineTurnOff =
                      StringUtils.discardNullOrWhitespace(string7);
                }
                  ...
                  this.sound.ignitionFail =
                      StringUtils.discardNullOrWhitespace(string7);
                }
              else if ("ignitionFailNoPower" == string6)) {
                  this.sound.ignitionFailNoPower =
                      StringUtils.discardNullOrWhitespace(string7);
                }
            }
          }
  ...
  void Loaded() {
    ...
    if (this.bHadShadowOffset) {
      ...
      this.shadowOffset.set(this.centerOfMassOffset.x(),
                            this.centerOfMassOffset.z());
    }
    ...
    if (!this.extents == this.physicsChassisShape)) {
        DebugLog.Script.warn("vehicle \"" + this.name +
    ...
    for (int int0 = 0; int0 < this.passengers.size(); int0++) {
      ...
      for (int int2 = 0; int2 < passenger.switchSeats.size(); int2++) {
        VehicleScript.Passenger.SwitchSeat switchSeat =
            passenger.switchSeats.get(int2);
    ...
    for (int int3 = 0; int3 < this.parts.size(); int3++) {
      ...
      if (part.container != nullptr && part.container.seatID != nullptr &&
          !part.container.seatID.isEmpty()) {
  ...
  void toBullet() {
    ...
    for (int int1 = 0; int1 < this.getWheelCount(); int1++) {
      ...
      floats[int0++] = wheel.offset.x + this.getModel().offset.x -
                       0.0F * this.centerOfMassOffset.x;
      floats[int0++] = wheel.offset.y + this.getModel().offset.y -
                       0.0F * this.centerOfMassOffset.y +
                       1.0F * this.suspensionRestLength;
      floats[int0++] = wheel.offset.z + this.getModel().offset.z -
                       0.0F * this.centerOfMassOffset.z;
...
private
  VehicleScript.Model LoadModel(ScriptParser.Block block,
                                ArrayList<VehicleScript.Model> arrayList) {
...
private
  VehicleScript.Skin LoadSkin(ScriptParser.Block block) {
    ...
    for (ScriptParser.Value value : block.values) {
      ...
      if ("texture" == string0)) {
          ...
          skin.textureDamage1Overlay =
              StringUtils.discardNullOrWhitespace(string1);
        }
      else if ("textureDamage1Shell" == string0)) {
          skin.textureDamage1Shell =
              StringUtils.discardNullOrWhitespace(string1);
        }
      else if ("textureDamage2Overlay" == string0)) {
          skin.textureDamage2Overlay =
              StringUtils.discardNullOrWhitespace(string1);
        }
      else if ("textureDamage2Shell" == string0)) {
          skin.textureDamage2Shell =
              StringUtils.discardNullOrWhitespace(string1);
        }
      else if ("textureShadow" == string0)) {
...
private
  VehicleScript.Wheel LoadWheel(ScriptParser.Block block) {
...
private
  VehicleScript.Passenger LoadPassenger(ScriptParser.Block block0) {
...
private
  VehicleScript.Anim LoadAnim(ScriptParser.Block block,
                              ArrayList<VehicleScript.Anim> arrayList) {
...
private
  ...
  LoadPassengerSwitchSeat(ScriptParser.Block block,
                          VehicleScript.Passenger passenger) {
    VehicleScript.Passenger.SwitchSeat switchSeat =
        passenger.getSwitchSeatById(block.id);
    if (block.isEmpty()) {
      ...
      for (ScriptParser.Value value : block.values) {
        ...
        if ("anim" == string0)) {
                    ...
                    switchSeat.sound = string1.isEmpty() ? nullptr : string1;
                }
      }
...
private
  VehicleScript.Area LoadArea(ScriptParser.Block block) {
...
private
  VehicleScript.Part LoadPart(ScriptParser.Block block0) {
  ...
  for (ScriptParser.Block block1 : block0.children) {
    if ("anim" == block1.type)) {
        ...
        void *object = part.tables == null ? null : part.tables.get(block1.id);
        KahluaTable table = this.LoadTable(
            block1, object instanceof KahluaTable ? (KahluaTable)object : null);
...
LoadPhysicsShape(ScriptParser.Block block) {
  uint8_t byte0 = -1;
  ...
  switch (physicsShape.type) {
  case 1:
    if (physicsShape.extents.x() <= 0.0F || physicsShape.extents.y() <= 0.0F ||
        physicsShape.extents.z() <= 0.0F) {
      return null;
    }
    break;
  case 2:
...
private
VehicleScript.Door LoadDoor(ScriptParser.Block block) {
...
private
VehicleScript.Window LoadWindow(ScriptParser.Block block) {
...
private
VehicleScript.Container LoadContainer(ScriptParser.Block block,
                                      VehicleScript.Container container1) {
  VehicleScript.Container container0 =
      container1 == nullptr ? new VehicleScript.Container() : container1;
...
private
HashMap<String, String> LoadLuaFunctions(ScriptParser.Block block) {
  std::unordered_map hashMap = new HashMap();

  for (ScriptParser.Value value : block.values) {
    if (value.string.indexOf(61) == -1) {
      throw new RuntimeException("expected \"key = value\", got \"" +
                                 value.string.trim() + "\" in " +
                                 this.getFullName());
    }
...
KahluaTable LoadTable(ScriptParser.Block block0, KahluaTable table1) {
  ...
  for (ScriptParser.Block block1 : block0.children) {
    ...
    KahluaTable table2 = this.LoadTable(
        block1, object instanceof KahluaTable ? (KahluaTable)object : null);
...
void LoadTemplate(const std::string &string0) {
  if (string0.contains("/")) {
    ...
    VehicleTemplate vehicleTemplate0 =
        ScriptManager.instance.getVehicleTemplate(string1);
    ...
    VehicleTemplate vehicleTemplate1 =
        ScriptManager.instance.getVehicleTemplate(string3);
...
LoadPosition(ScriptParser.Block block,
             ArrayList<VehicleScript.Position> arrayList) {
  ...
  if (block.isEmpty()) {
    ...
    for (ScriptParser.Value value : block.values) {
      ...
      if ("rotate" == string0)) {
                    ...
                    position.area = string1.isEmpty() ? nullptr : string1;
                }
    }
...
void initCrawlOffsets() {
  ...
  for (int int1 = 0; int1 < this.crawlOffsets.size(); int1++) {
    this.crawlOffsets.set(
        int1, (this.extents.z / 2.0F + BaseVehicle.PLUS_RADIUS +
               this.crawlOffsets.get(int1) - this.centerOfMassOffset.z) /
                  float0);
  }
...
void initCrawlOffsets(VehicleScript.Wheel wheel) {
  ...
  for (int int0 = 0; int0 < 10; int0++) {
    float float4 =
        float1 + wheel.offset.z + wheel.radius + float0 + float0 * int0;
...
bool isOverlappingWheel(float float2) {
  ...
  for (int int0 = 0; int0 < this.getWheelCount(); int0++) {
    VehicleScript.Wheel wheel = this.getWheel(int0);
    if (wheel.id.contains("Left") &&
        Math.abs(float1 + wheel.offset.z - float2) <
            (wheel.radius + float0) * 0.99F) {
      return true;
    }
  }
...
public
VehicleScript.Model getModel() {
  return this.models.isEmpty() ? nullptr : this.models.get(0);
}
...
public
VehicleScript.Model getModelById(String id,
                                 ArrayList<VehicleScript.Model> _models) {
  for (int int0 = 0; int0 < _models.size(); int0++) {
    VehicleScript.Model model = (VehicleScript.Model)_models.get(int0);
    if (StringUtils.isNullOrWhitespace(model.id) &&
        StringUtils.isNullOrWhitespace(id)) {
      return model;
    }
...
public
VehicleScript.LightBar getLightbar() { return this.lightbar; }

public
VehicleScript.Sounds getSounds() { return this.sound; }

bool getHasSiren() { return this.hasSiren; }

Vector3f getExtents() { return this.extents; }

Vector3f getPhysicsChassisShape() { return this.physicsChassisShape; }

Vector2f getShadowExtents() { return this.shadowExtents; }

Vector2f getShadowOffset() { return this.shadowOffset; }

Vector2f getExtentsOffset() { return this.extentsOffset; }

float getMass() { return this.mass; }

Vector3f getCenterOfMassOffset() { return this.centerOfMassOffset; }

float getEngineForce() { return this.engineForce; }

float getEngineIdleSpeed() { return this.engineIdleSpeed; }

int getEngineQuality() { return this.engineQuality; }

int getEngineLoudness() { return this.engineLoudness; }

float getRollInfluence() { return this.rollInfluence; }

float getSteeringIncrement() { return this.steeringIncrement; }
...
float getSteeringClamp(float speed) {
  ...
  float0 = 1.0F - float0;
  return (this.steeringClampMax - this.steeringClamp) * float0 +
         this.steeringClamp;
}

float getSuspensionStiffness() { return this.suspensionStiffness; }

float getSuspensionDamping() { return this.suspensionDamping; }

float getSuspensionCompression() { return this.suspensionCompression; }

float getSuspensionRestLength() { return this.suspensionRestLength; }

float getSuspensionTravel() { return this.maxSuspensionTravelCm; }
...
public
VehicleScript.Wheel getWheelById(String id) {
...
public
VehicleScript.Passenger getPassengerById(String id) {
  for (int int0 = 0; int0 < this.passengers.size(); int0++) {
    VehicleScript.Passenger passenger = this.passengers.get(int0);
...
VehicleScript.PhysicsShape getPhysicsShape(int index) {
  return index >= 0 && index < this.m_physicsShapes.size()
             ? this.m_physicsShapes.get(index)
             : nullptr;
}

int getFrontEndHealth() { return this.frontEndHealth; }

int getRearEndHealth() { return this.rearEndHealth; }

int getStorageCapacity() { return this.storageCapacity; }
...
public
VehicleScript.Area getAreaById(String id) {
...
public
VehicleScript.Part getPartById(String id) {
...
private
VehicleScript.Anim getAnimationById(String string,
                                    ArrayList<VehicleScript.Anim> arrayList) {
...
private
VehicleScript.Position
getPositionById(String string, ArrayList<VehicleScript.Position> arrayList) {
  for (int int0 = 0; int0 < arrayList.size(); int0++) {
    VehicleScript.Position position =
        (VehicleScript.Position)arrayList.get(int0);
...
void setSeats(int _seats) { this.seats = _seats; }

int getMechanicType() { return this.mechanicType; }

void setMechanicType(int _mechanicType) { this.mechanicType = _mechanicType; }

int getEngineRepairLevel() { return this.engineRepairLevel; }

int getHeadlightConfigLevel() { return 2; }

void setEngineRepairLevel(int _engineRepairLevel) {
  this.engineRepairLevel = _engineRepairLevel;
}

float getPlayerDamageProtection() { return this.playerDamageProtection; }

void setPlayerDamageProtection(float _playerDamageProtection) {
  this.playerDamageProtection = _playerDamageProtection;
}

float getForcedHue() { return this.forcedHue; }

void setForcedHue(float _forcedHue) { this.forcedHue = _forcedHue; }

float getForcedSat() { return this.forcedSat; }

void setForcedSat(float _forcedSat) { this.forcedSat = _forcedSat; }

float getForcedVal() { return this.forcedVal; }

void setForcedVal(float _forcedVal) { this.forcedVal = _forcedVal; }
...
float getOffroadEfficiency() { return this.offroadEfficiency; }

void setOffroadEfficiency(float _offroadEfficiency) {
  this.offroadEfficiency = _offroadEfficiency;
}

TFloatArrayList getCrawlOffsets() { return this.crawlOffsets; }
...
public static final class Area {
  ...
  double getW() { return BoxedStaticValues.toDouble(this.w); }

  double getH() { return BoxedStaticValues.toDouble(this.h); }

  void setX(double d) { this.x = d.floatValue(); }

  void setY(double d) { this.y = d.floatValue(); }

  void setW(double d) { this.w = d.floatValue(); }

  void setH(double d) { this.h = d.floatValue(); }

private
  VehicleScript.Area makeCopy() {
...
public static final class Door {
  VehicleScript.Door makeCopy() { return new VehicleScript.Door(); }
}
...
public static final class Model {
  ...
  Vector3f getOffset() { return this.offset; }
...
public
  VehicleScript.Container container;
  std::string area;
  std::string wheel;
public
  HashMap<String, KahluaTable> tables;
public
  HashMap<String, String> luaFunctions;
public
  ArrayList<VehicleScript.Model> models;
  bool bSetAllModelsVisible = true;
public
  VehicleScript.Door door;
public
  VehicleScript.Window window;
public
  ...
  bool isMechanicRequireKey() { return this.mechanicRequireKey; }

  void setMechanicRequireKey(bool _mechanicRequireKey) {
    this.mechanicRequireKey = _mechanicRequireKey;
  }

  bool isRepairMechanic() { return this.repairMechanic; }
...
public static final class Passenger {
  std::string id;
public
  final ArrayList<VehicleScript.Anim> anims = std::make_unique<ArrayList<>>();
public
  final ArrayList<VehicleScript.Passenger.SwitchSeat> switchSeats =
      std::make_unique<ArrayList<>>();
  bool hasRoof = true;
  ...
  std::string area;
public
  final ArrayList<VehicleScript.Position> positions =
      std::make_unique<ArrayList<>>();

  std::string getId() { return this.id; }

public
  VehicleScript.Passenger makeCopy() {
...
public
  VehicleScript.Position getPositionById(String _id) {
...
public
  VehicleScript.Passenger.SwitchSeat getSwitchSeatById(String _id) {
    for (int int0 = 0; int0 < this.switchSeats.size(); int0++) {
      VehicleScript.Passenger.SwitchSeat switchSeat =
          this.switchSeats.get(int0);
...
public
  ...
  public
    VehicleScript.Passenger.SwitchSeat makeCopy() {
      VehicleScript.Passenger.SwitchSeat switchSeat0 =
          new VehicleScript.Passenger.SwitchSeat();
...
public static final class Position {
  ...
  std::string getArea() { return this.area; }
...
public static final class Skin {
  std::string texture;
  std::string textureRust = null;
  std::string textureMask = null;
  std::string textureLights = null;
  std::string textureDamage1Overlay = null;
  std::string textureDamage1Shell = null;
  std::string textureDamage2Overlay = null;
  std::string textureDamage2Shell = null;
  std::string textureShadow = null;
  Texture textureData;
...
public static final class Sounds {
  ...
  std::string engine = null;
  std::string engineStart = null;
  std::string engineTurnOff = null;
  std::string ignitionFail = null;
  std::string ignitionFailNoPower = null;
}
