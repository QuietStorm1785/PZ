...
public:
  ...
  void Load(const std::string &name, const std::string &totalFile) {
    ...
    for (ScriptParser.Value value : block0.values) {
      ...
      if ("category" == string0)) {
          ...
          this.gameSound.maxInstancesPerEmitter =
              PZMath.tryParseInt(string1, -1);
        }
    }
  ...
  GameSoundClip LoadClip(ScriptParser.Block block) {
    ...
    for (ScriptParser.Value value : block.values) {
      ...
      if ("distanceMax" == string0)) {
          ...
          gameSoundClip.initFlags =
              (short)(gameSoundClip.initFlags |
                      GameSoundClip.INIT_FLAG_DISTANCE_MAX);
        }
      else if ("distanceMin" == string0)) {
          gameSoundClip.distanceMin = Integer.parseInt(string1);
          gameSoundClip.initFlags =
              (short)(gameSoundClip.initFlags |
                      GameSoundClip.INIT_FLAG_DISTANCE_MIN);
        }
  ...
  void reset() { this.gameSound.reset(); }
}
} // namespace objects
} // namespace scripting
} // namespace zombie
