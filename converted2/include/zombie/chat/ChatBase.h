...
protected
  final ArrayList<Short> members;
private
  final ArrayList<Short> justAddedMembers = std::make_unique<ArrayList<>>();
private
  final ArrayList<Short> justRemovedMembers = std::make_unique<ArrayList<>>();
protected
...
public
  ...
  IsoPlayer getChatOwner() {
    if (this.chatOwner != nullptr || this.mode == ChatMode.ServerMultiPlayer) {
      ...
      throw new NullPointerException("chat owner is nullptr");
    } else {
...
getJustAddedMembers() {
  synchronized(this.memberLock) { return this.justAddedMembers; }
}

public
ArrayList<Short> getJustRemovedMembers() {
  synchronized(this.memberLock) { return this.justRemovedMembers; }
}
