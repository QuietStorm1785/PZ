...
public
  GeneralChat(ByteBuffer byteBuffer, ChatTab chatTab, IsoPlayer player) {
...
public
  GeneralChat(int int0, ChatTab chatTab, bool boolean0) {
...
public
  GeneralChat() { super(ChatType.general); }
  ...
  void sendMessageToChatMembers(ChatMessage chatMessage) {
    if (this.discordEnabled) {
      ...
      if (chatMessage.isFromDiscord()) {
        ...
        GameServer.discordBot.sendMessage(chatMessage.getAuthor(),
                                          chatMessage.getText());
  ...
  void sendToDiscordGeneralChatDisabled() {
    GameServer.discordBot.sendMessage(
        "Server", Translator.getText("UI_chat_general_chat_disabled"));
  }
