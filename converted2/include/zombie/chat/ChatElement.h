...
/**
 * Turbo  shared display of chat lines functionallity for iso objects & players
 * (characters)
 */
class ChatElement {
public:
protected
  ChatElement.PlayerLines[] playerLines = new ChatElement.PlayerLines[4];
  ...
  std::string sayLine = null;
  std::string sayLineTag = null;
  TextDrawObject sayLineObject = null;
  bool Speaking = false;
  ...
  int bufferY = 0;
private
  static ChatElement.PlayerLinesList[] renderBatch =
      new ChatElement.PlayerLinesList[4];
private
  static HashSet<String> noLogText = std::make_unique<HashSet<>>();
...
public
  ...
  int getMaxChatLines() { return this.maxChatLines; }
  ...
  void setHistoryRange(float range) { this.historyRange = range; }

  void setUseEuclidean(bool b) { this.useEuclidean = b; }

  bool getHasChatToDisplay() { return this.hasChatToDisplay; }
  ...
  float getDistance(IsoPlayer player) {
    if (player == nullptr) {
      ...
      return this.useEuclidean
                 ? (float)Math.sqrt(
                       Math.pow(this.owner.getX() - player.x, 2.0) +
                       Math.pow(this.owner.getY() - player.y, 2.0))
                 : Math.abs(this.owner.getX() - player.x) +
                       Math.abs(this.owner.getY() - player.y);
    }
  }

  bool playerWithinBounds(IsoPlayer player, float float0) {
    return player == nullptr ? false
      ...
      for (int int0 = 0; int0 < IsoPlayer.numPlayers; int0++) {
        ...
        if (player != nullptr) {
          ...
          if (n < playerLinesx.chatLines.length) {
            if (playerLinesx.chatLines[n].getOriginal() != nullptr &&
                playerLinesx.chatLines[n].getOriginal() == text)) {
                playerLinesx.chatLines[n].setInternalTickClock(
                    playerLinesx.lineDisplayTime);
              }
            else {
              playerLinesx.chatLines[n].setSettings(true, true, true, true,
                                                    true, true);
              playerLinesx.chatLines[n].setInternalTickClock(
                  playerLinesx.lineDisplayTime);
              playerLinesx.chatLines[n].setCustomTag("default");
              playerLinesx.chatLines[n].setDefaultColors(1.0F, 1.0F, 1.0F,
                                                         1.0F);
              playerLinesx.chatLines[n].ReadString(UIFont.Medium, text,
...
public
  void addChatLine(String msg, float r, float g, float b, UIFont font,
                   float baseRange, String customTag, boolean bbcode,
                   boolean img, boolean icons, boolean colors, boolean fonts,
        ...
        if (player != nullptr &&
            (!player.Traits.Deaf.isSet() ||
             (this.owner instanceof
              IsoTelevision ? ((IsoTelevision)this.owner).isFacing(player)
                            : !(this.owner instanceof IsoRadio) &&
                                  !(this.owner instanceof VehiclePart)))) {
          ...
          if (float0 < 1.0F) {
            ...
            if (textDrawObject != nullptr) {
              textDrawObject.setSettings(bbcode, img, icons, colors, fonts,
                                         equalizeHeights);
              textDrawObject.setInternalTickClock(playerLinesx.lineDisplayTime);
              textDrawObject.setCustomTag(customTag);
              std::string string;
              if (float0 > 0.0F) {
                string = ZomboidRadio.getInstance().scrambleString(
                    msg, (int)(100.0F * float0), true, "...");
    ...
    if (this.owner == player) {
      ...
      if (this.owner.getSquare() != nullptr && player.getSquare() != nullptr) {
        if (player.getBuilding() != nullptr &&
          ...
          if (player.getSquare().getRoom() ==
              this.owner.getSquare().getRoom()) {
        ...
        if (Math.abs(player.getZ() - this.owner.getZ()) >= 1.0F) {
          float0 =
              (float)(float0 -
                      float0 *
                          (Math.abs(player.getZ() - this.owner.getZ()) * 0.25));
          boolean0 = true;
        }
      }
  ...
  void renderBatched(int playerNum, int x, int y, bool ignoreRadioLines) {
    if (playerNum < this.playerLines.length && this.hasChatToDisplay &&
        !GameServer.bServer) {
  ...
  void clear(int playerIndex) { this.playerLines[playerIndex].clear(); }

  static void RenderBatch(int playerNum) {
    if (renderBatch[playerNum] != nullptr &&
        renderBatch[playerNum].size() > 0) {
  ...
  protected
    TextDrawObject[] chatLines;

  public
    PlayerLines(int arg1) {
      this.chatLines = new TextDrawObject[arg1];

      for (int int0 = 0; int0 < this.chatLines.length; int0++) {
        this.chatLines[int0] =
            new TextDrawObject(0, 0, 0, true, true, true, true, true, true);
    ...
    TextDrawObject getNewLineObject() {
      if (this.chatLines != nullptr && this.chatLines.length > 0) {
        TextDrawObject textDrawObject =
            this.chatLines[this.chatLines.length - 1];
        ...
        if (ChatElement.this.hasChatToDisplay) {
          ...
          for (TextDrawObject textDrawObject : this.chatLines) {
            if (textDrawObject.getEnabled()) {
              if (textDrawObject.getWidth() > 0 &&
                  textDrawObject.getHeight() > 0) {
                float float0 = textDrawObject.getInternalClock();
                if (!(float0 <= 0.0F) &&
                        (!textDrawObject.getCustomTag() == "radio") ||
                    !this.ignoreRadioLines)) {
                    ...
                    bool boolean0 =
                        textDrawObject.getDefaultFontEnum() != UIFont.Dialogue;
                    if (ChatElement.doBackDrop &&
                        ChatElement.backdropTexture != nullptr) {
                      ChatElement.backdropTexture.renderInnerBased(
                          this.renderX - textDrawObject.getWidth() / 2,
                          this.renderY, textDrawObject.getWidth(),
                          textDrawObject.getHeight(), 0.0F, 0.0F, 0.0F, 0.4F);
                    }

                    if (int0 == 0) {
                      textDrawObject.Draw(this.renderX, this.renderY, boolean0,
                                          float1);
                    } else if (ChatElement.this.historyVal > 0.0F) {
                      textDrawObject.Draw(this.renderX, this.renderY, boolean0,
                                          float1 * ChatElement.this.historyVal);
                    }
      ...
      if (ChatElement.this.hasChatToDisplay) {
        ...
        for (int int0 = 0; int0 < this.chatLines.length; int0++) {
          if (!(this.chatLines[int0].getInternalClock() <= 0.0F)) {
            ...
            this.chatLines[int0].updateInternalTickClock(
                this.chatLines[int0].getInternalClock());
          }
        }
