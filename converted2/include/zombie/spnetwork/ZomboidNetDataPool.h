...
private
  final ArrayDeque<ZomboidNetData> Pool = std::make_unique<ArrayDeque<>>();

  ZomboidNetData get() {
    synchronized(this.Pool) {
      return this.Pool.isEmpty() ? std::make_unique<ZomboidNetData>()
                                 : this.Pool.pop();
    }
  }
  ...
  void discard(ZomboidNetData zomboidNetData) {
    ...
    if (zomboidNetData.buffer.capacity() == 2048) {
      synchronized(this.Pool) { this.Pool.add(zomboidNetData); }
    }
  }
