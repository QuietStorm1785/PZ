...
public:
private
  final ArrayDeque<IsoZombie> ReusableZombies =
      std::make_unique<ArrayDeque<>>();
private
  final HashSet<IsoZombie> ReusableZombieSet = std::make_unique<HashSet<>>();
private
  final ArrayList<IsoZombie> ReusedThisFrame = std::make_unique<ArrayList<>>();
private
  final ArrayList<IsoZombie> RecentlyRemoved = std::make_unique<ArrayList<>>();
  static VirtualZombieManager instance = new VirtualZombieManager();
  int MaxRealZombies = 1;
private
  final ArrayList<IsoZombie> m_tempZombies = std::make_unique<ArrayList<>>();
public
  final ArrayList<IsoGridSquare> choices = std::make_unique<ArrayList<>>();
private
  final ArrayList<IsoGridSquare> bestchoices = std::make_unique<ArrayList<>>();
  ...
  bool removeZombieFromWorld(IsoZombie z) {
    bool boolean0 = z.getCurrentSquare() != null;
  ...
  void reuseZombie(IsoZombie zombie0) {
    if (zombie0 != nullptr) {
      ...
      assert zombie0.getCurrentSquare() == nullptr ||
          !zombie0.getCurrentSquare().getMovingObjects().contains(zombie0);
  ...
  bool isReused(IsoZombie z) { return this.ReusableZombieSet.contains(z); }
  ...
  void update() {
    ...
    if (!GameClient.bClient && !GameServer.bServer) {
      for (int int1 = 0;
           int1 < IsoWorld.instance.CurrentCell.getZombieList().size();
           int1++) {
        IsoZombie zombie1 =
            IsoWorld.instance.CurrentCell.getZombieList().get(int1);
  ...
  IsoZombie createRealZombieAlways(int descriptorID, int ZombieDir,
                                   bool bDead) {
  ...
  IsoGridSquare pickEatingZombieSquare(float float2, float float3, float float1,
                                       float float0, int int0) {
    IsoGridSquare square = IsoWorld.instance.CurrentCell.getGridSquare(
        (double)float1, (double)float0, (double)int0);
    if (square == nullptr || !this.canSpawnAt(square.x, square.y, square.z) ||
        square.HasStairs()) {
      return null;
    } else {
      return PolygonalMap2.instance.lineClearCollide(
                 float2, float3, float1, float0, int0, nullptr, false, true)
                 ? nullptr
                 : square;
    }
  }
  ...
  void createEatingZombies(IsoDeadBody target, int nb) {
    if (!IsoWorld.getZombiesDisabled()) {
      for (int int0 = 0; int0 < nb; int0++) {
        ...
        IsoGridSquare square = this.pickEatingZombieSquare(
            target.x, target.y, float0, float1, (int)target.z);
        if (square != nullptr) {
          this.choices.clear();
  ...
  IsoZombie createRealZombie(int int0, bool boolean0) {
    return GameClient.bClient ? nullptr
                              : this.createRealZombieAlways(int0, boolean0);
  }
    ...
    for (int int0 = 0; int0 < nSize; int0++) {
      ...
      do {
        ...
        int1++;
               ...
               (object == nullptr || !((IsoGridSquare)object).isFree(false)));
      ...
      if (object != nullptr) {
        ...
        for (int int4 = 0; int4 < byte0; int4++) {
          ...
          chk.addBloodSplat(((IsoGridSquare)object).getX() + --float0,
                            ((IsoGridSquare)object).getY() + --float1,
                            ((IsoGridSquare)object).getZ(), Rand.Next(12) + 8);
        }
      }
    }
  }
...
public
  ArrayList<IsoZombie> addZombiesToMap(int nSize, RoomDef room) {
...
public
    ...
    return arrayList;
      ...
      for (int int0 = 0; int0 < room.rects.size(); int0++) {
        ...
        for (int int2 = roomRect.x; int2 < roomRect.getX2(); int2++) {
          for (int int3 = roomRect.y; int3 < roomRect.getY2(); int3++) {
            object =
                IsoWorld.instance.CurrentCell.getGridSquare(int2, int3, int1);
            if (object != nullptr && this.canSpawnAt(int2, int3, int1)) {
              ...
              for (int int4 = 0; int4 < IsoPlayer.numPlayers; int4++) {
                if (IsoPlayer.players[int4] != nullptr &&
                    ((IsoGridSquare)object).isSeen(int4)) {
                  boolean0 = true;
                }
              }
      ...
      for (int int5 = 0; int5 < nSize; int5++) {
        if (!this.choices.isEmpty()) {
          ...
          uint8_t byte0 = 4;
          IsoZombie zombie0 = this.createRealZombie(
              int6, bAllowDead ? Rand.Next((int)byte0) == 0 : false);
  ...
  void addIndoorZombies(int int4, RoomDef roomDef, bool boolean0) {
    ...
    for (int int5 = 0; int5 < int4; int5++) {
      if (!this.choices.isEmpty()) {
        ...
        uint8_t byte0 = 4;
        IsoZombie zombie0 = this.createRealZombie(
            int6, boolean0 ? Rand.Next((int)byte0) == 0 : false);
        if (zombie0 != nullptr && zombie0.getSquare() != nullptr) {
          ...
          for (int int7 = int3; int7 < int5; int7++) {
            for (int int8 = int4; int8 < int6; int8++) {
              IsoGridSquare square = chunk.getGridSquare(
                  int7 - chunk.wx * 10, int8 - chunk.wy * 10, int1);
  ...
  void addIndoorZombiesToChunk(IsoChunk chunk, IsoRoom room) {
    ...
    this.addIndoorZombiesToChunk(chunk, room, room.def.spawnCount,
                                 this.m_tempZombies);
    ZombieSpawnRecorder.instance.record(this.m_tempZombies,
  ...
  void createHordeFromTo(float spawnX, float spawnY, float targetX,
  ...
  IsoZombie createRealZombie(float x, float y, float z) {
    this.choices.clear();
    this.choices.add(IsoWorld.instance.CurrentCell.getGridSquare(
  ...
  int getZombieCountForRoom(IsoRoom room) {
    if (IsoWorld.getZombiesDisabled()) {
      ...
      IsoMetaChunk metaChunk =
          IsoWorld.instance.getMetaChunk(room.def.x / 10, room.def.y / 10);
      ...
      if (room.def.spawnCount > 0) {
        if (room.getBuilding().getDef().isFullyStreamedIn()) {
          std::vector arrayList =
              this.addZombiesToMap(room.def.spawnCount, room.def, false);
          ZombieSpawnRecorder.instance.record(arrayList, "roomSpotted");
        } else {
          this.m_tempZombies.clear();
          room.def.forEachChunk(
              (var2x, chunk)
                  ->this.addIndoorZombiesToChunk(
                      chunk, room, room.def.spawnCount, this.m_tempZombies));
          ZombieSpawnRecorder.instance.record(this.m_tempZombies,
                                              "roomSpotted");
        }
      }
  ...
  bool isBlockedInAllDirections(int int0, int int1, int int2) {
    ...
    if (square == nullptr) {
      ...
      bool boolean0 = IsoGridSquare.getMatrixBit(square.pathMatrix, 1, 0, 1) &&
                      square.nav[IsoDirections.N.index()] != null;
      bool boolean1 = IsoGridSquare.getMatrixBit(square.pathMatrix, 1, 2, 1) &&
                      square.nav[IsoDirections.S.index()] != null;
      bool boolean2 = IsoGridSquare.getMatrixBit(square.pathMatrix, 0, 1, 1) &&
                      square.nav[IsoDirections.W.index()] != null;
      bool boolean3 = IsoGridSquare.getMatrixBit(square.pathMatrix, 2, 1, 1) &&
                      square.nav[IsoDirections.E.index()] != null;
  ...
  bool canPathOnlyN(IsoGridSquare square) {
    while (true) {
      ...
      if ((int0 & (this.BLOCKED_W | this.BLOCKED_E)) !=
          (this.BLOCKED_W | this.BLOCKED_E)) {
        return false;
      }
  ...
  bool canPathOnlyW(IsoGridSquare square) {
    while (true) {
      int int0 = this.getBlockedBits(square);
      if ((int0 & (this.BLOCKED_N | this.BLOCKED_S)) !=
          (this.BLOCKED_N | this.BLOCKED_S)) {
        return false;
      }
  ...
  bool canSpawnAt(int x, int y, int z) {
    ...
    if (square != nullptr && square.isFree(false)) {
      int int0 = this.getBlockedBits(square);
      if (int0 ==
          (this.BLOCKED_N | this.BLOCKED_S | this.BLOCKED_W | this.BLOCKED_E)) {
        return false;
      } else {
        return (int0 & (this.BLOCKED_N | this.BLOCKED_S)) ==
                           (this.BLOCKED_N | this.BLOCKED_S) &&
                       this.canPathOnlyW(square) && this.canPathOnlyE(square)
                   ? false
                   : (int0 & (this.BLOCKED_W | this.BLOCKED_E)) !=
                             (this.BLOCKED_W | this.BLOCKED_E) ||
