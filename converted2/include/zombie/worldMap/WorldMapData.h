...
public:
public
  static final HashMap<String, WorldMapData> s_fileNameToData =
      std::make_unique<HashMap<>>();
  std::string m_relativeFileName;
public
  final ArrayList<WorldMapCell> m_cells = std::make_unique<ArrayList<>>();
public
  final HashMap<Integer, WorldMapCell> m_cellLookup =
      std::make_unique<HashMap<>>();
  int m_minX;
  int m_minY;
  int m_maxX;
...
public
  WorldMapData(AssetPath path, AssetManager manager) { super(path, manager); }

public
  WorldMapData(AssetPath path, AssetManager manager,
  ...
  int getWidthInSquares() { return this.getWidthInCells() * 300; }

  int getHeightInSquares() { return this.getHeightInCells() * 300; }
  ...
  int getCellKey(int int0, int int1) { return int0 + int1 * 1000; }
  ...
  void hitTest(float x, float y, ArrayList<WorldMapFeature> features) {
    ...
    if (int0 >= this.m_minX && int0 <= this.m_maxX && int1 >= this.m_minY &&
        int1 <= this.m_maxY) {
