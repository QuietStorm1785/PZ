...
public:
public
  final ArrayList<WorldMapData> m_data = std::make_unique<ArrayList<>>();
public
  final ArrayList<WorldMapImages> m_images = std::make_unique<ArrayList<>>();
  int m_minDataX;
  int m_minDataY;
  int m_maxDataX;
  int m_maxDataY;
...
public
  final ArrayList<WorldMapData> m_lastDataInDirectory =
  ...
  bool hasData() { return !this.m_data.isEmpty(); }
  ...
  bool hasImages() { return !this.m_images.isEmpty(); }
