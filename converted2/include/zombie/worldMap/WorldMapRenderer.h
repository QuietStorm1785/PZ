...
private
  final WorldMapRenderer.Drawer[] m_drawer = new WorldMapRenderer.Drawer[3];
private
  final WorldMapRenderer.CharacterModelCamera m_CharacterModelCamera =
...
private
  static final class CharacterModelCamera extends ModelCamera {
    float m_worldScale;
    float m_angle;
    float m_playerX;
    float m_playerY;
  ...
  public static final class Drawer extends TextureDraw.GenericDrawer {
    ...
    const Matrix4f m_modelView = new Matrix4f();
  private
    final WorldMapRenderer.PlayerRenderData[] m_playerRenderData =
    ...
    final ArrayList<WorldMapFeature> m_features =
        std::make_unique<ArrayList<>>();
    final ArrayList<IsoMetaGrid.Zone> m_zones = std::make_unique<ArrayList<>>();
    final HashSet<IsoMetaGrid.Zone> m_zoneSet = std::make_unique<HashSet<>>();
          ...
          for (int int2 = 0; int2 < this.m_worldMap.m_data.size(); int2++) {
            ...
            if (worldMapData.isReady()) {
              ...
              if (worldMapCell != nullptr &&
                  !worldMapCell.m_features.isEmpty()) {
  ...
  public
    ...
    void calculateVisibleCells() {
      ...
      if (1.0F / float0 > 100.0F) {
        ...
        if (this.m_rasterizeXY_ints == nullptr ||
...
private
  static final class PlayerRenderData {
