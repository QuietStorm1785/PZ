...
protected
  ...
  bool filter(WorldMapFeature feature, WorldMapStyleLayer.FilterArgs args) {
    return this.m_filter == nullptr ? false
                                    : this.m_filter.filter(feature, args);
  }

public
  abstract void render(WorldMapFeature feature,
...
public
  static final class RGBAf {
    float r;
    float g;
    float b;
    float a;
  public
    static final ObjectPool<WorldMapStyleLayer.RGBAf> s_pool =
