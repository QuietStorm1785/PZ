...
public:
  ...
  void execute(IsoGameCharacter character) {
    ...
    PathFindBehavior2.BehaviorResult behaviorResult =
        character.getPathFindBehavior2().update();
    if (behaviorResult == PathFindBehavior2.BehaviorResult.Failed) {
      ...
      IsoChunk chunk =
          GameServer.bServer
              ...
              : IsoWorld.instance.CurrentCell.getChunkForGridSquare(int0, int1,
                                                                    0);
      if (chunk == nullptr) {
        character.setVariable("bPathfind", false);
      ...
      if (character instanceof IsoZombie) {
        ...
        if (IngameState.instance.numberTicks - long0 == 2L) {
          ((IsoZombie)character)
              .parameterZombieState.setState(ParameterZombieState.State.Idle);
        }
      }
    }
  }
  ...
  bool isMoving(IsoGameCharacter character) { return character.isMoving(); }
}
} // namespace vehicles
} // namespace zombie
