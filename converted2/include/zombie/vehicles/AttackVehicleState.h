...
public:
  ...
  static AttackVehicleState instance() { return _instance; }
    ...
    if (zombie0.target instanceof IsoGameCharacter character1) {
      if (character1.isDead()) {
        if (character1.getLeaveBodyTimedown() > 3600.0F) {
          ...
          character1.setLeaveBodyTimedown(
              character1.getLeaveBodyTimedown() +
              GameTime.getInstance().getMultiplier() / 1.6F);
          if (!GameServer.bServer && !Core.SoundDisabled &&
            ...
            std::string string =
                zombie0.isFemale() ? "FemaleZombieEating" : "MaleZombieEating";
        ...
        if (vehicle != nullptr && vehicle.isCharacterAdjacentTo(character0)) {
          Vector3f vector3f = vehicle.chooseBestAttackPosition(
              character1, character0, this.worldPos);
          if (vector3f == nullptr) {
            if (zombie0.AllowRepathDelay <= 0.0F) {
              character0.pathToCharacter(character1);
              zombie0.AllowRepathDelay = 6.25F;
            }
          } else if (vector3f != nullptr &&
                     (Math.abs(vector3f.x - character0.x) > 0.1F ||
                      Math.abs(vector3f.y - character0.y) > 0.1F)) {
            if (!(Math.abs(vehicle.getCurrentSpeedKmHour()) > 0.8F) ||
                !vehicle.isCharacterAdjacentTo(character0) &&
                    !(vehicle.DistToSquared(character0) < 16.0F)) {
              if (zombie0.AllowRepathDelay <= 0.0F) {
                character0.pathToCharacter(character1);
                zombie0.AllowRepathDelay = 6.25F;
              }
      ...
      if (vehicle != nullptr) {
        if (!character1.isDead()) {
          if (animEvent.m_EventName.equalsIgnoreCase("AttackCollisionCheck")) {
            character1.getBodyDamage().AddRandomDamageFromZombie(zombie0,
                                                                 nullptr);
            ...
            if (vehicle.isInArea(string, character0)) {
              ...
              if (part1 != nullptr && part1.getDoor() != nullptr &&
                  part1.getInventoryItem() != nullptr &&
                  !part1.getDoor().isOpen()) {
            ...
            if (vehicleWindow != nullptr) {
              ...
              vehicle.setBloodIntensity(
                  vehicleWindow.part.getId(),
                  vehicle.getBloodIntensity(vehicleWindow.part.getId()) +
                      0.025F);
              if (!GameServer.bServer) {
                zombie0.setVehicleHitLocation(vehicle);
                character0.getEmitter().playSound("ZombieThumpVehicleWindow",
                                                  vehicle);
              }
              ...
              if (!GameServer.bServer) {
                zombie0.setVehicleHitLocation(vehicle);
                character0.getEmitter().playSound("ZombieThumpVehicle",
                                                  vehicle);
              }
            ...
            if (part0 != nullptr && part0.getWindow() == nullptr &&
                part0.getCondition() > 0) {
  ...
  bool isAttacking(IsoGameCharacter var1) { return true; }
