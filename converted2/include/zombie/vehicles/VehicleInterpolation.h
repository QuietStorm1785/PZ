...
public:
  ...
  bool buffering;
private
  static final ArrayDeque<VehicleInterpolationData> pool =
      std::make_unique<ArrayDeque<>>();
private
  static final List<VehicleInterpolationData> outdated =
  ...
  void interpolationDataCurrentAdd(BaseVehicle vehicle) {
    VehicleInterpolationData vehicleInterpolationData =
        pool.isEmpty() ? new VehicleInterpolationData() : pool.pop();
  ...
  bool interpolationDataGet(float[] floats1, float[] floats0, long long0) {
    ...
    VehicleInterpolationData vehicleInterpolationData1 =
        this.buffer.floor(temp);
    if (this.buffering) {
      if (this.buffer.size() < 2 || vehicleInterpolationData0 == nullptr ||
          vehicleInterpolationData1 == nullptr) {
        return false;
      }
