...
public
  ...
  bool isInventoryItemUninstalled() {
    return this.getItemType() != nullptr && !this.getItemType().isEmpty() &&
  ...
  void setAllModelsVisible(bool visible) {
    if (this.scriptPart != nullptr && this.scriptPart.models != nullptr &&
        !this.scriptPart.models.isEmpty()) {
  ...
  bool isContainer() {
    return this.scriptPart == nullptr ? false
  ...
  int getContainerCapacity(IsoGameCharacter chr) {
    if (!this.isContainer()) {
      ...
      return chr = = null ? this.getItemContainer().getCapacity()
...
public
  ...
  void createSpotLight(float xOffset, float yOffset, float dist,
    ...
    this.light =
        this.light == nullptr ? std::make_unique<VehicleLight>() : this.light;
  ...
  float getLightDistance() {
    return this.light == nullptr ? 0.0F
