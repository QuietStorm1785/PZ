...
private
  static final PolygonalMap2.VehiclePoly tempPoly =
...
private
  static final float[] wheelParams = new float[24];
private
  static final float[] physicsParams = new float[27];
  static const uint8_t POSITION_ORIENTATION_PACKET_SIZE = 102;
  static Texture vehicleShadow = null;
  int justBreakConstraintTimer = 0;
  BaseVehicle wasTowedBy = null;
  static const ColorInfo inf = new ColorInfo();
private
  static final float[] lowRiderParam = new float[4];
private
  final BaseVehicle.VehicleImpulse impulseFromServer =
      new BaseVehicle.VehicleImpulse();
private
  final BaseVehicle.VehicleImpulse[] impulseFromSquishedZombie =
      new BaseVehicle.VehicleImpulse[4];
private
  final ArrayList<BaseVehicle.VehicleImpulse> impulseFromHitZombie =
      std::make_unique<ArrayList<>>();
  const int netPlayerTimeoutMax = 30;
  const Vector4f tempVector4f = new Vector4f();
public
  final ArrayList<BaseVehicle.ModelInfo> models =
      std::make_unique<ArrayList<>>();
  IsoChunk chunk;
  bool polyDirty = true;
  ...
  VehicleInterpolation interpolation = null;
  bool waitFullUpdate;
  float throttle = 0.0F;
  double engineSpeed;
...
public
  ...
  float collideY = -1.0F;
public
  final PolygonalMap2.VehiclePoly shadowCoord = new PolygonalMap2.VehiclePoly();
public
  BaseVehicle.engineStateTypes engineState = BaseVehicle.engineStateTypes.Idle;
  long engineLastUpdateStateTime;
  static const int MAX_WHEELS = 4;
  static const int PHYSICS_PARAM_COUNT = 27;
public
  final BaseVehicle.WheelInfo[] wheelInfo = new BaseVehicle.WheelInfo[4];
  bool skidding = false;
  long skidSound;
  long ramSound;
  long ramSoundTime;
  VehicleEngineRPM vehicleEngineRPM = null;
public
  final long[] new_EngineSoundId = new long[8];
  long combinedEngineSound = 0L;
  int engineSoundIndex = 0;
  BaseSoundEmitter hornemitter = null;
  ...
  const IsoLightSource leftLight1 =
      new IsoLightSource(0, 0, 0, 1.0F, 0.0F, 0.0F, 8);
  const IsoLightSource leftLight2 =
      new IsoLightSource(0, 0, 0, 1.0F, 0.0F, 0.0F, 8);
  const IsoLightSource rightLight1 =
      new IsoLightSource(0, 0, 0, 0.0F, 0.0F, 1.0F, 8);
  const IsoLightSource rightLight2 =
      new IsoLightSource(0, 0, 0, 0.0F, 0.0F, 1.0F, 8);
  int leftLightIndex = -1;
  int rightLightIndex = -1;
public
  final BaseVehicle.ServerVehicleState[] connectionState =
      new BaseVehicle.ServerVehicleState[512];
protected
  BaseVehicle.Passenger[] passengers = new BaseVehicle.Passenger[1];
  std::string scriptName;
  VehicleScript script;
protected
  final ArrayList<VehiclePart> parts = std::make_unique<ArrayList<>>();
  VehiclePart battery;
  int engineQuality;
  int engineLoudness;
  int enginePower;
...
protected
  final ArrayList<VehiclePart> lights = std::make_unique<ArrayList<>>();
...
protected
  final PolygonalMap2.VehiclePoly polyPlusRadius =
      new PolygonalMap2.VehiclePoly();
...
private
  final HashMap<String, String> choosenParts = std::make_unique<HashMap<>>();
  std::string type = "";
  ...
  InventoryItem currentKey = null;
  bool doColor = true;
  float brekingSlowFactor = 0.0F;
private
  final ArrayList<IsoObject> brekingObjectsList =
      std::make_unique<ArrayList<>>();
  ...
  std::string specificDistributionId = null;
...
private
  static final HashMap<String, Integer> s_PartToMaskMap =
      std::make_unique<HashMap<>>();
  static const uint8_t BYTE_ZERO = (byte)0;
private
  final HashMap<String, Byte> bloodIntensity = std::make_unique<HashMap<>>();
  bool OptionBloodDecals = false;
  long createPhysicsTime = -1L;
  BaseVehicle vehicleTowing = null;
  BaseVehicle vehicleTowedBy = null;
  int constraintTowing = -1;
  int vehicleTowingID = -1;
  int vehicleTowedByID = -1;
  std::string towAttachmentSelf = null;
  std::string towAttachmentOther = null;
  float towConstraintZOffset = 0.0F;
  const ParameterVehicleBrake parameterVehicleBrake =
      new ParameterVehicleBrake(this);
  const ParameterVehicleEngineCondition parameterVehicleEngineCondition =
      new ParameterVehicleEngineCondition(this);
  const ParameterVehicleGear parameterVehicleGear =
      new ParameterVehicleGear(this);
  const ParameterVehicleLoad parameterVehicleLoad =
      new ParameterVehicleLoad(this);
  const ParameterVehicleRoadMaterial parameterVehicleRoadMaterial =
      new ParameterVehicleRoadMaterial(this);
  const ParameterVehicleRPM parameterVehicleRPM = new ParameterVehicleRPM(this);
  const ParameterVehicleSkid parameterVehicleSkid =
      new ParameterVehicleSkid(this);
  const ParameterVehicleSpeed parameterVehicleSpeed =
      new ParameterVehicleSpeed(this);
  const ParameterVehicleSteer parameterVehicleSteer =
      new ParameterVehicleSteer(this);
  const ParameterVehicleTireMissing parameterVehicleTireMissing =
      new ParameterVehicleTireMissing(this);
...
public
  static final ThreadLocal<BaseVehicle.Vector2ObjectPool> TL_vector2_pool =
      ThreadLocal.withInitial(BaseVehicle.Vector2ObjectPool::new);
public
  static final ThreadLocal<BaseVehicle.Vector2fObjectPool> TL_vector2f_pool =
      ThreadLocal.withInitial(BaseVehicle.Vector2fObjectPool::new);
public
  static final ThreadLocal<BaseVehicle.Vector3fObjectPool> TL_vector3f_pool =
      ThreadLocal.withInitial(BaseVehicle.Vector3fObjectPool::new);
public
  static final ThreadLocal<BaseVehicle.Matrix4fObjectPool> TL_matrix4f_pool =
      ThreadLocal.withInitial(BaseVehicle.Matrix4fObjectPool::new);
public
  static final ThreadLocal<BaseVehicle.QuaternionfObjectPool>
      TL_quaternionf_pool =
          ThreadLocal.withInitial(BaseVehicle.QuaternionfObjectPool::new);
...
protected
  final BaseVehicle.HitVars hitVars = new BaseVehicle.HitVars();

  int getSqlId() { return this.sqlID; }

  static Vector2 allocVector2() { return TL_vector2_pool.get().alloc(); }

  static void releaseVector2(Vector2 v) { TL_vector2_pool.get().release(v); }

  static Vector3f allocVector3f() { return TL_vector3f_pool.get().alloc(); }
...
public
  ...
  static void LoadAllVehicleTextures() {
    ...
    for (VehicleScript vehicleScript :
  ...
  static void LoadVehicleTextures(VehicleScript _script) {
    if (SystemDisabler.doVehiclesWithoutTextures) {
      ...
      skin0.textureDataDamage1Overlay =
          LoadVehicleTexture("vehicles_placeholder_damage1overlay");
      skin0.textureDataDamage1Shell =
          LoadVehicleTexture("vehicles_placeholder_damage1shell");
      skin0.textureDataDamage2Overlay =
          LoadVehicleTexture("vehicles_placeholder_damage2overlay");
      skin0.textureDataDamage2Shell =
          LoadVehicleTexture("vehicles_placeholder_damage2shell");
      skin0.textureDataLights =
          LoadVehicleTexture("vehicles_placeholder_lights");
  ...
  static void LoadVehicleTextures(VehicleScript.Skin skin) {
    ...
    skin.textureDataDamage1Overlay =
        LoadVehicleTexture(skin.textureDamage1Overlay);
    skin.textureDataDamage1Shell = LoadVehicleTexture(skin.textureDamage1Shell);
    skin.textureDataDamage2Overlay =
        LoadVehicleTexture(skin.textureDamage2Overlay);
  ...
  void
  setNetPlayerAuthorization(BaseVehicle.Authorization _netPlayerAuthorization,
                            int _netPlayerId) {
    ...
    DebugLog.Vehicle.trace(
        "vid%s=%d pid=%d %s",
        this.getVehicleTowing() != nullptr
            ? "-a"
            : (this.getVehicleTowedBy() != nullptr ? "-b" : ""),
        this.getId(), _netPlayerId, _netPlayerAuthorization.name());
  }

  bool
  isNetPlayerAuthorization(BaseVehicle.Authorization _netPlayerAuthorization) {
  ...
  short getNetPlayerId() { return this.netPlayerId; }

  std::string getAuthorizationDescription() {
    return String.format("vid:%s(%d) pid:(%d) auth=%s static=%b active=%b",
                         this.scriptName, this.VehicleID, this.netPlayerId,
                         this.netPlayerAuthorization.name(), this.isStatic,
                         this.isActive);
  }
  ...
  double getEngineSpeed() { return this.engineSpeed; }

  std::string getTransmissionNumberLetter() {
    return this.transmissionNumber.getString();
  }

  int getTransmissionNumber() { return this.transmissionNumber.getIndex(); }

  void setClientForce(float force) { this.physics.clientForce = force; }

  float getClientForce() { return this.physics.clientForce; }
  ...
  std::string getObjectName() { return "Vehicle"; }
  ...
  void createPhysics() {
    ...
    if (this.script != nullptr) {
      ...
      if (!this.bCreated) {
        this.bCreated = true;
        uint8_t byte0 = 30;
        if (SandboxOptions.getInstance().RecentlySurvivorVehicles.getValue() ==
            1) {
          byte0 = 0;
        }
      ...
      if (!this.getCell().getVehicles().contains(this) &&
          !this.getCell().addVehicles.contains(this)) {
        this.getCell().addVehicles.add(this);
      }

      this.square = this.getCell().getGridSquare((double)this.x, (double)this.y,
                                                 (double)this.z);
  ...
  bool isPreviouslyEntered() { return this.previouslyEntered; }
  ...
  void putKeyToContainer(ItemContainer container, IsoGridSquare sq,
                         IsoObject obj) {
  ...
  InventoryItem createVehicleKey() {
    ...
    item.setName(Translator.getText(
        "IGUI_CarKey",
        Translator.getText("IGUI_VehicleName" + this.getScript().getName())));
    Color color = Color.HSBtoRGB(this.colorHue, this.colorSaturation * 0.5F,
                                 this.colorValue);
    item.setColor(color);
  ...
  bool addKeyToSquare(IsoGridSquare sq) {
    ...
    for (int int0 = 0; int0 < 3; int0++) {
      for (int int1 = sq.getX() - 10; int1 < sq.getX() + 10; int1++) {
        for (int int2 = sq.getY() - 10; int2 < sq.getY() + 10; int2++) {
          ...
          if (object0 != nullptr) {
            for (int int3 = 0;
                 int3 < ((IsoGridSquare)object0).getObjects().size(); int3++) {
              IsoObject object1 =
                  ((IsoGridSquare)object0).getObjects().get(int3);
              if (object1.container != nullptr &&
                  ...
                  object1.container.type == "officedrawers")
                                        ...
                                        || object1.container.type == "desk")
                                          ...
                                          this.putKeyToContainer(
                                              object1.container,
                                              (IsoGridSquare)object0, object1);
                                          boolean0 = true;
                                          break;
                                        }
            }
            ...
            for (int int4 = 0;
                 int4 < ((IsoGridSquare)object0).getMovingObjects().size();
                 int4++) {
              if (((IsoGridSquare)object0).getMovingObjects().get(int4)
                      instanceof IsoZombie) {
                ((IsoZombie)((IsoGridSquare)object0)
                     .getMovingObjects()
                     .get(int4))
                    .addItemToSpawnAtDeath(this.createVehicleKey());
                boolean0 = true;
                break;
              }
            }
    ...
    if (Rand.Next(10) < 6) {
      while (!boolean0) {
        ...
        object0 = IsoWorld.instance.getCell().getGridSquare(
            (double)int5, (double)int6, (double)this.z);
        if (object0 != nullptr && !((IsoGridSquare)object0).isSolid() &&
            !((IsoGridSquare)object0).isSolidTrans() &&
            !((IsoGridSquare)object0).HasTree()) {
      ...
      if (vehicleDoor == nullptr) {
        ...
        if (_part != nullptr && _part.getInventoryItem() == nullptr) {
          ...
          VehicleWindow vehicleWindow =
              _part == null ? null : _part.getWindow();
          return vehicleWindow != nullptr &&
                 (vehicleWindow.isOpen() || vehicleWindow.isDestroyed());
        }
      }
    }
  }
  ...
  void createParts() {
    for (int int0 = 0; int0 < this.parts.size(); int0++) {
      ...
      if (part.bCreated && arrayList != nullptr && !arrayList.isEmpty() &&
          part.getInventoryItem() == nullptr &&
          part.getTable("install") == nullptr) {
        part.bCreated = false;
      } else if ((arrayList == nullptr || arrayList.isEmpty()) &&
                 part.getInventoryItem() != nullptr) {
        part.item = nullptr;
      }
    ...
    if (this.hasLightbar() && this.getScript().rightSirenCol != nullptr &&
        this.getScript().leftSirenCol != nullptr) {
      ...
      this.rightLight1.r = this.rightLight2.r =
          this.getScript().rightSirenCol.r;
      this.rightLight1.g = this.rightLight2.g =
          this.getScript().rightSirenCol.g;
      this.rightLight1.b = this.rightLight2.b =
          this.getScript().rightSirenCol.b;
    }
  }

  CarController getController() { return this.physics; }
  ...
  void updateSkin() {
    if (this.sprite != nullptr && this.sprite.modelSlot != nullptr &&
        this.sprite.modelSlot.model != nullptr) {
      VehicleModelInstance vehicleModelInstance =
          (VehicleModelInstance)this.sprite.modelSlot.model;
      VehicleScript.Skin skin = this.script.getTextures();
      VehicleScript vehicleScript = this.getScript();
      if (this.getSkinIndex() >= 0 &&
          this.getSkinIndex() < vehicleScript.getSkinCount()) {
  ...
  Texture getShadowTexture() {
    if (this.getScript() != nullptr) {
      ...
      if (this.getSkinIndex() >= 0 &&
          this.getSkinIndex() < this.getScript().getSkinCount()) {
  ...
  void setScript(const std::string &name) {
    if (!StringUtils.isNullOrWhitespace(name)) {
      ...
      bool boolean0 = this.script != null;
      ...
      if (this.script == nullptr) {
        ...
        if (!arrayList0.isEmpty()) {
          ...
          bool boolean1 = this.loaded && this.parts.isEmpty() ||
                          this.scriptName.contains("Burnt");
          if (boolean1 && !arrayList1.isEmpty()) {
            this.script =
                (VehicleScript)arrayList1.get(Rand.Next(arrayList1.size()));
          } else if (!arrayList0.isEmpty()) {
            this.script =
                (VehicleScript)arrayList0.get(Rand.Next(arrayList0.size()));
          }
      ...
      if (this.script != nullptr) {
        ...
        this.passengers =
            new BaseVehicle.Passenger[this.script.getPassengerCount()];
        ...
        for (int int2 = 0; int2 < this.script.getPartCount(); int2++) {
          ...
          for (int int3 = 0; int3 < arrayList2.size(); int3++) {
            ...
            if (part2.getScriptPart() != nullptr &&
                part0.id == part2.getScriptPart().id)) {
                part1 = part2;
                break;
              }
          ...
          if (part0.container != nullptr &&
              part0.container.contentType == nullptr) {
        ...
        this.frontEndDurability =
            Math.min(this.frontEndDurability, this.script.getFrontEndHealth());
        this.rearEndDurability =
            Math.min(this.rearEndDurability, this.script.getRearEndHealth());
      ...
      if (!this.loaded || this.colorHue == 0.0F &&
  ...
  void setScript() { this.setScript(this.scriptName); }
      ...
      for (int int1 = 0; int1 < this.passengers.length; int1++) {
        ...
        if (passenger != nullptr && passenger.character != nullptr) {
          VehicleScript.Position position =
              this.getPassengerPosition(int1, "inside");
          if (position != nullptr) {
...
protected
  BaseVehicle.ModelInfo setModelVisible(VehiclePart part,
    ...
    if (boolean0) {
      ...
      modelInfo1.modelScript =
          ScriptManager.instance.getModelScript(model.file);
...
protected
  BaseVehicle.ModelInfo getModelInfoForPart(VehiclePart part) {
    for (int int0 = 0; int0 < this.models.size(); int0++) {
...
protected
  VehicleScript.Passenger getScriptPassenger(int int0) {
    if (this.getScript() == nullptr) {
      return null;
    } else {
      return int0 >= 0 && int0 < this.getScript().getPassengerCount()
                 ? this.getScript().getPassenger(int0)
                 : nullptr;
    }
  }

  int getMaxPassengers() { return this.passengers.length; }
...
public
  BaseVehicle.Passenger getPassenger(int seat) {
    return seat >= 0 && seat < this.passengers.length ? this.passengers[seat]
                                                      : nullptr;
  }
  ...
  bool isDriver(IsoGameCharacter chr) { return this.getSeat(chr) == 0; }

  Vector3f getWorldPos(Vector3f localPos, Vector3f worldPos,
                       VehicleScript _script) {
    return this.getWorldPos(localPos.x, localPos.y, localPos.z, worldPos,
                            _script);
  }

  Vector3f getWorldPos(float localX, float localY, float localZ,
                       Vector3f worldPos, VehicleScript _script) {
    ...
    float float0 =
        this.jniTransform.origin.x + WorldSimulation.instance.offsetX;
    float float1 =
        this.jniTransform.origin.z + WorldSimulation.instance.offsetY;
  ...
  Vector3f getLocalPos(float worldX, float worldY, float worldZ,
                       Vector3f localPos) {
    Transform transform = this.getWorldTransform(this.tempTransform);
    transform.inverse();
    localPos.set(worldX - WorldSimulation.instance.offsetX, 0.0F,
                 worldY - WorldSimulation.instance.offsetY);
  ...
  Vector3f getPassengerLocalPos(int seat, Vector3f v) {
    BaseVehicle.Passenger passenger = this.getPassenger(seat);
    return passenger = = null ? null
                              : v.set(this.script.getModel().getOffset())
                                    .add(passenger.offset);
  }

  Vector3f getPassengerWorldPos(int seat, Vector3f out) {
    BaseVehicle.Passenger passenger = this.getPassenger(seat);
    return passenger = = null ? null
                              : this.getPassengerPositionWorldPos(
                                    passenger.offset.x, passenger.offset.y,
                                    passenger.offset.z, out);
  }

  Vector3f getPassengerPositionWorldPos(VehicleScript.Position posn,
                                        Vector3f out) {
    return this.getPassengerPositionWorldPos(posn.offset.x, posn.offset.y,
                                             posn.offset.z, out);
  }

  Vector3f getPassengerPositionWorldPos(float x, float y, float z,
                                        Vector3f out) {
...
public
  VehicleScript.Anim getPassengerAnim(int seat, String id) {
...
public
  VehicleScript.Position getPassengerPosition(int seat, String id) {
    VehicleScript.Passenger passenger = this.getScriptPassenger(seat);
      ...
      if (anim != nullptr && !StringUtils.isNullOrWhitespace(anim.anim)) {
        ...
        if (modelInfo != nullptr) {
          ...
          if (animationPlayer != nullptr && animationPlayer.isReady()) {
            if (animationPlayer.getMultiTrack().getIndexOfTrack(
                    modelInfo.m_track) != -1) {
            ...
            if (skinningData == nullptr ||
                skinningData.AnimationClips.containsKey(anim.anim)) {
              AnimationTrack animationTrack =
                  animationPlayer.play(anim.anim, anim.bLoop);
              ...
              if (animationTrack != nullptr) {
                ...
                if (!modelInfo.modelScript.boneWeights.isEmpty()) {
                  animationTrack.setBoneWeights(
                      modelInfo.modelScript.boneWeights);
                  animationTrack.initBoneWeights(skinningData);
                }

                if (part.getWindow() != nullptr) {
                  animationTrack.setCurrentTimeValue(
                      animationTrack.getDuration() *
                      part.getWindow().getOpenDelta());
                }
              }
            }
          }
  ...
  void playCharacterAnim(IsoGameCharacter character, VehicleScript.Anim anim,
                         bool boolean0) {
    ...
    if (anim.angle.lengthSquared() != 0.0F) {
      ...
      matrix4f.rotationXYZ((float)Math.toRadians(anim.angle.x),
                           (float)Math.toRadians(anim.angle.y),
                           (float)Math.toRadians(anim.angle.z));
    ...
    if (character.getAnimationPlayer() != nullptr) {
      character.getAnimationPlayer().setTargetAngle(
          character.getForwardDirection().getDirection());
      if (boolean0) {
      ...
      if (position != nullptr) {
        if (this.getCharacter(seat) == chr) {
          ...
          if (position.area == nullptr) {
            ...
            Vector2 vector1 =
                this.areaPositionWorld4PlayerInteract(area, vector0);
  ...
  void setCharacterPositionToAnim(IsoGameCharacter chr, int seat,
    ...
    if (anim != nullptr) {
      if (this.getCharacter(seat) == chr) {
        ...
        Vector3f vector3f =
            this.getWorldPos(anim.offset, TL_vector3f_pool.get().alloc());
        chr.setX(vector3f.x);
        chr.setY(vector3f.y);
        chr.setZ(0.0F);
        TL_vector3f_pool.get().release(vector3f);
...
public
  VehicleScript.Passenger.SwitchSeat getPassengerSwitchSeat(int seat,
                                                            int index) {
    VehicleScript.Passenger passenger = this.getScriptPassenger(seat);
    if (passenger == nullptr) {
      return null;
    } else {
      return index >= 0 && index < passenger.switchSeats.size()
                 ? passenger.switchSeats.get(index)
                 : nullptr;
    }
  }

private
  VehicleScript.Passenger.SwitchSeat getSwitchSeat(int int0, int int2) {
    ...
    if (passenger == nullptr) {
      ...
      for (int int1 = 0; int1 < passenger.switchSeats.size(); int1++) {
        VehicleScript.Passenger.SwitchSeat switchSeat =
            passenger.switchSeats.get(int1);
        if (switchSeat.seat == int2 &&
            this.getPartForSeatContainer(int2) != nullptr &&
            this.getPartForSeatContainer(int2).getInventoryItem() != nullptr) {
          return switchSeat;
        }
      }
  ...
  std::string getSwitchSeatAnimName(int seatFrom, int seatTo) {
    VehicleScript.Passenger.SwitchSeat switchSeat =
        this.getSwitchSeat(seatFrom, seatTo);
  ...
  void switchSeat(IsoGameCharacter chr, int seatTo) {
    ...
    if (int0 != -1) {
      ...
      VehicleScript.Position position =
          this.getPassengerPosition(seatTo, "inside");
      if (position == nullptr) {
        Vector3f vector3f = TL_vector3f_pool.get().alloc();
  ...
  bool isSeatOccupied(int seat) {
    ...
    return part != nullptr && part.getItemContainer() != nullptr &&
                   !part.getItemContainer().getItems().isEmpty()
               ? true
               : this.getCharacter(seat) != nullptr;
  }
        ...
        for (int int0 = 0; int0 < vehicleScript.getPassengerCount(); int0++) {
          if (!this.isEnterBlocked(chr, int0) && !this.isSeatOccupied(int0)) {
            ...
            if (position != nullptr) {
              ...
              if (float5 > 0.5F && IsoUtils.DistanceTo(chr.getX(), chr.getY(),
                                                       float3, float4) < 1.0F) {
                TL_vector3f_pool.get().release(vector3f0);
                TL_vector3f_pool.get().release(vector3f1);
                TL_vector2_pool.get().release(vector);
          ...
          if (part.getArea() != nullptr && this.isInArea(part.getArea(), chr)) {
            ...
            if (string != nullptr && !string == "")) {
                VehicleScript.Area area =
                    vehicleScript.getAreaById(part.getArea());
                if (area != nullptr) {
                  ...
                  if (vector1 != nullptr) {
                    ...
                    vector0.set(vector3f2.x - chr.getX(),
                                vector3f2.y - chr.getY());
                    vector0.normalize();
                    float float5 = vector0.dot(chr.getForwardDirection());
                    if (float5 > 0.5F &&
                        !PolygonalMap2.instance.lineClearCollide(
                            chr.x, chr.y, vector3f2.x, vector3f2.y, (int)chr.z,
                            this, false, true)) {
                      TL_vector2_pool.get().release(vector0);
  ...
  VehiclePart getClosestWindow(IsoGameCharacter chr) {
    if ((int)this.getZ() != (int)chr.getZ()) {
      ...
      for (int int0 = 0; int0 < this.parts.size(); int0++) {
        VehiclePart part = this.parts.get(int0);
        if (part.getWindow() != nullptr && part.getArea() != nullptr &&
            this.isInArea(part.getArea(), chr)) {
  ...
  void getFacingPosition(IsoGameCharacter chr, Vector2 out) {
    Vector3f vector3f0 = this.getLocalPos(chr.getX(), chr.getY(), chr.getZ(),
                                          TL_vector3f_pool.get().alloc());
    Vector3f vector3f1 = this.script.getExtents();
    Vector3f vector3f2 = this.script.getCenterOfMassOffset();
    float float0 = vector3f2.x - vector3f1.x / 2.0F;
    float float1 = vector3f2.x + vector3f1.x / 2.0F;
    ...
    if (vector3f0.x <= 0.0F && vector3f0.z >= float2 && vector3f0.z <= float3) {
      float5 = vector3f0.z;
    } else if (vector3f0.x > 0.0F && vector3f0.z >= float2 &&
               vector3f0.z <= float3) {
      float5 = vector3f0.z;
    } else if (vector3f0.z <= 0.0F && vector3f0.x >= float0 &&
               vector3f0.x <= float1) {
      float4 = vector3f0.x;
    } else if (vector3f0.z > 0.0F && vector3f0.x >= float0 &&
               vector3f0.x <= float1) {
  ...
  bool enter(int seat, IsoGameCharacter chr, Vector3f offset) {
    ...
    if (chr == nullptr) {
      ...
      if (GameClient.bClient) {
        VehicleManager.instance.sendEnter(GameClient.connection, this, chr,
                                          seat);
      }
  ...
  bool enter(int seat, IsoGameCharacter chr) {
    if (this.getPartForSeatContainer(seat) != nullptr &&
        this.getPartForSeatContainer(seat).getInventoryItem() != nullptr) {
      VehicleScript.Position position =
          this.getPassengerPosition(seat, "outside");
      return position != nullptr ? this.enter(seat, chr, position.offset)
                                 : false;
    } else {
      return false;
    }
  ...
  bool enterRSync(int seat, IsoGameCharacter chr, BaseVehicle v) {
    if (chr == nullptr) {
      ...
      VehicleScript.Position position =
          this.getPassengerPosition(seat, "inside");
      if (position != nullptr) {
  ...
  bool exit(IsoGameCharacter chr) {
    ...
    if (chr == nullptr) {
      ...
      if (int0 == -1) {
        ...
        if (GameClient.bClient) {
          VehicleManager.instance.sendExit(GameClient.connection, this, chr,
                                           int0);
        }
  ...
  void save(ByteBuffer output, bool IS_DEBUG_SAVE) {
    if (this.square != nullptr) {
      ...
      this.x = PZMath.clamp(this.x, this.square.x + float0,
                            this.square.x + 1 - float0);
      this.y = PZMath.clamp(this.y, this.square.y + float0,
                            this.square.y + 1 - float0);
    }
  ...
  void load(ByteBuffer input, int WorldVersion, bool IS_DEBUG_SAVE) {
    ...
    if (WorldVersion >= 173) {
      this.savedPhysicsZ =
          PZMath.clamp(input.getFloat(), 0.0F, (int)this.z + 2.4477F);
    }
  ...
  void trySpawnKey() {
    if (!GameClient.bClient) {
      if (this.script != nullptr &&
          this.script.getPartById("Engine") != nullptr) {
        if (this.keySpawned != 1) {
          if (SandboxOptions.getInstance().VehicleEasyUse.getValue()) {
            this.addKeyToGloveBox();
          } else {
            VehicleType vehicleType =
                VehicleType.getTypeFromName(this.getVehicleType());
            int int0 =
                vehicleType == null ? 70 : vehicleType.getChanceToSpawnKey();
  ...
  bool shouldCollideWithObjects() {
    return this.vehicleTowedBy != nullptr
               ? this.vehicleTowedBy.shouldCollideWithObjects()
  ...
  void brekingObjects() {
    ...
    if (boolean0 || boolean1) {
      ...
      Vector2 vector0 = TL_vector2_pool.get().alloc();
      float float0 =
          Math.max(vector3f.x / 2.0F, vector3f.z / 2.0F) + 0.3F + 1.0F;
      int int0 = (int)Math.ceil(float0);

      for (int int1 = -int0; int1 < int0; int1++) {
        for (int int2 = -int0; int2 < int0; int2++) {
          IsoGridSquare square = this.getCell().getGridSquare(
              (double)(this.x + int2), (double)(this.y + int1), (double)this.z);
          if (square != nullptr) {
            if (boolean1) {
              for (int int3 = 0; int3 < square.getObjects().size(); int3++) {
                ...
                if (!(object0 instanceof IsoWorldInventoryObject)) {
                  ...
                  if (!this.brekingObjectsList.contains(object0) &&
                      object0 != nullptr &&
                      object0.getProperties() != nullptr) {
                    if (object0.getProperties().Is("CarSlowFactor")) {
                      vector1 =
                          this.testCollisionWithObject(object0, 0.3F, vector0);
                    }
            ...
            if (boolean0) {
              for (int int4 = 0; int4 < square.getMovingObjects().size();
                   int4++) {
                IsoMovingObject movingObject0 =
                    square.getMovingObjects().get(int4);
                IsoZombie zombie0 =
                    Type.tryCastTo(movingObject0, IsoZombie.class);
                if (zombie0 != nullptr) {
                  if (zombie0.isProne()) {
                ...
                if (movingObject0 instanceof
                    IsoPlayer && movingObject0 != this.getDriver()) {
            ...
            if (boolean1) {
              for (int int5 = 0; int5 < square.getStaticMovingObjects().size();
                   int5++) {
                IsoMovingObject movingObject1 =
                    square.getStaticMovingObjects().get(int5);
                IsoDeadBody deadBody =
                    Type.tryCastTo(movingObject1, IsoDeadBody.class);
                if (deadBody != nullptr) {
      ...
      for (int int7 = 0; int7 < this.brekingObjectsList.size(); int7++) {
        ...
        if (vector2 == nullptr ||
            !object1.getSquare().getObjects().contains(object1)) {
  ...
  void updateVelocityMultiplier() {
    if (this.physics != nullptr && this.getScript() != nullptr) {
      Vector3f vector3f =
          this.getLinearVelocity(TL_vector3f_pool.get().alloc());
      vector3f.y = 0.0F;
  ...
  void playScrapePastPlantSound(IsoGridSquare square) {
    if (this.emitter != nullptr &&
        !this.emitter.isPlaying(this.soundScrapePastPlant)) {
      this.emitter.setPos(square.x + 0.5F, square.y + 0.5F, square.z);
      this.soundScrapePastPlant =
          this.emitter.playSoundImpl("VehicleScrapePastPlant", square);
    }
  }

  void checkCollisionWithPlant(IsoGridSquare square, IsoObject object,
                               Vector2 vector1) {
        ...
        for (int int2 = -int0; int2 < int0; int2++) {
          ...
          if (square0 != nullptr) {
            for (int int3 = 0; int3 < square0.getObjects().size(); int3++) {
              ...
              if (vector1 == nullptr && object.sprite != nullptr &&
                  (object.sprite.getProperties().Is("HitByCar") ||
                   object.sprite.getProperties().Is("CarSlowFactor"))) {
                vector1 = this.testCollisionWithObject(object, 1.0F, vector0);
              }

              if (vector1 == nullptr) {
                IsoGridSquare square1 = this.getCell().getGridSquare(
                    (double)(this.x + int2), (double)(this.y + int1), 1.0);
                if (square1 != nullptr &&
                    square1.getHasTypes().isSet(IsoObjectType.lightswitch)) {
                  vector1 = this.testCollisionWithObject(object, 1.0F, vector0);
                }
              }
              ...
              if (vector1 == nullptr) {
                IsoGridSquare square2 = this.getCell().getGridSquare(
                    (double)(this.x + int2), (double)(this.y + int1), 0.0);
                if (square2 != nullptr &&
                    square2.getHasTypes().isSet(IsoObjectType.lightswitch)) {
                  vector1 = this.testCollisionWithObject(object, 1.0F, vector0);
                }
              }
  ...
  void update() {
    if (this.removedFromWorld) {
      DebugLog.Vehicle.debugln("vehicle update() removedFromWorld=true id=%d",
                               this.VehicleID);
    } else if (!this.getCell().vehicles.contains(this)) {
      DebugLog.Vehicle.debugln(
          "vehicle update() not in cell.vehicles list id=%d x=%f y=%f %s",
          this.VehicleID, this.x, this.y, this);
      this.getCell().getRemoveList().add(this);
    } else {
      if (this.chunk == nullptr) {
        DebugLog.Vehicle.debugln(
            "vehicle update() chunk=nullptr id=%d x=%f y=%f", this.VehicleID,
            this.x, this.y);
      } else if (!this.chunk.vehicles.contains(this)) {
        DebugLog.Vehicle.debugln(
            "vehicle update() not in chunk.vehicles list id=%d x=%f y=%f",
            this.VehicleID, this.x, this.y);
        if (GameClient.bClient) {
          VehicleManager.instance.sendRequestGetPosition(
              this.VehicleID, PacketTypes.PacketType.VehiclesUnreliable);
        }
      } else if (!GameServer.bServer && this.chunk.refs.isEmpty()) {
        DebugLog.Vehicle.debugln("vehicle update() chunk was unloaded id=%d",
                                 this.VehicleID);
      ...
      if (this.getVehicleTowing() != nullptr && this.getDriver() != nullptr) {
        ...
        if (this.getVehicleTowing().scriptName == "Base.Trailer")) {
            VehiclePart part0 =
                this.getVehicleTowing().getPartById("TrailerTrunk");
            if (this.getCurrentSpeedKmHour() > 30.0F &&
                part0.getCondition() < 50 && !part0.container.Items.isEmpty()) {
              ...
              if (!arrayList.isEmpty()) {
                ...
                for (int int4 = 0; int4 < this.getVehicleTowing().parts.size();
                     int4++) {
                  VehiclePart part1 =
                      this.getVehicleTowing().getPartByIndex(int4);
                  if (part1 != nullptr && part1.item != nullptr) {
                    if (part1.partId != nullptr &&
                        part1.partId.contains("Suspension")) {
                      int2 += part1.getCondition();
                    } else if (part1.partId != nullptr &&
                               part1.partId.contains("Tire")) {
                ...
                int int5 = (int)(Math.pow(100 - int1 * 2, 2.0) * 0.3 *
                                 (1.0 + (100 - int2 / 2) * 0.005) *
                                 (1.0 + (100 - int2 / 2) * 0.005) *
                                 (1.0F + float1 / 3.0F));
                if (Rand.Next(0, Math.max(10000 - int5, 1)) == 0) {
                  InventoryItem item = (InventoryItem)arrayList.get(
                      Rand.Next(0, arrayList.size()));
                  item.setCondition(item.getCondition() -
                                    item.getConditionMax() / 10);
                  part0.getSquare().AddWorldInventoryItem(
                      item, Rand.Next(0.0F, 0.5F), Rand.Next(0.0F, 0.5F), 0.0F);
      ...
      if (this.physics != nullptr && this.vehicleTowingID != -1 &&
      ...
      if (this.getVehicleTowedBy() != nullptr &&
          this.getVehicleTowedBy().getController() != nullptr) {
        boolean0 = this.getVehicleTowedBy() != nullptr &&
                   this.getVehicleTowedBy().getController().isEnable;
        boolean1 = this.getVehicleTowing() != nullptr &&
                   this.getVehicleTowing().getDriver() != nullptr;
      }

      if (this.physics != nullptr) {
        bool boolean2 = this.getDriver() != null || boolean0 || boolean1;
        ...
        if (this.constraintChangedTime != -1L) {
          ...
          if (this.physicActiveCheck != -1L &&
              (boolean2 || !this.physics.isEnable)) {
            this.physicActiveCheck = -1L;
          }

          if (!boolean2 && this.physics.isEnable &&
              this.physicActiveCheck != -1L && this.physicActiveCheck < long0) {
            this.physicActiveCheck = -1L;
            this.setPhysicsActive(false);
          }
        }
        ...
        if (this.getVehicleTowedBy() != nullptr &&
            this.getScript().getWheelCount() > 0) {
        ...
        if (!GameServer.bServer && vehicleImpulse0 != nullptr &&
            vehicleImpulse0.enable) {
          vehicleImpulse0.enable = false;
          float float3 = 1.0F;
          Bullet.applyCentralForceToVehicle(this.VehicleID,
                                            vehicleImpulse0.impulse.x * float3,
                                            vehicleImpulse0.impulse.y * float3,
                                            vehicleImpulse0.impulse.z * float3);
          Vector3f vector3f0 = vehicleImpulse0.rel_pos.cross(
              vehicleImpulse0.impulse, TL_vector3f_pool.get().alloc());
          Bullet.applyTorqueToVehicle(this.VehicleID, vector3f0.x * float3,
                                      vector3f0.y * float3,
                                      vector3f0.z * float3);
          TL_vector3f_pool.get().release(vector3f0);
        }
        ...
        short short0 = 1000;
        if (System.currentTimeMillis() - this.engineCheckTime > short0 &&
            !GameClient.bClient) {
          ...
          if (!GameClient.bClient) {
            if (this.engineState != BaseVehicle.engineStateTypes.Idle) {
              ...
              if (GameServer.bServer) {
                int7 =
                    (int)(int7 * ServerOptions.getInstance()
                                     .CarEngineAttractionModifier.getValue());
                int6 =
                    (int)(int6 * ServerOptions.getInstance()
                                     .CarEngineAttractionModifier.getValue());
              }

              if (Rand.Next((int)(int7 *
                ...
                WorldSoundManager.instance.addSoundRepeating(
                    ...
                    Math.max(8, int6), int6 / 40, false);
              }

              if (Rand.Next((int)((int7 - 85) *
                                  GameTime.instance.getInvMultiplier())) == 0) {
                WorldSoundManager.instance.addSoundRepeating(
                    this, (int)this.getX(), (int)this.getY(), (int)this.getZ(),
                    Math.max(8, int6 / 2), int6 / 40, false);
              }

              if (Rand.Next((int)((int7 - 110) *
                                  GameTime.instance.getInvMultiplier())) == 0) {
                WorldSoundManager.instance.addSoundRepeating(
                    this, (int)this.getX(), (int)this.getY(), (int)this.getZ(),
                    Math.max(8, int6 / 4), int6 / 40, false);
              }

              WorldSoundManager.instance.addSoundRepeating(
                  this, (int)this.getX(), (int)this.getY(), (int)this.getZ(),
                  Math.max(8, int6 / 6), int6 / 40, false);
            }

            if (this.lightbarSirenMode.isEnable() &&
                this.getBatteryCharge() > 0.0F) {
              WorldSoundManager.instance.addSoundRepeating(
                  this, (int)this.getX(), (int)this.getY(), (int)this.getZ(),
                  100, 60, false);
            }
          ...
          if (this.engineState == BaseVehicle.engineStateTypes.Running &&
              !this.isEngineWorking()) {
            this.shutOff();
          }

          if (this.engineState == BaseVehicle.engineStateTypes.Running) {
            VehiclePart part2 = this.getPartById("Engine");
            if (part2 != nullptr && part2.getCondition() < 50 &&
                Rand.Next(Rand.AdjustForFramerate(part2.getCondition() * 12)) ==
                    0) {
              this.shutOff();
            }
          }
          ...
          if (this.engineState ==
                  BaseVehicle.engineStateTypes.RetryingStarting &&
              System.currentTimeMillis() - this.engineLastUpdateStateTime >
                  10L) {
            this.engineDoStarting();
          }

          if (this.engineState ==
                  BaseVehicle.engineStateTypes.StartingSuccess &&
              System.currentTimeMillis() - this.engineLastUpdateStateTime >
                  500L) {
            this.engineDoRunning();
          }
          ...
          if (this.engineState ==
                  BaseVehicle.engineStateTypes.StartingFailedNoPower &&
              System.currentTimeMillis() - this.engineLastUpdateStateTime >
                  500L) {
            this.engineDoIdle();
          }

          if (this.engineState == BaseVehicle.engineStateTypes.Stalling &&
              System.currentTimeMillis() - this.engineLastUpdateStateTime >
                  3000L) {
            this.engineDoIdle();
          }

          if (this.engineState == BaseVehicle.engineStateTypes.ShutingDown &&
              System.currentTimeMillis() - this.engineLastUpdateStateTime >
                  2000L) {
            this.engineDoIdle();
          }
        ...
        this.setX(this.jniTransform.origin.x +
                  WorldSimulation.instance.offsetX);
        this.setY(this.jniTransform.origin.z +
                  WorldSimulation.instance.offsetY);
        this.setZ(0.0F);
        IsoGridSquare square = this.getCell().getGridSquare(
            (double)this.x, (double)this.y, (double)this.z);
        if (square == nullptr && !this.chunk.refs.isEmpty()) {
          ...
          transform1.origin.set(this.x - WorldSimulation.instance.offsetX,
                                this.z,
                                this.y - WorldSimulation.instance.offsetY);
          this.setWorldTransform(transform1);
          this.current = this.getCell().getGridSquare(
              (double)this.x, (double)this.y, (double)this.z);
          DebugLog.Vehicle.debugln(
              "Vehicle vid=%d is moved into an unloaded area (%f;%f)",
              this.VehicleID, this.x, this.y);
        }
        ...
        if (this.current != nullptr && this.current.chunk != nullptr) {
          if (this.current.getChunk() != this.chunk) {
            ...
            if (!GameServer.bServer && this.chunk.refs.isEmpty()) {
              DebugLog.Vehicle.debugln(
                  "BaseVehicle.update() added to unloaded chunk id=%d",
                  this.VehicleID);
            }
        ...
        if (this.jniIsCollide && this.limitCrash.Check()) {
          ...
          DebugLog.Vehicle.debugln(
              ...
              this.VehicleID, this.lastLinearVelocity,
              this.lastLinearVelocity.length(), vector3f1, vector3f1.length(),
              float7);
          if (vector3f1.lengthSquared() >
                  this.lastLinearVelocity.lengthSquared() &&
              float7 > 6.0F) {
            DebugLog.Vehicle.trace(
                "Vehicle vid=%d got sharp speed increase delta=%f",
                this.VehicleID, float7);
            float7 = 6.0F;
          }
          ...
          if (float7 > 1.0F) {
            ...
            DebugLog.Vehicle.trace("Vehicle vid=%d crash delta=%f",
                                   this.VehicleID, float7);
      ...
      for (int int12 = 0; int12 < this.impulseFromSquishedZombie.length;
           int12++) {
        BaseVehicle.VehicleImpulse vehicleImpulse1 =
            this.impulseFromSquishedZombie[int12];
      ...
      for (int int13 = 0; int13 < IsoPlayer.numPlayers; int13++) {
        if (this.current == nullptr ||
            !this.current.lighting[int13].bCanSee()) {
      ...
      for (int int14 = 0; int14 < this.getScript().getPassengerCount();
           int14++) {
        if (this.getCharacter(int14) != nullptr) {
          Vector3f vector3f4 =
              this.getPassengerWorldPos(int14, TL_vector3f_pool.get().alloc());
      ...
      if (part3 != nullptr && this.lightbarLightsMode.isEnable() &&
          part3.getCondition() == 0 && !GameClient.bClient) {
        this.setLightbarLightsMode(0);
      }

      if (part3 != nullptr && this.lightbarSirenMode.isEnable() &&
          part3.getCondition() == 0 && !GameClient.bClient) {
      ...
      if (part4 != nullptr &&
          part4.getContainerContentAmount() > part4.getContainerCapacity()) {
  ...
  void updateEngineStarting() {
    if (this.getBatteryCharge() <= 0.1F) {
      ...
      if (part != nullptr && part.getContainerContentAmount() <= 0.0F) {
        ...
        int int0 = 0;
        float float0 = ClimateManager.getInstance().getAirTemperatureForSquare(
            this.getSquare());
        ...
        if (!SandboxOptions.instance.VehicleEasyUse.getValue() &&
            ...
            Rand.Next(this.engineQuality + 50 - int0) <= 30) {
          this.engineDoStartingFailed();
        } else {
  ...
  void applyImpulseFromHitZombies() {
    if (!this.impulseFromHitZombie.isEmpty()) {
      if ((!GameClient.bClient ||
           this.hasAuthorization(GameClient.connection)) &&
          !GameServer.bServer) {
        Vector3f vector3f0 =
            TL_vector3f_pool.get().alloc().set(0.0F, 0.0F, 0.0F);
        Vector3f vector3f1 =
            TL_vector3f_pool.get().alloc().set(0.0F, 0.0F, 0.0F);
        Vector3f vector3f2 =
            TL_vector3f_pool.get().alloc().set(0.0F, 0.0F, 0.0F);
        ...
        for (int int1 = 0; int1 < int0; int1++) {
          BaseVehicle.VehicleImpulse vehicleImpulse0 =
              this.impulseFromHitZombie.get(int1);
          vector3f0.add(vehicleImpulse0.impulse);
          vector3f1.add(vehicleImpulse0.rel_pos.cross(vehicleImpulse0.impulse,
                                                      vector3f2));
        ...
        float float1 = 30.0F;
        Bullet.applyCentralForceToVehicle(this.VehicleID, vector3f0.x * float1,
                                          vector3f0.y * float1,
                                          vector3f0.z * float1);
        Bullet.applyTorqueToVehicle(this.VehicleID, vector3f1.x * float1,
                                    vector3f1.y * float1, vector3f1.z * float1);
        if (GameServer.bServer) {
        }
        ...
        for (int int3 = this.impulseFromHitZombie.size(); int2 < int3; int2++) {
          BaseVehicle.VehicleImpulse vehicleImpulse1 =
              this.impulseFromHitZombie.get(int2);
  ...
  void applyImpulseFromProneCharacters() {
    if ((!GameClient.bClient || this.hasAuthorization(GameClient.connection)) &&
        !GameServer.bServer) {
      bool boolean0 = PZArrayUtil.contains(
          this.impulseFromSquishedZombie,
          vehicleImpulsex->vehicleImpulsex != null && vehicleImpulsex.enable);
      if (boolean0) {
        Vector3f vector3f0 =
            TL_vector3f_pool.get().alloc().set(0.0F, 0.0F, 0.0F);
        Vector3f vector3f1 =
        ...
        for (int int0 = 0; int0 < this.impulseFromSquishedZombie.length;
             int0++) {
          BaseVehicle.VehicleImpulse vehicleImpulse =
              this.impulseFromSquishedZombie[int0];
          if (vehicleImpulse != nullptr && vehicleImpulse.enable) {
            vector3f0.add(vehicleImpulse.impulse);
            vector3f1.add(vehicleImpulse.rel_pos.cross(vehicleImpulse.impulse,
                                                       vector3f2));
        ...
        if (vector3f0.lengthSquared() > 0.0F) {
          ...
          Bullet.applyCentralForceToVehicle(
              this.VehicleID, vector3f0.x * float1, vector3f0.y * float1,
              vector3f0.z * float1);
          Bullet.applyTorqueToVehicle(this.VehicleID, vector3f1.x * float1,
                                      vector3f1.y * float1,
                                      vector3f1.z * float1);
  ...
  float getFudgedMass() {
    if (this.getScriptName().contains("Trailer")) {
      ...
      if (vehicle1 != nullptr && vehicle1.getDriver() != nullptr &&
          vehicle1.isEngineRunning()) {
  ...
  bool isNullChunk(int int0, int int1) {
    if (!IsoWorld.instance.getMetaGrid().isValidChunk(int0, int1)) {
      return false;
               ...
               !ClientServerMap.isChunkLoaded(int0, int1)) {
      return true;
    } else {
      return GameClient.bClient && !PassengerMap.isChunkLoaded(this, int0, int1)
                 ? true
                 : this.getCell().getChunk(int0, int1) == nullptr;
    }
  }
  ...
  bool isInvalidChunkAround(bool moveW, bool moveE, bool moveN, bool moveS) {
    if (IsoChunkMap.ChunkGridWidth > 7) {
      if (moveE && (this.isNullChunk(this.chunk.wx + 1, this.chunk.wy) ||
                    this.isNullChunk(this.chunk.wx + 2, this.chunk.wy))) {
        return true;
      }
      ...
      if (moveS && (this.isNullChunk(this.chunk.wx, this.chunk.wy + 1) ||
                    this.isNullChunk(this.chunk.wx, this.chunk.wy + 2))) {
        return true;
      }
  ...
  void postupdate() {
    ...
    if (this.current == nullptr) {
      for (int int0 = (int)this.z; int0 >= 0; int0--) {
        this.current =
            this.getCell().getGridSquare((int)this.x, (int)this.y, int0);
        if (this.current != nullptr) {
          break;
        }
      }
    ...
    if (this.sprite.hasActiveModel()) {
      ...
      for (int int1 = 0; int1 < this.models.size(); int1++) {
        ...
        this.updateAnimationPlayer(modelInfo.getAnimationPlayer(),
                                   modelInfo.part);
      }
    }
  }

  void updateAnimationPlayer(AnimationPlayer animationPlayer,
                             VehiclePart part) {
    if (animationPlayer != nullptr && animationPlayer.isReady()) {
      ...
      for (int int0 = 0; int0 < animationMultiTrack.getTrackCount(); int0++) {
        AnimationTrack animationTrack0 =
            animationMultiTrack.getTracks().get(int0);
      ...
      if (part != nullptr) {
        ...
        if (modelInfo.m_track != nullptr &&
            animationMultiTrack.getIndexOfTrack(modelInfo.m_track) == -1) {
          modelInfo.m_track = nullptr;
        }
        ...
        if (modelInfo.m_track != nullptr) {
          ...
          if (vehicleWindow0 != nullptr) {
            ...
            animationTrack1.setCurrentTimeValue(animationTrack1.getDuration() *
                                                vehicleWindow0.getOpenDelta());
          }
        } else {
          VehicleDoor vehicleDoor = part.getDoor();
          if (vehicleDoor != nullptr) {
  ...
  void authorizationClientCollide(IsoPlayer driver) {
    if (driver != nullptr && this.getDriver() == nullptr) {
      this.setNetPlayerAuthorization(BaseVehicle.Authorization.LocalCollide,
                                     driver.getOnlineID());
      this.authSimulationTime = System.currentTimeMillis();
      this.interpolation.clear();
      if (this.getVehicleTowing() != nullptr) {
        this.getVehicleTowing().setNetPlayerAuthorization(
            BaseVehicle.Authorization.LocalCollide, driver.getOnlineID());
        this.getVehicleTowing().authSimulationTime = System.currentTimeMillis();
        this.getVehicleTowing().interpolation.clear();
      } else if (this.getVehicleTowedBy() != nullptr) {
        this.getVehicleTowedBy().setNetPlayerAuthorization(
            BaseVehicle.Authorization.LocalCollide, driver.getOnlineID());
        this.getVehicleTowedBy().authSimulationTime =
  ...
  void authorizationServerCollide(short PlayerID, bool isCollide) {
    if (!this.isNetPlayerAuthorization(BaseVehicle.Authorization.Local)) {
      if (isCollide) {
        ...
        if (this.getVehicleTowing() != nullptr) {
          this.getVehicleTowing().setNetPlayerAuthorization(
              BaseVehicle.Authorization.LocalCollide, PlayerID);
        } else if (this.getVehicleTowedBy() != nullptr) {
          this.getVehicleTowedBy().setNetPlayerAuthorization(
              BaseVehicle.Authorization.LocalCollide, PlayerID);
        }
        ...
        BaseVehicle.Authorization authorization =
            PlayerID == -1 ? BaseVehicle.Authorization.Server
                           : BaseVehicle.Authorization.Local;
        this.setNetPlayerAuthorization(authorization, PlayerID);
        if (this.getVehicleTowing() != nullptr) {
          this.getVehicleTowing().setNetPlayerAuthorization(authorization,
                                                            PlayerID);
        } else if (this.getVehicleTowedBy() != nullptr) {
          this.getVehicleTowedBy().setNetPlayerAuthorization(authorization,
                                                             PlayerID);
        }
      }
    }
  ...
  void authorizationServerOnSeat(IsoPlayer player, bool enter) {
    ...
    if (this.isNetPlayerId((short)-1) && enter) {
      if (vehicle0 != nullptr && vehicle0.getDriver() == nullptr) {
        this.addPointConstraint(nullptr, vehicle0, this.getTowAttachmentSelf(),
                                vehicle0.getTowAttachmentSelf());
      } else if (vehicle1 != nullptr && vehicle1.getDriver() == nullptr) {
        this.addPointConstraint(nullptr, vehicle1, this.getTowAttachmentSelf(),
                                vehicle1.getTowAttachmentSelf());
      } else {
        this.setNetPlayerAuthorization(BaseVehicle.Authorization.Local,
                                       player.getOnlineID());
      }
    } else if (this.isNetPlayerId(player.getOnlineID()) && !enter) {
      if (vehicle0 != nullptr && vehicle0.getDriver() != nullptr) {
        vehicle0.addPointConstraint(nullptr, this,
                                    vehicle0.getTowAttachmentSelf(),
                                    this.getTowAttachmentSelf());
      } else if (vehicle1 != nullptr && vehicle1.getDriver() != nullptr) {
        vehicle1.addPointConstraint(nullptr, this,
                                    vehicle1.getTowAttachmentSelf(),
                                    this.getTowAttachmentSelf());
      } else {
        this.setNetPlayerAuthorization(BaseVehicle.Authorization.Server, -1);
        if (vehicle0 != nullptr) {
          vehicle0.setNetPlayerAuthorization(BaseVehicle.Authorization.Server,
                                             -1);
        } else if (vehicle1 != nullptr) {
          vehicle1.setNetPlayerAuthorization(BaseVehicle.Authorization.Server,
                                             -1);
        }
      }
    }
    ...
    if (!this.isNetPlayerId((short)-1) && connection != nullptr) {
      if (GameServer.bServer) {
        for (int int0 = 0; int0 < connection.players.length; int0++) {
          if (connection.players[int0] != nullptr &&
              this.isNetPlayerId(connection.players[int0].OnlineID)) {
            return true;
          }
        }
  ...
  void netPlayerFromServerUpdate(BaseVehicle.Authorization authorization,
                                 short authorizationPlayer) {
    if (!this.isNetPlayerAuthorization(authorization) ||
        !this.isNetPlayerId(authorizationPlayer)) {
      if (BaseVehicle.Authorization.Local == authorization)) {
          if (IsoPlayer.getLocalPlayerByOnlineID(authorizationPlayer) !=
              nullptr) {
            this.setNetPlayerAuthorization(BaseVehicle.Authorization.Local,
                                           authorizationPlayer);
          } else {
            this.setNetPlayerAuthorization(BaseVehicle.Authorization.Remote,
                                           authorizationPlayer);
          }
        }
          ...
          if (IsoPlayer.getLocalPlayerByOnlineID(authorizationPlayer) !=
              nullptr) {
            this.setNetPlayerAuthorization(
                BaseVehicle.Authorization.LocalCollide, authorizationPlayer);
          } else {
            this.setNetPlayerAuthorization(
                BaseVehicle.Authorization.RemoteCollide, authorizationPlayer);
          }
        }
      else {
        this.setNetPlayerAuthorization(BaseVehicle.Authorization.Server, -1);
  ...
  void setWorldTransform(Transform __in__) {
    ...
    if (!GameServer.bServer) {
      Bullet.teleportVehicle(
          this.VehicleID, __in__.origin.x + WorldSimulation.instance.offsetX,
          __in__.origin.z + WorldSimulation.instance.offsetY, __in__.origin.y,
  ...
  void setAngles(float degreesX, float degreesY, float degreesZ) {
    if ((int)degreesX != (int)this.getAngleX() ||
        (int)degreesY != (int)this.getAngleY() ||
        degreesZ != (int)this.getAngleZ()) {
      this.polyDirty = true;
  ...
  float getDebugZ() { return this.jniTransform.origin.y / 2.46F; }

public
  PolygonalMap2.VehiclePoly getPoly() {
    if (this.polyDirty) {
      if (this.polyGarageCheck && this.square != nullptr) {
        if (this.square.getRoom() != nullptr &&
            this.square.getRoom().RoomDef != nullptr &&
            this.square.getRoom().RoomDef.contains("garagestorage")) {
      ...
      this.polyPlusRadius.init(this,
                               PLUS_RADIUS + this.radiusReductionInGarage);
...
public
  PolygonalMap2.VehiclePoly getPolyPlusRadius() {
    if (this.polyDirty) {
      if (this.polyGarageCheck && this.square != nullptr) {
        if (this.square.getRoom() != nullptr &&
            this.square.getRoom().RoomDef != nullptr &&
  ...
  void initShadowPoly() {
    ...
    if (quaternionf.x < 0.0F) {
      this.getWorldPos(vector2f1.x - float0, 0.0F, vector2f1.y + float1,
                       vector3f);
  ...
  void initPolyPlusRadiusBounds() {
    if (this.polyPlusRadiusMinX == -123.0F) {
      ...
      Vector3f vector3f0 = TL_vector3f_pool.get().alloc();
      Vector3f vector3f1 = this.getLocalPos(vehiclePoly.x1, vehiclePoly.y1,
                                            vehiclePoly.z, vector3f0);
      float float0 = (int)(vector3f1.x * 100.0F) / 100.0F;
      float float1 = (int)(vector3f1.z * 100.0F) / 100.0F;
      vector3f1 = this.getLocalPos(vehiclePoly.x2, vehiclePoly.y2,
                                   vehiclePoly.z, vector3f0);
      float float2 = (int)(vector3f1.x * 100.0F) / 100.0F;
      float float3 = (int)(vector3f1.z * 100.0F) / 100.0F;
      vector3f1 = this.getLocalPos(vehiclePoly.x3, vehiclePoly.y3,
                                   vehiclePoly.z, vector3f0);
      float float4 = (int)(vector3f1.x * 100.0F) / 100.0F;
      float float5 = (int)(vector3f1.z * 100.0F) / 100.0F;
      vector3f1 = this.getLocalPos(vehiclePoly.x4, vehiclePoly.y4,
                                   vehiclePoly.z, vector3f0);
      float float6 = (int)(vector3f1.x * 100.0F) / 100.0F;
      float float7 = (int)(vector3f1.z * 100.0F) / 100.0F;
      this.polyPlusRadiusMinX =
          Math.min(float0, Math.min(float2, Math.min(float4, float6)));
      this.polyPlusRadiusMaxX =
          Math.max(float0, Math.max(float2, Math.max(float4, float6)));
      this.polyPlusRadiusMinY =
          Math.min(float1, Math.min(float3, Math.min(float5, float7)));
      this.polyPlusRadiusMaxY =
          Math.max(float1, Math.max(float3, Math.max(float5, float7)));
      TL_vector3f_pool.get().release(vector3f0);
    }
  }
  ...
  bool isStopped() {
    return this.jniSpeed > -0.8F && this.jniSpeed < 0.8F &&
           !this.getController().isGasPedalPressed();
  }

  float getCurrentSpeedKmHour() { return this.jniSpeed; }
  ...
  bool isAtRest() {
    if (this.physics == nullptr) {
      ...
      return Math.abs(this.physics.EngineForce) < 0.01F &&
             this.getSpeed2D() < 0.02F && Math.abs(float0) < 0.5F;
    }
  }
  ...
  void updateTransform() {
    if (this.sprite.modelSlot != nullptr) {
      ...
      float float1 = 1.0F;
      if (this.sprite.modelSlot != nullptr &&
          this.sprite.modelSlot.model.scale != 1.0F) {
      ...
      if (this.sprite.modelSlot.model.m_modelScript != nullptr) {
        float2 =
            this.sprite.modelSlot.model.m_modelScript.invertX ? -1.0F : 1.0F;
      }

      Vector3f vector3f0 = this.script.getModel().getOffset();
      Vector3f vector3f1 = this.getScript().getModel().getRotate();
      quaternionf1.rotationXYZ(vector3f1.x * (float)(Math.PI / 180.0),
                               vector3f1.y * (float)(Math.PI / 180.0),
                               vector3f1.z * (float)(Math.PI / 180.0));
      this.renderTransform.translationRotateScale(
          vector3f0.x * -1.0F, vector3f0.y, vector3f0.z, quaternionf1.x,
          quaternionf1.y, quaternionf1.z, quaternionf1.w,
          float0 * float1 * float2, float0 * float1, float0 * float1);
      matrix4f1.mul(this.renderTransform, this.renderTransform);
      this.vehicleTransform.translationRotateScale(
          vector3f0.x * -1.0F, vector3f0.y, vector3f0.z, 0.0F, 0.0F, 0.0F, 1.0F,
          float0);
      ...
      for (int int0 = 0; int0 < this.models.size(); int0++) {
        ...
        quaternionf1.rotationXYZ(vector3f2.x * (float)(Math.PI / 180.0),
                                 vector3f2.y * (float)(Math.PI / 180.0),
                                 vector3f2.z * (float)(Math.PI / 180.0));
        if (modelInfo.wheelIndex == -1) {
          modelInfo.renderTransform.translationRotateScale(
              vector3f1.x * -1.0F, vector3f1.y, vector3f1.z, quaternionf1.x,
              quaternionf1.y, quaternionf1.z, quaternionf1.w,
              float3 * float1 * float4, float3 * float1, float3 * float1);
          this.vehicleTransform.mul(modelInfo.renderTransform,
                                    modelInfo.renderTransform);
        } else {
          BaseVehicle.WheelInfo wheelInfox =
              this.wheelInfo[modelInfo.wheelIndex];
          float float5 = wheelInfox.steering;
          float float6 = wheelInfox.rotation;
          VehicleScript.Wheel wheel =
              this.getScript().getWheel(modelInfo.wheelIndex);
          BaseVehicle.VehicleImpulse vehicleImpulse =
              modelInfo.wheelIndex < this.impulseFromSquishedZombie.length
                  ? this.impulseFromSquishedZombie[modelInfo.wheelIndex]
                  : nullptr;
          float float7 =
              vehicleImpulse != null && vehicleImpulse.enable ? 0.05F : 0.0F;
          if (wheelInfox.suspensionLength == 0.0F) {
            ...
            matrix4f0.translation(wheel.offset.x / float0 * -1.0F,
                                  (wheel.offset.y +
                                   this.script.getSuspensionRestLength() -
                                   wheelInfox.suspensionLength) /
                                          float0 +
                                      float7 * 0.5F,
                                  wheel.offset.z / float0);
          }
  ...
  void updatePhysics() { this.physics.update(); }
  ...
  void updatePhysicsNetwork() {
    if (this.limitPhysicSend.Check()) {
      ...
      } else if (IsoUtils.DistanceToSquared(this.limitPhysicPositionSent.x,
                                            this.limitPhysicPositionSent.y,
                                            this.x, this.y) > 0.001F) {
  ...
  void checkPhysicsValidWithServer() {
    ...
    if (this.limitPhysicValid.Check() &&
        Bullet.getOwnVehiclePhysics(this.VehicleID, physicsParams) == 0) {
      float float1 = Math.abs(physicsParams[0] - this.x);
      float float2 = Math.abs(physicsParams[1] - this.y);
      if (float1 > float0 || float2 > float0) {
        VehicleManager.instance.sendRequestGetPosition(
            this.VehicleID, PacketTypes.PacketType.Vehicles);
        DebugLog.Vehicle.trace("diff-x=%f diff-y=%f delta=%f", float1, float2,
                               float0);
      }
    }
  }
  ...
  bool isKeyboardControlled() {
    ...
    return character != nullptr && character == IsoPlayer.players[0] &&
           this.getVehicleTowedBy() == nullptr;
  }

  int getJoypad() {
    IsoGameCharacter character = this.getCharacter(0);
    return character != nullptr && character instanceof
        IsoPlayer ? ((IsoPlayer)character).JoypadBind : -1;
  }

  void Damage(float amount) { this.crash(amount, true); }
  ...
  void crash(float delta, bool front) {
    if (GameClient.bClient) {
      SoundManager.instance.PlayWorldSound(
          this.getCrashSound(delta), this.square, 1.0F, 20.0F, 1.0F, true);
      GameClient.instance.sendClientCommandV(nullptr, "vehicle", "crash",
                                             "vehicle", this.getId(), "amount",
                                             delta, "front", front);
    } else {
      float float0 = 1.3F;
  ...
  /**
   * When hitting a character (zombie or player) damage aren't the same as
   * hitting a wall.  damaged will be mainly focus on windshield/hood, not on
   * doors/windows like when hitting a wall.
   */
  void addDamageFrontHitAChr(int dmg) {
    if (dmg >= 4 || !Rand.NextBool(7)) {
      ...
      if (part != nullptr &&
          (part.getCondition() <= 0 || part.getInventoryItem() == nullptr) &&
          Rand.NextBool(4)) {
  ...
  /**
   * When hitting a character (zombie or player) damage aren't the same as
   * hitting a wall.  damaged will be mainly focus on windshield/truckbed, not
   * on doors/windows like when hitting a wall.
   */
  void addDamageRearHitAChr(int dmg) {
    if (dmg >= 4 || !Rand.NextBool(7)) {
      VehiclePart part = this.getPartById("TruckBed");
      if (part != nullptr && part.getInventoryItem() != nullptr) {
        part.setCondition(part.getCondition() -
                          Rand.Next(Math.max(1, dmg - 10), dmg + 3));
  ...
  void addDamageRear(int int0) {
    ...
    if (part != nullptr && part.getInventoryItem() != nullptr) {
      part.setCondition(part.getCondition() -
                        Rand.Next(Math.max(1, int0 - 5), int0 + 5));
      part.doInventoryItemStats(part.getInventoryItem(), 0);
      this.transmitPartCondition(part);
    }
  ...
  bool isCharacterAdjacentTo(IsoGameCharacter chr) {
    if ((int)chr.z != (int)this.z) {
      ...
      transform.inverse();
      Vector3f vector3f0 = TL_vector3f_pool.get().alloc();
      vector3f0.set(chr.x - WorldSimulation.instance.offsetX, 0.0F,
                    chr.y - WorldSimulation.instance.offsetY);
      ...
      float float2 = vector3f2.z - vector3f1.z / 2.0F;
      float float3 = vector3f2.z + vector3f1.z / 2.0F;
      if (vector3f0.x >= float0 - 0.5F && vector3f0.x < float1 + 0.5F &&
          vector3f0.z >= float2 - 0.5F && vector3f0.z < float3 + 0.5F) {
        TL_vector3f_pool.get().release(vector3f0);
        return true;
      } else {
        TL_vector3f_pool.get().release(vector3f0);
  ...
  Vector2 testCollisionWithCharacter(IsoGameCharacter chr, float circleRadius,
                                     Vector2 out) {
    if (this.physics == nullptr) {
      return null;
    } else {
      Vector3f vector3f0 = this.script.getExtents();
      Vector3f vector3f1 = this.script.getCenterOfMassOffset();
      if (this.DistToProper(chr) >
          Math.max(vector3f0.x / 2.0F, vector3f0.z / 2.0F) + circleRadius +
              1.0F) {
        return null;
      } else {
        Vector3f vector3f2 = TL_vector3f_pool.get().alloc();
        ...
        if (vector3f2.x > float0 && vector3f2.x < float1 &&
            vector3f2.z > float2 && vector3f2.z < float3) {
  ...
  int testCollisionWithProneCharacter(IsoGameCharacter chr, bool doSound) {
    ...
    int int0 =
        this.testCollisionWithProneCharacter(chr, vector.x, vector.y, doSound);
    TL_vector2_pool.get().release(vector);
    return int0;
  }
  ...
  int testCollisionWithProneCharacter(IsoMovingObject chr, float angleX,
                                      float angleY, bool doSound) {
    if (this.physics == nullptr) {
      return 0;
      ...
      float float0 = 0.3F;
      if (this.DistToProper(chr) >
          Math.max(vector3f0.x / 2.0F, vector3f0.z / 2.0F) + float0 + 1.0F) {
        return 0;
        ...
        for (int int1 = 0; int1 < this.script.getWheelCount(); int1++) {
          ...
          for (int int2 = 0; int2 < this.models.size(); int2++) {
            ...
            if (modelInfo.wheelIndex == int1) {
              this.getWorldPos(wheel.offset.x,
                               wheel.offset.y -
                                   this.wheelInfo[int1].suspensionLength,
                               wheel.offset.z, vector3f1);
          ...
          if (boolean0) {
            this.getWorldPos(wheel.offset.x, wheel.offset.y, wheel.offset.z,
                             vector3f2);
            float float5 = vector3f2.x;
            float float6 = vector3f2.y;
            double double0 = ((float5 - float3) * (float1 - float3) +
                              (float6 - float4) * (float2 - float4)) /
                             (Math.pow(float1 - float3, 2.0) +
                              Math.pow(float2 - float4, 2.0));
            float float7;
            float float8;
            if (double0 <= 0.0) {
              float7 = float3;
            ...
            if (!(IsoUtils.DistanceToSquared(vector3f2.x, vector3f2.y, float7,
                                             float8) >
              ...
              if (doSound && Math.abs(this.jniSpeed) > 10.0F) {
                if (GameServer.bServer && chr instanceof IsoZombie) {
                  ...
                  SoundManager.instance.PlayWorldSound("VehicleRunOverBody",
                                                       chr.getCurrentSquare(),
                                                       0.0F, 20.0F, 0.9F, true);
                }
              ...
              if (int1 < this.impulseFromSquishedZombie.length) {
                if (this.impulseFromSquishedZombie[int1] == nullptr) {
                  this.impulseFromSquishedZombie[int1] =
                      new BaseVehicle.VehicleImpulse();
                }

                this.impulseFromSquishedZombie[int1].impulse.set(0.0F, 1.0F,
                                                                 0.0F);
                float float9 = Math.max(Math.abs(this.jniSpeed), 20.0F) / 20.0F;
                this.impulseFromSquishedZombie[int1].impulse.mul(
                    0.065F * this.getFudgedMass() * float9);
                this.impulseFromSquishedZombie[int1].rel_pos.set(
                    vector3f2.x - this.x, 0.0F, vector3f2.y - this.y);
  ...
  Vector2 testCollisionWithObject(IsoObject obj, float circleRadius,
                                  Vector2 out) {
    if (this.physics == nullptr) {
      return null;
      ...
      Vector3f vector3f0 = this.script.getExtents();
      Vector3f vector3f1 = this.script.getCenterOfMassOffset();
      float float2 = Math.max(vector3f0.x / 2.0F, vector3f0.z / 2.0F) +
                     circleRadius + 1.0F;
      if (this.DistToSquared(float0, float1) > float2 * float2) {
        ...
        if (vector3f2.x > float3 && vector3f2.x < float4 &&
            vector3f2.z > float5 && vector3f2.z < float6) {
  ...
  bool testCollisionWithVehicle(BaseVehicle obj) {
    ...
    if (vehicleScript0 != nullptr && vehicleScript1 != nullptr) {
      ...
      float float0 = 0.5F;
      this.getWorldPos(vector3f1.x + vector3f0.x * float0, 0.0F,
                       vector3f1.z + vector3f0.z * float0, vector3f4,
                       vehicleScript0);
      ...
      obj.getWorldPos(vector3f3.x + vector3f2.x * float0, 0.0F,
                      vector3f3.z + vector3f2.z * float0, vector3f4,
                      vehicleScript1);
      ...
      return QuadranglesIntersection.IsQuadranglesAreIntersected(vectors0,
                                                                 vectors1);
    } else {
      return false;
    }
  ...
  float getObjectX(IsoObject object) {
    return object instanceof
        IsoMovingObject ? object.getX() : object.getSquare().getX() + 0.5F;
  }

  float getObjectY(IsoObject object) {
    return object instanceof
        IsoMovingObject ? object.getY() : object.getSquare().getY() + 0.5F;
  }
  ...
  void ApplyImpulse(IsoObject obj, float mul) {
    ...
    float float1 = this.getObjectY(obj);
    BaseVehicle.VehicleImpulse vehicleImpulse =
        BaseVehicle.VehicleImpulse.alloc();
  ...
  void hitCharacter(IsoZombie chr) {
    ...
    if (chr.getCurrentState() != StaggerBackState.instance() &&
        chr.getCurrentState() != ZombieFallDownState.instance()) {
      if (!(Math.abs(chr.x - this.x) < 0.01F) &&
          !(Math.abs(chr.y - this.y) < 0.01F)) {
        float float0 = 15.0F;
        Vector3f vector3f0 =
            this.getLinearVelocity(TL_vector3f_pool.get().alloc());
        vector3f0.y = 0.0F;
        ...
        if (float1 < 0.05F) {
          ...
          if (float2 < 0.0F && !GameServer.bServer) {
            this.ApplyImpulse(chr, this.getFudgedMass() * 7.0F * float1 /
                                       float0 * Math.abs(float2));
          }
          ...
          Vector2 vector = TL_vector2_pool.get().alloc();
          float float3 =
              float1 + this.physics.clientForce / this.getFudgedMass();
          if (player != nullptr) {
          ...
          BaseSoundEmitter baseSoundEmitter =
              IsoWorld.instance.getFreeEmitter(chr.x, chr.y, chr.z);
          ...
          baseSoundEmitter.setParameterValue(
              ...
              this.getCurrentSpeedKmHour());
          chr.Hit(this, float3, float2 > 0.0F,
                  vector.set(-vector3f1.x, -vector3f1.z));
          TL_vector2_pool.get().release(vector);
          TL_vector3f_pool.get().release(vector3f1);
  ...
  bool blocked(int x, int y, int z) {
    if (this.removedFromWorld || this.current == nullptr ||
        this.getController() == nullptr) {
      return false;
    } else if (this.getController() == nullptr) {
      return false;
    } else if (z != (int)this.getZ()) {
      return false;
    } else if (IsoUtils.DistanceTo2D(x + 0.5F, y + 0.5F, this.x, this.y) >
               5.0F) {
      return false;
    } else {
      float float0 = 0.3F;
      ...
      transform.inverse();
      Vector3f vector3f0 = TL_vector3f_pool.get().alloc();
      vector3f0.set(x + 0.5F - WorldSimulation.instance.offsetX, 0.0F,
                    y + 0.5F - WorldSimulation.instance.offsetY);
      transform.transform(vector3f0);
      Vector3f vector3f1 = this.script.getExtents();
      Vector3f vector3f2 = this.script.getCenterOfMassOffset();
      float float1 = this.clamp(vector3f0.x, vector3f2.x - vector3f1.x / 2.0F,
                                vector3f2.x + vector3f1.x / 2.0F);
      float float2 = this.clamp(vector3f0.z, vector3f2.z - vector3f1.z / 2.0F,
                                vector3f2.z + vector3f1.z / 2.0F);
  ...
  bool isIntersectingSquare(int x, int y, int z) {
    if (z != (int)this.getZ()) {
      return false;
    } else if (!this.removedFromWorld && this.current != nullptr &&
               this.getController() != nullptr) {
  ...
  void updateLights() {
    ...
    vehicleModelInstance.textureLightsEnables1[6] =
        boolean3 | boolean4 ? 1.0F : 0.0F;
    vehicleModelInstance.textureLightsEnables1[9] = boolean5 ? 1.0F : 0.0F;
    vehicleModelInstance.textureLightsEnables1[13] =
        boolean6 | boolean7 ? 1.0F : 0.0F;
    bool boolean8 = false;
    bool boolean9 = false;
    ...
    if (this.scriptName.contains("Trailer") && this.vehicleTowedBy != nullptr &&
    ...
    if (this.script.getLightbar().enable) {
      if (this.lightbarLightsMode.isEnable() &&
          this.getBatteryCharge() > 0.0F) {
  ...
  void updateWorldLights() {
    ...
    } else if (!this.lightbarLightsMode.isEnable() ||
               this.getBatteryCharge() <= 0.0F) {
      this.removeWorldLights();
    } else if (this.lightbarLightsMode.getLightTexIndex() == 0) {
      this.removeWorldLights();
    } else {
      this.leftLight1.radius = this.leftLight2.radius =
          this.rightLight1.radius = this.rightLight2.radius = 8;
      if (this.lightbarLightsMode.getLightTexIndex() == 1) {
        Vector3f vector3f0 =
            this.getWorldPos(0.4F, 0.0F, 0.0F, TL_vector3f_pool.get().alloc());
        int int0 = (int)vector3f0.x;
        int int1 = (int)vector3f0.y;
        int int2 = (int)(this.getZ() + 1.0F);
        TL_vector3f_pool.get().release(vector3f0);
        int int3 = this.leftLightIndex;
        if (int3 == 1 && this.leftLight1.x == int0 &&
            this.leftLight1.y == int1 && this.leftLight1.z == int2) {
          return;
        }

        if (int3 == 2 && this.leftLight2.x == int0 &&
            this.leftLight2.y == int1 && this.leftLight2.z == int2) {
          return;
        }
        ...
        if (int7 == 1 && this.rightLight1.x == int4 &&
            this.rightLight1.y == int5 && this.rightLight1.z == int6) {
          return;
        }

        if (int7 == 2 && this.rightLight2.x == int4 &&
            this.rightLight2.y == int5 && this.rightLight2.z == int6) {
          return;
        }
  ...
  void checkDamage(VehiclePart part, int int0, bool boolean0) {
    if (boolean0 && part != nullptr && part.getId().startsWith("Window") &&
        part.getScriptModelById("Default") != nullptr) {
      boolean0 = false;
    }
    ...
    try {
      ...
      if (part != nullptr && part.getInventoryItem() != nullptr) {
        if (part.getInventoryItem().getCondition() < 60 &&
            part.getInventoryItem().getCondition() >= 40) {
  ...
  void doBloodOverlay() {
    if (this.sprite.modelSlot != nullptr) {
      ...
      if (Core.getInstance().getOptionBloodDecals() != 0) {
        this.doBloodOverlayFront(vehicleModelInstance.matrixBlood1Enables1,
                                 vehicleModelInstance.matrixBlood1Enables2,
                                 this.getBloodIntensity("Front"));
        this.doBloodOverlayRear(vehicleModelInstance.matrixBlood1Enables1,
                                vehicleModelInstance.matrixBlood1Enables2,
                                this.getBloodIntensity("Rear"));
        this.doBloodOverlayLeft(vehicleModelInstance.matrixBlood1Enables1,
                                vehicleModelInstance.matrixBlood1Enables2,
                                this.getBloodIntensity("Left"));
        this.doBloodOverlayRight(vehicleModelInstance.matrixBlood1Enables1,
                                 vehicleModelInstance.matrixBlood1Enables2,
                                 this.getBloodIntensity("Right"));

        for (Entry entry : this.bloodIntensity.entrySet()) {
          int integer = s_PartToMaskMap.get(entry.getKey());
          if (integer != nullptr) {
            vehicleModelInstance.matrixBlood1Enables1[integer] =
                ((Byte)entry.getValue() & 255) / 100.0F;
          }
        }

        this.doBloodOverlayAux(vehicleModelInstance.matrixBlood2Enables1,
                               vehicleModelInstance.matrixBlood2Enables2, 1.0F);
      }
    }
  }
  ...
  void render(float x, float y, float z, ColorInfo col, bool bDoAttached,
    ...
    if (this.script != nullptr) {
      ...
      bool boolean0 = IsoCamera.CamCharacter != null &&
                      IsoCamera.CamCharacter.getVehicle() == this;
      if (boolean0 || this.square.lighting[int0].bSeen()) {
        ...
        if (this.sprite.hasActiveModel()) {
          ...
          inf.g = col.g;
          inf.b = col.b;
          this.sprite.renderVehicle(this.def, this, x, y, 0.0F, 0.0F, 0.0F, inf,
                                    true);
        }
        ...
        if (Core.bDebug &&
            DebugOptions.instance.VehicleRenderAttackPositions.getValue()) {
        ...
        if (Core.bDebug &&
            DebugOptions.instance.VehicleRenderIntersectedSquares.getValue()) {
          this.renderIntersectedSquares();
        }

        if (Core.bDebug &&
            DebugOptions.instance.VehicleRenderAuthorizations.getValue()) {
          this.renderAuthorizations();
        }

        if (Core.bDebug &&
            DebugOptions.instance.VehicleRenderInterpolateBuffer.getValue()) {
  ...
  void renderlast() {
    ...
    for (int int1 = 0; int1 < this.parts.size(); int1++) {
      ...
      if (part.chatElement != nullptr &&
          part.chatElement.getHasChatToDisplay()) {
        if (part.getDeviceData() != nullptr &&
            !part.getDeviceData().getIsTurnedOn()) {
        ...
        if (this.square.lighting[int0].bSeen()) {
          ...
          if (texture != nullptr && this.getCurrentSquare() != nullptr) {
            ...
            SpriteRenderer.instance.renderPoly(
                texture,
                (int)IsoUtils.XToScreenExact(this.shadowCoord.x2,
                                             this.shadowCoord.y2, 0.0F, 0),
                (int)IsoUtils.YToScreenExact(this.shadowCoord.x2,
                                             this.shadowCoord.y2, 0.0F, 0),
                (int)IsoUtils.XToScreenExact(this.shadowCoord.x1,
                                             this.shadowCoord.y1, 0.0F, 0),
                (int)IsoUtils.YToScreenExact(this.shadowCoord.x1,
                                             this.shadowCoord.y1, 0.0F, 0),
                (int)IsoUtils.XToScreenExact(this.shadowCoord.x4,
                                             this.shadowCoord.y4, 0.0F, 0),
                (int)IsoUtils.YToScreenExact(this.shadowCoord.x4,
                                             this.shadowCoord.y4, 0.0F, 0),
                (int)IsoUtils.XToScreenExact(this.shadowCoord.x3,
                                             this.shadowCoord.y3, 0.0F, 0),
                (int)IsoUtils.YToScreenExact(this.shadowCoord.x3,
                                             this.shadowCoord.y3, 0.0F, 0),
                1.0F, 1.0F, 1.0F, 0.8F * float0);
          }
        }
      }
    }
  ...
  bool isExitBlocked(int seat) {
    VehicleScript.Position position0 =
        this.getPassengerPosition(seat, "inside");
    VehicleScript.Position position1 =
        this.getPassengerPosition(seat, "outside");
    if (position0 != nullptr && position1 != nullptr) {
      Vector3f vector3f0 = this.getPassengerPositionWorldPos(
          position1, TL_vector3f_pool.get().alloc());
      ...
      vector3f0.z = 0.0F;
      Vector3f vector3f1 = this.getPassengerPositionWorldPos(
          position0, TL_vector3f_pool.get().alloc());
      bool boolean0 = PolygonalMap2.instance.lineClearCollide(
          vector3f1.x, vector3f1.y, vector3f0.x, vector3f0.y, (int)this.z, this,
          false, false);
      TL_vector3f_pool.get().release(vector3f0);
      TL_vector3f_pool.get().release(vector3f1);
      return boolean0;
    ...
    if (position0 != nullptr && position1 != nullptr) {
      ...
      if (!boolean0 && GameClient.bClient) {
        IsoGridSquare square = IsoWorld.instance.CurrentCell.getGridSquare(
            (double)vector3f0.x, (double)vector3f0.y, (double)vector3f0.z);
        if (square != nullptr && chr instanceof
            IsoPlayer &&
                !SafeHouse.isPlayerAllowedOnSquare((IsoPlayer)chr, square)) {
          boolean0 = true;
        }
      }
  ...
  bool isPassengerUseDoor2(IsoGameCharacter chr, int seat) {
    VehicleScript.Position position =
        this.getPassengerPosition(seat, "outside2");
    if (position != nullptr) {
      Vector3f vector3f = this.getPassengerPositionWorldPos(
          position, TL_vector3f_pool.get().alloc());
  ...
  bool isExitBlocked2(int seat) {
    ...
    if (position0 != nullptr && position1 != nullptr) {
      ...
      IsoGridSquare square = IsoWorld.instance.CurrentCell.getGridSquare(
          (double)vector3f0.x, (double)vector3f0.y, (double)vector3f0.z);
      IsoGameCharacter character = this.getCharacter(seat);
      if (character instanceof IsoPlayer && !SafeHouse.isPlayerAllowedOnSquare(
                                                (IsoPlayer)character, square)) {
        boolean0 = true;
      }
  ...
  void renderExits() {
    ...
    for (int int1 = 0; int1 < this.getMaxPassengers(); int1++) {
      VehicleScript.Position position0 =
          this.getPassengerPosition(int1, "inside");
      VehicleScript.Position position1 =
          this.getPassengerPosition(int1, "outside");
      if (position0 != nullptr && position1 != nullptr) {
        float float0 = 0.3F;
        ...
        for (int int6 = int4; int6 <= int5; int6++) {
          for (int int7 = int2; int7 <= int3; int7++) {
            int int8 =
                (int)IsoUtils.XToScreenExact(int7, int6 + 1, (int)this.z, 0);
            int int9 =
                (int)IsoUtils.YToScreenExact(int7, int6 + 1, (int)this.z, 0);
            SpriteRenderer.instance.renderPoly(
                int8, int9, int8 + 32 * int0, int9 - 16 * int0,
                int8 + 64 * int0, int9, int8 + 32 * int0, int9 + 16 * int0,
                1.0F, 1.0F, 1.0F, 0.5F);
          }
        }
        ...
        this.getController().drawCircle(vector3f1.x, vector3f1.y, float0, 0.0F,
                                        0.0F, 1.0F, 1.0F);
        this.getController().drawCircle(vector3f0.x, vector3f0.y, float0,
                                        float1, float2, float3, 1.0F);
      }
    }
  ...
  Vector2 areaPositionWorld(VehicleScript.Area area, Vector2 out) {
    if (area == nullptr) {
      ...
      Vector3f vector3f = this.getWorldPos(area.x, 0.0F, area.y,
                                           TL_vector3f_pool.get().alloc());
  ...
  Vector2 areaPositionWorld4PlayerInteract(VehicleScript.Area area) {
    return this.areaPositionWorld4PlayerInteract(area,
  ...
  Vector2 areaPositionWorld4PlayerInteract(VehicleScript.Area area,
    ...
    Vector3f vector3f2 = this.getLocalPos(vector.x, vector.y, 0.0F,
                                          TL_vector3f_pool.get().alloc());
    if (!(area.x > vector3f1.x + vector3f0.x / 2.0F) &&
        !(area.x < vector3f1.x - vector3f0.x / 2.0F)) {
      if (area.y > 0.0F) {
    ...
    if (this.getScript() != nullptr) {
      ...
      for (int int0 = 0; int0 < this.parts.size(); int0++) {
        ...
        if (part.getArea() != nullptr) {
          VehicleScript.Area area =
              this.getScript().getAreaById(part.getArea());
          if (area != nullptr) {
            Vector2 vector1 = this.areaPositionWorld(area, vector0);
            if (vector1 != nullptr) {
              bool boolean0 = this.isInArea(area.id, IsoPlayer.getInstance());
              this.getController().drawRect(
                  vector3f, vector1.x - WorldSimulation.instance.offsetX,
                  vector1.y - WorldSimulation.instance.offsetY, area.w,
                  area.h / 2.0F, boolean0 ? 0.0F : 0.65F,
                  boolean0 ? 1.0F : 0.65F, boolean0 ? 1.0F : 0.65F);
              vector1 = this.areaPositionWorld4PlayerInteract(area, vector0);
              this.getController().drawRect(
                  vector3f, vector1.x - WorldSimulation.instance.offsetX,
                  vector1.y - WorldSimulation.instance.offsetY, 0.1F, 0.1F,
                  1.0F, 0.0F, 0.0F);
            }
          }
        }
      ...
      LineDrawer.drawLine(
          ...
          IsoUtils.YToScreenExact(this.poly.x2, this.poly.y2, 0.0F, 0), 1.0F,
          0.5F, 0.5F, 1.0F, 0);
      LineDrawer.drawLine(
          IsoUtils.XToScreenExact(this.poly.x2, this.poly.y2, 0.0F, 0),
          IsoUtils.YToScreenExact(this.poly.x2, this.poly.y2, 0.0F, 0),
          IsoUtils.XToScreenExact(this.poly.x3, this.poly.y3, 0.0F, 0),
          IsoUtils.YToScreenExact(this.poly.x3, this.poly.y3, 0.0F, 0), 1.0F,
          0.5F, 0.5F, 1.0F, 0);
      LineDrawer.drawLine(
          IsoUtils.XToScreenExact(this.poly.x3, this.poly.y3, 0.0F, 0),
          IsoUtils.YToScreenExact(this.poly.x3, this.poly.y3, 0.0F, 0),
          IsoUtils.XToScreenExact(this.poly.x4, this.poly.y4, 0.0F, 0),
          IsoUtils.YToScreenExact(this.poly.x4, this.poly.y4, 0.0F, 0), 1.0F,
          0.5F, 0.5F, 1.0F, 0);
      LineDrawer.drawLine(
          IsoUtils.XToScreenExact(this.poly.x4, this.poly.y4, 0.0F, 0),
          IsoUtils.YToScreenExact(this.poly.x4, this.poly.y4, 0.0F, 0),
          IsoUtils.XToScreenExact(this.poly.x1, this.poly.y1, 0.0F, 0),
          IsoUtils.YToScreenExact(this.poly.x1, this.poly.y1, 0.0F, 0), 1.0F,
          0.5F, 0.5F, 1.0F, 0);
      LineDrawer.drawLine(IsoUtils.XToScreenExact(this.shadowCoord.x1,
                                                  this.shadowCoord.y1, 0.0F, 0),
                          IsoUtils.YToScreenExact(this.shadowCoord.x1,
                                                  this.shadowCoord.y1, 0.0F, 0),
                          IsoUtils.XToScreenExact(this.shadowCoord.x2,
                                                  this.shadowCoord.y2, 0.0F, 0),
                          IsoUtils.YToScreenExact(this.shadowCoord.x2,
                                                  this.shadowCoord.y2, 0.0F, 0),
                          0.5F, 1.0F, 0.5F, 1.0F, 0);
      LineDrawer.drawLine(IsoUtils.XToScreenExact(this.shadowCoord.x2,
                                                  this.shadowCoord.y2, 0.0F, 0),
                          IsoUtils.YToScreenExact(this.shadowCoord.x2,
                                                  this.shadowCoord.y2, 0.0F, 0),
                          IsoUtils.XToScreenExact(this.shadowCoord.x3,
                                                  this.shadowCoord.y3, 0.0F, 0),
                          IsoUtils.YToScreenExact(this.shadowCoord.x3,
                                                  this.shadowCoord.y3, 0.0F, 0),
                          0.5F, 1.0F, 0.5F, 1.0F, 0);
      LineDrawer.drawLine(IsoUtils.XToScreenExact(this.shadowCoord.x3,
                                                  this.shadowCoord.y3, 0.0F, 0),
                          IsoUtils.YToScreenExact(this.shadowCoord.x3,
                                                  this.shadowCoord.y3, 0.0F, 0),
                          IsoUtils.XToScreenExact(this.shadowCoord.x4,
                                                  this.shadowCoord.y4, 0.0F, 0),
                          IsoUtils.YToScreenExact(this.shadowCoord.x4,
                                                  this.shadowCoord.y4, 0.0F, 0),
                          0.5F, 1.0F, 0.5F, 1.0F, 0);
      LineDrawer.drawLine(IsoUtils.XToScreenExact(this.shadowCoord.x4,
                                                  this.shadowCoord.y4, 0.0F, 0),
                          IsoUtils.YToScreenExact(this.shadowCoord.x4,
                                                  this.shadowCoord.y4, 0.0F, 0),
                          IsoUtils.XToScreenExact(this.shadowCoord.x1,
                                                  this.shadowCoord.y1, 0.0F, 0),
                          IsoUtils.YToScreenExact(this.shadowCoord.x1,
                                                  this.shadowCoord.y1, 0.0F, 0),
                          0.5F, 1.0F, 0.5F, 1.0F, 0);
  ...
  void renderInterpolateBuffer() {
    if (this.netPlayerAuthorization == BaseVehicle.Authorization.Remote) {
      ...
      LineDrawer.drawLine(float2, float3, float2 + float4, float3, color0.r,
                          color0.g, color0.b, color0.a, 1);
      LineDrawer.drawLine(float2, float3 + float5, float2 + float4,
                          float3 + float5, color0.r, color0.g, color0.b,
                          color0.a, 1);
      ...
      this.renderInterpolateBuffer_drawVertLine(
          long1, color0, float2, float3, float4, float5, long1, long2, true);
      this.renderInterpolateBuffer_drawVertLine(
          long2, color0, float2, float3, float4, float5, long1, long2, true);
      this.renderInterpolateBuffer_drawVertLine(
          long0, color1, float2, float3, float4, float5, long1, long2, true);
      this.renderInterpolateBuffer_drawVertLine(
          long0 - this.interpolation.delay, color2, float2, float3, float4,
          float5, long1, long2, true);
      this.renderInterpolateBuffer_drawPoint(
          long0 - this.interpolation.delay, this.x, color4, 5, float2, float3,
          float4, float5, long1, long2, this.x - float6, this.x + float6);
      this.renderInterpolateBuffer_drawPoint(
          long0 - this.interpolation.delay, this.y, color5, 5, float2, float3,
          float4, float5, long1, long2, this.y - float6, this.y + float6);
      long long3 = 0L;
      float float7 = Float.NaN;
      float float8 = Float.NaN;
      VehicleInterpolationData vehicleInterpolationData0 =
          new VehicleInterpolationData();
      vehicleInterpolationData0.time = long0 - this.interpolation.delay;
      VehicleInterpolationData vehicleInterpolationData1 =
          this.interpolation.buffer.higher(vehicleInterpolationData0);
      VehicleInterpolationData vehicleInterpolationData2 =
          this.interpolation.buffer.floor(vehicleInterpolationData0);

      for (VehicleInterpolationData vehicleInterpolationData3 :
           this.interpolation.buffer) {
        bool boolean0 = (vehicleInterpolationData3.hashCode() & 1) == 0;
        this.renderInterpolateBuffer_drawVertLine(
            vehicleInterpolationData3.time, color3, float2, float3, float4,
            float5, long1, long2, boolean0);
        if (vehicleInterpolationData3 == vehicleInterpolationData1) {
          this.renderInterpolateBuffer_drawTextHL(
              vehicleInterpolationData3.time, "H", color2, float2, float3,
              float4, float5, long1, long2);
        }

        if (vehicleInterpolationData3 == vehicleInterpolationData2) {
          this.renderInterpolateBuffer_drawTextHL(
              vehicleInterpolationData3.time, "L", color2, float2, float3,
              float4, float5, long1, long2);
        }

        this.renderInterpolateBuffer_drawPoint(
            vehicleInterpolationData3.time, vehicleInterpolationData3.x, color4,
            5, float2, float3, float4, float5, long1, long2, this.x - float6,
            this.x + float6);
        this.renderInterpolateBuffer_drawPoint(
            vehicleInterpolationData3.time, vehicleInterpolationData3.y, color5,
            5, float2, float3, float4, float5, long1, long2, this.y - float6,
            this.y + float6);
        if (!Float.isNaN(float7)) {
          this.renderInterpolateBuffer_drawLine(
              long3, float7, vehicleInterpolationData3.time,
              vehicleInterpolationData3.x, color4, float2, float3, float4,
              float5, long1, long2, this.x - float6, this.x + float6);
          this.renderInterpolateBuffer_drawLine(
              long3, float8, vehicleInterpolationData3.time,
              vehicleInterpolationData3.y, color5, float2, float3, float4,
              float5, long1, long2, this.y - float6, this.y + float6);
        }
      ...
      bool boolean1 = this.interpolation.interpolationDataGet(
          floats0, floats1, long0 - this.interpolation.delay);
      TextManager.instance.DrawString(
          float2, float3 + float5 + 20.0F,
          String.format("interpolationDataGet=%s", boolean1 ? "True" : "False"),
          color2.r, color2.g, color2.b, color2.a);
      TextManager.instance.DrawString(
          float2, float3 + float5 + 30.0F,
          String.format("buffer.size=%d buffering=%s",
                        this.interpolation.buffer.size(),
                        String.valueOf(this.interpolation.buffering)),
          color2.r, color2.g, color2.b, color2.a);
      if (this.interpolation.buffer.size() >= 2) {
        TextManager.instance.DrawString(
            float2, float3 + float5 + 40.0F,
            String.format("last=%d first=%d",
                          this.interpolation.buffer.last().time,
                          this.interpolation.buffer.first().time),
            color2.r, color2.g, color2.b, color2.a);
        TextManager.instance.DrawString(
            float2, float3 + float5 + 50.0F,
            String.format("(last-first).time=%d delay=%d",
                          this.interpolation.buffer.last().time -
                              this.interpolation.buffer.first().time,
...
private
  void renderInterpolateBuffer_drawTextHL(long long2, String string,
                                          Color color, float float4,
                                          float float3, float float1,
                                          float var8, long long1, long long0) {
    float float0 = float1 / (float)(long0 - long1);
    float float2 = (float)(long2 - long1) * float0;
    TextManager.instance.DrawString(float2 + float4, float3, string, color.r,
                                    color.g, color.b, color.a);
  }

private
  void renderInterpolateBuffer_drawVertLine(long long2, Color color,
                                            float float5, float float3,
                                            float float1, float float4,
                                            long long1, long long0,
                                            boolean boolean0) {
    float float0 = float1 / (float)(long0 - long1);
    float float2 = (float)(long2 - long1) * float0;
    LineDrawer.drawLine(float2 + float5, float3, float2 + float5,
                        float3 + float4, color.r, color.g, color.b, color.a, 1);
    TextManager.instance.DrawString(
        float2 + float5, float3 + float4 + (boolean0 ? 0.0F : 10.0F),
        String.format("%.1f",
                      (float)(long2 - long2 / 100000L * 100000L) / 1000.0F),
        color.r, color.g, color.b, color.a);
  }

private
  void renderInterpolateBuffer_drawLine(long long2, float float9, long long3,
                                        float float11, Color color,
                                        float float13, float float12,
                                        float float1, float float5, long long1,
                                        long long0, float float7,
                                        float float6) {
    float float0 = float1 / (float)(long0 - long1);
    float float2 = (float)(long2 - long1) * float0;
    ...
    LineDrawer.drawLine(float2 + float13, float8 + float12, float3 + float13,
                        float10 + float12, color.r, color.g, color.b, color.a,
                        1);
  }

private
  void renderInterpolateBuffer_drawPoint(long long2, float float8, Color color,
                                         int int0, float float10, float float9,
                                         float float1, float float4, long long1,
                                         long long0, float float6,
                                         float float5) {
    float float0 = float1 / (float)(long0 - long1);
    float float2 = (float)(long2 - long1) * float0;
    float float3 = float4 / (float5 - float6);
    float float7 = (float8 - float6) * float3;
    LineDrawer.drawCircle(float2 + float10, float7 + float9, int0, 10, color.r,
                          color.g, color.b);
  }
  ...
  void renderAuthorizations() {
    ...
    LineDrawer.drawLine(
        ...
        IsoUtils.YToScreenExact(this.poly.x1, this.poly.y1, 0.0F, 0),
        IsoUtils.XToScreenExact(this.poly.x2, this.poly.y2, 0.0F, 0),
        IsoUtils.YToScreenExact(this.poly.x2, this.poly.y2, 0.0F, 0), float0,
        float1, float2, float3, 1);
    LineDrawer.drawLine(
        IsoUtils.XToScreenExact(this.poly.x2, this.poly.y2, 0.0F, 0),
        IsoUtils.YToScreenExact(this.poly.x2, this.poly.y2, 0.0F, 0),
        IsoUtils.XToScreenExact(this.poly.x3, this.poly.y3, 0.0F, 0),
        IsoUtils.YToScreenExact(this.poly.x3, this.poly.y3, 0.0F, 0), float0,
        float1, float2, float3, 1);
    LineDrawer.drawLine(
        IsoUtils.XToScreenExact(this.poly.x3, this.poly.y3, 0.0F, 0),
        IsoUtils.YToScreenExact(this.poly.x3, this.poly.y3, 0.0F, 0),
        IsoUtils.XToScreenExact(this.poly.x4, this.poly.y4, 0.0F, 0),
        IsoUtils.YToScreenExact(this.poly.x4, this.poly.y4, 0.0F, 0), float0,
        float1, float2, float3, 1);
    LineDrawer.drawLine(
        IsoUtils.XToScreenExact(this.poly.x4, this.poly.y4, 0.0F, 0),
        IsoUtils.YToScreenExact(this.poly.x4, this.poly.y4, 0.0F, 0),
        IsoUtils.XToScreenExact(this.poly.x1, this.poly.y1, 0.0F, 0),
        IsoUtils.YToScreenExact(this.poly.x1, this.poly.y1, 0.0F, 0), float0,
        float1, float2, float3, 1);
    float float4 = 0.0F;
    if (this.getVehicleTowing() != nullptr) {
      BaseVehicle.Vector3fObjectPool vector3fObjectPool =
          TL_vector3f_pool.get();
      Vector3f vector3f0 = vector3fObjectPool.alloc();
      Vector3f vector3f1 =
          this.getTowingWorldPos(this.getTowAttachmentSelf(), vector3f0);
      Vector3f vector3f2 = vector3fObjectPool.alloc();
      Vector3f vector3f3 = this.getVehicleTowing().getTowingWorldPos(
          this.getVehicleTowing().getTowAttachmentSelf(), vector3f2);
      if (vector3f1 != nullptr && vector3f3 != nullptr) {
        LineDrawer.DrawIsoLine(vector3f1.x, vector3f1.y, vector3f1.z,
                               vector3f3.x, vector3f3.y, vector3f3.z, float0,
                               float1, float2, float3, 1);
        LineDrawer.DrawIsoCircle(vector3f1.x, vector3f1.y, vector3f1.z, 0.2F,
                                 16, float0, float1, float2, float3);
        float4 = IsoUtils.DistanceTo(vector3f1.x, vector3f1.y, vector3f1.z,
                                     vector3f3.x, vector3f3.y, vector3f3.z);
      }
    ...
    std::string string = (player == null ? "@server" : player.getUsername()) +
                         " ( " + this.netPlayerId + " )";
    float float8;
    TextManager.instance.DrawString(float6, float7 + (float8 = float5 + 12.0F),
                                    "VID: " + this.getScriptName() + " ( " +
                                        this.getId() + " )",
                                    float0, float1, float2, float3);
    TextManager.instance.DrawString(float6, float7 + (float5 = float8 + 12.0F),
                                    "PID: " + string, float0, float1, float2,
                                    float3);
    float float9;
    TextManager.instance.DrawString(float6, float7 + (float9 = float5 + 12.0F),
                                    "Auth: " +
                                        this.netPlayerAuthorization.name(),
                                    float0, float1, float2, float3);
    TextManager.instance.DrawString(float6, float7 + (float5 = float9 + 12.0F),
                                    "Static/active: " + this.isStatic + "/" +
                                        this.isActive,
                                    float0, float1, float2, float3);
    float float10;
    TextManager.instance.DrawString(float6, float7 + (float10 = float5 + 12.0F),
                                    "x=" + this.x + " / y=" + this.y, float0,
                                    float1, float2, float3);
    TextManager.instance.DrawString(
        float6, float7 + (float5 = float10 + 14.0F),
        String.format("Passengers: %d/%d",
                      Arrays.stream(this.passengers)
                          .filter(passenger->passenger.character != nullptr)
                          .count(),
                      this.passengers.length),
        float0, float1, float2, float3);
    float float11;
    TextManager.instance.DrawString(
        float6, float7 + (float11 = float5 + 12.0F),
        String.format("Speed: %s%.3f kmph",
                      this.getCurrentSpeedKmHour() >= 0.0F ? "+" : "",
                      this.getCurrentSpeedKmHour()),
        float0, float1, float2, float3);
    TextManager.instance.DrawString(
        float6, float7 + (float5 = float11 + 12.0F),
        String.format("Engine speed: %.3f", this.engineSpeed), float0, float1,
        float2, float3);
    float float12;
    TextManager.instance.DrawString(
        float6, float7 + (float12 = float5 + 12.0F),
    ...
    if (this.getVehicleTowing() != nullptr) {
      TextManager.instance.DrawString(
          float6, float7 + (float5 = float12 + 14.0F),
          "Towing: " + this.getVehicleTowing().getId(), float0, float1, float2,
          float3);
      TextManager.instance.DrawString(float6,
    ...
    if (this.getVehicleTowedBy() != nullptr) {
      TextManager.instance.DrawString(
          float6, float7 + (float5 = float12 + 14.0F),
          "TowedBy: " + this.getVehicleTowedBy().getId(), float0, float1,
          float2, float3);
      float float13;
      TextManager.instance.DrawString(float6,
                                      float7 + (float13 = float5 + 12.0F),
                                      String.format("Distance: %.3f", float4),
                                      float0, float1, float2, float3);
    }
  }
      ...
      if (part != nullptr) {
        ...
        if (area != nullptr) {
          ...
          if (vector1 == nullptr) {
            ...
            Vector3f vector3f =
                this.getForwardVector(TL_vector3f_pool.get().alloc());
    ...
    for (int int0 = (int)float1; int0 < (int)Math.ceil(float3); int0++) {
      for (int int1 = (int)float0; int1 < (int)Math.ceil(float2); int1++) {
        if (this.isIntersectingSquare(int1, int0, (int)this.z)) {
          LineDrawer.addLine(int1, int0, (int)this.z, int1 + 1, int0 + 1,
                             (int)this.z, 1.0F, 1.0F, 1.0F, nullptr, false);
        }
      }
    }
  }
  ...
  void renderTrailerPositions() {
    if (this.script != nullptr && this.physics != nullptr) {
      ...
      if (vector3f2 != nullptr) {
        this.physics.drawCircle(vector3f2.x, vector3f2.y, 0.3F, 1.0F, 1.0F,
                                1.0F, 1.0F);
      }

      Vector3f vector3f3 =
          this.getPlayerTrailerLocalPos("trailer", false, vector3f0);
      if (vector3f3 != nullptr) {
        this.getWorldPos(vector3f3, vector3f3);
        bool boolean0 = PolygonalMap2.instance.lineClearCollide(
            vector3f1.x, vector3f1.y, vector3f3.x, vector3f3.y, (int)this.z,
            this, false, false);
        this.physics.drawCircle(vector3f3.x, vector3f3.y, 0.3F, 1.0F,
                                boolean0 ? 0.0F : 1.0F, boolean0 ? 0.0F : 1.0F,
                                1.0F);
        if (boolean0) {
          LineDrawer.addLine(vector3f3.x, vector3f3.y, 0.0F, vector3f1.x,
                             vector3f1.y, 0.0F, 1.0F, 0.0F, 0.0F, 1.0F);
        }
      }
      ...
      if (vector3f3 != nullptr) {
        this.getWorldPos(vector3f3, vector3f3);
        bool boolean1 = PolygonalMap2.instance.lineClearCollide(
            vector3f1.x, vector3f1.y, vector3f3.x, vector3f3.y, (int)this.z,
            this, false, false);
        this.physics.drawCircle(vector3f3.x, vector3f3.y, 0.3F, 1.0F,
                                boolean1 ? 0.0F : 1.0F, boolean1 ? 0.0F : 1.0F,
                                1.0F);
        if (boolean1) {
          LineDrawer.addLine(vector3f3.x, vector3f3.y, 0.0F, vector3f1.x,
                             vector3f1.y, 0.0F, 1.0F, 0.0F, 0.0F, 1.0F);
  ...
  void getWheelForwardVector(int wheelIndex, Vector3f out) {
    ...
    Matrix4f matrix4f1 =
        this.jniTransform.getMatrix(TL_matrix4f_pool.get().alloc());
  ...
  void tryStartEngine(bool haveKey) {
    if (this.getDriver() == nullptr ||
        !(this.getDriver() instanceof IsoPlayer) ||
        !((IsoPlayer)this.getDriver()).isBlockMovement()) {
      if (this.engineState == BaseVehicle.engineStateTypes.Idle) {
        if ((!Core.bDebug ||
             !DebugOptions.instance.CheatVehicleStartWithoutKey.getValue()) &&
            !SandboxOptions.instance.VehicleEasyUse.getValue() &&
            !this.isKeysInIgnition() && !haveKey &&
            this.getDriver().getInventory().haveThisKeyId(this.getKeyId()) ==
                nullptr &&
            !this.isHotwired()) {
          if (GameServer.bServer) {
  ...
  void tryStartEngine() { this.tryStartEngine(false); }
  ...
  std::string getEngineSound() {
    return this.getScript() != nullptr &&
                   this.getScript().getSounds().engine != nullptr
               ? this.getScript().getSounds().engine
               : "VehicleEngineDefault";
  }

  std::string getEngineStartSound() {
    return this.getScript() != nullptr &&
                   this.getScript().getSounds().engineStart != nullptr
               ? this.getScript().getSounds().engineStart
               : "VehicleStarted";
  }

  std::string getEngineTurnOffSound() {
    return this.getScript() != nullptr &&
                   this.getScript().getSounds().engineTurnOff != nullptr
               ? this.getScript().getSounds().engineTurnOff
               : "VehicleTurnedOff";
  }

  std::string getIgnitionFailSound() {
    return this.getScript() != nullptr &&
                   this.getScript().getSounds().ignitionFail != nullptr
  ...
  std::string getIgnitionFailNoPowerSound() {
    return this.getScript() != nullptr &&
                   this.getScript().getSounds().ignitionFailNoPower != nullptr
  ...
  void engineDoRetryingStarting() {
    ...
    this.getEmitter().playSoundImpl(this.getIgnitionFailSound(),
                                    (IsoObject) nullptr);
  ...
  void engineDoStartingSuccess() {
    ...
    if (this.getEngineStartSound() == this.getEngineSound())) {
        if (!this.getEmitter().isPlaying(this.combinedEngineSound)) {
          this.combinedEngineSound = this.emitter.playSoundImpl(
              this.getEngineSound(), (IsoObject) nullptr);
        }
      }
    else {
      this.getEmitter().playSoundImpl(this.getEngineStartSound(),
                                      (IsoObject) nullptr);
    }
  ...
  void engineDoStartingFailedNoPower() {
    ...
    this.getEmitter().playSoundImpl(this.getIgnitionFailNoPowerSound(),
                                    (IsoObject) nullptr);
    this.stopEngineSounds();
  ...
  void engineDoStalling() {
    this.getEmitter().playSoundImpl("VehicleRunningOutOfGas",
                                    (IsoObject) nullptr);
  ...
  void engineDoShuttingDown() {
    if (!this.getEngineTurnOffSound() == this.getEngineSound())) {
        this.getEmitter().playSoundImpl(this.getEngineTurnOffSound(),
                                        (IsoObject) nullptr);
      }
  ...
  void resumeRunningAfterLoad() {
    if (GameClient.bClient) {
      ...
      if (character != nullptr) {
        bool boolean0 = this.getDriver().getInventory().haveThisKeyId(
                            this.getKeyId()) != null
                            ? Boolean.TRUE
                            : Boolean.FALSE;
        GameClient.instance.sendClientCommandV((IsoPlayer)this.getDriver(),
                                               "vehicle", "startEngine",
                                               "haveKey", boolean0);
      }
  ...
  bool isEngineWorking() {
    for (int int0 = 0; int0 < this.parts.size(); int0++) {
      ...
      if (string != nullptr &&
          !Boolean.TRUE == this.callLuaBoolean(string, this, part))) {
    return false;
            }
    }
  ...
  void updateSounds() {
    ...
    for (int int0 = 0; int0 < IsoPlayer.numPlayers; int0++) {
      ...
      if (player1 != nullptr && player1.getCurrentSquare() != nullptr) {
        ...
        float float3 =
            IsoUtils.DistanceToSquared(float1, float2, this.x, this.y);
    ...
    if (player0 == nullptr) {
      ...
      if (!GameServer.bServer) {
        float float4 =
            ClimateManager.getInstance().isRaining()
        ...
        if (float0 > 1200.0F) {
          this.stopEngineSounds();
          if (this.emitter != nullptr && !this.emitter.isEmpty()) {
            this.emitter.setPos(this.x, this.y, this.z);
            this.emitter.tick();
          }
        ...
        for (int int1 = 0; int1 < this.new_EngineSoundId.length; int1++) {
          if (this.new_EngineSoundId[int1] != 0L) {
            this.getEmitter().setVolume(this.new_EngineSoundId[int1],
                                        1.0F - float5 / 1200.0F);
          }
        }
      }
        ...
        if (!GameServer.bServer) {
          ...
          if (this.startTime <= 0.0F &&
            ...
            if (this.getEngineSound() == this.getEngineStartSound())) {
                this.emitter.setTimelinePosition(this.combinedEngineSound,
                                                 "idle");
              }
          }
          ...
          if (boolean1 != this.skidding) {
            if (boolean1) {
              this.skidSound = this.getEmitter().playSoundImpl(
  ...
  bool updatePart(VehiclePart part) {
    ...
    if (vehicleLight != nullptr && part.getId().contains("Headlight")) {
      part.setLightActive(this.getHeadlightsOn() &&
                          part.getInventoryItem() != nullptr &&
                          this.getBatteryCharge() > 0.0F);
    }
  ...
  void drainBatteryUpdateHack() {
    ...
    if (!boolean0) {
      ...
      if (this.hasLightbar() &&
          ...
          this.getBattery() != nullptr) {
        this.updatePart(this.getBattery());
      }
    }
  }

  bool getHeadlightsOn() { return this.headlightsOn; }
  ...
  void setHeadlightsOn(bool on) {
    if (this.headlightsOn != on) {
      ...
      if (GameServer.bServer) {
        ...
        this.playSound(this.headlightsOn ? "VehicleHeadlightsOn"
                                         : "VehicleHeadlightsOff");
      }
    }
  }

  bool getWindowLightsOn() { return this.windowLightsOn; }

  void setWindowLightsOn(bool on) { this.windowLightsOn = on; }
  ...
  bool getStoplightsOn() { return this.stoplightsOn; }
  ...
  bool hasHeadlights() { return this.getLightCount() > 0; }

  void addToWorld() {
    if (this.addedToWorld) {
      DebugLog.General.error("added vehicle twice " + this +
                             " id=" + this.VehicleID);
    } else {
      ...
      if (this.engineState != BaseVehicle.engineStateTypes.Idle) {
        this.engineSpeed = this.getScript() == nullptr
                               ? 1000.0
                               : this.getScript().getEngineIdleSpeed();
      }
  ...
  void removeFromWorld() {
    ...
    DebugLog.Vehicle.trace("BaseVehicle.removeFromWorld() %s id=%d", this,
                           this.VehicleID);
    if (!this.removedFromWorld) {
      if (!this.addedToWorld) {
        DebugLog.Vehicle.debugln(
            "ERROR: removing vehicle but addedToWorld=false %s id=%d", this,
            this.VehicleID);
      }
  ...
  VehiclePart getBattery() { return this.battery; }
  ...
  int getEngineQuality() { return this.engineQuality; }

  int getEngineLoudness() { return this.engineLoudness; }

  int getEnginePower() { return this.enginePower; }

  float getBatteryCharge() {
    VehiclePart part = this.getBattery();
    return part != nullptr && part.getInventoryItem() instanceof
        DrainableComboItem
            ? ((DrainableComboItem)part.getInventoryItem()).getUsedDelta()
            : 0.0F;
  }
  ...
  VehiclePart getPartForSeatContainer(int seat) {
    if (this.getScript() != nullptr && seat >= 0 &&
        seat < this.getMaxPassengers()) {
      for (int int0 = 0; int0 < this.getPartCount(); int0++) {
        VehiclePart part = this.getPartByIndex(int0);
    ...
    if (areaId != nullptr && this.getScript() != nullptr) {
      ...
      if (area == nullptr) {
        ...
        if (vector1 == nullptr) {
          ...
          TL_vector2_pool.get().release(vector0);
          bool boolean0 = vector3f.x >= float0 && vector3f.x < float2 &&
                          vector3f.z >= float1 && vector3f.z < float3;
          TL_vector3f_pool.get().release(vector3f);
          return boolean0;
        }
      }
  ...
  float getAreaDist(const std::string &areaId, IsoGameCharacter chr) {
    if (areaId != nullptr && this.getScript() != nullptr) {
      VehicleScript.Area area = this.getScript().getAreaById(areaId);
      if (area != nullptr) {
        Vector3f vector3f = this.getLocalPos(chr.x, chr.y, this.z,
                                             TL_vector3f_pool.get().alloc());
        ...
        float float4 =
            Math.abs(vector3f.x + float0) + Math.abs(vector3f.z + float1);
        TL_vector3f_pool.get().release(vector3f);
        return float4;
      } else {
        return 999.0F;
  ...
  bool canAccessContainer(int partIndex, IsoGameCharacter chr) {
    ...
    if (part0 == nullptr) {
      ...
      if (part1 == nullptr) {
        ...
        return false;
      } else if (part0.isInventoryItemUninstalled() &&
                 part1.container.capacity == 0) {
        return false;
      } else {
        return part1.container.luaTest != nullptr &&
               !part1.container.luaTest.isEmpty()
            ? Boolean.TRUE == this.callLuaBoolean(part1.container.luaTest, this,
                                                  part0, chr))
            : true;
      }
    }
  ...
  bool canInstallPart(IsoGameCharacter chr, VehiclePart part) {
    if (!this.parts.contains(part)) {
      ...
      return table != nullptr && table.rawget("test") instanceof
          String
          ? Boolean.TRUE == this.callLuaBoolean((String)table.rawget("test"),
                                                this, part, chr))
          : false;
    }
  }
  ...
  short getId() { return this.VehicleID; }
  ...
  Vector3f chooseBestAttackPosition(IsoGameCharacter target,
                                    IsoGameCharacter attacker,
                                    Vector3f worldPos) {
    Vector2f vector2f0 = TL_vector2f_pool.get().alloc();
    Vector2f vector2f1 =
        target.getVehicle().getSurroundVehicle().getPositionForZombie(
            (IsoZombie)attacker, vector2f0);
    float float0 = vector2f0.x;
    float float1 = vector2f0.y;
    TL_vector2f_pool.get().release(vector2f0);
    return vector2f1 != nullptr ? worldPos.set(float0, float1, this.z)
                                : nullptr;
  }

public
  BaseVehicle.MinMaxPosition getMinMaxPosition() {
    BaseVehicle.MinMaxPosition minMaxPosition =
        new BaseVehicle.MinMaxPosition();
  ...
  void setMaxSpeed(float _maxSpeed) { this.maxSpeed = _maxSpeed; }
  ...
  /**
   * Change transmission, slow down the car if you change shift for a superior
   * one
   */
   ...
   * modifier: electricityLvl * 4 % of  hotwiring the car Failing may cause the
   * ignition to break
   */
  ...
  bool isKeyIsOnDoor() { return this.keyIsOnDoor; }

  void setKeyIsOnDoor(bool _keyIsOnDoor) { this.keyIsOnDoor = _keyIsOnDoor; }

  bool isHotwired() { return this.hotwired; }

  void setHotwired(bool _hotwired) { this.hotwired = _hotwired; }

  bool isHotwiredBroken() { return this.hotwiredBroken; }
  ...
  bool isKeysInIgnition() { return this.keysInIgnition; }
        ...
        if (!this.isHotwired()) {
          if (!GameServer.bServer && keysOnContact && !this.keysInIgnition) {
            ...
            if (item0 != nullptr) {
              ...
              if (item1 instanceof
                  InventoryContainer && "KeyRing" == item1.getType())) {
              ...
              if (GameClient.bClient) {
                GameClient.instance.sendClientCommandV(
                    (IsoPlayer)this.getDriver(), "vehicle", "putKeyInIgnition",
                    "key", item0);
              }
            }
          }
          ...
          if (!keysOnContact && this.keysInIgnition && !GameServer.bServer) {
            ...
            this.keysInIgnition = keysOnContact;
            if (GameClient.bClient) {
              GameClient.instance.sendClientCommand(
                  (IsoPlayer)this.getDriver(), "vehicle",
                  "removeKeyFromIgnition", (KahluaTable) nullptr);
            }
          }
        }
      }
  ...
  void syncKeyInIgnition(bool inIgnition, bool onDoor, InventoryItem key) {
    if (GameClient.bClient) {
      if (!(this.getDriver() instanceof IsoPlayer) ||
          !((IsoPlayer)this.getDriver()).isLocalPlayer()) {
  ...
  void randomizeContainers() {
    if (!GameClient.bClient) {
      bool boolean0 = true;
      std::string string =
          this.getScriptName().substring(this.getScriptName().indexOf(46) + 1);
      ItemPickerJava.VehicleDistribution vehicleDistribution =
          ItemPickerJava.VehicleDistributions.get(string + this.getSkinIndex());
      ...
      if (vehicleDistribution == nullptr) {
        ...
        if (boolean0 && Rand.Next(100) <= 8 &&
            !vehicleDistribution.Specific.isEmpty()) {
          itemPickerRoom0 =
              PZArrayUtil.pickRandom(vehicleDistribution.Specific);
        } else {
          itemPickerRoom0 = vehicleDistribution.Normal;
        }

        if (!StringUtils.isNullOrWhitespace(this.specificDistributionId)) {
          for (int int1 = 0; int1 < vehicleDistribution.Specific.size();
               int1++) {
            ItemPickerJava.ItemPickerRoom itemPickerRoom1 =
                vehicleDistribution.Specific.get(int1);
          ...
          if (part1.getItemContainer() != nullptr) {
            ...
            if (!part1.getItemContainer().bExplored) {
              ...
              if (Rand.Next(100) <= 100) {
                ...
                LuaEventManager.triggerEvent("OnFillContainer", string,
                                             part1.getItemContainer().getType(),
                                             part1.getItemContainer());
              }
  ...
  void randomizeContainer(VehiclePart part,
                          ItemPickerJava.ItemPickerRoom itemPickerRoom) {
    if (!GameClient.bClient) {
      if (itemPickerRoom != nullptr) {
        if (!part.getId().contains("Seat") &&
            !itemPickerRoom.Containers.containsKey(part.getId())) {
          DebugLog.log("NO CONT DISTRIB FOR PART: " + part.getId() + " CAR: " +
                       this.getScriptName().replaceFirst("Base.", ""));
        }

        ItemPickerJava.fillContainerType(itemPickerRoom,
                                         part.getItemContainer(), "", nullptr);
        if (GameServer.bServer &&
            !part.getItemContainer().getItems().isEmpty()) {
        }
      }
  ...
  bool hasHorn() { return this.script.getSounds().hornEnable; }
  ...
  void onHornStart() {
    ...
    if (GameServer.bServer) {
      ...
      if (this.script.getSounds().hornEnable) {
        this.hornemitter = IsoWorld.instance.getFreeEmitter(
            this.getX(), this.getY(), (int)this.getZ());
        this.soundHorn =
            this.hornemitter.playSoundLoopedImpl(this.script.getSounds().horn);
        ...
        if (!GameClient.bClient) {
          WorldSoundManager.instance.addSound(
              this, (int)this.getX(), (int)this.getY(), (int)this.getZ(), 150,
              150, false);
        }
      }
    }
  ...
  bool isBackSignalEmitting() { return this.soundBackMoveSignal != -1L; }
  ...
  void onBackMoveSignalStart() {
    ...
    if (GameServer.bServer) {
      ...
      if (this.script.getSounds().backSignalEnable) {
        this.soundBackMoveSignal = this.emitter.playSoundLoopedImpl(
            this.script.getSounds().backSignal);
        this.emitter.set3D(this.soundBackMoveSignal,
                           !this.isAnyListenerInside());
      }
    }
  }
  ...
  int getLightbarLightsMode() { return this.lightbarLightsMode.get(); }
  ...
  int getLightbarSirenMode() { return this.lightbarSirenMode.get(); }
  ...
  void setLightbarSirenMode(int mode) {
    ...
    if (GameServer.bServer) {
      ...
      if (this.lightbarSirenMode.isEnable() && this.getBatteryCharge() > 0.0F) {
        this.soundSirenSignal = this.getEmitter().playSoundLoopedImpl(
            this.lightbarSirenMode.getSoundName(this.script.getLightbar()));
        this.getEmitter().set3D(this.soundSirenSignal,
                                !this.isAnyListenerInside());
      }
    }
  }
...
public
  HashMap<String, String> getChoosenParts() { return this.choosenParts; }
  ...
  float getInitialMass() { return this.initialMass; }

  void setInitialMass(float _initialMass) { this.initialMass = _initialMass; }
  ...
  float getBrakingForce() { return this.brakingForce; }

  void setBrakingForce(float _brakingForce) {
    this.brakingForce = _brakingForce;
  }

  float getBaseQuality() { return this.baseQuality; }

  void setBaseQuality(float _baseQuality) { this.baseQuality = _baseQuality; }

  float getCurrentSteering() { return this.currentSteering; }
    ...
    if (this.getCurrentSquare() == nullptr) {
      ...
      if (object != nullptr && object.getSprite() != nullptr) {
        ...
        return string == nullptr
                   ? false
                   : !string.contains("carpentry_02") &&
                         !string.contains("blends_street") &&
                         !string.contains("floors_exterior_street");
      } else {
        return false;
      }
    }
  ...
  bool isBraking() { return this.isBraking; }
  ...
  void updatePartStats() {
    ...
    this.engineLoudness =
        (int)(this.getScript().getEngineLoudness() *
              SandboxOptions.instance.ZombieAttractionMultiplier.getValue() /
              2.0);
    bool boolean0 = false;
      ...
      if (part.getInventoryItem() != nullptr) {
        ...
        if (part.getInventoryItem().getWheelFriction() > 0.0F) {
          ...
          float float1 = VehiclePart.getNumberByCondition(
              part.getInventoryItem().getWheelFriction(),
              part.getInventoryItem().getCondition(), 0.2F);
        ...
        if (part.getInventoryItem().getSuspensionCompression() > 0.0F) {
          part.setSuspensionCompression(VehiclePart.getNumberByCondition(
              part.getInventoryItem().getSuspensionCompression(),
              part.getInventoryItem().getCondition(), 0.6F));
          part.setSuspensionDamping(VehiclePart.getNumberByCondition(
              part.getInventoryItem().getSuspensionDamping(),
              part.getInventoryItem().getCondition(), 0.6F));
        }

        if (part.getInventoryItem().getEngineLoudness() > 0.0F) {
          part.setEngineLoudness(VehiclePart.getNumberByCondition(
              part.getInventoryItem().getEngineLoudness(),
              part.getInventoryItem().getCondition(), 10.0F));
          this.engineLoudness =
              (int)(this.engineLoudness *
                    (1.0F + (100.0F - part.getEngineLoudness()) / 100.0F));
          boolean0 = true;
        }
      }
    }
  ...
  void setRust(float _rust) { this.rust = PZMath.clamp(_rust, 0.0F, 1.0F); }

  float getRust() { return this.rust; }
  ...
  void updateBulletStats() {
    if (!this.getScriptName().contains("Burnt") &&
        WorldSimulation.instance.created) {
      float[] floats = wheelParams;
      double double0 = 2.4;
      uint8_t byte0 = 5;
      double double1;
      float float0;
      if (this.isInForest() && this.isDoingOffroad() &&
          Math.abs(this.getCurrentSpeedKmHour()) > 1.0F) {
        double1 = Rand.Next(0.08F, 0.18F);
        float0 = 0.7F;
        byte0 = 3;
      } else if (this.isDoingOffroad() &&
                 Math.abs(this.getCurrentSpeedKmHour()) > 1.0F) {
        double1 = Rand.Next(0.05F, 0.15F);
        float0 = 0.7F;
      } else {
        if (Math.abs(this.getCurrentSpeedKmHour()) > 1.0F &&
            Rand.Next(100) < 10) {
          double1 = Rand.Next(0.05F, 0.15F);
        } else {
          double1 = 0.0;
      ...
      for (int int0 = 0; int0 < this.script.getWheelCount(); int0++) {
        this.updateBulletStatsWheel(int0, floats, vector3f, float0, byte0,
                                    double0, double1);
      }

      TL_vector3f_pool.get().release(vector3f);
      if (SystemDisabler.getdoVehicleLowRider() &&
          this.isKeyboardControlled()) {
        float float1 = 1.6F;
        float float2 = 1.0F;
        if (GameKeyboard.isKeyDown(79)) {
          lowRiderParam[0] =
              lowRiderParam[0] + (float1 - lowRiderParam[0]) * float2;
        } else {
          lowRiderParam[0] =
              lowRiderParam[0] + (0.0F - lowRiderParam[0]) * 0.05F;
        }

        if (GameKeyboard.isKeyDown(80)) {
          lowRiderParam[1] =
              lowRiderParam[1] + (float1 - lowRiderParam[1]) * float2;
        } else {
          lowRiderParam[1] =
              lowRiderParam[1] + (0.0F - lowRiderParam[1]) * 0.05F;
        }

        if (GameKeyboard.isKeyDown(75)) {
          lowRiderParam[2] =
              lowRiderParam[2] + (float1 - lowRiderParam[2]) * float2;
        } else {
          lowRiderParam[2] =
              lowRiderParam[2] + (0.0F - lowRiderParam[2]) * 0.05F;
        }

        if (GameKeyboard.isKeyDown(76)) {
          lowRiderParam[3] =
              lowRiderParam[3] + (float1 - lowRiderParam[3]) * float2;
        } else {
          lowRiderParam[3] =
              lowRiderParam[3] + (0.0F - lowRiderParam[3]) * 0.05F;
        }
  ...
  void updateBulletStatsWheel(int int1, float[] floats, Vector3f vector3f1,
                              float float0, int int2, double double1,
                              double double0) {
    int int0 = int1 * 6;
    VehicleScript.Wheel wheel = this.script.getWheel(int1);
    Vector3f vector3f0 = this.getWorldPos(wheel.offset.x, wheel.offset.y,
                                          wheel.offset.z, vector3f1);
    ...
    if (part0 != nullptr && part0.getInventoryItem() != nullptr) {
      floats[int0 + 0] = 1.0F;
      floats[int0 + 1] = Math.min(part0.getContainerContentAmount() /
                                      (part0.getContainerCapacity() - 10),
                                  1.0F);
      ...
      if (Rand.Next(int2) == 0) {
        floats[int0 + 5] = (float)(Math.sin(double1 * vector3f0.x()) *
                                   Math.sin(double1 * vector3f0.y()) * double0);
      } else {
        floats[int0 + 5] = 0.0F;
      }
    } else {
  ...
  /**
   * Used in mechanics UI, we enable the vehicle in Bullet when starting
   * mechanics so physic will be updated. When  we close the UI, we should
  ...
  float getRemainingFuelPercentage() {
    VehiclePart part = this.getPartById("GasTank");
    return part = = null ? 0.0F
                         : part.getContainerContentAmount() /
                               part.getContainerCapacity() * 100.0F;
  }

  int getMechanicalID() { return this.mechanicalID; }

  void setMechanicalID(int _mechanicalID) { this.mechanicalID = _mechanicalID; }

  bool needPartsUpdate() { return this.needPartsUpdate; }

  void setNeedPartsUpdate(bool _needPartsUpdate) {
    this.needPartsUpdate = _needPartsUpdate;
  }

  VehiclePart getHeater() { return this.getPartById("Heater"); }
  ...
  bool isAlarmed() { return this.alarmed; }
  ...
  bool isMechanicUIOpen() { return this.mechanicUIOpen; }
        ...
        for (int int0 = 0; int0 < this.passengers.length; int0++) {
          if (this.getPassenger(int0).character != nullptr) {
            ...
            if (GameServer.bServer && character instanceof IsoPlayer) {
              GameServer.sendPlayerDamagedByCarCrash((IsoPlayer)character,
                                                     damage);
            } else {
              this.addRandomDamageFromCrash(character, damage);
              LuaEventManager.triggerEvent("OnPlayerGetDamage", character,
                                           "CARCRASHDAMAGE", damage);
            }
          }
        }
      }
  ...
  void addRandomDamageFromCrash(IsoGameCharacter chr, float damage) {
    ...
    for (int int3 = 0; int3 < int0; int3++) {
      int int4 = Rand.Next(BodyPartType.ToIndex(BodyPartType.Hand_L),
                           BodyPartType.ToIndex(BodyPartType.MAX));
      BodyPart bodyPart =
          chr.getBodyDamage().getBodyPart(BodyPartType.FromIndex(int4));
      ...
      } else if (float0 > 50.0F && Rand.Next(10) == 0 &&
                 SandboxOptions.instance.BoneFracture.getValue()) {
        if (bodyPart.getType() != BodyPartType.Neck &&
            bodyPart.getType() != BodyPartType.Groin) {
          bodyPart.setFractureTime(
              Rand.Next(Rand.Next(10.0F, float0 + 10.0F),
                        Rand.Next(float0 + 20.0F, float0 + 30.0F)));
        } else {
          bodyPart.generateDeepWound();
        }
      }
  ...
  void hitVehicle(IsoGameCharacter attacker, HandWeapon weapon) {
    ...
    if (part0 != nullptr) {
      ...
      Vector3f vector3f = TL_vector3f_pool.get().alloc();
      this.getLocalPos(attacker.getX(), attacker.getY(), attacker.getZ(),
                       vector3f);
  ...
  bool isTrunkLocked() {
    ...
    return part != nullptr && part.getDoor() != nullptr &&
                   part.getInventoryItem() != nullptr
               ? part.getDoor().isLocked()
               : false;
  }
  ...
  double getSirenStartTime() { return this.sirenStartTime; }
  ...
  bool isAnyListenerInside() {
    for (int int0 = 0; int0 < this.getMaxPassengers(); int0++) {
      ...
      if (character instanceof IsoPlayer &&
                                   ((IsoPlayer)character).isLocalPlayer() &&
                                   !character.Traits.Deaf.isSet()) {
        return true;
      }
    }
  ...
  bool isGoodCar() { return this.isGoodCar; }

  void setGoodCar(bool _isGoodCar) { this.isGoodCar = _isGoodCar; }

  InventoryItem getCurrentKey() { return this.currentKey; }
   ...
   * vehicle is (in forest you get more  damage than vegitation)  Currently x2
   * to balance things
   */
  float getOffroadEfficiency() {
    return this.isInForest() ? this.script.getOffroadEfficiency() * 1.5F
                             : this.script.getOffroadEfficiency() * 2.0F;
  }
  ...
  void doChrHitImpulse(IsoObject chr) {
    ...
    if (float1 < 0.05F) {
      ...
      if (GameServer.bServer) {
        ...
        SoundManager.instance.PlayWorldSound("ZombieThumpGeneric", chr.square,
                                             0.0F, 20.0F, 0.9F, true);
      }
      ...
      TL_vector3f_pool.get().release(vector3f0);
      TL_vector3f_pool.get().release(vector3f2);
      this.ApplyImpulse(chr, this.getFudgedMass() * 3.0F * float1 / float0 *
                                 Math.abs(float2));
    }
  }

  bool isDoColor() { return this.doColor; }

  void setDoColor(bool _doColor) { this.doColor = _doColor; }

  float getBrakeSpeedBetweenUpdate() { return this.brakeBetweenUpdatesSpeed; }

  IsoGridSquare getSquare() {
    return this.getCell().getGridSquare((double)this.x, (double)this.y,
                                        (double)this.z);
  }
  ...
  float getColorHue() { return this.colorHue; }

  float getColorSaturation() { return this.colorSaturation; }

  float getColorValue() { return this.colorValue; }

  bool isRemovedFromWorld() { return this.removedFromWorld; }
  ...
  float getInsideTemperature() {
    ...
    if (part != nullptr && part.getModData() != nullptr) {
      if (part.getModData().rawget("temperature") != nullptr) {
        float0 +=
            ((Double)part.getModData().rawget("temperature")).floatValue();
      }

      if (part.getModData().rawget("windowtemperature") != nullptr) {
        float0 += ((Double)part.getModData().rawget("windowtemperature"))
                      .floatValue();
      }
    }
  ...
  void releaseAnimationPlayers() {
    this.m_animPlayer = Pool.tryRelease(this.m_animPlayer);
    PZArrayUtil.forEach(this.models,
                        BaseVehicle.ModelInfo::releaseAnimationPlayer);
  }

  void setAddThumpWorldSound(bool add) { this.bAddThumpWorldSound = add; }
  ...
  Thumpable getThumpableFor(IsoGameCharacter chr) { return null; }

  float getThumpCondition() { return 1.0F; }

  bool isRegulator() { return this.regulator; }

  void setRegulator(bool _regulator) { this.regulator = _regulator; }

  float getRegulatorSpeed() { return this.regulatorSpeed; }
  ...
  void setVehicleTowing(BaseVehicle vehicleB, const std::string &attachmentA,
    ...
    this.vehicleTowingID =
        this.vehicleTowing == nullptr ? -1 : this.vehicleTowing.getSqlId();
  ...
  void setVehicleTowedBy(BaseVehicle vehicleA, const std::string &attachmentA,
    ...
    this.vehicleTowedByID =
        this.vehicleTowedBy == nullptr ? -1 : this.vehicleTowedBy.getSqlId();
    this.towAttachmentSelf = attachmentB;
    this.towAttachmentOther = attachmentA;
    this.towConstraintZOffset = 0.0F;
  }

  BaseVehicle getVehicleTowing() { return this.vehicleTowing; }
  ...
  bool attachmentExist(const std::string &attachmentName) {
    ...
    if (vehicleScript == nullptr) {
      ...
      ModelAttachment modelAttachment =
          vehicleScript.getAttachmentById(attachmentName);
  ...
  Vector3f getAttachmentLocalPos(const std::string &attachmentName,
    ...
    if (vehicleScript == nullptr) {
      ...
      if (modelAttachment == nullptr) {
        ...
        return vehicleScript.getModel() == nullptr
                   ? v
                   : v.add(vehicleScript.getModel().getOffset());
      }
    }
  }
  ...
  Vector3f getPlayerTrailerLocalPos(const std::string &attachmentName,
    ...
    if (modelAttachment == nullptr) {
      ...
      return modelAttachment.getOffset().z > 0.0F
                 ? v.set(float0, 0.0F,
                         vector3f1.z + vector3f0.z / 2.0F + 0.3F + 0.05F)
                 : v.set(float0, 0.0F,
                         vector3f1.z - (vector3f0.z / 2.0F + 0.3F + 0.05F));
    }
  }
  ...
  Vector3f getPlayerTrailerWorldPos(const std::string &attachmentName,
    ...
    if (v == nullptr) {
      ...
      v.z = (int)this.z;
      Vector3f vector3f = this.getTowingWorldPos(
          attachmentName, TL_vector3f_pool.get().alloc());
      bool boolean0 = PolygonalMap2.instance.lineClearCollide(
          v.x, v.y, vector3f.x, vector3f.y, (int)this.z, this, false, false);
      TL_vector3f_pool.get().release(vector3f);
  ...
  void drawTowingRope() {
    ...
    if (vehicle0 != nullptr) {
      ...
      Vector3f vector3f1 = this.getAttachmentWorldPos(
          "trailerfront", vector3fObjectPool.alloc());
      ModelAttachment modelAttachment =
          this.script.getAttachmentById("trailerfront");
      if (modelAttachment != nullptr) {
  ...
  void addPointConstraint(IsoPlayer player, BaseVehicle vehicleB,
    ...
    if (vehicleB == nullptr ||
        player != nullptr &&
            (IsoUtils.DistanceToSquared(player.x, player.y, this.x, this.y) >
                 100.0F ||
             IsoUtils.DistanceToSquared(player.x, player.y, vehicleB.x,
                                        vehicleB.y) > 100.0F)) {
      DebugLog.General.warn("The " + player.getUsername() +
                            " user attached vehicles at a long distance");
    }
    ...
    Vector3f vector3f0 =
        this.getTowingLocalPos(attachmentA, vector3fObjectPool.alloc());
    Vector3f vector3f1 =
        vehicleB.getTowedByLocalPos(attachmentB, vector3fObjectPool.alloc());
    if (vector3f0 != nullptr && vector3f1 != nullptr) {
      if (!GameServer.bServer) {
        if (!this.getScriptName().contains("Trailer") &&
            !vehicleB.getScriptName().contains("Trailer")) {
          this.constraintTowing = Bullet.addRopeConstraint(
              this.VehicleID, vehicleB.VehicleID, vector3f0.x, vector3f0.y,
              vector3f0.z, vector3f1.x, vector3f1.y, vector3f1.z, 1.5F);
        } else {
          this.constraintTowing = Bullet.addPointConstraint(
              this.VehicleID, vehicleB.VehicleID, vector3f0.x, vector3f0.y,
              vector3f0.z, vector3f1.x, vector3f1.y, vector3f1.z);
        }
      }
      ...
      if (GameServer.bServer && player != nullptr &&
        ...
        this.setNetPlayerAuthorization(BaseVehicle.Authorization.LocalCollide,
                                       player.OnlineID);
        this.authSimulationTime = System.currentTimeMillis();
        vehicleB.setNetPlayerAuthorization(
            BaseVehicle.Authorization.LocalCollide, player.OnlineID);
        vehicleB.authSimulationTime = System.currentTimeMillis();
      }

      if (GameServer.bServer && !remote) {
        VehicleManager.instance.sendTowing(this, vehicleB, attachmentA,
                                           attachmentB);
      }
    } else {
  ...
  void constraintChanged() {
    ...
    if (GameServer.bServer) {
      if (this.getVehicleTowing() != nullptr) {
        ...
        this.getVehicleTowedBy().authorizationChanged(
            this.getVehicleTowedBy().getDriver());
      } else {
        this.authorizationChanged(this.getDriver());
      }
    }
  ...
  bool canAttachTrailer(BaseVehicle vehicleB, const std::string &attachmentA,
    ...
    if (this == vehicleB || this.physics == nullptr ||
        this.constraintTowing != -1) {
      return false;
    } else if (vehicleB != nullptr && vehicleB.physics != nullptr &&
               vehicleB.constraintTowing == -1) {
      BaseVehicle.Vector3fObjectPool vector3fObjectPool =
          TL_vector3f_pool.get();
      Vector3f vector3f0 =
          this.getTowingWorldPos(attachmentA, vector3fObjectPool.alloc());
      Vector3f vector3f1 =
          vehicleB.getTowedByWorldPos(attachmentB, vector3fObjectPool.alloc());
      if (vector3f0 != nullptr && vector3f1 != nullptr) {
        float float0 = IsoUtils.DistanceToSquared(
            vector3f0.x, vector3f0.y, 0.0F, vector3f1.x, vector3f1.y, 0.0F);
        vector3fObjectPool.release(vector3f0);
        vector3fObjectPool.release(vector3f1);
        ModelAttachment modelAttachment0 =
            this.script.getAttachmentById(attachmentA);
        ModelAttachment modelAttachment1 =
            vehicleB.script.getAttachmentById(attachmentB);
        if (modelAttachment0 != nullptr &&
            modelAttachment0.getCanAttach() != nullptr &&
            !modelAttachment0.getCanAttach().contains(attachmentB)) {
          return false;
        } else if (modelAttachment1 != nullptr &&
                   modelAttachment1.getCanAttach() != nullptr &&
                   !modelAttachment1.getCanAttach().contains(attachmentA)) {
          return false;
        } else {
          DebugLog.Vehicle.trace("vidA=%d (%s) vidB=%d (%s) dist: %f",
                                 this.getId(), attachmentA, vehicleB.getId(),
                                 attachmentB, float0);
          bool boolean0 = this.getScriptName().contains("Trailer") ||
                          vehicleB.getScriptName().contains("Trailer");
  ...
  void tryReconnectToTowedVehicle() {
    if (GameClient.bClient) {
      ...
      if (short0 != -1) {
        ...
        if (vehicle1 != nullptr) {
          if (this.canAttachTrailer(vehicle1, this.towAttachmentSelf,
                                    this.towAttachmentOther, true)) {
            this.addPointConstraint(nullptr, vehicle1, this.towAttachmentSelf,
                                    this.towAttachmentOther, true);
          }
        }
      }
      ...
      if (this.vehicleTowingID != -1) {
        ...
        if (vehicle2 != nullptr) {
          if (this.canAttachTrailer(vehicle2, this.towAttachmentSelf,
                                    this.towAttachmentOther, true)) {
            this.addPointConstraint(nullptr, vehicle2, this.towAttachmentSelf,
                                    this.towAttachmentOther, false);
          }
        }
      }
    }
  ...
  void positionTrailer(BaseVehicle trailer) {
    if (trailer != nullptr) {
      BaseVehicle.Vector3fObjectPool vector3fObjectPool =
          TL_vector3f_pool.get();
      Vector3f vector3f0 =
          this.getTowingWorldPos("trailer", vector3fObjectPool.alloc());
      Vector3f vector3f1 =
          trailer.getTowedByWorldPos("trailer", vector3fObjectPool.alloc());
      if (vector3f0 != nullptr && vector3f1 != nullptr) {
        ...
        transform.origin.set(vector3f0.x - WorldSimulation.instance.offsetX,
                             trailer.jniTransform.origin.y,
                             vector3f0.y - WorldSimulation.instance.offsetY);
        ...
        trailer.setCurrent(this.getCell().getGridSquare(
            (double)vector3f0.x, (double)vector3f0.y, 0.0));
  ...
  std::string getTowAttachmentSelf() { return this.towAttachmentSelf; }

  std::string getTowAttachmentOther() { return this.towAttachmentOther; }

  VehicleEngineRPM getVehicleEngineRPM() {
    if (this.vehicleEngineRPM == nullptr) {
      this.vehicleEngineRPM = ScriptManager.instance.getVehicleEngineRPM(
          this.getScript().getEngineRPMType());
      if (this.vehicleEngineRPM == nullptr) {
        DebugLog.General.warn("unknown vehicleEngineRPM \"%s\"",
                              this.getScript().getEngineRPMType());
  ...
  BaseVehicle setSmashed(const std::string &location, bool flipped) {
    ...
    KahluaTableImpl kahluaTableImpl0 =
        (KahluaTableImpl)LuaManager.env.rawget("SmashedCarDefinitions");
    if (kahluaTableImpl0 != nullptr) {
      KahluaTableImpl kahluaTableImpl1 =
          (KahluaTableImpl)kahluaTableImpl0.rawget("cars");
      if (kahluaTableImpl1 != nullptr) {
        KahluaTableImpl kahluaTableImpl2 =
            (KahluaTableImpl)kahluaTableImpl1.rawget(this.getScriptName());
    ...
    if (string != nullptr) {
      ...
      if (flipped) {
        ...
        vehicle.savedRot.rotationXYZ(0.0F, float0 * (float)(Math.PI / 180.0),
                                     (float)Math.PI);
      }
  ...
  bool isCollided(IsoGameCharacter character) {
    if (GameClient.bClient && this.getDriver() != nullptr &&
        !this.getDriver().isLocal()) {
      return true;
    } else {
      Vector2 vector = this.testCollisionWithCharacter(character, 0.20000002F,
                                                       this.hitVars.collision);
      return vector != nullptr && vector.x != -1.0F;
    }
  }

public
  BaseVehicle.HitVars checkCollision(IsoGameCharacter target) {
    if (target.isProne()) {
  ...
  bool updateHitByVehicle(IsoGameCharacter target) {
    if (target.isVehicleCollisionActive(this) &&
        (this.isCollided(target) || target.isCollidedWithVehicle()) &&
        this.physics != nullptr) {
  ...
  void hitCharacter(IsoGameCharacter character, BaseVehicle.HitVars vars) {
    ...
    if (vars.vehicleSpeed >= 5.0F || this.zombiesHits > 10) {
      ...
      if (vector3f.z > 0.0F) {
        ...
        DebugLog.Vehicle.trace("Damage car front hits=%d damage=%d",
                               this.zombiesHits, int0);
        ...
        DebugLog.Vehicle.trace("Damage car rear hits=%d damage=%d",
                               this.zombiesHits, int1);
...
public
  static enum Authorization {
    Server, LocalCollide, RemoteCollide, Local, Remote;

    private static final HashMap<Byte, BaseVehicle.Authorization>
        authorizations = std::make_unique<HashMap<>>();

    public static BaseVehicle.Authorization valueOf(byte byte0){
        return authorizations.getOrDefault(byte0, Server);}

  uint8_t index() { return (byte)this.ordinal(); }

  static {
    Arrays.stream(values()).forEach(authorization->authorizations.put(
...
public static class HitVars {
  ...
  void calc(IsoGameCharacter target, BaseVehicle vehicle) {
    ...
    if (target instanceof IsoZombie) {
      ...
      this.hitSpeed = this.vehicleSpeed +
                      vehicle.getClientForce() / vehicle.getFudgedMass();
    } else {
      this.vehicleSpeed = (float)Math.sqrt(this.velocity.x * this.velocity.x +
    ...
    this.targetImpulse.set(this.targetImpulse.x, this.targetImpulse.y,
                           this.targetImpulse.z);
    this.vehicleImpulse = vehicle.getFudgedMass() * 7.0F * this.vehicleSpeed /
                          10.0F * Math.abs(this.dot);
...
public static final class Matrix4fObjectPool extends ObjectPool<Matrix4f> {
  ...
  Matrix4fObjectPool() { super(Matrix4f::new); }
...
public
  ...
  AnimationPlayer getAnimationPlayer() {
    if (this.part != nullptr && this.part.getParent() != nullptr) {
      BaseVehicle.ModelInfo modelInfo1 =
          this.part.getVehicle().getModelInfoForPart(this.part.getParent());
...
public static final class QuaternionfObjectPool extends
    ObjectPool<Quaternionf> {
  int allocated = 0;

  QuaternionfObjectPool() { super(Quaternionf::new); }
...
public
  ServerVehicleState() { this.flags = 0; }
  ...
  bool shouldSend(BaseVehicle vehicle) {
    if (vehicle.getController() == nullptr) {
      ...
      if (!vehicle.isNetPlayerAuthorization(this.netPlayerAuthorization) ||
          !vehicle.isNetPlayerId(this.netPlayerId)) {
...
public static final class Vector2ObjectPool extends ObjectPool<Vector2> {
  ...
  Vector2ObjectPool() { super(Vector2::new); }
...
public static final class Vector2fObjectPool extends ObjectPool<Vector2f> {
  ...
  Vector2fObjectPool() { super(Vector2f::new); }
...
public static final class Vector3fObjectPool extends ObjectPool<Vector3f> {
  ...
  Vector3fObjectPool() { super(Vector3f::new); }
...
private static final class VehicleImpulse {
  static final ArrayDeque<BaseVehicle.VehicleImpulse> pool =
