...
private
  ...
  bool bExit = false;
  std::string m_initialScript = null;
private
  ...
  void enter() {
    ...
    if (this.m_selfUI.size() == 0) {
      this.m_luaEnv.caller.pcall(
          ...
          this.m_luaEnv.env.rawget("EditVehicleState_InitUI"));
...
public
  ...
  void setScript(const std::string &scriptName) {
    if (this.m_table == nullptr) {
      ...
      this.m_luaEnv.caller.pcall(this.m_luaEnv.thread,
                                 this.m_table.rawget("setScript"), this.m_table,
                                 scriptName);
    }
  }
    ...
    default:
      ...
      case 0:
        VehicleScript vehicleScript =
            ScriptManager.instance.getVehicle((String)arg0);
        if (vehicleScript == nullptr) {
          throw new NullPointerException("vehicle script \"" + arg0 +
                                         "\" not found");
        }

        std::vector arrayList = this.readScript(vehicleScript.getFileName());
        if (arrayList != nullptr) {
          this.updateScript(vehicleScript.getFileName(), arrayList,
                            vehicleScript);
        }
...
private
            ...
            for (int int3 = arrayList1.size() - 1; int3 >= 0; int3--) {
              ...
              if (string5.startsWith("vehicle")) {
                ...
                if (string6 == vehicleScript.getName())) {
                    string5 =
                        this.vehicleScriptToText(vehicleScript, string5).trim();
  ...
  std::string vehicleScriptToText(VehicleScript vehicleScript,
    ...
    if (model != nullptr && block1 != nullptr) {
      ...
      block1.setValue("offset",
                      String.format(Locale.US, "%.4f %.4f %.4f",
                                    vector3f0.x / float0, vector3f0.y / float0,
                                    vector3f0.z / float0));
    }
    ...
    for (int int1 = 0; int1 < vehicleScript.getPhysicsShapeCount(); int1++) {
      VehicleScript.PhysicsShape physicsShape =
          vehicleScript.getPhysicsShape(int1);
      bool boolean0 = int1 < arrayList.size();
      ScriptParser.Block block3 = boolean0
                                      ? (ScriptParser.Block)arrayList.get(int1)
                                      : new ScriptParser.Block();
      block3.type = "physics";
    ...
    for (int int3 = 0; int3 < vehicleScript.getAttachmentCount(); int3++) {
      ...
      if (modelAttachment.getCanAttach() != nullptr) {
        block5.setValue("canAttach",
                        PZArrayUtil.arrayToString(
                            modelAttachment.getCanAttach(), "", "", ","));
      }

      if (modelAttachment.getZOffset() != 0.0F) {
        block5.setValue("zoffset", String.format(Locale.US, "%.4f",
                                                 modelAttachment.getZOffset()));
      }
    ...
    block0.setValue("extents",
                    String.format(Locale.US, "%.4f %.4f %.4f",
                                  vector3f1.x / float0, vector3f1.y / float0,
                                  vector3f1.z / float0));
    Vector3f vector3f2 = vehicleScript.getPhysicsChassisShape();
    block0.setValue("physicsChassisShape",
                    String.format(Locale.US, "%.4f %.4f %.4f",
                                  vector3f2.x / float0, vector3f2.y / float0,
                                  vector3f2.z / float0));
    Vector3f vector3f3 = vehicleScript.getCenterOfMassOffset();
    block0.setValue("centerOfMassOffset",
                    String.format(Locale.US, "%.4f %.4f %.4f",
                                  vector3f3.x / float0, vector3f3.y / float0,
                                  vector3f3.z / float0));
    Vector2f vector2f0 = vehicleScript.getShadowExtents();
    bool boolean1 = block0.getValue("shadowExtents") != null;
    block0.setValue("shadowExtents",
                    String.format(Locale.US, "%.4f %.4f", vector2f0.x / float0,
                                  vector2f0.y / float0));
    if (!boolean1) {
    ...
    block0.setValue("shadowOffset",
                    String.format(Locale.US, "%.4f %.4f", vector2f1.x / float0,
                                  vector2f1.y / float0));
    if (!boolean1) {
    ...
    for (int int4 = 0; int4 < vehicleScript.getAreaCount(); int4++) {
      ...
      if (block6 != nullptr) {
        block6.setValue(
            "xywh", String.format(Locale.US, "%.4f %.4f %.4f %.4f",
                                  area.getX() / float0, area.getY() / float0,
                                  area.getW() / float0, area.getH() / float0));
      }
    }
    ...
    for (int int5 = 0; int5 < vehicleScript.getPassengerCount(); int5++) {
      ...
      ScriptParser.Block block7 =
          block0.getBlock("passenger", passenger.getId());
      if (block7 != nullptr) {
        for (VehicleScript.Position position : passenger.positions) {
          ScriptParser.Block block8 = block7.getBlock("position", position.id);
          if (block8 != nullptr) {
            block8.setValue("offset",
                            String.format(Locale.US, "%.4f %.4f %.4f",
                                          position.offset.x / float0,
                                          position.offset.y / float0,
                                          position.offset.z / float0));
            block8.setValue("rotate",
                            String.format(Locale.US, "%.4f %.4f %.4f",
                                          position.rotate.x / float0,
                                          position.rotate.y / float0,
                                          position.rotate.z / float0));
          }
        }
      }
    }
    ...
    for (int int6 = 0; int6 < vehicleScript.getWheelCount(); int6++) {
      ...
      if (block9 != nullptr) {
        block9.setValue("offset", String.format(Locale.US, "%.4f %.4f %.4f",
                                                wheel.offset.x / float0,
...
public
  LuaEnvironment(J2SEPlatform _platform,
                 KahluaConverterManager converterManager, KahluaTable _env) {
