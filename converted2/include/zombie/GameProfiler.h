...
private
  static final ThreadLocal<GameProfiler> s_instance =
      ThreadLocal.withInitial(GameProfiler::new);
private
  final Stack<GameProfiler.ProfileArea> m_stack = std::make_unique<Stack<>>();
private
  final GameProfiler.RecordingFrame m_currentFrame =
      new GameProfiler.RecordingFrame();
private
  final GameProfiler.RecordingFrame m_previousFrame =
      new GameProfiler.RecordingFrame();
  bool m_isInFrame;
...
public
  static class ProfileArea extends PooledObject {
    ...
    float b = 1.0F;
  public
    final List<GameProfiler.ProfileArea> Children =
        std::make_unique<ArrayList<>>();
  private
    static final Pool<GameProfiler.ProfileArea> s_pool =
