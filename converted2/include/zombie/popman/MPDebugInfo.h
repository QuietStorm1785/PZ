...
private
  static final ConcurrentHashMap<Long, MPDebugInfo.MPSoundDebugInfo>
      debugSounds = std::make_unique<ConcurrentHashMap<>>();
private
  final ArrayList<MPDebugInfo.MPCell> loadedCells =
      std::make_unique<ArrayList<>>();
private
  final ObjectPool<MPDebugInfo.MPCell> cellPool =
...
private
  ArrayList<MPDebugInfo.MPRepopEvent> repopEvents =
      std::make_unique<ArrayList<>>();
private
  final ObjectPool<MPDebugInfo.MPRepopEvent> repopEventPool =
...
private
  static native boolean n_hasData(boolean var0);

private
  static native void n_requestData();

private
  static native int n_getLoadedCellsCount();

private
  static native int n_getLoadedCellsData(int var0, ByteBuffer var1);

private
  static native int n_getLoadedAreasCount();

private
  static native int n_getLoadedAreasData(int var0, ByteBuffer var1);

private
  static native int n_getRepopEventCount();

private
  static native int n_getRepopEventData(int var0, ByteBuffer var1);
  ...
  void addRepopEvent(int int0, int int1, float float1) {
    ...
    while (!this.repopEvents.isEmpty() &&
    ...
    if (GameClient.bClient &&
      ...
      for (Entry entry0 : GameClient.positions.entrySet()) {
        ...
        zombiePopulationRenderer.renderString(
            vector.x, vector.y,
            player0 == nullptr ? String.valueOf(entry0.getKey())
                               : player0.getUsername(),
