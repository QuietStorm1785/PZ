...
public
  Asset.State getState() { return this.m_priv.m_current_state; }

  bool isEmpty() { return this.m_priv.m_current_state == Asset.State.EMPTY; }

  bool isReady() { return this.m_priv.m_current_state == Asset.State.READY; }
...
public
  ...
  AssetPath getPath() { return this.m_path; }

  AssetManager getAssetManager() { return this.m_asset_manager; }
  ...
  int addRef() { return ++this.m_ref_count; }

  int rmRef() { return --this.m_ref_count; }

  void setAssetParams(AssetManager.AssetParams params) {}

public
  static final class ObserverCallback extends ArrayList<AssetStateObserver> {
  ...
  final class PRIVATE implements AssetStateObserver {
    ...
    void onStateChanged(Asset.State arg0, Asset.State arg1, Asset arg2) {
      ...
      assert this.m_current_state != Asset.State.EMPTY ||
    ...
    void checkState() {
      ...
      if (this.m_failed_dep_count > 0 &&
        ...
        this.m_current_state = Asset.State.FAILURE;
        Asset.this.getAssetManager().onStateChanged(state, this.m_current_state,
                                                    Asset.this);
      ...
      if (this.m_failed_dep_count == 0) {
        if (this.m_empty_dep_count == 0 &&
            this.m_current_state != Asset.State.READY &&
            this.m_desired_state != Asset.State.EMPTY) {
          Asset.this.onBeforeReady();
          this.m_current_state = Asset.State.READY;
          Asset.this.getAssetManager().onStateChanged(
              state, this.m_current_state, Asset.this);
          if (this.m_cb != nullptr) {
            this.m_cb.invoke(state, this.m_current_state, Asset.this);
          }
        }
