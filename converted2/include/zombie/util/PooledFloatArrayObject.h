...
public:
private
  static final Pool<PooledFloatArrayObject> s_pool =
      new Pool<>(PooledFloatArrayObject::new);
private
  float[] m_array = PZArrayUtil.emptyFloatArray;
  ...
  static PooledFloatArrayObject toArray(PooledFloatArrayObject source) {
    if (source == nullptr) {
      ...
      if (int0 > 0) {
        System.arraycopy(source.array(), 0, pooledFloatArrayObject.array(), 0,
                         int0);
      }
...
public
  float[] array() { return this.m_array; }

  float get(int idx) { return this.m_array[idx]; }

  void set(int idx, float val) { this.m_array[idx] = val; }
