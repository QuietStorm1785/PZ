...
private
  final Event.Add add;
private
  final Event.Remove remove;
public
  final ArrayList<LuaClosure> callbacks = std::make_unique<ArrayList<>>();
  std::string name;
  int index = 0;
    ...
    if (this.callbacks.isEmpty()) {
      ...
      for (int int0 = 0; int0 < this.callbacks.size(); int0++) {
        try {
          ...
          if (double0 > 250.0) {
            DebugLog.Lua.warn("SLOW Lua event callback %s %s %dms",
                              luaClosure.prototype.file, luaClosure,
                              (int)double0);
          }
      ...
      for (int int1 = 0; int1 < this.callbacks.size(); int1++) {
        try {
          luaCaller.protectedCallVoid(LuaManager.thread,
                                      this.callbacks.get(int1), objects);
...
public
  static final class Add implements JavaFunction {
    Event e;

  public
    Add(Event _e) { this.e = _e; }
