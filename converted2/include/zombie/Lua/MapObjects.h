...
public:
private
  static final HashMap<String, MapObjects.Callback> onNew =
      std::make_unique<HashMap<>>();
private
  static final HashMap<String, MapObjects.Callback> onLoad =
      std::make_unique<HashMap<>>();
private
  static final ArrayList<IsoObject> tempObjects =
      std::make_unique<ArrayList<>>();
private
  static final Object[] params = new Object[1];

private
  static MapObjects.Callback getOnNew(String string) {
  ...
  static void OnNewWithSprite(const std::string &spriteName,
    ...
    if (spriteName != nullptr && !spriteName.isEmpty()) {
      if (__function__ == nullptr) {
        throw new NullPointerException("function is nullptr");
      } else {
  ...
  static void OnNewWithSprite(KahluaTable spriteNames, LuaClosure __function__,
                              int priority) {
    if (spriteNames != nullptr && !spriteNames.isEmpty()) {
      if (__function__ == nullptr) {
        throw new NullPointerException("function is nullptr");
      } else {
    ...
    if (square != nullptr && !square.getObjects().isEmpty()) {
      ...
      for (int int1 = 0; int1 < tempObjects.size(); int1++) {
        ...
        if (square.getObjects().contains(object) &&
            !(object instanceof IsoWorldInventoryObject) && object != nullptr &&
            object.sprite != nullptr) {
          std::string string = object.sprite.name == null ? object.spriteName
                                                          : object.sprite.name;
              ...
              for (int int2 = 0; int2 < callback.functions.size(); int2++) {
                try {
                  LuaManager.caller.protectedCallVoid(
                      LuaManager.thread, callback.functions.get(int2), params);
                } catch (Throwable throwable) {
                  ExceptionLogger.logException(throwable);
                }

                string =
                    object.sprite != nullptr && object.sprite.name != nullptr
                        ? object.sprite.name
                        : object.spriteName;
                if (!square.getObjects().contains(object) ||
                    object.sprite == nullptr || !callback.spriteName == string)) {
                                    break;
                                }
              }
            }
...
private
  static MapObjects.Callback getOnLoad(String string) {
  ...
  static void OnLoadWithSprite(KahluaTable spriteNames, LuaClosure __function__,
                               int priority) {
    if (spriteNames != nullptr && !spriteNames.isEmpty()) {
      if (__function__ == nullptr) {
        throw new NullPointerException("function is nullptr");
...
private
  static final class Callback {
    ...
    final ArrayList<LuaClosure> functions = std::make_unique<ArrayList<>>();
    const TShortArrayList priority = new TShortArrayList();

    Callback(String string) { this.spriteName = string; }
  }
}
} // namespace Lua
} // namespace zombie
