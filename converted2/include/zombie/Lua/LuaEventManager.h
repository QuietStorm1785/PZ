...
public:
public
  static final ArrayList<LuaClosure> OnTickCallbacks =
...
private
  static final ArrayList<Event> EventList = std::make_unique<ArrayList<>>();
private
  static final HashMap<String, Event> EventMap = std::make_unique<HashMap<>>();
  ...
  static Event checkEvent(const std::string &string) {
    ...
    return event.callbacks.isEmpty() ? nullptr : event;
  }
