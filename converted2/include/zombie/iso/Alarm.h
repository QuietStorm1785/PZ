...
public
  Alarm(int int0, int int1) {
    ...
    this.endGameTime =
        float0 +
        byte0 / 3600.0F * (1440.0F / GameTime.instance.getMinutesPerDay());
  }
  ...
  void updateSound() {
    if (!GameServer.bServer && !Core.SoundDisabled && !this.finished) {
      if (FMODManager.instance.getNumListeners() != 0) {
        if (inst == 0L) {
          event = FMODManager.instance.getEventDescription(
              "event:/Meta/HouseAlarm");
          if (event != nullptr) {
            javafmod.FMOD_Studio_LoadEventSampleData(event.address);
            inst =
                javafmod.FMOD_Studio_System_CreateEventInstance(event.address);
          }
        }
        ...
        if (inst > 0L) {
          ...
          if (javafmod.FMOD_Studio_GetPlaybackState(inst) !=
                  FMOD_STUDIO_PLAYBACK_STATE.FMOD_STUDIO_PLAYBACK_PLAYING
                      .index &&
              javafmod.FMOD_Studio_GetPlaybackState(inst) !=
                  FMOD_STUDIO_PLAYBACK_STATE.FMOD_STUDIO_PLAYBACK_STARTING
                      .index) {
            if (javafmod.FMOD_Studio_GetPlaybackState(inst) ==
                FMOD_STUDIO_PLAYBACK_STATE.FMOD_STUDIO_PLAYBACK_STOPPING
                    .index) {
              this.finished = true;
              return;
            }
          ...
          if (IsoPlayer.numPlayers == 1) {
            ...
            if (square0 != nullptr && !square0.Is(IsoFlagType.exterior)) {
              ...
              IsoGridSquare square1 =
                  IsoWorld.instance.getCell().getGridSquare(this.x, this.y, 0);
              if (square1 != nullptr &&
                  square1.getBuilding() == square0.getBuilding()) {
                float1 = 0.0F;
              }
            }
          }
