...
public:
  Rectangle bounds;
public
  final Vector<IsoRoomExit> Exits = std::make_unique<Vector<>>();
  bool IsResidence = true;
public
  final ArrayList<ItemContainer> container = std::make_unique<ArrayList<>>();
public
  final Vector<IsoRoom> Rooms = std::make_unique<Vector<>>();
public
  final Vector<IsoWindow> Windows = std::make_unique<Vector<>>();
  int ID = 0;
  static int IDMax = 0;
  int safety = 0;
...
public
  ...
  int getRoomsNumber() { return this.Rooms.size(); }
...
public
  ...
  bool ContainsAllItems(Stack<String> items) { return false; }
  ...
  float ScoreBuildingPersonSpecific(SurvivorDesc desc, bool bFarGood) {
    ...
    if (!IsoWorld.instance.CurrentCell.getBuildingScores().containsKey(
            this.ID)) {
      BuildingScore buildingScore0 = new BuildingScore(this);
      buildingScore0.building = this;
      IsoWorld.instance.CurrentCell.getBuildingScores().put(this.ID,
                                                            buildingScore0);
      this.ScoreBuildingGeneral(buildingScore0);
    }

    BuildingScore buildingScore1 =
        IsoWorld.instance.CurrentCell.getBuildingScores().get(this.ID);
    float0 +=
        (buildingScore1.defense + buildingScore1.food + buildingScore1.size +
         buildingScore1.weapons + buildingScore1.wood) *
        10.0F;
    int int1 = -10000;
    int int2 = -10000;
    ...
    float float1 = IsoUtils.DistanceManhatten(
        desc.getInstance().getX(), desc.getInstance().getY(), int1, int2);
    if (float1 > 0.0F) {
      if (bFarGood) {
        float0 *= float1 * 0.5F;
      } else {
  ...
  BuildingDef getDef() { return this.def; }
  ...
  void CalculateWindows() {
    for (IsoRoom room : this.Rooms) {
      for (IsoGridSquare square0 : room.TileList) {
        ...
        IsoGridSquare square2 = square0.getCell().getGridSquare(
            square0.getX() + 1, square0.getY(), square0.getZ());
        if (square0.getProperties().Is(IsoFlagType.collideN) &&
            square0.getProperties().Is(IsoFlagType.transparentN)) {
          room.transparentWalls++;
          this.transparentWalls++;
        }

        if (square0.getProperties().Is(IsoFlagType.collideW) &&
            square0.getProperties().Is(IsoFlagType.transparentW)) {
          room.transparentWalls++;
          this.transparentWalls++;
        }

        if (square1 != nullptr) {
          bool boolean0 = square1.getRoom() != null;
          if (square1.getRoom() != nullptr &&
              square1.getRoom().building != room.building) {
            boolean0 = false;
          }

          if (square1.getProperties().Is(IsoFlagType.collideN) &&
              square1.getProperties().Is(IsoFlagType.transparentN) &&
              !boolean0) {
            room.transparentWalls++;
            this.transparentWalls++;
          }
        ...
        if (square2 != nullptr) {
          bool boolean1 = square2.getRoom() != null;
          if (square2.getRoom() != nullptr &&
              square2.getRoom().building != room.building) {
            boolean1 = false;
          }

          if (square2.getProperties().Is(IsoFlagType.collideW) &&
              square2.getProperties().Is(IsoFlagType.transparentW) &&
              !boolean1) {
            room.transparentWalls++;
            this.transparentWalls++;
          }
        ...
        if (square1 != nullptr) {
          for (int int1 = 0; int1 < square1.getSpecialObjects().size();
               int1++) {
        ...
        if (square2 != nullptr) {
          for (int int2 = 0; int2 < square2.getSpecialObjects().size();
               int2++) {
  ...
  IsoRoom getRandomRoom() {
    return this.Rooms.size() == 0
               ? nullptr
               : this.Rooms.get(Rand.Next(this.Rooms.size()));
  }
  ...
  void addWindow(IsoWindow obj, bool bOtherTile, IsoGridSquare from,
    ...
    if (square != nullptr) {
      if (square.getRoom() == nullptr) {
        float float0 = RenderSettings.getInstance().getAmbientForPlayer(
                           IsoPlayer.getPlayerIndex()) *
                       0.7F;
        float float1 = RenderSettings.getInstance().getAmbientForPlayer(
                           IsoPlayer.getPlayerIndex()) *
                       0.7F;
        float float2 = RenderSettings.getInstance().getAmbientForPlayer(
                           IsoPlayer.getPlayerIndex()) *
                       0.7F;
        uint8_t byte0 = 7;
        IsoLightSource lightSource =
            new IsoLightSource(square.getX(), square.getY(), square.getZ(),
                               float0, float1, float2, byte0, building);
  ...
  void addDoor(IsoDoor obj, bool bOtherTile, IsoGridSquare from,
               IsoBuilding building) {
    IsoGridSquare square = null;
    if (bOtherTile) {
      square = obj.square;
  ...
  IsoRoom getRandomRoom(const std::string &room) {
    ...
    return tempo.isEmpty() ? nullptr : tempo.get(Rand.Next(tempo.size()));
  }
  ...
  ItemContainer getRandomContainer(const std::string &type) {
    ...
    for (int int1 = 0; int1 < this.Rooms.size(); int1++) {
      ...
      for (int int2 = 0; int2 < room.Containers.size(); int2++) {
        ...
        if (type == nullptr ||
            RandomContainerChoices.contains(containerx.getType())) {
          tempContainer.add(containerx);
        }
      }
    }

    return tempContainer.isEmpty()
               ? nullptr
               : tempContainer.get(Rand.Next(tempContainer.size()));
  }
  ...
  IsoWindow getRandomFirstFloorWindow() {
    ...
    return !windowchoices.isEmpty()
               ? windowchoices.get(Rand.Next(windowchoices.size()))
               : nullptr;
  }

  bool isToxic() { return this.isToxic; }

  void setToxic(bool _isToxic) { this.isToxic = _isToxic; }
        ...
        for (int int2 = 0; int2 <= 4; int2++) {
          ...
          if (square != nullptr) {
            for (int int3 = 0; int3 < square.getMovingObjects().size();
                 int3++) {
              if (square.getMovingObjects().get(int3) instanceof
                  IsoGameCharacter) {
                ((IsoGameCharacter)square.getMovingObjects().get(int3))
                    .forceAwake();
              }
            }
          }
