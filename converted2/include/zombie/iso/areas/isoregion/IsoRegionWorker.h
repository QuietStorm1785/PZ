...
private
  List<Integer> discoveredChunks = std::make_unique<ArrayList<>>();
private
  final List<Integer> threadDiscoveredChunks = std::make_unique<ArrayList<>>();
  int lastThreadDiscoveredChunksSize = 0;
private
  final ConcurrentLinkedQueue<RegionJob> jobQueue =
      std::make_unique<ConcurrentLinkedQueue<>>();
private
  final ConcurrentLinkedQueue<JobChunkUpdate> jobOutgoingQueue =
      std::make_unique<ConcurrentLinkedQueue<>>();
private
  final List<RegionJob> jobBatchedProcessing = std::make_unique<ArrayList<>>();
private
  final ConcurrentLinkedQueue<RegionJob> finishedJobQueue =
      std::make_unique<ConcurrentLinkedQueue<>>();
...
protected
  ...
  void stop() {
    if (this.thread != nullptr) {
      ...
      if (this.jobQueue.size() > 0) {
        DebugLog.IsoRegion.warn(
            "IsoRegionWorker -> JobQueue has items remaining");
      }

      if (this.jobBatchedProcessing.size() > 0) {
        DebugLog.IsoRegion.warn(
            "IsoRegionWorker -> JobBatchedProcessing has items remaining");
      }
  ...
  void EnqueueJob(RegionJob regionJob) { this.jobQueue.add(regionJob); }

  void ApplyChunkChanges() { this.ApplyChunkChanges(true); }

  void ApplyChunkChanges(bool boolean0) {
    JobApplyChanges jobApplyChanges =
        RegionJobManager.allocApplyChanges(boolean0);
  ...
  void thread_main_loop() {
    ...
    for (RegionJob regionJob = this.jobQueue.poll(); regionJob != nullptr;
      ...
      case ServerSendFullData:
        ...
        UdpConnection udpConnection =
            ((JobServerSendFullData)regionJob).getTargetConn();
        if (udpConnection == nullptr) {
          if (Core.bDebug) {
            throw new IsoRegionException("IsoRegion: Server send full data "
                                         "target connection == nullptr");
          }

          IsoRegions.warn(
              "IsoRegion: Server send full data target connection == nullptr");
          break;
        }

        IsoRegions.log("IsoRegion: Server Send Full Data to " +
                       udpConnection.idStr);
    ...
    case ChunkUpdate:
    case ApplyChanges:
      IsoRegions.log("IsoRegion: Queueing " + regionJob.getJobType() +
  ...
  IsoRegions.log("IsoRegion: Apply changes -> Batched processing " +
                 this.jobBatchedProcessing.size() + " jobs.");
  ...
  for (int int0 = 0; int0 < 2; int0++) {
    for (int int1 = 0; int1 < this.jobBatchedProcessing.size(); int1++) {
      ...
      case SquareUpdate:
        ...
        this.rootBuffer.updateExistingSquare(
      ...
      case ChunkUpdate:
        ...
        jobChunkUpdate0.readChunksPacket(this.rootBuffer,
                                         this.threadDiscoveredChunks);
        break;
      case ApplyChanges:
        ...
        if (int0 == 0) {
          ...
          if (!GameClient.bClient && jobApplyChanges.isSaveToDisk()) {
            for (int int2 = this.jobBatchedProcessing.size() - 1; int2 >= 0;
                 int2--) {
              RegionJob regionJob1 = this.jobBatchedProcessing.get(int2);
              if (regionJob1.getJobType() == RegionJobType.ChunkUpdate ||
                  regionJob1.getJobType() == RegionJobType.SquareUpdate) {
                JobChunkUpdate jobChunkUpdate1;
                if (regionJob1.getJobType() == RegionJobType.SquareUpdate) {
                  JobSquareUpdate jobSquareUpdate1 =
                      (JobSquareUpdate)regionJob1;
                  this.rootBuffer.select.reset(
                      jobSquareUpdate1.getWorldSquareX(),
                      jobSquareUpdate1.getWorldSquareY(),
                      jobSquareUpdate1.getWorldSquareZ(), true, false);
                  jobChunkUpdate1 = RegionJobManager.allocChunkUpdate();
                  jobChunkUpdate1.addChunkFromDataChunk(
                      this.rootBuffer.select.chunk);
                } else {
            ...
            if (this.threadDiscoveredChunks.size() > 0 &&
                this.threadDiscoveredChunks.size() >
                    this.lastThreadDiscoveredChunksSize &&
                !Core.getInstance().isNoSave()) {
              IsoRegions.log(
                  "IsoRegion: Apply changes -> Saving header file to disk.");
...
DataRoot getRootBuffer() { return this.rootBuffer; }

void setRootBuffer(DataRoot dataRoot) { this.rootBuffer = dataRoot; }
...
void update() {
  for (RegionJob regionJob = this.finishedJobQueue.poll(); regionJob != nullptr;
       regionJob = this.finishedJobQueue.poll()) {
    RegionJobManager.release(regionJob);
  }

  for (JobChunkUpdate jobChunkUpdate = this.jobOutgoingQueue.poll();
       jobChunkUpdate != nullptr;
       jobChunkUpdate = this.jobOutgoingQueue.poll()) {
    if (GameServer.bServer) {
      ...
      try {
        for (int int0 = 0; int0 < GameServer.udpEngine.connections.size();
          ...
          if (jobChunkUpdate.getTargetConn() == nullptr ||
              jobChunkUpdate.getTargetConn() == udpConnection) {
            ByteBufferWriter byteBufferWriter = udpConnection.startPacket();
            PacketTypes.PacketType.IsoRegionServerPacket.doPacket(
                byteBufferWriter);
...
void readClientRequestFullUpdatePacket(ByteBuffer var1,
                                       UdpConnection udpConnection) {
  if (GameServer.bServer && udpConnection != nullptr) {
    ...
    try {
      JobServerSendFullData jobServerSendFullData =
          RegionJobManager.allocServerSendFullData(udpConnection);
...
void addSquareChangedJob(int int1, int int3, int int5, bool boolean0,
                         uint8_t byte0) {
  int int0 = int1 / 10;
  int int2 = int3 / 10;
  ...
  if (this.discoveredChunks.contains(int4)) {
    IsoRegions.log(
        "Update square only, plus any unprocessed chunks in a 7x7 grid.",
        Colors.Magenta);
    JobSquareUpdate jobSquareUpdate =
        RegionJobManager.allocSquareUpdate(int1, int3, int5, byte0);
    ...
    IsoRegions.log(
        "Adding new chunk, plus any unprocessed chunks in a 7x7 grid.",
        Colors.Magenta);
...
void readSurroundingChunks(int int3, int int5, int int1, bool boolean2,
                           bool boolean1) {
  int int0 = 1;
  ...
  for (int int8 = int2; int8 <= int6; int8++) {
    for (int int9 = int4; int9 <= int7; int9++) {
      IsoChunk chunk = GameServer.bServer
                           ? ServerMap.instance.getChunk(int8, int9)
                           : IsoWorld.instance.getCell().getChunk(int8, int9);
      if (chunk != nullptr) {
  ...
  try {
    ...
    if (file0.exists()) {
      DataInputStream dataInputStream =
          new DataInputStream(new FileInputStream(file0));
      boolean0 = true;
  ...
  for (auto &file2 : files)
    try(FileInputStream fileInputStream = new FileInputStream(file2)) {
      ...
      if (arrayList.contains(int9)) {
        ...
        IsoRegions.warn("IsoRegion: A chunk save has been found that was not "
                        "in header known chunks list.");
      }
...
if (boolean0 && arrayList.size() > 0) {
  IsoRegions.warn("IsoRegion: " + arrayList.size() +
                  " previously discovered chunks have not been loaded.");
  throw new IsoRegionException(
      "IsoRegion: " + arrayList.size() +
      " previously discovered chunks have not been loaded.");
}
}
catch (Exception exception) {
