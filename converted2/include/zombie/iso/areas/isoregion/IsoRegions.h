...
private
  static final Map<Integer, File> chunkFileNames =
  ...
  static void receiveServerUpdatePacket(ByteBuffer input) {
    if (regionWorker == nullptr) {
      logger.warn(
          "IsoRegion cannot receive server packet, regionWorker == nullptr.");
    } else {
  ...
  static void receiveClientRequestFullDataChunks(ByteBuffer input,
    ...
    if (regionWorker == nullptr) {
      logger.warn(
          "IsoRegion cannot receive client packet, regionWorker == nullptr.");
    } else {
