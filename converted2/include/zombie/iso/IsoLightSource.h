...
public
  ArrayList<IsoLightSwitch> switches = new ArrayList<>(0);
  IsoChunk chunk;
  void *lightMap;

public
  IsoLightSource(int _x, int _y, int _z, float _r, float _g, float _b,
                 int _radius) {
    this.x = _x;
    this.y = _y;
    this.z = _z;
...
public
  IsoLightSource(int _x, int _y, int _z, float _r, float _g, float _b,
                 int _radius, IsoBuilding building) {
    this.x = _x;
    this.y = _y;
    this.z = _z;
    this.r = _r;
...
public
  ...
  void update() {
    ...
    if (!this.bHydroPowered || IsoWorld.instance.isHydroPowerOn() ||
      ...
      if (this.bActive) {
        ...
        for (int int0 = this.x - this.radius; int0 < this.x + this.radius;
             int0++) {
          for (int int1 = this.y - this.radius; int1 < this.y + this.radius;
               int1++) {
            for (int int2 = 0; int2 < 8; int2++) {
              square =
                  IsoWorld.instance.CurrentCell.getGridSquare(int0, int1, int2);
              if (square != nullptr &&
                  (this.localToBuilding == nullptr ||
                   this.localToBuilding == square.getBuilding())) {
                LosUtil.TestResults testResults = LosUtil.lineClear(
                    square.getCell(), this.x, this.y, this.z, square.getX(),
                    square.getY(), square.getZ(), false);
                if (square.getX() == this.x && square.getY() == this.y &&
                  ...
                  if (Math.abs(square.getZ() - this.z) <= 1) {
                    float1 =
                        IsoUtils.DistanceTo(this.x, this.y, 0.0F, square.getX(),
                                            square.getY(), 0.0F);
                  } else {
                    float1 = IsoUtils.DistanceTo(this.x, this.y, this.z,
   ...
   */
  void setActive(bool _bActive) { this.bActive = _bActive; }

  /**
   * @return the bWasActive
   */
  bool wasActive() { return this.bWasActive; }

  /**
   *
   * @param _bWasActive the bWasActive to set
   */
  void setWasActive(bool _bWasActive) { this.bWasActive = _bWasActive; }

  /**
   * @return the switches
   */
public
  ArrayList<IsoLightSwitch> getSwitches() { return this.switches; }
  ...
  bool isHydroPowered() { return this.bHydroPowered; }

  IsoBuilding getLocalToBuilding() { return this.localToBuilding; }
}
} // namespace iso
} // namespace zombie
