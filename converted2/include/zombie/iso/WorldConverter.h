...
public
  ...
  void convertchunks(const std::string &string0) {
    ...
    for (auto &string1 : strings0)
      if (string1.contains(".bin") && !string1 == "map.bin")
                && !string1 == "map_p.bin")
                ...
                && !string1.startsWith("chunkdata_")) {
                  String[] strings1 = string1.replace(".bin", "")
                                          .replace("map_", "")
                                          .split("_");
    ...
    if (string1.contains(".bin") && !string1 == "map.bin")
                ...
                && !string1.startsWith("chunkdata_")) {
                  ...
                  File file1 = new File(
                      ...
                      File.separator + string0 + File.separator + string1);
  ...
  if (file.exists()) {
    try {
      RandomAccessFile randomAccessFile =
          new RandomAccessFile(file.getAbsolutePath(), "r");
      ...
      for (int int2 = 0; int2 < int1; int2++) {
        ...
        for (int int6 = 0; int6 < int5; int6++) {
          IsoSprite sprite =
              IsoSpriteManager.instance.NamedMap.get(string3 + "_" + int6);
...
void softreset() {
  ...
  if (CoopSlave.instance != nullptr) {
    CoopSlave.instance.sendMessage("softreset-count", nullptr,
                                   Integer.toString(strings0.length));
  }
  ...
  if (string1.startsWith("zpop_")) {
    ...
    String[] strings1 =
        string1.replace(".bin", "").replace("map_", "").split("_");
    ...
    if (CoopSlave.instance != nullptr && int0 % 10 == 0) {
      CoopSlave.instance.sendMessage("softreset-remaining", nullptr,
                                     Integer.toString(int0));
    }
  }
}
