...
public
  static final HashMap<Integer, IsoChunk> SharedChunks =
...
public
  static final int[] SWorldX = new int[4];
public
  static final int[] SWorldY = new int[4];
public
  static final ConcurrentLinkedQueue<IsoChunk> chunkStore =
...
private
  static final ArrayList<IsoChunk> saveList = std::make_unique<ArrayList<>>();
private
  static final ArrayList<ArrayList<IsoFloorBloodSplat>> splatByType =
...
public
  final ArrayList<String> filenameServerRequests =
      std::make_unique<ArrayList<>>();
protected
  IsoChunk[] chunksSwapB;
protected
  IsoChunk[] chunksSwapA;
...
public
  ...
  void processAllLoadGridSquare() {
    for (IsoChunk chunk = IsoChunk.loadGridSquare.poll(); chunk != nullptr;
        ...
        if (chunk != nullptr && chunk.bLoaded) {
          ...
          if (list != nullptr && chunk.vehicles.size() != list.size()) {
            for (int int2 = 0; int2 < list.size(); int2++) {
              ...
              if (!boolean0 &&
                  VehicleManager.instance.getVehicleByID(short0) == nullptr) {
                VehicleManager.instance.sendRequestGetFull(
                    short0, PacketTypes.PacketType.Vehicles);
              }
            }
          }
        }
  ...
  void checkIntegrity() {
    ...
    for (int int0 =
             IsoWorld.instance.CurrentCell.ChunkMap[0].getWorldXMinTiles();
         int0 < IsoWorld.instance.CurrentCell.ChunkMap[0].getWorldXMaxTiles();
         int0++) {
      for (int int1 =
               IsoWorld.instance.CurrentCell.ChunkMap[0].getWorldYMinTiles();
        ...
        if (square != nullptr &&
            (square.getX() != int0 || square.getY() != int1)) {
          int int2 = int0 / 10;
          int int3 = int1 / 10;
          ...
          ((IsoChunk)object)
              .refs.add(IsoWorld.instance.CurrentCell.ChunkMap[0]);
          WorldStreamer.instance.addJob((IsoChunk)object, int0 / 10, int1 / 10,
                                        false);
    ...
    for (int int0 =
      ...
      for (int int1 =
        ...
        if (square != nullptr &&
          ...
          WorldStreamer.instance.addJobInstant(chunk, int0, int1, int0 / 10,
                                               int1 / 10);
        }
  ...
  IsoChunk getChunkForGridSquare(int x, int y) {
    ...
    return !this.isTileOutOfrange(x) && !this.isTileOutOfrange(y)
               ? this.getChunk(tileToChunk(x), tileToChunk(y))
               : nullptr;
  }
  ...
  IsoChunk getChunkCurrent(int x, int y) {
    if (x < 0 || x >= ChunkGridWidth || y < 0 || y >= ChunkGridWidth) {
      ...
      return !this.bReadBufferA ? this.chunksSwapA[ChunkGridWidth * y + x]
                                : this.chunksSwapB[ChunkGridWidth * y + x];
    }
  }
  ...
  void setGridSquare(IsoGridSquare square, int x, int y, int z) {
    ...
    if (!this.isTileOutOfrange(int0) && !this.isTileOutOfrange(int1) &&
        !this.isGridSquareOutOfRangeZ(z)) {
      ...
      if (chunk != nullptr) {
        ...
        chunk.setSquare(this.tileToGridSquare(int0),
                        this.tileToGridSquare(int1), z, square);
      }
    }
  }
  ...
  IsoGridSquare getGridSquareDirect(int tileX, int tileY, int tileZ) {
    if (!this.isTileOutOfrange(tileX) && !this.isTileOutOfrange(tileY) &&
        !this.isGridSquareOutOfRangeZ(tileZ)) {
      IsoChunk chunk = this.getChunk(tileToChunk(tileX), tileToChunk(tileY));
      return chunk = = null ? null
                            : chunk.getGridSquare(this.tileToGridSquare(tileX),
                                                  this.tileToGridSquare(tileY),
                                                  tileZ);
    } else {
      return null;
    }
  ...
  int tileToGridSquare(int int0) { return int0 % 10; }

  static int tileToChunk(int int0) { return int0 / 10; }

  bool isTileOutOfrange(int int0) {
    return int0 < 0 || int0 >= this.getWidthInTiles();
  }

  bool isGridSquareOutOfRangeZ(int int0) { return int0 < 0 || int0 >= 8; }
  ...
  bool setChunkDirect(IsoChunk c, bool bRequireLock) {
    ...
    if (!c.refs.isEmpty() && int0 >= 0 && int1 >= 0 && int0 < ChunkGridWidth &&
        int1 < ChunkGridWidth) {
      try {
        if (this.bReadBufferA) {
        ...
        if (LightingThread.DebugLockTime && double1 > 10.0) {
          DebugLog.log("setChunkDirect time " + double0 + "/" + double1 +
                       " ms");
        }
      } finally {
        if (bRequireLock) {
        ...
        if (chunk != nullptr) {
          ...
          if (square == nullptr) {
            TextManager.instance.DrawString(
                short0, byte0, "wx:" + chunk.wx + " wy:" + chunk.wy);
          }
        }
      }
  ...
  void LoadLeft() {
    ...
    for (int int0 = -(ChunkGridWidth / 2); int0 <= ChunkGridWidth / 2; int0++) {
      this.LoadChunkForLater(this.WorldX - ChunkGridWidth / 2,
                             this.WorldY + int0, 0, int0 + ChunkGridWidth / 2);
    }
  ...
  void LoadRight() {
    ...
    for (int int0 = -(ChunkGridWidth / 2); int0 <= ChunkGridWidth / 2; int0++) {
      this.LoadChunkForLater(this.WorldX + ChunkGridWidth / 2,
                             this.WorldY + int0, ChunkGridWidth - 1,
                             int0 + ChunkGridWidth / 2);
    }
  ...
  void LoadUp() {
    ...
    for (int int0 = -(ChunkGridWidth / 2); int0 <= ChunkGridWidth / 2; int0++) {
      this.LoadChunkForLater(this.WorldX + int0,
                             this.WorldY - ChunkGridWidth / 2,
                             int0 + ChunkGridWidth / 2, 0);
    }
  ...
  void LoadDown() {
    ...
    for (int int0 = -(ChunkGridWidth / 2); int0 <= ChunkGridWidth / 2; int0++) {
      this.LoadChunkForLater(this.WorldX + int0,
                             this.WorldY + ChunkGridWidth / 2,
                             int0 + ChunkGridWidth / 2, ChunkGridWidth - 1);
    }
    ...
    for (int int1 = 0; int1 < int0; int1++) {
      for (int int2 = 0; int2 < int0; int2++) {
        for (int int3 = 0; int3 < 8; int3++) {
          IsoGridSquare square =
              this.getGridSquare(int1 + this.getWorldXMinTiles(),
                                 int2 + this.getWorldYMinTiles(), int3);
          IsoWorld.instance.CurrentCell.setCacheGridSquareLocal(
              int1, int2, int3, square, this.PlayerID);
        }
      }
    }
  }
  ...
  int getWorldXMin() { return this.WorldX - ChunkGridWidth / 2; }

  int getWorldYMin() { return this.WorldY - ChunkGridWidth / 2; }
  ...
  void ProcessChunkPos(IsoGameCharacter chr) {
    ...
    if (IsoPlayer.getInstance() != nullptr &&
        IsoPlayer.getInstance().getVehicle() != nullptr) {
    ...
    if (int0 != this.WorldX || int1 != this.WorldY) {
      ...
      try {
        if (Math.abs(int0 - this.WorldX) < ChunkGridWidth &&
            Math.abs(int1 - this.WorldY) < ChunkGridWidth) {
          if (int0 != this.WorldX) {
            if (int0 < this.WorldX) {
              this.LoadLeft();
            } else {
          ...
          if (LightingJNI.init) {
            LightingJNI.teleport(this.PlayerID, int0 - ChunkGridWidth / 2,
                                 int1 - ChunkGridWidth / 2);
          }
  ...
  IsoRoom getRoom(int iD) { return null; }

  int getWidthInTiles() { return ChunkWidthInTiles; }
    ...
    if (DebugOptions.instance.Terrain.RenderTiles.BloodDecals.getValue()) {
      if (!(zza > IsoCamera.CamCharacter.z)) {
        if (Core.OptionBloodDecals != 0) {
          ...
          for (int int1 = 0; int1 < IsoFloorBloodSplat.FloorBloodTypes.length;
               int1++) {
          ...
          for (int int2 = 0; int2 < ChunkGridWidth; int2++) {
            for (int int3 = 0; int3 < ChunkGridWidth; int3++) {
              ...
              if (chunk != nullptr) {
                for (int int4 = 0; int4 < chunk.FloorBloodSplatsFade.size();
                     int4++) {
                  IsoFloorBloodSplat floorBloodSplat0 =
                      chunk.FloorBloodSplatsFade.get(int4);
                  if ((floorBloodSplat0.index < 1 ||
                       floorBloodSplat0.index > 10 ||
                       IsoChunk.renderByIndex[Core.OptionBloodDecals - 1]
                                             [floorBloodSplat0.index - 1] !=
                           0) &&
                      (int)floorBloodSplat0.z == zza &&
                      floorBloodSplat0.Type >= 0 &&
                      floorBloodSplat0.Type <
                          IsoFloorBloodSplat.FloorBloodTypes.length) {
                    floorBloodSplat0.chunk = chunk;
                    splatByType.get(floorBloodSplat0.Type)
                        .add(floorBloodSplat0);
                  }
                }

                if (!chunk.FloorBloodSplats.isEmpty()) {
                  for (int int5 = 0; int5 < chunk.FloorBloodSplats.size();
                       int5++) {
                    IsoFloorBloodSplat floorBloodSplat1 =
                        chunk.FloorBloodSplats.get(int5);
                    if ((floorBloodSplat1.index < 1 ||
                        ...
                        floorBloodSplat1.Type <
                            IsoFloorBloodSplat.FloorBloodTypes.length) {
                      floorBloodSplat1.chunk = chunk;
                      splatByType.get(floorBloodSplat1.Type)
                          .add(floorBloodSplat1);
                    }
                  }
                }
              }
          ...
          for (int int6 = 0; int6 < splatByType.size(); int6++) {
            ...
            if (!arrayList.isEmpty()) {
              ...
              for (int int7 = 0; int7 < arrayList.size(); int7++) {
                IsoFloorBloodSplat floorBloodSplat2 =
                    (IsoFloorBloodSplat)arrayList.get(int7);
                inf.r = 1.0F;
                inf.g = 1.0F;
                inf.b = 1.0F;
                inf.a = 0.27F;
                float float1 = (floorBloodSplat2.x +
                                floorBloodSplat2.y / floorBloodSplat2.x) *
                               (floorBloodSplat2.Type + 1);
                float float2 =
                    float1 * floorBloodSplat2.x / floorBloodSplat2.y *
                    (floorBloodSplat2.Type + 1) / (float1 + floorBloodSplat2.y);
                float float3 = float2 * float1 * float2 * floorBloodSplat2.x /
                               (floorBloodSplat2.y + 2.0F);
                float1 *= 42367.543F;
                float2 *= 6367.123F;
                float3 *= 23367.133F;
                float1 %= 1000.0F;
                ...
                if (floorBloodSplat2.fade > 0) {
                  inf.a = inf.a * (floorBloodSplat2.fade /
                                   (PerformanceSettings.getLockFPS() * 5.0F));
                  if (--floorBloodSplat2.fade == 0) {
                    floorBloodSplat2.chunk.FloorBloodSplatsFade.remove(
                        floorBloodSplat2);
                  }
                }

                IsoGridSquare square = floorBloodSplat2.chunk.getGridSquare(
                    (int)floorBloodSplat2.x, (int)floorBloodSplat2.y,
                    (int)floorBloodSplat2.z);
                if (square != nullptr) {
  ...
  static {
    for (int int0 = 0; int0 < IsoFloorBloodSplat.FloorBloodTypes.length;
         int0++) {
