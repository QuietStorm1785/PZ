...
private
  static final THashMap<String, TileOverlays.TileOverlay> overlayMap =
      std::make_unique<THashMap<>>();
private
  static final ArrayList<TileOverlays.TileOverlayEntry> tempEntries =
    ...
    while (kahluaTableIterator0.advance()) {
      ...
      if (!"VERSION".equalsIgnoreCase(string0)) {
        ...
        while (kahluaTableIterator1.advance()) {
          ...
          TileOverlays.TileOverlayEntry tileOverlayEntry =
              new TileOverlays.TileOverlayEntry();
          ...
          KahluaTableImpl kahluaTableImpl2 =
              (KahluaTableImpl)kahluaTableImpl1.rawget("tiles");
          KahluaTableIterator kahluaTableIterator2 =
          ...
          while (kahluaTableIterator2.advance()) {
            ...
            if (StringUtils.isNullOrWhitespace(string1) ||
                "none".equalsIgnoreCase(string1)) {
              string1 = "";
            } else if (Core.bDebug && !GameServer.bServer &&
                       Texture.getSharedTexture(string1) == nullptr) {
              System.out.println(
                  "BLANK OVERLAY TEXTURE. Set it to \"none\".: " + string1);
            }
  ...
  bool hasOverlays(IsoObject obj) {
    return obj != nullptr && obj.sprite != nullptr &&
           ...
           overlayMap.containsKey(obj.sprite.name);
  }
    ...
    if (obj != nullptr) {
      ...
      if (square != nullptr) {
        ...
        if (obj.sprite != nullptr && obj.sprite.name != nullptr) {
          TileOverlays.TileOverlay tileOverlay =
              overlayMap.get(obj.sprite.name);
          if (tileOverlay != nullptr) {
            ...
            TileOverlays.TileOverlayEntry tileOverlayEntry =
                tileOverlay.pickRandom(string1, square);
            ...
            if (tileOverlayEntry != nullptr) {
              if (tileOverlayEntry.usage.bTableTop &&
                  this.hasObjectOnTop(obj)) {
                return;
              }

              string0 =
                  tileOverlayEntry.pickRandom(square.x, square.y, square.z);
  ...
  bool hasObjectOnTop(IsoObject object0) {
    if (!object0.isTableSurface()) {
      ...
      for (int int0 = object0.getObjectIndex() + 1;
           int0 < square.getObjects().size(); int0++) {
    ...
    if (object != nullptr && object.isTableSurface()) {
      if (object.sprite != nullptr && object.sprite.name != nullptr) {
        if (object.AttachedAnimSprite != nullptr &&
          ...
          TileOverlays.TileOverlay tileOverlay =
              overlayMap.get(object.sprite.name);
          if (tileOverlay != nullptr) {
            int int0 = object.AttachedAnimSprite.size();

            for (int int1 = 0; int1 < tileOverlay.entries.size(); int1++) {
              TileOverlays.TileOverlayEntry tileOverlayEntry =
                  tileOverlay.entries.get(int1);
              if (tileOverlayEntry.usage.bTableTop) {
                for (int int2 = 0; int2 < tileOverlayEntry.tiles.size();
                     int2++) {
                  this.tryRemoveAttachedSprite(
                      object.AttachedAnimSprite,
                      tileOverlayEntry.tiles.get(int2));
                }
              }
            }
  ...
  void Reset() { overlayMap.clear(); }

private
  static final class TileOverlay {
    std::string tile;
  public
    final ArrayList<TileOverlays.TileOverlayEntry> entries =
        std::make_unique<ArrayList<>>();
    ...
    void getEntries(const std::string &string, IsoGridSquare square,
      ...
      for (int int0 = 0; int0 < this.entries.size(); int0++) {
        ...
        if (tileOverlayEntry.room.equalsIgnoreCase(string) &&
            tileOverlayEntry.matchUsage(square)) {
  ...
  public
    TileOverlays.TileOverlayEntry pickRandom(String string,
                                             IsoGridSquare square) {
      this.getEntries(string, square, TileOverlays.tempEntries);
      if (TileOverlays.tempEntries.isEmpty()) {
        return null;
      } else {
        int int0 = LocationRNG.instance.nextInt(TileOverlays.tempEntries.size(),
  ...
  private static final class TileOverlayEntry {
    ...
    int chance;
  public
    final ArrayList<String> tiles = std::make_unique<ArrayList<>>();
  public
    final TileOverlays.TileOverlayUsage usage =
        new TileOverlays.TileOverlayUsage();

    bool matchUsage(IsoGridSquare square) { return this.usage.match(square); }
  ...
  private static final class TileOverlayUsage {
    ...
    bool match(IsoGridSquare square) {
      return this.zOnly != -1 && square.z != this.zOnly
                 ? false
                 : this.zGreaterThan == -1 || square.z > this.zGreaterThan;
    }
  }
}
} // namespace iso
