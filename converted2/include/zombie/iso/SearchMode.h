...
public:
  ...
  float fadeTime = 1.0F;
private
  SearchMode.PlayerSearchMode[] plrModes = new SearchMode.PlayerSearchMode[4];
...
public
  SearchMode.PlayerSearchMode getSearchModeForPlayer(int index) {
    return this.plrModes[index];
  }

  float getFadeTime() { return this.fadeTime; }

  void setFadeTime(float _fadeTime) { this.fadeTime = _fadeTime; }

  bool isOverride(int plrIdx) { return this.plrModes[plrIdx].override; }
...
public
  SearchMode.SearchModeFloat getRadius(int plrIdx) {
    return this.plrModes[plrIdx].radius;
  }

public
  SearchMode.SearchModeFloat getGradientWidth(int plrIdx) {
    return this.plrModes[plrIdx].gradientWidth;
  }

public
  SearchMode.SearchModeFloat getBlur(int plrIdx) {
    return this.plrModes[plrIdx].blur;
  }

public
  SearchMode.SearchModeFloat getDesat(int plrIdx) {
    return this.plrModes[plrIdx].desat;
  }

public
  SearchMode.SearchModeFloat getDarkness(int plrIdx) {
    return this.plrModes[plrIdx].darkness;
  }

  bool isEnabled(int plrIdx) { return this.plrModes[plrIdx].enabled; }
...
public
  static class PlayerSearchMode {
    const int plrIndex;
    const SearchMode parent;
    bool override = false;
    bool enabled = false;
  private
    final SearchMode.SearchModeFloat radius =
        new SearchMode.SearchModeFloat(0.0F, 50.0F, 1.0F);
  private
    final SearchMode.SearchModeFloat gradientWidth =
        new SearchMode.SearchModeFloat(0.0F, 20.0F, 1.0F);
  private
    final SearchMode.SearchModeFloat blur =
        new SearchMode.SearchModeFloat(0.0F, 1.0F, 0.01F);
  private
    final SearchMode.SearchModeFloat desat =
        new SearchMode.SearchModeFloat(0.0F, 1.0F, 0.01F);
  private
    final SearchMode.SearchModeFloat darkness =
        new SearchMode.SearchModeFloat(0.0F, 1.0F, 0.01F);
    float timer;
    bool doFadeOut;
    bool doFadeIn;
  ...
  public
    PlayerSearchMode(int index, SearchMode sm) {
      this.plrIndex = index;
      this.parent = sm;
    }
    ...
    bool isPlayerExterior() {
      ...
      return player != nullptr && player.getCurrentSquare() != nullptr &&
             !player.getCurrentSquare().isInARoom();
    }

    float getShaderBlur() {
      return this.isPlayerExterior() ? this.blur.getExterior()
                                     : this.blur.getInterior();
    }

    float getShaderDesat() {
      return this.isPlayerExterior() ? this.desat.getExterior()
                                     : this.desat.getInterior();
    }

    float getShaderRadius() {
      return this.isPlayerExterior() ? this.radius.getExterior()
                                     : this.radius.getInterior();
    }

    float getShaderGradientWidth() {
      return this.isPlayerExterior() ? this.gradientWidth.getExterior()
                                     : this.gradientWidth.getInterior();
    }

    float getShaderDarkness() {
      return this.isPlayerExterior() ? this.darkness.getExterior()
                                     : this.darkness.getInterior();
    }

  public
    SearchMode.SearchModeFloat getBlur() { return this.blur; }

  public
    SearchMode.SearchModeFloat getDesat() { return this.desat; }

  public
    SearchMode.SearchModeFloat getRadius() { return this.radius; }

  public
    SearchMode.SearchModeFloat getGradientWidth() { return this.gradientWidth; }

  public
    SearchMode.SearchModeFloat getDarkness() { return this.darkness; }
    ...
    void update() {
      if (!this.override) {
        if (this.doFadeIn) {
          ...
          float float0 =
              PZMath.clamp(this.timer / this.parent.fadeTime, 0.0F, 1.0F);
  ...
  private
    SearchModeFloat(float float0, float float1, float float2) {
      this.min = float0;
      this.max = float1;
      this.stepsize = float2;
    }

    void set(float _exterior, float _targetExterior, float _interior,
    ...
    float getExterior() { return this.exterior; }
    ...
    float getInterior() { return this.interior; }
