...
public:
public
  static final HashMap<String, LotHeader> InfoHeaders =
      std::make_unique<HashMap<>>();
public
  static final ArrayList<String> InfoHeaderNames =
      std::make_unique<ArrayList<>>();
public
  static final HashMap<String, String> InfoFileNames =
      std::make_unique<HashMap<>>();
public
  static final ObjectPool<IsoLot> pool = new ObjectPool<>(IsoLot::new);
  std::string m_lastUsedPath = "";
  int wx = 0;
  int wy = 0;
  final int[] m_offsetInData = new int[800];
  const TIntArrayList m_data = new TIntArrayList();
  RandomAccessFile m_in = null;
  LotHeader info;
...
public
  static synchronized void put(IsoLot lot) {
    lot.info = nullptr;
...
public
  static synchronized IsoLot get(Integer cX, Integer cY, Integer wX, Integer wY,
                                 IsoChunk ch) {
  ...
  void load(int cX, int cY, int wX, int wY, IsoChunk ch) {
    ...
    try {
      ...
      if (this.m_in == nullptr ||
          ...
          this.m_in =
              new BufferedRandomAccessFile(file.getAbsolutePath(), "r", 4096);
          this.m_lastUsedPath = file.getAbsolutePath();
        }
