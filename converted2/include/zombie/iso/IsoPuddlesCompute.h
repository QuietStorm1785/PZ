...
private
  static final ObjectPool<Vector3f> pool_vector3f =
      new ObjectPool<>(Vector3f::new);
private
  static final ArrayList<Vector3f> allocated_vector3f =
  ...
  static Vector3f allocVector3f(Vector3f vector3f) { return allocVector3f(); }

  static Vector3f floor(Vector3f vector3f) { return allocVector3f(); }

  static Vector3f fract(Vector3f vector3f) { return allocVector3f(); }
  ...
  static float FuncHash(Vector3f vector3f1) {
    Vector3f vector3f0 = allocVector3f(vector3f1.dot(HashVector31),
                                       vector3f1.dot(HashVector32), 0.0F);
    return fract();
  }
  ...
  static float FuncNoise(Vector3f vector3f1) {
    ...
    Vector3f vector3f3 =
        allocVector3f(vector3f2.x * vector3f2.x * (4.5F - 3.5F * vector3f2.x),
                      vector3f2.y * vector3f2.y * (4.5F - 3.5F * vector3f2.y),
                      vector3f2.z * vector3f2.z * (4.5F - 3.5F * vector3f2.z));
    float float0 =
        mix(FuncHash(vector3f0),
            FuncHash(allocVector3f(vector3f0).add(add_xyy)), vector3f3.x);
    float float1 =
        mix(FuncHash(allocVector3f(vector3f0).add(add_yxy)),
            FuncHash(allocVector3f(vector3f0).add(add_xxy)), vector3f3.x);
    float float2 =
        mix(FuncHash(allocVector3f(vector3f0).add(add_yyx)),
            FuncHash(allocVector3f(vector3f0).add(add_xyx)), vector3f3.x);
    float float3 =
        mix(FuncHash(allocVector3f(vector3f0).add(add_yxx)),
            FuncHash(allocVector3f(vector3f0).add(add_xxx)), vector3f3.x);
    ...
    if (hd_quality) {
      ...
      return (
          float)(float0 *
                 Math.min(
                     1.0,
                     2.0 * FuncNoise(allocVector3f(vector3f).mul(0.02F)) *
                         Math.min(1.0,
                                  1.0 * FuncNoise(allocVector3f(vector3f).mul(
                                            0.1F)))));
    } else {
      return FuncNoise();
    }
  ...
  static float getPuddles(Vector2f vector2f) {
    ...
    float3 =
        (float)(float3 + float0 *
                             Math.sin((vector2f.x * 1.0 + vector2f.y * 2.0) *
                                      3.1415F * 1.0) *
                             Math.cos((vector2f.x * 1.0 + vector2f.y * 2.0) *
                                      3.1415F * 1.0) *
                             2.0);
    float3 =
        (float)(float3 + float1 *
    ...
    float float4 =
        PerlinNoise(allocVector3f(vector2f.x * 1.0F, 0.0F, vector2f.y * 2.0F));
    float float5 = Math.min(0.7F, float3 * float4);
    float4 = Math.min(0.7F, PerlinNoise(allocVector3f(vector2f.x * 0.7F, 1.0F,
                                                      vector2f.y * 0.7F)));
  ...
  static float computePuddle(IsoGridSquare square) {
    ...
    if (!Core.getInstance().getUseShaders()) {
      return -0.1F;
    } else if (Core.getInstance().getPerfPuddlesOnLoad() == 3 ||
               Core.getInstance().getPerfPuddles() == 3) {
      return -0.1F;
      ...
      if (puddlesSize <= 0.0F) {
        ...
        float float0 =
            IsoUtils.XToScreen(square.x + 0.5F - square.z * 3.0F,
                               square.y + 0.5F - square.z * 3.0F, 0.0F, 0) -
            int0;
        float float1 =
            IsoUtils.YToScreen(square.x + 0.5F - square.z * 3.0F,
                               square.y + 0.5F - square.z * 3.0F, 0.0F, 0) -
            int1;
        ...
        if (Core.getInstance().getPerfPuddles() <= 1) {
          ...
          puddlesDirNE =
              (square.getPuddles().pdne[0] + square.getPuddles().pdne[2]) *
              0.5F;
          puddlesDirNW =
              (square.getPuddles().pdnw[0] + square.getPuddles().pdnw[2]) *
              0.5F;
          puddlesDirAll =
              (square.getPuddles().pda[0] + square.getPuddles().pda[2]) * 0.5F;
          puddlesDirNone =
              (square.getPuddles().pnon[0] + square.getPuddles().pnon[2]) *
              0.5F;
        } else {
          puddlesDirNE = 0.0F;
          puddlesDirNW = 0.0F;
        ...
        Vector2f vector2f = temp_vector2f.set(
            (float0 * vector4f.z + vector4f.x) * 8.0E-4F + square.z * 7.0F,
            (float1 * vector4f.w + vector4f.y) * 8.0E-4F + square.z * 7.0F);
