...
public:
  ...
  static void render(int int1, int int2, int int5) {
    ...
    if (TEXTURE != nullptr && TEXTURE.isReady()) {
      ...
      float float4 = IsoUtils.XToScreen(int1 + 0.5F, int2 + 0.5F, int5, 0) +
                     IsoSprite.globalOffsetX;
      float float5 = IsoUtils.YToScreen(int1 + 0.5F, int2 + 0.5F, int5, 0) +
      ...
      SpriteRenderer.instance.render(TEXTURE, float4, float5, int6, int6, 1.0F,
                                     1.0F, 1.0F, 1.0F, float0, float1, float2,
                                     float1, float2, float3, float0, float3);
    }
  }
  ...
  static void update() {
    if (!GameServer.bServer) {
      COUNTER =
          COUNTER +
          GameTime.getInstance().getRealworldSecondsSinceLastUpdate() * 1000.0F;
      float float0 = 20.0F;
  ...
  static void Reset() { TEXTURE = nullptr; }
}
} // namespace sprite
} // namespace iso
} // namespace zombie
