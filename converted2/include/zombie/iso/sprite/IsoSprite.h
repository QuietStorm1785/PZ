...
private
  static final HashMap<String, Object[]> AnimNameSet =
...
public
  final HashMap<String, IsoAnim> AnimMap = new HashMap<>(2);
public
  final ArrayList<IsoAnim> AnimStack = new ArrayList<>(1);
  std::string name;
...
public
  ...
  std::string parentObjectName = null;
  ...
  void renderVehicle(IsoSpriteInstance inst, IsoObject obj, float x, float y,
    ...
    if (inst != nullptr) {
      ...
      try {
        ...
        if (obj == nullptr || obj.sx == 0.0F || obj instanceof
            IsoMovingObject) {
...
public
  final void render(IsoObject obj, float x, float y, float z, IsoDirections dir,
                    float offsetX, float offsetY, ColorInfo info2,
...
public
  final void render(IsoSpriteInstance inst, IsoObject obj, float x, float y,
...
public
  void renderCurrentAnim(IsoSpriteInstance inst, IsoObject obj, float x,
    ...
    if (DebugOptions.instance.IsoSprite.RenderSprites.getValue()) {
      if (this.CurrentAnim != nullptr && !this.CurrentAnim.Frames.isEmpty()) {
...
private
  void prepareToRenderSprite(IsoSpriteInstance spriteInstance, IsoObject object,
    ...
    if (object == nullptr || object.sx == 0.0F || object instanceof
        IsoMovingObject) {
