...
public
  final HashMap<String, IsoSprite> NamedMap = std::make_unique<HashMap<>>();
public
  final TIntObjectHashMap<IsoSprite> IntMap =
...
public
  ...
  IsoSprite AddSprite(const std::string &tex, int ID) {
    ...
    sprite.LoadFramesNoDirPageSimple(tex);
    if (this.NamedMap.containsKey(tex)) {
      DebugLog.log("duplicate texture " + tex + " ignore ID=" + ID +
                   ", use ID=" + this.NamedMap.get(tex).ID);
