...
public
  static synchronized SkyBox getInstance() {
    if (instance == nullptr) {
    ...
    if (!this.isUpdated) {
      ...
      if (float3 < float0 || float3 > float1) {
        ...
        if (float3 > float1) {
          ...
          this.SkyHColourDusk.interp(this.SkyHColourDawn, float5,
                                     this.SkyBoxParamSkyHColour);
          this.SkyLColourDusk.interp(this.SkyLColourDawn, float5,
                                     this.SkyBoxParamSkyLColour);
          ...
          this.SkyHColourDusk.interp(this.SkyHColourDawn, float6,
                                     this.SkyBoxParamSkyHColour);
          this.SkyLColourDusk.interp(this.SkyLColourDawn, float6,
                                     this.SkyBoxParamSkyLColour);
          this.SkyBoxParamSunLight.set(0.35F, 0.22F, 0.3F);
          this.SkyBoxParamSunLight.normalize();
        ...
        this.SkyHColourDawn.interp(this.SkyHColourDay, float7,
                                   this.SkyBoxParamSkyHColour);
        this.SkyLColourDawn.interp(this.SkyLColourDay, float7,
                                   this.SkyBoxParamSkyLColour);
        ...
        this.SkyHColourDay.interp(this.SkyHColourDusk, float8,
                                  this.SkyBoxParamSkyHColour);
        this.SkyLColourDay.interp(this.SkyLColourDusk, float8,
                                  this.SkyBoxParamSkyLColour);
      ...
      this.SkyBoxParamSkyHColour.interp(this.SkyHColourNight,
                                        cm.getNightStrength(),
                                        this.SkyBoxParamSkyHColour);
      this.SkyBoxParamSkyLColour.interp(this.SkyLColourNight,
                                        cm.getNightStrength(),
                                        this.SkyBoxParamSkyLColour);
      this.SkyBoxParamCloudCount =
          Math.min(Math.max(cm.getCloudIntensity(),
                            cm.getPrecipitationIntensity() * 2.0F),
                   0.999F);
      ...
      this.SkyBoxParamCloudLight =
          (float)(1.0 - (1.0 - 1.0 * Math.pow(1000.0,
                                              -cm.getPrecipitationIntensity() -
                                                  cm.getNightStrength())));
      float float9 =
          (1.0F - (cm.getWindAngleIntensity() + 1.0F) * 0.5F + 0.25F) % 1.0F;
      float9 *= 360.0F;
      this.SkyBoxParamWind.set((float)Math.cos(Math.toRadians(float9)), 0.0F,
                               (float)Math.sin(Math.toRadians(float9)));
      ...
      if (!this.isSetAVG) {
        ...
        this.SkyBoxParamCloudCountAVG =
            this.SkyBoxParamCloudCountAVG +
            (this.SkyBoxParamCloudCount - this.SkyBoxParamCloudCountAVG) * 0.1F;
        this.SkyBoxParamCloudSizeAVG =
            this.SkyBoxParamCloudSizeAVG +
            (this.SkyBoxParamCloudSizeAVG + this.SkyBoxParamCloudSize) * 0.1F;
        this.SkyBoxParamSunLightAVG.lerp(this.SkyBoxParamSunLight, 0.1F);
        this.SkyBoxParamSunColorAVG.interp(this.SkyBoxParamSunColor, 0.1F,
                                           this.SkyBoxParamSunColorAVG);
        this.SkyBoxParamSkyHColourAVG.interp(this.SkyBoxParamSkyHColour, 0.1F,
                                             this.SkyBoxParamSkyHColourAVG);
        this.SkyBoxParamSkyLColourAVG.interp(this.SkyBoxParamSkyLColour, 0.1F,
                                             this.SkyBoxParamSkyLColourAVG);
        this.SkyBoxParamCloudLightAVG =
            this.SkyBoxParamCloudLightAVG +
            (this.SkyBoxParamCloudLight - this.SkyBoxParamCloudLightAVG) * 0.1F;
        this.SkyBoxParamStarsAVG =
            this.SkyBoxParamStarsAVG +
            (this.SkyBoxParamStars - this.SkyBoxParamStarsAVG) * 0.1F;
        this.SkyBoxParamFogAVG =
            this.SkyBoxParamFogAVG +
            (this.SkyBoxParamFog - this.SkyBoxParamFogAVG) * 0.1F;
  ...
  Color getShaderSunColor() { return this.SkyBoxParamSunColor; }

  Color getShaderSkyHColour() { return this.SkyBoxParamSkyHColour; }

  Color getShaderSkyLColour() { return this.SkyBoxParamSkyLColour; }

  float getShaderCloudLight() { return this.SkyBoxParamCloudLight; }

  float getShaderStars() { return this.SkyBoxParamStars; }

  float getShaderFog() { return this.SkyBoxParamFog; }

  Vector3f getShaderWind() { return this.SkyBoxParamWindINT; }

public
  SkyBox() {
    this.texAM = Texture.getSharedTexture(
        "media/textures/CMVehicleReflection/ref_am.png");
    this.texPM = Texture.getSharedTexture(
        "media/textures/CMVehicleReflection/ref_am.png");
  ...
  ITexture getTextureCurrent() {
    if (!Core.getInstance().getUseShaders() ||
        Core.getInstance().getPerfSkybox() == 2) {
      return this.texAM;
    } else {
      return this.isCurrentA ? this.textureFBOA.getTexture()
                             : this.textureFBOB.getTexture();
    }
  }
  ...
  ITexture getTexturePrev() {
    if (!Core.getInstance().getUseShaders() ||
      ...
      return this.isCurrentA ? this.textureFBOB.getTexture()
                             : this.textureFBOA.getTexture();
    }
  }
  ...
  float getTextureShift() {
    return Core.getInstance().getUseShaders() &&
                   Core.getInstance().getPerfSkybox() != 2
  ...
  void render() {
    if (Core.getInstance().getUseShaders() &&
      ...
      if (!this.renderLimit.Check()) {
        ...
        SpriteRenderer.instance.drawSkyBox(
            this.Effect, int0, this.apiId,
            this.getTextureFBOPrev().getBufferId());
  ...
  void draw() {
    if (Core.bDebug && DebugOptions.instance.SkyboxShow.getValue()) {
      ((Texture)this.getTextureCurrent())
          .render(0.0F, 0.0F, 512.0F, 512.0F, 1.0F, 1.0F, 1.0F, 1.0F, nullptr);
    }
  }
}
} // namespace sprite
