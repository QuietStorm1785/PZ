...
private
  static Map<Integer, String> roofGroups = std::make_unique<HashMap<>>();
private
  static IsoRoofFixer.PlaceFloorInfo[] placeFloorInfos =
      new IsoRoofFixer.PlaceFloorInfo[10000];
  static int floorInfoIndex = 0;
private
  static IsoGridSquare[] sqCache;
  static IsoRoom workingRoom;
private
  static int[] interiorAirSpaces;
  ...
  static void ensureCapacityFloorInfos() {
    if (floorInfoIndex == placeFloorInfos.length) {
      ...
      placeFloorInfos =
          new IsoRoofFixer.PlaceFloorInfo[placeFloorInfos.length + 400];
      System.arraycopy(placeFloorInfosx, 0, placeFloorInfos, 0,
                       placeFloorInfosx.length);
    }
  }
  ...
  static void setRoofTileCache(IsoObject object) {
    ...
    if (roofTileCache != sprite) {
      ...
      if (sprite != nullptr && sprite.getProperties() != nullptr &&
          sprite.getProperties().Val("RoofGroup") != nullptr) {
        try {
    ...
    if (roofTileGlassCacheDirty) {
      ...
      if (roofTileCache != nullptr) {
        ...
        if (propertyContainer != nullptr) {
          ...
          roofTileIsGlass =
              string != nullptr && string.equalsIgnoreCase("glass");
        }
      }
  ...
  static void FixRoofsPerRoomAt(IsoGridSquare square0) {
    floorInfoIndex = 0;
    if (square0.getZ() > 0 && !square0.TreatAsSolidFloor() &&
      ...
      if (room != nullptr && !room.def.isRoofFixed()) {
        ...
        for (int int0 = 0; int0 < arrayList.size(); int0++) {
          ...
          if (square2 != nullptr) {
            ...
            placeFloorInfos[floorInfoIndex++].set(square2,
                                                  roofTilePlaceFloorIndexCache);
          }
        }
    ...
    for (int int1 = 0; int1 < floorInfoIndex; int1++) {
      placeFloorInfos[int1].square.addFloor(
          roofGroups.get(placeFloorInfos[int1].floorType));
    }
  }
      ...
      label163: {
        if (int1 >= square0.getZ() + 1) {
          IsoGridSquare square1 =
              cell.getGridSquare(square0.x, square0.y, int1);
          if (square1 == nullptr) {
            if (int1 == square0.getZ() + 1 && int1 > 0 &&
                !isStairsBelow(square0.x, square0.y, int1)) {
              square1 = IsoGridSquare.getNew(cell, nullptr, square0.x,
                                             square0.y, int1);
        ...
        for (int int2 = 0; int2 < int0; int2++) {
          ...
          if (square3.getRoom() == nullptr &&
              isInteriorAirSpace(square3.getX(), square3.getY(),
                                 square3.getZ())) {
            return null;
          }
            ...
            for (int int4 = square3.y - 3; int4 <= square3.y + 3; int4++) {
              if (int3 != square3.x || int4 != square3.y) {
                IsoGridSquare square4 =
                    cell.getGridSquare(int3, int4, square3.z);
                if (square4 != nullptr) {
                  for (int int5 = 0; int5 < square4.getObjects().size();
                       int5++) {
                  ...
                  IsoGridSquare square5 =
                      cell.getGridSquare(square4.x, square4.y, square4.z + 1);
                  if (square5 != nullptr && square5.getObjects().size() > 0) {
                    for (int int6 = 0; int6 < square5.getObjects().size();
                         int6++) {
  ...
  static void FixRoofsPerTileAt(IsoGridSquare square) {
    if (square.getZ() > 0 && !square.TreatAsSolidFloor() &&
        square.getRoom() == nullptr && hasRoomBelow(square) &&
      ...
      for (int int0 = square0.x - 3; int0 <= square0.x + 3; int0++) {
        for (int int1 = square0.y - 3; int1 <= square0.y + 3; int1++) {
          if (int0 != square0.x || int1 != square0.y) {
            IsoGridSquare square1 =
                square0.getCell().getGridSquare(int0, int1, square0.z);
  ...
  static bool isObjectRoof(IsoObject object) {
    return object != nullptr && (object.getType() == IsoObjectType.WestRoofT ||
                                 object.getType() == IsoObjectType.WestRoofB ||
                                 object.getType() == IsoObjectType.WestRoofM);
  }
  ...
  static bool isObjectRoofFlatFloor(IsoObject object) {
    if (isObjectRoof(object)) {
      PropertyContainer propertyContainer = object.getProperties();
      if (propertyContainer != nullptr &&
          propertyContainer.Is(IsoFlagType.solidfloor)) {
        return !propertyContainer.Is(IsoFlagType.FloorHeightOneThird) &&
               !propertyContainer.Is(IsoFlagType.FloorHeightTwoThirds);
      }
    }
  ...
  static bool hasRoomBelow(IsoGridSquare square) { return getRoomBelow(); }
  ...
  static IsoRoom getRoomBelow(IsoGridSquare square0) {
    if (square0 == nullptr) {
      ...
      for (int int0 = square0.z - 1; int0 >= 0; int0--) {
        IsoGridSquare square1 =
            square0.getCell().getGridSquare(square0.x, square0.y, int0);
        if (square1 != nullptr) {
  ...
  static bool isInteriorAirSpace(int var0, int var1, int int0) {
    if (interiorAirSpaces[int0] != 0) {
      ...
      bool boolean0 = false;
      if (arrayList.size() > 0 &&
          int0 > ((IsoGridSquare)arrayList.get(0)).getZ()) {
        for (int int1 = 0; int1 < workingRoom.rects.size(); int1++) {
          ...
          for (int int2 = roomRect.getX(); int2 < roomRect.getX2(); int2++) {
            if (hasRailing(int2, roomRect.getY(), int0, IsoDirections.N) ||
                hasRailing(int2, roomRect.getY2() - 1, int0, IsoDirections.S)) {
              boolean0 = true;
              break;
            }
          }
          ...
          for (int int3 = roomRect.getY(); int3 < roomRect.getY2(); int3++) {
            if (hasRailing(roomRect.getX(), int3, int0, IsoDirections.W) ||
                hasRailing(roomRect.getX2() - 1, int3, int0, IsoDirections.E)) {
              boolean0 = true;
              break;
            }
          }
  ...
  static bool hasRailing(int int0, int int1, int int2,
...
private
  static final class PlaceFloorInfo {
