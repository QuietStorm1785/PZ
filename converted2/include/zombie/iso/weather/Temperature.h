...
public:
  ...
  static std::string getCelsiusPostfix() { return CELSIUS_POSTFIX; }

  static std::string getFahrenheitPostfix() { return FAHRENHEIT_POSTFIX; }

  static std::string getTemperaturePostfix() {
    return Core.OptionTemperatureDisplayCelsius ? CELSIUS_POSTFIX
                                                : FAHRENHEIT_POSTFIX;
  }

  static std::string getTemperatureString(float celsius) {
    float float0 = Core.OptionTemperatureDisplayCelsius
                       ? celsius
  ...
  static float WindchillCelsiusKph(float t, float v) {
    float float0 = 13.12F + 0.6215F * t - 11.37F * (float)Math.pow(v, 0.16F) +
                   0.3965F * t * (float)Math.pow(v, 0.16F);
  ...
  static float getFractionForRealTimeRatePerMin(float rate) {
    if (DO_DEFAULT_BASE) {
      return rate /
             (1440.0F / SandboxOptions.instance.getDayLengthMinutesDefault());
    } else if (!DO_DAYLEN_MOD) {
      return rate / (1440.0F / SandboxOptions.instance.getDayLengthMinutes());
    } else {
      float float0 = (float)SandboxOptions.instance.getDayLengthMinutes() /
                     SandboxOptions.instance.getDayLengthMinutesDefault();
      if (float0 < 1.0F) {
  ...
  static float getWindChillAmountForPlayer(IsoPlayer player) {
    if (player.getVehicle() == nullptr &&
        (player.getSquare() == nullptr || !player.getSquare().isInARoom())) {
