...
private
  static final int[] day_year =
      new int[]{-1, -1, 30, 58, 89, 119, 150, 180, 211, 241, 272, 303, 333};
private
  static final String[] moon_phase_name = new String[]{
      "New",  "Waxing crescent", "First quarter", "Waxing gibbous",
      "Full", "Waning gibbous",  "Third quarter", "Waning crescent"};
private
  static final float[] units =
      new float[]{0.0F, 0.25F, 0.5F, 0.75F, 1.0F, 0.75F, 0.5F, 0.25F};
  ...
  static ClimateMoon getInstance() { return instance; }
  ...
  static void updatePhase(int year, int month, int day) {
    if (year != last_year || month != last_month || day != last_day) {
      ...
      DebugLog.log("Updated MoonPhase = " + getPhaseName() +
                   ", float = " + current_float + ", int = " + current_phase);
    }
  }

  static std::string getPhaseName() { return moon_phase_name[current_phase]; }

  static float getMoonFloat() { return current_float; }

  int getCurrentMoonPhase() { return current_phase; }
  ...
  static int getMoonPhase(int int3, int int0, int int2) {
    ...
    int int5 = int3 % 19 + 1;
    int int6 =
        (11 * int5 + 20 + (8 * int4 + 5) / 25 - 5 - (3 * int4 / 4 - 12)) % 30;
    if (int6 <= 0) {
      int6 += 30;
    }
