...
public:
public
  static final ArrayList<MultiStageBuilding.Stage> stages =
      std::make_unique<ArrayList<>>();

public
  static ArrayList<MultiStageBuilding.Stage>
  getStages(IsoGameCharacter chr, IsoObject itemClicked, boolean cheat) {
    std::vector arrayList = new ArrayList();

    for (int int0 = 0; int0 < stages.size(); int0++) {
      MultiStageBuilding.Stage stage = stages.get(int0);
      if (stage.canBeDone(chr, itemClicked, cheat) &&
          !arrayList.contains(stage)) {
        arrayList.add(stage);
      }
    }
...
public
  class Stage {
    std::string name;
  public
    ArrayList<String> previousStage = std::make_unique<ArrayList<>>();
  ...
  public
    HashMap<String, Integer> xp = std::make_unique<HashMap<>>();
  public
    HashMap<String, Integer> perks = std::make_unique<HashMap<>>();
  public
    HashMap<String, Integer> items = std::make_unique<HashMap<>>();
  public
    ArrayList<String> itemsToKeep = std::make_unique<ArrayList<>>();
    ...
    std::string getNorthSprite() { return this.northSprite; }

    std::string getThumpSound() { return this.thumpSound; }

    std::string getRecipeName() { return this.recipeName; }

    std::string getKnownRecipe() { return this.knownRecipe; }

    int getTimeNeeded(IsoGameCharacter chr) {
      int int0 = this.timeNeeded;

      for (Entry entry : this.xp.entrySet()) {
        int0 -= chr.getPerkLevel(
                    PerkFactory.Perks.FromString((String)entry.getKey())) *
                10;
      }
  ...
  public
    ArrayList<String> getItemsToKeep() { return this.itemsToKeep; }

  public
    ArrayList<String> getPreviousStages() { return this.previousStage; }

    std::string getCraftingSound() { return this.craftingSound; }
    ...
    KahluaTable getPerksLua() {
      ...
      for (Entry entry : this.perks.entrySet()) {
        table.rawset(PerkFactory.Perks.FromString((String)entry.getKey()),
    ...
    void doStage(IsoGameCharacter chr, IsoThumpable item, bool removeItems) {
      ...
      IsoThumpable thumpable =
          new IsoThumpable(IsoWorld.instance.getCell(), item.square, string0,
                           item.north, item.getTable());
      ...
      if (this.bonusHealthSkill) {
        while (iterator.hasNext()) {
          ...
          int3 +=
              chr.getPerkLevel(PerkFactory.Perks.FromString(string1)) * byte0;
        }
      }
      ...
      for (String string3 : this.xp.keySet()) {
        chr.getXp().AddXP(PerkFactory.Perks.FromString(string3),
                          (float)this.xp.get(string3).intValue());
      }
      ...
      if (chr != nullptr &&
          !StringUtils.isNullOrWhitespace(this.completionSound)) {
    ...
    bool canBeDone(IsoGameCharacter chr, IsoObject itemClicked, bool cheat) {
      ...
      for (int int0 = 0; int0 < this.previousStage.size(); int0++) {
        if (this.previousStage.get(int0).equalsIgnoreCase(
                itemClicked.getName())) {
          boolean0 = true;
          break;
        }
