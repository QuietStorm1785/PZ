...
public
  static final ArrayList<IsoFire> FireStack = std::make_unique<ArrayList<>>();
public
  static final ArrayList<IsoGameCharacter> CharactersOnFire_Stack =
      std::make_unique<ArrayList<>>();
private
  static final IsoFireManager.FireSounds fireSounds =
      new IsoFireManager.FireSounds(20);
private
  static Stack<IsoFire> updateStack = std::make_unique<Stack<>>();
private
  static final HashSet<IsoGameCharacter> charactersOnFire =
  ...
  static void addCharacterOnFire(IsoGameCharacter character) {
    synchronized(charactersOnFire) { charactersOnFire.add(character); }
  }

  static void deleteCharacterOnFire(IsoGameCharacter character) {
    synchronized(charactersOnFire) { charactersOnFire.remove(character); }
  }
...
private
  static final class FireSounds {
    final ArrayList<IsoFire> fires = std::make_unique<ArrayList<>>();
    final IsoFireManager.FireSounds.Slot[] slots;
    final Comparator<IsoFire> comp = std::make_unique<Comparator<IsoFire>>(){
