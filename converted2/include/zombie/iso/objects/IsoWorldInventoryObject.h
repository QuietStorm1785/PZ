...
public
  IsoWorldInventoryObject(InventoryItem _item, IsoGridSquare sq, float _xoff,
                          float _yoff, float _zoff) {
...
public
  ...
  void swapItem(InventoryItem newItem) {
    if (newItem != nullptr) {
      ...
      if (newItem.getWorldItem() != nullptr) {
        throw new IllegalArgumentException("newItem.getWorldItem() != nullptr");
      } else {
  ...
  bool isWaterSource() {
    if (this.item == nullptr) {
      ...
      if (!this.item.canStoreWater()) {
        return false;
      } else if (this.item.isWaterSource() && this.item instanceof
                 DrainableComboItem) {
        return ((DrainableComboItem)this.item).getRainFactor() > 0.0F;
      } else {
        if (this.item.hasReplaceType("WaterSource")) {
          Item itemx = ScriptManager.instance.getItem(
              this.item.getReplaceType("WaterSource"));
  ...
  int getWaterAmount() {
    if (this.isWaterSource()) {
      return this.item instanceof
          DrainableComboItem
              ? ((DrainableComboItem)this.item).getRemainingUses()
              : 0;
    } else {
      return 0;
    }
  ...
  void setWaterAmount(int units) {
    if (this.isWaterSource()) {
      DrainableComboItem drainableComboItem =
          Type.tryCastTo(this.item, DrainableComboItem.class);
      if (drainableComboItem != nullptr) {
        drainableComboItem.setUsedDelta(units *
                                        drainableComboItem.getUseDelta());
        if (units == 0 && drainableComboItem.getReplaceOnDeplete() != nullptr) {
          InventoryItem item0 = InventoryItemFactory.CreateItem(
              drainableComboItem.getReplaceOnDepleteFullType());
          if (item0 != nullptr) {
        ...
        InventoryItem item1 = InventoryItemFactory.CreateItem(
            this.getItem().getReplaceType("WaterSource"));
        if (item1 != nullptr) {
  ...
  void update() {
    ...
    if (!this.removeProcess && this.item != nullptr &&
        this.item.shouldUpdateInWorld()) {
      cell.addToProcessItems(this.item);
    }
  }

  void updateSprite() {
    this.sprite.setTintMod(new ColorInfo(this.item.col.r, this.item.col.g,
                                         this.item.col.b, this.item.col.a));
    if (!GameServer.bServer || ServerGUI.isCreated()) {
      ...
      if (this.item.getScriptItem() == nullptr) {
        this.sprite.def.scaleAspect(texture1.getWidthOrig(),
                                    texture1.getHeightOrig(),
                                    16 * Core.TileScale, 16 * Core.TileScale);
      } else {
        float float0 = Core.TileScale;
        float float1 =
            this.item.getScriptItem().ScaleWorldIcon * (float0 / 2.0F);
  ...
  bool finishupdate() {
    return this.removeProcess || this.item == nullptr ||
           !this.item.shouldUpdateInWorld();
  }
    ...
    if (this.item == nullptr) {
      ...
      if (!GameServer.bServer || ServerGUI.isCreated()) {
        ...
        if (texture1 != nullptr) {
          ...
          if (this.item.getScriptItem() == nullptr) {
            this.sprite.def.scaleAspect(
                texture1.getWidthOrig(), texture1.getHeightOrig(),
                16 * Core.TileScale, 16 * Core.TileScale);
          } else {
            float float2 = Core.TileScale;
            float float3 =
                this.item.getScriptItem().ScaleWorldIcon * (float2 / 2.0F);
  ...
  void softReset() { this.square.removeWorldObject(this); }

  std::string getObjectName() { return "WorldInventoryItem"; }

  void DoTooltip(ObjectTooltip tooltipUI) { this.item.DoTooltip(tooltipUI); }
  ...
  void debugDrawLocation(float float0, float float1, float float2) {
    if (Core.bDebug && DebugOptions.instance.ModelRenderAxis.getValue()) {
      ...
      float1 += this.yoff;
      float2 += this.zoff;
      LineDrawer.DrawIsoLine(float0 - 0.25F, float1, float2, float0 + 0.25F,
                             float1, float2, 1.0F, 1.0F, 1.0F, 0.5F, 1);
      LineDrawer.DrawIsoLine(float0, float1 - 0.25F, float2, float0,
                             float1 + 0.25F, float2, 1.0F, 1.0F, 1.0F, 0.5F, 1);
    }
  }
  ...
  void render(float x, float y, float z, ColorInfo col, bool bDoChild,
    ...
    if (this.getItem().getScriptItem().isWorldRender()) {
      if (WorldItemModelDrawer.renderMain(
              this.getItem(), this.getSquare(), this.getX() + this.xoff,
              this.getY() + this.yoff, this.getZ() + this.zoff, 0.0F)) {
        this.debugDrawLocation(x, y, z);
      } else if (this.sprite.CurrentAnim != nullptr &&
                 !this.sprite.CurrentAnim.Frames.isEmpty()) {
        Texture texture =
            this.sprite.CurrentAnim.Frames.get(0).getTexture(this.dir);
        if (texture != nullptr) {
          float float0 =
              texture.getWidthOrig() * this.sprite.def.getScaleX() / 2.0F;
          float float1 = texture.getHeightOrig() * this.sprite.def.getScaleY() *
                         3.0F / 4.0F;
          int int0 = IsoCamera.frameState.playerIndex;
          float float2 = this.getAlpha(int0);
          float float3 = this.getTargetAlpha(int0);
          float float4 =
              PZMath.min(getSurfaceAlpha(this.square, this.zoff), float2);
          this.setAlphaAndTarget(int0, float4);
          this.sprite.render(this, x + this.xoff, y + this.yoff, z + this.zoff,
                             this.dir, this.offsetX + float0,
                             this.offsetY + float1, col, true);
  ...
  void addToWorld() {
    if (this.item != nullptr && this.item.shouldUpdateInWorld() &&
        !IsoWorld.instance.CurrentCell.getProcessWorldItems().contains(this)) {
  ...
  float getScreenPosX(int playerIndex) {
    float float0 =
        IsoUtils.XToScreen(this.getX() + this.xoff, this.getY() + this.yoff,
                           this.getZ() + this.zoff, 0);
    PlayerCamera playerCamera = IsoCamera.cameras[playerIndex];
    return (float0 - playerCamera.getOffX()) /
           Core.getInstance().getZoom(playerIndex);
  }

  float getScreenPosY(int playerIndex) {
    Texture texture = this.sprite == null
                          ? null
                          : this.sprite.getTextureForCurrentFrame(this.dir);
    float float0 = texture == null
                       ? 0.0F
                       : texture.getHeightOrig() * this.sprite.def.getScaleY() *
                             1.0F / 4.0F;
    float float1 =
        IsoUtils.YToScreen(this.getX() + this.xoff, this.getY() + this.yoff,
                           this.getZ() + this.zoff, 0);
    PlayerCamera playerCamera = IsoCamera.cameras[playerIndex];
    return (float1 - playerCamera.getOffY() - float0) /
           Core.getInstance().getZoom(playerIndex);
  }

  void setIgnoreRemoveSandbox(bool b) { this.ignoreRemoveSandbox = b; }

  bool isIgnoreRemoveSandbox() { return this.ignoreRemoveSandbox; }

  float getWorldPosX() { return this.getX() + this.xoff; }

  float getWorldPosY() { return this.getY() + this.yoff; }

  float getWorldPosZ() { return this.getZ() + this.zoff; }
