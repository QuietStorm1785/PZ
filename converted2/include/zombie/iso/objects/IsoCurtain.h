...
public
  IsoCurtain(IsoCell cell, IsoGridSquare gridSquare, IsoSprite gid, bool _north,
             bool spriteclosed) {
    ...
    if (spriteclosed) {
      ...
      this.closedSprite =
          IsoSprite.getSprite(IsoSpriteManager.instance, gid, -4);
      this.openSprite = gid;
    }
...
public
  IsoCurtain(IsoCell cell) { super(cell); }

  std::string getObjectName() { return "Curtain"; }
  ...
  bool canInteractWith(IsoGameCharacter chr) {
    if (chr != nullptr && chr.getCurrentSquare() != nullptr) {
      ...
      return (this.isAdjacentToSquare(square) ||
              square == this.getOppositeSquare()) &&
             !this.getSquare().isBlockedTo(square);
    } else {
      return false;
    }
  }
  ...
  IsoGridSquare getOppositeSquare() {
    if (this.getType() == IsoObjectType.curtainN) {
      return this.getCell().getGridSquare((double)this.getX(),
      ...
      return this.getCell().getGridSquare((double)(this.getX() - 1.0F),
                                          (double)this.getY(),
                                          (double)this.getZ());
    } else {
  ...
  bool isAdjacentToSquare(IsoGridSquare square1, IsoGridSquare square2) {
    if (square1 != nullptr && square2 != nullptr) {
      return this.getType() != IsoObjectType.curtainN &&
                     this.getType() != IsoObjectType.curtainS
                 ? square1.x == square2.x &&
                       Math.abs(square1.y - square2.y) <= 1
                 : square1.y == square2.y &&
                       Math.abs(square1.x - square2.x) <= 1;
    } else {
      return false;
    }
  }
...
public
  IsoObject.VisionResult TestVision(IsoGridSquare from, IsoGridSquare to) {
    if (to.getZ() != from.getZ()) {
      ...
      if (from == this.square && (this.getType() == IsoObjectType.curtainW ||
                                  this.getType() == IsoObjectType.curtainN) ||
          from != this.square && (this.getType() == IsoObjectType.curtainE ||
                                  this.getType() == IsoObjectType.curtainS)) {
  ...
  void ToggleDoor(IsoGameCharacter chr) {
    if (!this.Barricaded) {
      this.DirtySlice();
      if (!this.Locked || chr == nullptr ||
          chr.getCurrentSquare().getRoom() != nullptr || this.open) {
        this.open = !this.open;
  ...
  void render(float x, float y, float z, ColorInfo col, bool bDoAttached,
    ...
    if (object != nullptr && this.getSquare().getTargetDarkMulti(int0) <=
                                 object.getSquare().getTargetDarkMulti(int0)) {
    ...
    if (int0 == -1) {
      ...
      if (this.getType() != IsoObjectType.curtainW &&
        ...
        if (this.getType() == IsoObjectType.curtainE ||
          ...
          if (square != nullptr) {
            ...
            for (int int1 = pZArrayList.size() - 1; int1 >= 0; int1--) {
              BarricadeAble barricadeAble0 = Type.tryCastTo(
                  (IsoObject)pZArrayList.get(int1), BarricadeAble.class);
              if (barricadeAble0 != nullptr &&
                  boolean0 == barricadeAble0.getNorth()) {
        ...
        for (int int2 = int0 - 1; int2 >= 0; int2--) {
          BarricadeAble barricadeAble1 = Type.tryCastTo(
              (IsoObject)pZArrayList.get(int2), BarricadeAble.class);
          if (barricadeAble1 != nullptr &&
              boolean1 == barricadeAble1.getNorth()) {
  ...
  std::string getSoundPrefix() {
    if (this.closedSprite == nullptr) {
      ...
      return propertyContainer.Is("CurtainSound")
                 ? "Curtain" + propertyContainer.Val("CurtainSound")
                 : "CurtainShort";
    }
  }
  ...
  static bool isSheet(IsoObject curtain) {
    ...
    if (curtain != nullptr && ((IsoObject)curtain).getSprite() != nullptr) {
      ...
      return sprite.getProperties().Is("CurtainSound")
          ? "Sheet" == sprite.getProperties().Val("CurtainSound")) : false;
    } else {
      return false;
    }
  }
