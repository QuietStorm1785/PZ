...
public:
  bool hasTree;
private
  ArrayList<Float> LightInfluenceB;
private
  ArrayList<Float> LightInfluenceG;
private
  ArrayList<Float> LightInfluenceR;
public
  final IsoGridSquare[] nav = new IsoGridSquare[8];
  int collideMatrix = -1;
  int pathMatrix = -1;
  int visionMatrix = -1;
  IsoRoom room = null;
...
private
  final boolean[] playerCutawayFlags = new boolean[4];
private
  final long[] playerCutawayFlagLockUntilTimes = new long[4];
private
  final boolean[] targetPlayerCutawayFlags = new boolean[4];
private
  final boolean[] playerIsDissolvedFlags = new boolean[4];
private
  final long[] playerIsDissolvedFlagLockUntilTimes = new long[4];
private
  final boolean[] targetPlayerIsDissolvedFlags = new boolean[4];
  IsoWaterGeometry water = null;
  IsoPuddlesGeometry puddles = null;
  float puddlesCacheSize = -1.0F;
  float puddlesCacheLevel = -1.0F;
public
  final IsoGridSquare.ILighting[] lighting = new IsoGridSquare.ILighting[4];
  int x;
  int y;
  int z;
...
private
  final ArrayList<IsoGameCharacter> DeferedCharacters =
      std::make_unique<ArrayList<>>();
  int DeferredCharacterTick = -1;
private
  final ArrayList<IsoMovingObject> StaticMovingObjects = new ArrayList<>(0);
private
  final ArrayList<IsoMovingObject> MovingObjects = new ArrayList<>(0);
protected
  final PZArrayList<IsoObject> Objects = new PZArrayList<>(IsoObject.class, 2);
protected
  final PZArrayList<IsoObject> localTemporaryObjects =
      new PZArrayList<>(IsoObject.class, 2);
private
  final ArrayList<IsoWorldInventoryObject> WorldObjects =
      std::make_unique<ArrayList<>>();
...
private
  final ArrayList<IsoObject> SpecialObjects = new ArrayList<>(0);
  bool haveRoof = false;
  bool burntOut = false;
  bool bHasFlies = false;
  IsoGridOcclusionData OcclusionDataCache = null;
private
  static final PZArrayList<IsoWorldInventoryObject> tempWorldInventoryObjects =
      new PZArrayList<>(IsoWorldInventoryObject.class, 16);
public
  static final ConcurrentLinkedQueue<IsoGridSquare> isoGridSquareCache =
      std::make_unique<ConcurrentLinkedQueue<>>();
public
  static ArrayDeque<IsoGridSquare> loadGridSquareCache;
...
public
  static final ArrayList<IsoGridSquare> choices =
      std::make_unique<ArrayList<>>();
...
public
  static final IsoGridSquare.CellGetSquare cellGetSquare =
      new IsoGridSquare.CellGetSquare();
  bool propertiesDirty = true;
  static bool UseSlowCollision = false;
  static bool bDoSlowPathfinding = false;
private
  static final Comparator<IsoMovingObject> comp =
      (movingObject0, movingObject1)->movingObject0.compareToY(movingObject1);
  ...
  int splashFrameNum;
private
  final ColorInfo[] lightInfo = new ColorInfo[4];
...
private
  ErosionData.Square erosion;
private
  static final int[] SURFACE_OFFSETS = new int[8];
  ...
  static int setMatrixBit(int matrix, uint8_t _x, uint8_t _y, uint8_t _z,
                          bool val) {
    return val ? matrix | 1 << _x + _y * 3 + _z * 9
               : matrix & ~(1 << _x + _y * 3 + _z * 9);
  }

  void setPlayerCutawayFlag(int playerIndex, bool bCutaway,
    ...
    if (currentTimeMillis > this.playerCutawayFlagLockUntilTimes[playerIndex] &&
      ...
      this.playerCutawayFlags[playerIndex] =
          this.targetPlayerCutawayFlags[playerIndex];
      this.playerCutawayFlagLockUntilTimes[playerIndex] =
  ...
  void setIsDissolved(int playerIndex, bool bDissolved,
                      long currentTimeMillis) {
    this.targetPlayerIsDissolvedFlags[playerIndex] = bDissolved;
    if (currentTimeMillis >
            this.playerIsDissolvedFlagLockUntilTimes[playerIndex] &&
  ...
  IsoWaterGeometry getWater() {
    if (this.water != nullptr && this.water.m_adjacentChunkLoadedCounter !=
                                     this.chunk.m_adjacentChunkLoadedCounter) {
      this.water.m_adjacentChunkLoadedCounter =
          this.chunk.m_adjacentChunkLoadedCounter;
  ...
  float getPuddlesInGround() {
    if (this.isInARoom()) {
      ...
      if (Math.abs(IsoPuddles.getInstance().getPuddlesSize() +
                   Core.getInstance().getPerfPuddles() +
                   IsoCamera.frameState.OffscreenWidth -
                   this.puddlesCacheSize) > 0.01) {
  ...
  IsoGridOcclusionData getOcclusionData() { return this.OcclusionDataCache; }
  ...
  /**
   * Check if there's any object on this grid that has a sneak modifier, we use
   * this to check if we reduce the chance of being spotted while crouching
   */
  float getGridSneakModifier(bool onlySolidTrans) {
    if (!onlySolidTrans) {
      if (this.Properties.Is("CloseSneakBonus")) {
        return Integer.parseInt(this.Properties.Val("CloseSneakBonus")) /
               100.0F;
      }
  ...
  bool isSomethingTo(IsoGridSquare other) {
    return this.isWallTo(other) || this.isWindowTo(other) ||
           this.isDoorTo(other);
  }
  ...
  IsoObject getTransparentWallTo(IsoGridSquare other) {
    if (other == nullptr || other == this || !this.isWallTo(other)) {
      return null;
               ...
               other.Properties.Is(IsoFlagType.SpearOnlyAttackThrough) &&
               !other.Properties.Is(IsoFlagType.WindowW)) {
      return other.getWall();
    } else if (this.x > other.x &&
               this.Properties.Is(IsoFlagType.SpearOnlyAttackThrough) &&
               !this.Properties.Is(IsoFlagType.WindowW)) {
      return this.getWall();
    } else if (other.y > this.y &&
               other.Properties.Is(IsoFlagType.SpearOnlyAttackThrough) &&
               !other.Properties.Is(IsoFlagType.WindowN)) {
      return other.getWall();
      ...
      if (other.x != this.x && other.y != this.y) {
        IsoObject object0 = this.getTransparentWallTo(
            IsoWorld.instance.CurrentCell.getGridSquare(other.x, this.y,
                                                        this.z));
        IsoObject object1 = this.getTransparentWallTo(
            IsoWorld.instance.CurrentCell.getGridSquare(this.x, other.y,
                                                        this.z));
        if (object0 != nullptr) {
          return object0;
        }
        ...
        object0 = other.getTransparentWallTo(
            IsoWorld.instance.CurrentCell.getGridSquare(other.x, this.y,
                                                        this.z));
        object1 = other.getTransparentWallTo(
            IsoWorld.instance.CurrentCell.getGridSquare(this.x, other.y,
                                                        this.z));
        if (object0 != nullptr) {
          return object0;
  ...
  bool isWallTo(IsoGridSquare other) {
    if (other == nullptr || other == this) {
      return false;
    } else if (other.x > this.x && other.Properties.Is(IsoFlagType.collideW) &&
               !other.Properties.Is(IsoFlagType.WindowW)) {
      return true;
    } else if (this.x > other.x && this.Properties.Is(IsoFlagType.collideW) &&
               !this.Properties.Is(IsoFlagType.WindowW)) {
      return true;
    } else if (other.y > this.y && other.Properties.Is(IsoFlagType.collideN) &&
               !other.Properties.Is(IsoFlagType.WindowN)) {
      return true;
    } else if (this.y > other.y && this.Properties.Is(IsoFlagType.collideN) &&
               !this.Properties.Is(IsoFlagType.WindowN)) {
      return true;
    } else {
      if (other.x != this.x && other.y != this.y) {
        if (this.isWallTo(IsoWorld.instance.CurrentCell.getGridSquare(
                other.x, this.y, this.z)) ||
            this.isWallTo(IsoWorld.instance.CurrentCell.getGridSquare(
                this.x, other.y, this.z))) {
          return true;
        }

        if (other.isWallTo(IsoWorld.instance.CurrentCell.getGridSquare(
                other.x, this.y, this.z)) ||
            other.isWallTo(IsoWorld.instance.CurrentCell.getGridSquare(
                this.x, other.y, this.z))) {
          return true;
        }
      }
  ...
  bool isWindowTo(IsoGridSquare other) {
    if (other == nullptr || other == this) {
      ...
      if (other.x != this.x && other.y != this.y) {
        if (this.isWindowTo(IsoWorld.instance.CurrentCell.getGridSquare(
                other.x, this.y, this.z)) ||
            this.isWindowTo(IsoWorld.instance.CurrentCell.getGridSquare(
                this.x, other.y, this.z))) {
          return true;
        }

        if (other.isWindowTo(IsoWorld.instance.CurrentCell.getGridSquare(
                other.x, this.y, this.z)) ||
            other.isWindowTo(IsoWorld.instance.CurrentCell.getGridSquare(
                this.x, other.y, this.z))) {
          return true;
        }
      }
  ...
  bool hasDoorOnEdge(IsoDirections edge, bool ignoreOpen) {
    for (int int0 = 0; int0 < this.SpecialObjects.size(); int0++) {
      IsoDoor door =
          Type.tryCastTo(this.SpecialObjects.get(int0), IsoDoor.class);
      if (door != nullptr && door.getSpriteEdge(ignoreOpen) == edge) {
        return true;
      }

      IsoThumpable thumpable =
          Type.tryCastTo(this.SpecialObjects.get(int0), IsoThumpable.class);
  ...
  bool hasClosedDoorOnEdge(IsoDirections directions) {
    ...
    for (int int0 = 0; int0 < this.SpecialObjects.size(); int0++) {
      ...
      if (door != nullptr && !door.IsOpen() &&
          door.getSpriteEdge(boolean0) == directions) {
        return true;
      }

      IsoThumpable thumpable =
          Type.tryCastTo(this.SpecialObjects.get(int0), IsoThumpable.class);
      if (thumpable != nullptr && !thumpable.IsOpen() &&
          thumpable.getSpriteEdge(boolean0) == directions) {
        return true;
      }
    }
  ...
  bool isDoorTo(IsoGridSquare other) {
    if (other == nullptr || other == this) {
      ...
      if (other.x != this.x && other.y != this.y) {
        if (this.isDoorTo(IsoWorld.instance.CurrentCell.getGridSquare(
                other.x, this.y, this.z)) ||
            this.isDoorTo(IsoWorld.instance.CurrentCell.getGridSquare(
                this.x, other.y, this.z))) {
          return true;
        }

        if (other.isDoorTo(IsoWorld.instance.CurrentCell.getGridSquare(
                other.x, this.y, this.z)) ||
            other.isDoorTo(IsoWorld.instance.CurrentCell.getGridSquare(
                this.x, other.y, this.z))) {
          return true;
        }
      }
  ...
  bool isBlockedTo(IsoGridSquare other) {
    return this.isWallTo(other) || this.isWindowBlockedTo(other) ||
  ...
  bool isWindowBlockedTo(IsoGridSquare other) {
    if (other == nullptr) {
      ...
      if (other.x != this.x && other.y != this.y) {
        if (this.isWindowBlockedTo(IsoWorld.instance.CurrentCell.getGridSquare(
                other.x, this.y, this.z)) ||
            this.isWindowBlockedTo(IsoWorld.instance.CurrentCell.getGridSquare(
                this.x, other.y, this.z))) {
          return true;
        }

        if (other.isWindowBlockedTo(IsoWorld.instance.CurrentCell.getGridSquare(
                other.x, this.y, this.z)) ||
            other.isWindowBlockedTo(IsoWorld.instance.CurrentCell.getGridSquare(
                this.x, other.y, this.z))) {
          return true;
        }
      }
  ...
  bool isDoorBlockedTo(IsoGridSquare other) {
    if (other == nullptr) {
      ...
      if (other.x != this.x && other.y != this.y) {
        if (this.isDoorBlockedTo(IsoWorld.instance.CurrentCell.getGridSquare(
                other.x, this.y, this.z)) ||
            this.isDoorBlockedTo(IsoWorld.instance.CurrentCell.getGridSquare(
                this.x, other.y, this.z))) {
          return true;
        }

        if (other.isDoorBlockedTo(IsoWorld.instance.CurrentCell.getGridSquare(
                other.x, this.y, this.z)) ||
            other.isDoorBlockedTo(IsoWorld.instance.CurrentCell.getGridSquare(
                this.x, other.y, this.z))) {
          return true;
        }
      }
  ...
  bool hasBlockedDoor(bool north) {
    for (int int0 = 0; int0 < this.Objects.size(); int0++) {
      ...
      if (object instanceof IsoThumpable thumpable && thumpable.isDoor() &&
                                thumpable.getNorth() == north) {
  ...
  IsoCurtain getCurtain(IsoObjectType curtainType) {
    for (int int0 = 0; int0 < this.getSpecialObjects().size(); int0++) {
      IsoCurtain curtain =
          Type.tryCastTo(this.getSpecialObjects().get(int0), IsoCurtain.class);
  ...
  IsoObject getHoppable(bool north) {
    for (int int0 = 0; int0 < this.Objects.size(); int0++) {
      ...
      if (propertyContainer != nullptr &&
          propertyContainer.Is(north ? IsoFlagType.HoppableN
                                     : IsoFlagType.HoppableW)) {
        return object;
      }

      if (propertyContainer != nullptr &&
          propertyContainer.Is(north ? IsoFlagType.WindowN
                                     : IsoFlagType.WindowW)) {
        return object;
      }
    }
    ...
    if (next != nullptr && next != this) {
      ...
      if (next.x != this.x && next.y != this.y) {
        IsoGridSquare square0 =
            this.getCell().getGridSquare(this.x, next.y, this.z);
        IsoGridSquare square1 =
            this.getCell().getGridSquare(next.x, this.y, this.z);
  ...
  bool isHoppableTo(IsoGridSquare other) {
    if (other == nullptr) {
      ...
      return other.y > this.y && other.Properties.Is(IsoFlagType.HoppableN)
                 ? true
                 : this.y > other.y &&
                       this.Properties.Is(IsoFlagType.HoppableN);
    }
  }
  ...
  float DistTo(IsoGridSquare sq) {
    return IsoUtils.DistanceManhatten(this.x + 0.5F, this.y + 0.5F, sq.x + 0.5F,
                                      sq.y + 0.5F);
  }

  float DistToProper(IsoGridSquare sq) {
    return IsoUtils.DistanceTo(this.x + 0.5F, this.y + 0.5F, sq.x + 0.5F,
                               sq.y + 0.5F);
  }

  float DistTo(IsoMovingObject other) {
    return IsoUtils.DistanceManhatten(this.x + 0.5F, this.y + 0.5F,
                                      other.getX(), other.getY());
  }

  float DistToProper(IsoMovingObject other) {
    return IsoUtils.DistanceTo(this.x + 0.5F, this.y + 0.5F, other.getX(),
                               other.getY());
  }
    ...
    if (string0.length() > 64) {
      ...
      if (string1.length() > 64) {
        ...
        try {
          ...
          Logger.getLogger(IsoGridSquare.class.getName())
              .log(Level.SEVERE, nullptr, malformedURLException);
        } catch (IOException iOException) {
          Logger.getLogger(IsoGridSquare.class.getName())
              .log(Level.SEVERE, nullptr, iOException);
        }
  ...
  void renderAttachedSpritesWithNoWallLighting(IsoObject object,
                                               ColorInfo colorInfo) {
    if (object.AttachedAnimSprite != nullptr &&
        !object.AttachedAnimSprite.isEmpty()) {
      bool boolean0 = false;
      ...
      for (int int0 = 0; int0 < object.AttachedAnimSprite.size(); int0++) {
        ...
        if (spriteInstance0.parentSprite != nullptr &&
            spriteInstance0.parentSprite.Properties.Is(
      ...
      if (boolean0) {
        ...
        for (int int1 = 0; int1 < object.AttachedAnimSprite.size(); int1++) {
          IsoSpriteInstance spriteInstance1 =
              object.AttachedAnimSprite.get(int1);
          if (spriteInstance1.parentSprite != nullptr &&
              spriteInstance1.parentSprite.Properties.Is(
                  IsoFlagType.NoWallLighting)) {
            defColorInfo.a = spriteInstance1.alpha;
            spriteInstance1.render(
                object, this.x, this.y, this.z, object.dir, object.offsetX,
...
public
  void DoCutawayShader(IsoObject obj, IsoDirections dir, boolean cutawaySW,
                       boolean cutawayNW, boolean cutawayNE, boolean bHasDoorN,
                       boolean bHasDoorW, boolean bHasWindowN,
                       boolean bHasWindowW, WallShaperWhole texdModifier) {
    ...
    if (texture0 != nullptr && texture0.getID() != -1) {
      ...
      try {
        ...
        IsoGridSquare.CircleStencilShader circleStencilShader =
            IsoGridSquare.CircleStencilShader.instance;
        if (dir == IsoDirections.N || dir == IsoDirections.NW) {
          ...
          if (bHasDoorN) {
            short2 = 904;
            if (obj.sprite.name.contains("garage") ||
                obj.sprite.name.contains("industry_trucks")) {
          ...
          if (Core.bDebug &&
              DebugOptions.instance.DebugDraw_SkipWorldShading.getValue()) {
            coll2 = -1;
            colu2 = -1;
            coll = -1;
            colu = -1;
          ...
          if (obj.sprite.getProperties().Is(IsoFlagType.WallSE)) {
            SpriteRenderer.instance.setCutawayTexture(
                texture0, short1 + (int)float0, short2 + (int)float1, 6 - int2,
                196 - int3);
          } else {
            SpriteRenderer.instance.setCutawayTexture(
                texture0, short0 + (int)float0, short2 + (int)float1, 66 - int2,
                226 - int3);
          }

          if (dir == IsoDirections.N) {
            SpriteRenderer.instance.setExtraWallShaderParams(
                SpriteRenderer.WallShaderTexRender.All);
          } else {
            SpriteRenderer.instance.setExtraWallShaderParams(
                SpriteRenderer.WallShaderTexRender.RightOnly);
          }
          ...
          obj.renderWallTileOnly(this.x, this.y, this.z,
                                 boolean0 ? colorInfo : defColorInfo,
                                 circleStencilShader, texdModifier);
        }
        ...
        if (dir == IsoDirections.W || dir == IsoDirections.NW) {
          ...
          if (obj.sprite.getProperties().Is(IsoFlagType.WallSE)) {
            SpriteRenderer.instance.setCutawayTexture(
                texture0, short4 + (int)float0, short5 + (int)float1, 6 - int2,
                196 - int3);
          } else {
            SpriteRenderer.instance.setCutawayTexture(
                texture0, short3 + (int)float0, short5 + (int)float1, 66 - int2,
                226 - int3);
          }
          ...
          if (dir == IsoDirections.W) {
            ...
            SpriteRenderer.instance.setExtraWallShaderParams(
                SpriteRenderer.WallShaderTexRender.LeftOnly);
          }
      ...
      obj.renderAttachedAndOverlaySprites(
          this.x, this.y, this.z, boolean0 ? colorInfo : defColorInfo, false,
          !boolean0, nullptr, texdModifier);
    }
  }

  void DoCutawayShaderSprite(IsoSprite sprite, IsoDirections dir,
                             bool cutawaySW, bool cutawayNW, bool cutawayNE) {
    IsoGridSquare.CircleStencilShader circleStencilShader =
        IsoGridSquare.CircleStencilShader.instance;
    ...
    if (texture0 != nullptr && texture0.getID() != -1) {
      ...
      try {
        Texture texture1 = sprite.CurrentAnim.Frames.get((int)sprite.def.Frame)
                               .getTexture(dir);
        float float0 = 0.0F;
        ...
        if (dir == IsoDirections.N || dir == IsoDirections.NW) {
          ...
          if (sprite.getProperties().Is(IsoFlagType.WallSE)) {
            SpriteRenderer.instance.setCutawayTexture(
                texture0, short1 + (int)float0, 0 + (int)float1, 6 - int2,
                196 - int3);
          } else {
            SpriteRenderer.instance.setCutawayTexture(
                texture0, short0 + (int)float0, 0 + (int)float1, 66 - int2,
                226 - int3);
          }
          ...
          IndieGL.bindShader(
              circleStencilShader, sprite, dir, wallShaperWhole,
              (spritex, directions, wallShaperWholex)
                  ->spritex.render(nullptr, this.x, this.y, this.z, directions,
                                   WeatherFxMask.offsetX, WeatherFxMask.offsetY,
                                   defColorInfo, false, wallShaperWholex));
        }
        ...
        if (dir == IsoDirections.W || dir == IsoDirections.NW) {
          ...
          if (sprite.getProperties().Is(IsoFlagType.WallSE)) {
            SpriteRenderer.instance.setCutawayTexture(
                texture0, short3 + (int)float0, 0 + (int)float1, 6 - int2,
                196 - int3);
          } else {
            SpriteRenderer.instance.setCutawayTexture(
                texture0, short2 + (int)float0, 0 + (int)float1, 66 - int2,
                226 - int3);
          }
...
public
  int DoWallLightingNW(IsoObject obj, int stenciled, boolean cutawaySW,
                       boolean cutawayNW, boolean cutawayNE, boolean bHasDoorN,
                       boolean bHasDoorW, boolean bHasWindowN,
                       boolean bHasWindowW, Shader wallRenderShader) {
    if (!DebugOptions.instance.Terrain.RenderTiles.IsoGridSquare.Walls.NW
             .getValue()) {
      return stenciled;
    } else {
      ...
      if (DebugOptions.instance.Terrain.RenderTiles.IsoGridSquare.Walls
              .LightingDebug.getValue()) {
        colu = -65536;
        coll = -16711936;
        colr = -16711681;
        colu2 = -16776961;
      ...
      bool boolean2 = obj.sprite.getType() == IsoObjectType.doorFrN ||
                      obj.sprite.getType() == IsoObjectType.doorN;
      bool boolean3 = obj.sprite.getType() == IsoObjectType.doorFrW ||
                      obj.sprite.getType() == IsoObjectType.doorW;
      bool boolean4 = false;
      bool boolean5 = false;
      bool boolean6 =
          (boolean2 || boolean4 || boolean3 || boolean4) && boolean0 ||
          obj.sprite.getProperties().Is(IsoFlagType.NoWallLighting);
      boolean1 = this.calculateWallAlphaAndCircleStencilCorner(
          obj, cutawaySW, cutawayNW, cutawayNE, bHasDoorN, bHasDoorW,
          bHasWindowN, bHasWindowW, boolean1, int0, boolean2, boolean3,
          boolean4, boolean5);
      if (USE_WALL_SHADER && boolean1 && boolean0) {
        this.DoCutawayShader(obj, directions, cutawaySW, cutawayNW, cutawayNE,
                             bHasDoorN, bHasDoorW, bHasWindowN, bHasWindowW,
        ...
        stenciled = this.performDrawWall(obj, stenciled, int0, boolean6,
                                         wallShaperN, wallRenderShader);
        ...
        return this.performDrawWall(obj, stenciled, int0, boolean6, wallShaperW,
                                    wallRenderShader);
      }
    }
  }
...
public
  int DoWallLightingN(IsoObject obj, int stenciled, boolean cutawayNW,
                      boolean cutawayNE, boolean bHasDoorN, boolean bHasWindowN,
                      Shader wallRenderShader) {
    if (!DebugOptions.instance.Terrain.RenderTiles.IsoGridSquare.Walls.N
             .getValue()) {
      return stenciled;
    } else {
      ...
      return this.performDrawWallSegmentSingle(
          obj, stenciled, false, cutawayNW, false, false, cutawayNE, bHasDoorN,
          bHasWindowN, boolean0, boolean1, objectType0, objectType1, boolean2,
          flagType0, flagType1, flagType2, directions, boolean3,
          wallShaperWhole, wallRenderShader);
    }
  }

public
  int DoWallLightingW(IsoObject obj, int stenciled, boolean cutawaySW,
                      boolean cutawayNW, boolean bHasDoorW, boolean bHasWindowW,
                      Shader wallRenderShader) {
    if (!DebugOptions.instance.Terrain.RenderTiles.IsoGridSquare.Walls.W
             .getValue()) {
      return stenciled;
    } else {
      ...
      wallShaperWhole.col[3] = coll;
      return this.performDrawWallSegmentSingle(
          obj, stenciled, cutawaySW, cutawayNW, bHasDoorW, bHasWindowW, false,
          false, false, boolean0, boolean1, objectType0, objectType1, boolean2,
          flagType0, flagType1, flagType2, directions, boolean3,
          wallShaperWhole, wallRenderShader);
    }
  }
...
private
  int performDrawWallSegmentSingle(
      IsoObject object, int int1, boolean boolean7, boolean boolean8,
      boolean boolean11, boolean boolean13, boolean boolean9, boolean boolean10,
      boolean boolean12, boolean boolean5, boolean boolean6,
      IsoObjectType objectType1, IsoObjectType objectType0, boolean boolean4,
      IsoFlagType flagType0, IsoFlagType flagType1, IsoFlagType flagType2,
      IsoDirections directions, boolean boolean0,
      WallShaperWhole wallShaperWhole, Shader shader) {
    int int0 = IsoCamera.frameState.playerIndex;
    if (this.z != (int)IsoCamera.CamCharacter.z) {
      boolean0 = false;
    }

    bool boolean1 = object.sprite.getType() == objectType1 ||
                    object.sprite.getType() == objectType0;
    bool boolean2 = object instanceof IsoWindow;
    bool boolean3 =
        (boolean1 || boolean2) && boolean4 ||
        object.sprite.getProperties().Is(IsoFlagType.NoWallLighting);
    boolean0 = this.calculateWallAlphaAndCircleStencilEdge(
        object, boolean5, boolean6, boolean4, flagType0, flagType1, flagType2,
        boolean0, int0, boolean1, boolean2);
    if (USE_WALL_SHADER && boolean0 && boolean4) {
      this.DoCutawayShader(object, directions, boolean7, boolean8, boolean9,
                           boolean10, boolean11, boolean12, boolean13,
                           wallShaperWhole);
      bWallCutawayN = bWallCutawayN | directions == IsoDirections.N;
      bWallCutawayW = bWallCutawayW | directions == IsoDirections.W;
      return int1;
    } else {
      return this.performDrawWall(object, int1, int0, boolean3, wallShaperWhole,
                                  shader);
    }
  }

  int performDrawWallOnly(IsoObject object, int int0, int int1, bool boolean0,
                          Consumer<TextureDraw> consumer, Shader shader) {
    if (int0 == 0 && !boolean0) {
    ...
    if (DebugOptions.instance.Terrain.RenderTiles.IsoGridSquare.Walls.Render
            .getValue()) {
      object.renderWallTile(this.x, this.y, this.z,
                            boolean0 ? lightInfoTemp : defColorInfo, true,
                            !boolean0, shader, consumer);
    }
  ...
  int performDrawWall(IsoObject object, int int1, int int0, bool boolean0,
                      Consumer<TextureDraw> consumer, Shader shader) {
    lightInfoTemp.set(this.lightInfo[int0]);
    if (Core.bDebug &&
        DebugOptions.instance.DebugDraw_SkipWorldShading.getValue()) {
      object.render(this.x, this.y, this.z, defColorInfo, true, !boolean0,
                    nullptr);
      return int1;
    } else {
      int int2 = this.performDrawWallOnly(object, int1, int0, boolean0,
                                          consumer, shader);
      if (DebugOptions.instance.Terrain.RenderTiles.IsoGridSquare.Walls
              .AttachedSprites.getValue()) {
  ...
  void calculateWallAlphaCommon(IsoObject object, bool boolean2, bool boolean3,
                                bool boolean4, int int0, bool boolean0,
                                bool boolean1) {
...
private
  boolean calculateWallAlphaAndCircleStencilEdge(
      IsoObject object, boolean boolean5, boolean boolean4, boolean boolean3,
      IsoFlagType flagType1, IsoFlagType flagType0, IsoFlagType var7,
      boolean boolean2, int int0, boolean boolean0, boolean boolean1) {
    if (boolean0 || boolean1) {
      if (!object.sprite.getProperties().Is("GarageDoor")) {
        boolean2 = false;
      }

      this.calculateWallAlphaCommon(object, boolean3, !boolean5, !boolean4,
                                    int0, boolean0, boolean1);
    }

    if (boolean2 && object.sprite.getType() == IsoObjectType.wall &&
...
private
  boolean calculateWallAlphaAndCircleStencilCorner(
      IsoObject object, boolean boolean10, boolean boolean4, boolean boolean5,
      boolean boolean0, boolean boolean6, boolean boolean1, boolean boolean7,
      boolean boolean11, int int0, boolean boolean2, boolean boolean8,
      boolean boolean3, boolean boolean9) {
    this.calculateWallAlphaCommon(object, boolean4 || boolean5, boolean0,
                                  boolean1, int0, boolean2, boolean3);
    this.calculateWallAlphaCommon(object, boolean4 || boolean10, boolean6,
                                  boolean7, int0, boolean8, boolean9);
    boolean11 = boolean11 && !boolean2 && !boolean3;
    if (boolean11 && object.sprite.getType() == IsoObjectType.wall &&
        (object.sprite.getProperties().Is(IsoFlagType.transparentN) ||
         object.sprite.getProperties().Is(IsoFlagType.transparentW)) &&
        !object.getSprite().getProperties().Is(IsoFlagType.exterior) &&
  ...
  bool HasDoor(bool boolean0) {
    for (int int0 = 0; int0 < this.SpecialObjects.size(); int0++) {
      if (this.SpecialObjects.get(int0) instanceof
          IsoDoor &&
              ((IsoDoor)this.SpecialObjects.get(int0)).north == boolean0) {
        return true;
      }
  ...
  bool HasStairs() { return this.HasStairsNorth() || this.HasStairsWest(); }

  bool HasStairsNorth() {
    return this.Has(IsoObjectType.stairsTN) ||
           this.Has(IsoObjectType.stairsMN) || this.Has(IsoObjectType.stairsBN);
  }

  bool HasStairsWest() {
    return this.Has(IsoObjectType.stairsTW) ||
           this.Has(IsoObjectType.stairsMW) || this.Has(IsoObjectType.stairsBW);
  }
  ...
  bool HasStairsBelow() {
    if (this.z == 0) {
      ...
      IsoGridSquare square1 =
          this.getCell().getGridSquare(this.x, this.y, this.z - 1);
  ...
  bool HasElevatedFloor() {
    return this.Has(IsoObjectType.stairsTN) ||
           this.Has(IsoObjectType.stairsMN) ||
           this.Has(IsoObjectType.stairsTW) || this.Has(IsoObjectType.stairsMW);
  }
  ...
  bool HasSlopedRoofWest() {
    return this.Has(IsoObjectType.WestRoofB) ||
           this.Has(IsoObjectType.WestRoofM) ||
           this.Has(IsoObjectType.WestRoofT);
  }
  ...
  bool HasTree() { return this.hasTree; }
  ...
  bool shouldSave() { return !this.Objects.isEmpty(); }
      ...
      for (int int1 = 0; int1 < this.Objects.size(); int1++) {
        ...
        if (IS_DEBUG_SAVE) {
          GameWindow.WriteStringUTF(
              output, this.Objects.get(int1).getClass().getName());
        }
  ...
  static void savematrix(boolean[][][] booleans,
...
public
  ...
  void load(ByteBuffer b, int WorldVersion, bool IS_DEBUG_SAVE) {
    ...
    BitHeaderRead bitHeaderRead0 =
        BitHeader.allocRead(BitHeader.HeaderSize.Byte, b);
    if (!bitHeaderRead0 == 0)) {
          ...
          for (int int0 = 0; int0 < short0; int0++) {
            ...
            if (object == nullptr) {
              if (IS_DEBUG_SAVE) {
                int int3 = b.position();
                if (int3 - int1 != int2) {
                  DebugLog.log("***** Object loaded size " + (int3 - int1) +
                               " != saved size " + int2 +
                               ", reading obj size: " + short0 +
                               ", Object == nullptr");
                  if (object.getSprite() != nullptr &&
                      object.getSprite().getName() != nullptr) {
                    DebugLog.log("Obj sprite = " +
              ...
              if (IS_DEBUG_SAVE) {
                int int4 = b.position();
                if (int4 - int1 != int2) {
                  DebugLog.log("***** Object loaded size " + (int4 - int1) +
                               " != saved size " + int2 +
                               ", reading obj size: " + short0);
                  if (object.getSprite() != nullptr &&
                      object.getSprite().getName() != nullptr) {
              ...
              if (object instanceof IsoWorldInventoryObject) {
                ...
                if (((IsoWorldInventoryObject)object).dropTime > -1.0 &&
                    SandboxOptions.instance.HoursForWorldItemRemoval
                            .getValue() > 0.0 &&
                    (SandboxOptions.instance.WorldItemRemovalList.getValue()
                             .contains(strings[0]) &&
                         !SandboxOptions.instance.ItemRemovalListBlacklistToggle
                              .getValue() ||
                     !SandboxOptions.instance.WorldItemRemovalList.getValue()
                             .contains(strings[0]) &&
                         SandboxOptions.instance.ItemRemovalListBlacklistToggle
              ...
              if (!(object instanceof IsoWindow) ||
                  ...
                  if (boolean1) {
                    if (Core.bDebug &&
                      ...
                      DebugLog.log("Bitflags = " + byte0 + ", obj name = " +
                                   object.getObjectName() + ", sprite = " +
                                   (object.getSprite() != nullptr
                                        ? object.getSprite().getName()
                                        : "unknown"));
                    }
        ...
        if (bitHeaderRead0.hasFlags(64)) {
          ...
          if (bitHeaderRead1.hasFlags(1)) {
            ...
            for (int int5 = 0; int5 < short1; int5++) {
              ...
              try {
                movingObject = (IsoMovingObject)IsoObject.factoryFromFileInput(
                    this.getCell(), b);
  ...
  float scoreAsWaypoint(int _x, int _y) {
    float float0 = 2.0F;
    return float0 -
           IsoUtils.DistanceManhatten(_x, _y, this.getX(), this.getY()) * 5.0F;
  }

  void InvalidateSpecialObjectPaths() {}

  bool isSolid() { return this.Properties.Is(IsoFlagType.solid); }

  bool isSolidTrans() { return this.Properties.Is(IsoFlagType.solidtrans); }
  ...
  bool isFree(bool bCountOtherCharacters) {
    if (bCountOtherCharacters && this.MovingObjects.size() > 0) {
      ...
      if (this.Properties.Is(IsoFlagType.solid) ||
          this.Properties.Is(IsoFlagType.solidtrans) ||
          this.Has(IsoObjectType.tree)) {
        this.CacheIsFree = false;
      }
      ...
      if (this.Has(IsoObjectType.stairsBN) ||
          this.Has(IsoObjectType.stairsMN) ||
          this.Has(IsoObjectType.stairsTN)) {
        this.CacheIsFree = true;
      } else if (this.Has(IsoObjectType.stairsBW) ||
                 this.Has(IsoObjectType.stairsMW) ||
                 this.Has(IsoObjectType.stairsTW)) {
        this.CacheIsFree = true;
      }
  ...
  bool isFreeOrMidair(bool bCountOtherCharacters, bool bDoZombie) {
    ...
    if (this.Has(IsoObjectType.stairsBN) || this.Has(IsoObjectType.stairsMN) ||
        this.Has(IsoObjectType.stairsTN)) {
      boolean0 = true;
    } else if (this.Has(IsoObjectType.stairsBW) ||
               this.Has(IsoObjectType.stairsMW) ||
  ...
  bool connectedWithFloor() {
    if (this.getZ() == 0) {
      ...
      void *object = null;
      object = this.getCell().getGridSquare(this.getX() - 1, this.getY(),
                                            this.getZ());
      if (object != nullptr &&
          ((IsoGridSquare)object).Properties.Is(IsoFlagType.solidfloor)) {
        return true;
      } else {
        object = this.getCell().getGridSquare(this.getX() + 1, this.getY(),
                                              this.getZ());
        if (object != nullptr &&
          ...
          object = this.getCell().getGridSquare(this.getX(), this.getY() - 1,
                                                this.getZ());
          if (object != nullptr &&
              ((IsoGridSquare)object).Properties.Is(IsoFlagType.solidfloor)) {
            return true;
            ...
            return object != nullptr &&
                   ((IsoGridSquare)object)
                       .Properties.Is(IsoFlagType.solidfloor);
          }
        }
      }
    }
   ...
   * gonna check the tile "behind" the one w<e're trying to build something has
   * a floor (only one is required)
   *
  ...
  bool isAdjacentToWindow() {
    if (this.getWindow() != nullptr) {
      ...
      return true;
    } else if (this.getThumpableWindow(false) == nullptr &&
               this.getThumpableWindow(true) == nullptr) {
      IsoGridSquare square1 = this.nav[IsoDirections.S.index()];
      if (square1 == nullptr ||
          square1.getWindow(true) == nullptr &&
              square1.getWindowFrame(true) == nullptr &&
              square1.getThumpableWindow(true) == nullptr) {
        IsoGridSquare square2 = this.nav[IsoDirections.E.index()];
        return square2 != nullptr &&
               (square2.getWindow(false) != nullptr ||
                square2.getWindowFrame(false) != nullptr ||
                square2.getThumpableWindow(false) != nullptr);
      } else {
        return true;
      }
    } else {
  ...
  IsoThumpable getThumpableWindow(bool north) {
    for (int int0 = 0; int0 < this.SpecialObjects.size(); int0++) {
      IsoObject object = this.SpecialObjects.get(int0);
      if (object instanceof IsoThumpable thumpable && thumpable.isWindow() &&
                                north == thumpable.north) {
        return thumpable;
      }
    }
  ...
  IsoThumpable getHoppableThumpable(bool north) {
    for (int int0 = 0; int0 < this.SpecialObjects.size(); int0++) {
      IsoObject object = this.SpecialObjects.get(int0);
      if (object instanceof IsoThumpable thumpable && thumpable.isHoppable() &&
                                north == thumpable.north) {
        return thumpable;
      }
    }
  ...
  IsoObject getWallHoppable(bool north) {
    for (int int0 = 0; int0 < this.Objects.size(); int0++) {
      if (this.Objects.get(int0).isHoppable() &&
          north == this.Objects.get(int0).isNorthHoppable()) {
  ...
  IsoObject getWindowFrame(bool north) {
    for (int int0 = 0; int0 < this.Objects.size(); int0++) {
      IsoObject object = this.Objects.get(int0);
      if (!(object instanceof IsoWorldInventoryObject) &&
          IsoWindowFrame.isWindowFrame(object, north)) {
        return object;
      }
    }
  ...
  bool hasWindowOrWindowFrame() {
    for (int int0 = 0; int0 < this.Objects.size(); int0++) {
      IsoObject object = this.Objects.get(int0);
      if (!(object instanceof IsoWorldInventoryObject) &&
          (this.isWindowOrWindowFrame(object, true) ||
           this.isWindowOrWindowFrame(object, false))) {
        return true;
      }
    }
    ...
    for (int int0 = this.SpecialObjects.size() - 1; int0 >= 0; int0--) {
      ...
      if (object0 instanceof IsoThumpable thumpable) {
        if (thumpable.isStairs() ||
            !thumpable.isThumpable() && !thumpable.isWindow() &&
                !thumpable.isDoor() ||
      ...
      if (object0 instanceof
          IsoDoor door && boolean0 == door.north && !door.open) {
        return door;
      }
    }

    if ((!boolean0 || this.Is(IsoFlagType.WindowN)) &&
        (boolean0 || this.Is(IsoFlagType.WindowW))) {
    ...
    while (square != nullptr) {
      for (int int0 = 0; int0 < square.getObjects().size(); int0++) {
        ...
        if (object.getProperties() != nullptr &&
            object.getProperties().Is(flagType)) {
          int int1 =
              Integer.parseInt(object.getSprite().getName().split("_")[2]);
          if (int1 > 14) {
            return false;
          }

          std::string string = object.getSprite().getName().split("_")[0] +
                               "_" + object.getSprite().getName().split("_")[1];
          int1 += 40;
          object.setSprite(
              IsoSpriteManager.instance.getSprite(string + "_" + int1));
      ...
      square = square.getCell().getGridSquare(square.getX(), square.getY(),
                                              square.getZ() + 1);
    }
  ...
  bool removeSheetRopeFromBottom(IsoPlayer player, bool north) {
    ...
    if (north) {
      ...
      for (int int1 = 0; int1 < square0.getObjects().size(); int1++) {
        ...
        if (object1.sprite != nullptr && object1.sprite.getName() != nullptr &&
    ...
    for (square1 = nullptr; square0 != nullptr; boolean0 = false) {
          ...
          if (GameServer.bServer) {
            if (player != nullptr) {
              player.sendObjectChange("addItemOfType",
                                      new Object[]{"type", object2.getName()});
            }
      ...
      square0 = square0.getCell().getGridSquare(square0.getX(), square0.getY(),
                                                square0.getZ() + 1);
    }

    if (!boolean0) {
      square0 = square1.getCell().getGridSquare(square1.getX(), square1.getY(),
                                                square1.getZ());
      IsoGridSquare square2 = north ? square0.nav[IsoDirections.S.index()]
                                    : square0.nav[IsoDirections.E.index()];
      if (square2 == nullptr) {
        return true;
      }
      ...
      for (int int3 = 0; int3 < square2.getObjects().size(); int3++) {
        ...
        if (object3.getProperties() != nullptr &&
            (object3.getProperties().Is(flagType0) ||
             object3.getProperties().Is(flagType1))) {
  ...
  IsoObject getSpecialSolid() {
    for (int int0 = 0; int0 < this.SpecialObjects.size(); int0++) {
      IsoObject object0 = this.SpecialObjects.get(int0);
      if (object0 instanceof IsoThumpable thumpable && !thumpable.isStairs() &&
                                 thumpable.isThumpable() &&
                                 thumpable.isBlockAllTheSquare()) {
        if (thumpable.getProperties().Is(IsoFlagType.solidtrans) &&
            this.isAdjacentToWindow()) {
          return null;
        }
    ...
    if (next == nullptr || next == this) {
      ...
      if (object4 != nullptr) {
        ...
        if (object4 != nullptr) {
          ...
          IsoGridSquare square0 =
              this.getCell().getGridSquare(this.x, this.y - 1, this.z);
          ...
          IsoGridSquare square1 =
              this.getCell().getGridSquare(this.x - 1, this.y, this.z);
          ...
          if (square0 == nullptr || this.isBlockedTo(square0) ||
              square1 == nullptr || this.isBlockedTo(square1)) {
            return null;
      ...
      if (object5 != nullptr) {
        ...
        if (square2 != nullptr && !this.isBlockedTo(square2) &&
            square3 != nullptr && !this.isBlockedTo(square3)) {
      ...
      if (square4 != nullptr && !this.isBlockedTo(square4) &&
          square5 != nullptr && !this.isBlockedTo(square5)) {
      ...
      if (object9 != nullptr) {
        ...
        if (square6 != nullptr && !this.isBlockedTo(square6) &&
            square7 != nullptr && !this.isBlockedTo(square7)) {
  ...
  IsoObject getDoorFrameTo(IsoGridSquare next) {
    ...
    for (int int0 = 0; int0 < arrayList.size(); int0++) {
      ...
      } else if (arrayList.get(int0) instanceof
                 IsoThumpable && ((IsoThumpable)arrayList.get(int0)).isDoor) {
  ...
  getSquaresForThread(ArrayDeque<IsoGridSquare> isoGridSquareCacheDest,
    ...
    for (int int0 = 0; int0 < count; int0++) {
      ...
      if (square == nullptr) {
        isoGridSquareCacheDest.add(
            new IsoGridSquare(nullptr, nullptr, 0, 0, 0));
      } else {
  ...
  static IsoGridSquare getNew(IsoCell cell, SliceY slice, int _x, int _y,
                              int _z) {
    IsoGridSquare square = isoGridSquareCache.poll();
    if (square == nullptr) {
  ...
  static IsoGridSquare getNew(ArrayDeque<IsoGridSquare> _isoGridSquareCache,
                              IsoCell cell, SliceY slice, int _x, int _y,
                              int _z) {
...
public
  IsoGridSquare(IsoCell cell, SliceY slice, int _x, int _y, int _z) {
    this.ID = ++IDMax;
    this.x = _x;
    this.y = _y;
    this.z = _z;
  ...
  IsoObject getWall() {
    for (int int0 = 0; int0 < this.Objects.size(); int0++) {
      ...
      if (object != nullptr && object.sprite != nullptr &&
          (object.sprite.cutW || object.sprite.cutN)) {
        return object;
      }
    }
  ...
  IsoObject getThumpableWall(bool bNorth) {
    ...
    return object != nullptr && object instanceof
        IsoThumpable ? object : nullptr;
  }
  ...
  IsoObject getHoppableWall(bool bNorth) {
    for (int int0 = 0; int0 < this.Objects.size(); int0++) {
      ...
      if (object != nullptr && object.sprite != nullptr) {
        PropertyContainer propertyContainer = object.getProperties();
        bool boolean0 = propertyContainer.Is(IsoFlagType.TallHoppableW) &&
                        !propertyContainer.Is(IsoFlagType.WallWTrans);
        bool boolean1 = propertyContainer.Is(IsoFlagType.TallHoppableN) &&
                        !propertyContainer.Is(IsoFlagType.WallNTrans);
  ...
  bool getWallFull() {
    for (int int0 = 0; int0 < this.Objects.size(); int0++) {
      IsoObject object = this.Objects.get(int0);
      if (object != nullptr && object.sprite != nullptr &&
          (object.sprite.cutN || object.sprite.cutW ||
  ...
  IsoObject getWall(bool bNorth) {
    for (int int0 = 0; int0 < this.Objects.size(); int0++) {
      IsoObject object = this.Objects.get(int0);
      if (object != nullptr && object.sprite != nullptr &&
          (object.sprite.cutN && bNorth || object.sprite.cutW && !bNorth)) {
        return object;
      }
    }
  ...
  IsoObject getWallSE() {
    for (int int0 = 0; int0 < this.Objects.size(); int0++) {
      IsoObject object = this.Objects.get(int0);
      if (object != nullptr && object.sprite != nullptr &&
          object.sprite.getProperties().Is(IsoFlagType.WallSE)) {
        return object;
      }
    }
  ...
  IsoObject getFloor() {
    for (int int0 = 0; int0 < this.Objects.size(); int0++) {
      ...
      if (object.sprite != nullptr &&
          object.sprite.getProperties().Is(IsoFlagType.solidfloor)) {
        return object;
      }
    }
  ...
  IsoObject getPlayerBuiltFloor() {
    return this.getBuilding() == nullptr && this.roofHideBuilding == nullptr
               ? this.getFloor()
               : nullptr;
  }
  ...
  void EnsureSurroundNotNull() {
    ...
    for (int int0 = -1; int0 <= 1; int0++) {
      for (int int1 = -1; int1 <= 1; int1++) {
        if ((int0 != 0 || int1 != 0) &&
            IsoWorld.instance.isValidSquare(this.x + int0, this.y + int1,
                                            this.z) &&
            this.getCell().getChunkForGridSquare(this.x + int0, this.y + int1,
                                                 this.z) != nullptr) {
          IsoGridSquare square1 = this.getCell().getGridSquare(
              this.x + int0, this.y + int1, this.z);
          if (square1 == nullptr) {
            square1 = getNew(this.getCell(), nullptr, this.x + int0,
                             this.y + int1, this.z);
            IsoGridSquare square2 =
    ...
    for (int int0 = 0; int0 < this.getObjects().size(); int0++) {
      ...
      if (_sprite != nullptr &&
          (_sprite.getProperties().Is(IsoFlagType.solidfloor) ||
           ...
           _sprite.getProperties().Is(IsoFlagType.vegitation) &&
               object1.getType() != IsoObjectType.tree ||
           _sprite.getName() != nullptr &&
               _sprite.getName().startsWith("blends_grassoverlays"))) {
        if (_sprite.getName() != nullptr &&
            _sprite.getName().startsWith("floors_rugs")) {
          boolean0 = true;
        } else {
  ...
  IsoThumpable AddStairs(bool north, int level, const std::string &sprite,
    ...
    IsoThumpable thumpable0 =
        new IsoThumpable(this.getCell(), this, sprite, north, _table);
    if (north) {
      if (level == 0) {
      ...
      if (level == 2) {
        ...
        thumpable0.sprite.getProperties().Set(north ? IsoFlagType.cutN
                                                    : IsoFlagType.cutW);
      }
    }
    ...
    if (boolean0 && level == 2) {
      int int0 = this.z - 1;
      IsoGridSquare square0 =
          this.getCell().getGridSquare(this.x, this.y, int0);
      if (square0 == nullptr) {
        square0 =
            new IsoGridSquare(this.getCell(), nullptr, this.x, this.y, int0);
        this.getCell().ConnectNewSquare(square0, true);
      }

      while (int0 >= 0) {
        IsoThumpable thumpable1 = new IsoThumpable(this.getCell(), square0,
                                                   pillarSprite, north, _table);
        ...
        if (this.getCell().getGridSquare(square0.x, square0.y, --int0) ==
            nullptr) {
          square0 = new IsoGridSquare(this.getCell(), nullptr, square0.x,
                                      square0.y, int0);
          this.getCell().ConnectNewSquare(square0, true);
        } else {
    ...
    if (level == 2) {
      ...
      if (north) {
        if (IsoWorld.instance.isValidSquare(this.x, this.y - 1, this.z + 1)) {
          square1 =
              this.getCell().getGridSquare(this.x, this.y - 1, this.z + 1);
          if (square1 == nullptr) {
            square1 = new IsoGridSquare(this.getCell(), nullptr, this.x,
                                        this.y - 1, this.z + 1);
            this.getCell().ConnectNewSquare(square1, false);
          }
      ...
      } else if (IsoWorld.instance.isValidSquare(this.x - 1, this.y,
                                                 this.z + 1)) {
        square1 = this.getCell().getGridSquare(this.x - 1, this.y, this.z + 1);
        if (square1 == nullptr) {
          square1 = new IsoGridSquare(this.getCell(), nullptr, this.x - 1,
                                      this.y, this.z + 1);
          this.getCell().ConnectNewSquare(square1, false);
        }
      ...
      if (square1 == nullptr) {
        square1 = new IsoGridSquare(this.getCell(), nullptr, this.x, this.y,
                                    this.z + 1);
        this.getCell().ConnectNewSquare(square1, false);
      }
    }
      ...
      for (int int2 = this.getY() - 1; int2 <= this.getY() + 1; int2++) {
        for (int int3 = this.getZ() - 1; int3 <= this.getZ() + 1; int3++) {
          if (IsoWorld.instance.isValidSquare(int1, int2, int3)) {
            IsoGridSquare square2 =
                this.getCell().getGridSquare(int1, int2, int3);
            if (square2 == nullptr) {
              square2 =
                  new IsoGridSquare(this.getCell(), nullptr, int1, int2, int3);
  ...
  void ReCalculateAll(IsoGridSquare square0,
  ...
  void ReCalculateAll(bool boolean0, IsoGridSquare square0,
                      IsoGridSquare.GetSquare getSquare) {
    if (square0 != nullptr && square0 != this) {
      this.SolidFloorCached = false;
      square0.SolidFloorCached = false;
      ...
      for (int int1 = this.getY() - 1; int1 <= this.getY() + 1; int1++) {
        for (int int2 = this.getZ() - 1; int2 <= this.getZ() + 1; int2++) {
          if (int2 >= 0) {
            ...
            if (int3 != 0 || int4 != 0 || int5 != 0) {
              IsoGridSquare square1 =
                  this.getCell().getGridSquare(int0, int1, int2);
              if (square1 != nullptr) {
                square1.DirtySlice();
  ...
  bool IsWindow(int int2, int int1, int int0) {
    IsoGridSquare square0 = this.getCell().getGridSquare(
        this.x + int2, this.y + int1, this.z + int0);
    return this.getWindowTo(square0) != nullptr ||
           this.getWindowThumpableTo(square0) != nullptr;
  }
  ...
  void RemoveAllWith(IsoFlagType flagType) {
    for (int int0 = 0; int0 < this.Objects.size(); int0++) {
      IsoObject object = this.Objects.get(int0);
      if (object.sprite != nullptr &&
          object.sprite.getProperties().Is(flagType)) {
  ...
  bool hasSupport() {
    ...
    return square0 != nullptr && square0.Properties.Is(IsoFlagType.cutN) &&
                   !square0.Properties.Is(IsoFlagType.halfheight)
               ? true
               : square2 != nullptr &&
                     square2.Properties.Is(IsoFlagType.cutW) &&
                     !square0.Properties.Is(IsoFlagType.halfheight);
  }
  ...
  void splatBlood(int dist, float alpha) {
    ...
    for (int int0 = 0; int0 < dist; int0++) {
      if (square0 != nullptr) {
        square0 = this.getCell().getGridSquare(this.getX(), this.getY() - int0,
                                               this.getZ());
      }

      if (square1 != nullptr) {
        square1 = this.getCell().getGridSquare(this.getX() - int0, this.getY(),
                                               this.getZ());
      }

      float float0 = 0.0F;
      if (square1 != nullptr &&
          square1.testCollideAdjacent(nullptr, -1, 0, 0)) {
        bool boolean0 = false;
        bool boolean1 = false;
        uint8_t byte0 = 0;
        uint8_t byte1 = 0;
        if (square1.getS() != nullptr &&
            square1.getS().testCollideAdjacent(nullptr, -1, 0, 0)) {
          boolean0 = true;
        }

        if (square1.getN() != nullptr &&
            square1.getN().testCollideAdjacent(nullptr, -1, 0, 0)) {
          boolean1 = true;
        }
        ...
        IsoGridSquare square2 = this.getCell().getGridSquare(
            square1.getX(), square1.getY() + byte2, square1.getZ());
        IsoGridSquare square3 = this.getCell().getGridSquare(
            square1.getX(), square1.getY() + byte3, square1.getZ());
        if (square2 == nullptr || square3 == nullptr ||
            !square2.Is(IsoFlagType.cutW) || !square3.Is(IsoFlagType.cutW) ||
        ...
        if (boolean2) {
          ...
          square2.DoSplat("overlay_blood_wall_01_" + (int2 + 1), false,
                          IsoFlagType.cutW, float0, float1, alpha);
          square3.DoSplat("overlay_blood_wall_01_" + (int2 + 0), false,
                          IsoFlagType.cutW, float0, float1, alpha);
        } else {
          int int3 = 0;
          switch (Rand.Next(3)) {
          ...
          if (square1.Is(IsoFlagType.HoppableW)) {
            square1.DoSplat("overlay_blood_fence_01_" + int3, false,
                            IsoFlagType.HoppableW, float0, 0.0F, alpha);
          } else {
            square1.DoSplat("overlay_blood_wall_01_" + int3, false,
                            IsoFlagType.cutW, float0, float1, alpha);
          }
        }
      ...
      if (square0 != nullptr &&
          square0.testCollideAdjacent(nullptr, 0, -1, 0)) {
        bool boolean3 = false;
        bool boolean4 = false;
        uint8_t byte4 = 0;
        uint8_t byte5 = 0;
        if (square0.getW() != nullptr &&
            square0.getW().testCollideAdjacent(nullptr, 0, -1, 0)) {
          boolean3 = true;
        }

        if (square0.getE() != nullptr &&
            square0.getE().testCollideAdjacent(nullptr, 0, -1, 0)) {
          boolean4 = true;
        }
        ...
        IsoGridSquare square4 = this.getCell().getGridSquare(
            square0.getX() + byte6, square0.getY(), square0.getZ());
        IsoGridSquare square5 = this.getCell().getGridSquare(
            square0.getX() + byte7, square0.getY(), square0.getZ());
        if (square4 == nullptr || square5 == nullptr ||
            !square4.Is(IsoFlagType.cutN) || !square5.Is(IsoFlagType.cutN) ||
        ...
        if (boolean5) {
          ...
          square4.DoSplat("overlay_blood_wall_01_" + (int5 + 0), false,
                          IsoFlagType.cutN, float0, float2, alpha);
          square5.DoSplat("overlay_blood_wall_01_" + (int5 + 1), false,
                          IsoFlagType.cutN, float0, float2, alpha);
        } else {
          int int6 = 0;
          switch (Rand.Next(3)) {
          ...
          if (square0.Is(IsoFlagType.HoppableN)) {
            square0.DoSplat("overlay_blood_fence_01_" + int6, false,
                            IsoFlagType.HoppableN, float0, float2, alpha);
          } else {
            square0.DoSplat("overlay_blood_wall_01_" + int6, false,
                            IsoFlagType.cutN, float0, float2, alpha);
          }
        }
    ...
    if (Core.OptionBloodDecals == 0) {
      ...
      for (int int0 = 0; int0 < this.getObjects().size(); int0++) {
        ...
        if (object.wallBloodSplats != nullptr &&
            !object.wallBloodSplats.isEmpty()) {
          return true;
        }
      }

      for (int int1 = 0; int1 < this.getChunk().FloorBloodSplats.size();
           int1++) {
        IsoFloorBloodSplat floorBloodSplat =
            this.getChunk().FloorBloodSplats.get(int1);
        float float0 = floorBloodSplat.x + this.getChunk().wx * 10;
        float float1 = floorBloodSplat.y + this.getChunk().wy * 10;
        if ((int)float0 - 1 <= this.x && (int)float0 + 1 >= this.x &&
            (int)float1 - 1 <= this.y && (int)float1 + 1 >= this.y) {
          return true;
        }
      }
  ...
  void removeBlood(bool remote, bool onlyWall) {
    ...
    if (!onlyWall) {
      for (int int1 = 0; int1 < this.getChunk().FloorBloodSplats.size();
        ...
        if (int2 >= this.getX() - 1 && int2 <= this.getX() + 1 &&
            int3 >= this.getY() - 1 && int3 <= this.getY() + 1) {
  ...
  void DoSplat(const std::string &id, bool bFlip, IsoFlagType prop, float offX,
    ...
    for (int int0 = 0; int0 < this.getObjects().size(); int0++) {
      IsoObject object = this.getObjects().get(int0);
      if (object.sprite != nullptr && object.sprite.getProperties().Is(prop) &&
          (!(object instanceof IsoWindow) || !object.isDestroyed())) {
        IsoSprite sprite =
        ...
        IsoWallBloodSplat wallBloodSplat = new IsoWallBloodSplat(
            (float)GameTime.getInstance().getWorldAgeHours(), sprite);
  ...
  int RemoveTileObject(IsoObject obj) {
    ...
    if (obj != nullptr && this.Objects.contains(obj)) {
      if (obj.isTableSurface()) {
        for (int int1 = this.Objects.indexOf(obj) + 1;
             int1 < this.Objects.size(); int1++) {
          IsoObject object0 = this.Objects.get(int1);
          if (object0.isTableTopObject() || object0.isTableSurface()) {
            object0.setRenderYOffset(object0.getRenderYOffset() -
                                     obj.getSurfaceOffset());
            object0.sx = 0.0F;
            object0.sy = 0.0F;
          }
        }
  ...
  void AddSpecialObject(IsoObject obj) { this.AddSpecialObject(obj, -1); }
  ...
  void AddTileObject(IsoObject obj) { this.AddTileObject(obj, -1); }
  ...
  int placeWallAndDoorCheck(IsoObject obj, int index) {
    ...
    if (obj.sprite != nullptr) {
      ...
      bool boolean0 = objectType == IsoObjectType.doorN ||
                      objectType == IsoObjectType.doorW;
      boolean boolean1 = !boolean0 && (obj.sprite.cutW || obj.sprite.cutN ||
      ...
      if (boolean1 || boolean0) {
        for (int int1 = 0; int1 < this.Objects.size(); int1++) {
          ...
          if (object.sprite != nullptr) {
            ...
            if (boolean1 && (objectType == IsoObjectType.doorN ||
                             objectType == IsoObjectType.doorW)) {
              int0 = int1;
            }
            ...
            if (boolean0 &&
                (objectType == IsoObjectType.doorFrN ||
                 objectType == IsoObjectType.doorFrW || object.sprite.cutW ||
                 object.sprite.cutN || object.sprite.treatAsWallOrder)) {
              int0 = int1;
            }
          }
        }
  ...
  int transmitRemoveItemFromSquare(IsoObject obj) {
    if (obj != nullptr && this.Objects.contains(obj)) {
      ...
      return GameServer.bServer ? GameServer.RemoveItemFromMap(obj)
  ...
  InventoryItem AddWorldInventoryItem(const std::string &String, float _x,
    ...
    if (item == nullptr) {
      ...
      IsoWorldInventoryObject worldInventoryObject =
          new IsoWorldInventoryObject(item, this, _x, _y, height);
      item.setAutoAge();
  ...
  InventoryItem AddWorldInventoryItem(InventoryItem item, float _x, float _y,
                                      float height) {
    return this.AddWorldInventoryItem(item, _x, _y, height, true);
  }

  InventoryItem AddWorldInventoryItem(InventoryItem item, float _x, float _y,
                                      float height, bool transmit) {
  ...
  void restackSheetRope() {
    if (this.Is(IsoFlagType.climbSheetW) || this.Is(IsoFlagType.climbSheetN) ||
        this.Is(IsoFlagType.climbSheetE) || this.Is(IsoFlagType.climbSheetS)) {
  ...
  void BurnWalls(bool explode) {
    if (!GameClient.bClient) {
      if (GameServer.bServer &&
          SafeHouse.isSafeHouse(this, nullptr, false) != nullptr) {
      ...
      for (int int0 = 0; int0 < this.SpecialObjects.size(); int0++) {
        IsoObject object0 = this.SpecialObjects.get(int0);
        if (object0 instanceof
            IsoThumpable && ((IsoThumpable)object0).haveSheetRope()) {
        ...
        if (IsoWindowFrame.isWindowFrame(object0) &&
            IsoWindowFrame.haveSheetRope(object0)) {
          IsoWindowFrame.removeSheetRope(object0, nullptr);
        }

        if (object0 instanceof BarricadeAble) {
          IsoBarricade barricade0 =
              ((BarricadeAble)object0).getBarricadeOnSameSquare();
          IsoBarricade barricade1 =
              ((BarricadeAble)object0).getBarricadeOnOppositeSquare();
      ...
      if (!this.getProperties().Is(IsoFlagType.burntOut)) {
        ...
        for (int int1 = 0; int1 < this.Objects.size(); int1++) {
          ...
          if (object1.getSprite() != nullptr &&
            ...
            if (object1 instanceof
                IsoThumpable && object1.getSprite().burntTile != nullptr) {
              IsoObject object2 = IsoObject.getNew();
              object2.setSprite(IsoSpriteManager.instance.getSprite(
                  object1.getSprite().burntTile));
              ...
              object1.sprite = IsoSpriteManager.instance.getSprite(
                  object1.getSprite().burntTile);
              ...
              object1.sprite = IsoSpriteManager.instance.getSprite(
                  "fencing_burnt_01_" + (Rand.Next(15, 19) + 1));
              object1.RemoveAttachedAnims();
              if (object1.Children != nullptr) {
                object1.Children.clear();
              }
              ...
              if (!(object1 instanceof IsoBarricade) &&
                  !(object1 instanceof IsoMannequin)) {
                if (object1 instanceof IsoGenerator generator) {
                  ...
                  if (object1.getType() == IsoObjectType.wall &&
                      ...
                      !object1.getSprite().getName().startsWith(
                          "walls_exterior_roofs_") &&
                      !object1.getSprite().getName().startsWith("fencing_") &&
                      !object1.getSprite().getName().startsWith(
                          "fixtures_railings_")) {
                    if (object1.getSprite().getProperties().Is(
                            IsoFlagType.collideW) &&
                        !object1.getSprite().getProperties().Is(
                            IsoFlagType.collideN)) {
                      object1.sprite = IsoSpriteManager.instance.getSprite(
                          "walls_burnt_01_" + (Rand.Next(2) == 0 ? "0" : "4"));
                    } else if (object1.getSprite().getProperties().Is(
                                   IsoFlagType.collideN) &&
                               !object1.getSprite().getProperties().Is(
                                   IsoFlagType.collideW)) {
                      object1.sprite = IsoSpriteManager.instance.getSprite(
                          "walls_burnt_01_" + (Rand.Next(2) == 0 ? "1" : "5"));
                    } else if (object1.getSprite().getProperties().Is(
                                   IsoFlagType.collideW) &&
                               object1.getSprite().getProperties().Is(
                                   IsoFlagType.collideN)) {
                      object1.sprite = IsoSpriteManager.instance.getSprite(
                          "walls_burnt_01_" + (Rand.Next(2) == 0 ? "2" : "6"));
                    } else if (object1.getProperties().Is(IsoFlagType.WallSE)) {
                      object1.sprite = IsoSpriteManager.instance.getSprite(
                          "walls_burnt_01_" + (Rand.Next(2) == 0 ? "3" : "7"));
                    }
                  } else {
                    if (object1 instanceof IsoDoor || object1 instanceof
                        IsoWindow || object1 instanceof IsoCurtain) {
                      if (GameServer.bServer) {
                        GameServer.RemoveItemFromMap(object1);
                      } else {
                        this.RemoveTileObject(object1);
                    ...
                    if (object1.getProperties().Is(IsoFlagType.WindowW)) {
                      object1.sprite = IsoSpriteManager.instance.getSprite(
                          "walls_burnt_01_" + (Rand.Next(2) == 0 ? "8" : "12"));
                    } else if (object1.getProperties().Is("WindowN")) {
                      object1.sprite = IsoSpriteManager.instance.getSprite(
                          "walls_burnt_01_" + (Rand.Next(2) == 0 ? "9" : "13"));
                    } else if (object1.getProperties().Is(
                      ...
                      if (object1.getContainer() != nullptr &&
                          object1.getContainer().getItems() != nullptr) {
                        void *object3 = null;

                        for (int int2 = 0;
                             int2 < object1.getContainer().getItems().size();
                             int2++) {
                          object3 = object1.getContainer().getItems().get(int2);
                          if (object3 instanceof
                              Food && ((Food)object3).isAlcoholic() ||
                                  ((InventoryItem)object3).getType() ==
                                      "PetrolCan")
                        ...
                        object1.sprite = IsoSpriteManager.instance.getSprite(
                            "floors_burnt_01_" + Rand.Next(1, 2));

                        for (int int3 = 0; int3 < object1.getContainerCount();
                             int3++) {
                          ItemContainer container =
                              object1.getContainerByIndex(int3);
                      ...
                      } else if (object1.getSprite().getName().startsWith(
                                     "walls_exterior_roofs_")) {
                        object1.sprite = IsoSpriteManager.instance.getSprite(
                            "walls_burnt_roofs_01_" +
                            object1.getSprite().getName().substring(
                                object1.getSprite().getName().lastIndexOf("_") +
                                    1,
                                object1.getSprite().getName().length()));
                      } else if (!object1.getSprite().getName().startsWith(
                                     "roofs_accents")) {
                        if (object1.getSprite().getName().startsWith(
                                "roofs_")) {
                          object1.sprite = IsoSpriteManager.instance.getSprite(
                              "roofs_burnt_01_" +
                              object1.getSprite().getName().substring(
                                  object1.getSprite().getName().lastIndexOf(
                                      "_") +
                                      1,
                                  object1.getSprite().getName().length()));
                        } else if ((object1.getSprite().getName().startsWith(
                  ...
                  if (!boolean1 && !(object1 instanceof IsoThumpable)) {
                    ...
                    if (GameServer.bServer) {
                      object1.sendObjectChange("replaceWith", "object",
                                               object4);
                    }
        ...
        if (byte0 > 0 && explode) {
          if (GameServer.bServer) {
            GameServer.PlayWorldSoundServer("BurnedObjectExploded", false, this,
                                            0.0F, 50.0F, 1.0F, false);
          } else {
            SoundManager.instance.PlayWorldSound("BurnedObjectExploded", this,
                                                 0.0F, 50.0F, 1.0F, false);
          }
  ...
  bool CalculateCollide(IsoGridSquare gridSquare, bool bVision, bool bPathfind,
                        bool bIgnoreSolidTrans) {
    return this.CalculateCollide(gridSquare, bVision, bPathfind,
                                 bIgnoreSolidTrans, false);
  }

  bool CalculateCollide(IsoGridSquare gridSquare, bool bVision, bool bPathfind,
                        bool bIgnoreSolidTrans, bool bIgnoreSolid) {
    return this.CalculateCollide(gridSquare, bVision, bPathfind,
                                 bIgnoreSolidTrans, bIgnoreSolid,
                                 cellGetSquare);
  }

public
  boolean CalculateCollide(IsoGridSquare gridSquare, boolean bVision,
                           boolean bPathfind, boolean bIgnoreSolidTrans,
    ...
    if (gridSquare == nullptr && bPathfind) {
      ...
      if (!bIgnoreSolid && gridSquare.Properties.Is(IsoFlagType.solid)) {
        return this.Has(IsoObjectType.stairsTW) && !bPathfind &&
                       gridSquare.x < this.x && gridSquare.y == this.y &&
                       gridSquare.z == this.z
                   ? false
                   : !this.Has(IsoObjectType.stairsTN) || bPathfind ||
                         gridSquare.x != this.x || gridSquare.y >= this.y ||
                         gridSquare.z != this.z;
      } else {
        if (!bIgnoreSolidTrans &&
            gridSquare.Properties.Is(IsoFlagType.solidtrans)) {
          if (this.Has(IsoObjectType.stairsTW) && !bPathfind &&
              gridSquare.x < this.x && gridSquare.y == this.y &&
              gridSquare.z == this.z) {
            return false;
          }

          if (this.Has(IsoObjectType.stairsTN) && !bPathfind &&
              gridSquare.x == this.x && gridSquare.y < this.y &&
              gridSquare.z == this.z) {
            return false;
          }
          ...
          bool boolean4 = false;
          if (gridSquare.Properties.Is(IsoFlagType.windowW) ||
              gridSquare.Properties.Is(IsoFlagType.windowN)) {
            boolean4 = true;
          }

          if (!boolean4 && (gridSquare.Properties.Is(IsoFlagType.WindowW) ||
                            gridSquare.Properties.Is(IsoFlagType.WindowN))) {
            boolean4 = true;
          }
          ...
          if (!boolean4) {
            IsoGridSquare square0 =
                getter.getGridSquare(gridSquare.x, gridSquare.y + 1, this.z);
            if (square0 != nullptr && (square0.Is(IsoFlagType.windowN) ||
          ...
          if (!boolean4) {
            IsoGridSquare square1 =
                getter.getGridSquare(gridSquare.x + 1, gridSquare.y, this.z);
            if (square1 != nullptr && (square1.Is(IsoFlagType.windowW) ||
                                       square1.Is(IsoFlagType.WindowW))) {
              boolean4 = true;
            }
          }
        ...
        if (gridSquare.x != this.x && gridSquare.y != this.y &&
            this.z != gridSquare.z && bPathfind) {
          return true;
        } else if (!bPathfind || gridSquare.z >= this.z ||
                   (this.SolidFloorCached ? this.SolidFloor
                                          : this.TreatAsSolidFloor())) {
          if (bPathfind && gridSquare.z == this.z) {
            if (gridSquare.x > this.x && gridSquare.y == this.y &&
                gridSquare.Properties.Is(IsoFlagType.windowW)) {
              return false;
            }

            if (gridSquare.y > this.y && gridSquare.x == this.x &&
                gridSquare.Properties.Is(IsoFlagType.windowN)) {
              return false;
            }
          ...
          if (gridSquare.x > this.x && gridSquare.z < this.z &&
              gridSquare.Has(IsoObjectType.stairsTW)) {
            return false;
          } else if (gridSquare.y > this.y && gridSquare.z < this.z &&
                     gridSquare.Has(IsoObjectType.stairsTN)) {
            return false;
          } else {
            IsoGridSquare square2 = getter.getGridSquare(
                gridSquare.x, gridSquare.y, gridSquare.z - 1);
            if (gridSquare.x == this.x || gridSquare.z != this.z ||
                !gridSquare.Has(IsoObjectType.stairsTN) ||
                square2 != nullptr && square2.Has(IsoObjectType.stairsTN) &&
                    !bPathfind) {
              if (gridSquare.y <= this.y || gridSquare.x != this.x ||
                  gridSquare.z != this.z ||
                  !gridSquare.Has(IsoObjectType.stairsTN) ||
                ...
                if (gridSquare.x <= this.x || gridSquare.y != this.y ||
                    gridSquare.z != this.z ||
                    !gridSquare.Has(IsoObjectType.stairsTW) ||
                    square2 != nullptr && square2.Has(IsoObjectType.stairsTW) &&
                        !bPathfind) {
                  if (gridSquare.y == this.y || gridSquare.z != this.z ||
                      !gridSquare.Has(IsoObjectType.stairsTW) ||
                      square2 != nullptr &&
                          square2.Has(IsoObjectType.stairsTW) && !bPathfind) {
                    if (gridSquare.x != this.x && gridSquare.z == this.z &&
                        gridSquare.Has(IsoObjectType.stairsMN)) {
                      return true;
                    } else if (gridSquare.y != this.y &&
                               gridSquare.z == this.z &&
                               gridSquare.Has(IsoObjectType.stairsMW)) {
                      return true;
                    } else if (gridSquare.x != this.x &&
                               gridSquare.z == this.z &&
                               gridSquare.Has(IsoObjectType.stairsBN)) {
                      return true;
                    } else if (gridSquare.y != this.y &&
                               gridSquare.z == this.z &&
                               gridSquare.Has(IsoObjectType.stairsBW)) {
                      return true;
                    } else if (gridSquare.x != this.x &&
                               gridSquare.z == this.z &&
                               this.Has(IsoObjectType.stairsTN)) {
                      ...
                      return true;
                               ...
                               gridSquare.z == this.z &&
                               this.Has(IsoObjectType.stairsBN)) {
                      return true;
                    } else if (gridSquare.y != this.y &&
                               gridSquare.z == this.z &&
                               this.Has(IsoObjectType.stairsBW)) {
                      return true;
                    } else if (gridSquare.y < this.y &&
                               gridSquare.x == this.x &&
                               gridSquare.z > this.z &&
                      ...
                      return false;
                               ...
                               (gridSquare.SolidFloorCached
                                    ? !gridSquare.SolidFloor
                                    : !gridSquare.TreatAsSolidFloor()) &&
                               bPathfind) {
                      return true;
                    } else {
                      ...
                      if (this.z == gridSquare.z) {
                        bool boolean5 = boolean2 && this.Properties.Is(
                                                        IsoFlagType.collideN);
                        bool boolean6 = boolean0 && this.Properties.Is(
                                                        IsoFlagType.collideW);
                        bool boolean7 = boolean3 && gridSquare.Properties.Is(
                                                        IsoFlagType.collideN);
                        bool boolean8 = boolean1 && gridSquare.Properties.Is(
                                                        IsoFlagType.collideW);
                        if (boolean5 && bPathfind &&
                            this.Properties.Is(IsoFlagType.canPathN)) {
                          boolean5 = false;
                        }

                        if (boolean6 && bPathfind &&
                            this.Properties.Is(IsoFlagType.canPathW)) {
                          boolean6 = false;
                        }

                        if (boolean7 && bPathfind &&
                            gridSquare.Properties.Is(IsoFlagType.canPathN)) {
                          boolean7 = false;
                        }

                        if (boolean8 && bPathfind &&
                            gridSquare.Properties.Is(IsoFlagType.canPathW)) {
                          boolean8 = false;
                        }

                        if (boolean6 && this.Has(IsoObjectType.stairsTW) &&
                            !bPathfind) {
                          boolean6 = false;
                        }

                        if (boolean5 && this.Has(IsoObjectType.stairsTN) &&
                            !bPathfind) {
                          boolean5 = false;
                        }
                        ...
                        if (!boolean5 && !boolean6 && !boolean7 && !boolean8) {
                          bool boolean9 =
                              gridSquare.x != this.x && gridSquare.y != this.y;
                          if (boolean9) {
                            IsoGridSquare square3 = getter.getGridSquare(
                                this.x, gridSquare.y, this.z);
                            IsoGridSquare square4 = getter.getGridSquare(
                                gridSquare.x, this.y, this.z);
                            if (square3 != nullptr && square3 != this &&
                                square3 != gridSquare) {
                              square3.RecalcPropertiesIfNeeded();
                            }

                            if (square4 != nullptr && square4 != this &&
                                square4 != gridSquare) {
                              square4.RecalcPropertiesIfNeeded();
                            }

                            if (gridSquare == this || square3 == square4 ||
                                square3 == this || square4 == this ||
                                square3 == gridSquare ||
                                square4 == gridSquare) {
                              return true;
                            }
                            ...
                            if (gridSquare.x == this.x + 1 &&
                                ...
                                square3 != nullptr && square4 != nullptr &&
                            ...
                            if (this.CalculateCollide(
                                    square3, bVision, bPathfind,
                                    bIgnoreSolidTrans, false, getter)) {
                              return true;
                            }

                            if (this.CalculateCollide(
                                    square4, bVision, bPathfind,
                                    bIgnoreSolidTrans, false, getter)) {
                              return true;
                            }

                            if (gridSquare.CalculateCollide(
                                    square3, bVision, bPathfind,
                                    bIgnoreSolidTrans, false, getter)) {
                              return true;
                            }
          ...
          return gridSquare.Has(IsoObjectType.stairsTN) ||
                 gridSquare.Has(IsoObjectType.stairsTW);
        }
      }
    }
  ...
  bool CalculateVisionBlocked(IsoGridSquare gridSquare,
    ...
    if (gridSquare == nullptr) {
      return false;
    } else if (Math.abs(gridSquare.getX() - this.getX()) <= 1 &&
               Math.abs(gridSquare.getY() - this.getY()) <= 1) {
      bool boolean0 = false;
      bool boolean1 = false;
      bool boolean2 = false;
      bool boolean3 = false;
      ...
      if (!gridSquare.Properties.Is(IsoFlagType.trans) &&
          !this.Properties.Is(IsoFlagType.trans)) {
        if (this.z != gridSquare.z) {
          if (gridSquare.z > this.z) {
            ...
            IsoGridSquare square0 =
                getter.getGridSquare(this.x, this.y, gridSquare.z);
            if (square0 == nullptr) {
              return false;
            }

            if ((square0.SolidFloorCached ? square0.SolidFloor
                                          : square0.TreatAsSolidFloor()) &&
                !square0.getProperties().Is(IsoFlagType.transparentFloor)) {
              return true;
            }
          } else {
            if ((this.SolidFloorCached ? this.SolidFloor
                                       : this.TreatAsSolidFloor()) &&
                !this.getProperties().Is(IsoFlagType.transparentFloor)) {
              return true;
            }
            ...
            IsoGridSquare square1 =
                getter.getGridSquare(gridSquare.x, gridSquare.y, this.z);
            if (square1 == nullptr) {
              return false;
            }

            if ((square1.SolidFloorCached ? square1.SolidFloor
        ...
        if (gridSquare.x > this.x &&
            gridSquare.Properties.Is(IsoFlagType.transparentW)) {
          return false;
        } else if (gridSquare.y > this.y &&
                   gridSquare.Properties.Is(IsoFlagType.transparentN)) {
          return false;
        } else if (gridSquare.x < this.x &&
                   this.Properties.Is(IsoFlagType.transparentW)) {
          return false;
        } else if (gridSquare.y < this.y &&
                   this.Properties.Is(IsoFlagType.transparentN)) {
          return false;
        } else if (gridSquare.x > this.x &&
                   gridSquare.Properties.Is(IsoFlagType.doorW)) {
          return false;
        } else if (gridSquare.y > this.y &&
                   gridSquare.Properties.Is(IsoFlagType.doorN)) {
          return false;
        } else if (gridSquare.x < this.x &&
                   this.Properties.Is(IsoFlagType.doorW)) {
          return false;
          ...
          bool boolean6 =
              boolean3 && gridSquare.Properties.Is(IsoFlagType.collideN);
          bool boolean7 =
              boolean1 && gridSquare.Properties.Is(IsoFlagType.collideW);
          if (!boolean4 && !boolean5 && !boolean6 && !boolean7) {
            ...
            if (!gridSquare.Properties.Is(IsoFlagType.solid) &&
                !gridSquare.Properties.Is(IsoFlagType.blocksight)) {
              if (boolean8) {
                IsoGridSquare square2 =
                    getter.getGridSquare(this.x, gridSquare.y, this.z);
                IsoGridSquare square3 =
                    getter.getGridSquare(gridSquare.x, this.y, this.z);
                if (square2 != nullptr && square2 != this &&
                    square2 != gridSquare) {
  ...
  IsoGameCharacter FindFriend(IsoGameCharacter g, int range,
    ...
    for (int int0 = 0; int0 < g.getLocalList().size(); int0++) {
      ...
      if (movingObject != g && movingObject != g.getFollowingTarget() &&
              movingObject instanceof
          IsoGameCharacter && !(movingObject instanceof IsoZombie) &&
...
IsoGameCharacter
FindEnemy(IsoGameCharacter g, int range, ArrayList<IsoMovingObject> EnemyList,
          IsoGameCharacter RangeTest, int TestRangeMax) {
  float float0 = 1000000.0F;
  IsoGameCharacter character0 = null;
  ...
  for (int int0 = 0; int0 < EnemyList.size(); int0++) {
    ...
    float1 += Math.abs(this.getY() - character1.getY());
    float1 += Math.abs(this.getZ() - character1.getZ());
    if (float1 < range && float1 < float0 &&
        character1.DistTo(RangeTest) < TestRangeMax) {
      character0 = character1;
      float0 = float1;
    }
  }
...
IsoGameCharacter FindEnemy(IsoGameCharacter g, int range,
...
void RecalcProperties() {
  ...
  for (int int1 = 0; int1 < int0; int1++) {
    ...
    if (object != nullptr) {
      PropertyContainer propertyContainer = object.getProperties();
      if (propertyContainer != nullptr &&
          !propertyContainer.Is(IsoFlagType.blueprint)) {
        ...
        if (!boolean3 && propertyContainer.Is(IsoFlagType.collideN) &&
            !propertyContainer.Is(IsoFlagType.HoppableN)) {
          boolean3 = true;
        }

        if (!boolean4 && propertyContainer.Is(IsoFlagType.collideW) &&
            !propertyContainer.Is(IsoFlagType.HoppableW)) {
          boolean4 = true;
        }

        if (!boolean5 && propertyContainer.Is(IsoFlagType.cutN) &&
            !propertyContainer.Is(IsoFlagType.transparentN)) {
          boolean5 = true;
        }

        if (!boolean6 && propertyContainer.Is(IsoFlagType.cutW) &&
            !propertyContainer.Is(IsoFlagType.transparentW)) {
          boolean6 = true;
        }
      }
    }
  ...
  if (this.chunk != nullptr) {
    this.chunk.lightCheck[0] = this.chunk.lightCheck[1] =
        this.chunk.lightCheck[2] = this.chunk.lightCheck[3] = true;
  }
...
void ReCalculateCollide(IsoGridSquare square, IsoGridSquare.GetSquare getter) {
  if (1 + (square.x - this.x) < 0 || 1 + (square.y - this.y) < 0 ||
      1 + (square.z - this.z) < 0) {
    DebugLog.log("ERROR");
  }

  bool boolean0 =
      this.CalculateCollide(square, false, false, false, false, getter);
  this.collideMatrix =
      setMatrixBit(this.collideMatrix, 1 + (square.x - this.x),
                   1 + (square.y - this.y), 1 + (square.z - this.z), boolean0);
}
...
void ReCalculatePathFind(IsoGridSquare square, IsoGridSquare.GetSquare getter) {
  bool boolean0 =
      this.CalculateCollide(square, false, true, false, false, getter);
  this.pathMatrix =
      setMatrixBit(this.pathMatrix, 1 + (square.x - this.x),
                   1 + (square.y - this.y), 1 + (square.z - this.z), boolean0);
}
...
void ReCalculateVisionBlocked(IsoGridSquare square,
  ...
  this.visionMatrix =
      setMatrixBit(this.visionMatrix, 1 + (square.x - this.x),
                   1 + (square.y - this.y), 1 + (square.z - this.z), boolean0);
}

static bool testCollideSpecialObjects(IsoMovingObject movingObject,
                                      IsoGridSquare square1,
                                      IsoGridSquare square0) {
...
bool testCollideAdjacent(IsoMovingObject collideObject, int _x, int _y,
                         int _z) {
  if (collideObject instanceof
      IsoPlayer && ((IsoPlayer)collideObject).isNoClip()) {
    return false;
  } else if (this.collideMatrix == -1) {
    return true;
  } else if (_x >= -1 && _x <= 1 && _y >= -1 && _y <= 1 && _z >= -1 &&
             _z <= 1) {
    if (this.x + _x >= 0 && this.y + _y >= 0 &&
        IsoWorld.instance.MetaGrid.isValidChunk((this.x + _x) / 10,
                                                (this.y + _y) / 10)) {
      IsoGridSquare square0 =
          this.getCell().getGridSquare(this.x + _x, this.y + _y, this.z + _z);
      SafeHouse safeHouse = null;
      if ((GameServer.bServer || GameClient.bClient) && collideObject instanceof
          IsoPlayer &&
              !ServerOptions.instance.SafehouseAllowTrepass.getValue()) {
        IsoGridSquare square1 =
            this.getCell().getGridSquare(this.x + _x, this.y + _y, 0);
        safeHouse = SafeHouse.isSafeHouse(
            square1, ((IsoPlayer)collideObject).getUsername(), true);
      }
      ...
      if (safeHouse != nullptr) {
        ...
        if (UseSlowCollision) {
          ...
          if (collideObject instanceof
              IsoPlayer &&
                  getMatrixBit(this.collideMatrix, _x + 1, _y + 1, _z + 1)) {
            this.RecalcAllWithNeighbours(true);
          }
...
bool testCollideAdjacentAdvanced(int _x, int _y, int _z, bool ignoreDoors) {
  if (this.collideMatrix == -1) {
    ...
    return UseSlowCollision
               ? this.CalculateCollide(square, false, false, false)
               : getMatrixBit(this.collideMatrix, _x + 1, _y + 1, _z + 1);
  } else {
    return true;
  }
...
static void setCollisionMode() { UseSlowCollision = !UseSlowCollision; }
...
bool testPathFindAdjacent(IsoMovingObject mover, int _x, int _y, int _z,
  ...
  if (_x >= -1 && _x <= 1 && _y >= -1 && _y <= 1 && _z >= -1 && _z <= 1) {
    if (this.Has(IsoObjectType.stairsTN) || this.Has(IsoObjectType.stairsTW)) {
      IsoGridSquare square0 =
          getter.getGridSquare(_x + this.x, _y + this.y, _z + this.z);
      if (square0 == nullptr) {
        return true;
      }

      if (this.Has(IsoObjectType.stairsTN) && square0.y < this.y &&
          square0.z == this.z) {
        return true;
      }

      if (this.Has(IsoObjectType.stairsTW) && square0.x < this.x &&
          square0.z == this.z) {
        return true;
      }
    }
...
public
LosUtil.TestResults testVisionAdjacent(int _x, int _y, int _z,
                                       boolean specialDiag,
                                       boolean bIgnoreDoors) {
  if (_x >= -1 && _x <= 1 && _y >= -1 && _y <= 1 && _z >= -1 && _z <= 1) {
    if (_z == 1 && (_x != 0 || _y != 0) && this.HasElevatedFloor()) {
      IsoGridSquare square0 =
          this.getCell().getGridSquare(this.x, this.y, this.z + _z);
      if (square0 != nullptr) {
    ...
    if (_x != 0 && _y != 0 && specialDiag) {
      ...
      if (square3 != nullptr && square3.z == this.z) {
        if (!this.SpecialObjects.isEmpty()) {
          for (int int0 = 0; int0 < this.SpecialObjects.size(); int0++) {
            ...
            IsoObject.VisionResult visionResult0 =
                object0.TestVision(this, square3);
            if (visionResult0 != IsoObject.VisionResult.NoEffect) {
              if (visionResult0 == IsoObject.VisionResult.Unblocked &&
                      object0 instanceof
                  IsoDoor) {
                testResults1 = ((IsoDoor)object0).IsOpen()
                             ...
                             object0 instanceof
                         IsoThumpable && ((IsoThumpable)object0).isDoor) {
                ...
                if (visionResult0 == IsoObject.VisionResult.Blocked &&
                        object0 instanceof
                    IsoThumpable && ((IsoThumpable)object0).isWindow()) {
                  return LosUtil.TestResults.Blocked;
                }
        ...
        if (!square3.SpecialObjects.isEmpty()) {
          for (int int1 = 0; int1 < square3.SpecialObjects.size(); int1++) {
            ...
            IsoObject.VisionResult visionResult1 =
                object1.TestVision(this, square3);
            if (visionResult1 != IsoObject.VisionResult.NoEffect) {
              if (visionResult1 == IsoObject.VisionResult.Unblocked &&
                      object1 instanceof
                  IsoDoor) {
                testResults1 = ((IsoDoor)object1).IsOpen()
                             ...
                             object1 instanceof
                         IsoThumpable && ((IsoThumpable)object1).isDoor) {
                testResults1 = LosUtil.TestResults.ClearThroughOpenDoor;
              } else if (visionResult1 == IsoObject.VisionResult.Unblocked &&
                             object1 instanceof
                         IsoWindow) {
                ...
                if (visionResult1 == IsoObject.VisionResult.Blocked &&
                        object1 instanceof
                    IsoThumpable && ((IsoThumpable)object1).isWindow()) {
                  return LosUtil.TestResults.Blocked;
                }
      ...
      return !getMatrixBit(this.visionMatrix, _x + 1, _y + 1, _z + 1)
                 ? testResults1
...
void AddSpecialTileObject(IsoObject obj) { this.AddSpecialObject(obj); }
...
void renderCharacters(int maxZ, bool deadRender, bool doBlendFunc) {
  if (this.z < maxZ) {
    ...
    for (int int2 = 0; int2 < int1; int2++) {
      ...
      if ((movingObject0.sprite != nullptr || movingObject0 instanceof
           IsoDeadBody) &&
          (!deadRender || movingObject0 instanceof
           IsoDeadBody && !this.HasStairs()) &&
          (deadRender || !(movingObject0 instanceof IsoDeadBody) ||
           this.HasStairs())) {
        movingObject0.render(movingObject0.getX(), movingObject0.getY(),
                             movingObject0.getZ(), colorInfo, true, false,
                             nullptr);
      }
    }
    ...
    for (int int3 = 0; int3 < int1; int3++) {
      ...
      if (movingObject1 != nullptr && movingObject1.sprite != nullptr) {
        ...
        if ((!deadRender || boolean0) && (deadRender || !boolean0)) {
          movingObject1.render(movingObject1.getX(), movingObject1.getY(),
                               movingObject1.getZ(), colorInfo, true, false,
                               nullptr);
        }
      }
    }
...
void renderDeferredCharacters(int maxZ) {
  if (!this.DeferedCharacters.isEmpty()) {
    if (this.DeferredCharacterTick != this.getCell().DeferredCharacterTick) {
      ...
      short short0 =
          this.getCell().getStencilValue2z(this.x, this.y, this.z - 1);
      ...
      Texture.getWhite().renderwallnw(float0, float1, 64 * Core.TileScale,
                                      32 * Core.TileScale, -1, -1, -1, -1, -1,
                                      -1);
      ...
      for (int int0 = 0; int0 < this.DeferedCharacters.size(); int0++) {
        ...
        if (character.sprite != nullptr) {
          ...
          character.render(character.getX(), character.getY(), character.getZ(),
                           colorInfo, true, false, nullptr);
          character.renderObjectPicker(character.getX(), character.getY(),
                                       character.getZ(), colorInfo);
...
bool IsOnScreen() { return this.IsOnScreen(false); }
...
bool IsOnScreen(bool halfTileBorder) {
  ...
  if (this.hasTree) {
    ...
    if (float0 + int1 <= 0 - int0) {
      ...
      return float0 - int1 >= IsoCamera.frameState.OffscreenWidth + int0
                 ? false
                 : !(float1 - int2 >=
                     IsoCamera.frameState.OffscreenHeight + int0);
    }
    ...
    return float0 - 32 * Core.TileScale >=
                   IsoCamera.frameState.OffscreenWidth + int0
               ? false
               : !(float1 - 96 * Core.TileScale >=
                   IsoCamera.frameState.OffscreenHeight + int0);
  }
}
...
void setLightInfoServerGUIOnly(ColorInfo c) { this.lightInfo[0] = c; }
...
int renderFloorInternal(Shader shader) {
  ...
  bool boolean1 = GameClient.bClient && IsoPlayer.players[int0] != null &&
                  IsoPlayer.players[int0].isSeeNonPvpZone();
  bool boolean2 = Core.bDebug && GameClient.bClient &&
                  SafeHouse.isSafeHouse(this, null, true) != null;
  bool boolean3 = true;
  float float1 = 1.0F;
  float float2 = 1.0F;
  if (square1 != nullptr) {
    ...
    if (int1 != -1) {
      ...
      if (int2 == -1 &&
          IsoWorld.instance.CurrentCell.CanBuildingSquareOccludePlayer(this,
                                                                       int0)) {
        boolean3 = false;
        float1 = 1.0F;
        float2 = 1.0F;
  ...
  bool boolean4 = waterGeometry != null && waterGeometry.bShore;
  ...
  for (int int4 = 0; int4 < int3; int4++) {
    ...
    bool boolean5 = true;
    if (object0.sprite != nullptr && !object0.sprite.solidfloor &&
        object0.sprite.renderLayer != 1) {
      boolean5 = false;
      byte0 |= 4;
    }
    ...
    if (!boolean5) {
      bool boolean6 = object0.sprite != null &&
                      (object0.sprite.isBush || object0.sprite.canBeRemoved ||
                       object0.sprite.attachedFloor);
      ...
      if (DebugOptions.instance.Terrain.RenderTiles.RenderGridSquares
              .getValue() &&
          object0.sprite != nullptr) {
        IndieGL.StartShader(shader, int0);
        FloorShaperAttachedSprites floorShaperAttachedSprites =
            FloorShaperAttachedSprites.instance;
        void *object1;
        if (!object0.getProperties().Is(IsoFlagType.diamondFloor) &&
            !object0.getProperties().Is(IsoFlagType.water)) {
        ...
        if (DebugOptions.instance.Terrain.RenderTiles.IsoGridSquare.Floor
                .LightingDebug.getValue()) {
          int5 = -65536;
          int6 = -65536;
          int7 = -16776961;
        ...
        floorShaperAttachedSprites.setWaterDepth(float3, float4, float5,
                                                 float6);
        ...
        object0.renderFloorTile(
            ...
            PerformanceSettings.LightingFrameSkip < 3 ? defColorInfo
                                                      : colorInfo,
            true, false, shader, (Consumer<TextureDraw>)object1,
  ...
  if ((this.getCell().rainIntensity > 0 ||
       RainManager.isRaining() && RainManager.RainIntensity > 0.0F) &&
      this.isExteriorCache && !this.isVegitationCache &&
      this.isSolidFloorCache && this.isCouldSee(int0)) {
    if (!IsoCamera.frameState.Paused) {
      int int9 =
          this.getCell().rainIntensity == 0
              ? (int)Math.min(
                    Math.floor(RainManager.RainIntensity / 0.2F) + 1.0, 5.0)
              : this.getCell().rainIntensity;
      if (this.splashFrame < 0.0F &&
          Rand.Next(Rand.AdjustForFramerate((int)(5.0F / int9) * 100)) == 0) {
    ...
    if (this.splashFrame >= 0.0F) {
      ...
      if (texture != nullptr) {
        float float7 = IsoUtils.XToScreen(this.x + this.splashX,
                                          this.y + this.splashY, this.z, 0) -
                       IsoCamera.frameState.OffX;
        float float8 = IsoUtils.YToScreen(this.x + this.splashX,
                                          this.y + this.splashY, this.z, 0) -
                       IsoCamera.frameState.OffY;
        float7 -= texture.getWidth() / 2 * Core.TileScale;
        float8 -= texture.getHeight() / 2 * Core.TileScale;
        float float9 = 0.6F * (this.getCell().rainIntensity > 0
                                   ? 1.0F
                                   : RainManager.RainIntensity);
        float float10 = Core.getInstance().RenderShader != null ? 0.6F : 1.0F;
        SpriteRenderer.instance.render(
            texture, float7, float8, texture.getWidth() * Core.TileScale,
            texture.getHeight() * Core.TileScale, 0.8F * colorInfo.r,
            0.9F * colorInfo.g, 1.0F * colorInfo.b, float9 * float10, nullptr);
      }

      if (!IsoCamera.frameState.Paused &&
          this.splashFrameNum != IsoCamera.frameState.frameCount) {
        this.splashFrame = this.splashFrame +
                           0.08F * (30.0F / PerformanceSettings.getLockFPS());
...
bool isSpriteOnSouthOrEastWall(IsoObject object) {
  if (object instanceof IsoBarricade) {
    return object.getDir() == IsoDirections.S ||
           object.getDir() == IsoDirections.E;
  } else if (object instanceof IsoCurtain curtain) {
    return curtain.getType() == IsoObjectType.curtainS ||
           curtain.getType() == IsoObjectType.curtainE;
  } else {
    PropertyContainer propertyContainer = object.getProperties();
...
void RenderOpenDoorOnly() {
  ...
  try {
    ...
    for (int int2 = byte0; int2 <= int1; int2++) {
      ...
      if (object.sprite != nullptr &&
          (object.sprite.getProperties().Is(IsoFlagType.attachedN) ||
           object.sprite.getProperties().Is(IsoFlagType.attachedW))) {
...
bool RenderMinusFloorFxMask(int maxZ, bool doSE, bool vegitationRender) {
  ...
  try {
    ...
    for (int int3 = int1; doSE ? int3 >= int2 : int3 <= int2;
         int3 += doSE ? -1 : 1) {
      ...
      if (object.sprite != nullptr) {
        ...
        bool boolean2 = object.sprite.isBush || object.sprite.canBeRemoved ||
                        object.sprite.attachedFloor;
        if ((!vegitationRender || boolean2 && this.bFlattenGrassEtc) &&
            (vegitationRender || !boolean2 || !this.bFlattenGrassEtc)) {
          if ((objectType == IsoObjectType.WestRoofB ||
               objectType == IsoObjectType.WestRoofM ||
               objectType == IsoObjectType.WestRoofT) &&
              this.z == maxZ - 1 &&
          ...
          if (boolean1) {
            if (!object.sprite.cutW && !object.sprite.cutN) {
              ...
              bool boolean5 =
                  square0 != null && square0.getPlayerCutawayFlag(int4, long0);
              bool boolean6 = this.getPlayerCutawayFlag(int4, long0);
              bool boolean7 =
                  square1 != null && square1.getPlayerCutawayFlag(int4, long0);
              ...
              this.DoCutawayShaderSprite(object.sprite, directions, boolean5,
                                         boolean6, boolean7);
            }
          }
        }
...
bool isWindowOrWindowFrame(IsoObject object, bool boolean0) {
  if (object != nullptr && object.sprite != nullptr) {
    if (boolean0 && object.sprite.getProperties().Is(IsoFlagType.windowN)) {
      ...
      return thumpable != nullptr && thumpable.isWindow()
                 ? boolean0 == thumpable.getNorth()
                 : IsoWindowFrame.isWindowFrame(object, boolean0);
    }
  } else {
    return false;
  }
...
bool renderMinusFloor(int int0, bool boolean1, bool boolean2, bool boolean3,
                      bool boolean4, bool boolean5, Shader shader) {
  bool boolean0 = false;
  if (!this.localTemporaryObjects.isEmpty()) {
    boolean0 =
        this.renderMinusFloor(this.localTemporaryObjects, int0, boolean1,
                              boolean2, boolean3, boolean4, boolean5, shader);
  }

  return this.renderMinusFloor(this.Objects, int0, boolean1, boolean2, boolean3,
                               boolean4, boolean5, shader) ||
         boolean0;
}

boolean renderMinusFloor(PZArrayList<IsoObject> pZArrayList, int int8,
                         boolean boolean4, boolean boolean13, boolean boolean11,
                         boolean boolean9, boolean boolean10, Shader shader) {
  if (!DebugOptions.instance.Terrain.RenderTiles.IsoGridSquare.RenderMinusFloor
           .getValue()) {
    return false;
  } else {
    IndieGL.glBlendFunc(770, 771);
    ...
    if (!boolean4) {
      for (int int5 = int3; int5 <= int4; int5++) {
        ...
        if (object0.sprite != nullptr &&
            (object0.sprite.getType() == IsoObjectType.doorFrN ||
             object0.sprite.getType() == IsoObjectType.doorN) &&
            (boolean9 || boolean10)) {
        ...
        if (object0.sprite != nullptr &&
            (object0.sprite.getType() == IsoObjectType.doorFrW ||
             object0.sprite.getType() == IsoObjectType.doorW) &&
            (boolean9 || boolean11)) {
    ...
    for (int int7 = int3; boolean4 ? int7 >= int4 : int7 <= int4;
         int7 += boolean4 ? -1 : 1) {
      ...
      CircleStencil = false;
      if (object1.sprite != nullptr &&
          (object1.sprite.solidfloor || object1.sprite.renderLayer == 1)) {
        boolean12 = false;
      }
      ...
      if (this.z >= int8 &&
          (object1.sprite == nullptr || !object1.sprite.alwaysDraw)) {
        boolean12 = false;
      }

      bool boolean14 = object1.sprite != null &&
                       (object1.sprite.isBush || object1.sprite.canBeRemoved ||
                        object1.sprite.attachedFloor);
      if ((!boolean13 || boolean14 && this.bFlattenGrassEtc) &&
          (boolean13 || !boolean14 || !this.bFlattenGrassEtc)) {
        if (object1.sprite != nullptr &&
            (objectType == IsoObjectType.WestRoofB ||
             objectType == IsoObjectType.WestRoofM ||
             objectType == IsoObjectType.WestRoofT) &&
            this.z == int8 - 1 &&
            this.z == (int)IsoCamera.CamCharacter.getZ()) {
        ...
        boolean boolean17 = object1 instanceof
            IsoDoor && ((IsoDoor)object1).open || object1 instanceof
            IsoThumpable && ((IsoThumpable)object1).open;
        bool boolean18 = object1.container != null;
        bool boolean19 =
            object1.sprite != null &&
            object1.sprite.getProperties().Is(IsoFlagType.waterPiped);
        if (object1.sprite != nullptr && objectType == IsoObjectType.MAX &&
            !(object1 instanceof IsoDoor) && !(object1 instanceof IsoWindow) &&
            !boolean18 && !boolean19) {
          if (boolean15 ||
              !object1.sprite.getProperties().Is(IsoFlagType.attachedW) ||
              !boolean1 && !boolean11 && !boolean9) {
            if (!boolean16 &&
                object1.sprite.getProperties().Is(IsoFlagType.attachedN) &&
                (boolean1 || boolean9 || boolean10)) {
        ...
        if (object1.sprite != nullptr && !object1.sprite.solidfloor &&
            IsoPlayer.getInstance().isClimbing()) {
          boolean12 = true;
        }
        ...
        if (boolean12) {
          ...
          if (object1.sprite == nullptr || !boolean15 && !boolean16) {
            if (DebugOptions.instance.Terrain.RenderTiles.IsoGridSquare.Objects
                    .getValue()) {
              if (this.getRoomID() != -1 && this.getRoomID() != int6 &&
                  IsoPlayer.players[int1].isSeatedInVehicle() &&
                  IsoPlayer.players[int1]
                          .getVehicle()
                          .getCurrentSpeedKmHour() >= 50.0F) {
                break;
              }
              ...
              if (objectType != IsoObjectType.WestRoofB &&
                      objectType != IsoObjectType.WestRoofM &&
                      objectType != IsoObjectType.WestRoofT ||
                  (this.getRoomID() != -1 || !boolean10 && !boolean11 ||
                   (!(IsoPlayer.players[int1].getX() < this.x) ||
                    !(IsoPlayer.players[int1].getY() < this.y)) &&
                       !(IsoPlayer.players[int1].getZ() < this.z)) &&
                      (this.getRoomID() == -1 && int6 == -1 ||
                       !(IsoPlayer.players[int1].getX() +
                             IsoPlayer.players[int1].getY() <=
                         this.x + this.y + 6) ||
                       !(IsoPlayer.players[int1].getZ() < this.z + 1))) {
                if (square1 != nullptr && !boolean0 &&
                    this.getRoomID() != int6 && float0 < 0.5F) {
                  ...
                  if (IsoPlayer.getInstance() != nullptr &&
                          ...
                          (object1.getProperties().Is(IsoFlagType.solid) ||
                           ...
                           object1.getProperties().Is(
                               IsoFlagType.attachedCeiling) ||
                           object1.getSprite().getProperties().Is(
                               IsoFlagType.attachedE) ||
                           object1.getSprite().getProperties().Is(
                               IsoFlagType.attachedS)) ||
                      objectType.index() > 2 && objectType.index() < 9 &&
                          IsoCamera.frameState.CamCharacterZ <=
                              object1.getZ()) {
                    uint8_t byte0 = 2;
                    float float3 = 0.75F;
                    ...
                    int int9 =
                        this.getX() - (int)IsoPlayer.getInstance().getX();
                    int int10 =
                        this.getY() - (int)IsoPlayer.getInstance().getY();
                    if (int9 > 0 && int9 < byte0 && int10 >= 0 &&
                            int10 < byte0 ||
                        int10 > 0 && int10 < byte0 && int9 >= 0 &&
                            int9 < byte0) {
                      object1.setTargetAlpha(int1, float3);
                    }

                    IsoZombie zombie0 =
                        IsoCell.getInstance().getNearestVisibleZombie(int1);
                    if (zombie0 != nullptr &&
                        zombie0.getCurrentSquare() != nullptr &&
                        zombie0.getCurrentSquare().isCanSee(int1)) {
                      int int11 = this.getX() - (int)zombie0.x;
                      int int12 = this.getY() - (int)zombie0.y;
                      if (int11 > 0 && int11 < byte0 && int12 >= 0 &&
                              int12 < byte0 ||
                          int12 > 0 && int12 < byte0 && int11 >= 0 &&
                              int11 < byte0) {
                        object1.setTargetAlpha(int1, float3);
                      }
                    }
              ...
              if (object1 instanceof IsoWindow window) {
                if (object1.getTargetAlpha(int1) < 1.0E-4F) {
                  ...
                  if (square6 != nullptr && square6 != this &&
                      square6.lighting[int1].bSeen()) {
                    object1.setTargetAlpha(
                        int1, square6.lighting[int1].darkMulti() * 2.0F);
                  }
                }

                if (object1.getTargetAlpha(int1) > 0.4F && boolean9 &&
                    (boolean10 && object1.sprite.getProperties().Is(
                                      IsoFlagType.windowN) ||
                     boolean11 && object1.sprite.getProperties().Is(
                                      IsoFlagType.windowW))) {
                  float float4 = 0.4F;
                  float float5 = 0.1F;
                  ...
                  if (player != nullptr) {
                    float float6 = 5.0F;
                    float float7 = Math.abs(player.x - this.x) *
                                       Math.abs(player.x - this.x) +
                                   Math.abs(player.y - this.y) *
                                       Math.abs(player.y - this.y);
                    float float8 =
                        float4 * (float)(1.0 - Math.sqrt(float7 / float6));
              ...
              if (object1 instanceof IsoTree) {
                if (boolean2 &&
                    ...
                    square1 != nullptr && square1.Is(IsoFlagType.exterior)) {
                  ((IsoTree)object1).bRenderFlag = true;
                  object1.setTargetAlpha(
                      int1, Math.min(0.99F, object1.getTargetAlpha(int1)));
                } else {
                  ((IsoTree)object1).bRenderFlag = false;
                }
              }

              IsoWorldInventoryObject worldInventoryObject0 =
                  Type.tryCastTo(object1, IsoWorldInventoryObject.class);
              if (worldInventoryObject0 != nullptr) {
                tempWorldInventoryObjects.add(worldInventoryObject0);
              }

              object1.render(this.x, this.y, this.z, colorInfo, true, false,
                             nullptr);
            }
          } else if (PerformanceSettings.LightingFrameSkip < 3) {
            if (DebugOptions.instance.Terrain.RenderTiles.IsoGridSquare
                    .DoorsAndWalls.getValue()) {
              CircleStencil = true;
              if (square1 != nullptr && this.getRoomID() != -1 && int6 == -1 &&
                  boolean1) {
                object1.setTargetAlpha(int1, 0.5F);
                object1.setAlpha(int1, 0.5F);
              } else if (this.getRoomID() == int6 || boolean0 ||
                         !object1.getProperties().Is(
                             IsoFlagType.transparentN) &&
                             !object1.getProperties().Is(
                                 IsoFlagType.transparentW)) {
              ...
              if (object1.sprite.cutW && object1.sprite.cutN) {
                int0 = this.DoWallLightingNW(object1, int0, boolean11, boolean9,
                                             boolean10, boolean5, boolean6,
                                             boolean7, boolean8, shader);
              } else if (object1.sprite.getType() == IsoObjectType.doorFrW ||
                         objectType == IsoObjectType.doorW ||
                         object1.sprite.cutW) {
                int0 = this.DoWallLightingW(object1, int0, boolean11, boolean9,
                                            boolean6, boolean8, shader);
              } else if (objectType == IsoObjectType.doorFrN ||
                         objectType == IsoObjectType.doorN ||
                         object1.sprite.cutN) {
                int0 = this.DoWallLightingN(object1, int0, boolean9, boolean10,
                                            boolean5, boolean7, shader);
              }

              if (object1 instanceof
                  IsoWindow && object1.getTargetAlpha(int1) < 1.0F) {
                bWallCutawayN = bWallCutawayN | object1.sprite.cutN;
                bWallCutawayW = bWallCutawayW | object1.sprite.cutW;
              }
            }
          } else if (DebugOptions.instance.Terrain.RenderTiles.IsoGridSquare
                         .DoorsAndWalls_SimpleLighting.getValue()) {
            if (this.z != (int)IsoCamera.frameState.CamCharacterZ ||
                    objectType == IsoObjectType.doorFrW ||
                    objectType == IsoObjectType.doorFrN || object1 instanceof
                IsoWindow) {
              boolean2 = false;
            }
    ...
    Arrays.sort(
        tempWorldInventoryObjects.getElements(), 0,
        tempWorldInventoryObjects.size(),
        (worldInventoryObject0x, worldInventoryObject1x)->{
          float float0x =
              worldInventoryObject0x.xoff * worldInventoryObject0x.xoff +
              worldInventoryObject0x.yoff * worldInventoryObject0x.yoff;
          float float1x =
              worldInventoryObject1x.xoff * worldInventoryObject1x.xoff +
              worldInventoryObject1x.yoff * worldInventoryObject1x.yoff;
          if (float0x == float1x) {
            return 0;
          } else {
    ...
    for (int int13 = 0; int13 < tempWorldInventoryObjects.size(); int13++) {
      IsoWorldInventoryObject worldInventoryObject1 =
          tempWorldInventoryObjects.get(int13);
      worldInventoryObject1.render(this.x, this.y, this.z, colorInfo, true,
                                   false, nullptr);
    }
...
void RereouteWallMaskTo(IsoObject object1) {
  for (int int0 = 0; int0 < this.Objects.size(); int0++) {
    ...
    if (object0.sprite.getProperties().Is(IsoFlagType.collideW) ||
        object0.sprite.getProperties().Is(IsoFlagType.collideN)) {
...
void setBlockedGridPointers(IsoGridSquare.GetSquare getSquare) {
  ...
  if (this.s != nullptr &&
      this.testPathFindAdjacent(nullptr, this.s.x - this.x, this.s.y - this.y,
                                this.s.z - this.z, getSquare)) {
    this.s = nullptr;
  }

  if (this.w != nullptr &&
      this.testPathFindAdjacent(nullptr, this.w.x - this.x, this.w.y - this.y,
                                this.w.z - this.z, getSquare)) {
    this.w = nullptr;
  }

  if (this.n != nullptr &&
      this.testPathFindAdjacent(nullptr, this.n.x - this.x, this.n.y - this.y,
                                this.n.z - this.z, getSquare)) {
    this.n = nullptr;
  }

  if (this.e != nullptr &&
      this.testPathFindAdjacent(nullptr, this.e.x - this.x, this.e.y - this.y,
                                this.e.z - this.z, getSquare)) {
    this.e = nullptr;
  }

  if (this.sw != nullptr &&
      this.testPathFindAdjacent(nullptr, this.sw.x - this.x, this.sw.y - this.y,
                                this.sw.z - this.z, getSquare)) {
    this.sw = nullptr;
  }

  if (this.se != nullptr &&
      this.testPathFindAdjacent(nullptr, this.se.x - this.x, this.se.y - this.y,
                                this.se.z - this.z, getSquare)) {
    this.se = nullptr;
  }

  if (this.nw != nullptr &&
      this.testPathFindAdjacent(nullptr, this.nw.x - this.x, this.nw.y - this.y,
                                this.nw.z - this.z, getSquare)) {
    this.nw = nullptr;
  }

  if (this.ne != nullptr &&
      this.testPathFindAdjacent(nullptr, this.ne.x - this.x, this.ne.y - this.y,
                                this.ne.z - this.z, getSquare)) {
    this.ne = nullptr;
  }
}
...
IsoObject getContainerItem(const std::string &type) {
  ...
  for (int int1 = 0; int1 < int0; int1++) {
    ...
    if (object.getContainer() != nullptr &&
        type == object.getContainer().getType())) {
    return object;
            }
  }
...
void explode() { IsoFireManager.explode(this.getCell(), this, 100000); }

int getHourLastSeen() { return this.hourLastSeen; }
...
void CalcVisibility(int playerIndex) {
  ...
  if (GameServer.bServer ||
      player != nullptr &&
          (!player.isDead() || player.ReanimatedCorpse != nullptr)) {
    if (player != nullptr) {
      ...
      if (square != nullptr) {
        ...
        if (chunkx != nullptr) {
          ...
          LosUtil.TestResults testResults = LosUtil.lineClearCached(
              this.getCell(), this.x, this.y, this.z, (int)lightInfox.x,
              (int)lightInfox.y, (int)lightInfox.z, false, playerIndex);
          float float4 = -0.2F;
          ...
          if (!(float1 > float4) &&
            ...
            if (this.room != nullptr && this.room.def != nullptr &&
                !this.room.def.bExplored) {
              uint8_t byte0 = 10;
              if (lightInfox.square != nullptr &&
                  lightInfox.square.getBuilding() == this.room.building) {
                byte0 = 50;
              }

              if ((!GameServer.bServer || !(player instanceof IsoPlayer) ||
                   !player.isGhostMode()) &&
                  IsoUtils.DistanceManhatten(lightInfox.x, lightInfox.y, this.x,
                                             this.y) < byte0 &&
                  this.z == (int)lightInfox.z) {
                if (GameServer.bServer) {
                  DebugLog.log(DebugType.Zombie,
                               "bExplored room=" + this.room.def.ID);
                }
            ...
            if (!GameServer.bServer) {
              if (iLighting.bSeen()) {
                float float5 = RenderSettings.getInstance().getAmbientForPlayer(
                ...
                if (this.room == nullptr && square.getRoom() == nullptr) {
                  iLighting.targetDarkMulti(float5);
                           ...
                           this.room.building == square.getRoom().building) {
                ...
                } else if (iLighting.lampostTotalR() +
                               iLighting.lampostTotalG() +
                               iLighting.lampostTotalB() ==
                           0.0F) {
                  iLighting.targetDarkMulti(0.0F);
                }
          ...
          if (!GameServer.bServer) {
            for (int int0 = 0; int0 < lightInfox.torches.size(); int0++) {
              ...
              if ((int)torchInfo.x == this.getX() &&
                  (int)torchInfo.y == this.getY() &&
                  (int)torchInfo.z == this.getZ()) {
                float1 = -1.0F;
              }

              bool boolean0 = false;
              if (IsoUtils.DistanceManhatten(this.getX(), this.getY(),
                                             torchInfo.x,
                                             torchInfo.y) < torchInfo.dist &&
                  (torchInfo.bCone && float1 < -torchInfo.dot ||
                   float1 == -1.0F || !torchInfo.bCone && float1 < 0.8F)) {
                boolean0 = true;
              }

              if ((torchInfo.bCone && float0 < torchInfo.dist ||
                   !torchInfo.bCone && float0 < torchInfo.dist) &&
                  iLighting.bCanSee() && boolean0 &&
                ...
                iLighting.targetDarkMulti(iLighting.targetDarkMulti() +
                                          torchInfo.strength * (1.0F - float6) *
                                              3.0F);
...
private
LosUtil.TestResults DoDiagnalCheck(int int0, int int2, int int1,
                                   boolean boolean0) {
  LosUtil.TestResults testResults0 =
      this.testVisionAdjacent(int0, 0, int1, false, boolean0);
  if (testResults0 == LosUtil.TestResults.Blocked) {
    return LosUtil.TestResults.Blocked;
  } else {
    LosUtil.TestResults testResults1 =
        this.testVisionAdjacent(0, int2, int1, false, boolean0);
    if (testResults1 == LosUtil.TestResults.Blocked) {
      return LosUtil.TestResults.Blocked;
    } else {
      return testResults0 != LosUtil.TestResults.ClearThroughWindow &&
                     testResults1 != LosUtil.TestResults.ClearThroughWindow
 ...
 * @return the darkStep
 */
static float getDarkStep() { return darkStep; }

/**
 *
 * @param aDarkStep the darkStep to set
 */
static void setDarkStep(float aDarkStep) { darkStep = aDarkStep; }

/**
 * @return the RecalcLightTime
 */
static int getRecalcLightTime() { return RecalcLightTime; }
 ...
 */
static int getLightcache() { return lightcache; }

/**
 *
 * @param aLightcache the lightcache to set
 */
static void setLightcache(int aLightcache) { lightcache = aLightcache; }
 ...
 * @return the canSee
 */
bool isCanSee(int playerIndex) { return this.lighting[playerIndex].bCanSee(); }
 ...
 * @return the e
 */
IsoGridSquare getE() { return this.e; }

/**
 *
 * @param _e the e to set
 */
void setE(IsoGridSquare _e) { this.e = _e; }

/**
 * @return the LightInfluenceB
 */
public
ArrayList<Float> getLightInfluenceB() { return this.LightInfluenceB; }
 ...
 */
public
ArrayList<Float> getLightInfluenceG() { return this.LightInfluenceG; }
 ...
 */
public
ArrayList<Float> getLightInfluenceR() { return this.LightInfluenceR; }
 ...
 */
public
ArrayList<IsoMovingObject> getStaticMovingObjects() {
 ...
 */
public
ArrayList<IsoMovingObject> getMovingObjects() { return this.MovingObjects; }

/**
 * @return the n
 */
IsoGridSquare getN() { return this.n; }

/**
 *
 * @param _n the n to set
 */
void setN(IsoGridSquare _n) { this.n = _n; }

/**
 * @return the Objects
 */
public
PZArrayList<IsoObject> getObjects() { return this.Objects; }
 ...
 * @return the room
 */
IsoRoom getRoom() { return this.roomID == -1 ? nullptr : this.room; }

/**
 *
 * @param _room the room to set
 */
void setRoom(IsoRoom _room) { this.room = _room; }
 ...
 * @return the s
 */
IsoGridSquare getS() { return this.s; }

/**
 *
 * @param _s the s to set
 */
void setS(IsoGridSquare _s) { this.s = _s; }

/**
 * @return the SpecialObjects
 */
public
ArrayList<IsoObject> getSpecialObjects() { return this.SpecialObjects; }

/**
 * @return the w
 */
IsoGridSquare getW() { return this.w; }

/**
 *
 * @param _w the w to set
 */
void setW(IsoGridSquare _w) { this.w = _w; }

/**
 * @return the lampostTotalR
 */
float getLampostTotalR() { return this.lighting[0].lampostTotalR(); }
 ...
 */
float getLampostTotalG() { return this.lighting[0].lampostTotalG(); }
 ...
 */
float getLampostTotalB() { return this.lighting[0].lampostTotalB(); }
 ...
 * @return the bSeen
 */
bool isSeen(int playerIndex) { return this.lighting[playerIndex].bSeen(); }
 ...
 */
public
ArrayList<IsoGameCharacter> getDeferedCharacters() {
 ...
 */
bool isCacheIsFree() { return this.CacheIsFree; }

/**
 *
 * @param _CacheIsFree the CacheIsFree to set
 */
void setCacheIsFree(bool _CacheIsFree) { this.CacheIsFree = _CacheIsFree; }

/**
 * @return the CachedIsFree
 */
bool isCachedIsFree() { return this.CachedIsFree; }

/**
 *
 * @param _CachedIsFree the CachedIsFree to set
 */
void setCachedIsFree(bool _CachedIsFree) { this.CachedIsFree = _CachedIsFree; }

/**
 * @return the bDoSlowPathfinding
 */
static bool isbDoSlowPathfinding() { return bDoSlowPathfinding; }
 ...
 */
bool isSolidFloorCached() { return this.SolidFloorCached; }
 ...
 */
bool isSolidFloor() { return this.SolidFloor; }

/**
 *
 * @param _SolidFloor the SolidFloor to set
 */
void setSolidFloor(bool _SolidFloor) { this.SolidFloor = _SolidFloor; }

/**
 * @return the defColorInfo
 */
static ColorInfo getDefColorInfo() { return defColorInfo; }

bool isOutside() { return this.Properties.Is(IsoFlagType.exterior); }
...
bool getCanSee(int playerIndex) { return this.lighting[playerIndex].bCanSee(); }

bool getSeen(int playerIndex) { return this.lighting[playerIndex].bSeen(); }

IsoChunk getChunk() { return this.chunk; }
...
IsoObject getDoorOrWindow(bool north) {
  for (int int0 = this.SpecialObjects.size() - 1; int0 >= 0; int0--) {
    ...
    if (object instanceof IsoThumpable &&
                              ...
                              (((IsoThumpable)object).isDoor() ||
                               ((IsoThumpable)object).isWindow())) {
      return object;
    }
...
IsoObject getDoorOrWindowOrWindowFrame(IsoDirections dir, bool ignoreOpen) {
  for (int int0 = this.Objects.size() - 1; int0 >= 0; int0--) {
    ...
    if (IsoWindowFrame.isWindowFrame(object)) {
      ...
      if (IsoWindowFrame.isWindowFrame(object, false) &&
          dir == IsoDirections.W) {
        return object;
      }
    }
...
IsoObject getOpenDoor(IsoDirections dir) {
  for (int int0 = 0; int0 < this.SpecialObjects.size(); int0++) {
    ...
    if (thumpable != nullptr && thumpable.open &&
        thumpable.getSpriteEdge(false) == dir) {
      return thumpable;
    }
  }
...
public
ArrayList<IsoWorldInventoryObject> getWorldObjects() {
  return this.WorldObjects;
}

public
PZArrayList<IsoObject> getLocalTemporaryObjects() {
...
bool hasModData() { return this.table != nullptr && !this.table.isEmpty(); }

ZomboidBitFlag getHasTypes() { return this.hasTypes; }
...
void setRainDrop(IsoRaindrop drop) { this.RainDrop = drop; }

IsoRaindrop getRainDrop() { return this.RainDrop; }

void setRainSplash(IsoRainSplash splash) { this.RainSplash = splash; }

IsoRainSplash getRainSplash() { return this.RainSplash; }
...
bool isOverlayDone() { return this.overlayDone; }

void setOverlayDone(bool _overlayDone) { this.overlayDone = _overlayDone; }

public
ErosionData.Square getErosionData() {
  if (this.erosion == nullptr) {
...
void drawCircleExplosion(int radius, IsoTrap trap,
  ...
  for (int int0 = this.getX() - radius; int0 <= this.getX() + radius; int0++) {
    for (int int1 = this.getY() - radius; int1 <= this.getY() + radius;
         int1++) {
      if (!(IsoUtils.DistanceTo(int0 + 0.5F, int1 + 0.5F, this.getX() + 0.5F,
                                this.getY() + 0.5F) > radius)) {
        LosUtil.TestResults testResults = LosUtil.lineClear(
            this.getCell(), (int)trap.getX(), (int)trap.getY(),
            (int)trap.getZ(), int0, int1, this.z, false);
        if (testResults != LosUtil.TestResults.Blocked &&
            testResults != LosUtil.TestResults.ClearThroughClosedDoor) {
          IsoGridSquare square =
              this.getCell().getGridSquare(int0, int1, this.getZ());
          if (square != nullptr &&
              NonPvpZone.getNonPvpZone(square.getX(), square.getY()) ==
                  nullptr) {
            ...
            if (explosionMode == IsoTrap.ExplosionMode.Explosion) {
              if (!GameClient.bClient && trap.getExplosionPower() > 0 &&
                  Rand.Next(80 - trap.getExplosionPower()) <= 0) {
                square.Burn();
              }

              square.explosion(trap);
              if (!GameClient.bClient && trap.getExplosionPower() > 0 &&
                  Rand.Next(100 - trap.getExplosionPower()) == 0) {
                IsoFireManager.StartFire(this.getCell(), square, true, 20);
              }
            }

            if (explosionMode == IsoTrap.ExplosionMode.Fire &&
                !GameClient.bClient &&
                Rand.Next(100 - trap.getFirePower()) == 0) {
    ...
    for (int int0 = 0; int0 < this.getMovingObjects().size(); int0++) {
      ...
      if (movingObject instanceof IsoGameCharacter) {
        if (GameServer.bServer || !(movingObject instanceof IsoZombie) ||
            ((IsoZombie)movingObject).isLocal()) {
          ...
          movingObject.Hit(
              ...
              Rand.Next(int1 / 30.0F, int1 / 30.0F * 2.0F) +
                  trap.getExtraDamage(),
              false, 1.0F);
            ...
            while (boolean0) {
              ...
              BodyPart bodyPart =
                  ((IsoGameCharacter)movingObject)
                      .getBodyDamage()
                      .getBodyPart(BodyPartType.FromIndex(Rand.Next(15)));
              bodyPart.setBurned();
        ...
        if (GameClient.bClient && movingObject instanceof
            IsoZombie && ((IsoZombie)movingObject).isRemoteZombie()) {
          movingObject.Hit(
              (HandWeapon)InventoryItemFactory.CreateItem("Base.Axe"),
              IsoWorld.instance.CurrentCell.getFakeZombieForHit(), 0.0F, true,
              0.0F);
        }
      }
    }
...
void explodeTrap() {
  IsoGridSquare square0 = this.getCell().getGridSquare(this.getTrapPositionX(),
                                                       this.getTrapPositionY(),
                                                       this.getTrapPositionZ());
  if (square0 != nullptr) {
    for (int int0 = 0; int0 < square0.getObjects().size(); int0++) {
      ...
      if (object instanceof IsoTrap trap) {
        ...
        for (int int2 = square0.getX() - int1; int2 <= square0.getX() + int1;
             int2++) {
          for (int int3 = square0.getY() - int1; int3 <= square0.getY() + int1;
               int3++) {
            if (IsoUtils.DistanceTo(int2 + 0.5F, int3 + 0.5F,
                                    square0.getX() + 0.5F,
                                    square0.getY() + 0.5F) <= int1) {
...
bool haveElectricity() {
  if ((this.chunk == nullptr || !this.chunk.bLoaded) && this.haveElectricity) {
    ...
    return !SandboxOptions.getInstance().AllowExteriorGenerator.getValue() &&
                   this.Is(IsoFlagType.exterior)
               ? false
               : this.chunk != nullptr && this.chunk.isGeneratorPoweringSquare(
                                              this.x, this.y, this.z);
  }
}
...
void FixStackableObjects() {
  ...
  for (int int0 = 0; int0 < this.Objects.size(); int0++) {
    ...
    if (!(object1 instanceof IsoWorldInventoryObject) &&
        object1.sprite != nullptr) {
      PropertyContainer propertyContainer0 = object1.sprite.getProperties();
      if (propertyContainer0.getStackReplaceTileOffset() != 0) {
        object1.sprite = IsoSprite.getSprite(
            IsoSpriteManager.instance,
            object1.sprite.ID + propertyContainer0.getStackReplaceTileOffset());
        if (object1.sprite == nullptr) {
          continue;
        }
      ...
      if (propertyContainer0.isTable() || propertyContainer0.isTableTop()) {
        float float0 = propertyContainer0.isSurfaceOffset()
                           ? propertyContainer0.getSurface()
                           : 0.0F;
        if (object0 != nullptr) {
          object1.setRenderYOffset(object0.getRenderYOffset() +
                                   object0.getSurfaceOffset() - float0);
        } else {
      ...
      if (object1 instanceof IsoLightSwitch &&
        ...
        if (!StringUtils.isNullOrWhitespace(string1) && !string1 == string0)) {
            ...
            if (int5 != 0) {
              IsoSprite sprite =
                  IsoSpriteManager.instance.getSprite(object1.sprite.ID + int5);
              if (sprite != nullptr) {
...
IWorldRegion getIsoWorldRegion() {
  if (GameServer.bServer) {
    ...
    if (!this.hasSetIsoWorldRegion) {
      this.isoWorldRegion =
          IsoRegions.getIsoWorldRegion(this.x, this.y, this.z);
...
bool isInARoom() {
  return this.getRoom() != nullptr ||
         this.getIsoWorldRegion() != nullptr &&
...
int getRoomSize() {
  if (this.getRoom() != nullptr) {
    ...
    return this.getIsoWorldRegion() != nullptr &&
                   this.getIsoWorldRegion().isPlayerRoom()
               ? this.getIsoWorldRegion().getSquareSize()
               : -1;
  }
}
...
int getWallType() {
  ...
  IsoGridSquare square1 = this.nav[IsoDirections.E.index()];
  if (square1 != nullptr && (square1.getProperties().Is(IsoFlagType.WallW) ||
                             square1.getProperties().Is(IsoFlagType.WallNW))) {
    byte0 |= 8;
  }

  IsoGridSquare square2 = this.nav[IsoDirections.S.index()];
  if (square2 != nullptr && (square2.getProperties().Is(IsoFlagType.WallN) ||
                             square2.getProperties().Is(IsoFlagType.WallNW))) {
    byte0 |= 2;
  }
          ...
          if (sprite.name != nullptr) {
            if (sprite.name == "street_trafficlines_01_2")
                                ...
                                || sprite.name == "street_trafficlines_01_32")) {
                                  byte0 = IsoGridSquare.PuddlesDirection
                                              .PUDDLES_DIR_NW;
                                }
...
IsoBuilding getRoofHideBuilding() { return this.roofHideBuilding; }
...
float getApparentZ(float dx, float dy) {
  ...
  if (this.Has(IsoObjectType.stairsTN)) {
    ...
    return this.Has(IsoObjectType.stairsBW)
               ? this.getZ() + PZMath.lerp(0.01F, 0.3333F, 1.0F - dx)
               : this.getZ();
  }
}
  ...
  if (roomx != nullptr && roomx.def != nullptr && !roomx.def.bExplored) {
    ...
    if (player != nullptr) {
      if (this.z == (int)player.z) {
        ...
        if (IsoUtils.DistanceToSquared(player.x, player.y, this.x + 0.5F,
                                       this.y + 0.5F) < byte0 * byte0) {
...
bool hasFlies() { return this.bHasFlies; }

void setHasFlies(bool hasFlies) { this.bHasFlies = hasFlies; }

float getLightLevel(int playerIndex) {
  return (this.lighting[playerIndex].lightInfo().r +
          this.lighting[playerIndex].lightInfo().g +
          this.lighting[playerIndex].lightInfo().b) /
         3.0F;
}

public
static class CellGetSquare implements IsoGridSquare.GetSquare {
...
public
static final class CircleStencilShader extends Shader {
public
  static final IsoGridSquare.CircleStencilShader instance =
      new IsoGridSquare.CircleStencilShader();
  int a_wallShadeColor = -1;

public
  CircleStencilShader() { super("CircleStencil"); }

  void onCompileSuccess(ShaderProgram shaderProgram) {
    this.Start();
    this.a_wallShadeColor =
        GL20.glGetAttribLocation(this.getID(), "a_wallShadeColor");
...
public
static final class Lighting implements IsoGridSquare.ILighting {
private
  final int[] lightverts = new int[8];
  ...
  int lightverts(int int0) { return this.lightverts[int0]; }

  float lampostTotalR() { return this.lampostTotalR; }

  float lampostTotalG() { return this.lampostTotalG; }

  float lampostTotalB() { return this.lampostTotalB; }

  bool bSeen() { return this.bSeen; }

  bool bCanSee() { return this.bCanSee; }

  bool bCouldSee() { return this.bCouldSee; }
  ...
  void lightverts(int int1, int int0) { this.lightverts[int1] = int0; }

  void lampostTotalR(float float0) { this.lampostTotalR = float0; }

  void lampostTotalG(float float0) { this.lampostTotalG = float0; }

  void lampostTotalB(float float0) { this.lampostTotalB = float0; }

  void bSeen(bool boolean0) { this.bSeen = boolean0; }

  void bCanSee(bool boolean0) { this.bCanSee = boolean0; }

  void bCouldSee(bool boolean0) { this.bCouldSee = boolean0; }

  void darkMulti(float float0) { this.darkMulti = float0; }
...
public
static final class NoCircleStencilShader {
public
  static final IsoGridSquare.NoCircleStencilShader instance =
      new IsoGridSquare.NoCircleStencilShader();
  ...
  void initShader() {
    this.shaderProgram =
        ShaderProgram.createShaderProgram("NoCircleStencil", false, true);
    if (this.shaderProgram.isCompiled()) {
      this.ShaderID = this.shaderProgram.getShaderID();
      this.a_wallShadeColor =
          GL20.glGetAttribLocation(this.ShaderID, "a_wallShadeColor");
    }
  }
}
...
public
static final class ResultLight {
  int id;
  int x;
  int y;
  int z;
...
public
  IsoGridSquare.ResultLight copyFrom(IsoGridSquare.ResultLight other) {
    this.id = other.id;
    this.x = other.x;
    this.y = other.y;
    this.z = other.z;
...
private static final class s_performance {
  static const PerformanceProfileProbe renderFloor =
      new PerformanceProfileProbe("IsoGridSquare.renderFloor", false);
}
}
} // namespace iso
} // namespace zombie
