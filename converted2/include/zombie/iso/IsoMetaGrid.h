...
private
  static ArrayList<String> s_PreferredZoneTypes =
      std::make_unique<ArrayList<>>();
  static Clipper s_clipper = null;
  static ClipperOffset s_clipperOffset = null;
  static ByteBuffer s_clipperBuffer = null;
private
  static final ThreadLocal<IsoGameCharacter.Location> TL_Location =
      ThreadLocal.withInitial(IsoGameCharacter.Location::new);
private
  static final ThreadLocal<ArrayList<IsoMetaGrid.Zone>> TL_ZoneList =
...
private
  final ArrayList<RoomDef> tempRooms = std::make_unique<ArrayList<>>();
private
  final ArrayList<IsoMetaGrid.Zone> tempZones1 =
      std::make_unique<ArrayList<>>();
private
  final ArrayList<IsoMetaGrid.Zone> tempZones2 =
      std::make_unique<ArrayList<>>();
private
  final IsoMetaGrid.MetaGridLoaderThread[] threads =
      new IsoMetaGrid.MetaGridLoaderThread[8];
  int minX = 10000000;
  int minY = 10000000;
  int maxX = -10000000;
  int maxY = -10000000;
public
  final ArrayList<IsoMetaGrid.Zone> Zones = std::make_unique<ArrayList<>>();
public
  final ArrayList<BuildingDef> Buildings = std::make_unique<ArrayList<>>();
public
  final ArrayList<IsoMetaGrid.VehicleZone> VehiclesZones =
      std::make_unique<ArrayList<>>();
public
  IsoMetaCell[][] Grid;
public
  final ArrayList<IsoGameCharacter> MetaCharacters =
      std::make_unique<ArrayList<>>();
  final ArrayList<Vector2> HighZombieList = std::make_unique<ArrayList<>>();
  int width;
  int height;
...
public
  ArrayList<IsoMetaGrid.Zone> getZonesAt(int x, int y, int z,
    ...
    IsoMetaChunk metaChunk = this.getChunkDataFromTile(x, y);
    return metaChunk != nullptr ? metaChunk.getZonesAt(x, y, z, result)
                                : result;
  }

public
  ArrayList<IsoMetaGrid.Zone> getZonesIntersecting(int x, int y, int z, int w,
                                                   int h) {
  ...
  getZonesIntersecting(int x, int y, int z, int w, int h,
    ...
    for (int int0 = y / 300; int0 <= (y + h) / 300; int0++) {
      for (int int1 = x / 300; int1 <= (x + w) / 300; int1++) {
        if (int1 >= this.minX && int1 <= this.maxX && int0 >= this.minY &&
            int0 <= this.maxY &&
            this.Grid[int1 - this.minX][int0 - this.minY] != nullptr) {
          this.Grid[int1 - this.minX][int0 - this.minY].getZonesIntersecting(
              x, y, z, w, h, result);
        }
      }
    }
...
public
  IsoMetaGrid.Zone getZoneWithBoundsAndType(int x, int y, int z, int w, int h,
                                            String type) {
    ...
    for (int int0 = 0; int0 < arrayList.size(); int0++) {
      ...
      if (zone.x == x && zone.y == y && zone.z == z && zone.w == w &&
          zone.h == h && StringUtils.equalsIgnoreCase(zone.type, type)) {
        return zone;
      }
    }
...
public
  IsoMetaGrid.VehicleZone getVehicleZoneAt(int x, int y, int z) {
    IsoMetaCell metaCell = this.getMetaGridFromTile(x, y);
    if (metaCell != nullptr && !metaCell.vehicleZones.isEmpty()) {
  ...
  BuildingDef getBuildingAt(int x, int y) {
    for (int int0 = 0; int0 < this.Buildings.size(); int0++) {
      ...
      if (buildingDef.x <= x && buildingDef.y <= y &&
          buildingDef.getW() > x - buildingDef.x &&
          buildingDef.getH() > y - buildingDef.y) {
        return buildingDef;
      }
    }
  ...
  BuildingDef getBuildingAtRelax(int x, int y) {
    for (int int0 = 0; int0 < this.Buildings.size(); int0++) {
      BuildingDef buildingDef = this.Buildings.get(int0);
      if (buildingDef.x <= x + 1 && buildingDef.y <= y + 1 &&
          buildingDef.getW() > x - buildingDef.x - 1 &&
          buildingDef.getH() > y - buildingDef.y - 1) {
        return buildingDef;
      }
    }
  ...
  RoomDef getEmptyOutsideAt(int x, int y, int z) {
    IsoMetaChunk metaChunk = this.getChunkDataFromTile(x, y);
    return metaChunk != nullptr ? metaChunk.getEmptyOutsideAt(x, y, z)
                                : nullptr;
  }
...
public
  IsoMetaGrid.Zone registerZone(String name, String type, int x, int y, int z,
                                int _width, int _height) {
    return this.registerZone(name, type, x, y, z, _width, _height,
                             IsoMetaGrid.ZoneGeometryType.INVALID, nullptr, 0);
  }
...
public
  IsoMetaGrid.Zone registerZone(String name, String type, int x, int y, int z,
                                ...
                                TIntArrayList points, int polylineWidth) {
    name = this.sharedStrings.get(name);
    type = this.sharedStrings.get(type);
    IsoMetaGrid.Zone zone =
        new IsoMetaGrid.Zone(name, type, x, y, z, _width, _height);
    ...
    if (x >= this.minX * 300 - 100 && y >= this.minY * 300 - 100 &&
        x + _width <= (this.maxX + 1) * 300 + 100 &&
        y + _height <= (this.maxY + 1) * 300 + 100 && z >= 0 && z < 8 &&
...
public
  IsoMetaGrid.Zone registerGeometryZone(String name, String type, int z,
                                        String geometry,
                                        KahluaTable pointsTable,
    ...
    IsoMetaGrid.ZoneGeometryType zoneGeometryType = switch (geometry) {
        ...
        Double double0 = zoneGeometryType == IsoMetaGrid.ZoneGeometryType.Polyline && properties != nullptr
            ...
            :
              ...
              if (double0 != nullptr) {
                int[] ints = new int[4];
                this.calculatePolylineOutlineBounds(tIntArrayList,
                                                    double0.intValue(), ints);
                int0 = ints[0];
                int1 = ints[1];
                int2 = ints[2];
                int3 = ints[3];
              ...
              if (!type == "Vehicle") && !type == "ParkingStall")) {
                  IsoMetaGrid.Zone zone0 = this.registerZone(
                      name, type, int0, int1, z, int2 - int0 + 1,
                      int3 - int1 + 1, zoneGeometryType, tIntArrayList,
                      double0 == nullptr ? 0 : double0.intValue());
                  tIntArrayList.clear();
                  return zone0;
                }
              else {
                IsoMetaGrid.Zone zone1 = this.registerVehiclesZone(
                    name, type, int0, int1, z, int2 - int0 + 1, int3 - int1 + 1,
                    properties);
                if (zone1 != nullptr) {
            ...
            void calculatePolylineOutlineBounds(TIntArrayList tIntArrayList,
                                                int int0, int[] ints) {
              ...
              s_clipperOffset.addPath(
                  tIntArrayList.size() / 2, s_clipperBuffer,
                  ClipperOffset.JoinType.jtMiter.ordinal(),
                  ClipperOffset.EndType.etOpenButt.ordinal());
          ...
          public
            IsoMetaGrid.Zone registerZoneNoOverlap(String name, String type,
                                                   int x, int y, int z,
                                                   int _width, int _height) {
              return x >= this.minX * 300 - 100 && y >= this.minY * 300 - 100 &&
                             x + _width <= (this.maxX + 1) * 300 + 100 &&
                             y + _height <= (this.maxY + 1) * 300 + 100 &&
                             z >= 0 && z < 8 && _width <= 600 && _height <= 600
                         ? this.registerZone(name, type, x, y, z, _width,
                                             _height)
                         : nullptr;
            }
            ...
            void addZone(IsoMetaGrid.Zone zone) {
              ...
              for (int int0 = zone.y / 300; int0 <= (zone.y + zone.h) / 300;
                   int0++) {
                for (int int1 = zone.x / 300; int1 <= (zone.x + zone.w) / 300;
                     int1++) {
                  if (int1 >= this.minX && int1 <= this.maxX &&
                      int0 >= this.minY && int0 <= this.maxY &&
                      this.Grid[int1 - this.minX][int0 - this.minY] !=
                          nullptr) {
                    this.Grid[int1 - this.minX][int0 - this.minY].addZone(
                        zone, int1 * 300, int0 * 300);
                  }
                }
              }
            }
              ...
              for (int int0 = zone.y / 300; int0 <= (zone.y + zone.h) / 300;
                ...
                for (int int1 = zone.x / 300; int1 <= (zone.x + zone.w) / 300;
                  ...
                  if (int1 >= this.minX && int1 <= this.maxX &&
                      ...
                      this.Grid[int1 - this.minX][int0 - this.minY] !=
                          nullptr) {
                    this.Grid[int1 - this.minX][int0 - this.minY].removeZone(
                        zone);
                  }
                }
              }
            ...
            void removeZonesForCell(int cellX, int cellY) {
              ...
              if (metaCell != nullptr) {
                ...
                for (int int0 = 0; int0 < 900; int0++) {
                  metaCell.ChunkMap[int0].getZonesIntersecting(
                      cellX * 300, cellY * 300, 0, 300, 300, arrayList0);
                }

                for (int int1 = 0; int1 < arrayList0.size(); int1++) {
                  IsoMetaGrid.Zone zone =
                      (IsoMetaGrid.Zone)arrayList0.get(int1);
                  std::vector arrayList1 = this.tempZones2;
                  if (zone.difference(cellX * 300, cellY * 300, 0, 300, 300,
                                      arrayList1)) {
            ...
            void processZones() {
              ...
              for (int int1 = this.minX; int1 <= this.maxX; int1++) {
                for (int int2 = this.minY; int2 <= this.maxY; int2++) {
                  if (this.Grid[int1 - this.minX][int2 - this.minY] !=
                      nullptr) {
                    for (int int3 = 0; int3 < 30; int3++) {
                      for (int int4 = 0; int4 < 30; int4++) {
                        int0 = Math.max(
                            int0, this.Grid[int1 - this.minX][int2 - this.minY]
                                      .getChunk(int4, int3)
                                      .numZones());
                      }
                    }
          ...
          public
            IsoMetaGrid.Zone registerVehiclesZone(
                String name, String type, int x, int y, int z, int _width,
                int _height, KahluaTable properties) {
              if (!type == "Vehicle") && !type == "ParkingStall")) {
    return null;
        }
              else {
                ...
                IsoMetaGrid.VehicleZone vehicleZone =
                    new IsoMetaGrid.VehicleZone(name, type, x, y, z, _width,
                                                _height, properties);
                this.VehiclesZones.add(vehicleZone);
                int int0 =
                    (int)Math.ceil((vehicleZone.x + vehicleZone.w) / 300.0F);
                int int1 =
                    (int)Math.ceil((vehicleZone.y + vehicleZone.h) / 300.0F);

                for (int int2 = vehicleZone.y / 300; int2 < int1; int2++) {
                  for (int int3 = vehicleZone.x / 300; int3 < int0; int3++) {
                    if (int3 >= this.minX && int3 <= this.maxX &&
                        int2 >= this.minY && int2 <= this.maxY &&
                        this.Grid[int3 - this.minX][int2 - this.minY] !=
                            nullptr) {
                      this.Grid[int3 - this.minX][int2 - this.minY]
                          .vehicleZones.add(vehicleZone);
                    }
                  }
                }
            ...
            void checkVehiclesZones() {
              ...
              while (int0 < this.VehiclesZones.size()) {
                ...
                for (int int1 = 0; int1 < int0; int1++) {
                  ...
                  if (zone0.getX() == zone1.getX() &&
                      zone0.getY() == zone1.getY() && zone0.h == zone1.h &&
                      zone0.w == zone1.w) {
                    boolean0 = false;
                    DebugLog.log(
                        "checkVehiclesZones: ERROR! Zone '" + zone0.name +
                        "':'" + zone0.type + "' (" + zone0.x + ", " + zone0.y +
                        ") duplicate with Zone '" + zone1.name + "':'" +
                        zone1.type + "' (" + zone1.x + ", " + zone1.y + ")");
                    break;
                  }
                }
          ...
          public
            IsoMetaGrid.Zone registerMannequinZone(
                String name, String type, int x, int y, int z, int _width,
                int _height, KahluaTable properties) {
              if (!"Mannequin" == type)) {
    return null;
                ...
                name = this.sharedStrings.get(name);
                type = this.sharedStrings.get(type);
                IsoMannequin.MannequinZone mannequinZone =
                    new IsoMannequin.MannequinZone(name, type, x, y, z, _width,
                                                   _height, properties);
                int int0 = (int)Math.ceil((mannequinZone.x + mannequinZone.w) /
                                          300.0F);
                int int1 = (int)Math.ceil((mannequinZone.y + mannequinZone.h) /
                                          300.0F);
                  ...
                  for (int int3 = mannequinZone.x / 300; int3 < int0; int3++) {
                    if (int3 >= this.minX && int3 <= this.maxX &&
                        ...
                        this.Grid[int3 - this.minX][int2 - this.minY] !=
                            nullptr) {
                      this.Grid[int3 - this.minX][int2 - this.minY]
                          .mannequinZones.add(mannequinZone);
                    }
                  }
                }
            ...
            bool isZoneAbove(IsoMetaGrid.Zone zone1, IsoMetaGrid.Zone zone2,
                             int x, int y, int z) {
            ...
            void savePart(ByteBuffer output, int part, bool fromServer) {
              if (part == 0) {
                  ...
                  for (int int1 = 0; int1 < this.Grid[0].length; int1++) {
                    ...
                    if (metaCell.info != nullptr) {
                      for (BuildingDef buildingDef : metaCell.info.Buildings) {
                        ...
                        output.put(
                            (byte)(buildingDef.isHasBeenVisited() ? 1 : 0));
                ...
                for (int int3 = 0; int3 < SafeHouse.getSafehouseList().size();
                     int3++) {
                ...
                for (int int4 = 0; int4 < NonPvpZone.getAllZones().size();
                     int4++) {
                ...
                for (int int5 = 0; int5 < Faction.getFactions().size();
                     int5++) {
                ...
                for (int int7 = 0; int7 < RBBasic.getUniqueRDSSpawned().size();
                     int7++) {
                  GameWindow.WriteString(
                      output, RBBasic.getUniqueRDSSpawned().get(int7));
                }
              }
            }

            void load() {
              File file = ZomboidFileSystem.instance.getFileInCurrentSave(
                  "map_meta.bin");
            ...
            void load(ByteBuffer input) {
              ...
              if (int0 >= 194) {
                ...
                int5 = input.getInt();
                int6 = input.getInt();
                if (int5 == 40 && int6 == 42 && this.width == 66 &&
                    this.height == 53 &&
                    this.getLotDirectories().contains("Muldraugh, KY")) {
                  int1 = 10;
                  int2 = 3;
                }
              ...
              if (int5 != this.Grid.length || int6 != this.Grid[0].length) {
                DebugLog.log("map_meta.bin world size (" + int5 + "x" + int6 +
                             ") does not match the current map size (" +
                             this.Grid.length + "x" + this.Grid[0].length +
                             ")");
              }
                ...
                for (int int10 = int2; int10 <= int4; int10++) {
                  ...
                  for (int int12 = 0; int12 < int11; int12++) {
                    ...
                    if (metaCell0 != nullptr && metaCell0.info != nullptr) {
                      RoomDef roomDef =
                          int0 < 194 ? metaCell0.info.Rooms.get(int13)
                                     : metaCell0.info.RoomByMetaID.get(long0);
                      if (roomDef != nullptr) {
                        ...
                        DebugLog.General.error(
                            "invalid room ID #" + int13 + " in cell " + int9 +
                            "," + int10 + " while reading map_meta.bin");
                      } else {
                        DebugLog.General.error(
                            "invalid room metaID #" + long0 + " in cell " +
                            int9 + "," + int10 + " while reading map_meta.bin");
                      }
                    }
                  }
                  ...
                  for (int int15 = 0; int15 < int14; int15++) {
                    ...
                    if (metaCell0 != nullptr && metaCell0.info != nullptr) {
                      ...
                      if (int0 >= 194) {
                        buildingDef =
                            metaCell0.info.BuildingByMetaID.get(long1);
                      ...
                      if (buildingDef != nullptr) {
                        ...
                        DebugLog.General.error(
                            "invalid building metaID #" + long1 + " in cell " +
                            int9 + "," + int10 + " while reading map_meta.bin");
                      }
                    }
                  }
            ...
            IsoMetaCell getCellData(int x, int y) {
              return x - this.minX >= 0 && y - this.minY >= 0 &&
            ...
            IsoMetaCell getCellDataAbs(int x, int y) { return this.Grid[x][y]; }

            IsoMetaCell getCurrentCellData() {
              int int0 = IsoWorld.instance.CurrentCell
                             .ChunkMap[IsoPlayer.getPlayerIndex()]
                             .WorldX;
              int int1 = IsoWorld.instance.CurrentCell
                             .ChunkMap[IsoPlayer.getPlayerIndex()]
                             .WorldY;
              float float0 = int0;
              float float1 = int1;
            ...
            IsoMetaChunk getCurrentChunkData() {
              ...
              int1 = (int)float1;
              return this.getCellData(int0, int1)
                  .getChunk(IsoWorld.instance.CurrentCell
                                    .ChunkMap[IsoPlayer.getPlayerIndex()]
                                    .WorldX -
                                int0 * 30,
            ...
            IsoMetaChunk getChunkData(int cx, int cy) {
              ...
              return metaCell = = null ? null
                                       : metaCell.getChunk(cx - int0 * 30,
                                                           cy - int1 * 30);
            }
            ...
            IsoMetaChunk getChunkDataFromTile(int x, int y) {
              ...
              return metaCell = = null ? null
                                       : metaCell.getChunk(int0 - int2 * 30,
                                                           int1 - int3 * 30);
            }
            ...
            bool isValidChunk(int wx, int wy) {
              ...
              if (wx < this.minX * 300) {
                ...
                return wy >= (this.maxY + 1) * 300
                           ? false
                           : this.Grid[wx / 300 - this.minX]
                                      [wy / 300 - this.minY]
                                          .info != nullptr;
              }
            }
            ...
            void CreateStep1() {
              ...
              for (auto &string0 : arrayList)
                string0 = ZomboidFileSystem.instance.getString("media/maps/" +
                                                               string0 + "/");
              ...
              for (int int0 = 0; int0 < strings0.length; int0++) {
                if (!IsoLot.InfoFileNames.containsKey(strings0[int0])) {
                  if (strings0[int0].endsWith(".lotheader")) {
                    ...
                    IsoLot.InfoFileNames.put(
                        strings0[int0], file.getAbsolutePath() +
                                            File.separator + strings0[int0]);
  ...
  if (this.maxX >= this.minX && this.maxY >= this.minY) {
    this.Grid =
        new IsoMetaCell[this.maxX - this.minX + 1][this.maxY - this.minY + 1];
    this.width = this.maxX - this.minX + 1;
    this.height = this.maxY - this.minY + 1;
    long long1 = System.currentTimeMillis() - long0;
    DebugLog.log("IsoMetaGrid.Create: finished scanning directories in " +
                 (float)long1 / 1000.0F + " seconds");
    DebugLog.log("IsoMetaGrid.Create: begin loading");
    this.createStartTime = System.currentTimeMillis();

    for (int int3 = 0; int3 < 8; int3++) {
      IsoMetaGrid.MetaGridLoaderThread metaGridLoaderThread =
          new IsoMetaGrid.MetaGridLoaderThread(this.minY + int3);
  ...
  for (int int2 = 0; int2 < this.Buildings.size(); int2++) {
    ...
    if (!Core.GameMode == "LastStand") && buildingDef.rooms.size() > 2) {
        uint8_t byte0 = 11;
        if (SandboxOptions.instance.getElecShutModifier() > -1 &&
            GameTime.instance.NightsSurvived <
  ...
  DebugLog.log("IsoMetaGrid.Create: finished loading in " +
               (float)long0 / 1000.0F + " seconds");
}
...
Vector2 getRandomIndoorCoord() { return null; }
...
RoomDef getRandomRoomBetweenRange(float x, float y, float min, float max) {
  ...
  for (int int0 = 0; int0 < IsoLot.InfoHeaderNames.size(); int0++) {
    ...
    if (!((LotHeader)object1).RoomList.isEmpty()) {
      for (int int1 = 0; int1 < ((LotHeader)object1).RoomList.size(); int1++) {
        ...
        float0 = IsoUtils.DistanceManhatten(x, y, ((RoomDef)object0).x,
                                            ((RoomDef)object0).y);
  ...
  return !roomChoices.isEmpty() ? roomChoices.get(Rand.Next(roomChoices.size()))
                                : nullptr;
}
...
RoomDef getRandomRoomNotInRange(float x, float y, int range) {
  ...
  do {
    ...
    do {
      object = IsoLot.InfoHeaders.get(
          IsoLot.InfoHeaderNames.get(Rand.Next(IsoLot.InfoHeaderNames.size())));
    } while (((LotHeader)object).RoomList.isEmpty());

    roomDef = ((LotHeader)object)
                  .RoomList.get(Rand.Next(((LotHeader)object).RoomList.size()));
  } while (roomDef == nullptr ||
           IsoUtils.DistanceManhatten(x, y, roomDef.x, roomDef.y) < range);
...
void save() {
  try {
    File file0 =
        ZomboidFileSystem.instance.getFileInCurrentSave("map_meta.bin");

    try(FileOutputStream fileOutputStream0 = new FileOutputStream(file0);
        BufferedOutputStream bufferedOutputStream0 =
            new BufferedOutputStream(fileOutputStream0);) {
      synchronized(SliceY.SliceBufferLock) {
        SliceY.SliceBuffer.clear();
        this.save(SliceY.SliceBuffer);
        bufferedOutputStream0.write(SliceY.SliceBuffer.array(), 0,
                                    SliceY.SliceBuffer.position());
      }
    }

    File file1 =
        ZomboidFileSystem.instance.getFileInCurrentSave("map_zone.bin");

    try(FileOutputStream fileOutputStream1 = new FileOutputStream(file1);
        BufferedOutputStream bufferedOutputStream1 =
            new BufferedOutputStream(fileOutputStream1);) {
      synchronized(SliceY.SliceBufferLock) {
        SliceY.SliceBuffer.clear();
        this.saveZone(SliceY.SliceBuffer);
        bufferedOutputStream1.write(SliceY.SliceBuffer.array(), 0,
                                    SliceY.SliceBuffer.position());
      }
    }
  } catch (Exception exception) {
...
void loadZone(ByteBuffer input, int WorldVersion) {
  ...
  if (!GameServer.bServer && WorldVersion >= 34 ||
      GameServer.bServer && WorldVersion >= 36) {
    ...
    IsoMetaGrid.ZoneGeometryType[] zoneGeometryTypes =
        IsoMetaGrid.ZoneGeometryType.values();
    ...
    if (WorldVersion >= 141) {
      ...
      for (int int8 = 0; int8 < int7; int8++) {
        std::string string1 =
            (String)hashMap.get(Integer.valueOf(input.getShort()));
        std::string string2 =
            (String)hashMap.get(Integer.valueOf(input.getShort()));
        ...
        IsoMetaGrid.ZoneGeometryType zoneGeometryType =
            IsoMetaGrid.ZoneGeometryType.INVALID;
        tIntArrayList.clear();
        int int13 = 0;
        if (WorldVersion >= 185) {
        ...
        IsoMetaGrid.Zone zone1 = this.registerZone(
            string1, string2, int9, int10, byte4, int11, int12,
            zoneGeometryType,
            zoneGeometryType == IsoMetaGrid.ZoneGeometryType.INVALID
                ? nullptr
                : tIntArrayList,
            int13);
    ...
    for (int int21 = 0; int21 < int20; int21++) {
      ...
      IsoMetaGrid.Zone zone2 = this.registerZone(string4, string5, int22, int23,
                                                 int24, int25, int26);
...
void saveZone(ByteBuffer output) {
  ...
  if (arrayList0.size() > 32767) {
    throw new IllegalStateException(
        "IsoMetaGrid.saveZone() string table is too large");
  } else {
    ...
    for (int int3 = 0; int3 < this.Zones.size(); int3++) {
      ...
      output.putShort(
          ((Integer)hashMap.get(zone1.getOriginalName())).shortValue());
    ...
    for (String string : IsoWorld.instance.getSpawnedZombieZone().keySet()) {
      std::vector arrayList1 =
          IsoWorld.instance.getSpawnedZombieZone().get(string);
...
public
ArrayList<String> getLotDirectories() {
...
private
final class MetaGridLoaderThread extends Thread {
  const SharedStrings sharedStrings = new SharedStrings();
  final ArrayList<BuildingDef> Buildings = std::make_unique<ArrayList<>>();
  final ArrayList<RoomDef> tempRooms = std::make_unique<ArrayList<>>();
  int wY;

  MetaGridLoaderThread(int int0) { this.wY = int0; }
  ...
  void runInner() {
    for (int int0 = this.wY; int0 <= IsoMetaGrid.this.maxY; int0 += 8) {
      for (int int1 = IsoMetaGrid.this.minX; int1 <= IsoMetaGrid.this.maxX;
           int1++) {
  ...
  void loadCell(int int0, int int1) {
    ...
    IsoMetaGrid.this
        .Grid[int0 - IsoMetaGrid.this.minX][int1 - IsoMetaGrid.this.minY] =
        metaCell;
      ...
      if (lotHeader != nullptr) {
        ...
        if (file.exists()) {
          ...
          try(BufferedRandomAccessFile bufferedRandomAccessFile =
                  new BufferedRandomAccessFile(file.getAbsolutePath(), "r",
                                               4096)) {
            ...
            for (int int5 = 0; int5 < int4; int5++) {
              ...
              RoomDef roomDef0 =
                  new RoomDef(int5, this.sharedStrings.get(string2));
              ...
              if (lotHeader.RoomByMetaID.contains(roomDef0.metaID)) {
                DebugLog.General.error(
                    "duplicate RoomDef.metaID for room at %d,%d,%d", roomDef0.x,
                    roomDef0.y, roomDef0.level);
              }
              ...
              for (int int9 = 0; int9 < int8; int9++) {
                ...
                roomDef0.objects.add(
                    new MetaObject(int10, int11 + int0 * 300 - roomDef0.x,
                                   int12 + int1 * 300 - roomDef0.y, roomDef0));
              }
            ...
            for (int int14 = 0; int14 < int13; int14++) {
              ...
              for (int int16 = 0; int16 < int15; int16++) {
                RoomDef roomDef1 = lotHeader.Rooms.get(
                    IsoLot.readInt(bufferedRandomAccessFile));
...
public
  ...
  KahluaTable getModData() { return this.data; }
}
...
public
  ...
  bool isFaceDirection() { return (this.flags & 1) != 0; }
}
...
public
  final HashMap<String, Integer> spawnedZombies = std::make_unique<HashMap<>>();
  std::string zombiesTypeToSpawn = null;
  bool spawnSpecialZombies = null;
  std::string name;
  std::string type;
  int x;
  int y;
...
public
  ...
  void setW(int int0) { this.w = int0; }

  void setH(int int0) { this.h = int0; }
  ...
  void setPickedXForZoneStory(int int0) { this.pickedXForZoneStory = int0; }

  void setPickedYForZoneStory(int int0) { this.pickedYForZoneStory = int0; }
  ...
  bool haveCons() { return this.haveConstruction; }

  int getZombieDensity() {
    IsoMetaChunk metaChunk =
        IsoWorld.instance.MetaGrid.getChunkDataFromTile(this.x, this.y);
  ...
  bool contains(int int1, int int2, int int0) {
    if (int0 != this.z) {
      ...
      if (this.polylineWidth > 0) {
        ...
        return this.isPointInPolyline_WindingNumber(int1 + 0.5F, int2 + 0.5F,
                                                    0) ==
               IsoMetaGrid.Zone.PolygonHit.Inside;
      } else {
        return false;
      }
    } else {
      return this.isPolygon() ? this.isPointInPolygon_WindingNumber(
                                    int1 + 0.5F, int2 + 0.5F, 0) ==
                                    IsoMetaGrid.Zone.PolygonHit.Inside
                              : true;
    }
  }
      ...
      if (int3 + int4 <= this.y || int3 >= this.y + this.h) {
        ...
        if (this.polylineWidth > 0) {
          ...
          for (byte byte0 = 0; byte0 < this.points.size() - 2; byte0 += 2) {
            ...
            if (LIANG_BARSKY.lineRectIntersect(int5, int6, int7 - int5,
                                               int8 - int6, int1, int3,
                                               int1 + int2, int3 + int4)) {
              return true;
            }
  ...
  bool difference(int int0, int int1, int int2, int int3, int int4,
    ...
    if (!this.intersects(int0, int1, int2, int3, int4)) {
      ...
      if (this.x < int0) {
        ...
        arrayList.add(new IsoMetaGrid.Zone(this.name, this.type, this.x, int5,
                                           int2, int0 - this.x, int6 - int5));
      }
      ...
      if (int0 + int3 < this.x + this.w) {
        ...
        arrayList.add(
            new IsoMetaGrid.Zone(this.name, this.type, int0 + int3, int7, int2,
                                 this.x + this.w - (int0 + int3), int8 - int7));
      }

      if (this.y < int1) {
        arrayList.add(new IsoMetaGrid.Zone(this.name, this.type, this.x, this.y,
                                           int2, this.w, int1 - this.y));
      }

      if (int1 + int4 < this.y + this.h) {
        arrayList.add(new IsoMetaGrid.Zone(this.name, this.type, this.x,
                                           int1 + int4, int2, this.w,
                                           this.y + this.h - (int1 + int4)));
      }
      ...
      if (this.isPolygon()) {
        ...
        for (int int10 = 0; int10 < int9; int10++) {
          ...
          if (short0 < 3) {
            ...
            IsoMetaGrid.Zone zone1 = new IsoMetaGrid.Zone(
                this.name, this.type, this.x, this.y, this.z, this.w, this.h);
  ...
  int pickRandomTriangle() {
    float[] floats =
        this.isPolygon()
            ...
            : (this.isPolyline() ? this.getPolylineOutlineTriangles()
                                 : nullptr);
    if (floats == nullptr) {
      return -1;
    } else {
  ...
  Vector2 pickRandomPointInTriangle(int int0, Vector2 vector) {
    ...
    if (boolean0) {
      ...
      float8 = (1.0F - float6) * (float2 - float0) +
               (1.0F - float7) * (float4 - float0);
      float9 = (1.0F - float6) * (float3 - float1) +
               (1.0F - float7) * (float5 - float1);
    }
...
public
    ...
    if (this.isPolygon() || this.isPolyline() && this.polylineWidth > 0) {
      ...
      if (int0 == -1) {
        ...
        for (int int1 = 0; int1 < 20; int1++) {
          Vector2 vector =
              this.pickRandomPointInTriangle(int0, BaseVehicle.allocVector2());
      ...
      return !this.isPoint() && !this.isPolyline()
                 ? location.set(Rand.Next(this.x, this.x + this.w),
                                Rand.Next(this.y, this.y + this.h), this.z)
                 : nullptr;
    }
  }
  ...
  IsoGridSquare getRandomSquareInZone() {
    IsoGameCharacter.Location location =
        this.pickRandomLocation(IsoMetaGrid.TL_Location.get());
    return location = = null ? null
                             : IsoWorld.instance.CurrentCell.getGridSquare(
                                   location.x, location.y, location.z);
  }

  IsoGridSquare getRandomUnseenSquareInZone() { return null; }

  void addSquare(IsoGridSquare var1) {}

public
  ArrayList<IsoGridSquare> getSquares() { return null; }
  ...
  void setLastActionTimestamp(int int0) { this.lastActionTimestamp = int0; }
  ...
  float getTotalArea() {
    if (!this.isRectangle() && !this.isPoint() &&
        (!this.isPolyline() || this.polylineWidth > 0)) {
      this.getPolygonTriangles();
  ...
  int getClippedSegmentOfPolyline(int int7, int int6, int int5, int int4,
                                  double[] doubles) {
    if (!this.isPolyline()) {
      return -1;
    } else {
      ...
      for (byte byte0 = 0; byte0 < this.points.size() - 2; byte0 += 2) {
        ...
        if (LIANG_BARSKY.lineRectIntersect(int0 + float0, int1 + float0,
                                           int2 - int0, int3 - int1, int7, int6,
                                           int5, int4, doubles)) {
          return byte0 / 2;
        }
      }
    ...
    if (this.polylineOutlinePoints == nullptr) {
      if (this.isPolyline()) {
        if (this.polylineWidth > 0) {
          ...
          byteBuffer.flip();
          clipperOffset.addPath(this.points.size() / 2, byteBuffer,
                                ClipperOffset.JoinType.jtMiter.ordinal(),
                                ClipperOffset.EndType.etOpenButt.ordinal());
  ...
  isPointInPolygon_WindingNumber(float float1, float float0, int var3) {
    ...
    for (byte byte0 = 0; byte0 < this.points.size(); byte0 += 2) {
      ...
      if (int2 <= float0) {
        if (int4 > float0 &&
            this.isLeft(int1, int2, int3, int4, float1, float0) > 0.0F) {
          int0++;
        }
      } else if (int4 <= float0 &&
                 this.isLeft(int1, int2, int3, int4, float1, float0) < 0.0F) {
    ...
    return int0 = = 0 ? IsoMetaGrid.Zone.PolygonHit.Outside
                      : IsoMetaGrid.Zone.PolygonHit.Inside;
  }

  IsoMetaGrid.Zone.PolygonHit
  isPointInPolyline_WindingNumber(float float5, float float4, int var3) {
    int int0 = 0;
    ...
    if (floats == nullptr) {
      ...
      for (byte byte0 = 0; byte0 < floats.length; byte0 += 2) {
        ...
        if (float1 <= float4) {
          if (float3 > float4 && this.isLeft(float0, float1, float2, float3,
                                             float5, float4) > 0.0F) {
            int0++;
          }
        } else if (float3 <= float4 &&
                   this.isLeft(float0, float1, float2, float3, float5, float4) <
                       0.0F) {
          int0--;
        }
      }
  ...
  bool polygonRectIntersect(int int1, int int0, int int3, int int2) {
    return this.x >= int1 && this.x + this.w <= int1 + int3 && this.y >= int0 &&
                   this.y + this.h <= int0 + int2
               ? true
               : this.lineSegmentIntersects(int1, int0, int1 + int3, int0) ||
                     this.lineSegmentIntersects(int1 + int3, int0, int1 + int3,
                                                int0 + int2) ||
                     this.lineSegmentIntersects(int1 + int3, int0 + int2, int1,
                                                int0 + int2) ||
  ...
  bool lineSegmentIntersects(float float3, float float1, float float2,
    ...
    return this.isPointInPolygon_WindingNumber((float3 + float2) / 2.0F,
           ...
           IsoMetaGrid.Zone.PolygonHit.Outside;
  }
  ...
  bool polylineOutlineRectIntersect(int int1, int int0, int int3, int int2) {
    if (this.polylineOutlinePoints == nullptr) {
      ...
      return this.x >= int1 && this.x + this.w <= int1 + int3 &&
                     this.y >= int0 && this.y + this.h <= int0 + int2
                 ? true
                 : this.polylineOutlineSegmentIntersects(int1, int0,
                                                         int1 + int3, int0) ||
                       this.polylineOutlineSegmentIntersects(
                           int1 + int3, int0, int1 + int3, int0 + int2) ||
                       this.polylineOutlineSegmentIntersects(
                           int1 + int3, int0 + int2, int1, int0 + int2) ||
                       this.polylineOutlineSegmentIntersects(int1, int0 + int2,
                                                             int1, int0);
    }
  }

  bool polylineOutlineSegmentIntersects(float float3, float float1,
    ...
    return this.isPointInPolyline_WindingNumber((float3 + float2) / 2.0F,
                                                (float1 + float0) / 2.0F, 0) !=
           IsoMetaGrid.Zone.PolygonHit.Outside;
  }
...
public
  ...
  void initTriangleAreas() {
    ...
    for (byte byte0 = 0; byte0 < this.triangles.length; byte0 += 6) {
      ...
      float float6 =
          this.triangleArea(float0, float1, float2, float3, float4, float5);
...
public
  float[] getPolylineOutlineTriangles() {
    if (this.triangles != nullptr) {
      return this.triangles;
...
public static enum ZoneGeometryType {
  INVALID, Point, Polyline, Polygon;
}
}
} // namespace iso
} // namespace zombie
