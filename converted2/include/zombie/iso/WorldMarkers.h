...
private
  final List<WorldMarkers.GridSquareMarker> gridSquareMarkers =
      std::make_unique<ArrayList<>>();
private
  final WorldMarkers.PlayerHomingPointList[] homingPoints =
      new WorldMarkers.PlayerHomingPointList[4];
private
  final WorldMarkers.DirectionArrowList[] directionArrows =
      new WorldMarkers.DirectionArrowList[4];
  static const ColorInfo stCol = new ColorInfo();
private
  final WorldMarkers.PlayerScreen playerScreen =
      new WorldMarkers.PlayerScreen();
private
  WorldMarkers.Point intersectPoint = new WorldMarkers.Point(0.0F, 0.0F);
private
  WorldMarkers.Point arrowStart = new WorldMarkers.Point(0.0F, 0.0F);
private
  WorldMarkers.Point arrowEnd = new WorldMarkers.Point(0.0F, 0.0F);
private
  WorldMarkers.Line arrowLine =
      new WorldMarkers.Line(this.arrowStart, this.arrowEnd);
...
public
  WorldMarkers.PlayerHomingPoint getHomingPoint(int id) {
    for (int int0 = 0; int0 < this.homingPoints.length; int0++) {
...
public
  WorldMarkers.PlayerHomingPoint addPlayerHomingPoint(IsoPlayer player, int x,
                                                      int y) {
    return this.addPlayerHomingPoint(player, x, y, "arrow_triangle", 1.0F, 1.0F,
                                     1.0F, 1.0F, true, 20);
  }

public
  WorldMarkers.PlayerHomingPoint addPlayerHomingPoint(IsoPlayer player, int x,
                                                      int y, float r, float g,
                                                      float b, float a) {
    return this.addPlayerHomingPoint(player, x, y, "arrow_triangle", r, g, b, a,
                                     true, 20);
  }

public
  WorldMarkers.PlayerHomingPoint
  addPlayerHomingPoint(IsoPlayer player, int x, int y, String texname, float r,
                       float g, float b, float a, boolean homeOnTarget,
                       int homeOnDist) {
    if (GameServer.bServer) {
      return null;
    } else {
      WorldMarkers.PlayerHomingPoint playerHomingPoint =
          new WorldMarkers.PlayerHomingPoint(player.PlayerIndex);
  ...
  bool removePlayerHomingPoint(IsoPlayer player,
  ...
  bool removePlayerHomingPoint(IsoPlayer player, int id) {
    for (int int0 = this.homingPoints[player.PlayerIndex].size() - 1; int0 >= 0;
         int0--) {
...
public
  WorldMarkers.DirectionArrow getDirectionArrow(int id) {
    for (int int0 = 0; int0 < this.directionArrows.length; int0++) {
      for (int int1 = this.directionArrows[int0].size() - 1; int1 >= 0;
           int1--) {
...
public
  WorldMarkers.DirectionArrow addDirectionArrow(IsoPlayer player, int x, int y,
                                                int z, String texname, float r,
                                                float g, float b, float a) {
    if (GameServer.bServer) {
      return null;
    } else {
      WorldMarkers.DirectionArrow directionArrow =
          new WorldMarkers.DirectionArrow(player.PlayerIndex);
  ...
  bool removePlayerDirectionArrow(IsoPlayer player,
  ...
  bool removePlayerDirectionArrow(IsoPlayer player, int id) {
    for (int int0 = this.directionArrows[player.PlayerIndex].size() - 1;
         int0 >= 0; int0--) {
  ...
  void updateDirectionArrows() {
    ...
    for (int int1 = 0; int1 < this.directionArrows.length; int1++) {
      if (int1 == int0 && this.directionArrows[int1].size() != 0) {
        for (int int2 = this.directionArrows[int1].size() - 1; int2 >= 0;
             int2--) {
        ...
        for (int int3 = 0; int3 < this.directionArrows[int1].size(); int3++) {
          WorldMarkers.DirectionArrow directionArrow =
              this.directionArrows[int1].get(int3);
          if (directionArrow.active && IsoPlayer.players[int1] != nullptr) {
            ...
            if (player.getSquare() != nullptr) {
              ...
              int int7 = this.GetDistance(int4, int5, directionArrow.x,
                                          directionArrow.y);
              bool boolean0 = false;
              bool boolean1 = false;
              float float1 = 0.0F;
              ...
              if (int7 < 300) {
                boolean0 = true;
                float1 = playerCamera.XToScreenExact(
                             directionArrow.x, directionArrow.y, int6, 0) /
                         float0;
                float2 = playerCamera.YToScreenExact(
                             directionArrow.x, directionArrow.y, int6, 0) /
                         float0;
              ...
              if (boolean1) {
                ...
                if (int6 == directionArrow.z) {
                  directionArrow.renderTexture =
                      directionArrow.texDown != nullptr
                          ? directionArrow.texDown
                          : directionArrow.texture;
                } else if (directionArrow.z > int6) {
                  directionArrow.renderTexture =
                      directionArrow.texStairsUp != nullptr
                          ? directionArrow.texStairsUp
                          : directionArrow.texture;
                } else {
                  directionArrow.renderTexture =
                      directionArrow.texStairsDown != nullptr
                          ? directionArrow.texStairsUp
                          : directionArrow.texture;
                }
                ...
                float float5 = 0.0F;
                if (!boolean0) {
                  float5 = this.getAngle(directionArrow.x, directionArrow.y,
                                         int4, int5);
                  float5 = this.angleDegrees(180.0F - float5);
                  float5 = this.angleDegrees(float5 + 45.0F);
                } else {
                  float5 = this.getAngle((int)float3, (int)float4, (int)float1,
                                         (int)float2);
                  float5 = this.angleDegrees(180.0F - float5);
                ...
                if (float5 != directionArrow.angle) {
                  if (!directionArrow.lastWasWithinView) {
                    directionArrow.angle =
                        PZMath.lerpAngle(PZMath.degToRad(directionArrow.angle),
                                         PZMath.degToRad(float5),
                                         directionArrow.angleLerpVal *
                                             GameTime.instance.getMultiplier());
                    directionArrow.angle =
                        PZMath.radToDeg(directionArrow.angle);
                  } else {
                ...
                float float6 =
                    float3 + 32000.0F * (float)Math.sin(Math.toRadians(
                                            directionArrow.angle));
                float float7 =
                    float4 + 32000.0F * (float)Math.cos(Math.toRadians(
                                            directionArrow.angle));
                ...
                for (int int8 = 0; int8 < lines.length; int8++) {
                  this.intersectPoint.set(Float.POSITIVE_INFINITY,
                                          Float.POSITIVE_INFINITY);
                  if (intersectLineSegments(this.arrowLine, lines[int8],
                                            this.intersectPoint)) {
    ...
    for (int int1 = 0; int1 < this.homingPoints.length; int1++) {
      if (int1 == int0 && this.homingPoints[int1].size() != 0) {
        ...
        for (int int3 = 0; int3 < this.homingPoints[int1].size(); int3++) {
          WorldMarkers.PlayerHomingPoint playerHomingPoint =
              this.homingPoints[int1].get(int3);
          if (playerHomingPoint.active && IsoPlayer.players[int1] != nullptr) {
            ...
            if (player.getSquare() != nullptr) {
              ...
              int int4 = player.getSquare().getX();
              int int5 = player.getSquare().getY();
              playerHomingPoint.dist = this.GetDistance(
                  int4, int5, playerHomingPoint.x, playerHomingPoint.y);
              playerHomingPoint.targetOnScreen = false;
              if (playerHomingPoint.dist < 200.0F) {
                playerHomingPoint.targetScreenX =
                    playerCamera.XToScreenExact(playerHomingPoint.x,
                                                playerHomingPoint.y, 0.0F, 0) /
                    float0;
                playerHomingPoint.targetScreenY =
                    playerCamera.YToScreenExact(playerHomingPoint.x,
                                                playerHomingPoint.y, 0.0F, 0) /
                    float0;
                playerHomingPoint.targetScreenX =
                    playerHomingPoint.targetScreenX +
                    playerHomingPoint.homeOnOffsetX / float0;
                playerHomingPoint.targetScreenY =
                    playerHomingPoint.targetScreenY +
                    playerHomingPoint.homeOnOffsetY / float0;
                playerHomingPoint.targetOnScreen = this.playerScreen.isOnScreen(
                    playerHomingPoint.targetScreenX,
                    playerHomingPoint.targetScreenY);
              }
              ...
              if (!playerHomingPoint.customTargetAngle) {
                ...
                if (!playerHomingPoint.targetOnScreen) {
                  float5 = this.getAngle(playerHomingPoint.x,
                                         playerHomingPoint.y, int4, int5);
                  float5 = this.angleDegrees(180.0F - float5);
                  float5 = this.angleDegrees(float5 + 45.0F);
                } else {
                  float5 = this.getAngle((int)float2, (int)float4,
              ...
              if (playerHomingPoint.targetAngle != playerHomingPoint.angle) {
                playerHomingPoint.angle = PZMath.lerpAngle(
                    ...
                    playerHomingPoint.angleLerpVal *
                        GameTime.instance.getMultiplier());
                playerHomingPoint.angle =
                    PZMath.radToDeg(playerHomingPoint.angle);
              }

              float float6 = playerHomingPoint.stickToCharDist / float0;
              playerHomingPoint.targRenderX =
                  float2 + float6 * (float)Math.sin(Math.toRadians(
                                        playerHomingPoint.angle));
              playerHomingPoint.targRenderY =
                  float4 + float6 * (float)Math.cos(Math.toRadians(
                                        playerHomingPoint.angle));
              float float7 = playerHomingPoint.movementLerpVal;
              if (playerHomingPoint.targetOnScreen) {
                float float8 =
                    this.GetDistance((int)playerHomingPoint.targRenderX,
                                     (int)playerHomingPoint.targRenderY,
                                     (int)playerHomingPoint.targetScreenX,
                                     (int)playerHomingPoint.targetScreenY);
                float float9 =
                    this.GetDistance((int)float2, (int)float4,
                                     (int)playerHomingPoint.targetScreenX,
                                     (int)playerHomingPoint.targetScreenY);
                if (float9 < float8 ||
                    playerHomingPoint.homeOnTargetInView &&
                        playerHomingPoint.dist <=
                            playerHomingPoint.homeOnTargetDist) {
                  float9 *= 0.75F;
                  playerHomingPoint.targRenderX =
                      float2 + float9 * (float)Math.sin(Math.toRadians(
                                            playerHomingPoint.targetAngle));
                  playerHomingPoint.targRenderY =
                      float4 + float9 * (float)Math.cos(Math.toRadians(
                                            playerHomingPoint.targetAngle));
                }
              }

              playerHomingPoint.targRenderX = this.playerScreen.clampToInnerX(
                  playerHomingPoint.targRenderX);
              playerHomingPoint.targRenderY = this.playerScreen.clampToInnerY(
                  playerHomingPoint.targRenderY);
              if (playerHomingPoint.targRenderX != playerHomingPoint.renderX) {
                playerHomingPoint.renderX = PZMath.lerp(
                    playerHomingPoint.renderX, playerHomingPoint.targRenderX,
                    float7 * GameTime.instance.getMultiplier());
              }

              if (playerHomingPoint.targRenderY != playerHomingPoint.renderY) {
                playerHomingPoint.renderY = PZMath.lerp(
                    playerHomingPoint.renderY, playerHomingPoint.targRenderY,
                    float7 * GameTime.instance.getMultiplier());
              }
            }
          }
    ...
    if (IsoCamera.frameState.playerIndex == 0) {
      if (this.gridSquareMarkers.size() != 0) {
        ...
        for (int int1 = 0; int1 < this.gridSquareMarkers.size(); int1++) {
          WorldMarkers.GridSquareMarker gridSquareMarker =
              this.gridSquareMarkers.get(int1);
          if (gridSquareMarker.alphaInc) {
            gridSquareMarker.alpha = gridSquareMarker.alpha +
                                     GameTime.getInstance().getMultiplier() *
                                         gridSquareMarker.fadeSpeed;
...
public
  WorldMarkers.GridSquareMarker getGridSquareMarker(int id) {
...
public
  WorldMarkers.GridSquareMarker addGridSquareMarker(IsoGridSquare gs, float r,
                                                    float g, float b,
                                                    boolean doAlpha,
                                                    float size) {
    return this.addGridSquareMarker("circle_center", "circle_only_highlight",
                                    gs, r, g, b, doAlpha, size, 0.006F, 0.3F,
                                    1.0F);
  }

public
  WorldMarkers.GridSquareMarker
  addGridSquareMarker(String texid, String overlay, IsoGridSquare gs, float r,
                      float g, float b, boolean doAlpha, float size) {
    return this.addGridSquareMarker(texid, overlay, gs, r, g, b, doAlpha, size,
                                    0.006F, 0.3F, 1.0F);
  }
...
public
  ...
  addGridSquareMarker(String texid, String overlay, IsoGridSquare gs, float r,
                      float g, float b, boolean doAlpha, float size,
                      float fadeSpeed, float fadeMin, float fadeMax) {
    if (GameServer.bServer) {
      return null;
    } else {
      WorldMarkers.GridSquareMarker gridSquareMarker =
          new WorldMarkers.GridSquareMarker();
  ...
  void renderGridSquareMarkers(IsoCell.PerPlayerRender perPlayerRender,
    ...
    if (!GameServer.bServer && this.gridSquareMarkers.size() != 0) {
      ...
      if (player != nullptr) {
        for (int int0 = 0; int0 < this.gridSquareMarkers.size(); int0++) {
          WorldMarkers.GridSquareMarker gridSquareMarker =
              this.gridSquareMarkers.get(int0);
          if (gridSquareMarker.z == zLayer &&
              gridSquareMarker.z == player.getZ() && gridSquareMarker.active) {
            float float0 = 0.0F;
            float float1 = 0.0F;
            stCol.set(gridSquareMarker.r, gridSquareMarker.g,
                      gridSquareMarker.b, gridSquareMarker.a);
            if (gridSquareMarker.doBlink) {
              gridSquareMarker.sprite.alpha = Core.blinkAlpha;
            } else {
              gridSquareMarker.sprite.alpha =
                  gridSquareMarker.doAlpha ? gridSquareMarker.alpha : 1.0F;
            }

            gridSquareMarker.sprite.render(
                nullptr, gridSquareMarker.x, gridSquareMarker.y,
                gridSquareMarker.z, IsoDirections.N, float0, float1, stCol);
            if (gridSquareMarker.spriteOverlay != nullptr) {
              gridSquareMarker.spriteOverlay.alpha = 1.0F;
              gridSquareMarker.spriteOverlay.render(
                  nullptr, gridSquareMarker.x, gridSquareMarker.y,
                  gridSquareMarker.z, IsoDirections.N, float0, float1, stCol);
            }
          }
      ...
      for (int int1 = 0; int1 < this.homingPoints.length; int1++) {
        if (int1 == int0 && this.homingPoints[int1].size() != 0) {
          for (int int2 = 0; int2 < this.homingPoints[int1].size(); int2++) {
            WorldMarkers.PlayerHomingPoint playerHomingPoint =
                this.homingPoints[int1].get(int2);
            if (playerHomingPoint.active &&
                playerHomingPoint.texture != nullptr) {
              ...
              if (ClimateManager.getInstance().getFogIntensity() > 0.0F &&
                  float1 < 1.0F) {
                float float2 = 1.0F - float1;
                float1 += float2 *
                          ClimateManager.getInstance().getFogIntensity() * 2.0F;
                float1 = PZMath.clamp_01(float1);
              }

              this.DrawTextureAngle(
                  playerHomingPoint.texture, playerHomingPoint.renderWidth,
                  playerHomingPoint.renderHeight, playerHomingPoint.renderX,
                  playerHomingPoint.renderY, float0, playerHomingPoint.r,
                  playerHomingPoint.g, playerHomingPoint.b, float1,
      ...
      for (int int1 = 0; int1 < this.directionArrows.length; int1++) {
        if (int1 == int0 && this.directionArrows[int1].size() != 0) {
          for (int int2 = 0; int2 < this.directionArrows[int1].size(); int2++) {
            WorldMarkers.DirectionArrow directionArrow =
                this.directionArrows[int1].get(int2);
            if (directionArrow.active &&
                directionArrow.renderTexture != nullptr &&
                directionArrow.isDrawOnWorld == worldDraw) {
              float float0 = 0.0F;
              ...
              this.DrawTextureAngle(
                  directionArrow.renderTexture, directionArrow.renderWidth,
                  directionArrow.renderHeight, directionArrow.renderScreenX,
                  directionArrow.renderScreenY, float0, directionArrow.r,
                  directionArrow.g, directionArrow.b, directionArrow.a,
...
private
  void DrawTextureAngle(Texture texture, float float1, float float4,
                        double double14, double double15, double double1,
    ...
    SpriteRenderer.instance.render(
        texture, double6, double7, double8, double9, double10, double11,
        double12, double13, float5, float6, float7, float8, float5, float6,
        float7, float8, float5, float6, float7, float8, float5, float6, float7,
        float8, nullptr);
  }

  static bool intersectLineSegments(WorldMarkers.Line l1, WorldMarkers.Line l2,
    ...
    float float6 = l2.e.x;
    float float7 = l2.e.y;
    float float8 = (float7 - float5) * (float2 - float0) -
    ...
    if (float8 == 0.0F) {
      ...
      float float11 =
          ((float6 - float4) * float9 - (float7 - float5) * float10) / float8;
      if (!(float11 < 0.0F) && !(float11 > 1.0F)) {
        float float12 =
            ((float2 - float0) * float9 - (float3 - float1) * float10) / float8;
        if (!(float12 < 0.0F) && !(float12 > 1.0F)) {
          if (intersection != nullptr) {
            intersection.set(float0 + (float2 - float0) * float11,
                             float1 + (float3 - float1) * float11);
          }
...
public
  ...
  private
    WorldMarkers.DirectionArrow.DebugStuff debugStuff;
    int ID;
    bool active = true;
  ...
  public
    ...
    void setTexture(const std::string &texname) {
      ...
      this.texture = Texture.getSharedTexture("media/textures/highlights/" +
                                              texname + ".png");
    }
     ...
     */
    float getRenderWidth() { return this.renderWidth; }

    void setRenderWidth(float _renderWidth) { this.renderWidth = _renderWidth; }

    float getRenderHeight() { return this.renderHeight; }
  ...
  public
    GridSquareMarker() { this.ID = WorldMarkers.NextGridSquareMarkerID++; }
    ...
    void init(const std::string &texid, const std::string &overlay, int _x,
      ...
      Texture texture = Texture.getSharedTexture("media/textures/highlights/" +
                                                 texid + ".png");
      float float0 = texture.getWidth();
      float float1 = 64.0F * Core.TileScale;
      this.scaleRatio = 1.0F / (float0 / float1);
      this.sprite = new IsoSpriteInstance(IsoSpriteManager.instance.getSprite(
          "media/textures/highlights/" + texid + ".png"));
      if (overlay != nullptr) {
        this.spriteOverlay =
            new IsoSpriteInstance(IsoSpriteManager.instance.getSprite(
                "media/textures/highlights/" + overlay + ".png"));
      }
     ...
     * settings.
     */
    bool isDoBlink() { return this.doBlink; }

    void setDoBlink(bool _doBlink) { this.doBlink = _doBlink; }

    bool isScaleCircleTexture() { return this.bScaleCircleTexture; }
  ...
  private static class Line {
    ...
    std::string toString() {
      return String.format("{s: %s, e: %s}", this.s.toString(),
                           this.e.toString());
    }
  }
  ...
  public
    ...
    /**
     * The lerp value for angle adjustment, can be tweaked to be more slowly or
     * faster responding.
     */
    float getAngleLerpVal() { return this.angleLerpVal; }
    ...
    /**
     * The lerp value for jumping to target (homeOneTarget), can be tweaked to
     * be more slowly or faster responding.
     */
    float getMovementLerpVal() { return this.movementLerpVal; }
     ...
     */
    float getStickToCharDist() { return this.stickToCharDist; }
    ...
    /**
     * The base render position for pointers is the center of the screen, adjust
     * this to have it more at feet or head of character for example.
     */
    float getRenderOffsetX() { return this.renderOffsetX; }

    void setRenderOffsetX(float _renderOffsetX) {
      this.renderOffsetX = _renderOffsetX;
    }

    float getRenderOffsetY() { return this.renderOffsetY; }
     ...
     * offsetting Y value
     */
    float getHomeOnOffsetX() { return this.homeOnOffsetX; }

    void setHomeOnOffsetX(float _homeOnOffsetX) {
      this.homeOnOffsetX = _homeOnOffsetX;
    }

    float getHomeOnOffsetY() { return this.homeOnOffsetY; }
  ...
  class PlayerHomingPointList extends
      ArrayList<WorldMarkers.PlayerHomingPoint> {
  }
  ...
  class PlayerScreen {
    ...
    float innerY2;
  private
    WorldMarkers.Line borderTop = new WorldMarkers.Line(
        new WorldMarkers.Point(0.0F, 0.0F), new WorldMarkers.Point(0.0F, 0.0F));
  private
    WorldMarkers.Line borderRight = new WorldMarkers.Line(
        new WorldMarkers.Point(0.0F, 0.0F), new WorldMarkers.Point(0.0F, 0.0F));
  private
    WorldMarkers.Line borderBot = new WorldMarkers.Line(
        new WorldMarkers.Point(0.0F, 0.0F), new WorldMarkers.Point(0.0F, 0.0F));
  private
    WorldMarkers.Line borderLeft = new WorldMarkers.Line(
        new WorldMarkers.Point(0.0F, 0.0F), new WorldMarkers.Point(0.0F, 0.0F));
  private
    WorldMarkers.Line[] borders = new WorldMarkers.Line[4];
  ...
  private
    WorldMarkers.Line getBorderTop() {
  ...
  private
    WorldMarkers.Line getBorderRight() {
  ...
  private
    WorldMarkers.Line getBorderBot() {
  ...
  private
    WorldMarkers.Line getBorderLeft() {
    ...
    bool isOnScreen(float float1, float float0) {
      return float1 >= this.x && float1 < this.x + this.width &&
             float0 >= this.y && float0 < this.y + this.height;
    }

    bool isWithinInner(float float1, float float0) {
      return float1 >= this.innerX && float1 < this.innerX2 &&
             float0 >= this.innerY && float0 < this.innerY2;
    }
  }
  ...
  public
    WorldMarkers.Point set(float arg0, float arg1) {
      this.x = arg0;
      this.y = arg1;
      return this;
    }
    ...
    std::string toString() { return String.format("{%f, %f}", this.x, this.y); }
  }
}
} // namespace iso
} // namespace zombie
