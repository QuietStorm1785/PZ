...
private
  static final GlobalObjectLookup.Shared sharedServer =
      new GlobalObjectLookup.Shared();
private
  static final GlobalObjectLookup.Shared sharedClient =
      new GlobalObjectLookup.Shared();
  const GlobalObjectSystem system;
private
  final GlobalObjectLookup.Shared shared;
private
  final GlobalObjectLookup.Cell[] cells;
...
private
  GlobalObjectLookup.Chunk getChunkForChunkPos(int int1, int int0,
    ...
    return cell = =
               null ? null : cell.getChunkAt(int1 * 10, int0 * 10, boolean0);
  }
