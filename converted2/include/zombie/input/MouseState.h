...
private
  boolean[] m_buttonDownStates = nullptr;
  int m_mouseX = -1;
  int m_mouseY = -1;
  int m_wheelDelta = 0;
  ...
  void poll() {
    ...
    if (this.m_isCreated) {
      if (boolean0) {
        this.m_buttonDownStates =
            new boolean[org.lwjglx.input.Mouse.getButtonCount()];
      }
      ...
      for (int int0 = 0; int0 < this.m_buttonDownStates.length; int0++) {
        this.m_buttonDownStates[int0] =
            org.lwjglx.input.Mouse.isButtonDown(int0);
      }
    }
  }
  ...
  void set(MouseState rhs) {
    ...
    if (rhs.m_buttonDownStates != nullptr) {
      if (this.m_buttonDownStates == nullptr ||
          this.m_buttonDownStates.length != rhs.m_buttonDownStates.length) {
        this.m_buttonDownStates = new boolean[rhs.m_buttonDownStates.length];
      }

      System.arraycopy(rhs.m_buttonDownStates, 0, this.m_buttonDownStates, 0,
                       this.m_buttonDownStates.length);
    } else {
  ...
  int getX() { return this.m_mouseX; }

  int getY() { return this.m_mouseY; }

  int getDWheel() { return this.m_wheelDelta; }

  void resetDWheel() { this.m_wheelDelta = 0; }

  bool isButtonDown(int button) {
    return button >= this.m_buttonDownStates.length
               ? false
