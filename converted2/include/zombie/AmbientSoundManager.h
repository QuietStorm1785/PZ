...
public:
public
  final ArrayList<AmbientSoundManager.Ambient> ambient =
  ...
  void addRandomAmbient() {
    if (!GameServer.Players.isEmpty()) {
      IsoPlayer player =
          GameServer.Players.get(Rand.Next(GameServer.Players.size()));
      if (player != nullptr) {
        ...
        if (string != nullptr) {
          ...
          float0 += this.tempo.x;
          float1 += this.tempo.y;
          AmbientSoundManager.Ambient ambientx =
              new AmbientSoundManager.Ambient(string, float0, float1, 50.0F,
                                              Rand.Next(0.2F, 0.5F));
          this.ambient.add(ambientx);
          GameServer.sendAmbient(string, (int)float0, (int)float1, 50,
                                 Rand.Next(0.2F, 0.5F));
        }
      }
    }
  ...
  void doGunEvent() {
    ...
    if (!arrayList.isEmpty()) {
      ...
      WorldSoundManager.instance.addSound(
          nullptr, (int)float0 + Rand.Next(-10, 10),
          (int)float1 + Rand.Next(-10, 10), 0, 600, 600);
      switch (Rand.Next(6)) {
      case 0:
      ...
      this.ambient.add(ambientx);
      GameServer.sendAmbient(string, (int)float0, (int)float1,
                             (int)Math.ceil(ambientx.radius), ambientx.volume);
    }
  }

  void doAlarm(RoomDef roomDef) {
    if (roomDef != nullptr && roomDef.building != nullptr &&
        roomDef.building.bAlarmed) {
      float float0 = 1.0F;
      AmbientSoundManager.Ambient ambientx = new AmbientSoundManager.Ambient(
          "burglar2", roomDef.x + roomDef.getW() / 2,
          roomDef.y + roomDef.getH() / 2, 700.0F, float0);
      ambientx.duration = 49;
      ambientx.worldSoundDelay = 3;
      roomDef.building.bAlarmed = false;
      roomDef.building.setAllExplored(true);
      this.ambient.add(ambientx);
      GameServer.sendAlarm(roomDef.x + roomDef.getW() / 2,
                           roomDef.y + roomDef.getH() / 2);
    }
  }
  ...
  void updatePowerSupply() {
    ...
    if (this.electricityShutOffState == 0) {
      ...
      } else if (this.electricityShutOffTime != 0L &&
                 System.currentTimeMillis() >= this.electricityShutOffTime) {
        this.electricityShutOffTime = 0L;
