...
protected
  final ArrayList<AttachedModelName> models = std::make_unique<ArrayList<>>();
  ...
  void initFrom(AttachedItems attachedItems) {
    ...
    for (int int0 = 0; int0 < attachedItems.size(); int0++) {
      ...
      if (!StringUtils.isNullOrWhitespace(string0)) {
        std::string string1 = this.group.getLocation(attachedItem.getLocation())
                                  .getAttachmentName();
        HandWeapon weapon =
            Type.tryCastTo(attachedItem.getItem(), HandWeapon.class);
        float float0 = weapon == null ? 0.0F : weapon.getBloodLevel();
        AttachedModelName attachedModelName0 =
            new AttachedModelName(string1, string0, float0);
  ...
  AttachedModelName get(int index) { return this.models.get(index); }

  void clear() { this.models.clear(); }
}
} // namespace AttachedItems
} // namespace characters
} // namespace zombie
