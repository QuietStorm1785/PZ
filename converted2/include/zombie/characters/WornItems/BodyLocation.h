...
public:
  ...
  const std::string id;
protected
  final ArrayList<String> aliases = std::make_unique<ArrayList<>>();
protected
  final ArrayList<String> exclusive = std::make_unique<ArrayList<>>();
protected
  final ArrayList<String> hideModel = std::make_unique<ArrayList<>>();
...
public
  ...
  void checkId(const std::string &string0, const std::string &string1) {
    if (string0 == nullptr) {
      throw new NullPointerException(string1 + " is nullptr");
