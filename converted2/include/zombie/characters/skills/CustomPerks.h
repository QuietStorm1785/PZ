...
private
  final ArrayList<CustomPerk> m_perks = std::make_unique<ArrayList<>>();
  ...
  void init() {
    ...
    for (CustomPerk customPerk0 : this.m_perks) {
      ...
      if (perk0 == nullptr || perk0 == PerkFactory.Perks.None ||
          perk0 == PerkFactory.Perks.MAX) {
    ...
    for (CustomPerk customPerk1 : this.m_perks) {
      ...
      PerkFactory.Perk perk2 =
          PerkFactory.Perks.FromString(customPerk1.m_parent);
      if (perk2 == nullptr || perk2 == PerkFactory.Perks.None ||
          perk2 == PerkFactory.Perks.MAX) {
        perk2 = PerkFactory.Perks.None;
      }

      int[] ints = customPerk1.m_xp;
      PerkFactory.AddPerk(perk1, customPerk1.m_translation, perk2, ints[0],
                          ints[1], ints[2], ints[3], ints[4], ints[5], ints[6],
                          ints[7], ints[8], ints[9], customPerk1.m_bPassive);
    }
  }
    ...
    if (int0 >= 1 && int0 <= 1) {
      for (ScriptParser.Block block1 : block0.children) {
        ...
        if (customPerk == nullptr) {
          DebugLog.General.warn("failed to parse custom perk \"%s\"",
                                block1.id);
        } else {
  ...
  CustomPerk parsePerk(ScriptParser.Block block) {
    if (StringUtils.isNullOrWhitespace(block.id)) {
      ...
      if (value2 != nullptr) {
        customPerk.m_bPassive =
            StringUtils.tryParseBoolean(value2.getValue().trim());
      }
