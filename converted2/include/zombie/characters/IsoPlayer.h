...
public
  static final IsoPlayer[] players = new IsoPlayer[4];
...
private
  static final Stack<String> StaticTraits = std::make_unique<Stack<>>();
...
private
  HashMap<Integer, Integer> spottedPlayerTimer = std::make_unique<HashMap<>>();
  float extUpdateCount = 0.0F;
  static const int s_randomIdleFidgetInterval = 5000;
  bool attackStarted = false;
private
  static final PredicatedFileWatcher m_isoPlayerTriggerWatcher =
      new PredicatedFileWatcher(ZomboidFileSystem.instance.getMessagingDirSub(
                                    "Trigger_ResetIsoPlayerModel.xml"),
                                IsoPlayer::onTrigger_ResetIsoPlayerModel);
...
protected
  Stack<IsoMovingObject> LastSpotted = std::make_unique<Stack<>>();
  bool bChangeCharacterDebounce = false;
  int followID = 0;
protected
  final Stack<IsoGameCharacter> FollowCamStack = std::make_unique<Stack<>>();
  bool bSeenThisFrame = false;
  bool bCouldBeSeenThisFrame = false;
  float AsleepTime = 0.0F;
protected
  final Stack<IsoMovingObject> spottedList = std::make_unique<Stack<>>();
  int TicksSinceSeenZombie = 9999999;
  bool Waiting = true;
  IsoSurvivor DragCharacter = null;
  ...
  int DialogMood = 1;
  int ping = 0;
  IsoMovingObject DragObject = null;
...
private
  final IsoPlayer.InputState inputState = new IsoPlayer.InputState();
...
private
  final HashMap<Long, Long> mechanicsItem = std::make_unique<HashMap<>>();
  int sleepingPillsTaken = 0;
  long lastPillsTaken = 0L;
  long heavyBreathInstance = 0L;
  std::string heavyBreathSoundName = null;
  ...
  long steamID;
private
  final IsoPlayer.VehicleContainerData vehicleContainerData =
      new IsoPlayer.VehicleContainerData();
...
private
  ArrayList<String> alreadyReadBook = std::make_unique<ArrayList<>>();
  uint8_t bleedingLevel = 0;
  const NetworkPlayerAI networkAI;
  ReplayManager replay = null;
private
  static final ArrayList<IsoPlayer> RecentlyRemoved =
      std::make_unique<ArrayList<>>();
  bool pathfindRun = false;
private
  static final IsoPlayer.MoveVars s_moveVars = new IsoPlayer.MoveVars();
  int atkTimer = 0;
private
  static final ArrayList<HitInfo> s_targetsProne =
      std::make_unique<ArrayList<>>();
private
  static final ArrayList<HitInfo> s_targetsStanding =
      std::make_unique<ArrayList<>>();
  ...
  std::string WeaponT = null;
  const ParameterCharacterMovementSpeed parameterCharacterMovementSpeed =
      new ParameterCharacterMovementSpeed(this);
  const ParameterEquippedBaggageContainer parameterEquippedBaggageContainer =
      new ParameterEquippedBaggageContainer(this);
  ...
  const ParameterLocalPlayer parameterLocalPlayer =
      new ParameterLocalPlayer(this);
  const ParameterMeleeHitSurface parameterMeleeHitSurface =
      new ParameterMeleeHitSurface(this);
  const ParameterPlayerHealth parameterPlayerHealth =
      new ParameterPlayerHealth(this);
...
public
  IsoPlayer(IsoCell cell) { this(cell, nullptr, 0, 0, 0); }
...
public
  IsoPlayer(IsoCell cell, SurvivorDesc desc, int x, int y, int z) {
    ...
    this.SpeakColour = new Color(Rand.Next(135) + 120, Rand.Next(135) + 120,
                                 Rand.Next(135) + 120, 255);
    if (GameClient.bClient) {
      if (Core.getInstance().getMpTextColor() != nullptr) {
        this.SpeakColour =
            new Color(Core.getInstance().getMpTextColor().r,
                      Core.getInstance().getMpTextColor().g,
                      Core.getInstance().getMpTextColor().b, 1.0F);
      } else {
        Core.getInstance().setMpTextColor(new ColorInfo(
            this.SpeakColour.r, this.SpeakColour.g, this.SpeakColour.b, 1.0F));
    ...
    if (Core.bDebug &&
        DebugOptions.instance.CheatPlayerStartInvisible.getValue()) {
    ...
    this.m_setClothingTriggerWatcher = new PredicatedFileWatcher(
        ZomboidFileSystem.instance.getMessagingDirSub(
            "Trigger_SetClothing.xml"),
        TriggerXmlFile.class, this ::onTrigger_setClothingToXmlTriggerFile);
    this.networkAI = new NetworkPlayerAI(this);
    this.initFMODParameters();
  }

  void setOnlineID(short value) { this.OnlineID = value; }

  void registerVariableCallbacks() {
    this.setVariable("CombatSpeed", ()->this.combatSpeed,
                     float0->this.combatSpeed = float0);
    this.setVariable("TurnDelta", ()->this.m_turnDelta,
                     float0->this.m_turnDelta = float0);
    this.setVariable("sneaking", this ::isSneaking, this ::setSneaking);
    this.setVariable("initiateAttack", ()->this.initiateAttack,
    ...
    this.setVariable("WalkSpeed", ()->this.m_walkSpeed,
                     float0->this.m_walkSpeed = float0);
    this.setVariable("WalkInjury", ()->this.m_walkInjury,
                     float0->this.m_walkInjury = float0);
    this.setVariable("RunSpeed", ()->this.m_runSpeed,
                     float0->this.m_runSpeed = float0);
    this.setVariable("IdleSpeed", ()->this.m_idleSpeed,
                     float0->this.m_idleSpeed = float0);
    this.setVariable("DeltaX", ()->this.m_deltaX,
                     float0->this.m_deltaX = float0);
    this.setVariable("DeltaY", ()->this.m_deltaY,
                     float0->this.m_deltaY = float0);
    this.setVariable("Windspeed", ()->this.m_windspeed,
                     float0->this.m_windspeed = float0);
    this.setVariable("WindForce", ()->this.m_windForce,
                     float0->this.m_windForce = float0);
    ...
    this.setVariable("IsPerformingAnAction", this ::isPerformingAnAction,
                     this ::setPerformingAnAction);
  }
  ...
  Vector2 getDeferredMovement(Vector2 out_result) {
    ...
    if (DebugOptions.instance.CheatPlayerInvisibleSprint.getValue() &&
        this.isGhostMode() && (this.IsRunning() || this.isSprinting()) &&
  ...
  float getTurnDelta() {
    return !DebugOptions.instance.CheatPlayerInvisibleSprint.getValue() ||
                   !this.isGhostMode() ||
  ...
  void setPerformingAnAction(bool val) { this.m_isPerformingAnAction = val; }

  bool isPerformingAnAction() { return this.m_isPerformingAnAction; }
  ...
  /**
   * The IsoPlayer.instance thread-safe invoke.   Calls the supplied callback if
   * the IsoPlayer.instance is non-null.   Performs this in a thread-safe
   * manner.    It is intended that, should any thread intend to use the
   * IsoPlayer.instance, and does not want another thread   to change the ptr in
   * the meanwhile, it should call invokeOnPlayerInstance(Runnable callback) eg.
   * IsoPlayer.invokeOnPlayerInstance(()->     { IsoPlayer.instance.doStuff(); }
   */
  ...
  static IsoPlayer getInstance() { return instance; }

  static void setInstance(IsoPlayer newInstance) {
    synchronized(instanceLock) { instance = newInstance; }
  }
  ...
  static void onTrigger_ResetIsoPlayerModel(const std::string &string) {
    if (instance != nullptr) {
      ...
      DebugLog.log(DebugType.General,
                   "DebugFileWatcher Hit. Player instance nullptr : " + string);
    }
  }
...
public
  static ArrayList<String> getAllFileNames() {
  ...
  static std::string getUniqueFileName() {
    ...
    return ZomboidFileSystem.instance.getFileNameInCurrentSave("map_p" +
                                                               ++int0 + ".bin");
  }

public
  static ArrayList<IsoPlayer> getAllSavedPlayers() {
  ...
  static bool isServerPlayerIDValid(const std::string &id) {
    if (GameClient.bClient) {
      ...
      return string != nullptr && !string.isEmpty() ? string == id) : true;
    } else {
      return true;
    }
  }
  ...
  static bool allPlayersDead() {
    ...
    return IsoWorld.instance == nullptr ||
           IsoWorld.instance.AddCoopPlayers.isEmpty();
  }
...
public
  ...
  static bool getCoopPVP() { return CoopPVP; }

  static void setCoopPVP(bool enabled) { CoopPVP = enabled; }

  void TestZombieSpotPlayer(IsoMovingObject chr) {
    if (GameServer.bServer && chr instanceof
        IsoZombie && ((IsoZombie)chr).target != this &&
            ((IsoZombie)chr).isLeadAggro(this)) {
  ...
  bool isGhostMode() { return this.isInvisible(); }

  void setGhostMode(bool aGhostMode) { this.setInvisible(aGhostMode); }

  bool isSeeEveryone() {
    return Core.bDebug &&
           DebugOptions.instance.CheatPlayerSeeEveryone.getValue();
  }
  ...
  Vector2 getPlayerMoveDir() { return this.playerMoveDir; }
  ...
  void load(ByteBuffer input, int WorldVersion, bool IS_DEBUG_SAVE) {
    ...
    this.setTagColor(new ColorInfo(input.getFloat(), input.getFloat(),
                                   input.getFloat(), 1.0F));
  ...
  void save(ByteBuffer output, bool IS_DEBUG_SAVE) {
    ...
    if (this.wornItems.size() > 127) {
      ...
      output.putShort(
          (short)this.savedInventoryItems.indexOf(this.getPrimaryHandItem()));
      output.putShort(
          (short)this.savedInventoryItems.indexOf(this.getSecondaryHandItem()));
      ...
      for (int int0 = 0; int0 < this.alreadyReadBook.size(); int0++) {
        output.putShort(
            WorldDictionary.getItemRegistryID(this.alreadyReadBook.get(int0)));
      }
  ...
  void save() {
    synchronized(SliceY.SliceBufferLock) {
      ...
      GameWindow.WriteString(
          byteBuffer, this.bMultiplayer
                          ? ServerOptions.instance.ServerPlayerID.getValue()
                          : "");
      ...
      File file = new File(
          ZomboidFileSystem.instance.getFileNameInCurrentSave("map_p.bin"));
  ...
  void save(const std::string &fileName) {
    ...
    synchronized(SliceY.SliceBufferLock) {
      ...
      GameWindow.WriteString(
          SliceY.SliceBuffer,
          this.bMultiplayer ? ServerOptions.instance.ServerPlayerID.getValue()
                            : "");
  ...
  static void UpdateRemovedEmitters() {
    ...
    for (int int0 = RecentlyRemoved.size() - 1; int0 >= 0; int0--) {
      ...
      if ((cell.getObjectList().contains(player) ||
           cell.getAddList().contains(player)) &&
          !cell.getRemoveList().contains(player)) {
  ...
  static void Reset() { RecentlyRemoved.clear(); }
  ...
  bool isSaveFileInUse() {
    for (int int0 = 0; int0 < numPlayers; int0++) {
      ...
      if (player0 != nullptr) {
        ...
        if (this.SaveFileName != nullptr &&
            this.SaveFileName == player0.SaveFileName)) {
    return true;
                }
      }
    }
  ...
  bool isSaveFileIPValid() { return isServerPlayerIDValid(); }

  std::string getObjectName() { return "Player"; }

  int getJoypadBind() { return this.JoypadBind; }

  bool isLBPressed() {
    return this.JoypadBind == -1
               ? false
               : JoypadManager.instance.isLBPressed(this.JoypadBind);
  }
  ...
  Vector2 getMouseAimVector(Vector2 vec) {
    int int0 = Mouse.getX();
    int int1 = Mouse.getY();
    vec.x = IsoUtils.XToIso(int0, int1 + 55.0F * this.def.getScaleY(),
                            this.getZ()) -
            this.getX();
    vec.y = IsoUtils.YToIso(int0, int1 + 55.0F * this.def.getScaleY(),
                            this.getZ()) -
            this.getY();
    vec.normalize();
    return vec;
  ...
  Vector2 getAimVector(Vector2 vec) {
    return this.JoypadBind == -1 ? this.getMouseAimVector(vec)
                                 : this.getControllerAimDir(vec);
  }

  float getGlobalMovementMod(bool bDoNoises) {
    return !this.isGhostMode() && !this.isNoClip()
  ...
  float getMoveSpeed() {
    ...
    for (int int0 = BodyPartType.ToIndex(BodyPartType.UpperLeg_L);
         int0 <= BodyPartType.ToIndex(BodyPartType.Foot_R); int0++) {
      BodyPart bodyPart =
    ...
    } else if (this.getMoodles().getMoodleLevel(MoodleType.Panic) >= 4 &&
  ...
  bool isPathfindRunning() { return this.pathfindRun; }

  void setPathfindRunning(bool newvalue) { this.pathfindRun = newvalue; }

  bool isBannedAttacking() { return this.bBannedAttacking; }

  void setBannedAttacking(bool b) { this.bBannedAttacking = b; }
        ...
        if (!Core.getInstance().getOptionShowCursorWhileAiming()) {
          if (Core.getInstance().getIsoCursorVisibility() != 0) {
            if (!UIManager.isForceCursorVisible()) {
              ...
              if (int0 >= IsoCamera.getScreenLeft(0) &&
                  int0 <= IsoCamera.getScreenLeft(0) +
                              IsoCamera.getScreenWidth(0)) {
                if (int1 >= IsoCamera.getScreenTop(0) &&
                    int1 <= IsoCamera.getScreenTop(0) +
                                IsoCamera.getScreenHeight(0)) {
  ...
  void render(float x, float y, float z, ColorInfo col, bool bDoChild,
    ...
    if (DebugOptions.instance.Character.Debug.Render.DisplayRoomAndZombiesZone
            .getValue()) {
      std::string string = "";
      ...
      IsoMetaGrid.Zone zone =
          ZombiesZoneDefinition.getDefinitionZoneAt((int)x, (int)y, (int)z);
      if (zone != nullptr) {
  ...
  float doBeatenVehicle(float vehicleSpeed) {
    if (GameClient.bClient && this.isLocalPlayer()) {
      ...
      LuaEventManager.triggerEvent("OnPlayerGetDamage", this, "CARHITDAMAGE",
                                   float0);
      if (this.isAlive()) {
        if (GameClient.bClient) {
          ...
          } else if (!this.isOnFloor() && !(vehicleSpeed > 15.0F) &&
        ...
        } else if (this.getCurrentState() !=
                       PlayerHitReactionState.instance() &&
  ...
  void update() {
    IsoPlayer.s_performance.update.invokeAndMeasure(this,
                                                    IsoPlayer::updateInternal1);
  }

  void updateInternal1() {
    if (GameClient.bClient && !this.isLocal() &&
        System.currentTimeMillis() - this.lastRemoteUpdate > 5000L) {
  ...
  void setBeenMovingSprinting() {
    if (this.isJustMoved()) {
      this.setBeenMovingFor(this.getBeenMovingFor() +
    ...
    if (this.isJustMoved() && this.isSprinting()) {
      this.setBeenSprintingFor(this.getBeenSprintingFor() +
                               1.25F * GameTime.getInstance().getMultiplier());
    } else {
  ...
  bool updateInternal2() {
    ...
    if ((this.isRunning() || this.isSprinting()) &&
        this.getDeferredMovement(tempo).getLengthSquared() > 0.0F) {
      this.runningTime =
    ...
    if (this.getLastCollideTime() > 0.0F) {
      this.setLastCollideTime(this.getLastCollideTime() -
    ...
    if (!GameClient.bClient && !GameServer.bServer && this.bDeathFinished) {
      ...
      if (!GameClient.bClient && this.getCurrentBuildingDef() != nullptr &&
          !this.isInvisible()) {
      ...
      if (this.checkSafehouse > 0 && GameServer.bServer) {
        ...
        if (this.checkSafehouse == 0) {
          ...
          SafeHouse safeHouse =
              SafeHouse.isSafeHouse(this.getCurrentSquare(), null, false);
      ...
      this.TimeSinceLastNetData =
          (int)(this.TimeSinceLastNetData + GameTime.instance.getMultiplier());
      this.TimeSinceOpenDoor =
      ...
      if (this.updateRemotePlayer()) {
        ...
        this.lastSeenZombieTime =
            this.lastSeenZombieTime +
            GameTime.instance.getGameWorldSecondsSinceLastUpdate() / 60.0F /
                60.0F;
        ...
        if (this.updateWhileDead()) {
          ...
          if ((this.getVehicle() == nullptr ||
               ...
               Core.getInstance().getKey("Shout") !=
                   Core.getInstance().getKey("VehicleHorn")) &&
              !this.isAsleep() && this.PlayerIndex == 0 && !this.Speaking &&
          ...
          if (this.getIgnoreMovement() || this.isAsleep()) {
            ...
            if (this.getVehicle() != nullptr &&
                ...
                this.getVehicle().getController() != nullptr) {
            ...
            if (this.checkActionsBlockingMovement()) {
              ...
              if (this.updateUseKey()) {
                ...
                if (!this.isBlockMovement() && !this.isNPC) {
                  if (!this.isCharging && !this.isChargingLT) {
                    ...
                    this.chargeTime = this.chargeTime +
                                      1.0F * GameTime.instance.getMultiplier();
                  }
                ...
                if (this.playerMoveDir.x == 0.0 &&
                    this.playerMoveDir.y == 0.0) {
                ...
                if (this.stateMachine.getCurrent() !=
                  ...
                  if (!this.JustMoved && this.hasPath() &&
                      !this.getPathFindBehavior2().bStopping) {
                    this.JustMoved = true;
                  }
                  ...
                  if (this.isJustMoved() && !this.isNPC && !this.hasPath()) {
                    if (UIManager.getSpeedControls().getCurrentGameSpeed() >
                        1) {
                      UIManager.getSpeedControls().SetCurrentGameSpeed(1);
                    }
                  } else if (this.stats.endurance <
                                 this.stats.endurancedanger &&
                             Rand.Next(
                                 (int)(300.0F *
                                       GameTime.instance.getInvMultiplier())) ==
                                 0) {
                  ...
                  if (this.isNPC && this.GameCharacterAIBrain != nullptr) {
                    boolean1 =
                        this.GameCharacterAIBrain.HumanControlVars.bMelee;
                    this.bBannedAttacking =
                        this.GameCharacterAIBrain.HumanControlVars
                            .bBannedAttacking;
                  }
                  ...
                  if (boolean1) {
                    ...
                    if (!this.bBannedAttacking &&
                        this.isAuthorizeShoveStomp() && this.CanAttack() &&
                  ...
                  if (this.isAiming() && !this.isNPC) {
                    if (this.JoypadBind != -1 && !this.bJoypadMovementActive) {
                      if (this.getForwardDirection().getLengthSquared() >
                          0.0F) {
                  ...
                  if (this.getForwardDirection().x == 0.0F &&
                      this.getForwardDirection().y == 0.0F) {
                    this.setForwardDirection(this.dir.ToVector());
                  }

                  if (this.lastAngle.x != this.getForwardDirection().x ||
                  ...
                  this.stats.endurance =
                      PZMath.clamp(this.stats.endurance, 0.0F, 1.0F);
                  AnimationPlayer animationPlayer = this.getAnimationPlayer();
                  if (animationPlayer != nullptr && animationPlayer.isReady()) {
                    float float4 = animationPlayer.getAngle() +
                                   animationPlayer.getTwistAngle();
                    this.dir = IsoDirections.fromAngle(
                        tempVector2.setLengthAndDirection(float4, 1.0F));
                  ...
                  if (!boolean2 && !this.isForceOverrideAnim()) {
                    if (this.getPath2() == nullptr) {
                      if (this.CurrentSpeed > 0.0F &&
                          (!this.bClimbing || this.lastFallSpeed > 0.0F)) {
                        if (!this.isRunning() && !this.isSprinting()) {
                  ...
                  if (this.slowTimer > 0.0F) {
                    ...
                    this.slowFactor =
                        this.slowFactor -
                        GameTime.instance.getMultiplier() / 100.0F;
                  ...
                  if (this.isNPC && this.GameCharacterAIBrain != nullptr) {
                    ...
                    this.setInitiateAttack(
                        this.GameCharacterAIBrain.HumanControlVars
                            .initiateAttack);
                    this.setRunning(
                        this.GameCharacterAIBrain.HumanControlVars.bRunning);
                    float0 = this.GameCharacterAIBrain.HumanControlVars.strafeX;
                    float1 = this.GameCharacterAIBrain.HumanControlVars.strafeY;
                    this.setJustMoved(
                        this.GameCharacterAIBrain.HumanControlVars.JustMoved);
                    this.updateMovementRates();
                  }

                  this.m_isPlayerMoving =
                      this.isJustMoved() ||
                      this.getPath2() != nullptr &&
                          !this.getPathFindBehavior2().bStopping;
                  bool boolean3 = this.isInTrees();
                  if (boolean3) {
                    float float5 = "parkranger".equals(
                                       this.getDescriptor().getProfession())
                                       ? 1.3F
                                       : 1.0F;
                                        ...
                                        this.setVariable("WalkSpeedTrees",
                                                         float5);
                  }

                  if ((boolean3 || this.m_walkSpeed < 0.4F ||
                       this.m_walkInjury > 0.5F) &&
                      this.isSprinting() && !this.isGhostMode()) {
                  ...
                  this.m_windspeed =
                      ClimateManager.getInstance().getWindSpeedMovement();
                  float float6 = this.getForwardDirection().getDirectionNeg();
                  this.m_windForce =
                      ClimateManager.getInstance().getWindForceMovement(this,
                                                                        float6);
                  return true;
                } else {
                  return true;
  ...
  void updateMovementFromJoypad(IsoPlayer.MoveVars moveVars) {
    ...
    this.playerMoveDir.setLength(0.05F *
                                 (float)Math.pow(vector0.getLength(), 9.0));
    if (float0 != 0.0F || float1 != 0.0F) {
      Vector2 vector1 = tempVector2.set(float0, float1);
      vector1.normalize();
      moveVars.NewFacing = IsoDirections.fromAngle(vector1);
    } else if ((float2 != 0.0F || float3 != 0.0F) &&
               this.playerMoveDir.getLengthSquared() > 0.0F) {
    ...
    if (this.playerMoveDir.x == 0.0F && this.playerMoveDir.y == 0.0F &&
        this.getPath2() != nullptr && pathFindBehavior2.isStrafing() &&
        !pathFindBehavior2.bStopping) {
      this.playerMoveDir.set(pathFindBehavior2.getTargetX() - this.x,
                             pathFindBehavior2.getTargetY() - this.y);
    ...
    if (this.playerMoveDir.x != 0.0F || this.playerMoveDir.y != 0.0F) {
      if (this.isStrafing()) {
        ...
        tempo.normalize();
        float float4 =
            this.legsSprite.modelSlot.model.AnimPlayer.getRenderedAngle();
        if (float4 > Math.PI * 2) {
  ...
  void updateMovementFromKeyboardMouse(IsoPlayer.MoveVars moveVars) {
    ...
    if (!boolean0 && !boolean1 && !boolean2 && !boolean3 ||
      ...
      if (moveVars.moveX == 0.0F && moveVars.moveY == 0.0F &&
          ...
          !pathFindBehavior2.bStopping) {
        Vector2 vector0 = tempo.set(pathFindBehavior2.getTargetX() - this.x,
                                    pathFindBehavior2.getTargetY() - this.y);
  ...
  void updateAimingStance() {
    ...
    if (this.isAiming() && !this.isCurrentState(SwipeStatePlayer.instance())) {
      ...
      SwipeStatePlayer.instance().calcValidTargets(
          this, weapon, true, s_targetsProne, s_targetsStanding);
      HitInfo hitInfo0 =
          s_targetsStanding.isEmpty() ? null : s_targetsStanding.get(0);
      HitInfo hitInfo1 =
          s_targetsProne.isEmpty() ? null : s_targetsProne.get(0);
      if (SwipeStatePlayer.instance().isProneTargetBetter(this, hitInfo0,
                                                          hitInfo1)) {
        hitInfo0 = nullptr;
      }

      bool boolean0 = this.isAttackAnim() ||
                      this.getVariableBoolean("ShoveAnim") ||
                      this.getVariableBoolean("StompAnim");
      if (!boolean0) {
  ...
  void updateStats_Sleeping() {
    ...
    if (this.stats.fatigue > 0.0F) {
      ...
      float float3 = 1.0F / GameTime.instance.getMinutesPerDay() / 60.0F *
      ...
      this.stats.hunger =
          (float)(this.stats.hunger +
                  ZomboidGlobals.HungerIncreaseWhileAsleep *
                      ...
                      float6 * GameTime.instance.getMultiplier() *
    ...
    bool boolean0 =
        float7 >= this.ForceWakeUpTime && float8 < this.ForceWakeUpTime;
    ...
    if (this.isSitOnGround()) {
      ...
      if (this.CurrentSpeed > 0.0F &&
        ...
        if (this.Moodles.getMoodleLevel(MoodleType.HeavyLoad) == 0) {
          this.stats.endurance =
              (float)(this.stats.endurance -
                      double0 * float1 * 0.5 * float2 *
                          GameTime.instance.getMultiplier() * float0);
        } else {
          float float3 = 2.8F;
          ...
          this.stats.endurance =
              (float)(this.stats.endurance -
                      double0 * float1 * 0.5 * float2 *
                          GameTime.instance.getMultiplier() * float3 * float0);
        }
      } else if (this.CurrentSpeed > 0.0F &&
                 this.Moodles.getMoodleLevel(MoodleType.HeavyLoad) > 2) {
        float float4 = 0.7F;
        if (this.Traits.Asthmatic.isSet()) {
          float4 = 1.4F;
        }
        ...
        this.stats.endurance =
            (float)(this.stats.endurance -
                    ZomboidGlobals.RunningEnduranceReduce * float5 * 0.5 *
                        float4 * float0 * GameTime.instance.getMultiplier() *
                        float6 / 2.0);
      }
      ...
      if (!this.isSprinting() && !this.isRunning() &&
        ...
        if (this.getMoodles().getMoodleLevel(MoodleType.Endurance) < 2) {
          if (this.Moodles.getMoodleLevel(MoodleType.HeavyLoad) <= 1) {
            this.stats.endurance =
                (float)(this.stats.endurance +
                        ZomboidGlobals.ImobileEnduranceReduce / 4.0 *
                            SandboxOptions.instance
                                .getEnduranceRegenMultiplier() *
                            this.getRecoveryMod() * float9);
          }
        } else {
          this.stats.endurance =
              (float)(this.stats.endurance -
                      ZomboidGlobals.RunningEnduranceReduce / 7.0 * float0);
        }
      }
    }
  }
  ...
  void updateSneakKey() {
    if (this.PlayerIndex != 0) {
      ...
      if (this.isBlockMovement() ||
          !GameKeyboard.isKeyDown(Core.getInstance().getKey("Crouch"))) {
        this.bSneakDebounce = false;
    ...
    if (Core.bDebug) {
      if (this.PlayerIndex == 0 && GameKeyboard.isKeyDown(22)) {
        if (!this.bChangeCharacterDebounce) {
          ...
          for (int int0 = 0; int0 < this.getCell().getObjectList().size();
               int0++) {
            IsoMovingObject movingObject =
                this.getCell().getObjectList().get(int0);
          ...
          if (!this.FollowCamStack.isEmpty()) {
            ...
            IsoCamera.SetCharacterToFollow(
                this.FollowCamStack.get(this.followID));
            this.followID++;
          }
        }
      } else {
  ...
  void updateEnableModelsKey() {
    if (Core.bDebug) {
      if (this.PlayerIndex == 0 &&
          GameKeyboard.isKeyPressed(
              Core.getInstance().getKey("ToggleModelsEnabled"))) {
        ModelManager.instance.bDebugEnableModels =
            !ModelManager.instance.bDebugEnableModels;
      }
    }
  }
          ...
          this.setDeathDragDown(false);
            ...
            for (int int0 = -1; int0 <= 1; int0++) {
              for (int int1 = -1; int1 <= 1; int1++) {
                ...
                if (square != nullptr) {
                  for (int int2 = 0; int2 < square.getMovingObjects().size();
                       int2++) {
                    IsoMovingObject movingObject =
                        square.getMovingObjects().get(int2);
                    IsoZombie zombie0 =
                        Type.tryCastTo(movingObject, IsoZombie.class);
                    if (zombie0 != nullptr && zombie0.isAlive() &&
                        !zombie0.isOnFloor()) {
            ...
            if (GameClient.bClient) {
              DebugLog.Multiplayer.warn(
                  "UpdateDeathDragDown: no zombies found around player \"%s\"",
                  this.getUsername());
  ...
  void updateGodModeKey() {
    if (Core.bDebug) {
      if (GameKeyboard.isKeyPressed(
              Core.getInstance().getKey("ToggleGodModeInvisible"))) {
        ...
        if (this == player0) {
          ...
          DebugLog.General.println("Toggle GodMode: %s",
                                   boolean0 ? "ON" : "OFF");
  ...
  void checkReloading() {
    ...
    if (weapon != nullptr && weapon.isReloadable(this)) {
      ...
      if (this.PlayerIndex == 0) {
        bool boolean3 =
            GameKeyboard.isKeyDown(Core.getInstance().getKey("ReloadWeapon"));
        if (boolean3) {
        ...
        boolean3 =
            GameKeyboard.isKeyDown(Core.getInstance().getKey("Rack Firearm"));
        if (boolean3) {
  ...
  void postupdate() {
    IsoPlayer.s_performance.postUpdate.invokeAndMeasure(
        this, IsoPlayer::postupdateInternal);
  }

  void postupdateInternal() {
    bool boolean0 = this.hasHitReaction();
    super.postupdate();
    if (boolean0 && this.hasHitReaction() &&
        !this.isCurrentState(PlayerHitReactionState.instance()) &&
    ...
    if (this.isNPC) {
      ...
      float float0 = 1.0F / gameTime.getMinutesPerDay() / 60.0F *
                     gameTime.getMultiplier() / 2.0F;
      if (Core.bLastStand) {
        float0 = 1.0F / gameTime.getMinutesPerDay() / 60.0F *
                 gameTime.getUnmoddedMultiplier() / 2.0F;
      }
    ...
    if (this.isLocalPlayer() && !this.isNPC) {
      if (this.isAiming()) {
        if (Core.getInstance().getOptionAimOutline() != 1) {
          IsoPlayer.s_performance.highlightRangedTargets.invokeAndMeasure(
              this, IsoPlayer::highlightRangedTargetsInternal);
        }
      }
    }
  }
  ...
  void highlightRangedTargetsInternal() {
    HandWeapon weapon0 =
        Type.tryCastTo(this.getPrimaryHandItem(), HandWeapon.class);
    if (weapon0 == nullptr || weapon0.getSwingAnim() == nullptr ||
        weapon0.getCondition() <= 0) {
    ...
    if (Core.getInstance().getOptionAimOutline() != 2 || weapon0.isRanged()) {
      ...
      for (int int0 = 0; int0 < arrayList.size(); int0++) {
        ...
        if (movingObject instanceof IsoZombie || movingObject instanceof
            IsoPlayer) {
          ColorInfo colorInfo = new ColorInfo();
          Core.getInstance().getBadHighlitedColor().interp(
              Core.getInstance().getGoodHighlitedColor(),
              hitInfo.chance / 100.0F, colorInfo);
          movingObject.bOutline[this.PlayerIndex] = true;
          if (movingObject.outlineColor[this.PlayerIndex] == nullptr) {
            movingObject.outlineColor[this.PlayerIndex] =
                std::make_unique<ColorInfo>();
          }
  ...
  bool isPushableForSeparate() {
    if (this.isCurrentState(PlayerHitReactionState.instance())) {
      ...
      return this.isCurrentState(SwipeStatePlayer.instance())
                 ? false
                 : super.isPushableForSeparate();
    }
  }

  bool isPushedByForSeparate(IsoMovingObject other) {
    if (!this.isPlayerMoving() && other.isZombie() &&
        ((IsoZombie)other).isAttacking()) {
      return false;
    } else {
      return !GameClient.bClient || this.isLocalPlayer() && this.isJustMoved()
  ...
  void updateExt() {
    if (!this.isSneaking()) {
      ...
      if (!this.getAdvancedAnimator().containsAnyIdleNodes() &&
          !this.isSitOnGround()) {
  ...
  void onIdlePerformFidgets() {
    ...
    } else if (moodles.getMoodleLevel(MoodleType.Hyperthermia) > 0 &&
               Rand.NextBool(7)) {
      this.setVariable("Ext", "WipeBrow");
    } else if (moodles.getMoodleLevel(MoodleType.Sick) > 0 &&
               Rand.NextBool(7)) {
      if (Rand.NextBool(4)) {
    ...
    } else if (moodles.getMoodleLevel(MoodleType.Endurance) > 2 &&
               Rand.NextBool(10)) {
    ...
    } else if (moodles.getMoodleLevel(MoodleType.Tired) > 2 &&
               Rand.NextBool(10)) {
      if (Rand.NextBool(7)) {
    ...
    } else if (bodyDamage.doBodyPartsHaveInjuries(BodyPartType.Head,
                                                  BodyPartType.Neck) &&
               Rand.NextBool(7)) {
      if (bodyDamage.areBodyPartsBleeding(BodyPartType.Head,
                                          BodyPartType.Neck) &&
          Rand.NextBool(2)) {
    ...
    } else if (bodyDamage.doBodyPartsHaveInjuries(BodyPartType.UpperArm_L,
                                                  BodyPartType.ForeArm_L) &&
               Rand.NextBool(7)) {
      if (bodyDamage.areBodyPartsBleeding(BodyPartType.UpperArm_L,
                                          BodyPartType.ForeArm_L) &&
          Rand.NextBool(2)) {
    ...
    } else if (bodyDamage.doBodyPartsHaveInjuries(BodyPartType.UpperArm_R,
                                                  BodyPartType.ForeArm_R) &&
               Rand.NextBool(7)) {
      if (bodyDamage.areBodyPartsBleeding(BodyPartType.UpperArm_R,
                                          BodyPartType.ForeArm_R) &&
          Rand.NextBool(2)) {
    ...
    } else if (bodyDamage.doesBodyPartHaveInjury(BodyPartType.Hand_L) &&
               Rand.NextBool(7)) {
      this.setVariable("Ext", "PainHandL");
    } else if (bodyDamage.doesBodyPartHaveInjury(BodyPartType.Hand_R) &&
               Rand.NextBool(7)) {
      this.setVariable("Ext", "PainHandR");
    } else if (!this.isSitOnGround() &&
               bodyDamage.doBodyPartsHaveInjuries(BodyPartType.UpperLeg_L,
                                                  BodyPartType.LowerLeg_L) &&
               Rand.NextBool(7)) {
      if (bodyDamage.areBodyPartsBleeding(BodyPartType.UpperLeg_L,
                                          BodyPartType.LowerLeg_L) &&
          Rand.NextBool(2)) {
        ...
        this.setVariable("Ext", "PainLegL");
               ...
               bodyDamage.doBodyPartsHaveInjuries(BodyPartType.UpperLeg_R,
                                                  BodyPartType.LowerLeg_R) &&
               Rand.NextBool(7)) {
      if (bodyDamage.areBodyPartsBleeding(BodyPartType.UpperLeg_R,
                                          BodyPartType.LowerLeg_R) &&
          Rand.NextBool(2)) {
    ...
    } else if (bodyDamage.doBodyPartsHaveInjuries(BodyPartType.Torso_Upper,
                                                  BodyPartType.Torso_Lower) &&
               Rand.NextBool(7)) {
      if (bodyDamage.areBodyPartsBleeding(BodyPartType.Torso_Upper,
                                          BodyPartType.Torso_Lower) &&
          Rand.NextBool(2)) {
  ...
  bool updateUseKey() {
    if (GameServer.bServer) {
      ...
      this.timePressedContext =
          this.timePressedContext +
          GameTime.instance.getRealworldSecondsSinceLastUpdate();
      bool boolean0 =
          GameKeyboard.isKeyDown(Core.getInstance().getKey("Interact"));
      if (boolean0 && this.timePressedContext < 0.5F) {
        this.bPressContext = true;
      } else {
        if (this.bPressContext &&
            (Core.CurrentTextEntryBox != nullptr &&
                 Core.CurrentTextEntryBox.DoingTextEntry ||
             !GameKeyboard.doLuaKeyPressed)) {
  ...
  void updateHitByVehicle() {
    if (!GameServer.bServer) {
      if (this.isLocalPlayer()) {
        if (this.vehicle4testCollision != nullptr &&
            this.ulBeatenVehicle.Check() &&
            SandboxOptions.instance.DamageToPlayerFromHitByACar.getValue() >
                1) {
          ...
          if (vehicle.isEngineRunning() && this.getVehicle() != vehicle) {
            ...
            Vector2 vector1 =
                vehicle.testCollisionWithCharacter(this, 0.20000002F, vector0);
      ...
      if (!GameClient.bClient ||
        ...
        if (this.primaryHandModel != nullptr &&
            !StringUtils.isNullOrEmpty(
                this.primaryHandModel.maskVariableValue) &&
            this.secondaryHandModel != nullptr &&
            !StringUtils.isNullOrEmpty(
                this.secondaryHandModel.maskVariableValue)) {
          this.setDoShove(false);
          ...
          this.setAttackType(nullptr);
                   ...
                   this.getPrimaryHandItem().getItemReplacementPrimaryHand() !=
                       nullptr &&
                   this.getSecondaryHandItem() != nullptr &&
                   this.getSecondaryHandItem().getItemReplacementSecondHand() !=
                       nullptr) {
          ...
          if (!GameClient.bClient || this.isLocalPlayer()) {
            this.setAttackType(
                PZArrayUtil.pickRandom(weaponType.possibleAttack));
          }
          ...
          if (this.attackVars.bDoShove && !this.isAuthorizeShoveStomp()) {
            ...
            this.targetOnGround = (IsoGameCharacter)this.attackVars
                                      .targetOnGround.getMovingObject();
            if (this.attackVars.getWeapon(this) != nullptr &&
                !StringUtils.isNullOrEmpty(
                    this.attackVars.getWeapon(this).getFireMode())) {
              this.setVariable("FireMode",
                               this.attackVars.getWeapon(this).getFireMode());
            } else {
            ...
            if (this.useHandWeapon != nullptr && weaponType.isRanged &&
              ...
              float float0 =
                  int0 *
                  (1.0F - this.getPerkLevel(PerkFactory.Perks.Aiming) / 30.0F);
            ...
            if (calcAttackVars) {
              SwipeStatePlayer.instance().CalcHitList(
                  this, true, this.attackVars, this.hitList);
            }

            IsoGameCharacter character = null;
            if (!this.hitList.isEmpty()) {
              character = Type.tryCastTo(this.hitList.get(0).getObject(),
                                         IsoGameCharacter.class);
            }

            if (character == nullptr) {
              if (this.isAiming() && !this.m_meleePressed &&
              ...
              if (weaponType.canMiss && !this.isAimAtFloor() &&
                  (!GameClient.bClient || this.isLocalPlayer())) {
              ...
              float float1 =
                  IsoUtils.DistanceTo(character.x, character.y, this.x, this.y);
              this.setVariable("TargetDist", float1);
              int int2 = this.calculateCritChance(character);
              if (character instanceof IsoZombie) {
                IsoZombie zombie0 =
                    this.getClosestZombieToOtherZombie((IsoZombie)character);
                if (!this.attackVars.bAimAtFloor && float1 > 1.25 &&
                    weaponType == WeaponType.spear &&
                    (zombie0 == nullptr ||
                     IsoUtils.DistanceTo(character.x, character.y, zombie0.x,
                                         zombie0.y) > 1.7)) {
                  if (!GameClient.bClient || this.isLocalPlayer()) {
              ...
              if (this.isAttackFromBehind()) {
                if (character instanceof
                    IsoZombie && ((IsoZombie)character).target == nullptr) {
                  int2 += 30;
                } else {
                  int2 += 5;
                }
              ...
              if (character instanceof
                  IsoPlayer && weaponType.isRanged && !this.bDoShove) {
                int2 =
                    (int)(this.attackVars.getWeapon(this).getStopPower() *
                          (1.0F + this.getPerkLevel(PerkFactory.Perks.Aiming) /
                                      15.0F));
              }
              ...
              if (!GameClient.bClient || this.isLocalPlayer()) {
                ...
                if (this.isAttackFromBehind() && this.attackVars.bCloseKill &&
                        character instanceof
                    IsoZombie && ((IsoZombie)character).target == nullptr) {
                  this.setCriticalHit(true);
                }

                if (this.isCriticalHit() && !this.attackVars.bCloseKill &&
                    !this.bDoShove && weaponType == WeaponType.knife) {
                  this.setCriticalHit(false);
                }

                this.setAttackWasSuperAttack(false);
                if (this.stats.NumChasingZombies > 1 &&
                    this.attackVars.bCloseKill && !this.bDoShove &&
              ...
              if (Core.bDebug) {
                DebugLog.Combat.debugln(
                    "Hit zombie dist: " + float1 +
                    " crit: " + this.isCriticalHit() + " (" + int2 +
                    "%) from behind: " + this.isAttackFromBehind());
              }
              ...
              if (this.useHandWeapon != nullptr && weaponType.isRanged) {
                this.setRecoilDelay(
                    this.useHandWeapon.getRecoilDelay() -
                    this.getPerkLevel(PerkFactory.Perks.Aiming) * 2);
              }
  ...
  bool getAttackAnim() { return false; }
  ...
  int calculateCritChance(IsoGameCharacter target) {
    if (this.bDoShove) {
      ...
      if (target instanceof IsoPlayer player0) {
        ...
        if (GameClient.bClient && !player0.isLocalPlayer()) {
          ...
          if (player0.getNutrition().getWeight() < 80.0) {
            int0 = (int)(int0 +
                         Math.abs((player0.getNutrition().getWeight() - 80.0) /
                                  2.0));
          } else {
            int0 =
                (int)(int0 - (player0.getNutrition().getWeight() - 80.0) / 2.0);
          }
        }
      }
      ...
      int0 = (int)(int0 -
                   this.getMoodles().getMoodleLevel(MoodleType.Panic) * 1.3);
      return int0 + this.getPerkLevel(PerkFactory.Perks.Strength) * 2;
    } else if (this.bDoShove &&
                   target.getStateMachine().getCurrent() ==
                       StaggerBackState.instance() &&
                   target instanceof
               IsoZombie) {
      return 100;
    } else if (this.getPrimaryHandItem() != nullptr && this.getPrimaryHandItem()
      ...
      if (weapon.isAlwaysKnockdown()) {
        ...
        if (weaponType.isRanged) {
          int1 = (int)(int1 + weapon.getAimingPerkCritModifier() *
                                  (this.getPerkLevel(PerkFactory.Perks.Aiming) /
                                   2.0F));
          if (this.getBeenMovingFor() >
              weapon.getAimingTime() +
                  this.getPerkLevel(PerkFactory.Perks.Aiming) * 2) {
            int1 = (int)(int1 -
                         (this.getBeenMovingFor() -
                          (weapon.getAimingTime() +
                           this.getPerkLevel(PerkFactory.Perks.Aiming) * 2)));
          }
          ...
          if (weapon.isTwoHandWeapon() &&
              (this.getPrimaryHandItem() != weapon ||
          ...
          int1 += int2 * 3;
          if (target instanceof IsoPlayer player1 && GameClient.bClient &&
                                    !player1.isLocalPlayer()) {
            int1 = (int)(int1 - player1.remoteStrLvl * 1.5);
            if (player1.getNutrition().getWeight() < 80.0) {
              int1 =
                  (int)(int1 +
                        Math.abs((player1.getNutrition().getWeight() - 80.0) /
                                 2.0));
            } else {
              int1 = (int)(int1 -
                           (player1.getNutrition().getWeight() - 80.0) / 2.0);
            }
          }
        }
  ...
  bool isAimControlActive() {
    if (this.isForceAim()) {
      ...
      return this.isAimKeyDown()
                 ? true
                 : GameWindow.ActivatedJoyPad != nullptr &&
                       this.JoypadBind != -1 && this.bJoypadMovementActive &&
    ...
    if (!MPDebugAI.updateInputState(this, inputStatex)) {
      if (GameWindow.ActivatedJoyPad != nullptr && this.JoypadBind != -1) {
        if (this.bJoypadMovementActive) {
          ...
          if (this.bJoypadIgnoreAimUntilCentered) {
            float float0 =
                JoypadManager.instance.getAimingAxisX(this.JoypadBind);
            float float1 =
                JoypadManager.instance.getAimingAxisY(this.JoypadBind);
            if (float0 == 0.0F && float1 == 0.0F) {
              this.bJoypadIgnoreAimUntilCentered = false;
            }
          }
        ...
        inputStatex.isAttacking =
            this.isCharging && Mouse.isButtonDownUICheck(0);
        if (GameKeyboard.isKeyDown(Core.getInstance().getKey("Melee")) &&
            this.authorizeMeleeAction) {
          inputStatex.bMelee = true;
          inputStatex.isAttacking = false;
        }
      }
      ...
      if (GameWindow.ActivatedJoyPad != nullptr && this.JoypadBind != -1) {
        if (this.bJoypadMovementActive) {
          inputStatex.isCharging =
              JoypadManager.instance.isRTPressed(this.JoypadBind);
          inputStatex.isChargingLT =
              JoypadManager.instance.isLTPressed(this.JoypadBind);
        ...
        inputStatex.isAiming =
            (this.isAimKeyDown() ||
             Mouse.isButtonDownUICheck(1) && this.TimeRightPressed >= 0.15F) &&
            this.getPlayerNum() == 0 &&
            StringUtils.isNullOrEmpty(this.getVariableString("BumpFallType"));
        if (Mouse.isButtonDown(1)) {
          this.TimeRightPressed =
              this.TimeRightPressed +
        ...
        if (this.isAllowSprint()) {
          if (!Core.OptiondblTapJogToSprint) {
            ...
            if (!GameKeyboard.wasKeyDown(int0) &&
                GameKeyboard.isKeyDown(int0) && this.pressedRunTimer < 30.0F &&
                this.pressedRun) {
              inputStatex.bSprinting = true;
            }

            if (GameKeyboard.wasKeyDown(int0) &&
                !GameKeyboard.isKeyDown(int0)) {
              inputStatex.bSprinting = false;
        ...
        if (this.PlayerIndex == 0 && Core.getInstance().isToggleToSprint()) {
          ...
          if (boolean3 && !boolean4) {
            this.sprintKeyDownMS = long1;
                     ...
                     long1 - this.sprintKeyDownMS < 500L) {
  ...
  IsoZombie getClosestZombieToOtherZombie(IsoZombie closestTo) {
    ...
    for (int int1 = 0; int1 < arrayList0.size(); int1++) {
      ...
      float float1 =
          IsoUtils.DistanceTo(zombie1.x, zombie1.y, closestTo.x, closestTo.y);
  ...
  IsoGameCharacter getClosestZombieDist() {
    ...
    for (int int0 = (int)testHitPosition.x - (int)weapon.getMaxRange();
         int0 <= (int)testHitPosition.x + (int)weapon.getMaxRange(); int0++) {
      for (int int1 = (int)testHitPosition.y - (int)weapon.getMaxRange();
           int1 <= (int)testHitPosition.y + (int)weapon.getMaxRange(); int1++) {
        IsoGridSquare square = IsoWorld.instance.CurrentCell.getGridSquare(
            (double)int0, (double)int1, (double)this.z);
        if (square != nullptr && square.getMovingObjects().size() > 0) {
          for (int int2 = 0; int2 < square.getMovingObjects().size(); int2++) {
            ...
            if (movingObject instanceof IsoZombie) {
              ...
              Vector2 vector1 =
                  tempVector2_2.set(movingObject.getX(), movingObject.getY());
              ...
              if (boolean0 && ((IsoZombie)movingObject).Health > 0.0F) {
                ((IsoZombie)movingObject)
                    .setHitFromBehind(this.isBehind((IsoZombie)movingObject));
                ((IsoZombie)movingObject)
                    .setHitAngle(
                        ((IsoZombie)movingObject).getForwardDirection());
                ((IsoZombie)movingObject)
                    .setPlayerAttackPosition(
                        ((IsoZombie)movingObject).testDotSide(this));
                float float2 = IsoUtils.DistanceTo(
                    movingObject.x, movingObject.y, this.x, this.y);
...
void hitConsequences(HandWeapon weapon, IsoGameCharacter wielder,
  ...
  if ("Shot" == string0)) {
      wielder.setCriticalHit(Rand.Next(100) <
                             ((IsoPlayer)wielder).calculateCritChance(this));
    }

  if (wielder instanceof
      IsoPlayer && (GameServer.bServer || GameClient.bClient)) {
...
HandWeapon getWeapon() {
  if (this.getPrimaryHandItem() instanceof HandWeapon) {
    ...
    return this.getSecondaryHandItem() instanceof
        HandWeapon ? (HandWeapon)this.getSecondaryHandItem()
...
void updateMechanicsItems() {
  if (!GameServer.bServer && !this.mechanicsItem.isEmpty()) {
    ...
    while (iterator.hasNext()) {
      ...
      if (GameTime.getInstance().getCalender().getTimeInMillis() >
          long1 + 86400000L) {
        arrayList.add(long0);
      }
    }
...
void enterExitVehicle() {
  bool boolean0 = this.PlayerIndex == 0 &&
                  GameKeyboard.isKeyDown(Core.getInstance().getKey("Interact"));
  if (boolean0) {
    this.bUseVehicle = true;
    this.useVehicleDuration =
        this.useVehicleDuration +
        GameTime.instance.getRealworldSecondsSinceLastUpdate();
  }

  if (!this.bUsedVehicle && this.bUseVehicle &&
      (!boolean0 || this.useVehicleDuration > 0.5F)) {
    this.bUsedVehicle = true;
    if (this.getVehicle() != nullptr) {
      LuaEventManager.triggerEvent("OnUseVehicle", this, this.getVehicle(),
                                   this.useVehicleDuration > 0.5F);
    } else {
        ...
        if (chunk != nullptr) {
          for (int int6 = 0; int6 < chunk.vehicles.size(); int6++) {
            ...
            if (vehicle.getScript() != nullptr &&
                ...
                (!this.isLocalPlayer() ||
                 vehicle.getTargetAlpha(this.PlayerIndex) != 0.0F) &&
                !(this.DistToSquared((int)vehicle.x, (int)vehicle.y) >=
                  16.0F) &&
                PolygonalMap2.instance.intersectLineWithVehicle(
                    this.x, this.y,
                    this.x + this.getForwardDirection().x * 4.0F,
                    this.y + this.getForwardDirection().y * 4.0F, vehicle,
                    tempVector2) &&
                !PolygonalMap2.instance.lineClearCollide(
                    this.x, this.y, tempVector2.x, tempVector2.y, (int)this.z,
                    vehicle, false, true)) {
              return vehicle;
            }
          }
  ...
  if (GameClient.bClient && this.getVehicle().getSeat(this) == -1) {
    ...
    if (this.getVehicle().isDriver(this)) {
      ...
      if (vehicle.getDriver() == nullptr &&
          vehicle.engineSpeed > vehicle.getScript().getEngineIdleSpeed()) {
        vehicle.engineSpeed = Math.max(
            vehicle.engineSpeed -
                50.0F * (GameTime.getInstance().getMultiplier() / 0.8F),
            (double)vehicle.getScript().getEngineIdleSpeed());
      }
    ...
    if (this.vehicle != nullptr) {
      ...
      if (animationPlayer != nullptr && animationPlayer.isReady()) {
        ...
        float float1 =
            animationPlayer.getAngle() + animationPlayer.getTwistAngle();
        this.dir = IsoDirections.fromAngle(
            tempVector2.setLengthAndDirection(float1, 1.0F));
      }
...
void attackWhileInVehicle() {
  ...
  if (GameWindow.ActivatedJoyPad != nullptr && this.JoypadBind != -1) {
    ...
    this.isCharging =
        this.isAiming() && JoypadManager.instance.isRTPressed(this.JoypadBind);
    this.isChargingLT =
        this.isAiming() && JoypadManager.instance.isLTPressed(this.JoypadBind);
  } else {
    bool boolean2 = this.isAimKeyDown();
    this.setIsAiming(boolean2 || Mouse.isButtonDownUICheck(1) &&
                                     this.TimeRightPressed >= 0.15F);
    if (this.isCharging) {
      this.isCharging = boolean2 || Mouse.isButtonDown(1);
    } else {
      this.isCharging = boolean2 || Mouse.isButtonDownUICheck(1) &&
                                        this.TimeRightPressed >= 0.15F;
    }
...
void setAngleFromAim() {
  ...
  if (GameWindow.ActivatedJoyPad != nullptr && this.JoypadBind != -1) {
    ...
    vector.x =
        vector.x -
        IsoUtils.XToIso(int0, int1 + 55.0F * this.def.getScaleY(), this.getZ());
    vector.y =
        vector.y -
        IsoUtils.YToIso(int0, int1 + 55.0F * this.def.getScaleY(), this.getZ());
    vector.x = -vector.x;
    vector.y = -vector.y;
  }
...
void updateTorchStrength() {
  if (this.getTorchStrength() > 0.0F || this.flickTorch) {
    DrainableComboItem drainableComboItem =
        Type.tryCastTo(this.getActiveLightItem(), DrainableComboItem.class);
    if (drainableComboItem == nullptr) {
      return;
    }

    if (Rand.Next(600 - (int)(0.4 / drainableComboItem.getUsedDelta() *
                              100.0)) == 0) {
...
void calculateContext() {
  ...
  if (this.dir == IsoDirections.N) {
    squares[2] = this.getCell().getGridSquare(
        (double)(float0 - 1.0F), (double)(float1 - 1.0F), (double)float2);
    squares[1] = this.getCell().getGridSquare(
        (double)float0, (double)(float1 - 1.0F), (double)float2);
    squares[3] = this.getCell().getGridSquare(
        (double)(float0 + 1.0F), (double)(float1 - 1.0F), (double)float2);
    ...
    squares[1] = this.getCell().getGridSquare(
        (double)(float0 + 1.0F), (double)(float1 - 1.0F), (double)float2);
    squares[3] = this.getCell().getGridSquare((double)(float0 + 1.0F),
                                              (double)float1, (double)float2);
  } else if (this.dir == IsoDirections.E) {
    squares[2] = this.getCell().getGridSquare(
        (double)(float0 + 1.0F), (double)(float1 - 1.0F), (double)float2);
    squares[1] = this.getCell().getGridSquare((double)(float0 + 1.0F),
                                              (double)float1, (double)float2);
    squares[3] = this.getCell().getGridSquare(
        (double)(float0 + 1.0F), (double)(float1 + 1.0F), (double)float2);
  } else if (this.dir == IsoDirections.SE) {
    squares[2] = this.getCell().getGridSquare((double)(float0 + 1.0F),
                                              (double)float1, (double)float2);
    squares[1] = this.getCell().getGridSquare(
        (double)(float0 + 1.0F), (double)(float1 + 1.0F), (double)float2);
    squares[3] = this.getCell().getGridSquare(
...
bool isSafeToClimbOver(IsoDirections dir) {
  ...
  switch (dir) {
  case N:
    object = this.getCell().getGridSquare(
        (double)this.x, (double)(this.y - 1.0F), (double)this.z);
    break;
  case S:
    object = this.getCell().getGridSquare(
        (double)this.x, (double)(this.y + 1.0F), (double)this.z);
    break;
  case W:
    object = this.getCell().getGridSquare((double)(this.x - 1.0F),
  ...
  if (object == nullptr) {
    ...
    return !((IsoGridSquare)object).TreatAsSolidFloor()
               ? ((IsoGridSquare)object).HasStairsBelow()
               : true;
  }
}
...
bool doContextNSWE(IsoDirections directions) {
  assert directions = = IsoDirections.N || directions == IsoDirections.S ||
  ...
  if (this.current == nullptr) {
    return false;
             ...
             this.canClimbSheetRope(this.current)) {
    this.climbSheetRope();
    return true;
    ...
    bool boolean0 = IsoWindow.isTopOfSheetRopeHere(square0) &&
                    this.canClimbDownSheetRope(square0);
    ...
    if (object0 != nullptr) {
      ...
      if (GameKeyboard.isKeyDown(42) && this.current != nullptr &&
          this.ticksSincePressedMovement > 15.0F) {
        IsoObject object1 = this.current.getDoor(true);
        if (object1 instanceof
            IsoDoor && ((IsoDoor)object1).isFacingSheet(this)) {
        ...
        if (object2 instanceof
            IsoDoor && ((IsoDoor)object2).isFacingSheet(this)) {
        ...
        if (directions == IsoDirections.E) {
          IsoGridSquare square1 = IsoWorld.instance.CurrentCell.getGridSquare(
              (double)(this.x + 1.0F), (double)this.y, (double)this.z);
          IsoObject object3 = square1 != null ? square1.getDoor(true) : null;
          if (object3 instanceof
              IsoDoor && ((IsoDoor)object3).isFacingSheet(this)) {
        ...
        if (directions == IsoDirections.S) {
          ...
          if (object4 instanceof
              IsoDoor && ((IsoDoor)object4).isFacingSheet(this)) {
      ...
      if (this.timePressedContext < 0.5F && !boolean1) {
        ...
        return true;
                 ...
                 this.getCurrentSquare().Is(IsoFlagType.HoppableW)) {
        this.climbOverFence(directions);
        return true;
      } else if (directions == IsoDirections.S &&
                 ...
                 IsoWorld.instance.CurrentCell
                     .getGridSquare((double)this.x, (double)(this.y + 1.0F),
                                    (double)this.z)
                     .Is(IsoFlagType.HoppableN)) {
        this.climbOverFence(directions);
        return true;
      } else if (directions == IsoDirections.E &&
                 IsoWorld.instance.CurrentCell.getGridSquare(
                     ...
                     nullptr &&
                 IsoWorld.instance.CurrentCell
                     .getGridSquare((double)(this.x + 1.0F), (double)this.y,
                                    (double)this.z)
                     .Is(IsoFlagType.HoppableW)) {
        this.climbOverFence(directions);
        return true;
      } else {
...
void doContextDoorOrWindowOrWindowFrame(IsoDirections directions,
                                        IsoObject object) {
  IsoGridSquare square = this.current.nav[directions.index()];
  bool boolean0 = IsoWindow.isTopOfSheetRopeHere(square) &&
                  this.canClimbDownSheetRope(square);
  if (object instanceof IsoDoor door) {
    if (GameKeyboard.isKeyDown(42) && door.HasCurtains() != nullptr &&
        door.isFacingSheet(this) && this.ticksSincePressedMovement > 15.0F) {
      door.toggleCurtain();
    ...
    if (this.timePressedContext >= 0.5F) {
      ...
      thumpable0.ToggleDoor(this);
             ...
             IsoWindow && !object.getSquare().getProperties().Is(
                              IsoFlagType.makeWindowInvincible)) {
    IsoWindow window = (IsoWindow)object;
    if (GameKeyboard.isKeyDown(42)) {
      IsoCurtain curtain0 = window.HasCurtains();
      if (curtain0 != nullptr && this.current != nullptr &&
          !curtain0.getSquare().isBlockedTo(this.current)) {
        curtain0.ToggleDoor(this);
      }
    } else if (this.timePressedContext >= 0.5F) {
      if (window.canClimbThrough(this)) {
        this.climbThroughWindow(window);
      } else if (!window.PermaLocked && !window.isBarricaded() &&
                 !window.IsOpen() && !window.isDestroyed()) {
      ...
      if (!this.isSafeToClimbOver(directions) &&
          !object.getSquare().haveSheetRope && !boolean0) {
        return;
      }
      ...
      if (!window.isBarricaded()) {
        this.climbThroughWindow(window);
             ...
             IsoThumpable && !object.getSquare().getProperties().Is(
                                 IsoFlagType.makeWindowInvincible)) {
    IsoThumpable thumpable1 = (IsoThumpable)object;
    if (GameKeyboard.isKeyDown(42)) {
      IsoCurtain curtain1 = thumpable1.HasCurtains();
      if (curtain1 != nullptr && this.current != nullptr &&
          !curtain1.getSquare().isBlockedTo(this.current)) {
    ...
    if (GameKeyboard.isKeyDown(42)) {
      ...
      if (curtain2 != nullptr && this.current != nullptr &&
          !curtain2.getSquare().isBlockedTo(this.current)) {
        curtain2.ToggleDoor(this);
      }
    } else if ((this.timePressedContext >= 0.5F ||
                this.isSafeToClimbOver(directions) || boolean0) &&
               IsoWindowFrame.canClimbThrough(object, this)) {
  ...
  if (dir == IsoDirections.NW) {
    if (float1 < float0) {
      return this.hopFence(IsoDirections.N, bTest)
                 ? true
                 : this.hopFence(IsoDirections.W, bTest);
    } else {
      return this.hopFence(IsoDirections.W, bTest)
                 ? true
                 : this.hopFence(IsoDirections.N, bTest);
    }
  } else if (dir == IsoDirections.NE) {
    float0 = 1.0F - float0;
    if (float1 < float0) {
      return this.hopFence(IsoDirections.N, bTest)
                 ? true
                 : this.hopFence(IsoDirections.E, bTest);
    } else {
    ...
    if (float1 < float0) {
      ...
      return this.hopFence(IsoDirections.E, bTest)
                 ? true
                 : this.hopFence(IsoDirections.S, bTest);
    }
  } else if (dir == IsoDirections.SW) {
    float1 = 1.0F - float1;
    if (float1 < float0) {
  ...
  if (this.isSprinting()) {
    ...
    if (square0 != nullptr && square0.HasSlopedRoof()) {
      ...
      if (square1.haveRoof) {
        ...
        IsoGridSquare square2 = IsoWorld.instance.CurrentCell.getGridSquare(
            square1.x, square1.y, square1.z + 1);
        if (square2 != nullptr && square2.HasSlopedRoof()) {
          ...
          return IsoWindow.canClimbThroughHelper(this, this.current, square1,
                                                 dir == IsoDirections.N ||
                                                     dir == IsoDirections.S);
        }
      }
...
void updateSleepingPillsTaken() {
  if (this.getSleepingPillsTaken() > 0 && this.lastPillsTaken > 0L &&
      GameTime.instance.Calender.getTimeInMillis() - this.lastPillsTaken >
          7200000L) {
    this.setSleepingPillsTaken(this.getSleepingPillsTaken() - 1);
  }
}

bool AttemptAttack() { return this.DoAttack(this.useChargeTime); }
...
int getPlayerNum() { return this.PlayerIndex; }
...
void updateLOS() {
  ...
  if (this.getCurrentSquare() != nullptr) {
        ...
        if (movingObject == this) {
          ...
          if (square != nullptr) {
            if (this.isSeeEveryone()) {
              ...
              if (player1 != nullptr && movingObject != player1 &&
                  character != nullptr && character.isInvisible() &&
                  player1.accessLevel.isEmpty()) {
                ...
                if (!this.isAsleep() &&
                    (boolean3 || float6 < float7 && boolean2)) {
                  this.TestZombieSpotPlayer(movingObject);
                  if (character != nullptr &&
                      character.IsVisibleToPlayer[int0]) {
                    ...
                    if (zombie0 != nullptr) {
                      ...
                      if (float5 >= float2 - 1.0F && float6 < 7.0F &&
                          !zombie0.Ghost && !zombie0.isFakeDead() &&
                          square.getRoom() ==
                    ...
                    if (float6 < float8 && character instanceof
                        IsoZombie && (int)float5 == (int)float2 &&
                            !this.isGhostMode() && !boolean1) {
                ...
                if (float6 < 2.0F &&
                    movingObject.getTargetAlpha(int0) == 1.0F &&
                    !this.bRemote) {
    ...
    if (this.isAlive() && int2 > 0 && this.stats.LastVeryCloseZombies == 0 &&
        this.stats.NumVisibleZombies > 0 &&
      ...
      this.getEmitter().setVolume(
          long0, Core.getInstance().getOptionJumpScareVolume() / 10.0F);
    }
    ...
    if (this.timeSinceLastStab < 600.0F) {
      this.timeSinceLastStab = this.timeSinceLastStab +
                               GameTime.getInstance().getMultiplier() / 1.6F;
    }
...
float getSeeNearbyCharacterDistance() { return 3.5F - this.stats.getFatigue(); }
...
bool checkSpottedPLayerTimer(IsoPlayer player0) {
  if (!player0.spottedByPlayer) {
    ...
    if (this.spottedPlayerTimer.containsKey(player0.getRemoteID())) {
      this.spottedPlayerTimer.put(
          player0.getRemoteID(),
          this.spottedPlayerTimer.get(player0.getRemoteID()) + 1);
    } else {
...
bool checkCanSeeClient(IsoPlayer remoteChr) {
  ...
  if (!GameClient.bClient || remoteChr == this || !this.isLocalPlayer()) {
    ...
    float float1 =
        this.current == null
            ? 0.0F
            : remoteChr.getCurrentSquare().DistTo(this.getCurrentSquare());
    if (float1 <= 2.0F) {
      remoteChr.spottedByPlayer = true;
      return true;
    } else if (ServerOptions.getInstance().HidePlayersBehindYou.getValue() &&
               float0 < -0.5) {
      ...
      IsoGridSquare.ILighting iLighting =
          remoteChr.getCurrentSquare().lighting[this.getPlayerNum()];
      if (!iLighting.bCouldSee()) {
        return this.checkSpottedPLayerTimer(remoteChr);
      } else if (remoteChr.isSneaking() &&
                 ServerOptions.getInstance()
                     .SneakModeHideFromOtherPlayers.getValue() &&
                 !remoteChr.isSprinting()) {
        if (float1 > 30.0F) {
          remoteChr.spottedByPlayer = false;
        }
        ...
        if (remoteChr.spottedByPlayer) {
          ...
          remoteChr.doRenderShadow = true;
          float float2 =
              (float)(Math.pow(Math.max(40.0F - float1, 0.0F), 3.0) / 12000.0);
          float float3 =
              (float)(1.0 - remoteChr.remoteSneakLvl / 10.0F * 0.9 + 0.3);
          float float4 = 1.0F;
          if (float0 < 0.8) {
            float4 = 0.3F;
          }
          ...
          float float5 =
              (iLighting.lightInfo().getR() + iLighting.lightInfo().getG() +
               iLighting.lightInfo().getB()) /
              3.0F;
          float float6 = (float)((1.0 - this.getMoodles().getMoodleLevel(
                                            MoodleType.Tired) /
                                            5.0F) *
                                     0.7 +
                                 0.3);
          ...
          ArrayList arrayList = PolygonalMap2.instance.getPointInLine(
              remoteChr.getX(), remoteChr.getY(), this.getX(), this.getY(),
              (int)this.getZ());
          ...
          for (int int0 = 0; int0 < arrayList.size(); int0++) {
            PolygonalMap2.Point point =
                (PolygonalMap2.Point)arrayList.get(int0);
            square = IsoCell.getInstance().getGridSquare(
                (double)point.x, (double)point.y, (double)this.getZ());
            if (square != nullptr) {
          ...
          float float12 = Math.max(
              0.1F, 1.0F - ClimateManager.getInstance().getFogIntensity());
          float float13 = float4 * float2 * float5 * float3 * float6 * float7 *
                          float11 * float12;
          if (float13 >= 1.0F) {
            remoteChr.spottedByPlayer = true;
            return true;
            ...
            float13 =
                (float)(1.0 - Math.pow(1.0F - float13,
...
bool IsUsingAimWeapon() {
  if (this.leftHandItem == nullptr) {
    ...
    return !this.isAiming() ? false
                            : ((HandWeapon)this.leftHandItem).bIsAimedFirearm;
  }
}
...
bool IsUsingAimHandWeapon() {
  if (this.leftHandItem == nullptr) {
    ...
    return !this.isAiming()
               ? false
               : ((HandWeapon)this.leftHandItem).bIsAimedHandWeapon;
  }
}

bool DoAimAnimOnAiming() { return this.IsUsingAimWeapon(); }

int getSleepingPillsTaken() { return this.sleepingPillsTaken; }
...
bool isOutside() {
  return this.getCurrentSquare() != nullptr &&
         this.getCurrentSquare().getRoom() == nullptr && !this.isInARoom();
}

double getLastSeenZomboidTime() { return this.lastSeenZombieTime; }
...
InventoryItem getActiveLightItem() {
  ...
  } else if (this.leftHandItem != nullptr &&
             this.leftHandItem.isEmittingLight()) {
...
bool pressedMovement(bool ignoreBlock) {
  if (this.isNPC) {
    ...
    if (ignoreBlock ||
        !this.isBlockMovement() && !this.isIgnoreInputsForDirection()) {
      if (this.PlayerIndex != 0 ||
        ...
        if (this.JoypadBind != -1) {
          float float0 =
              JoypadManager.instance.getMovementAxisY(this.JoypadBind);
          float float1 =
              JoypadManager.instance.getMovementAxisX(this.JoypadBind);
          float float2 = JoypadManager.instance.getDeadZone(this.JoypadBind, 0);
...
bool pressedCancelAction() {
  if (this.isNPC) {
    ...
    return this.networkAI.isPressedCancelAction();
             ...
             GameKeyboard.isKeyDown(
                 Core.getInstance().getKey("CancelAction"))) {
    if (GameClient.bClient && this.isLocal()) {
...
bool checkWalkTo() {
  if (this.isNPC) {
    return false;
             ...
             GameKeyboard.isKeyDown(Core.getInstance().getKey("WalkTo"))) {
...
long getSteamID() { return this.steamID; }
...
int getOffSetXUI() { return this.offSetXUI; }

void setOffSetXUI(int _offSetXUI) { this.offSetXUI = _offSetXUI; }

int getOffSetYUI() { return this.offSetYUI; }

void setOffSetYUI(int _offSetYUI) { this.offSetYUI = _offSetYUI; }
...
std::string getUsername(bool canShowFirstname) {
  ...
  if (canShowFirstname && GameClient.bClient &&
      ServerOptions.instance.ShowFirstAndLastName.getValue() &&
      this.isAccessLevel("None")) {
    string = this.getDescriptor().getForename() + " " +
...
void updateUsername() {
  if (!GameClient.bClient && !GameServer.bServer) {
    this.username =
        this.getDescriptor().getForename() + this.getDescriptor().getSurname();
  }
}
...
bool isOnlyPlayerAsleep() {
  if (!this.isAsleep()) {
    ...
    for (int int0 = 0; int0 < numPlayers; int0++) {
      if (players[int0] != nullptr && !players[int0].isDead() &&
          players[int0] != this && players[int0].isAsleep()) {
        return false;
      }
    }
...
bool isNoClip() { return this.noClip; }

void setNoClip(bool _noClip) { this.noClip = _noClip; }

void setAuthorizeMeleeAction(bool enabled) {
  this.authorizeMeleeAction = enabled;
}

bool isAuthorizeMeleeAction() { return this.authorizeMeleeAction; }

void setAuthorizeShoveStomp(bool enabled) {
  this.authorizeShoveStomp = enabled;
}

bool isAuthorizeShoveStomp() { return this.authorizeShoveStomp; }

bool isBlockMovement() { return this.blockMovement; }
...
void startReceivingBodyDamageUpdates(IsoPlayer other) {
  if (GameClient.bClient && other != nullptr && other != this &&
      this.isLocalPlayer() && !other.isLocalPlayer()) {
...
Nutrition getNutrition() { return this.nutrition; }

Fitness getFitness() { return this.fitness; }
...
bool updateRemotePlayer() {
  if (!this.bRemote) {
    ...
    if (GameClient.bClient) {
      ...
      float float1;
      float float2;
      if (!this.networkAI.isCollisionEnabled() &&
          !this.networkAI.isNoCollisionTimeout()) {
          ...
          if (eventPacket.isTimeout()) {
            ...
            if (Core.bDebug) {
              DebugLog.log(DebugType.Multiplayer,
                           String.format("Event timeout (%d) : %s",
                                         this.networkAI.events.size(),
                                         eventPacket.getDescription()));
            }
      ...
      if (!boolean0 && this.networkAI.collidePointX > -1.0F &&
          this.networkAI.collidePointY > -1.0F &&
          ((int)this.x != (int)this.networkAI.collidePointX ||
           (int)this.y != (int)this.networkAI.collidePointY)) {
        float0 = this.networkAI.collidePointX;
        float1 = this.networkAI.collidePointY;
        DebugLog.log(DebugType.ActionSystem,
                     "Player " + this.username + ": collide point (" + float0 +
                         ", " + float1 +
                         ") has not been reached, so move to it");
      }
      ...
      if (!this.networkAI.forcePathFinder && this.isCollidedThisFrame() &&
          IsoUtils.DistanceManhatten(float0, float1, this.x, this.y) > 3.0F) {
        this.networkAI.forcePathFinder = true;
      }

      if (this.networkAI.forcePathFinder &&
              !PolygonalMap2.instance.lineClearCollide(
                  this.x, this.y, float0, float1, (int)this.z, this.vehicle,
                  false, true) &&
              IsoUtils.DistanceManhatten(float0, float1, this.x, this.y) <
                  2.0F ||
      ...
      if (!this.networkAI.needToMovingUsingPathFinder &&
          !this.networkAI.forcePathFinder) {
        if (this.networkAI.usePathFind) {
          pathFindBehavior2.reset();
          this.setPath2(nullptr);
          this.networkAI.usePathFind = false;
        ...
        if (this.getCurrentState() != ClimbOverWallState.instance() &&
            this.getCurrentState() != ClimbOverFenceState.instance()) {
          float float3 =
              IsoUtils.DistanceTo(this.x, this.y, this.networkAI.targetX,
                                  this.networkAI.targetY) /
              IsoUtils.DistanceTo(this.realx, this.realy,
        ...
        this.m_isPlayerMoving =
            !boolean0 && IsoUtils.DistanceManhatten(float0, float1, this.x,
                                                    this.y) > 0.2F ||
            (int)float0 != (int)this.x || (int)float1 != (int)this.y ||
        ...
        if (!this.networkAI.usePathFind ||
            float0 != pathFindBehavior2.getTargetX() ||
            float1 != pathFindBehavior2.getTargetY()) {
        ...
        if (behaviorResult == PathFindBehavior2.BehaviorResult.Failed) {
          ...
          if (this.networkAI.forcePathFinder) {
            this.networkAI.forcePathFinder = false;
          } else if (NetworkTeleport.teleport(
                         this, NetworkTeleport.Type.teleportation, float0,
                         float1, (byte)float2, 1.0F) &&
                     GameServer.bServer) {
            DebugLog.Multiplayer.warn(
                String.format("Player %d teleport from (%.2f, %.2f, %.2f) to "
                              "(%.2f, %.2f, %.2f)",
                              this.getOnlineID(), this.x, this.y, this.z,
                              float0, float1, float2));
          }
        } else if (behaviorResult ==
                   PathFindBehavior2.BehaviorResult.Succeeded) {
      ...
      if (!this.m_isPlayerMoving || this.isAiming()) {
        ...
        tempo.rotate(float5);
        tempo.setLength(Math.min(
            IsoUtils.DistanceTo(float0, float1, this.x, this.y), 1.0F));
...
void setMeleeHitSurface(const std::string &material) {
  try {
    ...
    this.parameterMeleeHitSurface.setMaterial(
        ParameterMeleeHitSurface.Material.Default);
  }
}
...
void updateHeartSound() {
  if (!GameServer.bServer) {
    if (this.isLocalPlayer()) {
      ...
      bool boolean0 = gameSound != null && gameSound.getUserVolume() > 0.0F &&
                      this.stats.Panic > 0.0F;
      if (!this.Asleep && boolean0 &&
          GameTime.getInstance().getTrueMultiplier() == 1.0F) {
        this.heartDelay =
            this.heartDelay - GameTime.getInstance().getMultiplier() / 1.6F;
        if (this.heartEventInstance == 0L ||
            !this.getEmitter().isPlaying(this.heartEventInstance)) {
          this.heartEventInstance =
              this.getEmitter().playSoundImpl("HeartBeat", nullptr);
          this.getEmitter().setVolume(this.heartEventInstance, 0.0F);
        }

        if (this.heartDelay <= 0.0F) {
          this.heartDelayMax =
              (int)((1.0F - this.stats.Panic / 100.0F * 0.7F) * 25.0F) * 2;
          this.heartDelay = this.heartDelayMax;
          if (this.heartEventInstance != 0L) {
            this.getEmitter().setVolume(this.heartEventInstance,
    ...
    if (this.isLocalPlayer()) {
      if (this.getPlayerNum() == 0 &&
          (this.worldAmbianceInstance == 0L ||
           !this.getEmitter().isPlaying(this.worldAmbianceInstance))) {
        this.worldAmbianceInstance =
            this.getEmitter().playSoundImpl("WorldAmbiance", nullptr);
  ...
  if (!GameServer.bServer) {
    if (this.isLocalPlayer()) {
      ...
      if (item != nullptr && item.IsInventoryContainer()) {
        std::string string0 =
            item.getSoundParameter("EquippedBaggageContainer");
        ...
        if (item != nullptr && item.IsInventoryContainer()) {
          ...
          if (item != nullptr && item.IsInventoryContainer()) {
            ...
            this.parameterEquippedBaggageContainer.setContainerType(
...
void checkVehicleContainers() {
        ...
        for (int int10 = 0; int10 < chunk.vehicles.size(); int10++) {
          ...
          if (vehicleScript != nullptr) {
            for (int int11 = 0; int11 < vehicleScript.getPartCount(); int11++) {
              ...
              if (part.container != nullptr && part.area != nullptr &&
                  vehicle.isInArea(part.area, this)) {
                IsoPlayer.VehicleContainer vehicleContainer0 =
                    this.vehicleContainerData.freeContainers.isEmpty()
  ...
  if (arrayList.size() != this.vehicleContainerData.containers.size()) {
    this.vehicleContainerData.freeContainers.addAll(
        this.vehicleContainerData.containers);
    ...
    for (int int12 = 0; int12 < arrayList.size(); int12++) {
      IsoPlayer.VehicleContainer vehicleContainer1 =
          (IsoPlayer.VehicleContainer)arrayList.get(int12);
      IsoPlayer.VehicleContainer vehicleContainer2 =
          this.vehicleContainerData.containers.get(int12);
...
std::string setPlayerStats(ByteBuffer bb, const std::string &adminUsername) {
  ...
  if (!string1 == this.getDescriptor().getForename())) {
      if (GameServer.bServer) {
        string5 =
            adminUsername + " Changed " + string0 + " forname in " + string1;
      } else {
  ...
  if (!string2 == this.getDescriptor().getSurname())) {
      if (GameServer.bServer) {
        string5 =
            adminUsername + " Changed " + string0 + " surname in " + string2;
      } else {
  ...
  if (!string3 == this.getDescriptor().getProfession())) {
      if (GameServer.bServer) {
        string5 =
            adminUsername + " Changed " + string0 + " profession to " + string3;
      } else {
  ...
  if (!this.getDisplayName() == string0)) {
      if (GameServer.bServer) {
        string5 = adminUsername + " Changed display name \"" +
                  this.getDisplayName() + "\" to \"" + string0 + "\"";
  ...
  if (boolean0 != this.isAllChatMuted()) {
    if (boolean0) {
      if (GameServer.bServer) {
        string5 =
            adminUsername + " Banned " + string0 + " from using /all chat";
      } else {
...
bool isAllChatMuted() { return this.allChatMuted; }

void setAllChatMuted(bool _allChatMuted) { this.allChatMuted = _allChatMuted; }
...
void addMechanicsItem(const std::string &itemid, VehiclePart part, long milli) {
  ...
  if (this.mechanicsItem.get(Long.parseLong(itemid)) == nullptr) {
    if (part.getTable("uninstall") != nullptr &&
        part.getTable("uninstall").rawget("skills") != nullptr) {
      String[] strings =
          ((String)part.getTable("uninstall").rawget("skills")).split(";");
  ...
  this.getXp().AddXP(PerkFactory.Perks.Mechanics,
                     (float)Rand.Next((int)byte0, (int)byte1));
}
...
void updateTemperatureCheck() {
  ...
  if (this.hypothermiaCache == -1 || this.hypothermiaCache != int0) {
    if (int0 >= 3 && int0 > this.hypothermiaCache && this.isAsleep() &&
        !this.ForceWakeUp) {
      this.forceAwake();
    }
  ...
  if (this.hyperthermiaCache == -1 || this.hyperthermiaCache != int1) {
    if (int1 >= 3 && int1 > this.hyperthermiaCache && this.isAsleep() &&
        !this.ForceWakeUp) {
      this.forceAwake();
    }
...
float getZombieRelevenceScore(IsoZombie z) {
  if (z.getCurrentSquare() == nullptr) {
    ...
    if (z.getCurrentSquare().getRoom() != nullptr &&
        this.current.getRoom() == nullptr) {
      float0 -= 20.0F;
    }
...
ItemVisuals getItemVisuals() { return this.itemVisuals; }
...
Vector2 getLastAngle() { return this.lastAngle; }

void setLastAngle(Vector2 _lastAngle) { this.lastAngle.set(_lastAngle); }

int getDialogMood() { return this.DialogMood; }

void setDialogMood(int _DialogMood) { this.DialogMood = _DialogMood; }

int getPing() { return this.ping; }

void setPing(int _ping) { this.ping = _ping; }

IsoMovingObject getDragObject() { return this.DragObject; }

void setDragObject(IsoMovingObject _DragObject) {
  this.DragObject = _DragObject;
}

float getAsleepTime() { return this.AsleepTime; }

void setAsleepTime(float _AsleepTime) { this.AsleepTime = _AsleepTime; }

public
Stack<IsoMovingObject> getSpottedList() { return this.spottedList; }

int getTicksSinceSeenZombie() { return this.TicksSinceSeenZombie; }
...
void setWaiting(bool _Waiting) { this.Waiting = _Waiting; }

IsoSurvivor getDragCharacter() { return this.DragCharacter; }

void setDragCharacter(IsoSurvivor _DragCharacter) {
  this.DragCharacter = _DragCharacter;
}

float getHeartDelay() { return this.heartDelay; }

void setHeartDelay(float _heartDelay) { this.heartDelay = _heartDelay; }

float getHeartDelayMax() { return this.heartDelayMax; }

void setHeartDelayMax(int _heartDelayMax) {
  this.heartDelayMax = _heartDelayMax;
}

double getHoursSurvived() { return this.HoursSurvived; }

void setHoursSurvived(double hrs) { this.HoursSurvived = hrs; }

float getMaxWeightDelta() { return this.maxWeightDelta; }
...
int getFollowID() { return this.followID; }

void setFollowID(int _followID) { this.followID = _followID; }

bool isbSeenThisFrame() { return this.bSeenThisFrame; }

void setbSeenThisFrame(bool _bSeenThisFrame) {
  this.bSeenThisFrame = _bSeenThisFrame;
}

bool isbCouldBeSeenThisFrame() { return this.bCouldBeSeenThisFrame; }

void setbCouldBeSeenThisFrame(bool _bCouldBeSeenThisFrame) {
  this.bCouldBeSeenThisFrame = _bCouldBeSeenThisFrame;
}

float getTimeSinceLastStab() { return this.timeSinceLastStab; }
...
public
Stack<IsoMovingObject> getLastSpotted() { return this.LastSpotted; }

void setLastSpotted(Stack<IsoMovingObject> _LastSpotted) {
  this.LastSpotted = _LastSpotted;
}

int getClearSpottedTimer() { return this.ClearSpottedTimer; }
...
bool IsAiming() { return this.isAiming(); }
...
void setTagColor(ColorInfo _tagColor) { this.tagColor.set(_tagColor); }
...
bool isSeeNonPvpZone() {
  return this.seeNonPvpZone ||
         DebugOptions.instance.MultiplayerSeeNonPvpZones.getValue();
}
...
bool isShowTag() { return this.showTag; }

void setShowTag(bool show) { this.showTag = show; }

bool isFactionPvp() { return this.factionPvp; }

void setFactionPvp(bool pvp) { this.factionPvp = pvp; }

bool isForceAim() { return this.forceAim; }

void setForceAim(bool _forceAim) { this.forceAim = _forceAim; }
...
bool isForceSprint() { return this.forceSprint; }

void setForceSprint(bool _forceSprint) { this.forceSprint = _forceSprint; }
...
bool isForceRun() { return this.forceRun; }

void setForceRun(bool _forceRun) { this.forceRun = _forceRun; }
...
bool isDeaf() { return this.Traits.Deaf.isSet(); }

bool isForceOverrideAnim() { return this.forceOverrideAnim; }
...
bool isWearingNightVisionGoggles() { return this.isWearingNightVisionGoggles; }
 ...
 * @param isCulled Whether or not this object is culled from the visible scene
 * or not.
 */
    ...
    if (this.getCurrentSquare() != nullptr) {
      if (this.getCurrentSquare().getBrokenGlass() != nullptr) {
        BodyPartType bodyPartType0 = BodyPartType.FromIndex(
            Rand.Next(BodyPartType.ToIndex(BodyPartType.Foot_L),
      ...
      if (this.getCurrentSquare().getZone() != nullptr &&
              (this.getCurrentSquare().getZone().getType() == "Forest") ||
          this.getCurrentSquare().getZone().getType() == "DeepForest"))) {
                    boolean0 = true;
                }
      ...
      if (object != nullptr && object.getSprite() != nullptr &&
        ...
        if (string.contains("blends_natural_01") && boolean0) {
          byte0 = 2;
        } else if (!string.contains("blends_natural_01") &&
                   this.getCurrentSquare().getBuilding() == nullptr) {
          byte0 = 1;
        }
      }
      ...
      if (byte0 != 0) {
        ...
        if (Rand.Next(Rand.AdjustForFramerate(8500 - this.footInjuryTimer)) <=
            0) {
          ...
          if (bodyPart1.getScratchTime() > 30.0F) {
            if (!bodyPart1.isCut()) {
              ...
              bodyPart1.setCutTime(bodyPart1.getCutTime() +
                                   Rand.Next(1.0F, 3.0F));
            }
          } else {
            if (!bodyPart1.scratched()) {
              ...
              bodyPart1.setScratchTime(bodyPart1.getScratchTime() +
                                       Rand.Next(1.0F, 3.0F));
            }

            if (bodyPart1.getScratchTime() > 20.0F &&
                bodyPart1.getBleedingTime() == 0.0F) {
...
bool isAttackStarted() { return this.attackStarted; }

bool isBehaviourMoving() { return this.hasPath() || super.isBehaviourMoving(); }

bool isJustMoved() { return this.JustMoved; }

void setJustMoved(bool val) { this.JustMoved = val; }

bool isPlayerMoving() { return this.m_isPlayerMoving; }

float getTimedActionTimeModifier() {
  return this.getBodyDamage().getThermoregulator() != nullptr
             ? this.getBodyDamage()
                   .getThermoregulator()
...
void setInitiateAttack(bool initiate) { this.initiateAttack = initiate; }

bool isIgnoreInputsForDirection() { return this.ignoreInputsForDirection; }

void setIgnoreInputsForDirection(bool _ignoreInputsForDirection) {
  this.ignoreInputsForDirection = _ignoreInputsForDirection;
}

bool isIgnoreContextKey() { return this.ignoreContextKey; }

void setIgnoreContextKey(bool _ignoreContextKey) {
  this.ignoreContextKey = _ignoreContextKey;
}

bool isIgnoreAutoVault() { return this.ignoreAutoVault; }

void setIgnoreAutoVault(bool _ignoreAutoVault) {
  this.ignoreAutoVault = _ignoreAutoVault;
}

bool isAllowSprint() { return this.allowSprint; }

void setAllowSprint(bool _allowSprint) { this.allowSprint = _allowSprint; }

bool isAllowRun() { return this.allowRun; }

void setAllowRun(bool _allowRun) { this.allowRun = _allowRun; }
...
bool isCanSeeAll() { return this.canSeeAll; }

void setCanSeeAll(bool b) { this.canSeeAll = b; }

bool isNetworkTeleportEnabled() { return NetworkTeleport.enable; }

void setNetworkTeleportEnabled(bool b) { NetworkTeleport.enable = b; }
...
float getRelevantAndDistance(float x, float y, float RelevantRange) {
  return Math.abs(this.x - x) <= RelevantRange * 10.0F &&
                 Math.abs(this.y - y) <= RelevantRange * 10.0F
             ? IsoUtils.DistanceTo(this.x, this.y, x, y)
             : Float.POSITIVE_INFINITY;
}

bool isCanHearAll() { return this.canHearAll; }

void setCanHearAll(bool b) { this.canHearAll = b; }

public
ArrayList<String> getAlreadyReadBook() { return this.alreadyReadBook; }

void setMoodleCantSprint(bool b) { this.MoodleCantSprint = b; }

void setAttackFromBehind(bool _attackFromBehind) {
  this.attackFromBehind = _attackFromBehind;
}

bool isAttackFromBehind() { return this.attackFromBehind; }
...
float getDamageFromHitByACar(float vehicleSpeed) {
  ...
  if (float1 > 0.0F) {
    ...
    for (int int1 = 0; int1 < int0; int1++) {
      int int2 = Rand.Next(BodyPartType.ToIndex(BodyPartType.Hand_L),
                           BodyPartType.ToIndex(BodyPartType.MAX));
      BodyPart bodyPart =
          this.getBodyDamage().getBodyPart(BodyPartType.FromIndex(int2));
      ...
      if (float2 > 10.0F && Rand.Next(100) <= 10 &&
          SandboxOptions.instance.BoneFracture.getValue()) {
        bodyPart.setFractureTime(
            Rand.Next(Rand.Next(10.0F, float2 + 10.0F),
                      Rand.Next(float2 + 20.0F, float2 + 30.0F)));
      }

      if (float2 > 30.0F && Rand.Next(100) <= 80 &&
          SandboxOptions.instance.BoneFracture.getValue() &&
      ...
      if (float2 > 10.0F && Rand.Next(100) <= 60 &&
          SandboxOptions.instance.BoneFracture.getValue() &&
          int2 > BodyPartType.ToIndex(BodyPartType.Groin)) {
        bodyPart.setFractureTime(
            Rand.Next(Rand.Next(10.0F, float2 + 20.0F),
                      Rand.Next(float2 + 30.0F, float2 + 40.0F)));
      }
    }
...
float Hit(BaseVehicle vehicle, float speed, bool isHitFromBehind, float hitDirX,
          float hitDirY) {
...
bool shouldDoInventory() { return this.isLocalPlayer(); }
...
void setFitnessSpeed() {
  ...
  float float0 = this.getPerkLevel(PerkFactory.Perks.Fitness) / 5.0F / 1.1F -
                 this.getMoodleLevel(MoodleType.Endurance) / 20.0F;
  if (float0 > 1.5F) {
    float0 = 1.5F;
  }
...
bool isLocal() { return super.isLocal() || this.isLocalPlayer(); }

bool isClimbOverWallSuccess() { return this.climbOverWallSuccess; }

void setClimbOverWallSuccess(bool _climbOverWallSuccess) {
  this.climbOverWallSuccess = _climbOverWallSuccess;
}

bool isClimbOverWallStruggle() { return this.climbOverWallStruggle; }
...
bool isVehicleCollisionActive(BaseVehicle testVehicle) {
  if (!super.isVehicleCollisionActive(testVehicle)) {
    ...
    IsoPlayer player = GameClient.IDToPlayerMap.get(
        this.vehicle4testCollision.getNetPlayerId());
    if (!SwipeStatePlayer.checkPVP(player, this)) {
      return false;
    } else if (SandboxOptions.instance.DamageToPlayerFromHitByACar.getValue() <
               1) {
      return false;
    } else {
...
bool isShowMPInfos() { return this.showMPInfos; }

void setShowMPInfos(bool b) { this.showMPInfos = b; }
...
public
  IsoPlayer.VehicleContainer set(BaseVehicle vehiclex, int int0) {
  ...
  bool equals(void *object) {
    return object instanceof
        IsoPlayer.VehicleContainer &&
            ...
            this.containerIndex ==
                ((IsoPlayer.VehicleContainer)object).containerIndex;
  }
}

private static class VehicleContainerData {
  ArrayList<IsoPlayer.VehicleContainer> tempContainers =
      std::make_unique<ArrayList<>>();
  ArrayList<IsoPlayer.VehicleContainer> containers =
      std::make_unique<ArrayList<>>();
  Stack<IsoPlayer.VehicleContainer> freeContainers =
      std::make_unique<Stack<>>();
}

private static class s_performance {
  static const PerformanceProfileProbe postUpdate =
      new PerformanceProfileProbe("IsoPlayer.postUpdate");
  static const PerformanceProfileProbe highlightRangedTargets =
      new PerformanceProfileProbe("IsoPlayer.highlightRangedTargets");
  static const PerformanceProfileProbe update =
      new PerformanceProfileProbe("IsoPlayer.update");
}
}
} // namespace characters
} // namespace zombie
