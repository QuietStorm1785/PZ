...
private
  final ArrayList<ActionState> m_childStates = std::make_unique<ArrayList<>>();
  std::string m_previousStateName = null;
  bool m_statesChanged = false;
public
  final ArrayList<IActionStateChanged> onStateChanged =
      std::make_unique<ArrayList<>>();
  const ActionContextEvents occurredEvents = new ActionContextEvents();

public
  ActionContext(IAnimatable owner) { this.m_owner = owner; }

  IAnimatable getOwner() { return this.m_owner; }

  void update() {
    ActionContext.s_performance.update.invokeAndMeasure(
        this, ActionContext::updateInternal);
  }
  ...
  void updateInternal() {
    if (this.m_currentState == nullptr) {
      ...
      ActionContext.s_performance.evaluateCurrentStateTransitions
          .invokeAndMeasure(this,
                            ActionContext::evaluateCurrentStateTransitions);
      ActionContext.s_performance.evaluateSubStateTransitions.invokeAndMeasure(
          this, ActionContext::evaluateSubStateTransitions);
  ...
  ActionState getNextState() {
    ...
    for (int int0 = 0; int0 < this.m_currentState.transitions.size(); int0++) {
      ActionTransition actionTransition0 =
          this.m_currentState.transitions.get(int0);
      if (actionTransition0.passes(this, 0) &&
          !StringUtils.isNullOrWhitespace(actionTransition0.transitionTo)) {
        ActionState actionState1 =
            this.m_stateGroup.get(actionTransition0.transitionTo);
        if (actionState1 != nullptr && !this.hasChildState(actionState1)) {
          if (!actionTransition0.asSubstate ||
              !this.currentStateSupportsChildState(actionState1)) {
      ...
      for (int int2 = 0; int2 < actionState3.transitions.size(); int2++) {
        ...
        if (actionTransition1.passes(this, 1)) {
          ...
          if (!StringUtils.isNullOrWhitespace(actionTransition1.transitionTo)) {
            ActionState actionState4 =
                this.m_stateGroup.get(actionTransition1.transitionTo);
      ...
      if (actionTransition.passes(this, 0)) {
        if (StringUtils.isNullOrWhitespace(actionTransition.transitionTo)) {
          DebugLog.ActionSystem.warn(
              "%s> Transition's target state not specified: \"%s\"",
              this.getOwner().getUID(), actionTransition.transitionTo);
        } else {
          ActionState actionState =
              this.m_stateGroup.get(actionTransition.transitionTo);
          if (actionState == nullptr) {
            DebugLog.ActionSystem.warn(
                "%s> Transition's target state not found: \"%s\"",
                this.getOwner().getUID(), actionTransition.transitionTo);
          } else if (!this.hasChildState(actionState)) {
            if (!actionTransition.asSubstate ||
                !this.currentStateSupportsChildState(actionState)) {
              if (this.m_owner instanceof IsoPlayer) {
                DebugLog.log(
                    DebugType.ActionSystem,
                    "Player '" + ((IsoPlayer)this.m_owner).getUsername() +
                        "' transits from " + this.m_currentState.getName() +
                        " to " + actionTransition.transitionTo);
              }
  ...
  bool currentStateSupportsChildState(ActionState actionState) {
    return this.m_currentState == nullptr
               ? false
               : this.m_currentState.canHaveSubState(actionState);
  }

  bool hasChildState(ActionState actionState) {
    int int0 =
        this.indexOfChildState(actionState0->actionState0 == actionState);
    return int0 > -1;
  }
      ...
      if (snapshot.stateName == nullptr) {
        ...
        if (PZArrayUtil.isNullOrEmpty(snapshot.childStateNames)) {
          ...
          for (int int0 = 0; int0 < this.childStateCount(); int0++) {
            ...
            bool boolean0 =
                StringUtils.contains(snapshot.childStateNames, string0,
  ...
  ActionStateSnapshot getPlaybackStateSnapshot() {
    if (this.m_currentState == nullptr) {
      ...
      actionStateSnapshot.childStateNames =
          new String[this.m_childStates.size()];

      for (int int0 = 0; int0 < actionStateSnapshot.childStateNames.length;
           int0++) {
        actionStateSnapshot.childStateNames[int0] =
            this.m_childStates.get(int0).name;
      }
  ...
  bool setCurrentState(ActionState actionState0) {
    if (actionState0 == this.m_currentState) {
      ...
      this.m_previousStateName =
          this.m_currentState == nullptr ? "" : this.m_currentState.getName();
  ...
  void onStatesChanged() { this.m_statesChanged = true; }

  void logCurrentState() {
    if (this.m_owner.isAnimationRecorderActive()) {
      this.m_owner.getAnimationPlayerRecorder().logActionState(
          this.m_currentState, this.m_childStates);
    }
  }
  ...
  ActionState getCurrentState() { return this.m_currentState; }

  void setGroup(ActionGroup group) {
    std::string string =
        this.m_currentState == null ? null : this.m_currentState.name;
  ...
  ActionState getChildStateAt(int idx) {
    if (idx >= 0 && idx < this.childStateCount()) {
      ...
      throw new IndexOutOfBoundsException(
          String.format("Index %d out of bounds. childCount: %d", idx,
                        this.childStateCount()));
    }
  }

public
  List<ActionState> getChildStates() { return this.m_childStates; }

  std::string getCurrentStateName() { return this.m_currentState.name; }

  std::string getPreviousStateName() { return this.m_previousStateName; }
  ...
  static {
    CharacterVariableCondition.Factory factory =
        new CharacterVariableCondition.Factory();
    ...
    IActionCondition.registerFactory("eventOccurred",
                                     new EventOccurred.Factory());
    IActionCondition.registerFactory("eventNotOccurred",
                                     new EventNotOccurred.Factory());
...
private
  static class s_performance {
    static const PerformanceProfileProbe update =
        new PerformanceProfileProbe("ActionContext.update");
    static const PerformanceProfileProbe evaluateCurrentStateTransitions =
        new PerformanceProfileProbe(
            "ActionContext.evaluateCurrentStateTransitions");
    static const PerformanceProfileProbe evaluateSubStateTransitions =
        new PerformanceProfileProbe(
            "ActionContext.evaluateSubStateTransitions");
  }
}
} // namespace action
} // namespace characters
