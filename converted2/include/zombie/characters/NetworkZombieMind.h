...
public
  NetworkZombieMind(IsoZombie _zombie) { this.zombie = _zombie; }

  void set(ZombiePacket packet) {
    PathFindBehavior2 pathFindBehavior2 = this.zombie.getPathFindBehavior2();
    if (pathFindBehavior2.getIsCancelled() || pathFindBehavior2.isGoalNone() ||
        pathFindBehavior2.bStopping || this.zombie.realState == nullptr ||
    ...
    if (!this.pfbIsCanceled) {
      ...
      if (this.pfbType == 1) {
        if (GameServer.bServer) {
          this.pfbTargetCharacter =
              GameServer.IDToPlayerMap.get(packet.pfbTarget);
        } else if (GameClient.bClient) {
          this.pfbTargetCharacter =
              GameClient.IDToPlayerMap.get(packet.pfbTarget);
        }
  ...
  void doRestorePFBTarget() {
    if (!this.pfbIsCanceled) {
      if (this.pfbType == 1 && this.pfbTargetCharacter != nullptr) {
        ...
        this.zombie.pathToLocationF(this.pfbTargetX, this.pfbTargetY,
                                    this.pfbTargetZ);
      } else if (this.pfbType == 3) {
        this.zombie.pathToSound((int)this.pfbTargetX, (int)this.pfbTargetY,
                                (int)this.pfbTargetZ);
        this.zombie.alerted = false;
        this.zombie.setLastHeardSound(
            (int)this.pfbTargetX, (int)this.pfbTargetY, (int)this.pfbTargetZ);
