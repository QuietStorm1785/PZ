...
public:
public
  final ArrayList<BodyPart> BodyParts = new ArrayList<>(18);
public
  final ArrayList<BodyPartLast> BodyPartsLastState = new ArrayList<>(18);
...
public
  ...
  bool IsFakeInfected() { return this.isIsFakeInfected(); }

  void OnFire(bool OnFire) { this.setIsOnFire(OnFire); }

  bool IsOnFire() { return this.isIsOnFire(); }

  bool WasBurntToDeath() { return this.isBurntToDeath(); }
  ...
  void IncreasePanicFloat(float delta) {
    ...
    stats.Panic =
        stats.Panic + this.getPanicIncreaseValueFrame() * delta * float0;
  ...
  void IncreasePanic(int NumNewZombiesSeen) {
    ...
    Stats stats = this.ParentChar.getStats();
    stats.Panic =
        stats.Panic + this.getPanicIncreaseValue() * NumNewZombiesSeen * float0;
    if (this.getParentChar().getStats().Panic > 100.0F) {
      this.ParentChar.getStats().Panic = 100.0F;
    }
  }
  ...
  void ReducePanic() {
    if (!(this.ParentChar.getStats().Panic <= 0.0F)) {
      float float0 = this.getPanicReductionValue() *
                     (GameTime.getInstance().getMultiplier() / 1.6F);
      int int0 = (int)Math.floor(
          new Double(GameTime.instance.getNightsSurvived()) / 30.0);
      if (int0 > 5) {
        int0 = 5;
      }
  ...
  void JustDrankBooze(Food food, float percentage) {
    ...
    if (food.getBaseHunger() != 0.0F) {
      percentage =
          food.getHungChange() * percentage / food.getBaseHunger() * 2.0F;
    }

    float0 *= percentage;
    if (food.getName().toLowerCase().contains("beer") ||
        food.hasTag("LowAlcohol")) {
      float0 *= 0.25F;
    }
    ...
    Stats stats = this.ParentChar.getStats();
    stats.Drunkenness =
        stats.Drunkenness + this.getDrunkIncreaseValue() * float0;
  ...
  void JustTookPill(InventoryItem Pill) {
    if ("PillsBeta" == Pill.getType())) {
        if (this.getParentChar() != nullptr &&
            this.getParentChar().getStats().Drunkenness > 10.0F) {
        ...
        if (this.getParentChar() instanceof IsoPlayer) {
          ((IsoPlayer)this.getParentChar())
              .setSleepingPillsTaken(
                  ((IsoPlayer)this.getParentChar()).getSleepingPillsTaken() +
                  1);
        }
      }
  ...
  void JustAteFood(Food NewFood, float percentage) {
    ...
    if (NewFood.getReduceInfectionPower() > 0.0F) {
      this.getParentChar().setReduceInfectionPower(
          NewFood.getReduceInfectionPower());
    }

    this.setBoredomLevel(this.getBoredomLevel() +
                         NewFood.getBoredomChange() * percentage);
    if (this.getBoredomLevel() < 0.0F) {
      this.setBoredomLevel(0.0F);
    }

    this.setUnhappynessLevel(this.getUnhappynessLevel() +
                             NewFood.getUnhappyChange() * percentage);
    ...
    if (this.getParentChar().getStats().hunger <= 0.0F) {
      ...
      this.setHealthFromFoodTimer(
          (int)(this.getHealthFromFoodTimer() +
                float1 * this.getHealthFromFoodTimeByHunger()));
      if (NewFood.isCooked()) {
        this.setHealthFromFoodTimer(
            (int)(this.getHealthFromFoodTimer() +
                  float1 * this.getHealthFromFoodTimeByHunger()));
    ...
    if (!"Tutorial" == Core.getInstance().getGameMode())) {
        ...
        if (NewFood.getAge() >= NewFood.getOffAgeMax()) {
          ...
          if (NewFood.getOffAgeMax() > NewFood.getOffAge()) {
            int0 =
                (int)(float2 / (NewFood.getOffAgeMax() - NewFood.getOffAge()) *
                      100.0F);
          } else {
            int0 = 100;
          }
          ...
          if (Rand.Next(100) < int0 && !this.isInfected()) {
            this.PoisonLevel =
                this.PoisonLevel +
                5.0F * Math.abs(NewFood.getHungChange() * 10.0F) * percentage;
          }
        }
      }
  }

  void JustAteFood(Food NewFood) { this.JustAteFood(NewFood, 100.0F); }

  float getHealthFromFoodTimeByHunger() { return 13000.0F; }
  ...
  void UpdateWetness() {
    ...
    bool boolean0 = square == null || !square.isInARoom() && !square.haveRoof;
    if (vehicle != nullptr &&
        vehicle.hasRoof(vehicle.getSeat(this.getParentChar()))) {
      boolean0 = false;
    }
    ...
    if (boolean0 && character.isAsleep() && character.getBed() != nullptr &&
    ...
    if (boolean0 && ClimateManager.getInstance().isRaining()) {
      ...
      float float5 = ClimateManager.getInstance().getAirTemperatureForCharacter(
          this.getParentChar());
      float float6 = 0.1F;
      if (float5 > 5.0F) {
    ...
    if (!this.isHasACold() && float8 > 0.1F) {
      ...
      this.setCatchACold(this.getCatchACold() +
                         (float)ZomboidGlobals.CatchAColdIncreaseRate * float8 *
    ...
    if (float8 <= 0.1F) {
      this.setCatchACold(this.getCatchACold() -
                         (float)ZomboidGlobals.CatchAColdDecreaseRate);
  ...
  void TriggerSneezeCough() {
    if (this.getSneezeCoughActive() <= 0) {
      ...
      if (this.getParentChar().getMoodles().getMoodleLevel(
              MoodleType.HasACold) == 2) {
        this.setSneezeCoughActive(1);
      }
      ...
      if (this.getParentChar().getMoodles().getMoodleLevel(
        ...
        this.setTimeToSneezeOrCough(
            this.getMildColdSneezeTimerMin() +
            Rand.Next(this.getMildColdSneezeTimerMax() -
                      this.getMildColdSneezeTimerMin()));
      }

      if (this.getParentChar().getMoodles().getMoodleLevel(
              MoodleType.HasACold) == 3) {
        this.setTimeToSneezeOrCough(this.getColdSneezeTimerMin() +
                                    Rand.Next(this.getColdSneezeTimerMax() -
                                              this.getColdSneezeTimerMin()));
      }

      if (this.getParentChar().getMoodles().getMoodleLevel(
              MoodleType.HasACold) == 4) {
        this.setTimeToSneezeOrCough(
            this.getNastyColdSneezeTimerMin() +
            Rand.Next(this.getNastyColdSneezeTimerMax() -
                      this.getNastyColdSneezeTimerMin()));
      }
      ...
      if (this.getParentChar().getPrimaryHandItem() == nullptr ||
          !this.getParentChar().getPrimaryHandItem().getType() == "Tissue")
                    && !this.getParentChar().getPrimaryHandItem().getType() == "ToiletPaper")) {
                      if (this.getParentChar().getSecondaryHandItem() !=
                                  nullptr &&
                              (this.getParentChar()
                    ...
                    && ((Drainable)this.getParentChar().getSecondaryHandItem()).getUsedDelta() > 0.0F) {
                      ((Drainable)this.getParentChar().getSecondaryHandItem())
                          .setUsedDelta(((Drainable)this.getParentChar()
                                             .getSecondaryHandItem())
                                            .getUsedDelta() -
                                        0.1F);
                      if (((Drainable)this.getParentChar()
                               .getSecondaryHandItem())
                              .getUsedDelta() <= 0.0F) {
      ...
      else if (((Drainable)this.getParentChar().getPrimaryHandItem())
                   .getUsedDelta() > 0.0F) {
        ((Drainable)this.getParentChar().getPrimaryHandItem())
            .setUsedDelta(((Drainable)this.getParentChar().getPrimaryHandItem())
                              .getUsedDelta() -
                          0.1F);
        if (((Drainable)this.getParentChar().getPrimaryHandItem())
                .getUsedDelta() <= 0.0F) {
      ...
      if (boolean0) {
        ...
        WorldSoundManager.instance.addSound(
            this.getParentChar(), (int)this.getParentChar().getX(),
            (int)this.getParentChar().getY(), (int)this.getParentChar().getZ(),
            byte0, byte1, true);
      }
    }
  }

  int IsSneezingCoughing() { return this.getSneezeCoughActive(); }

  void UpdateCold() {
    if (this.isHasACold()) {
      bool boolean0 = true;
      IsoGridSquare square = this.getParentChar().getCurrentSquare();
      if (square == nullptr || !square.isInARoom() ||
          this.getParentChar().getMoodles().getMoodleLevel(MoodleType.Wet) >
              0 ||
          this.getParentChar().getMoodles().getMoodleLevel(
              MoodleType.Hypothermia) >= 1 ||
      ...
      if (this.getColdReduction() > 0.0F) {
        boolean0 = true;
        this.setColdReduction(this.getColdReduction() -
                              0.005F * GameTime.instance.getMultiplier());
        if (this.getColdReduction() < 0.0F) {
      ...
      if (boolean0) {
        ...
        this.setColdStrength(this.getColdStrength() -
                             this.getColdProgressionRate() * float0 *
                                 GameTime.instance.getMultiplier());
        if (this.getColdReduction() > 0.0F) {
          this.setColdStrength(this.getColdStrength() -
                               this.getColdProgressionRate() * float0 *
        ...
        this.setColdStrength(this.getColdStrength() +
                             this.getColdProgressionRate() * float1 *
                                 GameTime.instance.getMultiplier());
      ...
      if (this.getParentChar().getMoodles().getMoodleLevel(
              MoodleType.HasACold) > 1 &&
          this.getTimeToSneezeOrCough() >= 0 && !this.ParentChar.IsSpeaking()) {
  ...
  void AddGeneralHealth(float Val) {
    ...
    if (int0 > 0) {
      ...
      for (int int2 = 0; int2 < BodyPartType.ToIndex(BodyPartType.MAX);
           int2++) {
  ...
  void ReduceGeneralHealth(float Val) {
    ...
    if (!(Val <= 0.0F)) {
      ...
      for (int int0 = 0; int0 < BodyPartType.ToIndex(BodyPartType.MAX);
           int0++) {
        this.getBodyParts().get(int0).ReduceHealth(
            float0 / BodyPartType.getDamageModifyer(int0));
      }
    }
  }
  ...
  void DamageFromWeapon(HandWeapon weapon) {
    if (GameServer.bServer) {
      if (weapon != nullptr) {
        this.getParentChar().sendObjectChange(
            "DamageFromWeapon", new Object[]{"weapon", weapon.getFullType()});
      }
    } else if (!(this.getParentChar() instanceof IsoPlayer) ||
               ((IsoPlayer)this.getParentChar()).isLocalPlayer()) {
      int int0 = 0;
      uint8_t byte0 = 1;
      bool boolean0 = true;
      int0 = Rand.Next(BodyPartType.ToIndex(BodyPartType.Hand_L),
                       BodyPartType.ToIndex(BodyPartType.MAX));
      if (DebugOptions.instance.MultiplayerTorsoHit.getValue()) {
        int0 = Rand.Next(BodyPartType.ToIndex(BodyPartType.Torso_Upper),
                         BodyPartType.ToIndex(BodyPartType.Head));
      }
      ...
      float float0 = this.getParentChar().getBodyPartClothingDefense(
          bodyPart.getIndex(), boolean2, boolean3);
      if (Rand.Next(100) < float0) {
        boolean0 = false;
        this.getParentChar().addHoleFromZombieAttacks(
            BloodBodyPartType.FromIndex(int0), false);
      }
      ...
      if (boolean0) {
        ...
        float float1 =
            Rand.Next(weapon.getMinDamage(), weapon.getMaxDamage()) * 15.0F;
        ...
        if (GameClient.bClient) {
          if (weapon.isRanged()) {
            float1 = (float)(float1 * ServerOptions.getInstance()
                                          .PVPFirearmDamageModifier.getValue());
          } else {
            float1 = (float)(float1 * ServerOptions.getInstance()
                                          .PVPMeleeDamageModifier.getValue());
          }
        }
        ...
        case 0:
          ...
          stats0.Pain = stats0.Pain + this.getInitialThumpPain() *
                                          BodyPartType.getPainModifyer(int0);
          break;
        case 1:
          Stats stats1 = this.ParentChar.getStats();
          stats1.Pain = stats1.Pain + this.getInitialScratchPain() *
                                          BodyPartType.getPainModifyer(int0);
          break;
        case 2:
          Stats stats2 = this.ParentChar.getStats();
          stats2.Pain = stats2.Pain + this.getInitialBitePain() *
                                          BodyPartType.getPainModifyer(int0);
        }
        ...
        if (this.ParentChar instanceof
            IsoPlayer && GameClient.bClient &&
                ((IsoPlayer)this.ParentChar).isLocalPlayer()) {
  ...
  /**
   * This gonna decide the strength of the damage you'll get.  Getting
  ...
  bool AddRandomDamageFromZombie(IsoZombie zombie,
    ...
    if (GameServer.bServer) {
      this.getParentChar().sendObjectChange(
          "AddRandomDamageFromZombie", new Object[]{"zombie", zombie.OnlineID});
      return true;
    } else {
      uint8_t byte0 = 0;
      int int0 = 0;
      ...
      if (!"EndDeath" == this.getParentChar().getHitReaction())) {
          if (!this.getParentChar().isGodMod() && int4 >= byte1 &&
      ...
      if (!zombie.bCrawling) {
        if (Rand.Next(10) == 0) {
          int0 = Rand.Next(BodyPartType.ToIndex(BodyPartType.Hand_L),
                           BodyPartType.ToIndex(BodyPartType.Groin) + 1);
        } else {
          int0 = Rand.Next(BodyPartType.ToIndex(BodyPartType.Hand_L),
        ...
        if (int0 == BodyPartType.ToIndex(BodyPartType.Head) ||
          ...
          if (Rand.Next(100) > byte2) {
            ...
            while (!boolean2) {
              boolean2 = true;
              int0 =
                  Rand.Next(BodyPartType.ToIndex(BodyPartType.Torso_Lower) + 1);
              if (int0 == BodyPartType.ToIndex(BodyPartType.Head) ||
                  int0 == BodyPartType.ToIndex(BodyPartType.Neck) ||
        ...
        if (Rand.Next(10) == 0) {
          int0 = Rand.Next(BodyPartType.ToIndex(BodyPartType.Groin),
                           BodyPartType.ToIndex(BodyPartType.MAX));
        } else {
          int0 = Rand.Next(BodyPartType.ToIndex(BodyPartType.UpperLeg_L),
                           BodyPartType.ToIndex(BodyPartType.MAX));
        }
      }
      ...
      if (GameServer.bServer && this.ParentChar instanceof
          IsoPlayer || Core.bDebug && this.ParentChar instanceof IsoPlayer) {
        DebugLog.log(
            DebugType.Combat,
            "zombie did " + float1 + " dmg to " +
      ...
      if (Rand.Next(100) > int1) {
        zombie.scratch = true;
        this.getParentChar().helmetFall(
            int0 == BodyPartType.ToIndex(BodyPartType.Neck) ||
            int0 == BodyPartType.ToIndex(BodyPartType.Head));
        ...
        if (zombie.scratch) {
          float float2 = this.getParentChar().getBodyPartClothingDefense(
              int0, false, false);
          zombie.parameterZombieState.setState(
              ParameterZombieState.State.AttackScratch);
          if (this.getHealth() > 0.0F) {
            this.getParentChar().getEmitter().playSoundImpl(
                "ZombieScratch", (IsoObject) nullptr);
          }

          if (Rand.Next(100) < float2) {
            this.getParentChar().addHoleFromZombieAttacks(
                BloodBodyPartType.FromIndex(int0), boolean4);
            return false;
          }

          bool boolean5 = this.getParentChar().addHole(
              BloodBodyPartType.FromIndex(int0), true);
          if (boolean5) {
            this.getParentChar().getEmitter().playSoundImpl(
                "ZombieRipClothing", (IsoObject) nullptr);
          }

          boolean3 = true;
          this.AddDamage(int0, float1);
          this.SetScratched(int0, true);
          this.getParentChar().addBlood(BloodBodyPartType.FromIndex(int0), true,
                                        false, true);
          byte0 = 1;
          if (GameServer.bServer && this.ParentChar instanceof IsoPlayer) {
            DebugLog.log(DebugType.Combat,
                         "zombie scratched " +
                             ((IsoPlayer)this.ParentChar).username);
          }
        } else if (zombie.laceration) {
          float float3 = this.getParentChar().getBodyPartClothingDefense(
              int0, false, false);
          zombie.parameterZombieState.setState(
              ParameterZombieState.State.AttackLacerate);
          if (this.getHealth() > 0.0F) {
            this.getParentChar().getEmitter().playSoundImpl(
                "ZombieScratch", (IsoObject) nullptr);
          }
          ...
          bool boolean6 = this.getParentChar().addHole(
              BloodBodyPartType.FromIndex(int0), true);
          if (boolean6) {
            this.getParentChar().getEmitter().playSoundImpl(
                "ZombieRipClothing", (IsoObject) nullptr);
          ...
          float float4 = this.getParentChar().getBodyPartClothingDefense(
              int0, true, false);
          zombie.parameterZombieState.setState(
              ParameterZombieState.State.AttackBite);
          if (this.getHealth() > 0.0F) {
            this.getParentChar().getEmitter().playSoundImpl(
                "ZombieBite", (IsoObject) nullptr);
          }
          ...
          bool boolean7 = this.getParentChar().addHole(
              BloodBodyPartType.FromIndex(int0), true);
          if (boolean7) {
            this.getParentChar().getEmitter().playSoundImpl(
                "ZombieRipClothing", (IsoObject) nullptr);
          ...
          if (int0 == BodyPartType.ToIndex(BodyPartType.Neck)) {
            this.getParentChar().addBlood(BloodBodyPartType.FromIndex(int0),
                                          false, true, true);
            this.getParentChar().addBlood(BloodBodyPartType.FromIndex(int0),
                                          false, true, true);
            this.getParentChar().addBlood(BloodBodyPartType.Torso_Upper, false,
                                          true, false);
          }
  ...
  bool doBodyPartsHaveInjuries(BodyPartType partA, BodyPartType partB) {
    return this.doesBodyPartHaveInjury(partA) ||
           this.doesBodyPartHaveInjury(partB);
  }
  ...
  void DrawUntexturedQuad(int X, int Y, int Width, int Height, float r, float g,
                          float b, float a) {
    SpriteRenderer.instance.renderi(nullptr, X, Y, Width, Height, r, g, b, a,
                                    nullptr);
  }
  ...
  float getHealth() { return this.getOverallBodyHealth(); }

  float getInfectionLevel() { return this.InfectionLevel; }

  float getApparentInfectionLevel() {
    float float0 = this.getFakeInfectionLevel() > this.InfectionLevel
                       ? this.getFakeInfectionLevel()
                       : this.InfectionLevel;
    return this.getFoodSicknessLevel() > float0 ? this.getFoodSicknessLevel()
                                                : float0;
  }
  ...
  bool IsDeepWounded(BodyPartType BodyPart) {
    return this.getBodyParts()
        .get(BodyPartType.ToIndex(BodyPart))
        .deepWounded();
  }
  ...
  void RestoreToFullHealth() {
    ...
    if (this.getParentChar() != nullptr &&
        this.getParentChar().getStats() != nullptr) {
  ...
  void SetBleedingStemmed(BodyPartType BodyPart, bool BleedingStemmed) {
    this.getBodyParts()
        .get(BodyPartType.ToIndex(BodyPart))
        .SetBleedingStemmed(BleedingStemmed);
  }
  ...
  BodyPart setScratchedWindow() {
    int int0 = Rand.Next(BodyPartType.ToIndex(BodyPartType.Hand_L),
                         BodyPartType.ToIndex(BodyPartType.ForeArm_R) + 1);
  ...
  void UpdateBoredom() {
    if (!(this.getParentChar() instanceof IsoSurvivor)) {
      if (!(this.getParentChar() instanceof IsoPlayer) ||
          !((IsoPlayer)this.getParentChar()).Asleep) {
        if (this.getParentChar().getCurrentSquare().isInARoom()) {
          if (!this.getParentChar().isReading()) {
            this.setBoredomLevel(
                (float)(this.getBoredomLevel() +
                        ZomboidGlobals.BoredomIncreaseRate *
                            GameTime.instance.getMultiplier()));
          } else {
            this.setBoredomLevel(
                (float)(this.getBoredomLevel() +
                        ZomboidGlobals.BoredomIncreaseRate / 5.0 *
                            GameTime.instance.getMultiplier()));
          }

          if (this.getParentChar().IsSpeaking() &&
              !this.getParentChar().callOut) {
            this.setBoredomLevel(
                (float)(this.getBoredomLevel() -
                        ZomboidGlobals.BoredomDecreaseRate *
                            GameTime.instance.getMultiplier()));
          }
          ...
          if (this.getParentChar().getNumSurvivorsInVicinity() > 0) {
            this.setBoredomLevel(
                (float)(this.getBoredomLevel() -
                        ZomboidGlobals.BoredomDecreaseRate * 0.1F *
                            GameTime.instance.getMultiplier()));
          }
        } else if (this.getParentChar().getVehicle() != nullptr) {
          float float0 =
              this.getParentChar().getVehicle().getCurrentSpeedKmHour();
          if (Math.abs(float0) <= 0.1F) {
            ...
            this.setBoredomLevel(
                (float)(this.getBoredomLevel() -
                        ZomboidGlobals.BoredomDecreaseRate * 0.5 *
                            GameTime.instance.getMultiplier()));
          }
        } else {
          this.setBoredomLevel(
        ...
        if (this.getParentChar().getStats().Drunkenness > 20.0F) {
          this.setBoredomLevel(
              (float)(this.getBoredomLevel() -
                      ZomboidGlobals.BoredomDecreaseRate * 2.0 *
                          GameTime.instance.getMultiplier()));
        }
        ...
        if (this.getParentChar().getMoodles().getMoodleLevel(MoodleType.Bored) >
                1 &&
            !this.getParentChar().isReading()) {
          this.setUnhappynessLevel(
              (float)(this.getUnhappynessLevel() +
        ...
        if (this.getParentChar().getMoodles().getMoodleLevel(
          ...
          this.setUnhappynessLevel(
              (float)(this.getUnhappynessLevel() +
                      ZomboidGlobals.UnhappinessIncrease / 2.0 *
                          this.getParentChar().getMoodles().getMoodleLevel(
                              MoodleType.Stress) *
                          GameTime.instance.getMultiplier()));
        }
        ...
        if (this.getParentChar().Traits.Smoker.isSet()) {
          this.getParentChar().setTimeSinceLastSmoke(
              this.getParentChar().getTimeSinceLastSmoke() +
              1.0E-4F * GameTime.instance.getMultiplier());
          if (this.getParentChar().getTimeSinceLastSmoke() > 1.0F) {
            double double0 =
                Math.floor(this.getParentChar().getTimeSinceLastSmoke() /
                           10.0F) +
                1.0;
            if (double0 > 10.0) {
              double0 = 10.0;
            ...
            this.getParentChar().getStats().setStressFromCigarettes(
                (float)(this.getParentChar()
                            .getStats()
                            .getStressFromCigarettes() +
                        ZomboidGlobals.StressFromBiteOrScratch / 8.0 * double0 *
                            GameTime.instance.getMultiplier()));
          }
        }
      }
  ...
  float getUnhappynessLevel() { return this.UnhappynessLevel; }

  float getBoredomLevel() { return this.BoredomLevel; }

  void UpdateStrength() {
    if (this.getParentChar() == this.getParentChar()) {
      int int0 = 0;
      if (this.getParentChar().getMoodles().getMoodleLevel(MoodleType.Hungry) ==
          2) {
        int0++;
      }
      ...
      if (this.getParentChar().getMoodles().getMoodleLevel(MoodleType.Thirst) ==
          2) {
        int0++;
      }
      ...
      if (this.getParentChar().getMoodles().getMoodleLevel(MoodleType.Sick) ==
          2) {
        int0++;
      }
      ...
      if (this.getParentChar().getMoodles().getMoodleLevel(
              MoodleType.Bleeding) == 2) {
        int0++;
      }

      if (this.getParentChar().getMoodles().getMoodleLevel(
              MoodleType.Bleeding) == 3) {
        int0++;
      }

      if (this.getParentChar().getMoodles().getMoodleLevel(
              MoodleType.Bleeding) == 4) {
        int0++;
      }

      if (this.getParentChar().getMoodles().getMoodleLevel(
              MoodleType.Injured) == 2) {
        int0++;
      }
      ...
      if (this.getParentChar().getMoodles().getMoodleLevel(
              MoodleType.Injured) == 4) {
        int0 += 3;
      }

      this.getParentChar().setMaxWeight(
          (int)(this.getParentChar().getMaxWeightBase() *
                this.getParentChar().getWeightMod()) -
          int0);
      ...
      if (this.getParentChar().getMoodles().getMoodleLevel(
              MoodleType.FoodEaten) > 0) {
        this.getParentChar().setMaxWeight(this.getParentChar().getMaxWeight() +
                                          2);
      }

      if (this.getParentChar() instanceof IsoPlayer) {
        this.getParentChar().setMaxWeight(
            (int)(this.getParentChar().getMaxWeight() *
                  ((IsoPlayer)this.getParentChar()).getMaxWeightDelta()));
      }
    }
  }
  ...
  void Update() {
    if (!(this.getParentChar() instanceof IsoZombie)) {
      if (GameServer.bServer) {
        ...
        if (byte0 > 0) {
          float float0 =
              1.0F / byte0 * 200.0F * GameTime.instance.getInvMultiplier();
      ...
      } else if (GameClient.bClient && this.getParentChar() instanceof
                 IsoPlayer && ((IsoPlayer)this.getParentChar()).bRemote) {
        if (this.getParentChar().isAlive()) {
          this.RestoreToFullHealth();
          uint8_t byte1 = ((IsoPlayer)this.getParentChar()).bleedingLevel;
          if (byte1 > 0) {
            float float1 =
                1.0F / byte1 * 200.0F * GameTime.instance.getInvMultiplier();
        ...
        if (int1 > 0) {
          float float3 =
              1.0F / int1 * 200.0F * GameTime.instance.getInvMultiplier();
        ...
        if (this.getOverallBodyHealth() != 0.0F) {
          if (this.PoisonLevel == 0.0F && this.getFoodSicknessLevel() > 0.0F) {
            this.setFoodSicknessLevel(
                ...
                (float)(ZomboidGlobals.FoodSicknessDecrease *
                        GameTime.instance.getMultiplier()));
          }
          ...
          if (!this.isInfected()) {
            ...
            if (this.isInfected() && this.getInfectionTime() < 0.0F &&
          ...
          if (!this.isInfected() && !this.isIsFakeInfected()) {
            for (int int3 = 0; int3 < BodyPartType.ToIndex(BodyPartType.MAX);
                 int3++) {
          ...
          if (this.isIsFakeInfected() && !this.isReduceFakeInfection() &&
              this.getParentChar().getReduceInfectionPower() == 0.0F) {
            this.setFakeInfectionLevel(this.getFakeInfectionLevel() +
          ...
          stats.Drunkenness =
              stats.Drunkenness -
              this.getDrunkReductionValue() * GameTime.instance.getMultiplier();
          ...
          if (this.getHealthFromFoodTimer() > 0.0F) {
            float4 +=
                this.getHealthFromFood() * GameTime.instance.getMultiplier();
            this.setHealthFromFoodTimer(this.getHealthFromFoodTimer() -
                                        1.0F *
                                            GameTime.instance.getMultiplier());
          }
          ...
          switch (byte2) {
          case 0:
            float4 += this.getStandardHealthAddition() *
                      GameTime.instance.getMultiplier();
            break;
          case 1:
            float4 += this.getReducedHealthAddition() *
                      GameTime.instance.getMultiplier();
            break;
          case 2:
            float4 += this.getSeverlyReducedHealthAddition() *
                      GameTime.instance.getMultiplier();
            break;
          case 3:
            float4 += 0.0F;
          ...
          if (this.getParentChar().isAsleep()) {
            if (GameClient.bClient) {
              float4 += 15.0F *
                        GameTime.instance.getGameWorldSecondsSinceLastUpdate() /
                        3600.0F;
            } else {
              float4 += this.getSleepingHealthAddition() *
                        GameTime.instance.getMultiplier();
            }
          ...
          if (this.PoisonLevel > 0.0F) {
            if (this.PoisonLevel > 10.0F &&
                this.getParentChar().getMoodles().getMoodleLevel(
                    MoodleType.Sick) >= 1) {
              float5 = 0.0035F * Math.min(this.PoisonLevel / 10.0F, 3.0F) *
                       GameTime.instance.getMultiplier();
              float4 += float5;
            }
            ...
            if (this.getParentChar().getMoodles().getMoodleLevel(
                    MoodleType.FoodEaten) > 0) {
              float11 =
                  1.5E-4F * this.getParentChar().getMoodles().getMoodleLevel(
                                MoodleType.FoodEaten);
            }

            this.PoisonLevel =
                (float)(this.PoisonLevel -
                        (float11 + ZomboidGlobals.PoisonLevelDecrease *
                                       GameTime.instance.getMultiplier()));
            if (this.PoisonLevel < 0.0F) {
              this.PoisonLevel = 0.0F;
            }
            ...
            this.setFoodSicknessLevel(
                this.getFoodSicknessLevel() +
                this.getInfectionGrowthRate() *
                    (2 + Math.round(this.PoisonLevel / 10.0F)) *
                    GameTime.instance.getMultiplier());
          ...
          if (this.getParentChar().getMoodles().getMoodleLevel(
                  MoodleType.Hungry) == 4) {
            float6 = this.getHealthReductionFromSevereBadMoodles() / 50.0F *
                     GameTime.instance.getMultiplier();
            float4 += float6;
          }
          ...
          if (this.getParentChar().getMoodles().getMoodleLevel(
                  MoodleType.Sick) == 4 &&
              this.FoodSicknessLevel > this.InfectionLevel) {
            float7 = this.getHealthReductionFromSevereBadMoodles() *
                     GameTime.instance.getMultiplier();
            float4 += float7;
          }
          ...
          if (this.getParentChar().getMoodles().getMoodleLevel(
                  MoodleType.Bleeding) == 4) {
            float8 = this.getHealthReductionFromSevereBadMoodles() *
                     GameTime.instance.getMultiplier();
            float4 += float8;
          }
          ...
          if (this.getParentChar().getMoodles().getMoodleLevel(
                  MoodleType.Thirst) == 4) {
            float9 = this.getHealthReductionFromSevereBadMoodles() / 10.0F *
                     GameTime.instance.getMultiplier();
            float4 += float9;
          }

          if (this.getParentChar().getMoodles().getMoodleLevel(
                  MoodleType.HeavyLoad) > 2 &&
              this.getParentChar().getVehicle() == nullptr &&
              !this.getParentChar().isAsleep() &&
              !this.getParentChar().isSitOnGround() &&
              this.getThermoregulator().getMetabolicTarget() !=
                  Metabolics.SeatedResting.getMet() &&
              this.getHealth() > 75.0F &&
              Rand.Next(Rand.AdjustForFramerate(10)) == 0) {
            float10 = this.getHealthReductionFromSevereBadMoodles() /
                      ((5 - this.getParentChar().getMoodles().getMoodleLevel(
          ...
          if (float7 > 0.0F) {
            LuaEventManager.triggerEvent("OnPlayerGetDamage", character, "SICK",
                                         float7);
          }
          ...
          if (this.ParentChar.getPainEffect() > 0.0F) {
            ...
            stats.Pain =
                stats.Pain -
                0.023333333F * (GameTime.getInstance().getMultiplier() / 1.6F);
            this.ParentChar.setPainEffect(
                this.ParentChar.getPainEffect() -
                GameTime.getInstance().getMultiplier() / 1.6F);
          } else {
            this.ParentChar.setPainDelta(0.0F);
            float4 = 0.0F;

            for (int int4 = 0; int4 < BodyPartType.ToIndex(BodyPartType.MAX);
                 int4++) {
              float4 += this.getBodyParts().get(int4).getPain() *
                        BodyPartType.getPainModifyer(int4);
            }
            ...
            if (float4 > this.ParentChar.getStats().Pain) {
              ...
              stats.Pain = stats.Pain +
                           (float4 - this.ParentChar.getStats().Pain) / 500.0F;
            } else {
              this.ParentChar.getStats().Pain = float4;
            }
          }

          this.setPainReduction(this.getPainReduction() -
                                0.005F *
          ...
          if (this.isInfected()) {
            ...
            if (int5 == 1) {
              ...
              float6 = (float12 - this.InfectionTime) /
                       this.InfectionMortalityDuration;
              float6 = Math.min(float6, 1.0F);
              this.setInfectionLevel(float6 * 100.0F);
              if (float6 == 1.0F) {
                this.ReduceGeneralHealth(110.0F);
                LuaEventManager.triggerEvent("OnPlayerGetDamage",
                                             this.ParentChar, "INFECTION", 110);
              } else {
                float6 *= float6;
                float6 *= float6;
          ...
          for (int int6 = 0; int6 < BodyPartType.ToIndex(BodyPartType.MAX);
               int6++) {
          ...
          this.calculateOverallHealth();
          if (this.getOverallBodyHealth() <= 0.0F) {
            if (GameClient.bClient && this.getParentChar() instanceof
                IsoPlayer && !((IsoPlayer)this.getParentChar()).bRemote) {
            ...
            if (this.isIsOnFire()) {
              ...
              for (int int7 = 0; int7 < BodyPartType.ToIndex(BodyPartType.MAX);
                   int7++) {
          ...
          if (this.isReduceFakeInfection() &&
              this.getOverallBodyHealth() > 0.0F) {
            this.setFakeInfectionLevel(this.getFakeInfectionLevel() -
                                       this.getInfectionGrowthRate() *
                                           GameTime.instance.getMultiplier() *
                                           2.0F);
          ...
          if (this.getParentChar().getReduceInfectionPower() > 0.0F &&
            ...
            this.setFakeInfectionLevel(this.getFakeInfectionLevel() -
                                       this.getInfectionGrowthRate() *
                                           GameTime.instance.getMultiplier());
            this.getParentChar().setReduceInfectionPower(
                this.getParentChar().getReduceInfectionPower() -
                this.getInfectionGrowthRate() *
                    GameTime.instance.getMultiplier());
            if (this.getParentChar().getReduceInfectionPower() < 0.0F) {
          ...
          if (this.getFakeInfectionLevel() <= 0.0F) {
            for (int int8 = 0; int8 < BodyPartType.ToIndex(BodyPartType.MAX);
                 int8++) {
          ...
          if (float2 == this.ParentChar.getStats().Pain) {
            ...
            stats.Pain =
                (float)(stats.Pain -
                        0.25 * (GameTime.getInstance().getMultiplier() / 1.6F));
          }
  ...
  void calculateOverallHealth() {
    ...
    for (int int0 = 0; int0 < BodyPartType.ToIndex(BodyPartType.MAX); int0++) {
      BodyPart bodyPart = this.getBodyParts().get(int0);
      float0 += (100.0F - bodyPart.getHealth()) *
                BodyPartType.getDamageModifyer(int0);
    }
  ...
  void UpdateIllness() {
    if (SandboxOptions.instance.DecayingCorpseHealthImpact.getValue() != 1) {
      ...
      if (float0 > 0.0F) {
        this.setFoodSicknessLevel(this.getFoodSicknessLevel() +
                                  float0 *
                                      GameTime.getInstance().getMultiplier());
      }
    }
   ...
   */
public
  ArrayList<BodyPart> getBodyParts() { return this.BodyParts; }
   ...
   */
  float getInfectionGrowthRate() { return this.InfectionGrowthRate; }
  ...
  void setInfected(bool infected) { this.IsInfected = infected; }

  float getInfectionTime() { return this.InfectionTime; }

  void setInfectionTime(float worldHours) { this.InfectionTime = worldHours; }
  ...
  float getCurrentTimeForInfection() {
    return this.getParentChar() instanceof
        IsoPlayer ? (float)((IsoPlayer)this.getParentChar()).getHoursSurvived()
   ...
   * @return the inf
   */
  bool isInf() { return this.IsInfected; }

  /**
   *
   * @param inf the inf to set
   */
  void setInf(bool inf) { this.IsInfected = inf; }

  /**
   * @return the FakeInfectionLevel
   */
  float getFakeInfectionLevel() { return this.FakeInfectionLevel; }
   ...
   */
  bool isIsFakeInfected() { return this.IsFakeInfected; }
   ...
   */
  float getOverallBodyHealth() { return this.OverallBodyHealth; }
   ...
   */
  float getStandardHealthAddition() { return this.StandardHealthAddition; }
   ...
   */
  float getReducedHealthAddition() { return this.ReducedHealthAddition; }
  ...
  /**
   *
   * @param _SeverlyReducedHealthAddition the SeverlyReducedHealthAddition to
   * set
   */
   ...
   */
  float getSleepingHealthAddition() { return this.SleepingHealthAddition; }
   ...
   */
  float getHealthFromFood() { return this.HealthFromFood; }
  ...
  /**
   *
   * @param _HealthReductionFromSevereBadMoodles the
   * HealthReductionFromSevereBadMoodles to set
   */
  void setHealthReductionFromSevereBadMoodles(
      float _HealthReductionFromSevereBadMoodles) {
    this.HealthReductionFromSevereBadMoodles =
        _HealthReductionFromSevereBadMoodles;
  }
   ...
   */
  float getHealthFromFoodTimer() { return this.HealthFromFoodTimer; }
   ...
   */
  float getInitialThumpPain() { return this.InitialThumpPain; }
   ...
   */
  float getInitialScratchPain() { return this.InitialScratchPain; }
   ...
   */
  float getInitialBitePain() { return this.InitialBitePain; }
   ...
   */
  float getInitialWoundPain() { return this.InitialWoundPain; }
   ...
   */
  float getContinualPainIncrease() { return this.ContinualPainIncrease; }
   ...
   */
  float getPainReductionFromMeds() { return this.PainReductionFromMeds; }
  ...
  /**
   *
   * @param _StandardPainReductionWhenWell the StandardPainReductionWhenWell to
   * set
   */
   ...
   */
  int getOldNumZombiesVisible() { return this.OldNumZombiesVisible; }
   ...
   */
  int getCurrentNumZombiesVisible() { return this.CurrentNumZombiesVisible; }
   ...
   */
  float getPanicIncreaseValue() { return this.PanicIncreaseValue; }

  float getPanicIncreaseValueFrame() { return this.PanicIncreaseValueFrame; }
   ...
   */
  float getPanicReductionValue() { return this.PanicReductionValue; }
   ...
   */
  float getDrunkIncreaseValue() { return this.DrunkIncreaseValue; }
   ...
   */
  float getDrunkReductionValue() { return this.DrunkReductionValue; }
   ...
   * @return the IsOnFire
   */
  bool isIsOnFire() { return this.IsOnFire; }

  /**
   *
   * @param _IsOnFire the IsOnFire to set
   */
  void setIsOnFire(bool _IsOnFire) { this.IsOnFire = _IsOnFire; }

  /**
   * @return the BurntToDeath
   */
  bool isBurntToDeath() { return this.BurntToDeath; }
   ...
   */
  float getCatchACold() { return this.CatchACold; }

  /**
   *
   * @param _CatchACold the CatchACold to set
   */
  void setCatchACold(float _CatchACold) { this.CatchACold = _CatchACold; }

  /**
   * @return the HasACold
   */
  bool isHasACold() { return this.HasACold; }

  /**
   *
   * @param _HasACold the HasACold to set
   */
  void setHasACold(bool _HasACold) { this.HasACold = _HasACold; }
   ...
   */
  float getColdProgressionRate() { return this.ColdProgressionRate; }
   ...
   */
  int getTimeToSneezeOrCough() { return this.TimeToSneezeOrCough; }
   ...
   */
  int getMildColdSneezeTimerMin() { return this.MildColdSneezeTimerMin; }
   ...
   */
  int getMildColdSneezeTimerMax() { return this.MildColdSneezeTimerMax; }
   ...
   */
  int getColdSneezeTimerMin() { return this.ColdSneezeTimerMin; }
   ...
   */
  int getColdSneezeTimerMax() { return this.ColdSneezeTimerMax; }
   ...
   */
  int getNastyColdSneezeTimerMin() { return this.NastyColdSneezeTimerMin; }
   ...
   */
  int getNastyColdSneezeTimerMax() { return this.NastyColdSneezeTimerMax; }
   ...
   */
  int getSneezeCoughActive() { return this.SneezeCoughActive; }
   ...
   */
  int getSneezeCoughTime() { return this.SneezeCoughTime; }
   ...
   */
  int getSneezeCoughDelay() { return this.SneezeCoughDelay; }
   ...
   */
  IsoGameCharacter getParentChar() { return this.ParentChar; }
  ...
  void setPoisonLevel(float poisonLevel) { this.PoisonLevel = poisonLevel; }

  float getPoisonLevel() { return this.PoisonLevel; }

  float getFoodSicknessLevel() { return this.FoodSicknessLevel; }

  void setFoodSicknessLevel(float foodSicknessLevel) {
    this.FoodSicknessLevel = Math.max(foodSicknessLevel, 0.0F);
  }

  bool isReduceFakeInfection() { return this.reduceFakeInfection; }
  ...
  void AddRandomDamage() {
    BodyPart bodyPart =
        this.getBodyParts().get(Rand.Next(this.getBodyParts().size()));
    switch (Rand.Next(4)) {
    case 0:
      bodyPart.generateDeepWound();
      if (Rand.Next(4) == 0) {
  ...
  float getColdReduction() { return this.coldReduction; }

  void setColdReduction(float _coldReduction) {
    this.coldReduction = _coldReduction;
  }

  int getRemotePainLevel() { return this.RemotePainLevel; }

  void setRemotePainLevel(int painLevel) { this.RemotePainLevel = painLevel; }

  float getColdDamageStage() { return this.ColdDamageStage; }

  void setColdDamageStage(float coldDamageStage) {
    this.ColdDamageStage = coldDamageStage;
  }

  Thermoregulator getThermoregulator() { return this.thermoregulator; }
