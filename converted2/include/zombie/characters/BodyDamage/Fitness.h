...
class Fitness {
public:
  IsoGameCharacter parent = null;
private
  HashMap<String, Float> regularityMap = std::make_unique<HashMap<>>();
  int fitnessLvl = 0;
  int strLvl = 0;
private
  final HashMap<String, Integer> stiffnessTimerMap =
      std::make_unique<HashMap<>>();
private
  final HashMap<String, Float> stiffnessIncMap = std::make_unique<HashMap<>>();
private
  final ArrayList<String> bodypartToIncStiffness =
      std::make_unique<ArrayList<>>();
private
  final HashMap<String, Fitness.FitnessExercise> exercises =
      std::make_unique<HashMap<>>();
private
  final HashMap<String, Long> exeTimer = std::make_unique<HashMap<>>();
  int lastUpdate = -1;
private
  Fitness.FitnessExercise currentExe;
...
public
  Fitness(IsoGameCharacter _parent) { this.setParent(_parent); }
  ...
  void decreaseRegularity() {
    for (String string : this.regularityMap.keySet()) {
      if (this.exeTimer.containsKey(string) &&
          GameTime.getInstance().getCalender().getTimeInMillis() -
                  this.exeTimer.get(string) >
              86400000L) {
  ...
  void increasePain(const std::string &string) {
    if ("arms" == string)) {
        for (int int0 = BodyPartType.ForeArm_L.index();
          ...
          BodyPart bodyPart0 = this.parent.getBodyDamage().getBodyPart(
              BodyPartType.FromIndex(int0));
          bodyPart0.setStiffness(bodyPart0.getStiffness() + 2.5F);
        }
      }

    if ("legs" == string)) {
        for (int int1 = BodyPartType.UpperLeg_L.index();
             int1 < BodyPartType.LowerLeg_R.index() + 1; int1++) {
          BodyPart bodyPart1 = this.parent.getBodyDamage().getBodyPart(
              BodyPartType.FromIndex(int1));
    ...
    if ("chest" == string)) {
        BodyPart bodyPart2 =
            this.parent.getBodyDamage().getBodyPart(BodyPartType.Torso_Upper);
        bodyPart2.setStiffness(bodyPart2.getStiffness() + 2.5F);
      }

    if ("abs" == string)) {
        BodyPart bodyPart3 =
            this.parent.getBodyDamage().getBodyPart(BodyPartType.Torso_Lower);
  ...
  void updateExeTimer() {
    this.exeTimer.put(this.currentExe.type,
                      GameTime.getInstance().getCalender().getTimeInMillis());
  }

  /**
   * Increase the regularity when you've done a repeat of an exercice  Depend on
   * fitness (using logarithm), the more fitness, the LESS regularity you get
   * Regularity will influence on the stiffness you get once you've finished an
   * exercise
   */
  void incRegularity() {
    float float0 = 0.08F;
  ...
  /**
   * Reduce endurance, using metabolics (to know what kind of exercise it is,
   * some are more exhausting than others), regularity, current carrying weight.
   */
  void reduceEndurance() {
    ...
    float0 *=
        1 + this.parent.getMoodles().getMoodleLevel(MoodleType.HeavyLoad) / 3;
    this.parent.getStats().setEndurance(this.parent.getStats().getEndurance() -
                                        float0);
  }

  /**
   * We setup a timer after finishing an exercice, 12h after, we gonna start to
   * increase stiffness (add pains in muscles)  When adding the stiffness, we
   * decrease slowly our vars while increasing pain, untill no more stiffness is
   * to be added.  Stiffness induced will depend on regularity, fatigue. Numbers
   * approx: At 0 regularity, 60min exercises should gives almost 4h of
   * stiffness (gets additional pain)
   */
  void incFutureStiffness() {
    ...
    for (int int0 = 0; int0 < this.currentExe.stiffnessInc.size(); int0++) {
      ...
      if (!this.stiffnessTimerMap.containsKey(string) &&
          !this.bodypartToIncStiffness.contains(string)) {
      ...
      float1 *=
          1 + this.parent.getMoodles().getMoodleLevel(MoodleType.Tired) / 3;
    ...
    if (WorldVersion >= 167) {
      ...
      if (int0 > 0) {
        for (int int1 = 0; int1 < int0; int1++) {
          this.stiffnessIncMap.put(GameWindow.ReadString(input),
                                   input.getFloat());
        }
      }
      ...
      if (int0 > 0) {
        for (int int2 = 0; int2 < int0; int2++) {
          this.stiffnessTimerMap.put(GameWindow.ReadString(input),
                                     input.getInt());
        }
      }
      ...
      if (int0 > 0) {
        for (int int3 = 0; int3 < int0; int3++) {
          this.regularityMap.put(GameWindow.ReadString(input),
                                 input.getFloat());
        }
      }
  ...
  bool onGoingStiffness() { return !this.bodypartToIncStiffness.isEmpty(); }
  ...
  IsoGameCharacter getParent() { return this.parent; }
...
public
  HashMap<String, Float> getRegularityMap() { return this.regularityMap; }
  ...
  void init() {
    if (this.exercises.isEmpty()) {
      KahluaTableImpl kahluaTableImpl0 =
          (KahluaTableImpl)LuaManager.env.rawget("FitnessExercises");
      KahluaTableImpl kahluaTableImpl1 =
          (KahluaTableImpl)kahluaTableImpl0.rawget("exercisesType");

      for (Entry entry : kahluaTableImpl1.delegate.entrySet()) {
        this.exercises.put(
            (String)entry.getKey(),
            new Fitness.FitnessExercise((KahluaTableImpl)entry.getValue()));
      }
...
public
  static final class FitnessExercise {
  ...
  public
    FitnessExercise(KahluaTableImpl exeDatas) {
      this.type = exeDatas.rawgetStr("type");
      this.metabolics = (Metabolics)exeDatas.rawget("metabolics");
      this.stiffnessInc = new ArrayList<>(
          Arrays.asList(exeDatas.rawgetStr("stiffness").split(",")));
