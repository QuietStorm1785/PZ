...
class ModUtilsJava {
public:
  static std::string getRandomUUID() { return UUID.randomUUID().toString(); }
  ...
  static bool sendItemListNet(IsoPlayer player0,
    ...
    if (arrayList != nullptr) {
      ...
      if (GameClient.bClient) {
        ...
        return GameClient.sendItemListNet(player0, arrayList, player1, string0,
                                          string1);
      }

      if (GameServer.bServer) {
        return GameServer.sendItemListNet(nullptr, player0, arrayList, player1,
                                          string0, string1);
      }
    }
