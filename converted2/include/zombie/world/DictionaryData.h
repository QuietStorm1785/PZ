...
public:
protected
  final Map<Short, ItemInfo> itemIdToInfoMap = std::make_unique<HashMap<>>();
protected
  final Map<String, ItemInfo> itemTypeToInfoMap = std::make_unique<HashMap<>>();
protected
  final Map<String, Integer> spriteNameToIdMap = std::make_unique<HashMap<>>();
protected
  final Map<Integer, String> spriteIdToNameMap = std::make_unique<HashMap<>>();
protected
  final Map<String, Byte> objectNameToIdMap = std::make_unique<HashMap<>>();
protected
  final Map<Byte, String> objectIdToNameMap = std::make_unique<HashMap<>>();
protected
  final ArrayList<String> unsetObject = std::make_unique<ArrayList<>>();
protected
  final ArrayList<String> unsetSprites = std::make_unique<ArrayList<>>();
  short NextItemID = 0;
    ...
    if (!Core.getInstance().isNoSave()) {
      ...
      if (!file.exists()) {
        ...
        try {
          ...
          exception.printStackTrace();
          throw new WorldDictionaryException("Error loading WorldDictionary.",
                                             exception);
        }
      }
    }
  ...
  void save() {
    if (!Core.getInstance().isNoSave()) {
      try {
        ...
        exception.printStackTrace();
        throw new WorldDictionaryException("Error saving WorldDictionary.",
                                           exception);
      }
    }
  }
