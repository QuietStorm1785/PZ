...
public:
  ...
  bool is3D = true;
public
  final ArrayList<GameSoundClip> clips = std::make_unique<ArrayList<>>();
  float userVolume = 1.0F;
public
  GameSound.MasterVolume master = GameSound.MasterVolume.Primary;
  ...
  bool isLooped() { return this.loop; }
  ...
  float getUserVolume() {
    return !SystemDisabler.getEnableAdvancedSoundOptions() ? 1.0F
                                                           : this.userVolume;
  }
...
public
  static enum MasterVolume { Primary, Ambient, Music, VehicleEngine; }
}
} // namespace audio
} // namespace zombie
