...
public:
private
  ParameterVehicleHitLocation.HitLocation location =
      ParameterVehicleHitLocation.HitLocation.Front;

public
  ParameterVehicleHitLocation() { super("VehicleHitLocation"); }

  float calculateCurrentValue() { return this.location.label; }

public
  static ParameterVehicleHitLocation.HitLocation
  calculateLocation(BaseVehicle vehicle, float x, float y, float z) {
    ...
    if (vehicleScript == nullptr) {
      ...
      Vector3f vector3f0 = vehicle.getLocalPos(
          x, y, z, BaseVehicle.TL_vector3f_pool.get().alloc());
...
public
  static enum HitLocation {
    ...
    private HitLocation(int int1){this.label = int1;}
}
} // namespace parameters
} // namespace audio
} // namespace zombie
