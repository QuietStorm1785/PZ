...
public
  ...
  float calculateCurrentValue() {
    ...
    if (character == nullptr) {
      ...
      uint8_t byte0 = 0;
      this.m_zones.clear();
      IsoWorld.instance.MetaGrid.getZonesIntersecting(
          (int)character.x - 40, (int)character.y - 40, byte0, 80, 80,
          this.m_zones);
      ...
      for (int int0 = 0; int0 < this.m_zones.size(); int0++) {
        ...
        if (this.m_zoneName.equalsIgnoreCase(zone.getType())) {
          ...
          float float3 = PZMath.max(
              PZMath.abs(character.x - float1) - zone.w / 2.0F, 0.0F);
          float float4 = PZMath.max(
              PZMath.abs(character.y - float2) - zone.h / 2.0F, 0.0F);
