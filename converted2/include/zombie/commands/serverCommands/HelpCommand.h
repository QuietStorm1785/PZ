...
class HelpCommand : public CommandBase {
    public:
    public
      HelpCommand(const std::string &string0, const std::string &string1,
      ...
      std::string Command() {
        ...
        if (string0 != nullptr) {
          ...
          return clazz0 != nullptr ? getHelp(clazz0)
                                   : "Unknown command /" + string0;
        } else {
          ...
          if (!GameServer.bServer) {
            for (String string2 : ServerOptions.getClientCommandList(
