...
class GodModeCommand : public CommandBase {
        public:
        public
          GodModeCommand(const std::string &string0, const std::string &string1,
          ...
          std::string Command() {
            ...
            if (this.getCommandArgsCount() == 2 ||
                ...
                if (this.connection != nullptr &&
                    this.connection.accessLevel == 2 && !string1 == string1)) {
            ...
            if (player != nullptr) {
              ...
              if (player.isGodMod()) {
                LoggerManager.getLogger("admin").write(
                    this.getExecutorUsername() + " enabled godmode on " +
                    string0);
                return "User " + string0 + " is now invincible.";
              } else {
                LoggerManager.getLogger("admin").write(
                    this.getExecutorUsername() + " disabled godmode on " +
                    string0);
