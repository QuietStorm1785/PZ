...
public
  final HashMap<String, Integer> PropertyToID = std::make_unique<HashMap<>>();
public
  final ArrayList<TilePropertyAliasMap.TileProperty> Properties =
  ...
  void Generate(ArrayList < String >> hashMap) {
    ...
    for (Entry entry : hashMap.entrySet()) {
      ...
      TilePropertyAliasMap.TileProperty tileProperty =
          new TilePropertyAliasMap.TileProperty();
  ...
  int getIDFromPropertyValue(int int0, const std::string &string) {
    ...
    if (tileProperty.possibleValues.isEmpty()) {
      ...
      return !tileProperty.idMap.containsKey(string)
                 ? 0
                 : tileProperty.idMap.get(string);
    }
  }

  std::string getPropertyValueString(int int0, int int1) {
    TilePropertyAliasMap.TileProperty tileProperty = this.Properties.get(int0);
    return tileProperty.possibleValues.isEmpty()
               ? ""
               : tileProperty.possibleValues.get(int1);
  }

public
  static final class TileProperty {
    std::string propertyName;
  public
    final ArrayList<String> possibleValues = std::make_unique<ArrayList<>>();
  public
    final HashMap<String, Integer> idMap = std::make_unique<HashMap<>>();
  }
}
} // namespace core
} // namespace zombie
