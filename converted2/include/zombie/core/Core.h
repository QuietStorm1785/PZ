...
public:
  ...
  const ColorInfo objectHighlitedColor =
      new ColorInfo(0.98F, 0.56F, 0.11F, 1.0F);
  ...
  static std::string OptionLanguageName = null;
private
  static final boolean[] OptionSingleContextMenu = new boolean[4];
  ...
  std::string blinkingMoodle = null;
  bool tutorialDone = false;
  bool vehiclesWarningShow = false;
  std::string poisonousBerry = null;
  std::string poisonousMushroom = null;
...
private
  static final HashMap<String, Object> optionsOnStartup =
  ...
  static UITextBox2 CurrentTextEntryBox = null;
  Shader RenderShader;
private
  Map<String, Integer> keyMaps = nullptr;
...
private
  static final boolean[] bAutoZoom = new boolean[4];
  ...
  static int zz = 0;
public
  final HashMap<Integer, Float> FloatParamMap = std::make_unique<HashMap<>>();
  ...
  static std::string ChallengeID = null;
  static bool bExiting = false;
  std::string m_delayResetLua_activeMods = null;
  std::string m_delayResetLua_reason = null;

  bool isMultiThread() { return true; }

  void setChallenge(bool _bChallenge) { this.bChallenge = _bChallenge; }

  bool isChallenge() { return this.bChallenge; }

  std::string getChallengeID() { return ChallengeID; }
  ...
  bool loadedShader() { return this.RenderShader != nullptr; }
  ...
  bool getUseShaders() { return true; }

  int getPerfSkybox() { return this.iPerfSkybox_new; }

  int getPerfSkyboxOnLoad() { return this.iPerfSkybox; }

  void setPerfSkybox(int val) { this.iPerfSkybox_new = val; }

  bool getPerfReflections() { return this.bPerfReflections_new; }

  bool getPerfReflectionsOnLoad() { return this.bPerfReflections; }

  void setPerfReflections(bool val) { this.bPerfReflections_new = val; }

  int getPerfPuddles() { return this.iPerfPuddles_new; }

  int getPerfPuddlesOnLoad() { return this.iPerfPuddles; }

  void setPerfPuddles(int val) { this.iPerfPuddles_new = val; }

  int getVidMem() { return SafeMode ? 5 : this.vidMem; }
  ...
  void initShaders() {
    try {
      if (this.RenderShader == nullptr && !SafeMode && !SafeModeForced) {
        RenderThread.invokeOnRenderContext(()->this.RenderShader =
                                               new WeatherShader("screen"));
      }
  ...
  static Core getInstance() { return core; }
  ...
  static void setFullScreen(bool bool) { fullScreen = bool; }

public
  static int[] flipPixels(int[] ints1, int int0, int int1) {
    int[] ints0 = nullptr;
    if (ints1 != nullptr) {
  ...
  void TakeScreenshot() { this.TakeScreenshot(256, 256, 1028); }
  ...
  void TakeScreenshot(int x, int y, int _width, int _height, int readBuffer) {
    ...
    for (int int0 = 0; int0 < ints.length; int0++) {
      int int1 = int0 * 3;
      ints[int0] = 0xFF000000 | (byteBuffer.get(int1) & 255) << 16 |
                   (byteBuffer.get(int1 + 1) & 255) << 8 |
                   (byteBuffer.get(int1 + 2) & 255) << 0;
    }
  ...
  void TakeFullScreenshot(const std::string &filename) {
    RenderThread.invokeOnRenderContext(
        filename, string->{
          ...
          File file = new File(ZomboidFileSystem.instance.getScreenshotDir() +
                               File.separator + string);

          for (int int2 = 0; int2 < ints.length; int2++) {
            int int3 = int2 * 3;
            ints[int2] = 0xFF000000 | (byteBuffer.get(int3) & 255) << 16 |
  ...
  static bool supportNPTTexture() { return false; }
  ...
  void sharedInit() { this.supportsFBO(); }

  void MoveMethodToggle() { bAltMoveMethod = !bAltMoveMethod; }
  ...
  void EndFrameUI() {
    ...
    if (LuaManager.thread != nullptr && LuaManager.thread.bStep) {
      ...
      if (UIManager.useUIFBO) {
        ...
        SpriteRenderer.instance.renderi((Texture)UIManager.UIFBO.getTexture(),
                                        0, height, width, -height, 1.0F, 1.0F,
                                        1.0F, 1.0F, nullptr);
  ...
  static void UnfocusActiveTextEntryBox() {
    if (CurrentTextEntryBox != nullptr &&
        !CurrentTextEntryBox.getUIName().contains("chat text entry")) {
      CurrentTextEntryBox.DoingTextEntry = false;
      if (CurrentTextEntryBox.Frame != nullptr) {
        CurrentTextEntryBox.Frame.Colour =
            CurrentTextEntryBox.StandardFrameColour;
      }
  ...
  int getOffscreenWidth(int playerIndex) {
    if (this.OffscreenBuffer == nullptr) {
      return IsoPlayer.numPlayers > 1 ? this.getScreenWidth() / 2
                                      : this.getScreenWidth();
    } else {
  ...
  int getOffscreenHeight(int playerIndex) {
    if (this.OffscreenBuffer == nullptr) {
      return IsoPlayer.numPlayers > 2 ? this.getScreenHeight() / 2
  ...
  int getOffscreenTrueWidth() {
    return this.OffscreenBuffer != nullptr &&
                   this.OffscreenBuffer.Current != nullptr
               ? this.OffscreenBuffer.getTexture(0).getWidth()
               : this.getScreenWidth();
  }
  ...
  int getOffscreenTrueHeight() {
    return this.OffscreenBuffer != nullptr &&
                   this.OffscreenBuffer.Current != nullptr
               ? this.OffscreenBuffer.getTexture(0).getHeight()
               : this.getScreenHeight();
  }
    ...
    if (!file0.exists()) {
      ...
      if (!GameServer.bServer) {
        try {
          ...
          int[] ints2 = new int[1];
          int[] ints3 = new int[1];
          GLFW.glfwGetMonitorWorkarea(GLFW.glfwGetPrimaryMonitor(), ints0,
                                      ints1, ints2, ints3);

          for (int int2 = 0; int2 < displayModes.length; int2++) {
            if (displayModes[int2].getWidth() > int0 &&
                displayModes[int2].getWidth() < ints2[0] &&
                displayModes[int2].getHeight() < ints3[0]) {
      ...
      try {
        ...
        while ((string0 = bufferedReader.readLine()) != nullptr) {
          if (string0.startsWith("version=")) {
            ...
            fullScreen =
                Boolean.parseBoolean(string0.replaceFirst("fullScreen=", ""));
          } else if (string0.startsWith("frameRate=")) {
            PerformanceSettings.setLockFPS(
                Integer.parseInt(string0.replaceFirst("frameRate=", "")));
          } else if (string0.startsWith("uncappedFPS=")) {
            PerformanceSettings.setUncappedFPS(
                Boolean.parseBoolean(string0.replaceFirst("uncappedFPS=", "")));
          } else if (string0.startsWith("iso_cursor=")) {
            getInstance().setIsoCursorVisibility(
                Integer.parseInt(string0.replaceFirst("iso_cursor=", "")));
          } else if (string0.startsWith("showCursorWhileAiming=")) {
            OptionShowCursorWhileAiming = Boolean.parseBoolean(
                string0.replaceFirst("showCursorWhileAiming=", ""));
          } else if (string0.startsWith("water=")) {
            PerformanceSettings.WaterQuality =
                Integer.parseInt(string0.replaceFirst("water=", ""));
          } else if (string0.startsWith("puddles=")) {
            PerformanceSettings.PuddlesQuality =
                Integer.parseInt(string0.replaceFirst("puddles=", ""));
          } else if (string0.startsWith("lighting=")) {
            PerformanceSettings.LightingFrameSkip =
                Integer.parseInt(string0.replaceFirst("lighting=", ""));
          } else if (string0.startsWith("lightFPS=")) {
            PerformanceSettings.instance.setLightingFPS(
                Integer.parseInt(string0.replaceFirst("lightFPS=", "")));
          } else if (string0.startsWith("perfSkybox=")) {
            this.iPerfSkybox =
                Integer.parseInt(string0.replaceFirst("perfSkybox=", ""));
            this.iPerfSkybox_new = this.iPerfSkybox;
          } else if (string0.startsWith("perfPuddles=")) {
            this.iPerfPuddles =
                Integer.parseInt(string0.replaceFirst("perfPuddles=", ""));
            this.iPerfPuddles_new = this.iPerfPuddles;
          } else if (string0.startsWith("bPerfReflections=")) {
            this.bPerfReflections = Boolean.parseBoolean(
                string0.replaceFirst("bPerfReflections=", ""));
            ...
            OptionZoom =
                Boolean.parseBoolean(string0.replaceFirst("zoom=", ""));
          } else if (string0.startsWith("autozoom=")) {
            String[] strings0 =
                string0.replaceFirst("autozoom=", "").split(",");
            ...
            this.setOptionFontSize(
                Integer.parseInt(string0.replaceFirst("fontSize=", "").trim()));
          } else if (string0.startsWith("contextMenuFont=")) {
            OptionContextMenuFont =
                string0.replaceFirst("contextMenuFont=", "").trim();
          } else if (string0.startsWith("inventoryFont=")) {
            OptionInventoryFont =
                string0.replaceFirst("inventoryFont=", "").trim();
          } else if (string0.startsWith("inventoryContainerSize=")) {
            OptionInventoryContainerSize = PZMath.tryParseInt(
                string0.replaceFirst("inventoryContainerSize=", ""), 1);
            OptionInventoryContainerSize =
                PZMath.clamp(OptionInventoryContainerSize, 1, 3);
            ...
            OptionMeasurementFormat =
                string0.replaceFirst("measurementsFormat=", "").trim();
          } else if (string0.startsWith("clockFormat=")) {
            OptionClockFormat =
                Integer.parseInt(string0.replaceFirst("clockFormat=", ""));
          } else if (string0.startsWith("clockSize=")) {
            OptionClockSize =
                Integer.parseInt(string0.replaceFirst("clockSize=", ""));
          } else if (string0.startsWith("clock24Hour=")) {
            OptionClock24Hour =
                Boolean.parseBoolean(string0.replaceFirst("clock24Hour=", ""));
          } else if (string0.startsWith("vsync=")) {
            OptionVSync =
                Boolean.parseBoolean(string0.replaceFirst("vsync=", ""));
          } else if (string0.startsWith("voiceEnable=")) {
            OptionVoiceEnable =
                Boolean.parseBoolean(string0.replaceFirst("voiceEnable=", ""));
          } else if (string0.startsWith("voiceMode=")) {
            OptionVoiceMode =
                Integer.parseInt(string0.replaceFirst("voiceMode=", ""));
          } else if (string0.startsWith("voiceVADMode=")) {
            OptionVoiceVADMode =
                Integer.parseInt(string0.replaceFirst("voiceVADMode=", ""));
          } else if (string0.startsWith("voiceAGCMode=")) {
            OptionVoiceAGCMode =
                Integer.parseInt(string0.replaceFirst("voiceAGCMode=", ""));
          } else if (string0.startsWith("voiceVolumeMic=")) {
            OptionVoiceVolumeMic =
                Integer.parseInt(string0.replaceFirst("voiceVolumeMic=", ""));
          } else if (string0.startsWith("voiceVolumePlayers=")) {
            OptionVoiceVolumePlayers = Integer.parseInt(
                string0.replaceFirst("voiceVolumePlayers=", ""));
          } else if (string0.startsWith("voiceRecordDeviceName=")) {
            OptionVoiceRecordDeviceName =
                string0.replaceFirst("voiceRecordDeviceName=", "");
          } else if (string0.startsWith("soundVolume=")) {
            OptionSoundVolume =
                Integer.parseInt(string0.replaceFirst("soundVolume=", ""));
          } else if (string0.startsWith("musicVolume=")) {
            OptionMusicVolume =
                Integer.parseInt(string0.replaceFirst("musicVolume=", ""));
          } else if (string0.startsWith("ambientVolume=")) {
            OptionAmbientVolume =
                Integer.parseInt(string0.replaceFirst("ambientVolume=", ""));
          } else if (string0.startsWith("jumpScareVolume=")) {
            OptionJumpScareVolume =
                Integer.parseInt(string0.replaceFirst("jumpScareVolume=", ""));
          } else if (string0.startsWith("musicActionStyle=")) {
            OptionMusicActionStyle =
                Integer.parseInt(string0.replaceFirst("musicActionStyle=", ""));
          } else if (string0.startsWith("musicLibrary=")) {
            OptionMusicLibrary =
                Integer.parseInt(string0.replaceFirst("musicLibrary=", ""));
          } else if (string0.startsWith("vehicleEngineVolume=")) {
            OptionVehicleEngineVolume = Integer.parseInt(
                string0.replaceFirst("vehicleEngineVolume=", ""));
          } else if (string0.startsWith("reloadDifficulty=")) {
            OptionReloadDifficulty =
                Integer.parseInt(string0.replaceFirst("reloadDifficulty=", ""));
          } else if (string0.startsWith("rackProgress=")) {
            OptionRackProgress =
                Boolean.parseBoolean(string0.replaceFirst("rackProgress=", ""));
            ...
            this.tutorialDone =
                Boolean.parseBoolean(string0.replaceFirst("tutorialDone=", ""));
          } else if (string0.startsWith("vehiclesWarningShow=")) {
            this.vehiclesWarningShow = Boolean.parseBoolean(
                string0.replaceFirst("vehiclesWarningShow=", ""));
          } else if (string0.startsWith("bloodDecals=")) {
            this.setOptionBloodDecals(
                Integer.parseInt(string0.replaceFirst("bloodDecals=", "")));
          } else if (string0.startsWith("borderless=")) {
            OptionBorderlessWindow =
                Boolean.parseBoolean(string0.replaceFirst("borderless=", ""));
          } else if (string0.startsWith("lockCursorToWindow=")) {
            OptionLockCursorToWindow = Boolean.parseBoolean(
                string0.replaceFirst("lockCursorToWindow=", ""));
          } else if (string0.startsWith("textureCompression=")) {
            OptionTextureCompression = Boolean.parseBoolean(
                string0.replaceFirst("textureCompression=", ""));
          } else if (string0.startsWith("modelTextureMipmaps=")) {
            OptionModelTextureMipmaps = Boolean.parseBoolean(
                string0.replaceFirst("modelTextureMipmaps=", ""));
          } else if (string0.startsWith("texture2x=")) {
            OptionTexture2x =
                Boolean.parseBoolean(string0.replaceFirst("texture2x=", ""));
          } else if (string0.startsWith("maxTextureSize=")) {
            OptionMaxTextureSize =
                Integer.parseInt(string0.replaceFirst("maxTextureSize=", ""));
            OptionMaxTextureSize = PZMath.clamp(OptionMaxTextureSize, 1, 4);
          } else if (string0.startsWith("maxVehicleTextureSize=")) {
            OptionMaxVehicleTextureSize = Integer.parseInt(
                string0.replaceFirst("maxVehicleTextureSize=", ""));
            OptionMaxVehicleTextureSize =
                PZMath.clamp(OptionMaxVehicleTextureSize, 1, 4);
            ...
            OptionShowChatTimestamp = Boolean.parseBoolean(
                string0.replaceFirst("showChatTimestamp=", ""));
          } else if (string0.startsWith("showChatTitle=")) {
            OptionShowChatTitle = Boolean.parseBoolean(
                string0.replaceFirst("showChatTitle=", ""));
            ...
            OptionMinChatOpaque =
                Float.parseFloat(string0.replaceFirst("minChatOpaque=", ""));
          } else if (string0.startsWith("maxChatOpaque=")) {
            OptionMaxChatOpaque =
                Float.parseFloat(string0.replaceFirst("maxChatOpaque=", ""));
          } else if (string0.startsWith("chatFadeTime=")) {
            OptionChatFadeTime =
                Float.parseFloat(string0.replaceFirst("chatFadeTime=", ""));
          } else if (string0.startsWith("chatOpaqueOnFocus=")) {
            OptionChatOpaqueOnFocus = Boolean.parseBoolean(
                string0.replaceFirst("chatOpaqueOnFocus=", ""));
          } else if (string0.startsWith("doneNewSaveFolder=")) {
            this.doneNewSaveFolder = Boolean.parseBoolean(
                string0.replaceFirst("doneNewSaveFolder=", ""));
          } else if (string0.startsWith("contentTranslationsEnabled=")) {
            OptionEnableContentTranslations = Boolean.parseBoolean(
                string0.replaceFirst("contentTranslationsEnabled=", ""));
          } else if (string0.startsWith("showYourUsername=")) {
            this.showYourUsername = Boolean.parseBoolean(
                string0.replaceFirst("showYourUsername=", ""));
          } else if (string0.startsWith("riversideDone=")) {
            this.riversideDone = Boolean.parseBoolean(
                string0.replaceFirst("riversideDone=", ""));
          } else if (string0.startsWith("rosewoodSpawnDone=")) {
            this.rosewoodSpawnDone = Boolean.parseBoolean(
                string0.replaceFirst("rosewoodSpawnDone=", ""));
          } else if (string0.startsWith("gotNewBelt=")) {
            this.gotNewBelt =
                Boolean.parseBoolean(string0.replaceFirst("gotNewBelt=", ""));
          } else if (string0.startsWith("mpTextColor=")) {
            String[] strings1 =
                string0.replaceFirst("mpTextColor=", "").split(",");
            ...
            String[] strings2 =
                string0.replaceFirst("objHighlightColor=", "").split(",");
            ...
            String[] strings3 =
                string0.replaceFirst("goodHighlightColor=", "").split(",");
            ...
            String[] strings4 =
                string0.replaceFirst("badHighlightColor=", "").split(",");
            ...
            this.setToggleToAim(
                Boolean.parseBoolean(string0.replaceFirst("toggleToAim=", "")));
          } else if (string0.startsWith("toggleToRun=")) {
            this.setToggleToRun(
                Boolean.parseBoolean(string0.replaceFirst("toggleToRun=", "")));
          } else if (string0.startsWith("toggleToSprint=")) {
            this.setToggleToSprint(Boolean.parseBoolean(
                string0.replaceFirst("toggleToSprint=", "")));
          } else if (string0.startsWith("celsius=")) {
            this.setCelsius(
                Boolean.parseBoolean(string0.replaceFirst("celsius=", "")));
          } else if (!string0.startsWith("mapOrder=")) {
            if (string0.startsWith("showFirstTimeSneakTutorial=")) {
              this.setShowFirstTimeSneakTutorial(Boolean.parseBoolean(
                  string0.replaceFirst("showFirstTimeSneakTutorial=", "")));
            } else if (string0.startsWith("showFirstTimeSearchTutorial=")) {
              this.setShowFirstTimeSearchTutorial(Boolean.parseBoolean(
                  string0.replaceFirst("showFirstTimeSearchTutorial=", "")));
            } else if (string0.startsWith("termsOfServiceVersion=")) {
              this.termsOfServiceVersion = Integer.parseInt(
                  string0.replaceFirst("termsOfServiceVersion=", ""));
            } else if (string0.startsWith("uiRenderOffscreen=")) {
              OptionUIFBO = Boolean.parseBoolean(
                  string0.replaceFirst("uiRenderOffscreen=", ""));
            } else if (string0.startsWith("uiRenderFPS=")) {
              OptionUIRenderFPS =
                  Integer.parseInt(string0.replaceFirst("uiRenderFPS=", ""));
            } else if (string0.startsWith("radialMenuKeyToggle=")) {
              OptionRadialMenuKeyToggle = Boolean.parseBoolean(
                  string0.replaceFirst("radialMenuKeyToggle=", ""));
            } else if (string0.startsWith("reloadRadialInstant=")) {
              OptionReloadRadialInstant = Boolean.parseBoolean(
                  string0.replaceFirst("reloadRadialInstant=", ""));
            } else if (string0.startsWith("panCameraWhileAiming=")) {
              OptionPanCameraWhileAiming = Boolean.parseBoolean(
                  string0.replaceFirst("panCameraWhileAiming=", ""));
            } else if (string0.startsWith("panCameraWhileDriving=")) {
              OptionPanCameraWhileDriving = Boolean.parseBoolean(
                  string0.replaceFirst("panCameraWhileDriving=", ""));
            } else if (string0.startsWith("temperatureDisplayCelsius=")) {
              OptionTemperatureDisplayCelsius = Boolean.parseBoolean(
                  string0.replaceFirst("temperatureDisplayCelsius=", ""));
            } else if (string0.startsWith("doWindSpriteEffects=")) {
              OptionDoWindSpriteEffects = Boolean.parseBoolean(
                  string0.replaceFirst("doWindSpriteEffects=", ""));
            } else if (string0.startsWith("doDoorSpriteEffects=")) {
              OptionDoDoorSpriteEffects = Boolean.parseBoolean(
                  string0.replaceFirst("doDoorSpriteEffects=", ""));
            } else if (string0.startsWith("doContainerOutline=")) {
              OptionDoContainerOutline = Boolean.parseBoolean(
                  string0.replaceFirst("doContainerOutline=", ""));
            } else if (string0.startsWith("updateSneakButton2=")) {
              OptionUpdateSneakButton = true;
            } else if (string0.startsWith("updateSneakButton=")) {
              OptionUpdateSneakButton = Boolean.parseBoolean(
                  string0.replaceFirst("updateSneakButton=", ""));
            } else if (string0.startsWith("dblTapJogToSprint=")) {
              OptiondblTapJogToSprint = Boolean.parseBoolean(
                  string0.replaceFirst("dblTapJogToSprint=", ""));
            } else if (string0.startsWith("aimOutline=")) {
              this.setOptionAimOutline(PZMath.tryParseInt(
                  string0.replaceFirst("aimOutline=", ""), 2));
            } else if (string0.startsWith("cycleContainerKey=")) {
              OptionCycleContainerKey =
                  string0.replaceFirst("cycleContainerKey=", "");
            } else if (string0.startsWith("dropItemsOnSquareCenter=")) {
              OptionDropItemsOnSquareCenter = Boolean.parseBoolean(
                  string0.replaceFirst("dropItemsOnSquareCenter=", ""));
            } else if (string0.startsWith("timedActionGameSpeedReset=")) {
              OptionTimedActionGameSpeedReset = Boolean.parseBoolean(
                  string0.replaceFirst("timedActionGameSpeedReset=", ""));
            } else if (string0.startsWith("shoulderButtonContainerSwitch=")) {
              OptionShoulderButtonContainerSwitch = Integer.parseInt(
                  string0.replaceFirst("shoulderButtonContainerSwitch=", ""));
            } else if (string0.startsWith("singleContextMenu=")) {
              this.readPerPlayerBoolean(
                  string0.replaceFirst("singleContextMenu=", ""),
                  OptionSingleContextMenu);
            } else if (string0.startsWith("renderPrecipIndoors=")) {
              OptionRenderPrecipIndoors = Boolean.parseBoolean(
                  string0.replaceFirst("renderPrecipIndoors=", ""));
            } else if (string0.startsWith("autoProneAtk=")) {
              OptionAutoProneAtk = Boolean.parseBoolean(
                  string0.replaceFirst("autoProneAtk=", ""));
            } else if (string0.startsWith("3DGroundItem=")) {
              Option3DGroundItem = Boolean.parseBoolean(
                  string0.replaceFirst("3DGroundItem=", ""));
            } else if (string0.startsWith("tieredZombieUpdates=")) {
              this.setOptionTieredZombieUpdates(Boolean.parseBoolean(
                  string0.replaceFirst("tieredZombieUpdates=", "")));
            } else if (string0.startsWith("progressBar=")) {
              this.setOptionProgressBar(Boolean.parseBoolean(
                  string0.replaceFirst("progressBar=", "")));
            } else if (string0.startsWith("corpseShadows=")) {
              OptionCorpseShadows = Boolean.parseBoolean(
                  string0.replaceFirst("corpseShadows=", ""));
            } else if (string0.startsWith("simpleClothingTextures=")) {
              this.setOptionSimpleClothingTextures(PZMath.tryParseInt(
                  string0.replaceFirst("simpleClothingTextures=", ""), 1));
            } else if (string0.startsWith("simpleWeaponTextures=")) {
              OptionSimpleWeaponTextures = Boolean.parseBoolean(
                  string0.replaceFirst("simpleWeaponTextures=", ""));
            } else if (string0.startsWith("autoDrink=")) {
              OptionAutoDrink =
                  Boolean.parseBoolean(string0.replaceFirst("autoDrink=", ""));
            } else if (string0.startsWith("leaveKeyInIgnition=")) {
              OptionLeaveKeyInIgnition = Boolean.parseBoolean(
                  string0.replaceFirst("leaveKeyInIgnition=", ""));
            } else if (string0.startsWith("autoWalkContainer=")) {
              OptionAutoWalkContainer = Boolean.parseBoolean(
                  string0.replaceFirst("autoWalkContainer=", ""));
            } else if (string0.startsWith("searchModeOverlayEffect=")) {
              OptionSearchModeOverlayEffect = Integer.parseInt(
                  string0.replaceFirst("searchModeOverlayEffect=", ""));
            } else if (string0.startsWith("ignoreProneZombieRange=")) {
              this.setOptionIgnoreProneZombieRange(PZMath.tryParseInt(
                  string0.replaceFirst("ignoreProneZombieRange=", ""), 1));
            } else if (string0.startsWith("fogQuality=")) {
              PerformanceSettings.FogQuality =
                  Integer.parseInt(string0.replaceFirst("fogQuality=", ""));
            } else if (string0.startsWith("renderPrecipitation=")) {
              OptionRenderPrecipitation = Integer.parseInt(
                  string0.replaceFirst("renderPrecipitation=", ""));
            } else if (string0.startsWith("showItemModInfo=")) {
              OptionShowItemModInfo = Boolean.parseBoolean(
                  string0.replaceFirst("showItemModInfo=", ""));
            } else if (string0.startsWith("showSurvivalGuide=")) {
              this.OptionShowSurvivalGuide = Boolean.parseBoolean(
                  string0.replaceFirst("showSurvivalGuide=", ""));
            } else if (string0.startsWith("enableLeftJoystickRadialMenu=")) {
              OptionEnableLeftJoystickRadialMenu = Boolean.parseBoolean(
                  string0.replaceFirst("enableLeftJoystickRadialMenu=", ""));
            }
          } else {
            if (this.version < 7) {
              string0 = "mapOrder=";
            ...
            String[] strings5 =
                string0.replaceFirst("mapOrder=", "").split(";");

            for (auto &string2 : strings5)
      ...
      if (!this.doneNewSaveFolder) {
        ...
        try {
          for (auto &string3 : arrayList)
            object = new File(ZomboidFileSystem.instance.getCacheDir() +
                              File.separator + string3);
          file3 = new File(ZomboidFileSystem.instance.getSaveDir() +
                           File.separator + string3);
          if (object.exists()) {
            file3.mkdir();
            Files.move(object.toPath(), file3.toPath(),
...
bool isDefaultOptions() { return !this.bLoadedOptions; }
  ...
  if (file0.isDirectory()) {
    ...
    for (int int0 = 0; int0 < strings.length; int0++) {
      this.searchFolders(
          ...
          string + File.separator + strings[int0]);
    }
  } else {
...
void copyPasteFile(File file1, const std::string &string) {
  ...
  try {
    ...
    fileOutputStream.getChannel().transferFrom(fileInputStream.getChannel(), 0L,
                                               file1.length());
  } catch (Exception exception) {
    exception.printStackTrace();
  } finally {
...
void saveOptions() {
  ...
  try {
    ...
    fileWriter.write("contentTranslationsEnabled=" +
    ...
    if (this.mpTextColor != nullptr) {
      fileWriter.write("mpTextColor=" + this.mpTextColor.r + "," +
                       this.mpTextColor.g + "," + this.mpTextColor.b + "\r\n");
    }

    fileWriter.write("objHighlightColor=" + this.objectHighlitedColor.r + "," +
                     this.objectHighlitedColor.g + "," +
                     this.objectHighlitedColor.b + "\r\n");
    ...
    fileWriter.write("showFirstTimeSneakTutorial=" +
                     this.isShowFirstTimeSneakTutorial() + "\r\n");
    fileWriter.write("showFirstTimeSearchTutorial=" +
                     this.isShowFirstTimeSearchTutorial() + "\r\n");
    ...
    fileWriter.write("panCameraWhileDriving=" + OptionPanCameraWhileDriving +
                     "\r\n");
    fileWriter.write("temperatureDisplayCelsius=" +
    ...
    fileWriter.write(
        "dropItemsOnSquareCenter=" + OptionDropItemsOnSquareCenter + "\r\n");
    fileWriter.write("timedActionGameSpeedReset=" +
                     OptionTimedActionGameSpeedReset + "\r\n");
    fileWriter.write("shoulderButtonContainerSwitch=" +
                     OptionShoulderButtonContainerSwitch + "\r\n");
    fileWriter.write("singleContextMenu=" +
                     this.getPerPlayerBooleanString(OptionSingleContextMenu) +
                     "\r\n");
    ...
    fileWriter.write(
        "tieredZombieUpdates=" + this.getOptionTieredZombieUpdates() + "\r\n");
    fileWriter.write("progressBar=" + this.isOptionProgressBar() + "\r\n");
    fileWriter.write("corpseShadows=" + this.getOptionCorpseShadows() + "\r\n");
    fileWriter.write("simpleClothingTextures=" +
                     this.getOptionSimpleClothingTextures() + "\r\n");
    fileWriter.write("simpleWeaponTextures=" +
                     this.getOptionSimpleWeaponTextures() + "\r\n");
    fileWriter.write("autoDrink=" + this.getOptionAutoDrink() + "\r\n");
    fileWriter.write(
        "leaveKeyInIgnition=" + this.getOptionLeaveKeyInIgnition() + "\r\n");
    fileWriter.write("autoWalkContainer=" + this.getOptionAutoWalkContainer() +
                     "\r\n");
    fileWriter.write("searchModeOverlayEffect=" +
                     this.getOptionSearchModeOverlayEffect() + "\r\n");
    fileWriter.write("ignoreProneZombieRange=" +
                     this.getOptionIgnoreProneZombieRange() + "\r\n");
    ...
    fileWriter.write("showSurvivalGuide=" + this.OptionShowSurvivalGuide +
                     "\r\n");
    fileWriter.write("enableLeftJoystickRadialMenu=" +
                     OptionEnableLeftJoystickRadialMenu + "\r\n");
    fileWriter.write("doContainerOutline=" + OptionDoContainerOutline + "\r\n");
    fileWriter.write("goodHighlightColor=" + this.goodHighlitedColor.r + "," +
                     this.goodHighlitedColor.g + "," +
                     this.goodHighlitedColor.b + "\r\n");
    fileWriter.write("badHighlightColor=" + this.badHighlitedColor.r + "," +
                     this.badHighlitedColor.g + "," + this.badHighlitedColor.b +
                     "\r\n");
  } catch (Exception exception) {
    exception.printStackTrace();
  } finally {
...
KahluaTable getScreenModes() {
  ...
  File file = new File(LuaManager.getLuaCacheDir() + File.separator +
  ...
  try {
    if (!file.exists()) {
      ...
      std::string string;
      for (string = nullptr; (string = bufferedReader.readLine()) != nullptr;
           int0++) {
  ...
  return table;
    ...
    static void setDisplayMode(int _width, int _height, bool fullscreen) {
        RenderThread.invokeOnRenderContext(
            () -> {
  if (Display.getWidth() != _width || Display.getHeight() != _height ||
    ...
    try {
      ...
      if (!fullscreen) {
        if (OptionBorderlessWindow) {
          ...
          displayMode0 =
              new DisplayMode(gLFWVidMode.width(), gLFWVidMode.height());
        } else {
        ...
        for (auto &displayMode2 : displayModes)
          if (displayMode2.getWidth() == _width &&
              displayMode2.getHeight() == _height &&
              displayMode2.isFullscreenCapable()) {
            if ((displayMode0 == nullptr ||
                 displayMode2.getFrequency() >= int2) &&
                (displayMode0 == nullptr ||
                 displayMode2.getBitsPerPixel() >
                     displayMode0.getBitsPerPixel())) {
              displayMode0 = displayMode2;
              int2 = displayMode2.getFrequency();
            }

            if (displayMode2.getBitsPerPixel() ==
                    Display.getDesktopDisplayMode().getBitsPerPixel() &&
                displayMode2.getFrequency() ==
        ...
        if (displayMode2.isFullscreenCapable() &&
            (displayMode1 == nullptr ||
             Math.abs(displayMode2.getWidth() - _width) <
                 Math.abs(displayMode1.getWidth() - _width) ||
             displayMode2.getWidth() == displayMode1.getWidth() &&
                 displayMode2.getFrequency() > int2)) {
          displayMode1 = displayMode2;
          int2 = displayMode2.getFrequency();
          System.out.println("closest width=" + displayMode2.getWidth() +
                             " freq=" + displayMode2.getFrequency());
        }
      }
    ...
    if (displayMode0 == nullptr) {
      DebugLog.log("Failed to find value mode: " + _width + "x" + _height +
                   " fs=" + fullscreen);
      return;
    }
    ...
    if (Display.isCreated()) {
      DebugLog.log("Display mode changed to " + Display.getWidth() + "x" +
                   Display.getHeight() +
    ...
    DebugLog.log("Unable to setup mode " + _width + "x" + _height +
                 " fullscreen=" + fullscreen + lWJGLException);
  }
                }
            }
        );
    ...
    bool isFunctionKey(int int0) {
      return int0 >= 59 && int0 <= 68 || int0 >= 87 && int0 <= 105 ||
             int0 == 113;
    }
    ...
    bool isDoingTextEntry() {
      if (CurrentTextEntryBox == nullptr) {
        ...
        return !CurrentTextEntryBox.IsEditable
                   ? false
                   : CurrentTextEntryBox.DoingTextEntry;
      }
    }
    ...
    void updateKeyboardAux(UITextBox2 uITextBox2, int int0) {
      ...
      if (int0 == 28 || int0 == 156) {
        bool boolean2 = false;
        if (UIManager.getDebugConsole() != nullptr &&
            uITextBox2 == UIManager.getDebugConsole().CommandLine) {
          boolean2 = true;
        }
        ...
        if (uITextBox2.multipleLine) {
          if (uITextBox2.Lines.size() < uITextBox2.getMaxLines()) {
            if (uITextBox2.TextEntryCursorPos != uITextBox2.ToSelectionIndex) {
              int int1 = Math.min(uITextBox2.TextEntryCursorPos,
                                  uITextBox2.ToSelectionIndex);
              int int2 = Math.max(uITextBox2.TextEntryCursorPos,
                                  uITextBox2.ToSelectionIndex);
              if (uITextBox2.internalText.length() > 0) {
                uITextBox2.internalText =
                    uITextBox2.internalText.substring(0, int1) + "\n" +
            ...
            uITextBox2.CursorLine =
                uITextBox2.toDisplayLine(uITextBox2.TextEntryCursorPos);
          }
        } else {
        ...
        if (boolean2 &&
            (!GameClient.bClient || GameClient.connection.accessLevel != 1 ||
             GameClient.connection != nullptr &&
                 GameClient.connection.isCoopHost)) {
        ...
        if (int0 == 199) {
          ...
          if (!uITextBox2.Lines.isEmpty()) {
            uITextBox2.TextEntryCursorPos =
                uITextBox2.TextOffsetOfLineStart.get(uITextBox2.CursorLine);
          }
          ...
          if (uITextBox2.Lines.size() - 1 > uITextBox2.CursorLine &&
              uITextBox2.CursorLine + 1 < uITextBox2.getMaxLines()) {
            int int5 =
                uITextBox2.TextEntryCursorPos -
                uITextBox2.TextOffsetOfLineStart.get(uITextBox2.CursorLine);
            uITextBox2.CursorLine++;
                ...
                if (int0 != 56) {
                  if (int0 != 184) {
                    if (int0 == 203) {
                      ...
                      uITextBox2.TextEntryCursorPos++;
                      if (uITextBox2.TextEntryCursorPos >
                          uITextBox2.internalText.length()) {
                        uITextBox2.TextEntryCursorPos =
                            uITextBox2.internalText.length();
                      }
                      ...
                      if ((int0 == 211 || int0 == 14) &&
                          uITextBox2.TextEntryCursorPos !=
                              uITextBox2.ToSelectionIndex) {
                        int int6 = Math.min(uITextBox2.TextEntryCursorPos,
                                            uITextBox2.ToSelectionIndex);
                        int int7 = Math.max(uITextBox2.TextEntryCursorPos,
                                            uITextBox2.ToSelectionIndex);
                        uITextBox2.internalText =
                            uITextBox2.internalText.substring(0, int6) +
                            uITextBox2.internalText.substring(int7);
                        ...
                        if (uITextBox2.internalText.length() != 0 &&
                          ...
                          if (uITextBox2.TextEntryCursorPos > 0) {
                            uITextBox2.internalText =
                                uITextBox2.internalText.substring(
                                    0, uITextBox2.TextEntryCursorPos) +
                                uITextBox2.internalText.substring(
                                    uITextBox2.TextEntryCursorPos + 1);
                          } else {
                            uITextBox2.internalText =
                                uITextBox2.internalText.substring(1);
                          }
                        ...
                        if (uITextBox2.internalText.length() != 0 &&
                          ...
                          if (uITextBox2.TextEntryCursorPos >
                            ...
                            uITextBox2.internalText =
                                uITextBox2.internalText.substring(
                                    0, uITextBox2.internalText.length() - 1);
                          } else {
                            int int8 = uITextBox2.TextEntryCursorPos;
                            uITextBox2.internalText =
                                uITextBox2.internalText.substring(0, int8 - 1) +
                        ...
                        if (string1 != nullptr) {
                          if (uITextBox2.TextEntryCursorPos !=
                              uITextBox2.ToSelectionIndex) {
                            int int9 = Math.min(uITextBox2.TextEntryCursorPos,
                                                uITextBox2.ToSelectionIndex);
                            int int10 = Math.max(uITextBox2.TextEntryCursorPos,
                                                 uITextBox2.ToSelectionIndex);
                            uITextBox2.internalText =
                                uITextBox2.internalText.substring(0, int9) +
                                string1 +
                            ...
                            if (uITextBox2.TextEntryCursorPos <
                              ...
                              uITextBox2.internalText =
                                  uITextBox2.internalText + string1;
                            }
                            ...
                            uITextBox2.TextEntryCursorPos =
                                ...
                                string1.length();
                            uITextBox2.ToSelectionIndex =
                                uITextBox2.ToSelectionIndex + string1.length();
                          }
                        ...
                        if (uITextBox2.TextEntryCursorPos !=
                          ...
                          int int11 = Math.min(uITextBox2.TextEntryCursorPos,
                                               uITextBox2.ToSelectionIndex);
                          int int12 = Math.max(uITextBox2.TextEntryCursorPos,
                                               uITextBox2.ToSelectionIndex);
                          std::string string2 =
                              uITextBox2.Text.substring(int11, int12);
                        ...
                        if (uITextBox2.TextEntryCursorPos !=
                            uITextBox2.ToSelectionIndex) {
                          uITextBox2.updateText();
                          int int13 = Math.min(uITextBox2.TextEntryCursorPos,
                                               uITextBox2.ToSelectionIndex);
                          int int14 = Math.max(uITextBox2.TextEntryCursorPos,
                                               uITextBox2.ToSelectionIndex);
                          std::string string3 =
                              uITextBox2.Text.substring(int13, int14);
                          if (string3 != nullptr && string3.length() > 0) {
                            Clipboard.setClipboard(string3);
                          }

                          uITextBox2.internalText =
                              uITextBox2.internalText.substring(0, int13) +
                        ...
                        if (uITextBox2.internalText.length() <
                            uITextBox2.TextEntryMaxLength) {
                          char char0 = Keyboard.getEventCharacter();
                          if (char0 != 0) {
                            if (uITextBox2.isOnlyNumbers() && char0 != '.' &&
                                char0 != '-') {
                              try {
                            ...
                            if (uITextBox2.TextEntryCursorPos ==
                              ...
                              if (int15 < uITextBox2.internalText.length()) {
                                uITextBox2.internalText =
                                    uITextBox2.internalText.substring(0,
                                                                      int15) +
                                    char0 +
                              ...
                              int int16 =
                                  Math.min(uITextBox2.TextEntryCursorPos,
                                           uITextBox2.ToSelectionIndex);
                              int int17 =
                                  Math.max(uITextBox2.TextEntryCursorPos,
                                           uITextBox2.ToSelectionIndex);
                              if (uITextBox2.internalText.length() > 0) {
                                uITextBox2.internalText =
                                    uITextBox2.internalText.substring(0,
    ...
    bool supportRes(int _width, int _height) {
      ...
      for (int int0 = 0; int0 < displayModes.length; int0++) {
        if (displayModes[int0].getWidth() == _width &&
            displayModes[int0].getHeight() == _height &&
            displayModes[int0].isFullscreenCapable()) {
          return true;
        }
      }
    ...
    void init(int _width, int _height) {
      System.setProperty("org.lwjgl.opengl.Window.undecorated",
                         OptionBorderlessWindow ? "true" : "false");
      if (!System.getProperty("os.name").contains("OS X") &&
          !System.getProperty("os.name").startsWith("Win")) {
        DebugLog.log(
            "Creating display. If this fails, you may need to install xrandr.");
      }
      ...
      fullScreen = Display.isFullscreen();
      DebugLog.log("GraphicsCard: " + GL11.glGetString(7936) + " " +
                   GL11.glGetString(7937));
      DebugLog.log("OpenGL version: " + GL11.glGetString(7938));
      DebugLog.log("Desktop resolution " +
                   Display.getDesktopDisplayMode().getWidth() + "x" +
                   Display.getDesktopDisplayMode().getHeight());
      DebugLog.log("Initial resolution " + width + "x" + height +
                   " fullScreen=" + fullScreen);
    ...
    bool setupMultiFBO() {
      try {
        if (!this.OffscreenBuffer.test()) {
          ...
          this.OffscreenBuffer.setZoomLevelsFromOption(
              TileScale == 2 ? OptionZoomLevels2x : OptionZoomLevels1x);
    ...
    void setScreenSize(int _width, int _height) {
      if (width != _width || _height != height) {
        int int0 = width;
        int int1 = height;
        DebugLog.log("Screen resolution changed from " + int0 + "x" + int1 +
                     " to " + _width + "x" + _height +
                     " fullScreen=" + fullScreen);
        width = _width;
        height = _height;
        ...
        try {
          LuaEventManager.triggerEvent("OnResolutionChange", int0, int1, _width,
                                       _height);
        } catch (Exception exception1) {
          exception1.printStackTrace();
        }
    ...
    void StartFrame() {
      if (LuaManager.thread == nullptr || !LuaManager.thread.bStep) {
        ...
        if (!boolean0) {
          ...
          if (DebugOptions.instance.Terrain.RenderTiles.HighContrastBg
                  .getValue()) {
        ...
        IndieGL.glDoStartFrame(this.getScreenWidth(), this.getScreenWidth(),
                               this.getCurrentPlayerZoom(), 0);
        this.frameStage = 1;
      }
    }
    ...
    void StartFrame(int nPlayer, bool clear) {
      if (!LuaManager.thread.bStep) {
        ...
        if (this.RenderShader != nullptr &&
            this.OffscreenBuffer.Current != nullptr) {
          this.RenderShader.setTexture(
              this.OffscreenBuffer.getTexture(nPlayer));
        }
        ...
        IndieGL.glDoStartFrame(this.getScreenWidth(), this.getScreenHeight(),
                               this.getZoom(nPlayer), nPlayer);
    ...
    void DoStartFrameStuff(int w, int h, float zoom, int player,
                           bool isTextFrame) {
      this.DoStartFrameStuffInternal(w, h, zoom, player, isTextFrame, false,
                                     false);
    }
    ...
    void DoStartFrameStuffInternal(int int2, int int3, float float0, int int4,
                                   bool boolean2, bool boolean0,
                                   bool boolean1) {
      GL32.glEnable(3042);
    ...
    void DoPushIsoStuff(float ox, float oy, float oz, float useangle,
                        bool vehicle) {
      ...
      matrix4f0.setOrtho(-((float)double3) / 2.0F, (float)double3 / 2.0F,
                         -((float)double4) / 2.0F, (float)double4 / 2.0F,
                         -10.0F, 10.0F);
      ...
      matrix4f1.translate(-((float)double5), (float)(oz - double2) * 2.5F,
                          -((float)double6));
      if (vehicle) {
    ...
    void DoPushIsoParticleStuff(float ox, float oy, float oz) {
      ...
      GL11.glLoadIdentity();
      GL11.glOrtho(-double3 / 2.0, double3 / 2.0, -double4 / 2.0, double4 / 2.0,
                   -10.0, 10.0);
      GL11.glMatrixMode(5888);
      GL11.glPushMatrix();
      GL11.glLoadIdentity();
    ...
    void StartFrameText(int nPlayer) {
      if (LuaManager.thread == nullptr || !LuaManager.thread.bStep) {
        IndieGL.glDoStartFrame(IsoCamera.getScreenWidth(nPlayer),
                               IsoCamera.getScreenHeight(nPlayer), 1.0F,
                               nPlayer, true);
        this.frameStage = 2;
      }
    }
      ...
      if (LuaManager.thread != nullptr && LuaManager.thread.bStep) {
        ...
        if (UIManager.useUIFBO) {
          if (UIManager.defaultthread == LuaManager.debugthread) {
            ...
            this.UIRenderAccumulator =
                this.UIRenderAccumulator +
                GameTime.getInstance().getMultiplier() / 1.6F;
            this.UIRenderThisFrame =
                this.UIRenderAccumulator >= 30.0F / OptionUIRenderFPS;
          }
  ...
  public
    Map<String, Integer> getKeyMaps() { return this.keyMaps; }

    void setKeyMaps(Integer > _keyMaps) { this.keyMaps = _keyMaps; }
    ...
    int getKey(const std::string &keyName) {
      if (this.keyMaps == nullptr) {
        ...
        return this.keyMaps.get(keyName) != nullptr ? this.keyMaps.get(keyName)
                                                    : 0;
      }
    }
    ...
    static bool isLastStand() { return bLastStand; }

    std::string getVersion() { return gameVersion + ".16"; }

    GameVersion getGameVersion() { return gameVersion; }

    std::string getSteamServerVersion() { return this.steamServerVersion; }

    void DoFrameReady() { this.updateKeyboard(); }
    ...
    float getZoom(int playerIndex) {
      return this.OffscreenBuffer != nullptr
                 ? this.OffscreenBuffer.zoom[playerIndex] * (TileScale / 2.0F)
                 : 1.0F;
    }

    float getNextZoom(int playerIndex, int del) {
      return this.OffscreenBuffer != nullptr
                 ? this.OffscreenBuffer.getNextZoom(playerIndex, del)
                 : 1.0F;
    }

    float getMinZoom() {
      return this.OffscreenBuffer != nullptr
                 ? this.OffscreenBuffer.getMinZoom() * (TileScale / 2.0F)
                 : 1.0F;
    }

    float getMaxZoom() {
      return this.OffscreenBuffer != nullptr
                 ? this.OffscreenBuffer.getMaxZoom() * (TileScale / 2.0F)
                 : 1.0F;
    }
    ...
    std::string getSaveFolder() { return this.saveFolder; }

    bool getOptionZoom() { return OptionZoom; }

    void setOptionZoom(bool zoom) { OptionZoom = zoom; }
    ...
    bool isZoomEnabled() { return this.OffscreenBuffer.bZoomEnabled; }
    ...
    bool getAutoZoom(int playerIndex) { return bAutoZoom[playerIndex]; }
    ...
    bool getOptionVSync() { return OptionVSync; }
    ...
    int getOptionSoundVolume() { return OptionSoundVolume; }

    float getRealOptionSoundVolume() { return OptionSoundVolume / 10.0F; }
    ...
    int getOptionMusicVolume() { return OptionMusicVolume; }
    ...
    int getOptionAmbientVolume() { return OptionAmbientVolume; }
    ...
    int getOptionJumpScareVolume() { return OptionJumpScareVolume; }

    void setOptionJumpScareVolume(int volume) {
      OptionJumpScareVolume = PZMath.clamp(volume, 0, 10);
    }

    int getOptionMusicActionStyle() { return OptionMusicActionStyle; }

    void setOptionMusicActionStyle(int v) {
      OptionMusicActionStyle = PZMath.clamp(v, 1, 2);
    }

    int getOptionMusicLibrary() { return OptionMusicLibrary; }
    ...
    int getOptionVehicleEngineVolume() { return OptionVehicleEngineVolume; }

    void setOptionVehicleEngineVolume(int volume) {
      OptionVehicleEngineVolume = Math.max(0, Math.min(10, volume));
      if (SoundManager.instance != nullptr) {
        SoundManager.instance.setVehicleEngineVolume(OptionVehicleEngineVolume /
                                                     10.0F);
      }
    }

    bool getOptionVoiceEnable() { return OptionVoiceEnable; }
    ...
    int getOptionVoiceMode() { return OptionVoiceMode; }
    ...
    int getOptionVoiceVADMode() { return OptionVoiceVADMode; }
    ...
    int getOptionVoiceAGCMode() { return OptionVoiceAGCMode; }
    ...
    int getOptionVoiceVolumeMic() { return OptionVoiceVolumeMic; }
    ...
    int getOptionVoiceVolumePlayers() { return OptionVoiceVolumePlayers; }
    ...
    int getOptionReloadDifficulty() { return 2; }

    void setOptionReloadDifficulty(int d) {
      OptionReloadDifficulty = Math.max(1, Math.min(3, d));
    }

    bool getOptionRackProgress() { return OptionRackProgress; }
    ...
    bool getOptionClock24Hour() { return OptionClock24Hour; }

    void setOptionClock24Hour(bool b24Hour) { OptionClock24Hour = b24Hour; }

    bool getOptionModsEnabled() { return OptionModsEnabled; }

    void setOptionModsEnabled(bool enabled) { OptionModsEnabled = enabled; }

    int getOptionBloodDecals() { return OptionBloodDecals; }
    ...
    bool getOptionBorderlessWindow() { return OptionBorderlessWindow; }

    void setOptionBorderlessWindow(bool b) { OptionBorderlessWindow = b; }

    bool getOptionLockCursorToWindow() { return OptionLockCursorToWindow; }

    void setOptionLockCursorToWindow(bool b) { OptionLockCursorToWindow = b; }

    bool getOptionTextureCompression() { return OptionTextureCompression; }

    void setOptionTextureCompression(bool b) { OptionTextureCompression = b; }

    bool getOptionTexture2x() { return OptionTexture2x; }
    ...
    void setOptionZoomLevels1x(const std::string& levels) {
        OptionZoomLevels1x = levels == nullptr ? "" :
              levels;
            }
          ...
          public
            ...
            void setOptionActiveController(int controllerIndex, bool active) {
              if (controllerIndex >= 0 &&
                  controllerIndex < GameWindow.GameInput.getControllerCount()) {
                Controller controller =
                    GameWindow.GameInput.getController(controllerIndex);
                if (controller != nullptr) {
                  JoypadManager.instance.setControllerActive(
                      controller.getGUID(), active);
                }
              }
            }
            ...
            bool isOptionShowChatTimestamp() { return OptionShowChatTimestamp; }
            ...
            bool getOptionUIFBO() { return OptionUIFBO; }
            ...
            int getOptionAimOutline() { return OptionAimOutline; }

            void setOptionAimOutline(int choice) {
              OptionAimOutline = PZMath.clamp(choice, 1, 3);
            }

            int getOptionUIRenderFPS() { return OptionUIRenderFPS; }

            void setOptionUIRenderFPS(int fps) { OptionUIRenderFPS = fps; }
            ...
            bool getOptionAutoDrink() { return OptionAutoDrink; }

            void setOptionAutoDrink(bool enable) { OptionAutoDrink = enable; }
            ...
            bool getOptionCorpseShadows() { return OptionCorpseShadows; }
            ...
            std::string getPerPlayerBooleanString(boolean[] booleans) {
              return String.format("%b,%b,%b,%b", booleans[0], booleans[1],
            ...
            void ResetLua(const std::string &activeMods,
              ...
              CustomSandboxOptions.instance.initInstance(
            ...
            bool isShowPing() { return this.showPing; }

            void setShowPing(bool _showPing) { this.showPing = _showPing; }

            bool isForceSnow() { return this.forceSnow; }

            void setForceSnow(bool _forceSnow) { this.forceSnow = _forceSnow; }

            bool isZombieGroupSound() { return this.zombieGroupSound; }
            ...
            bool isTutorialDone() { return this.tutorialDone; }

            void setTutorialDone(bool done) { this.tutorialDone = done; }

            bool isVehiclesWarningShow() { return this.vehiclesWarningShow; }
            ...
            void initPoisonousBerry() {
              ...
              this.setPoisonousBerry(
                  (String)arrayList.get(Rand.Next(0, arrayList.size() - 1)));
            }
            ...
            bool isDoneNewSaveFolder() { return this.doneNewSaveFolder; }

            void setDoneNewSaveFolder(bool _doneNewSaveFolder) {
              this.doneNewSaveFolder = _doneNewSaveFolder;
            }

            static int getTileScale() { return TileScale; }

            bool isSelectingAll() { return this.isSelectingAll; }
            ...
            bool isShowYourUsername() { return this.showYourUsername; }
            ...
            ColorInfo getMpTextColor() {
              if (this.mpTextColor == nullptr) {
                this.mpTextColor =
                    new ColorInfo((Rand.Next(135) + 120) / 255.0F,
                                  (Rand.Next(135) + 120) / 255.0F,
                                  (Rand.Next(135) + 120) / 255.0F, 1.0F);
              }
            ...
            bool isAzerty() { return this.isAzerty; }

            void setAzerty(bool _isAzerty) { this.isAzerty = _isAzerty; }
            ...
            ColorInfo getBadHighlitedColor() { return this.badHighlitedColor; }
            ...
            bool isToggleToAim() { return this.toggleToAim; }

            void setToggleToAim(bool enable) { this.toggleToAim = enable; }

            bool isToggleToRun() { return this.toggleToRun; }
            ...
            int getXAngle(int _width, float angle) {
              ...
              return new Long(
                         Math.round((Math.sqrt(2.0) * Math.cos(double0) + 1.0) *
                                    (_width / 2)))
                  .intValue();
            }
            ...
            int getYAngle(int _width, float angle) {
              double double0 = Math.toRadians(225.0F + angle);
              return new Long(
                         Math.round((Math.sqrt(2.0) * Math.sin(double0) + 1.0) *
                                    (_width / 2)))
                  .intValue();
            }

            bool isCelsius() { return this.celsius; }

            void setCelsius(bool _celsius) { this.celsius = _celsius; }

            bool isInDebug() { return bDebug; }

            bool isRiversideDone() { return this.riversideDone; }

            void setRiversideDone(bool _riversideDone) {
              this.riversideDone = _riversideDone;
            }

            bool isNoSave() { return this.noSave; }

            void setNoSave(bool _noSave) { this.noSave = _noSave; }

            bool isShowFirstTimeVehicleTutorial() {
              return this.showFirstTimeVehicleTutorial;
            }

            void setShowFirstTimeVehicleTutorial(
                bool _showFirstTimeVehicleTutorial) {
            ...
            void setShowFirstTimeWeatherTutorial(
                bool _showFirstTimeWeatherTutorial) {
            ...
            bool isNewReloading() { return this.newReloading; }
            ...
            void setShowFirstTimeSneakTutorial(
                bool _showFirstTimeSneakTutorial) {
            ...
            void setShowFirstTimeSearchTutorial(
                bool _showFirstTimeSearchTutorial) {
            ...
            bool isOptiondblTapJogToSprint() { return OptiondblTapJogToSprint; }

            bool isToggleToSprint() { return this.toggleToSprint; }

            void setToggleToSprint(bool _toggleToSprint) {
              this.toggleToSprint = _toggleToSprint;
            }

            int getIsoCursorVisibility() { return this.isoCursorVisibility; }
            ...
            bool gotNewBelt() { return this.gotNewBelt; }

            void setGotNewBelt(bool gotit) { this.gotNewBelt = gotit; }

            void setAnimPopupDone(bool done) { this.bAnimPopupDone = done; }

            bool isAnimPopupDone() { return this.bAnimPopupDone; }

            void setModsPopupDone(bool done) { this.bModsPopupDone = done; }

            bool isModsPopupDone() { return this.bModsPopupDone; }

            bool isRenderPrecipIndoors() { return OptionRenderPrecipIndoors; }

            void setRenderPrecipIndoors(bool optionRenderPrecipIndoors) {
              OptionRenderPrecipIndoors = optionRenderPrecipIndoors;
            }

            bool isCollideZombies() { return this.collideZombies; }

            void setCollideZombies(bool _collideZombies) {
              this.collideZombies = _collideZombies;
            }

            bool isFlashIsoCursor() { return this.flashIsoCursor; }

            void setFlashIsoCursor(bool _flashIsoCursor) {
              this.flashIsoCursor = _flashIsoCursor;
            }

            bool isOptionProgressBar() { return true; }
            ...
            std::string getOptionLanguageName() { return OptionLanguageName; }
            ...
            bool isOptionAutoProneAtk() { return OptionAutoProneAtk; }
            ...
            void countMissing3DItems() {
              ...
              for (auto &item : arrayList)
                if (item.type != Item.Type.Weapon &&
                    ...
                    !item.name.contains("Hat") && !item.getObsolete() &&
            ...
            System.out.println("total missing: " + int0 + "/" +
                               arrayList.size());
    }

    bool getOptionShowItemModInfo() { return OptionShowItemModInfo; }

    void setOptionShowItemModInfo(bool b) { OptionShowItemModInfo = b; }

    bool getOptionShowSurvivalGuide() { return this.OptionShowSurvivalGuide; }
    ...
    std::string getVersionNumber() { return gameVersion.toString(); }
    }
    } // namespace core
    } // namespace zombie
