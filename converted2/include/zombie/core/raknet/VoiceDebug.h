...
public
  ...
  void paintComponent(Graphics graphics) {
    ...
    for (int int2 = 0; int2 < this.scores.size(); int2 += int1) {
      ...
      int int4 = (int)((this.scores_max - this.scores.get(int2)) * double1 +
                       30.0 - int0);
    ...
    graphics2D.drawLine(30, this.getHeight() - 30, this.getWidth() - 30,
                        this.getHeight() - 30);
    ...
    for (int int5 = 0; int5 < 10; int5++) {
      ...
      uint8_t byte1 = 42;
      int int6 =
          this.getHeight() - ((int5 + 1) * (this.getHeight() - 60) / 10 + 30);
  ...
  static void createAndShowGui() {
    ...
    mainPanel4 =
        new VoiceDebug(arrayList3, "FMODSoundBuffer - first 100 sample");
  ...
  static void updateGui(SoundBuffer soundBuffer,
    ...
    for (byte byte0 = 0; byte0 < fMODSoundBuffer.buf().length / 2; byte0 += 2) {
      mainPanel4.scores.add(fMODSoundBuffer.buf()[byte0 + 1] * 256 +
                            fMODSoundBuffer.buf()[byte0]);
    }
