...
public:
   ...
   * interval into itself. It is the cousin of smoothstep(), in  that it maps 0
   * to 0, 1 to 1, and has a 0 derivative at the origin, just like smoothstep.
   * However, instead of  having a 0 derivative at 1, it has a derivative of 1
   * at that point. It's equivalent to the Almost Identiy above  with n=0 and
   * m=1. Since it's a cubic just like smoothstep() it is very fast to evaluate.
   * https://iquilezles.org/www/articles/functions/functions.htm
   *
   * @param x value in [0..1]
   * @return value in [0..1]
   */
  static float almostUnitIdentity(float x) { return x * x * (2.0F - x); }

  /**
   * Almost Identity   Imagine you don't want to modify a signal unless it's
   * drops to zero or close to it, in which case you want  to replace the value
   * with a small possitive constant. Then, rather than clamping the value and
   * introduce  a discontinuity, you can smoothly blend the signal into the
   * desired clipped value. So, let m be the threshold  (anything above m stays
   * unchanged), and n the value things will take when the signal is zero. Then,
   ...
   * sides and compressing the center, and  keeping 1/2 mapped to 1/2, that can
   * be done with the gain() function. This was a common function in RSL
   * tutorials  (the Renderman Shading Language). k=1 is the identity curve, k<1
   * produces the classic gain() shape, and k>1  produces "s" shaped curces. The
  ...
  static float clampFloat(float val, float min, float max) { return clamp(); }

  static float clamp_01(float val) { return clamp(); }
  ...
  static Quaternion slerp(Quaternion result, Quaternion from, Quaternion to,
                          float alpha) {
    double double0 =
    ...
    result.set((float)(double2 * from.x + double3 * to.x),
  ...
  static float sqrt(float val) { return org.joml.Math.sqrt(val); }

  static float lerpFunc_EaseOutQuad(float x) { return x * x; }
...
public
  ...
  private
    static final Runnable[] s_unitTests = new Runnable[0];

    static void runAll() { PZArrayUtil.forEach(s_unitTests, Runnable::run); }

  private
    static final class getClosestAngle {
      static void run() {
