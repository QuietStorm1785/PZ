...
public:
private
  volatile ConcurrentAutoTable.CAT _cat =
      new ConcurrentAutoTable.CAT(nullptr, 4, 0L);
private
  static final
      AtomicReferenceFieldUpdater<ConcurrentAutoTable, ConcurrentAutoTable.CAT>
          _catUpdater = AtomicReferenceFieldUpdater.newUpdater(
              ConcurrentAutoTable.class, ConcurrentAutoTable.CAT.class, "_cat");

  void add(long long0) { this.add_if_mask(long0, 0L); }

  void decrement() { this.add_if_mask(-1L, 0L); }

  void increment() { this.add_if_mask(1L, 0L); }

  void set(long long0) {
    ConcurrentAutoTable.CAT cat =
        new ConcurrentAutoTable.CAT(nullptr, 4, long0);

    while (!this.CAS_cat(this._cat, cat)) {
    }
  }

  long get() { return this._cat.sum(0L); }

  int intValue() { return (int)this._cat.sum(0L); }
...
private
  ...
  private
    static final AtomicLongFieldUpdater<ConcurrentAutoTable.CAT>
        _resizerUpdater = AtomicLongFieldUpdater.newUpdater(
            ConcurrentAutoTable.CAT.class, "_resizers");
  private
    final ConcurrentAutoTable.CAT _next;
  private
    volatile long _sum_cache;
  private
  ...
  private
    ...
    static bool CAS(long[] longs, int int0, long long0, long long1) {
      return _unsafe.compareAndSwapLong(longs, rawIndex(longs, int0), long0,
                                        long1);
    }
    ...
    long add_if_mask(long long1, long long2, int int1,
        ...
        while (true) {
          ...
          if (CAS(longs, int0, long0, long0 + long1)) {
            ...
            ConcurrentAutoTable.CAT cat1 =
                new ConcurrentAutoTable.CAT(this, longs.length * 2, 0L);
    ...
    std::string toString(long long0) { return Long.toString(this.sum(long0)); }
