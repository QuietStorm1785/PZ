...
public:
  ...
  static Vector3f GetKeyFramePosition(Keyframe[] keyframes, float float0,
                                      double double0) {
    ...
    if (keyframes.length == 0) {
      ...
      while (int0 < keyframes.length - 1 &&
             !(float0 < keyframes[int0 + 1].Time)) {
        int0++;
      }
  ...
  static Quaternion GetKeyFrameRotation(Keyframe[] keyframes, float float0,
                                        double double0) {
