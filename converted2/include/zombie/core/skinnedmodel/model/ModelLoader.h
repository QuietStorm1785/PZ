...
private
  final ThreadLocal<SharedStrings> sharedStrings =
  ...
  ModelTxt loadTxt(const std::string &string0, bool boolean0, bool boolean1,
    ...
    VertexBufferObject.VertexFormat vertexFormat =
        new VertexBufferObject.VertexFormat(boolean0 ? 4 : 6);
    ...
    vertexFormat.setElement(3, VertexBufferObject.VertexType.TextureCoordArray,
                            8);
    if (!boolean0) {
      vertexFormat.setElement(4, VertexBufferObject.VertexType.BlendWeightArray,
                              16);
      vertexFormat.setElement(5, VertexBufferObject.VertexType.BlendIndexArray,
                              16);
    }
        ...
        if (string1.indexOf(35) != 0) {
          ...
          if (int0 > 0) {
            ...
            case VertexCount:
              ...
              modelTxt.vertices =
                  new VertexBufferObject.VertexArray(vertexFormat, int1);
              break;
            case VertexBuffer:
              int int5 = 0;
              ...
              for (; int5 < int1; int5++) {
                ...
                if (!boolean0) {
                  modelTxt.vertices.setElement(int5, 4, float11, float12,
                                               float13, float14);
            ...
            case BindPose:
              for (int int15 = 0; int15 < int4; int15++) {
                ...
                modelTxt.bindPose.add(
                    int15,
                    this.getMatrix(string1, string12, string13, string14));
                string1 = bufferedReader.readLine();
              }
            ...
            case InvBindPose:
              for (int int14 = 0; int14 < int4; int14++) {
                ...
                modelTxt.invBindPose.add(
                    int14,
                    this.getMatrix(string1, string9, string10, string11));
                string1 = bufferedReader.readLine();
              }
            ...
            case SkinOffsetMatrices:
              for (int int13 = 0; int13 < int4; int13++) {
                ...
                modelTxt.skinOffsetMatrices.add(
                    int13, this.getMatrix(string1, string6, string7, string8));
                string1 = bufferedReader.readLine();
              }
            ...
            case Anim:
              ...
              AnimationClip animationClip0 =
                  new AnimationClip(float15, arrayList0, string1, false);
              arrayList0.clear();
      ...
      if (!boolean0 && skinningData != nullptr) {
        try {
          ...
          std::vector arrayList2 =
              new ArrayList<>(skinningData.SkeletonHierarchy);
          ...
          for (int int23 = 0; int23 < modelTxt.vertices.m_numVertices;
               int23++) {
          ...
          modelTxt.skinOffsetMatrices = this.RemapMatrices(
              ints, modelTxt.skinOffsetMatrices, modelTxt.boneIndices.size());
          modelTxt.bindPose = this.RemapMatrices(ints, modelTxt.bindPose,
                                                 modelTxt.boneIndices.size());
          modelTxt.invBindPose = this.RemapMatrices(
              ints, modelTxt.invBindPose, modelTxt.boneIndices.size());
  ...
  void applyToMesh(ModelTxt modelTxt, ModelMesh modelMesh,
                   SkinningData skinningData) {
    if (modelTxt.bStatic) {
      if (!ModelManager.NoOpenGL) {
        ...
        RenderThread.queueInvokeOnRenderContext(()->{
          modelMesh.SetVertexBuffer(
              new VertexBufferObject(modelTxt.vertices, modelTxt.elements));
          if (ModelManager.instance.bCreateSoftwareMeshes) {
      ...
      modelMesh.skinningData =
          new SkinningData(modelTxt.clips, modelTxt.bindPose,
                           modelTxt.invBindPose, modelTxt.skinOffsetMatrices,
                           modelTxt.SkeletonHierarchy, modelTxt.boneIndices);
      if (!ModelManager.NoOpenGL) {
        modelMesh.m_bHasVBO = true;
        RenderThread.queueInvokeOnRenderContext(()->{
          modelMesh.SetVertexBuffer(new VertexBufferObject(
              modelTxt.vertices, modelTxt.elements, modelTxt.bReverse));
          if (ModelManager.instance.bCreateSoftwareMeshes) {
          }
        });
      }
  ...
  void applyToAnimation(ModelTxt modelTxt, AnimationAsset animationAsset) {
    ...
    animationAsset.assetParams.animationsMesh.skinningData.AnimationClips
        .putAll(modelTxt.clips);
  }

private
  ArrayList<Matrix4f> RemapMatrices(int[] ints, ArrayList<Matrix4f> arrayList1,
                                    int int0) {
