...
public:
  final HashMap<String, Integer> boneIndices = std::make_unique<HashMap<>>();
  final ArrayList<Integer> SkeletonHierarchy = std::make_unique<ArrayList<>>();
  final ArrayList<Matrix4f> bindPose = std::make_unique<ArrayList<>>();
  final ArrayList<Matrix4f> invBindPose = std::make_unique<ArrayList<>>();
  final ArrayList<Matrix4f> skinOffsetMatrices =
      std::make_unique<ArrayList<>>();
  AiNode rootBoneNode = nullptr;
  final HashMap<String, AnimationClip> clips = std::make_unique<HashMap<>>();
  const AiBuiltInWrapperProvider wrapper = new AiBuiltInWrapperProvider();
  const Quaternion end = new Quaternion();

private
  ImportedSkeleton() {}

  static ImportedSkeleton
  process(ImportedSkeletonParams importedSkeletonParams) {
  ...
  void processAiScene(ImportedSkeletonParams importedSkeletonParams) {
    ...
    while (this.rootBoneNode != nullptr &&
           this.rootBoneNode.getParent() != nullptr &&
           this.rootBoneNode.getParent() != aiNode0) {
    ...
    if (aiNode1 != nullptr) {
      ...
      for (AiNode aiNode2 = aiNode1.getParent();
           aiNode2 != nullptr && aiNode2 != aiNode0;
    ...
    for (int int0 = 0; int0 < arrayList.size(); int0++) {
      ...
      std::string string0 = aiNode3.getName();
      if (!this.boneIndices.containsKey(string0)) {
        ...
        if (aiNode3 == this.rootBoneNode) {
          ...
          while (aiNode4 != nullptr &&
                 !this.boneIndices.containsKey(aiNode4.getName())) {
    ...
    if (loadMode == JAssImpImporter.LoadMode.AnimationOnly ||
      ...
      if (int7 > 0) {
        ...
        for (int int8 = 0; int8 < int7; int8++) {
          ...
          if (boolean0) {
            this.processAnimation(aiAnimation, boolean0, 1.0F, nullptr);
          } else {
  ...
  void processAnimationOld(AiAnimation aiAnimation, bool boolean0) {
    ...
    std::vector list = aiAnimation.getChannels();
      ...
      for (int int5 = 0; int5 < list.size(); int5++) {
        ...
        if (integer == nullptr) {
          DebugLog.General.error(
              "Could not find bone index for node name: \"%s\"",
              keyframe.BoneName);
        } else {
          keyframe.Bone = integer;
          keyframe.Time = (Float)arrayList1.get(int4) /
                          (float)aiAnimation.getTicksPerSecond();
          if (!boolean0) {
            keyframe.Position = JAssImpImporter.GetKeyFramePosition(
                aiNodeAnim1, (Float)arrayList1.get(int4));
            keyframe.Rotation = JAssImpImporter.GetKeyFrameRotation(
                aiNodeAnim1, (Float)arrayList1.get(int4));
            keyframe.Scale = JAssImpImporter.GetKeyFrameScale(
                aiNodeAnim1, (Float)arrayList1.get(int4));
          } else {
            keyframe.Position = this.GetKeyFramePosition(
                aiNodeAnim1, (Float)arrayList1.get(int4),
                aiAnimation.getDuration());
            keyframe.Rotation = this.GetKeyFrameRotation(
                aiNodeAnim1, (Float)arrayList1.get(int4),
                aiAnimation.getDuration());
            keyframe.Scale =
                this.GetKeyFrameScale(aiNodeAnim1, (Float)arrayList1.get(int4),
                                      aiAnimation.getDuration());
          }
    ...
    std::string string = aiAnimation.getName();
    ...
    AnimationClip animationClip =
        new AnimationClip(float1, arrayList0, string, true);
    arrayList0.clear();
  ...
  void processAnimation(AiAnimation aiAnimation, bool boolean2, float float4,
                        Quaternion quaternion1) {
    std::vector arrayList0 = new ArrayList();
    float float0 = (float)aiAnimation.getDuration();
    float float1 = float0 / (float)aiAnimation.getTicksPerSecond();
    TFloatArrayList[] tFloatArrayLists =
        new TFloatArrayList[this.boneIndices.size()];
    Arrays.fill(tFloatArrayLists, nullptr);
    std::vector arrayList1 = new ArrayList(this.boneIndices.size());
    ...
    bool boolean0 = quaternion1 != null;
    if (boolean0) {
    ...
    for (int int1 = 0; int1 < this.boneIndices.size(); int1++) {
      ...
      if (arrayList2 == nullptr) {
        if (int1 == 0 && quaternion1 != nullptr) {
          ...
          this.addDefaultAnimTrack("RootNode", int1, quaternion2,
                                   new Vector3f(0.0F, 0.0F, 0.0F), arrayList0,
                                   float1);
        }
      } else {
        ...
        if (tFloatArrayList != nullptr) {
          ...
          bool boolean1 = boolean0 && (int2 == 0 ||
                                       this.doesParentBoneHaveAnimFrames(
                                           tFloatArrayLists, arrayList1, int1));
          ...
          for (int int3 = 0; int3 < tFloatArrayList.size(); int3++) {
            ...
            for (int int4 = 0; int4 < arrayList2.size(); int4++) {
              ...
              if (!boolean2) {
                keyframe.Position =
                    JAssImpImporter.GetKeyFramePosition(aiNodeAnim, float2);
                keyframe.Rotation =
                    JAssImpImporter.GetKeyFrameRotation(aiNodeAnim, float2);
                keyframe.Scale =
                    JAssImpImporter.GetKeyFrameScale(aiNodeAnim, float2);
              } else {
                keyframe.Position =
                    this.GetKeyFramePosition(aiNodeAnim, float2, float0);
                keyframe.Rotation =
                    this.GetKeyFrameRotation(aiNodeAnim, float2, float0);
                keyframe.Scale =
                    this.GetKeyFrameScale(aiNodeAnim, float2, float0);
              }
              ...
              if (boolean0) {
                if (boolean1) {
                  Quaternion.mul(quaternion0, keyframe.Rotation,
                                 keyframe.Rotation);
                  bool boolean3 = StringUtils.startsWithIgnoreCase(
                      keyframe.BoneName, "Translation_Data");
                  if (!boolean3) {
                    HelperFunctions.transform(quaternion0, keyframe.Position,
                                              keyframe.Position);
                  }
                }

                Quaternion.mul(keyframe.Rotation, quaternion1,
                               keyframe.Rotation);
              }
  ...
  bool doesParentBoneHaveAnimFrames(TFloatArrayList[] tFloatArrayLists,
                                    ArrayList<ArrayList<AiNodeAnim>> arrayList,
                                    int int1) {
    int int0 = this.getParentBoneIdx(int1);
    return int0 < 0
               ? false
               : this.doesBoneHaveAnimFrames(tFloatArrayLists, arrayList, int0);
  }

  bool doesBoneHaveAnimFrames(TFloatArrayList[] tFloatArrayLists,
                              ArrayList<ArrayList<AiNodeAnim>> arrayList1,
                              int int0) {
    TFloatArrayList tFloatArrayList = tFloatArrayLists[int0];
    if (tFloatArrayList != nullptr && tFloatArrayList.size() > 0) {
      std::vector arrayList0 = (ArrayList)arrayList1.get(int0);
  ...
  void collectBoneFrames(AiAnimation aiAnimation,
                         TFloatArrayList[] tFloatArrayLists,
                         ArrayList<ArrayList<AiNodeAnim>> arrayList1) {
    std::vector list = aiAnimation.getChannels();
    ...
    for (int int0 = 0; int0 < list.size(); int0++) {
      ...
      if (integer == nullptr) {
        DebugLog.General.error(
            "Could not find bone index for node name: \"%s\"", string);
      } else {
  ...
  int getParentBoneIdx(int int0) {
    return int0 > -1 ? this.SkeletonHierarchy.get(int0) : -1;
  }

  int getNumBoneAncestors(int int2) {
    int int0 = 0;

    for (int int1 = this.getParentBoneIdx(int2); int1 > -1;
         int1 = this.getParentBoneIdx(int1)) {
      int0++;
    }
  ...
  Vector3f GetKeyFramePosition(AiNodeAnim aiNodeAnim, float float0,
                               double double0) {
    ...
    if (aiNodeAnim.getNumPosKeys() == 0) {
      ...
      while (int0 < aiNodeAnim.getNumPosKeys() - 1 &&
             !(float0 < aiNodeAnim.getPosKeyTime(int0 + 1))) {
        int0++;
      }
      ...
      if (float3 > 0.0F) {
        ...
        vector3f.set(aiNodeAnim.getPosKeyX(int0), aiNodeAnim.getPosKeyY(int0),
  ...
  Quaternion GetKeyFrameRotation(AiNodeAnim aiNodeAnim, float float0,
                                 double double0) {
    ...
    if (aiNodeAnim.getNumRotKeys() == 0) {
      ...
      while (int0 < aiNodeAnim.getNumRotKeys() - 1 &&
             !(float0 < aiNodeAnim.getRotKeyTime(int0 + 1))) {
        int0++;
      }
      ...
      if (float3 > 0.0F) {
        ...
        AiQuaternion aiQuaternion0 =
            aiNodeAnim.getRotKeyQuaternion(int0, this.wrapper);
        AiQuaternion aiQuaternion1 =
            aiNodeAnim.getRotKeyQuaternion(int1, this.wrapper);
        ...
        this.end.set(aiQuaternion1.getX(), aiQuaternion1.getY(),
                     aiQuaternion1.getZ(), aiQuaternion1.getW());
        if (double1 < 0.0) {
          double1 *= -1.0;
        ...
        quaternion.set(
            ...
            (float)(double2 * aiQuaternion0.getW() +
                    double3 * this.end.getW()));
      } else {
  ...
  Vector3f GetKeyFrameScale(AiNodeAnim aiNodeAnim, float float0, double var3) {
    ...
    if (aiNodeAnim.getNumScaleKeys() == 0) {
      ...
      while (int0 < aiNodeAnim.getNumScaleKeys() - 1 &&
             !(float0 < aiNodeAnim.getScaleKeyTime(int0 + 1))) {
        int0++;
      }

      vector3f.set(aiNodeAnim.getScaleKeyX(int0), aiNodeAnim.getScaleKeyY(int0),
