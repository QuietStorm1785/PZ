...
private
  ...
  const ModelInstanceRenderDataList modelData =
      new ModelInstanceRenderDataList();
  const ModelInstanceRenderDataList readyModelData =
      new ModelInstanceRenderDataList();
  ...
  bool bReady;
public
  final ModelInstance.EffectLight[] effectLights =
      new ModelInstance.EffectLight[5];
  float centerOfMassY;
...
private
  static final ObjectPool<ModelSlotRenderData> pool =
      new ObjectPool<>(ModelSlotRenderData::new);
...
public
  ...
  ModelSlotRenderData init(ModelManager.ModelSlot _modelSlot) {
    ...
    if (vehicle0 != nullptr) {
      ...
      this.centerOfMassY =
          vehicle0.jniTransform.origin.y - BaseVehicle.CENTER_OF_MASS_MAGIC;
      ...
      VehicleModelInstance vehicleModelInstance =
          (VehicleModelInstance)_modelSlot.model;
      ...
      for (int int2 = 0; int2 < lightSources.length; int2++) {
        ...
        if (lightSource != nullptr) {
          Vector3f vector3f1 =
              vehicle0.getLocalPos(lightSource.x + 0.5F, lightSource.y + 0.5F,
                                   lightSource.z + 0.75F, vector3f0);
          vehicle0.fixLightbarModelLighting(lightSource, vector3f0);
          this.effectLights[int2].set(vector3f1.x, vector3f1.y, vector3f1.z,
                                      lightSource.r, lightSource.g,
                                      lightSource.b, lightSource.radius);
        }
      }

      BaseVehicle.TL_vector3f_pool.get().release(vector3f0);
      float float0 = 1.0F - Math.min(RenderSettings.getInstance()
      ...
      for (int int3 = 0; int3 < this.effectLights.length; int3++) {
        ModelInstance.EffectLight effectLight =
            playerData.effectLightsMain[int3];
        this.effectLights[int3].set(effectLight.x, effectLight.y, effectLight.z,
      ...
      this.bOutside = this.character.getCurrentSquare() != nullptr &&
                      this.character.getCurrentSquare().isOutside();
      ...
      if (this.bInVehicle) {
        ...
        this.centerOfMassY =
            vehicle1.jniTransform.origin.y - BaseVehicle.CENTER_OF_MASS_MAGIC;
        this.x = vehicle1.x;
        this.y = vehicle1.y;
        this.z = vehicle1.z;
        Vector3f vector3f2 = BaseVehicle.TL_vector3f_pool.get().alloc();
        vehicle1.getPassengerLocalPos(vehicle1.getSeat(this.character),
                                      vector3f2);
        ...
        Vector3f vector3f3 = vehicle1.vehicleTransform.getEulerAnglesZYX(
    ...
    if (_modelSlot.model.model.isReady() &&
        (_modelSlot.model.AnimPlayer == nullptr ||
         _modelSlot.model.AnimPlayer.isReady())) {
      modelInstanceRenderData0 =
          ModelInstanceRenderData.alloc().init(_modelSlot.model);
    ...
    for (int int4 = 0; int4 < this.modelData.size(); int4++) {
      ModelInstanceRenderData modelInstanceRenderData1 =
          this.modelData.get(int4);
      if (this.character != nullptr &&
          modelInstanceRenderData1.modelInstance ==
              this.character.primaryHandModel &&
          this.character.isMuzzleFlash()) {
        modelInstanceRenderData1.m_muzzleFlash = true;
      }

      if (modelInstanceRenderData1.modelInstance != nullptr &&
          modelInstanceRenderData1.modelInstance.hasTextureCreator()) {
        boolean1 = true;
      }
    }
    ...
    if (this.character != nullptr &&
        (this.textureCreator != nullptr || boolean1)) {
      assert this.readyModelData.isEmpty();

      ModelInstanceRenderData.release(this.readyModelData);
      this.readyModelData.clear();

      for (int int5 = 0; int5 < this.character.getReadyModelData().size();
           int5++) {
        ModelInstance modelInstance =
            this.character.getReadyModelData().get(int5);
        ModelInstanceRenderData modelInstanceRenderData2 =
            ModelInstanceRenderData.alloc().init(modelInstance);
        modelInstanceRenderData2.transformToParent(
            this.getParentData(modelInstance));
  ...
  initRenderData(ModelInstance modelInstance,
                 ModelInstanceRenderData modelInstanceRenderData1) {
    ModelInstanceRenderData modelInstanceRenderData0 =
        ModelInstanceRenderData.alloc().init(modelInstance);
  ...
  void initRenderData(ArrayList<ModelInstance> arrayList,
                      ModelInstanceRenderData modelInstanceRenderData) {
    for (int int0 = 0; int0 < arrayList.size(); int0++) {
      ModelInstance modelInstance = (ModelInstance)arrayList.get(int0);
      if (modelInstance.model.isReady() &&
          (modelInstance.AnimPlayer == nullptr ||
           modelInstance.AnimPlayer.isReady())) {
  ...
  void renderVehicleDebug() {
    if (Core.bDebug) {
      ...
      if (this.m_debugRenderData != nullptr && !this.modelData.isEmpty()) {
      ...
      if (DebugOptions.instance.ModelRenderAttachments.getValue()) {
        ...
        for (int int0 = 0; int0 < vehicle0.getScript().getAttachmentCount();
             int0++) {
          ModelAttachment modelAttachment =
              vehicle0.getScript().getAttachment(int0);
          modelInstanceRenderData.modelInstance.getAttachmentMatrix(
              modelAttachment, matrix4f1);
      ...
      if (Core.bDebug && DebugOptions.instance.ModelRenderAxis.getValue() &&
          !this.modelData.isEmpty()) {
        BaseVehicle vehicle1 = (BaseVehicle)this.object;
        GL11.glMatrixMode(5888);
        Vector3f vector3f = this.vehicleTransform.getEulerAnglesZYX(
            BaseVehicle.TL_vector3f_pool.get().alloc());
        GL11.glRotatef((float)java.lang.Math.toDegrees(vector3f.z), 0.0F, 0.0F,
                       1.0F);
        GL11.glRotatef((float)java.lang.Math.toDegrees(vector3f.y), 0.0F, 1.0F,
                       0.0F);
        GL11.glRotatef((float)java.lang.Math.toDegrees(vector3f.x), 1.0F, 0.0F,
                       0.0F);
        ...
        for (int int1 = 1; int1 < this.modelData.size(); int1++) {
          VehicleSubModelInstance vehicleSubModelInstance =
              Type.tryCastTo(this.modelData.get(int1).modelInstance,
                             VehicleSubModelInstance.class);
          if (vehicleSubModelInstance != nullptr &&
              vehicleSubModelInstance.modelInfo.wheelIndex >= 0) {
            float float2 = 1.0F;
            VehicleScript.Wheel wheel = vehicle1.getScript().getWheel(
                vehicleSubModelInstance.modelInfo.wheelIndex);
            uint8_t byte0 = -1;
            vector.set(wheel.offset.x * byte0,
                       vehicle1.getScript().getModel().offset.y +
                           wheel.offset.y +
                           vehicle1.getScript().getSuspensionRestLength(),
                       wheel.offset.z);
            Model.debugDrawAxis(
                vector.x / float2, vector.y / float2, vector.z / float2,
                vehicle1.getScript().getSuspensionRestLength() / float2, 2.0F);
          }
        }
      }
  ...
  void performRenderCharacter() {
    ...
    for (int int0 = 0; int0 < modelInstanceRenderDataList.size(); int0++) {
      ModelInstanceRenderData modelInstanceRenderData0 =
          modelInstanceRenderDataList.get(int0);
    ...
    if (Core.bDebug) {
      ...
      for (int int1 = 0; int1 < modelInstanceRenderDataList.size(); int1++) {
        ModelInstanceRenderData modelInstanceRenderData1 =
            modelInstanceRenderDataList.get(int1);
  ...
  void performRenderCharacterOutline() {
    ...
    solidColor.Start();
    solidColor.getShaderProgram().setVector4(
        "u_color", this.outlineColor.r, this.outlineColor.g,
        this.outlineColor.b, this.outlineColor.a);
    solidColor.End();
    solidColorStatic.Start();
    solidColorStatic.getShaderProgram().setVector4(
        "u_color", this.outlineColor.r, this.outlineColor.g,
        this.outlineColor.b, this.outlineColor.a);
    solidColorStatic.End();
    bool boolean0 =
        ModelOutlines.instance.beginRenderOutline(this.outlineColor);
    ...
    for (int int0 = 0; int0 < modelInstanceRenderDataList.size(); int0++) {
      ...
      try {
        modelInstanceRenderData.model.Effect =
            modelInstanceRenderData.model.bStatic ? solidColorStatic
                                                  : solidColor;
    ...
    if (modelInstanceTextureCreator != nullptr) {
      ...
      if (modelInstanceTextureCreator.renderRefCount <= 0) {
        if (modelInstanceTextureCreator.isRendered()) {
          ...
          if (modelInstanceTextureCreator ==
              this.character.getTextureCreator()) {
            this.character.setTextureCreator(nullptr);
          }
        } else if (modelInstanceTextureCreator !=
                   this.character.getTextureCreator()) {
          modelInstanceTextureCreator.postRender();
        }
      }
  ...
  static ModelSlotRenderData alloc() { return pool.alloc(); }
}
} // namespace model
} // namespace skinnedmodel
} // namespace core
