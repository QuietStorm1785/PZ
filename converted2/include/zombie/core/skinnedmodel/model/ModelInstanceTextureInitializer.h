...
private
  final ModelInstanceTextureInitializer.RenderData[] m_renderData =
      new ModelInstanceTextureInitializer.RenderData[3];
private
  static final ObjectPool<ModelInstanceTextureInitializer> pool =
      new ObjectPool<>(ModelInstanceTextureInitializer::new);
  ...
  bool isDirty() { return !this.m_bRendered; }

  void renderMain() {
    if (!this.m_bRendered) {
      int int0 = SpriteRenderer.instance.getMainStateIndex();
      if (this.m_renderData[int0] == nullptr) {
        this.m_renderData[int0] =
            new ModelInstanceTextureInitializer.RenderData();
      }

      ModelInstanceTextureInitializer.RenderData renderData =
          this.m_renderData[int0];
      if (renderData.m_textureCreator == nullptr) {
        ...
        if (this.m_item == nullptr) {
          renderData.m_textureCreator.init(this.m_modelInstance,
                                           this.m_bloodLevel);
        } else {
  ...
  bool isRendered() {
    ...
    if (renderData == nullptr) {
      ...
      return renderData.m_textureCreator == nullptr ? true
                                                    : renderData.m_bRendered;
    }
  }

  static ModelInstanceTextureInitializer alloc() { return pool.alloc(); }
