...
public
  Matrix4(float[] floats) {
    this();
    this.put(floats);
  }

public
  Matrix4(Matrix4 matrix41) {
    this();
    this.put(matrix41);
  }
  ...
  Matrix4 clearToOrtho(float float5, float float4, float float3, float float2,
  ...
  Matrix4 clearToPerspective(float float1, float float4, float float5,
  ...
  float get(int int0) { return this.matrix.get(int0); }
  ...
  Matrix4 put(float[] floats) {
    if (floats.length < 16) {
      throw new IllegalArgumentException(
          "float array must have at least 16 values.");
    } else {
  ...
  Matrix4 mult(float[] floats1) {
    ...
    for (byte byte0 = 0; byte0 < 16; byte0 += 4) {
      floats0[byte0 + 0] =
          this.get(0) * floats1[byte0] + this.get(4) * floats1[byte0 + 1] +
          this.get(8) * floats1[byte0 + 2] + this.get(12) * floats1[byte0 + 3];
      floats0[byte0 + 1] =
          this.get(1) * floats1[byte0] + this.get(5) * floats1[byte0 + 1] +
          this.get(9) * floats1[byte0 + 2] + this.get(13) * floats1[byte0 + 3];
      floats0[byte0 + 2] =
          this.get(2) * floats1[byte0] + this.get(6) * floats1[byte0 + 1] +
          this.get(10) * floats1[byte0 + 2] + this.get(14) * floats1[byte0 + 3];
      floats0[byte0 + 3] =
          this.get(3) * floats1[byte0] + this.get(7) * floats1[byte0 + 1] +
          this.get(11) * floats1[byte0 + 2] + this.get(15) * floats1[byte0 + 3];
    }
  ...
  Matrix4 rotate(float float1, float float4, float float5, float float6) {
    ...
    float[] floats = new float[16];
    floats[0] =
        vector3.x() * vector3.x() + (1.0F - vector3.x() * vector3.x()) * float0;
    ...
    floats[5] =
        vector3.y() * vector3.y() + (1.0F - vector3.y() * vector3.y()) * float0;
    ...
    floats[10] =
        vector3.z() * vector3.z() + (1.0F - vector3.z() * vector3.z()) * float0;
    floats[15] = 1.0F;
    return this.mult(floats);
  }
  ...
  static { Identity.clearToIdentity(); }
}
} // namespace skinnedmodel
} // namespace core
} // namespace zombie
