...
private
  final HashMap<String, Model> m_modelMap = std::make_unique<HashMap<>>();
  Model m_maleModel;
  Model m_femaleModel;
...
public
  final HashMap<String, SoftwareSkinnedModelAnim> SoftwareMeshAnims =
      std::make_unique<HashMap<>>();
private
  final ArrayList<ModelManager.ModelSlot> m_modelSlots =
      std::make_unique<ArrayList<>>();
private
  final ObjectPool<ModelInstance> m_modelInstancePool =
      new ObjectPool<>(ModelInstance::new);
private
  final ArrayList<WeaponPart> m_tempWeaponPartList =
      std::make_unique<ArrayList<>>();
  ModelMesh m_animModel;
private
  final HashMap<String, AnimationAsset> m_animationAssets =
      std::make_unique<HashMap<>>();
private
  final ModelManager.ModAnimations m_gameAnimations =
      new ModelManager.ModAnimations("game");
private
  final HashMap<String, ModelManager.ModAnimations> m_modAnimations =
      std::make_unique<HashMap<>>();
private
  final ArrayList<StaticAnimation> m_cachedAnims =
      std::make_unique<ArrayList<>>();
private
  final HashSet<IsoGameCharacter> m_contains = std::make_unique<HashSet<>>();
private
  final ArrayList<IsoGameCharacter.TorchInfo> m_torches =
      std::make_unique<ArrayList<>>();
private
  final Stack<IsoLightSource> m_freeLights = std::make_unique<Stack<>>();
private
  final ArrayList<IsoLightSource> m_torchLights =
      std::make_unique<ArrayList<>>();
private
  final ArrayList<IsoGameCharacter> ToRemove = std::make_unique<ArrayList<>>();
private
  final ArrayList<IsoGameCharacter> ToResetNextFrame =
      std::make_unique<ArrayList<>>();
private
  final ArrayList<IsoGameCharacter> ToResetEquippedNextFrame =
      std::make_unique<ArrayList<>>();
private
  final ArrayList<ModelManager.ModelSlot> m_resetAfterRender =
      std::make_unique<ArrayList<>>();
private
  final Stack<IsoLightSource> m_lights = std::make_unique<Stack<>>();
private
  final Stack<IsoLightSource> m_lightsTemp = std::make_unique<Stack<>>();
  const Vector2 m_tempVec2 = new Vector2();
  const Vector2 m_tempVec2_2 = new Vector2();
private
  static final TreeMap<String, ModelManager.ModelMetaData> modelMetaData =
  ...
  bool isCreated() { return this.m_bCreated; }
    ...
    if (!this.m_bCreated) {
      if (!GameServer.bServer || ServerGUI.isCreated()) {
        ...
        try {
          ...
          PerformanceSettings.UseFBOs = false;
          DebugLog.Animation.error(
              "FBO not compatible with gfx card at this time.");
          return;
        }
      }
      ...
      this.m_modAnimations.put(this.m_gameAnimations.m_modID,
                               this.m_gameAnimations);
      AnimationsMesh animationsMesh =
          ScriptManager.instance.getAnimationsMesh("Human");
      ModelMesh modelMesh = animationsMesh.modelMesh;
      if (!NoOpenGL && this.bCreateSoftwareMeshes) {
        SoftwareSkinnedModelAnim softwareSkinnedModelAnim =
            new SoftwareSkinnedModelAnim(
                this.m_cachedAnims.toArray(new StaticAnimation[0]),
                modelMesh.softwareMesh, modelMesh.skinningData);
        this.SoftwareMeshAnims.put(modelMesh.getPath().getPath(),
  ...
  void loadAdditionalModel(const std::string &meshName, const std::string &tex,
    ...
    Model model = this.loadModelInternal(meshName, tex, shaderName,
                                         this.m_animModel, bStatic);
    if (boolean0) {
      SoftwareSkinnedModelAnim softwareSkinnedModelAnim =
          new SoftwareSkinnedModelAnim(
              this.m_cachedAnims.toArray(new StaticAnimation[0]),
              model.softwareMesh, (SkinningData)model.Tag);
      this.SoftwareMeshAnims.put(meshName.toLowerCase(),
                                 softwareSkinnedModelAnim);
    }
  }
  ...
  void loadAnimsFromDir(const std::string &string, ModelMesh modelMesh) {
    ...
    this.loadAnimsFromDir(ZomboidFileSystem.instance.baseURI,
                          ...
                          file, modelMesh, this.m_gameAnimations);
  }

  void loadAnimsFromDir(URI uri0, URI uri1, File file0, ModelMesh modelMesh,
                        ModelManager.ModAnimations modAnimations) {
    if (!file0.exists()) {
      DebugLog.General.error("ERROR: %s", file0.getPath());

      for (File file1 = file0.getParentFile(); file1 != nullptr;
           file1 = file1.getParentFile()) {
        DebugLog.General.error(" - Parent exists: %B, %s", file1.exists(),
                               file1.getPath());
      }
    }
    ...
    if (file0.isDirectory()) {
      ...
      if (files != nullptr) {
        ...
        for (auto &file2 : files)
          if (file2.isDirectory()) {
            ...
            std::string string =
                ZomboidFileSystem.instance.getAnimName(uri1, file2);
      ...
      if (boolean0) {
        DebugFileWatcher.instance.add(
            new ModelManager
                .AnimDirReloader(uri0, uri1, file0.getPath(), modelMesh,
                                 modAnimations)
                .GetFileWatcher());
      }
    }
  ...
  PlayerCamera playerCamera =
      SpriteRenderer.instance.getRenderingPlayerCamera(userId);
  GL11.glViewport(0, 0, playerCamera.OffscreenWidth,
                  playerCamera.OffscreenHeight);
  switch (apiId) {
  case 1:
...
void Reset(IsoGameCharacter chr) {
  if (chr.legsSprite != nullptr && chr.legsSprite.modelSlot != nullptr) {
    ...
    for (int int0 = 0; int0 < modelSlot.sub.size(); int0++) {
      ...
      if (modelInstance != chr.primaryHandModel &&
          modelInstance != chr.secondaryHandModel &&
          !modelSlot.attachedModels.contains(modelInstance)) {
    ...
    modelSlot.model.m_modelScript = ScriptManager.instance.getModelScript(
        chr.isFemale() ? "FemaleBody" : "MaleBody");
      ...
      if (this.ToRemove.contains(chr)) {
        ...
        if (modelSlot.model != nullptr) {
          RenderThread.invokeOnRenderContext(
              modelSlot.model::destroySmartTextures);
        }

        this.dressInRandomOutfit(chr);
        Model model = this.getBodyModel(chr);
        modelSlot.model =
            this.newInstance(model, chr, chr.getAnimationPlayer());
        modelSlot.model.setOwner(modelSlot);
        modelSlot.model.m_modelScript = ScriptManager.instance.getModelScript(
            chr.isFemale() ? "FemaleBody" : "MaleBody");
        this.DoCharacterModelParts(chr, modelSlot);
        ...
        modelSlot.model.SetForceDir(
            modelSlot.model.character.getForwardDirection());
        ...
        if (modelSlot.model.AnimPlayer != nullptr &&
            modelSlot.model.AnimPlayer.isBoneTransformsNeedFirstFrame()) {
          try {
            modelSlot.Update();
          } catch (Throwable throwable) {
...
void dressInRandomOutfit(IsoGameCharacter chr) {
  ...
  if (zombie0 != nullptr && !zombie0.isReanimatedPlayer() &&
      !zombie0.wasFakeDead()) {
    if (DebugOptions.instance.ZombieOutfitRandom.getValue() &&
        !chr.isPersistentOutfitInit()) {
    ...
    if (GameClient.bClient && zombie0 != nullptr &&
        !chr.isPersistentOutfitInit() && chr.getPersistentOutfitID() != 0) {
      zombie0.dressInPersistentOutfitID(chr.getPersistentOutfitID());
    }
  }
}
...
Model getBodyModel(IsoGameCharacter chr) {
  if (chr.isZombie() && ((IsoZombie)chr).isSkeleton()) {
    return chr.isFemale() ? this.m_skeletonFemaleModel
                          : this.m_skeletonMaleModel;
  } else {
...
/**
 * Returns TRUE if the character is currently in the visible render list, and
 * has not been flagged for removal.
 */
...
void ResetCharacterEquippedHands(IsoGameCharacter chr) {
  if (chr != nullptr && chr.legsSprite != nullptr &&
      chr.legsSprite.modelSlot != nullptr) {
    this.DoCharacterModelEquipped(chr, chr.legsSprite.modelSlot);
  }
}

void DoCharacterModelEquipped(IsoGameCharacter character,
  ...
  for (int int1 = 0; int1 < character.getAttachedItems().size(); int1++) {
    ...
    if (!StringUtils.isNullOrWhitespace(string0)) {
      std::string string1 = character.getAttachedItems()
                                .getGroup()
                                .getLocation(attachedItem.getLocation())
                                .getAttachmentName();
      ModelInstance modelInstance1 =
          this.addStatic(modelSlot.model, string0, string1, string1);
      if (modelInstance1 != nullptr) {
        modelInstance1.setOwner(modelSlot);
        modelSlot.sub.add(modelInstance1);
        HandWeapon weapon =
            Type.tryCastTo(attachedItem.getItem(), HandWeapon.class);
        if (weapon != nullptr) {
          ...
          if (!Core.getInstance().getOptionSimpleWeaponTextures()) {
            ...
            modelInstance1.setTextureInitializer(
  ...
  bool boolean0 = false;
  BaseAction baseAction = character.getCharacterActions().isEmpty()
                              ? null
                              : character.getCharacterActions().get(0);
  if (baseAction != nullptr && baseAction.overrideHandModels) {
    ...
    if (baseAction.getPrimaryHandItem() != nullptr) {
      ...
      character.primaryHandModel = this.addStatic(
          modelSlot, baseAction.getPrimaryHandMdl(), "Bip01_Prop1");
    }
    ...
    if (baseAction.getSecondaryHandItem() != nullptr) {
      ...
      character.secondaryHandModel = this.addStatic(
          modelSlot, baseAction.getSecondaryHandMdl(), "Bip01_Prop2");
    }
  }
  ...
  if (!StringUtils.isNullOrEmpty(character.overridePrimaryHandModel)) {
    boolean0 = true;
    character.primaryHandModel = this.addStatic(
        modelSlot, character.overridePrimaryHandModel, "Bip01_Prop1");
  }

  if (!StringUtils.isNullOrEmpty(character.overrideSecondaryHandModel)) {
    boolean0 = true;
    character.secondaryHandModel = this.addStatic(
        modelSlot, character.overrideSecondaryHandModel, "Bip01_Prop2");
  }

  if (item0 != nullptr) {
    ItemReplacement itemReplacement0 = item0.getItemReplacementPrimaryHand();
    character.primaryHandModel = this.addEquippedModelInstance(
        character, modelSlot, item0, "Bip01_Prop1", itemReplacement0, boolean0);
  }

  if (item1 != nullptr && item0 != item1) {
    ItemReplacement itemReplacement1 = item1.getItemReplacementSecondHand();
    character.secondaryHandModel = this.addEquippedModelInstance(
        character, modelSlot, item1, "Bip01_Prop2", itemReplacement1, boolean0);
  }
}

private
ModelInstance addEquippedModelInstance(IsoGameCharacter character,
                                       ModelManager.ModelSlot modelSlot,
                                       InventoryItem item, String string1,
  ...
  if (weapon != nullptr) {
    ...
    if (item != nullptr) {
      if (itemReplacement != nullptr &&
          ...
          (itemReplacement.clothingItem != nullptr ||
           !StringUtils.isNullOrWhitespace(item.getStaticModel()))) {
        return this.addMaskingModel(modelSlot, character, item, itemReplacement,
                                    itemReplacement.maskVariableValue,
                                    itemReplacement.attachment, string1);
      }
...
private
ModelInstance addMaskingModel(ModelManager.ModelSlot modelSlot,
                              IsoGameCharacter character, InventoryItem item,
                              ItemReplacement itemReplacement, String string3,
                              String string1, String string2) {
  ModelInstance modelInstance = null;
  ItemVisual itemVisual = item.getVisual();
  if (itemReplacement.clothingItem != nullptr && itemVisual != nullptr) {
    modelInstance = PopTemplateManager.instance.addClothingItem(
        character, modelSlot, itemVisual, itemReplacement.clothingItem);
  } else {
    ...
    if (itemVisual != nullptr && item.getClothingItem() != nullptr) {
      string0 = item.getClothingItem().getTextureChoices().get(
          itemVisual.getTextureChoice());
    }

    if (!StringUtils.isNullOrEmpty(string1)) {
      modelInstance = this.addStaticForcedTex(
          modelSlot.model, item.getStaticModel(), string1, string1, string0);
    } else {
      modelInstance = this.addStaticForcedTex(modelSlot, item.getStaticModel(),
                                              string2, string0);
    }
...
void addWeaponPartModels(ModelManager.ModelSlot modelSlot, HandWeapon weapon,
                         ModelInstance modelInstance1) {
  std::vector arrayList0 = weapon.getModelWeaponPart();
  if (arrayList0 != nullptr) {
    std::vector arrayList1 =
        weapon.getAllWeaponParts(this.m_tempWeaponPartList);
      ...
      for (int int1 = 0; int1 < arrayList0.size(); int1++) {
        ...
        if (weaponPart.getFullType() == modelWeaponPart.partType)) {
            ModelInstance modelInstance0 =
                this.addStatic(modelInstance1, modelWeaponPart.modelName,
...
void resetModelInstanceRecurse(ModelInstance modelInstance,
                               void *expectedOwner) {
  if (modelInstance != nullptr) {
...
void resetModelInstancesRecurse(ArrayList<ModelInstance> modelInstances,
                                void *expectedOwner) {
...
void DoCharacterModelParts(IsoGameCharacter character,
                           ModelManager.ModelSlot modelSlot) {
...
void update() {
  ...
  for (int int5 = 0; int5 < this.m_torches.size(); int5++) {
    ...
    IsoLightSource lightSource =
        this.m_freeLights.isEmpty()
            ? new IsoLightSource(0, 0, 0, 1.0F, 1.0F, 1.0F, 1)
            : this.m_freeLights.pop();
...
private
ModelManager.ModelSlot addNewSlot(IsoGameCharacter character) {
  ModelManager.ModelSlot modelSlot =
      new ModelManager.ModelSlot(this.m_modelSlots.size(), nullptr, character);
  this.m_modelSlots.add(modelSlot);
  return modelSlot;
}

public
ModelManager.ModelSlot getSlot(IsoGameCharacter chr) {
  ...
  if (!chr.isSceneCulled()) {
    if (!this.ToRemove.contains(chr)) {
      ...
      throw new IllegalStateException(
          "IsoGameCharacter.isSceneCulled() = true inconsistent with "
          "ModelManager.ContainsChar() = true");
    }
  }
}
...
void ResetEquippedNextFrame(IsoGameCharacter isoGameCharacter) {
  ...
  try {
    if (!this.m_contains.isEmpty()) {
      IsoGameCharacter[] characters =
          this.m_contains.toArray(new IsoGameCharacter[0]);

      for (auto &character1 : characters)
  ...
  exception.printStackTrace();
    ...
    void getClosestThreeLights(IsoMovingObject movingObject,
                               IsoLightSource[] lightSources) {
      this.m_lightsTemp.clear();

      for (IsoLightSource lightSource : this.m_lights) {
        if (lightSource.bActive && lightSource.life != 0 &&
            (lightSource.localToBuilding == nullptr ||
             movingObject.getCurrentBuilding() ==
                 lightSource.localToBuilding) &&
            !(IsoUtils.DistanceTo(movingObject.x, movingObject.y,
                                  lightSource.x + 0.5F, lightSource.y + 0.5F) >=
              lightSource.radius) &&
            LosUtil.lineClear(IsoWorld.instance.CurrentCell,
                              (int)movingObject.x, (int)movingObject.y,
                              (int)movingObject.z, lightSource.x, lightSource.y,
                              lightSource.z,
      ...
      if (movingObject instanceof BaseVehicle) {
        for (int int0 = 0; int0 < this.m_torches.size(); int0++) {
          ...
          if (!(IsoUtils.DistanceTo(movingObject.x, movingObject.y, torchInfo.x,
                                    torchInfo.y) >= torchInfo.dist) &&
              LosUtil.lineClear(IsoWorld.instance.CurrentCell,
                                (int)movingObject.x, (int)movingObject.y,
                                (int)movingObject.z, (int)torchInfo.x,
                                (int)torchInfo.y, (int)torchInfo.z,
                                false) != LosUtil.TestResults.Blocked) {
            if (torchInfo.bCone) {
      ...
      PZArrayUtil.sort(
          this.m_lightsTemp,
          Lambda.comparator(
              movingObject, (lightSource0, lightSource1, movingObjectx)->{
                float float0x =
                    movingObjectx.DistTo(lightSource0.x, lightSource0.y);
                float float1 =
                    movingObjectx.DistTo(lightSource1.x, lightSource1.y);
                if (float0x > float1) {
                  return 1;
                } else {
      ...
      if (this.m_bCreated) {
        if (!GameServer.bServer || ServerGUI.isCreated()) {
          if (vehicle != nullptr && vehicle.getScript() != nullptr) {
            ...
            if (model0 == nullptr) {
              DebugLog.Animation.error("Failed to find vehicle model: %s",
                                       string);
            } else {
              if (DebugLog.isEnabled(DebugType.Animation)) {
              ...
              VehicleModelInstance vehicleModelInstance =
                  new VehicleModelInstance();
              vehicleModelInstance.init(model0, nullptr,
                                        vehicle.getAnimationPlayer());
              vehicleModelInstance.applyModelScriptScale(string);
              vehicle.getSkin();
              VehicleScript.Skin skin = vehicleScript.getTextures();
              if (vehicle.getSkinIndex() >= 0 &&
                  vehicle.getSkinIndex() < vehicleScript.getSkinCount()) {
              ...
              if (vehicleModelInstance.tex != nullptr) {
                ...
                DebugLog.Animation.error("texture not found:",
                                         vehicle.getSkin());
              }
              ...
              for (int int0 = 0; int0 < vehicle.models.size(); int0++) {
                ...
                if (model1 == nullptr) {
                  DebugLog.Animation.error("vehicle.models[%d] not found: %s",
                                           int0, modelInfo.scriptModel.file);
                } else {
                  VehicleSubModelInstance vehicleSubModelInstance =
                      new VehicleSubModelInstance();
                  vehicleSubModelInstance.init(model1, nullptr,
                                               modelInfo.getAnimationPlayer());
                  vehicleSubModelInstance.setOwner(modelSlot);
                  vehicleSubModelInstance.applyModelScriptScale(
                      modelInfo.scriptModel.file);
    ...
    ModelInstance newStaticInstance(ModelManager.ModelSlot slot,
      ...
      Model model =
          this.tryGetLoadedModel(meshName, texName, true, shaderName, false);
      if (model == nullptr && meshName != nullptr) {
        ...
        if (model == nullptr) {
          if (DebugLog.isEnabled(DebugType.Animation)) {
            DebugLog.Animation.error("Model not found. model:" + meshName +
                                     " tex:" + texName);
          }
      ...
      if (meshName == nullptr) {
        model = this.tryGetLoadedModel("vehicles_wheel02",
                                       "vehicles/vehicle_wheel02", true,
                                       "vehiclewheel", false);
      }

      ModelInstance modelInstance =
          this.newInstance(model, slot.character, slot.model.AnimPlayer);
      modelInstance.parent = slot.model;
      if (slot.model.AnimPlayer != nullptr) {
        modelInstance.parentBone = slot.model.AnimPlayer.getSkinningBoneIndex(
            boneName, modelInstance.parentBone);
    ...
    ModelInstance addStaticForcedTex(ModelManager.ModelSlot modelSlot,
      ...
      if (modelScript != nullptr) {
        ...
        ModelInstance modelInstance =
            this.addStatic(modelSlot, string0, string2, string5, string3);
        if (modelInstance != nullptr) {
    ...
    ModelInstance addStaticForcedTex(ModelInstance parentInst,
      ...
      ModelScript modelScript =
          ScriptManager.instance.getModelScript(modelName);
      if (modelScript != nullptr) {
        string0 = modelScript.getMeshName();
      ...
      Model model =
          this.tryGetLoadedModel(string0, string1, true, string2, false);
      if (model == nullptr && string0 != nullptr) {
        ...
        if (model == nullptr) {
          if (DebugLog.isEnabled(DebugType.Animation)) {
            DebugLog.Animation.error("Model not found. model:" + string0 +
                                     " tex:" + string1);
          }
      ...
      if (model == nullptr) {
        ...
        if (parentInst != nullptr) {
          modelInstance.init(model, parentInst.character,
                             parentInst.AnimPlayer);
    ...
    Model loadModelInternal(const std::string &string1,
      ...
      if (string0 != nullptr &&
          StringUtils.startsWithIgnoreCase(string0, "vehicle")) {
      ...
      std::string string3 =
          this.createModelKey(string1, string2, boolean0, string0);
      Model model = (Model)ModelAssetManager.instance.load(
          new AssetPath(string3), modelAssetParams);
      if (model != nullptr) {
    ...
    Model getLoadedModel(const std::string &meshName) {
      ...
      if (modelScript != nullptr) {
        if (modelScript.loadedModel != nullptr) {
          ...
          modelScript.shaderName =
              this.modifyShaderName(modelScript.shaderName);
          Model model0 = this.tryGetLoadedModel(
              modelScript.getMeshName(), modelScript.getTextureName(),
              modelScript.bStatic, modelScript.getShaderName(), false);
          if (model0 != nullptr) {
            modelScript.loadedModel = model0;
            return model0;
          } else {
            AnimationsMesh animationsMesh =
                modelScript.animationsMesh == nullptr
                    ? nullptr
                    : ScriptManager.instance.getAnimationsMesh(
                          modelScript.animationsMesh);
            ModelMesh modelMesh =
                animationsMesh == null ? null : animationsMesh.modelMesh;
            model0 = modelScript.bStatic
                         ? this.loadModelInternal(modelScript.getMeshName(),
        ...
        if (modelMetaDatax != nullptr) {
          modelMetaDatax.shaderName =
              this.modifyShaderName(modelMetaDatax.shaderName);
          Model model1 = this.tryGetLoadedModel(
              modelMetaDatax.meshName, modelMetaDatax.textureName,
              modelMetaDatax.bStatic, modelMetaDatax.shaderName, false);
          if (model1 != nullptr) {
            return model1;
          } else {
            return modelMetaDatax.bStatic
                       ? this.loadStaticModel(modelMetaDatax.meshName,
                                              modelMetaDatax.textureName,
                                              modelMetaDatax.shaderName)
                       : this.loadModel(modelMetaDatax.meshName,
                                        modelMetaDatax.textureName,
                                        this.m_animModel);
          }
        } else {
          Model model2 =
              this.tryGetLoadedModel(meshName, null, false, null, false);
          if (model2 != nullptr) {
            return model2;
          } else {
            ...
            for (Entry entry : this.m_modelMap.entrySet()) {
              ...
              if (string1.startsWith(string0)) {
                ...
                if (model3 != nullptr &&
                    (string1.length() == string0.length() ||
                     string1.charAt(string0.length()) == '&')) {
                  model2 = model3;
                  break;
                }
              }
            ...
            if (model2 == nullptr && DebugLog.isEnabled(DebugType.Animation)) {
              DebugLog.Animation.error(
                  "ModelManager.getLoadedModel> Model missing for key=\"" +
                  string0 + "\"");
            }
    ...
    Model tryGetLoadedModel(const std::string &meshName, const std::string &tex,
      ...
      if (string == nullptr) {
        ...
        if (model == nullptr && logError &&
            DebugLog.isEnabled(DebugType.Animation)) {
          DebugLog.Animation.error(
              "ModelManager.getLoadedModel> Model missing for key=\"" + string +
              "\"");
        }
    ...
    void putLoadedModel(const std::string &name, const std::string &tex,
      ...
      if (string != nullptr) {
        ...
        if (_model != model) {
          if (_model != nullptr) {
            DebugLog.Animation.debugln("Override key=\"%s\" old=%s new=%s",
                                       string, _model, model);
          } else {
    ...
    std::string createModelKey(const std::string &string0,
      ...
      if (string0 == nullptr) {
        ...
        if (!toLowerKeyRoot.containsKey(string0)) {
          toLowerKeyRoot.put(string0,
                             string0.toLowerCase(Locale.ENGLISH).trim());
        }
    ...
    AnimationAsset loadAnim(const std::string &string, ModelMesh modelMesh,
      ...
      AnimationAsset.AnimationAssetParams animationAssetParams =
          new AnimationAsset.AnimationAssetParams();
      animationAssetParams.animationsMesh = modelMesh;
      AnimationAsset animationAsset =
          (AnimationAsset)AnimationAssetManager.instance.load(
              new AssetPath(string), animationAssetParams);
    ...
    void putAnimationAsset(const std::string &string1,
      ...
      AnimationAsset animationAsset0 =
          modAnimations.m_animationAssetMap.getOrDefault(string0, null);
      if (animationAsset0 != nullptr) {
        DebugLog.Animation.debugln("Overwriting asset: %s",
                                   this.animAssetToString(animationAsset0));
        DebugLog.Animation.debugln("New asset        : %s",
                                   this.animAssetToString(animationAsset1));
    ...
    std::string animAssetToString(AnimationAsset animationAsset) {
      if (animationAsset == nullptr) {
        ...
        return assetPath = =
                   null ? "null-path" : String.valueOf(assetPath.getPath());
      }
    }
    ...
    AnimationAsset getAnimationAssetRequired(const std::string &string) {
      ...
      if (animationAsset == nullptr) {
        throw new NullPointerException("Required Animation Asset not found: " +
                                       string);
      } else {
        return animationAsset;
      }
  ...
  public
    Collection<AnimationClip> getAllAnimationClips() {
      return this.m_animModel.skinningData.AnimationClips.values();
    }

    ModelInstance newInstance(Model model, IsoGameCharacter chr,
    ...
    void loadModAnimations() {
      for (ModelManager.ModAnimations modAnimations0 :
           this.m_modAnimations.values()) {
        modAnimations0.setPriority(
            modAnimations0 == this.m_gameAnimations ? 0 : -1);
      }
      ...
      for (int int0 = 0; int0 < arrayList1.size(); int0++) {
        ...
        if (mod != nullptr && mod.animsXFile.isDirectory()) {
          ModelManager.ModAnimations modAnimations1 =
              this.m_modAnimations.get(string0);
          if (modAnimations1 != nullptr) {
            ...
            for (auto &animationsMesh : arrayList0)
                ...
                if (animationsMesh.modelMesh.isReady()) {
                  ...
                  if (file.exists()) {
                    this.loadAnimsFromDir(
                        mod.baseFile.toURI(), mod.mediaFile.toURI(), file,
                        animationsMesh.modelMesh, modAnimations1);
                  }
                }
              }
          }
    ...
    void setActiveAnimations() {
      ...
      for (ModelManager.ModAnimations modAnimations :
           this.m_modAnimations.values()) {
        if (modAnimations.isActive()) {
          for (AnimationAsset animationAsset0 :
               modAnimations.m_animationAssetList) {
            AnimationAsset animationAsset1 =
                this.m_animationAssets.get(animationAsset0.modelManagerKey);
            if (animationAsset1 == nullptr ||
                animationAsset1 == animationAsset0 ||
                animationAsset1.modAnimations.m_priority <=
                    modAnimations.m_priority) {
              this.m_animationAssets.put(animationAsset0.modelManagerKey,
                                         animationAsset0);
              if (animationAsset0.isReady()) {
                animationAsset0.skinningData.AnimationClips.putAll(
                    animationAsset0.AnimationClips);
              }
            }
          }
        }
    ...
    void animationAssetLoaded(AnimationAsset animationAsset) {
      if (animationAsset.modAnimations.isActive()) {
        AnimationAsset _animationAsset =
            this.m_animationAssets.get(animationAsset.modelManagerKey);
        if (_animationAsset == nullptr || _animationAsset == animationAsset ||
            _animationAsset.modAnimations.m_priority <=
                animationAsset.modAnimations.m_priority) {
          this.m_animationAssets.put(animationAsset.modelManagerKey,
                                     animationAsset);
          animationAsset.skinningData.AnimationClips.putAll(
              animationAsset.AnimationClips);
        }
      }
    }
    ...
    void initAnimationMeshes(bool bReloading) {
      ...
      for (auto &animationsMesh0 : arrayList)
        ModelMesh.MeshAssetParams meshAssetParams =
            new ModelMesh.MeshAssetParams();
      meshAssetParams.bStatic = false;
      meshAssetParams.animationsMesh = nullptr;
      animationsMesh0.modelMesh =
          (ModelMesh)MeshAssetManager.instance.getAssetTable().get(
              animationsMesh0.meshFile);
      if (animationsMesh0.modelMesh == nullptr) {
        animationsMesh0.modelMesh = (ModelMesh)MeshAssetManager.instance.load(
            new AssetPath(animationsMesh0.meshFile), meshAssetParams);
      }
    ...
    if (!bReloading) {
      ...
      for (auto &animationsMesh1 : arrayList)
        for (String string : animationsMesh1.animationDirectories) {
          if (animationsMesh1.modelMesh.isReady()) {
            File file = new File(ZomboidFileSystem.instance.base,
                                 "media/anims_X/" + string);
            if (file.exists()) {
              this.loadAnimsFromDir("media/anims_X/" + string,
                                    animationsMesh1.modelMesh);
            }
          }
        }
    }
    ...
    bool isLoadingAnimationMeshes() {
      for (AnimationsMesh animationsMesh :
           ScriptManager.instance.getAllAnimationsMeshes()) {
        if (!animationsMesh.modelMesh.isFailure() &&
            !animationsMesh.modelMesh.isReady()) {
          return true;
        }
      }
    ...
    void loadHumanAnimations(ChooseGameInfo.Mod mod,
                             ModelManager.ModAnimations modAnimations) {
      AnimationsMesh animationsMesh =
          ScriptManager.instance.getAnimationsMesh("Human");
      if (animationsMesh != nullptr && animationsMesh.modelMesh != nullptr &&
          animationsMesh.modelMesh.isReady()) {
          ...
          for (auto &file : files)
            if (file.isDirectory()) {
              if (!this.isAnimationsMeshDirectory(file.getName())) {
                this.loadAnimsFromDir(mod.baseFile.toURI(),
                                      mod.mediaFile.toURI(), file,
                                      animationsMesh.modelMesh, modAnimations);
              }
            } else {
              std::string string =
                  ZomboidFileSystem.instance.getAnimName(uri, file);
    ...
    class AnimDirReloader implements
    ...
    public
      AnimDirReloader(URI uri0, URI uri1, const std::string &string,
        ...
        this.m_dirAbsolute = ZomboidFileSystem.instance.normalizeFolderPath(
            new File(new File(this.m_baseURI), this.m_dir).toString());
        if (this.m_dir.contains("/anims/")) {
          this.m_dirSecondary = this.m_dir.replace("/anims/", "/anims_X/");
          this.m_dirSecondaryAbsolute =
              ZomboidFileSystem.instance.normalizeFolderPath(
                  new File(new File(this.m_baseURI), this.m_dirSecondary)
                      .toString());
        }
      ...
      bool IsInDir(const std::string &string) {
        ...
        try {
          return this.m_dirSecondary == nullptr
                     ...
                     : string.startsWith(this.m_dirAbsolute) ||
                           string.startsWith(this.m_dirSecondaryAbsolute);
        } catch (Exception exception) {
          exception.printStackTrace();
          return false;
        }
      ...
      void call(const std::string &string1) {
        ...
        if (string0.endsWith(".fbx") || string0.endsWith(".x") ||
          ...
          std::string string2 = ZomboidFileSystem.instance.getAnimName(
              this.m_mediaURI, new File(string1));
          AnimationAsset animationAsset =
              ModelManager.this.getAnimationAsset(string2);
          if (animationAsset != nullptr) {
            if (!animationAsset.isEmpty()) {
              DebugLog.General.debugln(
                  "Reloading animation: %s",
                  ModelManager.this.animAssetToString(animationAsset));
              ...
              AnimationAssetManager.instance.reload(animationAsset,
                                                    animationAssetParams);
            }
          } else {
            ModelManager.this.loadAnim(string2, this.m_animationsModel,
                                       this.m_modAnimations);
          }
        }
      }
  ...
  public
    static final class ModAnimations {
      const std::string m_modID;
    public
      final ArrayList<AnimationAsset> m_animationAssetList =
          std::make_unique<ArrayList<>>();
    public
      final HashMap<String, AnimationAsset> m_animationAssetMap =
          std::make_unique<HashMap<>>();
      int m_priority;
    ...
    public
      ...
      bool isActive() { return this.m_priority != -1; }
    }
    ...
    public
      final ArrayList<ModelInstance> sub = std::make_unique<ArrayList<>>();
    ...
    public
      ModelSlot(int _ID, ModelInstance _model, IsoGameCharacter _character) {
        this.ID = _ID;
        this.model = _model;
      ...
      void reset() {
        ...
        if (this.character != nullptr) {
          ...
          ModelManager.instance.derefModelInstances(
              this.character.getReadyModelData());
