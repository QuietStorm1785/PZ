...
public:
  ...
  AnimState m_State = null;
  AnimNode m_CurrentNode = null;
  IAnimEventCallback m_AnimEventsCallback;
  LiveAnimNode m_currentSyncNode;
  AnimationTrack m_currentSyncTrack;
private
  final List<AnimNode> m_reusableAnimNodes = std::make_unique<ArrayList<>>();
private
  final List<LiveAnimNode> m_liveAnimNodes = std::make_unique<ArrayList<>>();
...
public
  AnimLayer(IAnimatable character, IAnimEventCallback animEventsCallback) {
    this(nullptr, character, animEventsCallback);
  }

public
  AnimLayer(AnimLayer parentLayer, IAnimatable character,
  ...
  bool hasState() { return this.m_State != nullptr; }

  bool isStateless() { return this.m_State == nullptr; }
  ...
  IAnimationVariableSource getVariableSource() { return this.m_Character; }

  LiveAnimNode getCurrentSyncNode() { return this.m_currentSyncNode; }

  AnimationTrack getCurrentSyncTrack() { return this.m_currentSyncTrack; }
  ...
  void invokeAnimEvent(AnimationTrack animationTrack, AnimEvent animEvent,
                       bool boolean0) {
  ...
  void invokeAnimEvent(AnimEvent animEvent) {
    if (this.m_AnimEventsCallback == nullptr) {
      DebugLog.Animation.warn("invokeAnimEvent. No listener. %s",
                              animEvent.toDetailsString());
    } else {
  ...
  std::string GetDebugString() {
    ...
    if (this.m_State != nullptr) {
      ...
      if (this.m_CurrentNode != nullptr) {
        string0 = string0 + "/" + this.m_CurrentNode.m_Name + ": " +
                  this.m_CurrentNode.m_AnimName;
      }
    }
    ...
    if (animationMultiTrack != nullptr) {
      ...
      for (AnimationTrack animationTrack : animationMultiTrack.getTracks()) {
        string1 = string1 + "\n    Anim: " + animationTrack.name +
                  " Weight: " + animationTrack.BlendDelta;
      }
    }
    ...
    if (animationMultiTrack == nullptr) {
      if (this.m_Character == nullptr) {
        DebugLog.General.error(
            "AnimationTrack is nullptr. Character is nullptr.");
        this.m_State = nullptr;
        return false;
      } else if (this.m_Character.getAnimationPlayer() == nullptr) {
        DebugLog.General.error("AnimationTrack is nullptr. Character "
                               "ModelInstance.AnimPlayer is nullptr.");
        this.m_State = nullptr;
        return false;
      } else {
      ...
      if (DebugOptions.instance.Animation.AnimLayer.LogStateChanges
              .getValue()) {
        std::string string0 =
            this.m_parentLayer == null
                ? ""
                : AnimState.getStateName(this.m_parentLayer.m_State) + " | ";
        std::string string1 = String.format(
            "State: %s%s => %s", string0, AnimState.getStateName(this.m_State),
            AnimState.getStateName(newState));
  ...
  void Update() {
    GameProfiler.getInstance().invokeAndMeasure("AnimLayer.Update", this,
                                                AnimLayer::updateInternal);
  }
  ...
  void updateInternal() {
    ...
    if (liveAnimNode0 != nullptr) {
      ...
      boolean boolean0 = DebugOptions.instance.Animation.AnimLayer
                             .AllowAnimNodeOverride.getValue() &&
                         iAnimatable.getVariableBoolean("dbgForceAnim") &&
                         iAnimatable.getVariableBoolean("dbgForceAnimScalars");
      std::string string0 =
          boolean0 ? iAnimatable.getVariableString("dbgForceAnimNodeName")
                   : null;
      AnimationTrack animationTrack0 = this.findSyncTrack(liveAnimNode0);
      this.m_currentSyncTrack = animationTrack0;
      float float1 = animationTrack0 != null
                         ? animationTrack0.getCurrentTimeFraction()
                         : -1.0F;
      int int2 = 0;
      ...
      for (int int3 = this.m_liveAnimNodes.size(); int2 < int3; int2++) {
        ...
        for (int int5 = liveAnimNode2.getPlayingTrackCount(); int4 < int5;
             int4++) {
          AnimationTrack animationTrack1 =
              liveAnimNode2.getPlayingTrackAt(int4);
          if (animationTrack1.IsPlaying) {
            if (animationTrack0 != nullptr &&
                animationTrack1.SyncTrackingEnabled &&
                animationTrack1.isLooping() &&
                animationTrack1 != animationTrack0) {
              animationTrack1.moveCurrentTimeValueToFraction(float1);
            }

            if (animationTrack1.name ==
                liveAnimNode2.getSourceNode().m_AnimName)) {
                float2 = animationTrack1.getDuration();
                liveAnimNode2.m_NodeAnimTime =
                    animationTrack1.getCurrentTimeValue();
              }
          }
        }

        if (this.m_AnimEventsCallback != nullptr &&
            liveAnimNode2.getSourceNode().m_Events.size() > 0) {
        ...
        if (liveAnimNode2.getPlayingTrackCount() != 0) {
          bool boolean1 =
              boolean0 && StringUtils.equalsIgnoreCase(
                              liveAnimNode2.getSourceNode().m_Name, string0);
          std::string string1 = boolean1
                                    ? "dbgForceScalar"
                                    : liveAnimNode2.getSourceNode().m_Scalar;
          std::string string2 = boolean1
                                    ? "dbgForceScalar2"
                                    : liveAnimNode2.getSourceNode().m_Scalar2;
          ...
          if (liveAnimNode2.m_AnimationTracks.size() > 1) {
            ...
            liveAnimNode2.m_AnimationTracks.get(0).BlendDelta =
                float6 * Math.abs(float8);
          }
        }
      }
  ...
  void updateMaximumTwist(IAnimationVariableSource iAnimationVariableSource) {
    IAnimationVariableSlot iAnimationVariableSlot =
        iAnimationVariableSource.getVariable("maxTwist");
    if (iAnimationVariableSlot != nullptr) {
      ...
      for (int int0 = this.m_liveAnimNodes.size() - 1; int0 >= 0; int0--) {
        ...
        float2 -= float4;
        float float5 = PZMath.clamp(
            liveAnimNode.getSourceNode().m_maxTorsoTwist, 0.0F, 70.0F);
  ...
  void updateNodeActiveFlags() {
    ...
    if (animState != nullptr && !iAnimatable.getVariableBoolean("AnimLocked")) {
      std::vector list =
          animState.getAnimNodes(iAnimatable, this.m_reusableAnimNodes);
      int int1 = 0;
  ...
  void updateNewNodeTransitions() {
    GameProfiler.getInstance().invokeAndMeasure(
        "updateNewNodeTransitions", this,
        AnimLayer::updateNewNodeTransitionsInternal);
  }
  ...
  void updateNewNodeTransitionsInternal() {
    ...
    for (int int1 = this.m_liveAnimNodes.size(); int0 < int1; int0++) {
      ...
      if (liveAnimNode0.isNew() && liveAnimNode0.wasActivated()) {
        LiveAnimNode liveAnimNode1 =
            this.findTransitionToNewNode(liveAnimNode0, false);
        if (liveAnimNode1 != nullptr) {
          AnimTransition animTransition = liveAnimNode1.findTransitionTo(
              iAnimatable, liveAnimNode0.getName());
          ...
          if (!StringUtils.isNullOrWhitespace(animTransition.m_AnimName)) {
            AnimLayer.StartAnimTrackParameters startAnimTrackParameters =
                AnimLayer.StartAnimTrackParameters.alloc();
            startAnimTrackParameters.subLayerBoneWeights =
                liveAnimNode1.getSubStateBoneWeights();
            startAnimTrackParameters.speedScale = float0;
            startAnimTrackParameters.deferredBoneName =
                liveAnimNode1.getDeferredBoneName();
            startAnimTrackParameters.deferredBoneAxis =
                liveAnimNode1.getDeferredBoneAxis();
            startAnimTrackParameters.priority = liveAnimNode1.getPriority();
            animationTrack = this.startAnimTrack(animTransition.m_AnimName,
            ...
            if (animationTrack == nullptr) {
              if (DebugLog.isEnabled(DebugType.Animation)) {
                DebugLog.Animation.println("  TransitionTo failed to play "
                                           "transition track: %s -> %s -> %s",
            ...
            if (DebugLog.isEnabled(DebugType.Animation)) {
              DebugLog.Animation.println("  TransitionTo found: %s -> %s -> %s",
                                         liveAnimNode1.getName(),
                                         animTransition.m_AnimName,
                                         liveAnimNode0.getName());
            }
          } else if (DebugLog.isEnabled(DebugType.Animation)) {
            DebugLog.Animation.println(
                "  TransitionTo found: %s -> <no anim> -> %s",
                liveAnimNode1.getName(), liveAnimNode0.getName());
          }

          liveAnimNode0.startTransitionIn(liveAnimNode1, animTransition,
                                          animationTrack);
  ...
  LiveAnimNode findTransitionToNewNode(LiveAnimNode newNode, bool isSubNode) {
    ...
    for (int int1 = this.m_liveAnimNodes.size(); int0 < int1; int0++) {
      LiveAnimNode liveAnimNode1 = this.m_liveAnimNodes.get(int0);
      if (liveAnimNode1 != newNode &&
          (isSubNode || liveAnimNode1.wasDeactivated())) {
        AnimNode animNode = liveAnimNode1.getSourceNode();
        AnimTransition animTransition =
            animNode.findTransitionTo(this.m_Character, newNode.getName());
  ...
  void removeFadedOutNodes() {
    for (int int0 = this.m_liveAnimNodes.size() - 1; int0 >= 0; int0--) {
      ...
      if (!liveAnimNode.isActive() &&
          (!liveAnimNode.isTransitioningIn() ||
           !(liveAnimNode.getTransitionInWeight() > 0.01F)) &&
  ...
  void render() {
    ...
    boolean boolean0 = DebugOptions.instance.Animation.AnimLayer
                       ...
                       iAnimatable.getVariableBoolean("dbgForceAnimScalars");
    std::string string0 =
        boolean0 ? iAnimatable.getVariableString("dbgForceAnimNodeName") : null;
    int int0 = 0;
    ...
    for (int int1 = this.m_liveAnimNodes.size(); int0 < int1; int0++) {
      ...
      if (liveAnimNode.m_AnimationTracks.size() > 1) {
        bool boolean1 =
            boolean0 && StringUtils.equalsIgnoreCase(
                            liveAnimNode.getSourceNode().m_Name, string0);
        std::string string1 =
            boolean1 ? "dbgForceScalar" : liveAnimNode.getSourceNode().m_Scalar;
        std::string string2 = boolean1 ? "dbgForceScalar2"
                                       : liveAnimNode.getSourceNode().m_Scalar2;
  ...
  void logBlendWeights() {
    AnimationPlayerRecorder animationPlayerRecorder =
        this.m_Character.getAnimationPlayer().getRecorder();
    int int0 = 0;
  ...
  void applyBlendField(LiveAnimNode liveAnimNode, float float0, float float1) {
    if (liveAnimNode.isActive()) {
      ...
      Anim2DBlendPicker.PickResults pickResults =
          anim2DBlendPicker.Pick(float0, float1);
      ...
      for (int int0 = 0; int0 < liveAnimNode.m_AnimationTracks.size(); int0++) {
        ...
        AnimationTrack animationTrack0 =
            liveAnimNode.m_AnimationTracks.get(int0);
        if (anim2DBlend3 == anim2DBlend0) {
          animationTrack0.blendFieldWeight = AnimationPlayer.lerpBlendWeight(
              animationTrack0.blendFieldWeight, float2, 0.15F);
        } else if (anim2DBlend3 == anim2DBlend1) {
          animationTrack0.blendFieldWeight = AnimationPlayer.lerpBlendWeight(
              animationTrack0.blendFieldWeight, float3, 0.15F);
        } else if (anim2DBlend3 == anim2DBlend2) {
          animationTrack0.blendFieldWeight = AnimationPlayer.lerpBlendWeight(
              animationTrack0.blendFieldWeight, float4, 0.15F);
        } else {
          animationTrack0.blendFieldWeight = AnimationPlayer.lerpBlendWeight(
              animationTrack0.blendFieldWeight, 0.0F, 0.15F);
        }
        ...
        animationTrack0.blendFieldWeight =
            PZMath.clamp(animationTrack0.blendFieldWeight, 0.0F, 1.0F);
      }
    }
  ...
  LiveAnimNode findLiveNode(AnimNode animNode) {
    ...
    for (int int1 = this.m_liveAnimNodes.size(); int0 < int1; int0++) {
      ...
      if (!liveAnimNode1.m_TransitioningOut) {
        ...
        if (liveAnimNode1.getSourceNode().m_State == animNode.m_State &&
            liveAnimNode1.getSourceNode().m_Name == animNode.m_Name)) {
            liveAnimNode0 = liveAnimNode1;
            break;
          }
      }
  ...
  void startAnimTrack(const std::string &string, LiveAnimNode liveAnimNode) {
    ...
    AnimLayer.StartAnimTrackParameters startAnimTrackParameters =
        AnimLayer.StartAnimTrackParameters.alloc();
    startAnimTrackParameters.subLayerBoneWeights =
        animNode.m_SubStateBoneWeights;
    startAnimTrackParameters.syncTrackingEnabled =
        animNode.m_SyncTrackingEnabled;
    ...
    startAnimTrackParameters.useDeferredRotation =
        animNode.m_useDeferedRotation;
    startAnimTrackParameters.priority = animNode.getPriority();
    AnimationTrack animationTrack =
        this.startAnimTrack(string, startAnimTrackParameters);
    startAnimTrackParameters.release();
    if (animationTrack != nullptr) {
  ...
  startAnimTrack(const std::string &string,
    ...
    if (!animationPlayer.isReady()) {
      ...
      AnimationTrack animationTrack =
          animationPlayer.play(string, startAnimTrackParameters.isLooped);
      if (animationTrack == nullptr) {
        return null;
      } else {
        ...
        SkinningBone skinningBone =
            skinningData.getBone(startAnimTrackParameters.deferredBoneName);
        if (skinningBone == nullptr) {
          DebugLog.Animation.error("Deferred bone not found: \"%s\"",
                                   startAnimTrackParameters.deferredBoneName);
        }

        animationTrack.SpeedDelta = startAnimTrackParameters.speedScale;
        animationTrack.SyncTrackingEnabled =
            startAnimTrackParameters.syncTrackingEnabled;
        animationTrack.setDeferredBone(
            skinningBone, startAnimTrackParameters.deferredBoneAxis);
        animationTrack.setUseDeferredRotation(
            startAnimTrackParameters.useDeferredRotation);
   ...
   * great-grandparent, etc) does this layer have.
   */
  int getDepth() {
    return this.m_parentLayer != nullptr ? this.m_parentLayer.getDepth() + 1
                                         : 0;
  }
  ...
  AnimationTrack findSyncTrack(LiveAnimNode liveAnimNode) {
    ...
    for (int int1 = liveAnimNode.getPlayingTrackCount(); int0 < int1; int0++) {
      ...
      if (animationTrack1.SyncTrackingEnabled && animationTrack1.hasClip() &&
          (animationTrack0 == nullptr ||
           animationTrack1.BlendDelta > animationTrack0.BlendDelta)) {
  ...
  std::string getDebugNodeName() {
    ...
    if (this.m_State != nullptr) {
      ...
      if (this.m_CurrentNode != nullptr) {
        string = string + "/" + this.m_CurrentNode.m_Name + ": " +
                 this.m_CurrentNode.m_AnimName;
...
public
  List<LiveAnimNode> getLiveAnimNodes() { return this.m_liveAnimNodes; }
...
private
  static class StartAnimTrackParameters extends PooledObject {
    int priority;
  ...
  private
    static final Pool<AnimLayer.StartAnimTrackParameters> s_pool =
        new Pool<>(AnimLayer.StartAnimTrackParameters::new);
  ...
  public
    static AnimLayer.StartAnimTrackParameters alloc() { return s_pool.alloc(); }
  }
}
} // namespace advancedanimation
} // namespace skinnedmodel
