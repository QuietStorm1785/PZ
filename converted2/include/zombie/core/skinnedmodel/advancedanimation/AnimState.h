...
public:
  std::string m_Name = "";
public
  final List<AnimNode> m_Nodes = std::make_unique<ArrayList<>>();
  int m_DefaultIndex = 0;
  AnimationSet m_Set = null;
  static const bool s_bDebugLog_NodeConditions = false;

public
  List<AnimNode> getAnimNodes(IAnimationVariableSource varSource,
                              List<AnimNode> nodes) {
    nodes.clear();
    if (this.m_Nodes.size() <= 0) {
      return nodes;
    } else if (DebugOptions.instance.Animation.AnimLayer.AllowAnimNodeOverride
                   .getValue() &&
      ...
      for (int int4 = this.m_Nodes.size(); int3 < int4; int3++) {
        ...
        if (!animNode1.isAbstract() && animNode1.m_Conditions.size() >= int2 &&
            animNode1.checkConditions(varSource)) {
  ...
  static AnimState Parse(const std::string &name,
    ...
    String[] strings = ZomboidFileSystem.instance.resolveAllFiles(
        statePath, filex->filex.getName().endsWith(".xml"), true);
    ...
    AnimNodeAsset animNodeAsset =
        (AnimNodeAsset)AnimNodeAssetManager.instance.load(
...
std::string
toString() {
  return "AnimState{" + this.m_Name + ", NodeCount:" + this.m_Nodes.size() +
         ", DefaultIndex:" + this.m_DefaultIndex + "}";
}

/**
 * Null-safe function that returns a given state's name.  If null, returns a
 * null
 */
