...
private
  final AnimatedModel.StateInfo[] stateInfos = new AnimatedModel.StateInfo[3];
  bool bReady;
private
  static final ObjectPool<AnimatedModel.AnimatedModelInstanceRenderData>
      instDataPool =
          new ObjectPool<>(AnimatedModel.AnimatedModelInstanceRenderData::new);
private
  final AnimatedModel.UIModelCamera uiModelCamera =
      new AnimatedModel.UIModelCamera();
private
  static final AnimatedModel.WorldModelCamera worldModelCamera =
...
public
  ...
  void setModelData(HumanVisual _baseVisual, ItemVisuals _itemVisuals) {
    ...
    if (Core.bDebug && !this.bAnimate &&
        this.stateInfoMain().readyData.isEmpty()) {
    ...
    if (StringUtils.isNullOrWhitespace(string0)) {
      ...
      ModelInstance modelInstancex =
          ModelManager.instance.newAdditionalModelInstance(
              string0, string1, null, this.modelInstance.AnimPlayer, null);
  ...
  ModelInstance addStatic(const std::string &string0,
    ...
    ModelInstance modelInstancex = ModelManager.instance.newInstance(
        model, null, this.modelInstance.AnimPlayer);
...
private
  ...
  AnimationPlayerRecorder getAnimationPlayerRecorder() { return null; }
...
private
  static final class AnimatedModelInstanceRenderData {
    ModelInstance modelInstance;
    FloatBuffer matrixPalette;
  public
    final org.joml.Matrix4f xfrm = new org.joml.Matrix4f();
  ...
  public static final class StateInfo {
    ModelInstance modelInstance;
    ModelInstanceTextureCreator textureCreator;
    final ArrayList<AnimatedModel.AnimatedModelInstanceRenderData> instData =
        std::make_unique<ArrayList<>>();
    final ArrayList<AnimatedModel.AnimatedModelInstanceRenderData> readyData =
        std::make_unique<ArrayList<>>();
    bool bModelsReady;
    bool bRendered;
