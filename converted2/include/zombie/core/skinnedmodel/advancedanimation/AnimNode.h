...
public:
private
  static final Comparator<AnimEvent> s_eventsComparator =
      (animEvent1, animEvent0)
          ->Float.compare(animEvent1.m_TimePc, animEvent0.m_TimePc);
  std::string m_Name = "";
  int m_Priority = 5;
  ...
  std::string m_SpeedScaleVariable = null;
...
public
  List<Anim2DBlend> m_2DBlends = std::make_unique<ArrayList<>>();
public
  List<AnimCondition> m_Conditions = std::make_unique<ArrayList<>>();
public
  List<AnimEvent> m_Events = std::make_unique<ArrayList<>>();
public
  List<Anim2DBlendTriangle> m_2DBlendTri = std::make_unique<ArrayList<>>();
public
  List<AnimTransition> m_Transitions = std::make_unique<ArrayList<>>();
public
  List<AnimBoneWeight> m_SubStateBoneWeights = std::make_unique<ArrayList<>>();
  Anim2DBlendPicker m_picker;
  AnimState m_State = null;
  AnimTransition m_transitionOut;

  /**
   * Loads an AnimNode from the specified source.  The source can either be a
  ...
  static AnimNode Parse(const std::string &source) {
    try {
      ...
      if (animNode.m_SubStateBoneWeights.isEmpty()) {
        animNode.m_SubStateBoneWeights.add(
            new AnimBoneWeight("Bip01_Spine1", 0.5F));
        animNode.m_SubStateBoneWeights.add(
            new AnimBoneWeight("Bip01_Neck", 1.0F));
        animNode.m_SubStateBoneWeights.add(
            new AnimBoneWeight("Bip01_BackPack", 1.0F));
        animNode.m_SubStateBoneWeights.add(
            new AnimBoneWeight("Bip01_Prop1", 1.0F));
        animNode.m_SubStateBoneWeights.add(
            new AnimBoneWeight("Bip01_Prop2", 1.0F));
      }

      for (int int0 = 0; int0 < animNode.m_SubStateBoneWeights.size(); int0++) {
        AnimBoneWeight animBoneWeight =
            animNode.m_SubStateBoneWeights.get(int0);
        animBoneWeight.boneName = JAssImpImporter.getSharedString(
            animBoneWeight.boneName, "AnimBoneWeight.boneName");
      }
      ...
      System.err.println("AnimNode.Parse threw an exception reading file: " +
                         source);
  ...
  float getSpeedScale(IAnimationVariableSource varSource) {
    return this.m_SpeedScaleF != Float.POSITIVE_INFINITY
               ? this.m_SpeedScaleF
               : varSource.getVariableFloat(this.m_SpeedScale, 1.0F);
  }

  /**
   * Returns TRUE if this AnimNode represents an Idle animation.  TODO: Make
   * this a flag in the AnimNode, instead of relying on the name
   */
  ...
  AnimTransition findTransitionTo(IAnimationVariableSource varSource,
    ...
    for (int int1 = this.m_Transitions.size(); int0 < int1; int0++) {
      ...
      if (StringUtils.equalsIgnoreCase(animTransition1.m_Target, targetName) &&
          AnimCondition.pass(varSource, animTransition1.m_Conditions)) {
  ...
  std::string toString() {
    return String.format("AnimNode{ Name: %s, AnimName: %s, Conditions: %s }",
                         this.m_Name, this.m_AnimName,
                         this.getConditionsString());
  }

  std::string getConditionsString() {
    return PZArrayUtil.arrayToString(
        this.m_Conditions, AnimCondition::getConditionString, "( ", " )", ", ");
  }

  bool isAbstract() {
    return !StringUtils.isNullOrWhitespace(this.m_AnimName)
               ? false
               : this.m_2DBlends.isEmpty();
  }
  ...
  float getBlendOutTime() {
    if (this.m_transitionOut != nullptr) {
      ...
      return this.m_BlendOutTime >= 0.0F ? this.m_BlendOutTime
                                         : this.m_BlendTime;
    }
  }

  std::string getDeferredBoneName() {
    return StringUtils.isNullOrWhitespace(this.m_DeferredBoneName)
               ? "Translation_Data"
               : this.m_DeferredBoneName;
  }

  BoneAxis getDeferredBoneAxis() { return this.m_deferredBoneAxis; }
