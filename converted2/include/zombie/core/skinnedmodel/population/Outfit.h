...
public:
  ...
  bool m_Pants = true;
public
  final ArrayList<String> m_TopTextures = std::make_unique<ArrayList<>>();
public
  final ArrayList<String> m_PantsTextures = std::make_unique<ArrayList<>>();
public
  final ArrayList<ClothingItemReference> m_items =
...
public
  final Outfit.RandomData RandomData = new Outfit.RandomData();
  ...
  Outfit clone() {
    try {
      ...
      throw new RuntimeException("Outfit clone failed.",
  ...
  bool isEmpty() {
    for (int int0 = 0; int0 < this.m_items.size(); int0++) {
      ...
      if (clothingItem != nullptr && clothingItem.isEmpty()) {
        return true;
      }
