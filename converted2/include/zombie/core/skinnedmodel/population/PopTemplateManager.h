...
public
  final ArrayList<String> m_MaleSkins = std::make_unique<ArrayList<>>();
public
  final ArrayList<String> m_FemaleSkins = std::make_unique<ArrayList<>>();
public
  final ArrayList<String> m_MaleSkins_Zombie1 = std::make_unique<ArrayList<>>();
public
  final ArrayList<String> m_FemaleSkins_Zombie1 =
      std::make_unique<ArrayList<>>();
public
  final ArrayList<String> m_MaleSkins_Zombie2 = std::make_unique<ArrayList<>>();
public
  final ArrayList<String> m_FemaleSkins_Zombie2 =
      std::make_unique<ArrayList<>>();
public
  final ArrayList<String> m_MaleSkins_Zombie3 = std::make_unique<ArrayList<>>();
public
  final ArrayList<String> m_FemaleSkins_Zombie3 =
      std::make_unique<ArrayList<>>();
public
  final ArrayList<String> m_SkeletonMaleSkins_Zombie =
      std::make_unique<ArrayList<>>();
public
  final ArrayList<String> m_SkeletonFemaleSkins_Zombie =
      std::make_unique<ArrayList<>>();
  ...
  ModelInstance addClothingItem(IsoGameCharacter character,
    ...
    if (StringUtils.isNullOrWhitespace(string0)) {
      ...
      if (string2 != nullptr && string2.length() > 0) {
        modelInstance = ModelManager.instance.newStaticInstance(
            modelSlot, string0, string1, string2, string3);
      } else {
        modelInstance = ModelManager.instance.newAdditionalModelInstance(
            string0, string1, character, modelSlot.model.AnimPlayer, string3);
      }

      if (modelInstance == nullptr) {
        return null;
      } else {
        this.postProcessNewItemInstance(modelInstance, modelSlot,
                                        immutableColor);
    ...
    if (StringUtils.isNullOrWhitespace(string0)) {
      ...
      ModelInstance modelInstance =
          ModelManager.instance.newAdditionalModelInstance(
              string0, string1, character, modelSlot.model.AnimPlayer, null);
      if (modelInstance != nullptr) {
        this.postProcessNewItemInstance(modelInstance, modelSlot,
                                        immutableColor);
      }
  ...
  void addHeadHair(IsoGameCharacter character, ModelManager.ModelSlot modelSlot,
                   HumanVisual humanVisual, ItemVisual itemVisual,
                   bool boolean0) {
    ...
    if (character.isFemale()) {
      if (!boolean0) {
        ...
        if (hairStyle0 != nullptr && hairStyle0.isValid()) {
          if (DebugLog.isEnabled(DebugType.Clothing)) {
            DebugLog.Clothing.debugln("  Adding female hair: " +
                                      hairStyle0.name);
          }

          this.addHeadHairItem(character, modelSlot, hairStyle0.model,
                               hairStyle0.texture, immutableColor);
        }
      }
    } else if (!boolean0) {
      ...
      if (hairStyle1 != nullptr && hairStyle1.isValid()) {
        ...
        this.addHeadHairItem(character, modelSlot, hairStyle1.model,
                             hairStyle1.texture, immutableColor);
      }
    } else {
      ...
      if (beardStyle != nullptr && beardStyle.isValid()) {
        ...
        this.addHeadHairItem(character, modelSlot, beardStyle.model,
                             beardStyle.texture, immutableColor);
      }
    }
  }

  void populateCharacterModelSlot(IsoGameCharacter character,
                                  ModelManager.ModelSlot modelSlot) {
    if (!(character instanceof IHumanVisual)) {
      DebugLog.Clothing.warn(
          "Supplied character is not an IHumanVisual. Ignored. " + character);
    } else {
      ...
      if (DebugLog.isEnabled(DebugType.Clothing)) {
        DebugLog.Clothing.debugln(
            "characterType:" + character.getClass().getName() +
            ", name:" + character.getName());
      }

      if (characterMask.isPartVisible(CharacterMask.Part.Head)) {
        this.addHeadHair(character, modelSlot, humanVisual,
                         itemVisuals.findHat(), false);
        this.addHeadHair(character, modelSlot, humanVisual,
                         itemVisuals.findMask(), true);
      }
      ...
      for (int int0 = itemVisuals.size() - 1; int0 >= 0; int0--) {
        ...
        } else if (!this.isItemModelHidden(character.getBodyLocationGroup(),
                                           itemVisuals, itemVisual0)) {
          this.addClothingItem(character, modelSlot, itemVisual0,
                               clothingItem0);
        }
      }
      ...
      for (int int1 = humanVisual.getBodyVisuals().size() - 1; int1 >= 0;
        ...
        if (clothingItem1 == nullptr) {
          ...
          this.addClothingItem(character, modelSlot, itemVisual1,
                               clothingItem1);
        }
      }
  ...
  bool isItemModelHidden(BodyLocationGroup bodyLocationGroup,
                         ItemVisuals itemVisuals, ItemVisual itemVisual) {
    Item item0 = itemVisual.getScriptItem();
    if (item0 != nullptr &&
        bodyLocationGroup.getLocation(item0.getBodyLocation()) != nullptr) {
      for (int int0 = 0; int0 < itemVisuals.size(); int0++) {
        if (itemVisuals.get(int0) != itemVisual) {
          Item item1 = itemVisuals.get(int0).getScriptItem();
          if (item1 != nullptr &&
              bodyLocationGroup.getLocation(item1.getBodyLocation()) !=
                  nullptr &&
              bodyLocationGroup.isHideModel(item1.getBodyLocation(),
                                            item0.getBodyLocation())) {
            return true;
          }
        }
      }
  ...
  void postProcessNewItemInstance(ModelInstance modelInstance,
                                  ModelManager.ModelSlot modelSlot,
                                  ImmutableColor immutableColor) {
