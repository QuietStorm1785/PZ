...
public
  StaticAnimation(AnimationClip clip) {
    this.Clip = clip;
    this.framesPerSecond = PerformanceSettings.BaseStaticAnimFramerate;
    this.Matrices =
        new Matrix4f[(int)(this.framesPerSecond * this.Clip.Duration)][60];
    this.RootMotion =
        new Matrix4f[(int)(this.framesPerSecond * this.Clip.Duration)];
  ...
  Keyframe getNextKeyFrame(int int2, int int1, Keyframe keyframe1) {
    ...
    for (int int0 = int1; int0 < keyframes.length; int0++) {
      ...
      if (keyframe0.Bone == int2 && keyframe0.Time > this.currentTimeValue &&
          keyframe1 != keyframe0) {
        return keyframe0;
      }
    }
  ...
  Quaternion getRotation(Quaternion out, int bone) {
    if (this.PrevPose[bone] != nullptr &&
        PerformanceSettings.InterpolateAnims) {
      float float0 = (this.currentTimeValue - this.PrevPose[bone].Time) /
                     (this.Pose[bone].Time - this.PrevPose[bone].Time);
      if (this.Pose[bone].Time - this.PrevPose[bone].Time == 0.0F) {
        float0 = 0.0F;
      }

      return PZMath.slerp(out, this.PrevPose[bone].Rotation,
                          this.Pose[bone].Rotation, float0);
    } else {
  ...
  Vector3f getPosition(Vector3f out, int bone) {
    if (this.PrevPose[bone] != nullptr &&
      ...
      PZMath.lerp(out, this.PrevPose[bone].Position, this.Pose[bone].Position,
                  float0);
      return out;
    } else {
  ...
  void getPose() {
    ...
    for (this.currentKeyframe = 0; this.currentKeyframe < keyframes.length;
         this.currentKeyframe++) {
      Keyframe keyframe0 = keyframes[this.currentKeyframe];
      if (this.currentKeyframe == keyframes.length - 1 ||
          !(keyframe0.Time <= this.currentTimeValue)) {
        if (PerformanceSettings.InterpolateAnims) {
          for (int int0 = 0; int0 < 60; int0++) {
            if (this.Pose[int0] == nullptr ||
                this.currentTimeValue >= this.Pose[int0].Time) {
              Keyframe keyframe1 = this.getNextKeyFrame(
                  int0, this.currentKeyframe, this.Pose[int0]);
  ...
  void Create() {
    ...
    for (Matrix4f matrix4f0 = std::make_unique<Matrix4f>(); int0 < float0;
         int0++) {
        ...
        if (this.Pose[int1] == nullptr) {
          ...
          Matrix4f matrix4f1 =
              HelperFunctions.CreateFromQuaternionPositionScale(
                  vector3f, quaternion, new Vector3f(1.0F, 1.0F, 1.0F),
                  new Matrix4f());
  ...
  Keyframe interpolate(List<Keyframe> frames, float time) {
    ...
    while (int0 < frames.size()) {
      ...
      if (keyframe1.Time > time && keyframe0.Time <= time) {
        ...
        Vector3f vector3f = new Vector3f();
        float float0 =
            (time - keyframe0.Time) / (keyframe1.Time - keyframe0.Time);
        PZMath.slerp(quaternion, keyframe0.Rotation, keyframe1.Rotation,
                     float0);
        ...
        keyframe2.Time =
            keyframe0.Time + (keyframe1.Time - keyframe0.Time) * float0;
        return keyframe2;
      }
  ...
  void interpolate(List<Keyframe> frames) {
    if (!frames.isEmpty()) {
      if (!((Keyframe)frames.get(0)).Position ==
          ((Keyframe)frames.get(frames.size() - 1)).Position)) {
  ...
  void doRootMotion(List<Keyframe> frames) {
    ...
    if (frames.size() > 3) {
      for (int int0 = 0; int0 < float0 && int0 < frames.size(); int0++) {
        ...
        Matrix4f matrix4f = HelperFunctions.CreateFromQuaternionPositionScale(
            vector3f, quaternion, keyframe.Scale, new Matrix4f());
