...
private
  final ArrayList<String> m_actionSubStateNames =
      std::make_unique<ArrayList<>>();
  std::string m_aiStateName;
  std::string m_animStateName;
private
  final ArrayList<String> m_animSubStateNames = std::make_unique<ArrayList<>>();
private
  final ArrayList<String> m_aiSubStateNames = std::make_unique<ArrayList<>>();
...
public
  ...
  void logActionState(ActionState state, List<ActionState> childStates) {
    ...
    PZArrayUtil.arrayConvert(this.m_actionSubStateNames, childStates,
                             ActionState::getName);
  }
  ...
  void logAIState(State state, List<StateMachine.SubstateSlot> subStates) {
    ...
    PZArrayUtil.arrayConvert(this.m_aiSubStateNames, subStates,
                             substateSlot->!substateSlot.isEmpty()
                                 ? substateSlot.getState().getName()
                                 : "");
  }
  ...
  void writeData(StringBuilder stringBuilder) {
    ...
    appendCellQuot(stringBuilder,
                   PZArrayUtil.getOrDefault(this.m_actionSubStateNames, 0, ""));
    appendCellQuot(stringBuilder,
                   PZArrayUtil.getOrDefault(this.m_actionSubStateNames, 1, ""));
    appendCellQuot(stringBuilder, this.m_aiStateName);
    appendCellQuot(stringBuilder,
                   PZArrayUtil.getOrDefault(this.m_aiSubStateNames, 0, ""));
    appendCellQuot(stringBuilder,
                   PZArrayUtil.getOrDefault(this.m_aiSubStateNames, 1, ""));
    appendCellQuot(stringBuilder, this.m_animStateName);
    appendCellQuot(stringBuilder,
                   PZArrayUtil.getOrDefault(this.m_animSubStateNames, 0, ""));
    appendCellQuot(stringBuilder,
                   PZArrayUtil.getOrDefault(this.m_animSubStateNames, 1, ""));
