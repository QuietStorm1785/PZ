...
public:
public
  final ArrayList<TextureCombinerCommand> commands =
...
private
  final HashMap<Integer, ArrayList<Integer>> categoryMap =
      std::make_unique<HashMap<>>();
  static SmartShader bodyMask;
  static SmartShader bodyMaskTint;
  static SmartShader bodyMaskHue;
private
  static final ArrayList<TextureCombinerShaderParam> bodyMaskParams =
      std::make_unique<ArrayList<>>();
  static SmartShader addHole;
private
  static final ArrayList<TextureCombinerShaderParam> addHoleParams =
      std::make_unique<ArrayList<>>();
  static SmartShader removeHole;
private
  static final ArrayList<TextureCombinerShaderParam> removeHoleParams =
      std::make_unique<ArrayList<>>();
  static SmartShader blit;

public
  SmartTexture() { this.name = "SmartTexture"; }
  ...
  TextureCombinerCommand getFirstFromCategory(int int0) {
    return !this.categoryMap.containsKey(int0)
               ? nullptr
  ...
  void addTint(Texture texture, int int0, float float0, float float1,
               float float2) {
    if (tint == nullptr) {
      this.create();
    }
  ...
  Texture addHole(BloodBodyPartType bloodBodyPartType) {
    std::string string =
        ...
        CharacterSmartTexture.MaskFiles[bloodBodyPartType.index()] + ".png";
    if (addHole == nullptr) {
      this.create();
    }
    ...
    this.clear();
    this.result = nullptr;
    this.commands.add(TextureCombinerCommand.get().initSeparate(
        texture, addHole, addHoleParams, getTextureWithFlags(string), 770, 0, 1,
        771));
    this.dirty = true;
    return texture;
  }
  ...
  void removeHole(const std::string &string1,
    ...
    std::string string0 =
        "media/textures/HoleTextures/" +
        CharacterSmartTexture.MaskFiles[bloodBodyPartType.index()] + ".png";
    this.removeHole(getTextureWithFlags(string1), getTextureWithFlags(string0),
                    bloodBodyPartType);
  }
  ...
  void removeHole(Texture texture0, Texture texture1, BloodBodyPartType var3) {
    ...
    this.addToCat(CharacterSmartTexture.ClothingItemCategory);
    this.commands.add(TextureCombinerCommand.get().init(
        texture0, removeHole, removeHoleParams, texture1, 770, 771));
    this.dirty = true;
  }
  ...
  void mask(Texture texture0, Texture texture1, int int0) {
    ...
    this.addToCat(int0);
    this.commands.add(TextureCombinerCommand.get().init(
        texture0, bodyMask, bodyMaskParams, texture1, 770, 771));
    this.dirty = true;
  }
  ...
  void maskHue(Texture texture0, Texture texture1, int int0, float float0) {
    ...
    arrayList.add(new TextureCombinerShaderParam("HueChange", float0));
    this.commands.add(TextureCombinerCommand.get().init(
        texture0, bodyMaskHue, arrayList, texture1, 770, 771));
    this.dirty = true;
  }
  ...
  void maskTint(Texture texture0, Texture texture1, int int0, float float0,
    ...
    arrayList.add(new TextureCombinerShaderParam("B", float2));
    this.commands.add(TextureCombinerCommand.get().init(
        texture0, bodyMaskTint, arrayList, texture1, 770, 771));
    this.dirty = true;
  }
  ...
  static void addMaskFlags(SmartTexture smartTexture,
    ...
    Consumer consumer = Lambda.consumer(
        ...
        (part, smartTexturex, stringx, texturex, integer)
            ->smartTexturex.mask(
                texturex, getTextureWithFlags(stringx + "/" + part + ".png"),
                integer));
  ...
  static void addMaskFlagsHue(SmartTexture smartTexture,
    ...
    Consumer consumer = Lambda.consumer(
        ...
        (part, smartTexturex, stringx, texturex, integer, float0x)
            ->smartTexturex.maskHue(
                texturex, getTextureWithFlags(stringx + "/" + part + ".png"),
                integer, float0x));
    characterMask.forEachVisible(consumer);
  }
...
private
  static void addMaskFlagsTint(SmartTexture smartTexture,
                               CharacterMask characterMask, String string,
                               Texture texture, int int0,
                               ImmutableColor immutableColor) {
    Consumer consumer = Lambda.consumer(
        smartTexture, string, texture, int0, immutableColor,
        (part, smartTexturex, stringx, texturex, integer, immutableColorx)
            ->smartTexturex.maskTint(
                texturex, getTextureWithFlags(stringx + "/" + part + ".png"),
                integer, immutableColorx.r, immutableColorx.g,
                immutableColorx.b));
    characterMask.forEachVisible(consumer);
  }

private
  static void addMaskedTexture(SmartTexture smartTexture,
                               CharacterMask characterMask, String string,
                               Texture texture, int int0,
                               ImmutableColor immutableColor, float float0) {
    if (!characterMask.isNothingVisible()) {
      if (characterMask.isAllVisible()) {
        if (!ImmutableColor.white == immutableColor)) {
            smartTexture.addTint(texture, int0, immutableColor.r,
                                 immutableColor.g, immutableColor.b);
          }
        ...
        if (!ImmutableColor.white == immutableColor)) {
            addMaskFlagsTint(smartTexture, characterMask, string, texture, int0,
                             immutableColor);
          }
        else if (!(float0 < -1.0E-4F) && !(float0 > 1.0E-4F)) {
          addMaskFlags(smartTexture, characterMask, string, texture, int0);
        } else {
          addMaskFlagsHue(smartTexture, characterMask, string, texture, int0,
                          float0);
        }
      }
    }
  ...
  static void addTexture(SmartTexture smartTexture, const std::string &string,
                         ...
                         float float0) {
    if (!ImmutableColor.white == immutableColor)) {
        smartTexture.addTint(string, int0, immutableColor.r, immutableColor.g,
                             immutableColor.b);
      }
    else if (!(float0 < -1.0E-4F) && !(float0 > 1.0E-4F)) {
...
public
  ...
  void calculate() {
    synchronized(this) {
      ...
      try {
        ...
        DebugLog.General.error(exception.getClass().getSimpleName() +
                               " encountered while combining texture.");
        DebugLog.General.error(
            "Intended width : " +
            TextureCombiner.getResultingWidth(this.commands));
        DebugLog.General.error(
            "Intended height: " +
            TextureCombiner.getResultingHeight(this.commands));
        DebugLog.General.error("");
        DebugLog.General.error("Commands list: " +
                               PZArrayUtil.arrayToString(this.commands));
        DebugLog.General.error("");
  ...
  void add(Texture texture, SmartShader smartShader,
           ArrayList<TextureCombinerShaderParam> arrayList) {
    this.commands.add(
        TextureCombinerCommand.get().init(texture, smartShader, arrayList));
    this.dirty = true;
  }
  ...
  void add(Texture texture0, SmartShader smartShader, Texture texture1,
           int int0, int int1) {
    this.commands.add(TextureCombinerCommand.get().init(texture0, smartShader,
                                                        texture1, int0, int1));
    this.dirty = true;
  }
  ...
  void add(const std::string &string1, SmartShader smartShader,
           ...
           int int1) {
    this.add(getTextureWithFlags(string1), smartShader,
             getTextureWithFlags(string0), arrayList, int0, int1);
  }

  void add(Texture texture0, SmartShader smartShader, Texture texture1,
           ArrayList<TextureCombinerShaderParam> arrayList, int int0,
           int int1) {
    this.addSeparate(texture0, smartShader, texture1, arrayList, int0, int1, 1,
                     771);
  }

public
  void addSeparate(String string1, SmartShader smartShader, String string0,
                   ArrayList<TextureCombinerShaderParam> arrayList, int int0,
                   int int1, int int2, int int3) {
    this.addSeparate(getTextureWithFlags(string1), smartShader,
                     getTextureWithFlags(string0), arrayList, int0, int1, int2,
                     int3);
  }

public
  void addSeparate(Texture texture0, SmartShader smartShader, Texture texture1,
                   ArrayList<TextureCombinerShaderParam> arrayList, int int0,
                   int int1, int int2, int int3) {
    this.commands.add(TextureCombinerCommand.get().initSeparate(
        texture0, smartShader, arrayList, texture1, int0, int1, int2, int3));
    this.dirty = true;
  }
  ...
  bool isEmpty() {
    return this.result == nullptr ? true : this.result.isEmpty();
  }
