...
public
   ...
   */
public
  ShortList(int size) { this(true, size); }

  /**
   * FloatList constructor comment.
   */
public
  ShortList(bool _fastExpand, int size) {
  ...
  short remove(int idx) {
    if (idx < this.count && idx >= 0) {
      ...
      if (idx < this.count - 1) {
        System.arraycopy(this.value, idx + 1, this.value, idx,
                         this.count - idx - 1);
      }
      ...
      throw new IndexOutOfBoundsException("Referenced " + idx +
                                          ", size=" + this.count);
    }
  }
...
public
  short[] array() { return this.value; }

  int capacity() { return this.value.length; }
   ...
   * get method comment.
   */
  short get(int index) { return this.value[index]; }
