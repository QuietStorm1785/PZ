...
private
  final Controller[] controllers = new Controller[16];
private
  final ArrayList<Controller> newlyConnected = std::make_unique<ArrayList<>>();
private
  final ArrayList<Controller> newlyDisconnected =
      std::make_unique<ArrayList<>>();
private
  final boolean[][] controllerPressed = new boolean[16][15];
private
  final boolean[][] controllerWasPressed = new boolean[16][15];
private
  final float[][] controllerPov = new float[16][2];
  const ControllerStateCache m_controllerStateCache =
      new ControllerStateCache();
  ...
  Controller checkControllerButton(int int0, int int1) {
    ...
    if (controller == nullptr) {
      ...
      return int1 >= 0 && int1 < controller.getButtonCount() ? controller
                                                             : nullptr;
    }
  }
  ...
  bool isButtonPressedD(int button, int index) {
    if (index == -1) {
      ...
      return controller = =
                 null ? false : this.controllerPressed[index][button];
    }
  }
   ...
   * @return true if the controller button was in the pressed state the previous
   * frame.
   */
  bool wasButtonPressed(int index, int button) {
    Controller controller = this.checkControllerButton(index, button);
    return controller = =
               null ? false : this.controllerWasPressed[index][button];
  }

  bool isButtonStartPress(int index, int button) {
    return !this.wasButtonPressed(index, button) &&
           this.isButtonPressedD(button, index);
  }
  ...
  Controller getController(int index) {
    return index >= 0 && index < this.controllers.length
      ...
      for (int int2 = 0; int2 < this.getControllerCount(); int2++) {
        ...
        if (controller2 != nullptr) {
          ...
          for (int int4 = 0; int4 < int3; int4++) {
            this.controllerWasPressed[int2][int4] =
                this.controllerPressed[int2][int4];
            if (this.controllerPressed[int2][int4] &&
                !controller2.isButtonPressed(int4)) {
              this.controllerPressed[int2][int4] = false;
            } else if (!this.controllerPressed[int2][int4] &&
                       controller2.isButtonPressed(int4)) {
          ...
          if (float1 != this.controllerPov[int2][0] ||
              float2 != this.controllerPov[int2][1]) {
  ...
  void quit() { this.m_controllerStateCache.quit(); }
}
} // namespace input
} // namespace core
} // namespace zombie
