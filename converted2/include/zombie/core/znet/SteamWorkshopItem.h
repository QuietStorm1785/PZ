...
private
  ...
  bool bHasMap;
private
  final ArrayList<String> modIDs = std::make_unique<ArrayList<>>();
private
  final ArrayList<String> mapFolders = std::make_unique<ArrayList<>>();
...
public
  static ArrayList<String> getAllowedTags() {
...
public
  ...
  std::string getChangeNote() { return this.changeNote; }

  bool create() { return SteamWorkshop.instance.CreateWorkshopItem(this); }
  ...
  bool getUpdateProgress(KahluaTable table) {
    if (table == nullptr) {
      throw new NullPointerException("table is nullptr");
    } else {
  ...
  int getUpdateProgressTotal() { return 1; }
        ...
        for (auto &path1 : directoryStream)
          if (Files.isDirectory(path1)) {
            ...
            if (!string1.equalsIgnoreCase("pyramid.zip") &&
                (string1.endsWith(".exe") || string1.endsWith(".dll") ||
                 string1.endsWith(".bat") || string1.endsWith(".app") ||
                 string1.endsWith(".dylib") || string1.endsWith(".sh") ||
                 string1.endsWith(".so") || string1.endsWith(".zip"))) {
...
std::string validateMapDotInfo(Path var1) { return null; }
...
std::string validateMapFolder(Path path0) {
  ...
  try(DirectoryStream directoryStream = Files.newDirectoryStream(path0)) {
    for (auto &path1 : directoryStream)
      if (!Files.isDirectory(path1) &&
          "map.info" == path1.getFileName().toString())) {
...
std::string validateMediaFolder(Path path0) {
  try {
    try(DirectoryStream directoryStream = Files.newDirectoryStream(path0)) {
      for (auto &path1 : directoryStream)
        if (Files.isDirectory(path1) &&
            "maps" == path1.getFileName().toString())) {
...
std::string validateBuildingsFolder(Path var1) { return null; }

std::string validateCreativeFolder(Path var1) { return null; }
  ...
  try {
    ...
    if (!Files.isDirectory(path0)) {
      ...
      if (string != nullptr) {
        ...
        try(DirectoryStream directoryStream = Files.newDirectoryStream(path0)) {
          for (auto &path2 : directoryStream)
        ...
        return !boolean0 ? "EmptyContentsFolder"
                         : this.validateFileTypes(path0);
      }
      catch (Exception exception) {
        exception.printStackTrace();
        return "IOError";
        ...
        while ((string1 = bufferedReader.readLine()) != nullptr) {
          ...
          if (!string1.isEmpty() && !string1.startsWith("#") &&
            ...
            if (string1.startsWith("id=")) {
              ...
              this.description =
                  this.description + string1.replace("description=", "");
            } else if (string1.startsWith("tags=")) {
              this.tags.addAll(
                  Arrays.asList(string1.replace("tags=", "").split(";")));
            } else if (string1.startsWith("title=")) {
              this.title = string1.replace("title=", "");
            } else if (!string1.startsWith("version=") &&
                       string1.startsWith("visibility=")) {
...
bool writeWorkshopTxt() {
  ...
  try {
    ...
    bufferedWriter.write(
        "id=" + (this.PublishedFileId == nullptr ? "" : this.PublishedFileId));
    bufferedWriter.newLine();
...
static enum ItemState {
  None(0), Subscribed(1), LegacyItem(2), Installed(4), NeedsUpdate(8),
  Downloading(16), DownloadPending(32);

  const int value;

  private ItemState(int int1){this.value = int1;}
...
bool and (long long0) { return (this.value & long0) != 0L; }

bool not(long long0) { return (this.value & long0) == 0L; }
