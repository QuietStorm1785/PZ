...
public:
private
  final ArrayList<DeviceList> m_devices = std::make_unique<ArrayList<>>();
private
  final ArrayList<FileSystemImpl.AsyncItem> m_in_progress =
      std::make_unique<ArrayList<>>();
private
  final ArrayList<FileSystemImpl.AsyncItem> m_pending =
      std::make_unique<ArrayList<>>();
  int m_last_id = 0;
  DiskFileDevice m_disk_device;
  MemoryFileDevice m_memory_device;
private
  final HashMap<String, TexturePackDevice> m_texturepack_devices =
      std::make_unique<HashMap<>>();
private
  final HashMap<String, DeviceList> m_texturepack_devicelists =
      std::make_unique<HashMap<>>();
  DeviceList m_default_device;
  const ExecutorService executor;
  const AtomicBoolean lock = new AtomicBoolean(false);
private
  final ArrayList<FileSystemImpl.AsyncItem> m_added =
      std::make_unique<ArrayList<>>();
public
  static final HashMap<String, Boolean> TexturePackCompression =
      std::make_unique<HashMap<>>();
...
public
  ...
  bool hasWork() {
    if (this.m_pending.isEmpty() && this.m_in_progress.isEmpty()) {
      ...
      bool boolean0 = !this.m_added.isEmpty();
      this.lock.set(false);
      return boolean0;
    } else {
      return true;
