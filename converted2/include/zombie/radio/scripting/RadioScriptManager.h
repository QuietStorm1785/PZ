...
public:
private
  final Map<Integer, RadioChannel> channels =
      std::make_unique<LinkedHashMap<>>();
  static RadioScriptManager instance;
  int currentTimeStamp = 0;
private
  ArrayList<RadioChannel> channelsList = std::make_unique<ArrayList<>>();
...
public
  Map<Integer, RadioChannel> getChannels() { return this.channels; }
  ...
  void simulateScriptsUntil(int days, bool force) {
    for (Entry entry : this.channels.entrySet()) {
      this.simulateChannelUntil(((RadioChannel)entry.getValue()).GetFrequency(),
                                days, force);
    }
  }
  ...
  int getCurrentTimeStamp() { return this.currentTimeStamp; }

  void PlayerListensChannel(int chanfrequency, bool mode, bool sourceIsTV) {
    if (this.channels.containsKey(chanfrequency) &&
        this.channels.get(chanfrequency).IsTv() == sourceIsTV) {
  ...
  void AddChannel(RadioChannel channel, bool overwrite) {
    if (channel == nullptr ||
        !overwrite && this.channels.containsKey(channel.GetFrequency())) {
      std::string string0 = channel != null ? channel.GetName() : "null";
      DebugLog.log(DebugType.Radio,
                   "Error adding radiochannel (" + string0 +
                       "), channel is nullptr or frequency key already exists");
    } else {
      this.channels.put(channel.GetFrequency(), channel);
      std::string string1 = channel.GetCategory().name();
      ZomboidRadio.getInstance().addChannelName(
          channel.GetName(), channel.GetFrequency(), string1, overwrite);
    }
  }
  ...
  void UpdateScripts(int day, int hour, int mins) {
    ...
    for (Entry entry : this.channels.entrySet()) {
      ((RadioChannel)entry.getValue())
          .UpdateScripts(this.currentTimeStamp, day);
    }
  }
  ...
  void Save(Writer w) {
    for (Entry entry : this.channels.entrySet()) {
      ...
      w.write("," + (radioBroadCast != nullptr
                         ? radioBroadCast.getCurrentLineNumber() + ""
                         : "-1"));
