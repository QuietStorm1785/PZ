...
public
  ...
  bool IsTv() { return this.isTv; }

  ChannelCategory GetCategory() { return this.category; }

  RadioScript getCurrentScript() { return this.currentScript; }

  RadioBroadCast getAiringBroadcast() { return this.airingBroadcast; }

  std::string getLastAiredLine() { return this.lastAiredLine; }

  int getCurrentScriptLoop() { return this.currentScriptLoop; }

  int getCurrentScriptMaxLoops() { return this.currentScriptMaxLoops; }

  std::string getLastBroadcastID() { return this.lastBroadcastID; }

  RadioData getRadioData() { return this.radioData; }

  void setRadioData(RadioData _radioData) { this.radioData = _radioData; }

  bool isTimeSynced() { return this.isTimeSynced; }

  void setTimeSynced(bool _isTimeSynced) { this.isTimeSynced = _isTimeSynced; }
  ...
  void SetPlayerIsListening(bool isListening) {
    ...
    if (this.playerIsListening && this.airingBroadcast == nullptr &&
  ...
  bool GetPlayerIsListening() { return this.playerIsListening; }
      ...
      if (this.currentScript != nullptr) {
        ...
        if (DebugLog.isEnabled(DebugType.Radio)) {
          DebugLog.Radio.println(
              "Script: " + scriptName + ", day = " + day +
              ", minloops = " + this.currentScript.getLoopMin() +
              ", maxloops = " + this.currentScriptMaxLoops);
        }
      }
    }
  }
    ...
    if (this.currentScript != nullptr) {
      if (this.currentScriptLoop < this.currentScriptMaxLoops) {
        ...
        if (exitOption != nullptr) {
          this.setActiveScript(exitOption.getScriptname(),
                               int0 + exitOption.getStartDelay());
        }
      }
    }
  }
  ...
  void UpdateScripts(int timestamp, int day) {
    ...
    if (this.currentScript != nullptr &&
        !this.currentScript.UpdateScript(timestamp)) {
      ...
      if (this.airCounter < 0.0F) {
        ...
        if (radioLine == nullptr) {
          ...
          if (!ZomboidRadio.DISABLE_BROADCASTING) {
            ...
            if (this.louisvilleObfuscate &&
                ZomboidRadio.LOUISVILLE_OBFUSCATION) {
              string = ZomboidRadio.getInstance().scrambleString(string, 85,
                                                                 true, nullptr);
              ZomboidRadio.getInstance().SendTransmission(
                  0, 0, this.frequency, string, nullptr, "", 0.7F, 0.5F, 0.5F,
                  -1, this.isTv);
            } else {
              ZomboidRadio.getInstance().SendTransmission(
                  0, 0, this.frequency, string, nullptr,
                  radioLine.getEffectsString(), radioLine.getR(),
                  radioLine.getG(), radioLine.getB(), -1, this.isTv);
            }
          }
  ...
  void AddRadioScript(RadioScript script) {
    if (script != nullptr && !this.scripts.containsKey(script.GetName())) {
      ...
      std::string string = script != null ? script.GetName() : "null";
      DebugLog.log(DebugType.Radio, "Error while attempting to add script (" +
                                        string +
                                        "), nullptr or name already exists.");
    }
  }
  ...
  void setAirCounterMultiplier(float _airCounterMultiplier) {
    this.airCounterMultiplier =
        PZMath.clamp(_airCounterMultiplier, 0.1F, 10.0F);
  }
}
} // namespace scripting
} // namespace radio
