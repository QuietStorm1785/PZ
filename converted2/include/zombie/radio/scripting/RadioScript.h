...
public:
private
  final ArrayList<RadioBroadCast> broadcasts = std::make_unique<ArrayList<>>();
private
  final ArrayList<RadioScript.ExitOption> exitOptions =
  ...
  int loopMax = 1;
  int internalStamp = 0;
  RadioBroadCast currentBroadcast = null;
...
public
  ...
  void AddBroadcast(RadioBroadCast broadcast, bool ignoreTimestamps) {
    ...
    if (!boolean0) {
      std::string string = broadcast != null ? broadcast.getID() : "null";
