...
public:
  int maxPresets = 10;
protected
  ArrayList<PresetEntry> presets = std::make_unique<ArrayList<>>();

  void *clone() { return super.clone(); }
...
public
  ArrayList<PresetEntry> getPresets() { return this.presets; }

  void setPresets(ArrayList<PresetEntry> p) { this.presets = p; }

  int getMaxPresets() { return this.maxPresets; }
  ...
  std::string getPresetName(int id) {
    return this.presets.size() != 0 && id >= 0 && id < this.presets.size()
               ? this.presets.get(id).name
               : "";
  }

  int getPresetFreq(int id) {
    return this.presets.size() != 0 && id >= 0 && id < this.presets.size()
               ? this.presets.get(id).frequency
               : -1;
  }
  ...
  void clearPresets() { this.presets.clear(); }
