...
public:
  ...
  const int spawning;
private
  final ArrayList<MediaData.MediaLineData> lines =
...
public
  MediaData(const std::string &_id, const std::string &_itemDisplayName,
    ...
    if (Core.bDebug) {
      if (_itemDisplayName == nullptr) {
        throw new RuntimeException("ItemDisplayName may not be nullptr.");
      }

      if (_id == nullptr) {
        throw new RuntimeException("Id may not be nullptr.");
      }
    }
  }
  ...
  std::string getTranslatedSubTitle() {
    return this.subtitle != nullptr ? Translator.getText(this.subtitle)
                                    : nullptr;
  }
  ...
  short getIndex() { return this.index; }

  void setIndex(short short0) { this.index = short0; }
  ...
  uint8_t getMediaType() {
    if (this.category == nullptr) {
      DebugLog.log(
          "Warning MediaData has no category set, mediadata = " +
          (this.itemDisplayName != nullptr ? this.itemDisplayName : "unknown"));
    }
...
public
  MediaData.MediaLineData getLine(int _index) {
    return _index >= 0 && _index < this.lines.size() ? this.lines.get(_index)
                                                     : nullptr;
  }

public
  static final class MediaLineData {
  ...
  public
    ...
    std::string getTranslatedText() { return Translator.getText(this.text); }

    Color getColor() { return this.color; }

    float getR() { return this.color.r; }

    float getG() { return this.color.g; }

    float getB() { return this.color.b; }

    std::string getCodes() { return this.codes; }

    std::string getTextGuid() { return this.text; }
  }
}
} // namespace media
} // namespace radio
