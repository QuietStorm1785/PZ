...
private
  static final byte[] FILE_MAGIC = new byte[]{87, 77, 83, 89};
  std::string m_mapID;
      ...
      try(FileInputStream fileInputStream = new FileInputStream(file);
        ...
        if (!Arrays == bytes, FILE_MAGIC)) {
            throw new IOException(file.getAbsolutePath() +
                                  " does not appear to be map_symbols.bin");
          }
...
public
  ...
  int getSaveType() { return Item.Type.Map.ordinal(); }
  ...
  WorldMapSymbols getSymbols() { return this.m_symbols; }
