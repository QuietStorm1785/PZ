...
public:
private
  static final ArrayList<InventoryItem> tempList =
      std::make_unique<ArrayList<>>();
private
  static final ArrayList<IsoObject> s_tempObjects =
      std::make_unique<ArrayList<>>();
  bool active = false;
  bool dirty = true;
  bool IsDevice = false;
  ...
  int Capacity = 50;
  InventoryItem containingItem = null;
public
...
public
  ...
  std::string openSound = null;
  std::string closeSound = null;
  std::string putSound = null;
  std::string OnlyAcceptCategory = null;
  std::string AcceptItemFunction = null;
  int weightReduction = 0;
  std::string containerPosition = null;
  std::string freezerPosition = null;
private
  static final ThreadLocal<ItemContainer.Comparators> TL_comparators =
      ThreadLocal.withInitial(ItemContainer.Comparators::new);
private
  static final ThreadLocal<ItemContainer.InventoryItemListPool>
      TL_itemListPool =
          ThreadLocal.withInitial(ItemContainer.InventoryItemListPool::new);
private
  static final ThreadLocal<ItemContainer.Predicates> TL_predicates =
...
public
  ItemContainer(int _ID) { this.ID = _ID; }

public
  ItemContainer() { this.ID = -1; }

  static float floatingPointCorrection(float val) {
    uint8_t byte0 = 100;
    float float0 = val * byte0;
    return (float)((int)(float0 - (int)float0 >= 0.5F ? float0 + 1.0F
                                                      : float0)) /
           byte0;
  }
  ...
  void setCapacity(int int0) { this.Capacity = int0; }
  ...
  int getEffectiveCapacity(IsoGameCharacter chr) {
    if (chr != nullptr && !(this.parent instanceof IsoGameCharacter) &&
        !(this.parent instanceof IsoDeadBody) && !"floor" == this.getType())) {
        if (chr.Traits.Organized.isSet()) {
          return (int)Math.max(this.Capacity * 1.3F,
                               (float)(this.Capacity + 1));
        }
  ...
  bool hasRoomFor(IsoGameCharacter chr, InventoryItem item) {
    if (this.vehiclePart != nullptr &&
        this.vehiclePart.getId().contains("Seat") && this.Items.isEmpty() &&
        item.getUnequippedWeight() <= 50.0F) {
      return true;
    } else if (floatingPointCorrection(this.getCapacityWeight()) +
                   item.getUnequippedWeight() <=
               this.getEffectiveCapacity(chr)) {
      return this.getContainingItem() != nullptr &&
                     this.getContainingItem().getEquipParent() != nullptr &&
                     this.getContainingItem().getEquipParent().getInventory() !=
                         nullptr &&
                     !this.getContainingItem()
                          .getEquipParent()
                          .getInventory()
                          .contains(item)
                 ? floatingPointCorrection(this.getContainingItem()
                                               .getEquipParent()
                                               .getInventory()
                                               .getCapacityWeight()) +
                           item.getUnequippedWeight() <=
  ...
  bool isItemAllowed(InventoryItem item) {
    if (item == nullptr) {
      ...
      if (string0 != nullptr && !string0.equalsIgnoreCase(item.getCategory())) {
        ...
        if (string1 != nullptr) {
          ...
          if (object != nullptr) {
            bool boolean0 = LuaManager.caller.protectedCallBoolean(
                LuaManager.thread, object, this, item);
        ...
        if (this.parent != nullptr &&
            !this.parent.isItemAllowedInContainer(this, item)) {
          return false;
    ...
    return false;
                 ...
                 this.getParent().getProperties().Val("CustomName") !=
                     nullptr &&
                 this.getParent().getProperties().Val("CustomName") ==
                     "Toaster")
    ...
    return false;
          ...
          if (this.getParent() != nullptr &&
              this.getParent().getProperties() != nullptr &&
              this.getParent().getProperties().Val("GroupName") != nullptr) {
  ...
  bool isRemoveItemAllowed(InventoryItem item) {
    return item = = null ? false
                         : this.parent == null ||
                               this.parent.isRemoveItemAllowedFromContainer(
                                   this, item);
  }

  bool isExplored() { return this.bExplored; }

  void setExplored(bool b) { this.bExplored = b; }
  ...
  bool isInside(InventoryItem item) {
    if (this.containingItem == nullptr) {
      ...
      return this.containingItem == item
                 ? true
                 : this.containingItem.getContainer() != nullptr &&
                       this.containingItem.getContainer().isInside(item);
    }
  }

  InventoryItem getContainingItem() { return this.containingItem; }

  InventoryItem DoAddItem(InventoryItem item) { return this.AddItem(item); }
...
public
  ArrayList<InventoryItem> AddItems(String item, int use) {
  ...
  for (int int1 = 0; int1 < this.Items.size(); int1++) {
    ...
    if (item.getFullType() == findItem) || item.getType() == findItem)) {
      ...
      int0 += ((InventoryContainer)item)
                  .getItemContainer()
                  .getNumberOfItem(findItem);
    } else if (includeReplaceOnDeplete && item instanceof
               DrainableComboItem &&
                   ((DrainableComboItem)item).getReplaceOnDeplete() !=
                       nullptr) {
      DrainableComboItem drainableComboItem = (DrainableComboItem)item;
...
InventoryItem addItem(InventoryItem item) { return this.AddItem(item); }
...
void removeItemOnServer(InventoryItem item) {
  if (GameClient.bClient) {
    if (this.containingItem != nullptr &&
        this.containingItem.getWorldItem() != nullptr) {
      GameClient.instance.addToItemRemoveSendBuffer(
          this.containingItem.getWorldItem(), this, item);
    } else {
...
void addItemOnServer(InventoryItem item) {
  if (GameClient.bClient) {
    if (this.containingItem != nullptr &&
        this.containingItem.getWorldItem() != nullptr) {
      GameClient.instance.addToItemSendBuffer(
          this.containingItem.getWorldItem(), this, item);
    } else {
...
bool contains(InventoryItem itemToFind, bool doInv) {
  ...
  for (int int0 = 0; int0 < this.Items.size(); int0++) {
    ...
    if (item == nullptr) {
      ...
      if (doInv && item instanceof
          InventoryContainer &&
              ((InventoryContainer)item).getInventory() != nullptr &&
              !inventoryItemList.contains(item)) {
  ...
  for (int int1 = 0; int1 < inventoryItemList.size(); int1++) {
    ItemContainer container =
        ((InventoryContainer)inventoryItemList.get(int1)).getInventory();
...
bool contains(const std::string &_type, bool doInv, bool ignoreBroken) {
  ...
  if (_type.contains("Type:")) {
    for (int int0 = 0; int0 < this.Items.size(); int0++) {
      ...
      if (_type.contains("Weapon") && item0 instanceof
          HandWeapon && this.testBroken(ignoreBroken, item0)) {
        TL_itemListPool.get().release(inventoryItemList);
        return true;
      }
      ...
      if (_type.contains("AlarmClockClothing") && item0 instanceof
          AlarmClockClothing) {
        TL_itemListPool.get().release(inventoryItemList);
        return true;
      }

      if (doInv && item0 instanceof
          InventoryContainer &&
    ...
    for (auto &string : strings)
      for (int int1 = 0; int1 < this.Items.size(); int1++) {
        ...
        if (compareType(string.trim(), item1) &&
            this.testBroken(ignoreBroken, item1)) {
          TL_itemListPool.get().release(inventoryItemList);
          return true;
        }

        if (doInv && item1 instanceof
            InventoryContainer &&
  ...
  for (int int2 = 0; int2 < this.Items.size(); int2++) {
    ...
    if (item2 == nullptr) {
      ...
      if (compareType(_type.trim(), item2) &&
          this.testBroken(ignoreBroken, item2)) {
        TL_itemListPool.get().release(inventoryItemList);
        return true;
      }

      if (doInv && item2 instanceof
          InventoryContainer &&
...
for (int int3 = 0; int3 < inventoryItemList.size(); int3++) {
  ItemContainer container =
      ((InventoryContainer)inventoryItemList.get(int3)).getInventory();
...
getBestOf(ItemContainer.InventoryItemList inventoryItemList,
  ...
  if (inventoryItemList != nullptr && !inventoryItemList.isEmpty()) {
...
InventoryItem getBest(Predicate<InventoryItem> predicate,
                      Comparator<InventoryItem> comparator) {
  ItemContainer.InventoryItemList inventoryItemList =
      TL_itemListPool.get().alloc();
...
InventoryItem getBestRecurse(Predicate<InventoryItem> predicate,
                             Comparator<InventoryItem> comparator) {
  ItemContainer.InventoryItemList inventoryItemList =
      TL_itemListPool.get().alloc();
...
InventoryItem getBestEval(LuaClosure predicateObj, LuaClosure comparatorObj) {
  ItemContainer.EvalPredicate evalPredicate =
      TL_predicates.get().eval.alloc().init(predicateObj);
  ItemContainer.EvalComparator evalComparator =
      TL_comparators.get().eval.alloc().init(comparatorObj);
...
InventoryItem getBestEvalRecurse(LuaClosure predicateObj,
...
InventoryItem getBestEvalArg(LuaClosure predicateObj, LuaClosure comparatorObj,
                             void *arg) {
  ItemContainer.EvalArgPredicate evalArgPredicate =
      TL_predicates.get().evalArg.alloc().init(predicateObj, arg);
  ItemContainer.EvalArgComparator evalArgComparator =
      TL_comparators.get().evalArg.alloc().init(comparatorObj, arg);
...
InventoryItem getBestEvalArgRecurse(LuaClosure predicateObj,
...
InventoryItem getBestCondition(Predicate<InventoryItem> predicate) {
  ItemContainer.ConditionComparator conditionComparator =
      TL_comparators.get().condition.alloc();
...
InventoryItem getBestConditionEval(LuaClosure functionObj) {
  ItemContainer.EvalPredicate evalPredicate =
      TL_predicates.get().eval.alloc().init(functionObj);
...
InventoryItem getBestConditionEvalArg(LuaClosure functionObj, void *arg) {
  ItemContainer.EvalArgPredicate evalArgPredicate =
      TL_predicates.get().evalArg.alloc().init(functionObj, arg);
  InventoryItem item = this.getBestCondition(evalArgPredicate);
  TL_predicates.get().evalArg.release(evalArgPredicate);
  return item;
}

InventoryItem getBestConditionEvalArgRecurse(LuaClosure functionObj,
                                             void *arg) {
  ItemContainer.EvalArgPredicate evalArgPredicate =
      TL_predicates.get().evalArg.alloc().init(functionObj, arg);
...
static bool compareType(const std::string &string0,
  ...
  if (string0 != nullptr && string0.contains("/")) {
    ...
    if (int0 == -1) {
      ...
      char char1 = int0 + string1.length() < string0.length()
                       ? string0.charAt(int0 + string1.length())
                       : 0;
      return char0 = = 0 && char1 == '/' || char0 == '/' && char1 == 0 ||
                       char0 == '/' && char1 == '/';
    }
  } else {
...
public
ArrayList<InventoryItem> getSome(Predicate<InventoryItem> predicate, int count,
...
public
ArrayList<InventoryItem> getSomeRecurse(Predicate<InventoryItem> predicate,
                                        int count,
                                        ArrayList<InventoryItem> result) {
  ItemContainer.InventoryItemList inventoryItemList =
      TL_itemListPool.get().alloc();
...
public
ArrayList<InventoryItem> getAll(Predicate<InventoryItem> predicate,
                                ArrayList<InventoryItem> result) {
  for (int int0 = 0; int0 < this.Items.size(); int0++) {
    InventoryItem item = this.Items.get(int0);
    if (item == nullptr) {
...
public
ArrayList<InventoryItem> getAllRecurse(Predicate<InventoryItem> predicate,
                                       ArrayList<InventoryItem> result) {
  ItemContainer.InventoryItemList inventoryItemList =
      TL_itemListPool.get().alloc();
...
public
ArrayList<InventoryItem> getSomeCategory(String category, int count,
                                         ArrayList<InventoryItem> result) {
  ItemContainer.CategoryPredicate categoryPredicate =
      TL_predicates.get().category.alloc().init(category);
...
getSomeCategoryRecurse(String category, int count,
                       ArrayList<InventoryItem> result) {
  ItemContainer.CategoryPredicate categoryPredicate =
      TL_predicates.get().category.alloc().init(category);
...
public
ArrayList<InventoryItem> getSomeTag(String tag, int count,
                                    ArrayList<InventoryItem> result) {
  ItemContainer.TagPredicate tagPredicate =
      TL_predicates.get().tag.alloc().init(tag);
...
public
ArrayList<InventoryItem> getSomeTagEval(String tag, LuaClosure functionObj,
                                        int count,
                                        ArrayList<InventoryItem> result) {
  ItemContainer.TagEvalPredicate tagEvalPredicate =
      TL_predicates.get().tagEval.alloc().init(tag, functionObj);
...
public
ArrayList<InventoryItem> getSomeTagEvalArg(String tag, LuaClosure functionObj,
                                           Object arg, int count,
                                           ArrayList<InventoryItem> result) {
  ItemContainer.TagEvalArgPredicate tagEvalArgPredicate =
      TL_predicates.get().tagEvalArg.alloc().init(tag, functionObj, arg);
...
public
ArrayList<InventoryItem> getSomeTagRecurse(String tag, int count,
                                           ArrayList<InventoryItem> result) {
  ItemContainer.TagPredicate tagPredicate =
      TL_predicates.get().tag.alloc().init(tag);
...
public
ArrayList<InventoryItem>
getSomeTagEvalRecurse(String tag, LuaClosure functionObj, int count,
                      ArrayList<InventoryItem> result) {
  ItemContainer.TagEvalPredicate tagEvalPredicate =
      TL_predicates.get().tagEval.alloc().init(tag, functionObj);
...
public
ArrayList<InventoryItem>
getSomeTagEvalArgRecurse(String tag, LuaClosure functionObj, Object arg,
                         int count, ArrayList<InventoryItem> result) {
  ItemContainer.TagEvalArgPredicate tagEvalArgPredicate =
      TL_predicates.get().tagEvalArg.alloc().init(tag, functionObj, arg);
  std::vector arrayList =
      this.getSomeRecurse(tagEvalArgPredicate, count, result);
  TL_predicates.get().tagEvalArg.release(tagEvalArgPredicate);
  return arrayList;
}

public
ArrayList<InventoryItem> getSomeType(String _type, int count,
                                     ArrayList<InventoryItem> result) {
  ItemContainer.TypePredicate typePredicate =
      TL_predicates.get().type.alloc().init(_type);
...
public
ArrayList<InventoryItem> getSomeTypeEval(String _type, LuaClosure functionObj,
                                         int count,
                                         ArrayList<InventoryItem> result) {
  ItemContainer.TypeEvalPredicate typeEvalPredicate =
      TL_predicates.get().typeEval.alloc().init(_type, functionObj);
...
public
ArrayList<InventoryItem> getSomeTypeEvalArg(String _type,
                                            LuaClosure functionObj, Object arg,
                                            int count,
                                            ArrayList<InventoryItem> result) {
  ItemContainer.TypeEvalArgPredicate typeEvalArgPredicate =
      TL_predicates.get().typeEvalArg.alloc().init(_type, functionObj, arg);
...
public
ArrayList<InventoryItem> getSomeTypeRecurse(String _type, int count,
                                            ArrayList<InventoryItem> result) {
  ItemContainer.TypePredicate typePredicate =
      TL_predicates.get().type.alloc().init(_type);
...
public
ArrayList<InventoryItem>
getSomeTypeEvalRecurse(String _type, LuaClosure functionObj, int count,
                       ArrayList<InventoryItem> result) {
  ItemContainer.TypeEvalPredicate typeEvalPredicate =
      TL_predicates.get().typeEval.alloc().init(_type, functionObj);
...
public
ArrayList<InventoryItem>
getSomeTypeEvalArgRecurse(String _type, LuaClosure functionObj, Object arg,
                          int count, ArrayList<InventoryItem> result) {
  ItemContainer.TypeEvalArgPredicate typeEvalArgPredicate =
      TL_predicates.get().typeEvalArg.alloc().init(_type, functionObj, arg);
  std::vector arrayList =
      this.getSomeRecurse(typeEvalArgPredicate, count, result);
  TL_predicates.get().typeEvalArg.release(typeEvalArgPredicate);
  return arrayList;
}

public
ArrayList<InventoryItem> getSomeEval(LuaClosure functionObj, int count,
                                     ArrayList<InventoryItem> result) {
  ItemContainer.EvalPredicate evalPredicate =
      TL_predicates.get().eval.alloc().init(functionObj);
...
public
ArrayList<InventoryItem> getSomeEvalArg(LuaClosure functionObj, Object arg,
                                        int count,
                                        ArrayList<InventoryItem> result) {
  ItemContainer.EvalArgPredicate evalArgPredicate =
      TL_predicates.get().evalArg.alloc().init(functionObj, arg);
...
public
ArrayList<InventoryItem> getSomeEvalRecurse(LuaClosure functionObj, int count,
                                            ArrayList<InventoryItem> result) {
  ItemContainer.EvalPredicate evalPredicate =
      TL_predicates.get().eval.alloc().init(functionObj);
...
public
ArrayList<InventoryItem>
getSomeEvalArgRecurse(LuaClosure functionObj, Object arg, int count,
                      ArrayList<InventoryItem> result) {
  ItemContainer.EvalArgPredicate evalArgPredicate =
      TL_predicates.get().evalArg.alloc().init(functionObj, arg);
...
public
ArrayList<InventoryItem> getAllCategory(String category,
                                        ArrayList<InventoryItem> result) {
  ItemContainer.CategoryPredicate categoryPredicate =
      TL_predicates.get().category.alloc().init(category);
...
public
ArrayList<InventoryItem>
getAllCategoryRecurse(String category, ArrayList<InventoryItem> result) {
  ItemContainer.CategoryPredicate categoryPredicate =
      TL_predicates.get().category.alloc().init(category);
...
public
ArrayList<InventoryItem> getAllTag(String tag,
                                   ArrayList<InventoryItem> result) {
  ItemContainer.TagPredicate tagPredicate =
      TL_predicates.get().tag.alloc().init(tag);
...
public
ArrayList<InventoryItem> getAllTagEval(String tag, LuaClosure functionObj,
                                       ArrayList<InventoryItem> result) {
  ItemContainer.TagEvalPredicate tagEvalPredicate =
      TL_predicates.get().tagEval.alloc().init(tag, functionObj);
...
public
ArrayList<InventoryItem> getAllTagEvalArg(String tag, LuaClosure functionObj,
                                          Object arg,
                                          ArrayList<InventoryItem> result) {
  ItemContainer.TagEvalArgPredicate tagEvalArgPredicate =
      TL_predicates.get().tagEvalArg.alloc().init(tag, functionObj, arg);
...
public
ArrayList<InventoryItem> getAllTagRecurse(String tag,
                                          ArrayList<InventoryItem> result) {
  ItemContainer.TagPredicate tagPredicate =
      TL_predicates.get().tag.alloc().init(tag);
...
public
ArrayList<InventoryItem> getAllTagEvalRecurse(String tag,
                                              LuaClosure functionObj,
                                              ArrayList<InventoryItem> result) {
  ItemContainer.TagEvalPredicate tagEvalPredicate =
      TL_predicates.get().tagEval.alloc().init(tag, functionObj);
...
public
ArrayList<InventoryItem>
getAllTagEvalArgRecurse(String tag, LuaClosure functionObj, Object arg,
                        ArrayList<InventoryItem> result) {
  ItemContainer.TagEvalArgPredicate tagEvalArgPredicate =
      TL_predicates.get().tagEvalArg.alloc().init(tag, functionObj, arg);
...
public
ArrayList<InventoryItem> getAllType(String _type,
                                    ArrayList<InventoryItem> result) {
  ItemContainer.TypePredicate typePredicate =
      TL_predicates.get().type.alloc().init(_type);
...
public
ArrayList<InventoryItem> getAllTypeEval(String _type, LuaClosure functionObj,
                                        ArrayList<InventoryItem> result) {
  ItemContainer.TypeEvalPredicate typeEvalPredicate =
      TL_predicates.get().typeEval.alloc().init(_type, functionObj);
...
public
ArrayList<InventoryItem> getAllTypeEvalArg(String _type, LuaClosure functionObj,
                                           Object arg,
                                           ArrayList<InventoryItem> result) {
  ItemContainer.TypeEvalArgPredicate typeEvalArgPredicate =
      TL_predicates.get().typeEvalArg.alloc().init(_type, functionObj, arg);
...
public
ArrayList<InventoryItem> getAllTypeRecurse(String _type,
                                           ArrayList<InventoryItem> result) {
  ItemContainer.TypePredicate typePredicate =
      TL_predicates.get().type.alloc().init(_type);
...
public
ArrayList<InventoryItem>
getAllTypeEvalRecurse(String _type, LuaClosure functionObj,
                      ArrayList<InventoryItem> result) {
  ItemContainer.TypeEvalPredicate typeEvalPredicate =
      TL_predicates.get().typeEval.alloc().init(_type, functionObj);
...
public
ArrayList<InventoryItem>
getAllTypeEvalArgRecurse(String _type, LuaClosure functionObj, Object arg,
                         ArrayList<InventoryItem> result) {
  ItemContainer.TypeEvalArgPredicate typeEvalArgPredicate =
      TL_predicates.get().typeEvalArg.alloc().init(_type, functionObj, arg);
...
public
ArrayList<InventoryItem> getAllEval(LuaClosure functionObj,
                                    ArrayList<InventoryItem> result) {
  ItemContainer.EvalPredicate evalPredicate =
      TL_predicates.get().eval.alloc().init(functionObj);
...
public
ArrayList<InventoryItem> getAllEvalArg(LuaClosure functionObj, Object arg,
                                       ArrayList<InventoryItem> result) {
  ItemContainer.EvalArgPredicate evalArgPredicate =
      TL_predicates.get().evalArg.alloc().init(functionObj, arg);
...
public
ArrayList<InventoryItem> getAllEvalRecurse(LuaClosure functionObj,
                                           ArrayList<InventoryItem> result) {
  ItemContainer.EvalPredicate evalPredicate =
      TL_predicates.get().eval.alloc().init(functionObj);
...
public
ArrayList<InventoryItem> getAllEvalArgRecurse(LuaClosure functionObj,
                                              Object arg,
                                              ArrayList<InventoryItem> result) {
  ItemContainer.EvalArgPredicate evalArgPredicate =
      TL_predicates.get().evalArg.alloc().init(functionObj, arg);
...
ArrayList<InventoryItem> getSomeEvalArg(LuaClosure functionObj, Object arg,
                                        int count) {
  return this.getSomeEvalArg(functionObj, arg, count,
                             std::make_unique<ArrayList<>>());
}
...
ArrayList<InventoryItem> getSomeTypeEval(String _type, LuaClosure functionObj,
                                         int count) {
  return this.getSomeTypeEval(_type, functionObj, count,
                              std::make_unique<ArrayList<>>());
}
...
ArrayList<InventoryItem> getSomeTypeEvalArg(String _type,
                                            LuaClosure functionObj, Object arg,
                                            int count) {
  return this.getSomeTypeEvalArg(_type, functionObj, arg, count,
                                 std::make_unique<ArrayList<>>());
}

public
ArrayList<InventoryItem> getSomeEvalRecurse(LuaClosure functionObj, int count) {
  return this.getSomeEvalRecurse(functionObj, count,
                                 std::make_unique<ArrayList<>>());
}

public
ArrayList<InventoryItem> getSomeEvalArgRecurse(LuaClosure functionObj,
                                               Object arg, int count) {
  return this.getSomeEvalArgRecurse(functionObj, arg, count,
                                    std::make_unique<ArrayList<>>());
}
...
ArrayList<InventoryItem>
getSomeTagEvalRecurse(String tag, LuaClosure functionObj, int count) {
  return this.getSomeTagEvalRecurse(tag, functionObj, count,
                                    std::make_unique<ArrayList<>>());
}

public
ArrayList<InventoryItem> getSomeTagEvalArgRecurse(String tag,
                                                  LuaClosure functionObj,
                                                  Object arg, int count) {
  return this.getSomeTagEvalArgRecurse(tag, functionObj, arg, count,
                                       std::make_unique<ArrayList<>>());
}
...
ArrayList<InventoryItem>
getSomeTypeEvalRecurse(String _type, LuaClosure functionObj, int count) {
  return this.getSomeTypeEvalRecurse(_type, functionObj, count,
                                     std::make_unique<ArrayList<>>());
}

public
ArrayList<InventoryItem> getSomeTypeEvalArgRecurse(String _type,
                                                   LuaClosure functionObj,
                                                   Object arg, int count) {
  return this.getSomeTypeEvalArgRecurse(_type, functionObj, arg, count,
                                        std::make_unique<ArrayList<>>());
}
...
public
ArrayList<InventoryItem> getAllTagEval(String _type, LuaClosure functionObj) {
  return this.getAllTagEval(_type, functionObj,
                            std::make_unique<ArrayList<>>());
}

public
ArrayList<InventoryItem> getAllTagEvalArg(String _type, LuaClosure functionObj,
                                          Object arg) {
  return this.getAllTagEvalArg(_type, functionObj, arg,
                               std::make_unique<ArrayList<>>());
}

public
ArrayList<InventoryItem> getAllTypeEval(String _type, LuaClosure functionObj) {
  return this.getAllTypeEval(_type, functionObj,
                             std::make_unique<ArrayList<>>());
}
...
ArrayList<InventoryItem> getAllTypeEvalArg(String _type, LuaClosure functionObj,
                                           Object arg) {
  return this.getAllTypeEvalArg(_type, functionObj, arg,
                                std::make_unique<ArrayList<>>());
}
...
ArrayList<InventoryItem> getAllEvalArgRecurse(LuaClosure functionObj,
                                              Object arg) {
  return this.getAllEvalArgRecurse(functionObj, arg,
                                   std::make_unique<ArrayList<>>());
}
...
public
ArrayList<InventoryItem> getAllTypeEvalRecurse(String _type,
                                               LuaClosure functionObj) {
  return this.getAllTypeEvalRecurse(_type, functionObj,
                                    std::make_unique<ArrayList<>>());
}
...
ArrayList<InventoryItem>
getAllTypeEvalArgRecurse(String _type, LuaClosure functionObj, Object arg) {
  return this.getAllTypeEvalArgRecurse(_type, functionObj, arg,
                                       std::make_unique<ArrayList<>>());
}
...
public
ArrayList<InventoryItem> FindAndReturn(String _type, int count) {
...
InventoryItem FindAndReturn(const std::string &_type,
  ...
  if (_type == nullptr) {
    ...
    for (int int0 = 0; int0 < this.Items.size(); int0++) {
      InventoryItem item = this.Items.get(int0);
      if (item.type != nullptr && compareType(_type, item) &&
          !itemToCheck.contains(item)) {
        return item;
      }
    }
...
public
ArrayList<InventoryItem> FindAll(String _type) {
...
InventoryItem FindAndReturnStack(const std::string &_type) {
  for (int int0 = 0; int0 < this.Items.size(); int0++) {
    ...
    if (compareType(_type, item0)) {
      InventoryItem item1 =
          InventoryItemFactory.CreateItem(item0.module + "." + _type);
    ...
    for (int int1 = 0; int1 < this.Items.size(); int1++) {
      ...
      if (item1 instanceof
          InventoryContainer &&
              ...
              ((InventoryContainer)item1)
                  .getItemContainer()
                  .RemoveOneOf(String, insideInv)) {
        return true;
      }
    }
  }
...
int getWeight() {
  if (this.parent instanceof
      IsoPlayer && ((IsoPlayer)this.parent).isGhostMode()) {
    return 0;
  } else {
    float float0 = 0.0F;
...
float getMaxWeight() {
  return this.parent instanceof
      IsoGameCharacter ? ((IsoGameCharacter)this.parent).getMaxWeight()
                       : this.Capacity;
}
...
float getCapacityWeight() {
  if (this.parent instanceof IsoPlayer) {
    if (Core.bDebug && ((IsoPlayer)this.parent).isGhostMode() ||
        !((IsoPlayer)this.parent).isAccessLevel("None") &&
            ((IsoPlayer)this.parent).isUnlimitedCarry()) {
      return 0.0F;
    }
  ...
  return this.parent instanceof
      IsoGameCharacter ? ((IsoGameCharacter)this.parent).getInventoryWeight()
                       : this.getContentsWeight();
}

bool isEmpty() { return this.Items == nullptr || this.Items.isEmpty(); }
  ...
  if (square0 == nullptr) {
    ...
    if (boolean0 && square0.getRoom() != nullptr) {
      ...
      if (boolean0 && square0.getRoom() == nullptr) {
        ...
        if (this.isSquareInRoom(square1) || this.isSquareInRoom(square2) ||
            this.isSquareInRoom(square3) || this.isSquareInRoom(square4)) {
          return true;
        }
      }
...
float getTemprature() {
  if (this.customTemperature != 0.0F) {
    ...
    bool boolean0 = false;
    if (this.getParent() != nullptr &&
        this.getParent().getSprite() != nullptr) {
...
bool isTemperatureChanging() {
  return this.parent instanceof
      IsoStove ? ((IsoStove)this.parent).isTemperatureChanging() : false;
}

public
ArrayList<InventoryItem> save(ByteBuffer output,
                              IsoGameCharacter noCompress) throws IOException {
  GameWindow.WriteString(output, this.type);
  output.put((byte)(this.bExplored ? 1 : 0));
  std::vector arrayList = CompressIdenticalItems.save(output, this.Items, null);
...
public
ArrayList<InventoryItem> save(ByteBuffer output) throws IOException {
  return this.save(output, nullptr);
}

public
ArrayList<InventoryItem> load(ByteBuffer input,
                              int WorldVersion) throws IOException {
  this.type = GameWindow.ReadString(input);
  this.bExplored = input.get() == 1;
  std::vector arrayList = CompressIdenticalItems.load(
      input, WorldVersion, this.Items, this.IncludingObsoleteItems);
...
bool isDrawDirty() { return this.drawDirty; }

void setDrawDirty(bool b) { this.drawDirty = b; }
 ...
 */
void setDirty(bool _dirty) { this.dirty = _dirty; }

/**
 * @return the IsDevice
 */
bool isIsDevice() { return this.IsDevice; }

/**
 *
 * @param _IsDevice the IsDevice to set
 */
void setIsDevice(bool _IsDevice) { this.IsDevice = _IsDevice; }

/**
 * @return the ageFactor
 */
float getAgeFactor() { return this.ageFactor; }

/**
 *
 * @param _ageFactor the ageFactor to set
 */
void setAgeFactor(float _ageFactor) { this.ageFactor = _ageFactor; }

/**
 * @return the CookingFactor
 */
float getCookingFactor() { return this.CookingFactor; }
 ...
 */
public
ArrayList<InventoryItem> getItems() { return this.Items; }

/**
 *
 * @param _Items the Items to set
 */
void setItems(ArrayList<InventoryItem> _Items) { this.Items = _Items; }

/**
 * @return the parent
 */
IsoObject getParent() { return this.parent; }

/**
 *
 * @param _parent the parent to set
 */
void setParent(IsoObject _parent) { this.parent = _parent; }

/**
 * @return the SourceGrid
 */
IsoGridSquare getSourceGrid() { return this.SourceGrid; }

/**
 *
 * @param _SourceGrid the SourceGrid to set
 */
void setSourceGrid(IsoGridSquare _SourceGrid) { this.SourceGrid = _SourceGrid; }
...
public
ArrayList<InventoryItem> getAllWaterFillables() {
  tempList.clear();
...
static int getUses(ItemContainer.InventoryItemList inventoryItemList) {
  ...
  for (int int1 = 0; int1 < inventoryItemList.size(); int1++) {
    DrainableComboItem drainableComboItem =
        Type.tryCastTo(inventoryItemList.get(int1), DrainableComboItem.class);
...
bool containsRecursive(InventoryItem item) {
  for (int int0 = 0; int0 < this.getItems().size(); int0++) {
    ...
    if (_item instanceof InventoryContainer && ((InventoryContainer)_item)
                                                   .getInventory()
                                                   .containsRecursive(item)) {
      return true;
    }
  }
...
int getItemCountFromTypeRecurse(const std::string &_type) {
  ...
  for (int int1 = 0; int1 < this.getItems().size(); int1++) {
    ...
    if (item instanceof InventoryContainer) {
      int int2 = ((InventoryContainer)item)
                     .getInventory()
                     .getItemCountFromTypeRecurse(_type);
      int0 += int2;
    }
  }
...
float getCustomTemperature() { return this.customTemperature; }
...
InventoryItem getItemFromType(const std::string &_type, IsoGameCharacter chr,
  ...
  for (int int0 = 0; int0 < this.getItems().size(); int0++) {
    ...
    if (!item0.getFullType() == _type) && !item0.getType() == _type)) {
        if (includeInv && item0 instanceof
            InventoryContainer &&
                ((InventoryContainer)item0).getInventory() != nullptr &&
                !inventoryItemList.contains(item0)) {
          inventoryItemList.add(item0);
    ...
    else if ((!notEquipped || chr == nullptr ||
              !chr.isEquippedClothing(item0)) &&
             this.testBroken(ignoreBroken, item0)) {
      TL_itemListPool.get().release(inventoryItemList);
      return item0;
    }
  }
  ...
  for (int int1 = 0; int1 < inventoryItemList.size(); int1++) {
    ItemContainer container =
        ((InventoryContainer)inventoryItemList.get(int1)).getInventory();
    InventoryItem item1 = container.getItemFromType(_type, chr, notEquipped,
                                                    ignoreBroken, includeInv);
    if (item1 != nullptr) {
      TL_itemListPool.get().release(inventoryItemList);
      return item1;
    }
...
public
ArrayList<InventoryItem> getItemsFromType(String _type) {
  return this.getAllType(_type);
}

public
ArrayList<InventoryItem> getItemsFromFullType(String _type) {
  return _type != nullptr && _type.contains(".")
             ? this.getAllType(_type)
             : std::make_unique<ArrayList<>>();
}
...
public
ArrayList<InventoryItem> getItemsFromType(String _type, boolean includeInv) {
  return includeInv ? this.getAllTypeRecurse(_type) : this.getAllType(_type);
}

public
ArrayList<InventoryItem> getItemsFromCategory(String category) {
...
void requestSync() {
  if (GameClient.bClient) {
    if (this.parent == nullptr || this.parent.square == nullptr ||
        this.parent.square.chunk == nullptr) {
      return;
    }

    GameClient.instance.worldObjectsSyncReq.putRequestSyncIsoChunk(
        this.parent.square.chunk);
  }
}
...
void requestServerItemsForContainer() {
  if (this.parent != nullptr && this.parent.square != nullptr) {
    ...
    if (int0 == -1 && this.parent.square.getStaticMovingObjects().indexOf(
                          this.parent) != -1) {
      ...
      byteBufferWriter.putInt(
          ((IsoWorldInventoryObject)this.parent).getItem().id);
...
void sendContentsToRemoteContainer(UdpConnection connection) {
  ...
  byteBufferWriter.putByte(
      (byte)this.parent.square.getObjects().indexOf(this.parent));
    ...
    if (item instanceof
        InventoryContainer &&
            ...
            !((InventoryContainer)item)
                 ...
                 .getItems()
                 .isEmpty()) {
      item = ((InventoryContainer)item)
                 .getItemContainer()
                 .getItemWithIDRecursiv(id);
      if (item != nullptr) {
        return item;
      }
    }
...
bool removeItemWithIDRecurse(int id) {
  for (int int0 = 0; int0 < this.Items.size(); int0++) {
    ...
    if (item instanceof
        InventoryContainer && ((InventoryContainer)item)
                                  .getInventory()
                                  .removeItemWithIDRecurse(id)) {
      return true;
    }
  }
...
bool isHasBeenLooted() { return this.hasBeenLooted; }
...
IsoGameCharacter getCharacter() {
  if (this.getParent() instanceof IsoGameCharacter) {
    ...
    return this.containingItem != nullptr &&
                   this.containingItem.getContainer() != nullptr
               ? this.containingItem.getContainer().getCharacter()
               : nullptr;
  }
}

void emptyIt() { this.Items = std::make_unique<ArrayList<>>(); }

public
LinkedHashMap<String, InventoryItem> getItems4Admin() {
  ...
  for (int int0 = 0; int0 < this.getItems().size(); int0++) {
    ...
    if (item.getCat() != ItemType.Drainable &&
      ...
      ((InventoryItem)linkedHashMap.get(item.getFullType()))
          .setCount(((InventoryItem)linkedHashMap.get(item.getFullType()))
                        .getCount() +
                    1);
...
getAllItems(LinkedHashMap<String, InventoryItem> items, boolean inInv) {
  if (items == nullptr) {
  ...
  for (int int0 = 0; int0 < this.getItems().size(); int0++) {
    ...
    if (item.getCat() != ItemType.Drainable &&
        item.getCat() != ItemType.Weapon &&
        items.get(item.getFullType()) != nullptr) {
      ((InventoryItem)items.get(item.getFullType()))
          .setCount(((InventoryItem)items.get(item.getFullType())).getCount() +
                    1);
    ...
    if (item instanceof
        InventoryContainer &&
            ...
            !((InventoryContainer)item)
                 ...
                 .getItems()
                 .isEmpty()) {
      items = ((InventoryContainer)item)
                  .getItemContainer()
                  .getAllItems(items, true);
    }
  }
...
bool isExistYet() {
  if (!SystemDisabler.doWorldSyncEnable) {
  ...
  } else if (this.containingItem != nullptr &&
             this.containingItem.worldItem != nullptr) {
    ...
    return !square.getObjects().contains(this.parent)
               ? false
               : this.parent.getContainerIndex(this) != -1;
  }
}
...
VehiclePart getVehiclePart() { return this.vehiclePart; }

private
static final class CategoryPredicate implements Predicate<InventoryItem> {
  std::string category;
...
private static final class Comparators {
  ObjectPool<ItemContainer.ConditionComparator> condition =
      new ObjectPool<>(ItemContainer.ConditionComparator::new);
  ObjectPool<ItemContainer.EvalComparator> eval =
      new ObjectPool<>(ItemContainer.EvalComparator::new);
  ObjectPool<ItemContainer.EvalArgComparator> evalArg =
      new ObjectPool<>(ItemContainer.EvalArgComparator::new);
}

private static final class ConditionComparator implements
...
private static final class EvalArgComparator implements
    Comparator<InventoryItem> {
  LuaClosure functionObj;
  void *arg;
  ...
  int compare(InventoryItem item0, InventoryItem item1) {
    LuaReturn luaReturn = LuaManager.caller.protectedCall(
        LuaManager.thread, this.functionObj, item0, item1, this.arg);
    if (luaReturn.isSuccess() && !luaReturn.isEmpty() && luaReturn.getFirst()
            instanceof Double) {
...
private static final class EvalArgPredicate implements
  ...
  bool test(InventoryItem item) {
    return LuaManager.caller.protectedCallBoolean(LuaManager.thread,
                                                  this.functionObj, item,
...
private static final class EvalComparator implements Comparator<InventoryItem> {
  ...
  int compare(InventoryItem item0, InventoryItem item1) {
    LuaReturn luaReturn = LuaManager.caller.protectedCall(
        LuaManager.thread, this.functionObj, item0, item1);
    if (luaReturn.isSuccess() && !luaReturn.isEmpty() && luaReturn.getFirst()
            instanceof Double) {
      double double0 = (Double)luaReturn.getFirst();
      return Double.compare(double0, 0.0);
...
private static final class EvalPredicate implements Predicate<InventoryItem> {
  ...
  bool test(InventoryItem item) {
    return LuaManager.caller.protectedCallBoolean(
               LuaManager.thread, this.functionObj, item) == Boolean.TRUE;
  }
}

private static final class InventoryItemList extends ArrayList<InventoryItem> {
  bool equals(void *object) { return this = = object; }
}

private static final class InventoryItemListPool extends
    ObjectPool<ItemContainer.InventoryItemList> {
public
  InventoryItemListPool() { super(ItemContainer.InventoryItemList::new); }
...
private static final class Predicates {
  final ObjectPool<ItemContainer.CategoryPredicate> category =
      new ObjectPool<>(ItemContainer.CategoryPredicate::new);
  final ObjectPool<ItemContainer.EvalPredicate> eval =
      new ObjectPool<>(ItemContainer.EvalPredicate::new);
  final ObjectPool<ItemContainer.EvalArgPredicate> evalArg =
      new ObjectPool<>(ItemContainer.EvalArgPredicate::new);
  final ObjectPool<ItemContainer.TagPredicate> tag =
      new ObjectPool<>(ItemContainer.TagPredicate::new);
  final ObjectPool<ItemContainer.TagEvalPredicate> tagEval =
      new ObjectPool<>(ItemContainer.TagEvalPredicate::new);
  final ObjectPool<ItemContainer.TagEvalArgPredicate> tagEvalArg =
      new ObjectPool<>(ItemContainer.TagEvalArgPredicate::new);
  final ObjectPool<ItemContainer.TypePredicate> type =
      new ObjectPool<>(ItemContainer.TypePredicate::new);
  final ObjectPool<ItemContainer.TypeEvalPredicate> typeEval =
      new ObjectPool<>(ItemContainer.TypeEvalPredicate::new);
  final ObjectPool<ItemContainer.TypeEvalArgPredicate> typeEvalArg =
      new ObjectPool<>(ItemContainer.TypeEvalArgPredicate::new);
}

private static final class TagEvalArgPredicate implements
    Predicate<InventoryItem> {
  std::string tag;
  LuaClosure functionObj;
  void *arg;

  ItemContainer.TagEvalArgPredicate init(String string, LuaClosure luaClosure,
                                         Object object) {
    this.tag = string;
    this.functionObj = Objects.requireNonNull(luaClosure);
    this.arg = object;
  ...
  bool test(InventoryItem item) {
    return item.hasTag(this.tag) && LuaManager.caller.protectedCallBoolean(
                                        LuaManager.thread, this.functionObj,
                                        item, this.arg) == Boolean.TRUE;
  }
}

private static final class TagEvalPredicate implements
    Predicate<InventoryItem> {
  std::string tag;
  LuaClosure functionObj;
...
private static final class TagPredicate implements Predicate<InventoryItem> {
  ...
  bool test(InventoryItem item) { return item.hasTag(this.tag); }
}

private static final class TypeEvalArgPredicate implements
    Predicate<InventoryItem> {
  std::string type;
  LuaClosure functionObj;
  void *arg;

  ItemContainer.TypeEvalArgPredicate init(String string, LuaClosure luaClosure,
                                          Object object) {
    this.type = string;
    this.functionObj = Objects.requireNonNull(luaClosure);
    this.arg = object;
  ...
  bool test(InventoryItem item) {
    return ItemContainer.compareType(this.type, item) &&
           LuaManager.caller.protectedCallBoolean(LuaManager.thread,
                                                  this.functionObj, item,
                                                  this.arg) == Boolean.TRUE;
  }
}

private static final class TypeEvalPredicate implements
    Predicate<InventoryItem> {
  std::string type;
  LuaClosure functionObj;
