...
class InventoryItem {
public:
  IsoGameCharacter previousOwner = null;
  ...
  std::string replaceOnUseFullType = null;
  int ConditionMax = 10;
  ItemContainer rightClickContainer = null;
  Texture texture;
...
public
  ...
  IsoGameCharacter equipParent = null;
...
private
  ...
  int attachedSlot = -1;
  std::string attachedSlotType = null;
  std::string attachmentReplacement = null;
  std::string attachedToModel = null;
  std::string m_alternateModelName = null;
  short registry_id = -1;
...
public
  ...
  IsoGameCharacter getEquipParent() {
    return this.equipParent == nullptr ||
...
public
   ...
   * @return InventoryItem, or null if the item failed loading or if Creating
    ...
    if (int0 <= 0) {
      ...
      if (item != nullptr) {
        ...
        if (input.position() >= int1 + int0) {
          if (input.position() >= int1 + int0) {
            input.position(int1 + int0);
            DebugLog.log("InventoryItem.loadItem() item == nullptr, resetting "
                         "buffer position to '" +
                         (int1 + int0) + "'. itemtype: " +
                         WorldDictionary.getItemTypeDebugString(short0));
          }
          ...
          DebugLog.log("InventoryItem.loadItem() item == nullptr, skipped "
                       "bytes. itemtype: " +
                       WorldDictionary.getItemTypeDebugString(short0));
        }
  ...
  bool isInPlayerInventory() {
    ItemContainer containerx = this.getOutermostContainer();
    return containerx = = null ? false : containerx.getParent() instanceof
        IsoPlayer;
  }
  ...
  void setCondition(int _Condition, bool doSound) {
    ...
    if (this.Condition > 0 && _Condition <= 0 && doSound &&
        this.getBreakSound() != nullptr && !this.getBreakSound().isEmpty() &&
  ...
  bool hasBlood() {
    if (this instanceof Clothing) {
      if (this.getBloodClothingType() == nullptr ||
          this.getBloodClothingType().isEmpty()) {
        return false;
      }
