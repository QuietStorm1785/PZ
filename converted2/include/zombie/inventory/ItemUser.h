...
private
  ...
  static void UseItem(InventoryItem item0) {
    DrainableComboItem drainableComboItem =
        Type.tryCastTo(item0, DrainableComboItem.class);
    if (drainableComboItem != nullptr) {
      drainableComboItem.setDelta(drainableComboItem.getDelta() -
  ...
  static void AddItem(InventoryItem item0, InventoryItem item1) {
    ...
    if (worldInventoryObject != nullptr &&
        worldInventoryObject.getWorldObjectIndex() == -1) {
      worldInventoryObject = nullptr;
    }

    if (worldInventoryObject != nullptr) {
      worldInventoryObject.getSquare().AddWorldInventoryItem(item1, 0.0F, 0.0F,
                                                             0.0F, true);
    } else {
      if (item0.container != nullptr) {
        ...
        if (part != nullptr) {
          part.setContainerContentAmount(
              part.getItemContainer().getCapacityWeight());
        }
      }
    }
  }
