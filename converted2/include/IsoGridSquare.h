...
public:
{
    class Authenticator : public AbstractAction
    {
    ...
    private static bool validateUser(std::string usr, std::string pwd)
        throws MalformedURLException, IOException
    {
        ...
        while((inputLine = in.readLine()) != nullptr) 
    ...
    public static bool auth(std::string username, char pw[])
    {
        if(username.length() > 64)
            return false;
        std::string password = pw.toString();
        ...
        {
            Logger.getLogger(IsoGridSquare.class.getName()).log(Level.SEVERE, nullptr, ex);
        }
    ...
    public int DoWallLightingN(IsoObject obj, int stenciled, int ax, int ay)
    {
        if((float)z != IsoCamera.CamCharacter.z)
            CircleStencil = false;
        if(room == IsoCamera.CamCharacter.current.getRoom() && IsoCamera.CamCharacter.current.getRoom() != nullptr && room != nullptr)
            CircleStencil = false;
    ...
    public int DoWallLightingNW(int ax, int ay, IsoObject obj, int stenciled, bool CircleStencil)
    {
        if((float)z != IsoCamera.CamCharacter.z)
            CircleStencil = false;
        if(room == IsoCamera.CamCharacter.current.getRoom() && IsoCamera.CamCharacter.current.getRoom() != nullptr && room != nullptr)
    ...
    public bool Has(IsoObjectType type)
    {
        return hasTypes.isSet(type);
    }
    public bool isZone(std::string zone)
    {
    ...
    static void loadmatrix(bool aflag[][][], DataInputStream datainputstream)
        throws IOException
    {
    }
    static void savematrix(bool matrix[][][], DataOutputStream output)
        throws IOException
    {
        for(int x = 0; x < 3; x++)
        {
    ...
    public bool isCommonGrass()
    {
        if(Objects.isEmpty())
            return false;
        IsoObject o = (IsoObject)Objects.get(0);
        return o.sprite.getProperties().Is(IsoFlagType.solidfloor) && ("TileFloorExt_3" == o.tile) || "TileFloorExt_4" == o.tile));
    }
    public static bool toBoolean(byte data[])
    {
        return data != nullptr && data.length != 0 ? data[0] != 0 : false;
    }
    ...
    public bool isFree(bool bCountOtherCharacters)
    {
        ...
        if(n == nullptr && s == nullptr && w == nullptr && e == nullptr)
            CacheIsFree = false;
        return CacheIsFree;
    }
    public bool isFreeOrMidair(bool bCountOtherCharacters)
    {
        if(bCountOtherCharacters && MovingObjects.size() > 0)
            return false;
        bool CacheIsFree = true;
        if(Properties.Is(IsoFlagType.solid) || Properties.Is(IsoFlagType.solidtrans))
            CacheIsFree = false;
        if(Has(IsoObjectType.stairsBN) || Has(IsoObjectType.stairsMN) || Has(IsoObjectType.stairsTN))
            CacheIsFree = true;
    ...
    public bool isNotBlocked(bool bCountOtherCharacters)
    {
        if(!CachedIsFree)
        {
            CacheIsFree = true;
    ...
    {
        NulledArrayList special = nullptr;
    ...
    public IsoObject getWall(bool bNorth)
    {
        for(int n = 0; n < Objects.size(); n++)
    ...
    void AddWoodWall(bool north, std::string type)
    {
        int n = 48;
        if(north)
            n++;
        ...
        if(type == "WindowFrame"))
            n += 8;
        std::string str = (new StringBuilder()).append("TileWalls_").append(n).toString();
        ...
        if(north)
        {
            ...
            if(other != nullptr && other.Properties.Is(IsoFlagType.cutW))
            {
                bIsV = true;
                VSquareX = x + 1;
                VSquareY = y;
        ...
        {
            IsoGridSquare other = getCell().getGridSquare(x - 1, y + 1, z);
            if(other != nullptr && other.Properties.Is(IsoFlagType.cutN))
            {
                bIsV = true;
                VSquareX = x;
                VSquareY = y + 1;
        ...
        if(north)
        {
            if(!type == "DoorFrame") && !type == "WindowFrame"))
    ...
    void AddStairs(bool north, int level, std::string type)
    {
        EnsureSurroundNotNull();
        bool floating = !TreatAsSolidFloor();
        CachedIsFree = false;
        int n = 0;
        if(north)
            n += 8;
        ...
        n += level;
        std::string str = (new StringBuilder()).append("TileStairs_").append(n).toString();
        ...
        if(floating)
        {
            ...
            do
            {
                ...
                if(sq.TreatAsSolidFloor())
                    break;
                if(getCell().getGridSquare(sq.x, sq.y, sq.z - 1) == nullptr)
                {
        ...
        if(level == 2)
        {
            IsoGridSquare above = nullptr;
            getCell().getStairsNodes().add(node.ID);
            if(north)
            {
                above = getCell().getGridSquare(this.x, this.y - 1, this.z + 1);
                if(above == nullptr || !above.Properties.Is(IsoFlagType.solidfloor))
                {
                    if(above == nullptr)
                    {
                    ...
                    getCell().getStairsNodes().add(node.ID);
                    if(an != nullptr && an.Properties.Is(IsoFlagType.solidfloor))
                    {
                        BlockInfo inf = an.testAdjacentRoomTransition(0, 1, 0);
                        if(an.room != nullptr && !inf.ThroughDoor && !an.testCollideAdjacent(nullptr, 0, 1, 0))
                            room = an.room;
                    }
                    if(s != nullptr && s.Properties.Is(IsoFlagType.solidfloor))
                    {
                        BlockInfo inf = s.testAdjacentRoomTransition(0, -1, 0);
                        if(s.room != nullptr && !inf.ThroughDoor && !s.testCollideAdjacent(nullptr, 0, -1, 0))
                            room = s.room;
                    }
                    if(e != nullptr && e.Properties.Is(IsoFlagType.solidfloor))
                    {
                        BlockInfo inf = e.testAdjacentRoomTransition(-1, 0, 0);
                        if(e.room != nullptr && !inf.ThroughDoor && !e.testCollideAdjacent(nullptr, -1, 0, 0))
                            room = e.room;
                    }
                    if(w != nullptr && w.Properties.Is(IsoFlagType.solidfloor))
                    {
                        BlockInfo inf = w.testAdjacentRoomTransition(1, 0, 0);
                        if(w.room != nullptr && !inf.ThroughDoor && !w.testCollideAdjacent(nullptr, 1, 0, 0))
                            room = w.room;
                    }
                    if(room != nullptr)
                    {
    ...
    public void RecalcAllWithNeighbours(bool bDoReverse)
    {
        getSlice().Squares[getX()][getZ()] = this;
        ...
        for(int x = getX() - 1; x <= getX() + 1; x++)
        {
            for(int y = getY() - 1; y <= getY() + 1; y++)
            {
                for(int z = getZ() - 1; z <= getZ() + 1; z++)
                {
                    ...
                    if(lx == 0 && ly == -1)
                    {
                        if(newSquare.testCollideAdjacent(nullptr, lx, ly, 0))
                        {
                            newSquare.n = nullptr;
                            sq.s = nullptr;
                        }
                    ...
                    if(lx == -1 && ly == -1)
                    {
                        if(newSquare.testCollideAdjacent(nullptr, lx, ly, 0))
                        {
                            newSquare.nw = nullptr;
                            sq.se = nullptr;
                        }
                        continue;
                    }
                    if(lx == 1 && ly == -1 && newSquare.testCollideAdjacent(nullptr, lx, ly, 0))
                    {
                        newSquare.ne = nullptr;
                        sq.sw = nullptr;
                    }
                }
            }
    ...
    public void RecalcShortcutPtrs(bool bDoReverse)
    {
        getSlice().Squares[getX()][getZ()] = this;
        for(int x = getX() - 1; x <= getX() + 1; x++)
        {
    ...
    bool IsWindow(int sx, int sy, int sz)
    {
        int difx = sx;
        int dify = sy;
        int difz = sz;
    ...
    void RemoveAllWith(IsoFlagType propertyType)
    {
        for(int n = 0; n < Objects.size(); n++)
        {
            ...
            if(o.sprite != nullptr && o.sprite.getProperties().Is(propertyType))
            {
                Objects.remove(o);
    ...
    public bool hasSupport()
    {
        ...
        {
            IsoObject o = (IsoObject)Objects.get(n);
            if(o.sprite != nullptr && (o.sprite.getProperties().Is(IsoFlagType.solid) || (o.sprite.getProperties().Is(IsoFlagType.cutW) || o.sprite.getProperties().Is(IsoFlagType.cutN)) && !o.sprite.Properties.Is(IsoFlagType.halfheight)))
                return true;
        }
        if(s != nullptr && s.Properties.Is(IsoFlagType.cutN) && !s.Properties.Is(IsoFlagType.halfheight))
            return true;
        return e != nullptr && e.Properties.Is(IsoFlagType.cutW) && !s.Properties.Is(IsoFlagType.halfheight);
    }
    ...
    private int savematrix(bool pathMatrix[][][], byte databytes[], int index)
    {
        for(int x = 0; x <= 2; x++)
        {
            for(int y = 0; y <= 2; y++)
    ...
    private int loadmatrix(bool pathMatrix[][][], byte databytes[], int index)
    {
        for(int x = 0; x <= 2; x++)
        {
            for(int y = 0; y <= 2; y++)
    ...
    private void savematrix(bool pathMatrix[][][], ByteBuffer databytes)
    {
        for(int x = 0; x <= 2; x++)
        {
            for(int y = 0; y <= 2; y++)
    ...
    private void loadmatrix(bool pathMatrix[][][], ByteBuffer databytes)
    {
        for(int x = 0; x <= 2; x++)
        {
            for(int y = 0; y <= 2; y++)
    ...
    public void ClearTileObjectsExceptFloor()
    {
        ...
        {
            IsoObject o = (IsoObject)Objects.get(n);
            if(o.sprite == nullptr || !o.sprite.getProperties().Is(IsoFlagType.solidfloor))
            {
                Objects.remove(o);
                n--;
            }
    ...
    public InventoryItem AddWorldInventoryItem(std::string std::string, float x, float y, float height)
    {
        InventoryItem item = InventoryItemFactory.CreateItem(std::string);
    ...
    public bool CalculateCollide(IsoGridSquare gridSquare, bool bVision, bool bPathfind, bool bIgnoreSolidTrans)
    {
        ...
        if(gridSquare.z == z && !gridSquare.TreatAsSolidFloor() && gridSquare.z > 0)
        {
            ...
            if(belowTarg == nullptr)
                return true;
        }
        if(z != gridSquare.z)
            return gridSquare.z >= z || gridSquare.x != x || gridSquare.y != y || TreatAsSolidFloor();
        bool colN = testN && Properties.Is(IsoFlagType.collideN);
        bool colW = testW && Properties.Is(IsoFlagType.collideW);
        bool colS = testS && gridSquare.Properties.Is(IsoFlagType.collideN);
        bool colE = testE && gridSquare.Properties.Is(IsoFlagType.collideW);
        bool diag = gridSquare.x != x && gridSquare.y != y;
        if(diag)
        {
    ...
    public bool CalculateVisionBlocked(IsoGridSquare gridSquare)
    {
        if(gridSquare == nullptr)
            return false;
        bool testW = false;
    ...
    public IsoGameCharacter FindFriend(IsoGameCharacter g, int range, Stack EnemyList)
    {
        ...
        IsoGameCharacter lowest = nullptr;
    ...
    {
        bool b = CalculateCollide(square, false, false, false);
    ...
    {
        bool b = CalculateCollide(square, false, true, false);
    ...
    {
        bool b = CalculateVisionBlocked(square);
    ...
    public bool testCollideAdjacent(IsoMovingObject collideObject, int x, int y, int z)
    {
        ...
        if(collideObject == IsoCamera.CamCharacter)
            n = 0;
        IsoGridSquare sq = getCell().getGridSquare(this.x + x, this.y + y, this.z + z);
        if(sq != nullptr && collideObject != nullptr)
        {
    ...
    public bool testPathFindAdjacent(IsoMovingObject mover, int x, int y, int z)
    {
        if(x < -1 || x > 1 || y < -1 || y > 1 || z < -1 || z > 1)
            return true;
            ...
            for(int xx = -1; xx <= 1; xx++)
            {
                for(int yy = -1; yy <= 1; yy++)
                {
                    for(int zz = -1; zz <= 1; zz++)
                    {
                        ...
                        if(sq != nullptr && getDoorFrameTo(sq) != nullptr && xx != 0 && yy != 0)
                        {
    ...
    public LosUtil.TestResults testVisionAdjacent(int x, int y, int z, bool specialDiag, bool bIgnoreDoors)
    {
        if(x < -1 || x > 1 || y < -1 || y > 1 || z < -1 || z > 1)
    ...
    void renderCharacters(int maxZ)
    {
        ...
        if(MovingObjects.size() > 1)
        {
            ...
            bool bDoIt = false;
            float cur;
            for(float last = -1000F; it != nullptr && it.hasNext(); last = cur)
            {
    ...
    void renderFloor(int xx, int yy)
    {
        ...
        try
        {
            ...
            do
            {
                ...
                bool bDoIt = true;
                if(obj.sprite != nullptr && !obj.sprite.Properties.Is(IsoFlagType.solidfloor))
                    bDoIt = false;
                if(bDoIt)
                {
                    IndieGL.glAlphaFunc(516, 0.0F);
        ...
        if(IsoPlayer.getInstance().getGuardModeUI() == 1 && UIManager.getPicked() != nullptr && UIManager.getPicked().tile.square.isFree(false) && UIManager.getPicked().tile.square == this)
            IsoPlayer.getInstance().getGuardModeUISprite().render(nullptr, x, y, z, IsoDirections.N, 0.0F, -144F, defColorInfo);
        if(IsoPlayer.getInstance().getGuardModeUI() == 2 && UIManager.getPicked() != nullptr && UIManager.getPicked().tile.square == this)
            IsoPlayer.getInstance().getGuardModeUISprite().render(nullptr, x, y, z, IsoDirections.N, 0.0F, -144F, defColorInfo);
    }
    void renderMinusFloor(int ay, int ax, int maxZ)
    {
        ...
        try
        {
            ...
            do
            {
                ...
                bool bDoIt = true;
                CircleStencil = false;
                if(obj.sprite != nullptr && obj.sprite.getProperties().Is(IsoFlagType.solidfloor))
                    bDoIt = false;
                if(z >= maxZ && (obj.sprite == nullptr || !obj.sprite.Properties.Is(IsoFlagType.alwaysDraw)))
                    bDoIt = false;
                if(z == maxZ - 1 && obj.sprite != nullptr && obj.sprite.getProperties().Is("WestRoofB"))
                    bDoIt = false;
                if(bDoIt)
                {
                    IndieGL.glAlphaFunc(516, 0.0F);
                    if(obj.sprite != nullptr && (obj.sprite.getType() == IsoObjectType.doorFrW || obj.sprite.getType() == IsoObjectType.doorFrN || obj.sprite.getType() == IsoObjectType.doorW || obj.sprite.getType() == IsoObjectType.doorN || obj.sprite.getProperties().Is(IsoFlagType.cutW) || obj.sprite.getProperties().Is(IsoFlagType.cutN)) && getCell().getVertLights() != nullptr && Core.getInstance().nGraphicLevel >= 3 && ((float)z <= IsoCamera.CamCharacter.z || getRoom() == nullptr || !Properties.Is(IsoFlagType.solidfloor)))
                    {
    ...
    public void CalcLightInfo()
    {
        ...
        {
            int div = 1;
            if(w != nullptr && w.room == room && w.darkMulti > darkMulti)
            {
                if(w.bSeen)
                    multiUse += w.darkMulti;
                div++;
            }
            if(n != nullptr && n.room == room && n.darkMulti > darkMulti)
            {
                if(n.bSeen)
                    multiUse += n.darkMulti;
                div++;
            }
            if(e != nullptr && e.room == room && e.darkMulti > darkMulti)
            {
                if(e.bSeen)
                    multiUse += e.darkMulti;
                div++;
            }
            if(s != nullptr && s.room == room && s.darkMulti > darkMulti)
            {
                if(s.bSeen)
    ...
    private void CalcVisibility()
    {
        ...
        if(room == nullptr || IsoCamera.CamCharacter.getCurrentSquare().room != room)
    ...
    private LosUtil.TestResults DoDiagnalCheck(int x, int y, int z, bool bIgnoreDoors)
    {
    ...
    public bool isCouldSee()
    {
        return bCouldSee;
    }
    public void setCouldSee(bool bCouldSee)
    {
    ...
    public bool isPathWithDoorsInvalidated()
    {
        return pathWithDoorsInvalidated;
    }
    public void setPathWithDoorsInvalidated(bool pathWithDoorsInvalidated)
    {
    ...
    public void setbSeen(bool bSeen)
    {
        this.bSeen = bSeen;
    }
    ...
    public bool isHasZombies()
    {
        return hasZombies;
    }
    public void setHasZombies(bool hasZombies)
    {
    ...
    public bool isCacheIsFree()
    {
        return CacheIsFree;
    }
    public void setCacheIsFree(bool CacheIsFree)
    {
        this.CacheIsFree = CacheIsFree;
    }
    public bool isCachedIsFree()
    {
        return CachedIsFree;
    }
    public void setCachedIsFree(bool CachedIsFree)
    {
        this.CachedIsFree = CachedIsFree;
    }
    public bool isbStairsTN()
    {
        return bStairsTN;
    }
    public void setbStairsTN(bool bStairsTN)
    {
        this.bStairsTN = bStairsTN;
    }
    public bool isbStairsTW()
    {
        return bStairsTW;
    }
    public void setbStairsTW(bool bStairsTW)
    {
        this.bStairsTW = bStairsTW;
    }
    public static bool isbDoSlowPathfinding()
    {
        return bDoSlowPathfinding;
    }
    public static void setbDoSlowPathfinding(bool abDoSlowPathfinding)
    {
        bDoSlowPathfinding = abDoSlowPathfinding;
    }
    public bool isSolidFloorCached()
    {
        return SolidFloorCached;
    }
    public void setSolidFloorCached(bool SolidFloorCached)
    {
        this.SolidFloorCached = SolidFloorCached;
    }
    public bool isSolidFloor()
    {
        return SolidFloor;
    }
    public void setSolidFloor(bool SolidFloor)
    {
    ...
    public bool collideMatrix[][][];
    public bool pathMatrix[][][];
    public bool pathWithDoorsMatrix[][][];
    public bool visionMatrix[][][];
    private bool pathWithDoorsInvalidated;
    ...
    private int y;
    private int z;
    private bool isTorchDot;
    private bool hasZombies;
    private NulledArrayList DeferedCharacters;
    public bool bDirty;
    private bool CacheIsFree;
    private bool CachedIsFree;
    public int ID;
    ...
    public bool bLightDirty;
    public static bool UseSlowCollision = false;
    private bool bStairsTN;
    private bool bStairsTW;
    private static bool bDoSlowPathfinding = false;
    private bool SolidFloorCached;
    private bool SolidFloor;
    private static Comparator comp;
    public static bool isOnScreenLast = false;
