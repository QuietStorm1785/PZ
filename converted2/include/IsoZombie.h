...
public:
{
    ...
    public IsoZombie(IsoCell cell, int palette)
    {
    	...
    	/*      */ 
    	/*  348 */     int val = palette;
    	/*  349 */     std::string str = "Zombie_palette";
    	/*      */ 
    	/*  351 */     if (val == 10)
    	/*      */     {
    	/*  353 */       str = str + "10";
        ...
        throws IOException
    {
    	...
    	/*  402 */     std::string str = "Zombie_palette";
    	/*      */ 
    	/*  404 */     if (val == 10)
    	/*      */     {
    	/*  406 */       str = str + "10";
    ...
    public void spotted(IsoMovingObject other)
    {
        ...
        if(other instanceof IsoPlayer)
            n = 0;
        if(getCurrentSquare() == nullptr || other.getCurrentSquare() == nullptr)
            ensureOnTile();
        if(other.getCurrentSquare().getRoom() != getCurrentSquare().getRoom())
        {
            chance = 100;
            if(other.getCurrentSquare().getRoom() != nullptr && getCurrentSquare().getRoom() == nullptr || other.getCurrentSquare().getRoom() == nullptr && getCurrentSquare().getRoom() != nullptr)
            {
                chance = 50;
        ...
        if(spottedLast == TutorialManager.instance.wife && other == IsoPlayer.instance && TimeSinceSeenFlesh < 30)
            return;
        if(spottedLast != other && target != nullptr)
        {
    ...
    {
        int n;
        if((sprite.CurrentAnim.name == "ZombieDeath") || sprite.CurrentAnim.name == "Die")) && (int)def.Frame == sprite.CurrentAnim.Frames.size() - 1)
            n = 0;
        if(IsoCamera.CamCharacter == this)
            n = 0;
        LuaEventManager.TriggerEvent("OnZombieUpdate", this);
        BonusSpotTime--;
        if(BonusSpotTime > 0 && spottedLast != nullptr && !((IsoGameCharacter)spottedLast).isDead())
            spotted(spottedLast);
        if(this == group.Leader || group.Leader == nullptr || group.Leader.isDead())
            group.update();
        ...
        {
            if(walkVariantUse == nullptr || cur != LungeState.instance())
        ...
        shootable = true;
        solid = true;
        if(TutorialManager.instance.wife != nullptr && TutorialManager.instance.wife.Health > 0.0F && TutorialManager.instance.wife.getCurrentSquare() != nullptr && getCurrentSquare() != nullptr && getCurrentSquare().getRoom() != nullptr && getCurrentSquare().getRoom() == TutorialManager.instance.wife.getCurrentSquare().getRoom() && AllowRepathDelay <= 0)
            ...
            for(int ox = -1; ox <= 1; ox++)
            {
                for(int oy = -1; oy <= 1; oy++)
                {
                    for(int oz = -1; oz <= 1; oz++)
                    {
                        ...
                        if(sq != nullptr && !doneGrids.contains(sq))
                        {
                            choiceGrids.add(sq);
                            lcx = ox;
                            lcy = oy;
    ...
    public IsoMovingObject target = nullptr;
    ...
    public bool bRightie = false;
    private int BonusSpotTime = 0;
    public std::string SpriteName = "Zombie";
    ...
    public IsoMovingObject spottedLast = nullptr;
    ...
    public std::string walkVariantUse = nullptr;
    public std::string walkVariant = "ZombieWalk";
    public bool bLunger = false;
}
} // namespace
