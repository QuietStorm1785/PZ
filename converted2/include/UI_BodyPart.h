...
public:
{
    public UI_BodyPart(BodyPartType type, int x, int y, std::string backgroundTex, IsoGameCharacter character, bool RenderFlipped)
    {
        alpha = 1.0F;
        IsFlipped = false;
        ...
        if(NewHealthPanel.instance.MousedOverPart == this)
        {
            ...
            if(UIManager.getDragInventory() != nullptr)
            {
                ...
                if(UIManager.getDragInventory() instanceof Food)
                {
                    ...
                    if(!"TinnedSoup" == UIManager.DragInventory.getType()) && this == NewHealthPanel.instance.Head)
                    {
                ...
                if("RippedSheets" == UIManager.getDragInventory().getType()) && (chr.getBodyDamage().IsScratched(BodyPartType) || chr.getBodyDamage().IsBitten(BodyPartType) || chr.getBodyDamage().IsWounded(BodyPartType)))
                {
                    UIManager.getDragInventory().Use();
    ...
    private std::string getPartStatusString()
    {
