...
class MasterSurvivorBehavior : public Behavior {
public:
{
    ...
    public zombie.behaviors.Behavior.BehaviorResult process(DecisionPath path, IsoGameCharacter character)
    {
        ...
        if(character.getPersonalNeed() != nullptr && character.getPersonalNeed().isCritical())
        {
        ...
        if(orders == toProcess && ((IsoSurvivor)character).getVeryCloseEnemyList().size() > 3)
            timeTillProcessChange = -1;
        if(timeTillProcessChange <= 0 || toProcess == nullptr)
        {
            if(character == IsoCamera.CamCharacter)
                n = 0;
            ...
            if(character.getThreatLevel() > 0 && !character.getLocalRelevantEnemyList().isEmpty() && character.getCurrentSquare().getRoom() != nullptr && priorityToAttack > 0.0F)
        ...
        if(toProcess != nullptr)
        {
            zombie.behaviors.Behavior.BehaviorResult res = toProcess.process(nullptr, character);
            toProcess.last = res;
            return res;
        } else
        {
