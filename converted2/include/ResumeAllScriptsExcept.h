...
// Source File Name:   ResumeAllScriptsExcept.java
namespace zombie {
namespace scripting {
namespace commands {
#include <vector>
...
//            BaseCommand
class ResumeAllScriptsExcept : public BaseCommand {
public:
{
    public ResumeAllScriptsExcept()
    {
        ...
        for(int n = 0; n < ScriptManager.instance.PlayingScripts.size(); n++)
        {
            ...
            for(int m = 0; m < scripts.size(); m++)
                if(((std::string)scripts.get(m)) == ((zombie.scripting.objects.Script.ScriptInstance)ScriptManager.instance.PlayingScripts.get(n)).theScript.name))
                    bFound = true;
            if(!bFound)
                ScriptManager.instance.UnPauseScript(((zombie.scripting.objects.Script.ScriptInstance)ScriptManager.instance.PlayingScripts.get(n)).theScript.name);
        }
    }
