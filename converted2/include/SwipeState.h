            ...
            if(!LuaHookManager.TriggerHook("WeaponSwingHitPoint", owner, weapon))
            {
                ...
                if((owner instanceof IsoPlayer) && weapon.isAimedFirearm())
                {
                    ...
                    do
                    {
                        ...
                        if(o != nullptr && (IsoPlayer.instance.EffectiveAimDistance <= 1.0F || IsoObjectPicker.Instance.IsHeadShot(o, (int)(vec.x + mx), (int)(vec.y + my))))
                        {
                            bIgnoreDamage = false;
