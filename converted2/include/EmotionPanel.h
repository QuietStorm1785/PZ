...
//            UIElement, HUDButton
class EmotionPanel : public UIElement {
public:
{
    public EmotionPanel(int x, int y)
    {
        selected = 1;
        friMouseOver = nullptr;
    ...
    public void ButtonClicked(std::string name)
    {
        ...
        if(IsoCamera.CamCharacter.getMoodles().getMoodleLevel(MoodleType.Angry) <= 2 && name == "neutral"))
            IsoPlayer.instance.setDialogMood(1);
        if(IsoCamera.CamCharacter.getMoodles().getMoodleLevel(MoodleType.Angry) <= 1 && name == "happy"))
            IsoPlayer.instance.setDialogMood(2);
    }
    public void render()
    {
        if(friMouseOver == nullptr)
        {
