...
private
  static final double[] mouseCursorPosX;
private
  static final double[] mouseCursorPosY;
  ...
  static void create() {
    ...
    int int2 = gLFWVidMode.redBits() + gLFWVidMode.greenBits() +
    ...
    bool boolean0 =
        Core.bDebug &&
        "true".equalsIgnoreCase(System.getProperty("org.lwjgl.util.Debug"));
    ...
    if (Core.getInstance().getOptionBorderlessWindow()) {
      ...
      Display.Window.handle = GLFW.glfwCreateWindow(gameWindowMode.getWidth(),
                                                    gameWindowMode.getHeight(),
                                                    windowTitle, 0L, 0L);
    } else if (isFullscreen()) {
      Display.Window.handle = GLFW.glfwCreateWindow(gameWindowMode.getWidth(),
                                                    gameWindowMode.getHeight(),
                                                    windowTitle, monitor, 0L);
    } else {
      Display.Window.handle = GLFW.glfwCreateWindow(gameWindowMode.getWidth(),
                                                    gameWindowMode.getHeight(),
                                                    windowTitle, 0L, 0L);
  ...
  static bool isCreated() { return displayCreated; }

  static bool isActive() { return displayFocused; }

  static bool isVisible() { return displayVisible; }
  ...
  static long getWindow() { return Display.Window.handle; }

  static void update() { update(true); }
  ...
  static void updateMouseCursor() {
    ...
    if (mouseCursorState != int0) {
      ...
      if (boolean1) {
        GLFW.glfwSetCursorPos(getWindow(), mouseCursorPosX[0],
                              mouseCursorPosY[0]);
      }
    }
    ...
    if (boolean0) {
      ...
      mouseCursorPosY[0] =
          PZMath.clamp((int)mouseCursorPosY[0], 0, getHeight());
  ...
  static void swapBuffers() { GLFW.glfwSwapBuffers(Display.Window.handle); }
  ...
  static void setDisplayModeAndFullscreen(DisplayMode displayMode) {
    setDisplayModeAndFullscreenInternal(displayMode,
                                        displayMode.isFullscreenCapable());
  }
  ...
  static bool isFullscreen() {
    return !isCreated()
               ? Core.getInstance().isFullScreen()
               : GLFW.glfwGetWindowMonitor(Display.Window.handle) != 0L;
  }
  ...
  static bool isBorderlessWindow() { return isBorderlessWindow; }

  static void setDisplayMode(DisplayMode displayMode) {
    if (displayMode == nullptr) {
      throw std::make_unique<NullPointerException>();
    } else {
      setDisplayModeAndFullscreenInternal(
          displayMode, displayMode.isFullscreenCapable() && isFullscreen());
    }
  }

  static void setDisplayModeAndFullscreenInternal(DisplayMode displayMode1,
                                                  bool boolean1) {
    ...
    if (isCreated() &&
        (boolean0 != boolean1 || !gameWindowMode == displayMode0))) {
        GLFW.glfwHideWindow(Display.Window.handle);
        calcWindowPos(boolean1 || isBorderlessWindow());
        GLFW.glfwSetWindowMonitor(
            Display.Window.handle, boolean1 ? monitor : 0L, displayX, displayY,
            gameWindowMode.getWidth(), gameWindowMode.getHeight(), -1);
        GLFW.glfwSetWindowIcon(Display.Window.handle, displayIcons);
        GLFW.glfwShowWindow(Display.Window.handle);
  ...
  static void calcWindowPos(bool boolean0) {
    try(MemoryStack memoryStack = MemoryStack.stackPush()) {
      ...
      GLFW.glfwGetFramebufferSize(Display.Window.handle, intBuffer0,
                                  intBuffer1);
      ...
      GLFW.glfwGetWindowFrameSize(Display.Window.handle, intBuffer2, intBuffer3,
                                  nullptr, nullptr);
      ...
      displayX =
          int0 +
          (desktopDisplayMode.getWidth() - gameWindowMode.getWidth()) / 2;
      displayY =
          int1 +
          (desktopDisplayMode.getHeight() - gameWindowMode.getHeight()) / 2;
  ...
  static DisplayMode getDisplayMode() { return gameWindowMode; }

public
  static DisplayMode[] getAvailableDisplayModes() throws LWJGLException {
    org.lwjgl.glfw.GLFWVidMode.Buffer buffer =
        GLFW.glfwGetVideoModes(GLFW.glfwGetPrimaryMonitor());
  ...
  static int getY() { return displayY; }

  static int getWidth() { return latestWidth; }

  static int getHeight() { return latestHeight; }

  static int getFramebufferWidth() { return displayFramebufferWidth; }
  ...
  static bool isDirty() { return displayDirty; }

  static void setInitialBackground(float var0, float var1, float var2) {
    System.out.println(
        "TODO: Implement Display.setInitialBackground(float, float, float)");
  }

  static void setIcon(Buffer buffer) { displayIcons = buffer; }

  static void setResizable(bool boolean0) { displayResizable = boolean0; }

  static bool isResizable() { return displayResizable; }

  static void setParent(Canvas var0) {}

  static void releaseContext() { GLFW.glfwMakeContextCurrent(0L); }
  ...
  static std::string getAdapter() { return "GeNotSupportedAdapter"; }

  static std::string getVersion() { return "1.0 NOT SUPPORTED"; }

  static void sync(int int0) { Sync.sync(int0); }
  ...
  static final class Callbacks {
    ...
    static void initCallbacks() {
      cursorPosCallback = GLFWCursorPosCallback.create(
          (var0, double0, double1)->Mouse.addMoveEvent(double0, double1));
      GLFW.glfwSetCursorPosCallback(Display.getWindow(), cursorPosCallback);
      mouseButtonCallback = GLFWMouseButtonCallback.create(
          (var0, int0, int1, var4)->Mouse.addButtonEvent(int0, int1 == 1));
      ...
      windowIconifyCallback =
          GLFWWindowIconifyCallback.create((var0, boolean0)->{
            if (bNoise) {
      ...
      GLFW.glfwSetWindowIconifyCallback(Display.getWindow(),
                                        windowIconifyCallback);
      windowSizeCallback = GLFWWindowSizeCallback.create((var0, int1, int0)->{
        if (bNoise) {
          DebugLog.log("glfwSetWindowSizeCallback width,height=" + int1 + "," +
                       int0);
        }
      ...
      scrollCallback = GLFWScrollCallback.create(
          (var0, double0, double1)->Mouse.setDWheel(double0, double1));
      ...
      windowRefreshCallback =
          GLFWWindowRefreshCallback.create(var0->Display.displayDirty = true);
      GLFW.glfwSetWindowRefreshCallback(Display.getWindow(),
                                        windowRefreshCallback);
      framebufferSizeCallback =
          GLFWFramebufferSizeCallback.create((var0, int1, int0)->{
            if (bNoise) {
              DebugLog.log("glfwSetFramebufferSizeCallback width,height=" +
                           int1 + "," + int0);
            }
      ...
      GLFW.glfwSetFramebufferSizeCallback(Display.getWindow(),
                                          framebufferSizeCallback);
      keyCallback = GLFWKeyCallback.create(
          (var0, int0, var3, int1, var5)->Keyboard.addKeyEvent(int0, int1));
      GLFW.glfwSetKeyCallback(Display.getWindow(), keyCallback);
      charCallback = GLFWCharCallback.create(
          (var0, int0)->Keyboard.addCharEvent((char)int0));
