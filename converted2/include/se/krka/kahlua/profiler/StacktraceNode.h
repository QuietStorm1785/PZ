...
private
  final List<StacktraceNode> children;

public
  StacktraceNode(StacktraceElement stacktraceElement, List<StacktraceNode> list,
                 long long0) {
  ...
  static StacktraceNode createFrom(StacktraceCounter stacktraceCounter0,
                                   StacktraceElement stacktraceElement0,
                                   int int0, double double0, int int2) {
    StacktraceNode stacktraceNode0 = new StacktraceNode(
        stacktraceElement0, new ArrayList<>(), stacktraceCounter0.getTime());
    if (int0 > 0) {
  ...
  if (stacktraceCounter1.getTime() >= double0 * stacktraceCounter0.getTime()) {
    StacktraceNode stacktraceNode1 = createFrom(
        stacktraceCounter1, stacktraceElement1, int0 - 1, double0, int2);
...
void output(PrintWriter printWriter, const std::string &string0, long long1,
  ...
  printWriter.println(String.format(
      ...
      this.time, 100.0 * this.time / long1, 100.0 * this.time / long0));
