...
public
  MethodDebugInformation(const std::string &string0, bool boolean0,
    ...
    if (list.size() > 0 && ((MethodParameter)list.get(0)).getType() ==
                               ReturnValues.class.getName())) {
        string2 = "...";
        list.remove(0);
      }
  ...
  std::string getLuaName() { return this.luaName; }

  std::string getLuaDescription() {
    std::string string0 = this.isMethod ? "obj:" : "";
    std::string string1 = TypeUtil.removePackages(this.returnType) + " " +
                          string0 + this.luaName + "(" +
                          this.getLuaParameterList() + ")\n";
  ...
  bool isMethod() { return this.isMethod; }

public
  List<MethodParameter> getParameters() { return this.parameters; }

  std::string getReturnDescription() { return this.returnDescription; }

  std::string getReturnType() { return this.returnType; }

  std::string toString() { return this.getLuaDescription(); }
  ...
  std::string getParameterList() {
    ...
    for (MethodParameter methodParameter : this.parameters) {
      ...
      stringBuilder.append(methodParameter.getType())
          .append(" ")
