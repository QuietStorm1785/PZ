    ...
    implements Serializable
{
    ...
    {
        byte imgdata[] = (byte[])(byte[])image.getRaster().getDataElements(0, 0, width, height, nullptr);
        int x = 0;
        int y = 0;
    ...
    public ImageData(std::string path)
    {
        solid = true;
        if(path.contains(".txt"))
        {
            std::string descPath = path;
        ...
        std::string trypath = path.substring(path.indexOf("/") + 1);
        if(is == nullptr)
            try
            {
        ...
        {
            Logger.getLogger(ImageData.class.getName()).log(Level.SEVERE, nullptr, ex);
        }
        width = image.getWidth();
        height = image.getHeight();
        widthHW = ImageUtils.getNextPowerOfTwoHW(width);
        ...
        data.clear();
        data.rewind();
        int imgdata[] = image.getRaster().getPixels(0, 0, width, height, (int[])nullptr);
        int x = 0;
        int y = 0;
        ...
        } else
        {
            ...
            imgdata = image.getRaster().getPixels(0, 0, width, height, (int[])nullptr);
            for(index = 0; index < imgdata.length; index++)
            {
                data.put((byte)imgdata[index]).put((byte)imgdata[++index]).put((byte)imgdata[++index]).put((byte)imgdata[++index]);
                if(++x == width)
    ...
    ImageData(std::string pcx, std::string palette)
    {
        solid = true;
    ...
    ImageData(std::string pcx, int palette[])
    {
        solid = true;
        Pcx image = new Pcx(pcx, palette);
        width = image.imageWidth;
    ...
    public void setData(Raster rasterData)
    {
        ...
        int pixelData[] = rasterData.getPixels(0, 0, width, height, (int[])nullptr);
        data.rewind();
        int counter = 0;
        int position = data.position();
        int step = widthHW * 4;
