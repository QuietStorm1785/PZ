...
private
  ArrayList<ISearchNode> successorNodes = std::make_unique<ArrayList<>>();
  const IOpenSet openSet = new OpenSet(new AStar.SearchNodeComparator());
  const IClosedSet closedSetHash =
      new ClosedSetHash(new AStar.SearchNodeComparator());
  const IClosedSet closedSetNoHash =
      new ClosedSet(new AStar.SearchNodeComparator());

public
  ArrayList<ISearchNode> shortestPath(ISearchNode iSearchNode1,
                                      IGoalNode iGoalNode) {
  ...
  ISearchNode search(ISearchNode iSearchNode0, IGoalNode iGoalNode) {
    bool boolean0 = iSearchNode0.keyCode() != null;
    iSearchNode0.setDepth(0);
    this.openSet.clear();
    this.openSet.add(iSearchNode0);
    IClosedSet iClosedSet =
        boolean0 ? this.closedSetHash : this.closedSetNoHash;
    iClosedSet.clear();

    for (this.numSearchSteps = 0;
         this.openSet.size() > 0 &&
         (this.maxSteps < 0 || this.numSearchSteps < this.maxSteps);
         this.numSearchSteps++) {
...
public
  static ArrayList<ISearchNode> path(ISearchNode iSearchNode0) {
  ...
  int numSearchSteps() { return this.numSearchSteps; }

  ISearchNode bestNodeAfterSearch() { return this.bestNodeAfterSearch; }

  void setMaxSteps(int int0) { this.maxSteps = int0; }
