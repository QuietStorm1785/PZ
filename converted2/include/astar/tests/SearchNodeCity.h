...
public:
  ...
  SearchNodeCity goal;
private
  HashMap<String, HashMap<String, Integer>> adjacencyMatrix =
...
public
  ...
  double c(ISearchNode iSearchNode) {
    return this.adjacencyMatrix.get(this.name)
        .get(this.castToSearchNodeCity(iSearchNode).getName())
        .intValue();
  }
...
public
  ...
  SearchNodeCity getParent() { return this.parent; }

  void setParent(ISearchNode iSearchNode) {
    this.parent = this.castToSearchNodeCity(iSearchNode);
  }

  int keyCode() { return this.name.hashCode(); }

  bool equals(void *object) {
    return object instanceof
        SearchNodeCity searchNodeCity0 ? this.name == searchNodeCity0.getName()
                                       : false;
  }
  ...
  std::string toString() { return this.name + ",f:" + this.f(); }
}
} // namespace tests
} // namespace astar
