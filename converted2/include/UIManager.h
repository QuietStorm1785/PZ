...
public:
{
    ...
    public static void AddTutorial(float x, float y, std::string title, std::string string, bool bAutoExpand)
    {
    ...
    public static void DoModal(std::string name, std::string help, bool bYesNo)
    {
        if(getModal() != nullptr)
    ...
    public static void DoModal(std::string name, std::string help, bool bYesNo, UIEventHandler handler)
    {
        if(getModal() != nullptr)
            getUI().remove(getModal());
        setModal(new ModalDialog(name, help, bYesNo));
    ...
    public static void AddTutorial(UIElement parent, int x, int y, std::string title, std::string string, bool bAutoExpand)
    {
        if(!HelpIcon.doOthers)
            return;
        if(getDoneTutorials().contains(title))
    ...
    public static void AddTutorial(float x, float y, float z, std::string title, std::string string, bool bAutoExpand, float yoff)
    {
        if(!HelpIcon.doOthers)
            return;
        if(getDoneTutorials().contains(title))
    ...
    public static void CloseContainers()
    {
        for(int i = getUI().size() - 1; i >= 0; i--)
        {
            ...
            if(!(ui instanceof NewContainerPanel))
                continue;
            if(getDragInventory() != nullptr && ((NewContainerPanel)ui).Flow.Container == getDragInventory().getContainer())
            {
    ...
    public static void init()
    {
        ...
        {
            Logger.getLogger(UIManager.class.getName()).log(Level.SEVERE, nullptr, ex);
        }
    ...
    public static void render()
    {
        ...
        if(IsoPlayer.instance != nullptr && IsoPlayer.instance.IsAiming() && IsoPlayer.instance.getPrimaryHandItem() != nullptr && (IsoPlayer.instance.getPrimaryHandItem() instanceof HandWeapon) && (IsoPlayer.instance.isCharging || ((HandWeapon)IsoPlayer.instance.getPrimaryHandItem()).isAimedFirearm()) && IsoPlayer.instance.IsUsingAimWeapon())
            if(Core.bDoubleSize)
        ...
        if(getLastAlpha() != 1.0F && getFadeAlpha() == 1.0F && IsoPlayer.getInstance() != nullptr && IsoPlayer.getInstance().isAsleep() && !IsoPlayer.instance.getCanUpgradePerk().isEmpty() && IsoPlayer.getInstance().getNumberOfPerksToPick() > 0)
        {
        ...
        if(getSpeedControls() != nullptr && getSpeedControls().getCurrentGameSpeed() == 0 && !Levelup.isVisible() && (getModal() == nullptr || !Modal.isVisible()))
        ...
        if(IsoPlayer.getInstance() != nullptr && IsoPlayer.getInstance().isAsleep() && getLevelup().isVisible())
        {
            getLevelup().render();
            if(getModal() != nullptr && getModal().isVisible())
                getModal().render();
        }
    ...
    public static void update()
    {
        ...
        if(IsoPlayer.DemoMode || IngameState.DebugPathfinding || IngameState.AlwaysDebugPathfinding)
            ...
            if(getHealthPanel() != nullptr)
                getHealthPanel().SetCharacter(IsoCamera.CamCharacter);
            Stack tutorialStack = new Stack();
            if(getDragInventory() != nullptr && !IsoPlayer.instance.getInventory().contains(DragInventory.getType()) && getDragInventory().getContainer() == IsoPlayer.getInstance().getInventory())
                setDragInventory(nullptr);
            bool consumedClick = false;
            bool consumedRClick = false;
            bool consumedMove = false;
            int mx = zombie.input.Mouse.getXA();
            int my = zombie.input.Mouse.getYA();
            int mxw = zombie.input.Mouse.getX();
            int myw = zombie.input.Mouse.getY();
            if(getBlendTest() != nullptr)
            ...
            if(zombie.input.Mouse.isLeftPressed())
            {
                ...
                do
                {
                    if(i < 0)
                        break;
                    UIElement ui = (UIElement)getUI().get(i);
                    if((getModal() == nullptr || getModal() == ui || !getModal().isVisible()) && (ui.isIgnoreLossControl() || !TutorialManager.instance.StealControl) && ui.isVisible() && ((float)mx > ui.getX() && (float)my > ui.getY() && (float)mx < ui.getX() + (float)ui.getWidth() && (float)my < ui.getY() + (float)ui.getHeight() || ui.isCapture()) && ui.onMouseDown(mx - (int)ui.getX(), my - (int)ui.getY()))
                    {
                        consumedClick = true;
                        break;
                    }
                ...
                if(getSidebar() != nullptr)
                    sidebar.setClickedValue(nullptr);
                if(!consumedClick && !Keyboard.isKeyDown(57))
                {
                    if(getBlendTest() != nullptr)
                        getBlendTest().MoveTo(-53F, 0.0F, 0.4F);
                    if(NewCraftingPanel.instance != nullptr)
                        NewCraftingPanel.instance.setVisible(false);
                }
                if(!consumedClick)
                {
                    if(IsoPlayer.getInstance() != nullptr && IsoPlayer.getInstance().getGuardModeUI() > 0 && getPicked().square.isFree(false))
                    ...
                    CloseContainers();
                    if(IsoWorld.instance.CurrentCell != nullptr && !IsoWorld.instance.CurrentCell.DoBuilding(false) && getPicked() != nullptr && IsoPlayer.getInstance() != nullptr && !IsoPlayer.isIsAiming() && !IsoPlayer.instance.isAsleep())
                    {
            ...
            if(zombie.input.Mouse.isLeftReleased())
            {
                ...
                do
                {
                    if(i < 0)
                        break;
                    UIElement ui = (UIElement)getUI().get(i);
                    if((ui.isIgnoreLossControl() || !TutorialManager.instance.StealControl) && ui.isVisible() && (getModal() == nullptr || getModal() == ui || !getModal().isVisible()) && ((float)mx > ui.getX() && (float)my > ui.getY() && (float)mx < ui.getX() + (float)ui.getWidth() && (float)my < ui.getY() + (float)ui.getHeight() || ui.isCapture()) && ui.onMouseUp(mx - (int)ui.getX(), my - (int)ui.getY()))
                    {
                        if(ui != getRadial())
                            getRadial().close();
                        break;
            ...
            if(zombie.input.Mouse.isRightPressed())
            {
                ...
                do
                {
                    if(i < 0)
                        break;
                    UIElement ui = (UIElement)getUI().get(i);
                    if(ui.isVisible() && (getModal() == nullptr || getModal() == ui || !getModal().isVisible()) && ((float)mx > ui.getX() && (float)my > ui.getY() && (float)mx < ui.getX() + (float)ui.getWidth() && (float)my < ui.getY() + (float)ui.getHeight() || ui.isCapture()) && ui.onRightMouseDown(mx - (int)ui.getX(), my - (int)ui.getY()))
                    {
                        consumedRClick = true;
                        break;
                    }
                ...
                if(IsoWorld.instance.CurrentCell != nullptr && getPicked() != nullptr && !IsoPlayer.isIsAiming() && !IsoPlayer.instance.isAsleep())
                {
            ...
            {
                int i = 0;
                if(IsoPlayer.getInstance() != nullptr && IsoPlayer.getInstance().getGuardModeUI() > 0)
                    IsoPlayer.instance.setGuardModeUI(0);
                i = getUI().size() - 1;
                do
                {
                    if(i < 0)
                        break;
                    UIElement ui = (UIElement)getUI().get(i);
                    if((ui.isIgnoreLossControl() || !TutorialManager.instance.StealControl) && ui.isVisible() && (getModal() == nullptr || getModal() == ui || !getModal().isVisible()) && ((float)mx > ui.getX() && (float)my > ui.getY() && (float)mx < ui.getX() + (float)ui.getWidth() && (float)my < ui.getY() + (float)ui.getHeight() || ui.isCapture()) && ui.onRightMouseUp(mx - (int)ui.getX(), my - (int)ui.getY()))
                        break;
                    i--;
                } while(true);
                if(i == -1)
                {
                    setDragInventory(nullptr);
                    if(getPicked() == nullptr);
                }
                if(IsoPlayer.getInstance() != nullptr && IsoPlayer.getInstance() != nullptr)
                    IsoPlayer.instance.setDragObject(nullptr);
                if(IsoWorld.instance.CurrentCell != nullptr && getRightDownObject() != nullptr && !IsoPlayer.isIsAiming() && !IsoPlayer.instance.isAsleep())
                {
            ...
            if(!consumedMove && getPicked() != nullptr)
            {
    ...
    public static bool isbFadeBeforeUI()
    {
        return bFadeBeforeUI;
    }
    public static void setbFadeBeforeUI(bool abFadeBeforeUI)
    {
    ...
    public static void setFadingOut(bool aFadingOut)
    {
    ...
    public static bool isLastDoubleSize()
    {
        return lastDoubleSize;
    }
    public static void setLastDoubleSize(bool aLastDoubleSize)
    {
        lastDoubleSize = aLastDoubleSize;
    }
    public static bool isDoMouseControls()
    {
        return DoMouseControls;
    }
    public static void setDoMouseControls(bool aDoMouseControls)
    {
    ...
    public static zombie.iso.IsoObjectPicker.ClickObject Picked = nullptr;
    public static Clock clock;
    public static EnduranceWidget Endurance;
    public static Stack UI = new Stack();
    public static ObjectTooltip toolTip = nullptr;
    ...
    public static bool bFadeBeforeUI = false;
    public static ActionProgressBar ProgressBar;
    public static BodyDamage BD_Test = nullptr;
    public static LevelUpScreen Levelup = nullptr;
    ...
    public static IsoObject LastPicked = nullptr;
    public static Stack DoneTutorials = new Stack();
    public static float lastOffX = 0.0F;
    public static float lastOffY = 0.0F;
    public static bool lastDoubleSize = false;
    public static bool DoMouseControls = true;
    public static ModalDialog Modal = nullptr;
    static ArrayList toRemove = new ArrayList();
    static ArrayList toAdd = new ArrayList();
    public static Cursor nativeCursor = nullptr;
    public static Texture black = nullptr;
    ...
    public static IsoObject RightDownObject = nullptr;
    static Texture mouse = nullptr;
