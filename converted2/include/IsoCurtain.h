...
public:
{
    public IsoCurtain(IsoCell cell, IsoGridSquare gridSquare, IsoSprite gid, bool north)
    {
        Barricaded = false;
    ...
    public IsoCurtain(IsoCell cell, IsoGridSquare gridSquare, std::string gid, bool north)
    {
        Barricaded = false;
        BarricideMaxStrength = Integer.valueOf(0);
        BarricideStrength = Integer.valueOf(0);
    ...
    public void ToggleDoor(IsoGameCharacter chr)
    {
        if(Barricaded)
            return;
        DirtySlice();
        if(Locked && chr != nullptr && chr.getCurrentSquare().getRoom() == nullptr && !open)
            return;
        for(int x = 0; x < 200; x++)
        {
            for(int y = 0; y < 200; y++)
