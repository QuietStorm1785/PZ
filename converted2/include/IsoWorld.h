...
public:
{
    ...
    public void CreateSurvivorGroup(IsoGridSquare sq, IsoPlayer player)
    {
        ...
        for(int n = 0; n < c; n++)
        {
            ...
            } while(sq == nullptr || !sq.isFree(true));
    ...
    public void LoadTileDefinitions(IsoSpriteManager sprMan, std::string filename)
    {
        try
        {
            ...
            for(int n = 0; n < numTilesheets; n++)
            {
                ...
                for(int m = 0; m < nTiles; m++)
                {
                    ...
                    int fdfd;
                    if(spr.name == "TileFrames_13"))
                        fdfd = 0;
                    int nProps = readInt(in);
                    for(int l = 0; l < nProps; l++)
                    {
                        str = readString(in);
                        std::string prop = str.trim();
                        str = readString(in);
                        std::string val = str.trim();
    ...
    public void init()
    {
        ...
        try
        {
            if(LoadGamePopupState.bLoadCharacter)
                LoadPlayerCell();
            std::string oplayerCell = playerCell;
        ...
        if(!bLoaded && !FrameLoader.bServer && !FrameLoader.bClient)
        {
            ...
            if(splayer == nullptr)
            {
                ...
                if(!FrameLoader.bClient && IsoPlayer.instance != nullptr)
    ...
    public void moveArea(std::string map, float x, float y, float z)
    {
        playerCell = map;
        CurrentCell.Dispose();
        System.gc();
        ...
        try
        {
            ...
            if(file.exists())
                ...
                CurrentCell = CellLoader.LoadCell((std::string)ScriptManager.instance.MapMap.get(playerCell), mapUseJar);
        }
        catch(ParserConfigurationException ex)
        {
            Logger.getLogger(IsoWorld.class.getName()).log(Level.SEVERE, nullptr, ex);
    ...
    public void KillCell()
    {
        ...
        for(int n = 0; n < WorldSoundManager.instance.Cache.length; n++)
            WorldSoundManager.instance.Cache[n] = nullptr;
        CurrentCell.Dispose();
        CurrentCell = nullptr;
        CellLoader.wanderRoom = nullptr;
    ...
    private void AddZombieGroup(bool flag, int i)
    {
    }
    ...
    public void update()
    {
        ...
        for(int n = 0; n < Groups.size(); n++)
        {
            ...
            for(int m = 0; m < g.Members.size(); m++)
                if(((SurvivorDesc)g.Members.get(m)).getInstance() != nullptr && !((SurvivorDesc)g.Members.get(m)).getInstance().isDead())
                    bRemove = false;
            if(bRemove)
            {
                Groups.remove(g);
    ...
    public static std::string mapPath = "media/";
    public static bool mapUseJar = true;
    bool bLoaded;
    public IsoSpriteManager spriteManager;
    public std::string cellMap[][];
    static OnceEvery e = new OnceEvery(0.4F, false);
    int worldX;
    int worldY;
    static SurvivorGroup TestGroup = nullptr;
}
} // namespace
