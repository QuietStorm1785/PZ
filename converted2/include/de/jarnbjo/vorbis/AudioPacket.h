...
private
  boolean[] noResidues;
private
  static final float[][] windows = new float[8][];

protected
  AudioPacket(VorbisStream vorbisStream, BitInputStream bitInputStream) {
    SetupHeader setupHeader = vorbisStream.getSetupHeader();
    IdentificationHeader identificationHeader =
        vorbisStream.getIdentificationHeader();
    ...
    if (bitInputStream.getInt(1) != 0) {
      ...
      for (int int3 = 0; int3 < int0; int3++) {
        ...
        Floor floor = setupHeader.getFloors()[int5].decodeFloor(vorbisStream,
                                                                bitInputStream);
      ...
      if (!boolean0) {
        ...
        for (int int7 = 0; int7 < this.mapping.getSubmaps(); int7++) {
          ...
          residue.decodeResidue(vorbisStream, bitInputStream, this.mode, int8,
                                booleans, this.pcm);
        }

        for (int int11 = this.mapping.getCouplingSteps() - 1; int11 >= 0;
             int11--) {
          double double0 = 0.0;
          double double1 = 0.0;
          ...
          for (int int12 = 0; int12 < floats0.length; int12++) {
            ...
            if (float0 > 0.0F) {
              floats1[int12] =
                  float1 > 0.0F ? float1 - float0 : float1 + float0;
            } else {
              floats0[int12] =
                  float1 > 0.0F ? float1 + float0 : float1 - float0;
        ...
        for (int int13 = 0; int13 < int0; int13++) {
          if (this.channelFloors[int13] != nullptr) {
            this.channelFloors[int13].computeFloor(this.pcm[int13]);
          }
        }

        for (int int14 = 0; int14 < int0; int14++) {
          MdctFloat mdctFloat = this.blockFlag
                                    ? identificationHeader.getMdct1()
                                    : identificationHeader.getMdct0();
...
private
  float[] getComputedWindow() {
    int int0 = (this.blockFlag ? 4 : 0) + (this.previousWindowFlag ? 2 : 0) +
               (this.nextWindowFlag ? 1 : 0);
    ...
    if (floats == nullptr) {
      ...
      for (int int3 = 0; int3 < this.rightN; int3++) {
        float float1 =
            (float)((this.rightN - int3 - 0.5) / this.rightN * Math.PI / 2.0);
  ...
  int getPcm(AudioPacket audioPacket1, int[][] ints3) {
    ...
    if (this.leftWindowEnd + 1 < this.rightWindowStart) {
      for (int int6 = 0; int6 < int0; int6++) {
        System.arraycopy(this.pcmInt[int6], this.leftWindowEnd, ints3[int6],
                         this.leftWindowEnd - this.leftWindowStart,
                         this.rightWindowStart - this.leftWindowEnd);
      }
    }
  ...
  void getPcm(AudioPacket audioPacket1, byte[] bytes) {
    int int0 = this.pcm.length;
    ...
    for (int int1 = 0; int1 < int0; int1++) {
      ...
      for (int int6 = this.leftWindowEnd; int6 < this.rightWindowStart;
           int6++) {
        int int7 = ints1[int6];
        if (int7 > 32767) {
          int7 = 32767;
...
protected
  float[] getWindow() { return this.window; }

  int getLeftWindowStart() { return this.leftWindowStart; }

  int getLeftWindowEnd() { return this.leftWindowEnd; }

  int getRightWindowStart() { return this.rightWindowStart; }

  int getRightWindowEnd() { return this.rightWindowEnd; }

public
  int[][] getPcm() { return this.pcmInt; }

public
  float[][] getFreqencyDomain() { return this.pcm; }
}
} // namespace vorbis
} // namespace jarnbjo
} // namespace de
