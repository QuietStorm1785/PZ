...
class StaggerBackDieState : public State {
public:
{
    ...
    public void execute(IsoGameCharacter owner)
    {
        ...
        if(!owner.isIgnoreStaggerBack() && owner.getReanimPhase() == 0)
        {
            ...
            if(owner.getHitBy() != nullptr)
                owner.getHitBy().getStats().stress -= 0.0016F;
        }
        if(owner.isIgnoreStaggerBack() || owner.getStateEventDelayTimer() <= 30F * owner.getHitForce() * owner.getStaggerTimeMod() && !owner.sprite.CurrentAnim.name == "ZombieDeath") && !owner.sprite.CurrentAnim.name == "ZombieGetUp") && !owner.sprite.CurrentAnim.name == "Die"))
        {
            owner.PlayAnimUnlooped("ZombieDeath");
            owner.setVisibleToNPCs(false);
            owner.setShootable(false);
        }
        if((owner.sprite.CurrentAnim.name == "ZombieGetUp") || owner.sprite.CurrentAnim.name == "ZombieDeath") || owner.sprite.CurrentAnim.name == "Die")) && (int)owner.def.Frame == owner.sprite.CurrentAnim.Frames.size() - 1)
        {
            ...
            } else
            {
                ...
                if(owner.getAttackedBy() != nullptr)
