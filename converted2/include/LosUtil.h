...
public:
{
    ...
    public static int DistToWindow(IsoCell cell, int x1, int y1, int z1, int x0, int y0, int z0)
    {
        ...
        if(Math.abs(dx) > Math.abs(dy) && Math.abs(dx) > Math.abs(dz))
        {
            ...
            while(x0 != x1) 
            {
                ...
                if(a != nullptr && b != nullptr && a.testVisionAdjacent(b.getX() - a.getX(), b.getY() - a.getY(), b.getZ() - a.getZ(), true, true) == TestResults.ClearThroughWindow)
                    return dist;
                b = a;
                lx = x0;
                ly = (int)t;
    ...
    public static TestResults lineClear(IsoCell cell, int x1, int y1, int z1, int x0, int y0, int z0, bool bIgnoreDoors)
    {
    ...
    public static int lineClearCollideCount(IsoGameCharacter chr, IsoCell cell, int x1, int y1, int z1, int x0, int y0, int z0)
    {
        ...
        if(Math.abs(dx) > Math.abs(dy) && Math.abs(dx) > Math.abs(dz))
        {
            ...
            while(x0 != x1) 
            {
                ...
                if(a != nullptr && b != nullptr)
                {
                    bool bTest = a.testCollideAdjacent(chr, b.getX() - a.getX(), b.getY() - a.getY(), b.getZ() - a.getZ());
                    if(bTest)
                        return l;
                }
                l++;
    ...
    public static bool lineClearCollide(IsoMovingObject chr, IsoCell cell, int x1, int y1, int z1, int x0, int y0, int z0)
    {
        int dy = y1 - y0;
        int dx = x1 - x0;
        int dz = z1 - z0;
    ...
    public static TestResults lineClearCached(IsoCell cell, int x1, int y1, int z1, int x0, int y0, int z0, bool bIgnoreDoors)
    {
        int sx = x0;
        int sy = y0;
        int sz = z0;
    ...
    public static bool cachecleared = true;
}
} // namespace
