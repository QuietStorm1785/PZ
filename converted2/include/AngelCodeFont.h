...
public:
{
    ...
    public AngelCodeFont(std::string fntFile, Texture image)
    {
    	/* 114 */     this.fontImage = image;
    	/*     */ 
    	/* 116 */     std::string path = fntFile;
    ...
    public AngelCodeFont(std::string fntFile, std::string imgFile)        throws FileNotFoundException
    {
    	this.fontImage = Texture.getSharedTexture(imgFile);
    	/*     */ 
    	/* 173 */     std::string path = fntFile;
    ...
    public void drawString(float x, float y, std::string text)
    {
        drawString(x, y, text, Color.white);
    }
    public void drawString(float x, float y, std::string text, Color col)
    {
        drawString(x, y, text, col, 0, text.length() - 1);
    }
    public void drawString(float x, float y, std::string text, Color col, int startIndex, int endIndex)
    {
        xoff = (int)x;
        yoff = (int)y;
        curCol = col;
    ...
    public int getHeight(std::string text)
    {
    ...
    public int getWidth(std::string text)
    {
        DisplayList displayList = nullptr;
        if(displayListCaching)
        {
    ...
    public int getYOffset(std::string text)
    {
        DisplayList displayList = nullptr;
        if(displayListCaching)
        {
    ...
    private CharDef parseChar(std::string line)
    {
    ...
    private void parseFnt(InputStream fntFile)
    {
        ...
        try
        {
            ...
            std::string info = in.readLine();
            std::string common = in.readLine();
            std::string page = in.readLine();
    ...
    private void render(std::string text, int start, int end)
    {
        int x = 0;
        int y = 0;
        CharDef lastCharDef = nullptr;
    ...
    private bool displayListCaching;
    ...
    public int xoff;
    public int yoff;
    public static Color curCol = nullptr;
}
} // namespace
