...
public:
{
    ...
    {
        if(UIManager.getProgressBar() != nullptr)
        {
        ...
        try
        {
            ...
            if(IsoPlayer.getInstance() != nullptr)
            {
                ...
                if(IsoPlayer.getInstance().getCurrentSquare().getRoom() == nullptr);
            ...
            for(int i$ = 0; i$ < len$; i$++)
            {
                ...
                std::string str = (std::string)TextureID.TextureIDMap.get(obj);
                y += 10;
            }
        }
        catch(Exception ex)
    ...
    public void StartMusic(std::string s)
    {
    }
    public GameStateMachine.StateAction update()
    {
        if(finder == nullptr && AlwaysDebugPathfinding)
            finder = new AStarPathFinder(nullptr, IsoWorld.instance.CurrentCell.getPathMap(), 800, true, new ManhattanHeuristic(1));
        if(AlwaysDebugPathfinding)
        {
            finder.maxSearchDistance = 1000;
            Path path = finder.findPath(0, nullptr, 170, 110, 0, 85, 110, 0);
        ...
        if(IsoPlayer.getInstance() != nullptr && IsoPlayer.getInstance().getStats().Sanity < 0.5F)
        ...
        if(!PZConsole.instance.isVisible() && Keyboard.isKeyDown(50) && !MDebounce)
        {
            ...
            if(!SoundManager.instance.AllowMusic)
            {
                ...
                instance.musicTrack = nullptr;
            }
        } else
        if(!FrameLoader.bServer && !Keyboard.isKeyDown(50))
            MDebounce = false;
        if(IsoPlayer.getInstance() != nullptr && IsoPlayer.getInstance().getHealth() > 0.0F && IsoPlayer.getInstance().getBodyDamage().getHealth() > 0.0F)
            TimeSinceDeath = 0;
        else
            TimeSinceDeath++;
        if(Mouse.isLeftDown() && TimeSinceDeath > 100)
        {
            ...
            {
                Logger.getLogger(IngameState.class.getName()).log(Level.SEVERE, nullptr, ex);
            }
        ...
        try
        {
            ...
            if(IsoPlayer.getInstance() != nullptr && (IsoPlayer.getInstance() != nullptr && IsoPlayer.getInstance().isWaiting() || UIManager.getSpeedControls().getCurrentGameSpeed() > 1))
            {
                for(int n = 0; n < WaitMul; n++)
                    if(!Paused)
                        ...
                        {
                            Iterator it;
                            for(it = IsoWorld.instance.CurrentCell.getSurvivorList().iterator(); it != nullptr && it.hasNext();)
                            {
    ...
    public static bool AlwaysDebugPathfinding = false;
