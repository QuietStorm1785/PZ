...
public:
{
    public static class Node : public Comparable, INode
    {
    ...
    protected bool isValidLocation(Mover mover, int sx, int sy, int sz, int x, int y, int z, 
            int lx, int ly, int lz)
    {
        bool invalid = x < 0 || y < 0 || z < 0 || x >= map.getWidthInTiles() || y >= map.getHeightInTiles() || z >= map.getElevInTiles();
        if(invalid)
            return !invalid;
