...
public:
{
    private static class RingBuffer
    {
        private class StateRun
        {
            void render()
            {
                ...
                if(texture0 != lastRenderedTexture0)
                {
                    if(texture0 != nullptr)
                    {
                        if(lastRenderedTexture0 == nullptr)
                            GL11.glEnable(3553);
                        texture0.bind();
                    } else
                    if(lastRenderedTexture0 != nullptr)
        ...
        void add(TextureDraw s, Style newStyle, float engineAlpha)
        {
            ...
            if(currentRun == nullptr || newStyle != currentStyle && (currentStyle == nullptr || newStyle.getStyleID() != currentStyle.getStyleID()) || newTexture0 != currentTexture0)
            {
        ...
        void add(Style s)
        {
            ...
            if(data == nullptr)
            {
                ...
                currentRun.length = -1;
                currentRun.style = s;
                currentRun.texture0 = nullptr;
                currentRun.startIndex = 0;
                numRuns++;
                if(numRuns == stateRun.length)
                    growStateRuns();
                currentStyle = nullptr;
                currentTexture0 = nullptr;
                return;
            }
        ...
        private void next()
        {
            ...
            if(vertices[sequence] == nullptr || verticesBytes[sequence] != buf)
            {
            ...
            if(indices[sequence] == nullptr || indicesBytes[sequence] != ibuf)
            {
        ...
        void render()
        {
            vbo[sequence].unmap();
            ibo[sequence].unmap();
            lastRenderedStyle = nullptr;
            lastRenderedTexture0 = nullptr;
            restoreVBOs = true;
            ...
            float b, float a)
    {
        ...
        } else
        {
            ...
            addSprite(TransparentStyle.instance, nullptr);
            return;
        }
    }
    public void render(Texture tex, int x, int y, int width, int height, float r, float g, 
