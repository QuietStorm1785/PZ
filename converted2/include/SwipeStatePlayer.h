...
public:
{
    ...
    public void enter(IsoGameCharacter owner)
    {
        ...
        do
        {
            ...
            if(lowestDist < weapon.getMinRange())
            {
                ...
                if(((IsoLivingCharacter)owner).bareHands.getSwingAnim() != nullptr)
                {
    ...
    public void execute(IsoGameCharacter owner)
    {
        ...
        RemoveList.clear();
        if(weapon != nullptr && owner.getAttackDelay() == owner.getAttackDelayUse())
        {
            ...
            if((owner instanceof IsoPlayer) && weapon.isAimedFirearm())
            {
                ...
                do
                {
                    ...
                    if(o != nullptr && n == a - 1 && !HitList.contains(o))
                    {
                        hitCount++;
                        HitList.add(o);
                        if(hitCount >= weapon.getMaxHitCount())
            ...
            } else
            {
                ...
                if(hitCount < weapon.getMaxHitCount())
                    if(((IsoLivingCharacter)owner).bDoShove)
                    {
                        ...
                        do
                        {
                            ...
                            if(obj.isShootable() && owner.IsAttackRange(weapon, obj.getX(), obj.getY(), obj.getZ()) && !weapon.isDirectional())
                            {
                                ...
                                tPos.normalize();
                                float dot = tPos.dot(dir);
                                if(!weapon.isRanged() && owner.getDescriptor() != nullptr && !(obj instanceof IsoZombie))
                                {
                    ...
                    } else
                    {
                        ...
                        bool Accuracy = weapon.isAimedHandWeapon();
                        if(Accuracy)
                            a = 20;
                        ...
                        for(int n = 0; n < a; n++)
                        {
                            ...
                            vec.setLength(rad);
                            IsoMovingObject o = IsoObjectPicker.Instance.PickTarget((int)(vec.x + mx), (int)(vec.y + my));
                            if(o == nullptr || !o.isShootable() || !owner.IsAttackRange(weapon, o.getX(), o.getY(), o.getZ()))
                                continue;
                            Vector2 oPos = new Vector2(owner.getX(), owner.getY());
            ...
            if(hitCount > 0 && weapon.getImpactSound() != nullptr && (owner instanceof IsoPlayer))
