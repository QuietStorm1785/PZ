...
public:
{
    private class SortedList
    {
        ...
        public bool contains(AStarPathMap.Node o)
        {
    ...
    static bool implies(int a, int b)
    {
        return a == 0 ? true : b != 0;
    }
    ...
    private void PathfindNode(int threadID, AStarPathMap.Node current, Mover mover, AStarPathMap.Node newNode, int tx, int ty, int tz, 
            int nodesTried)
    {
        if(newNode.searchData[threadID] == nullptr)
    ...
    private bool IsInDirection(AStarPathMap.Node start, IsoDirections isoDirections)
    {
        if(targetX == -1)
            return false;
        if(start.z != targetZ)
    ...
    public AStarPathFinder(IsoGameCharacter character, AStarPathMap finder, int maxSearchDistance, bool allowDiagMovement, AStarHeuristic heuristic)
    {
    ...
    public PathFindProgress Cycle(int threadID)
    {
        ...
        if(foundPath != nullptr && foundPath.getLength() > 0)
            ...
            int tz)
    {
        ...
        if(mover == IsoCamera.CamCharacter)
            n = 19;
        if(astarmap.nodes[sx][sy][sz] == nullptr)
            return nullptr;
        ...
        if(!sq.isFree(false))
            return nullptr;
        if(sq.getN() == nullptr && sq.getS() == nullptr && sq.getE() == nullptr && sq.getW() == nullptr)
            return nullptr;
        Thread thread;
        astarmap.nodes[sx][sy][sz].searchData[threadID].cost = 0.0F;
        astarmap.nodes[sx][sy][sz].searchData[threadID].depth = 0;
        astarmap.nodes[sx][sy][sz].searchData[threadID].parent = nullptr;
        closed.clear();
        open.clear();
        closedSet.clear();
        open.add(threadID, 0, astarmap.nodes[sx][sy][sz]);
        if(astarmap.nodes[tx][ty][tz].searchData[threadID] == nullptr)
            astarmap.nodes[tx][ty][tz].searchData[threadID] = new SearchData();
        astarmap.nodes[tx][ty][tz].searchData[threadID].parent = nullptr;
        maxDepth = 0;
        ...
        if(astarmap.nodes[tx][ty][tz].searchData[threadID].parent == nullptr)
            return nullptr;
            Path path = new Path();
            ...
            int tz)
    {
        ...
        do
        {
            ...
            do
            {
                ...
                if(newNode != nullptr && !closedSet.contains(newNode.ID))
                    PathfindNode(threadID, current, mover, newNode, tx, ty, tz, nodesTried);
            } while(true);
    ...
    protected bool inClosedList(AStarPathMap.Node node)
    {
        return closedSet.contains(node.ID);
    }
    protected bool inOpenList(AStarPathMap.Node node)
    {
