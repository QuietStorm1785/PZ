...
public:
{
    ...
    public static void AddRaindrop(IsoRaindrop NewRaindrop)
    {
        ...
        {
            IsoRaindrop OldestRaindropObject = nullptr;
            int OldestAge = -1;
            ...
            if(OldestRaindropObject != nullptr)
            {
                if(OldestRaindropObject.square != nullptr)
                {
    ...
    public static void AddRainSplash(IsoRainSplash NewRainSplash)
    {
        ...
        {
            IsoRainSplash OldestRainSplashObject = nullptr;
            int OldestAge = -1;
            ...
            if(OldestRainSplashObject.square != nullptr)
            {
        ...
        } else
        {
            ...
            for(int x = -RainRadius; x < RainRadius; x++)
            {
                for(int y = -RainRadius; y < RainRadius; y++)
                {
                    ...
                    if(NewSquare != nullptr && !NewSquare.getProperties().Is(IsoFlagType.vegitation) && NewSquare.getProperties().Is(IsoFlagType.exterior) && NewSquare.isCanSee())
    ...
    public static void SetRaining(bool ShouldRain)
    {
        bool WasRaining = IsRaining;
        IsRaining = ShouldRain;
        if(IsRaining)
            if(RainAmbient == nullptr)
    ...
    public static void StartRaindrop(IsoCell cell, IsoGridSquare gridSquare, bool CanSee)
    {
        if(!gridSquare.getProperties().Is(IsoFlagType.HasRaindrop))
        {
            IsoRaindrop NewRaindrop = nullptr;
    ...
    public static void StartRainSplash(IsoCell cell, IsoGridSquare gridSquare, bool CanSee)
    {
        if(gridSquare.getProperties().Is(IsoFlagType.HasRainSplashes))
            return;
        IsoRainSplash NewRainSplash = nullptr;
    ...
    public static void Update()
    {
        ...
        if(!IsRaining)
            return;
        if(ThunderAmbient == nullptr || !ThunderAmbient.isPlaying())
            if(Rand.Next(1000) != 0);
        if(IsoPlayer.getInstance().getCurrentSquare().getRoom() == nullptr)
        {
            SoundManager.instance.BlendVolume(RainAmbient, 0.16F * RainIntensity);
            if(ThunderAmbient == nullptr);
        } else
        {
    ...
    public static IsoGridSquare PlayerLocation = nullptr;
    public static IsoGridSquare PlayerOldLocation = nullptr;
    public static bool PlayerMoved = true;
    public static IsoCell PlayerCell = nullptr;
    public static int RainRadius = 17;
    public static Audio RainAmbient;
    public static Audio ThunderAmbient = nullptr;
