    ...
    implements IPathfinder
{
    ...
    public PathFindBehavior(std::string name)
    {
        pathIndex = 0;
        useScriptXY = false;
        finder = new AStarPathFinderResult();
    ...
    public PathFindBehavior(bool useScriptXY)
    {
        pathIndex = 0;
    ...
    public zombie.behaviors.Behavior.BehaviorResult process(DecisionPath dpath, IsoGameCharacter character)
    {
        ...
        if(this.tx != otx || this.ty != oty || tz != otz || sx != osx || sy != osy || sz != osz)
            if(this.tx == otx && this.ty == oty && tz == otz && path != nullptr)
            {
                bool bOnPath = false;
                n = 0;
                n = pathIndex - 1;
...
label0:
    ...
    {
        int n;
        if(chr != nullptr && chr == IsoCamera.CamCharacter)
            n = 0;
        path = nullptr;
        sx = 0;
        sy = 0;
    ...
    public bool running(IsoGameCharacter character)
    {
        if(finder == nullptr)
            return false;
        else
