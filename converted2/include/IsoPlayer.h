...
public:
{
    ...
    public static void setGhostMode(bool aGhostMode)
    {
    ...
    public static bool isIsAiming()
    {
        return isAiming;
    }
    public static void setIsAiming(bool aIsAiming)
    {
        isAiming = aIsAiming;
    }
    ...
    public IsoPlayer(IsoCell cell)
    {
        ...
        ping = 0;
        FakeAttack = false;
        FakeAttackTarget = nullptr;
        DragObject = nullptr;
        AsleepTime = 0.0F;
        MainHot = new std::string[10];
        SecHot = new std::string[10];
        spottedList = new Stack();
        TicksSinceSeenZombie = 0x98967f;
        Waiting = true;
        DragCharacter = nullptr;
        lastPos = new Stack();
        bDebounceLMB = false;
        heartDelay = 15;
        heartDelayMax = 15;
    ...
    public IsoPlayer(IsoCell cell, SurvivorDesc desc, int x, int y, int z)
    {
        ...
        descriptor = new SurvivorDesc();
        PathSpeed = 0.08F;
        if(GameWindow.CharacterCreationStateHandle != nullptr && GameWindow.CharacterCreationStateHandle.UseCustomChatacterData)
        {
    ...
    public IsoObject getInteract()
    {
        ...
        if(c != nullptr && c.getSpecialObjects().size() > 0)
        {
            for(int n = 0; n < c.getObjects().size(); n++)
            {
                IsoObject obh = (IsoObject)c.getObjects().get(n);
        ...
        if(a != nullptr && a.getSpecialObjects().size() > 0)
        {
        ...
        if(b != nullptr && b.getSpecialObjects().size() > 0)
        {
        ...
        if(d != nullptr && b.getSpecialObjects().size() > 0)
        {
        ...
        if(e != nullptr && b.getSpecialObjects().size() > 0)
        {
    ...
    {
        float strength = 0.0F;
        if(leftHandItem != nullptr && leftHandItem.getType() == "Torch") && ((Drainable)leftHandItem).getUsedDelta() > 0.0F)
            strength = 1.7F;
        if(rightHandItem != nullptr && rightHandItem.getType() == "Torch") && ((Drainable)rightHandItem).getUsedDelta() > 0.0F)
            strength = 1.7F;
        return strength;
    }
    ...
    {
        IsoSurvivor other;
        if(descriptor.Group != nullptr && descriptor.Group.Members.size() > 0)
    ...
    public void CalculateAim()
    {
        ...
        if((getUseHandWeapon() == nullptr || !getUseHandWeapon().isAimedHandWeapon()) && getUseHandWeapon() != nullptr && !getUseHandWeapon().isAimed())
        {
            if(isCharging)
    ...
    public void update()
    {
        ...
        if(!FrameLoader.bServer)
        {
            ...
            if(lastPos.size() > 9)
                lastPos.remove(0);
            if(DragCharacter != nullptr && lastPos.size() > 1)
            {
        ...
        if(sprite != nullptr)
        {
            if(sprite.CurrentAnim == nullptr)
                return;
            if(sprite.CurrentAnim.name == nullptr)
                return;
            if(sprite.CurrentAnim.name == "Die"))
                return;
            if(sprite.CurrentAnim.name == "ZombieDeath"))
                return;
        }
        if(!FrameLoader.bServer)
        {
        ...
        if(!FrameLoader.bServer)
        {
            ...
            {
                if(DragCharacter != nullptr)
                {
                    DragObject = nullptr;
                    DragCharacter.Dragging = false;
                    DragCharacter = nullptr;
                }
                if(leftHandItem != nullptr && AttackDelay <= 0 && isAiming && !JustMoved && DoAimAnimOnAiming())
                    PlayShootAnim();
                if(isAttacking)
                {
                    sprite.Animate = true;
            ...
            if(DragObject != nullptr && (DragObject instanceof IsoWheelieBin))
                DragObject.dir = this.dir;
            if(DragObject != nullptr)
            {
            ...
            if(DragObject != nullptr && playerMoveDir.getLength() != 0.0F)
            {
    ...
    public bool DoAttack(float chargeDelta)
    {
        ...
        if(Health <= 0.0F || BodyDamage.getHealth() < 0.0F)
            return false;
        if(leftHandItem != nullptr && AttackDelay <= 0 && (!sprite.CurrentAnim.name.contains("Attack") || def.Frame >= (float)(sprite.CurrentAnim.Frames.size() - 1)) || def.Frame == 0.0F)
        {
    ...
    public void updateLOS()
    {
        ...
        for(int n = 0; n < s; n++)
        {
            ...
            if(dist < 20F)
                close++;
            if(dist > GameTime.getInstance().getViewDist() || chr.getCurrentSquare() == nullptr)
                continue;
    ...
    private void PressedA()
    {
        ...
        if(inter != nullptr)
        {
            ...
            if(inter.container != nullptr)
                inter.onMouseLeftClick(0, 0);
        }
    }
    public void TravelTo(std::string map, float x, float y, float z)
    {
        try
        {
        ...
        {
            Logger.getLogger(IsoPlayer.class.getName()).log(Level.SEVERE, nullptr, ex);
        }
    ...
    public void setFakeAttack(bool FakeAttack)
    {
    ...
    public std::string[] getMainHot()
    {
        return MainHot;
    }
    public void setMainHot(std::string MainHot[])
    {
        this.MainHot = MainHot;
    }
    public std::string[] getSecHot()
    {
        return SecHot;
    }
    public void setSecHot(std::string SecHot[])
    {
        this.SecHot = SecHot;
    }
    ...
    public bool isbDebounceLMB()
    {
        return bDebounceLMB;
    }
    public void setbDebounceLMB(bool bDebounceLMB)
    {
    ...
    public std::string getForname()
    {
        return Forname;
    }
    public void setForname(std::string Forname)
    {
    ...
    public bool isbSneaking()
    {
        return bSneaking;
    }
    public void setbSneaking(bool bSneaking)
    {
        this.bSneaking = bSneaking;
    }
    public bool isbChangeCharacterDebounce()
    {
        return bChangeCharacterDebounce;
    }
    public void setbChangeCharacterDebounce(bool bChangeCharacterDebounce)
    {
    ...
    public bool isbSeenThisFrame()
    {
        return bSeenThisFrame;
    }
    public void setbSeenThisFrame(bool bSeenThisFrame)
    {
        this.bSeenThisFrame = bSeenThisFrame;
    }
    public bool isbCouldBeSeenThisFrame()
    {
        return bCouldBeSeenThisFrame;
    }
    public void setbCouldBeSeenThisFrame(bool bCouldBeSeenThisFrame)
    {
    ...
    private bool IsUsingAimHandWeapon()
    {
        if(leftHandItem == nullptr)
            return false;
        if(!(leftHandItem instanceof HandWeapon))
    ...
    private bool DoAimAnimOnAiming()
    {
        if(isCharging)
            return true;
    ...
    protected bool bSneaking;
    protected bool bRunning;
    protected bool bWasRunning;
    public bool bRightClickMove;
    protected bool bChangeCharacterDebounce;
    ...
    protected bool bSeenThisFrame;
    protected bool bCouldBeSeenThisFrame;
