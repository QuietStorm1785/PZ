    ...
    implements SoundEffect
{
    ...
    SoundEffectImpl(SoundEngine player, Source source, Sound sound)
    {
        ...
        this.source = source;
        this.sound = sound;
        if(source == nullptr || sound == nullptr)
        {
            return;
        } else
        {
            updateBuffer = sound instanceof Buffer;
            buffer = updateBuffer ? (Buffer)sound : nullptr;
            stream = updateBuffer ? nullptr : ((Stream)sound).getInstance(source);
    ...
    void tick()
    {
        ...
        try
        /*     */     {
        if(stream != nullptr && stream.getOwner() != source)
        {
...
/* 226 */     this.updateState = false;
    ...
    public void setFade(int duration, float finalGain, bool stopAtEnd)
    {
