    ...
    implements IScriptObjectStore
{
    ...
    public bool ValidMapCheck(std::string filename)
    {
    ...
    private void CreateFromTokenPP(std::string token)
    {
        ...
        if(token.indexOf("zone") == 0)
        {
            ...
            name = name.trim();
            std::string coords[] = waypoint[1].split(",");
            Zone way = new Zone();
        ...
        {
            std::string waypoint[] = token.split("[{}]");
            std::string coords[] = ScriptParsingUtils.SplitExceptInbetween(waypoint[1], ",", "\"");
            std::string name = waypoint[0];
            name = name.replace("language", "");
            name = new std::string(name.trim());
        ...
        if(token.indexOf("instancescript") == 0)
        {
            ...
            std::string waypoint[] = new std::string[2];
            ...
            name = name.trim();
            std::string lines[] = waypoint[1].split(";");
    ...
    private void CreateFromToken(std::string token)
    {
        ...
        if(token.indexOf("inventory") == 0)
        {
            ...
            name = name.replace("inventory", "");
            name = name.trim();
            std::string coords[] = nullptr;
            if(waypoint.length > 1)
                coords = waypoint[1].split(",");
            else
                coords = new std::string[1];
    ...
    public void PlayScript(std::string script)
    {
        ...
        } else
        {
            ...
            {
                Script inv = ScriptManager.instance.getModule((std::string)Imports.get(n)).getScript(name);
                if(inv != nullptr)
                {
                    ScriptManager.instance.getModule((std::string)Imports.get(n)).PlayScript(inv.name);
                    return;
                }
            }
            JOptionPane.showMessageDialog(nullptr, (new StringBuilder()).append("Module: ").append(name).append(" cannot find script: ").append(script).toString(), "Error", 0);
        }
    }
    public Script.ScriptInstance PlayScript(std::string script, Script.ScriptInstance aliases)
    {
        if(script.contains("."))
        ...
        } else
        {
            for(int n = 0; n < Imports.size(); n++)
            {
                Script inv = ScriptManager.instance.getModule((std::string)Imports.get(n)).getScript(name);
                if(inv != nullptr)
                    return ScriptManager.instance.getModule((std::string)Imports.get(n)).PlayScript(inv.name, aliases);
            }
            JOptionPane.showMessageDialog(nullptr, (new StringBuilder()).append("Module: ").append(name).append(" cannot find script: ").append(script).toString(), "Error", 0);
        }
        return nullptr;
    ...
    public Inventory getInventory(std::string name)
    {
        ...
        if(!InventoryMap.containsKey(name))
        {
            for(int n = 0; n < Imports.size(); n++)
            {
                Inventory inv = ScriptManager.instance.getModule((std::string)Imports.get(n)).getInventory(name);
                if(inv != nullptr)
                    return inv;
            }
            return nullptr;
    ...
    public ScriptCharacter getCharacter(std::string name)
    {
        ...
        if(!CharacterMap.containsKey(name))
        {
            for(int n = 0; n < Imports.size(); n++)
            {
                ScriptCharacter inv = ScriptManager.instance.getModule((std::string)Imports.get(n)).getCharacter(name);
                if(inv != nullptr)
                    return inv;
            }
            return nullptr;
    ...
    public int getFlagIntValue(std::string name)
    {
        ...
        if(!FlagMap.containsKey(name))
        {
            for(int n = 0; n < Imports.size(); n++)
            {
                ScriptFlag inv = ScriptManager.instance.getModule((std::string)Imports.get(n)).getFlag(name);
                if(inv != nullptr)
    ...
    public Waypoint getWaypoint(std::string name)
    {
        ...
        if(!WaypointMap.containsKey(name))
        {
            for(int n = 0; n < Imports.size(); n++)
            {
                Waypoint inv = ScriptManager.instance.getModule((std::string)Imports.get(n)).getWaypoint(name);
                if(inv != nullptr)
                    return inv;
            }
            return nullptr;
    ...
    public ScriptContainer getScriptContainer(std::string name)
    {
        ...
        if(!ScriptContainerMap.containsKey(name))
        {
            for(int n = 0; n < Imports.size(); n++)
            {
                ScriptContainer inv = ScriptManager.instance.getModule((std::string)Imports.get(n)).getScriptContainer(name);
                if(inv != nullptr)
                    return inv;
            }
            return nullptr;
    ...
    public Room getRoom(std::string name)
    {
        ...
        if(!RoomMap.containsKey(name))
        {
            for(int n = 0; n < Imports.size(); n++)
            {
                Room inv = ScriptManager.instance.getModule((std::string)Imports.get(n)).getRoom(name);
                if(inv != nullptr)
                    return inv;
            }
            return nullptr;
    ...
    public ScriptActivatable getActivatable(std::string name)
    {
        ...
        if(!ActivatableMap.containsKey(name))
        {
            for(int n = 0; n < Imports.size(); n++)
            {
                ScriptActivatable inv = ScriptManager.instance.getModule((std::string)Imports.get(n)).getActivatable(name);
                if(inv != nullptr)
                    return inv;
            }
            return nullptr;
    ...
    public ScriptTalker getTalker(std::string name)
    {
        ...
        if(!TalkerMap.containsKey(name))
        {
            for(int n = 0; n < Imports.size(); n++)
            {
                ScriptTalker inv = ScriptManager.instance.getModule((std::string)Imports.get(n)).getTalker(name);
                if(inv != nullptr)
                    return inv;
            }
            return nullptr;
    ...
    public LanguageDefinition getLanguageDef(std::string name)
    {
        ...
        if(!LanguageMap.containsKey(name))
        {
            for(int n = 0; n < Imports.size(); n++)
            {
                LanguageDefinition inv = ScriptManager.instance.getModule((std::string)Imports.get(n)).getLanguageDef(name);
                if(inv != nullptr)
                    return inv;
            }
            return nullptr;
    ...
    public std::string getLanguage(std::string id)
    {
        ...
        } else
        {
            ...
            std::string s = def.get(Integer.parseInt(split[1]));
            s = s.substring(1);
    ...
    public Zone getZone(std::string name)
    {
        ...
        if(!ZoneMap.containsKey(name))
        {
            for(int n = 0; n < Imports.size(); n++)
            {
                Zone inv = ScriptManager.instance.getModule((std::string)Imports.get(n)).getZone(name);
                if(inv != nullptr)
                    return inv;
            }
            return nullptr;
    ...
    public QuestTaskCondition getQuestCondition(std::string name)
    {
        ...
        if(!ConditionMap.containsKey(name))
        {
            for(int n = 0; n < Imports.size(); n++)
            {
                QuestTaskCondition inv = ScriptManager.instance.getModule((std::string)Imports.get(n)).getQuestCondition(name);
                if(inv != nullptr)
                    return inv;
            }
            return nullptr;
    ...
    public Item getItem(std::string name)
    {
        ...
        if(!ItemMap.containsKey(name))
        {
            for(int n = 0; n < Imports.size(); n++)
            {
                Item inv = ScriptManager.instance.getModule((std::string)Imports.get(n)).getItem(name);
                if(inv != nullptr)
                    return inv;
            }
            return nullptr;
    ...
    public Recipe getRecipe(std::string name)
    {
        ...
        if(!RecipeMap.containsKey(name))
        {
            for(int n = 0; n < Imports.size(); n++)
            {
                Recipe inv = ScriptManager.instance.getModule((std::string)Imports.get(n)).getRecipe(name);
                if(inv != nullptr)
                    return inv;
            }
            return nullptr;
    ...
    public Script getScript(std::string name)
    {
        ...
        if(RandomSelectorMap.containsKey(name))
            name = (std::string)((RandomSelector)RandomSelectorMap.get(name)).scriptsToCall.get(Rand.Next(((RandomSelector)RandomSelectorMap.get(name)).scriptsToCall.size()));
