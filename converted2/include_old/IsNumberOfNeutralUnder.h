...
// Source File Name:   IsNumberOfNeutralUnder.java
namespace zombie {
namespace scripting {
namespace commands {
namespace Character {
...
#include "objects.h"
class IsNumberOfNeutralUnder : public BaseCommand {
public:
{
    public IsNumberOfNeutralUnder()
    {
        number = 0;
        invert = false;
    }
    ...
    public bool getValue()
    {
        ...
        if(chr == nullptr)
            return false;
        if(number > chr.getLocalNeutralList().size())
            return !invert;
        else
            return invert;
    }
