...
#include "AStarPathFinderResult.h"
#include "states.h"
#include "BehaviorHub.h"
...
public:
{
    ...
    public bool onMouseLeftClick(int x, int y)
    {
        ...
        if(this != TutorialManager.instance.wife && UIManager.getDragInventory() == nullptr)
            UIManager.LaunchRadial(this);
        if(IsoPlayer.instance.getCurrentSquare().getRoom() == getCurrentSquare().getRoom() && IsoPlayer.instance.DistTo(this) < 4F && UIManager.getDragInventory() != nullptr)
        ...
        if(Draggable && this == TutorialManager.instance.wife && IsoUtils.DistanceTo(IsoPlayer.instance.getX(), IsoPlayer.instance.getY(), getX(), getY()) < 2.0F)
        {
            ...
            {
                IsoPlayer.instance.DragCharacter = nullptr;
            }
        }
    ...
    public void update()
    {
        ...
        if(getLastSquare() != getCurrentSquare() && getCurrentSquare() != nullptr && getLastSquare() != nullptr)
        {
            IsoDoor door = getCurrentSquare().getDoorFrameTo(getLastSquare());
            if(door != nullptr && door.open && RemoteID == -1)
        ...
        if(descriptor.Group.Safehouse != nullptr && !descriptor.IsLeader() && RemoteID == -1 && Order == nullptr && !IsInBuilding(descriptor.Group.Safehouse))
            GiveOrder(new GotoBuildingOrder(this, descriptor.Group.Safehouse), false);
        if(descriptor.IsLeader() && RemoteID == -1)
            if(descriptor.Group.Safehouse == nullptr && IsoWorld.instance.CurrentCell.getBuildingList().size() > 5)
            {
                IsoBuilding b;
                for(b = nullptr; b == nullptr || b.Exits.isEmpty() || b.Rooms.size() <= 1; b = (IsoBuilding)IsoWorld.instance.CurrentCell.getBuildingList().get(Rand.Next(IsoWorld.instance.CurrentCell.getBuildingList().size())));
                IsoBuilding best = b;
                if(best != nullptr)
                {
            ...
            {
                if(descriptor.Group.Safehouse != nullptr && !descriptor.Group.Safehouse.Exits.isEmpty() && Order == nullptr && !IsInBuilding(descriptor.Group.Safehouse) && Rand.Next(800) == 0)
                    GiveOrder(new GotoBuildingOrder(this, descriptor.Group.Safehouse), true);
                if(descriptor.Group.Safehouse != nullptr && Order == nullptr && (IsoPlayer.DemoMode || descriptor.Group.getTotalNeedPriority() > 50 && descriptor.Group.Safehouse.safety >= 0 && Rand.Next(1000) == 0 && GameTime.getInstance().getTimeOfDay() > 8F && GameTime.getInstance().getTimeOfDay() < 13F))
        ...
        if(NeightbourUpdate <= 0)
        {
            ...
            ClosestTwoSurvivors[0] = nullptr;
            ClosestTwoSurvivors[1] = nullptr;
            ...
            synchronized(LocalList)
            {
                for(n = 0; n < LocalList.size(); n++)
                {
                    ...
                    if(mov == this || !(mov instanceof IsoGameCharacter) || (mov instanceof IsoZombie) && ((IsoZombie)mov).Ghost || (mov instanceof IsoGameCharacter) && !((IsoGameCharacter)mov).VisibleToNPCs || mov.getCurrentSquare() == nullptr)
                        continue;
                    ...
                    if(ydist < 1)
                        ydist = 1;
                    if((mov instanceof IsoZombie) && mov.getCurrentSquare() != nullptr && mov.getCurrentSquare().getRoom() == getCurrentSquare().getRoom() && mov.getZ() == getZ())
                    {
                    ...
                    if(xdist < 8 && ydist < 8 && mov.getCurrentSquare().getRoom() == getCurrentSquare().getRoom())
                    {
                        ...
                        if((mov instanceof IsoSurvivor) || (mov instanceof IsoPlayer))
                            if(ClosestTwoSurvivors[0] == nullptr)
                                ClosestTwoSurvivors[0] = (IsoGameCharacter)mov;
                            else
                            if(ClosestTwoSurvivors[1] == nullptr)
                    ...
                    if(mov.getCurrentSquare() != nullptr && mov.getCurrentSquare().getRoom() == getCurrentSquare().getRoom() && mov.getCurrentSquare() != nullptr && getCurrentSquare() != nullptr && !(mov instanceof IsoZombie))
        ...
        if(RemoteID == -1 && !isDead())
            ...
            if(lmove.getLength() == 0.0F)
                if(legsSprite != nullptr && !legsSprite.CurrentAnim.name.contains("Attack_"))
                {
                    if(aimAt == nullptr)
                    {
                        PlayAnim("Idle");
    ...
    private void MeetAgain(IsoGameCharacter survivor)
    {
        if(survivor.BodyDamage.getNumPartsBitten() > 0 || survivor.BodyDamage.getNumPartsScratched() > 0)
        {
            if(BodyDamage.getNumPartsBitten() <= 0 && BodyDamage.getNumPartsScratched() <= 0)
            {
                ...
                ScriptManager.instance.PlayInstanceScript(nullptr, (new StringBuilder()).append("Base.YouBeenBit").append(Rand.Next(count.intValue()) + 1).toString(), "Bitten", survivor, "Other", this);
            }
        } else
        {
            std::string thing = nullptr;
            thing = "Base.MeetAgain";
            IsoGameCharacter a = nullptr;
            IsoGameCharacter b = nullptr;
            ...
            ScriptManager.instance.PlayInstanceScript(nullptr, (new StringBuilder()).append(thing).append(Rand.Next(count.intValue()) + 1).toString(), "Met", b, "Other", a);
        }
    }
    public void FollowMe(IsoGameCharacter leader)
    {
        ...
        ScriptManager.instance.PlayInstanceScript(nullptr, thing, "Follower", b, "Leader", a);
    }
    ...
    public void MeetFirstTime(IsoGameCharacter survivor, bool reverseRoles, bool Checkbitten)
    {
    ...
    public void Killed(IsoGameCharacter gameCharacter)
    {
        ...
        if(c == nullptr || !c.getActiveInInstances().isEmpty() || c.DistTo(this) > 8F)
        {
            c = nullptr;
            Integer integer = n;
        ...
        if(b != nullptr && (!b.getActiveInInstances().isEmpty() || b.DistTo(this) > 8F))
        {
            b = c;
            c = nullptr;
            Integer integer1 = n;
        ...
        Integer count = Integer.valueOf(ScriptManager.instance.getFlagIntValue((new StringBuilder()).append(thing).append("Count").toString()));
        if(n.intValue() == 3)
            ScriptManager.instance.PlayInstanceScript(nullptr, (new StringBuilder()).append(thing).append(Rand.Next(count.intValue()) + 1).toString(), "B", c, "A", b, "Killer", a);
        if(n.intValue() == 2)
            ScriptManager.instance.PlayInstanceScript(nullptr, (new StringBuilder()).append(thing).append(Rand.Next(count.intValue()) + 1).toString(), "A", b, "Killer", a);
        if(n.intValue() == 1)
            ScriptManager.instance.PlayInstanceScript(nullptr, (new StringBuilder()).append(thing).append(Rand.Next(count.intValue()) + 1).toString(), "Killer", a);
    }
    public void ChewedByZombies()
    {
        ...
        IsoGameCharacter c = ClosestTwoSurvivors[1];
        Integer n = Integer.valueOf(3);
        if(c == nullptr || !c.getActiveInInstances().isEmpty())
        {
            c = nullptr;
            Integer integer = n;
            Integer integer3 = n = Integer.valueOf(n.intValue() - 1);
        ...
        if(n.intValue() == 3)
            n = Integer.valueOf(2);
        n = Integer.valueOf(Rand.Next(n.intValue()) + 1);
        std::string thing = "Base.ChewedByZombies";
        thing = (new StringBuilder()).append(thing).append("_").append(n).append("Man").toString();
        Integer count = Integer.valueOf(ScriptManager.instance.getFlagIntValue((new StringBuilder()).append(thing).append("Count").toString()));
        if(n.intValue() == 2)
            ScriptManager.instance.PlayInstanceScript(nullptr, (new StringBuilder()).append(thing).append(Rand.Next(count.intValue()) + 1).toString(), "A", b, "Chewed", a);
        if(n.intValue() == 1)
            ScriptManager.instance.PlayInstanceScript(nullptr, (new StringBuilder()).append(thing).append(Rand.Next(count.intValue()) + 1).toString(), "Chewed", a);
    }
    private void DoRandomTalk()
    {
    }
    public void GivenItemBy(IsoGameCharacter survivor, std::string type, bool bNeeded)
    {
        if(Speaking)
            return;
        if(!getActiveInInstances().isEmpty())
        ...
        b = survivor;
        Integer count = Integer.valueOf(ScriptManager.instance.getFlagIntValue((new StringBuilder()).append(thing).append("Count").toString()));
        ScriptManager.instance.PlayInstanceScript(nullptr, (new StringBuilder()).append(thing).append(Rand.Next(count.intValue()) + 1).toString(), "Giver", b, "Taker", a);
    }
    public void PatchedUpBy(IsoGameCharacter survivor)
    {
        ...
        } else
        {
            ...
            b = survivor;
            Integer count = Integer.valueOf(ScriptManager.instance.getFlagIntValue((new StringBuilder()).append(thing).append("Count").toString()));
            ScriptManager.instance.PlayInstanceScript(nullptr, (new StringBuilder()).append(thing).append(Rand.Next(count.intValue()) + 1).toString(), "Medic", b, "Hurt", a);
            return;
        }
    }
    public Stack getAvailableMembers()
    {
        ...
        for(int n = 0; n < descriptor.Group.Members.size(); n++)
        {
            ...
            if(desc == descriptor)
                continue;
            if(desc.Instance.getCurrentSquare().getRoom() != nullptr && getCurrentSquare().getRoom() != nullptr && desc.Instance.getCurrentSquare().getRoom().building == getCurrentSquare().getRoom().building)
                bAvail = true;
    ...
    private bool HasPersonalNeed(std::string type)
    {
        for(int n = 0; n < PersonalNeeds.size(); n++)
            if(((Order)PersonalNeeds.get(n)).type == type))
                return true;
        return false;
    }
    public bool isCollidedWithPushableThisFrame()
    {
        return bCollidedWithPushable;
    }
    public bool SatisfiedWithInventory(zombie.behaviors.survivor.orders.LootBuilding.LootStyle lootStyle, SatisfiedBy satisfiedBy)
    {
        float score = 0.0F;
    ...
    public bool bLastSpottedPlayer;
    public bool bSpottedPlayer;
    public bool bWillJoinPlayer;
    ...
    Stack availableTemp;
    public bool bCollidedWithPushable;
