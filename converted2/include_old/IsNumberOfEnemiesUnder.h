...
// Source File Name:   IsNumberOfEnemiesUnder.java
namespace zombie {
namespace scripting {
namespace commands {
namespace Character {
...
#include "objects.h"
class IsNumberOfEnemiesUnder : public BaseCommand {
public:
{
    public IsNumberOfEnemiesUnder()
    {
        number = 0;
        invert = false;
    }
    ...
    public bool getValue()
    {
        ...
        if(chr == nullptr)
            return false;
        if(number > chr.getLocalRelevantEnemyList().size())
            return !invert;
        else
            return invert;
    }
