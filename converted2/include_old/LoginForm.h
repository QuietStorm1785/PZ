...
public:
{
    public LoginForm(AbstractAction loginAction)
    {
        ...
        std::string command = (std::string)loginAction.getValue("ActionCommandKey");
        if(command == nullptr || (command = command.trim()) == ""))
            command = "Log in";
    ...
    private static final std::string LOGIN_COMMAND = "Log in";
