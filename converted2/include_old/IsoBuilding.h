...
public:
{
    ...
    public bool ContainsAllItems(Stack items)
    {
        return false;
    }
    public float ScoreBuildingPersonSpecific(SurvivorDesc desc, bool bFarGood)
    {
        float score = 0.0F;
        score += Rooms.size() * 5;
        score += Exits.size() * 15;
    ...
    public void update()
    {
        ...
        if(AlarmTriggered && AlarmTime > 0)
        {
            ...
            {
                if(burgSound != nullptr && AlarmTime % 18 == 0)
                    burgSound = SoundManager.instance.PlayWorldSoundWav("burgalarm", sq, 0.0F, 40F, 1.0F, true);
                AlarmTime--;
                if(AlarmTime % 120 == 0)
                    WorldSoundManager.instance.addSound(nullptr, sq.getX(), sq.getY(), sq.getZ(), 90, 90);
                if(AlarmTime == 0 && burgSound != nullptr)
                {
                    burgSound.stop();
                    burgSound = nullptr;
                }
        ...
        for(IsoRoomExit exit : this.Exits)
        {
            ...
            for(int n = 0; n < a.getSpecialObjects().size(); n++)
            {
                ...
                if(spec instanceof IsoDoor)
                    ((IsoDoor)spec).Barricade(nullptr, nullptr);
            }
...
/*     */     IsoRoom room;
    ...
    public void CalculateWindows()
    {
        for(Iterator i$ = Rooms.iterator(); i$.hasNext();)
        {
            ...
            while(i$1.hasNext()) 
            {
                ...
                if(s != nullptr)
                {
                    bool bSameBuilding = s.getRoom() != nullptr;
                    if(s.getRoom() != nullptr && s.getRoom().building != room.building)
                        bSameBuilding = false;
                ...
                if(e != nullptr)
                {
                    bool bSameBuilding = e.getRoom() != nullptr;
                    if(e.getRoom() != nullptr && e.getRoom().building != room.building)
                        bSameBuilding = false;
    ...
    public void FillContainers()
    {
        ...
        if(!bIsTutHouse || !Core.GameMode == "KateBaldspot"))
            ItemContainerFiller.DistributionTarget.add(this);
    }
    public bool FullyBarricaded()
    {
        ...
        for(int n = 0; n < container.size(); n++)
        {
            ...
            for(int m = 0; m < con.Items.size(); m++)
            {
                ...
                if(!(item instanceof HandWeapon))
                    continue;
                float s = ((HandWeapon)item).getScore(nullptr);
                if(s < 0.0F)
                    s = 0.0F;
                score.weapons += s;
            }
    ...
    {
        for(Iterator it = Rooms.iterator(); it != nullptr && it.hasNext();)
        {
