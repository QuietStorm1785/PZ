...
public:
{
    public Food(std::string module, std::string name, std::string itemType, std::string texName)
    {
        ...
        if(container != nullptr)
        {
            ...
            if(IsCookable && temp > 1.6F)
            {
                ...
                if((int)CurrentCookMinute != LastCookMinute)
                {
                    ...
                    {
                        if(container != nullptr && !container.SourceGrid.getCell().IsZone("tutArea", container.SourceGrid.getX(), container.SourceGrid.getY()) && Burnt && IsoPlayer.getInstance().getPerkLevel(zombie.characters.skills.PerkFactory.Perks.Cooking) == 0 && Rand.Next(40) < 3)
    ...
    {
        return Math.abs(Heat - ((Food)item).Heat) < 0.1F && isCooked() == ((Food)item).isCooked() && Age == ((Food)item).Age && Burnt == ((Food)item).Burnt && name == item.getName());
    }
    bool CanStackNoTemp(InventoryItem item)
    {
        return isCooked() == ((Food)item).isCooked() && Age == ((Food)item).Age && Burnt == ((Food)item).Burnt && name == item.getName());
    }
    ...
    public bool isBadCold()
    {
        return bBadCold;
    }
    public void setBadCold(bool bBadCold)
    {
        this.bBadCold = bBadCold;
    }
    public bool isGoodHot()
    {
        return bGoodHot;
    }
    public void setGoodHot(bool bGoodHot)
    {
    ...
    public std::string getUseOnConsume()
    {
        return useOnConsume;
    }
    public void setUseOnConsume(std::string useOnConsume)
    {
    ...
    public void setRotten(bool rotten)
    {
        this.rotten = rotten;
    }
    public bool isbDangerousUncooked()
    {
        return bDangerousUncooked;
    }
    public void setbDangerousUncooked(bool bDangerousUncooked)
    {
    ...
    protected bool bBadCold;
    protected bool bGoodHot;
    protected float Heat;
    protected float endChange;
    protected float hungChange;
    protected std::string requireInHandOrInventory;
    protected std::string useOnConsume;
    protected bool rotten;
    protected bool bDangerousUncooked;
    protected bool alcoholic;
