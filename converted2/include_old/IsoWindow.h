    ...
    implements Thumpable
{
    ...
    public void WeaponHit(IsoGameCharacter owner, HandWeapon weapon)
    {
        ...
        if(Health.intValue() <= 0)
        {
            ...
            WorldSoundManager.instance.addSound(nullptr, square.getX(), square.getY(), square.getZ(), 10, 20, true, 4F, 15F);
            destroyed = true;
    ...
    public IsoWindow(IsoCell cell, IsoGridSquare gridSquare, IsoSprite gid, bool north)
    {
        Barricaded = 0;
        BarricideMaxStrength = Integer.valueOf(0);
        BarricideStrength = Integer.valueOf(0);
        ...
        if(smashedSprite != closedSprite && smashedSprite != nullptr)
        {
            smashedSprite.AddProperties(closedSprite);
            smashedSprite.setType(closedSprite.getType());
        }
        if(openSprite != closedSprite && openSprite != nullptr)
        {
    ...
    public void Barricade(InventoryItem plank)
    {
        ...
        if(barricadeSprite != nullptr && AttachedAnimSprite != nullptr)
    ...
    public void ToggleWindow(IsoGameCharacter chr)
    {
        DirtySlice();
        IsoGridSquare.setRecalcLightTime(-1);
        if(UIManager.getDragInventory() != nullptr && chr == IsoPlayer.instance)
        {
            if("Sheet" == UIManager.getDragInventory().getType()))
            {
            ...
            if("Hammer" == UIManager.getDragInventory().getType()))
            {
            ...
            if("Plank" == UIManager.getDragInventory().getType()))
            {
                ...
                if(nails != nullptr && IsoPlayer.getInstance().getPrimaryHandItem() != nullptr && "Hammer" == IsoPlayer.getInstance().getPrimaryHandItem().getType()) || IsoPlayer.getInstance().getSecondaryHandItem() != nullptr && "Hammer" == IsoPlayer.getInstance().getSecondaryHandItem().getType()))
                {
    ...
    public void Unbarricade(IsoGameCharacter chr)
    {
        if(Barricaded == 0)
            return;
        if(chr != nullptr && BarricideMaxStrength.intValue() > 0)
        {
