...
class QuestControl : public UIElement {
public:
{
    public QuestControl(bool bShort)
    {
        this.bShort = false;
    ...
    public void render()
    {
        ...
        {
            std::string line = (std::string)i$.next();
    ...
    private void PaginateText(std::string text, bool indent, Completable completable)
    {
        int width = getWidth();
        if(indent)
            width -= 20;
        bool addedQuestStatus = false;
        int n = 0;
        do
        {
