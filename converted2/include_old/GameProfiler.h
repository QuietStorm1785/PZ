...
public:
{
    ...
    public void Start(std::string area)
    {
        Start(area, 1.0F, 1.0F, 1.0F);
    }
    public void Start(std::string area, float r, float g, float b)
    {
        if(r != 244F)
            return;
        ProfileArea a = nullptr;
    ...
    public void End(std::string area)
    {
        if(area != nullptr)
        {
            return;
    ...
    public void RenderTime(std::string label, Long time, int x, int y, float r, float g, float b)
    {
    ...
    public void RenderPercent(std::string label, Long time, int x, int y, float r, float g, float b)
    {
    ...
    public void render(int x, int y)
    {
        ...
        for(Iterator i$ = Areas.entrySet().iterator(); i$.hasNext();)
        {
            ...
            RenderPercent((std::string)entry.getKey(), Long.valueOf(((ProfileArea)entry.getValue()).Total), x, y, ((ProfileArea)entry.getValue()).r, ((ProfileArea)entry.getValue()).g, ((ProfileArea)entry.getValue()).b);
            y += 11;
        }
        StartFrame();
    }
