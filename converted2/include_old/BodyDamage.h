...
public:
{
    ...
    {
        if(getParentChar().getCurrentSquare().getRoom() != nullptr || !RainManager.IsRaining)
        {
    ...
    public void TriggerSneezeCough()
    {
        ...
        bool TissueConsumed = false;
        if(getParentChar().getPrimaryHandItem() != nullptr && getParentChar().getPrimaryHandItem().getType() == "Tissue"))
        {
        ...
        if(getParentChar().getSecondaryHandItem() != nullptr && getParentChar().getSecondaryHandItem().getType() == "Tissue") && ((Drainable)getParentChar().getSecondaryHandItem()).getUsedDelta() > 0.0F)
        {
    ...
    public void UpdateCold()
    {
        if(isHasACold())
        {
            ...
            if(getParentChar().getCurrentSquare().getRoom() == nullptr || getParentChar().getMoodles().getMoodleLevel(MoodleType.Wet) > 0 || getParentChar().getStats().fatigue > 0.5F || getParentChar().getStats().hunger > 0.25F || getParentChar().getStats().thirst > 0.25F)
                Recovering = false;
            if(Recovering)
            {
                float Delta = 1.0F;
    ...
    public void AddRandomDamageFromZombie()
    {
        ...
        int PainType = 0;
        if("Idle" == getParentChar().getSprite().CurrentAnim.name))
            DamageChance *= 3;
    ...
    public void ApplyNetDamage(DamagePlayer response)
    {
        ...
        bool Scratch = response.Scratch;
        bool Bitten = response.Bitten;
        bool infected = response.Infected;
        if(Scratch)
        {
    ...
    public bool DoesBodyPartHaveInjury(BodyPartType BodyPart)
    {
    ...
    public void DrawUntexturedQuad(int X, int Y, int Width, int Height, float r, float g, float b, 
            float a)
    {
        SpriteRenderer.instance.render(nullptr, X, Y, Width, Height, r, g, b, a);
    }
    ...
    public std::string getBodyPartName(BodyPartType BodyPart)
    {
        return BodyPartType.ToString(BodyPart);
    }
    public std::string getBodyPartName(int BodyPartIndex)
    {
    ...
    public void UpdateBoredom()
    {
        ...
        if(getParentChar() == IsoPlayer.instance && IsoPlayer.instance.Asleep)
            return;
        if(getParentChar().getCurrentSquare().getRoom() != nullptr)
        {
    ...
    public bool UseBandageOnMostNeededPart()
    {
        int highestScore = 0;
        ...
        if(highestScore > 0 && part != nullptr)
        {
    ...
    public bool isIsFakeInfected()
    {
        return IsFakeInfected;
    }
    public void setIsFakeInfected(bool IsFakeInfected)
    {
    ...
    public bool isBurntToDeath()
    {
        return BurntToDeath;
    }
    public void setBurntToDeath(bool BurntToDeath)
    {
    ...
    public bool isHasACold()
    {
        return HasACold;
    }
    public void setHasACold(bool HasACold)
    {
