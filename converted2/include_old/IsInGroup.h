...
// Source File Name:   IsInGroup.java
namespace zombie {
namespace scripting {
namespace commands {
namespace Character {
...
#include "objects.h"
class IsInGroup : public BaseCommand {
public:
{
    public IsInGroup()
    {
    ...
    public bool getValue()
    {
        ...
        if(chr == nullptr)
            return false;
        if(chr.getDescriptor().getGroup() != nullptr && chr.getDescriptor().getGroup().HasOtherMembers(chr.getDescriptor()))
            return !invert;
        else
            return invert;
    }
