...
{
    public Texture(TextureID data, std::string name)
    {
        flip = false;
        offsetX = 0.0F;
        offsetY = 0.0F;
    ...
    public Texture(std::string file, bool useAlphaChannel)
    {
        this(new TextureID(file), file);
        setUseAlphaChannel(useAlphaChannel);
    }
    public Texture(int width, int height, std::string name)
    {
        this(new TextureID(width, height), name);
    }
    public Texture(int width, int height)
    {
        this(new TextureID(width, height), ((std::string) (nullptr)));
    }
    public Texture(std::string file, int red, int green, int blue)
    {
    ...
    public static Texture getSharedTexture(std::string name)
    {
    ...
    public static Texture trygetTexture(std::string name)
    {
        if(FrameLoader.bServer)
            return nullptr;
    ...
    static Texture getSharedTextureInternal(std::string name)
    {
        if(FrameLoader.bServer)
            return nullptr;
    ...
    public static Texture getSharedTexture(std::string name, std::string palette)
    {
    ...
    public static Texture getSharedTexture(std::string name, int palette[], std::string paletteName)
    {
    ...
    public static Texture getTexture(std::string name)
    {
        if(!name.contains(".txt"))
        {
            std::string subname = name;
    ...
    public void destroy()
    {
        if(destroyed)
            return;
        if(dataid != nullptr && --dataid.referenceCount == 0)
            dataid.destroy();
        destroyed = true;
    }
    public bool equals(Texture other)
    {
        return other.xStart == xStart && other.xEnd == xEnd && other.yStart == yStart && other.yEnd == yEnd && other.width == width && other.height == height && other.solid == solid && (dataid == nullptr || other.dataid == nullptr || other.dataid.pathFileName == nullptr || dataid.pathFileName == nullptr || other.dataid.pathFileName == dataid.pathFileName));
    }
    ...
    public bool getUseAlphaChannel()
    {
        return !solid;
    }
    public int getWidth()
            ...
            float a)
    {
        ...
        {
            bDoingQuad = false;
            Logger.getLogger(GameApplet.class.getName()).log(Level.SEVERE, nullptr, ex1);
        }
        return;
    }
    ...
    {
        x--;
        SpriteRenderer.instance.render(nullptr, x, y, x + width / 2, y - height / 2, x + width, y, x + width / 2, y + height / 2, rl, gl, bl, al, ru, gu, bu, au, rr, gr, br, ar, rd, gd, bd, ad);
    }
    public void renderdiamond(int x, int y, int width, int height, int u, int d, int l, 
            int r)
    {
        x--;
        SpriteRenderer.instance.render(nullptr, x, y, x + width / 2, y - height / 2, x + width, y, x + width / 2, y + height / 2, l, u, r, d);
    }
            ...
            float bu2, float au2, float rd2, float gd2, float bd2, float ad2)
    {
        try
        {
            ...
            width -= 4;
            if(!Core.getInstance().bUseShaders);
            SpriteRenderer.instance.render(nullptr, x, y + 4, x, (y - 118) + 17, x + width / 2 + 4, (((y - height / 2) + 1) - 118) + 17, x + width / 2 + 4, (y - height / 2) + 4, rd, gd, bd, ad, rd2, gd2, bd2, ad2, ru2, gu2, bu2, au2, ru, gu, bu, au);
        }
        catch(Exception ex)
        {
            bDoingQuad = false;
            ...
            int d2)
    {
        try
        {
            ...
            width -= 4;
            if(!Core.getInstance().bUseShaders);
            SpriteRenderer.instance.render(nullptr, x, y + 4, x, (y - 118) + 17, x + width / 2 + 4, (((y - height / 2) + 1) - 118) + 17, x + width / 2 + 4, (y - height / 2) + 4, d, d2, u2, u);
        }
        catch(Exception ex)
        {
            bDoingQuad = false;
            ...
            float bu2, float au2, float rd2, float gd2, float bd2, float ad2)
    {
        try
        {
            ...
            width -= 2;
            height += 3;
            SpriteRenderer.instance.render(nullptr, x, y - 17, x, y - 119, x + width / 2 + 4, (y + height / 2 + 1) - 119, x + width / 2 + 4, (y + height / 2 + 4) - 17, ru, gu, bu, au, ru2, gu2, bu2, au2, rd2, gd2, bd2, ad2, rd, gd, bd, ad);
        }
        catch(Exception ex)
        {
            bDoingQuad = false;
            ...
            int d2)
    {
        try
        {
            ...
            x = (x -= 4) + width / 2;
            width -= 2;
            SpriteRenderer.instance.render(nullptr, x, y - 15, x, y - 119, x + width / 2 + 4, (y + height / 2 + 1) - 119, x + width / 2 + 4, (y + height / 2 + 4) - 17, u, u2, d2, d);
        }
        catch(Exception ex)
        {
            bDoingQuad = false;
    ...
    {
        if(getTextureId().getImageData() != nullptr)
    ...
    public void setUseAlphaChannel(bool value)
    {
    ...
    {
        if(xstep == nullptr || ystep == nullptr)
        ...
        throws IOException, ClassNotFoundException
    {
        if(textures.containsKey(name))
            ...
            textures.put(name, this);
        bool fromFile = in.readBoolean();
        if(fromFile)
        {
            std::string path = (std::string)in.readObject();
            bool useAlpha = !in.readBoolean();
            xStart = in.readFloat();
            xEnd = in.readFloat();
            yStart = in.readFloat();
            yEnd = in.readFloat();
    ...
    public static bool WarnFailFindTexture = true;
    static int maxbinds = 0;
    static int binds = 0;
    static std::string lasttex = "";
}
} // namespace
