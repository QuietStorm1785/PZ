...
public:
   public static final HashMap<String, WorldMapData> s_fileNameToData = std::make_unique<HashMap<>>();
    std::string m_relativeFileName;
   public final ArrayList<WorldMapCell> m_cells = std::make_unique<ArrayList<>>();
   public final HashMap<Integer, WorldMapCell> m_cellLookup = std::make_unique<HashMap<>>();
    int m_minX;
    int m_minY;
    int m_maxX;
    int m_maxY;
